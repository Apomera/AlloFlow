...
const out = process.argv[2] ? path.resolve(ROOT, process.argv[2]) : path.join(ROOT, 'scratch', 'water-outlet-banks-review');
...
(async () => {
  ...
  try {
    ...
    const shoulders=await page.evaluate(()=>{
      const scene=pilotReview.scene,bank=scene.getObjectByName('pilot-stream-wet-banks');
      const shore=scene.getObjectByName('pilot-lake-shore-ground'),a=bank.geometry.attributes.position,b=shore.geometry.attributes.position;
      let low=0,maxHeight=-Infinity,shoreLow=0,raised=0;
      for(let i=0;i<a.count;i++){
        const r=Math.hypot(a.getX(i)-268,a.getZ(i)+104);
        if(r>=34&&r<35.5){low++;maxHeight=Math.max(maxHeight,a.getY(i));}
      }
      for(let i=2;i<b.count;i+=4){const angle=Math.atan2(b.getZ(i),b.getX(i));
        const delta=Math.abs((angle<0?angle+Math.PI*2:angle)-2.63);
        if(delta>.2&&delta<.3&&b.getY(i)<8.8)shoreLow++;
        if(delta>.5&&Math.abs(b.getY(i)-9)<.001)raised++;
      }
      return {low,maxHeight,shoreLow,raised,finite:[a,b].every(p=>Array.from(p.array).every(Number.isFinite))};
    });assert(shoulders.low>4&&shoulders.maxHeight<8.7&&shoulders.shoreLow>1&&shoulders.raised>60&&shoulders.finite,JSON.stringify(shoulders));
    console.log('Graded bank measurements',shoulders);
