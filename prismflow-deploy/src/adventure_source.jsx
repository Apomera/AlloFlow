                    ...
                    {characters.map((char, i) => (
                        <div key={i} className="bg-gradient-to-br from-slate-50 to-violet-50 rounded-2xl border border-violet-100 p-4 flex flex-col items-center text-center transition-all hover:shadow-lg hover:border-violet-300 relative group/card">
                            ...
                            {editingField?.idx === i && editingField?.field === 'appearance' ? (
                                ...
                                <p onClick={() => startFieldEdit(i, 'appearance')} className="text-[11px] text-slate-500 mt-1 leading-relaxed cursor-pointer hover:text-slate-700 hover:underline decoration-dashed underline-offset-2 transition-colors" title={t('adventure.edit_appearance')}>{char.appearance}</p>
                            )}
                            {char.portrait && !char.isGenerating && (
                                <div className="mt-2 flex flex-wrap gap-1.5 justify-center">
                                    {char.isUserUploaded && (
                                        <span className="text-[10px] px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-full border border-emerald-200 font-medium">📷 User Photo</span>
                                    )}
