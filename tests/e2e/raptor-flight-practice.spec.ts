...
test.describe('Raptor smooth flight and practice trail', () => {
  ...
  test('keeps instrument states and accessible shortcuts aligned with the selected controls',async({page})=>{
    ...
    await expect(page.getByRole('button',{name:'Strike unavailable until takeoff',exact:true})).toHaveAttribute('aria-keyshortcuts','Space Enter');
    await expect(page.locator('[data-readout-kind=strike]')).toContainText('Hold Take off');
    await expect(page.locator('.rh-flight-btn-strike')).toBeDisabled();
    ...
    await expect(page.locator('[data-readout-kind=strike]')).toContainText('Launch first');
    await expect(page.getByRole('button',{name:'Strike unavailable until takeoff',exact:true})).toHaveAttribute('aria-keyshortcuts','R');
    ...
    await expect(page.locator('[data-readout-kind=status] .rh-flight-readout-value')).toHaveText('Perched');
    await page.locator('[data-raptor-canvas]').evaluate((c:any)=>{c._rhCommand('hold',{key:' ',pressed:true});for(let i=0;i<4;i++)(window as any).stepPractice(40);c._rhCommand('hold',{key:' ',pressed:false});});
    await expect(page.locator('[data-readout-kind=status] .rh-flight-readout-value')).not.toHaveText('Perched');
    await expect(page.getByRole('button',{name:'Strike target - keyboard R',exact:true})).toBeEnabled();
