...
const wave = require('../dev-tools/eppp_native_quality_wave_47_data.cjs');
...
describe('EPPP expanded quality wave 47', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_47.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 23, applicationItems: 13, analysisItems: 11, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    expect(audit.summary.distractorWarningsAfter.uniqueKeyStemLexicalLeakageCandidates).toBe(17);
    expect(audit.summary.distractorWarningsAfter.asymmetricExtremeDistractorCandidates).toBe(28);
    expect(audit.summary.distractorWarningsAfter.advancedDirectRecallCandidates).toBe(1);
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicatePairs).toBe(41);
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicateClusters).toBe(29);
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(378);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1059);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(518);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(491);
    expect(audit.summary.feedbackWarningsAfter.choiceRestatementOptions).toBe(114);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_47.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_47.json'));
  });

  it('replays after wave 46', () => {
    const builder = read('dev-tools/build_eppp_1500_expansion.cjs');
    const i46 = builder.indexOf('repair_eppp_native_quality_wave_46.cjs');
    const i47 = builder.indexOf('repair_eppp_native_quality_wave_47.cjs', i46);
    expect(i47).toBeGreaterThan(i46);
  });
});
