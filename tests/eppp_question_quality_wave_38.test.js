...
const wave = require('../dev-tools/eppp_native_quality_wave_38_data.cjs');
...
describe('EPPP expanded quality wave 38', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_38.json');
    ...
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(442);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1230);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(587);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(591);
    expect(audit.summary.feedbackWarningsAfter.choiceRestatementOptions).toBe(139);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_38.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_38.json'));
  });

  it('replays after wave 37', () => {
