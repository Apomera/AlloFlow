...
const FONT_CHOICES = [
  { label: 'Inter / system sans', value: "'Inter', system-ui, sans-serif" },
  { label: 'Georgia serif', value: "Georgia, 'Times New Roman', serif" },
  { label: 'System serif', value: "ui-serif, Georgia, serif" },
  { label: 'Verdana (high legibility)', value: "Verdana, Geneva, sans-serif" },
...
function BrandProfileEditor(props) {
  ...
  const t = typeof props.t === 'function' ? props.t : function (k, d) { return d || k; };
  const addToast = typeof props.addToast === 'function' ? props.addToast : function () {};

  const api = BP();

  const blankDraft = React.useCallback(function () {
    const d = api ? api.DEFAULT_COLORS : { heading: '#1e3a5f', accent: '#2563eb', body: '#1f2937', bg: '#ffffff', cardBg: '#f8fafc', cardBorder: '#e2e8f0' };
    const f = api ? api.DEFAULT_FONTS : { body: "'Inter', system-ui, sans-serif", heading: null };
    return {
      id: null,
      name: '',
  ...
  React.useEffect(function () {
    ...
    return function () { window.removeEventListener('alloflow:brand-profile-changed', onChange); };
  }, [refresh]);

  // Live validation of the current draft (errors block save; warnings advise).
  const validation = React.useMemo(function () {
    if (!api) return { ok: true, errors: [], warnings: [] };
    try { return api.validateBrandProfile(draft); } catch (e) { return { ok: false, errors: ['Validation failed: ' + e.message], warnings: [] }; }
  }, [api, draft]);

  function setColor(key, value) {
    setDraft(function (d) { return Object.assign({}, d, { colors: Object.assign({}, d.colors, { [key]: value }) }); });
  }
  function setField(key, value) {
    setDraft(function (d) { return Object.assign({}, d, { [key]: value }); });
  }
  function setHeader(patch) {
    setDraft(function (d) { return Object.assign({}, d, { header: Object.assign({}, d.header, patch) }); });
  }
  function setFooter(patch) {
    setDraft(function (d) { return Object.assign({}, d, { footer: Object.assign({}, d.footer, patch) }); });
  }

  function editProfile(p) { setDraft(JSON.parse(JSON.stringify(p))); }
  function newProfile() { setDraft(blankDraft()); }

  function autoFix() {
    if (!api) return;
    try {
      const res = api.autoFixBrandColors(draft, 4.5);
      setDraft(function (d) { return Object.assign({}, d, { colors: res.profile.colors }); });
    ...
    } catch (e) { addToast('Auto-fix failed: ' + e.message, 'error'); }
  }
  ...
  function save() {
    ...
    if (res.ok) {
      ...
      setDraft(function (d) { return Object.assign({}, d, { id: res.id }); });
      refresh();
    } else {
  ...
  function exportProfile(id) {
    ...
    if (!json) { addToast(t('brand.export_failed', 'Nothing to export'), 'error'); return; }
    try {
  ...
  function onLogoFile(e) {
    ...
    reader.onload = function () {
      setDraft(function (d) {
        return Object.assign({}, d, { logo: { src: String(reader.result || ''), alt: (d.logo && d.logo.alt) || '', width: null, height: null } });
      });
    };
  ...
  function setLogoAlt(alt) {
    setDraft(function (d) { return Object.assign({}, d, { logo: d.logo ? Object.assign({}, d.logo, { alt: alt }) : { src: '', alt: alt } }); });
  }
  function removeLogo() {
    setDraft(function (d) { return Object.assign({}, d, { logo: null, header: Object.assign({}, d.header, { showLogo: false }) }); });
  }
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <aside className="w-56 shrink-0 border-r border-slate-200 bg-slate-50 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {profiles.length === 0 ? (
                <p className="text-xs text-slate-500 p-2">{t('brand.empty', 'No brand profiles yet. Create one to brand your documents and PDFs.')}</p>
          ...
          <div className="flex-1 overflow-y-auto p-5 min-w-0">
            ...
            <div className="mb-4">
              <span className="text-xs font-bold uppercase tracking-wide text-slate-500">{t('brand.colors', 'Colors')}</span>
            ...
            {/* Logo */}
            <div className="mb-4">
              <span className="text-xs font-bold uppercase tracking-wide text-slate-500">{t('brand.logo', 'Logo (optional)')}</span>
              <div className="mt-1 flex items-start gap-3">
                {draft.logo && draft.logo.src ? (
                  <img src={draft.logo.src} alt={draft.logo.alt || ''} className="h-12 w-auto max-w-[96px] rounded border border-slate-200 bg-white object-contain" />
                ) : null}
          ...
          <aside className="w-72 shrink-0 border-l border-slate-200 flex flex-col">
            <div className="flex-1 overflow-y-auto p-3">
              ...
              <div className="mt-1 rounded-lg overflow-hidden border border-slate-200" style={{ background: c.bg, fontFamily: bodyFont }}>
                {(draft.header.text || (draft.header.showLogo && draft.logo && draft.logo.src)) && (
                  <div style={{ background: c.heading, color: c.bg, padding: '8px 10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    {draft.header.showLogo && draft.logo && draft.logo.src && <img src={draft.logo.src} alt={draft.logo.alt || ''} style={{ maxHeight: '24px', width: 'auto' }} />}
                    <span style={{ fontSize: '12px', fontWeight: 700 }}>{draft.header.text}</span>
                  </div>
                )}
                ...
                {(draft.footer.text || draft.footer.showPageNumber) && (
                  <div style={{ borderTop: '1px solid ' + c.cardBorder, color: c.body, padding: '6px 10px', fontSize: '10px', display: 'flex', justifyContent: 'space-between' }}>
                    <span>{draft.footer.text}</span>{draft.footer.showPageNumber && <span>{t('brand.preview_page', 'Page 1')}</span>}
                  </div>
                )}
              </div>
