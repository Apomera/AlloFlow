...
const createTTS = deps => {
  ...
  const _normalizeTtsSpeechProfile = (languageValue, localeValue, dialectValue) => {
    ...
    const clean = (value, maxLength) => String(value == null ? '' : value).trim().replace(/\s+/g, ' ').slice(0, maxLength);
    ...
    const cacheIdentity = !locale && !dialect ? baseLanguage : [baseLanguage.toLocaleLowerCase(), locale.toLowerCase(), dialect.toLocaleLowerCase()].join('\u241f');
    return {
      baseLanguage,
      locale,
      dialect,
  ...
  const _cloudTtsPrompt = (textValue, speechProfile) => {
    ...
    return 'Pronounce the following ' + profile.baseLanguage + ' text' + qualifierText + ' with native ' + profile.baseLanguage + ' phonology: ' + textValue;
  };
  ...
  const _externalTtsOwners = () => {
    const owners = [];
    try {
      const ai = getAi && getAi();
      if (ai) owners.push(ai);
    } catch (_) {}
    try {
      if (window._kokoroTTS) owners.push(window._kokoroTTS);
    } catch (_) {}
    try {
      if (window._piperTTS) owners.push(window._piperTTS);
    } catch (_) {}
    return owners;
  };
  try {
    window.__alloTtsCacheOwnsUrl = url => {
      ...
      return _externalTtsOwners().some(owner => {
        try {
          return typeof owner.ownsUrl === 'function' && owner.ownsUrl(url);
        } catch (_) {
          return false;
        }
      });
  ...
  const awaitTtsHardDeadline = (promise, timeoutMs, onTimeout, message, signal) => new Promise((resolve, reject) => {
    ...
    if (signal) {
      try {
        if (signal.aborted) rejectAborted();else signal.addEventListener('abort', rejectAborted, {
          once: true
        });
      } catch (_) {}
    }
  ...
  const fetchTTSBytes = (text, voiceName, speed = 1, language = 'English', signal = null, requestPriority = 'normal') => {
    ...
    const queuedTask = state[queueSlot].then(async () => {
      ...
      if (signal && watchdogController) {
        try {
          if (signal.aborted) watchdogController.abort();else signal.addEventListener('abort', callerAbortHandler, {
            once: true
          });
        } catch (_) {}
      }
  ...
  const _mathLooksReal = content => /\\[a-zA-Z]+|[\^_]/.test(content) || /=/.test(content) && content.length <= 80;
  ...
  const _mathToSpeakable = async (raw, language, speechOptions = null) => {
    try {
      ...
      jobs.forEach((job, index) => {
        ...
        out += spoken[index] && String(spoken[index]).trim() ? ' ' + String(spoken[index]).trim() + ' ' : job.raw;
        cursor = job.end;
      });
  ...
  const ensurePiperTts = async () => {
    try {
      ...
      if (!piperLoadPromise) {
        piperLoadPromise = Promise.resolve(window.__loadPiperTTS()).then(() => window._piperTTS || null).finally(() => {
  ...
  const ensureKokoroTts = async timeoutMs => {
    try {
      ...
      if (!kokoroLoadPromise) {
        ...
        kokoroLoadPromise = Promise.resolve(window.__loadKokoroTTS()).then(() => window._kokoroTTS && window._kokoroTTS.ready ? window._kokoroTTS : null).catch(error => {
  ...
  const callTTS = async (text, voiceName, speed = 1, maxRetriesOrOpts = 2, languageArg) => {
    ...
    var _emitResolvedProfile = (url, profile) => {
      ...
      if (!_resolvedProfileEmitted && typeof _callOpts.onResolvedProfile === 'function') {
        ...
        try {
          _callOpts.onResolvedProfile(profile || {});
        } catch (_) {}
      }
      return url;
    };
    var _resolvedKokoroVoice = requested => {
      try {
        return window._kokoroTTS && typeof window._kokoroTTS.resolveVoice === 'function' ? window._kokoroTTS.resolveVoice(requested) : null;
      } catch (_) {
        return null;
      }
    };
    ...
    if (_isCanvasEnv && _earlyTtsProvider !== 'local') {
      ...
      if (!_isKokoroVoice && !_forceRefresh && state.urlCache.has(canvasCacheKey)) {
        ...
        return _emitResolvedProfile(state.urlCache.get(canvasCacheKey), _resolutionProfile('gemini', 'gemini-tts', GEMINI_MODELS?.tts, voiceName, 1, {
          cacheHit: true
        }));
      }
      if (_isKokoroVoice) {
        ...
        _ttsTrace('calltts:canvas-skip-timeout', {
          untilMs: timeoutCooldownMs
        });
        ...
        const fetchCanvasTTSBytes = async () => {
          ...
          if (!entry || isInteractive || _forceRefresh || entryAge > CALLTTS_JOIN_MAX_AGE_MS) {
            ...
            entry = {
              promise: fetchTTSBytes(text, voiceName, speed, _speechProfile, null, _callOpts.priority),
              startedAt: Date.now()
            };
        ...
        for (let canvasAttempt = 0; canvasAttempt < canvasMaxAttempts; canvasAttempt++) {
          try {
            ...
            if (ttsResult) {
              ...
              return _emitResolvedProfile(url, _resolutionProfile('gemini', 'gemini-tts', GEMINI_MODELS?.tts, voiceName, 1, {
                cacheHit: false
              }));
            }
            ...
            if (isDeadlineTimeout) {
              ...
              if (ttsTimeoutStrikes < 2 && canvasAttempt < canvasMaxAttempts - 1) {
                _ttsTrace('calltts:canvas-timeout-retry', {
                  strike: ttsTimeoutStrikes
                });
              ...
              _ttsTrace('calltts:canvas-timeout-fallback', {
                cooldownMs: TTS_TIMEOUT_COOLDOWN_MS,
                strikes: ttsTimeoutStrikes
              });
        ...
        try {
          ...
          if (window._kokoroTTS) {
            ...
            const relayKokoroAbort = () => {
              try {
                kokoroController?.abort();
              } catch (_) {}
            };
            if (_signal && kokoroController) {
              if (_signal.aborted) relayKokoroAbort();else _signal.addEventListener('abort', relayKokoroAbort, {
                once: true
              });
            }
            ...
            try {
              ...
              if (url) {
                ...
                return _emitResolvedProfile(url, _resolutionProfile('local', 'kokoro-browser', null, _resolvedKokoroVoice(kokoroVoice), 1, {
              ...
              try {
                _signal?.removeEventListener?.('abort', relayKokoroAbort);
              } catch (_) {}
            }
          }
        } catch (e) {
        ...
        try {
          if (PIPER_HANDLES_ENGLISH && window._piperTTS) {
            ...
            if (url) {
              ...
              return _emitResolvedProfile(url, _resolutionProfile('local', 'piper-browser', null, _resolvedPiperVoice('en'), 1, {
                languageCode: 'en',
                fallbackFrom: 'gemini'
              }));
            }
        ...
        try {
          if (window._piperTTS && window._piperTTS.supportsLanguage(ttsLang)) {
            ...
            if (url) {
              ...
              return _emitResolvedProfile(url, _resolutionProfile('local', 'piper-browser', null, _resolvedPiperVoice(ttsLang), 1, {
      ...
      if (!_isEnglish) {
        ...
        try {
          ...
          if (kokoroUrl) {
            ...
            return _emitResolvedProfile(kokoroUrl, _resolutionProfile('local', 'kokoro-browser', null, _resolvedKokoroVoice(voiceName), 1));
          }
        ...
        try {
          const wokenUrl = await window._kokoroTTS.speak(cleanTextForLocalTTS(text), voiceName, speed, {
            signal: _signal
          });
          if (wokenUrl) {
            _routeNote('kokoro', 'engine woken from on-device model');
            return _emitResolvedProfile(wokenUrl, _resolutionProfile('local', 'kokoro-browser', null, _resolvedKokoroVoice(voiceName), 1));
          }
    ...
    if (!_isEnglish && (_localTtsChosen || _kokoroKeyless)) {
      try {
        ...
        if (piper?.supportsLanguage?.(piperLanguage)) {
          const piperUrl = await piper.speak(cleanTextForLocalTTS(text), piperLanguage, speed, {
            signal: _signal
          });
          if (piperUrl) {
            _routeNote('piper', 'local multilingual fallback: ' + piperLanguage);
            return _emitResolvedProfile(piperUrl, _resolutionProfile('local', 'piper-browser', null, _resolvedPiperVoice(piperLanguage), 1, {
    ...
    if (_ttsOvr === 'local' || _ttsOvr === 'browser' || _ttsOvr === 'off' || _ttsOvr === 'auto' && _isLocalAI) {
      try {
        const result = await _ai.textToSpeech(text, {
          ...
          speed,
          language: _language,
          locale: _speechProfile.locale,
          dialect: _speechProfile.dialect,
          signal: _signal,
          force: _forceRefresh
        });
        _routeNote('provider', 'ttsProvider=' + _ttsOvr);
        return _emitResolvedProfile(result, _resolutionProfile(_ttsOvr === 'auto' && _isLocalAI ? 'local' : _ttsOvr, 'ai-provider-' + (_aiUserConfig && _aiUserConfig.backend || 'custom'), _ai && _ai.models && _ai.models.tts || _aiUserConfig && _aiUserConfig.models && _aiUserConfig.models.tts || null, voiceName, 1));
      } catch (e) {
        if (_isAbortError(e)) throw e;
    ...
    if (!_forceRefresh && state.urlCache.has(cacheKey)) {
      ...
      return _emitResolvedProfile(state.urlCache.get(cacheKey), _resolutionProfile('gemini', 'gemini-tts', GEMINI_MODELS?.tts, voiceName, 1, {
        cacheHit: true
      }));
    }
  ...
  const callTTSDirect = async (text, voiceName, speed = 1, maxRetries = 2) => {
    ...
    maxRetries = typeof maxRetries === 'number' ? maxRetries : typeof _directOpts.maxRetries === 'number' ? _directOpts.maxRetries : 2;
    ...
    text = await _mathToSpeakable(text, _directLanguage, _directOpts.mathSpeech || null);
    voiceName = _resolveRequestedVoice(voiceName);
    ...
    if (_isCanvasEnv && _directTtsProvider !== 'local') {
      if (Date.now() < (state.timeoutRetryAt || 0)) {
        _ttsTrace('callttsdirect:canvas-skip-timeout', {
          untilMs: Math.max(0, state.timeoutRetryAt - Date.now())
        });
      } else if (Date.now() >= state.rateLimitedUntil) {
        ...
        for (let botAttempt = 0; botAttempt < botCanvasMaxAttempts; botAttempt++) {
          try {
            ...
            if (isDeadlineTimeout) {
              state.timeoutRetryAt = Date.now() + TTS_TIMEOUT_COOLDOWN_MS;
              _ttsTrace('callttsdirect:canvas-timeout-fallback', {
                cooldownMs: TTS_TIMEOUT_COOLDOWN_MS
              });
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        const queuedTask = state.botQueue.then(async () => {
          ...
          try {
            _directSignal?.addEventListener?.('abort', onDirectAbort, {
              once: true
            });
          } catch (_) {}
          let response;
          try {
            ...
            try {
              _directSignal?.removeEventListener?.('abort', onDirectAbort);
            } catch (_) {}
          }
