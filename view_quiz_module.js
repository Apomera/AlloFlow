...
function sequenceSenseTruth(presentedOrder, intentionallyWrongIndex, itemCount) {
  ...
  var valid = n > 0 && Array.isArray(presentedOrder) && presentedOrder.length === n && presentedOrder.every(function (value) {
    return Number.isInteger(value) && value >= 0 && value < n;
  }) && new Set(presentedOrder).size === n;
  if (!valid || presentedOrder.every(function (value, index) {
    return value === index;
  })) return {
    orderIsCorrect: true,
    misplaced: []
  ...
  if (!misplaced.length) {
    misplaced = Number.isInteger(intentionallyWrongIndex) && intentionallyWrongIndex >= 0 && intentionallyWrongIndex < n ? [intentionallyWrongIndex] : presentedOrder.map(function (_, index) {
      return index;
    });
  }
  return {
...
function describeSequenceMisplaced(misplaced) {
  var numbers = (Array.isArray(misplaced) ? misplaced : []).map(function (index) {
    return index + 1;
  });
...
function SequenceSenseCard(p) {
  ...
  var presentedOrderState = _quizUseDraftField(p.draftNamespace, draftItemKey, 'presentedOrder', function () {
    ...
    var isPermutation = authored.length === canonicalItems.length && authored.every(function (value) {
      return Number.isInteger(value) && value >= 0 && value < canonicalItems.length;
    }) && new Set(authored).size === authored.length;
    return isPermutation ? authored.slice() : canonicalItems.map(function (_, i) {
      return i;
    });
  });
  ...
  var arrangedItems = orderAnswer.map(function (canonicalIdx) {
  ...
  function answerPrinciple(p2) {
    ...
    var arrangeCorrect = orderAnswer.every(function (canonicalIdx, position) {
      return canonicalIdx === position;
    });
  ...
  _quizUseVoiceController(p, {
    getState: function () {
      ...
      var prompt = step === 1 ? 'Is the displayed order correct? Say the order is correct or the order is wrong.' : step === 2 ? 'Choose the misplaced item by its displayed number or exact text.' : step === 3 ? 'Put the items in the correct order. Say move item 2 up or move item 3 down, then say done.' : step === 4 ? 'Choose the ordering principle: ' + principleOptions.join(', ') + '.' : grade ? 'This sequence response has been checked. Say try again to reset it.' : '';
      return {
        type: 'sequence-sense',
        step: step,
    ...
    execute: function (action, request) {
      ...
      if (step === 1) {
        ...
        return {
          ...
          state: 'sequence-step',
          step: verify === 'no' ? 2 : 3,
          message: verify === 'no' ? 'Order marked wrong. Choose the misplaced item by its displayed number or exact text.' : 'Order marked correct. Put the items in the correct order, or say done to keep them as they are.'
        };
      }
      if (step === 2) {
        ...
        return {
          ...
          message: 'Item ' + (itemIdx + 1) + ' selected. Now put the items in the correct order: say move item 2 up or move item 3 down, then say done.'
        };
      }
      if (step === 3) {
        ...
        if (/^(?:done|finished|confirm|keep|next|the order is right|that is the order)$/.test(arrangeText)) {
          ...
          return {
            ...
            state: 'sequence-step',
            step: 4,
            message: 'Order saved. Choose the ordering principle: ' + principleOptions.join(', ') + '.'
          };
        }
        var move = /^move\s+(?:item\s+)?(.+?)\s+(up|down)$/.exec(arrangeText);
        var movePosition = move ? _quizVoiceNamedChoiceIndex(move[1], arrangedItems) : -1;
        if (!move || movePosition < 0) return {
          ok: false,
          state: 'invalid-choice',
          message: 'Say move item 2 up, move item 3 down, or done.'
        };
        if (!moveOrderItem(movePosition, move[2] === 'up' ? -1 : 1)) return {
          ok: false,
          state: 'invalid-choice',
          message: 'Item ' + (movePosition + 1) + ' cannot move ' + move[2] + '.'
        };
        return {
          ok: true,
          state: 'sequence-step',
          step: 3,
          message: 'Moved ' + arrangedItems[movePosition] + ' ' + move[2] + '. Say done when the order is right.'
        };
      }
      var principleIdx = _quizVoiceNamedChoiceIndex(choice, principleOptions);
      if (step !== 4 || principleIdx < 0) return {
        ok: false,
        state: 'invalid-choice',
      ...
      var resultMessage = result.status === 'correct' ? 'Sequence response checked. All four parts are correct.' : result.status === 'partially-correct' ? 'Sequence response checked. You earned ' + result.score + ' of 4 points.' : 'Sequence response checked. This sequence needs review.';
      return {
        ok: true,
        state: 'checked',
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Step 1 of 4 — Is this order correct?"), /*#__PURE__*/React.createElement("div", {
  ...
  }, "Step 2 of 4 — Click an item that's out of place above.")), step === 3 && /*#__PURE__*/React.createElement("div", {
    className: "p-3 rounded-lg bg-indigo-50 border border-indigo-200"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-sm font-semibold text-indigo-900 mb-2"
  }, "Step 3 of 4 — Put the items in the correct order. If they are already right, leave them."), /*#__PURE__*/React.createElement("ol", {
    className: "space-y-1.5 mb-2",
    "aria-label": "Your order"
  }, orderAnswer.map(function (canonicalIdx, position) {
    return /*#__PURE__*/React.createElement("li", {
      key: canonicalIdx,
      className: "flex items-center gap-2 px-3 py-2 rounded-lg border bg-white border-indigo-200"
    }, /*#__PURE__*/React.createElement("span", {
      className: "flex-shrink-0 text-xs font-bold text-slate-600 w-6"
    }, position + 1 + '.'), /*#__PURE__*/React.createElement("span", {
      className: "flex-1 text-sm text-slate-800"
    }, canonicalItems[canonicalIdx]), /*#__PURE__*/React.createElement("button", {
      type: "button",
      disabled: position === 0,
      "aria-label": 'Move up: ' + canonicalItems[canonicalIdx],
      onClick: function () {
        moveOrderItem(position, -1);
      },
      className: "px-2 py-1 rounded border border-indigo-300 bg-white text-xs font-bold text-indigo-800 disabled:opacity-40"
    }, "▲"), /*#__PURE__*/React.createElement("button", {
      type: "button",
      disabled: position === orderAnswer.length - 1,
      "aria-label": 'Move down: ' + canonicalItems[canonicalIdx],
      onClick: function () {
        moveOrderItem(position, 1);
      },
      className: "px-2 py-1 rounded border border-indigo-300 bg-white text-xs font-bold text-indigo-800 disabled:opacity-40"
    ...
    className: "px-3 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold transition-colors motion-reduce:transition-none"
  }, "Done arranging")), step === 4 && /*#__PURE__*/React.createElement("div", {
    className: "p-3 rounded-lg bg-indigo-50 border border-indigo-200"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-sm font-semibold text-indigo-900 mb-2"
  }, "Step 4 of 4 — What's the ordering principle?"), /*#__PURE__*/React.createElement("div", {
  ...
  }, grade.status === 'idk' ? '🤔 Marked "I don\'t know"' : grade.score + ' / 4 — ' + (grade.status === 'correct' ? 'All correct' : grade.status === 'partially-correct' ? 'Partial' : 'Needs review'))), grade.status !== 'idk' && /*#__PURE__*/React.createElement("ul", {
    className: 'space-y-1 text-sm text-' + statusColor + '-900'
  }, /*#__PURE__*/React.createElement("li", null, (grade.step1Correct ? '✓ ' : '✗ ') + 'Verify: ' + (actualOrderIsCorrect ? 'order was correct' : describeSequenceMisplaced(misplacedPositions)) + (grade.step1Correct ? '' : ' (you said "' + verifyAnswer + '")')), verifyAnswer === 'no' && /*#__PURE__*/React.createElement("li", null, (grade.step2Correct ? '✓ ' : '✗ ') + 'Diagnose: ' + (grade.step2Correct ? 'you found a misplaced item' : actualOrderIsCorrect ? 'nothing was misplaced' : describeSequenceMisplaced(misplacedPositions) + (misplacedPositions.length > 1 ? ' (either one counts)' : ''))), /*#__PURE__*/React.createElement("li", null, (grade.arrangeCorrect ? '✓ ' : '✗ ') + 'Arrange: ' + (grade.arrangeCorrect ? 'your order matched' : 'correct order is ' + canonicalItems.join(' → '))), /*#__PURE__*/React.createElement("li", null, (grade.step3Correct ? '✓ ' : '✗ ') + 'Principle: ' + (grade.step3Correct ? '"' + orderingPrinciple + '"' : 'correct answer was "' + orderingPrinciple + '" (you picked "' + principleAnswer + '")'))), /*#__PURE__*/React.createElement("div", {
...
function _quizAuditAssessment(data) {
  ...
  questions.forEach(function (q, index) {
    ...
    if (type === 'mcq') {
      ...
      var orderIsPermutation = order.length === items.length && !order.some(function (value) {
      ...
      if (orderIsPermutation && items.length >= 3) {
        ...
        if (sequenceTruth.orderIsCorrect && authoredWrong != null) add(index, 'warning', 'The displayed order is already correct, so no position is misplaced. Clear the misplaced position or change the displayed order.', 'sequence-misplaced');else if (!sequenceTruth.orderIsCorrect && (authoredWrong == null || sequenceTruth.misplaced.indexOf(authoredWrong) === -1)) add(index, 'warning', 'The misplaced position does not match the displayed order (positions out of place: ' + sequenceTruth.misplaced.map(function (value) {
          return value + 1;
        }).join(', ') + ').', 'sequence-misplaced');
      }
...
function AssessmentPresentationItem(p) {
  ...
  if (type === 'multi-select') {
    ...
    }, (Array.isArray(q.options) ? q.options : []).map(function (option, index) {
    ...
    body = /*#__PURE__*/React.createElement("div", {
      ...
      className: "space-y-2"
    }, (Array.isArray(q.answerOptions) ? q.answerOptions : []).map(function (option, index) {
      return /*#__PURE__*/React.createElement("div", {
        key: index,
      ...
      className: "font-bold text-slate-700 mb-2"
    }, "Part 2 - supporting evidence"), q.evidencePrompt && /*#__PURE__*/React.createElement("p", {
      className: "mb-3 text-base text-slate-700"
    }, renderText(q.evidencePrompt)), /*#__PURE__*/React.createElement("div", {
      className: "space-y-2"
    }, (Array.isArray(q.evidenceOptions) ? q.evidenceOptions : []).map(function (option, index) {
      return /*#__PURE__*/React.createElement("div", {
        key: index,
    ...
    answerGuide = /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
      className: "font-bold"
    }, "Answer guide: "), q.expectedAnswer || q.sampleAnswer || q.correctAnswer || q.rubric || 'Teacher review');
  }
  if (p.compact) {
    var written = ['fill-blank', 'short-answer', 'self-explanation'].indexOf(type) !== -1;
    return /*#__PURE__*/React.createElement("div", {
      "data-review-question-type": type,
      className: "rounded-xl border-2 border-slate-200 bg-white p-4 sm:p-6 text-left text-slate-800"
    }, /*#__PURE__*/React.createElement("p", {
      className: "mb-3 text-xs font-black uppercase tracking-wider text-teal-800"
    }, _quizPresentationTypeLabel(type)), written ? /*#__PURE__*/React.createElement("p", {
      className: "text-base text-slate-700"
    }, type === 'self-explanation' ? 'Explain your thinking aloud or on paper. Include how or why the idea works.' : 'Share your answer aloud or write it down before revealing the guide.') : body, showAnswer && /*#__PURE__*/React.createElement("div", {
  ...
  }, renderText(q.question_en)))), q.imageUrl && /*#__PURE__*/React.createElement("img", {
    src: q.imageUrl,
    alt: _quizAuthoredImageAlt(q.imageAltText),
    className: "mb-5 max-h-80 w-full rounded-lg object-contain"
  }), /*#__PURE__*/React.createElement("div", {
...
function useQuizGameSetupModule(open, moduleKey, componentKey, loaderKey) {
  ...
  React.useEffect(() => {
    ...
    return () => {
      settled = true;
      clearInterval(timer);
      window.removeEventListener('alloflow:module-registry-changed', inspect);
    };
...
function QuizView(props) {
  ...
  function quizCopy(key, fallback) {
    var value = t(key);
    return !value || value === key ? fallback : value;
  }
  function closeQuizGames() {
    setQuizGamesOpen(false);
    quizGamesButtonRef.current?.focus();
  }
  function launchQuizGame(action) {
    closeQuizGames();
  ...
  if (soloQuestOpen && (isTeacherMode || !activeSessionCode)) {
    return soloQuestSetup.ready && window.AlloModules?.ConceptQuestSolo ? /*#__PURE__*/React.createElement(window.AlloModules.ConceptQuestSolo, {
      ...
      appId: appId,
      t: t,
      onClose: closeSoloQuest
    }) : /*#__PURE__*/React.createElement("section", {
      className: "rounded-xl border border-indigo-200 bg-indigo-50 p-5"
    }, /*#__PURE__*/React.createElement("p", {
      role: soloQuestSetup.failed ? 'alert' : 'status'
    }, soloQuestSetup.failed ? quizCopy('concept_quest.solo_load_failed', 'Concept Quest could not load. Check your connection and try again.') : quizCopy('concept_quest.solo_loading', 'Loading Concept Quest...')), /*#__PURE__*/React.createElement("div", {
      className: "mt-3 flex gap-2"
    }, soloQuestSetup.failed && /*#__PURE__*/React.createElement("button", {
      type: "button",
      onClick: soloQuestSetup.retry,
      className: "min-h-11 rounded-lg bg-indigo-700 px-4 py-2 font-bold text-white"
    }, quizCopy('common.retry', 'Retry')), /*#__PURE__*/React.createElement("button", {
      type: "button",
      onClick: closeSoloQuest,
      className: "min-h-11 rounded-lg border border-slate-300 bg-white px-4 py-2 font-bold"
    }, quizCopy('common.close', 'Close'))));
  }
  return /*#__PURE__*/React.createElement("div", {
    className: "space-y-6"
    ...
    role: boardSetup.failed ? 'alert' : 'status'
  }, boardSetup.failed ? t('lesson_board.load_failed', {
    defaultValue: 'The lesson board could not load. Check your connection and try again.'
  }) : t('lesson_board.loading', {
    defaultValue: 'Loading the lesson board…'
  })), boardSetup.failed && /*#__PURE__*/React.createElement("button", {
    type: "button",
    "data-retry-lesson-board": true,
    onClick: boardSetup.retry,
    ...
    role: connectedSetup.failed ? 'alert' : 'status'
  }, connectedSetup.failed ? t('connected_escape.load_failed', {
    defaultValue: 'The escape room could not load. Check your connection and try again.'
  }) : t('connected_escape.loading', {
    defaultValue: 'Loading the escape room…'
  })), connectedSetup.failed && /*#__PURE__*/React.createElement("button", {
    type: "button",
    "data-retry-connected-room": true,
    onClick: connectedSetup.retry,
    className: "mr-3 font-bold"
  }, t('common.retry', {
    defaultValue: 'Retry'
  })), /*#__PURE__*/React.createElement("button", {
    ...
    "aria-expanded": quizGamesOpen,
    "aria-controls": "quiz-games-tools",
    onClick: () => setQuizGamesOpen(open => !open),
    className: "min-h-11 flex items-center gap-2 rounded-lg border border-indigo-300 bg-white px-3 py-2 text-sm font-bold text-indigo-800 hover:bg-indigo-50"
    ...
    className: "flex min-h-11 items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-left text-sm font-bold text-slate-800 hover:bg-indigo-50 disabled:opacity-50 disabled:cursor-not-allowed"
  }, /*#__PURE__*/React.createElement(XCircle, {
    size: 16,
    "aria-hidden": "true"
  }), quizCopy('quiz.exit_review_game', 'Exit review game')), escapeRoomState.isActive && /*#__PURE__*/React.createElement("button", {
    type: "button",
    "data-quiz-exit-puzzle": true,
    onClick: () => {
      closeQuizGames();
    ...
    size: 14
  }), showQuizAnswers ? isIndependentMode ? t('quiz.hide_answers_student') : isParentMode ? 'Hide Scores' : t('quiz.hide_key') : isIndependentMode ? t('quiz.check_answers') : isParentMode ? 'View Scores' : t('quiz.show_key')))), quizGamesOpen && /*#__PURE__*/React.createElement("section", {
    ref: quizGamesPanelRef,
    id: "quiz-games-tools",
    "aria-labelledby": "quiz-games-title",
    className: "mt-3 rounded-xl border border-indigo-200 bg-indigo-50/70 p-3"
  }, /*#__PURE__*/React.createElement("h3", {
    id: "quiz-games-title",
    ...
    type: "button",
    "data-quiz-game": "review",
    onClick: () => launchQuizGame(handleToggleIsReviewGame),
    disabled: isPresentationMode || isReviewGame || isEditingQuiz || !!escapeRoomState.isActive || !!sessionData?.quizState?.isActive || !!sessionData?.escapeRoomState?.isActive,
    className: "flex min-h-11 w-full items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-left text-sm font-bold text-slate-800 hover:bg-indigo-50 disabled:opacity-50 disabled:cursor-not-allowed"
  }, /*#__PURE__*/React.createElement(Gamepad2, {
    size: 16,
    "aria-hidden": "true"
  }), t('quiz.review_game')), /*#__PURE__*/React.createElement("p", {
    className: "mt-1 px-1 text-sm text-slate-600"
  }, quizCopy('quiz.review_game_description', 'Answer every question on a category board and award team points.'))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("button", {
    type: "button",
    onClick: () => {
      closeQuizGames();
      if (escapeRoomState.isActive) {
    ...
    size: 14
  }), escapeRoomState.isActive ? t('common.close') : isTeacherMode && activeSessionCode ? t('escape_room.launch_live_btn') : t('escape_room.title')), /*#__PURE__*/React.createElement("p", {
    className: "mt-1 px-1 text-sm text-slate-600"
  }, quizCopy('quiz.puzzle_challenge_description', 'Solve a series of question-based puzzles.'))), (isTeacherMode || !activeSessionCode) && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("button", {
    type: "button",
    "data-open-lesson-board": true,
    onClick: () => launchQuizGame(() => setBoardSetupOpen(true)),
    disabled: isPresentationMode || isReviewGame || isEditingQuiz || !!escapeRoomState.isActive || !!sessionData?.quizState?.isActive || !!sessionData?.escapeRoomState?.isActive,
    className: "flex min-h-11 w-full items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-left text-sm font-bold text-slate-800 hover:bg-indigo-50 disabled:opacity-50 disabled:cursor-not-allowed"
  }, /*#__PURE__*/React.createElement(Gamepad2, {
    size: 16,
    ...
    className: "mt-1 px-1 text-sm text-slate-600"
  }, quizCopy('quiz.lesson_board_description', 'Explore lesson locations and solve activities, solo or together.'))), (isTeacherMode || !activeSessionCode) && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("button", {
    type: "button",
    "data-open-connected-room": true,
    onClick: () => launchQuizGame(() => setConnectedSetupOpen(true)),
    disabled: isPresentationMode || isReviewGame || isEditingQuiz || !!escapeRoomState.isActive || !!sessionData?.quizState?.isActive || !!sessionData?.escapeRoomState?.isActive,
    className: "flex min-h-11 w-full items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-left text-sm font-bold text-slate-800 hover:bg-indigo-50 disabled:opacity-50 disabled:cursor-not-allowed"
  }, /*#__PURE__*/React.createElement(DoorOpen, {
    size: 16,
    ...
    className: "mt-1 px-1 text-sm text-slate-600"
  }, quizCopy('quiz.escape_room_description', 'Explore a room, collect clues, and solve connected puzzles.'))), (isTeacherMode || !activeSessionCode) && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("button", {
    type: "button",
    "data-open-concept-quest-solo": true,
    onClick: () => launchQuizGame(() => setSoloQuestOpen(true)),
    disabled: isPresentationMode || isReviewGame || isEditingQuiz || !!escapeRoomState.isActive || !!sessionData?.quizState?.isActive || !!sessionData?.escapeRoomState?.isActive,
    className: "flex min-h-11 w-full items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-left text-sm font-bold text-slate-800 hover:bg-indigo-50 disabled:opacity-50 disabled:cursor-not-allowed"
  }, /*#__PURE__*/React.createElement(Gamepad2, {
    size: 16,
    "aria-hidden": "true"
  }), quizCopy('concept_quest.solo_title', 'Concept Quest solo')), /*#__PURE__*/React.createElement("p", {
    className: "mt-1 px-1 text-sm text-slate-600"
  }, quizCopy('quiz.concept_quest_description', 'Explore a role-playing adventure with abilities, encounters, and a final boss.'))), isTeacherMode && activeSessionCode && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("button", {
    type: "button",
    "data-open-concept-quest": true,
    onClick: () => launchQuizGame(launchConceptQuest),
    disabled: isPresentationMode || isReviewGame || isEditingQuiz || !!escapeRoomState.isActive || !!sessionData?.quizState?.isActive || !!sessionData?.escapeRoomState?.isActive,
    className: "flex min-h-11 w-full items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-left text-sm font-bold text-slate-800 hover:bg-indigo-50 disabled:opacity-50 disabled:cursor-not-allowed"
  }, /*#__PURE__*/React.createElement(Gamepad2, {
    size: 16,
    "aria-hidden": "true"
  }), quizCopy('quiz.concept_quest_live', 'Concept Quest with class')), /*#__PURE__*/React.createElement("p", {
    className: "mt-1 px-1 text-sm text-slate-600"
  }, quizCopy('quiz.concept_quest_description', 'Send teams on a live collaborative concept adventure.')))))), isTeacherMode && activeSessionCode && (sessionData?.escapeRoomState?.isActive || sessionData?.escapeRoomState?.isGameOver) && /*#__PURE__*/React.createElement(ErrorBoundary, {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "text-2xl sm:text-3xl font-black text-yellow-300 tracking-wide uppercase drop-shadow-md flex items-center gap-3"
    }, /*#__PURE__*/React.createElement(Gamepad2, {
      size: 32
    }), " ", t('review_game.title')), /*#__PURE__*/React.createElement("p", {
      className: "text-slate-200 text-sm mt-2 font-medium"
    }, quizCopy('review_game.all_formats_help', 'Pick a tile, answer together, then reveal the guide and award points. Every assessment item is included.')), /*#__PURE__*/React.createElement("p", {
      "data-review-progress": true,
      role: "status",
      "aria-live": "polite",
      className: "mt-2 text-sm font-bold text-yellow-200"
    }, reviewCompleted + ' of ' + reviewQuestions.length + ' items completed · ' + reviewCategories.length + (reviewCategories.length === 1 ? ' category' : ' categories'))), /*#__PURE__*/React.createElement("div", {
      ...
      type: "button",
      "aria-label": t('review_game.reset'),
      onClick: () => {
        setConfirmDialog({
      ...
      "data-review-board": true,
      className: "grid gap-2 sm:gap-4 max-w-4xl mx-auto w-full flex-grow content-start relative z-10",
      style: {
        gridTemplateColumns: 'repeat(' + Math.max(1, reviewCategories.length) + ', minmax(0, 1fr))'
      }
    }, reviewCategories.map((cat, cIdx) => {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: "text-xs normal-case tracking-normal font-medium text-slate-300"
      }, cat.questions.length + (cat.questions.length === 1 ? ' item' : ' items'))), cat.questions.map((q, qIdx) => {
        const isClaimed = reviewClaimed.has(q.originalIndex);
        return /*#__PURE__*/React.createElement("button", {
          type: "button",
          key: q.originalIndex,
          "data-review-tile": q.originalIndex,
          onClick: () => !isClaimed && handleReviewTileClick(q, q.points),
          disabled: isClaimed,
          "aria-label": `Category: ${cat.name}, question ${q.originalIndex + 1}, ${q.points} points${isClaimed ? ', completed' : ''}`,
        ...
        }, isClaimed ? /*#__PURE__*/React.createElement(CheckCircle2, {
          size: 24,
          "aria-hidden": "true"
        }) : q.points));
      }));
    })), reviewQuestions.length === 0 && /*#__PURE__*/React.createElement("p", {
      className: "mt-6 text-center text-slate-200"
    }, "No assessment items are available yet. Add questions to start a review game."), activeReviewQuestion && /*#__PURE__*/React.createElement("div", {
      ref: reviewDialogRef,
      tabIndex: -1,
      className: "fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/90 p-3 sm:p-6 backdrop-blur-sm",
      role: "dialog",
      "aria-modal": "true",
      ...
      className: "relative flex max-h-[90vh] w-full max-w-4xl flex-col overflow-y-auto rounded-2xl border-4 border-yellow-500 bg-blue-900 p-4 text-center shadow-2xl sm:p-8"
    }, /*#__PURE__*/React.createElement("div", {
      className: "flex items-start justify-between gap-3"
    }, /*#__PURE__*/React.createElement("span", {
      className: "rounded-full bg-yellow-400 px-4 py-2 text-lg font-black text-blue-950"
    }, activeReviewQuestion.points + ' points'), /*#__PURE__*/React.createElement("button", {
      type: "button",
      ref: reviewCloseBtnRef,
      "aria-label": t('common.close'),
      onClick: () => closeReviewModal(false),
      className: "inline-flex min-h-11 min-w-11 items-center justify-center rounded-lg text-blue-100 hover:bg-blue-800 hover:text-white"
    }, /*#__PURE__*/React.createElement(X, {
      size: 24
    }))), /*#__PURE__*/React.createElement("div", {
      className: "my-6"
    ...
    }, 'Assessment question ' + (activeReviewQuestion.originalIndex + 1)), /*#__PURE__*/React.createElement("h3", {
      id: "quiz-review-question-title",
      className: "text-2xl font-bold leading-tight text-white sm:text-3xl"
    }, formatInlineText(activeReviewQuestion.question || '', false, true)), activeReviewQuestion.question_en && /*#__PURE__*/React.createElement("p", {
      className: "mt-4 text-lg italic text-blue-200"
    }, formatInlineText(activeReviewQuestion.question_en, false, true))), activeReviewQuestion.imageUrl && /*#__PURE__*/React.createElement("img", {
      src: activeReviewQuestion.imageUrl,
      alt: _quizAuthoredImageAlt(activeReviewQuestion.imageAltText),
      className: "mb-5 max-h-80 w-full rounded-lg bg-white object-contain"
    }), activeReviewIsMcq ? /*#__PURE__*/React.createElement("div", {
      "data-review-question-type": "mcq",
      className: "mb-6 text-left"
    }, /*#__PURE__*/React.createElement("div", {
      className: "grid grid-cols-1 gap-3 sm:grid-cols-2"
    }, activeReviewQuestion.options.map((option, optionIndex) => {
      const correct = _quizAnswerMatches(option, activeReviewQuestion.correctAnswer);
      return /*#__PURE__*/React.createElement("div", {
        key: optionIndex,
        className: 'rounded-xl border-2 p-4 text-lg font-medium ' + (reviewGameState.showAnswer && correct ? 'border-green-400 bg-green-800 text-white' : 'border-blue-500 bg-blue-800 text-white')
      }, /*#__PURE__*/React.createElement("span", {
        className: "mr-2 inline-block font-bold"
      }, String.fromCharCode(65 + optionIndex) + '.'), formatInlineText(option, false, true), reviewGameState.showAnswer && correct && /*#__PURE__*/React.createElement("span", {
        className: "ml-2 text-sm font-bold"
      }, "✓ Correct"));
    })), reviewGameState.showAnswer && /*#__PURE__*/React.createElement("div", {
      "data-review-answer-guide": true,
      ...
      compact: true,
      q: activeReviewQuestion,
      index: activeReviewQuestion.originalIndex,
      showAnswer: reviewGameState.showAnswer,
      formatInlineText: formatInlineText,
      renderFormattedText: renderFormattedText
    })), /*#__PURE__*/React.createElement("div", {
      className: "mt-auto flex flex-col items-center gap-5 border-t border-blue-700 pt-5"
      ...
      className: "min-h-11 rounded-full bg-yellow-400 px-6 py-3 text-lg font-bold text-blue-950 hover:bg-yellow-300"
    }, t('review_game.reveal_answer')) : /*#__PURE__*/React.createElement("div", {
      className: "w-full"
    }, /*#__PURE__*/React.createElement("p", {
      className: "mb-3 text-sm font-bold text-blue-100"
    }, quizCopy('review_game.award_help', 'Use the answer guide to decide which team earned the points.')), /*#__PURE__*/React.createElement("div", {
      className: "flex flex-wrap justify-center gap-3"
    }, gameTeams.map(team => /*#__PURE__*/React.createElement("button", {
      type: "button",
      "aria-label": `Award ${activeReviewQuestion.points} points to ${team.name}`,
      key: team.id,
      onClick: () => handleAwardPoints(team.id, activeReviewQuestion.points),
      className: team.color + ' min-h-11 rounded-lg px-4 py-2 font-bold shadow-md border-b-4 border-black/20 hover:opacity-90 ' + (team.color.includes('yellow') ? 'text-indigo-900' : 'text-white')
      ...
      className: "min-h-11 rounded-lg bg-slate-700 px-4 py-2 font-bold text-white hover:bg-slate-600"
    }, t('review_game.no_points')))))))));
  })() : escapeRoomState.isActive ? window.AlloModules && window.AlloModules.EscapeRoomGameplay ? /*#__PURE__*/React.createElement(window.AlloModules.EscapeRoomGameplay, {
    ...
    "data-assessment-presentation": true,
    ...
    "aria-pressed": presentationAllQuestions,
    className: "min-h-11 rounded-lg border border-indigo-300 px-3 py-2 text-sm font-bold text-indigo-800"
  }, presentationAllQuestions ? 'Show one question' : 'Show all questions'), /*#__PURE__*/React.createElement("span", {
    role: "status",
    "aria-live": "polite",
    className: "text-sm font-bold text-slate-700"
  }, presentationAllQuestions ? presentationSlides.length + ' questions' : presentationSlides.length ? 'Question ' + (presentationCurrent + 1) + ' of ' + presentationSlides.length : 'No questions'), !presentationAllQuestions && presentationSlides.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
    type: "button",
    disabled: presentationCurrent === 0,
    onClick: () => setPresentationQuestionIndex(presentationCurrent - 1),
    className: "min-h-11 rounded-lg border border-slate-300 px-3 py-2 text-sm font-bold disabled:opacity-50"
  }, "Previous question"), /*#__PURE__*/React.createElement("label", {
    className: "flex items-center gap-2 text-sm font-semibold"
  }, "Go to question", /*#__PURE__*/React.createElement("select", {
    "aria-label": "Go to presentation question",
    value: presentationCurrent,
    onChange: event => setPresentationQuestionIndex(Number(event.target.value)),
    className: "min-h-11 rounded-lg border border-slate-300 px-2"
  }, presentationSlides.map((index, position) => /*#__PURE__*/React.createElement("option", {
    key: index,
    value: position
  }, position + 1)))), /*#__PURE__*/React.createElement("button", {
    type: "button",
    disabled: presentationCurrent >= presentationSlides.length - 1,
    onClick: () => setPresentationQuestionIndex(presentationCurrent + 1),
    className: "min-h-11 rounded-lg border border-slate-300 px-3 py-2 text-sm font-bold disabled:opacity-50"
  }, "Next question"))), presentationQuestions.map((q, i) => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: i,
      "data-presentation-question-type": "mcq",
      className: "bg-white p-4 sm:p-8 rounded-2xl border-2 border-slate-200 shadow-md hover:shadow-lg transition-all motion-reduce:transition-none"
    }, /*#__PURE__*/React.createElement("div", {
      className: "flex gap-4 mb-6"
    }, /*#__PURE__*/React.createElement("div", {
      className: "bg-teal-100 text-teal-800 w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold shrink-0 shadow-sm"
      ...
      className: "text-lg text-slate-600 italic mt-2"
    }, formatInlineText(q.question_en, false)))), q.imageUrl && /*#__PURE__*/React.createElement("img", {
      src: q.imageUrl,
      alt: _quizAuthoredImageAlt(q.imageAltText),
      className: "mb-5 max-h-80 w-full rounded-lg object-contain"
    }), /*#__PURE__*/React.createElement("div", {
      ...
      }, q.optionImageUrls?.[optIdx] && /*#__PURE__*/React.createElement("img", {
        src: q.optionImageUrls[optIdx],
        alt: _quizAuthoredImageAlt(q.optionImageAltTexts?.[optIdx]),
        className: "mb-3 h-32 w-full rounded object-contain"
      }), /*#__PURE__*/React.createElement("span", {
        className: "mr-2"
      }, String.fromCharCode(65 + optIdx), "."), formatInlineText(opt, false), q.options_en && q.options_en[optIdx] && /*#__PURE__*/React.createElement("div", {
    ...
    className: "space-y-8"
  }, presentationReflections.map((ref, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: "bg-indigo-800/50 p-6 rounded-xl border border-indigo-700"
  }, /*#__PURE__*/React.createElement("p", {
    className: "text-2xl font-medium leading-relaxed"
  }, typeof ref === 'string' ? ref : ref.text), ref?.text_en && /*#__PURE__*/React.createElement("p", {
