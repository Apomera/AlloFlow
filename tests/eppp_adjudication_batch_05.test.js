...
describe('EPPP claim-level adjudication batch 05', () => {
  it('reviews ten high-risk candidates without releasing or repeating them', () => {
    const report = read('quality/eppp_provenance/evidence/adjudication/adjudication_batch_05.json');
    const priorIds = new Set(['01', '02', '03', '04'].flatMap((batch) => read(`quality/eppp_provenance/evidence/adjudication/adjudication_batch_${batch}.json`).items.map((item) => item.legacyId)));
  ...
  it('requires complete option feedback, findings, and named provenance', () => {
    const report = read('quality/eppp_provenance/evidence/adjudication/adjudication_batch_05.json');
  ...
  it('keeps JSON and Markdown canonical and outside runtime publication', () => {
    expect(fs.existsSync(resolve(root, 'quality/eppp_provenance/evidence/adjudication/adjudication_batch_05.json'))).toBe(true);
    expect(fs.existsSync(resolve(root, 'quality/eppp_provenance/evidence/adjudication/adjudication_batch_05.md'))).toBe(true);
    expect(fs.existsSync(resolve(root, 'desktop/web-app/public/test_prep/eppp_legacy/adjudication_batch_05.json'))).toBe(false);
  });
});
