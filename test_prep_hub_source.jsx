                  ...
                  {visible.length ? <div className="grid gap-4 md:grid-cols-2">
                    {visible.map((route) => {
                      ...
                      return <article key={route.id} data-test-prep-foundational-route-id={route.id} className="flex flex-col rounded-2xl border border-emerald-300 bg-white p-5 shadow-sm">
                        ...
                        <div className="mt-3 flex flex-wrap gap-2" aria-label="Linked topic drills">{routeTopicRecords.slice(0, 10).map((topic) => <button key={topic.topicId} type="button" disabled={!topic.itemIds.length} title={topic.label || 'Topic ' + topic.topicId} aria-label={'Practice topic ' + topic.topicId + (topic.label ? ': ' + topic.label : '')} onClick={() => startStudyRoute({ ...route, title: 'Foundational document: ' + route.title + ' · Topic ' + topic.topicId, itemIds: topic.itemIds })} className="rounded-full border border-slate-400 bg-slate-100 px-3 py-1 text-left text-xs font-bold text-slate-700 hover:bg-slate-200 disabled:cursor-not-allowed disabled:opacity-60 focus:ring-2 focus:ring-emerald-700">Practice topic {topic.topicId}{topic.label ? ' · ' + topic.label : ''} <span className="font-black">({topic.itemIds.length})</span></button>)}{(route.topicIds || []).length > 10 && <span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold text-slate-700">+{route.topicIds.length - 10} more</span>}</div>
