...
// A model field the prompt declares as text is not guaranteed to BE text, and React throws
  ...
  function renderReadingLevelExplanation(definitionData, t, renderFormattedText) {
    ...
    return React.createElement('div', {
      ...
      React.createElement('div', { className: 'flex items-center gap-2 mb-1.5 flex-wrap' },
        React.createElement('span', { className: 'text-[10px] font-bold uppercase tracking-wide text-indigo-700' },
          t('glossary.popups.reading_level_explanation') || 'Reading-level explanation'),
        React.createElement('span', { className: 'text-[10px] font-semibold text-indigo-600 bg-white border border-indigo-200 rounded-full px-1.5 py-0.5' },
          t('glossary.popups.ai_generated') || 'AI-generated')),
      React.createElement('div', { className: 'text-sm text-slate-700 leading-relaxed' },
  ...
  function resolveSimplifiedReferences(adaptedBody, adaptedReferences, inputReferences, citationAudit) {
    ...
    var auditAllowsFallback = !citationAudit ||
      (citationAudit.enabled === true && Number(citationAudit.sourceCitationCount || 0) > 0);
  ...
  function getInstructionalContextApi() {
    try {
      return window.AlloModules && window.AlloModules.InstructionalContext
  ...
  function fallbackInstructionalText(item) {
    ...
    var rawAuthorization = raw.replacementAuthorization && typeof raw.replacementAuthorization === 'object'
    ...
    var form = ['original', 'same-text-supported', 'adapted'].indexOf(raw.form) >= 0
      ? raw.form
      : (resource.type === 'simplified' ? 'adapted' : 'original');
    ...
    return {
      ...
      designationSource: ['educator', 'workflow-default', 'legacy-inferred'].indexOf(raw.designationSource) >= 0
        ? raw.designationSource
        : 'legacy-inferred',
      ...
      complexity: {
        ...
        measuredGrade: rawComplexity.measuredGrade !== undefined
          ? rawComplexity.measuredGrade
          : (resource.localStats && resource.localStats.score !== undefined ? resource.localStats.score : null),
  ...
  function getSimplifiedInstructionalText(item) {
    ...
    if (api && typeof api.getInstructionalText === 'function') {
      try { return api.getInstructionalText(item); } catch (_) {}
    }
  ...
  function updateSimplifiedInstructionalRole(item, requestedRole) {
    var resource = item && typeof item === 'object' ? item : {};
    var role = ['primary', 'supplemental', 'unspecified'].indexOf(requestedRole) >= 0
      ? requestedRole
      : 'unspecified';
    ...
    var nextProfile = Object.assign({}, current, {
      ...
      replacementAuthorization: role === 'primary' && current.form === 'adapted'
        ? { authorized: true, source: 'educator' }
        : { authorized: false, source: 'none' }
    });
    var api = getInstructionalContextApi();
    if (api && typeof api.normalizeInstructionalText === 'function') {
      try { nextProfile = api.normalizeInstructionalText(nextProfile); } catch (_) {}
    }
    return Object.assign({}, resource, { instructionalText: nextProfile });
  }

  function artifactIdentityValues(item) {
    if (!item || typeof item !== 'object') return [];
    return [item.id, item.uiId, item.artifactId, item.resourceId]
  ...
  function artifactsMatch(left, right) {
    ...
    if (leftIds.some(function (value) { return rightIds.indexOf(value) >= 0; })) return true;
    return left === right || (
  ...
  function resolveSimplifiedCompareSource(history, adaptedItem, fallbackText) {
    ...
    if (snapshot) return { text: snapshot.text, artifact: { id: snapshot.sourceArtifactId, config: { language: snapshot.language }, data: snapshot.text }, selection: 'captured-source' };
    var profile = getSimplifiedInstructionalText(adaptedItem);
    var linkedIds = [profile.sourceArtifactId, profile.primaryArtifactId]
    ...
    for (var linkIndex = 0; linkIndex < linkedIds.length; linkIndex += 1) {
      for (var historyIndex = safeHistory.length - 1; historyIndex >= 0; historyIndex -= 1) {
        ...
        if (linkedText) return { text: linkedText, artifact: candidate, selection: 'linked-artifact' };
      }
    }
    return { text: '', artifact: null, selection: 'original-not-captured' };
  }
  ...
  function findSimplifiedComparisonSupportOwner(history, current, comparison, displayedText) {
    ...
    if ((!snapshot || snapshot.text !== comparison.text) && getSimplifiedInstructionalText(owner).form !== 'adapted') {
      snapshot = api.createSourceSnapshot?.(comparison.text, { sourceArtifactId: owner.id });
    }
  ...
  function findSimplifiedReadingCompanion(history, original, snapshot) {
    ...
    return (Array.isArray(history) ? history : []).slice().reverse().find(function (item) {
      return item?.type === 'simplified' && getSimplifiedInstructionalText(item).form === 'adapted'
        && api.sameReadingSourceFamily(item, original)
        && sameSimplifiedReadingSnapshot(api.getSourceSnapshot(item), snapshot);
    }) || null;
  }
  ...
  function getSimplifiedComplexityDisplay(item, ambientGrade) {
    ...
    var hasCanonicalProfile = !!(
      resource.instructionalText || resource.textProfile ||
      resourceConfig.instructionalText || resourceConfig.textProfile
    );
    ...
    if (!hasCanonicalProfile && !Number.isFinite(measured) &&
        resource.localStats && resource.localStats.score !== undefined) {
      rawMeasured = resource.localStats.score;
      measured = Number(rawMeasured);
    }
    var targetGrade = complexity.requestedGrade || resource.targetGradeLevel ||
      resourceConfig.grade || ambientGrade || '';
    var api = getInstructionalContextApi();
    var currentFingerprint = '';
    if (api && typeof api.fingerprintText === 'function' && typeof resource.data === 'string') {
      try { currentFingerprint = api.fingerprintText(resource.data); } catch (_) {}
    }
    if (complexity.contentFingerprint && currentFingerprint && complexity.contentFingerprint !== currentFingerprint) {
      return { measuredGrade: null, targetGrade: targetGrade, status: 'stale', target: null };
    }
    if (!Number.isFinite(measured)) {
      return { measuredGrade: null, targetGrade: targetGrade, status: complexity.status || 'unavailable', target: null };
    }
    var languageIsEnglish = true;
    if (api && typeof api.isEnglishLanguage === 'function') {
      try { languageIsEnglish = api.isEnglishLanguage(complexity.language || 'English'); } catch (_) {}
    }
    var status = !languageIsEnglish
      ? 'unavailable'
      : api && typeof api.complexityStatus === 'function'
        ? api.complexityStatus(measured, targetGrade)
        : (['below-target', 'within-target', 'above-target'].indexOf(complexity.status) >= 0
          ? complexity.status
          : 'unavailable');
    var target = null;
    if (api && typeof api.getComplexityTarget === 'function') {
      try { target = api.getComplexityTarget(targetGrade); } catch (_) {}
    }
    return { measuredGrade: measured, targetGrade: targetGrade, status: status, target: target };
  }
  ...
  async function checkSimplifiedAlignment(deps) {
    ...
    var alignmentContext = contextModule && typeof contextModule.resolveArtifactContext === 'function'
    ...
    var alignmentStandardsText = (function () {
      ...
      var entries = Array.isArray(alignmentStandardsValue.standards)
        ? alignmentStandardsValue.standards
        : (Array.isArray(alignmentStandardsValue) ? alignmentStandardsValue : []);
    ...
    try {
      ...
      var fingerprintText = contextModule && typeof contextModule.fingerprintText === 'function'
        ? contextModule.fingerprintText(textToCheck)
        : String(textToCheck.length) + '|' + textToCheck.slice(0, 64) + '|' + textToCheck.slice(-64);
      var fingerprintStandards = contextModule && typeof contextModule.fingerprintValue === 'function'
        ? contextModule.fingerprintValue(alignmentStandardsValue)
        : '';
      var analysis = {
        ...parsedAnalysis,
      ...
      var updatedContent = { ...generatedContent, alignmentCheck: analysis };
      setGeneratedContent(updatedContent);
      setHistory(function (previous) {
        return previous.map(function (item) { return item.id === generatedContent.id ? updatedContent : item; });
      });
  ...
  async function regenerateSimplifiedWithRigor(deps) {
    ...
    try {
      ...
      var originalParts = isLeveledText
        ? splitReferencesFromBody(rawText)
        : { body: rawText, references: '' };
      ...
      var validateRigorCitations = function (original, candidate) {
        ...
        var normalizeDecision = function (result) {
          if (typeof result === 'boolean') return { known: true, valid: result };
          if (!result || typeof result !== 'object') return { known: false, valid: false };
          if (Object.prototype.hasOwnProperty.call(result, 'valid')) return { known: true, valid: result.valid === true };
          if (Object.prototype.hasOwnProperty.call(result, 'ok')) return { known: true, valid: result.ok === true };
          if (Object.prototype.hasOwnProperty.call(result, 'conserved')) return { known: true, valid: result.conserved === true };
          return { known: false, valid: false };
        };
        ...
        try {
          ...
          var orderChanged = originalOccurrences.length !== candidateOccurrences.length ||
            originalOccurrences.some(function (entry, index) { return entry?.key !== candidateOccurrences[index]?.key; });
      ...
      var contextModule = typeof window !== 'undefined' && window.AlloModules
        ? window.AlloModules.InstructionalContext
        : null;
      var ambientStandardsContext = typeof activeResolvedStandardsContext !== 'undefined'
        ? activeResolvedStandardsContext
        : null;
      var ambientStandards = typeof standardsInput !== 'undefined'
        ? standardsInput
        : (typeof targetStandards !== 'undefined' ? targetStandards : null);
      var rigorContext = contextModule && typeof contextModule.resolveArtifactContext === 'function'
        ? contextModule.resolveArtifactContext(generatedContent, {
            grade: gradeLevel,
            language: leveledTextLanguage,
      ...
      var rigorStandards = (function () {
        ...
        var entries = Array.isArray(rigorStandardsValue.standards)
      ...
      var prompt = `
        ...
        `;
      var rigorTranslationChoices = typeof translationTargetChoices === 'function'
        ? translationTargetChoices(rigorLanguage, currentUiLanguage, typeof selectedLanguages !== 'undefined' ? selectedLanguages : [])
        : [];
      var rigorTranslationPolicy = typeof resolveTranslationPolicy === 'function'
      ...
      if (isLeveledText) {
        ...
        var originalForValidation = sourceExtraction.isBilingual
          ? originalParts.body
          : currentText;
        var candidateForValidation = sourceExtraction.isBilingual
          ? candidateBody
          : candidateTarget;
        var conservation = validateRigorCitations(originalForValidation, candidateForValidation);
        var shouldValidateGeneratedEnglish = !sourceExtraction.isBilingual
          && (candidateExtraction.isBilingual || String(rigorLanguage || '').trim().toLowerCase() !== 'english');
        if (shouldValidateGeneratedEnglish) {
          var englishConservation = validateRigorCitations(
            candidateTarget,
      ...
      var priorConfig = generatedContent.config && typeof generatedContent.config === 'object'
        ? generatedContent.config
        : {};
      var priorAudit = priorConfig.citationAudit && typeof priorConfig.citationAudit === 'object'
      ...
      if (isLeveledText && typeof applySimplifiedTextMutation === 'function') {
        ...
        var fallbackContextModule = typeof window !== 'undefined' && window.AlloModules
          ? window.AlloModules.InstructionalContext
          : null;
  ...
  function getReadingNavigationSections(text) {
    ...
    const append = end => {
      ...
      if (sectionText.trim()) sections.push({ index: sections.length, start, end, text: sectionText });
    };
    while ((match = breaks.exec(source))) { append(match.index); start = match.index + match[0].length; }
  ...
  function findRelatedOriginalPassage(sourceText, adaptedSectionText, options = {}) {
    ...
    const stopWords = new Set(('a an the and or but if so than then that this these those to of in on at by for from with without as into upon over under ' +
      'is are was were be been being am do does did done have has had having will would shall should can could may might must not no nor ' +
      ...
      'all any both each every few more most other some such only own same too very just also there here now again still about after before during ' +
      'first second third one two three said says say text passage paragraph scene act chapter mr mrs ms').split(/\s+/));
    let segmenter;
    try { if (typeof Intl !== 'undefined' && Intl.Segmenter) segmenter = new Intl.Segmenter(undefined, { granularity: 'word' }); } catch (_) {}
    const tokenize = text => {
      const words = segmenter ? Array.from(segmenter.segment(text)).filter(part => part.isWordLike).map(part => part.segment)
    ...
    const prepared = sections.map(section => {
      const words = tokenize(section.text); totalWords += words.length;
      return { ...section, words, terms: new Set(words.filter(informative)) };
    });
    ...
    for (let start = 0; start < prepared.length; start++) {
      for (let count = 1; count <= 3 && start + count <= prepared.length; count++) {
        ...
        candidates.push({ start: group[0].start, end: group[group.length - 1].end,
          sectionIndexes: group.map(section => section.index), score,
          evidence: { sharedTerms: shared.slice(0, 16), sharedPhrases: Array.from(phrases).slice(0, 6), queryCoverage: Math.round(queryCoverage * 100) / 100 } });
      }
    }
    ...
    const project = candidate => ({ start: candidate.start, end: candidate.end, sectionIndexes: candidate.sectionIndexes, evidence: candidate.evidence });
    if (alternatives.length) return { status: 'ambiguous', reason: 'multiple-related-passages', candidates: [best, ...alternatives].slice(0, 3).map(project) };
    return { status: 'matched', ...project(best), text: sourceText.slice(best.start, best.end),
  ...
  function findReadingGlossOccurrences(text, query) {
    const source = String(text || ''), word = String(query || '').trim(), matches = [];
    const finish = truncated => { Object.defineProperty(matches, 'truncated', { value: truncated, enumerable: false }); return matches; };
    if (!word || word.length > 160) return finish(false);
    const letters = /[\p{L}\p{M}\p{N}_]/u, characters = Array.from(word);
    let segments;
    try { segments = typeof Intl.Segmenter === 'function' ? new Intl.Segmenter(undefined, { granularity: 'word' }).segment(source) : null; } catch (_) {}
  ...
  function ReadingGlossEditor({ item, supports, request, onUpdate, disabled, adapted, snapshot: adaptedSnapshot, embedded }) {
    ...
    const termRef = React.useRef(null), definitionRef = React.useRef(null), addRef = React.useRef(null), toggleRef = React.useRef(null), keepRef = React.useRef(null);
    ...
    const performAction = async action => {
      ...
      if (action.type === 'close' || action.type === 'finish') {
        ...
        if (action.type === 'close') { setOpen(false); toggleRef.current?.focus(); }
    ...
    const listed = entries.filter(entry => (kind === 'all' || (kind === 'essential' ? entry.priority === 'essential' : kind === 'educator' ? entry.origin === 'educator' : entry.origin !== 'educator'))
      && (!needle || (entry.quote + ' ' + (entry.definition || entry.explanation || entry.text || '')).toLocaleLowerCase().includes(needle)));
    ...
    const matches = React.useMemo(() => {
      ...
      if (anchor) return sourceText.slice(anchor.start, anchor.end) === anchor.quote
        ? [{ ...anchor, context: contextFor(anchor) }] : [];
    ...
    return <section data-reading-gloss-editor className={embedded ? 'text-sm text-slate-800' : 'my-4 rounded-xl border border-indigo-200 bg-white p-3 text-sm text-slate-800'}>
      ...
      {isOpen && <div id={panelId} className={embedded ? 'space-y-3' : 'mt-2 space-y-3'}>
        ...
        {canFilter && <div data-gloss-filter role="search" aria-label={simplifiedText('simplified.gloss_find_a_word_support', 'Find a word support')} className="flex flex-wrap items-end gap-2">
          <label className="flex min-w-0 flex-1 flex-col font-semibold">{simplifiedText('simplified.gloss_find_a_word_support', 'Find a word support')}<input type="search" value={filterQuery} onChange={event => setFilterQuery(event.target.value)} className="mt-1 min-h-11 w-full min-w-0 rounded border border-slate-400 bg-white px-2 font-normal" /></label>
          <label className="flex flex-col font-semibold">{simplifiedText('simplified.gloss_filter_show', 'Show')}<select value={filterKind} onChange={event => setFilterKind(event.target.value)} className="mt-1 min-h-11 rounded border border-slate-400 bg-white px-2 font-normal"><option value="all">{simplifiedText('simplified.gloss_filter_all', 'All supports')}</option><option value="essential">{simplifiedText('simplified.gloss_essential', 'Essential')}</option><option value="educator">{simplifiedText('simplified.gloss_teacher_edited', 'Teacher edited')}</option><option value="suggested">{simplifiedText('simplified.gloss_suggested', 'Suggested')}</option></select></label>
  ...
  function AdaptedWordHelp({ item, teacher, disabled, onUpdate, onGenerate, languageTag, direction, onListen, listening, renderCredits, originalSupports, passageSelector, language, markWords, quiet }) {
    ...
    const panel = teacher && <details data-adapted-word-help-teacher open={panelOpen} onToggle={event => setPanelOpen(event.currentTarget.open)} className="my-4 space-y-3 rounded-xl border border-indigo-200 bg-white p-3 text-sm text-slate-800">
      <summary className="min-h-11 cursor-pointer rounded-lg px-2 py-2 font-bold text-indigo-900 focus-visible:ring-2 focus-visible:ring-indigo-600">{simplifiedText('simplified.word_help_word_help', 'Word help')}<span data-word-help-summary className="font-normal text-slate-700">{' · ' + (stale ? simplifiedText('simplified.word_help_summary_needs_review', 'Needs review after an edit') : !entries.length ? simplifiedText('simplified.word_help_summary_none', 'No supports yet') : (entries.length === 1 ? simplifiedText('simplified.word_help_summary_one', '1 support') : simplifiedText('simplified.word_help_summary_count', '{count} supports', { count: entries.length })) + ' · ' + (help.shown ? simplifiedText('simplified.word_help_summary_shown', 'Shown to students') : simplifiedText('simplified.word_help_summary_hidden', 'Hidden from students')))}</span></summary>
  ...
  function SimplifiedView(props) {
    ...
    var buildSimplifiedContentParts = function (rawText) {
      ...
      var split = { body: fullText, references: '' };
      try {
    ...
    try {
      if (inputText && typeof splitReferencesFromBody === 'function')
        simplifiedInputReferences = String((splitReferencesFromBody(inputText) || {}).references || '');
    } catch (_) {}
    ...
    var saveTtsAsPlayed_state = React.useState(function () {
      ...
      try { return localStorage.getItem('allo_save_karaoke_audio') !== '0'; } catch (_) { return true; }
    });
      ...
      try {
        ...
        if (st) {
          ...
          if (typeof st.getCompatible === 'function') {
            var compatibleUrl = st.getCompatible(sentenceText, { voice: voice, speed: speed, language: language });
    ...
    var getReadAloudAudioProvenance = function (sentence, identityOptions) {
      ...
      if (source === 'human-teacher') return { source: source, label: simplifiedTeacherRecordingLabel, metadata: metadata, stale: false };
      if (source === 'human-student') return { source: source, label: simplifiedStudentRecordingLabel, metadata: metadata, stale: false };
      if (source && String(source).indexOf('human') === 0) return { source: source, label: simplifiedHumanRecordingLabel, metadata: metadata, stale: false };
    ...
    var [helpAudioState, setHelpAudioState] = React.useState({ key: null, status: 'idle' });
    ...
    var simplifiedPopupSpokenText = function (parts) {
      return (Array.isArray(parts) ? parts : [parts])
        .map(function (part) { return String(part == null ? '' : part).replace(/\s+/g, ' ').trim(); })
        .filter(Boolean)
        .join('. ');
    };
    ...
    function containSimplifiedModalFocus(e, container, onEscape) {
      ...
      var focusable = Array.prototype.slice.call(container.querySelectorAll('button:not([disabled]), textarea:not([disabled]), input:not([disabled]):not([type="hidden"]), select:not([disabled]), a[href], summary, [tabindex]:not([tabindex="-1"])')).filter(function (el) {
        ...
        for (var parent = el; parent && parent !== container; parent = parent.parentElement) {
          ...
          if (parent.tagName === 'DETAILS' && !parent.open) {
            var summary = Array.prototype.find.call(parent.children, function (child) { return child.tagName === 'SUMMARY'; });
      ...
      else if (!focusable.includes(document.activeElement)) { e.preventDefault(); (e.shiftKey ? last : first).focus(); }
    }
    React.useEffect(function () {
    ...
    React.useEffect(function () {
      ...
      var previouslyFocused = document.activeElement;
      var timer = setTimeout(function () { if (definitionCloseRef.current) definitionCloseRef.current.focus(); }, 0);
      return function () {
        clearTimeout(timer);
    ...
    React.useEffect(function () {
      ...
      var previouslyFocused = document.activeElement;
      var timer = setTimeout(function () { if (revisionCloseRef.current) revisionCloseRef.current.focus(); }, 0);
      return function () {
        clearTimeout(timer);
    ...
    var replacementIsEducatorAuthorized = !!(
      instructionalTextProfile.replacementAuthorization &&
      instructionalTextProfile.replacementAuthorization.authorized === true &&
    ...
    var instructionalRoleTone = instructionalRole === 'supplemental'
      ...
      : instructionalRole === 'primary' && replacementIsEducatorAuthorized
    ...
    React.useEffect(function () {
      ...
      if (wasFocusViewRef.current && !isZenMode && document.activeElement === document.body) {
        focusViewButtonRef.current?.focus({ preventScroll: true });
      }
      wasFocusViewRef.current = !!isZenMode;
    }, [isZenMode]);
    var [practiceOpen, setPracticeOpen] = React.useState(false);
    var priorReadingRef = React.useRef({ id: generatedContent?.id, text: generatedContent?.data });
    React.useEffect(function () {
      var previous = priorReadingRef.current;
      priorReadingRef.current = { id: generatedContent?.id, text: generatedContent?.data };
    ...
    React.useEffect(function () {
      ...
      return function () { if (opener && document.contains(opener)) opener.focus(); };
    }, [!!selectionMenu]);
    // Session-only reading bookmarks are scoped to an exact artifact and source.
    var readerSurfaceRef = React.useRef(null);
    var navigationRef = React.useRef({ positions: new Map(), companions: [], pending: null });
    ...
    function rememberReadingPosition() {
      ...
      var panes = {};
      surface.querySelectorAll('[data-compare-version]').forEach(node => { panes[node.dataset.compareVersion] = node.scrollTop; });
      ...
      var bookmark = { outer: outer.scrollTop, panes, anchor: anchor?.getAttribute('data-reading-paragraph'), offset: anchor ? anchor.getBoundingClientRect().top - top : null };
      var positions = navigationRef.current.positions;
      positions.delete(navigationViewKey); positions.set(navigationViewKey, bookmark);
    ...
    function prepareReadingNavigation(version, focusPassage) {
      ...
      navigationRef.current.pending = { from: navigationViewKey, version, focusPassage: !!focusPassage };
    }
    React.useLayoutEffect(function () {
      ...
      var surface = readerSurfaceRef.current, outer = readingOuterScroller();
      ...
      if (saved) {
        surface.querySelectorAll('[data-compare-version]').forEach(node => { node.scrollTop = saved.panes[node.dataset.compareVersion] || 0; });
        ...
        surface.querySelectorAll('[data-compare-version]').forEach(node => { node.scrollTop = 0; });
    ...
    function renderSimplifiedComparison() {
      ...
      function showRelatedOriginal() {
        ...
        var result = findRelatedOriginalPassage(sourceText, chosenSection.text, { sameLanguage: true });
        setRelatedPassage(Object.assign({ key: passageKey }, result));
    ...
    return <div ref={readerSurfaceRef} data-adapted-reader={isTeacherMode ? "teacher" : "student"} className="space-y-6">{comparisonKaraoke && isCompareMode && KaraokeReaderOverlay && <KaraokeReaderOverlay isOpen={true} isTeacher={isTeacherMode} playbackOnly={true} onClose={() => setComparisonKaraoke(null)} text={comparisonKaraoke.text} sentenceList={comparisonKaraoke.sentences} sentenceLanguages={comparisonKaraoke.languages} language={simplifiedLanguageTag(comparisonKaraoke.language)} getAudioUrl={getComparisonKaraokeAudioUrl} />}<span className="sr-only" role="status" aria-live="polite" aria-atomic="true" data-read-aloud-status>{activeReadAloudStatus}</span>{isImmersiveReaderActive && generatedContent?.immersiveData && <div ref={immersiveDialogRef} role="dialog" aria-modal="true" aria-label={t('immersive.title') || 'Immersive Reader'} tabIndex={-1} onKeyDown={e => containSimplifiedModalFocus(e, immersiveDialogRef.current, handleCloseImmersiveReader)} className="fixed inset-0 z-[200] overflow-y-auto animate-in motion-reduce:animate-none fade-in zoom-in-95 duration-300 motion-reduce:animate-none motion-reduce:transition-none flex flex-col font-sans" style={{
        backgroundColor: immersiveSettings.bgColor || '#fdfbf7'
          ...
          if (!next) {
            ...
            return <div className="flex min-w-0 flex-wrap items-center justify-center gap-2"><h4 className="break-words font-comic font-bold text-xl text-orange-800">{isTeacherMode ? `${t('simplified.target_level_label')}: ${displayGrade}` : generatedContent?.title || generatedContent?.topic || sourceTopic || readerText('simplified.your_reading', 'Your reading')}</h4>{displayLang !== 'English' && <span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full font-bold border border-blue-200">{displayLang}</span>}{isTeacherMode && displayInterests.length > 0 && <span className="bg-red-100 text-red-800 text-xs px-2 py-1 rounded-full font-bold border border-red-200 flex items-center gap-1"><Heart size={10} /> {t('simplified.engagement_optimized')}</span>}{isTeacherMode && typeof displayStandards === 'string' && displayStandards && <span className="bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full font-bold border border-green-200 flex items-center gap-1 cursor-help" title={`${t('simplified.label_standard')}: ${displayStandards}`}><CheckCircle size={10} />{displayStandards.length > 20 ? displayStandards.substring(0, 20) + '...' : displayStandards}</span>}</div>;
          })()}</div>}<div className={`flex items-center gap-2 ${isZenMode ? 'justify-center mb-4' : 'justify-center'}`}><div className="flex flex-col gap-1 items-center min-w-0 w-full"><div role="group" aria-label={readerText('simplified.reading_actions', 'Reading tools')} className="flex flex-wrap justify-center w-full min-w-0 bg-white rounded-2xl p-2 border border-indigo-200 shadow-sm gap-2">
              {[
            ...
            {isTeacherMode && !generatedContent.levelCheck && simplifiedComplexityDisplay.measuredGrade !== null && (() => {
              ...
              const tone = status === 'above-target' ? 'bg-amber-50 border-amber-200 text-amber-900'
