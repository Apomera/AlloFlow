...
function aeOverviewDueBand(teacher, today = aeToday()) {
  ...
  const days = Math.round((Date.parse(teacher.dueDate + 'T12:00:00Z') - Date.parse(today + 'T12:00:00Z')) / 86400000);
...
function aeOverviewMatches(teacher, action, filters, today = aeToday()) {
  ...
  return (!query || text.includes(query)) && (!filters.owner || filters.owner === 'all' || action?.owner === filters.owner)
    && (!filters.due || filters.due === 'all' || aeOverviewDueBand(teacher, today) === filters.due);
}
...
function AeOverview({ openWorkflowAction = null, workspace, selectedTeacher, setSelectedTeacherId, role, setRole, updateTeacher, setTab, aiReflectionEnabled, askForReflection, reflection, readOnlyPreview = false, isRemote = false, requestActionReview, requestStaffAdd }) {
  ...
  activeTeachers.forEach((teacher) => { const band = aeOverviewDueBand(teacher, today); if (Object.prototype.hasOwnProperty.call(workload, band)) workload[band] += 1; });
  ...
  const evaluatorQueueAll = matchingTeachers.map((teacher) => ({ teacher, action: nextActions.get(teacher.id) }))
  ...
  React.useEffect(() => { setShowAllQueue(false); }, [worklistFilters.query, worklistFilters.owner, worklistFilters.due]);
  const evaluatorQueue = showAllQueue ? evaluatorQueueAll : evaluatorQueueAll.slice(0, 6);
  const openNextAction = (teacher, action) => { if (openWorkflowAction) { openWorkflowAction(teacher, action); return; } setSelectedTeacherId(teacher.id); setTab(action.tab); setTimeout(() => { const panel = document.getElementById('ae-panel'); if (panel) panel.focus(); }, 0); };
  return <div className="ae-page">
    ...
    <div className="ae-grid">
      ...
      {isEvaluator && <section className="ae-card ae-span-12" aria-labelledby="ae-worklist-filter-title">
        <h3 id="ae-worklist-filter-title">{t("educator_evaluation.find_your_next_record_20260908", "Find your next record")}</h3>
        <p className="ae-sub" id="ae-worklist-filter-help">{t("educator_evaluation.worklist_filter_help_20260908", "Search and filters apply to the action list and roster below. Completion totals still include every active educator you can access.")}</p>
        <div className="ae-worklist-tools">
          <label className="ae-field"><span>{t("educator_evaluation.search_worklist_20260908", "Search educators")}</span><input ref={worklistSearch} className="ae-input" type="search" maxLength={100} autoComplete="off" aria-controls="ae-next-actions-list ae-roster-status-list" aria-describedby="ae-worklist-filter-help" placeholder={t("educator_evaluation.worklist_search_placeholder_20260908", "Name, staff code, assignment, building, or evaluator")} value={worklistFilters.query} onChange={(event) => setWorklistFilters({ ...worklistFilters, query: event.target.value })}/></label>
          <label className="ae-field"><span id="ae-worklist-owner-label">{t("educator_evaluation.next_step_owner_20260908", "Next step owner")}</span><select aria-labelledby="ae-worklist-owner-label" className="ae-select" value={worklistFilters.owner} onChange={(event) => setWorklistFilters({ ...worklistFilters, owner: event.target.value })}><option value="all">{t("educator_evaluation.all_steps_20260908", "All steps")}</option><option value="evaluator">{t("educator_evaluation.evaluators_turn_20260908", "Evaluator's turn")}</option><option value="teacher">{t("educator_evaluation.educators_turn_20260908", "Educator's turn")}</option><option value="complete">{t("educator_evaluation.finalized_4cmc2p", "Finalized")}</option></select></label>
          <label className="ae-field"><span id="ae-worklist-due-label">{t("educator_evaluation.cycle_due_mjzagf", "Cycle due")}</span><select aria-labelledby="ae-worklist-due-label" className="ae-select" value={worklistFilters.due} onChange={(event) => setWorklistFilters({ ...worklistFilters, due: event.target.value })}><option value="all">{t("educator_evaluation.all_due_dates_20260908", "All due dates")}</option><option value="overdue">{t("educator_evaluation.past_due_20260908", "Past due")}</option><option value="soon">{t("educator_evaluation.next_14_days_20260908", "Today through 14 days")}</option><option value="month">{t("educator_evaluation.next_15_30_days_20260908", "15 to 30 days")}</option><option value="none">{t("educator_evaluation.open_without_due_date_20260908", "Open without a due date")}</option></select></label>
          <button type="button" className="ae-btn" onClick={() => { setWorklistFilters({ query: '', owner: 'all', due: 'all' }); worklistSearch.current?.focus(); }}>{t("educator_evaluation.clear_worklist_filters_20260908", "Clear filters")}</button>
        </div>
        <p className="ae-help" role="status" aria-live="polite" aria-atomic="true">{matchingTeachers.length} {t("educator_evaluation.of_20260823", "of").trim()} {activeTeachers.length} {t("educator_evaluation.educators_match_20260908", "educators match")}</p>
      </section>}
      {isEvaluator && <section className="ae-card ae-span-12" id="ae-next-actions-list" aria-labelledby="ae-next-actions-title"><div className="ae-record-head"><div><h3 id="ae-next-actions-title">{t("educator_evaluation.needs_your_attention_1ru7wgv", "Needs your attention")}</h3><p className="ae-sub">{t("educator_evaluation.your_next_evaluator_owned_steps_ordered_by_cycle_due_date__o610zt", "Your next evaluator-owned steps, ordered by cycle due date. Open an action to select the educator and go directly to the right workflow.")}</p></div><span className="ae-chip ae-chip-neutral">{evaluatorQueueAll.length > evaluatorQueue.length ? evaluatorQueue.length + t("educator_evaluation.of_20260823", ' of ') + evaluatorQueueAll.length + t("educator_evaluation.shown_20260823", ' shown') : evaluatorQueue.length + t("educator_evaluation.shown_20260823", ' shown')}</span></div>
        {evaluatorQueue.length === 0 ? <div className="ae-empty" style={{ marginTop: 12 }}><strong>{filtersApplied ? t("educator_evaluation.no_matching_evaluator_steps_20260908", "No evaluator steps match these filters.") : t("educator_evaluation.no_evaluator_owned_steps_are_waiting_z6bvc5", "No evaluator-owned steps are waiting.")}</strong><p>{t("educator_evaluation.use_the_roster_below_to_see_educator_owned_steps_and_compl_16a3k8k", "Use the roster below to see educator-owned steps and completed cycles.")}</p></div> : <div className="ae-table-wrap" style={{ marginTop: 12 }}><table className="ae-table"><caption className="ae-live">{t("educator_evaluation.evaluator_next_action_queue_34xcyf", "Evaluator next-action queue")}</caption><thead><tr><th scope="col">{t("educator_evaluation.educator_8c1rq4", "Educator")}</th><th scope="col">{t("educator_evaluation.next_step_1bhqoze", "Next step")}</th><th scope="col">{t("educator_evaluation.why_y9m5zt", "Why")}</th><th scope="col">{t("educator_evaluation.cycle_due_mjzagf", "Cycle due")}</th></tr></thead><tbody>{evaluatorQueue.map(({ teacher, action }) => <tr key={teacher.id}><td><strong>{teacher.name}</strong><br/><span className="ae-sub">{teacher.code} · {teacher.building}</span></td><td><button type="button" className="ae-row-btn" onClick={() => openNextAction(teacher, action)}>{action.label}</button></td><td>{action.detail}</td><td>{aeDate(teacher.dueDate)}</td></tr>)}</tbody></table></div>}
      ...
      {isEvaluator && <section id="ae-roster-status-list" className="ae-card ae-span-12"><div className="ae-record-head"><div><h3>{t("educator_evaluation.roster_status_2shoo8", "Roster status")}</h3><p className="ae-sub">{t("educator_evaluation.select_a_row_to_open_the_educator_s_working_record_av3e05", "Select a row to open the educator’s working record.")}</p></div><button type="button" className="ae-btn" onClick={() => setTab('staff')}>{t("educator_evaluation.manage_staff_1kpuiqg", "Manage staff")}</button></div>
        {matchingTeachers.length === 0 ? <div className="ae-empty">{activeTeachers.length ? t("educator_evaluation.no_matching_educators_20260908", "No educators match. Try another search or clear the filters.") : t("educator_evaluation.no_educators_yet_add_your_roster_in_staff_1pab6mk", "No educators yet. Add your roster in Staff.")}</div> : <div className="ae-table-wrap" style={{ marginTop: 12 }}><table className="ae-table"><thead><tr><th>{t("educator_evaluation.educator_8c1rq4", "Educator")}</th><th>{t("educator_evaluation.next_action_107tu5e", "Next action")}</th><th>{t("educator_evaluation.assignment_10eds7k", "Assignment")}</th><th>{t("educator_evaluation.evaluator_125q2ii", "Evaluator")}</th><th>{t("educator_evaluation.formal_observation_16nrpso", "Formal observation")}</th><th>{t("educator_evaluation.walkthroughs_12hordq", "Walkthroughs")}</th><th>{t("educator_evaluation.spm_slo_18l13ic", "SPM / SLO")}</th><th>{t("educator_evaluation.final_record_1awuyqk", "Final record")}</th><th>{t("educator_evaluation.next_due_wbbyhu", "Next due")}</th></tr></thead><tbody>
          {matchingTeachers.map((teacher) => {
            ...
            return <tr key={teacher.id}><td><button className="ae-row-btn" type="button" onClick={() => setSelectedTeacherId(teacher.id)}>{teacher.name}</button><br/><span className="ae-sub">{teacher.code} · {teacher.building}</span></td><td><button className="ae-row-btn" type="button" onClick={() => openNextAction(teacher, action)}>{action.label}</button><br/><span className="ae-sub">{action.detail}</span></td><td>{teacher.assignment || t("educator_evaluation.no_assignment_16215uj", 'No assignment')}</td><td>{teacher.evaluator || t("educator_evaluation.not_set_20260823", 'Not set')}</td><td>{obs.length ? (obs.some((item) => !item.finalizedAt) ? t("educator_evaluation.in_progress_bgdh5x", 'In progress') : t("educator_evaluation.finalized_4cmc2p", 'Finalized')) : t("educator_evaluation.not_started_1mefwb3", 'Not started')}</td><td>{walks.length}</td><td>{spm ? aeWorkflowStatusLabel(spm.status) : t("educator_evaluation.not_started_1mefwb3", 'Not started')}</td><td><AeStatus status={aeTeacherStatus(teacher)} /></td><td>{aeDate(teacher.dueDate)}</td></tr>;
          })}
        </tbody></table></div>}
      </section>}
...
function AeStaffProfileEditor({ teacher, workspace, role, isRemote, updateStaffProfile }) {
  ...
  return <div role="group" ref={formRef} aria-label="Educator profile editor">
...
function AeWalkthroughs({ initialRecordId = '', workspace, selectedTeacher, setSelectedTeacherId, role, createWalkthrough, updateWalkthroughDraft, discardWalkthroughDraft, publishWalkthrough, addComment, acknowledgeWalkthrough, requestActionReview, addTeacher = null, canAddStaff = false, isRemote = false, readOnlyPreview = false }) {
  ...
  React.useEffect(() => { setDraftReleaseChecked(false); }, [openId, draftReviewSnapshot, role]);
...
function aeSpmRecordsFor(records, teacherId) {
  ...
  return (records || []).filter((record) => record.teacherId === teacherId).slice().sort((left, right) =>
    Number(left.status === 'locked') - Number(right.status === 'locked') || time(right) - time(left) || String(right.id).localeCompare(String(left.id)));
}
...
function AeSpmProgress({ record, cycleFinalized = false }) {
  ...
  return <section className="ae-card" aria-labelledby="ae-spm-progress-title" style={{ marginBottom: 16 }}>
    <div className="ae-record-head"><h3 id="ae-spm-progress-title">{t("educator_evaluation.spm_workflow_20260908", 'SPM / SLO progress')}</h3><span className={'ae-chip ' + (closed ? 'ae-chip-good' : 'ae-chip-blue')}>{owner}</span></div>
    <ol className="ae-stepper ae-spm-stepper" tabIndex={0} aria-label={t("educator_evaluation.spm_workflow_20260908", 'SPM / SLO progress')}>
      {steps.map((label, index) => <li key={index} className={'ae-step ' + (index < stage ? 'ae-step-done' : '') + (index === stage ? ' ae-step-current' : '')} aria-current={index === stage ? 'step' : undefined}>{label}</li>)}
    </ol>
    <p className="ae-sub" role="status" aria-live="polite">{cycleFinalized ? t("educator_evaluation.spm_guide_closed_cycle_20260908", 'The annual cycle is finalized. Existing SPM / SLO records remain available for review.') : (guidance[record.status] || guidance.draft)}</p>
  </section>;
}
...
function AeSpm({ initialRecordId = '', workspace, selectedTeacher, setSelectedTeacherId, role, createSpm, updateSpm, updateTeacher, addComment, readOnlyPreview = false }) {
  ...
  const active = recovery.active;
  React.useEffect(() => { const nextId = savedActive?.id || ''; if (nextId !== openId) setOpenId(nextId); }, [selectedTeacher?.id, savedActive?.id, openId]);
  React.useEffect(() => {
  ...
  return <div className="ae-page"><div className="ae-heading"><div><h2>{t("educator_evaluation.spm_slo_18l13ic", "SPM / SLO")}</h2><p>{AE_ACTIVE_FW.id === 'pa_act13' ? t("educator_evaluation.current_act_13_terminology_is_lea_selected_measure_student_81f9h8", 'Current Act 13 terminology is LEA Selected Measure · Student Performance Measure (SPM); SLO remains a familiar local alias.') : t("educator_evaluation.under_maine_pepg_this_record_holds_the_student_learning_an_k0ry7b", 'Under Maine PEPG this record holds the Student Learning & Growth measure; SPM/SLO remain familiar aliases.')}</p></div>{role === 'teacher' && selectedTeacher && !cycleFinalized && !records.some((record) => record.teacherId === selectedTeacher.id) && <button type="button" className="ae-btn ae-btn-primary" disabled={readOnlyPreview} title={readOnlyPreview ? t("educator_evaluation.preview_only_no_proposal_is_created_10gjfc9", 'Preview only; no proposal is created.') : undefined} onClick={() => { const id = createSpm(selectedTeacher.id); if (id) setOpenId(id); }}>{t("educator_evaluation.start_spm_proposal_9v6z62", "+ Start SPM proposal")}</button>}</div>
    ...
    <div className="ae-toolbar">
      {role === 'evaluator' ? <label className="ae-field" style={{ minWidth: 260, margin: 0 }}><span id="ae-spm-educator-label">{t("educator_evaluation.educator_8c1rq4", "Educator")}</span><select className="ae-select" aria-labelledby="ae-spm-educator-label" value={selectedTeacher ? selectedTeacher.id : ''} onChange={(event) => { setSelectedTeacherId(event.target.value); setOpenId(''); }}><option value="">{t("educator_evaluation.choose_an_educator_1l6d6bg", "Choose an educator")}</option>{teachers.map((teacher) => <option key={teacher.id} value={teacher.id}>{teacher.name} · {teacher.code}</option>)}</select></label> : selectedTeacher && <div className="ae-note">{t("educator_evaluation.viewing_records_for_1hjcyfh", "Viewing records for")} {selectedTeacher.name} · {selectedTeacher.code}</div>}
      {teacherRecords.length > 0 && <label className="ae-field" style={{ minWidth: 260, flex: 1, margin: 0 }}><span id="ae-spm-record-label">{t("educator_evaluation.spm_plan_record_20260908", 'SPM / SLO record')}</span><select className="ae-select" aria-labelledby="ae-spm-record-label" value={savedActive?.id || ''} onChange={(event) => setOpenId(event.target.value)}>{teacherRecords.map((record) => <option key={record.id} value={record.id}>{aeDate(record.createdAt)} · {aeWorkflowStatusLabel(record.status)} · {t("educator_evaluation.version_q0zd4n", 'Version')} {record.version || 1} · {String(record.goal || t("educator_evaluation.untitled_spm_plan_20260908", 'Untitled plan')).slice(0, 70)}</option>)}</select></label>}
    </div>
    {savedActive && <AeSpmProgress record={savedActive} cycleFinalized={cycleFinalized}/>}
    {!active ? <div className="ae-card ae-empty">{selectedTeacher ? (cycleFinalized ? t("educator_evaluation.finalized_cycle_no_spm_20260826", 'This finalized cycle is read-only; annual rollover is required before a new SPM / SLO can begin.') : (role === 'teacher' ? t("educator_evaluation.start_a_proposal_for_the_selected_educator_1xqnpae", 'Start a proposal for the selected educator.') : t("educator_evaluation.no_spm_has_been_submitted_for_this_educator_124nzll", 'No SPM has been submitted for this educator.'))) : t("educator_evaluation.choose_an_educator_x8s9xy", 'Choose an educator.')}</div> : (() => { const teacher = workspace.teachers.find((item) => item.id === active.teacherId); return <div className="ae-grid"><section className="ae-card ae-span-7"><fieldset disabled={cycleFinalized || recovery.changed} style={{ border: 0, padding: 0, margin: 0, minWidth: 0 }}><div className="ae-record-head"><div><h3>{teacher ? teacher.name : t("educator_evaluation.educator_8c1rq4", 'Educator')} {t("educator_evaluation.spm_plan_72fjsn", "· SPM plan")}</h3><p className="ae-sub">{t("educator_evaluation.version_q0zd4n", "Version")} {active.version || 1} {t("educator_evaluation.created_145lrzq", "· created")} {aeDateTime(active.createdAt)}</p></div><span className="ae-chip ae-chip-blue">{active.status.replace(/_/g, ' ')}</span></div>
      ...
      <fieldset disabled={!canEditPlan} style={{ border: 0, padding: 0, margin: '14px 0 0' }}><label className="ae-field"><span id="ae-spm-context-label">{t("educator_evaluation.classroom_context_and_priority_learning_need_1wozrno", "Classroom context and priority learning need")}</span><textarea aria-labelledby="ae-spm-context-label" className="ae-textarea" maxLength={20000} aria-required="true" value={active.context || ''} onChange={(event) => patch({ context: event.target.value }, 'DRAFT_SAVED', 'SPM draft saved')}/></label><label className="ae-field"><span id="ae-spm-baseline-label">{t("educator_evaluation.baseline_1ydg9r6", "Baseline")}</span><textarea aria-labelledby="ae-spm-baseline-label" className="ae-textarea" maxLength={20000} aria-required="true" value={active.baseline || ''} onChange={(event) => patch({ baseline: event.target.value }, 'DRAFT_SAVED', 'SPM draft saved')}/></label><label className="ae-field"><span id="ae-spm-goal-label">{t("educator_evaluation.unit_goal_statement_and_expected_outcomes_ak6a40", "Unit / goal statement and expected outcomes")}</span><textarea aria-labelledby="ae-spm-goal-label" className="ae-textarea" maxLength={20000} aria-required="true" value={active.goal || ''} onChange={(event) => patch({ goal: event.target.value }, 'DRAFT_SAVED', 'SPM draft saved')}/></label><label className="ae-field"><span id="ae-spm-measures-label">{t("educator_evaluation.performance_measures_and_indicators_zr1zqf", "Performance measures and indicators")}</span><textarea aria-labelledby="ae-spm-measures-label" className="ae-textarea" maxLength={20000} aria-required="true" value={active.measures || ''} onChange={(event) => patch({ measures: event.target.value }, 'DRAFT_SAVED', 'SPM draft saved')}/></label><label className="ae-field"><span id="ae-spm-actions-label">{t("educator_evaluation.action_plan_supports_and_evidence_sources_l7keoe", "Action plan, supports, and evidence sources")}</span><textarea aria-labelledby="ae-spm-actions-label" className="ae-textarea" maxLength={20000} aria-required="true" value={active.actionPlan || ''} onChange={(event) => patch({ actionPlan: event.target.value }, 'DRAFT_SAVED', 'SPM draft saved')}/></label></fieldset>
...
function AePrincipalShareSetup({ workspace, updateConfig }) {
  ...
  return <section className="ae-card ae-span-12 ae-setup-path ae-setup-path-primary" id="ae-principal-share-setup">
    ...
    <div role="group" aria-label={t("educator_evaluation.principal_share_helper_setup_checklist_c93h7n", "Principal share helper setup checklist")} style={{ marginTop: 8 }}>
...
function AeSimulationStudio({ workspace, onApply }) {
  ...
  // Tour editor (2026-09-02, form-based): a list of step cards, never JSON a
  // person has to type. Blank steps are named by number instead of failing.
  ...
  const moveStep = (index, delta) => setTourSteps((list) => { const next = list.slice(); const target = index + delta; if (target < 0 || target >= next.length) return list; const held = next[index]; next[index] = next[target]; next[target] = held; return next; });
  ...
  const saveTour = () => {
    ...
    if (blank !== -1) { setMessage(t("educator_evaluation.tour_step_blank_20260902", 'Step {n} needs a title and a sentence before the tour can be saved.').replace('{n}', String(blank + 1))); return; }
    const steps = aeSanitizeTourSteps(tourSteps);
    if (!steps) { setMessage(t("educator_evaluation.tour_invalid_steps_20260902", 'Keep the tour between 1 and 30 steps.')); return; }
    if (!aeWriteTourOverride(steps)) { setMessage(t("educator_evaluation.tour_save_failed_20260902", 'The tour could not be saved in this browser.')); return; }
    setTourCustom(true); setTourSteps(steps.map((step) => ({ ...step })));
    setMessage(t("educator_evaluation.tour_saved_20260902", 'Tour saved on this device. Replay the tour from Practice to see it.'));
  };
  const resetTour = () => { aeWriteTourOverride(null); setTourCustom(false); setTourSteps(AE_GUIDED_TOUR_STEPS.map((step) => ({ ...step }))); setMessage(t("educator_evaluation.tour_reset_20260902", 'Built-in tour restored.')); };
  const saveScenario = () => {
  ...
  const importScenarios = (event) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (tourSteps && aeWriteTourOverride(tourSteps)) { setTourCustom(true); setTourSteps(tourSteps.map((step) => ({ ...step }))); }
  ...
  return <section className="ae-card ae-span-12" id="ae-simulation-studio" tabIndex={-1}><div className="ae-record-head"><div><h3>{t("educator_evaluation.simulation_studio_1dltzgw", "Simulation Studio")}</h3><p className="ae-sub">{t("educator_evaluation.change_fictional_data_with_plain_language_manual_parameter_1kiciun", "Change fictional data with plain language, manual parameters, or both. Parsing runs locally; no prompt or record is sent to an AI service.")}</p></div><span className="ae-chip ae-chip-purple">{t("educator_evaluation.simulation_only_1oe9pdw", "Simulation only")}</span></div>
    ...
    <div className="ae-tour-script" id="ae-tour-script" tabIndex={-1} style={{ marginTop: 14 }} data-help-key="ae_tour_script">
      ...
      <p className="ae-sub">{t("educator_evaluation.tour_script_help_20260902", "The guided tour is a list of steps. Each one opens a tab and shows a title with a sentence or two. Change the wording, add a step about your district's policy, reorder, or trim. Saved on this device and included in scenario exports.")}</p>
      <ol className="ae-tour-steps" data-help-key="ae_tour_steps">{tourSteps.map((step, index) => <li key={index} className="ae-tour-step-card">
        <div className="ae-tour-step-head"><strong>{t("educator_evaluation.step_20260902", "Step")} {index + 1}</strong><div className="ae-actions" style={{ marginTop: 0 }}><button type="button" className="ae-btn ae-btn-small" onClick={() => moveStep(index, -1)} disabled={index === 0} aria-label={t("educator_evaluation.move_step_up_20260902", "Move step") + ' ' + (index + 1) + ' ' + t("educator_evaluation.up_20260902", "up")}>{t("educator_evaluation.move_up_20260902", "Move up")}</button><button type="button" className="ae-btn ae-btn-small" onClick={() => moveStep(index, 1)} disabled={index === tourSteps.length - 1} aria-label={t("educator_evaluation.move_step_down_20260902", "Move step") + ' ' + (index + 1) + ' ' + t("educator_evaluation.down_20260902", "down")}>{t("educator_evaluation.move_down_20260902", "Move down")}</button><button type="button" className="ae-btn ae-btn-small" onClick={() => removeStep(index)} disabled={tourSteps.length <= 1} aria-label={t("educator_evaluation.remove_step_20260902", "Remove step") + ' ' + (index + 1)}>{t("educator_evaluation.remove_20260902", "Remove")}</button></div></div>
        <div className="ae-form-grid">
          <label className="ae-field"><span>{t("educator_evaluation.tab_to_open_20260902", "Tab to open")}</span><select className="ae-select" value={step.tab} onChange={(event) => updateStep(index, 'tab', event.target.value)}>{AE_TOUR_TABS.map((tab) => <option key={tab} value={tab}>{AE_TOUR_TAB_LABELS[tab] || tab}</option>)}</select></label>
          <label className="ae-field"><span>{t("educator_evaluation.title_20260902", "Title")}</span><input className="ae-input" maxLength={120} value={step.title} onChange={(event) => updateStep(index, 'title', event.target.value)} placeholder={t("educator_evaluation.short_title_20260902", "Short title")}/></label>
          <label className="ae-field ae-field-wide"><span>{t("educator_evaluation.what_to_say_20260902", "What to say")}</span><textarea className="ae-textarea" style={{ minHeight: 64 }} maxLength={400} value={step.text} onChange={(event) => updateStep(index, 'text', event.target.value)} placeholder={t("educator_evaluation.one_or_two_sentences_20260902", "One or two sentences the evaluator reads at this step.")}/></label>
        </div>
      </li>)}</ol>
      <div className="ae-actions"><button type="button" className="ae-btn" onClick={addStep} disabled={tourSteps.length >= 30}>{t("educator_evaluation.add_a_step_20260902", "Add a step")}</button><button type="button" className="ae-btn ae-btn-primary" onClick={saveTour}>{t("educator_evaluation.save_tour_20260902", "Save tour")}</button><button type="button" className="ae-btn" onClick={resetTour} disabled={!tourCustom}>{t("educator_evaluation.restore_built_in_tour_20260902", "Restore built-in tour")}</button></div>
    </div>
...
function EducatorEvaluationPanel(props) {
  ...
  const openWorkflowAction = (teacher, action) => {
    ...
    setSelectedTeacherId(teacher.id); setTab(action.tab);
    setTimeout(() => {
      const target = document.getElementById(action.targetId || 'ae-panel');
      if (target && typeof target.focus === 'function') { target.focus({ preventScroll: true }); const scroller = target.closest?.('.ae-main'); if (scroller) scroller.scrollTop += target.getBoundingClientRect().top - scroller.getBoundingClientRect().top - 12; }
    }, 0);
  };
  ...
  if (!isRemote && localRecovery) {
    ...
    const recoveryBody = <div ref={dialogRef} tabIndex={-1} className="ae-workspace" role={standalone ? 'region' : 'dialog'} aria-modal={standalone ? undefined : 'true'} aria-labelledby="ae-recovery-title">
  ...
  if (isRemote && (remoteState.status === 'loading' || (remoteState.status === 'error' && !remoteState.currentUser))) {
    ...
    const gateBody = <div ref={dialogRef} tabIndex={-1} className="ae-workspace" role={standalone ? 'region' : 'dialog'} aria-modal={standalone ? undefined : 'true'} aria-labelledby="ae-title" aria-busy={failed ? undefined : 'true'}>
  ...
  const body = <div ref={dialogRef} tabIndex={-1} className="ae-workspace" role={standalone ? 'region' : 'dialog'} aria-modal={standalone ? undefined : 'true'} aria-labelledby="ae-title">
    <header className="ae-top">
      ...
      <div className="ae-top-actions">{!isRemote && !workspace.educatorPacketMode && <div className="ae-role" role="group" aria-label={workspace.config.sampleMode ? t("educator_evaluation.switch_between_evaluator_and_fictional_educator_rehearsal__mc8xtz", 'Switch between evaluator and fictional educator rehearsal roles') : t("educator_evaluation.view_this_workspace_as_evaluator_or_preview_the_educator_e_llqinz", 'View this workspace as evaluator or preview the educator experience')}><button type="button" aria-pressed={role === 'evaluator'} onClick={() => setRole('evaluator')}>{t("educator_evaluation.evaluator_125q2ii", "Evaluator")}</button><button type="button" aria-pressed={role === 'teacher'} onClick={() => setRole('teacher')}>{workspace.config.sampleMode ? t("educator_evaluation.fictional_educator_2qs1sj", 'Fictional educator') : t("educator_evaluation.educator_preview_jftn00", 'Educator preview')}</button></div>}{!isRemote && !workspace.educatorPacketMode && <div className="ae-practice" ref={practiceRef}>
        <button type="button" className="ae-btn" aria-haspopup="menu" aria-expanded={practiceOpen} aria-controls={practiceOpen ? 'ae-practice-menu' : undefined} data-help-key="ae_practice_menu" onClick={() => setPracticeOpen((value) => !value)}>{t("educator_evaluation.practice_20260902", "Practice")} <span aria-hidden="true">▾</span></button>
...
href="https://alloflow-cdn.pages.dev/educator-evaluation-manual" target="_blank" rel="noopener noreferrer" aria-label={t("educator_evaluation.user_manual_opens_in_a_new_tab_1t3w65", "User manual (opens in a new tab)")} title={t("educator_evaluation.how_to_run_a_full_evaluation_cycle_set_up_the_district_por_162ju8s", "How to run a full evaluation cycle, set up the district portal, and read the released summary")}>{t("educator_evaluation.manual_1wu7r43", "Manual")}</a>{!standalone && <button type="button" className="ae-close" onClick={requestClose} aria-label={t("educator_evaluation.close_educator_growth_and_evaluation_1d9f8pc", "Close Educator Growth and Evaluation")}>×</button>}</div>
    ...
    <main className="ae-main" onClickCapture={blockRemoteMutation} onChangeCapture={blockRemoteMutation} onInputCapture={blockRemoteMutation} onSubmitCapture={blockRemoteMutation}>
      ...
      {tab === 'overview' && <AeOverview openWorkflowAction={openWorkflowAction} requestStaffAdd={requestStaffAdd} workspace={workspace} selectedTeacher={selectedTeacher} setSelectedTeacherId={setSelectedTeacherId} role={role} setRole={setRole} aiReflectionEnabled={aiReflectionEnabled} askForReflection={askForReflection} reflection={visibleReflection} updateTeacher={updateTeacher} setTab={setTab} readOnlyPreview={localTeacherPreview} isRemote={isRemote} requestActionReview={requestActionReview}/>}
      ...
      {tab === 'walkthroughs' && <AeWalkthroughs initialRecordId={requestedWorkflowRecord} workspace={workspace} selectedTeacher={selectedTeacher} setSelectedTeacherId={setSelectedTeacherId} role={role} createWalkthrough={createWalkthrough} updateWalkthroughDraft={updateWalkthroughDraft} discardWalkthroughDraft={discardWalkthroughDraft} publishWalkthrough={publishWalkthrough} addComment={addComment} acknowledgeWalkthrough={acknowledgeWalkthrough} addTeacher={addTeacher} canAddStaff={!isRemote || !!(remoteState.currentUser && remoteState.currentUser.role === 'admin')} isRemote={isRemote} readOnlyPreview={localTeacherPreview} requestActionReview={requestActionReview}/>}
      {tab === 'formal' && <AeFormalObservations initialRecordId={requestedWorkflowRecord} workspace={workspace} selectedTeacher={selectedTeacher} setSelectedTeacherId={setSelectedTeacherId} role={role} setRole={setRole} setTab={setTab} createObservation={createObservation} updateObservation={updateObservation} updateTeacher={updateTeacher} addComment={addComment} readOnlyPreview={localTeacherPreview} fictionalRehearsal={!isRemote && !!workspace.config.sampleMode} requestActionReview={requestActionReview}/>}
      {tab === 'spm' && <AeSpm initialRecordId={requestedWorkflowRecord} workspace={workspace} selectedTeacher={selectedTeacher} setSelectedTeacherId={setSelectedTeacherId} role={role} createSpm={createSpm} updateSpm={updateSpm} updateTeacher={updateTeacher} addComment={addComment} readOnlyPreview={localTeacherPreview} requestActionReview={requestActionReview}/>}
