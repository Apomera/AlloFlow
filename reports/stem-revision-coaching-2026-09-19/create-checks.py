...
old=Path('tests/stem_investigation_refinement.test.js').read_text(encoding='utf-8')
header=old[:old.index("describe('design trial provenance'")]
body=r"""
...
describe('observation journal capacity and recovery',()=>{
 ...
 it('keeps removal undo available when a new save fills the freed slot',async()=>{
  ...
  expect(state().observationNotice).toContain('before restoring');
 });
...
"""
Path('tests/stem_revision_coaching.test.js').write_text(header+body,encoding='utf-8')
# Extend the preceding real-browser workflows, preserving their existing checks.
s=Path('reports/stem-investigation-2026-09-19/browser-qa.cjs').read_text(encoding='utf-8')
s=s.replace("'[data-design-trials]'", "'[data-openbim-workbench]'",1)
s=s.replace("   await button('Save design trial').click();await button('Rotate work area 90°').click();", "   assert((await page.locator('[data-design-constraint=fit]').innerText()).includes('width by 2 m'));\n   await button('Save design trial').click();await button('Rotate work area 90°').click();\n   assert((await page.locator('[data-design-constraint=fit]').innerText()).startsWith('Met:'));",1)
s=s.replace("   assert.equal(roundTrip.designTrials.length,2);assert.equal(roundTrip.status,'proposal');", """   assert.equal(roundTrip.designTrials.length,2);assert.equal(roundTrip.status,'proposal');
...
""",1)
s=s.replace("   await button('Review my evidence').click();await button('Save observation revision').click();", """   await page.locator('#oid-observation-note').fill('Six legs support the teaching group; a species would require more evidence.');
   ...
   assert((await page.locator('[data-observation-comparison]').innerText()).includes('Before: Six jointed legs'));
""",1)
s=s.replace("assert(text.includes('Next evidence to seek:'));", "assert(text.includes('Entry ID:'));assert(text.includes('Revision comparison:'));assert(text.includes('Next evidence to seek:'));",1)
s=s.replace("  await page.locator(selector).scrollIntoViewIfNeeded();\n  await page.screenshot", "  await page.locator(id==='openBim'?'[data-design-constraints]':'[data-observation-comparison]').scrollIntoViewIfNeeded();\n  await page.screenshot",1)
s=s.replace("await page.locator(selector).scrollIntoViewIfNeeded();\n   if(width", "await page.locator(id==='openBim'?'[data-design-constraints]':'[data-observation-comparison]').scrollIntoViewIfNeeded();\n   if(width",1)
Path('reports/stem-revision-coaching-2026-09-19/browser-qa.cjs').write_text(s,encoding='utf-8')
print('Added 15 targeted regression tests and extended browser workflows.')
