...
(function () {
  ...
  (function() {
    ...
    uiStyle.textContent = [
      ...
      "@media(pointer:coarse){.gw-shape-heading,.gw-shape-rotate{min-height:44px!important;min-width:44px!important}.gw-pointer-hint{display:none}}@media(max-width:520px){.gw-shape-heading .gw-shape-heading-text{display:none}.gw-placement-hint .gw-placement-aim{margin-left:0;margin-top:6px}}",
      "@media(max-width:800px){.gw-placement-hint{top:128px;bottom:auto;left:12px;transform:none;max-width:calc(100% - 174px);padding:7px 8px;font-size:11px;align-items:flex-start}.gw-root:has(.gwe-builder-dock[data-collapsed=\"false\"]) .gw-placement-hint{display:none}}@media(max-height:520px) and (orientation:landscape){.gw-root .gw-touch-actions{width:calc(100% - 170px);max-width:calc(100% - 170px);flex-direction:row!important;flex-wrap:wrap;justify-content:flex-end;bottom:132px!important}.gw-root .gw-touch-actions button{width:58px!important;min-width:58px!important}.gw-placement-hint{top:112px;bottom:auto;left:50%;transform:translateX(-50%);max-width:min(460px,calc(100% - 40px));padding:5px 8px;font-size:11px}.gw-placement-hint .gw-placement-aim{display:none}.gw-root:has(.gwe-builder-dock[data-collapsed=\"false\"]) .gw-placement-hint{display:none}}",
      /* Short touch landscapes */
      "@media(max-height:520px) and (orientation:landscape){#geoworld-fs-workspace[data-touch-active=\"true\"] .gw-touch-look-panel{top:82px!important;left:12px!important;right:auto!important;background:#112d2bf2!important;border-color:#aec9b647!important}#geoworld-fs-workspace[data-touch-active=\"true\"] .gw-touch-look-panel input{accent-color:#b1d3a6}#geoworld-fs-workspace[data-touch-active=\"true\"] .gw-touch-look-panel>.gw-touch-look-label{color:#e1eddc!important}#geoworld-fs-workspace[data-touch-active=\"true\"] .gw-touch-mode-hint{display:none}#geoworld-fs-workspace[data-touch-active=\"true\"] .gw-touch-look-zone{display:none}#geoworld-fs-workspace[data-touch-active=\"true\"] .gw-coordinate-hud{display:none!important}#geoworld-fs-workspace[data-touch-active=\"true\"][data-geometry-mode] .gw-action-bar.gw-action-bar{left:12px!important;right:auto!important;bottom:132px!important;width:max-content!important;max-width:calc(100% - 420px)!important;transform:none!important;flex-wrap:nowrap!important;justify-content:flex-start!important;overflow-x:auto;gap:3px!important}#geoworld-fs-workspace[data-touch-active=\"true\"] .gw-action-bar button{min-height:44px;flex-shrink:0}#geoworld-fs-workspace[data-touch-active=\"true\"] .gw-touch-joystick{bottom:16px!important;left:12px!important}#geoworld-fs-workspace[data-touch-active=\"true\"] .gw-hotbar{left:auto!important;right:12px!important;max-width:calc(100% - 150px)!important;transform:none!important}#geoworld-fs-workspace[data-touch-active=\"true\"] .gw-placement-hint{left:50%;transform:translateX(-50%);top:112px;bottom:auto;max-width:min(460px,calc(100% - 40px))}#geoworld-fs-workspace[data-touch-active=\"true\"] .gw-placement-hint .gw-placement-aim{display:none}}",
  ...
  window.StemLab.registerTool('geometryWorld', {
    ...
    render: function (ctx) {
      ...
      // Pointer-lock state for the on-screen hint. Runtime only: it is React
      ...
      function initEngine(container) {
        ...
        // Pitch the view to the nearby ground ahead so a first placement has
        // something to land on. Camera only: it never places, removes or moves a
        // block, and it syncs the look euler so the next mouse move continues from
        // here instead of snapping back.
        engine.aimAtBuildArea = function() {
          var THREE = window.THREE, cam = engine.camera;
          if (!THREE || !cam) return false;
          var fwd = cam.getWorldDirection(new THREE.Vector3());
        ...
        engine.placementForHit = function(hit) {
          ...
          return cell ? engine.getPlacementEligibility(cell.x,cell.y,cell.z) : { allowed:false, code:'no_target', reason:'Look down at the nearby ground or a block face, then press B or tap Place.', cell:null };
        };
        engine.publishPlacementPreview = function(preview) {
          ...
          // The no_target case used to be suppressed outright, so a student aiming
          // at the sky on their first visit saw nothing at all. It is now shown
          // while they have yet to place a block, with an explicit aim action; once
          // they have built something it goes quiet again, and the B / Place path
          // still explains itself on demand through interactAtCrosshair.
          var firstBlockPending = !(engine.blocksPlaced > 0);
          var showNoTarget = preview.code === 'no_target' && firstBlockPending && !engine._placingLessonBlocks;
          var hint = (preview.code !== 'no_target' || showNoTarget) && !engine._showcase ? { allowed:preview.allowed, code:preview.code, reason:preview.reason } : null;
          if (hint && preview.code === 'no_target') hint.aim = true;
          var signature = hint ? hint.allowed + '|' + hint.code + '|' + hint.reason + '|' + (hint.aim ? 'aim' : '') : '';
        ...
        document.addEventListener('pointerlockchange', _docH.pointerlockchange = function() {
          ...
          try { if (engine._setPointerLocked) engine._setPointerLocked(engine.isLocked); } catch (e) {}
      ...
      return el('div', { id: 'geoworld-fs-workspace', role: 'region', 'data-geometry-fullscreen-workspace': 'true', 'data-fullscreen': isWorkspaceFullscreen ? 'true' : 'false', 'data-toolbar-collapsed': toolbarCollapsed ? 'true' : 'false', 'data-hud-preset': hudPreset, 'data-measurement-expanded': isMobile && !!measureResult && measurementDetailsOpen ? 'true' : 'false', 'data-touch-mode': touchMode ? 'touch' : 'desktop', 'data-touch-active': isMobile && touchMode ? 'true' : 'false', className: 'gw-root', 'aria-label': __alloT('stem.geometryworld.tool_name', 'Geometry World'), style: { display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', background: 'var(--allo-stem-canvas, #000)' } },
        ...
        el('div', { className: 'gw-shape-tray', style: { position: 'absolute', bottom: '54px', left: '50%', transform: 'translateX(-50%)', zIndex: 20, display: 'flex', gap: '3px', background: 'rgba(0,0,0,0.65)', borderRadius: '10px', padding: '3px 5px', alignItems: 'center', backdropFilter: 'blur(6px)', border: '1px solid rgba(255,255,255,0.06)' } },
          ...
          }, el('span',{className:'gw-shape-rotate-icon','aria-hidden':'true'},'\u21BB'), el('span',null,(blockRotation*90)+'\u00b0'), el('kbd', { className: 'gw-key-badge', 'aria-hidden': 'true' }, 'R')),
        ...
        // Pointer-lock hint: a small chip in the top-left corner, never over the
        // crosshair or the placement preview. Captured: how to get the cursor back.
        // Free: how to start looking around. Touch layouts hide it (they have their
        // own look controls) and so does any open overlay.
        worldActive && !isMobile && openModals.length === 0 && !d.showcaseActive && el('div', {
          className: 'gw-pointer-hint', 'data-locked': pointerLocked ? 'true' : 'false', role: 'status', 'aria-live': 'polite', 'aria-atomic': 'true'
        }, pointerLocked
