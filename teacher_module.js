...
var ALLO_TEACHER_READING_THEME_IDS = ALLO_TEACHER_READING_THEME_OPTIONS.map(function(option) {
  return option.id;
});
...
function alloTeacherBoundedRosterNumber(value, max) {
  ...
  return Number.isFinite(parsed) ? Math.max(0, Math.min(max == null ? 1e5 : max, Math.floor(parsed))) : 0;
}
...
function alloTeacherRosterValueSet(value) {
  ...
  Object.keys(value && typeof value === "object" && !Array.isArray(value) ? value : {}).forEach(function(key) {
    out[key] = 1;
  });
  return out;
}
...
function alloTeacherSanitizeOrganizerEvidence(value) {
  ...
  return {
    ...
    score: alloTeacherBoundedRosterNumber(value.score, 1e5),
    correct: alloTeacherBoundedRosterNumber(value.correct, 1e5),
    total: alloTeacherBoundedRosterNumber(value.total, 1e5),
    attempts: alloTeacherBoundedRosterNumber(value.attempts, 1e4)
  };
}
  ...
  Object.keys(students || {}).forEach(function(codename) {
    ...
    var safeEntries = entries.map(function(entry) {
      ...
      var safe = {
        ...
        responseCount: alloTeacherBoundedRosterNumber(entry.responseCount, 1e4),
        resourcesOpened: alloTeacherBoundedRosterNumber(entry.resourcesOpened, 1e3),
        liveActivityCount: alloTeacherBoundedRosterNumber(entry.liveActivityCount, 1e3),
        liveSubmissionCount: alloTeacherBoundedRosterNumber(entry.liveSubmissionCount, 1e3),
        liveRevisionCount: alloTeacherBoundedRosterNumber(entry.liveRevisionCount, 1e3)
      };
...
function alloTeacherSanitizeRosterInsightBrief(value, studentSet, groupSet) {
  ...
  var safe = {
    ...
    submissions: alloTeacherBoundedRosterNumber(value.submissions, 1e5),
    revisions: alloTeacherBoundedRosterNumber(value.revisions, 1e5),
    feedbackSent: alloTeacherBoundedRosterNumber(value.feedbackSent, 1e5),
    votesCast: alloTeacherBoundedRosterNumber(value.votesCast, 1e5),
  ...
  safe.byKind = (Array.isArray(value.byKind) ? value.byKind : []).map(function(item) {
    ...
    return {
      ...
      invited: alloTeacherBoundedRosterNumber(item.invited, 1e5),
      submitted: alloTeacherBoundedRosterNumber(item.submitted, 1e5),
      revised: alloTeacherBoundedRosterNumber(item.revised, 1e5),
  ...
  safe.groups = (Array.isArray(value.groups) ? value.groups : []).map(function(item) {
    ...
    return {
      ...
      activityOpportunities: alloTeacherBoundedRosterNumber(item.activityOpportunities, 1e5),
      submissions: alloTeacherBoundedRosterNumber(item.submissions, 1e5),
      revisions: alloTeacherBoundedRosterNumber(item.revisions, 1e5),
  ...
  safe.nextMoves = (Array.isArray(value.nextMoves) ? value.nextMoves : []).map(function(move) {
    ...
    return code && label ? { code, count: alloTeacherBoundedRosterNumber(move.count, 1e5), label } : null;
...
function alloTeacherSanitizeRosterSessionHistory(value, students, groups) {
  ...
  return (Array.isArray(value) ? value : []).map(function(session) {
    ...
    if (!id) return null;
    var participants = /* @__PURE__ */ Object.create(null);
    ...
    Object.keys(rawParticipants).forEach(function(codename) {
      ...
      var safeRecord = {
        ...
        responseCount: alloTeacherBoundedRosterNumber(record.responseCount, 1e4),
        resourcesOpened: alloTeacherBoundedRosterNumber(record.resourcesOpened, 1e3),
        liveActivityCount: alloTeacherBoundedRosterNumber(record.liveActivityCount, 1e3),
        liveSubmissionCount: alloTeacherBoundedRosterNumber(record.liveSubmissionCount, 1e3),
        liveRevisionCount: alloTeacherBoundedRosterNumber(record.liveRevisionCount, 1e3)
      };
    ...
    var safe = {
      ...
      durationMinutes: session.durationMinutes === null ? null : alloTeacherBoundedRosterNumber(session.durationMinutes, 1e5),
      mode: session.mode === "mailbox" ? "mailbox" : "firebase",
      resourceTitles: (Array.isArray(session.resourceTitles) ? session.resourceTitles : []).map(function(title) {
        return alloTeacherBoundedRosterText(title, 160);
    ...
    safe.liveActivities = (Array.isArray(session.liveActivities) ? session.liveActivities : []).map(function(activity) {
      ...
      if (!kind) return null;
      var out = { kind, phase: alloTeacherBoundedRosterText(activity.phase, 20) || "closed" };
      ["invited", "submitted", "revised", "approved", "hidden", "revealed", "feedbackSent", "guesses", "showcased", "votesCast"].forEach(function(field) {
        out[field] = alloTeacherBoundedRosterNumber(activity[field], 1e5);
      });
...
function alloTeacherSanitizePortableSeating(value, students) {
  ...
  var layoutsSource = value.layouts && typeof value.layouts === "object" && !Array.isArray(value.layouts) ? value.layouts : {};
  var layouts = {};
  var layoutIdsTaken = /* @__PURE__ */ Object.create(null);
  Object.keys(layoutsSource).forEach(function(layoutKey) {
    ...
    var objectIdsTaken = /* @__PURE__ */ Object.create(null);
    var seats = [];
    ...
    var seatSet = /* @__PURE__ */ Object.create(null);
    seats.forEach(function(seat) {
      seatSet[seat.id] = 1;
    });
    var assignments = {};
    var assignedStudents = /* @__PURE__ */ Object.create(null);
    var rawAssignments = rawLayout.assignments && typeof rawLayout.assignments === "object" && !Array.isArray(rawLayout.assignments) ? rawLayout.assignments : {};
  ...
  var activeSeatSet = /* @__PURE__ */ Object.create(null);
  if (activeLayoutId) layouts[activeLayoutId].seats.forEach(function(seat) {
    activeSeatSet[seat.id] = 1;
  });
  var constraintSizes = {
    keep_apart: 2,
    keep_together: 2,
  ...
  var constraintIdsTaken = /* @__PURE__ */ Object.create(null);
  var constraints = [];
  (Array.isArray(value.constraints) ? value.constraints : []).forEach(function(rawConstraint) {
    ...
    var seenStudents = /* @__PURE__ */ Object.create(null);
    ...
    var safeConstraint = { id, type: rawConstraint.type, students: codenames };
  ...
  var history = (Array.isArray(value.history) ? value.history : []).filter(function(entry) {
    return entry && typeof entry === "object" && !Array.isArray(entry) && typeof entry.at === "string" && !Number.isNaN(Date.parse(entry.at.slice(0, 48)));
  }).map(function(entry) {
    return {
  ...
  return {
    ...
    layouts,
    constraints,
    solveSeed: clamp(value.solveSeed, 1, 1e9, 1),
    history
  };
}
function alloTeacherSanitizeSubmissionKey(value, classId) {
  ...
  var keyOps = (Array.isArray(jwk.key_ops) ? jwk.key_ops : []).filter(function(op) {
    return op === "encrypt";
  }).slice(0, 1);
  if (keyOps.length) publicJwk.key_ops = keyOps;
  var safe = { publicJwk, classId: classId || keyClassId };
...
function alloEnsureTeacherRosterIdentity(value) {
  ...
  var groups = value.groups && typeof value.groups === "object" && !Array.isArray(value.groups) ? value.groups : {};
  ...
  var currentPreferences = value.learnerPreferences && typeof value.learnerPreferences === "object" && !Array.isArray(value.learnerPreferences) ? value.learnerPreferences : {};
  var learnerIds = /* @__PURE__ */ Object.create(null);
  var learnerPreferences = /* @__PURE__ */ Object.create(null);
  var usedLearnerIds = /* @__PURE__ */ Object.create(null);
  var readingThemeDefault = alloNormalizeTeacherReadingTheme(value.readingThemeDefault, "default");
  var currentProgressHistory = value.progressHistory && typeof value.progressHistory === "object" && !Array.isArray(value.progressHistory) ? value.progressHistory : {};
  ...
  var hasLegacyIdentityMap = Object.prototype.hasOwnProperty.call(value, "displayNames") || Object.prototype.hasOwnProperty.call(value, "importAliases");
  var changed = classId !== value.classId || readingThemeDefault !== value.readingThemeDefault || hasLegacyIdentityMap || progressChanged || sessionsChanged || seatingChanged || submissionKeyChanged;
  Object.keys(students).forEach(function(codename) {
    ...
    var preferenceSource = existing && Object.prototype.hasOwnProperty.call(currentPreferences, existing) ? currentPreferences[existing] : currentPreferences[codename];
...
function alloRemoveTeacherStudentReferences(seatingValue, codename) {
  ...
  var changed = false;
  var layoutsSource = seatingValue.layouts && typeof seatingValue.layouts === "object" && !Array.isArray(seatingValue.layouts) ? seatingValue.layouts : {};
  var layouts = {};
  Object.keys(layoutsSource).forEach(function(layoutId) {
    ...
    var assignmentsSource = layout.assignments && typeof layout.assignments === "object" && !Array.isArray(layout.assignments) ? layout.assignments : {};
    var assignments = {};
    ...
    layouts[layoutId] = Object.keys(assignments).length === Object.keys(assignmentsSource).length ? layout : Object.assign({}, layout, { assignments });
  });
  var constraints = Array.isArray(seatingValue.constraints) ? seatingValue.constraints.filter(function(constraint) {
  ...
  return changed ? Object.assign({}, seatingValue, { layouts, constraints }) : seatingValue;
}
function alloRemoveTeacherGroupReferences(rosterValue, groupId) {
  ...
  var classGoals = Array.isArray(rosterValue.classGoals) ? rosterValue.classGoals.filter(function(goal) {
    return !(goal && goal.team === teamKey);
  }) : rosterValue.classGoals;
  var next = Object.assign({}, rosterValue, { groups, students });
  if (Array.isArray(rosterValue.classGoals)) next.classGoals = classGoals;
  return next;
}
const ALLO_ROSTER_RESERVED_KEYS = /* @__PURE__ */ new Set(["__proto__", "prototype", "constructor"]);
...
const alloNormalizeTeacherRosterImport = (data) => {
  ...
  groupEntries.forEach(([rawId, rawGroup]) => {
    ...
    ["dyslexiaFriendly", "useOpenDyslexic", "karaokeMode"].forEach((field) => {
      if (typeof rawProfile[field] === "boolean") profile[field] = rawProfile[field];
    });
  ...
  const importedClassId = alloTeacherSafeLearnerId(data.classId) || alloTeacherSafeLearnerId(data.submissionKey?.classId);
  const classGoals = (Array.isArray(data.classGoals) ? data.classGoals : []).map((goal) => {
    ...
    const tracked = alloRosterPlainRecord(goal.tracked) && (goal.tracked.metric === "xp_total" || goal.tracked.metric === "responded_each") ? { metric: goal.tracked.metric, threshold: Math.max(1, Math.min(1e6, Math.floor(Number(goal.tracked.threshold) || 1))) } : null;
    return {
      id: goal.id,
      label,
      ...
      metCount: alloTeacherBoundedRosterNumber(goal.metCount, 1e5),
  ...
  const normalizedRoster = alloEnsureTeacherRosterIdentity({
    className: typeof data.className === "string" ? data.className.slice(0, 120) : "",
    ...
    ...classGoals.length ? { classGoals } : {},
    ...classGoalLog.length ? { classGoalLog } : {}
  });
...
const alloTeacherRosterUpdatePlans = /* @__PURE__ */ new WeakMap();
let alloTeacherRosterUpdateSequence = 0;
const alloTeacherRosterUpdateSnapshot = (value) => {
  const ancestors = /* @__PURE__ */ new Set();
  let count = 0;
  const canonical = (item, depth) => {
    if (++count > 15e4 || depth > 80) throw new Error("This roster is too complex to update safely.");
...
const alloTeacherRosterUpdateIndex = (roster) => {
  if (!alloRosterPlainRecord(roster) || !alloRosterPlainRecord(roster.students) || !alloRosterPlainRecord(roster.groups) || !alloRosterPlainRecord(roster.learnerIds) || !roster.classId || alloTeacherSafeLearnerId(roster.classId) !== roster.classId) {
  ...
  const byCodename = /* @__PURE__ */ new Map();
...
const alloPlanTeacherRosterUpdate = (currentRoster, incomingRoster) => {
  ...
  const allowedFields = /* @__PURE__ */ new Set(["className", "classId", "groups", "students", "learnerIds", "learnerPreferences", "readingThemeDefault", "progressHistory", "sessionHistory", "seating", "classGoals", "classGoalLog", "submissionKey", "exportVersion", "exportDate"]);
  ...
  const prohibitedFields = /* @__PURE__ */ new Set(["displaynames", "importaliases", "identitymap", "mappings", "googleid", "sourceid", "userid", "courseid", "email", "emailaddress", "fullname", "givenname", "familyname", "accesstoken", "refreshtoken", "privatejwk"]);
  ...
  current.byId.forEach((entry) => {
    if (!incoming.byId.has(entry.learnerId)) retainedAbsences.push(entry);
  });
  ...
  const preview = Object.freeze({
    ...
    additions: freezeEntries(additions),
    retainedAbsences: freezeEntries(retainedAbsences),
    conflicts: freezeEntries(conflicts),
    matches: freezeEntries(matches),
    matchedCount,
...
const alloCancelTeacherRosterUpdate = (preview) => {
  if (preview) alloTeacherRosterUpdatePlans.delete(preview);
};
...
const buildRosterCodenameWorksheetHtml = (roster, options) => {
  ...
  const entries = Object.entries(students).map(([codename, groupId]) => ({
    ...
    if (settings.sortBy === "group") {
      const groupOrder = (a.groupName || "Unassigned").localeCompare(b.groupName || "Unassigned", void 0, { numeric: true, sensitivity: "base" });
      if (groupOrder) return groupOrder;
    }
    return a.codename.localeCompare(b.codename, void 0, { numeric: true, sensitivity: "base" });
  });
  const locationHeader = '<th scope="col">Location / seat (write in)</th>';
  const groupHeader = settings.sortBy === "group" ? '<th scope="col">Group</th>' : "";
  const headers = '<th scope="col">#</th><th scope="col">Codename</th>' + groupHeader + (settings.locationPosition === "before-name" ? locationHeader : "") + '<th scope="col">Student name (write in)</th>' + (settings.locationPosition === "after-name" ? locationHeader : "");
...
const buildRosterSessionEvidenceCsv = (session) => {
  ...
  Object.entries(source.participants && typeof source.participants === "object" ? source.participants : {}).sort(([a], [b]) => a.localeCompare(b)).forEach(([codename, rawRecord]) => {
    ...
    rows.push([
      ...
      record.organizer?.type || "",
      record.organizer?.status || "",
      record.organizer?.score || 0,
      record.organizer?.correct || 0,
      record.organizer?.total || 0,
      record.organizer?.attempts || 0,
      "",
      "",
      "",
      "",
...
const ROSTER_SESSION_FOLLOW_UP_FILTERS = /* @__PURE__ */ new Set(["all", "follow_up", "revision", "celebrate", "planned"]);
const normalizeRosterSessionTeacherNote = (value) => String(value || "").replace(/\u000d\u000a?/g, "\n").replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/g, "").trim().slice(0, 1e3);
...
const updateRosterSessionFollowUp = (rosterKey, sessionId, patch) => {
  const sourceRoster = rosterKey && typeof rosterKey === "object" && !Array.isArray(rosterKey) ? rosterKey : { groups: {}, students: {} };
...
const RosterKeyPanel = React.memo(({ isOpen, onClose, rosterKey, setRosterKey, onApplyGroup, onSyncToSession, onBatchGenerate, activeSessionCode, t, isParentMode, isIndependentMode, onOpenSubmissionInbox, onOpenSeatingChart, followUpResources = [], onOpenFollowUpResource, onPrepareFollowUpAssignment, onSendFollowUpToLiveSession, batchOpenRequest = 0, rosterStorageError = "" }) => {
  ...
  const safeFollowUpResources = useMemo(() => (Array.isArray(followUpResources) ? followUpResources : []).filter((item) => item && item.id && item.type).slice(0, 250), [followUpResources]);
  const sessionActivityKinds = useMemo(() => Array.from(new Set(
    (Array.isArray(rosterKey?.sessionHistory) ? rosterKey.sessionHistory : []).flatMap((session) => (Array.isArray(session?.liveActivities) ? session.liveActivities : []).map((activity) => String(activity?.kind || "").trim()).concat(String(session?.organizerActivity?.type || "").trim())).filter(Boolean)
  ...
  const sortCodenames = (names) => names.sort((a, b) => a.localeCompare(b, void 0, { numeric: true, sensitivity: "base" }));
  ...
  const visibleGroupIds = normalizedStudentQuery ? groupIds.filter((gId) => getStudentsInGroup(gId).some(matchesStudentQuery)) : groupIds;
  ...
  const noteDraftFor = (session) => Object.prototype.hasOwnProperty.call(sessionNoteDrafts, session.id) ? sessionNoteDrafts[session.id] : normalizeRosterSessionTeacherNote(session.teacherNote);
  ...
  const confirmRosterUpdate = () => {
    ...
    try {
      ...
      setRosterKey((current) => {
        try {
          alloApplyTeacherRosterUpdate(preview, current);
          return candidate;
        } catch (_) {
          return current;
        }
  ...
  const handleExport = () => {
    ...
    const exportData = {
      className: safeRoster.className || "",
      classId: safeRoster.classId || "",
      groups: safeRoster.groups || {},
      students: safeRoster.students || {},
      learnerIds: safeRoster.learnerIds || {},
      learnerPreferences: safeRoster.learnerPreferences || {},
      readingThemeDefault: safeRoster.readingThemeDefault || "default",
      progressHistory: safeRoster.progressHistory || {},
      sessionHistory: safeRoster.sessionHistory || [],
      ...safeRoster.submissionKey?.publicJwk ? { submissionKey: safeRoster.submissionKey } : {},
      ...safeRoster.seating ? { seating: safeRoster.seating } : {},
      ...Array.isArray(safeRoster.classGoals) ? { classGoals: safeRoster.classGoals } : {},
      ...Array.isArray(safeRoster.classGoalLog) ? { classGoalLog: safeRoster.classGoalLog } : {},
      exportVersion: 4,
  ...
  const handleStoreRosterExport = () => {
    ...
    try {
      ...
      link.click();
      timer = window.setTimeout(cleanup, 1e3);
  ...
  const handlePrintRosterWorksheet = (event) => {
    ...
    iframe.addEventListener("load", () => {
      try {
        ...
        timer = window.setTimeout(cleanup, 6e4);
  ...
  const handleUpdateGroupProfile = (gId, field, value) => {
    const safeValue = typeof value === "string" ? value.slice(0, field === "leveledTextCustomInstructions" ? 500 : 120) : value;
    setRosterKey((prev) => ({
      ...prev,
  ...
  const handleRunBatchGenerate = async () => {
    ...
    if (!selected.length) {
      setBatchStatus("Select at least one output type.");
      return;
    }
    setBatchStatus("");
    try {
  ...
  const updateLearnerReadingPreference = (name, updater) => {
    setRosterKey((previous) => {
      ...
      const next = alloNormalizeTeacherLearnerPreference({ ...candidate, updatedAt: (/* @__PURE__ */ new Date()).toISOString() });
  ...
  const handleToggleLearnerReadingFavorite = (name, readingTheme) => {
    updateLearnerReadingPreference(name, (current) => {
      ...
      return {
        ...current,
        favoriteReadingThemes: favorites.includes(readingTheme) ? favorites.filter((themeId) => themeId !== readingTheme) : favorites.concat(readingTheme)
      };
    });
  };
  ...
  const ProfileField = ({ label, value, field, gId, type = "text", options }) => /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2 text-xs" }, /* @__PURE__ */ React.createElement("span", { className: "font-bold text-slate-600 w-24 shrink-0" }, label, ":"), type === "select" ? /* @__PURE__ */ React.createElement(
    ...
    options.map((o) => {
      ...
      return /* @__PURE__ */ React.createElement("option", { key: optionValue, value: optionValue }, optionLabel);
    })
    ...
    "input",
    {
      type: "text",
      value: value || "",
      maxLength: field === "leveledTextCustomInstructions" ? 500 : 120,
      onChange: (e) => handleUpdateGroupProfile(gId, field, e.target.value),
      "aria-label": label,
      placeholder: "\u2014",
      className: "flex-1 px-2 py-1 rounded-lg border border-slate-400 text-slate-700 text-xs focus:ring-2 focus:ring-indigo-400 focus:outline-none"
  ...
  return /* @__PURE__ */ React.createElement("div", { ref: panelRef, role: "dialog", "aria-modal": "true", "aria-labelledby": "teacher-roster-panel-title", className: "fixed inset-0 z-[260] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-0 sm:p-4 animate-in motion-reduce:animate-none fade-in duration-200" }, /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-none sm:rounded-2xl shadow-2xl max-w-2xl w-full h-[100dvh] sm:h-auto sm:max-h-[85vh] flex flex-col border-0 sm:border-2 border-indigo-100 animate-in motion-reduce:animate-none zoom-in-95 duration-200 min-w-0" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-start justify-between gap-3 p-4 sm:p-5 border-b border-slate-100" }, /* @__PURE__ */ React.createElement("div", { "data-help-key": "roster_panel_header" }, /* @__PURE__ */ React.createElement("h2", { id: "teacher-roster-panel-title", className: "text-lg font-black text-slate-800 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(ClipboardList, { size: 20, className: "text-indigo-500" }), " ", isParentMode ? "Family Learning Profiles" : isIndependentMode ? "My Learning Profile" : t("roster.title") || "Class Roster & Progress Tracking"), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-600 mt-0.5" }, isParentMode ? "Manage family member profiles and track learning progress" : isIndependentMode ? "Manage your learning profile and track your progress" : t("roster.subtitle") || "Organize student groups with differentiated profiles for instruction")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: onClose, className: "p-2 rounded-full hover:bg-slate-100 transition-colors motion-reduce:transition-none", "aria-label": t("common.close") }, /* @__PURE__ */ React.createElement(X, { size: 20, className: "text-slate-600" }))), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2 px-4 sm:px-5 py-3 border-b border-slate-50 bg-slate-50/50" }, /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => rosterUpdateFileRef.current?.click(), disabled: !rosterKey?.classId, className: "px-3 py-1.5 bg-indigo-700 text-white rounded-lg text-xs font-bold hover:bg-indigo-800 transition-colors motion-reduce:transition-none flex items-center gap-1.5 disabled:opacity-40" }, /* @__PURE__ */ React.createElement(Upload, { size: 14 }), " Update roster safely"), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => fileInputRef.current?.click(), className: "px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg text-xs font-bold hover:bg-indigo-100 transition-colors motion-reduce:transition-none flex items-center gap-1.5" }, /* @__PURE__ */ React.createElement(Upload, { size: 14 }), " Import / replace roster"), !isParentMode && !isIndependentMode && /* @__PURE__ */ React.createElement("a", { href: ALLO_TEACHER_CLASSROOM_IMPORT_URL, target: "_blank", rel: "noopener noreferrer", className: "px-3 py-1.5 bg-blue-50 text-blue-800 rounded-lg text-xs font-bold hover:bg-blue-100 transition-colors motion-reduce:transition-none" }, "Google Classroom setup"), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: handleExport, disabled: !rosterKey, className: "px-3 py-1.5 bg-green-50 text-green-700 rounded-lg text-xs font-bold hover:bg-green-100 transition-colors motion-reduce:transition-none flex items-center gap-1.5 disabled:opacity-40" }, /* @__PURE__ */ React.createElement(Download, { size: 14 }), " ", t("roster.export") || "Export JSON"), !isParentMode && !isIndependentMode && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: handleStoreRosterExport, disabled: !rosterKey?.classId || !Object.keys(rosterKey?.students || {}).length, "aria-describedby": "roster-store-export-note", className: "px-3 py-1.5 min-h-[44px] bg-purple-50 text-purple-800 rounded-lg text-xs font-bold hover:bg-purple-100 transition-colors motion-reduce:transition-none flex items-center gap-1.5 disabled:opacity-40" }, /* @__PURE__ */ React.createElement(Download, { size: 14 }), " Store review file"), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: handlePrintRosterWorksheet, disabled: !Object.keys(rosterKey?.students || {}).length, className: "px-3 py-1.5 bg-cyan-50 text-cyan-800 rounded-lg text-xs font-bold hover:bg-cyan-100 transition-colors motion-reduce:transition-none flex items-center gap-1.5 disabled:opacity-40" }, /* @__PURE__ */ React.createElement(Printer, { size: 14 }), " Print worksheet"), !isParentMode && !isIndependentMode && /* @__PURE__ */ React.createElement("p", { id: "roster-store-export-note", className: "w-full text-xs text-slate-600" }, "Store review shares only class/learner IDs and codenames. An authorized Store administrator must review each match; no learning records or points are transferred."), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setShowPrintOptions((value) => !value), "aria-expanded": showPrintOptions, "aria-controls": "roster-print-options", className: "px-3 py-1.5 rounded-lg border border-cyan-200 bg-white text-xs font-bold text-cyan-900 hover:bg-cyan-50 transition-colors motion-reduce:transition-none" }, "Worksheet options"), /* @__PURE__ */ React.createElement(
  ...
  )), /* @__PURE__ */ React.createElement("label", { className: "flex items-center gap-2 text-xs font-bold text-slate-600" }, /* @__PURE__ */ React.createElement("span", { className: "shrink-0" }, "Reading default:"), /* @__PURE__ */ React.createElement(
    "select",
    {
    ...
    return /* @__PURE__ */ React.createElement("div", { key: gId, className: "border border-slate-400 rounded-xl overflow-hidden transition-all motion-reduce:transition-none hover:border-indigo-200" }, /* @__PURE__ */ React.createElement(
      "button",
      {
        ...
        "aria-controls": "roster-group-" + gId,
      ...
      /* @__PURE__ */ React.createElement("div", { className: "flex-1 min-w-0" }, /* @__PURE__ */ React.createElement("div", { className: "font-bold text-sm text-slate-800 truncate" }, group.name), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-600" }, normalizedStudentQuery ? `${shownStudents.length} of ${gStudents.length} matching` : `${gStudents.length} student${gStudents.length !== 1 ? "s" : ""}`, " \xB7 ", group.profile?.gradeLevel || "\u2014", " \xB7 ", group.profile?.leveledTextLanguage || "\u2014")),
      /* @__PURE__ */ React.createElement(ChevronDown, { size: 16, className: `text-slate-600 transition-transform motion-reduce:transition-none ${isExpanded ? "rotate-180" : ""}` })
    ), isExpanded && /* @__PURE__ */ React.createElement("div", { id: "roster-group-" + gId, className: "p-4 space-y-3 border-t border-slate-100 bg-white" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row gap-2 sm:items-center mb-2 min-w-0" }, /* @__PURE__ */ React.createElement(
      "input",
      {
        type: "text",
        value: group.name,
    ...
    ), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-1" }, COLORS.map((c) => /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        key: c,
  ...
  ), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-1" }, COLORS.slice(0, 4).map((c) => /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      key: c,
    ...
    "button",
    {
      ...
      disabled: !newGroupName.trim() || groupIds.length >= ALLO_ROSTER_MAX_GROUPS,
      title: groupIds.length >= ALLO_ROSTER_MAX_GROUPS ? "Maximum " + ALLO_ROSTER_MAX_GROUPS + " groups reached" : void 0,
  ...
  )), groupNameError && /* @__PURE__ */ React.createElement("p", { role: "alert", className: "text-xs font-bold text-red-700" }, groupNameError), groupIds.length >= ALLO_ROSTER_MAX_GROUPS && /* @__PURE__ */ React.createElement("p", { role: "status", className: "text-xs font-bold text-amber-800" }, "Group limit reached (", ALLO_ROSTER_MAX_GROUPS, "). Delete an unused group to add another."), /* @__PURE__ */ React.createElement("div", { className: "mt-4 pt-4 border-t border-slate-100" }, /* @__PURE__ */ React.createElement("div", { className: "text-xs font-bold text-slate-600 uppercase tracking-wider mb-2 flex items-center gap-1.5" }, /* @__PURE__ */ React.createElement(Users, { size: 14 }), " ", t("roster.manage_students") || "Manage Students", /* @__PURE__ */ React.createElement("span", { className: "ml-auto text-[11px] font-medium normal-case tracking-normal text-slate-500" }, studentCodenames.length, "/", ALLO_ROSTER_MAX_STUDENTS)), /* @__PURE__ */ React.createElement("div", { className: "flex flex-col gap-2" }, /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-700" }, "Add a codename only. Use the printable worksheet for a private handwritten name and location mapping."), /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row gap-2 sm:items-center min-w-0" }, /* @__PURE__ */ React.createElement("div", { className: "flex-1 flex flex-col gap-1.5" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row gap-1.5 sm:items-center min-w-0" }, /* @__PURE__ */ React.createElement(
    "select",
    {
      value: rosterAdjIsValid ? rosterAdj : "",
    ...
    "select",
    {
      value: rosterAnimalIsValid ? rosterAnimal : "",
  ...
  ), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: randomizeRosterName, title: t("common.randomize") || "Randomize", "aria-label": t("common.randomize") || "Randomize codename", className: "min-h-10 min-w-10 p-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 transition-colors motion-reduce:transition-none" }, /* @__PURE__ */ React.createElement(RefreshCw, { size: 12 })))), /* @__PURE__ */ React.createElement(
    "select",
    {
    ...
    "button",
    {
      ...
      disabled: !rosterAdjIsValid || !rosterAnimalIsValid || studentCodenames.length >= ALLO_ROSTER_MAX_STUDENTS,
      title: studentCodenames.length >= ALLO_ROSTER_MAX_STUDENTS ? "Maximum " + ALLO_ROSTER_MAX_STUDENTS + " codenames reached" : void 0,
    ...
    groupIds.map((gId) => /* @__PURE__ */ React.createElement("option", { key: gId, value: gId }, groups[gId].name))
  ), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setReadingPreferencesStudent(name), className: "w-9 h-9 inline-flex items-center justify-center rounded-full hover:bg-amber-100 hover:text-amber-950 transition-colors motion-reduce:transition-none", "aria-label": "Edit reading preferences for " + name, title: "Reading preferences" }, /* @__PURE__ */ React.createElement("span", { "aria-hidden": "true" }, "\u2605")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => handleRemoveStudent(name), className: "min-h-9 rounded-lg border border-red-200 px-2 text-red-700 hover:bg-red-50 transition-colors motion-reduce:transition-none", "aria-label": "Delete " + name + " from roster" }, "Delete"))))), readingPreferencesStudent && (() => {
    ...
    return /* @__PURE__ */ React.createElement("section", { className: "rounded-xl border-2 border-violet-200 bg-violet-50/70 p-4", "aria-labelledby": "roster-reading-preferences-title" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-start justify-between gap-3" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h3", { id: "roster-reading-preferences-title", className: "text-sm font-black text-violet-950" }, "Reading preferences \xB7 ", readingPreferencesStudent), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-xs text-violet-800" }, "Use neutral preferences only. The learner can privately change these choices at any time.")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setReadingPreferencesStudent(null), className: "min-h-9 min-w-9 inline-flex items-center justify-center rounded-full bg-white text-violet-800 hover:bg-violet-100 focus:outline-none focus:ring-2 focus:ring-violet-500", "aria-label": "Close reading preferences" }, /* @__PURE__ */ React.createElement(X, { size: 14 }))), /* @__PURE__ */ React.createElement("label", { className: "mt-3 block text-xs font-bold text-violet-950" }, "Preferred reading theme", /* @__PURE__ */ React.createElement("select", { value: preference.readingTheme || "", onChange: (event) => handleSetLearnerReadingTheme(readingPreferencesStudent, event.target.value), className: "mt-1 min-h-10 w-full rounded-lg border border-violet-300 bg-white px-3 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-violet-500" }, /* @__PURE__ */ React.createElement("option", { value: "" }, "Use group/class default (", fallbackLabel, ")"), ALLO_TEACHER_READING_THEME_OPTIONS.map((option) => /* @__PURE__ */ React.createElement("option", { key: option.id, value: option.id }, option.label)))), /* @__PURE__ */ React.createElement("fieldset", { className: "mt-3" }, /* @__PURE__ */ React.createElement("legend", { className: "text-xs font-bold text-violet-950" }, "Favorite themes"), /* @__PURE__ */ React.createElement("p", { className: "mt-0.5 text-[11px] text-violet-800" }, "Favorites appear first in this learner's reading-theme picker."), /* @__PURE__ */ React.createElement("div", { className: "mt-2 flex flex-wrap gap-2" }, ALLO_TEACHER_READING_THEME_OPTIONS.map((option) => {
      const selected = favorites.includes(option.id);
      return /* @__PURE__ */ React.createElement("button", { type: "button", key: option.id, onClick: () => handleToggleLearnerReadingFavorite(readingPreferencesStudent, option.id), "aria-pressed": selected, className: `min-h-9 rounded-full border px-3 text-xs font-bold transition-colors motion-reduce:transition-none ${selected ? "border-violet-700 bg-violet-700 text-white" : "border-violet-300 bg-white text-violet-900 hover:bg-violet-100"}` }, selected ? "\u2605 " : "\u2606 ", option.label);
    }))), /* @__PURE__ */ React.createElement("div", { className: "mt-3 flex items-center justify-between gap-3 border-t border-violet-200 pt-3" }, /* @__PURE__ */ React.createElement("span", { className: "text-[11px] text-violet-800" }, "Choice priority: learner \u2192 saved preference \u2192 group \u2192 class \u2192 device."), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => handleResetLearnerReadingPreference(readingPreferencesStudent), className: "min-h-9 shrink-0 rounded-lg border border-violet-300 bg-white px-3 text-xs font-bold text-violet-900 hover:bg-violet-100" }, "Reset to defaults")));
  })(), Array.isArray(rosterKey?.sessionHistory) && rosterKey.sessionHistory.length > 0 && /* @__PURE__ */ React.createElement("details", { className: "rounded-xl border border-cyan-200 bg-cyan-50/60 p-3" }, /* @__PURE__ */ React.createElement("summary", { className: "cursor-pointer font-bold text-sm text-cyan-900" }, "Saved session history (", rosterKey.sessionHistory.length, ")"), /* @__PURE__ */ React.createElement("div", { className: "mt-3 rounded-lg border border-cyan-100 bg-white p-2", "aria-label": "Saved session history filters" }, /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 gap-2 sm:grid-cols-2" }, /* @__PURE__ */ React.createElement("label", { className: "text-[11px] font-bold text-cyan-900" }, "Focus", /* @__PURE__ */ React.createElement("select", { value: sessionHistoryFocus, onChange: (event) => setSessionHistoryFocus(event.target.value), className: "mt-1 min-h-9 w-full rounded-lg border border-cyan-300 bg-white px-2 text-xs text-slate-800", "aria-label": "Filter saved sessions by follow-up focus" }, /* @__PURE__ */ React.createElement("option", { value: "all" }, "All saved sessions"), /* @__PURE__ */ React.createElement("option", { value: "follow_up" }, "Needs follow-up"), /* @__PURE__ */ React.createElement("option", { value: "revision" }, "Revision opportunities"), /* @__PURE__ */ React.createElement("option", { value: "celebrate" }, "Celebrate progress"), /* @__PURE__ */ React.createElement("option", { value: "planned" }, "Has follow-up plan"))), /* @__PURE__ */ React.createElement("label", { className: "text-[11px] font-bold text-cyan-900" }, "Activity", /* @__PURE__ */ React.createElement("select", { value: sessionHistoryActivityKind, onChange: (event) => setSessionHistoryActivityKind(event.target.value), className: "mt-1 min-h-9 w-full rounded-lg border border-cyan-300 bg-white px-2 text-xs text-slate-800", "aria-label": "Filter saved sessions by activity type" }, /* @__PURE__ */ React.createElement("option", { value: "all" }, "All activity types"), sessionActivityKinds.map((kind) => /* @__PURE__ */ React.createElement("option", { key: kind, value: kind }, kind.replace(/_/g, " "))))))), /* @__PURE__ */ React.createElement("div", { className: "mt-3 space-y-2" }, filteredSessionHistory.length === 0 && /* @__PURE__ */ React.createElement("p", { className: "rounded-lg border border-cyan-100 bg-white p-3 text-xs text-slate-600" }, "No saved sessions match these filters."), filteredSessionHistory.slice(0, 10).map((session) => /* @__PURE__ */ React.createElement("details", { key: session.id, className: "rounded-lg border border-cyan-100 bg-white p-2" }, /* @__PURE__ */ React.createElement("summary", { className: "cursor-pointer text-xs font-bold text-slate-700 flex flex-wrap gap-x-2" }, /* @__PURE__ */ React.createElement("span", null, session.endedAt ? new Date(session.endedAt).toLocaleString() : "Saved session"), /* @__PURE__ */ React.createElement("span", { className: "text-cyan-700" }, Object.keys(session.participants || {}).length, " present"), /* @__PURE__ */ React.createElement("span", { className: "text-slate-500" }, session.mode === "mailbox" ? "Mailbox" : "Firebase")), /* @__PURE__ */ React.createElement("div", { className: "mt-2 text-xs text-slate-600 space-y-1" }, /* @__PURE__ */ React.createElement("div", { className: "flex justify-end" }, /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => downloadRosterSessionEvidenceCsv(session), className: "min-h-9 rounded-lg border border-cyan-300 bg-cyan-50 px-2 py-1 text-[11px] font-bold text-cyan-900 hover:bg-cyan-100", "aria-label": "Download this privacy-safe session evidence report as CSV" }, "Download evidence CSV")), typeof session.durationMinutes === "number" && /* @__PURE__ */ React.createElement("p", null, "Duration: ", session.durationMinutes, " min"), /* @__PURE__ */ React.createElement("div", { className: "rounded-lg border border-violet-100 bg-violet-50/60 p-2", "data-session-follow-up-planner": session.id }, /* @__PURE__ */ React.createElement("p", { className: "font-bold text-violet-900" }, "Post-session follow-up"), /* @__PURE__ */ React.createElement("p", { className: "mt-0.5 text-[11px] text-violet-700" }, "Local planning only. Nothing is sent or assigned until you choose an action."), /* @__PURE__ */ React.createElement("label", { className: "mt-2 block text-[11px] font-bold text-slate-700" }, "Codename-only private note", /* @__PURE__ */ React.createElement(
    "textarea",
    {
      value: noteDraftFor(session),
      onChange: (event) => setSessionNoteDrafts((previous) => ({ ...previous, [session.id]: event.target.value.slice(0, 1e3) })),
      rows: 2,
      maxLength: 1e3,
  ...
  ), /* @__PURE__ */ React.createElement("span", { className: "mt-1 block font-normal text-violet-700" }, "Use codenames only. This note is stored on this device and included in roster backups.")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => saveSessionNote(session), className: "min-h-9 rounded-lg border border-violet-300 bg-white px-2 text-[11px] font-bold text-violet-900" }, "Save codename note"), (() => {
    const draft = planDraftFor(session);
    ...
    return /* @__PURE__ */ React.createElement("div", { className: "mt-2 border-t border-violet-100 pt-2" }, safeFollowUpResources.length === 0 && /* @__PURE__ */ React.createElement("p", { className: "mb-2 rounded-lg border border-amber-200 bg-amber-50 p-2 text-[11px] text-amber-900" }, "Create or restore a student-safe resource in History to prepare this follow-up. Any saved plan remains available to review or clear."), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 gap-2 sm:grid-cols-2" }, /* @__PURE__ */ React.createElement("label", { className: "text-[11px] font-bold text-slate-700" }, "Existing resource", /* @__PURE__ */ React.createElement("select", { value: draft.resourceId, disabled: safeFollowUpResources.length === 0 || liveSending, onChange: (event) => setSessionPlanDrafts((previous) => ({ ...previous, [session.id]: { ...draft, resourceId: event.target.value } })), "aria-label": "Choose follow-up resource for session " + session.id, className: "mt-1 min-h-9 w-full rounded-lg border border-violet-200 bg-white px-2 text-xs text-slate-800 disabled:bg-slate-100" }, safeFollowUpResources.length === 0 && /* @__PURE__ */ React.createElement("option", { value: "" }, "No student-safe resources in History"), safeFollowUpResources.map((item) => /* @__PURE__ */ React.createElement("option", { key: item.id, value: item.id }, item.title || item.type || "Untitled resource")))), /* @__PURE__ */ React.createElement("label", { className: "text-[11px] font-bold text-slate-700" }, "Planned audience", /* @__PURE__ */ React.createElement("select", { value: draft.audienceKey, disabled: liveSending, onChange: (event) => setSessionPlanDrafts((previous) => ({ ...previous, [session.id]: { ...draft, audienceKey: event.target.value } })), "aria-label": "Choose follow-up audience for session " + session.id, className: "mt-1 min-h-9 w-full rounded-lg border border-violet-200 bg-white px-2 text-xs text-slate-800" }, /* @__PURE__ */ React.createElement("option", { value: "class" }, "Whole class"), supportCohorts.map((cohort) => /* @__PURE__ */ React.createElement("option", { key: cohort.code, value: "cohort:" + cohort.code }, cohort.label, " (", cohort.count, ")"))))), /* @__PURE__ */ React.createElement("p", { className: "mt-2 text-[11px] text-slate-600" }, "Planned audience is a private distribution reminder. Prepared links are shareable, not recipient-restricted. When a live session is active, the explicit live action below re-resolves current recipients before using the existing class or individual delivery controls."), planDirty && /* @__PURE__ */ React.createElement("p", { role: "status", className: "mt-2 text-[11px] font-bold text-amber-800" }, "Save your plan changes before opening, assigning, or presenting this resource."), /* @__PURE__ */ React.createElement("div", { className: "mt-2 flex flex-wrap gap-2" }, /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => saveSessionPlan(session), disabled: safeFollowUpResources.length === 0 || liveSending, className: "min-h-9 rounded-lg border border-violet-300 bg-violet-100 px-2 text-[11px] font-bold text-violet-900 disabled:cursor-not-allowed disabled:opacity-50" }, "Save follow-up plan"), savedResource && typeof onOpenFollowUpResource === "function" && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => onOpenFollowUpResource(savedResource), disabled: planDirty || liveSending, className: "min-h-9 rounded-lg border border-sky-300 bg-sky-50 px-2 text-[11px] font-bold text-sky-900 disabled:cursor-not-allowed disabled:opacity-50" }, "Open planned resource"), savedResource && typeof onPrepareFollowUpAssignment === "function" && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => onPrepareFollowUpAssignment(savedResource), disabled: planDirty || liveSending, className: "min-h-9 rounded-lg border border-emerald-300 bg-emerald-50 px-2 text-[11px] font-bold text-emerald-900 disabled:cursor-not-allowed disabled:opacity-50" }, "Prepare assignment link"), canSendToLive && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => sendSessionPlanToLive(session), disabled: liveActionInFlight, "aria-label": liveActionLabel + " from saved follow-up plan", className: "min-h-9 rounded-lg border border-fuchsia-300 bg-fuchsia-50 px-2 text-[11px] font-bold text-fuchsia-900 disabled:cursor-not-allowed disabled:opacity-50" }, liveSending ? savedPlan.audience === "cohort" ? "Assigning\u2026" : "Presenting\u2026" : liveActionLabel), savedPlan && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setSessionPlanStatus(session, savedPlan.status === "completed" ? "planned" : "completed"), disabled: planDirty || liveSending, className: "min-h-9 rounded-lg border border-slate-300 bg-white px-2 text-[11px] font-bold text-slate-700 disabled:cursor-not-allowed disabled:opacity-50" }, savedPlan.status === "completed" ? "Reopen plan" : "Mark complete"), savedPlan && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => clearSessionPlan(session), disabled: liveSending, className: "min-h-9 rounded-lg border border-rose-200 bg-white px-2 text-[11px] font-bold text-rose-700 disabled:cursor-not-allowed disabled:opacity-50" }, "Clear plan")), savedPlan && /* @__PURE__ */ React.createElement("p", { className: "mt-2 text-[11px] text-violet-800" }, /* @__PURE__ */ React.createElement("span", { className: "font-bold" }, savedPlan.status === "completed" ? "Completed" : "Planned", ":"), " ", savedPlan.resourceTitle, " \xB7 ", savedPlan.cohortLabel, !savedResource ? " \xB7 resource no longer available in History" : ""));
  })(), sessionPlannerStatus[session.id] && /* @__PURE__ */ React.createElement("p", { role: "status", "aria-live": "polite", className: "mt-2 text-[11px] font-bold text-violet-800" }, sessionPlannerStatus[session.id])), Array.isArray(session.liveActivities) && session.liveActivities.length > 0 && /* @__PURE__ */ React.createElement("p", null, /* @__PURE__ */ React.createElement("span", { className: "font-bold" }, "Live activity evidence:"), " ", session.liveActivities.length, " activit", session.liveActivities.length === 1 ? "y" : "ies", " \xB7 ", session.liveActivities.reduce((sum, activity) => sum + (activity.submitted || 0), 0), " submissions \xB7 ", session.liveActivities.reduce((sum, activity) => sum + (activity.revised || 0), 0), " revisions"), session.organizerActivity && /* @__PURE__ */ React.createElement("p", null, /* @__PURE__ */ React.createElement("span", { className: "font-bold" }, "Visual organizer:"), " ", String(session.organizerActivity.type || "organizer").replace(/3d$/i, " 3D"), " \xB7 ", session.organizerActivity.statusCounts?.complete || 0, " complete \xB7 ", session.organizerActivity.statusCounts?.attempted || 0, " attempted \xB7 ", (session.organizerActivity.followUpCodenames || []).length, " launch follow-up"), session.insightBrief && /* @__PURE__ */ React.createElement("div", { className: "rounded-lg border border-indigo-100 bg-indigo-50/70 p-2" }, /* @__PURE__ */ React.createElement("p", null, /* @__PURE__ */ React.createElement("span", { className: "font-bold text-indigo-900" }, "Insight brief:"), " ", session.insightBrief.activityCount || 0, " activities \xB7 ", session.insightBrief.submissions || 0, " submissions \xB7 ", session.insightBrief.revisions || 0, " revisions \xB7 ", (session.insightBrief.followUpCodenames || []).length, " follow-up"), Array.isArray(session.insightBrief.evidenceCohorts) && session.insightBrief.evidenceCohorts.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "mt-2 space-y-1", "aria-label": "Saved evidence cohorts" }, session.insightBrief.evidenceCohorts.map((cohort) => /* @__PURE__ */ React.createElement("details", { key: cohort.code, className: "rounded border p-1.5 " + (cohort.intent === "celebrate" ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50") }, /* @__PURE__ */ React.createElement("summary", { className: "cursor-pointer font-bold text-slate-700" }, cohort.label, " - ", cohort.count), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-[11px] text-slate-600" }, cohort.recommendedAction), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-[11px] text-slate-600" }, (cohort.codenames || []).join(", "))))), Array.isArray(session.insightBrief.groups) && session.insightBrief.groups.some((group) => group.followUpCount > 0) && /* @__PURE__ */ React.createElement("p", null, /* @__PURE__ */ React.createElement("span", { className: "font-bold" }, "Group patterns:"), " ", session.insightBrief.groups.filter((group) => group.followUpCount > 0).map((group) => `${groups[group.groupId]?.name || group.groupId}: ${group.followUpCount} follow-up`).join(" \xB7 ")), Array.isArray(session.insightBrief.nextMoves) && session.insightBrief.nextMoves.length > 0 && /* @__PURE__ */ React.createElement("ul", { className: "mt-1 list-disc pl-4" }, session.insightBrief.nextMoves.map((move) => /* @__PURE__ */ React.createElement("li", { key: move.code }, move.label)))), Array.isArray(session.classGoals) && session.classGoals.length > 0 && /* @__PURE__ */ React.createElement("p", null, /* @__PURE__ */ React.createElement("span", { className: "font-bold" }, "Class Goals met:"), " ", session.classGoals.map((goal) => `${goal.label} (${goal.mode === "independent" ? goal.delivered + " students" : "+" + goal.tokens + " each, " + goal.delivered + " students"})`).join(" \xB7 ")), Object.entries(session.participants || {}).map(([codename, record]) => /* @__PURE__ */ React.createElement("p", { key: codename }, /* @__PURE__ */ React.createElement("span", { className: "font-bold" }, codename), ": ", record.responseCount || 0, " quiz response", record.responseCount === 1 ? "" : "s", record.liveActivityCount ? ` \xB7 ${record.liveSubmissionCount || 0}/${record.liveActivityCount} activity submissions` : "", record.liveRevisionCount ? ` \xB7 ${record.liveRevisionCount} revision${record.liveRevisionCount === 1 ? "" : "s"}` : "", record.organizer ? ` \xB7 organizer ${record.organizer.status}${record.organizer.total ? ` ${record.organizer.correct}/${record.organizer.total}` : ""}` : "", record.groupId ? ` \xB7 group ${record.groupId}` : "")), Number(session.unmatchedCount || 0) > 0 && /* @__PURE__ */ React.createElement("p", { className: "text-rose-700" }, /* @__PURE__ */ React.createElement("span", { className: "font-bold" }, "Unmatched entries:"), " ", session.unmatchedCount, " (typed values were not retained)")))), filteredSessionHistory.length > 10 && /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-cyan-800" }, "Showing the 10 most recent matching sessions of ", filteredSessionHistory.length, ". Export JSON for the complete retained history."))), "          ", rosterKey && /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-3 pt-3 border-t border-slate-100 text-[11px] text-slate-600 font-medium" }, /* @__PURE__ */ React.createElement("span", null, groupIds.length, " group", groupIds.length !== 1 ? "s" : ""), /* @__PURE__ */ React.createElement("span", null, Object.keys(students).length, " student", Object.keys(students).length !== 1 ? "s" : ""), /* @__PURE__ */ React.createElement("span", null, getUnassigned().length, " unassigned"), rosterStorageError ? /* @__PURE__ */ React.createElement("span", { role: "alert", className: "sm:ml-auto flex items-center gap-1 font-bold text-red-700" }, /* @__PURE__ */ React.createElement(AlertTriangle, { size: 12 }), " ", rosterStorageError) : /* @__PURE__ */ React.createElement("span", { className: "sm:ml-auto flex items-center gap-1" }, /* @__PURE__ */ React.createElement(ShieldCheck, { size: 10, className: "text-green-500" }), " Saved on this device \xB7 codenames only"))), submissionDialog && /* @__PURE__ */ React.createElement("div", { className: "absolute inset-0 z-20 bg-slate-900/70 flex items-center justify-center p-4" }, /* @__PURE__ */ React.createElement(
    "div",
    {
...
const StudentConceptQuestOverlay = React.memo(({ sessionData, user, activeSessionCode, targetAppId, t, playSound }) => {
  ...
  const [questSyncError, setQuestSyncError] = useState("");
  const questWriteRef = useRef("");
  const [localAction, setLocalAction] = useState(null);
  const encounterFocusRef = useRef(null);
  const pauseFocusRef = useRef(null);
  const currentScopeRef = useRef("");
  const turnKey = engine?.getTurnKey?.(quest) || quest?.turnKey || String(quest?.turn || 0);
  const actionScope = activeSessionCode + ":" + turnKey;
  currentScopeRef.current = actionScope;
  ...
  const validVotes = engine?.currentVotes?.(quest, progress.questVotes, progress.questVoteTurns) || {};
  const myVote = validVotes[user?.uid];
  const validActions = engine?.currentActions?.(quest, progress.questActions) || {};
  const myAction = validActions[user?.uid] || (localAction?.scope === actionScope ? localAction.value : null);
  const myRole = progress.questRoles?.[user?.uid];
  const peerRoster = Object.entries(sessionData?.roster || {}).filter(([uid, entry]) => entry && uid !== user?.uid).map(([uid, entry]) => ({ ...entry, uid }));
  useEffect(() => {
    ...
    const assign = async () => {
      try {
        ...
        warnLog("Concept Quest party assignment failed:", error);
      }
    };
    assign();
  ...
  useEffect(() => {
    ...
    questWriteRef.current = "";
  }, [actionScope]);
  useEffect(() => {
    if (!myAction) return;
    setAnswerIndex(myAction.answerIndex);
    setAbilityId(myAction.abilityId);
    setSupportId(myAction.supportId || "clarify");
    setSupportTargetUid(myAction.supportTargetUid || "");
  }, [myAction?.turnKey, myAction?.submittedAt, myAction?.answerIndex]);
  useEffect(() => {
    if (escapeState?.isPaused) pauseFocusRef.current?.focus();
    else encounterFocusRef.current?.focus();
  }, [actionScope, quest?.phase, escapeState?.isPaused]);
  useEffect(() => {
  ...
  const writeChoice = async (field, value) => {
    if (!user?.uid || !activeSessionCode || !escapeState.isActive || escapeState.isPaused || questWriteRef.current || !["explore", "battle"].includes(quest.phase)) return;
    if (field === "questActions" && (myAction || quest.phase !== "battle")) return;
    if (field === "questVotes" && quest.phase !== "explore") return;
    if (field === "questRoles" && myAction) return;
    const requestScope = actionScope;
    questWriteRef.current = requestScope;
    setQuestSyncError("");
    setIsSubmitting(true);
    try {
      const sessionRef = doc(db, "artifacts", targetAppId, "public", "data", "sessions", activeSessionCode);
      const updates = { [`escapeRoomState.teamProgress.All.${field}.${user.uid}`]: value };
      if (field === "questVotes" && quest.actionSchema === 1) updates[`escapeRoomState.teamProgress.All.questVoteTurns.${user.uid}`] = turnKey;
      await updateDoc(sessionRef, updates);
      if (currentScopeRef.current === requestScope && field === "questActions") setLocalAction({ scope: requestScope, value });
      playSound?.("click");
    } catch (error) {
      warnLog("Concept Quest action failed:", error);
      if (currentScopeRef.current === requestScope) setQuestSyncError(tr("choice_sync_failed", "Your choice could not be saved. Try again when the connection returns."));
    } finally {
      if (questWriteRef.current === requestScope) {
        questWriteRef.current = "";
        setIsSubmitting(false);
      }
    }
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "fixed inset-0 z-[9999] overflow-auto bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 text-white", role: "region", "aria-labelledby": "concept-quest-title" }, questSyncError && /* @__PURE__ */ React.createElement("p", { role: "alert", className: "m-3 rounded-lg bg-red-950 p-3 text-red-100" }, questSyncError), escapeState.isPaused && /* @__PURE__ */ React.createElement("div", { ref: pauseFocusRef, tabIndex: -1, role: "status", className: "sticky top-0 z-30 border-b border-amber-300 bg-amber-100 p-3 text-center font-bold text-amber-900" }, tr("paused_notice", "Quest paused. Your choices are saved while the teacher prepares the next step.")), /* @__PURE__ */ React.createElement("header", { className: "sticky top-0 z-20 border-b border-indigo-400/30 bg-slate-950/95 p-3 backdrop-blur" }, /* @__PURE__ */ React.createElement("div", { className: "mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h2", { id: "concept-quest-title", className: "text-xl font-black" }, "\u{1F5FA}\uFE0F ", quest.title), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-indigo-200" }, tr("turn_room", "Turn {turn} \xB7 {room}", { turn: quest.turn, room: currentRoom?.name || "" }))), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-3 text-sm font-bold", "aria-label": tr("party_health_aria", "Party health {current} of {maximum}", { current: quest.party.hp, maximum: quest.party.maxHp }) }, /* @__PURE__ */ React.createElement("span", null, "\u2764\uFE0F ", quest.party.hp, "/", quest.party.maxHp), /* @__PURE__ */ React.createElement("span", null, "\u{1F6E1}\uFE0F ", quest.party.shield || 0), /* @__PURE__ */ React.createElement("span", null, "\u2728 ", quest.party.xp || 0, " XP"), /* @__PURE__ */ React.createElement("span", null, "\u{1F537} ", (quest.sigils || []).length, "/", neededSigils))), /* @__PURE__ */ React.createElement("div", { role: "progressbar", "aria-label": tr("party_hp", "Party HP"), "aria-valuemin": 0, "aria-valuemax": quest.party.maxHp, "aria-valuenow": quest.party.hp, className: "mx-auto mt-2 h-2 max-w-6xl overflow-hidden rounded-full bg-slate-700" }, /* @__PURE__ */ React.createElement("div", { className: "h-full bg-emerald-500 transition-all motion-reduce:transition-none", style: { width: `${partyHpPercent}%` } }))), /* @__PURE__ */ React.createElement("main", { className: "mx-auto grid max-w-6xl gap-5 p-4 lg:grid-cols-[1.25fr_.75fr]" }, /* @__PURE__ */ React.createElement("section", { className: "rounded-2xl border border-indigo-400/30 bg-slate-900/80 p-4", "aria-labelledby": "quest-map-heading" }, /* @__PURE__ */ React.createElement("h3", { id: "quest-map-heading", className: "mb-3 text-lg font-black" }, tr("cooperative_map", "Cooperative map")), /* @__PURE__ */ React.createElement("div", { className: "relative grid grid-cols-2 gap-2 rounded-xl border border-slate-700 bg-slate-950/70 p-2 sm:block sm:h-[360px] sm:p-0", "aria-label": tr("map_aria", "Eight-room game map") }, /* @__PURE__ */ React.createElement("svg", { "aria-hidden": "true", className: "pointer-events-none absolute inset-0 hidden h-full w-full sm:block", viewBox: "0 0 100 100", preserveAspectRatio: "none" }, rooms.flatMap((room) => (room.neighbors || []).filter((id) => id > room.id).map((id) => {
    const other = rooms.find((entry) => entry.id === id);
    return other ? /* @__PURE__ */ React.createElement("line", { key: room.id + id, x1: room.x, y1: room.y, x2: other.x, y2: other.y, stroke: "#64748b", strokeWidth: "0.5" }) : null;
  }))), rooms.map((room) => {
    ...
    const isLocked = room.kind === "boss" && (quest.sigils || []).length < neededSigils;
    ...
    return /* @__PURE__ */ React.createElement("button", { key: room.id, type: "button", disabled: !isAdjacent || isLocked || quest.phase !== "explore" || escapeState.isPaused || isSubmitting, onClick: () => writeChoice("questVotes", room.id), style: { left: `clamp(3.5rem, ${room.x}%, calc(100% - 3.5rem))`, top: `${room.y}%` }, className: `min-h-20 min-w-0 break-words sm:absolute sm:min-h-14 sm:w-24 sm:-translate-x-1/2 sm:-translate-y-1/2 rounded-xl border-2 p-1 text-center text-[11px] font-bold focus:outline-none focus:ring-4 focus:ring-yellow-300 ${isCurrent ? "border-yellow-300 bg-yellow-500 text-slate-900" : isAdjacent && !isLocked ? "border-cyan-300 bg-indigo-800 hover:bg-indigo-700" : isVisited ? "border-emerald-500 bg-slate-800" : "border-slate-600 bg-slate-900 text-slate-300"} disabled:cursor-default`, "aria-pressed": myVote === room.id, "aria-current": isCurrent ? "location" : void 0, "aria-label": tr("room_aria", "{name}, {kind}{locked}{current}", { name: room.name, kind: tr(`room_kind_${room.kind}`, room.kind), locked: lockedSuffix, current: currentSuffix }) }, /* @__PURE__ */ React.createElement("span", { className: "block text-xl", "aria-hidden": "true" }, isLocked ? "\u{1F512}" : room.emoji), room.name);
  })), /* @__PURE__ */ React.createElement("div", { className: "mt-3 rounded-xl bg-indigo-950/60 p-3" }, /* @__PURE__ */ React.createElement("p", { className: "font-bold" }, currentRoom?.emoji, " ", currentRoom?.name), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-indigo-100" }, tr("concept_label", "Concept: {concept}", { concept: currentRoom?.concept || "" })), /* @__PURE__ */ React.createElement("div", { className: "mt-3" }, /* @__PURE__ */ React.createElement("p", { className: "mb-2 text-sm font-bold" }, tr("choose_role", "Choose a party role")), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-2" }, (quest.roles || engine.ROLES || []).map((role) => /* @__PURE__ */ React.createElement("button", { key: role.id, type: "button", disabled: !!myAction || isSubmitting || escapeState.isPaused || !["explore", "battle"].includes(quest.phase), onClick: () => {
    setAbilityId(role.abilityId);
    writeChoice("questRoles", role.id);
  }, "aria-pressed": myRole === role.id, className: `min-h-14 rounded-lg border p-2 text-left text-xs focus:ring-4 focus:ring-yellow-300 ${myRole === role.id ? "border-emerald-300 bg-emerald-900" : "border-slate-600 bg-slate-800"}` }, /* @__PURE__ */ React.createElement("strong", null, role.emoji, " ", gameLabel(role, "role")), /* @__PURE__ */ React.createElement("span", { className: "mt-1 block text-slate-300" }, gameLabel(role, "role", "description")))))), quest.phase === "explore" && /* @__PURE__ */ React.createElement("div", { className: "mt-3" }, /* @__PURE__ */ React.createElement("p", { className: "mb-2 text-sm" }, tr("vote_instruction", "Vote for a connected room. Earn {count} concept sigils to unlock the Mastery Gate.", { count: neededSigils })), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2" }, adjacent.map((room) => {
    const locked = room.kind === "boss" && (quest.sigils || []).length < neededSigils;
    return /* @__PURE__ */ React.createElement("button", { key: room.id, type: "button", disabled: isSubmitting || escapeState.isPaused || locked, onClick: () => writeChoice("questVotes", room.id), className: `min-h-11 rounded-lg px-3 py-2 text-sm font-bold focus:ring-4 focus:ring-yellow-300 disabled:opacity-50 ${myVote === room.id ? "bg-yellow-400 text-slate-950" : "bg-indigo-700 hover:bg-indigo-600"}` }, locked ? "\u{1F512}" : room.emoji, " ", room.name);
  })), myVote && /* @__PURE__ */ React.createElement("p", { role: "status", className: "mt-2 text-sm text-yellow-200" }, tr("vote_recorded", "Your vote is recorded."))))), /* @__PURE__ */ React.createElement("section", { className: `space-y-4 lg:col-start-2 lg:row-start-1 ${quest.phase === "battle" ? "order-first lg:order-none" : ""}`, "aria-label": tr("current_encounter_aria", "Current encounter") }, /* @__PURE__ */ React.createElement("h3", { ref: encounterFocusRef, tabIndex: -1, className: "text-lg font-black focus:outline-none focus:ring-2 focus:ring-yellow-300" }, tr("phase_" + quest.phase, quest.phase === "battle" ? "Choose your action" : quest.phase === "explore" ? "Choose a path together" : quest.phase === "defeat" ? "Regroup with your teacher" : "Quest complete")), isSubmitting && /* @__PURE__ */ React.createElement("p", { role: "status", className: "text-sm text-cyan-200" }, tr("saving_choice", "Saving your choice\u2026")), quest.activeEvent && /* @__PURE__ */ React.createElement("div", { className: "rounded-2xl border-2 border-amber-300 bg-amber-950/70 p-4", role: "status", "aria-live": "polite" }, /* @__PURE__ */ React.createElement("p", { className: "text-xs font-black uppercase text-amber-300" }, tr("teacher_event", "Teacher GM event")), /* @__PURE__ */ React.createElement("h3", { className: "text-lg font-black" }, quest.activeEvent.title), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-sm" }, quest.activeEvent.description)), quest.lastRound && /* @__PURE__ */ React.createElement("div", { className: "rounded-2xl border border-cyan-400/40 bg-cyan-950/60 p-4", role: "status", "aria-live": "polite" }, /* @__PURE__ */ React.createElement("p", { className: "font-black" }, tr("round_recap", "Round recap: {correct}/{total} checks succeeded", { correct: quest.lastRound.correct, total: quest.lastRound.total })), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-sm" }, tr("damage_recap", "The party dealt {damage} damage and took {incoming}.", { damage: quest.lastRound.damage, incoming: quest.lastRound.incoming })), quest.lastRound.combo && /* @__PURE__ */ React.createElement("p", { className: "mt-1 font-bold text-yellow-200" }, "\u26A1 ", tr("combo_full", "Concept Combo! Three different abilities worked together.")), quest.lastRound.synergyCount > 0 && /* @__PURE__ */ React.createElement("p", { className: "text-sm text-emerald-200" }, tr("role_synergies_activated", "{count} role synergies activated.", { count: quest.lastRound.synergyCount })), quest.lastRound.assistedCount > 0 && /* @__PURE__ */ React.createElement("p", { className: "text-sm text-sky-200" }, "\u{1F91D} ", tr("peer_assists_activated", "{count} peer assists activated.", { count: quest.lastRound.assistedCount })), quest.lastRound.encounterRule && /* @__PURE__ */ React.createElement("p", { className: "mt-1 rounded-lg bg-slate-900/60 p-2 text-sm text-cyan-100" }, quest.lastRound.encounterRule), /* @__PURE__ */ React.createElement("details", { className: "mt-2 text-sm" }, /* @__PURE__ */ React.createElement("summary", { className: "cursor-pointer font-bold" }, tr("review_concept", "Review the concept")), /* @__PURE__ */ React.createElement("p", { className: "mt-1 font-bold text-cyan-100" }, quest.lastRound.prompt), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-cyan-100" }, quest.lastRound.explanation))), quest.phase === "battle" && currentEnemy && /* @__PURE__ */ React.createElement("div", { className: "rounded-2xl border border-fuchsia-400/40 bg-slate-900 p-4" }, /* @__PURE__ */ React.createElement("div", { className: "text-center" }, /* @__PURE__ */ React.createElement("div", { className: "text-6xl", "aria-hidden": "true" }, currentEnemy.emoji), /* @__PURE__ */ React.createElement("h3", { className: "text-xl font-black" }, currentEnemy.name), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-fuchsia-200" }, tr("misconception_about", "A misconception about {concept}", { concept: currentRoom.concept })), currentRoom.kind === "puzzle" && /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-xs font-bold text-cyan-200" }, tr("reasoning_lock_student", "Reasoning Lock: reach two-thirds class consensus for bonus damage.")), currentRoom.kind === "boss" && /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-xs font-bold text-yellow-200" }, tr("mastery_barrier_student", "Mastery Barrier: reach 60% class accuracy to break through."))), /* @__PURE__ */ React.createElement("div", { role: "progressbar", "aria-valuemin": 0, "aria-valuemax": currentEnemy.maxHp, "aria-valuenow": currentEnemy.hp, className: "my-3 h-3 overflow-hidden rounded-full bg-slate-700", "aria-label": tr("enemy_health_aria", "Enemy health {current} of {maximum}", { current: currentEnemy.hp, maximum: currentEnemy.maxHp }) }, /* @__PURE__ */ React.createElement("div", { className: "h-full bg-fuchsia-500", style: { width: `${enemyHpPercent}%` } })), /* @__PURE__ */ React.createElement("fieldset", { disabled: !!myAction || isSubmitting || escapeState.isPaused, className: "space-y-3" }, /* @__PURE__ */ React.createElement("legend", { className: "font-bold" }, currentRoom.challenge.prompt), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-2" }, (quest.abilities || []).map((ability) => /* @__PURE__ */ React.createElement("button", { type: "button", key: ability.id, onClick: () => setAbilityId(ability.id), "aria-pressed": abilityId === ability.id, className: `min-h-16 rounded-lg border p-2 text-left text-xs focus:ring-4 focus:ring-yellow-300 ${abilityId === ability.id ? "border-cyan-300 bg-cyan-900" : "border-slate-600 bg-slate-800"}` }, /* @__PURE__ */ React.createElement("span", { className: "font-black" }, ability.emoji, " ", gameLabel(ability, "ability")), /* @__PURE__ */ React.createElement("span", { className: "mt-1 block text-slate-300" }, gameLabel(ability, "ability", "description"))))), /* @__PURE__ */ React.createElement("div", { className: "space-y-2" }, currentRoom.challenge.options.map((option, index) => /* @__PURE__ */ React.createElement("label", { key: index, className: `flex min-h-11 cursor-pointer items-center gap-2 rounded-lg border p-2 text-sm ${answerIndex === index ? "border-yellow-300 bg-yellow-950" : "border-slate-600 bg-slate-800"}` }, /* @__PURE__ */ React.createElement("input", { type: "radio", name: "concept-quest-answer", checked: answerIndex === index, onChange: () => setAnswerIndex(index) }), option))), peerRoster.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "rounded-xl border border-sky-500/40 bg-sky-950/50 p-3" }, /* @__PURE__ */ React.createElement("p", { className: "font-bold text-sky-100" }, tr("support_teammate", "Support a teammate")), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-sky-200" }, tr("support_optional", "Optional: a correct response can help one classmate this turn.")), /* @__PURE__ */ React.createElement("div", { className: "mt-2 grid grid-cols-3 gap-1" }, (quest.supports || engine.SUPPORTS || []).map((support) => /* @__PURE__ */ React.createElement("button", { key: support.id, type: "button", onClick: () => setSupportId(support.id), "aria-pressed": supportId === support.id, className: `min-h-11 rounded-lg border p-1 text-xs font-bold ${supportId === support.id ? "border-sky-300 bg-sky-800" : "border-slate-600 bg-slate-800"}` }, support.emoji, " ", gameLabel(support, "support")))), /* @__PURE__ */ React.createElement("label", { className: "mt-2 block text-xs font-bold text-sky-100" }, tr("teammate", "Teammate"), /* @__PURE__ */ React.createElement("select", { value: supportTargetUid, onChange: (event) => setSupportTargetUid(event.target.value), className: "mt-1 min-h-11 w-full rounded-lg border border-sky-400 bg-slate-900 p-2 text-white" }, /* @__PURE__ */ React.createElement("option", { value: "" }, tr("no_assist", "No assist this turn")), peerRoster.map((peer) => /* @__PURE__ */ React.createElement("option", { key: peer.uid, value: peer.uid }, peer.name || tr("classmate", "Classmate")))))), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => answerIndex != null && writeChoice("questActions", { abilityId, roleId: myRole || "", answerIndex, submittedAt: Date.now(), ...quest.actionSchema === 1 ? { turnKey } : {}, ...supportTargetUid ? { supportId, supportTargetUid } : {} }), disabled: answerIndex == null, className: "min-h-12 w-full rounded-xl bg-emerald-600 px-4 py-2 font-black hover:bg-emerald-500 disabled:opacity-50" }, tr("commit_turn", "Commit turn"))), myAction && /* @__PURE__ */ React.createElement("p", { role: "status", className: "mt-3 rounded-lg bg-emerald-950 p-2 text-center text-sm text-emerald-200" }, tr("committed_notice", "Turn committed. Discuss your reasoning while the co-GM resolves the round."))), quest.phase === "complete" && /* @__PURE__ */ React.createElement("div", { className: "rounded-2xl border-2 border-yellow-300 bg-emerald-950 p-6 text-center", role: "status" }, /* @__PURE__ */ React.createElement("div", { className: "text-6xl" }, "\u{1F3C6}"), /* @__PURE__ */ React.createElement("h3", { className: "text-2xl font-black" }, tr("gate_cleared", "Mastery Gate cleared!")), /* @__PURE__ */ React.createElement("p", null, tr("class_won", "The class won by combining its ideas.")), debrief && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { className: "mt-4 grid grid-cols-2 gap-2 text-sm sm:grid-cols-3" }, /* @__PURE__ */ React.createElement("span", { className: "rounded-lg bg-emerald-900 p-2" }, /* @__PURE__ */ React.createElement("strong", { className: "block text-xl" }, debrief.accuracy, "%"), tr("accuracy", "Accuracy")), /* @__PURE__ */ React.createElement("span", { className: "rounded-lg bg-emerald-900 p-2" }, /* @__PURE__ */ React.createElement("strong", { className: "block text-xl" }, debrief.combos), tr("combos", "Combos")), /* @__PURE__ */ React.createElement("span", { className: "rounded-lg bg-emerald-900 p-2" }, /* @__PURE__ */ React.createElement("strong", { className: "block text-xl" }, debrief.roleSynergies), tr("role_synergies", "Role synergies")), /* @__PURE__ */ React.createElement("span", { className: "rounded-lg bg-emerald-900 p-2" }, /* @__PURE__ */ React.createElement("strong", { className: "block text-xl" }, debrief.peerAssists), tr("peer_assists", "Peer assists")), /* @__PURE__ */ React.createElement("span", { className: "rounded-lg bg-emerald-900 p-2" }, /* @__PURE__ */ React.createElement("strong", { className: "block text-xl" }, debrief.puzzlesSolved), tr("puzzles", "Puzzles")), /* @__PURE__ */ React.createElement("span", { className: "rounded-lg bg-emerald-900 p-2" }, /* @__PURE__ */ React.createElement("strong", { className: "block text-xl" }, debrief.rounds), tr("rounds", "Rounds"))), debrief.strongestConcept && /* @__PURE__ */ React.createElement("p", { className: "mt-3 text-sm text-emerald-100" }, tr("strongest_concept", "Strongest concept: {concept} ({accuracy}%)", { concept: debrief.strongestConcept.concept, accuracy: debrief.strongestConcept.accuracy })))), quest.phase === "defeat" && /* @__PURE__ */ React.createElement("div", { className: "rounded-2xl border-2 border-rose-400 bg-rose-950 p-6 text-center", role: "alert" }, /* @__PURE__ */ React.createElement("h3", { className: "text-2xl font-black" }, tr("regroup_title", "The party needs a regroup")), /* @__PURE__ */ React.createElement("p", null, tr("regroup_body", "The teacher can introduce help, an item, or a new challenge\u2014this is not an individual loss."))), /* @__PURE__ */ React.createElement("div", { className: "rounded-2xl border border-slate-700 bg-slate-900 p-4" }, /* @__PURE__ */ React.createElement("h3", { className: "font-black" }, tr("shared_inventory", "Shared inventory")), quest.inventory.length ? /* @__PURE__ */ React.createElement("ul", { className: "mt-2 space-y-2 text-sm" }, quest.inventory.map((item, index) => /* @__PURE__ */ React.createElement("li", { key: `${item.id}-${index}`, className: "rounded-lg bg-slate-800 p-2" }, item.emoji, " ", /* @__PURE__ */ React.createElement("strong", null, item.name), " \u2014 ", item.description))) : /* @__PURE__ */ React.createElement("p", { className: "mt-2 text-sm text-slate-300" }, tr("inventory_empty", "Items awarded or generated by the teacher appear here."))))));
});
function alloLiveEscapeProgress(raw = {}, puzzles = []) {
  const solvedPuzzles = [.../* @__PURE__ */ new Set([...raw.solvedPuzzles || [], ...Object.values(raw.solved || {}).filter((id) => puzzles.some((p) => p.id === id))])];
  if (raw.progressVersion !== 2) return { ...raw, solvedPuzzles };
  const maxLives = Number.isFinite(raw.maxLives) ? raw.maxLives : 3;
  const wrongAttempts = Object.keys(raw.misses || {}).length;
  const lives = maxLives >= 99 ? maxLives : Math.max(0, maxLives - wrongAttempts);
  return {
    ...raw,
    solvedPuzzles,
    maxLives,
    lives,
    wrongAttempts,
    streak: Object.values(raw.solvedAt || {}).filter((at) => at > Math.max(0, ...Object.values(raw.misses || {}).filter(Number.isFinite), ...Object.values(raw.hintedAt || {}).filter(Number.isFinite))).length,
    hintsRemaining: Math.max(0, (raw.maxHints ?? 3) - Object.values(raw.revealedHints || {}).filter(Boolean).length),
    isGameOver: maxLives < 99 && lives <= 0,
    isEscaped: puzzles.length > 0 && puzzles.every((p) => solvedPuzzles.includes(p.id))
  };
}
function alloLiveEscapeSeconds(state, now = Date.now()) {
  if (state?.isGameOver) return 0;
  if (!state?.isPaused && Number.isFinite(state?.endsAt)) return Math.max(0, Math.ceil((state.endsAt - now) / 1e3));
  return Math.max(0, Number(state?.timeRemaining) || 0);
}
function useLiveEscapeSeconds(state) {
  const [now, setNow] = useState(Date.now);
  useEffect(() => {
    setNow(Date.now());
    if (!state?.isActive || state.isPaused || state.isGameOver || !Number.isFinite(state.endsAt)) return;
    const timer = setInterval(() => setNow(Date.now()), 250);
    return () => clearInterval(timer);
  }, [state?.isActive, state?.isPaused, state?.isGameOver, state?.endsAt]);
  return alloLiveEscapeSeconds(state, now);
}
function alloLiveEscapeAnswer(puzzle, answer, type) {
  const normalize = (value) => String(value ?? "").normalize("NFC").trim().replace(/\s+/g, " ").toLocaleLowerCase();
  if (type === "mcq") return Number.isInteger(answer) && answer === puzzle.correctIndex;
  if (type === "sequence") return Array.isArray(answer) && JSON.stringify(answer) === JSON.stringify(puzzle.correctOrder);
  if (type === "matching") return Array.isArray(answer) && answer.length === puzzle.pairs?.length && new Set(answer.map((pair) => pair.left)).size === answer.length && new Set(answer.map((pair) => pair.right)).size === answer.length && answer.every((pair) => puzzle.pairs.some((expected) => expected.left === pair.left && expected.right === pair.right));
  return !!normalize(answer) && [puzzle.answer, ...puzzle.acceptedAnswers || []].some((value) => value != null && normalize(value) === normalize(answer));
}
function alloLiveSequenceIndices(puzzle) {
  const items = puzzle?.items || [];
  const shuffled = puzzle?.shuffledItems || [];
  if (shuffled.length === items.length && shuffled.every(Number.isInteger) && new Set(shuffled).size === items.length && shuffled.every((i) => i >= 0 && i < items.length)) return shuffled.slice();
  const unused = items.map((_, i) => i);
  const indices = shuffled.map((item) => {
    const at = unused.findIndex((i) => items[i] === item);
    return at < 0 ? -1 : unused.splice(at, 1)[0];
  });
  return indices.length === items.length && !indices.includes(-1) ? indices : items.map((_, i) => i);
}
function alloLiveFieldKey(value) {
...
const ClassicStudentEscapeRoomOverlay = React.memo(({ sessionData, user, activeSessionCode, targetAppId, t, playSound, setIsEscapeTimerRunning }) => {
  const escapeState = sessionData?.escapeRoomState || {};
  const timeRemaining = useLiveEscapeSeconds(escapeState);
  const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  const closePuzzleDialog = () => {
    const puzzle = (escapeState.puzzles || []).find((p) => p.linkedObjectId === selectedPuzzle || p.id === selectedPuzzle);
    if (puzzle) draftsRef.current[puzzle.id] = { input: userInput, sequence: sequenceOrder, pairs: matchingPairs, selected: matchingSelected };
    setSelectedPuzzle(null);
    window.setTimeout(() => {
      if (!puzzleTriggerRef.current?.disabled) puzzleTriggerRef.current?.focus();
      else escapeMainRef.current?.focus();
    }, 0);
  };
  const openPuzzleDialog = (event, objectId) => {
    puzzleTriggerRef.current = event.currentTarget;
    const puzzle = (escapeState.puzzles || []).find((p) => p.linkedObjectId === objectId || p.id === objectId);
    if (!puzzle || escapeState.isPaused || submissionRef.current) return;
    const draft = draftsRef.current[puzzle.id] || {};
    setUserInput(draft.input || "");
    setSequenceOrder(draft.sequence || alloLiveSequenceIndices(puzzle));
    setMatchingPairs(draft.pairs || []);
    setMatchingSelected(draft.selected || null);
    setSyncError("");
  ...
  const userTeam = escapeState.teams?.[user?.uid];
  const teamProgress = alloLiveEscapeProgress(escapeState.teamProgress?.[userTeam], escapeState.puzzles || []);
  ...
  const roomTimedOut = !isPaused && timeRemaining <= 0 && Number.isFinite(escapeState.endsAt);
  const teamColors = {
  ...
  useEffect(() => {
    const completedState = escapeState.isGameOver || roomTimedOut || teamGameOver || teamEscaped;
    if (!userTeam || completedState) escapeStateScreenRef.current?.focus();
    else if (isPaused) pauseDialogRef.current?.focus();
    else if (wasPausedRef.current) {
      if (puzzleDialogRef.current) puzzleDialogRef.current.querySelector('[data-initial-focus="true"]')?.focus();
      else escapeMainRef.current?.focus();
    }
    wasPausedRef.current = isPaused;
  }, [userTeam, escapeState.isGameOver, roomTimedOut, teamGameOver, teamEscaped, isPaused]);
  useEffect(() => {
    const escapedTeams = allTeams.filter(
      (team) => alloLiveEscapeProgress(escapeState.teamProgress?.[team], puzzles).isEscaped && team !== userTeam
    );
  ...
  useEffect(() => {
    ...
    setSyncError("");
  }, [activeSessionCode, escapeState.startedAt, userTeam]);
  const handleSubmitAnswer = async (puzzleId, answer, answerType) => {
    if (!userTeam || !activeSessionCode || submissionRef.current || isPaused || !escapeState.isActive || escapeState.isGameOver || teamGameOver || teamEscaped || timeRemaining <= 0) return;
    const puzzle = puzzles.find((p) => p.id === puzzleId);
    if (!puzzle || solvedPuzzlesSet.has(puzzleId) || typeof answer === "string" && !answer.trim()) return;
    const isCorrect = alloLiveEscapeAnswer(puzzle, answer, answerType);
    ...
    try {
      if (isCorrect) {
        ...
        try {
          ...
          await updateDoc(sessionRef, teamProgress.progressVersion === 2 ? {
            [`escapeRoomState.teamProgress.${userTeam}.solved.${alloLiveFieldKey(puzzleId)}`]: puzzleId,
            [`escapeRoomState.teamProgress.${userTeam}.solvedAt.${alloLiveFieldKey(puzzleId)}`]: Date.now()
          } : {
        ...
        try {
          ...
          const missId = alloLiveFieldKey(user?.uid || "student") + "_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
          await updateDoc(sessionRef, teamProgress.progressVersion === 2 ? {
            [`escapeRoomState.teamProgress.${userTeam}.misses.${missId}`]: Date.now()
          } : {
      ...
      warnLog("Live escape room submission failed:", error);
      setSyncError("Your answer could not be saved. Your work is still here; try submitting again.");
    } finally {
  ...
  return /* @__PURE__ */ React.createElement("div", { ref: escapeMainRef, tabIndex: -1, role: "region", "aria-labelledby": "escape-room-active-title", className: "fixed top-0 right-0 bottom-0 left-0 z-[9999] bg-gradient-to-br from-slate-900 via-purple-900 to-indigo-900 overflow-auto focus:outline-none focus:ring-4 focus:ring-inset focus:ring-white" }, /* @__PURE__ */ React.createElement("div", { className: "sticky top-0 z-50 bg-slate-900/90 backdrop-blur-sm border-b border-purple-500/30 p-4" }, /* @__PURE__ */ React.createElement("div", { className: "max-w-6xl mx-auto flex items-center justify-between flex-wrap gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-3" }, /* @__PURE__ */ React.createElement(DoorOpen, { className: "text-purple-400", size: 24 }), /* @__PURE__ */ React.createElement("h2", { id: "escape-room-active-title", className: "text-white font-bold text-lg" }, escapeState.room?.theme || t("escape_room.title"))), /* @__PURE__ */ React.createElement("div", { className: "flex items-center flex-wrap gap-3" }, /* @__PURE__ */ React.createElement("div", { className: `flex items-center gap-2 px-3 py-1.5 rounded-full ${myTeamColors.bg} text-white font-bold text-sm`, "data-help-key": "escape_room_team" }, /* @__PURE__ */ React.createElement(Users, { size: 14 }), t(`escape_room.team_${userTeam.toLowerCase()}`, { defaultValue: `${userTeam} Team` })), /* @__PURE__ */ React.createElement("div", { className: "text-white font-mono", "data-help-key": "escape_room_progress" }, /* @__PURE__ */ React.createElement("span", { className: "text-purple-300" }, solvedPuzzlesSet.size), /* @__PURE__ */ React.createElement("span", { className: "text-slate-400" }, "/", puzzles.length)), teamMaxLives < 99 && /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-1 px-2 py-1 rounded-full bg-slate-700/50", role: "img", "aria-label": `${t("escape_room.lives")}: ${teamLives}/${teamMaxLives}`, title: t("escape_room.lives"), "data-help-key": "escape_room_lives" }, Array.from({ length: teamMaxLives }).map((_, i) => /* @__PURE__ */ React.createElement("span", { key: i, "aria-hidden": "true", className: `text-sm ${i < teamLives ? "text-red-500" : "opacity-30 grayscale"}` }, "\u2764\uFE0F"))), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-1 px-2 py-1 rounded-full bg-slate-700/50", title: t("escape_room.hints_used") }, /* @__PURE__ */ React.createElement(Lightbulb, { size: 14, className: "text-yellow-400" }), /* @__PURE__ */ React.createElement("span", { className: "text-white text-xs font-bold" }, teamHintsRemaining)), teamStreak >= 3 && /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-1 px-2 py-1 rounded-full bg-orange-500/20 text-orange-400 font-bold text-xs animate-pulse motion-reduce:animate-none", "data-help-key": "escape_room_streak" }, "\u{1F525} x", teamStreak), /* @__PURE__ */ React.createElement("div", { className: `px-3 py-1.5 rounded-full font-mono font-bold ${timeRemaining < 60 ? "bg-red-600 text-white animate-pulse motion-reduce:animate-none" : "bg-slate-700 text-white"}`, "data-help-key": "escape_room_timer" }, /* @__PURE__ */ React.createElement(Clock, { size: 14, className: "inline mr-1" }), formatTime(timeRemaining))))), /* @__PURE__ */ React.createElement("div", { className: "relative mx-4 mt-4 lg:fixed lg:right-4 lg:top-24 bg-slate-800/80 backdrop-blur-sm rounded-xl p-4 border border-purple-500/30 z-40", "data-help-key": "escape_room_leaderboard" }, /* @__PURE__ */ React.createElement("h4", { className: "text-xs font-bold text-slate-300 uppercase mb-3" }, t("escape_room.live_progress")), allTeams.map((team) => {
    const progress = alloLiveEscapeProgress(escapeState.teamProgress?.[team], puzzles);
  ...
  })), /* @__PURE__ */ React.createElement("div", { className: "max-w-4xl mx-auto p-4 sm:p-6 mt-4 lg:pr-40" }, /* @__PURE__ */ React.createElement("p", { className: "text-center text-purple-200 mb-6 italic" }, escapeState.room?.description), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4" }, objects.map((obj, idx) => {
  ...
  }))), currentPuzzle && !isPaused && !solvedPuzzlesSet.has(currentPuzzle.id) && /* @__PURE__ */ React.createElement("div", { className: "fixed inset-0 z-[10000] bg-black/80 flex items-center justify-center p-4", role: "presentation" }, /* @__PURE__ */ React.createElement(
    "div",
    {
      ref: puzzleDialogRef,
      role: "dialog",
    ...
    syncError && /* @__PURE__ */ React.createElement("p", { role: "alert", className: "mb-3 rounded-lg bg-red-950 p-3 text-red-100" }, syncError),
    /* @__PURE__ */ React.createElement("fieldset", { disabled: isSubmitting || isPaused, "aria-busy": isSubmitting, className: "min-w-0" }, /* @__PURE__ */ React.createElement("legend", { className: "sr-only" }, t("escape_room.submit_answer")), isSubmitting && /* @__PURE__ */ React.createElement("p", { role: "status", className: "text-purple-200 mb-3" }, t("common.saving", { defaultValue: "Saving\u2026" })), currentPuzzle.hint && /* @__PURE__ */ React.createElement("div", { className: "mb-4" }, teamRevealedHints[alloLiveFieldKey(currentPuzzle.id)] || teamRevealedHints[currentPuzzle.id] ? /* @__PURE__ */ React.createElement("div", { className: "p-3 bg-yellow-500/20 border border-yellow-500/40 rounded-lg text-yellow-200 text-sm animate-in motion-reduce:animate-none fade-in" }, /* @__PURE__ */ React.createElement(Lightbulb, { size: 14, className: "inline mr-2 text-yellow-400" }), currentPuzzle.hint) : /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        onClick: async () => {
          if (teamHintsRemaining <= 0 || submissionRef.current || isPaused || teamRevealedHints[alloLiveFieldKey(currentPuzzle.id)]) return;
          submissionRef.current = true;
          setIsSubmitting(true);
          setSyncError("");
          try {
            ...
            const sessionRef = doc(db, "artifacts", effectiveAppId, "public", "data", "sessions", activeSessionCode);
            await updateDoc(sessionRef, teamProgress.progressVersion === 2 ? {
              [`escapeRoomState.teamProgress.${userTeam}.revealedHints.${alloLiveFieldKey(currentPuzzle.id)}`]: true,
              [`escapeRoomState.teamProgress.${userTeam}.hintedAt.${alloLiveFieldKey(currentPuzzle.id)}`]: Date.now()
            } : {
            ...
            setSyncError("The hint could not be saved. Try again.");
          } finally {
            submissionRef.current = false;
            setIsSubmitting(false);
          }
        ...
        className: `flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-colors motion-reduce:transition-none whitespace-nowrap ${teamHintsRemaining > 0 ? "bg-yellow-500/20 text-yellow-300 hover:bg-yellow-500/30 border border-yellow-500/40" : "bg-slate-700 text-slate-300 cursor-not-allowed"}`
      },
      ...
      opt
    ))), (currentPuzzle.type === "cipher" || currentPuzzle.type === "riddle") && /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, currentPuzzle.encodedText && /* @__PURE__ */ React.createElement("div", { className: "bg-slate-900 p-4 rounded-lg font-mono text-purple-300 text-center" }, currentPuzzle.encodedText), /* @__PURE__ */ React.createElement(
      "input",
      {
    ...
    )), currentPuzzle.type === "fillin" && /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, currentPuzzle.sentence && /* @__PURE__ */ React.createElement("div", { className: "bg-slate-900 p-4 rounded-lg text-white text-center text-lg" }, currentPuzzle.sentence.replace(/_+/g, userInput ? `[${userInput}]` : "______")), currentPuzzle.wordbank && currentPuzzle.wordbank.length > 0 ? /* @__PURE__ */ React.createElement("div", { className: "space-y-3" }, /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-400 text-center uppercase font-bold" }, t("escape_room.select_word") || "Select the correct word:"), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2 justify-center" }, currentPuzzle.wordbank.map((word, idx) => /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        key: idx,
      ...
      "button",
      {
        ...
        disabled: idx === 0,
        "aria-label": `${t("escape_room.move_up") || "Move up"}: ${currentPuzzle.items[itemIndex]}`,
      ...
      "button",
      {
        ...
        "aria-label": `${t("escape_room.move_down") || "Move down"}: ${currentPuzzle.items[itemIndex]}`,
      ...
      {
        type: "button",
        onClick: () => setSequenceOrder(alloLiveSequenceIndices(currentPuzzle)),
    ...
    )), currentPuzzle.type === "matching" && /* @__PURE__ */ React.createElement("div", { className: "space-y-4", "data-help-key": "escape_room_matching_container" }, /* @__PURE__ */ React.createElement("p", { className: "text-sm text-purple-200 italic mb-2" }, t("escape_room.matching_instructions")), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-4", role: "group", "aria-label": t("escape_room.matching_columns") || "Matching columns" }, /* @__PURE__ */ React.createElement("div", { className: "space-y-2", role: "group", "aria-label": t("escape_room.left_column") || "Left column options" }, (currentPuzzle.leftColumn || currentPuzzle.pairs?.map((p) => p.left) || []).map((item, idx) => {
      ...
      return /* @__PURE__ */ React.createElement(
        "button",
        {
          ...
          className: `w-full p-3 rounded-lg text-left font-medium transition-all motion-reduce:transition-none focus:outline-none focus:ring-2 focus:ring-purple-400 ${isMatched ? "bg-green-700 text-white opacity-60" : isSelected ? "bg-purple-600 text-white ring-2 ring-purple-300" : "bg-slate-700 text-white hover:bg-slate-600"}`
        },
        item
      );
...
const ConnectedEscapeRoomLoader = React.memo(({ variant, ...props }) => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (window.AlloModules?.ConnectedEscapeRoomModule) setReady(true);
    }, 250);
    return () => clearInterval(timer);
  }, [ready]);
  const View = window.AlloModules?.["ConnectedEscapeRoom" + variant];
  return ready && View ? /* @__PURE__ */ React.createElement(View, { ...props }) : /* @__PURE__ */ React.createElement("div", { role: "status", className: variant === "Student" ? "fixed inset-0 z-[9999] bg-slate-950 p-8 text-white" : "rounded-xl bg-indigo-50 p-4 text-indigo-900" }, props.t?.("connected_escape.loading", { defaultValue: "Loading the connected room\u2026" }) || "Loading the connected room\u2026");
});
const LessonBoardLoader = React.memo(({ variant, ...props }) => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (window.AlloModules?.LessonBoardModule) setReady(true);
    }, 250);
    return () => clearInterval(timer);
  }, [ready]);
  const View = window.AlloModules?.["LessonBoard" + variant];
  return ready && View ? /* @__PURE__ */ React.createElement(View, { ...props }) : /* @__PURE__ */ React.createElement("div", { role: "status" }, props.t?.("lesson_board.loading", { defaultValue: "Loading the lesson board\u2026" }) || "Loading the lesson board\u2026");
});
const StudentEscapeRoomOverlay = React.memo((props) => props.sessionData?.escapeRoomState?.mode === "lesson-board" ? /* @__PURE__ */ React.createElement(LessonBoardLoader, { variant: "Student", ...props }) : props.sessionData?.escapeRoomState?.mode === "connected-room" ? /* @__PURE__ */ React.createElement(ConnectedEscapeRoomLoader, { variant: "Student", ...props }) : props.sessionData?.escapeRoomState?.mode === "concept-quest" ? /* @__PURE__ */ React.createElement(StudentConceptQuestOverlay, { ...props }) : /* @__PURE__ */ React.createElement(ClassicStudentEscapeRoomOverlay, { ...props }));
const ClassicEscapeRoomTeacherControls = React.memo(({ sessionData, activeSessionCode, appId: appId2, t, addToast }) => {
  const escapeState = sessionData?.escapeRoomState || {};
  const timeRemaining = useLiveEscapeSeconds(escapeState);
  const [controlBusy, setControlBusy] = useState(false);
  const [controlError, setControlError] = useState("");
  const controlRef = useRef(false);
  const warningsRef = useRef(/* @__PURE__ */ new Set());
  useEffect(() => {
    warningsRef.current.clear();
  }, [activeSessionCode, escapeState.startedAt]);
  const writeControl = async (patch) => {
    ...
    try {
      ...
      warnLog("Live escape room control failed:", error);
      setControlError("The session could not be updated. Check the connection and try again.");
      return false;
    } finally {
  ...
  useEffect(() => {
    if (!escapeState.isActive || escapeState.isPaused || escapeState.isGameOver || !activeSessionCode || !appId2) return;
    const sessionRef = doc(db, "artifacts", appId2, "public", "data", "sessions", activeSessionCode);
    if (!Number.isFinite(escapeState.endsAt)) {
      updateDoc(sessionRef, { "escapeRoomState.endsAt": Date.now() + Math.max(0, escapeState.timeRemaining || 0) * 1e3 }).catch((error) => {
        warnLog("Live escape clock failed:", error);
        setControlError("The timer could not start. Check the connection and use Resume to retry.");
      });
      return;
    }
    let stopped = false, writing = false;
    const tick = async () => {
      const seconds = alloLiveEscapeSeconds(escapeState);
      [60, 30].forEach((threshold) => {
        if (seconds > 0 && seconds <= threshold && !warningsRef.current.has(threshold)) {
          warningsRef.current.add(threshold);
          addToast?.(t(threshold === 60 ? "escape_room.one_minute_warning" : "escape_room.thirty_seconds_warning"), "warning");
        }
      });
      if (seconds > 0 || stopped || writing) return;
      writing = true;
      try {
        await updateDoc(sessionRef, { "escapeRoomState.timeRemaining": 0, "escapeRoomState.isGameOver": true, "escapeRoomState.gameOverReason": "time" });
        stopped = true;
        addToast?.(t("escape_room.time_up"), "error");
      } catch (error) {
        warnLog("Live escape expiry failed:", error);
        setControlError("Time has ended. Reconnecting to save the result\u2026");
      } finally {
        writing = false;
      }
    };
  ...
  }, [escapeState.isActive, escapeState.isPaused, escapeState.isGameOver, escapeState.endsAt, activeSessionCode, appId2]);
  const isCoopMode = escapeState.isCoopMode || false;
  const isPaused = escapeState.isPaused || false;
  const teamColors = {
  ...
  const escapedTeams = allTeams.filter((team) => alloLiveEscapeProgress(escapeState.teamProgress?.[team], puzzles).isEscaped);
  const studentsAssigned = Object.keys(escapeState.teams || {}).length;
  const handlePauseToggle = async () => {
    if (escapeState.isGameOver) return;
    const remaining = alloLiveEscapeSeconds(escapeState);
    await writeControl({
      "escapeRoomState.isPaused": !isPaused,
      "escapeRoomState.timeRemaining": remaining,
      "escapeRoomState.endsAt": isPaused ? Date.now() + remaining * 1e3 : null
    });
  };
  const handleEndGame = async () => {
    if (await writeControl({ "escapeRoomState.isActive": false, "escapeRoomState.isGameOver": false, "escapeRoomState.isPaused": false, "escapeRoomState.endsAt": null })) setShowEndConfirm(false);
  };
  const formatTime = (seconds) => {
    const m = Math.floor(seconds / 60);
    const s = seconds % 60;
...
const ConceptQuestTeacherControlsLoader = React.memo((props) => {
  ...
  useEffect(() => {
    if (window.AlloModules?.ConceptQuestTeacherControls) {
      setReady(true);
      return;
    }
    ...
    script.onerror = () => {
      script.onerror = null;
      script.src = "./concept_quest_teacher_module.js";
    };
    document.head.appendChild(script);
  }, []);
  const ConceptControls = window.AlloModules?.ConceptQuestTeacherControls;
  return ready && ConceptControls ? /* @__PURE__ */ React.createElement(ConceptControls, { ...props }) : /* @__PURE__ */ React.createElement("div", { role: "status", className: "rounded-xl bg-indigo-50 p-4 text-indigo-800" }, tr("loading_teacher_controls", "Loading Concept Quest co-GM controls\u2026"));
});
const EscapeRoomTeacherControls = React.memo((props) => {
  if (props.sessionData?.escapeRoomState?.mode === "lesson-board") return /* @__PURE__ */ React.createElement(LessonBoardLoader, { variant: "Teacher", ...props });
  if (props.sessionData?.escapeRoomState?.mode === "connected-room") return /* @__PURE__ */ React.createElement(ConnectedEscapeRoomLoader, { variant: "Teacher", ...props });
  if (props.sessionData?.escapeRoomState?.mode === "concept-quest") {
    return /* @__PURE__ */ React.createElement(ConceptQuestTeacherControlsLoader, { ...props });
  }
  return /* @__PURE__ */ React.createElement(ClassicEscapeRoomTeacherControls, { ...props });
});
const TeacherLiveQuizControls = React.memo(({ sessionData, generatedContent, activeSessionCode, appId: appId2, onGenerateImage, onRefineImage, onCreateGroup, onAssignStudent, onSetGroupResource, isPushingResource = {}, onSetGroupLanguage, onSetGroupProfile, onDeleteGroup, onUpdateQuestionRoutingRules, history = [], callGemini, addToast }) => {
  const { t } = useContext(LanguageContext);
  ...
  const quizWriteRef = useRef(false);
  const completedWriteRef = useRef("");
  useEffect(() => {
    completedWriteRef.current = "";
    setQuizError("");
  }, [activeSessionCode, quizState.activityId, quizState.roundId, currentQuestionIndex, phase]);
  const writeQuiz = async (sessionRef, patch) => {
    const key = patch["quizState.phase"] && patch["quizState.phase"] !== "idle" ? [quizState.roundId, currentQuestionIndex, phase, patch["quizState.phase"], patch["quizState.currentQuestionIndex"], patch["quizState.mode"]].join(":") : JSON.stringify(patch);
    if (quizWriteRef.current || completedWriteRef.current === key) return false;
    quizWriteRef.current = true;
    setQuizBusy(true);
    setQuizError("");
    try {
      await updateDoc(sessionRef, patch);
      completedWriteRef.current = key;
      return true;
    } catch (error) {
      warnLog("Live quiz control failed:", error);
      setQuizError("The live quiz could not be updated. Check the connection and try again.");
      return false;
    } finally {
      quizWriteRef.current = false;
      setQuizBusy(false);
    }
  };
  ...
  const handleBossPacingAdjustment = async (kind) => {
    ...
    if (kind === "soften") updates["quizState.bossStats.currentHP"] = Math.max(1, (bossStats.currentHP ?? bossStats.maxHP) - Math.round((bossStats.maxHP || 1e3) * 0.05));
  ...
  const handleEndQuiz = async () => {
    const sessionRef = doc(db, "artifacts", appId2, "public", "data", "sessions", activeSessionCode);
    if (await writeQuiz(sessionRef, { "quizState.isActive": false, "quizState.phase": "closed", "quizState.endedAt": Date.now() })) {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl shadow-xl border border-slate-400 overflow-hidden mb-6 animate-in motion-reduce:animate-none slide-in-from-top-4 duration-500", "aria-busy": quizBusy }, quizError && /* @__PURE__ */ React.createElement("p", { role: "alert", className: "m-3 rounded-lg bg-red-100 p-3 font-bold text-red-900" }, quizError), quizBusy && /* @__PURE__ */ React.createElement("p", { role: "status", className: "px-4 py-2 text-indigo-900" }, "Updating the live quiz\u2026"), /* @__PURE__ */ React.createElement("div", { className: "bg-indigo-900 text-white p-4 flex justify-between items-center flex-wrap gap-4" }, /* @__PURE__ */ React.createElement("h3", { className: "font-bold flex items-center gap-2" }, /* @__PURE__ */ React.createElement(MonitorPlay, { size: 20, className: "text-teal-700" }), " ", t("quiz.live_control_center")), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-4" }, /* @__PURE__ */ React.createElement(
    "select",
    {
      "aria-label": t("common.selection"),
      value: mode || "live-pulse",
      disabled: quizBusy || phase === "answering" || answeredCount > 0,
      title: "Choose a mode before starting an unanswered round.",
    ...
    "select",
    {
      ...
      className: `text-xs font-bold px-3 py-1.5 rounded-lg border focus:outline-none focus:ring-2 focus:ring-teal-400 cursor-pointer ${bossDifficulty === "easy" ? "bg-emerald-700 border-emerald-500 text-white" : bossDifficulty === "hard" ? "bg-red-600 border-red-500 text-white" : "bg-amber-700 border-amber-400 text-white"} ${!["lobby", "idle"].includes(phase) || answeredCount > 0 || (bossStats?.battleLog || []).length > 0 ? "opacity-60 cursor-not-allowed" : ""}`,
    ...
    {
      type: "button",
      "aria-label": showLocalStats ? t("quiz.show_game") : t("quiz.show_stats"),
      "aria-pressed": showLocalStats,
    ...
    if (rule.when && rule.when.confidencePattern) {
      ...
      return /* @__PURE__ */ React.createElement("div", { key: rule.id, className: "bg-white border border-sky-200 rounded p-1.5 text-xs space-y-1" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-center gap-1" }, /* @__PURE__ */ React.createElement("span", { className: "px-1 py-0.5 rounded bg-sky-100 text-sky-800 text-[10px] font-bold uppercase tracking-wide" }, "Confidence"), /* @__PURE__ */ React.createElement("span", { className: "text-slate-700" }, patternLabel, " across ", across.map((i) => `Q${i + 1}`).join(", ")), /* @__PURE__ */ React.createElement("span", { className: "text-slate-600" }, "\u2192"), /* @__PURE__ */ React.createElement(
        ...
        groupEntriesForRouting.map(([gid, g]) => /* @__PURE__ */ React.createElement("option", { key: gid, value: gid }, g.name || gid))
      ), /* @__PURE__ */ React.createElement(
        "button",
        {
          type: "button",
    ...
    {
      type: "button",
      "aria-label": t("quiz.reveal_results"),
...
const LearnerProgressView = React.memo(({
  ...
  gameCompletions = Array.isArray(gameCompletions) ? gameCompletions : Object.values(gameCompletions || {}).flat().filter((g) => g && typeof g === "object");
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "max-w-4xl mx-auto p-4 md:p-6 space-y-6 animate-in motion-reduce:animate-none fade-in slide-in-from-bottom-2 duration-500", "data-help-key": "learner_progress_panel" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h2", { className: "text-2xl md:text-3xl font-black text-slate-800 flex items-center gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "w-10 h-10 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl flex items-center justify-center shadow-lg" }, /* @__PURE__ */ React.createElement(TrendingUp, { size: 22, className: "text-white" })), heading), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-slate-600 mt-1 font-medium" }, isParentMode ? "Track your family's learning growth" : "Track your learning growth over time")), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2" }, /* @__PURE__ */ React.createElement(
    ...
    return dayStreak >= 2;
  })()) && /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-3 mb-2 px-3 py-2 bg-gradient-to-r from-orange-50 to-red-50 rounded-xl border border-orange-100" }, wordSoundsScore?.streak > 0 && /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-1" }, /* @__PURE__ */ React.createElement("span", { className: "text-lg" }, "\u{1F525}"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-sm font-black text-orange-600" }, wordSoundsScore.streak, " Streak"), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-orange-700 font-bold uppercase" }, t("learner.current_run")))), (() => {
    const days = /* @__PURE__ */ new Set();
    pointHistory.forEach((e) => {
      if (e.timestamp) days.add(new Date(e.timestamp).toDateString());
    });
    ...
    { label: "Quizzes", value: stats.quizCount, icon: "\u{1F4DD}", color: "bg-blue-50 text-blue-800" },
    { label: "Words Practiced", value: stats.wsTotal, icon: "\u{1F524}", color: "bg-purple-50 text-purple-800" },
    { label: "Games", value: stats.gamesPlayed, icon: "\u{1F3AE}", color: "bg-green-50 text-green-800" },
    { label: "Fluency Tests", value: stats.fluencyTests, icon: "\u23F1\uFE0F", color: "bg-orange-50 text-orange-800" }
  ].map((item) => /* @__PURE__ */ React.createElement("div", { key: item.label, className: `${item.color} rounded-lg px-3 py-2 text-center` }, /* @__PURE__ */ React.createElement("div", { className: "text-lg font-black" }, item.icon, " ", item.value), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold uppercase tracking-wider" }, item.label))))), /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border border-slate-400 shadow-sm p-5 hover:shadow-md transition-shadow motion-reduce:transition-none", "data-help-key": "learner_progress_skills_chart" }, /* @__PURE__ */ React.createElement("h3", { className: "text-xs font-bold text-slate-600 uppercase tracking-wider mb-3 flex items-center gap-1.5" }, /* @__PURE__ */ React.createElement(Target, { size: 14 }), " Skills Progress"), stats.wsTotal === 0 && Object.keys(phonemeMastery).length === 0 ? /* @__PURE__ */ React.createElement("div", { className: "text-center py-6 text-slate-600 italic text-sm" }, /* @__PURE__ */ React.createElement(BookOpen, { size: 32, className: "mx-auto mb-2 opacity-40" }), "Start practicing to see your skills grow!") : /* @__PURE__ */ React.createElement("div", { className: "space-y-3" }, stats.wsGradedTotal > 0 && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "flex justify-between items-center mb-1" }, /* @__PURE__ */ React.createElement("span", { className: "text-sm font-bold text-slate-600" }, t("learner.ws_accuracy")), /* @__PURE__ */ React.createElement("span", { className: `text-sm font-black ${stats.wsAccuracy >= 80 ? "text-green-600" : stats.wsAccuracy >= 60 ? "text-yellow-600" : "text-orange-500"}` }, stats.wsAccuracy, "%")), /* @__PURE__ */ React.createElement("div", { className: "w-full bg-slate-100 rounded-full h-2.5" }, /* @__PURE__ */ React.createElement(
    "div",
    {
  ...
  )), /* @__PURE__ */ React.createElement("div", { className: "flex justify-between items-center mt-1.5" }, /* @__PURE__ */ React.createElement("span", { className: "text-[11px] font-bold text-slate-600 uppercase tracking-wider" }, t("learner.ws_first_try_accuracy")), /* @__PURE__ */ React.createElement("span", { className: "text-[11px] font-black text-slate-700" }, stats.wsFirstTryAccuracy, "% ", /* @__PURE__ */ React.createElement("span", { className: "font-bold text-slate-600" }, "(", stats.wsFirstTryCorrect, "/", stats.wsGradedTotal, ")"))), stats.wsHasPracticeOnly && /* @__PURE__ */ React.createElement("div", { className: "text-[10px] text-slate-600 mt-1 leading-snug" }, t("learner.ws_practice_only_note"))), stats.masteredPhonemes.length > 0 && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-green-600 uppercase tracking-wider mb-1.5" }, "\u2728 ", stats.masteredPhonemes.length, " Sounds Mastered"), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-1" }, stats.masteredPhonemes.slice(0, 12).map(([phoneme]) => /* @__PURE__ */ React.createElement("span", { key: phoneme, className: "px-2 py-0.5 bg-green-50 text-green-700 text-xs font-bold rounded-full border border-green-200" }, "/", phoneme, "/")), stats.masteredPhonemes.length > 12 && /* @__PURE__ */ React.createElement("span", { className: "px-2 py-0.5 bg-green-50 text-green-800 text-xs font-bold rounded-full" }, "+", stats.masteredPhonemes.length - 12, " more"))), showDiagnostics && stats.practicingPhonemes.length > 0 && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-amber-600 uppercase tracking-wider mb-1.5" }, "\u{1F504} ", stats.practicingPhonemes.length, " Sounds In Progress"), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-1" }, stats.practicingPhonemes.slice(0, 8).map(([phoneme, data]) => /* @__PURE__ */ React.createElement("span", { key: phoneme, className: "px-2 py-0.5 bg-amber-50 text-amber-700 text-xs font-bold rounded-full border border-amber-200", title: `${Math.round(data.accuracy)}% accuracy` }, "/", phoneme, "/ ", /* @__PURE__ */ React.createElement("span", { className: "opacity-60" }, Math.round(data.accuracy), "%"))))))), /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border border-slate-400 shadow-sm p-5 hover:shadow-md transition-shadow motion-reduce:transition-none" }, /* @__PURE__ */ React.createElement("h3", { className: "text-xs font-bold text-slate-600 uppercase tracking-wider mb-3 flex items-center gap-1.5" }, /* @__PURE__ */ React.createElement(Award, { size: 14 }), " Achievements"), (() => {
    const milestones = [
  ...
  const handleBulkExportNotebooksPDF = async () => {
    ...
    const reportHtml = `
          <div style="font-family:Helvetica,sans-serif; padding:30px; color:#1e293b; font-size:11px; line-height:1.5;">
              ...
              <p style="font-size:11px; color:#64748b; margin-top:0;">${students.length} students \xB7 ${(/* @__PURE__ */ new Date()).toLocaleDateString()} \xB7 Generated by AlloFlow</p>
              ${studentSections}
          </div>
      `;
    ...
    try {
      await doc2.html(container, {
        callback: (d) => {
          d.save(`class_notebooks_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.pdf`);
          if (addToast) addToast(t("dashboard.bulk.notebooks_pdf_saved") || "Class notebooks PDF saved.", "success");
        },
        x: 8,
        y: 8,
        width: 195,
...
`.trim();
    ...
    const sections = results.map((r, i) => `
          <div style="margin-bottom:30px; padding:15px; background:#fffbeb; border-left:4px solid #f59e0b; ${i > 0 && i % 3 === 0 ? "page-break-before:always;" : ""}">
              <div style="font-size:13px; font-weight:bold; color:#92400e; margin-bottom:6px;">\u{1F4E8} ${r.name}</div>
    ...
    const reportHtml = `
          <div style="font-family:Helvetica,sans-serif; padding:30px; color:#1e293b; font-size:11px;">
              ...
              <p style="font-size:10px; color:#64748b;">Cut along the dashed lines and hand back to each student. ${(/* @__PURE__ */ new Date()).toLocaleDateString()}</p>
              ${sections}
          </div>
      `;
    const container = document.createElement("div");
    ...
    try {
      await doc2.html(container, {
        callback: (d) => {
          d.save(`class_feedback_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.pdf`);
          if (addToast) addToast(t("dashboard.bulk.feedback_pdf_saved") || "Feedback PDF saved.", "success");
        },
        x: 8,
        y: 8,
        width: 195,
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    /* @__PURE__ */ React.createElement("div", { className: "bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm shrink-0 z-10 flex flex-col" }, /* @__PURE__ */ React.createElement("div", { className: "p-3 sm:p-4 flex flex-wrap justify-between items-center gap-2" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2 sm:gap-3 min-w-0" }, /* @__PURE__ */ React.createElement("div", { className: "bg-indigo-100 p-2 rounded-lg text-indigo-600 shrink-0" }, /* @__PURE__ */ React.createElement(Layout, { size: 24 })), /* @__PURE__ */ React.createElement("h2", { id: "teacher-grading-dashboard-title", className: "text-base sm:text-xl font-black text-slate-800 truncate" }, t("dashboard.grading_dashboard"))), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-1 sm:gap-2 flex-wrap" }, dashboardData.length > 0 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(
      ...
      " ",
      /* @__PURE__ */ React.createElement("span", { className: "hidden sm:inline" }, t("dashboard.export_csv")),
      /* @__PURE__ */ React.createElement("span", { className: "sm:hidden" }, "CSV")
    ), /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
      ...
      /* @__PURE__ */ React.createElement(FileDown, { size: 14 }),
      " ",
      /* @__PURE__ */ React.createElement("span", { className: "hidden sm:inline" }, "\u{1F4CA} Research PDF"),
      /* @__PURE__ */ React.createElement("span", { className: "sm:hidden" }, "\u{1F4CA} PDF")
      ...
      /* @__PURE__ */ React.createElement("span", { className: "hidden sm:inline" }, t("dashboard.clear_all")),
    ...
    )) : /* @__PURE__ */ React.createElement(React.Fragment, null, activeTab === "students" && /* @__PURE__ */ React.createElement("div", { role: "tabpanel", id: "teacher-dashboard-panel-students", "aria-labelledby": "teacher-dashboard-tab-students", tabIndex: 0, className: "max-w-6xl mx-auto space-y-6 animate-in motion-reduce:animate-none fade-in slide-in-from-left-4 focus:outline-none focus:ring-2 focus:ring-indigo-500" }, /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-4" }, /* @__PURE__ */ React.createElement("div", { className: "bg-white p-4 rounded-xl shadow-sm border border-slate-400 flex items-center gap-4" }, /* @__PURE__ */ React.createElement("div", { className: "bg-blue-100 p-3 rounded-full text-blue-800" }, /* @__PURE__ */ React.createElement(Users, { size: 24 })), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-2xl font-black text-slate-800" }, dashboardData.length), /* @__PURE__ */ React.createElement("div", { className: "text-xs font-bold text-slate-600 uppercase" }, t("dashboard.stats.students_loaded")))), /* @__PURE__ */ React.createElement("div", { className: "bg-white p-4 rounded-xl shadow-sm border border-slate-400 flex items-center gap-4 relative group cursor-pointer hover:border-indigo-300 transition-colors motion-reduce:transition-none" }, /* @__PURE__ */ React.createElement("div", { className: "bg-green-100 p-3 rounded-full text-green-800" }, /* @__PURE__ */ React.createElement(Upload, { size: 24 })), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-sm font-bold text-green-700" }, t("dashboard.stats.add_files")), /* @__PURE__ */ React.createElement("div", { className: "text-xs text-slate-600" }, t("dashboard.stats.click_upload"))), /* @__PURE__ */ React.createElement(
      "input",
      {
        "aria-label": t("common.upload_json_file"),
        type: "file",
...
window.AlloModules.LiveEscapeRoomInternals = { progress: alloLiveEscapeProgress, seconds: alloLiveEscapeSeconds, answer: alloLiveEscapeAnswer, sequence: alloLiveSequenceIndices };
