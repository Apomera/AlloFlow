...
(function() {
  ...
  window.StemLab.registerTool('base10', {
    ...
    render: function(ctx) {
      ...
      // The control strip sits on the HOST surface - white in light and dark,
      // pure BLACK in the contrast theme, where these ran 2.8-3.0:1.
      ...
      var upd = function(obj) {
        if (typeof ctx.setToolData === 'function') {
          ctx.setToolData(function(prev) {
            var current = (prev && prev._manipulatives) || {};
            var changes = typeof obj === 'function' ? obj(current) : obj;
            changes = Object.assign({}, changes);
            if (changes.b10 && !Object.prototype.hasOwnProperty.call(changes, 'b10Trade')) changes.b10Trade = null;
            if (changes.b10 && !Object.prototype.hasOwnProperty.call(changes, 'b10Feedback')) changes.b10Feedback = null;
            var mn = Object.assign({}, current, changes);
      ...
      var doRegroup = function(fromPlace, toPlace) {
        ...
        upd({ b10: newB10, b10Trade: { before: Object.assign({}, b10), after: Object.assign({}, newB10), from: fromPlace, to: toPlace, removed: 10, added: 1 }, regroupFlash: toPlace, regroupCount: newRC });
      ...
      var doUngroup = function(fromPlace, toPlace) {
        if (exchangePending || b10[fromPlace] < 1) return;
        exchangePending = true;
        var newB10 = Object.assign({}, b10);
        newB10[fromPlace] = b10[fromPlace] - 1;
        newB10[toPlace] = b10[toPlace] + 10;
        var newRC = regroupCount + 1;
        upd({ b10: newB10, b10Trade: { before: Object.assign({}, b10), after: Object.assign({}, newB10), from: fromPlace, to: toPlace, removed: 1, added: 10 }, regroupFlash: toPlace, regroupCount: newRC });
        setTimeout(function() { upd({ regroupFlash: null }); }, 700);
        if (soundEnabled) sfxRegroup();
        checkBadges({ regroupCount: newRC });
      };
      ...
      var blockPlaces = ['thousands', 'hundreds', 'tens', 'ones'];
      var blockUnits = { thousands: 1000, hundreds: 100, tens: 10, ones: 1 };
      var blockNames = {
        ones: [__alloT('stem.manipulatives.unit_one', 'one'), __alloT('stem.manipulatives.unit_ones', 'ones')],
        tens: [__alloT('stem.manipulatives.unit_ten', 'ten'), __alloT('stem.manipulatives.unit_tens', 'tens')],
        hundreds: [__alloT('stem.manipulatives.unit_hundred', 'hundred'), __alloT('stem.manipulatives.unit_hundreds', 'hundreds')],
        thousands: [__alloT('stem.manipulatives.unit_thousand', 'thousand'), __alloT('stem.manipulatives.unit_thousands', 'thousands')]
      };
      function blockTotal(counts) { return blockPlaces.reduce(function(sum, place) { return sum + counts[place] * blockUnits[place]; }, 0); }
      function blockExpression(counts) { return blockPlaces.filter(function(place) { return counts[place] > 0; }).map(function(place) { return counts[place] + ' × ' + blockUnits[place]; }).join(' + ') || '0'; }
      function sameBlocks(a, b) { return a && b && blockPlaces.every(function(place) { return a[place] === b[place]; }); }
      function tradeComparison() {
        var trade = _m.b10Trade;
        if (!trade || !sameBlocks(trade.after, b10) || !trade.before || !blockUnits[trade.from] || !blockUnits[trade.to]) return null;
        var total = blockTotal(trade.after);
        return h('section', { className: 'bg-white rounded-xl border border-violet-200 p-3 mt-3', 'data-block-trade': true },
          h('h3', { className:'text-sm font-bold text-violet-800' }, __alloT('stem.manipulatives.trade_what_changed', 'What changed in this trade?')),
          h('p', { role:'status', className:'text-sm text-slate-800 mt-2', 'data-trade-equivalence':true }, trade.removed + ' × ' + blockUnits[trade.from] + ' = ' + trade.added + ' × ' + blockUnits[trade.to] + '. ' + __alloT('stem.manipulatives.trade_total_unchanged', 'The total stays the same: ') + total + '.'),
          h('div', { className:'grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3' },
            [{ label:__alloT('stem.manipulatives.before_trade', 'Before the trade'), counts:trade.before }, { label:__alloT('stem.manipulatives.after_trade', 'After the trade'), counts:trade.after }].map(function(item, index) {
              return h('div', { key:index, className:'rounded-lg border border-slate-300 p-3', 'data-trade-side':index ? 'after' : 'before' },
                h('h4', {className:'text-sm font-bold text-slate-800'}, item.label),
                h('ul', {className:'text-sm text-slate-700 mt-2 space-y-1'}, blockPlaces.filter(function(place) { return item.counts[place] || trade.before[place] !== trade.after[place]; }).map(function(place) {
                  var changed = trade.before[place] !== trade.after[place];
                  return h('li', {key:place, style:{fontWeight:changed ? 800 : 400}}, item.counts[place]+' '+blockNames[place][item.counts[place]===1?0:1] + (changed ? ' ←' : ''));
                })),
                h('p', {className:'text-sm font-mono text-slate-800 mt-2'}, blockExpression(item.counts)+' = '+blockTotal(item.counts)));
            })),
          h('p', {className:'text-xs text-slate-700 mt-2'}, __alloT('stem.manipulatives.trade_changed_places', 'The arrows mark the places that changed. The pieces changed; the quantity did not.')),
          h('button', {type:'button', className:'rounded-lg border border-violet-700 text-violet-800 px-3 py-2 text-sm font-bold mt-3', onClick:function() {
            upd(function(current) { var latest=current.b10Trade; return latest && sameBlocks(current.b10, latest.after) ? { b10:Object.assign({}, latest.before), b10Trade:null, regroupFlash:null } : {}; });
          }}, __alloT('stem.manipulatives.undo_trade', 'Undo this trade')));
      }
      ...
      var placeCol = function(label, symbol, place, color, lightColor, bw, bh, gridC, gridR) {
        return h('div', { 
          ...
          h('div', { className: 'flex items-center justify-center gap-1 sm:gap-2' },
            h('button', { disabled:b10[place] === 0, 'aria-label': __alloT('stem.manipulatives.remove_block_from', 'Remove one block from ') + label,
            ...
            h('button', { disabled:b10[place] >= 20, 'aria-describedby':b10[place] >= 20 ? 'b10-add-limit-'+place : undefined, 'aria-label': __alloT('stem.manipulatives.add_block_to', 'Add one block to ') + label,
              onClick: function() {
                if (b10[place] >= 20) return;
          ...
          b10[place] >= 20 && h('p', { id:'b10-add-limit-'+place, className:'text-xs text-slate-700 mt-2' }, __alloT('stem.manipulatives.add_after_trade', 'Trade or remove blocks from this place before adding more.')),
      ...
      var regroupBtn = function(label, from, to, enabled, colorFrom, colorTo) {
        var grouping = blockUnits[to] > blockUnits[from];
        var tradeLabel = (grouping ? '10 ' + blockNames[from][1] + ' → 1 ' + blockNames[to][0] : '1 ' + blockNames[from][0] + ' → 10 ' + blockNames[to][1]);
        return h('button', { type:'button', 'aria-label':tradeLabel, onClick: function() { if (grouping) doRegroup(from, to); else doUngroup(from, to); },
          disabled: !enabled,
          ...
          style: enabled ? { minHeight:44, background: 'linear-gradient(90deg, ' + colorFrom + ', ' + colorTo + ')' } : { minHeight:44 }
        }, tradeLabel);
      };
      ...
      var headerEl = h('div', { className: 'space-y-3 mb-4' },
            ...
            h('div', { className: 'flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between' },
              h('div', { className: 'min-w-0' },
                h('div', { className: 'flex items-center gap-2' },
                  ...
                  h('span', { className: 'rounded-full bg-orange-300/15 px-2.5 py-1 text-[0.625rem] font-black uppercase tracking-[0.18em] text-orange-100 ring-1 ring-orange-200/30' }, 'Representation studio')
                ),
                ...
                h('p', { className: 'mt-2 text-sm text-white' }, manipNext)
              ),
              h('div', { className: 'hidden sm:grid grid-cols-3 gap-2 lg:w-[22rem]' },
                [
                  ...
                  return h('div', { key: metric.label, className: 'min-w-0 rounded-xl border border-white/15 bg-white/10 px-2 py-3 text-center' },
                    ...
                    h('div', { className: 'mt-1 text-[0.625rem] font-bold uppercase tracking-wider text-orange-200' }, metric.label)
                  );
                })
              )
            ),
            h('details', { className: 'mt-3 text-white' }, h('summary', { className: 'cursor-pointer text-sm font-bold' }, __alloT('stem.manipulatives.learning_steps', 'Learning steps')), h('ol', { className: 'mt-4 grid gap-2 text-xs sm:grid-cols-3', 'aria-label': __alloT('stem.manipulatives.a11y_manipulative_reasoning_pathway', 'Manipulative reasoning pathway') },
              [
        ...
        h('div', { className: 'flex flex-wrap items-center justify-end gap-2' },
          ...
          h('div', { className: 'flex gap-1.5' },
            h('span', { className: 'text-xs font-bold text-emerald-700 self-center', style: { color: ctx.isContrast ? '#ffff00' : undefined } }, score.correct + '/' + score.total),
            h('button', { 'aria-label': __alloT('stem.manipulatives.badges', 'Badges'), onClick: function() { upd({ showBadgesPanel: !showBadgesPanel }); }, className: 'text-[0.6875rem] font-bold px-2 py-0.5 rounded-full border transition-all ' + (showBadgesPanel ? 'bg-amber-100 border-amber-600 text-amber-700' : 'bg-slate-100 border-slate-200 text-slate-600') }, '\uD83C\uDFC5 ' + Object.keys(earnedBadges).length + '/' + badgeDefs.length),
            ...
            callGemini && h('button', { 'aria-label': aiLoading ? 'AI Tutor thinking' : 'Ask AI Tutor', 'aria-busy': aiLoading, onClick: askAI, disabled: aiLoading, className: 'text-[0.6875rem] font-bold px-2 py-0.5 rounded-full border transition-all ' + (aiLoading ? 'bg-pink-100 border-pink-700 text-pink-700' : 'bg-pink-50 border-pink-700 text-pink-700 hover:bg-pink-100') }, aiLoading ? '\u23F3' : '\uD83E\uDD16 Tutor')
          )
        ),
            ...
            badgeDefs.map(function(badge) {
              ...
              return h('div', { key: badge.id, className: 'flex items-center gap-2 p-1.5 rounded-lg ' + (earned ? 'bg-amber-100 border border-amber-300' : 'bg-white border border-slate-400 opacity-40') },
                ...
                h('div', null,
                  ...
                  h('div', { className: 'text-[0.6875rem] ' + (earned ? 'text-amber-600' : 'text-slate-600') }, __alloT('stem.manipulatives.' + (badge.id) + '_desc', badge.desc))
                )
              );
            })
          )
        ...
        (function() {
          var choices = [{ id: 'blocks',        icon: '\uD83E\uDDF1', label: __alloT('stem.manipulatives.base_10_blocks', 'Base-10 Blocks') },
          ...
          var groups = [
            { label: __alloT('stem.manipulatives.group_count_place', 'Counting & place value'), ids: ['blocks','abacus','tenFrame','counters','pvDisks','hundredsChart','numberBonds'] },
            { label: __alloT('stem.manipulatives.group_fraction_relationships', 'Fractions & relationships'), ids: ['fracBars','cRods','slideRule','algebraTiles','cra'] },
            { label: __alloT('stem.manipulatives.group_shape_space', 'Shapes & space'), ids: ['patternBlocks','geoboard'] },
            { label: __alloT('stem.manipulatives.group_practice_inquiry', 'Practice & inquiry'), ids: ['quiz','challenges','puzzles','activities','inquiry'] },
            { label: __alloT('stem.manipulatives.group_teacher_reference', 'Teacher & reference'), ids: ['teacher','curriculum','history','help','glossary','standards','library'] }
          ];
          var activeGroup = groups.filter(function(group) { return group.ids.indexOf(manipMode) >= 0; })[0] || groups[0];
          return h('nav', { className: 'rounded-xl border bg-white p-3 space-y-2', 'aria-label': __alloT('stem.manipulatives.tool_navigation', 'Math manipulative tools') },
            h('label', { className: 'block text-sm font-bold text-slate-800' }, __alloT('stem.manipulatives.choose_tool', 'Choose a tool by learning goal'),
              h('select', { value: manipMode, onChange: function(e) { switchMode(e.target.value); }, className: 'mt-2 w-full rounded-lg border px-3 py-2 text-sm text-slate-900 bg-white' }, groups.map(function(group) {
      ...
      if (manipMode === 'blocks') {
        return h('div', { className: 'space-y-4 max-w-3xl mx-auto animate-in fade-in duration-200' },
          headerEl,
          totalValue === 0 && !b10Challenge && h('div', { className: 'rounded-xl border border-orange-300 bg-orange-50 p-3 text-orange-950' },
            h('p', { className: 'text-sm mb-2' }, __alloT('stem.manipulatives.first_exchange_prompt', 'Try this: build 14 ones, then trade ten ones for one ten. Does the total change?')),
            h('button', {type:'button',className:'rounded-lg border border-orange-700 px-3 py-2 text-sm font-bold',onClick:function(){upd({b10:{ones:14,tens:0,hundreds:0,thousands:0},b10Challenge:null,b10Feedback:null,b10AddMode:false,b10Addends:null});}},__alloT('stem.manipulatives.start_fourteen','Start with 14 ones'))
          ),
          ...
          h('div', { className: 'bg-gradient-to-b from-orange-50 to-amber-50 rounded-xl border-2 border-orange-200 p-6' },
            ...
            h('div', { className: 'text-center mb-4' },
              h('span', { className: 'text-4xl font-bold text-orange-800 font-mono' + onHostInk }, totalValue.toLocaleString()),
              showExpanded && totalValue > 0 && h('div', { className: 'text-xs text-orange-800 font-mono mt-1' }, expandedForm(totalValue)),
              showExpanded && totalValue > 0 && totalValue < 10000 && h('div', { className: 'text-[0.6875rem] text-orange-800 italic mt-0.5' }, '"' + numberWords(totalValue) + '"'),
              ...
              h('div', { className: 'flex items-center justify-center gap-2 mb-2' },
                h('button', {
                  ...
                  className: 'px-3 py-1 text-[0.6875rem] font-bold rounded-lg border transition-colors ' + (b10Solid ? 'border-pink-400 bg-pink-600 text-white' : 'border-orange-300 bg-white text-orange-700 hover:bg-orange-50')
                ...
                b10Solid && [['Turn left', { rotY: b10Rot.rotY - 25 }], ['Turn right', { rotY: b10Rot.rotY + 25 }],
                  ...
                  .map(function(b, i) {
                    return h('button', {
                      ...
                      className: 'w-8 h-7 text-[0.6875rem] rounded border border-orange-300 bg-white text-orange-700 hover:bg-orange-50'
                    }, ['◀', '▶', '▲', '▼'][i]);
                  })
              ),
              b10Solid && h('div', {
            ...
            h('div', { className: 'flex items-center justify-center gap-4 mb-3 text-[0.6875rem] font-bold text-slate-600' },
            ...
            h('div', { className: 'bg-gradient-to-r from-violet-50 to-fuchsia-50 rounded-xl border border-violet-200 p-3 mt-1' },
              h('p', { className: 'text-[0.6875rem] font-bold text-violet-700 uppercase tracking-wider mb-2 text-center' }, __alloT('stem.manipulatives.regroup_ungroup', '\u21C4 Regroup / Ungroup')),
              ...
              tradeComparison(),
              h('p', { className: 'text-[0.6875rem] text-violet-700 text-center mt-1.5 italic' }, __alloT('stem.manipulatives.10_of_one_place_value_always_equals_1_', '\uD83D\uDCA1 10 of one place value always equals 1 of the next!'))
            )
          ),

          // Difficulty selector
          h('div', { className: 'flex gap-1.5 items-center flex-wrap' },
            h('span', { className: 'text-[0.6875rem] font-bold text-slate-600' + onHostInk }, 'Difficulty:'),
            [{ id: 'ones', label: '1\u20139', color: '#c2410c' }, { id: 'tens', label: '10\u201399', color: '#047857' }, { id: 'hundreds', label: '100\u2013999', color: '#1d4ed8' }, { id: 'thousands', label: '1K\u20139K', color: '#be185d' }, { id: 'any', label: 'Any', color: '#475569' }].map(function(dl) {
              return h('button', { 'aria-label': __alloT('stem.manipulatives.challenge_range', 'Challenge range: ') + dl.label, 'aria-pressed':diffLevel === dl.id, key: dl.id, onClick: function() { upd({ diffLevel: dl.id }); },
                className: 'px-2.5 py-1 rounded-lg text-[0.6875rem] font-bold transition-all ' + (diffLevel === dl.id ? 'text-white shadow' : 'bg-white border border-slate-400 text-slate-600 hover:bg-slate-50'),
                style: diffLevel === dl.id ? { backgroundColor: dl.color } : {}
              }, dl.label);
            }),
            h('label', { className: 'flex items-center gap-1 text-[0.6875rem] font-bold text-orange-800 cursor-pointer ml-auto' + onHostInk },
          ...
          b10Challenge && !b10AddMode && h('div', { className: 'bg-orange-50 rounded-lg p-3 border border-orange-200' },
            ...
            h('div', { className: 'flex gap-2 items-center' },
              ...
              h('button', { 'aria-label': __alloT('stem.manipulatives.check', 'Check'), onClick: checkBase10, className: 'ml-auto px-4 py-1.5 bg-orange-700 text-white font-bold rounded-lg text-sm hover:bg-orange-800 transition-all' }, __alloT('stem.manipulatives.check_2', '\u2714 Check'))
            ),
          ...
          b10AddMode && b10Challenge && h('div', { className: 'bg-orange-50 rounded-lg p-3 border border-orange-200' },
            h('div', { className: 'flex gap-2 items-center' },
              ...
              h('button', { 'aria-label': __alloT('stem.manipulatives.check_sum', 'Check Sum'), onClick: checkBase10, className: 'ml-auto px-4 py-1.5 bg-emerald-700 text-white font-bold rounded-lg text-sm hover:bg-emerald-800 transition-all' }, __alloT('stem.manipulatives.check_sum_2', '\u2714 Check Sum'))
            ),
            b10Feedback && h('p', { className: 'text-sm font-bold mt-2 ' + (b10Feedback.correct ? 'text-green-600' : 'text-red-600') }, b10Feedback.msg)
          )
        );
      ...
      if (manipMode === 'abacus') {
        ...
        return h('div', { className: 'space-y-4 max-w-3xl mx-auto animate-in fade-in duration-200' },
          ...
          // Total + speed timer
          h('div', { className: 'text-center' },
            h('span', { className: 'text-4xl font-bold font-mono text-amber-800' + onHostInk }, abacusTotal.toLocaleString()),
            speedChallenge && h('div', { className: 'text-xs font-bold text-rose-500 mt-1' }, '\u23F1\uFE0F Target: ' + speedChallenge.target.toLocaleString() + ' \u2014 GO!'),
            speedBest && h('span', { className: 'text-[0.6875rem] text-amber-500 ml-2' }, '\uD83C\uDFC6 Best: ' + speedBest.toFixed(1) + 's')
          ),
          // Abacus frame
          h('div', {
            ...
            h('div', { className: 'flex justify-center gap-6' },
              rods.slice().reverse().map(function(val, ri) {
                ...
                return h('div', { key: rodIdx, className: 'flex flex-col items-center', style: { width: '48px' } },
                  ...
                  h('div', { className: 'text-[0.6875rem] font-bold mt-1', style: { color: rodColor } }, placeNames[rodIdx]),
          ...
          abacusChallenge && h('div', { className: 'bg-amber-50 rounded-lg p-3 border border-amber-200' },
            ...
            h('div', { className: 'flex gap-2 items-center' },
              ...
              h('button', { 'aria-label': __alloT('stem.manipulatives.check_3', 'Check'), onClick: checkAbacus, className: 'ml-auto px-4 py-1.5 bg-amber-700 text-white font-bold rounded-lg text-sm hover:bg-amber-800 transition-all' }, __alloT('stem.manipulatives.check_4', '\u2714 Check'))
            ),
          ...
          speedChallenge && h('div', { className: 'bg-rose-50 rounded-lg p-3 border border-rose-200' },
            ...
            h('div', { className: 'flex gap-2 items-center' },
              ...
              h('button', { 'aria-label': __alloT('stem.manipulatives.a11y_done', 'Done!'), onClick: checkSpeed, className: 'ml-auto px-4 py-1.5 bg-rose-700 text-white font-bold rounded-lg text-sm hover:bg-rose-600 transition-all' }, __alloT('stem.manipulatives.done', '\u2714 Done!'))
            )
          ),

          // Culture cards
          h('div', null,
            h('button', { 'aria-label': __alloT('stem.manipulatives.toggle_cultural_context_panel', 'Toggle cultural context panel'), onClick: function() { upd({ showCulture: !showCulture }); }, className: 'text-[0.6875rem] font-bold ' + (showCulture ? 'text-amber-600' : 'text-slate-600') + 'transition-colors  hover:text-amber-600' }, (showCulture ? '\u25B2' : '\u25BC') + ' \uD83C\uDF0F Abacus Around the World'),
      ...
      if (manipMode === 'slideRule') {
        ...
        return h('div', { className: 'space-y-4 max-w-3xl mx-auto animate-in fade-in duration-200' },
          ...
          h('div', { className: 'bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-xs text-yellow-800 space-y-2' },
            ...
            h('p', { className: 'text-[0.6875rem] text-amber-800 italic mt-1' }, __alloT('stem.manipulatives.nasa_engineers_used_slide_rules_for_ap', '\uD83D\uDE80 NASA engineers used slide rules for Apollo moon mission trajectories!'))
          )
        );
      }
        ...
        return h('div', { className: 'space-y-4 max-w-3xl mx-auto animate-in fade-in duration-200' },
          ...
          pvQuiz && h('div', { className: 'bg-white rounded-xl p-5 border-2 border-orange-200 shadow-sm' },
            h('div', { className: 'flex items-center gap-2 mb-3' },
              h('span', { className: 'text-[0.6875rem] font-bold uppercase px-2 py-0.5 rounded-full ' + (
      ...
      if (manipMode === 'tenFrame') {
        ...
        return h('div', { className: 'space-y-4 max-w-3xl mx-auto animate-in fade-in duration-200' },
          ...
          h('div', { className: 'flex items-center gap-3 justify-center mb-3' },
            h('p', { className: 'text-3xl font-black text-orange-800' + onHostInk }, tenFrameFilled),
          ...
          h('div', { className: 'flex gap-2 justify-center' },
            ...
            h('button', { onClick: function() { upd({ tenFrameFilled: Math.max(0, tenFrameFilled - 1) }); sfxClick(); },
              className: 'transition-colors px-3 py-1.5 rounded text-xs font-bold bg-orange-200 text-orange-900 hover:bg-orange-300' }, __alloT('stem.manipulatives.remove_one', '− Remove one')),
      ...
      if (manipMode === 'counters') {
        ...
        var dropOn = function(targetKind) {
          ...
          if (soundEnabled) sfxRegroup();
          announceToSR && announceToSR(__alloT('stem.manipulatives.sr_zero_pair_removed_by_drag', 'Zero pair removed by drag.'));
        };
        ...
        return h('div', { className: 'space-y-4 max-w-3xl mx-auto animate-in fade-in duration-200' },
          ...
          h('div', { className: 'bg-white rounded-xl border-2 border-rose-200 p-4 text-center' },
            ...
            zeroPairs > 0 && h('p', { className: 'text-[0.6875rem] text-slate-600 mt-1' },
            ...
            h('div', {
              ...
              h('div', { className: 'flex gap-1 mt-2' },
                h('button', { onClick: function() { upd({ counters: Object.assign({}, counters, { yellow: counters.yellow + 1 }) }); sfxClick(); },
                  className: 'transition-colors flex-1 px-2 py-1 rounded text-[0.6875rem] font-bold bg-amber-700 text-white hover:bg-amber-800' }, __alloT('stem.manipulatives.yellow', '+ Yellow')),
                h('button', { onClick: function() { upd({ counters: Object.assign({}, counters, { yellow: counters.yellow + 5 }) }); sfxClick(); },
                  className: 'transition-colors flex-1 px-2 py-1 rounded text-[0.6875rem] font-bold bg-amber-700 text-white hover:bg-amber-800' }, __alloT('stem.manipulatives.5', '+ 5'))
              )
            ),
            h('div', {
              ...
              h('div', { className: 'flex gap-1 mt-2' },
                h('button', { onClick: function() { upd({ counters: Object.assign({}, counters, { red: counters.red + 1 }) }); sfxClick(); },
                  className: 'transition-colors flex-1 px-2 py-1 rounded text-[0.6875rem] font-bold bg-rose-600 text-white hover:bg-rose-700' }, __alloT('stem.manipulatives.red', '+ Red')),
                h('button', { onClick: function() { upd({ counters: Object.assign({}, counters, { red: counters.red + 5 }) }); sfxClick(); },
                  className: 'transition-colors flex-1 px-2 py-1 rounded text-[0.6875rem] font-bold bg-rose-700 text-white hover:bg-rose-800' }, __alloT('stem.manipulatives.5_2', '+ 5'))
              )
            )
          ),
          // Drag hint
          (counters.red > 0 && counters.yellow > 0) && h('p', { className: 'text-[0.6875rem] text-center text-emerald-700 italic' },
        ...
        return h('div', { className: 'space-y-4 max-w-3xl mx-auto animate-in fade-in duration-200' },
          ...
          h('div', { className: 'flex gap-2 justify-center' },
            ...
            h('button', { onClick: function() { upd({ pvDisks: { ones: 0, tens: 0, hundreds: 0, thousands: 0 } }); },
              className: 'transition-colors px-3 py-1.5 rounded text-xs font-bold bg-rose-100 text-rose-800 hover:bg-rose-200' }, __alloT('stem.manipulatives.clear_2', '↺ Clear'))
          ),
      ...
      if (manipMode === 'hundredsChart') {
        var renderHCCell = function(n) {
          ...
          return h('button', {
            ...
            className: 'aspect-square text-xs font-bold transition-all rounded border ' +
              (highlighted ? 'bg-green-700 text-white border-green-800' :
        ...
        return h('div', { className: 'space-y-3 max-w-3xl mx-auto animate-in fade-in duration-200' },
          ...
          h('div', { className: 'bg-green-50 rounded-xl p-3 border border-green-200' },
            ...
            hundredsSkipCount && h('p', { className: 'text-[0.6875rem] text-green-700 italic mt-2' },
      ...
      if (manipMode === 'patternBlocks') {
        ...
        return h('div', { className: 'space-y-3 max-w-3xl mx-auto animate-in fade-in duration-200' },
            ...
            h('div', { className: 'grid grid-cols-6 gap-2' },
              PB_SHAPES.map(function(s) {
                return h('button', { key: 'pb-' + s.id,
                  ...
                  h('p', { className: 'text-[0.625rem] font-bold text-slate-700 mt-1' }, s.name),
                  h('p', { className: 'text-[0.625rem] text-slate-500 font-mono' }, s.fraction)
                );
              })
            )
          ),
          h('div', { className: 'bg-purple-50 rounded-xl border-2 border-purple-200 p-3' },
            ...
            patternBlocks.length > 0 && h('p', { className: 'text-[0.6875rem] text-purple-700 mt-2 text-center font-bold' },
        ...
        return h('div', { className: 'space-y-3 max-w-3xl mx-auto animate-in fade-in duration-200' },
          ...
          h('div', { className: 'grid grid-cols-3 gap-2' },
            h('div', { className: 'bg-sky-50 rounded p-2 border border-sky-200 text-center' },
              h('p', { className: 'text-[0.625rem] font-bold text-sky-700' }, __alloT('stem.manipulatives.segments', 'Segments')),
            ...
            h('div', { className: 'bg-sky-50 rounded p-2 border border-sky-200 text-center' },
              h('p', { className: 'text-[0.625rem] font-bold text-sky-700' }, __alloT('stem.manipulatives.grid_size', 'Grid size')),
          ...
          h('div', { className: 'flex gap-2 justify-center' },
            ...
            h('button', { onClick: function() { upd({ geoboardSize: Math.min(10, GB_SIZE + 1), geoboardSegments: [], geoboardSelected: null }); },
              className: 'transition-colors px-3 py-1.5 rounded text-xs font-bold bg-sky-100 text-sky-800 hover:bg-sky-200' }, __alloT('stem.manipulatives.larger_grid', '+ Larger grid')),
            h('button', { onClick: function() { upd({ geoboardSize: Math.max(3, GB_SIZE - 1), geoboardSegments: [], geoboardSelected: null }); },
              className: 'transition-colors px-3 py-1.5 rounded text-xs font-bold bg-sky-100 text-sky-800 hover:bg-sky-200' }, __alloT('stem.manipulatives.smaller_grid', '− Smaller grid'))
          ),
          ...
          h('div', { className: 'bg-amber-50 rounded-xl border-2 border-amber-200 p-3' },
            h('div', { className: 'flex items-center justify-between mb-2' },
              ...
              h('button', { onClick: function() { upd({ cRods: [] }); },
                className: 'transition-colors px-2 py-1 rounded text-[0.625rem] font-bold bg-rose-100 text-rose-800 hover:bg-rose-200' }, __alloT('stem.manipulatives.clear_5', '↺ Clear'))
            ),
            cRods.length === 0
              ? h('p', { className: 'text-[0.6875rem] italic text-slate-500 text-center py-4' }, __alloT('stem.manipulatives.no_rods_yet_click_a_colored_rod_above_', 'No rods yet. Click a colored rod above to add it.'))
            ...
            cRods.length >= 2 && h('p', { className: 'text-[0.6875rem] text-amber-700 italic mt-2 text-center' },
        ...
        return h('div', { className: 'space-y-3 max-w-3xl mx-auto animate-in fade-in duration-200' },
          ...
          h('div', { className: 'bg-white rounded-xl border-2 border-pink-200 p-4' },
            ...
            nbMode === 'explore' && h('div', { className: 'mt-4 space-y-2' },
              ...
              h('input', { type: 'range', 'aria-label': __alloT('stem.manipulatives.a11y_whole_number', 'Whole number'), 'aria-valuetext': String(nbWhole), min: 2, max: 20, value: nbWhole,
              ...
              h('input', { type: 'range', 'aria-label': __alloT('stem.manipulatives.a11y_first_part', 'First part'), 'aria-valuetext': String(nbPart1), min: 0, max: nbWhole, value: nbPart1,
          ...
          h('div', { className: 'bg-white rounded-xl border-2 border-pink-200 p-3' },
            ...
            nbWhole === 10 && h('p', { className: 'text-[0.6875rem] italic text-pink-700 mt-2' },
            ...
            [1, 2, 3].map(function(s) {
              ...
              return h('button', { key: 's' + s,
                ...
                className: 'px-4 py-2 rounded-lg text-xs font-bold whitespace-pre-line text-center transition-all ' +
                  (craStep === s ? 'bg-teal-700 text-white shadow-md scale-105' : 'bg-white text-teal-700 border border-teal-300 hover:bg-teal-100')
              }, s + '. ' + labels[s]);
            })
          ),
          // Stage body
          craTarget && h('div', { className: 'bg-white rounded-xl border-2 border-teal-200 p-4' },
                              ...
                              : craTarget.op === '×' ? (function() {
                                  ...
                                  for (var gI = 0; gI < craTarget.a; gI++) {
                                    ...
                                    groups.push(h('div', { key: 'g' + gI, className: 'bg-emerald-50 rounded p-2 border border-emerald-200' },
                                      h('p', { className: 'text-[0.625rem] font-bold text-emerald-700 mb-1' }, 'Group ' + (gI + 1)),
                                      h('div', null, grp)
                                    ));
                                  }
            ...
            h('div', { className: 'flex gap-2 mt-4 justify-between' },
              h('button', { onClick: craBack, disabled: craStep <= 1,
                className: 'px-3 py-1.5 rounded text-xs font-bold ' + (craStep <= 1 ? 'bg-slate-100 text-slate-600 cursor-not-allowed' : 'transition-colors bg-slate-200 text-slate-700 hover:bg-slate-300')
              }, __alloT('stem.manipulatives.previous_stage', '← Previous stage')),
              h('button', { onClick: craNext, disabled: craStep >= 3,
                className: 'px-3 py-1.5 rounded text-xs font-bold ' + (craStep >= 3 ? 'bg-slate-100 text-slate-600 cursor-not-allowed' : 'transition-colors bg-teal-700 text-white hover:bg-teal-800')
                  ...
                  grouped[g].map(function(s, idx) {
                    return h('div', { key: g + '-' + idx, className: 'transition-colors p-3 hover:bg-purple-50' },
                      h('div', { className: 'flex items-start gap-2 mb-1' },
                        h('span', { className: 'font-mono text-[0.6875rem] font-bold text-purple-900 bg-purple-100 px-2 py-0.5 rounded' }, s.code),
                        h('button', {
                          onClick: function() { switchMode(s.manip); upd({ aiInsight: '' }); },
                          className: 'transition-colors text-[0.6875rem] font-bold text-purple-700 hover:text-purple-900 underline'
      ...
      if (manipMode === 'library') {
        ...
        return h('div', { className: 'space-y-3 max-w-3xl mx-auto animate-in fade-in duration-200' },
          ...
          h('div', { className: 'bg-white rounded-xl border-2 border-slate-200 p-3' },
            ...
            _m.lastMode && h('p', { className: 'text-[0.6875rem] text-slate-500 italic mt-1' },
            ...
            h('div', { className: 'flex items-center justify-between mb-2' },
              ...
              h('div', { className: 'flex gap-1' },
                h('button', { onClick: exportConstr, disabled: names.length === 0,
                  className: 'transition-colors px-2 py-1 rounded text-[0.6875rem] font-bold bg-blue-100 text-blue-700 hover:bg-blue-200 disabled:opacity-50' }, __alloT('stem.manipulatives.export_json', 'Export JSON')),
                h('label', { className: 'transition-colors px-2 py-1 rounded text-[0.6875rem] font-bold bg-blue-100 text-blue-700 hover:bg-blue-200 cursor-pointer' },
                  ...
                  names.map(function(n) {
                    ...
                    return h('div', { key: n, className: 'py-2 flex items-center gap-2' },
                      h('div', { className: 'flex-1 min-w-0' },
                        ...
                        h('p', { className: 'text-[0.625rem] text-slate-500' },
                          (manipLabel && manipLabel[c.mode]) || c.mode, ' · ', fmtDate(c.savedAt))
                      ),
                      h('button', { onClick: function() { loadConstr(n); },
                        className: 'transition-colors px-2 py-1 rounded text-[0.6875rem] font-bold bg-emerald-100 text-emerald-800 hover:bg-emerald-200' },
                        __alloT('stem.manipulatives.open', 'Open')),
                      h('button', { onClick: function() { deleteConstr(n); },
                        className: 'transition-colors px-2 py-1 rounded text-[0.6875rem] font-bold bg-rose-100 text-rose-800 hover:bg-rose-200' },
                        '✕')
                    );
                  })
                )
      ...
      if (manipMode === 'teacher') {
        ...
        return h('div', { className: 'space-y-4 max-w-4xl mx-auto animate-in fade-in duration-200' },
          ...
          h('div', { className: 'bg-white rounded-xl border-2 border-slate-700 p-4' },
            ...
            h('div', { className: 'mt-3 grid grid-cols-2 sm:grid-cols-4 gap-2 text-[0.6875rem]' },
            ...
            h('p', { className: 'text-[0.6875rem] font-bold text-slate-700 mt-3 mb-1' }, __alloT('stem.manipulatives.mode_coverage', 'Mode coverage:')),
              ...
              ['blocks', 'abacus', 'slideRule', 'quiz', 'tenFrame', 'counters', 'pvDisks', 'hundredsChart', 'patternBlocks', 'geoboard', 'cRods', 'numberBonds'].map(function(mid) {
                return h('span', { key: mid,
                  className: 'px-2 py-0.5 rounded text-[0.625rem] font-mono ' +
                    (modesVisited[mid] ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600 line-through')
                }, mid);
              })
            )
          ),
          ...
          h('div', { className: 'bg-white rounded-xl border-2 border-slate-200 p-3' },
            ...
            h('div', { className: 'flex gap-2 flex-wrap' },
              ...
              h('button', { onClick: resetProgress,
                className: 'transition-colors px-3 py-1.5 rounded text-xs font-bold bg-rose-100 text-rose-800 hover:bg-rose-200 ml-auto' }, __alloT('stem.manipulatives.reset_progress', '↻ Reset progress'))
            )
          ),
              ...
              lessonPlans.map(function(lp, i) {
                return h('div', { key: 'lp' + i, className: 'p-3' },
                  h('div', { className: 'flex items-start justify-between gap-2 mb-1' },
                    h('div', null,
                      h('span', { className: 'inline-block px-1.5 py-0.5 bg-slate-200 text-slate-700 text-[0.625rem] font-bold rounded mr-2' }, 'Grade ' + lp.grade),
                    ...
                    h('div', { className: 'flex gap-1' },
                      lp.manips.map(function(m) {
                        return h('button', { key: m, onClick: function() { switchMode(m); },
                          className: 'transition-colors px-2 py-0.5 rounded text-[0.625rem] font-bold bg-emerald-100 text-emerald-800 hover:bg-emerald-200 whitespace-nowrap'
                        }, '→ ' + m);
                      })
                    )
                  ),
                  ...
                  h('div', { className: 'flex gap-1 flex-wrap' },
                    lp.standards.map(function(s) {
                      return h('span', { key: s, className: 'font-mono text-[0.625rem] bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded' }, s);
                    })
                  )
                );
              })
          ...
          h('div', { className: 'bg-white rounded-xl border-2 border-slate-200 p-3' },
            ...
            h('div', { className: 'space-y-2' },
              h('div', null,
                h('p', { className: 'text-[0.6875rem] font-bold text-slate-700 mb-1' }, __alloT('stem.manipulatives.color_palette_applies_to_counters_rods', 'Color palette (applies to counters, rods, geoboard, ten frame):')),
                h('div', { className: 'flex gap-1 flex-wrap' },
                  Object.keys(MANIP_PALETTES).map(function(pid) {
                    return h('button', { key: pid, onClick: function() { upd({ paletteId: pid }); },
                      className: 'px-2.5 py-1 rounded text-[0.6875rem] font-bold transition-all ' +
              ...
              h('label', { className: 'flex items-center gap-2 text-[0.6875rem] text-slate-700' },
          ...
          h('div', { className: 'bg-white rounded-xl border-2 border-amber-200 p-4' },
            ...
            crChallenge && h('div', { className: 'space-y-3' },
              h('div', { className: 'flex items-center justify-between' },
                h('span', { className: 'inline-block px-2 py-0.5 bg-amber-100 text-amber-800 text-[0.6875rem] font-bold rounded' }, crChallenge.cat),
                h('button', { onClick: chJumpToManip,
                  className: 'transition-colors text-[0.6875rem] text-blue-600 hover:text-blue-800 underline' },
              ...
              return h('div', { key: 'gl' + i, className: 'bg-white rounded-xl border-2 border-cyan-200 p-3' },
                h('div', { className: 'flex items-baseline gap-2 mb-1' },
                  ...
                  h('button', { onClick: function() { switchMode(g.manip); },
                    className: 'transition-colors ml-auto text-[0.625rem] font-bold bg-cyan-100 text-cyan-800 px-2 py-0.5 rounded hover:bg-cyan-200' },
                    '→ try in ' + g.manip)
                ),
        ...
        return h('div', { className: 'space-y-3 max-w-3xl mx-auto animate-in fade-in duration-200' },
          ...
          h('div', { className: 'bg-white rounded-xl border-2 border-pink-200 p-4' },
            h('div', { className: 'flex items-baseline justify-between mb-2' },
              ...
              h('span', { className: 'inline-block px-2 py-0.5 text-[0.625rem] font-bold rounded ' +
            ...
            pCurrent.tools && pCurrent.tools.length > 0 && h('p', { className: 'text-xs text-pink-700 italic mb-2' },
              ...
              pCurrent.tools.map(function(t, i) {
                return h('button', { key: t,
                  ...
                  className: 'transition-colors inline-block px-2 py-0.5 mx-0.5 bg-pink-100 text-pink-800 rounded hover:bg-pink-200 text-[0.6875rem] font-bold'
                }, t);
              })
            ),
            // Hint / answer
          ...
          h('div', { className: 'flex gap-2 justify-between' },
            h('button', {
              ...
              disabled: pIdx <= 0,
              className: 'px-4 py-2 rounded text-xs font-bold ' + (pIdx <= 0 ? 'bg-slate-100 text-slate-600 cursor-not-allowed' : 'transition-colors bg-slate-200 text-slate-700 hover:bg-slate-300')
            ...
            h('button', {
              ...
              className: 'px-4 py-2 rounded text-xs font-bold ' + (pIdx >= puzzles.length - 1 ? 'bg-slate-100 text-slate-600 cursor-not-allowed' : 'transition-colors bg-slate-200 text-slate-700 hover:bg-slate-300')
                  ...
                  t.tools.length > 0 && h('div', { className: 'flex gap-1' },
                    t.tools.map(function(tk) {
                      return h('button', { key: tk, onClick: function() { switchMode(tk); },
                        className: 'transition-colors px-2 py-0.5 rounded text-[0.625rem] font-bold bg-lime-100 text-lime-800 hover:bg-lime-200'
                      }, '→ ' + tk);
                    })
                  )
                ),
      ...
      if (manipMode === 'curriculum') {
        ...
        var cellClass = function(state) {
          if (state === 'core') return 'bg-emerald-700 text-white';
        ...
        return h('div', { className: 'space-y-3 max-w-4xl mx-auto animate-in fade-in duration-200' },
          ...
          h('div', { className: 'bg-white rounded-xl border-2 border-green-200 p-3' },
            ...
            h('div', { className: 'flex gap-2 flex-wrap text-[0.6875rem]' },
                  ...
                  return h('tr', { key: 'cm' + mk },
                    ...
                    grades.map(function(g) {
                      ...
                      return h('td', { key: g + '-' + mk, className: 'text-center' },
                        h('div', { className: 'rounded text-[0.625rem] font-bold uppercase py-1 ' + cellClass(st) }, st || '·')
                      );
                    })
                  );
                })
        ...
        return h('div', { className: 'space-y-3 max-w-3xl mx-auto animate-in fade-in duration-200' },
          ...
          h('div', { className: 'bg-white rounded-xl border-2 border-rose-200 p-4', style: { breakInside: 'avoid' } },
            h('div', { className: 'flex items-baseline justify-between mb-2' },
              ...
              h('div', { className: 'text-right' },
                h('span', { className: 'inline-block px-2 py-0.5 text-[0.625rem] font-bold rounded bg-rose-100 text-rose-800' }, 'Grade ' + curAct.grade),
                h('span', { className: 'inline-block ml-1 px-2 py-0.5 text-[0.625rem] font-bold rounded bg-amber-100 text-amber-800' }, curAct.time)
              )
            ),
          ...
          h('div', { className: 'flex gap-2 justify-between' },
            h('button', {
              ...
              disabled: aIdx <= 0,
              className: 'px-4 py-2 rounded text-xs font-bold ' + (aIdx <= 0 ? 'bg-slate-100 text-slate-600 cursor-not-allowed' : 'transition-colors bg-slate-200 text-slate-700 hover:bg-slate-300')
            ...
            h('button', {
              ...
              className: 'px-4 py-2 rounded text-xs font-bold ' + (aIdx >= activities.length - 1 ? 'bg-slate-100 text-slate-600 cursor-not-allowed' : 'transition-colors bg-slate-200 text-slate-700 hover:bg-slate-300')
            ...
            h('div', { className: 'grid grid-cols-1 sm:grid-cols-2 gap-1' },
              activities.map(function(a, i) {
                return h('button', { key: 'ac' + i, onClick: function() { upd({ activityIdx: i }); },
                  ...
                  h('span', { className: 'font-mono text-[0.625rem] bg-slate-200 px-1 rounded mr-1' }, a.grade),
                ...
                shortcuts.map(function(s) {
                  return h('tr', { key: 'sc' + s.keys, className: 'border-b border-slate-100' },
                    h('td', { className: 'py-2 pr-3' },
                      h('kbd', { className: 'inline-block px-1.5 py-0.5 bg-slate-100 border border-slate-300 rounded font-mono text-[0.6875rem]' }, s.keys)
                    ),
      ...
      if (manipMode === 'fracBars') {
        ...
        var renderBar = function(denom) {
          ...
          return h('div', { key: 'fb-row-' + denom, className: 'space-y-1' },
            ...
            fracVal > 0 && h('p', { className: 'text-[0.625rem] text-slate-600 ml-14' },
            ...
            h('div', { className: 'grid grid-cols-3 sm:grid-cols-6 gap-2' },
              palette.map(function(p) {
                return h('div', { key: 'pal-' + p.kind, className: 'flex flex-col items-center gap-1 p-2 bg-indigo-50 rounded border border-indigo-200' },
                  ...
                  h('p', { className: 'text-[0.625rem] font-bold text-indigo-700 text-center' }, p.name),
                  h('button', { onClick: function(k) { return function() { atAdd(k); }; }(p.kind),
                    className: 'transition-colors px-2 py-0.5 rounded text-[0.625rem] font-bold bg-indigo-600 text-white hover:bg-indigo-700' }, __alloT('stem.manipulatives.add', '+ Add'))
                );
              })
            )
          ),
