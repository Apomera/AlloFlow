> **Superseded:** Historical September 12, 2026 v1.5 assessment. See the [current VPAT](../../../VPAT-2.5-WCAG-AlloFlow.md).

# AlloFlow Accessibility Conformance Report

**WCAG Edition — based on VPAT® Version 2.5Rev**

> **Interim self-assessment.** This report is intentionally conservative. It documents current evidence for AlloFlow v1.5, but it does **not** claim full WCAG 2.1 or WCAG 2.2 conformance. Full-product keyboard, assistive-technology, zoom, text-spacing, generated-output, and representative-workflow verification remains in progress.
...
| **Evaluation target** | Web release metadata v1.5; HEAD `815d948165363182edfd776694c371dc64602080` plus pre-existing local changes. Existing compiled app bundle and current canonical source/module fixtures were tested separately; no clean release rebuild or hosted-deployment equivalence is claimed. |
| **Evaluation methods** | axe-core 4.12.1 with Playwright Chromium 148.0.7778.96; 616-file source heuristic scan; 808 existing accessibility regression files selected; desktop/320 CSS-pixel and text-spacing probes; scripted keyboard review of AI settings; targeted source and screenshot inspection. |
...
| **2.1.1 Keyboard** | Partially Supports | The AI settings trigger was activated with Enter and a 45-step Tab sample remained within the dialog; Escape closed it and returned focus to its trigger. The broader fixtures flag keyboard access to scrollable regions in Architecture Studio, Circuit Builder and Fire Ecology, requiring real-tool confirmation. Existing accessibility regressions cover many other controls. This scripted sample does not replace complete keyboard-only workflows, including canvas, editors, and games. |
...
| **1.4.3 Contrast (Minimum)** | Partially Supports | AUD-01 and AUD-04 corrected locally. The three Video Studio controls measure 6.29:1 in default/focus and 7.90:1 on hover. Quick Start College hover measures 7.07:1 after the previously overriding shared stylesheet loads. Targeted browser contrast checks pass. The broader baseline fixtures flag other theme-dependent contrast, including Number Line and Titration, whose real-host/style parity remains to be checked. Other themes, states and generated output remain incompletely evaluated. |
...
| **2.4.7 Focus Visible** | Partially Supports | Global focus styling and targeted component remediation exist. Rendered fixtures flag Geometry Sandbox overlay/focus warnings and SEL practiceJourneys missing focus baselines; three Coaster Lab focus cases were skipped. Manual confirmation across every theme, control state, and complex tool remains required. |
