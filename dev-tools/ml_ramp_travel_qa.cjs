...
source+=String.raw`
  ...
  for(const height of [0.2,1,3.9,4]){
    await mount('ramp',{rampLength:4,rampHeight:height});
    for(const [name,progress] of [['Halfway',0.5],['Full stroke',1]]){
      ...
      await pg.waitForFunction(p=>Math.abs(window.__qaShop.mlDemo.rampTravelCursor.position.x-1.45*p)<1e-8,progress);
      const framed=await pg.evaluate(()=>{
        const s=window.__qaShop,d=s.mlDemo;
        const points=[-2.7,2.7].flatMap(x=>[-1.075,1.075].map(z=>d.rampDeck.localToWorld(new THREE.Vector3(x,0,z)).project(s.camera)));
        return points.every(p=>Math.abs(p.x)<0.99&&Math.abs(p.y)<0.99&&p.z>-1&&p.z<1);
      });
      if(!framed)errors.push('Ramp deck clipped at height '+height+' and pose '+progress);
      const label='ramp-'+height+'-'+Math.round(progress*100);
  ...
  for(const width of [390,320])for(const bench of ['ramp']){
  ...
  await pg.emulateMedia({reducedMotion:'reduce'});await mount('ramp');
  await inspector.getByRole('button',{name:'Halfway',exact:true}).click();await checkPose(0.5);
  const before=await pg.evaluate(()=>window.__qaShop.mlDemo.rampTravelCursor.position.x);
  await pg.waitForTimeout(300);const after=await pg.evaluate(()=>window.__qaShop.mlDemo.rampTravelCursor.position.x);
  if(before!==after || Math.abs(after-0.725)>1e-8)errors.push('Reduced-motion inspection drifted');
  await pg.locator('.ml-shop-world').screenshot({path:path.join(OUT,'reduced-motion-ramp.png')});
