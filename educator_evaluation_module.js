...
function aeNormalizeRubric(raw) {
  ...
  for (let i = 0; i < domainsRaw.length; i++) {
    ...
    const components = Array.isArray(entry.components) ? entry.components.filter((pair) => Array.isArray(pair) && pair.length >= 2).map((pair) => [String(pair[0]), String(pair[1])]) : [];
    domains.push({
      id,
  ...
  const bands = Array.isArray(raw.bands) ? raw.bands.filter((band) => band && Number.isFinite(Number(band.min)) && String(band.label || "").trim()).map((band) => ({ min: Number(band.min), label: String(band.label).trim() })).sort((a, b) => b.min - a.min) : [];
  const name = String(raw.name == null ? "" : raw.name).trim() || "Custom rubric";
  return {
    name,
    versionTag: String(raw.versionTag == null ? "" : raw.versionTag).trim() || "custom-" + name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""),
