...
const TARGET = path.join(VENDOR_DIR, 'epubcheck');
...
function missingOrMismatched(entries) {
  const bad = [];
  for (const e of entries) {
    const abs = path.join(VENDOR_DIR, e.path);
...
function copyTree(fromRoot, entries) {
  ...
  for (const e of entries) {
    ...
    const dest = path.join(VENDOR_DIR, e.path);
...
function localInstallRoots() {
  const roots = [];
  if (process.env.EPUBCHECK_HOME) roots.push(process.env.EPUBCHECK_HOME);
  const tools = path.join(os.homedir(), '.alloflow-tools');
...
async function fetchRelease() {
  ...
  try {
    ...
    return copyTree(root, manifestEntries());
  } finally {
...
async function ensureEpubcheck(options = {}) {
  ...
  let bad = missingOrMismatched(entries);
  ...
  for (const root of localInstallRoots()) {
    ...
    copyTree(root, entries);
    bad = missingOrMismatched(entries);
