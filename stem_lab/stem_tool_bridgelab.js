  ...
  window.StemLab.registerTool('bridgeLab', {
    ...
    render: function(ctx) {
      ...
      if (!labToolData || !labToolData.bridgeLab) {
        ...
        return h('div', { style: { padding: 24, color: 'var(--allo-stem-text-soft, #94a3b8)', textAlign: 'center' } }, t('stem.bridgelab.initializing_bridge_lab', '🌉 Initializing Bridge Lab...'));
      }
      ...
      var TABS = [
        { id: 'build',     icon: '🔨', label: t('stem.bridgelab.stress_test', 'Stress Test') },
        { id: 'types',     icon: '🌉', label: t('stem.bridgelab.bridge_types', 'Bridge Types') },
        { id: 'materials', icon: '🧱', label: t('stem.bridgelab.materials', 'Materials') },
        { id: 'forces',    icon: '⚖️', label: t('stem.bridgelab.forces', 'Forces') },
        { id: 'cases',     icon: '📚', label: t('stem.bridgelab.case_studies', 'Case Studies') },
        { id: 'cycle',     icon: '🔁', label: t('stem.bridgelab.design_cycle', 'Design Cycle') },
        { id: 'quiz',      icon: '📝', label: t('stem.bridgelab.quiz', 'Quiz') },
        { id: 'print',     icon: '🖨', label: t('stem.bridgelab.print', 'Print') },
        { id: 'inquiry',   icon: '🔬', label: t('stem.bridgelab.inquiry', 'Inquiry') }
      ];

      var tabBar = h('div', {
        role: 'tablist', 'aria-label': t('stem.bridgelab.bridge_engineering_sections', 'Bridge Engineering sections'),
      ...
      function renderBuild() {
        ...
        return h('div', { style: { padding: 16 } },
          h('p', { style: { color: 'var(--allo-stem-text, #cbd5e1)', fontSize: 13, marginBottom: 12, lineHeight: 1.6 } },
            ...
            h('strong', { style: { color: '#f87171' } }, t('stem.bridgelab.red_tension', 'Red = tension')), ', ',
            h('strong', { style: { color: '#7dd3fc' } }, t('stem.bridgelab.blue_compression', 'blue = compression')),
            t('stem.bridgelab.thickness_shows_magnitude_forces_calcu', ', thickness shows magnitude. Forces calculated using the deep-beam approximation (real analysis would use method of joints or matrix structural analysis).')
          ),
          ...
          h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 14, alignItems: 'center', marginBottom: 14, fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)' } },
            h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 5 } }, h('span', { style: { width: 16, height: 4, borderRadius: 2, background: '#f87171', display: 'inline-block' } }), t('stem.bridgelab.tension', 'Tension')),
            h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 5 } }, h('span', { style: { width: 16, height: 4, borderRadius: 2, background: '#7dd3fc', display: 'inline-block' } }), t('stem.bridgelab.compression', 'Compression')),
            h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 5 } }, h('span', { style: { width: 16, height: 8, borderRadius: 2, background: '#94a3b8', display: 'inline-block' } }), t('stem.bridgelab.thicker_larger_force', 'Thicker = larger force')),
            h('span', null, t('stem.bridgelab.supports_pin_left_roller_right', 'Supports: pin (left) ▲  ·  roller (right) ▲⊙'))
          ),
          ...
          sectionCard('Loading',
            h('div', null,
              h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 } },
                [
                  { id: 'uniform', name: t('stem.bridgelab.uniform_load', 'Uniform load'), sub: 'distributed across all top joints (dead load equivalent)' },
                  { id: 'vehicle', name: t('stem.bridgelab.moving_vehicle', 'Moving vehicle'), sub: 'single point load you can drag across the span' }
                ].map(function(o) {
              ...
              loadMode === 'vehicle'
                ? h('div', { style: { fontSize: 11.5, color: 'var(--allo-stem-text, #fde68a)', lineHeight: 1.6, padding: 8, background: 'rgba(245,158,11,0.10)', borderRadius: 6, border: '1px solid rgba(245,158,11,0.3)' } },
                    h('strong', null, t('stem.bridgelab.moving_load_analysis', 'Moving-load analysis: ')),
                    t('stem.bridgelab.real_bridges_are_designed_by_sliding_t', 'real bridges are designed by sliding the worst possible vehicle across every point on the span and recording the maximum force in each member. The "influence line" of a member is how its force varies with the load\'s position. Each member has its own worst case — usually NOT in the same place as the worst case for any other member.')
                  )
                : h('div', { style: { fontSize: 11.5, color: 'var(--allo-stem-text-soft, #94a3b8)', lineHeight: 1.5, padding: 8 } }, t('stem.bridgelab.uniform_mode_treats_the_load_as_equall', 'Uniform mode treats the load as equally distributed across all top joints — useful for studying the bridge under its own weight + typical dead loads.'))
            )
          ),
          ...
          sectionCard('Truss style',
            h('div', null,
              h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 } },
                [
                  { id: 'warren', name: t('stem.bridgelab.warren', 'Warren'), sub: 'alternating diagonals, no verticals' },
                  { id: 'pratt',  name: t('stem.bridgelab.pratt', 'Pratt'),  sub: 'verticals + inward diagonals (in tension)' },
                  { id: 'howe',   name: t('stem.bridgelab.howe', 'Howe'),   sub: 'verticals + outward diagonals (in compression)' },
          ...
          sectionCard('Design notes (optional, included on print)',
            h('div', null,
              h('input', { type: 'text', value: d.designName || '',
                ...
                placeholder: t('stem.bridgelab.design_name_e_g_marie_s_pedestrian_bri', 'Design name (e.g., "Marie\'s pedestrian bridge over Back Cove")'),
              ...
              h('textarea', { value: d.designNotes || '',
                ...
                placeholder: t('stem.bridgelab.why_this_material_what_are_the_constra', 'Why this material? What are the constraints? What would you improve?'),
                rows: 3,
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 8, borderRadius: 6, background: 'var(--allo-stem-panel, #1e293b)', border: '1px solid var(--allo-stem-border, #334155)', marginBottom: 12 } },
                  h('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 4 } },
                    h('span', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)', fontWeight: 700 } }, t('stem.bridgelab.target_safety_factor', 'Target safety factor')),
                  ...
                  h('input', { type: 'range', min: 1.5, max: 6, step: 0.5, value: targetSF,
                    ...
                    'aria-label': t('stem.bridgelab.target_safety_factor_2', 'Target safety factor'),
                  ...
                  h('div', { style: { display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--allo-stem-text-soft, #64748b)', marginTop: 2 } },
                    h('span', null, t('stem.bridgelab.1_5_very_thin_margin', '1.5 (very thin margin)')),
                    h('span', null, t('stem.bridgelab.2_0_code_minimum', '2.0 (code minimum)')),
                    h('span', null, t('stem.bridgelab.4_0_conservative', '4.0+ (conservative)')),
                    h('span', null, t('stem.bridgelab.6_0_brooklyn_bridge', '6.0 (Brooklyn Bridge)'))
                  )
                ),
                top5.length === 0
                  ...
                  : h('div', null,
                      ...
                      h('div', { style: { marginTop: 8, padding: 8, borderRadius: 6, background: 'rgba(245,158,11,0.10)', border: '1px solid rgba(245,158,11,0.3)', fontSize: 11, color: 'var(--allo-stem-text, #fde68a)', lineHeight: 1.6 } },
                        h('strong', null, t('stem.bridgelab.click_the_winner', 'Click the winner: ')),
                        h('button', {
                        ...
                        }, t('stem.bridgelab.apply_this_design_to_my_bridge', 'Apply this design to my bridge'))
                      )
                    )
              );
            })(),
          ...
          (function() {
            ...
            return sectionCard('A note on this analysis',
              h('div', { style: { fontSize: 12, color: 'var(--allo-stem-text, #cbd5e1)', lineHeight: 1.65 } },
                useExact
                  ? h('span', null,
                      h('strong', { style: { color: '#86efac' } }, t('stem.bridgelab.exact_analysis_method_of_joints', '✓ Exact analysis (Method of Joints). ')),
                      t('stem.bridgelab.forces_are_computed_from_2d_pin_jointe', 'Forces are computed from 2D pin-jointed equilibrium: for each joint, sum of forces in x and sum of forces in y both equal zero. '),
      ...
      function renderTypes() {
        ...
        function suspensionAnalyzer() {
          ...
          function catenarySvg() {
            ...
            return h('svg', { viewBox: '0 0 ' + svgW + ' ' + svgH, width: '100%', height: svgH, role: 'img', 'aria-labelledby': 'catTitle catDesc' },
              h('title', { id: 'catTitle' }, t('stem.bridgelab.suspension_bridge_cable_shape', 'Suspension bridge cable shape')),
              ...
              h('text', { x: tx(0), y: 18, textAnchor: 'middle', fill: '#94a3b8', fontSize: 10 }, t('stem.bridgelab.tower', 'Tower')),
              h('text', { x: tx(cSpan), y: 18, textAnchor: 'middle', fill: '#94a3b8', fontSize: 10 }, t('stem.bridgelab.tower_2', 'Tower')),
              // Anchorages (ground)
              h('line', { x1: 0, y1: padT + plotH + 12, x2: svgW, y2: padT + plotH + 12, stroke: '#475569', strokeWidth: 1, strokeDasharray: '4 4' }),
              h('text', { x: 8, y: padT + plotH + 28, fill: '#94a3b8', fontSize: 10 }, t('stem.bridgelab.anchor', 'Anchor')),
              h('text', { x: svgW - 50, y: padT + plotH + 28, fill: '#94a3b8', fontSize: 10 }, t('stem.bridgelab.anchor_2', 'Anchor')),
              // Cable
          ...
          return h('div', null,
            h('p', { style: { margin: '0 0 10px', fontSize: 13, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } },
              t('stem.bridgelab.suspension_bridges_work_differently_fr', 'Suspension bridges work differently from trusses. The deck hangs from cables; cables hang in a curve between towers; towers transfer the load down. The cables are in PURE TENSION, the towers in PURE COMPRESSION. Anchorages resist the horizontal pull of the cables — they\'re typically massive concrete blocks buried in bedrock at each end of the bridge.')
            ),
            ...
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginTop: 12, marginBottom: 12 } },
              [
                { label: t('stem.bridgelab.span_m', 'Span (m)'), value: cSpan, min: 200, max: 2200, step: 50, key: 'suspSpan' },
                { label: t('stem.bridgelab.sag_m', 'Sag (m)'), value: cSag, min: 10, max: 300, step: 5, key: 'suspSag' },
                { label: t('stem.bridgelab.deck_load_kn_m', 'Deck load (kN/m)'), value: cLoad, min: 1, max: 50, step: 1, key: 'suspLoad' }
              ].map(function(s, i) {
            ...
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8, marginBottom: 10 } },
              [
                { label: t('stem.bridgelab.horizontal_h', 'Horizontal H'), value: H.toFixed(0) + ' kN/m', color: '#fca5a5', sub: 'Pulled into anchorages' },
                { label: t('stem.bridgelab.vertical_v', 'Vertical V'), value: V.toFixed(0) + ' kN/m', color: '#93c5fd', sub: 'Half the deck load (each)' },
                { label: t('stem.bridgelab.max_tension_t', 'Max tension T'), value: Tmax.toFixed(0) + ' kN/m', color: '#fbbf24', sub: 'At the tower top' },
                { label: t('stem.bridgelab.cable_length', 'Cable length'), value: cableLen.toFixed(1) + ' m', color: '#86efac', sub: '~' + ((cableLen / cSpan - 1) * 100).toFixed(1) + '% > span' },
                { label: t('stem.bridgelab.angle_at_support', 'Angle at support'), value: angleAtSupport.toFixed(1) + '°', color: '#c7d2fe', sub: 'From horizontal' },
                { label: t('stem.bridgelab.sag_span_ratio', 'Sag/span ratio'), value: '1:' + (1 / sagRatio).toFixed(1), color: sagRatioGood ? '#86efac' : '#fca5a5', sub: sagRatioGood ? 'real-bridge sweet spot' : 'outside typical 1/14-1/6 range' }
              ].map(function(s, i) {
        ...
        return h('div', { style: { padding: 16 } },
          h('p', { style: { color: 'var(--allo-stem-text, #cbd5e1)', fontSize: 13, marginBottom: 12, lineHeight: 1.6 } },
            t('stem.bridgelab.bridge_type_depends_on_span_environmen', 'Bridge type depends on span, environment, material, and budget. Engineers pick the right form for the constraints — not a favorite.')
          ),
          (function() {
            ...
            function archAnalyzer() {
              ...
              function archSvg() {
                ...
                return h('svg', { viewBox: '0 0 ' + svgW + ' ' + svgH, width: '100%', height: svgH, role: 'img', 'aria-labelledby': 'archTitle archDesc' },
                  h('title', { id: 'archTitle' }, t('stem.bridgelab.arch_bridge_thrust_diagram', 'Arch bridge thrust diagram')),
                  ...
                  h('text', { x: 18, y: ty(0) + 30, fill: '#cbd5e1', fontSize: 10 }, t('stem.bridgelab.abutment', 'Abutment')),
                  h('text', { x: svgW - 60, y: ty(0) + 30, fill: '#cbd5e1', fontSize: 10 }, t('stem.bridgelab.abutment_2', 'Abutment')),
                  ...
                  (function() {
                    ...
                    return h('g', null,
                      ...
                      h('line', { x1: fx, y1: fy, x2: fx - Hpx, y2: fy, stroke: '#ef4444', strokeWidth: 2 }),
                      h('polygon', { points: (fx - Hpx) + ',' + (fy - 3) + ' ' + (fx - Hpx) + ',' + (fy + 3) + ' ' + (fx - Hpx - 5) + ',' + fy, fill: '#ef4444' }),
                      h('text', { x: fx - Hpx - 10, y: fy + 4, textAnchor: 'end', fill: '#fca5a5', fontSize: 10, fontWeight: 700 }, t('stem.bridgelab.h_out', 'H (out)')),
              ...
              return h('div', null,
                h('p', { style: { margin: '0 0 10px', fontSize: 13, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } },
                  t('stem.bridgelab.arches_do_the_opposite_of_suspension_b', 'Arches do the opposite of suspension bridges: a load on top pushes the curve OUTWARD into the abutments rather than pulling them inward. The entire arch is in compression — there is no tension in a true arch under design loads. This is why stone arches work (stone is excellent in compression). The horizontal thrust at the supports is the design challenge — abutments must resist it without sliding or tipping.')
                ),
                archSvg(),
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginTop: 12, marginBottom: 12 } },
                  [
                    { label: t('stem.bridgelab.span_m_2', 'Span (m)'), value: aSpan, min: 10, max: 200, step: 5, key: 'archSpan' },
                    { label: t('stem.bridgelab.rise_m', 'Rise (m)'), value: aRise, min: 3, max: 100, step: 1, key: 'archRise' },
                    { label: t('stem.bridgelab.deck_load_kn_m_2', 'Deck load (kN/m)'), value: aLoad, min: 5, max: 100, step: 5, key: 'archLoad' }
                  ].map(function(s, i) {
                    return h('div', { key: i, style: { padding: 8, borderRadius: 6, background: 'var(--allo-stem-panel, #1e293b)', border: '1px solid var(--allo-stem-border, #334155)' } },
                      h('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 4 } },
                        h('span', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)', fontWeight: 700 } }, s.label),
                ...
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8, marginBottom: 10 } },
                  [
                    { label: t('stem.bridgelab.horizontal_thrust_h', 'Horizontal thrust H'), value: H.toFixed(0) + ' kN/m', color: '#fca5a5', sub: 'pushes outward — abutments must resist' },
                    { label: t('stem.bridgelab.vertical_reaction_v', 'Vertical reaction V'), value: V.toFixed(0) + ' kN/m', color: '#93c5fd', sub: 'half the deck load each side' },
                    { label: t('stem.bridgelab.max_compression', 'Max compression'), value: Cmax.toFixed(0) + ' kN/m', color: '#fbbf24', sub: 'at the springings (supports)' },
                    { label: t('stem.bridgelab.min_compression', 'Min compression'), value: Cmin.toFixed(0) + ' kN/m', color: '#86efac', sub: 'at the crown (top of arch)' },
                    { label: t('stem.bridgelab.rise_span_ratio', 'Rise/span ratio'), value: '1:' + (1 / riseRatio).toFixed(1), color: goodRatio ? '#86efac' : '#fca5a5', sub: goodRatio ? 'within typical 1/2 to 1/8 range' : 'outside typical range' }
                  ].map(function(s, i) {
                    return h('div', { key: i, style: { padding: 8, borderRadius: 6, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
                      h('div', { style: { fontSize: 10, color: 'var(--allo-stem-text-soft, #94a3b8)', textTransform: 'uppercase', letterSpacing: 0.5 } }, s.label),
                      h('div', { style: { fontSize: 14, fontWeight: 800, color: s.color, marginTop: 2 } }, s.value),
                ...
                h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(245,158,11,0.10)', border: '1px solid rgba(245,158,11,0.3)', fontSize: 11.5, color: 'var(--allo-stem-text, #fde68a)', lineHeight: 1.65, marginTop: 8 } },
                  h('strong', null, t('stem.bridgelab.why_high_thrust_matters', 'Why high thrust matters: ')),
                  t('stem.bridgelab.flatter_arches_smaller_rise_need_much_', 'Flatter arches (smaller rise) need much more horizontal thrust. Roman arches were typically semicircular (rise = half the span) for that reason — the thrust is moderate and easily resisted. Modern shallow steel arches need enormous engineered abutments anchored into rock, or tied-arch designs where horizontal tie rods at deck level absorb the thrust (so the abutments only resist vertical load). Sydney Harbour Bridge and New River Gorge are tied-arch designs.')
                )
              );
            }
            ...
            function beamAnalyzer() {
              ...
              function beamSvg() {
                ...
                return h('svg', { viewBox: '0 0 ' + svgW + ' ' + svgH, width: '100%', height: svgH, role: 'img', 'aria-labelledby': 'beamTitle beamDesc' },
                  h('title', { id: 'beamTitle' }, t('stem.bridgelab.beam_bending_diagram', 'Beam bending diagram')),
              ...
              return h('div', null,
                h('p', { style: { margin: '0 0 10px', fontSize: 13, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } },
                  t('stem.bridgelab.most_modern_bridges_over_short_spans_8', 'Most modern bridges over short spans (< 80 m) are simply beams sitting on two supports. The internal forces are very different from trusses: members carry BENDING + SHEAR (not just tension/compression along their axis). The bottom of a loaded beam stretches; the top compresses. This is the simplest structural analysis, the foundation of every other one.')
                ),
                beamSvg(),
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginTop: 12, marginBottom: 12 } },
                  [
                    { label: t('stem.bridgelab.span_m_3', 'Span (m)'), value: bSpan, min: 2, max: 30, step: 1, key: 'beamSpan' },
                    { label: t('stem.bridgelab.depth_mm', 'Depth (mm)'), value: bDepth, min: 200, max: 1500, step: 50, key: 'beamDepth' },
                    { label: t('stem.bridgelab.width_mm', 'Width (mm)'), value: bWidth, min: 100, max: 600, step: 25, key: 'beamWidth' },
                    { label: t('stem.bridgelab.point_load_kn', 'Point load (kN)'), value: bLoad, min: 10, max: 500, step: 10, key: 'beamLoad' },
                    { label: t('stem.bridgelab.load_position_0_1', 'Load position (0..1)'), value: bLoadPos, min: 0.05, max: 0.95, step: 0.05, key: 'beamLoadPos' }
                  ].map(function(s, i) {
                    return h('div', { key: i, style: { padding: 8, borderRadius: 6, background: 'var(--allo-stem-panel, #1e293b)', border: '1px solid var(--allo-stem-border, #334155)' } },
                      h('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 4 } },
                        h('span', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)', fontWeight: 700 } }, s.label),
                ...
                h('div', { style: { marginBottom: 12 } },
                  h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)', fontWeight: 700, marginBottom: 6 } }, t('stem.bridgelab.material', 'Material')),
                  h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6 } },
                    MATERIALS.map(function(m) {
                ...
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8, marginBottom: 10 } },
                  [
                    { label: t('stem.bridgelab.r_a_left', 'R_A (left)'), value: R_A.toFixed(1) + ' kN', color: '#93c5fd', sub: 'farther load = larger' },
                    { label: t('stem.bridgelab.r_b_right', 'R_B (right)'), value: R_B.toFixed(1) + ' kN', color: '#93c5fd', sub: '' },
                    { label: t('stem.bridgelab.max_moment_m', 'Max moment M'), value: M_max.toFixed(1) + ' kN·m', color: '#fbbf24', sub: 'at load point' },
                    { label: t('stem.bridgelab.max_shear_v', 'Max shear V'), value: V_max.toFixed(1) + ' kN', color: '#fbbf24', sub: 'at support' },
                    { label: t('stem.bridgelab.bending_stress', 'Bending stress σ'), value: sigmaMax_MPa.toFixed(0) + ' MPa', color: sf_yield >= 2 ? '#86efac' : sf_yield >= 1 ? '#fbbf24' : '#fca5a5', sub: 'yield ' + bMat.yieldMPa + ' MPa' },
                    { label: t('stem.bridgelab.shear_stress', 'Shear stress τ'), value: tauMax_MPa.toFixed(1) + ' MPa', color: '#c7d2fe', sub: '1.5V/A for rectangle' },
                    { label: t('stem.bridgelab.section_mod_s', 'Section mod. S'), value: (S_mm3 / 1000).toFixed(0) + ' ×10³ mm³', color: 'var(--allo-stem-text-soft, #94a3b8)', sub: 'I/c, larger = stiffer' },
                    { label: t('stem.bridgelab.deflection', 'Deflection δ'), value: delta_mm.toFixed(1) + ' mm', color: deflectionOK ? '#86efac' : '#fbbf24', sub: 'L/' + (deflectionRatio).toFixed(0) }
                  ].map(function(s, i) {
                    return h('div', { key: i, style: { padding: 8, borderRadius: 6, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
                      h('div', { style: { fontSize: 10, color: 'var(--allo-stem-text-soft, #94a3b8)', textTransform: 'uppercase', letterSpacing: 0.5 } }, s.label),
                      h('div', { style: { fontSize: 14, fontWeight: 800, color: s.color, marginTop: 2 } }, s.value),
            ...
            function cableStayedAnalyzer() {
              ...
              function csSvg() {
                ...
                return h('svg', { viewBox: '0 0 ' + svgW + ' ' + svgH, width: '100%', height: svgH, role: 'img', 'aria-labelledby': 'csTitle csDesc' },
                  h('title', { id: 'csTitle' }, t('stem.bridgelab.cable_stayed_bridge_diagram', 'Cable-stayed bridge diagram')),
                  ...
                  h('text', { x: twrX, y: twrTop - 5, textAnchor: 'middle', fill: '#cbd5e1', fontSize: 10, fontWeight: 700 }, t('stem.bridgelab.tower_3', 'Tower')),
                  h('text', { x: svgW - 8, y: deckY - 6, textAnchor: 'end', fill: '#fde68a', fontSize: 10 }, t('stem.bridgelab.stays_in_tension', 'Stays in tension'))
                );
              }

              return h('div', null,
                h('p', { style: { margin: '0 0 10px', fontSize: 13, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } },
                  t('stem.bridgelab.a_cable_stayed_bridge_uses_cables_runn', 'A cable-stayed bridge uses cables running directly from a tower (or pylons) to the deck — unlike a suspension bridge, which has a main draped cable + vertical hangers. Each stay is in tension; the tower is in compression. Faster + cheaper to build than a suspension bridge of comparable span, and noticeably stiffer in wind. The Millau Viaduct, Russky Bridge, + Sundial Bridge are all cable-stayed.')
                ),
                csSvg(),
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginTop: 12, marginBottom: 12 } },
                  [
                    { label: t('stem.bridgelab.main_span_m', 'Main span (m)'), value: csSpan, min: 100, max: 1500, step: 50, key: 'csSpan' },
                    { label: t('stem.bridgelab.tower_height_m', 'Tower height (m)'), value: csTowerH, min: 30, max: 350, step: 10, key: 'csTowerH' },
                    { label: t('stem.bridgelab.stays_per_side', 'Stays per side'), value: csNStays, min: 4, max: 30, step: 1, key: 'csNStays' },
                    { label: t('stem.bridgelab.deck_load_kn_m_3', 'Deck load (kN/m)'), value: csLoad, min: 5, max: 80, step: 5, key: 'csLoad' }
                  ].map(function(s, i) {
                    return h('div', { key: i, style: { padding: 8, borderRadius: 6, background: 'var(--allo-stem-panel, #1e293b)', border: '1px solid var(--allo-stem-border, #334155)' } },
                      h('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 4 } },
                        h('span', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)', fontWeight: 700 } }, s.label),
                ...
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 8, marginBottom: 10 } },
                  [
                    { label: t('stem.bridgelab.max_stay_force_longest', 'Max stay force (longest)'), value: fMax.toFixed(0) + ' kN', color: '#fbbf24', sub: 'shallowest, farthest from tower' },
                    { label: t('stem.bridgelab.min_stay_force_steepest', 'Min stay force (steepest)'), value: fMin.toFixed(0) + ' kN', color: '#86efac', sub: 'closest to tower' },
                    { label: t('stem.bridgelab.tributary_per_stay', 'Tributary per stay'), value: tribLen.toFixed(1) + ' m', color: '#c7d2fe', sub: 'deck length per stay' },
                    { label: t('stem.bridgelab.tower_compression', 'Tower compression'), value: towerComp.toFixed(0) + ' kN', color: '#fca5a5', sub: 'sum of all stay verticals' },
                    { label: t('stem.bridgelab.avg_stay_angle_to_horiz', 'Avg stay angle (to horiz.)'), value: (stays.reduce(function(a, s) { return a + s.angle; }, 0) / stays.length).toFixed(0) + '°', color: 'var(--allo-stem-text-soft, #94a3b8)', sub: 'steeper = lower force' }
                  ].map(function(s, i) {
                    return h('div', { key: i, style: { padding: 8, borderRadius: 6, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
                      h('div', { style: { fontSize: 10, color: 'var(--allo-stem-text-soft, #94a3b8)', textTransform: 'uppercase', letterSpacing: 0.5 } }, s.label),
                      h('div', { style: { fontSize: 14, fontWeight: 800, color: s.color, marginTop: 2 } }, s.value),
            ...
            return h('div', { key: t.id, style: { padding: 14, borderRadius: 12, background: 'var(--allo-stem-panel, #1e293b)', border: '1px solid var(--allo-stem-border, #334155)', marginBottom: 10 } },
              ...
              h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 } },
                h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(34,197,94,0.10)', border: '1px solid rgba(34,197,94,0.3)' } },
                  h('div', { style: { fontSize: 10, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.strength', 'Strength')),
                ...
                h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(220,38,38,0.10)', border: '1px solid rgba(220,38,38,0.3)' } },
                  h('div', { style: { fontSize: 10, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.weakness', 'Weakness')),
              ...
              includeSuspensionAnalyzer ? h('div', { style: { padding: 12, borderRadius: 10, background: '#0a0e1a', borderTop: '1px solid var(--allo-stem-border, #334155)', borderRight: '1px solid var(--allo-stem-border, #334155)', borderBottom: '1px solid var(--allo-stem-border, #334155)', borderLeft: '3px solid ' + AMBER, marginTop: 8 } },
                h('div', { style: { fontSize: 13, fontWeight: 800, color: '#fbbf24', marginBottom: 8 } }, t('stem.bridgelab.interactive_catenary_cable_analyzer', '⌣ Interactive catenary cable analyzer')),
                suspensionAnalyzer()
              ) : null,
              includeArchAnalyzer ? h('div', { style: { padding: 12, borderRadius: 10, background: '#0a0e1a', borderTop: '1px solid var(--allo-stem-border, #334155)', borderRight: '1px solid var(--allo-stem-border, #334155)', borderBottom: '1px solid var(--allo-stem-border, #334155)', borderLeft: '3px solid ' + AMBER, marginTop: 8 } },
                h('div', { style: { fontSize: 13, fontWeight: 800, color: '#fbbf24', marginBottom: 8 } }, t('stem.bridgelab.interactive_arch_thrust_analyzer', '∩ Interactive arch thrust analyzer')),
                archAnalyzer()
              ) : null,
              includeBeamAnalyzer ? h('div', { style: { padding: 12, borderRadius: 10, background: '#0a0e1a', borderTop: '1px solid var(--allo-stem-border, #334155)', borderRight: '1px solid var(--allo-stem-border, #334155)', borderBottom: '1px solid var(--allo-stem-border, #334155)', borderLeft: '3px solid ' + AMBER, marginTop: 8 } },
                h('div', { style: { fontSize: 13, fontWeight: 800, color: '#fbbf24', marginBottom: 8 } }, t('stem.bridgelab.interactive_beam_bending_analyzer', '═ Interactive beam bending analyzer')),
                beamAnalyzer()
              ) : null,
              includeCableStayedAnalyzer ? h('div', { style: { padding: 12, borderRadius: 10, background: '#0a0e1a', borderTop: '1px solid var(--allo-stem-border, #334155)', borderRight: '1px solid var(--allo-stem-border, #334155)', borderBottom: '1px solid var(--allo-stem-border, #334155)', borderLeft: '3px solid ' + AMBER, marginTop: 8 } },
                h('div', { style: { fontSize: 13, fontWeight: 800, color: '#fbbf24', marginBottom: 8 } }, t('stem.bridgelab.interactive_cable_stayed_analyzer', '✦ Interactive cable-stayed analyzer')),
                cableStayedAnalyzer()
              ) : null
            );
          });
        ...
        function railwayBridgesSection() {
          var RAIL = [
            { id: 'loads', name: t('stem.bridgelab.railway_bridge_loading_is_different', 'Railway bridge loading is different'), emoji: '🚂',
            ...
            { id: 'fatigue', name: t('stem.bridgelab.fatigue_is_the_dominant_concern', 'Fatigue is the dominant concern'), emoji: '🔁',
            ...
            { id: 'famous', name: t('stem.bridgelab.iconic_tragic_railway_bridges', 'Iconic + tragic railway bridges'), emoji: '🌉',
            ...
            { id: 'hsr', name: t('stem.bridgelab.high_speed_rail_bridges', 'High-speed rail bridges'), emoji: '⚡',
            ...
            { id: 'modern', name: t('stem.bridgelab.modern_railway_bridge_engineering', 'Modern railway bridge engineering'), emoji: '⚙️',
            ...
            { id: 'maine', name: t('stem.bridgelab.maine_railway_bridges', 'Maine railway bridges'), emoji: '🌲',
            ...
            { id: 'maglev', name: t('stem.bridgelab.maglev_the_future', 'Maglev + the future'), emoji: '🚄',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            ...
            h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(59,130,246,0.06)', borderLeft: '3px solid #3b82f6', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#93c5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.what_it_is', 'What it is')),
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.06)', borderLeft: '3px solid #22c55e', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.how_it_works_in_practice', 'How it works in practice')),
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(220,38,38,0.06)', borderLeft: '3px solid #ef4444' } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.honest_limit', 'Honest limit')),
        ...
        function tunnelsAndUnderwaterSection() {
          var TUN = [
            { id: 'cutcover', name: t('stem.bridgelab.cut_and_cover_tunnel', 'Cut-and-cover tunnel'), emoji: '⛏️', era: '1850s-present',
            ...
            { id: 'tbm', name: t('stem.bridgelab.tunnel_boring_machine_tbm', 'Tunnel Boring Machine (TBM)'), emoji: '⚙️', era: '1850s-present',
            ...
            { id: 'natm', name: t('stem.bridgelab.new_austrian_tunneling_method_natm', 'New Austrian Tunneling Method (NATM)'), emoji: '⛰️', era: '1960s-present',
            ...
            { id: 'immersed', name: t('stem.bridgelab.immersed_tube_tunnel', 'Immersed-tube tunnel'), emoji: '🌊', era: '1910s-present',
            ...
            { id: 'subsea', name: t('stem.bridgelab.subsea_bored_tunnel', 'Subsea bored tunnel'), emoji: '🏊', era: '1990s-present',
            ...
            { id: 'ventilation', name: t('stem.bridgelab.ventilation_fire_safety', 'Ventilation + fire safety'), emoji: '🌬️', era: 'Universal',
            ...
            { id: 'channel', name: t('stem.bridgelab.channel_tunnel_the_big_one', 'Channel Tunnel — the big one'), emoji: '🚇', era: '1988-1994',
            ...
            { id: 'limits', name: t('stem.bridgelab.when_not_to_tunnel', 'When NOT to tunnel'), emoji: '🚫', era: 'Engineering judgment',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            ...
            h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(59,130,246,0.06)', borderLeft: '3px solid #3b82f6', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#93c5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.how_it_works', 'How it works')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.how)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.06)', borderLeft: '3px solid #22c55e', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.forces_design_logic', 'Forces + design logic')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.forces)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(220,38,38,0.06)', borderLeft: '3px solid #ef4444' } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.honest_limits', 'Honest limits')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.limit)
              )
            )
          );
        ...
        function movableBridgesSection() {
          var MOV = [
            { id: 'bascule', name: t('stem.bridgelab.bascule_drawbridge', 'Bascule (drawbridge)'), emoji: '⚖️',
            ...
            { id: 'swing', name: t('stem.bridgelab.swing_bridge', 'Swing bridge'), emoji: '🔄',
            ...
            { id: 'lift', name: t('stem.bridgelab.vertical_lift_bridge', 'Vertical lift bridge'), emoji: '⬆️',
            ...
            { id: 'retract', name: t('stem.bridgelab.retractable_rolling_bridge', 'Retractable (rolling) bridge'), emoji: '↩️',
            ...
            { id: 'transporter', name: t('stem.bridgelab.transporter_bridge', 'Transporter bridge'), emoji: '🚠',
            ...
            { id: 'submarine', name: t('stem.bridgelab.submersible_submerged_floating_bridges', 'Submersible (submerged-floating) bridges'), emoji: '🌊',
            ...
            { id: 'pontoon', name: t('stem.bridgelab.floating_pontoon_bridge', 'Floating (pontoon) bridge'), emoji: '🛟',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            ...
            h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
              h('div', { style: { fontSize: 14, fontWeight: 800, color: '#fbbf24', marginBottom: 8 } }, topic.emoji + ' ' + topic.name),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(59,130,246,0.06)', borderLeft: '3px solid #3b82f6', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#93c5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.how_it_works_2', 'How it works')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.how)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.06)', borderLeft: '3px solid #22c55e', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.forces_structural_logic', 'Forces + structural logic')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.forces)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(220,38,38,0.06)', borderLeft: '3px solid #ef4444' } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.failure_modes_history', 'Failure modes + history')),
        ...
        function pedestrianAccessibleBridgesSection() {
          var ACC = [
            { id: 'slope', name: t('stem.bridgelab.slope_grade', 'Slope + grade'), emoji: '⬆️',
            ...
            { id: 'width', name: t('stem.bridgelab.clear_width_passing_zones', 'Clear width + passing zones'), emoji: '↔️',
            ...
            { id: 'surface', name: t('stem.bridgelab.surface_materials', 'Surface materials'), emoji: '🛣️',
            ...
            { id: 'wayfinding', name: t('stem.bridgelab.wayfinding_sensory_cues', 'Wayfinding + sensory cues'), emoji: '🧭',
            ...
            { id: 'rails', name: t('stem.bridgelab.handrails_guardrails', 'Handrails + guardrails'), emoji: '🛡️',
            ...
            { id: 'lighting', name: t('stem.bridgelab.lighting_visibility', 'Lighting + visibility'), emoji: '💡',
            ...
            { id: 'rest', name: t('stem.bridgelab.rest_stops_seating', 'Rest stops + seating'), emoji: '🪑',
            ...
            { id: 'highline', name: t('stem.bridgelab.case_study_the_high_line', 'Case study: The High Line'), emoji: '🌿',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            ...
            h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
              h('div', { style: { fontSize: 14, fontWeight: 800, color: '#fbbf24', marginBottom: 8 } }, topic.emoji + ' ' + topic.name),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(59,130,246,0.06)', borderLeft: '3px solid #3b82f6', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#93c5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.spec', 'Spec')),
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(220,38,38,0.06)', borderLeft: '3px solid #ef4444' } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.common_gotcha', 'Common gotcha')),
            ...
            h('div', { style: { marginTop: 12, padding: 10, borderRadius: 8, background: 'rgba(168,85,247,0.08)', border: '1px solid rgba(168,85,247,0.3)', fontSize: 11.5, color: '#e9d5ff', lineHeight: 1.65 } },
              h('strong', null, t('stem.bridgelab.the_framing_that_matters', 'The framing that matters: ')),
              t('stem.bridgelab.accessibility_is_not_a_special_needs_a', 'Accessibility is not a special-needs accommodation. About 1 in 4 US adults has a disability (CDC 2024). Add older adults, pregnant women, parents with strollers, people pulling luggage, kids on scooters, bike commuters, and the temporary injuries everyone has — and "accessible" design serves a clear MAJORITY of users. The "able-bodied adult on foot" the older codes were written for is the minority case, not the standard case.')
            ),
      ...
      function renderMaterials() {
        return h('div', { style: { padding: 16 } },
          h('p', { style: { color: 'var(--allo-stem-text, #cbd5e1)', fontSize: 13, marginBottom: 12, lineHeight: 1.6 } },
            t('stem.bridgelab.every_material_has_tradeoffs_strength_', 'Every material has tradeoffs. Strength, weight, cost, durability, environmental impact. The right material depends on the structure and the context.')
          ),
          ...
          sectionCard('Key terms',
            h('ul', { style: { margin: 0, padding: '0 0 0 22px', fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } },
              h('li', null, h('strong', null, t('stem.bridgelab.yield_strength', 'Yield strength: ')), t('stem.bridgelab.force_per_unit_area_at_which_a_materia', 'Force per unit area at which a material starts to deform permanently. Above this, the material is damaged even after the load is removed. In MPa (megapascals = N/mm²).')),
              h('li', null, h('strong', null, t('stem.bridgelab.modulus_of_elasticity_e', 'Modulus of elasticity (E): ')), t('stem.bridgelab.how_stiff_the_material_is_higher_e_les', 'How stiff the material is. Higher E = less deformation under the same force. Stress = E * strain. In GPa (gigapascals = 1000 MPa).')),
              h('li', null, h('strong', null, 'Density: '), t('stem.bridgelab.mass_per_unit_volume_determines_self_w', 'Mass per unit volume. Determines self-weight of the structure. A heavy material requires bigger members to support its own weight before any external load.')),
              h('li', null, h('strong', null, 'Ductility: '), t('stem.bridgelab.how_much_a_material_deforms_before_bre', 'How much a material deforms before breaking. Ductile (steel) gives warning. Brittle (cast iron, stone) breaks suddenly.'))
            )
          ),
          ...
          sectionCard('🔧 Repair + retrofit — extending bridge life',
            (function() {
              var TECHNIQUES = [
                { id: 'patch', name: t('stem.bridgelab.concrete_patch_repair', 'Concrete patch repair'), color: '#fbbf24',
                ...
                { id: 'cfrp', name: t('stem.bridgelab.cfrp_carbon_fiber_strengthening', 'CFRP (carbon-fiber) strengthening'), color: '#0ea5e9',
                ...
                { id: 'jacket', name: t('stem.bridgelab.steel_concrete_jacketing', 'Steel + concrete jacketing'), color: '#a855f7',
                ...
                { id: 'deck', name: t('stem.bridgelab.deck_replacement', 'Deck replacement'), color: '#22c55e',
                ...
                { id: 'cathodic', name: t('stem.bridgelab.cathodic_protection', 'Cathodic protection'), color: '#86efac',
                ...
                { id: 'scour', name: t('stem.bridgelab.scour_countermeasures', 'Scour countermeasures'), color: '#dc2626',
                ...
                { id: 'replace', name: t('stem.bridgelab.full_replacement', 'Full replacement'), color: 'var(--allo-stem-text-soft, #94a3b8)',
              ...
              return h('div', null,
                h('p', { style: { margin: '0 0 12px', fontSize: 13, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } },
                  t('stem.bridgelab.about_90_of_bridge_engineering_work_is', 'About 90% of bridge engineering work is on EXISTING bridges, not new ones. Repair + retrofit extend life at a fraction of replacement cost. The right intervention depends on what\'s deteriorating + how badly.')
                ),
                ...
                h('div', { style: { padding: 14, borderRadius: 12, background: 'var(--allo-stem-canvas, #0f172a)', borderTop: '1px solid var(--allo-stem-border, #334155)', borderRight: '1px solid var(--allo-stem-border, #334155)', borderBottom: '1px solid var(--allo-stem-border, #334155)', borderLeft: '3px solid ' + sel.color } },
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(56,189,248,0.08)', borderLeft: '3px solid #38bdf8', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#7dd3fc', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.how_it_works_3', 'How it works')),
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(34,197,94,0.08)', borderLeft: '3px solid #22c55e', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.when_to_use_it', 'When to use it')),
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(220,38,38,0.08)', borderLeft: '3px solid #dc2626' } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.limits_honest_caveats', 'Limits + honest caveats')),
              ...
              return h('div', null,
                ...
                (function() {
                  ...
                  return h('svg', { viewBox: '0 0 ' + svgW + ' ' + svgH, width: '100%', height: svgH, role: 'img', 'aria-labelledby': 'preTitle preDesc' },
                    h('title', { id: 'preTitle' }, t('stem.bridgelab.prestressed_beam_stress_diagram', 'Prestressed beam stress diagram')),
                    ...
                    h('text', { x: 50 + 250, y: 20, textAnchor: 'middle', fill: '#86efac', fontSize: 11 }, t('stem.bridgelab.external_load_causes_bending', 'External load (causes bending)')),
                    // Result label
                ...
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginTop: 12, marginBottom: 12 } },
                  [
                    { label: t('stem.bridgelab.precompression_from_tendons_mpa', 'Precompression from tendons (MPa)'), value: compStress, min: 0, max: 25, step: 1, key: 'preCompStress' },
                    { label: t('stem.bridgelab.bending_stress_from_external_load_mpa', 'Bending stress from external load (MPa)'), value: loadStress, min: 0, max: 30, step: 1, key: 'preLoadStress' }
                  ].map(function(s, i) {
                    return h('div', { key: i, style: { padding: 8, borderRadius: 6, background: 'var(--allo-stem-panel, #1e293b)', border: '1px solid var(--allo-stem-border, #334155)' } },
                      h('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 4 } },
                        h('span', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)', fontWeight: 700 } }, s.label),
                ...
                h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.10)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 12, color: '#c7d2fe', lineHeight: 1.65, marginBottom: 8 } },
                  h('strong', null, t('stem.bridgelab.two_methods_of_prestressing', 'Two methods of prestressing: ')),
                  h('ul', { style: { margin: '6px 0 0 22px', padding: 0, lineHeight: 1.7 } },
                    h('li', null, h('strong', null, 'Pre-tensioning: '), t('stem.bridgelab.tendons_are_tensioned_before_concrete_', 'Tendons are tensioned BEFORE concrete is poured + before it cures. Once concrete hardens + bonds to the tendons, anchors are released; the tendons pull inward through friction. Used for FACTORY-MADE beams (highway girders, double-tees, hollow-core planks).')),
                    h('li', null, h('strong', null, 'Post-tensioning: '), t('stem.bridgelab.concrete_is_poured_first_with_hollow_d', 'Concrete is poured first WITH HOLLOW DUCTS through it. After curing, tendons are threaded through, tensioned hydraulically, and anchored at the ends. Used for IN-PLACE construction (bridges, slabs, dams, nuclear reactor containment).'))
                  )
                ),
          ...
          sectionCard('🏗️ Foundations + soil mechanics — what the bridge stands on',
            (function() {
              var SOILS = [
                { id: 'rock', name: t('stem.bridgelab.solid_rock_bedrock', 'Solid rock (bedrock)'), bearingMPa: 10, color: 'var(--allo-stem-text-soft, #475569)',
                  desc: t('stem.bridgelab.granite_basalt_sound_limestone_the_str', 'Granite, basalt, sound limestone. The strongest natural foundation. Allowable bearing 5-15 MPa (and much higher with engineered anchors).'),
                ...
                { id: 'gravel', name: t('stem.bridgelab.dense_sand_gravel', 'Dense sand + gravel'), bearingMPa: 0.5, color: '#a16207',
                  desc: t('stem.bridgelab.glacial_outwash_well_graded_compacted_', 'Glacial outwash, well-graded compacted gravel. Allowable bearing typically 200-600 kPa.'),
                ...
                { id: 'clay', name: t('stem.bridgelab.stiff_clay', 'Stiff clay'), bearingMPa: 0.2, color: 'var(--allo-stem-text-soft, #a3a3a3)',
                  desc: t('stem.bridgelab.cohesive_fine_grained_allowable_bearin', 'Cohesive, fine-grained. Allowable bearing typically 100-300 kPa for stiff clay; much lower for soft clay.'),
                ...
                { id: 'soft', name: t('stem.bridgelab.soft_clay_silt', 'Soft clay / silt'), bearingMPa: 0.05, color: '#a3a3a3',
                  desc: t('stem.bridgelab.marine_deposits_lake_bed_recently_depo', 'Marine deposits, lake bed, recently-deposited mud. Allowable bearing 25-100 kPa.'),
                ...
                { id: 'fill', name: t('stem.bridgelab.loose_fill_made_ground', 'Loose fill / made ground'), bearingMPa: 0.05, color: '#a3a3a3',
                  desc: t('stem.bridgelab.man_made_fill_demolition_rubble_uncont', 'Man-made fill, demolition rubble, uncontrolled placement. Highly variable.'),
                ...
                { id: 'water', name: t('stem.bridgelab.underwater_river_bed_ocean_floor', 'Underwater (river bed, ocean floor)'), bearingMPa: 0.1, color: '#1e40af',
                  desc: t('stem.bridgelab.saturated_sand_mud_or_gravel_bearing_d', 'Saturated sand, mud, or gravel. Bearing depends on the specific material.'),
              ...
              var FOUNDATION_TYPES = [
                { name: t('stem.bridgelab.spread_footing', 'Spread footing'), use: 'Shallow, when good soil is within a few meters. Square or rectangular concrete pad spreads load.', cost: 'Cheap' },
                { name: t('stem.bridgelab.mat_foundation', 'Mat foundation'), use: 'Large continuous slab for multiple columns or when load is too concentrated. Common for skyscrapers and large bridge piers.', cost: 'Medium' },
                { name: t('stem.bridgelab.driven_piles', 'Driven piles'), use: 'Steel or concrete columns hammered down to bearing layer. Sound was the warning bell of soundness for medieval pile driving — Venice is built on millions of wooden piles still functioning after 1500+ years.', cost: 'Medium-high' },
                { name: t('stem.bridgelab.drilled_shafts_caissons', 'Drilled shafts (caissons)'), use: 'Holes drilled to bearing layer + filled with reinforced concrete. Used where soils are too hard for driving but require deep foundation.', cost: 'High' },
                { name: t('stem.bridgelab.spread_caisson', 'Spread caisson'), use: 'A large hollow box sunk into the soil + filled with concrete. Brooklyn Bridge caissons (1869-72) were among the first. Workers inside got "caisson disease" (decompression sickness) — the bridge cost many lives.', cost: 'Very high' }
              ];
              return h('div', null,
                h('p', { style: { margin: '0 0 12px', fontSize: 13, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } },
                  t('stem.bridgelab.every_bridge_stands_on_something_the_s', 'Every bridge stands on something. The strongest member designed by the best engineer fails if the FOUNDATION does. The 1907 Quebec Bridge, 1967 Silver Bridge, 1987 Schoharie Creek, and many others are partly foundation/anchorage failures. Soil mechanics + foundation engineering are as important as the superstructure design.')
                ),
                h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 } },
                ...
                h('div', { style: { padding: 14, borderRadius: 12, background: 'var(--allo-stem-canvas, #0f172a)', borderTop: '1px solid var(--allo-stem-border, #334155)', borderRight: '1px solid var(--allo-stem-border, #334155)', borderBottom: '1px solid var(--allo-stem-border, #334155)', borderLeft: '4px solid ' + sel.color, marginBottom: 12 } },
                  ...
                  h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 } },
                    h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(34,197,94,0.10)', borderLeft: '3px solid #22c55e' } },
                      h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.good_for', 'Good for')),
                    ...
                    h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(220,38,38,0.10)', borderLeft: '3px solid #dc2626' } },
                      h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.watch_out_for', 'Watch out for')),
                ...
                h('div', { style: { padding: 10, borderRadius: 8, background: 'var(--allo-stem-panel, #1e293b)', border: '1px solid var(--allo-stem-border, #334155)', marginBottom: 12 } },
                  h('div', { style: { fontSize: 12, color: 'var(--allo-stem-text-soft, #94a3b8)', marginBottom: 6, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 } }, t('stem.bridgelab.your_current_bridge_s_required_footing', 'Your current bridge\'s required footing')),
                  h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.65 } },
                    t('stem.bridgelab.reaction_at_each_support', 'Reaction at each support: '), h('strong', { style: { color: '#93c5fd' } }, reaction.toFixed(0) + ' kN'),
                    t('stem.bridgelab.on', ' · On '), h('strong', { style: { color: '#fbbf24' } }, sel.name.toLowerCase()),
                    ...
                    t('stem.bridgelab.or_a_square_footing_of', ' (or a square footing of ≈ '), h('strong', { style: { color: '#fbbf24' } }, footingSide.toFixed(1) + ' m'),
                    t('stem.bridgelab.on_each_side', ' on each side).')
                  )
                ),
          ...
          sectionCard('🌍 Sustainable bridge design + embodied carbon',
            (function() {
              ...
              var EC_DATA = {
                wood:     { kgCO2perKg: 0.43, recycle: '~80% reclaimed or burned for energy', durability: '50-100 yr w/ treatment + maintenance', note: t('stem.bridgelab.treated_wood_has_higher_ec_sustainably', 'Treated wood has higher EC; sustainably-managed forest wood has near-zero net EC over its lifetime (CO₂ sequestered while growing).') },
                stone:    { kgCO2perKg: 0.07, recycle: '~100% can be reused as crushed aggregate', durability: '500-2000+ yr', note: t('stem.bridgelab.lowest_embodied_carbon_of_any_building', 'Lowest embodied carbon of any building material if quarried near use. But quarrying disturbs ecosystems + transport costs energy.') },
                iron:     { kgCO2perKg: 2.0,  recycle: '~75% recycled', durability: '50-100 yr', note: t('stem.bridgelab.cast_iron_is_brittle_has_been_largely_', 'Cast iron is brittle + has been largely phased out for structural use. Lower embodied carbon than steel but worse mechanical properties.') },
                steel:    { kgCO2perKg: 1.99, recycle: '~88% recycled globally (highest of any material)', durability: '100-150+ yr', note: t('stem.bridgelab.new_steel_production_is_carbon_intensi', 'New steel production is carbon-intensive (most via blast furnace + coking coal). Recycled steel via electric-arc furnace cuts emissions by 70%+. "Green steel" via hydrogen is being deployed (HYBRIT in Sweden, ArcelorMittal in Spain).') },
                concrete: { kgCO2perKg: 0.13, recycle: '~30% recycled, mostly as aggregate', durability: '50-100 yr', note: t('stem.bridgelab.concrete_itself_has_low_embodied_carbo', 'Concrete itself has low embodied carbon per kg, but bridges use VAST amounts. Cement production alone is ~8% of global CO₂ emissions. Supplementary cementitious materials (fly ash, slag, calcined clay) cut this by 30-50%.') },
                composite:{ kgCO2perKg: 8.0,  recycle: '<10% recycled (hard to separate fibers + resin)', durability: '50-75 yr', note: t('stem.bridgelab.high_embodied_carbon_per_kg_difficult_', 'High embodied carbon per kg + difficult end-of-life. But weight savings can reduce total material. Best for retrofits + specialty applications, not yet competitive for primary structure.') }
              };
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', borderTop: '1px solid var(--allo-stem-border, #334155)', borderRight: '1px solid var(--allo-stem-border, #334155)', borderBottom: '1px solid var(--allo-stem-border, #334155)', borderLeft: '3px solid #22c55e', marginBottom: 12 } },
                  ...
                  h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8, marginBottom: 8 } },
                    [
                      { label: t('stem.bridgelab.mass', 'Mass'), value: (massKg / 1000).toFixed(1) + ' metric tons', color: 'var(--allo-stem-text, #cbd5e1)' },
                      { label: t('stem.bridgelab.embodied_co', 'Embodied CO₂'), value: (embCO2 / 1000).toFixed(2) + ' metric tons CO₂e', color: '#fca5a5' },
                      { label: t('stem.bridgelab.person_years_of_emissions', '= person-years of emissions'), value: equivPersonYears.toFixed(1) + ' US person-years', color: '#fbbf24' },
                      { label: t('stem.bridgelab.ec_factor', 'EC factor'), value: data.kgCO2perKg + ' kg CO₂e/kg', color: 'var(--allo-stem-text-soft, #94a3b8)' }
                    ].map(function(s, i) {
                ...
                h('div', { style: { padding: 10, borderRadius: 8, background: 'var(--allo-stem-panel, #1e293b)', border: '1px solid var(--allo-stem-border, #334155)', marginBottom: 10 } },
                  ...
                  h('div', { style: { fontSize: 12, color: 'var(--allo-stem-text, #e2e8f0)', marginBottom: 4 } }, h('strong', null, t('stem.bridgelab.typical_durability', 'Typical durability: ')), data.durability),
                ...
                h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.10)', border: '1px solid rgba(34,197,94,0.3)', fontSize: 12, color: '#a7f3d0', lineHeight: 1.65 } },
                  h('strong', null, t('stem.bridgelab.sustainable_design_strategies', 'Sustainable design strategies: ')),
                  h('ul', { style: { margin: '6px 0 0 22px', padding: 0, lineHeight: 1.7 } },
                    h('li', null, h('strong', null, t('stem.bridgelab.optimize_for_the_lifetime', 'Optimize for the LIFETIME: ')), t('stem.bridgelab.a_bridge_that_lasts_150_years_has_lowe', 'A bridge that lasts 150 years has lower per-year carbon than one rebuilt at 60. Long-term durability planning matters.')),
                    h('li', null, h('strong', null, t('stem.bridgelab.use_less_material', 'Use less material: ')), t('stem.bridgelab.high_strength_steel_advanced_cross_sec', 'High-strength steel + advanced cross-sections + optimized geometry can cut material 10-30% with no loss of capacity. The cost-optimizer in the Build tab is doing this.')),
                    h('li', null, h('strong', null, t('stem.bridgelab.recycled_content', 'Recycled content: ')), t('stem.bridgelab.specify_minimum_recycled_steel_content', 'Specify minimum recycled steel content (typically 70-90% achievable). Replace 30-50% of cement with fly ash, slag, or calcined clay.')),
                    h('li', null, h('strong', null, t('stem.bridgelab.design_for_disassembly', 'Design for disassembly: ')), t('stem.bridgelab.bolted_modular_connections_enable_salv', 'Bolted + modular connections enable salvage at end of life. Older welded + rusted bridges get scrapped; modular ones get repurposed.')),
                    h('li', null, h('strong', null, t('stem.bridgelab.local_sourcing', 'Local sourcing: ')), t('stem.bridgelab.transport_adds_embodied_carbon_sourcin', 'Transport adds embodied carbon. Sourcing materials within 500 km can cut total emissions 15-25%.'))
                  )
                ),
                h('div', { style: { marginTop: 8, padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.10)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 12, color: '#c7d2fe', lineHeight: 1.65 } },
                  h('strong', null, t('stem.bridgelab.why_this_matters_now', 'Why this matters now: ')),
                  t('stem.bridgelab.infrastructure_construction_accounts_f', 'Infrastructure construction accounts for ~25-30% of global emissions. The 1971 National Bridge Inspection Standards (post-Silver-Bridge) created a database showing about 1 in 13 US bridges is in poor condition; thousands need replacement in the next 20 years. The Infrastructure Investment and Jobs Act (2021) funds bridge replacement at scale. Done well: lower-carbon materials + longer service life. Done poorly: same emissions, repeating the same mistakes.')
                )
              );
            })(),
            '#22c55e'
            ...
            (function() {
              ...
              function snCurveSvg() {
                ...
                return h('svg', { viewBox: '0 0 ' + svgW + ' ' + svgH, width: '100%', height: svgH, role: 'img', 'aria-labelledby': 'snTitle snDesc' },
                  h('title', { id: 'snTitle' }, t('stem.bridgelab.s_n_curve_stress_vs_cycles_to_failure', 'S-N curve (stress vs cycles to failure)')),
                  ...
                  // Axis labels
                  h('text', { x: padL + plotW / 2, y: svgH - 8, textAnchor: 'middle', fill: '#cbd5e1', fontSize: 11 }, t('stem.bridgelab.cycles_to_failure_log_scale', 'Cycles to failure (log scale)')),
                  h('text', { x: 14, y: padT + plotH / 2, transform: 'rotate(-90 14 ' + (padT + plotH / 2) + ')', textAnchor: 'middle', fill: '#cbd5e1', fontSize: 11 }, t('stem.bridgelab.stress_amplitude', 'Stress amplitude'))
                );
              }

              return h('div', null,
                h('p', { style: { margin: '0 0 10px', fontSize: 13, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } },
                  t('stem.bridgelab.a_bridge_that_handles_its_design_load_', 'A bridge that handles its design load just fine on day one can still fail decades later from FATIGUE: tiny cracks that grow microscopically with every load cycle, until one final cycle propagates a crack to failure. Every truck crossing, every wind gust, every temperature swing is a cycle. A heavily-trafficked highway bridge sees 5,000-50,000 cycles per day; over 50 years that\'s ~10⁸-10⁹ cycles.')
                ),
                snCurveSvg(),
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginTop: 12, marginBottom: 12 } },
                  [
                    { label: t('stem.bridgelab.stress_amplitude_mpa', 'Stress amplitude (MPa)'), value: stress, min: 50, max: 250, step: 10, key: 'fatigueStressMPa' },
                    { label: t('stem.bridgelab.cycles_per_day', 'Cycles per day'), value: cycles, min: 100, max: 100000, step: 100, key: 'fatigueCyclesPerDay' }
                  ].map(function(s, i) {
                    return h('div', { key: i, style: { padding: 8, borderRadius: 6, background: 'var(--allo-stem-panel, #1e293b)', border: '1px solid var(--allo-stem-border, #334155)' } },
                      h('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 4 } },
                        h('span', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)', fontWeight: 700 } }, s.label),
                ...
                h('div', { style: { marginTop: 10, padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.10)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 11.5, color: '#c7d2fe', lineHeight: 1.65 } },
                  h('strong', null, t('stem.bridgelab.endurance_limit_steel_only', 'Endurance limit (steel only): ')),
                  t('stem.bridgelab.some_steels_have_a_special_property_be', 'Some steels have a special property — below a stress threshold (~100 MPa for typical structural steel), fatigue cracks essentially don\'t grow. This "endurance limit" lets steel bridges last centuries IF designed conservatively below it. Aluminum and most non-ferrous metals have NO endurance limit — they always fatigue eventually.')
                ),
        ...
        function demolitionAndDecommissioningSection() {
          var DEMO = [
            { id: 'why', name: t('stem.bridgelab.when_why_bridges_end', 'When + why bridges end'), emoji: '⌛',
            ...
            { id: 'explosive', name: t('stem.bridgelab.explosive_controlled_demolition', 'Explosive (controlled) demolition'), emoji: '💥',
            ...
            { id: 'sawcut', name: t('stem.bridgelab.mechanical_saw_cut_demolition', 'Mechanical + saw-cut demolition'), emoji: '🪚',
            ...
            { id: 'piecemeal', name: t('stem.bridgelab.piece_by_piece_decking_removal', 'Piece-by-piece + decking removal'), emoji: '🧱',
            ...
            { id: 'recycle', name: t('stem.bridgelab.recycling_material_recovery', 'Recycling + material recovery'), emoji: '♻️',
            ...
            { id: 'environment', name: t('stem.bridgelab.environmental_community_impact', 'Environmental + community impact'), emoji: '🌊',
            ...
            { id: 'famous', name: t('stem.bridgelab.famous_demolitions', 'Famous demolitions'), emoji: '🎬',
            ...
            { id: 'cycle', name: t('stem.bridgelab.demolition_as_engineering_education', 'Demolition as engineering education'), emoji: '🎓',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            ...
            h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
              h('div', { style: { fontSize: 14, fontWeight: 800, color: '#fbbf24', marginBottom: 8 } }, topic.emoji + ' ' + topic.name),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(59,130,246,0.06)', borderLeft: '3px solid #3b82f6', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#93c5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.what_happens', 'What happens')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.what)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.06)', borderLeft: '3px solid #22c55e', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.how_it_works_4', 'How it works')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.how)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(220,38,38,0.06)', borderLeft: '3px solid #ef4444' } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.honest_limit_2', 'Honest limit')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.limit)
              )
            )
          );
        ...
        function shmAndInspectionSection() {
          var SHM = [
            { id: 'visual', name: t('stem.bridgelab.visual_inspection_nbis', 'Visual inspection (NBIS)'), emoji: '👁️',
            ...
            { id: 'access', name: t('stem.bridgelab.hands_on_access', 'Hands-on access'), emoji: '🪜',
            ...
            { id: 'drone', name: t('stem.bridgelab.drone_based_inspection', 'Drone-based inspection'), emoji: '🛸',
            ...
            { id: 'sensor', name: t('stem.bridgelab.strain_gauges_accelerometers', 'Strain gauges + accelerometers'), emoji: '📊',
            ...
            { id: 'fiber', name: t('stem.bridgelab.distributed_fiber_optic_sensing', 'Distributed fiber-optic sensing'), emoji: '〰️',
            ...
            { id: 'gpr', name: t('stem.bridgelab.ground_penetrating_radar_ultrasonics', 'Ground-penetrating radar + ultrasonics'), emoji: '📡',
            ...
            { id: 'genoa', name: t('stem.bridgelab.when_shm_fails_the_genoa_case', 'When SHM fails — the Genoa case'), emoji: '⚠️',
            ...
            { id: 'ml', name: t('stem.bridgelab.ai_machine_learning_in_inspection', 'AI + machine learning in inspection'), emoji: '🤖',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            h('h3', { style: { margin: '0 0 6px', color: '#fbbf24', fontSize: 16 } }, t('stem.bridgelab.bridge_inspection_structural_health_mo', '🔍 Bridge inspection + Structural Health Monitoring (SHM)')),
            h('p', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #cbd5e1)', lineHeight: 1.65, margin: '0 0 12px' } },
              t('stem.bridgelab.a_bridge_is_not_designed_once_left_alo', 'A bridge is not designed once + left alone. It is inspected on a defined cycle, and increasingly, it is monitored continuously through sensors, drones, and AI-assisted analysis. The shift from "inspect every 2 years" to "monitor in real time" is one of the major operational changes in infrastructure engineering of the past 30 years. The technology is genuinely powerful + has clear limits.')
            ),
            h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 } },
              SHM.map(function(t) {
                var on = t.id === sel;
            ...
            h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
              h('div', { style: { fontSize: 14, fontWeight: 800, color: '#fbbf24', marginBottom: 8 } }, topic.emoji + ' ' + topic.name),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(59,130,246,0.06)', borderLeft: '3px solid #3b82f6', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#93c5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.technique_how_it_works', 'Technique + how it works')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.tech)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(220,38,38,0.06)', borderLeft: '3px solid #ef4444' } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.honest_limit_3', 'Honest limit')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.limit)
              )
            ),
        ...
        function extremeEnvironmentsSection() {
          var EXT = [
            { id: 'permafrost', name: t('stem.bridgelab.permafrost_thaw', 'Permafrost + thaw'), emoji: '❄️', where: 'Alaska, Yukon, Siberia, Northern Quebec',
            ...
            { id: 'ice', name: t('stem.bridgelab.ice_loads_ice_impact', 'Ice loads + ice impact'), emoji: '🧊', where: 'Saint Lawrence, Great Lakes, Baltic, Northern Norway',
            ...
            { id: 'wind', name: t('stem.bridgelab.sustained_high_wind_sites', 'Sustained high-wind sites'), emoji: '🌪️', where: 'Strait of Magellan, Scottish coasts, Hokkaido, Patagonia, Bering Strait proposals',
            ...
            { id: 'seismic', name: t('stem.bridgelab.high_seismic_zones', 'High-seismic zones'), emoji: '⚠️', where: 'California, Japan, Taiwan, Iran, Chile, New Zealand',
            ...
            { id: 'corrosion', name: t('stem.bridgelab.coastal_salt_corrosion', 'Coastal + salt corrosion'), emoji: '🌊', where: 'Florida Keys, Caribbean, Persian Gulf, Maine coast, Gulf coast',
            ...
            { id: 'wildfire', name: t('stem.bridgelab.wildfire_exposed_crossings', 'Wildfire-exposed crossings'), emoji: '🔥', where: 'California, Australia, Mediterranean Europe, parts of the western US',
            ...
            { id: 'remote', name: t('stem.bridgelab.remote_indigenous_territory_crossings', 'Remote + Indigenous-territory crossings'), emoji: '🛤️', where: 'Northern Canada, Alaska, Greenland, Russian Far North, Tibet',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            ...
            h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(220,38,38,0.06)', borderLeft: '3px solid #ef4444', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.the_challenge', 'The challenge')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.challenge)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.06)', borderLeft: '3px solid #22c55e', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.engineering_responses', 'Engineering responses')),
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(168,85,247,0.06)', borderLeft: '3px solid #a78bfa' } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#c4b5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.honest_limits_2', 'Honest limits')),
      ...
      function renderForces() {
        ...
        var CONNECTIONS = [
          { id: 'rivet', name: t('stem.bridgelab.rivets', 'Rivets'),
          ...
          { id: 'bolted', name: t('stem.bridgelab.high_strength_bolts', 'High-strength bolts'),
          ...
          { id: 'weld', name: t('stem.bridgelab.welds', 'Welds'),
        ...
        return h('div', { style: { padding: 16 } },
          h('p', { style: { color: 'var(--allo-stem-text, #cbd5e1)', fontSize: 13, marginBottom: 12, lineHeight: 1.6 } },
            t('stem.bridgelab.five_types_of_force_matter_for_structu', 'Five types of force matter for structural engineering. Every part of every bridge experiences some combination of these.')
          ),
          ...
          h('div', { style: { padding: 14, borderRadius: 12, background: 'var(--allo-stem-panel, #1e293b)', borderTop: '1px solid var(--allo-stem-border, #334155)', borderRight: '1px solid var(--allo-stem-border, #334155)', borderBottom: '1px solid var(--allo-stem-border, #334155)', borderLeft: '3px solid ' + selected.color, marginBottom: 16 } },
            ...
            h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 } },
              h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(34,197,94,0.10)', border: '1px solid rgba(34,197,94,0.3)' } },
                h('div', { style: { fontSize: 10, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.materials_that_handle_it_well', 'Materials that handle it well')),
                h('div', { style: { fontSize: 12, color: '#dcfce7', lineHeight: 1.55 } }, selected.good)
              ),
              h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(220,38,38,0.10)', border: '1px solid rgba(220,38,38,0.3)' } },
                h('div', { style: { fontSize: 10, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.watch_out_for_2', 'Watch out for')),
            ...
            (function() {
              ...
              return h('div', null,
                ...
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 8, marginBottom: 12 } },
                  [
                    { label: t('stem.bridgelab.peak_ground_acceleration_pga', 'Peak ground acceleration (PGA)'), value: pga.toFixed(3) + ' g', color: pga > 0.3 ? '#fca5a5' : pga > 0.1 ? '#fbbf24' : '#86efac', sub: 'fraction of gravity' },
                    { label: t('stem.bridgelab.mmi_intensity', 'MMI intensity'), value: mmi, color: '#c7d2fe', sub: 'Modified Mercalli' },
                    { label: t('stem.bridgelab.lateral_force_on_bridge', 'Lateral force on bridge'), value: lateralForce.toFixed(0) + ' kN', color: '#fca5a5', sub: 'F = m·a (inertia)' },
                    { label: t('stem.bridgelab.energy_release', 'Energy release'), value: '×' + Math.pow(10, 1.5 * (magnitude - 6)).toFixed(1), color: '#a78bfa', sub: 'vs M6 reference (each magnitude = 32× energy)' }
                  ].map(function(s, i) {
                    return h('div', { key: i, style: { padding: 8, borderRadius: 6, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
                      h('div', { style: { fontSize: 10, color: 'var(--allo-stem-text-soft, #94a3b8)', textTransform: 'uppercase', letterSpacing: 0.5 } }, s.label),
                      h('div', { style: { fontSize: 14, fontWeight: 800, color: s.color, marginTop: 2 } }, s.value),
                ...
                h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.10)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 12, color: '#c7d2fe', lineHeight: 1.65, marginBottom: 10 } },
                  h('strong', null, t('stem.bridgelab.three_seismic_design_strategies', 'Three seismic design strategies: ')),
                  h('ol', { style: { margin: '6px 0 0 22px', padding: 0, lineHeight: 1.7 } },
                    h('li', null, h('strong', null, 'Strength: '), t('stem.bridgelab.design_the_structure_to_elastically_re', 'Design the structure to elastically resist seismic forces. Works for moderate quakes but becomes uneconomic for major ones — a strength-only bridge would need to be extraordinarily massive.')),
                    h('li', null, h('strong', null, t('stem.bridgelab.ductility_the_modern_approach', 'Ductility (the modern approach): ')), t('stem.bridgelab.design_specific_elements_to_yield_defo', 'Design specific elements to YIELD + DEFORM plastically during major quakes — absorbing energy without rupturing. Plastic hinges in columns are common. Steel works wonderfully here; brittle materials (cast iron, plain concrete) do NOT. The bridge sustains damage but doesn\'t collapse.')),
                    h('li', null, h('strong', null, t('stem.bridgelab.base_isolation', 'Base isolation: ')), t('stem.bridgelab.place_the_bridge_superstructure_on_lea', 'Place the bridge superstructure on lead-rubber bearings or sliding bearings. The ground shakes; the bearings absorb most of the motion; the bridge above moves much less. Most aggressive new approach. Common in Japanese + Chilean + California bridges.'))
                  )
                ),

                h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.10)', border: '1px solid rgba(34,197,94,0.3)', fontSize: 12, color: '#a7f3d0', lineHeight: 1.65, marginBottom: 8 } },
                  h('strong', null, t('stem.bridgelab.akashi_kaiky_1995_kobe_earthquake_mid_', 'Akashi-Kaikyō (1995 Kobe earthquake mid-construction): ')),
                  t('stem.bridgelab.the_akashi_kaiky_bridge_was_being_buil', 'The Akashi-Kaikyō Bridge was being built when the M6.9 Hyogoken-Nanbu (Kobe) earthquake hit on January 17, 1995. The two towers moved 1 METER farther apart. Engineers redesigned the deck mid-construction to accommodate the new geometry. The bridge survived + opened in 1998. A real-world case study in seismic resilience under extreme + unpredictable conditions.')
                ),
          ...
          sectionCard('📡 Smart bridges + structural health monitoring (SHM)',
            (function() {
              var SENSORS = [
                { id: 'strain', name: t('stem.bridgelab.strain_gauges', 'Strain gauges'), color: '#fbbf24',
                ...
                { id: 'fbg', name: t('stem.bridgelab.fiber_bragg_gratings_fbg', 'Fiber Bragg gratings (FBG)'), color: '#7dd3fc',
                ...
                { id: 'accel', name: t('stem.bridgelab.accelerometers', 'Accelerometers'), color: '#a855f7',
                ...
                { id: 'gps', name: t('stem.bridgelab.gps_displacement_sensors', 'GPS displacement sensors'), color: '#86efac',
                ...
                { id: 'corrosion', name: t('stem.bridgelab.corrosion_sensors', 'Corrosion sensors'), color: '#dc2626',
                ...
                { id: 'temp', name: t('stem.bridgelab.temperature_sensors', 'Temperature sensors'), color: '#0ea5e9',
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 12, background: 'var(--allo-stem-canvas, #0f172a)', borderTop: '1px solid var(--allo-stem-border, #334155)', borderRight: '1px solid var(--allo-stem-border, #334155)', borderBottom: '1px solid var(--allo-stem-border, #334155)', borderLeft: '3px solid ' + sel.color } },
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(56,189,248,0.08)', borderLeft: '3px solid #38bdf8', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#7dd3fc', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.what_it_measures', 'What it measures')),
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(245,158,11,0.08)', borderLeft: '3px solid #f59e0b', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#fbbf24', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.how_it_works_5', 'How it works')),
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(168,85,247,0.08)', borderLeft: '3px solid #a855f7', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#d8b4fe', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.real_world_examples', 'Real-world examples')),
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(34,197,94,0.08)', borderLeft: '3px solid #22c55e' } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.why_it_matters', 'Why it matters')),
                ...
                h('div', { style: { marginTop: 8, padding: 10, borderRadius: 8, background: 'rgba(220,38,38,0.10)', border: '1px solid rgba(220,38,38,0.3)', fontSize: 12, color: '#fecaca', lineHeight: 1.65 } },
                  h('strong', null, t('stem.bridgelab.i_35w_mississippi_river_bridge_collaps', 'I-35W Mississippi River Bridge collapse (2007): ')),
                  t('stem.bridgelab.the_minneapolis_bridge_collapsed_durin', 'The Minneapolis bridge collapsed during evening rush hour, killing 13 + injuring 145. Investigation showed a design flaw (undersized gusset plates) combined with construction loads (resurfacing materials piled on the deck). The replacement bridge (St. Anthony Falls Bridge, 2008) was instrumented with 323 sensors during construction — strain gauges, accelerometers, temperature sensors, corrosion sensors. It is one of the most heavily-monitored bridges in the world. Lessons from the collapse drove federal investment in SHM technology across US bridges.')
                )
              );
            })(),
            '#0ea5e9'
            ...
            h('div', null,
              ...
              h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', borderTop: '1px solid var(--allo-stem-border, #334155)', borderRight: '1px solid var(--allo-stem-border, #334155)', borderBottom: '1px solid var(--allo-stem-border, #334155)', borderLeft: '3px solid ' + AMBER } },
                ...
                h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(56,189,248,0.10)', borderLeft: '3px solid #38bdf8', marginBottom: 8 } },
                  h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#7dd3fc', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.history', 'History')),
                ...
                h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(245,158,11,0.10)', borderLeft: '3px solid ' + AMBER, marginBottom: 8 } },
                  h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#fbbf24', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.how_it_works_6', 'How it works')),
                ...
                h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(220,38,38,0.10)', borderLeft: '3px solid #dc2626', marginBottom: 8 } },
                  h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.how_it_fails', 'How it fails')),
                  h('div', { style: { fontSize: 12, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.55 } }, sel.failure)
                ),
                h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(34,197,94,0.08)', borderLeft: '3px solid #22c55e' } },
                  h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.bridgelab.still_used', 'Still used')),
              ...
              h('div', { style: { marginTop: 10, padding: 10, borderRadius: 8, background: 'rgba(168,85,247,0.10)', border: '1px solid rgba(168,85,247,0.3)', fontSize: 11.5, color: '#e9d5ff', lineHeight: 1.6 } },
                h('strong', null, t('stem.bridgelab.the_hyatt_regency_lesson', 'The Hyatt Regency lesson: ')),
                t('stem.bridgelab.the_1981_walkway_collapse_was_a_connec', 'The 1981 walkway collapse was a connection failure. A late design change replaced one long rod (suspending both walkways) with two shorter rods (each walkway hung from the next). The change DOUBLED the load on the upper connection. No one re-engineered. 114 dead, 216 injured. Engineer of record carries final responsibility — both lead engineers lost their licenses. Now required reading in engineering ethics curricula.')
              )
            ),
            AMBER
          ),
        ...
        function seismicEngineeringSection() {
          var SEIS = [
            { id: 'why', name: t('stem.bridgelab.why_bridges_are_seismic_hard', 'Why bridges are seismic-hard'), emoji: '⚠️',
            ...
            { id: 'ground', name: t('stem.bridgelab.how_ground_motion_works', 'How ground motion works'), emoji: '〰️',
            ...
            { id: 'caltrans', name: t('stem.bridgelab.loma_prieta_caltrans_response', 'Loma Prieta + Caltrans response'), emoji: '🌉',
            ...
            { id: 'isolation', name: t('stem.bridgelab.base_isolation_2', 'Base isolation'), emoji: '🛏️',
            ...
            { id: 'damping', name: t('stem.bridgelab.energy_dissipation_dampers', 'Energy dissipation + dampers'), emoji: '🔥',
            ...
            { id: 'capacity', name: t('stem.bridgelab.capacity_design_fuses', 'Capacity design (fuses)'), emoji: '🔌',
            ...
            { id: 'restrainers', name: t('stem.bridgelab.restrainer_cables_seat_extensions', 'Restrainer cables + seat extensions'), emoji: '🔗',
            ...
            { id: 'liquefaction', name: t('stem.bridgelab.liquefaction_foundation_engineering', 'Liquefaction + foundation engineering'), emoji: '🪨',
            ...
            { id: 'codes', name: t('stem.bridgelab.codes_their_limits', 'Codes + their limits'), emoji: '📋',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            ...
            h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
              h('div', { style: { fontSize: 14, fontWeight: 800, color: '#fbbf24', marginBottom: 8 } }, topic.emoji + ' ' + topic.name),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(59,130,246,0.06)', borderLeft: '3px solid #3b82f6', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#93c5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.how_it_works_7', 'How it works')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.what)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(220,38,38,0.06)', borderLeft: '3px solid #ef4444' } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.honest_limit_4', 'Honest limit')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.limit)
              )
            ),
            h('div', { style: { marginTop: 12, padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.3)', fontSize: 11.5, color: '#dcfce7', lineHeight: 1.65 } },
              h('strong', null, t('stem.bridgelab.a_maine_note', 'A Maine note: ')),
              t('stem.bridgelab.maine_is_in_a_low_seismic_hazard_zone_', 'Maine is in a LOW seismic-hazard zone but not zero. The 1755 Cape Ann earthquake (estimated M 6.0-6.3) shook Boston + Maine; magnetic-anomaly + paleoseismic studies suggest similar events recur every few centuries. Maine + New England bridges use the AASHTO Seismic Zone 1 provisions (minimal seismic design). The Bridge to Treasure Island in Portland + the Casco Bay Bridge are designed for the local Zone 1 hazard. If you want to see major seismic engineering up close, the Penobscot Narrows Bridge (1996) + the recent I-295 over the Kennebec used some seismic-resistant detailing but at much lower intensity than California or Cascadia subduction zone designs would require.')
            )
          );
        }
        ...
        function windTunnelTestingSection() {
          var WT = [
            { id: 'why', name: t('stem.bridgelab.why_bridges_need_wind_testing', 'Why bridges need wind testing'), emoji: '🌬️',
            ...
            { id: 'history', name: t('stem.bridgelab.a_field_forged_by_failure', 'A field forged by failure'), emoji: '🏚️',
            ...
            { id: 'section', name: t('stem.bridgelab.section_model_tests', 'Section model tests'), emoji: '📦',
            ...
            { id: 'aeroelastic', name: t('stem.bridgelab.full_bridge_aeroelastic_models', 'Full-bridge aeroelastic models'), emoji: '🏗️',
            ...
            { id: 'cfd', name: t('stem.bridgelab.computational_les_validation', 'Computational + LES validation'), emoji: '💻',
            ...
            { id: 'mods', name: t('stem.bridgelab.aerodynamic_countermeasures', 'Aerodynamic countermeasures'), emoji: '🔧',
            ...
            { id: 'sites', name: t('stem.bridgelab.test_facilities_around_the_world', 'Test facilities around the world'), emoji: '🌐',
            ...
            { id: 'maine', name: t('stem.bridgelab.when_local_bridges_need_testing', 'When local bridges need testing'), emoji: '🌉',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            h('h3', { style: { margin: '0 0 6px', color: '#fbbf24', fontSize: 16 } }, t('stem.bridgelab.wind_tunnel_testing_of_long_span_bridg', '🌪️ Wind tunnel testing of long-span bridges')),
            h('p', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #cbd5e1)', lineHeight: 1.65, margin: '0 0 12px' } },
              t('stem.bridgelab.a_long_span_bridge_that_has_never_been', 'A long-span bridge that has never been in a wind tunnel is a long-span bridge that has skipped a fundamental design check. The discipline was forged in the wake of the 1940 Tacoma Narrows collapse + has matured into one of the most distinctive specialties in civil engineering. Every major suspension + cable-stayed bridge built since the 1970s has been wind-tunnel tested before construction.')
            ),
            h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 } },
              WT.map(function(t) {
                var on = t.id === sel;
        ...
        function cableSpinningSection() {
          var STEPS = [
            { id: 'anchor', name: t('stem.bridgelab.1_anchorages_first', '1. Anchorages first'), emoji: '⚓', when: 'Months 1-12',
            ...
            { id: 'towers', name: t('stem.bridgelab.2_towers_saddles', '2. Towers + saddles'), emoji: '🗼', when: 'Months 6-18',
            ...
            { id: 'catwalk', name: t('stem.bridgelab.3_catwalks_pilot_ropes', '3. Catwalks + pilot ropes'), emoji: '🚶', when: 'Month 18-24',
            ...
            { id: 'spinning', name: t('stem.bridgelab.4_cable_spinning_the_magic', '4. Cable spinning (the magic)'), emoji: '🕷️', when: 'Months 24-30',
            ...
            { id: 'aerial', name: t('stem.bridgelab.5_aerial_spun_vs_ppws', '5. Aerial Spun vs PPWS'), emoji: '⚙️', when: 'Variable',
            ...
            { id: 'hangers', name: t('stem.bridgelab.6_hangers_suspenders', '6. Hangers + suspenders'), emoji: '🎯', when: 'Month 30-36',
            ...
            { id: 'deck', name: t('stem.bridgelab.7_deck_erection', '7. Deck erection'), emoji: '🚛', when: 'Month 36-48',
            ...
            { id: 'tuning', name: t('stem.bridgelab.8_final_tensioning_acceptance', '8. Final tensioning + acceptance'), emoji: '🎚️', when: 'Month 48+',
      ...
      function tacomaFlutterDemo() {
        ...
        return h('div', { style: { padding: 14, borderRadius: 12, background: '#0a0e1a', border: '1px solid var(--allo-stem-border, #334155)', marginTop: 10, borderLeft: '3px solid #ef4444' } },
          h('div', { style: { fontSize: 13, fontWeight: 800, color: '#fca5a5', marginBottom: 6 } }, t('stem.bridgelab.aerodynamic_flutter_demo', '💨 Aerodynamic flutter demo')),
          h('p', { style: { margin: '0 0 10px', fontSize: 12, color: 'var(--allo-stem-text, #cbd5e1)', lineHeight: 1.6 } },
            t('stem.bridgelab.move_the_wind_speed_slider_watch_how_t', 'Move the wind speed slider. Watch how the deck behaves. The open H-shaped cross-section of the original Tacoma deck caused alternating lift forces, and the bridge tore itself apart at moderate wind speeds. Modern bridges use closed or aerodynamic cross-sections that don\'t flutter.')
          ),
          styleBlock,

          // SVG of the deck
          h('svg', { viewBox: '0 0 400 180', width: '100%', height: 180, role: 'img', 'aria-labelledby': 'flutterTitle flutterDesc' },
            h('title', { id: 'flutterTitle' }, t('stem.bridgelab.tacoma_narrows_flutter_animation', 'Tacoma Narrows flutter animation')),
            ...
            h('text', { x: 8, y: 25, fill: '#94a3b8', fontSize: 10 }, t('stem.bridgelab.wind', 'Wind →')),
          ...
          h('div', { style: { marginTop: 12 } },
            h('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 4 } },
              h('span', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)', fontWeight: 700 } }, t('stem.bridgelab.wind_speed', 'Wind speed')),
            ...
            h('input', { type: 'range', min: 0, max: 80, step: 1, value: windSpeed,
              ...
              'aria-label': t('stem.bridgelab.wind_speed_in_miles_per_hour', 'Wind speed in miles per hour'),
              style: { width: '100%', accentColor: '#ef4444' }
            }),
            h('div', { style: { display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--allo-stem-text-soft, #64748b)', marginTop: 2 } },
              h('span', null, t('stem.bridgelab.0_mph', '0 mph')),
              h('span', null, 'Critical: ~' + critical + ' mph'),
              h('span', null, t('stem.bridgelab.80_mph', '80 mph'))
            )
          ),
      ...
      function millenniumPedestrianDemo() {
        ...
        return h('div', { style: { padding: 14, borderRadius: 12, background: '#0a0e1a', border: '1px solid var(--allo-stem-border, #334155)', marginTop: 10, borderLeft: '3px solid #ef4444' } },
          ...
          h('svg', { viewBox: '0 0 400 160', width: '100%', height: 160, role: 'img', 'aria-labelledby': 'millTitle millDesc' },
            h('title', { id: 'millTitle' }, t('stem.bridgelab.millennium_bridge_pedestrian_synchroni', 'Millennium Bridge pedestrian synchronization animation')),
          ...
          h('div', { style: { marginTop: 12 } },
            h('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 4 } },
              h('span', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)', fontWeight: 700 } }, t('stem.bridgelab.pedestrians_on_the_bridge', 'Pedestrians on the bridge')),
            ...
            h('input', { type: 'range', min: 0, max: 600, step: 10, value: nPeds,
              ...
              'aria-label': t('stem.bridgelab.number_of_pedestrians', 'Number of pedestrians'),
              style: { width: '100%', accentColor: '#ef4444' }
            }),
            h('div', { style: { display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--allo-stem-text-soft, #64748b)', marginTop: 2 } },
              h('span', null, t('stem.bridgelab.0_people', '0 people')),
              h('span', null, 'Critical: ~' + critical + ' people'),
              h('span', null, t('stem.bridgelab.600_people', '600 people'))
            )
          ),
          ...
          h('div', { style: { marginTop: 10, padding: 10, borderRadius: 8, background: status === 'locked' ? 'rgba(239,68,68,0.10)' : status === 'synchronizing' ? 'rgba(245,158,11,0.10)' : 'rgba(34,197,94,0.10)', border: '1px solid ' + (status === 'locked' ? '#7f1d1d' : status === 'synchronizing' ? '#92400e' : '#14532d'), fontSize: 11.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.6 } },
            status === 'locked' ? h('span', null, h('strong', null, t('stem.bridgelab.locked_synchronization', 'Locked synchronization: ')), t('stem.bridgelab.pedestrians_are_now_unconsciously_phas', 'Pedestrians are now unconsciously phase-locked with the bridge\'s natural lateral mode. Each step adds to the swaying. The bridge wobbles violently. At 8-10 mm visible amplitude, panicked pedestrians grip the railing + slow down — actually making it worse by sustaining synchronization. London opening day reached ~70 mm amplitude.')) :
            status === 'synchronizing' ? h('span', null, h('strong', null, t('stem.bridgelab.synchronization_beginning', 'Synchronization beginning: ')), t('stem.bridgelab.bridge_motion_is_becoming_perceptible_', 'Bridge motion is becoming perceptible. Some pedestrians are unconsciously adjusting their gait. If more people arrive or stay, locked synchronization will develop.')) :
            h('span', null, h('strong', null, 'Stable: '), t('stem.bridgelab.below_the_critical_pedestrian_density_', 'Below the critical pedestrian density. Random gait phasing cancels out laterally. Modern pedestrian bridges include tuned mass dampers + lateral viscous dampers to suppress this even at high density — the Millennium retrofit added ~87 dampers total.'))
          )
        );
      }
      ...
      function renderCases() {
        ...
        return h('div', { style: { padding: 16 } },
          h('p', { style: { color: 'var(--allo-stem-text, #cbd5e1)', fontSize: 13, marginBottom: 12, lineHeight: 1.6 } },
            t('stem.bridgelab.engineering_is_learned_at_least_as_muc', 'Engineering is learned at least as much from failures as from successes. The collapse of Tacoma Narrows changed how every bridge in the world is designed; the success of Brooklyn proved suspension bridges could last centuries.')
          ),
          h('div', { style: { display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 } },
        ...
        function ancientEngineeringSection() {
          var ANC = [
            { id: 'roman', name: t('stem.bridgelab.roman_stone_arch', 'Roman stone arch'), emoji: '🏛️', when: '~300 BCE-400 CE',
            ...
            { id: 'mesopotamian', name: t('stem.bridgelab.mesopotamian_earliest_bridges', 'Mesopotamian + earliest bridges'), emoji: '⛲', when: '~4000-300 BCE',
            ...
            { id: 'inca', name: t('stem.bridgelab.inca_rope_suspension_bridges', 'Inca rope suspension bridges'), emoji: '🪢', when: '~1200-1533 CE',
            ...
            { id: 'chinese', name: t('stem.bridgelab.chinese_ancient_bridges', 'Chinese ancient bridges'), emoji: '🐉', when: '~600 BCE-1400 CE',
            ...
            { id: 'islamic', name: t('stem.bridgelab.islamic_golden_age_bridges', 'Islamic Golden Age bridges'), emoji: '☪️', when: '~750-1500 CE',
            ...
            { id: 'pontvieux', name: t('stem.bridgelab.medieval_european_bridges', 'Medieval European bridges'), emoji: '🏰', when: '~1100-1500 CE',
            ...
            { id: 'mughal', name: t('stem.bridgelab.mughal_india_bridges_waterworks', 'Mughal India bridges + waterworks'), emoji: '🕌', when: '~1500-1700 CE',
            ...
            { id: 'lessons', name: t('stem.bridgelab.what_the_ancients_knew', 'What the ancients knew'), emoji: '🎓', when: 'Across traditions',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            h('h3', { style: { margin: '0 0 6px', color: '#fbbf24', fontSize: 16 } }, t('stem.bridgelab.ancient_traditional_bridge_engineering', '🏛️ Ancient + traditional bridge engineering')),
            h('p', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #cbd5e1)', lineHeight: 1.65, margin: '0 0 12px' } },
              t('stem.bridgelab.modern_bridge_engineering_is_the_inher', 'Modern bridge engineering is the inheritor of at least 4000 years of trial-and-error + cultural transmission. Several pre-modern bridge traditions produced structures that still carry traffic, span rivers, + serve communities today — sometimes outperforming their 20th-century successors. Honoring these traditions is intellectual honesty AND practical: ancient engineering has lessons modern engineering is rediscovering.')
            ),
            h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 } },
              ANC.map(function(t) {
                var on = t.id === sel;
            ...
            h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(59,130,246,0.06)', borderLeft: '3px solid #3b82f6', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#93c5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.what_was_built', 'What was built')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.what)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.06)', borderLeft: '3px solid #22c55e', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.how_it_worked', 'How it worked')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.how)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(168,85,247,0.06)', borderLeft: '3px solid #a78bfa' } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#c4b5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.legacy_lessons', 'Legacy + lessons')),
        ...
        function militaryAndEmergencyBridgesSection() {
          var MIL = [
            { id: 'bailey', name: t('stem.bridgelab.bailey_bridge', 'Bailey bridge'), emoji: '🔩', year: '1941',
            ...
            { id: 'pontoon', name: t('stem.bridgelab.military_pontoon_ribbon_bridge', 'Military pontoon (ribbon) bridge'), emoji: '🌉', year: '1950s-current',
            ...
            { id: 'mab', name: t('stem.bridgelab.medium_girder_bridge_mgb', 'Medium Girder Bridge (MGB)'), emoji: '🛡️', year: '1971-current',
            ...
            { id: 'logsfloating', name: t('stem.bridgelab.floating_log_improvised_bridges', 'Floating-log + improvised bridges'), emoji: '🪵', year: 'Throughout history',
            ...
            { id: 'pucvr', name: t('stem.bridgelab.puc_vr_pont_utilisation_civile_civil_u', 'PUC-VR (Pont à Utilisation Civile / civil-use pontoon)'), emoji: '🚧', year: '1990s-current',
            ...
            { id: 'tactical', name: t('stem.bridgelab.tactical_floating_causeway_inls', 'Tactical Floating Causeway / INLS'), emoji: '⚓', year: '2000s-current',
              who: 'US Navy + USACE',
            ...
            { id: 'lessons', name: t('stem.bridgelab.what_emergency_bridges_teach_engineers', 'What emergency bridges teach engineers'), emoji: '🎓',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            ...
            h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(59,130,246,0.06)', borderLeft: '3px solid #3b82f6', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#93c5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.technical_design', 'Technical design')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.tech)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.06)', borderLeft: '3px solid #22c55e', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.operational_use', 'Operational use')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.use)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(220,38,38,0.06)', borderLeft: '3px solid #ef4444' } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.bridgelab.honest_limits_ethics', 'Honest limits + ethics')),
                h('div', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #e2e8f0)', lineHeight: 1.7 } }, topic.caveat)
              )
            )
          );
        ...
        function aestheticsAndIconicBridges() {
          var ICONIC = [
            { id: 'salginatobel', name: t('stem.bridgelab.salginatobel_bridge', 'Salginatobel Bridge'), year: 1930, where: 'Schiers, Switzerland', engineer: 'Robert Maillart',
            ...
            { id: 'fortgewone', name: t('stem.bridgelab.forth_bridge', 'Forth Bridge'), year: 1890, where: 'Firth of Forth, Scotland', engineer: 'Benjamin Baker + John Fowler',
            ...
            { id: 'firth', name: t('stem.bridgelab.akashi_kaiky_bridge', 'Akashi Kaikyō Bridge'), year: 1998, where: 'Kobe to Awaji Island, Japan', engineer: 'Honshu-Shikoku Bridge Authority',
            ...
            { id: 'sundial', name: t('stem.bridgelab.sundial_bridge', 'Sundial Bridge'), year: 2004, where: 'Redding, California', engineer: 'Santiago Calatrava',
            ...
            { id: 'menai', name: t('stem.bridgelab.menai_suspension_bridge', 'Menai Suspension Bridge'), year: 1826, where: 'Wales, UK', engineer: 'Thomas Telford',
            ...
            { id: 'erasmus', name: t('stem.bridgelab.erasmusbrug', 'Erasmusbrug'), year: 1996, where: 'Rotterdam, Netherlands', engineer: 'Ben van Berkel (UNStudio) + others',
            ...
            { id: 'iron', name: t('stem.bridgelab.the_iron_bridge', 'The Iron Bridge'), year: 1779, where: 'Shropshire, England', engineer: 'Abraham Darby III + Thomas Pritchard',
      ...
      function renderCycle() {
        ...
        return h('div', { style: { padding: 16 } },
          h('p', { style: { color: 'var(--allo-stem-text, #cbd5e1)', fontSize: 13, marginBottom: 12, lineHeight: 1.6 } },
            t('stem.bridgelab.engineering_is_iterative_you_design_te', 'Engineering is iterative. You design, test, fail, learn, redesign. Failure is information, not defeat. Every working bridge in the world is the descendant of many failures.')
          ),
          h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 14 } },
          ...
          sectionCard('Apply this to your bridge design',
            h('ol', { style: { margin: 0, padding: '0 0 0 22px', color: 'var(--allo-stem-text, #e2e8f0)', fontSize: 12.5, lineHeight: 1.7 } },
              h('li', null, h('strong', null, 'Define: '), t('stem.bridgelab.what_span_do_you_need_to_cross_what_lo', 'What span do you need to cross? What load (pedestrians? cars? trains?)? What materials are available locally? What\'s your budget?')),
              h('li', null, h('strong', null, 'Imagine: '), t('stem.bridgelab.sketch_3_different_bridge_types_you_co', 'Sketch 3 different bridge types you could use. Don\'t commit yet.')),
              h('li', null, h('strong', null, 'Plan: '), t('stem.bridgelab.pick_the_most_promising_use_the_stress', 'Pick the most promising. Use the Stress Test tab to size it. Calculate cost. Identify what could go wrong (wind, ice, overload, fatigue, foundation problems).')),
              h('li', null, h('strong', null, 'Create: '), t('stem.bridgelab.in_the_simulation_build_it_in_real_lif', 'In the simulation, build it. In real life, a physical model from cardboard, popsicle sticks, or 3D-printed parts.')),
              h('li', null, h('strong', null, 'Test: '), t('stem.bridgelab.apply_loads_in_the_simulation_for_phys', 'Apply loads in the simulation. For physical models: weights, books, water. Where does it fail first? Why?')),
              h('li', null, h('strong', null, 'Improve: '), t('stem.bridgelab.use_what_you_saw_to_redesign_add_mater', 'Use what you saw to redesign. Add material where stress is highest; reduce where stress is low. Iterate.'))
            )
          ),
          feaBasics(),
          abcAndPrefabSection(),
        ...
        function bridgeFinancingSection() {
          var FIN = [
            { id: 'fundlines', name: t('stem.bridgelab.where_the_money_comes_from', 'Where the money comes from'), emoji: '💵',
            ...
            { id: 'fed', name: t('stem.bridgelab.federal_funding_mechanics', 'Federal funding mechanics'), emoji: '🏛️',
            ...
            { id: 'tolls', name: t('stem.bridgelab.toll_bridges_revenue', 'Toll bridges + revenue'), emoji: '🪙',
            ...
            { id: 'procurement', name: t('stem.bridgelab.procurement_models', 'Procurement models'), emoji: '📋',
            ...
            { id: 'envjustice', name: t('stem.bridgelab.environmental_justice_community_impact', 'Environmental justice + community impact'), emoji: '⚖️',
            ...
            { id: 'lifecycle', name: t('stem.bridgelab.life_cycle_cost_analysis', 'Life-cycle cost analysis'), emoji: '📊',
            ...
            { id: 'collapse', name: t('stem.bridgelab.why_bridges_collapse_from_neglect', 'Why bridges collapse from neglect'), emoji: '⚠️',
            ...
            { id: 'politics', name: t('stem.bridgelab.who_decides_who_benefits', 'Who decides + who benefits'), emoji: '🗳️',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            h('h3', { style: { margin: '0 0 6px', color: '#fbbf24', fontSize: 16 } }, t('stem.bridgelab.bridge_financing_politics_procurement', '💰 Bridge financing, politics, + procurement')),
            h('p', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #cbd5e1)', lineHeight: 1.65, margin: '0 0 12px' } },
              t('stem.bridgelab.every_bridge_has_an_engineering_story_', 'Every bridge has an engineering story + a money story. Where the dollars come from, who decides what gets built, how contracts are awarded, who benefits from the project, who pays the maintenance — these questions shape what bridges actually exist + which communities get to use them. Engineering does not happen in a political vacuum + students who understand only the technical side will struggle to advocate for the bridges their communities need.')
            ),
            h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 } },
              FIN.map(function(t) {
                var on = t.id === sel;
        ...
        function abcAndPrefabSection() {
          var ABC = [
            ...
            { id: 'designs', name: t('stem.bridgelab.standard_abc_bridge_designs', 'Standard ABC bridge designs'), emoji: '📋',
              body: t('stem.bridgelab.several_states_have_developed_pre_engi', 'Several states have developed pre-engineered standard bridges for the most common short-span applications. The Vermont Agency of Transportation\'s ABC standard plans cover spans from 25-80 feet, with detailed precast deck + abutment specs that any local contractor can build. The Federal Lands Highway Program has similar plans for remote sites. The advantage: instead of paying for a custom design ($100K+ engineering), a small-town bridge replacement can use a standard plan ($5-10K design fee) + standard precast pieces. Lead time drops from 18 months to 6 months for a typical rural bridge.'),
            ...
            { id: 'limits', name: t('stem.bridgelab.what_abc_cannot_do_well', 'What ABC cannot do well'), emoji: '🚫',
              body: t('stem.bridgelab.abc_is_not_a_universal_answer_limitati', 'ABC is NOT a universal answer. Limitations: (a) Cost. The direct construction cost is higher. For rural bridges with low traffic, the user-cost savings may not justify the premium. (b) Lead time. Precast elements need to be ordered + manufactured. ABC saves on-site time but adds front-end time. Emergency bridge replacement (after a hurricane or earthquake) may not have time for prefab. (c) Geometry. Many old bridges have unusual skews, super-elevations, or non-standard cross-sections that don\'t match factory-standard products. Heavy customization erodes the ABC cost advantage. (d) Maintenance access. Some ABC connections (UHPC closure pours, grouted shear keys) are difficult to inspect + maintain in service. The connection details matter as much as the elements.'),
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            h('h3', { style: { margin: '0 0 6px', color: '#fbbf24', fontSize: 16 } }, t('stem.bridgelab.accelerated_bridge_construction_abc_pr', '🏭 Accelerated Bridge Construction (ABC) + prefab')),
            h('p', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #cbd5e1)', lineHeight: 1.65, margin: '0 0 12px' } },
              t('stem.bridgelab.modern_bridges_do_not_have_to_take_yea', 'Modern bridges do not have to take years to build. Accelerated Bridge Construction — using prefabricated factory-made elements installed during short on-site closures — has matured into a mainstream practice in the past 20 years. A bridge that would have closed a road for 6 months can now be replaced in a single weekend. The cost premium is real; the user-cost savings are typically larger.')
            ),
            h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 } },
              ABC.map(function(t) {
                var on = t.id === sel;
            ...
            h('div', { style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid var(--allo-stem-border, #334155)' } },
              ...
              h('div', { style: { fontSize: 11.5, color: 'var(--allo-stem-text, #cbd5e1)', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                h('strong', null, t('stem.bridgelab.honest_limit_7', 'Honest limit: ')), topic.caveat
              )
            )
          );
        }
        ...
        function feaBasics() {
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)' } },
            h('h3', { style: { margin: '0 0 6px', color: '#fbbf24', fontSize: 16 } }, t('stem.bridgelab.finite_element_analysis_how_modern_bri', '🧠 Finite Element Analysis — how modern bridges actually get analyzed')),
            h('p', { style: { fontSize: 12.5, color: 'var(--allo-stem-text, #cbd5e1)', lineHeight: 1.65, margin: '0 0 12px' } },
              t('stem.bridgelab.every_bridge_built_today_is_analyzed_w', 'Every bridge built today is analyzed with FEA somewhere in its design. It is the workhorse method, the reason engineers can design structures the hand-calculation generation could not. It is also the easiest tool in engineering to use badly. Both are worth understanding.')
            ),
            h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 } },
          ...
          return h('div', { style: { padding: 16 } },
            ...
            QUIZ_QUESTIONS.map(function(q, i) {
              ...
              return h('div', { key: i, style: { padding: 12, borderRadius: 10, background: 'var(--allo-stem-canvas, #0f172a)', border: '1px solid ' + (got ? 'rgba(34,197,94,0.4)' : 'rgba(220,38,38,0.4)'), borderLeft: '3px solid ' + (got ? '#22c55e' : '#dc2626'), marginBottom: 10 } },
                ...
                !got ? h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)', marginBottom: 4 } }, t('stem.bridgelab.your_answer', 'Your answer: '), q.choices[answers[i] != null ? answers[i] : 0]) : null,
                h('div', { style: { fontSize: 12, color: 'var(--allo-stem-text, #cbd5e1)', lineHeight: 1.6, fontStyle: 'italic' } }, q.explain)
              );
            }),
            h('button', { onClick: reset, style: { padding: '8px 16px', borderRadius: 8, border: 'none', background: AMBER, color: '#0f172a', fontWeight: 700, cursor: 'pointer' } }, t('stem.bridgelab.retake_quiz', 'Retake quiz'))
          );
        }
      ...
      function renderInquiry() {
        ...
        return h('div', { style: { padding: 16 } },
          h('div', { style: { background: '#0a1525', borderRadius: 12, padding: 16, border: '1px solid rgba(245,158,11,0.3)' } },
            h('h3', { style: { margin: '0 0 6px 0', color: '#fbbf24', fontSize: 16, fontWeight: 800 } }, t('stem.bridgelab.load_bearing_discovery', '🔬 Load-bearing discovery')),
            ...
            h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 } },
              h('div', null,
                h('label', { htmlFor: 'iq-span', style: { display: 'block', fontSize: 11, fontWeight: 700, color: '#cbd5e1', marginBottom: 4 } }, t('stem.bridgelab.span_length', 'Span length: '), h('span', { style: { color: '#fbbf24', fontFamily: 'monospace' } }, iq.spanM + ' m')),
                h('input', { id: 'iq-span', type: 'range', min: 6, max: 60, step: 1, value: iq.spanM,
                  ...
                  style: { width: '100%' }, 'aria-label': t('stem.bridgelab.span_length_in_meters', 'Span length in meters') })),
              h('div', null,
                h('label', { htmlFor: 'iq-area', style: { display: 'block', fontSize: 11, fontWeight: 700, color: '#cbd5e1', marginBottom: 4 } }, t('stem.bridgelab.cross_section_area', 'Cross-section area: '), h('span', { style: { color: '#fbbf24', fontFamily: 'monospace' } }, iq.areaMm2 + ' mm²')),
                h('input', { id: 'iq-area', type: 'range', min: 200, max: 8000, step: 100, value: iq.areaMm2,
                  ...
                  style: { width: '100%' }, 'aria-label': t('stem.bridgelab.cross_section_area_in_square_millimete', 'Cross-section area in square millimeters') }))
            ),
            ...
            h('div', { style: { display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12 } },
              h('button', { onClick: logObs, style: { padding: '4px 10px', background: '#1e293b', color: '#cbd5e1', border: '1px solid rgba(100,116,139,0.4)', borderRadius: 4, fontSize: 11, fontWeight: 700, cursor: 'pointer' } }, t('stem.bridgelab.log_observation', '📋 Log observation')),
              h('button', { onClick: function() { setIQ({ spanM: 18, areaMm2: 1500, materialId: 'steel', log: [], hypothesis: '', stuckRevealed: false, understood: false, explanation: '' }); },
                style: { padding: '4px 10px', background: '#0a1525', color: '#94a3b8', border: '1px solid rgba(100,116,139,0.4)', borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: 'pointer' } }, t('stem.bridgelab.reset', '↺ Reset')),
            ...
            h('div', { style: { marginBottom: 12 } },
              h('label', { htmlFor: 'iq-hypo', style: { display: 'block', fontSize: 11, fontWeight: 700, color: '#cbd5e1', marginBottom: 4 } },
                t('stem.bridgelab.your_hypothesis_free_text_no_right_ans', 'Your hypothesis (free text — no right answer):')),
              h('textarea', { id: 'iq-hypo', value: iq.hypothesis || '',
                onChange: function(e) { setIQ({ hypothesis: e.target.value }); },
                placeholder: t('stem.bridgelab.what_single_change_is_the_most_efficie', 'What single change is the most efficient way to move a FAILED bridge into SAFE? Does doubling span require doubling area? Type your own theory.'),
            ...
            h('div', { style: { marginBottom: 12 } },
              !iq.stuckRevealed && h('button', { onClick: function() { setIQ({ stuckRevealed: true }); },
                ...
                t('stem.bridgelab.i_m_stuck_show_me_questions_to_think_a', '🤔 I\'m stuck — show me questions to think about (no answers)')),
              iq.stuckRevealed && h('div', { style: { padding: 10, background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: 4, fontSize: 11, color: '#cbd5e1', lineHeight: 1.5 } },
                h('div', { style: { fontWeight: 700, color: '#fbbf24', marginBottom: 4 } }, t('stem.bridgelab.open_questions_investigate_by_manipula', 'Open questions — investigate by manipulating:')),
                h('ul', { style: { margin: 0, paddingLeft: 18 } },
                  h('li', null, t('stem.bridgelab.hold_material_and_area_fixed_sweep_spa', 'Hold material and area fixed. Sweep span from 6 to 60 m. At what span does the state flip from SAFE to MARGINAL? Now change material — does the flip-point move?')),
                  h('li', null, t('stem.bridgelab.pick_a_span_where_the_bridge_fails_wha', 'Pick a span where the bridge FAILS. What is cheaper: doubling the cross-section area, or switching to a stronger material? Test both and compare.')),
                  h('li', null, t('stem.bridgelab.some_materials_have_very_high_strength', 'Some materials have very high strength but are heavy or expensive in real life. The simulation does not penalize this. What would change about your hypotheses if cost mattered?')),
                  h('li', null, t('stem.bridgelab.log_4_5_safe_configurations_look_at_th', 'Log 4-5 SAFE configurations. Look at the safety factor column. What range do they share? Is there a single threshold or a band?')),
                  h('li', null, t('stem.bridgelab.in_real_engineering_the_rule_of_thumb_', 'In real engineering, the rule of thumb is safety factor ≥ 1.5 for steel structures. Why might the simulation show MARGINAL at sf 1-2 but SAFE only at sf ≥ 2?'))),
                h('div', { style: { fontSize: 10, fontStyle: 'italic', color: '#94a3b8', marginTop: 6 } }, t('stem.bridgelab.no_answers_will_be_revealed_investigat', 'No answers will be revealed. Investigate.')))
            ),
            // Self-mark
            h('div', { style: { padding: 10, background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 4 } },
              h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 } },
                ...
                h('label', { htmlFor: 'iq-und', style: { fontSize: 12, fontWeight: 700, color: '#10b981', cursor: 'pointer' } },
                  t('stem.bridgelab.i_think_i_understand_the_trade_offs_le', 'I think I understand the trade-offs — let me explain them in my own words'))),
              iq.understood && h('textarea', { value: iq.explanation || '',
                onChange: function(e) { setIQ({ explanation: e.target.value }); },
                placeholder: t('stem.bridgelab.explain_in_your_own_words_how_do_span_', 'Explain in your own words: how do span, cross-section area, and material strength interact to determine load-bearing capacity? Which one matters most? What is the trade-off a real engineer faces?'),
              ...
              iq.understood && (iq.explanation || '').trim().length >= 40 && h('div', { style: { marginTop: 6, fontSize: 10, fontStyle: 'italic', color: '#10b981' } },
                t('stem.bridgelab.saved_notice_nobody_checked_your_answe', '✓ Saved. Notice — nobody checked your answer. That is what learner-driven inquiry looks like.'))
            ),
            h('div', { style: { marginTop: 12, padding: 8, background: 'rgba(10,21,37,0.5)', border: '1px solid rgba(100,116,139,0.3)', borderRadius: 4, fontSize: 10, fontStyle: 'italic', color: '#94a3b8' } },
              t('stem.bridgelab.design_note_no_numeric_safety_factor_t', 'Design note: no numeric safety-factor target, no reveal button, no quiz validation. Safety is shown as a discrete 3-state marker (SAFE / MARGINAL / FAILS), not a continuous gradient — by design, to discourage optimization-gaming behavior. The point is the inquiry, not the number.'))
          )
        );
      }
      ...
      function renderPrint() {
        ...
        return h('div', { style: { padding: 16 } },
          h('div', { className: 'no-print', style: { padding: 12, borderRadius: 10, background: 'rgba(245,158,11,0.10)', borderTop: '1px solid rgba(245,158,11,0.4)', borderRight: '1px solid rgba(245,158,11,0.4)', borderBottom: '1px solid rgba(245,158,11,0.4)', borderLeft: '3px solid ' + AMBER, marginBottom: 12, fontSize: 12.5, color: 'var(--allo-stem-text, #fde68a)', lineHeight: 1.65 } },
            h('strong', null, t('stem.bridgelab.bridge_design_specification_sheet', '🖨 Bridge design specification sheet. ')),
            t('stem.bridgelab.a_one_page_artifact_for_engineering_po', 'A one-page artifact for engineering portfolios, design competitions, classroom presentations, or maker-faire submissions. Contains the design parameters, the stress analysis result, the material properties, and your design notes.')
          ),
          h('div', { className: 'no-print', style: { marginBottom: 14, textAlign: 'center' } },
            h('button', { onClick: function() { try { window.print(); } catch (e) {} },
              style: { padding: '8px 18px', borderRadius: 8, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #b45309 0%, #c2410c 100%)', color: '#fff', fontWeight: 800, fontSize: 13 } }, t('stem.bridgelab.print_save_as_pdf', '🖨 Print / Save as PDF'))
          ),
          h('style', null,
          ...
          h('div', { id: 'bridge-print-region', style: { padding: 18, borderRadius: 12, background: '#ffffff', color: '#0f172a', border: '1px solid #e2e8f0' } },
            h('div', { style: { display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', borderBottom: '2px solid #0f172a', paddingBottom: 8, marginBottom: 14 } },
              ...
              h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #475569)' } }, t('stem.bridgelab.ngss_ms_ets1_hs_ets1_hs_ps2', 'NGSS MS-ETS1 · HS-ETS1 · HS-PS2'))
            ),
            ...
            h('div', { style: { padding: 12, border: '2px solid #0f172a', borderRadius: 10, marginBottom: 12, pageBreakInside: 'avoid' } },
              h('div', { style: { fontSize: 13, fontWeight: 800, color: '#0f172a', marginBottom: 8 } }, t('stem.bridgelab.design_parameters', 'Design parameters')),
            ...
            h('div', { style: { padding: 12, border: '2px solid #0f172a', borderRadius: 10, marginBottom: 12, pageBreakInside: 'avoid' } },
              h('div', { style: { fontSize: 13, fontWeight: 800, color: '#0f172a', marginBottom: 8 } }, t('stem.bridgelab.stress_analysis_deep_beam_approximatio', 'Stress analysis (deep-beam approximation)')),
              h('table', { style: { width: '100%', fontSize: 12, color: '#0f172a', borderCollapse: 'collapse' } },
                h('tbody', null,
            ...
            h('div', { style: { padding: 12, border: '2px solid #0f172a', borderRadius: 10, marginBottom: 12, pageBreakInside: 'avoid' } },
              h('div', { style: { fontSize: 13, fontWeight: 800, color: '#0f172a', marginBottom: 8 } }, t('stem.bridgelab.quantities', 'Quantities')),
              h('table', { style: { width: '100%', fontSize: 12, color: '#0f172a', borderCollapse: 'collapse' } },
                h('tbody', null,
            ...
            d.designNotes ? h('div', { style: { padding: 12, border: '2px solid #0f172a', borderRadius: 10, marginBottom: 12, pageBreakInside: 'avoid' } },
              h('div', { style: { fontSize: 13, fontWeight: 800, color: '#0f172a', marginBottom: 8 } }, t('stem.bridgelab.design_notes', 'Design notes')),
            ...
            h('div', { style: { padding: 10, background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: 8, marginBottom: 12, fontSize: 11.5, color: '#78350f', lineHeight: 1.55 } },
              h('strong', null, t('stem.bridgelab.honest_limit_of_this_analysis', 'Honest limit of this analysis: ')),
              t('stem.bridgelab.a_deep_beam_approximation_real_bridge_', 'A deep-beam approximation. Real bridge design uses method of joints, method of sections, or matrix structural analysis to get exact member forces, then checks every individual member for yield, buckling, fatigue, and connection limits. This tool is for learning, not for actually building a bridge.')
            ),
            ...
            h('div', { style: { marginTop: 14, padding: 10, borderTop: '2px solid #0f172a', fontSize: 10.5, color: 'var(--allo-stem-text-soft, #475569)', lineHeight: 1.5 } },
              t('stem.bridgelab.sources_ngss_lead_states_2013_hibbeler', 'Sources: NGSS Lead States (2013) · Hibbeler, R.C. (2017), Structural Analysis · AASHTO LRFD Bridge Design Specifications · Petroski, H. (1994), Design Paradigms (Tacoma Narrows + Tay Bridge + Hyatt). Printed from AlloFlow STEM Lab.')
            )
          )
        );
      ...
      return h('div', { className: 'selh-bridgelab', style: { display: 'flex', flexDirection: 'column', height: '100%', background: BG, color: 'var(--allo-stem-text, #e2e8f0)' } },
        h('div', { style: { padding: '12px 16px', borderBottom: '1px solid var(--allo-stem-border, #1e293b)', background: 'linear-gradient(135deg, #78350f, var(--allo-stem-canvas, #0f172a))', display: 'flex', alignItems: 'center', gap: 12 } },
          ...
          h('div', null,
            h('h2', { style: { margin: 0, color: '#fbbf24', fontSize: 20, fontWeight: 900 } }, t('stem.bridgelab.bridge_engineering_lab', 'Bridge Engineering Lab')),
            h('div', { style: { fontSize: 12, color: 'var(--allo-stem-text-soft, #94a3b8)', marginTop: 2 } }, t('stem.bridgelab.engineering_design_structural_mechanic', 'Engineering Design · Structural Mechanics · NGSS HS-ETS1 + HS-PS2'))
          )
        ),
        tabBar,
