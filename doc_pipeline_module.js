    ...
    if (batchMaxFix > 0 && (curAxeResults && curAxeResults.totalViolations > 0 || bestAiScore < 90)) {
      for (let fp = 0; fp < batchMaxFix; fp++) {
        ...
        if (isFinalPass) {
          ...
          const [rv1, rv2, rv3] = await Promise.all([auditOutputAccessibility(accessibleHtml), auditOutputAccessibility(accessibleHtml), auditOutputAccessibility(accessibleHtml)]);
