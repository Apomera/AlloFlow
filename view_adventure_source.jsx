  ...
  var renderAdventureComposer = function (immersive) {
    ...
    return <section data-adventure-composer aria-labelledby={inputId + '-label'} style={adventureVisualTokens(theme, immersive)}
      ...
      <div className="mt-2 flex flex-wrap items-start justify-between gap-2">
        <p id={inputId + '-keys'} className="m-0 text-xs leading-relaxed text-[var(--av-muted)]">{adventureSettingsText(t, 'response_keyboard_help', 'Enter to send · Shift + Enter for a new line.')}</p>
        {renderTypingPace(immersive || theme !== 'light')}
      </div>
      <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:justify-between">
        <button type="button" data-adventure-dictation
          aria-pressed={isDictationMode}
          onClick={() => {
            const newState = !isDictationMode;
            setIsDictationMode(newState);
            if (newState) setTimeout(() => { if (adventureInputRef.current) adventureInputRef.current.focus(); }, 100);
          }}
        ...
        >
          {isDictationMode ? <Mic size={18} aria-hidden="true" /> : <MicOff size={18} aria-hidden="true" />}
          <span>{isDictationMode ? adventureSettingsText(t, 'response_dictation_stop', 'Stop dictation') : adventureSettingsText(t, 'response_dictation_start', 'Dictate')}</span>
        </button>
        <button type="button" data-help-key="adventure_input_send" onClick={() => handleAdventureTextSubmit()}
        ...
        >
          ...
          <span>{isDebate ? adventureSettingsText(t, 'response_argument_send', 'Send argument') : adventureSettingsText(t, 'response_action_send', 'Send action')}</span>
        </button>
      </div>
    </section>;
  };
