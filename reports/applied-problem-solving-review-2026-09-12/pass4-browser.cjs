...
async function main(){const browser=await chromium.launch({headless:true});try{
 ...
 await recovery.locator('#aps-question-suggestion summary').click();
 ...
 await audit(recovery,'Recovery banner 390');await recovery.screenshot({path:path.join(__dirname,'pass4-recovery-390.png'),fullPage:true});
 await recovery.getByRole('button',{name:'Undo last change',exact:true}).click();
 ...
 await audit(teacher,'Teacher source and task quality review 320');await teacher.screenshot({path:path.join(__dirname,'pass4-teacher-320.png'),fullPage:true});
 ...
 assert.deepEqual(results.errors,[]);fs.writeFileSync(path.join(__dirname,'pass4-browser-results.json'),JSON.stringify(results,null,2));console.log(JSON.stringify(results,null,2));
