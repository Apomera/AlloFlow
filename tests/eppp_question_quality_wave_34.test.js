...
const wave = require('../dev-tools/eppp_native_quality_wave_34_data.cjs');
...
describe('EPPP expanded quality wave 34', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_34.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 24, applicationItems: 15, analysisItems: 9, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    expect(audit.summary.distractorWarningsAfter.uniqueKeyStemLexicalLeakageCandidates).toBe(40);
    expect(audit.summary.distractorWarningsAfter.asymmetricExtremeDistractorCandidates).toBe(54);
    ...
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicateClusters).toBe(36);
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(464);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1289);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(602);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(633);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_34.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_34.json'));
  });

  it('replays after wave 33', () => {
    const builder = read('dev-tools/build_eppp_1500_expansion.cjs');
    const i33 = builder.indexOf('repair_eppp_native_quality_wave_33.cjs');
    const i34 = builder.indexOf('repair_eppp_native_quality_wave_34.cjs');
    expect(i34).toBeGreaterThan(i33);
  });
});
