...
const { wave } = require('./eppp_memory_aid_review_wave_07_data.cjs');

const root = path.resolve(__dirname, '..');
const testPrepRoot = path.join(root, 'test_prep');
const outputPath = path.join(testPrepRoot, 'eppp_memory_aid_review_wave_07.json');
const predecessorFilename = 'eppp_memory_aid_review_wave_06.json';
...
const historicalArtifactFilename = 'eppp_memory_aid_review_wave_07.json';
...
function fail(message) {
  throw new Error(`Wave 07 validation failed: ${message}`);
}
...
if (predecessor.waveId !== 'eppp-memory-aid-review-wave-06') fail('the predecessor does not identify itself as Wave 06.');
if (wave.waveId !== 'eppp-memory-aid-review-wave-07') fail('the data module has the wrong waveId.');
...
for (const item of wave.items) {
  ...
  if (waveIds.has(item.legacyId)) fail(`duplicate Wave 07 legacyId ${item.legacyId}.`);
...
const priorWavePattern = /^eppp_memory_aid_review_wave_0[1-6]\.json$/i;
...
if (checkOnly) {
  if (!fs.existsSync(outputPath)) fail('the Wave 07 JSON artifact does not exist.');
  if (fs.readFileSync(outputPath, 'utf8') !== expected) fail('the Wave 07 JSON artifact is stale relative to its data module.');
  console.log('EPPP memory-aid review Wave 07 artifact is current and guarded after Wave 06.');
} else {
