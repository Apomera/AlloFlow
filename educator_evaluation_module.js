...
function AeSimulationStudio({ workspace, onApply }) {
  ...
  const saveTour = () => {
    let parsed = null;
    try {
      parsed = JSON.parse(tourText);
    } catch (_) {
      setMessage(t("educator_evaluation.tour_invalid_json_20260902", "The tour script is not valid JSON."));
      return;
    }
    const steps = aeSanitizeTourSteps(parsed);
    if (!steps) {
      setMessage(t("educator_evaluation.tour_invalid_steps_20260902", "Each step needs a tab (overview, trends, staff, walkthroughs, formal, spm, audit, or about), a title, and a text, with 1 to 30 steps."));
      return;
    }
    if (!aeWriteTourOverride(steps)) {
      setMessage(t("educator_evaluation.tour_save_failed_20260902", "The tour script could not be saved in this browser."));
      return;
    }
    setTourCustom(true);
    setTourText(JSON.stringify(steps, null, 2));
  ...
  const resetTour = () => {
    ...
    setTourCustom(false);
    setTourText(JSON.stringify(AE_GUIDED_TOUR_STEPS, null, 2));
    ...
    reader.onload = () => {
      try {
        ...
        if (tourSteps && aeWriteTourOverride(tourSteps)) {
          setTourCustom(true);
          setTourText(JSON.stringify(tourSteps, null, 2));
        }
