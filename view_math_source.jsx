...
function _renderDiagramSvg(tool, state, titleText) {
  ...
  if (tool === 'numberline') {
    ...
    var W = 380, H = 84, padX = 24, axisY = 46;
    ...
    return '<svg viewBox="0 0 ' + W + ' ' + H + '" role="img" aria-label="' + nlTitle + ': ' + nlDesc + '" width="100%" style="max-width:420px"><title>' + nlTitle + '</title><desc>' + nlDesc + '</desc>'
                                    ...
                                    {isIndependentMode && (
                                        <div className="ml-4 sm:ml-12 mt-4 mb-4 space-y-3">
                                            {problem.manipulativeSupport && (() => {
                                               ...
                                               return _suppSvg ? (<div className="mb-2 flex justify-center bg-slate-50 rounded-lg border border-slate-100 p-3 overflow-hidden" dangerouslySetInnerHTML={{ __html: _suppSvg }} />) : null;
                                            })()}
                                            {problem.manipulativeSupport && (
                                               <button onClick={() => {
                                                  ...
                                                  if (problem.manipulativeSupport.tool === 'coordinate' && problem.manipulativeSupport.state?.points) {
                                                     ...
                                                     setFractionPieces({ numerator: problem.manipulativeSupport.state.numerator || 0, denominator: problem.manipulativeSupport.state.denominator || 8 });
                                                  } else if (problem.manipulativeSupport.tool === 'volume' && problem.manipulativeSupport.state?.dims) {
                                                     setCubeDims({ l: problem.manipulativeSupport.state.dims.l || 1, w: problem.manipulativeSupport.state.dims.w || 1, h: problem.manipulativeSupport.state.dims.h || 1 });
                                                  }
                                                  setShowStemLab(true);
