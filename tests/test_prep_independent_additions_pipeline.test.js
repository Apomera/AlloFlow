...
const { canonical, validPackVersion } =
  require('../dev-tools/apply_test_prep_independent_additions.cjs');
...
describe('manifest-backed independent test-prep additions', () => {
  it('preserves arithmetic operators in content canonicalization', () => {
    ...
    expect(new Set(forms.map((value) => canonical(value))).size)
      .toBe(forms.length - 2);
  ...
  it('keeps --check read-only across every manifest pack source and deploy artifact', () => {
    ...
    const before = Object.fromEntries(
      targets.map((file) => [file, digest(file)]),
    );
    execFileSync(
      process.execPath,
      ['dev-tools/apply_test_prep_independent_additions.cjs', '--check'],
      { cwd: root, stdio: 'pipe' },
    );
  ...
  it('registers exact hash-bound 100-item banks for every one-bank manifest pack', () => {
    ...
    for (const stem of [
      ...
      const itemBytes = fs.readFileSync(
        path.join(authoredDir, batches[0].files[0]),
      );
      const items = JSON.parse(itemBytes);
      const review = JSON.parse(fs.readFileSync(
        path.join(authoredDir, batches[0].reviewReports[0]),
        'utf8',
      ));
      ...
      expect(lineByLineReview.reviewedItemIds)
  ...
  it('keeps registered pack titles credential-specific and versions semantic after expansion', () => {
    for (const [stem, credential] of [
      ...
      ['school_psychologist_5403', 'School Psychologist (5403)'],
    ]) {
      const pack = JSON.parse(fs.readFileSync(
        path.join(root, 'test_prep', `${stem}_pack.json`),
        'utf8',
      ));
      expect(pack.title).toContain(credential);
      expect(pack.title).toMatch(
        /200 Source Questions \+ 100 Independent Practice Questions \+ 200 Guided Review$/,
      );
