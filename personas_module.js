...
const createPersonas = deps => {
  ...
  const handleGeneratePersonas = async () => {
    ...
    const sourceText = String(latestAnalysis && latestAnalysis.data && latestAnalysis.data.originalText ? latestAnalysis.data.originalText : inputText || '');
  ...
  const generatePersonaFollowUps = async (history, character, count = 2) => {
    ...
    if (activePersonaFollowUpRequest && activePersonaFollowUpRequest.token === requestToken && activePersonaFollowUpRequest.historyFingerprint === historyFingerprint && activePersonaFollowUpRequest.characterName === characterName) return;
    ...
    try {
      ...
      const suggestionPayload = Array.isArray(result) ? result : (() => {
        const rawText = result && typeof result === 'object' && typeof result.text === 'string' ? result.text : String(result || '');
        try {
  ...
  const generatePanelFollowUps = async (history, charA, charB) => {
    ...
    if (activePanelFollowUpRequest && activePanelFollowUpRequest.token === requestToken && activePanelFollowUpRequest.historyFingerprint === historyFingerprint && activePanelFollowUpRequest.participantNames.join('\u0000') === participantNames.join('\u0000')) return;
    const followUpRequest = {
      token: requestToken,
      resourceId,
      participantNames,
    ...
    const isFreshFollowUp = () => {
      ...
      return activePanelFollowUpRequest === followUpRequest && requestToken === personaSessionToken && getPersonaResourceId(liveRef.current.generatedContent) === resourceId && currentState.mode === 'panel' && currentNames.join('\u0000') === participantNames.join('\u0000') && createInterviewFingerprint(resourceId, 'panel', participantNames, currentState.chatHistory) === historyFingerprint;
    };
    const targetLang = resolvePersonaLanguage(leveledTextLanguage, selectedLanguages);
    const safeTargetLang = promptData(targetLang, 100);
    setPersonaState(prev => ({
    ...
    try {
      ...
      const prompt = `
            ...
            `;
      const result = await window.callGemini(prompt, true);
      const panelPayload = Array.isArray(result) ? result : result && typeof result === 'object' && !Array.isArray(result) && !result.text ? result : await resilientJsonParse(result && typeof result === 'object' && typeof result.text === 'string' ? result.text : String(result || ''));
  ...
  const handleStartPanelChat = async () => {
    ...
    const isCurrentPanelContext = () => {
      const currentState = liveRef.current.personaState || {};
      const currentNames = (Array.isArray(currentState.selectedCharacters) ? currentState.selectedCharacters : []).map(character => String(character && character.name || '').slice(0, 120));
      return requestToken === personaSessionToken && getPersonaResourceId(liveRef.current.generatedContent) === resourceId && currentNames.join('\u0000') === participantNames.join('\u0000');
    };
  ...
  const handleClosePersonaChat = () => {
    ...
    const persistedChatHistory = personaState.isLoading && liveChatHistory.at(-1)?.role === 'user' ? liveChatHistory.slice(0, -1) : liveChatHistory;
  ...
  const handlePersonaTopicSpark = async () => {
    ...
    const isFreshSpark = () => activeTopicSparkRequest === sparkRequest && requestToken === personaSessionToken && (liveRef.current.generatedContent && liveRef.current.generatedContent.id || null) === resourceId;
    const targetLang = resolvePersonaLanguage(leveledTextLanguage, selectedLanguages);
    const safeTargetLang = promptData(targetLang, 100);
  ...
  const handlePanelChatSubmit = async (userText, fromSuggestion = false) => {
    ...
    const isFreshTurn = () => {
      ...
      return activeTurnRequest === turnRequest && turnRequest.token === personaSessionToken && getPersonaResourceId(liveRef.current.generatedContent) === resourceId && currentState.mode === 'panel' && currentNames.join('\u0000') === participantNames.join('\u0000');
    };
    ...
    try {
      ...
      const panelResultText = resultRaw && typeof resultRaw === 'object' && typeof resultRaw.text === 'string' ? resultRaw.text : String(resultRaw || '');
      const parsedData = resultRaw && typeof resultRaw === 'object' && !Array.isArray(resultRaw) && !resultRaw.text ? resultRaw : await resilientJsonParse(panelResultText);
      if (!isFreshTurn()) return;
      ...
      if (!isPersonaFreeResponse) {
        const updatedHistory = [...personaState.chatHistory, {
          role: 'user',
          text: userText
  ...
  const handlePersonaChatSubmit = async (overrideInput = null, fromSuggestion = false) => {
    ...
    const isFreshTurn = () => {
      const currentState = liveRef.current.personaState || {};
      return activeTurnRequest === turnRequest && turnRequest.token === personaSessionToken && getPersonaResourceId(liveRef.current.generatedContent) === resourceId && currentState.mode !== 'panel' && currentState.selectedCharacter?.name === characterName;
    };
    ...
    try {
      ...
      const resultText = resultRaw && typeof resultRaw === 'object' && typeof resultRaw.text === 'string' ? resultRaw.text : String(resultRaw || '');
  ...
  const handleGeneratePersonaSummary = async (options = {}) => {
    ...
    const cachedSummaryItem = (Array.isArray(history) ? history : []).find(item => item && item.type === 'persona-summary' && item.config && item.config.summaryFingerprint === summaryFingerprint && item.data);
    ...
    if (cachedSummary && !force) {
      setHistory(prev => prev.map(item => item && item.id === cachedSummaryItem.id ? {
        ...item,
    ...
    const isFreshSummary = () => {
      ...
      return activeSummaryRequest === summaryRequest && requestToken === personaSessionToken && getPersonaResourceId(currentResource) === resourceId && currentSummaryFingerprint === summaryFingerprint;
    };
    setPersonaState(prev => ({
      ...prev,
  ...
  const handleSavePersonaChat = () => {
    ...
    const chatLog = personaState.chatHistory.slice(-200).map(message => {
      const speaker = message && message.role === 'user' ? 'Student' : String(message && message.speakerName || personaState.selectedCharacter.name).slice(0, 120);
    ...
    const exportFingerprint = createInterviewFingerprint(resourceId, isPanelSave ? 'panel' : 'single', participants.map(character => character.name), personaState.chatHistory);
