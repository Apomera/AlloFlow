...
const out = path.join(ROOT, 'scratch', 'water-coastal-water-review');
...
(async () => {
  ...
  try {
    ...
    await checkCoast();const pausedCoast=await coast();await page.waitForTimeout(500);assert.deepEqual(await coast(),pausedCoast);
    ...
    await page.evaluate(()=>{
      window.sunlightDisposals={ring:0,fill:0,texture:0,shelfGeometry:0,shelfMaterial:0,foamGeometry:0,foamMaterial:0};const scene=pilotReview.scene;
