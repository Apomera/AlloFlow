...
const slug='photosynthesis_grade7',folder='allopacks/media/'+slug+'/',pack=read('allopacks/illustrated/'+slug+'.allopack.json'),original=read('allopacks/'+slug+'.allopack.json'),manifest=read(folder+'manifest.json');
require('../dev-tools/photosynthesis_content_refinements.cjs').refine(original);
...
describe('photosynthesis_grade7 illustrated edition',()=>{
...
for(const tag of ['co2-in','water-in','energy-in','oxygen-out','sugar-out'])expect(svg('inputs')).toContain('data-flow="'+tag+'"');expect((svg('water').match(/<circle/g)||[]).length).toBe(3);expect((svg('co2').match(/<circle/g)||[]).length).toBe(3);expect((svg('oxygen').match(/<circle/g)||[]).length).toBe(2);expect(get('concept-sort').data.categories[0].label).toBe('INPUT (material or energy)');expect(get('simplified').data).toContain('Light supplies energy, not atoms');
});
