// Lazy Lucide icon resolution from window.AlloIcons (populated by
  ...
  function GlossaryView(props) {
    ...
    return <><div className="space-y-6"><div className="bg-blue-50 p-4 rounded-lg border border-blue-100 mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"><p className="text-sm text-blue-800 max-w-xs"><strong>{t('simplified.udl_goal').split(':')[0]}:</strong> {t('glossary.udl_goal_desc')}</p><div className="flex flex-col gap-2 w-full sm:w-auto"><div className="flex flex-wrap items-center gap-2"><div className="flex bg-white rounded-full p-1 border border-blue-200 shadow-sm"><button data-help-key="glossary_standard_flashcards" onClick={() => launchInteractiveFlashcards('standard')} className="px-3 py-1 rounded-full text-xs font-bold text-blue-600 hover:bg-blue-50 transition-colors flex items-center gap-1" title={t('flashcards.tooltip_launch_standard')}><MonitorPlay size={14} /> {t('flashcards.launch_standard')}</button>{selectedLanguages.length > 0 && <><div className="w-px bg-blue-200 my-1" /><button data-help-key="glossary_language_flashcards" onClick={() => launchInteractiveFlashcards('language')} className="px-3 py-1 rounded-full text-xs font-bold text-indigo-600 hover:bg-indigo-50 transition-colors flex items-center gap-1" title={t('flashcards.tooltip_launch_language')}><Languages size={14} /> {t('flashcards.launch_language')}</button></>}</div>{isTeacherMode && <button data-help-key="glossary_export_standard" onClick={() => handleExportFlashcards('standard')} className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold bg-indigo-600 text-white hover:bg-indigo-700 transition-all shadow-sm whitespace-nowrap" title={t('flashcards.tooltip_export_standard')}><GalleryHorizontal size={14} /> {t('flashcards.export_standard')}</button>}{isTeacherMode && selectedLanguages.length > 0 && <button data-help-key="glossary_export_language" onClick={() => handleExportFlashcards('language')} className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold bg-indigo-500 text-white hover:bg-indigo-600 transition-all shadow-sm whitespace-nowrap" title={t('flashcards.tooltip_export_language')}><Languages size={14} /> {t('flashcards.export_language')}</button>}</div><div className="flex flex-wrap items-center gap-2"><div className="flex items-center bg-teal-50 rounded-full p-0.5 border border-teal-200">{selectedLanguages.length > 0 && <select aria-label={t('common.selection')} data-help-key="glossary_puzzle_lang" value={wordSearchLang} onChange={e => {
                  setWordSearchLang(e.target.value);
                  if (gameMode === 'wordsearch') {
                    generateWordSearch(e.target.value);
                  }
                }} className="text-xs font-bold text-teal-700 bg-transparent border-r border-teal-600 px-3 py-1.5  focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 cursor-pointer hover:bg-teal-100 rounded-l-full" title={t('glossary.tooltips.select_puzzle_lang')}><option value="English">{t('common.english')}</option>{selectedLanguages.map(lang => <option key={lang} value={lang}>{lang}</option>)}</select>}<button aria-label={t('common.start_game')} data-help-key="glossary_word_search" onClick={() => generateWordSearch(wordSearchLang)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all whitespace-nowrap ${selectedLanguages.length > 0 ? 'rounded-l-none pl-2' : ''} bg-teal-700 text-white hover:bg-teal-800 shadow-sm`} title={t('glossary.tooltips.generate_word_search')}><Gamepad2 size={14} />{gameMode === 'wordsearch' ? t('common.regenerate') : t('glossary.word_search')}</button></div><button data-help-key="glossary_memory_game" onClick={handleSetIsMemoryGameToTrue} className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold bg-purple-600 text-white hover:bg-purple-700 transition-all shadow-sm whitespace-nowrap" title={t('glossary.tooltips.launch_memory')}><Brain size={14} /> {t('glossary.memory_game')}</button><button aria-label={t('common.start_game')} data-help-key="glossary_crossword" onClick={handleSetIsCrosswordGameToTrue} className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold bg-emerald-700 text-white hover:bg-emerald-800 transition-all shadow-sm whitespace-nowrap" title={t('glossary.tooltips.generate_crossword')}><Gamepad2 size={14} /> {t('glossary.crossword')}</button><button data-help-key="glossary_matching" onClick={handleSetIsMatchingGameToTrue} className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold bg-orange-700 text-white hover:bg-orange-800 transition-all shadow-sm whitespace-nowrap" title={t('glossary.tooltips.generate_matching')}><GitMerge size={14} /> {t('glossary.matching')}</button><button aria-label={t('common.start_game')} data-help-key="glossary_bingo" onClick={handleSetIsBingoGameToTrue} className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold bg-rose-700 text-white hover:bg-rose-800 transition-all shadow-sm whitespace-nowrap" title={t('glossary.tooltips.generate_bingo')}><Gamepad2 size={14} /> {t('glossary.bingo')}</button><button aria-label={t('common.start_game')} data-help-key="glossary_play_bingo" onClick={handleSetIsStudentBingoGameToTrue} className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold bg-pink-700 text-white hover:bg-pink-800 transition-all shadow-sm whitespace-nowrap" title={t('glossary.tooltips.launch_bingo')}><Gamepad2 size={14} /> {t('glossary.play_bingo')}</button><button data-help-key="glossary_scramble" onClick={handleSetIsWordScrambleGameToTrue} className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold bg-cyan-700 text-white hover:bg-cyan-800 transition-all shadow-sm whitespace-nowrap" title={t('glossary.tooltips.launch_scramble')}><Shuffle size={14} /> {t('glossary.scramble')}</button></div><div className="flex flex-wrap items-center gap-2">{isTeacherMode && <button data-help-key="glossary_edit" onClick={handleToggleIsEditingGlossary} aria-label={isEditingGlossary ? t('common.done') : t('common.edit')} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all shadow-sm whitespace-nowrap ${isEditingGlossary ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-white text-blue-700 border border-blue-200 hover:bg-blue-50'}`}>{isEditingGlossary ? <CheckCircle2 size={14} /> : <Pencil size={14} />}{isEditingGlossary ? t('common.done') : t('common.edit')}</button>}<div data-help-key="glossary_image_size" className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-full border border-blue-200 shadow-sm" title={t('glossary.image_size_tooltip')}><ImageIcon size={14} className="text-blue-700" /><input aria-label={t('common.range_slider')} type="range" min="64" max="300" step="16" value={glossaryImageSize} onChange={e => setGlossaryImageSize(Number(e.target.value))} className="w-20 h-1.5 bg-blue-100 rounded-lg appearance-none cursor-pointer accent-blue-500" /></div><div className="flex bg-white p-1 rounded-full border border-blue-200 shadow-sm shrink-0"><button data-help-key="glossary_filter_all" onClick={handleSetGlossaryFilterToAll} className={`px-3 py-0.5 rounded-full text-[11px] font-bold transition-all ${glossaryFilter === 'all' ? 'bg-blue-100 text-blue-700 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>{t('glossary.filter_all')}</button><button aria-label={t('common.close')} data-help-key="glossary_filter_tier2" onClick={handleSetGlossaryFilterToAcademic} className={`px-3 py-0.5 rounded-full text-[11px] font-bold transition-all ${glossaryFilter === 'academic' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>{t('glossary.filter_tier2')}</button><button aria-label={t('common.close')} data-help-key="glossary_filter_tier3" onClick={handleSetGlossaryFilterToDomain} className={`px-3 py-0.5 rounded-full text-[11px] font-bold transition-all ${glossaryFilter === 'domain' ? 'bg-purple-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>{t('glossary.filter_tier3')}</button></div><div data-help-key="glossary_search" className="relative w-full sm:w-auto"><Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-blue-700" size={14} /><input type="text" aria-label={t('glossary.search_placeholder') || 'Search glossary'} placeholder={t('glossary.search_placeholder')} value={glossarySearchTerm} onChange={e => setGlossarySearchTerm(e.target.value)} className="pl-8 pr-3 py-1.5 text-sm border border-blue-600 rounded  focus:ring-2 focus:ring-indigo-200 w-full" />{glossarySearchTerm && <button onClick={handleSetGlossarySearchTermConst} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-blue-700 hover:text-blue-900" aria-label={t('common.clear')}><X size={14} /></button>}</div><button onClick={() => {
    ...
    ><X size={16} aria-hidden="true" /></button>
                  ...
                  return <tr key={idx} className="hover:bg-slate-50 group/row">{isEditingGlossary && <td className="p-4 border-b border-slate-100 text-center align-top"><input aria-label={t('common.toggle_is_selected_false')} type="checkbox" className="mt-1.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4 cursor-pointer" checked={item.isSelected !== false} onChange={() => handleGlossarySelectionChange(idx)} title={t('glossary.tooltips.select_highlight')} /></td>}<td className="p-4 font-bold text-slate-800 align-top min-w-[200px]"><div className="flex flex-col gap-2 items-center">{!isEditingGlossary && isTeacherMode && item.tier && <span className={`text-[11px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-full border mb-1 ${isTier2 ? 'bg-blue-50 text-blue-700 border-blue-200' : isTier3 ? 'bg-purple-50 text-purple-700 border-purple-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>{isTier2 ? t('glossary.label_tier2') : isTier3 ? t('glossary.label_tier3') : item.tier}</span>}{isEditingGlossary ? <div className="w-full flex flex-col gap-2"><select aria-label={t('common.selection')} value={item.tier || ''} onChange={e => handleGlossaryChange(idx, 'tier', e.target.value)} className="text-[11px] font-bold uppercase tracking-wider w-full border border-slate-400 rounded px-1 py-1  focus:ring-1 focus:ring-indigo-500 bg-slate-50 text-slate-600" data-help-key="glossary_edit_tier"><option value="">{t('glossary.edit_tier_placeholder')}</option><option value="Academic">{t('glossary.edit_tier_academic')}</option><option value="Domain-Specific">{t('glossary.edit_tier_domain')}</option></select><div className="flex items-start gap-2 w-full"><textarea aria-label={t('glossary.edit_term') || 'Edit glossary term'} value={item.term} onChange={e => handleGlossaryChange(idx, 'term', e.target.value)} rows={getRows(item.term, 25)} className="w-full bg-white border border-blue-300 rounded px-2 py-1  focus:ring-2 focus:ring-blue-200 resize-y text-sm font-bold text-center" /><button aria-label={t('common.delete')} onClick={() => handleDeleteGlossaryItem(idx)} className="p-2 bg-red-50 text-red-500 hover:bg-red-100 rounded transition-colors shrink-0" title={t('glossary.tooltips.delete_term')}><Trash2 size={14} /></button></div></div> : <div className="px-2 py-1 font-bold text-slate-800 whitespace-pre-wrap text-center">{item.term}</div>}<div className="mt-1 px-1">{item.image ? <div className="flex flex-col gap-2"><div className="relative group/image inline-block w-fit"><img loading="lazy" src={item.image} alt={`${item.term} icon`} style={{
                          ...
                          handlePhonicsClick && <button onClick={ev => handlePhonicsClick(item.term, ev)} className="text-slate-600 hover:text-emerald-700 hover:bg-emerald-50 p-1 rounded-full transition-colors flex-shrink-0 font-bold text-[11px] tracking-tight" aria-label={(t('glossary.pronounce_term') || 'Pronounce') + ': ' + item.term} title={t('glossary.pronounce_term_title') || 'Show pronunciation, IPA, and syllables'} data-help-key="glossary_pronounce_term">/əˈ/</button>}<button onClick={() => handleDownloadAudio(item.term, `term-${idx}-audio`, `dl-term-${idx}`)} disabled={downloadingContentId === `dl-term-${idx}`} className="text-slate-600 hover:text-indigo-600 p-1 rounded-full transition-colors" data-help-key="glossary_download_audio">{downloadingContentId === `dl-term-${idx}` ? <RefreshCw size={14} className="animate-spin motion-reduce:animate-none" /> : <Download size={14} />}</button></div></div></td><td className="p-4 text-slate-600 align-top"><div className="flex flex-col gap-2 items-center">{isEditingGlossary ? <textarea aria-label={t('glossary.edit_definition') || 'Edit definition'} value={item.def} onChange={e => handleGlossaryChange(idx, 'def', e.target.value)} rows={getRows(item.def, 45)} className="w-full bg-white border border-blue-300 rounded px-2 py-1  focus:ring-2 focus:ring-blue-200 resize-y text-sm text-center" /> : <div className="px-2 py-1 text-slate-600 whitespace-pre-wrap">{currentUiLanguage !== 'English' && <span className="block font-bold text-indigo-900 text-xs mb-1 uppercase tracking-wide">{item.term}</span>}{item.def}</div>}<div className="flex items-center gap-1 opacity-50 group-hover/row:opacity-100 transition-opacity"><button onClick={() => handleSpeak(item.def, `def-${idx}`)} disabled={isGeneratingAudio && playingContentId !== `def-${idx}`} className={`p-1 rounded-full transition-colors flex-shrink-0 ${playingContentId === `def-${idx}` ? 'text-red-700 bg-red-50' : 'text-slate-600 hover:text-indigo-600 hover:bg-indigo-50'}`}>{playingContentId === `def-${idx}` && isGeneratingAudio ? <RefreshCw size={14} className="animate-spin motion-reduce:animate-none" /> : playingContentId === `def-${idx}` ? <StopCircle size={14} /> : <Volume2 size={14} />}</button><button onClick={() => handleDownloadAudio(item.def, `def-${idx}-audio`, `dl-def-${idx}`)} disabled={downloadingContentId === `dl-def-${idx}`} className="text-slate-600 hover:text-indigo-600 p-1 rounded-full transition-colors">{downloadingContentId === `dl-def-${idx}` ? <RefreshCw size={14} className="animate-spin motion-reduce:animate-none" /> : <Download size={14} />}</button></div>{item.etymology ? <details className="mt-2 text-xs w-full" data-help-key="glossary_etymology_info"><summary className="cursor-pointer text-indigo-700 font-medium hover:text-indigo-900 select-none">📜 {t('glossary.etymology_label') || 'Word roots'}</summary>{(() => {
                            ...
                            return <div className="mt-1 pl-4 border-l-2 border-indigo-200 space-y-2">{etyMap && Object.keys(etyMap).length > 1 && <div className="text-[10px] uppercase tracking-wide text-indigo-500 font-bold">{displayLang}</div>}<div className="flex items-start gap-1"><p className="text-slate-700 leading-relaxed italic flex-1">{etymologyProse}</p><button onClick={() => handleSpeak(etymologyProse, `etym-${idx}`)} disabled={isGeneratingAudio && playingContentId !== `etym-${idx}`} className={`p-1 rounded-full transition-colors flex-shrink-0 ${playingContentId === `etym-${idx}` ? 'text-red-700 bg-red-50' : 'text-slate-600 hover:text-indigo-600 hover:bg-indigo-50'}`} aria-label={t('glossary.etymology_label') || 'Word roots'}>{playingContentId === `etym-${idx}` && isGeneratingAudio ? <RefreshCw size={12} className="animate-spin motion-reduce:animate-none" /> : <Volume2 size={12} />}</button></div>{(() => {
