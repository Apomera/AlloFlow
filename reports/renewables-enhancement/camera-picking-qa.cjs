...
(async()=>{
  await new Promise(r=>server.listen(0,'127.0.0.1',r));const browser=await chromium.launch({headless:true,args:['--use-angle=swiftshader','--enable-unsafe-swiftshader']});const checks={views:[],audits:{},annotationFrames:0},errors=[],consoleErrors=[];let page;
  try{
    ...
    await click('Explore Solar PV');await click('Exploded view');
    for(const view of views){
      await click('Inspect PV array');await click(view[0]);await assertView(...view);
      const point=await page.evaluate(()=>{const {camera}=inspectionFrame,rect=document.querySelector('canvas').getBoundingClientRect(),p=new THREE.Vector3(5.4,1,0).project(camera);return{x:rect.x+(p.x+1)*rect.width/2,y:rect.y+(1-p.y)*rect.height/2};});
      await page.mouse.click(point.x,point.y);await ready();assert.equal(await page.getByRole('button',{name:'Inspect Inverter',exact:true}).getAttribute('aria-pressed'),'true','Picking from '+view[0]);checks.views.push(view[1]);
    }
    await click('Isolate selected component');await click('Inspect Sunlight');await page.getByLabel('Direct irradiance',{exact:true}).fill('0');for(const view of views){await click(view[0]);const frame=await geometry();assertLabels(frame);assert.ok(frame.direction.every(Number.isFinite));}await page.getByLabel('Direct irradiance',{exact:true}).fill('800');await click('Fit mechanism');await audit('emptyResourceRecovery');
    await page.setViewportSize({width:320,height:844});await click('Inspect PV array');await click('Side view');await assertView(...views[2]);await page.locator('.rn-energy-stage').screenshot({path:path.join(out,'camera-phone-focused.png')});await audit('phoneFocused');
    checks.pageErrors=errors;checks.consoleErrors=consoleErrors;assert.deepEqual(errors,[]);assert.deepEqual(consoleErrors,[]);checks.sourceSha256=require('crypto').createHash('sha256').update(fs.readFileSync(files['/tool.js'])).digest('hex');fs.writeFileSync(path.join(out,'camera-picking-results.json'),JSON.stringify(checks,null,2));console.log(JSON.stringify(checks,null,2));
