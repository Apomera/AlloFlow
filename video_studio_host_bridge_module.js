...
if (!React) { console.error('[VideoStudioHostBridgeView] React not found on window'); return; }
window.AlloModules = window.AlloModules || {};
if (window.AlloModules.VideoStudioHostBridgeView) return;
...
function VideoStudioHostBridgeView(props) {
  ...
  return /*#__PURE__*/React.createElement(CDNModuleGate, {
    ...
    onGetOfficialTutorial: async tutorialId => {
      ...
      const manifest = TC.buildTutorialManifest(GUIDED_STEPS, GUIDED_TOUR_MAP, t, {
        only: ['source-input', 'simplified'],
        wpm: 150
      });
      return {
    ...
    onRunOfficialTutorial: async (tutorialId, steps, hooks) => {
      ...
      _officialTutorialSnapshotRef.current = _officialTutorialSnapshotRef.current || {
        ...
        activeView,
        gradeLevel,
        expandedTools: expandedTools.slice()
      };
      _planRunRef.current = {
        running: true,
        stop: false
      ...
      const spotlight = async anchorId => {
        let el = null;
        for (let attempt = 0; !el && attempt < 20; attempt++) {
        ...
        if (!hooks || hooks.cursorEmphasis !== false) {
          ...
          requestAnimationFrame(() => {
            if (cursorMarker) cursorMarker.style.transform = 'scale(.72)';
          });
        }
        await wait(650);
      ...
      try {
        ...
        if (document.visibilityState !== 'visible') return {
          ok: false,
          completed: 0,
          reason: 'The AlloFlow tab never became visible.'
        };
        let completed = 0;
        for (let i = 0; i < list.length; i++) {
          ...
          if (step.id === 'source-input') {
            ...
            setGeneratedContent({
              id: 'official-text-adaptation-fixture',
              type: 'simplified',
              title: 'Photosynthesis - Grade 5 adaptation',
              data: fixtureAdapted,
              timestamp: new Date(),
              config: {
    ...
    onPlanDemo: async (goal, options = {}) => {
      ...
      if ((!steps || !steps.length) && typeof AC.routeUtterance === 'function') {
        ...
        try {
          const m = await AC.routeUtterance(_alloCmdCtx(), cleanGoal, {
            allowAi: true,
            preview: true,
            signal: options.signal || null
          });
          if (m && m.commandId) {
            ...
            const report = AC.validatePlan(_alloCmdCtx(), candidate, {
              demoSafeOnly: true
            });
      ...
      const cmds = AC.buildAlloCommands(_alloCmdCtx(), {
        includeGated: true
      });
      return {
        steps: steps.map(s => {
          ...
          return {
            ...
            params: s.params || {},
            paramNames: typeof AC.getCommandContract === 'function' ? AC.getCommandContract(c || s.commandId).params : [],
            why: s.why || '',
            label: c && c.label || s.commandId
          };
        })
      };
    },
    onValidateDemoPlan: async steps => {
      ...
      return AC.validatePlan(_alloCmdCtx(), steps, {
        demoSafeOnly: true
      });
    },
    onRunDemoPlan: async (steps, hooks, options) => {
      ...
      const readiness = AC.validatePlan(_alloCmdCtx(), steps, {
        demoSafeOnly: true
      });
      if (!readiness || !readiness.ok) {
        ...
        return {
          ok: false,
          completed: 0,
          reason: blocked && (blocked.label || blocked.commandId) + ': ' + blocked.detail || 'This demo plan is not ready to run.'
        };
      }
      if (_planRunRef.current && _planRunRef.current.running) throw new Error('AlloBot is already running a plan — stop it first.');
      _planRunRef.current = {
      ...
      const emphasizeCursorTarget = (cmd, label) => {
        ...
        requestAnimationFrame(() => {
          marker.style.transform = 'scale(.7)';
        });
        setTimeout(() => {
          marker.style.opacity = '0';
          setTimeout(() => {
            cursorMarkers.delete(marker);
            marker.remove();
          }, 320);
        }, 700);
      };
      try {
        if (rehearsal) {
          ...
          for (let i = 0; i < previewList.length; i++) {
            if (stopWanted()) return {
              ...
              reason: 'Rehearsal stopped by the teacher.'
            };
            const readyItem = readiness.items[i] || {};
            const previewLabel = readyItem.label || previewList[i].commandId;
            try {
              if (hooks && hooks.onStep) hooks.onStep(i, 'start', previewLabel, 'Checking: ' + previewLabel + '.');
            } catch (_) {}
            ...
            try {
              if (hooks && hooks.onStep) hooks.onStep(i, 'done', previewLabel, readyItem.detail || 'Command and prerequisites are ready.');
            } catch (_) {}
            previewCompleted++;
        ...
        if (stopWanted()) return {
          ...
          stopped: true,
          completed: 0,
          reason: 'Stopped before the demo began.'
        };
        if (document.visibilityState !== 'visible') {
          return {
            ok: false,
            completed: 0,
            reason: 'The AlloFlow tab never became visible, so no automatic actions were run.'
          };
        }
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          const r = await AC.runPlan(() => _alloCmdCtxRef.current || _alloCmdCtx(), [list[i]], {
            ...
            onStep: (j, phase, cmd, narr) => {
              ...
              if (phase === 'done') completionEvent = {
                label,
                narration: narr || ''
              };else {
                emphasizeCursorTarget(cmd, label);
                try {
                  if (hooks && hooks.onStep) hooks.onStep(i, phase, label, narr || '');
                } catch (_) {}
              }
            }
          });
          if (!r || !r.ok) {
            ...
            if (!stoppedByTeacher && hooks && typeof hooks.onDecision === 'function') {
              const choice = await new Promise(resolve => {
                ...
                const finish = v => {
                  if (!done) {
                    ...
                    clearInterval(guard);
                    resolve(v === 'retry' || v === 'skip' ? v : 'stop');
                  }
                };
                const startedWaiting = Date.now();
                guard = setInterval(() => {
                  if (stopWanted() || Date.now() - startedWaiting > 240000) finish('stop');
                }, 250);
                Promise.resolve(hooks.onDecision({
                  index: i,
                  label: list[i].commandId,
                  reason: failReason,
                  timedOut: !!(r && r.timedOut)
                })).then(finish, () => finish('stop'));
              });
              if (choice === 'retry') {
                i--;
                continue;
              ...
              return {
                ...
                timedOut: !!(r && r.timedOut),
                reason: failReason,
                stateSummary: stateSummaryForAudit()
              };
            }
            return {
              ok: false,
              completed,
              stopped: stoppedByTeacher,
              timedOut: !!(r && r.timedOut),
              reason: failReason,
              stateSummary: stateSummaryForAudit()
            };
          ...
          const done = completionEvent || {
            label: list[i].commandId,
            narration: 'Step complete.'
          };
          try {
            if (hooks && hooks.onStep) hooks.onStep(i, 'done', done.label, done.narration);
          } catch (_) {}
          completed++;
          if (stopWanted()) return {
            ok: false,
    ...
    onOpenCinematicStudio: () => {
      setIsVideoStudioOpen(false);
      setShowCinematicStudio(true);
    }
  }));
}
window.AlloModules.VideoStudioHostBridgeView = VideoStudioHostBridgeView;
window.AlloModules.VideoStudioHostBridgeView = window.AlloModules.VideoStudioHostBridgeView;
console.log('[CDN] VideoStudioHostBridgeView loaded');
})();
