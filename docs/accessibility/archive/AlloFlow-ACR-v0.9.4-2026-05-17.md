# Archived accessibility assessment — AlloFlow v0.9.4

> **Superseded historical record.** This self-assessment describes the May 17, 2026 v0.9.4 snapshot. It does not describe the current product and must not be used as evidence of v1.2 conformance. See the [current interim Accessibility Conformance Report](../../../VPAT-2.5-WCAG-AlloFlow.md).
...
> **Current evidence pointer.** For the newer, limited-scope automated and rendered evaluation of the Desktop command center and bundled application shell, see [the July 11, 2026 WCAG 2.2 current audit](../../../a11y-audit/WCAG-2.2-current-audit.md). That report also does not claim full-product conformance.
...
| **2.1.1 Keyboard** | Supports | `a11yClick` utility applied across 57 STEM tools and 19 SEL tools *at evaluation time* (these are utility-coverage counts, not tool totals; total SEL Hub tools is 70 — newer tools added since the evaluation are pending keyboard re-verification) ensures `onClick` handlers on non-button elements respond to Enter/Space via `onKeyDown`. `tabIndex={0}` applied to all custom interactive elements. Tab patterns use `tabIndex={-1}` for inactive tabs. Symbol Studio AAC Use mode has full ARIA grid keyboard navigation (arrow keys, Enter/Space, Home/End) with roving tabindex. Standalone board exports include complete keyboard navigation and 1-switch/2-switch scanning. May 2026 added Enter/Space `onKeyDown` handlers to immersive reader play-toggle viewport, crawl viewport, and clickable sentence spans. **Modal backdrops** (`onClick={onClose}`) intentionally lack direct keyboard handlers because each modal dialog provides an Escape-key dismissal path AND an explicit visible close button — both keyboard-reachable; backdrop click is a redundant convenience for mouse users only. **Global keyboard-trap safety net** installed at App.jsx: a bubble-phase Escape listener detects topmost `role="dialog"` and clicks its close button as a last-resort escape hatch if the per-component handler missed (re-checks dialog presence after 50ms so it never double-fires). |
