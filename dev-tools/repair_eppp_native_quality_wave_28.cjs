...
const { baselineMetrics, reviewedAt, reviewWave, revisions } = require('./eppp_native_quality_wave_28_data.cjs');
...
const auditBasename = 'eppp_question_quality_audit_wave_28.json';
...
if (fs.readFileSync(deployBankPath, 'utf8') !== sourceText) throw new Error('Source and deploy EPPP banks must match before wave 28.');
...
if (revisions.length !== 24 || new Set(revisions.map((revision) => revision.id)).size !== 24) throw new Error('Wave 28 needs twenty-four unique revisions.');
...
for (const revision of revisions) {
  ...
  if (!recognizedReplay && item.prompt !== revision.expectedPrompt) throw new Error(revision.id + ' prompt drifted before wave 28.');
...
const audit = {
  ...
  scope: 'Twenty-four EPPP items, three per domain, rewritten as applied or analytic decisions with parallel distractors and concise misconception-specific feedback.',
  summary: {
...
console.log('EPPP quality wave 28: rewrote 24 items across 8 domains; selected warnings after = 0.');
