...
const wave = require('../dev-tools/eppp_native_quality_wave_32_data.cjs');
...
describe('EPPP expanded quality wave 32', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_32.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 24, applicationItems: 19, analysisItems: 5, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    expect(audit.summary.distractorWarningsAfter.uniqueKeyStemLexicalLeakageCandidates).toBe(45);
    expect(audit.summary.distractorWarningsAfter.asymmetricExtremeDistractorCandidates).toBe(86);
    expect(audit.summary.distractorWarningsAfter.advancedDirectRecallCandidates).toBe(3);
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicatePairs).toBe(49);
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicateClusters).toBe(34);
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(474);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1314);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(624);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(636);
    expect(audit.summary.feedbackWarningsAfter.choiceRestatementOptions).toBe(148);
    expect(audit.summary.feedbackWarningsAfter.fullKeyEchoOptions).toBe(64);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_32.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_32.json'));
  });

  it('replays after wave 31', () => {
    const builder = read('dev-tools/build_eppp_1500_expansion.cjs');
    const i31 = builder.indexOf('repair_eppp_native_quality_wave_31.cjs');
    const i32 = builder.indexOf('repair_eppp_native_quality_wave_32.cjs');
    expect(i32).toBeGreaterThan(i31);
  });
});
