...
const data = createRequire(import.meta.url)('../dev-tools/eppp_application_rewrite_wave_52_data.cjs');
const readJson = (filePath) => JSON.parse(fs.readFileSync(filePath, 'utf8'));
const words = (text) => String(text || '').trim().split(/\s+/).filter(Boolean).length;

describe('EPPP applied rewrite wave 52', () => {
  ...
  it('separates the three duplicate concepts with applied rewrites', () => {
    const bank = readJson(sourceBankPath);
    const byId = new Map(bank.map((item) => [item.id, item]));
    expect(data.duplicateRepairs).toHaveLength(3);
    for (const repair of data.duplicateRepairs) {
      const item = byId.get(repair.id);
      expect(item).toBeTruthy();
      expect(item.answerIndex).toBe(repair.answerIndex);
      expect(item.prompt).toBe(repair.prompt);
      expect(item.choices).toEqual(repair.choices);
      expect(item.rationale).toBe(repair.rationale);
      const expectedFeedback = repair.choiceRationales.slice();
      expectedFeedback[repair.answerIndex] = repair.rationale;
      expect(item.choiceRationales).toEqual(expectedFeedback);
      expect(item.duplicatePairCleanupWave).toBe(data.reviewWave);
      expect(item.difficulty).toBe('intermediate');
      expect(item.cognitiveProcess).toBe('application');
    }
  });
  ...
  it('keeps source/deploy banks and wave audit synchronized', () => {
    const source = fs.readFileSync(sourceBankPath, 'utf8');
    expect(fs.readFileSync(deployBankPath, 'utf8')).toBe(source);
    const audit = readJson(path.join(root, 'test_prep/eppp_application_rewrite_audit_wave_52.json'));
    expect(audit.summary).toMatchObject({
      totalItems: 1500,
      rewrittenItems: 24,
      duplicatePairRepairs: 3,
