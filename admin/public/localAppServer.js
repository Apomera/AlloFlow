...
// Real LM Studio/local engine URL the server forwards to. Updated from
// local_config.json by buildLocalConfigScript(); the client always talks
...
function buildLocalConfigScript() {
    ...
    // Redirect the client's llmEngineUrl to this server for ALL providers.
    // Cloud providers need the token-holding proxy; LM Studio/local engines
    // need it because LM Studio does not send CORS headers, so direct
    // browser fetches to localhost:1234 are blocked. The server forwards
    // local requests to the real engine URL (kept in _lmStudioUpstream).
    const CLOUD_PROVIDERS = new Set(['gemini', 'copilot', 'openai', 'nvidia']);
    _lmStudioUpstream = (cfg.llmEngineUrl && !CLOUD_PROVIDERS.has(cfg.aiProvider))
        ? cfg.llmEngineUrl.replace(/\/$/, '')
        : _lmStudioUpstream;
...
async function handleV1Models(req, res) {
    ...
    if (provider === 'gemini') {
        ...
        try {
            const upstream = await fetch(`${_lmStudioUpstream}/v1/models`);
            const data = await upstream.json();
            // Enrich with LM Studio's native model API (loaded state + type) so
            // the client can prefer a model that is actually loaded instead of
            // triggering JIT-loading (or re-crashing) an unloaded one.
            try {
                const v0 = await fetch(`${_lmStudioUpstream}/api/v0/models`);
                if (v0.ok) {
                    const v0Data = await v0.json();
                    const meta = new Map((v0Data.data || []).map(m => [m.id, m]));
                    for (const m of (data.data || [])) {
                        const info = meta.get(m.id);
                        if (info) {
                            m.state = info.state;
                            m.type = info.type;
                            m.loaded_context_length = info.loaded_context_length;
                        }
                    }
                }
            } catch (_) { /* non-LM Studio engines won't have /api/v0 — fine */ }
...
function startLocalAppServer(port, isPackaged) {
    ...
    return new Promise((resolve, reject) => {
        _server = http.createServer((req, res) => {
            ...
            // ── Teacher account sync (feature/teacher-accounts) ───────────────
            // POST /api/teachers/sync — SQLite backend pushes teacher data after saves
            if (urlPath === '/api/teachers/sync' && req.method === 'POST') {
                let body = '';
                req.on('data', chunk => { body += chunk.toString(); });
                req.on('end', () => {
                    try {
                        ...
                        if (!teacherId) {
                            res.writeHead(400, { 'Content-Type': 'application/json' });
                            return res.end(JSON.stringify({ error: 'teacherId is required' }));
                        }
                        teachers[teacherId] = { ...teachers[teacherId], ...payload, syncedAt: new Date().toISOString() };
                        fs.writeFileSync(teachersFile, JSON.stringify(teachers, null, 2), 'utf-8');
                        res.writeHead(200, { 'Content-Type': 'application/json' });
            ...
            if (urlPath.startsWith('/api/teachers/') && req.method === 'GET') {
                ...
                if (!teachers[teacherId]) {
                    ...
                    return res.end(JSON.stringify({ error: 'Teacher not found' }));
                }
                res.writeHead(200, { 'Content-Type': 'application/json' });
                return res.end(JSON.stringify(teachers[teacherId]));
            }
