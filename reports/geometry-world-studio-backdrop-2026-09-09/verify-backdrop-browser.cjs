...
let body=(async function backdropReview(){
  ...
  async function measure(){return page.evaluate(()=>{
    ...
    return {composed,quality:e._renderProfile.tier,sky,floor,maxStep,line,draw,background:e.scene.background.toArray(),fog:e.scene.fog.color.toArray(),floorColor:e._showcase.studio.floor.material.color.toArray(),camera:e.camera.position.toArray(),quaternion:e.camera.quaternion.toArray(),rect:e._showcase.composition.rect,blocks:Object.keys(e.blocks).length,resources:e._showcase.studio.resources.length,materialsUnchanged:Object.entries(window.__materialRefs).every(([k,m])=>e.blocks[k]?.material===m),shaderErrors};
  });}
  try{
    ...
    const cases=[{q:'saver',v:'front',w:844,h:390},{q:'saver',v:'side',w:844,h:390},{q:'saver',v:'perspective',w:1200,h:820},{q:'saver',v:'perspective',w:390,h:844},{q:'balanced',v:'front',w:844,h:390},{q:'detail',v:'front',w:844,h:390},{q:'detail',v:'side',w:844,h:390},{q:'detail',v:'perspective',w:1200,h:820}];
    for(const c of cases){
      ...
      if(c.v==='front'&&['saver','detail'].includes(c.q)){
        const savedPose=await pose(),pending=page.waitForEvent('download');await page.getByRole('button',{name:'Save image',exact:true}).click();const d=await pending,file=stage+'-'+c.q+'-export.png';await d.saveAs(path.join(out,file));await page.waitForFunction(()=>!__geoWorldEngine._showcaseExporting);await frames();
