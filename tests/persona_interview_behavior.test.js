...
describe('Persona persistence contracts', () => {
  it('uses project-safe resource types and scoped, size-capped resume snapshots', () => {
    ...
    const handlers = fs.readFileSync('host_handlers_source.jsx', 'utf8').replace(/\b__d\./g, '');
    const editorSaveStart = handlers.indexOf('const savePersonaTeacherEditor =');
    const editorSaveEnd = handlers.indexOf('const _alloAlignmentGraphExportForContext =', editorSaveStart);
    expect(editorSaveStart).toBeGreaterThanOrEqual(0);
    expect(editorSaveEnd).toBeGreaterThan(editorSaveStart);
    const editorSaveContract = handlers.slice(editorSaveStart, editorSaveEnd);
    expect(editorContract).toContain('_alloHostHandlers().savePersonaTeacherEditor(...__a)');
    ...
    expect(editorSaveContract).toContain("guardrailsSource: 'teacher'");
    expect(editorSaveContract).toContain('Math.max(0, Math.min(100, Math.round(parsedDifficulty)))');
    expect(editorSaveContract).toContain('isCompleted: existing.isCompleted === true');
    expect(editorSaveContract).toContain('setGeneratedContent(nextResource)');
    expect(editorSaveContract).toContain('setHistory(prev => prev.map(item => item.id === generatedContent.id ? nextResource : item))');
