// Lazy Lucide icon resolution from window.AlloIcons (populated by
  ...
  function GlossaryView(props) {
    ...
    async function handlePrepareGlossaryAudio() {
      ...
      try {
        var result = await prepare({ includeTerms: true, includeDefinitions: false, languages: [] }, updateGlossaryAudioProgress, { source: 'glossary-teacher-tools' });
    ...
    return <><div className="space-y-6">{renderGlossaryToolbar()}{isInteractiveFlashcards && Array.isArray(generatedContent?.data) && generatedContent.data.length > 0 && <div ref={flashcardDialogRef} role="dialog" aria-modal="true" aria-label={t('flashcards.deck_standard') || 'Interactive flashcards'} tabIndex={-1} onKeyDown={e => containModalFocus(e, flashcardDialogRef.current, closeInteractiveFlashcards)} className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-start pt-20 sm:pt-24 p-4 animate-in fade-in duration-300 motion-reduce:animate-none motion-reduce:transition-none overflow-auto"><button ref={flashcardCloseRef} type="button" onClick={closeInteractiveFlashcards} className="absolute top-6 right-6 text-white/70 hover:text-white bg-white/10 hover:bg-white/20 p-2 rounded-full transition-colors z-50  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950" aria-label={t('common.close')}><X size={32} aria-hidden="true" /></button><div id="flashcard-live-status" className="sr-only" aria-live="polite" aria-atomic="true">{flashcardStatusText}</div><div className="absolute top-6 left-6 z-50 hidden sm:flex flex-col gap-2"><button type="button" aria-pressed={showFlashcardImages} onClick={handleToggleShowFlashcardImages} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-bold transition-colors shadow-lg border  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${showFlashcardImages ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-slate-800 text-slate-300 border-slate-700 hover:text-white'}`} title={t('flashcards.tooltip_toggle_images')} aria-label={showFlashcardImages ? t('flashcards.hide_images') : t('flashcards.show_images')}><ImageIcon size={16} aria-hidden="true" /> {showFlashcardImages ? t('flashcards.hide_images') : t('flashcards.show_images')}</button><button type="button" aria-pressed={isFlashcardQuizMode} onClick={handleToggleFlashcardQuizMode} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-bold transition-colors shadow-lg border  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${isFlashcardQuizMode ? 'bg-yellow-500 text-indigo-900 border-yellow-400' : 'bg-slate-800 text-slate-300 border-slate-700 hover:text-white'}`} title={t('flashcards.tooltip_toggle_quiz')} aria-label={isFlashcardQuizMode ? "Disable Quiz Mode" : "Enable Quiz Mode"}>{isFlashcardQuizMode ? <CheckCircle2 size={16} aria-hidden="true" /> : <Brain size={16} aria-hidden="true" />}{isFlashcardQuizMode ? t('flashcards.quiz_active') : t('flashcards.practice_mode')}</button></div>{flashcardMode === 'standard' && selectedLanguages.length > 0 && <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50"><select aria-label="Standard flashcard language" value={standardDeckLang} onChange={e => setStandardDeckLang(e.target.value)} className="bg-slate-800 text-white border border-slate-600 rounded-lg px-3 py-2 text-sm font-bold focus:ring-2 focus:ring-indigo-500  shadow-lg"><option value="English Only">{t('languages.english_only')}</option>{selectedLanguages.map(l => <option key={l} value={l}>+ {l}</option>)}</select></div>}{flashcardMode === 'language' && selectedLanguages.length > 1 && <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50"><select aria-label="Language flashcard deck" value={flashcardLang} onChange={e => setFlashcardLang(e.target.value)} className="bg-slate-800 text-white border border-slate-600 rounded-lg px-3 py-2 text-sm font-bold focus:ring-2 focus:ring-indigo-500 ">{selectedLanguages.map(l => <option key={l} value={l}>{l}</option>)}</select></div>}<div className="w-full max-w-4xl perspective-1000"><div className="mb-6 px-2"><div className="flex items-center justify-between text-white/80 mb-2"><span className="font-bold text-lg flex items-center gap-2">{flashcardMode === 'standard' ? t('flashcards.deck_standard') : t('flashcards.deck_language', {
              ...
              return <div className="text-center p-4 rounded-xl border-2 mb-6" style={{
                ...
                borderColor: risk.border
                  ...
                  return <button key={c} type="button" onClick={() => toggleLetterSelection(r, c)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleLetterSelection(r, c); } }} aria-pressed={selectedLetters.has(`${r}-${c}`)} aria-label={`${t('glossary.word_search_cell_aria') || 'Cell'} ${r + 1}-${c + 1}: ${char}`} className={`w-8 h-8 sm:w-9 sm:h-9 border border-slate-400 flex items-center justify-center font-mono text-sm sm:text-base font-bold cursor-pointer select-none transition-colors grid-cell  focus:ring-2 focus:ring-teal-500 focus:z-10 ${highlightClass}`}>{char}</button>;
                })}</div>)}</div><div className="mt-6 w-full max-w-md"><h4 className="text-sm font-bold text-slate-600 uppercase tracking-wider mb-2 text-center">{t('glossary.word_search_find')}</h4><div className="flex flex-wrap justify-center gap-x-6 gap-y-2 word-list">{gameData.words.map((word, i) => {
    ...
    ><X size={16} aria-hidden="true" /></button>
                  ...
                  return <tr key={getGlossaryEntryKey(item, idx)} className="hover:bg-slate-50 focus-within:bg-slate-50 group/row">{isEditingGlossary && <td className="p-4 border-b border-slate-100 text-center align-top"><input aria-label={t('common.toggle_is_selected_false')} type="checkbox" className="mt-1.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4 cursor-pointer" checked={item.isSelected !== false} onChange={() => handleGlossarySelectionChange(idx)} title={t('glossary.tooltips.select_highlight')} /></td>}<td className="p-4 font-bold text-slate-800 align-top min-w-[200px]"><div className="flex flex-col gap-2 items-center">{!isEditingGlossary && item.tier && <span className={`text-[11px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-full border mb-1 ${isTier2 ? 'bg-blue-50 text-blue-700 border-blue-200' : isTier3 ? 'bg-purple-50 text-purple-700 border-purple-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>{isTier2 ? 'Academic vocabulary' : isTier3 ? 'Subject vocabulary' : item.tier}</span>}{isEditingGlossary ? <div className="w-full flex flex-col gap-2"><select aria-label={(t('glossary.edit_tier_placeholder') || 'Vocabulary type') + ': ' + item.term} value={item.tier || ''} onChange={e => handleGlossaryChange(idx, 'tier', e.target.value)} className="text-[11px] font-bold uppercase tracking-wider w-full border border-slate-400 rounded px-1 py-1  focus:ring-1 focus:ring-indigo-500 bg-slate-50 text-slate-600" data-help-key="glossary_edit_tier"><option value="">{t('glossary.edit_tier_placeholder')}</option><option value="Academic">{t('glossary.edit_tier_academic')}</option><option value="Domain-Specific">{t('glossary.edit_tier_domain')}</option></select><div className="flex items-start gap-2 w-full"><textarea aria-label={t('glossary.edit_term') || 'Edit glossary term'} value={item.term} onChange={e => handleGlossaryChange(idx, 'term', e.target.value)} rows={getRows(item.term, 25)} className="w-full bg-white border border-blue-300 rounded px-2 py-1  focus:ring-2 focus:ring-blue-200 resize-y text-sm font-bold text-center" /><button type="button" aria-label={'Delete term: ' + item.term} onClick={() => handleDeleteGlossaryItem(idx)} className="min-h-11 min-w-11 inline-flex items-center justify-center bg-red-50 text-red-600 hover:bg-red-100 rounded transition-colors shrink-0" title={t('glossary.tooltips.delete_term')}><Trash2 size={14} /></button></div></div> : <div className="px-2 py-1 font-bold text-slate-800 whitespace-pre-wrap text-center">{item.term}</div>}<div className="mt-1 px-1">{item.image ? <div className="flex flex-col gap-2"><div className="relative group/image inline-block w-fit"><img loading="lazy" src={item.image} alt={`${item.term} icon`} style={{
                          ...
                          handlePhonicsClick && <button onClick={ev => handlePhonicsClick(item.term, ev)} className="text-slate-600 hover:text-emerald-700 hover:bg-emerald-50 min-h-11 min-w-11 inline-flex items-center justify-center rounded-full transition-colors flex-shrink-0 font-bold text-[11px] tracking-tight" aria-label={(t('glossary.pronounce_term') || 'Pronounce') + ': ' + item.term} title={t('glossary.pronounce_term_title') || 'Show pronunciation, IPA, and syllables'} data-help-key="glossary_pronounce_term">/əˈ/</button>}<button type="button" aria-label={'Download audio for term: ' + item.term} onClick={() => handleDownloadAudio(item.term, `term-${idx}-audio`, `dl-term-${idx}`)} disabled={downloadingContentId === `dl-term-${idx}`} className="min-h-11 min-w-11 inline-flex items-center justify-center text-slate-600 hover:text-indigo-600 rounded-full transition-colors" data-help-key="glossary_download_audio">{downloadingContentId === `dl-term-${idx}` ? <RefreshCw size={14} className="animate-spin motion-reduce:animate-none" /> : <Download size={14} />}</button></div></div></td><td className="p-4 text-slate-600 align-top"><div className="flex flex-col gap-2 items-center">{isEditingGlossary ? <textarea aria-label={t('glossary.edit_definition') || 'Edit definition'} value={item.def} onChange={e => handleGlossaryChange(idx, 'def', e.target.value)} rows={getRows(item.def, 45)} className="w-full bg-white border border-blue-300 rounded px-2 py-1  focus:ring-2 focus:ring-blue-200 resize-y text-sm text-center" /> : <div className="px-2 py-1 text-slate-600 whitespace-pre-wrap">{currentUiLanguage !== 'English' && <span className="block font-bold text-indigo-900 text-xs mb-1 uppercase tracking-wide">{item.term}</span>}{item.def}</div>}<div className="flex items-center gap-1 "><button type="button" aria-label={'Read definition for ' + item.term} onClick={() => handleSpeak(item.def, `def-${idx}`)} disabled={isGeneratingAudio && playingContentId !== `def-${idx}`} className={`min-h-11 min-w-11 inline-flex items-center justify-center rounded-full transition-colors flex-shrink-0 ${playingContentId === `def-${idx}` ? 'text-red-700 bg-red-50' : 'text-slate-600 hover:text-indigo-600 hover:bg-indigo-50'}`}>{playingContentId === `def-${idx}` && isGeneratingAudio ? <RefreshCw size={14} className="animate-spin motion-reduce:animate-none" /> : playingContentId === `def-${idx}` ? <StopCircle size={14} /> : <Volume2 size={14} />}</button><button type="button" aria-label={'Download definition audio for ' + item.term} onClick={() => handleDownloadAudio(item.def, `def-${idx}-audio`, `dl-def-${idx}`)} disabled={downloadingContentId === `dl-def-${idx}`} className="min-h-11 min-w-11 inline-flex items-center justify-center text-slate-600 hover:text-indigo-600 rounded-full transition-colors">{downloadingContentId === `dl-def-${idx}` ? <RefreshCw size={14} className="animate-spin motion-reduce:animate-none" /> : <Download size={14} />}</button></div>{item.etymology ? <details className="mt-2 text-xs w-full" data-help-key="glossary_etymology_info"><summary className="cursor-pointer text-indigo-700 font-medium hover:text-indigo-900 select-none">📜 {t('glossary.etymology_label') || 'Word roots'}</summary>{(() => {
                            ...
                            return <div className="mt-1 pl-4 border-l-2 border-indigo-200 space-y-2">{etyMap && Object.keys(etyMap).length > 1 && <div className="text-[10px] uppercase tracking-wide text-indigo-500 font-bold">{displayLang}</div>}<div className="flex items-start gap-1"><p className="text-slate-700 leading-relaxed italic flex-1">{etymologyProse}</p><button onClick={() => handleSpeak(etymologyProse, `etym-${idx}`)} disabled={isGeneratingAudio && playingContentId !== `etym-${idx}`} className={`min-h-11 min-w-11 inline-flex items-center justify-center rounded-full transition-colors flex-shrink-0 ${playingContentId === `etym-${idx}` ? 'text-red-700 bg-red-50' : 'text-slate-600 hover:text-indigo-600 hover:bg-indigo-50'}`} aria-label={t('glossary.etymology_label') || 'Word roots'}>{playingContentId === `etym-${idx}` && isGeneratingAudio ? <RefreshCw size={12} className="animate-spin motion-reduce:animate-none" /> : <Volume2 size={12} />}</button></div>{(() => {
