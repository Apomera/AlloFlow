...
const outputPath = path.join(authoredDir, 'school_psychologist_5403_batch3.json');
const reviewedAt = '2026-07-25';

const specs = [
  ...require('./authored/school_psychologist_5403_batch3_permeating.cjs'),
  ...require('./authored/school_psychologist_5403_batch3_student.cjs'),
  ...require('./authored/school_psychologist_5403_batch3_systems.cjs'),
  ...require('./authored/school_psychologist_5403_batch3_foundations.cjs'),
];
...
function sameCounts(actual, expected) {
  const stable = (value) => Object.fromEntries(
...
function feedbackForDistractor(distractor) {
  return `Not the best answer. ${distractor.reason} Recheck the problem definition, available evidence, student and family access, professional role, and safest appropriate next step.`;
}
...
function makeItem(bank, spec, position, catalog) {
  ...
  return {
    id: `sp5403-b3-${String(position).padStart(3, '0')}`,
    type: 'single-choice',
...
function build() {
  ...
  for (const spec of specs) {
    ...
    if (String(spec.prompt || '').trim().length < 70) {
      errors.push(`${spec.skillId}: prompt shorter than 70 characters`);
    }
    if (String(spec.rationale || '').trim().length < 150) {
      errors.push(`${spec.skillId}: rationale shorter than 150 characters`);
    }
    if (!Array.isArray(spec.distractors) || spec.distractors.length !== 3) {
      ...
      for (const distractor of spec.distractors) {
        if (String(distractor?.text || '').trim().length < 20
            || String(distractor?.reason || '').trim().length < 60) {
          errors.push(`${spec.skillId}: distractor text or reason is not substantive`);
        }
      }
    }
    if (!String(spec.correct || '').trim()
        || !String(spec.difficulty || '').trim()
  ...
  for (const bank of skillBanks) {
    ...
    if (!releasedSkill || !releasedChapter
        || releasedSkill.domainId !== bank.domainId
        || releasedSkill.chapterId !== bank.chapterId
        || releasedChapter.skillId !== bank.id
  ...
  const answerCounts = [0, 1, 2, 3]
    .map((index) => items.filter((item) => item.answerIndex === index).length);
  if (answerCounts.some((count) => count !== 25)) {
    errors.push(`answer balance ${answerCounts.join('/')}`);
  }
  ...
  for (const item of items) {
    ...
    if (itemPromptKeys.has(promptKey) || priorPromptKeys.has(promptKey)) {
      errors.push(`${item.id}: prompt collision`);
    }
    if (itemResponseKernels.has(responseKey) || priorResponseKernels.has(responseKey)) {
      errors.push(`${item.id}: response collision`);
    }
    if (itemContentKernels.has(contentKey) || priorContentKernels.has(contentKey)) {
      errors.push(`${item.id}: content collision`);
    }
    itemPromptKeys.add(promptKey);
    itemResponseKernels.add(responseKey);
    itemContentKernels.add(contentKey);

    if (item.choices.length !== 4 || new Set(item.choices.map(canonical)).size !== 4) {
      errors.push(`${item.id}: invalid choices`);
    }
    if (item.choiceRationales.length !== 4
        || item.choiceRationales.some((feedback) => feedback.length < 100)) {
      errors.push(`${item.id}: insufficient four-choice feedback`);
    }
    if (item.references.length !== item.referenceNames.length
        || item.referenceNames.some((name) => !name)) {
      errors.push(`${item.id}: invalid named references`);
    }

    const lengths = item.choices.map((choice) => canonical(choice).length);
    if (lengths[item.answerIndex]
        >= Math.max(...lengths.filter((_, index) => index !== item.answerIndex))) {
      keyedLongestOrTiedCount += 1;
    }
    const actionableWarnings = warningCodes(item)
      .filter((code) => code !== 'key-stem-lexical-leakage');
    if (actionableWarnings.length) {
      warningItems.push({ id: item.id, codes: actionableWarnings });
    }
  ...
  if (maximumPromptSimilarity > 0.82) {
    errors.push(`prompt similarity exceeds 0.82: ${JSON.stringify(closestPair)}`);
  }
  if (keyedLongestOrTiedCount > 35) {
    errors.push(`keyed answer is longest or tied in ${keyedLongestOrTiedCount} items (maximum 35)`);
  }
  if (warningItems.length) {
    errors.push(`shared warning findings: ${JSON.stringify(warningItems.slice(0, 24))}`);
  }
  ...
  if (process.argv.includes('--check')) {
    console.log(
      `Validated School Psychologist 5403 authored Batch 3: ${items.length} items; `
      + `domains 32/23/20/25; keys 25/25/25/25; keyed-longest `
      + `${keyedLongestOrTiedCount}; max source-prompt Jaccard `
      + `${maximumPromptSimilarity.toFixed(4)}.`,
    );
    return;
  }
  writeGeneratedFile(outputPath, output);
  console.log(
    `Built School Psychologist 5403 authored Batch 3: ${items.length} items; `
    + `domains 32/23/20/25; keys 25/25/25/25; keyed-longest `
    + `${keyedLongestOrTiedCount}; max source-prompt Jaccard `
    + `${maximumPromptSimilarity.toFixed(4)}.`,
  );
