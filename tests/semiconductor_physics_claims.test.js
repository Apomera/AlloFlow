...
describe("Moore's Law products have documented counts and scope", () => {
  const milestone = year => window.__SemiconductorCore.milestones.find(p => p.year === year);
  it('has the documented transistor counts', () => {
    expect(milestone(1971).transistors).toBe(2300);
    expect(milestone(2020).transistors).toBe(16e9);
    expect(milestone(2021).transistors).toBe(57e9);
    expect(milestone(2022).transistors).toBe(114e9);
    expect(milestone(2024).transistors).toBe(208e9);
  });
  it('names the products and identifies multi-die totals', () => {
    expect(milestone(2020).name).toBe('Apple M1');
    expect(milestone(2022).name).toBe('Apple M1 Ultra');
    expect(milestone(2022).dies).toBe(2);
    expect(milestone(2024).name).toBe('NVIDIA B200');
    expect(milestone(2024).dies).toBe(2);
    expect(milestone(2024).node).toBe('TSMC 4NP');
  });
  it('is not overridden by stale invented product names', () => {
  ...
  it('uses individually sourced observations instead of forcing a fit to a curve', () => {
    const rows = window.__SemiconductorCore.milestones;
    expect(rows).toHaveLength(8);
    expect(new Set(rows.map(p=>p.year)).size).toBe(rows.length);
    for(const p of rows) {
      expect(p.transistors).toBeGreaterThan(0);
      expect(p.source).toMatch(/^https:\/\/(www\.intel\.com|www\.apple\.com|nvidianews\.nvidia\.com)\//);
      expect([1,2]).toContain(p.dies);
    }
  });
});
