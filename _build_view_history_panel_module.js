...
const SOURCE = path.join(ROOT, 'view_history_panel_source.jsx');
const OUTPUT = path.join(ROOT, 'view_history_panel_module.js');
const TMP = path.join(ROOT, '_tmp_history_panel_entry.jsx');
if (!fs.existsSync(SOURCE)) { console.error('Source not found:', SOURCE); process.exit(1); }
const source = fs.readFileSync(SOURCE, 'utf-8');
const entry = `/* global React */\n\n${source}\n\nwindow.__historyPanelExports = { HistoryPanel };\n`;
fs.writeFileSync(TMP, entry, 'utf-8');
console.log('Compiling view_history_panel_source.jsx...');
try {
...
const compiled = fs.readFileSync(TMP + '.compiled.js', 'utf-8')
    .replace(/\/\*.*global.*\*\/\n/g, '')
    .replace(/window\.__historyPanelExports\s*=\s*\{[^}]+\};?\s*/, '')
    .trim();
fs.unlinkSync(TMP);
