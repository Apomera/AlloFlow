  ...
  window.StemLab.registerTool('geoSandbox', {
    ...
    render: function(ctx) {
      ...
      function renderStretchNotebookSearch(){
        ...
        return h('section',{className:'geo-notebook-search','aria-label':'Find saved investigations'},
      ...
      function renderGuidedExperiments(){
        ...
        return h('section',{className:'geo-section geo-guided-experiments','aria-label':'Guided experiments'},h('h3',null,'Guided experiments'),
      ...
      function renderStretchRecord(entry){
        ...
        return h('details',{key:entry.id,className:'geo-record',open:open,onToggle:function(e){if(e.currentTarget.open!==open)upd('stretchReviewEntry',e.currentTarget.open?entry.id:null);}},
          h('summary',null,entry.title),open&&h('div',null,
            ...
            edit?h('section',{'aria-label':'Edit saved reflection'},h('p',null,'Revise your reflection. Your original prediction and saved models stay intact.'),
      ...
      function renderPrismExplainer(selected){
        ...
        return h('section',{'aria-label':'Prism measurement explanations'},h('p',null,'Volume = base area × perpendicular height: '+geoRound(frame.baseArea)+' × '+geoRound(frame.height)+' = '+geoRound(frame.volume)+' '+unitDef.short+'³'),
          h('div',{className:'geo-action-row'},[{id:'base',label:'Base area',value:frame.baseArea,exp:'²'},{id:'height',label:'Perpendicular height',value:frame.height,exp:''},{id:'edge',label:'Side edge',value:frame.edge,exp:''}].map(function(item){return h('button',{key:item.id,type:'button','aria-label':'Highlight '+item.label.toLowerCase(),'aria-pressed':gd.stretchExplain===item.id,onClick:function(){upd('stretchExplain',gd.stretchExplain===item.id?null:item.id);}},item.label+' · '+geoRound(item.value)+' '+unitDef.short+item.exp);})),
      ...
      function renderSculptRepresentationDiagram(shape, rep) {
        ...
        return h('svg', { viewBox: '0 0 152 52', className: 'w-full h-14 text-cyan-200', role: 'img', 'aria-label': geoShapeTitle(shape) + ' representation: net ' + rep.net + ', cross-section ' + rep.crossSection },
      ...
      function renderSculptShapeChooser() {
        ...
        return h('section',{className:'geo-shape-chooser','aria-labelledby':'geo-shape-chooser-title'},
          ...
          h('p',{className:'geo-sculpt-note'},full?sculptLabel('shape_chooser_full','All 14 part slots are used. Open Edit and remove a part to make room.'):gd.sculptKeepAdding?sculptLabel('shape_chooser_repeat','Each shape is selected as you add it. Open Edit when you’re ready to refine.'):sculptLabel('shape_chooser_edit','Choose a shape to add it and open its size controls.')),
