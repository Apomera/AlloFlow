...
const scene = readFileSync(resolve(root, 'life_skills_safety/life_skills_safety.html'), 'utf8');
...
describe('Life Skills 3D home safety walkthrough', () => {
  it('defines six curated targets and three practice modes', () => {
    const literal = scene.match(/var TASKS = (\[[\s\S]*?\]);/);
    expect(literal).not.toBeNull();
    const tasks = Function('return ' + literal[1])();
    expect(tasks.map((task) => task.id)).toEqual(['alarm', 'exit', 'cleaners', 'stove', 'shutoff', 'meeting']);
    expect(scene).toContain('value="guided"');
    expect(scene).toContain('value="practice"');
    expect(scene).toContain('value="challenge"');
  });

  it('keeps the 3D view optional and provides a keyboard checklist fallback', () => {
    expect(scene).toContain('../immersive_geometry/vendor/aframe.min.js');
    expect(scene).toContain('safety-interactive');
    expect(scene).toContain('raycaster="objects: .clickable');
    expect(scene).toContain('Accessible safety checklist');
    ...
    expect(scene).toContain('alloflow-life-safety-3d-why-reveal');
    expect(scene).toContain('function pauseForHelp()');
    expect(scene).toContain('alloflow-life-safety-3d-help-pause');
    ...
    expect(scene).toContain('alloflow-life-safety-3d-confidence');
    ...
    expect(scene).toContain('alloflow-life-safety-3d-replay-complete');
    expect(scene).toContain('data-task-id');
    expect(scene).toContain('alloflow-life-safety-3d-next-step');
    expect(scene).toContain('alloflow-life-safety-3d-reflection');
    expect(scene).toContain("button.type = 'button'");
    expect(scene).toContain('prefers-reduced-motion');
  });

  it('keeps scene object IDs unique', () => {
    const ids = [...scene.matchAll(/\sid="([^"]+)"/g)].map((match) => match[1]);
    const duplicates = ids.filter((id, index) => ids.indexOf(id) !== index);
    expect(duplicates).toEqual([]);
  });

  it('launches the walkthrough from the existing Home Safety Lab', () => {
    expect(lifeSkills).toContain('function openLifeSkillsSafety3D()');
    expect(lifeSkills).toContain('/life_skills_safety/life_skills_safety.html');
    expect(lifeSkills).toContain('Open the 3D home safety walkthrough in a new window');
    expect(lifeSkills).toContain('🌐 Open 3D safety walkthrough ↗');
  });

  it('copies both the walkthrough and its local A-Frame runtime into builds', () => {
    expect(build).toContain("'immersive_geometry'");
    expect(build).toContain("'life_skills_safety'");
  });
});
