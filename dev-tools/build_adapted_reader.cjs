...
const styles=require(path.join(root,'_build_app_styles_module.js')).buildAppStylesModule(fs.readFileSync(path.join(root,'app_styles_source.jsx'),'utf8'));
for(const file of ['app_styles_module.js','desktop/web-app/public/app_styles_module.js']){
 const target=path.join(root,file);
 if(fs.existsSync(target)&&fs.readFileSync(target,'utf8')===styles)continue;
 const temp=target+'.reader-build-'+process.pid+'.tmp';
 try{fs.writeFileSync(temp,styles);fs.renameSync(temp,target);}finally{if(fs.existsSync(temp))fs.unlinkSync(temp);}
}
const modules=['view_simplified_module.js','pure_helpers_module.js','content_engine_module.js','app_styles_module.js'];
...
for(const f of ['AlloFlowANTI.txt','desktop/web-app/src/AlloFlowANTI.txt','desktop/web-app/src/App.jsx']){
 ...
 const after=before.replace(/https:\/\/alloflow-cdn\.pages\.dev\/(view_simplified_module\.js|pure_helpers_module\.js|content_engine_module\.js|app_styles_module\.js)(?:\?v=[^'"\s)]+)?/g,(_,name)=>'https://alloflow-cdn.pages.dev/'+name+'?v='+pins[name]);
