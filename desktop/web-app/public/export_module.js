...
const createExport = deps => {
  ...
  const getDefaultTitle = type => {
    ...
    switch (type) {
      ...
      case 'note-taking':
        return t('help_mode.tool_note_taking') || 'Note-Taking Templates';
      case 'anchor-chart':
        return t('help_mode.tool_anchor_chart') || 'Anchor Chart';
      case 'memory-aid':
        return t('help_mode.tool_memory_aid') || 'Memory Aid Studio';
      case 'applied-challenge':
        return t('help_mode.tool_applied_challenge') || 'Applied Challenge Studio';
      default:
  ...
  const _instructionalTextForExport = item => {
    ...
    try {
      ...
      if (api && typeof api.getInstructionalText === 'function') normalized = api.getInstructionalText(artifact);else if (raw && api && typeof api.normalizeInstructionalText === 'function') normalized = api.normalizeInstructionalText(raw, {
        defaultForm: inferredForm
      });
    } catch (_) {
      normalized = raw;
    }
    ...
    const auth = normalized.replacementAuthorization && typeof normalized.replacementAuthorization === 'object' ? normalized.replacementAuthorization : {};
    ...
    return {
      ...
      designationSource: ['educator', 'workflow-default', 'legacy-inferred'].includes(normalized.designationSource) ? normalized.designationSource : 'legacy-inferred',
      ...
      complexity: {
        ...
        measuredGrade: complexity.measuredGrade != null ? complexity.measuredGrade : artifact.localStats && artifact.localStats.gradeLevel != null ? artifact.localStats.gradeLevel : null,
  ...
  const _appBuild = () => {
    try {
      const m = Array.from(document.querySelectorAll('script[src]')).map(s => /Apomera\/AlloFlow@([0-9a-f]+)/.exec(s.src)).find(Boolean);
  ...
  const buildProvenanceRecord = item => {
    ...
    const modelCalibrationTarget = rawInstructionalText && rawInstructionalText.complexity ? rawInstructionalText.complexity.calibrationTarget || '' : '';
    return {
      id: item && item.id,
      type: item && item.type,
      ...
      instructionalText,
      sourceInstructionalText: item?.sourceInstructionalText && window.AlloModules?.InstructionalContext?.normalizeSourceInstructionalText ? window.AlloModules.InstructionalContext.normalizeSourceInstructionalText(item.sourceInstructionalText) : null,
  ...
  const _sequenceMisplacedPositions = (order, intentionallyWrongIndex, count) => {
    const valid = count > 0 && Array.isArray(order) && order.length === count && order.every(value => Number.isInteger(value) && value >= 0 && value < count) && new Set(order).size === count;
  ...
  const handleExportSlides = async (options = {}) => {
    ...
    if (Object.prototype.hasOwnProperty.call(options, 'liveHtml')) {
      try {
        ...
        const result = await api.build({
          html: options.liveHtml,
          title: options.liveTitle || sourceTopic,
          format: 'pptx'
        });
    ...
    try {
      ...
      history.forEach(item => {
        ...
        const itemTitle = (item.title || (originalReading ? 'Original with supports' : getDefaultTitle(type))) + (readingRoleLabel ? ' · ' + readingRoleLabel : '');
        if (originalReading) {
          ...
          const _activityLabels = () => {
            const out = {};
            [['protocol', 'brainstorm.protocol_label'], ['depth_literal', 'brainstorm.depth_literal'], ['depth_inferential', 'brainstorm.depth_inferential'], ['depth_evaluative', 'brainstorm.depth_evaluative'], ['talk_stems', 'brainstorm.talk_stems'], ['stems_agree', 'brainstorm.stems_agree'], ['stems_disagree', 'brainstorm.stems_disagree'], ['stems_clarify', 'brainstorm.stems_clarify'], ['stems_build', 'brainstorm.stems_build'], ['facilitation_notes', 'brainstorm.facilitation_notes'], ['look_fors', 'brainstorm.look_fors'], ['expert_group', 'brainstorm.expert_group'], ['teach_back_points', 'brainstorm.teach_back_points'], ['teach_back_questions', 'brainstorm.teach_back_questions'], ['home_group_task', 'brainstorm.home_group_task'], ['synthesis_organizer', 'brainstorm.synthesis_organizer'], ['accountability_check', 'brainstorm.accountability_check'], ['answer_key', 'brainstorm.answer_key']].forEach(([slot, key]) => {
              const v = t(key);
              if (v) out[slot] = v;
            });
            return out;
          };
          const text = (type === 'brainstorm' && Array.isArray(item.data) && _describeActivity ? item.data.map(entry => entry && (entry.kind === 'discussion' || entry.kind === 'jigsaw') ? _describeActivity(entry, _activityLabels()) : resourceText(entry)).filter(Boolean).join('\n\n') : resourceText(item.data)).replace(/\n{3,}/g, '\n\n').trim();
          ...
          cards.forEach((card, ci) => {
            ...
            if (cueBlock && cueBlock.visualDescription) runs.push({
              text: _nsT('memory_aid.export_visual_cue_described', 'Picture cue, described:') + ' ' + clip(cueBlock.visualDescription, 700),
              options: {
                fontSize: 13,
                color: "0F766E",
                breakLine: true,
            ...
            if (hook) {
              runs.push({
                text: _nsT('memory_aid.hook_heading', 'Did you know?') + ' ' + clip(hook.text, 400),
                options: {
                  fontSize: 12,
                  color: "9A3412",
                  breakLine: true,
              ...
              if (hook.webVerified && hook.sourceUrl) {
                ...
                runs.push({
                  text: _nsT('memory_aid.hook_from_web_note', 'From the web. Check the source:') + ' ' + clip(hook.sourceTitle || hook.sourceUrl, 200) + hookHost,
                  options: {
                    fontSize: 10,
                    color: "78716C",
                    breakLine: true,
                    ...
                    hyperlink: {
                      url: hook.sourceUrl,
                      tooltip: clip(hook.sourceTitle || hook.sourceUrl, 200)
                    }
                  }
                });
              } else {
                runs.push({
                  text: _nsT('memory_aid.hook_unsourced_note', 'Fun fact from AI knowledge. Ask your teacher if you want to check it.'),
                  options: {
                    fontSize: 10,
                    color: "78716C",
                    breakLine: true,
            ...
            if (c.mapping) runs.push({
              text: clip(c.mapping, 500),
              options: {
                fontSize: 12,
                italic: true,
                color: "475569",
            ...
            const cueNote = cue || (cueBlock && cueBlock.steps.length ? cueBlock.steps.join(' ') : '') || cueBlock && cueBlock.visualDescription || (cueBlock && cueBlock.prompts.length ? cueBlock.prompts.join(' ') : '');
          ...
          const header = [{
            text: _nsT('export.ppt_cue_col', 'Cue'),
            options: {
              bold: true,
              fill: "F1F5F9",
              color: darkText
            ...
            text: _nsT('export.ppt_notes_col', 'Notes'),
            options: {
              bold: true,
              fill: "F1F5F9",
              color: darkText
          ...
          for (let start = 0; start < Math.max(1, filled.length); start += 6) {
            ...
            if (page.length) slide.addTable([header].concat(page), {
              x: 0.5,
              y: 1.0,
              w: 9,
              colW: [3, 6],
          ...
          if (!context && !question && !facts.length) {
            skipped.push(itemTitle);
            return;
          }
          const runs = [];
          if (question) runs.push({
            text: question,
            options: {
              fontSize: 18,
              bold: true,
          ...
          if (context) {
            runs.push(heading(_nsT('export.ppt_context', 'Context')));
            runs.push(body(context));
          }
          ...
          if (facts.length || openQuestions.length) {
            const factRuns = [];
            if (facts.length) {
              factRuns.push(heading(_nsT('export.ppt_locked_facts', 'Lesson facts to keep accurate')));
              facts.forEach(f => factRuns.push(bullet(f)));
            }
            if (openQuestions.length) {
              factRuns.push(heading(_nsT('export.ppt_open_questions', 'Open questions')));
              openQuestions.forEach(q => factRuns.push(bullet(q)));
            }
          ...
          const lines = typeof item.data === 'string' ? [item.data] : _summarizeResource ? _summarizeResource(item, {
            maxChars: 4000
          }) : [];
      ...
      if (skipped.length) {
        ...
        let message = t('export_status.ppt_skipped', {
