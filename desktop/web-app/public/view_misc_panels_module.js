...
function _createPdfDiffOperationManager(controllerFactory) {
  ...
  const abortOwner = (owner) => {
    try {
      if (owner && owner.controller && typeof owner.controller.abort === "function") owner.controller.abort();
    } catch (_) {
    }
  };
  return {
...
function PdfDiffViewer(props) {
  ...
  const remarkupTokenIsCurrent = (token) => {
    ...
    if (typeof capturePdfHtmlCommitToken === "function") {
      ...
      return !!(current && current.documentEpoch === token.documentEpoch && current.revision === token.revision && current.html === token.html);
    }
    return pdfDocumentEpoch === token.documentEpoch && !!pdfFixResult && pdfFixResult.accessibleHtml === token.html;
  };
  ...
  return ReactDOM.createPortal((() => {
    ...
    const _ocrTagMap = /* @__PURE__ */ new Map();
    ...
    if (_ocrTagMap.size > 0 && _chunks) {
      for (const c of _chunks) {
        ...
        if (!tag) continue;
        if (tag.ocrConfidence === "high") {
          _ocrHighCount++;
          _ocrHighList.push({ id: c.id, value: c.value });
        ...
        try {
          ...
          if (surg && surg.html) {
            ...
            if (surgeryCoverage < 0.95) {
              warnLog2("[Diff] Surgery coverage below threshold:", Math.round(surgeryCoverage * 100) + "%", "\u2014 falling back to Gemini");
              newHtml = null;
              surgeryFailReason = "coverage-low-" + Math.round(surgeryCoverage * 100);
            }
          }
        } catch (surgErr) {
          warnLog2("[Diff] Text surgery threw, falling back to Gemini:", surgErr?.message || surgErr);
...
${_effectiveText}`;
          ...
          try {
            ...
            if (!remarkupOperationIsCurrent(_remarkupOwner)) return;
            warnLog2("[Diff] Gemini fallback remarkup failed:", gErr?.message || gErr);
          }
          if (remarkedHtml) {
            ...
            if (cov >= 0.9) {
              ...
              warnLog2("[Diff] Gemini fallback coverage too low:", Math.round(cov * 100) + "%");
            }
          }
        }
        if (!remarkupOperationIsCurrent(_remarkupOwner)) return;
        if (newHtml) {
          ...
          warnLog2("[Diff] Apply failed \u2014 both surgery and Gemini paths could not produce acceptable output. surgeryReason:", surgeryFailReason);
          const committed = commitRemarkupOperation(_remarkupOwner, (prev) => prev ? {
            ...prev,
            finalText: _effectiveText,
            _userEditedAt: (/* @__PURE__ */ new Date()).toISOString(),
    ...
    const _refineSelection = async () => {
      ...
      try {
        ...
        try {
          ...
          if (!remarkupOperationIsCurrent(_remarkupOwner)) return;
          warnLog2("[Diff] refine callGemini failed: " + (e && e.message || e));
        }
        if (!remarkupOperationIsCurrent(_remarkupOwner)) return;
        if (!rewritten) {
        ...
        try {
          ...
          warnLog2("[Diff] refine surgery threw: " + (e && e.message || e));
        }
        if (!remarkupOperationIsCurrent(_remarkupOwner)) return;
        if (!newHtml) {
          usedFallback = true;
          try {
            ...
            warnLog2("[Diff] refine gemini fallback failed: " + (e && e.message || e));
          }
        }
        if (!remarkupOperationIsCurrent(_remarkupOwner)) return;
        if (newHtml) {
          const committed = commitRemarkupOperation(_remarkupOwner, (prev) => prev ? { ...prev, accessibleHtml: newHtml, finalText: newEffective, _userEditedAt: (/* @__PURE__ */ new Date()).toISOString(), _preApplyHtml: _prevHtml, _preApplyFinalText: _prevFinal, _lastApplyPath: usedFallback ? "ai-refine-gemini" : "ai-refine-surgery", _applyVerificationFailed: null } : prev);
          if (!committed) return;
          setDiffChunks(null);
          setDiffSelection(null);
        ...
        diffLibReady && _chunks && /* @__PURE__ */ React.createElement(
          ...
          _chunks.map((c) => {
            ...
            if (c.type === "del") {
              ...
              const _delCls = !_ocrTag ? "bg-rose-100 text-rose-900 hover:ring-2 hover:ring-rose-400" : _ocrTag.ocrConfidence === "high" ? "bg-rose-200 text-rose-900 ring-2 ring-rose-500 font-bold hover:ring-rose-700" : "bg-amber-100 text-amber-900 ring-1 ring-amber-400 hover:ring-2 hover:ring-amber-500";
              const _delTitle = !_ocrTag ? c.rejected ? "Restored \u2014 click to keep removed" : "Removed from source \u2014 click to restore" : _ocrTag.ocrConfidence === "high" ? c.rejected ? "Restored. Both OCR engines saw this \u2014 likely dropped inadvertently. Click to keep removed." : "Both Tesseract AND Vision saw this in the source \u2014 likely dropped inadvertently. Click to restore." : c.rejected ? "Restored. Only " + _ocrTag.ocrSource + " saw this \u2014 could be OCR hallucination. Click to keep removed." : "Only " + _ocrTag.ocrSource + " saw this \u2014 could be OCR hallucination, please verify. Click to restore.";
              return /* @__PURE__ */ React.createElement(
                "del",
                {
                  key: c.id,
                ...
                c.value,
                _ocrTag && _ocrTag.ocrConfidence === "high" && /* @__PURE__ */ React.createElement("sup", { "aria-hidden": "true", className: "text-[8px] ml-0.5 text-rose-700" }, "\u2713\u2713")
              );
            }
...
function GroupSessionModal(props) {
  ...
  const moveResourceBy = async (resId, delta) => {
    ...
    try {
      ...
      warnLog2("Failed to reorder resource:", err);
  ...
  const handleDrop = async (e, targetResId) => {
    ...
    if (draggedIndex !== -1 && targetIndex !== -1) {
      ...
      try {
        ...
        warnLog2("Failed to reorder resources:", err);
      }
    }
    setDraggedResourceId(null);
    setDragOverResourceId(null);
  ...
  const getResourceDescription = (r) => {
    ...
    try {
      ...
      warnLog2("Caught error:", e?.message || e);
    }
    return "";
  };
  ...
  return /* @__PURE__ */ React.createElement("div", { role: "presentation", className: "fixed inset-0 bg-black/90 z-[160] flex items-center justify-center p-4 animate-in fade-in duration-200 motion-reduce:animate-none", onClick: handleSetShowGroupModalToFalse, "data-help-key": "group_modal_container" }, /* @__PURE__ */ React.createElement("div", { ref: groupDialogRef, tabIndex: -1, onKeyDown: containGroupFocus, className: "bg-white rounded-2xl shadow-2xl w-[95vw] h-[90vh] relative animate-in zoom-in-95 duration-200 motion-reduce:animate-none flex flex-col overflow-hidden", onClick: (e) => e.stopPropagation(), role: "dialog", "aria-modal": "true", "aria-labelledby": "group-session-title", "aria-describedby": "group-session-description" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between p-5 border-b border-slate-200 bg-gradient-to-r from-purple-50 to-indigo-50 flex-shrink-0" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-4" }, /* @__PURE__ */ React.createElement("div", { className: "bg-purple-600 p-3 rounded-xl shadow-md" }, /* @__PURE__ */ React.createElement(Users, { size: 28, className: "text-white" })), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h2", { id: "group-session-title", className: "text-2xl font-black text-slate-800" }, t("groups.modal_title")), /* @__PURE__ */ React.createElement("p", { id: "group-session-description", className: "text-sm text-slate-600" }, t("groups.modal_subtitle")))), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-3" }, /* @__PURE__ */ React.createElement(
    ...
    return /* @__PURE__ */ React.createElement(
      ...
      description && /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-purple-700 bg-purple-50 px-2 py-1 rounded-md mb-1", style: { display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" } }, description),
      (language || dateStr) && /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-1 mb-1" }, language && (Array.isArray(language) ? language.slice(0, 5).map((lang, li) => /* @__PURE__ */ React.createElement("span", { key: li, className: "inline-flex items-center gap-0.5 text-[11px] bg-blue-100 text-blue-800 px-1.5 py-0.5 rounded font-medium" }, /* @__PURE__ */ React.createElement(Globe, { size: 8 }), " ", lang)) : /* @__PURE__ */ React.createElement("span", { className: "inline-flex items-center gap-0.5 text-[11px] bg-blue-100 text-blue-800 px-1.5 py-0.5 rounded font-medium" }, /* @__PURE__ */ React.createElement(Globe, { size: 8 }), " ", language)), dateStr && /* @__PURE__ */ React.createElement("span", { className: "inline-flex items-center gap-0.5 text-[11px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded" }, /* @__PURE__ */ React.createElement(Clock, { size: 8 }), " ", dateStr)),
...
function FluencyModePanel(props) {
  ...
  const updateFluencyReviewWord = (index, field, value) => {
    setFluencyReviewDraft((prev) => {
      if (!prev) return prev;
      const wordData = prev.wordData.map(
        (word, wordIndex) => wordIndex === index ? { ...word, [field]: value } : word
      );
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "fixed inset-0 z-[200] bg-slate-900/95 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300" }, showFluencyConfetti && /* @__PURE__ */ React.createElement("div", { className: "absolute inset-0 pointer-events-none z-[250] flex items-center justify-center" }, /* @__PURE__ */ React.createElement(ConfettiExplosion, null)), /* @__PURE__ */ React.createElement(
    ...
    /* @__PURE__ */ React.createElement("div", { className: "flex-1 overflow-y-auto p-8 bg-slate-50 custom-scrollbar flex flex-col items-center" }, fluencyStatus === "complete" && fluencyResult ? /* @__PURE__ */ React.createElement("div", { className: "w-full max-w-2xl animate-in zoom-in duration-300" }, (() => {
      ...
      const evidenceSummary = typeof summarizeFluencyEvidence === "function" ? summarizeFluencyEvidence(evidenceRecords, { sampleSize: 3 }) : null;
...
function SurpriseTopicLauncher(props) {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "rounded border border-violet-200 bg-violet-50/70 p-2 text-[11px] text-slate-700" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-center gap-2" }, /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      onClick: rollTheDice,
  ...
  ), /* @__PURE__ */ React.createElement("span", { className: "text-slate-600" }, "a standard", gradeLevel ? " for " + gradeLevel : "", ", at random"), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
  ...
  )), seedOpen && /* @__PURE__ */ React.createElement("div", { className: "mt-1 flex gap-1" }, /* @__PURE__ */ React.createElement(
    "input",
    {
      type: "text",
      value: surpriseQuery,
  ...
  ))), resolution && resolution.status === "not-found" && (resolution.candidates || []).length > 0 && /* @__PURE__ */ React.createElement("div", { role: "status", className: "mt-1" }, "Closest standards in the loaded snapshots \u2014 pick one to ground the proposals:", (resolution.candidates || []).slice(0, 4).map((candidate) => /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      key: candidate.id,
    ...
    candidate.code,
    /* @__PURE__ */ React.createElement("span", { className: "font-normal" }, " \xB7 ", plainStandardText(candidate.label || candidate.text, 60))
  ))), resolution && resolution.status === "not-found" && !(resolution.candidates || []).length && /* @__PURE__ */ React.createElement("div", { role: "status", className: "mt-1" }, "No word-for-word match in the loaded snapshots, so I asked the AI which standard this is. If it turns out to be a", /* @__PURE__ */ React.createElement("strong", null, "context"), " rather than a skill \u2014 a gecko, a World Cup, a school garden \u2014 put it in the topic field above and choose a standard; the proposals will use it as the hook.", /* @__PURE__ */ React.createElement("div", { className: "mt-1" }, /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      onClick: askForCodes,
  ...
  ))), codeState === "done" && codeHits.length > 0 && /* @__PURE__ */ React.createElement("div", { role: "status", className: "mt-1" }, "Found in the loaded snapshots \u2014 pick one to ground the proposals:", codeHits.map((hit) => /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      key: hit.match.id,
      onClick: () => chooseCandidate(hit.match),
      className: "ml-1 mt-1 rounded border border-violet-300 bg-white px-1.5 py-0.5 text-left font-bold hover:bg-violet-100"
    },
    hit.match.code,
    /* @__PURE__ */ React.createElement("span", { className: "font-normal" }, " \xB7 ", plainStandardText(hit.match.label, 55))
  ))), codeState === "done" && codeMisses.length > 0 && /* @__PURE__ */ React.createElement("div", { role: "status", className: "mt-1 text-slate-600" }, "Also suggested, but ", /* @__PURE__ */ React.createElement("strong", null, "not in the loaded snapshots"), " \u2014 unverified, check before relying on it:", codeMisses.map((miss) => /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      key: miss.code,
      onClick: () => {
        if (setStandardInputValue) setStandardInputValue(miss.code);
        if (addToast) addToast("Code " + miss.code + " prefilled in Universal Settings \u2014 verify it before use.", "info");
      },
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "space-y-3 p-3 bg-emerald-50 rounded-xl border border-emerald-200 animate-in fade-in slide-in-from-top-1", "data-help-key": "volume_builder_panel" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2 text-emerald-800 font-bold text-sm" }, "\u{1F4E6} ", t("volume_builder.title") || "3D Volume Explorer"), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-1" }, /* @__PURE__ */ React.createElement("button", { onClick: () => setCubeScale((s) => Math.max(0.4, s - 0.15)), className: "w-7 h-7 rounded-full bg-white border border-emerald-300 text-emerald-700 font-bold text-sm hover:bg-emerald-100 transition-all flex items-center justify-center", "aria-label": t("volume_builder.zoom_out_aria") || "Zoom out" }, "\u2212"), /* @__PURE__ */ React.createElement("span", { className: "text-[11px] text-emerald-600 font-mono w-10 text-center" }, Math.round(cubeScale * 100), "%"), /* @__PURE__ */ React.createElement("button", { onClick: () => setCubeScale((s) => Math.min(2.5, s + 0.15)), className: "w-7 h-7 rounded-full bg-white border border-emerald-300 text-emerald-700 font-bold text-sm hover:bg-emerald-100 transition-all flex items-center justify-center", "aria-label": t("volume_builder.zoom_in_aria") || "Zoom in" }, "+"), /* @__PURE__ */ React.createElement("button", { onClick: () => {
    ...
    return /* @__PURE__ */ React.createElement(
      "button",
      {
        ...
        className: "px-3 py-1.5 rounded-full text-xs font-bold transition-all border-2 " + (sel ? "bg-emerald-700 text-white border-emerald-700 shadow" : "bg-white text-emerald-700 border-emerald-200 hover:bg-emerald-50")
      },
      s.label
    );
    ...
    setCubeFeedback(null);
    setCubeShowLayers(null);
  }, className: "flex-1 py-2 bg-gradient-to-r from-emerald-700 to-teal-700 text-white font-bold rounded-lg text-sm hover:from-emerald-800 hover:to-teal-800 transition-all shadow-md", "data-help-key": "volume_builder_random_challenge_btn" }, "\u{1F3B2} ", t("volume_builder.random_challenge") || "Random Challenge"), /* @__PURE__ */ React.createElement(
    "button",
    {
      onClick: () => {
