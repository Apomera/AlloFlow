...
const vm = require('vm');
const { reviewWave, reviewDate, corrections } = require('./eppp_diagram_quality_wave_06_data.cjs');
...
const artifactName = 'eppp_diagram_review_wave_06';
...
function assertPlacement(source, correction, allowCorrected, allowMissingOriginal = false, scopeLabel = 'source') {
  ...
  if (String(diagram.title || '') !== correction.expectedOriginal.title
    ...
    throw new Error(`${correction.placementId}: current ${scopeLabel} diagram differs from both the guarded original and the wave 06 result; refusing to overwrite it.`);
  }
...
const markdown = `# EPPP diagram review wave 06
...
- Projected source-reviewed placement count after learning-library regeneration: 28 -> 34
- Projected review-required placement count after learning-library regeneration: 30 -> 24
...
console.log(`EPPP diagram quality wave 06: corrected and source-reviewed ${items.length} inline placements.`);
