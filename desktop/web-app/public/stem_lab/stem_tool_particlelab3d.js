  ...
  window.StemLab.registerTool('particleLab3d', {
    ...
    render: function (ctx) {
      ...
      useEffect(function () {
        ...
        function onTouchGate(event) { controls.enabled = event.touches.length >= 2 || event.touches.length === 0; }
        ...
        function animate(now) {
          ...
          if (focusRing.visible && selected) { focusRing.position.set(selected.x, selected.y, selected.z); focusRing.quaternion.copy(camera.quaternion); if (!reducedMotion) focusRing.rotation.z += elapsed * 1.5; }
      ...
      function choosePreset(next) {
        setPreset(next); setRunning(false); setHistory([]); setCameraView('hero'); if (next === 'osmosis') { setMembrane(true); setPermeability(0.72); setMembraneSelectivity('a'); setGravity(0); } setResetKey(function (k) { return k + 1; });
        ...
        h('div', { className: 'grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]' },
          ...
          h('aside', { className: 'space-y-4' },
            h('div', { className: 'rounded-2xl border border-slate-200 bg-white p-4 shadow-sm' },
              ...
              h('label', { className: 'mt-4 block text-xs font-bold text-slate-700' }, 'Particle count: ', h('output', { className: 'text-cyan-700' }, countDraft)),
