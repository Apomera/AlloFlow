...
(function () {
  ...
  function advanceParticles(particles, settings, dt) {
    ...
    for (i = 0; i < particles.length; i += 1) {
      ...
      ['x', 'y', 'z'].forEach(function (axis) {
        ...
        if (p[axis] > half - radius) { if (p.settling) p.settling = false; else impulse += particleMass * Math.abs(p[vel]) * 2; p[axis] = half - radius; p[vel] = -Math.abs(p[vel]); collisions += 1; p.wallHits = (p.wallHits || 0) + 1; if (events.length < 8) events.push({ kind: 'wall', axis: axis, side: 1, x: p.x, y: p.y, z: p.z, power: particleMass * Math.abs(p[vel]) }); }
        if (p[axis] < -half + radius) { if (p.settling) p.settling = false; else impulse += particleMass * Math.abs(p[vel]) * 2; p[axis] = -half + radius; p[vel] = Math.abs(p[vel]); collisions += 1; p.wallHits = (p.wallHits || 0) + 1; if (events.length < 8) events.push({ kind: 'wall', axis: axis, side: -1, x: p.x, y: p.y, z: p.z, power: particleMass * Math.abs(p[vel]) }); }
      });
    }
  ...
  window.StemLab.registerTool('particleLab3d', {
    ...
    render: function (ctx) {
      ...
      var [boxSizeDraft, setBoxSizeDraft] = useState(boxSize); useEffect(function () { setBoxSizeDraft(boxSize); }, [boxSize]);
      ...
      useEffect(function () {
        ...
        [-1, 1].forEach(function (side, species) {
          ...
          ring.visible = stub.visible = species === 0 || isTransportPreset(preset); scene.add(ring); scene.add(stub); nozzles.push({ ring: ring, stub: stub, ringMat: ringMat, stubMat: stubMat, species: species });
        });
        ...
        function pumpIn(n, type, targetTemperature) {
          var batch = makePumpedParticles(n, settingsRef.current, targetTemperature, 97 + particles.length * 13 + (type ? 1 : 0), type ? 1 : 0);
          batch.forEach(function (p) { particles.push(p); spawnMesh(p); });
          var nozzle = nozzles[type ? 1 : 0]; nozzlePulse[type ? 1 : 0] = 1;
          if (!reducedMotion) pendingFlashEvents = pendingFlashEvents.concat([{ kind: 'pump', species: type ? 1 : 0, x: nozzle.ring.position.x, y: 0, z: 0, power: n * 0.3 }]).slice(-14);
        ...
        function releaseOut(n) {
          ...
          if (!reducedMotion) pendingFlashEvents = pendingFlashEvents.concat([{ kind: 'release', species: 0, x: 0, y: boxSize / 2, z: 0, power: n * 0.3 }]).slice(-14);
          return particles.length;
        }
        ...
        var dividerGeo = new THREE.BoxGeometry(0.14, boxSize * 0.91, boxSize * 0.91), dividerMat = new THREE.MeshBasicMaterial({ color: 0xa78bfa, transparent: true, opacity: 0.32, depthWrite: false }), dividerSlab = new THREE.Mesh(dividerGeo, dividerMat); dividerSlab.visible = false; scene.add(dividerSlab);
        ...
        function resize() {
          ...
          if (flatFrameKey !== w + 'x' + hh) { flatFrameKey = w + 'x' + hh; var flatAspect = w / hh, flatSpan = boxSize * 0.66 / Math.min(1, flatAspect); flatCamera.left = -flatSpan * flatAspect; flatCamera.right = flatSpan * flatAspect; flatCamera.top = flatSpan; flatCamera.bottom = -flatSpan; flatCamera.updateProjectionMatrix(); }
        }
      ...
      function applyProtocol(protocol) {
        ...
        setActiveProtocol(protocol.id); setPreset(protocol.preset); setTemperature(protocol.temperature); setCount(protocol.count); setBoxSize(protocol.boxSize); setAttraction(protocol.attraction); setGravity(protocolGravity); setMembrane(protocolTransport); setPermeability(protocolOsmosis ? 0.72 : (protocol.id === 'mixing' ? 0 : 1)); setMembraneSelectivity(protocolOsmosis ? 'a' : 'both'); setMassRatioB(1); setParticleDiameter(protocolDiameter); setTrace(protocolTracer); setFollowTracer(false); setCameraView('hero'); setSelectedParticle(0); setRunning(false); setTrials([]); setTracerTrials([]); setHistory([]); setResetKey(function (k) { return k + 1; });
      ...
      var protocols = [
        ...
        { id: 'avogadro', icon: '➕', title: 'Add Particles', law: 'Avogadro\'s law', accent: 'from-emerald-500 to-teal-600', preset: 'gas', temperature: 300, count: 48, boxSize: 12, attraction: 0, prompt: 'Record Trial 1, then pump particles in while temperature and volume stay fixed. Wait until the newcomers have hit the walls, then record Trial 2.', watch: 'Watch pressure and wall-collision frequency climb as N grows.' },
      ...
      var particleViewportNode = h('div', { key: 'viewport', id: 'particle-viewport', className: 'relative overflow-hidden bg-slate-950' },
              h('canvas', { ref: canvasRef, tabIndex: ready ? 0 : -1, role: 'application', 'aria-hidden': ready ? undefined : 'true', 'aria-roledescription': 'Interactive 3D particle chamber', 'aria-label': preset + ' particle simulation', 'aria-describedby': 'particle-chamber-help', 'aria-keyshortcuts': 'Space R T V E M G C L F H D P A 2 ? Escape ArrowLeft ArrowRight ArrowUp ArrowDown Plus -', onKeyDown: onLabKey, onPointerDown: function (event) { if (!ready) return; try { event.currentTarget.focus(); } catch (error) {} }, className: 'h-full w-full saturate-[1.15] contrast-[1.04] focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-[-4px] focus-visible:outline-cyan-200' }),
          ...
          h('div', { className: 'flex flex-wrap items-start justify-between gap-3' },
            ...
            h('div', { className: 'flex flex-wrap items-center justify-end gap-2' },
              h('details', { className: 'max-w-md rounded-xl border border-cyan-700/60 bg-slate-950/60 px-3 py-2 text-xs text-cyan-100' },
                ...
                h('p', { className: 'mt-2 leading-relaxed text-cyan-50' }, 'Lengths, speeds and pressure are model units, not SI, and the chamber has no real-world scale. Pressure is the momentum particles deliver to the walls per unit time and wall area, so it rises with particle count and temperature and falls with volume, as the ideal gas law predicts. The count and container sliders re-seed the chamber with no work done on the gas, as in an ideal thought experiment; the pump adds particles to the running chamber instead. Attraction turns potential energy into motion, so a condensing chamber warms above its setpoint: there is no thermostat.')),
          ...
          h('section', { id: 'particle-stage', 'data-fullscreen': fsActive ? 'true' : 'false', ref: stageRef, tabIndex: -1, onKeyDown: onStageKeyDown, className: 'relative min-w-0 overflow-hidden border border-cyan-500/30 bg-slate-950 shadow-2xl shadow-cyan-950/40 ring-1 ring-white/5 ' + (fsActive ? 'h-screen rounded-none flex flex-col' : 'rounded-[24px]'), style: cssFullscreen ? { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100dvh', zIndex: 99990, borderRadius: 0 } : undefined, 'aria-label': 'Three-dimensional particle simulation', 'aria-busy': ready ? 'false' : 'true' },
            ...
            fsActive && showHud && showFullscreenConditions && h('section', { id: 'particle-fullscreen-conditions', className: 'max-h-[38vh] overflow-y-auto border-t border-cyan-400/30 bg-slate-900/95 px-4 py-3 text-white', 'aria-label': 'Fullscreen experiment conditions' },
              ...
              h('div', { className: 'grid gap-x-5 gap-y-3 sm:grid-cols-2 xl:grid-cols-3' },
                ...
                h('label', { className: 'block text-xs font-bold' }, 'Container edge ', h('output', { className: 'text-cyan-300' }, boxSizeDraft + ' u'), h('input', { type: 'range', min: 7, max: 18, step: 1, value: boxSizeDraft, onChange: function (e) { var value = Number(e.target.value); setBoxSizeDraft(value); scheduleRebuild(function () { setBoxSize(value); setRunning(false); setResetKey(function (k) { return k + 1; }); persist({ boxSize: value }); }); }, className: 'mt-1 block min-h-11 w-full accent-cyan-400', 'aria-label': 'Fullscreen container edge length' })),
            ...
            h('div', { className: 'rounded-2xl border border-slate-200 bg-white p-4 shadow-sm' },
              ...
              h('details', { id: 'particle-advanced-conditions', open: advancedOpen, onToggle: function (event) { var next = !!event.currentTarget.open; setAdvancedOpen(next); persist({ advancedOpen: next }); if (ctx.announceToSR) ctx.announceToSR(next ? 'Advanced chamber conditions expanded.' : 'Advanced chamber conditions collapsed.'); } },
                ...
                h('div', { className: 'pt-3' },
                  h('label', { className: 'block text-xs font-bold text-slate-700' }, 'Container edge: ', h('output', { className: 'text-cyan-700' }, boxSizeDraft + ' u'), h('span', { className: 'ml-1 font-normal text-slate-500' }, '(volume ' + Math.round(boxSizeDraft * boxSizeDraft * boxSizeDraft) + ' u\u00B3)')),
              h('input', { type: 'range', min: 7, max: 18, step: 1, value: boxSizeDraft, onChange: function (e) { var value = Number(e.target.value); setBoxSizeDraft(value); scheduleRebuild(function () { setBoxSize(value); setRunning(false); setResetKey(function (k) { return k + 1; }); persist({ boxSize: value }); }); }, className: 'mt-1 w-full accent-cyan-600', 'aria-label': 'Container edge length and volume' }),
