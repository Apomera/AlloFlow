...
for(const [slug,terms,count] of [['states_of_matter_grade4',11,13],['plant_needs_grade3',10,14]]){
...
expect(questions[0].correctAnswer).toContain('arranged');
expect(pack.history.find(r=>r.id==='sm-reading').data).toContain('More heat does not always');
const models=pack.history.find(r=>r.title==='Models of tiny particles').data.visualPlan.panels;
expect(models.map(p=>p.id)).toEqual(['sm-img-solid-particles','sm-img-liquid-particles','sm-img-gas-particles']);
}else{
expect(questions[5].expectedAnswer).toContain('nutrients');
expect(pack.history.find(r=>r.id==='pl-reading').data).toContain('stored food');
expect(pack.history.find(r=>r.id==='pl-challenge').data.brief.context).toContain('seedlings');
}
});
});
}
