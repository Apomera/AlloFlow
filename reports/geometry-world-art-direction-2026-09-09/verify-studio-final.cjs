...
harness+='('+(async function(){
 ...
 try{
   ...
   const editorCamera=await page.evaluate(()=>{const c=__geoWorldEngine.camera;return {position:c.position.toArray(),quaternion:c.quaternion.toArray(),up:c.up.toArray(),fov:c.fov,far:c.far};});const originalGeometry=await geometry();results.checks.fixture={hash:originalGeometry.hash,selected:originalGeometry.selected,triangles:originalGeometry.triangles};
   ...
   for(const config of[{width:1180,height:860,look:'Studio',postFx:false,file:'studio-art-final-desktop.png'},{width:390,height:844,look:'Studio',postFx:true,file:'studio-art-final-phone.png'}]){
     ...
     results.checks.captionContrast=results.checks.captionContrast||[];
     for(const look of ['Meadow','Studio']){
       await page.getByRole('button',{name:look,exact:true}).click();
       const contrast=await page.evaluate(()=>{
         const caption=document.querySelector('.gwe-showcase-caption'),look=document.querySelector('.gwe-showcase').dataset.look;
         function rgb(value){return value.match(/[\d.]+/g).slice(0,3).map(Number);}
         function luminance(value){const c=rgb(value).map(n=>{n/=255;return n<=.04045?n/12.92:Math.pow((n+.055)/1.055,2.4);});return c[0]*.2126+c[1]*.7152+c[2]*.0722;}
         const background=look==='meadow'?getComputedStyle(caption,'::before').backgroundColor:__geoWorldEngine.scene.background.clone().convertLinearToSRGB().getStyle();
         const colors=['span','strong','p'].map(selector=>{const node=caption.querySelector(selector);if(!node)return null;const style=getComputedStyle(node),a=luminance(style.color),b=luminance(background);return {selector,color:style.color,display:style.display,ratio:(Math.max(a,b)+.05)/(Math.min(a,b)+.05)};}).filter(Boolean);
         return {look,background,colors};
       });
       contrast.viewport=[config.width,config.height];results.checks.captionContrast.push(contrast);
       assert.ok(contrast.colors.every(c=>c.display==='none'||c.ratio>=(c.selector==='strong'?3:4.5)),'Caption contrast '+look+' '+config.width);
       if(look==='Meadow')await page.screenshot({path:path.join(out,config.width>500?'meadow-caption-final-desktop.png':'meadow-caption-final-phone.png'),timeout:60000});
     }
     ...
     for(const view of ['Perspective','Front','Side','Top']){
       ...
       const fit=await page.evaluate(()=>{const e=__geoWorldEngine,box=new THREE.Box3();e._builderSelection.blocks.forEach(p=>box.expandByObject(e.blocks[[p.x,p.y,p.z].join(',')]));e.camera.updateMatrixWorld(true);const points=[];for(const x of[box.min.x,box.max.x])for(const y of[box.min.y,box.max.y])for(const z of[box.min.z,box.max.z])points.push(new THREE.Vector3(x,y,z).project(e.camera).toArray());return points;});
       assert.ok(fit.every(p=>Math.abs(p[0])<.98&&Math.abs(p[1])<.98&&p[2]<1),'All bounds fit '+config.width+' '+view);
     }
     await page.getByRole('button',{name:'Perspective',exact:true}).click();
     await page.getByRole('button',{name:'Rotate view right',exact:true}).click();await page.getByRole('button',{name:'Rotate view left',exact:true}).click();
     await page.screenshot({path:path.join(out,config.width>500?'studio-art-final-ui-desktop.png':'studio-art-final-ui-phone.png'),timeout:60000});
     ...
     const pixels=await page.evaluate(async file=>{const img=new Image();img.src='/reports/geometry-world-art-direction-2026-09-09/'+file;await img.decode();const c=document.createElement('canvas');c.width=img.width;c.height=img.height;const x=c.getContext('2d');x.drawImage(img,0,0);const data=x.getImageData(0,0,c.width,c.height).data,colors=new Set();let opaque=0;for(let i=0;i<data.length;i+=4*257){colors.add([data[i]>>4,data[i+1]>>4,data[i+2]>>4].join(','));if(data[i+3]===255)opaque++;}return {colors:colors.size,opaqueSamples:opaque};},config.file);
   ...
   const stage=await page.evaluate(()=>{const e=__geoWorldEngine,s=e._showcase.studio;window.__studioDisposed=0;s.resources.forEach(resource=>{const dispose=resource.dispose;resource.dispose=function(){__studioDisposed++;return dispose.apply(this,arguments);};});return {resources:s.resources.length,footprints:s.contactFootprints.length,textures:['gwe-studio-contact-shadow','gwe-studio-light-pool'].map(name=>{const map=s.group.getObjectByName(name).material.map;return {name,complete:!map.generateMipmaps&&map.minFilter===THREE.LinearFilter,minFilter:map.minFilter};}),shaders:e.renderer.info.programs.map(p=>p.diagnostics).filter(d=>d&&d.runnable===false)};});
   assert.equal(stage.resources,8);assert.ok(stage.textures.every(t=>t.complete));assert.ok(stage.footprints>0);assert.equal(stage.shaders.length,0);results.checks.stage=stage;
   await page.getByRole('button',{name:'Back to building',exact:true}).click();
   const restoredCamera=await page.evaluate(()=>{const c=__geoWorldEngine.camera;return {position:c.position.toArray(),quaternion:c.quaternion.toArray(),up:c.up.toArray(),fov:c.fov,far:c.far};});assert.deepEqual(restoredCamera,editorCamera);assert.equal(await page.evaluate(()=>__studioDisposed),8);assert.deepEqual(await geometry(),originalGeometry);results.checks.exitRestored=true;
   assert.equal(results.errors.length,0);assert.equal(results.consoleErrors.length,0);results.pass=true;
 }catch(e){results.failure=e.stack;process.exitCode=1;await page.screenshot({path:path.join(out,'studio-art-final-failure.png'),timeout:60000}).catch(()=>{});}
 finally{writeFile(path.join(out,'studio-art-final-results.json'),Buffer.from(JSON.stringify(results,null,2)));console.log(JSON.stringify(results,null,2));await browser.close();await new Promise(r=>server.close(r));}
