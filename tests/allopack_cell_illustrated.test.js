...
for(const tag of ['data-boundary="wall"','data-boundary="membrane"','data-organelle="central-vacuole"'])expect(svg('plant')).toContain(tag);expect(svg('membrane')).toContain('allowed-in');expect(get('simplified').data).toContain('cytosol');expect(get('concept-sort').data.items[9].categoryId).toBe('c-both');
});
