...
(function () {
  ...
  function LinguaPractice(props) {
    ...
    var rors=useState('due'), reviewOrder=rors[0], setReviewOrder=rors[1];
    var rszs=useState('all'), reviewSessionSize=rszs[0], setReviewSessionSize=rszs[1];
    ...
    useEffect(function(){closePlanEditor();setJournalReflectionText('');setJournalStatus('');setLastReviewUndo(null);setReviewSession(emptyReviewSession());setReviewSkippedIds([]);setReviewTag('all');setReviewOrder('due');setReviewSessionSize('all');},[profile.target]);
    ...
    function changeReviewSessionSize(value){
      ...
      setReviewSessionSize(next);var key=next==='all'?'review_session_size_all':next==='5'?'review_session_size_5':next==='10'?'review_session_size_10':'review_session_size_20';setReviewStatus(tr('review_session_size_changed',{size:tr(key)}));
    }
    function startAnotherReviewSession(){
      reviewFocusPendingRef.current=true;setLastReviewUndo(null);setReviewSession(emptyReviewSession());setReviewSkippedIds([]);setReviewRecall('');setReviewRevealed(false);setReviewStatus(tr('review_session_started'));
    }
            ...
            tab==='review'&&e('div',{ref:reviewRegionRef,tabIndex:-1,className:'max-w-3xl mx-auto p-5 sm:p-8'+focusTargetClass},
              ...
              currentReviewWords.length?e('section',{className:'mb-4 rounded-xl border border-slate-200 bg-slate-50 p-4','aria-labelledby':'lingua-review-queue-title'},
                ...
                e('div',{className:'grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3'},
                  ...
                  e('label',{htmlFor:'lingua-review-order',className:'block text-xs font-bold text-slate-700'},tr('review_order_label'),e('select',{id:'lingua-review-order',value:reviewOrder,onChange:function(x){var value=x.target.value;setReviewOrder(value);setReviewStatus(tr('review_order_changed',{order:tr(value==='reviews'?'review_order_reviews':value==='term'?'review_order_term':'review_order_due')}));},'aria-describedby':'review-order-help',className:'block w-full h-10 mt-1 rounded-lg border border-slate-300 bg-white px-3 text-sm'+focusClass},e('option',{value:'due'},tr('review_order_due')),e('option',{value:'reviews'},tr('review_order_reviews')),e('option',{value:'term'},tr('review_order_term')))),
