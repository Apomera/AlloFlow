...
const library = JSON.parse(fs.readFileSync(path.join(root, 'test_prep', 'school_psychologist_5403_learning_library.json'), 'utf8'));
...
const domainIds = new Set(['permeating-practices', 'student-level-services', 'systems-level-services', 'foundations']);
const findings = [];
...
if (library.schemaVersion !== 1 || library.libraryId !== 'praxis-school-psychologist-5403-learning-library' || library.packId !== 'praxis-school-psychologist-5403') add('Invalid library identity or schema.');
...
if (!library.simulation || library.simulation.questionCount !== 125 || library.simulation.timeMinutes !== 125) add('Timed simulation declaration must be 125 questions and 125 minutes.');
if (!/not a psychological evaluation, diagnosis, mental-health treatment, or legal advice/i.test(library.legalCaution || '') || !/consent, privacy, and mandated-reporting/i.test(library.legalCaution || '')) add('Evaluation, treatment, legal, consent, privacy, crisis, and reporting caution is missing.');
if (!/ETS 5403 study companion defines the tested blueprint/i.test(library.frameworkCaution || '') || !/NASP standards, ethics, and practice materials can be revised/i.test(library.frameworkCaution || '')) add('Framework-version caution is missing.');
...
for (const chapter of library.chapters || []) {
  ...
  if (chapter.reviewStatus !== 'source-reviewed-editorial-pass' || !/independent school-psychologist validation remains pending/i.test(chapter.reviewNote || '') || !validReferences(chapter.references)) add('Invalid chapter review metadata: ' + chapter.id);
...
const report = {
  ...
  standard: {
    ...
    limitation: 'This is not ETS or NASP approval, independent school-psychologist validation, psychometric validation, psychological evaluation, diagnosis, mental-health treatment, legal advice, or a substitute for current federal, state, local, district, credential, ethical, crisis, consent, privacy, and mandated-reporting requirements.',
  },
