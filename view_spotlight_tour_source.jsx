...
function SpotlightTourView(props) {
  ...
  var _viewportState = React.useState(function () { return { width: (typeof window !== 'undefined' && window.innerWidth) || 1024, height: (typeof window !== 'undefined' && window.innerHeight) || 768 }; });
  var viewport = _viewportState[0], setViewport = _viewportState[1];
  React.useEffect(function () {
    if (typeof window === 'undefined') return;
    var updateViewport = function () {
      var width = window.innerWidth || 1024, height = window.innerHeight || 768;
      setViewport(function (previous) { return previous.width === width && previous.height === height ? previous : { width: width, height: height }; });
    };
    window.addEventListener('resize', updateViewport);
    updateViewport();
    return function () { window.removeEventListener('resize', updateViewport); };
  }, []);
