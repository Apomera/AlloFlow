...
const out = path.join(ROOT, 'scratch', 'water-spring-discharge-review');
...
(async () => {
  ...
  try {
    ...
    for(const [x,z,scenario,body] of [[205,-46,'temperateCoast','stream'],[280,-140,'temperateCoast','lake'],[-120,80,'desertBasin','ocean']]){
      await restore('groundwater',scenario,x,z,0,.999);
      assert.equal((await impact()).visible,false,'Checkpoint does not invent a discharge');
      const route=await page.evaluate(()=>{
        const group=pilotReview.scene.getObjectByName('pilot-groundwater-cutaway'),mesh=pilotReview.scene.getObjectByName('pilot-groundwater-route');
      ...
      await page.waitForFunction(()=>document.querySelector('#wcPilotCanvas').dataset.waterLanding==='seep-ripples');
      const landing=await page.evaluate(()=>({position:pilotReview.scene.getObjectByName('pilot-water-parcel').position.toArray(),
        ...
        surface:waterReviewData.pilot.snapshot.surface,underground:pilotReview.scene.getObjectByName('pilot-groundwater-cutaway').visible}));
      ...
      const seep=await impact();assert(seep.visible&&!seep.spray&&seep.source==='groundwater'&&seep.surface===body);
      assert((await page.locator('.wc-pilot-notice').innerText()).includes('Groundwater discharge'));
      assert((await page.locator('.wc-pilot-notice').innerText()).includes('No phase change - no latent heat transfer in this step'));
      await page.waitForTimeout(600);assert.deepEqual(await impact(),seep,'The learning pause holds the seep ripple');
    ...
    await restore('groundwater','temperateCoast',200,0,0,.45);
    await page.evaluate(()=>{
      window.springGeometryDisposals=0;
      pilotReview.scene.getObjectByName('pilot-groundwater-route').geometry.addEventListener('dispose',()=>springGeometryDisposals++);
    });
    await restore('groundwater','temperateCoast',280,-140,0,.6);
    ...
    await page.evaluate(()=>{
      ...
      scene.getObjectByName('pilot-groundwater-route').geometry.addEventListener('dispose',()=>springFinalDisposals.route++);
    ...
    console.log('PASS: short-stream/lake/ocean route endpoints, actual groundwater discharge at matched surface height, cycle/energy preservation, distinct seep and rain cues, pause/reduced motion/expiry, both cameras, restore exclusions, mobile accessibility, route replacement and shared cleanup, no WebGL errors.');
