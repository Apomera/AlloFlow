...
(async()=>{
 ...
 try {
  ...
  await grams.waitFor();
  await page.locator('.chem-yield-panel').screenshot({path:path.join(out,'visual-yield-start.png')});
  await page.getByRole('button',{name:'Load practice example: Water formation',exact:true}).click();
  assert((await page.locator('main').textContent()).includes('18.015 g H2O'));
  report.checks.push('A practice example loads a balanced reaction and verified example quantities.');
  ...
  assert((await page.locator('main').textContent()).includes('Percent yield: 200.0%'));
  assert((await page.locator('main').textContent()).includes('used up together'));
  await reaction.fill('H2 + O2 -> H2O');
  assert(!(await page.locator('main').textContent()).includes('Theoretical yield:'));
  await reaction.press('Enter');
  ...
  assert((await page.locator('main').textContent()).includes('Theoretical yield: 18.015 g H2O (1 mol)'));
  await page.getByRole('spinbutton',{name:'Actual grams produced',exact:true}).fill('9.0075');
  assert((await page.locator('main').textContent()).includes('Percent yield: 50.0%'));
  ...
  for(const theme of ['theme-default','theme-dark','theme-contrast']) for(const width of [1200,360,320]){
   ...
   report.axe.push({theme,width,...result});
   if(width===1200) await page.locator('#chem-yield-qa').screenshot({path:path.join(out,'visual-yield-'+theme+'.png')});
   if(width===320 && theme==='theme-default') await page.locator('#chem-yield-qa').screenshot({path:path.join(out,'visual-yield-mobile.png')});
  }
  assert.deepEqual(report.axe.filter(r=>r.violations.length || r.overflow),[]);
  await page.emulateMedia({forcedColors:'active'});
  assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
  report.checks.push('Default, dark, and high-contrast layouts pass at three widths; forced-colors mode preserves reflow.');
 ...
 }finally{fs.writeFileSync(path.join(out,'visual-browser-results.json'),JSON.stringify(report,null,2));await browser.close();}
