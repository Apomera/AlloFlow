...
if (!React) { console.error('[AlloCommandContext] React not found on window'); return; }
window.AlloModules = window.AlloModules || {};
if (window.AlloModules.AlloCommandContext) return;
...
function buildAlloCommandContext(deps) {
  const {
    ...
    _alloCmdCtx,
    _alloCmdCtxRef,
    _alloEducatorAccessCodeRequired,
    _alloEvaluateObjectives,
    _alloIsStudentSafeResource,
    _alloNormalizeDirectionsData,
    _alloObjectiveSignals,
    _alloRecommendNextStations,
    _alloRequestStemPlugin,
    _alloStudentSafeResources,
    _alloVoiceInputAvailable,
    _contextualSuggestionRef,
    _editMainVoiceEditableField,
    _helpLookup,
    _listMainVoiceEditableFields,
    _selectMainVoiceEditableField,
    _sendUdlToChat,
    _voiceConverseWaitersRef,
    _voiceEditableFieldSelectionRef,
    activeBlueprint,
    activeSessionAppId,
    activeSessionCode,
    activeSidebarTab,
    activeView,
    addToast,
    adventureFluencyEnabled,
    adventureFreeResponseEnabled,
    adventureState,
    adventureTypingPaceEnabled,
    alloBotRef,
    alloVoiceActive,
    alloVoiceSessionStatus,
    appId,
    callGeminiAudio,
    closeReadThisPage,
    colorOverlay,
    ...
    handleSetGlossaryFilterToAcademic,
    handleSetGlossaryFilterToAll,
    handleSetGlossaryFilterToDomain,
    handleSetIsBingoGameToTrue,
    handleSetIsCrosswordGameToTrue,
    handleSetIsMatchingGameToTrue,
    handleSetIsMemoryGameToTrue,
    handleSetIsProjectSettingsOpenToTrue,
    handleSetIsStudentBingoGameToTrue,
    handleSetIsTranslateModalOpenToTrue,
    handleSetIsWordScrambleGameToTrue,
    handleSetIsZenModeToFalse,
    handleSetIsZenModeToTrue,
    handleSetRunTourToFalse,
    handleSetShowGroupModalToTrue,
    handleSetShowSaveModalToFalse,
    ...
    handleToggleIsEditingAnalysis,
    handleToggleIsEditingBrainstorm,
    handleToggleIsEditingFaq,
    handleToggleIsEditingGlossary,
    handleToggleIsEditingLeveledText,
    handleToggleIsEditingOutline,
    handleToggleIsEditingQuiz,
    handleToggleIsEditingScaffolds,
    handleToggleIsHelpMode,
    handleToggleIsPresentationMode,
    handleToggleIsReviewGame,
    handleToggleReadingRuler,
    handleToggleShowQuizAnswers,
    ...
    isGlobalMuted,
    isIndependentMode,
    isLinguaPracticeOpen,
    isOpenGrooveOpen,
    isParentMode,
    isProcessing,
    isProjectSettingsOpen,
    isReadingLibraryOpen,
    isResearchSuiteOpen,
    isStudentLinkMode,
    isSymbolStudioOpen,
    isTeacherMode,
    isTestPrepHubOpen,
    isTimelineStudioOpen,
    isTranslateModalOpen,
    isVideoStudioOpen,
    isZenMode,
    latestLessonPlan,
    latestStudentPreviewShare,
    launchInteractiveFlashcards,
    lessonTemplates,
    leveledTextLanguage,
    lineHeight,
    nextReadThisPageItem,
    normalizeAlloEvaluationPortalUrl,
    openExportPreview,
  ...
  const commandAudience = isStudentLinkMode || !isTeacherMode ? 'student' : isIndependentMode ? 'independent' : isParentMode ? 'parent' : 'teacher';
  const assignmentDirections = generatedContent && generatedContent.type === 'directions' ? generatedContent : [...(Array.isArray(history) ? history : [])].reverse().find(item => item && item.type === 'directions') || null;
  const getAssignmentProgress = () => {
    ...
    return {
      title: assignmentDirections.title || t('directions.title') || 'Assignment Directions',
      done: evaluated.filter(goal => goal.done).length,
  ...
  const getNextAssignmentStep = () => {
    ...
    if (recommendation.next && recommendation.next.item) return {
      item: recommendation.next.item,
      title: recommendation.next.item.title || recommendation.next.item.type,
      goalLabel: recommendation.next.goalLabel || ''
    };
    const openGoal = evaluated.find(goal => !goal.done);
    return openGoal ? {
      item: assignmentDirections,
      title: assignmentDirections.title || t('directions.title') || 'Assignment Directions',
  ...
  const getSuccessCriteria = () => {
    if (assignmentDirections) {
      ...
      if (goals.length) return {
        title: assignmentDirections.title || t('directions.your_goals') || 'Your goals',
        criteria: goals
      };
    }
    ...
    for (const item of candidates) {
      ...
      if (Array.isArray(data)) data.forEach(activity => {
        if (activity && activity.rubric) rubrics.push(activity.rubric);
      });
      for (const rubric of rubrics) {
        if (typeof rubric === 'string' && rubric.trim()) return {
          title: item.title || t('directions.success_criteria') || 'Success criteria',
          criteria: [rubric.replace(/\s+/g, ' ').trim().slice(0, 900)]
        };
        if (rubric && Array.isArray(rubric.criteria)) {
          ...
          if (criteria.length) return {
            title: rubric.title || item.title || t('directions.success_criteria') || 'Success criteria',
            criteria
          };
        }
      }
  ...
  const getTeacherFeedback = () => {
    ...
    for (const item of items) {
      ...
      for (const value of values) {
        ...
        if (text) return {
          title: item.title || t('feedback.review_teacher') || 'Teacher feedback',
  ...
  const rubricActivityIndex = Array.isArray(generatedContent && generatedContent.data) ? generatedContent.data.findIndex(activity => activity && activity.title && !(activity.rubric && Array.isArray(activity.rubric.criteria) && activity.rubric.criteria.length)) : -1;
  ...
  const getTutorialVoiceState = () => {
    ...
    if (guidedMode) {
      ...
      return {
        ...
        busy,
        nextReason: required && !completed ? 'Complete this required Guided milestone before moving on. Say focus guided tool to return to it.' : 'Complete the current Guided step before moving on, or say skip guided step.'
      };
    }
    return {};
  };
  const invokeTutorialVoiceAction = action => {
    ...
    if (action === 'previous') {
      ...
      if (state.kind === 'guided') setGuidedStep(value => Math.max(0, value - 1));else handlePrevTourStep();
  ...
  const isLearnerResourceSurfaceBlocked = () => !!(showAIBackendModal || showTextSettings || showVoiceSettings || isTranslateModalOpen || isGateOpen || isProjectSettingsOpen || showDirectionsComposer || showAssessmentBuilder || showUDLGuide || showLivePollingPanel || showStudentSignals || showLiveDock || showExportPreview || showExportMenu || showSessionModal || showRecentQrShares || showClassAnalytics || isResearchSuiteOpen || showNotebook || showStudentEntry || showSaveModal || showSubmitModal || showReadThisPage || isTestPrepHubOpen || isLinguaPracticeOpen || isTimelineStudioOpen || isOpenGrooveOpen || showResearchHub || showLitLab || showLearningWebExplorer || showMindMap || showPoetTree || showStemLab || showStoryForge || isAlloHavenOpen || showBehaviorLens || showReportWriter || isSymbolStudioOpen || isVideoStudioOpen || isAlloStudioOpen || showCinematicStudio || isAccessibilityLabOpen || isCommunityCatalogOpen || isReadingLibraryOpen || isDynamicAssessmentOpen || showSelHub || showLearningHub || showEducatorHub || runTour || guidedMode || !hasSelectedMode || !hasSelectedRole);
  const isLearnerResourceDiscoveryActive = () => !isLearnerResourceSurfaceBlocked() && (activeView === 'dashboard' || activeView === 'history' || activeSidebarTab === 'history');
  const getLearnerResourceFeedbackText = content => {
    if (!content || typeof content !== 'object') return '';
    const values = [content.teacherFeedback, content.returnedFeedback, content.feedbackFromTeacher, content.data && content.data.teacherFeedback, content.data && content.data.returnedFeedback, content.data && content.data.feedbackFromTeacher];
    for (const value of values) {
      if (typeof value === 'string' && value.trim()) return value.trim().slice(0, 1400);
      if (value && typeof value === 'object') {
        const text = [value.text || value.comment || value.message || value.notes, value.glow && 'Glow: ' + value.glow, value.grow && 'Grow: ' + value.grow].filter(Boolean).join(' ').trim();
  ...
  const getCurrentLearnerResource = () => {
    ...
    try {
      if (mediaApi && typeof mediaApi.getMediaDescriptionItems === 'function') {
        media = mediaApi.getMediaDescriptionItems({
          generatedContent,
          root: typeof document !== 'undefined' ? document.getElementById('main-content') : null
        });
      }
    } catch (_) {}
  ...
  const listCurrentActions = () => {
    ...
    try {
      ...
      if (commandApi && typeof commandApi.listActiveCommandScopes === 'function') {
        ...
        for (const scopeId of preferredIds) {
          ...
          let capabilities = {};
          try {
            capabilities = typeof scope.getCapabilities === 'function' ? scope.getCapabilities(ctx) || {} : {};
          } catch (_) {}
      ...
      if (commandApi && typeof commandApi.buildAlloCommands === 'function') {
        ...
        if (showAIBackendModal) {
          ...
          ids = commandAudience === 'student' ? ['read_this_page', 'check_assignment_progress', 'next_assignment_step', 'save_my_work', 'submit_work', 'review_teacher_feedback', 'close_current_surface', 'repeat_last_response'] : ['read_this_page', 'open_export_menu', 'open_translate', 'close_current_surface', 'repeat_last_response'];
        } else if (activeView === 'dashboard') {
          ids = commandAudience === 'student' ? ['open_learning_hub', 'open_test_prep_hub', 'open_notebook', 'describe_current_screen'] : ['open_educator_hub', 'open_learning_hub', 'open_source_input', 'open_history', 'describe_current_screen'];
        } else {
          ids = ['describe_current_screen', 'close_current_surface', 'repeat_last_response'];
        }
    ...
    return commandAudience === 'student' ? ['open Learning Hub', 'open Test Prep Hub', 'open my notebook', 'describe the current screen'] : ['open Educator Hub', 'open Learning Hub', 'open source input', 'open history', 'describe the current screen'];
  };
  const closeCurrentSurface = () => {
    if (showSaveModal) {
      handleSetShowSaveModalToFalse();
      return saveType === 'student' ? 'Save My Work closed.' : 'Save Project closed.';
    }
    if (showSubmitModal) {
      handleCloseSubmitModal();
      return 'Submit Work closed.';
    }
    if (showStudentEntry && hasSelectedRole && !isTeacherMode) {
      handleCloseStudentEntry();
      return 'Student Entry closed.';
    }
    if (runTour && tourRect) return invokeTutorialVoiceAction('exit') || 'Tutorial closed.';
    if (showAIBackendModal) {
      setShowAIBackendModal(false);
      return 'AI Backend Settings closed.';
    }
    if (showTextSettings) {
      setShowTextSettings(false);
      return 'Text Settings closed.';
    }
    if (showVoiceSettings) {
      setShowVoiceSettings(false);
      return 'Voice & Audio Settings closed.';
    }
    ...
    if (isGateOpen) {
      setIsGateOpen(false);
      setPendingRole(null);
      return 'Protected educator access canceled.';
    }
    if (isProjectSettingsOpen) {
      setIsProjectSettingsOpen(false);
      return 'Project Settings closed.';
    }
    if (showDirectionsComposer) {
      setShowDirectionsComposer(false);
      return 'Assignment Directions closed.';
    }
    if (showAssessmentBuilder) {
      setShowAssessmentBuilder(false);
      return 'Assessment Builder closed.';
    }
    if (showUDLGuide) {
      setShowUDLGuide(false);
      return 'UDL Guide closed.';
    }
    if (showLivePollingPanel) {
      setShowLivePollingPanel(false);
      return 'Live Poll closed.';
    }
    if (showStudentSignals) {
      setShowStudentSignals(false);
      return 'Teacher Signal panel closed.';
    }
    if (showLiveDock) {
      setShowLiveDock(false);
      return 'Live Dashboard closed.';
    }
    if (showExportPreview) {
      setShowExportPreviewWrapped(false);
      return 'Document Builder closed.';
    }
    if (showExportMenu) {
      setShowExportMenu(false);
      return 'Export Menu closed.';
    }
    if (showSessionModal) {
      setShowSessionModal(false);
      return 'Class Session closed.';
    }
    if (showRecentQrShares) {
      setShowRecentQrShares(false);
      return 'Share and Collect closed.';
    }
    if (isResearchSuiteOpen) {
      setIsResearchSuiteOpen(false);
      return 'Research Suite closed.';
    }
    if (showClassAnalytics) {
      setShowClassAnalytics(false);
      return 'Class Analytics closed.';
    }
    if (showNotebook) {
    ...
    if (showReadThisPage) {
      closeReadThisPage();
      return 'Read This Page closed.';
    }
    if (isTestPrepHubOpen) {
      setIsTestPrepHubOpen(false);
      return 'Test Prep Hub closed.';
    }
    if (isLinguaPracticeOpen) {
      setIsLinguaPracticeOpen(false);
      return 'Lingua Practice closed.';
    }
    if (isTimelineStudioOpen) {
      setIsTimelineStudioOpen(false);
      return 'Timeline Studio closed.';
    }
    if (isOpenGrooveOpen) {
      setIsOpenGrooveOpen(false);
      return 'Open Groove Studio closed.';
    }
    if (showResearchHub) {
      setShowResearchHub(false);
      return 'Research Hub closed.';
    }
    if (showLitLab) {
      setShowLitLab(false);
      return 'Lit Lab closed.';
    }
    if (showLearningWebExplorer) {
      setShowLearningWebExplorer(false);
      return 'Learning Web: Explore closed.';
    }
    if (showMindMap) {
      setShowMindMap(false);
      setThroughlineSeedUnitId(null);
      return 'Learning Web: Unit Path closed.';
    }
    if (showPoetTree) {
      setShowPoetTree(false);
      return 'Poet Tree closed.';
    }
    if (showStemLab) {
      setShowStemLab(false);
      return 'STEAM Lab closed.';
    }
    if (showStoryForge) {
      setShowStoryForge(false);
      return 'StoryForge closed.';
    }
    if (isAlloHavenOpen) {
      setIsAlloHavenOpen(false);
      return 'AlloHaven closed.';
    }
    if (showBehaviorLens) {
      setShowBehaviorLens(false);
      return 'BehaviorLens closed.';
    }
    if (showReportWriter) {
      setShowReportWriter(false);
      return 'Report Writer closed.';
    }
    if (isSymbolStudioOpen) {
      setIsSymbolStudioOpen(false);
      return 'Symbol Studio closed.';
    }
    if (isVideoStudioOpen) {
      setIsVideoStudioOpen(false);
      return 'Video Studio closed.';
    }
    if (isAlloStudioOpen) {
      setIsAlloStudioOpen(false);
      return 'AlloStudio closed.';
    }
    if (showCinematicStudio) {
      setShowCinematicStudio(false);
      return 'Cinematic Studio closed.';
    }
    if (isAccessibilityLabOpen) {
      setIsAccessibilityLabOpen(false);
      return 'Accessibility Lab closed.';
    }
    if (isCommunityCatalogOpen) {
      setIsCommunityCatalogOpen(false);
      return 'Community Catalog closed.';
    }
    if (isReadingLibraryOpen) {
      setIsReadingLibraryOpen(false);
      return 'Reading Library closed.';
    }
    if (isDynamicAssessmentOpen) {
      setIsDynamicAssessmentOpen(false);
      return 'Dynamic Assessment closed.';
    }
    if (showSelHub) {
      setShowSelHub(false);
      return 'SEL Hub closed.';
    }
    if (showLearningHub) {
      setShowLearningHub(false);
      return 'Learning Hub closed.';
    }
    if (showEducatorHub) {
      setShowEducatorHub(false);
      return 'Educator Hub closed.';
    }
    if (activeView !== 'dashboard') {
      handleSetActiveViewToDashboard();
      return 'Returned to the dashboard.';
    }
  ...
  const goBack = () => {
    if (runTour && tourRect) {
      ...
      return tutorial.canPrevious ? invokeTutorialVoiceAction('previous') || 'Moved to the previous tutorial step.' : invokeTutorialVoiceAction('exit') || 'Tutorial closed.';
    }
  ...
  const repeatLastResponse = () => {
    try {
      ...
      return 'There is no previous command response to repeat yet.';
    }
  };
  ...
  const _contextualLessonState = () => {
    ...
    const provider = standardsApi && typeof standardsApi.getRegisteredProvider === 'function' ? standardsApi.getRegisteredProvider() : null;
    const engine = modules.SurpriseMeEngine;
    const rawQueries = [...(Array.isArray(targetStandards) ? targetStandards : []), standardInputValue, standardsInput].map(value => String(value || '').trim()).filter(Boolean);
    let standard = null;
    ...
    if (standard && provider && engine && typeof engine.buildHood === 'function') {
      try {
        hood = engine.buildHood(provider, standard.id);
      } catch (_) {
        hood = null;
      }
    }
    ...
    const actions = [];
    const addAction = (actionId, label, description, focus) => actions.push({
      actionId,
      label,
      description,
      focus: focus || ''
    ...
    return {
      ...
      standard: standard ? {
        code: _contextualText(standard.code, 80),
        label: _contextualText(standard.label || standard.text, 220)
      } : null,
  ...
  const _contextualPrompt = (state, mode) => {
    ...
    const graphLines = [state.graph.prerequisites.length ? 'Prerequisites: ' + state.graph.prerequisites.join('; ') : '', state.graph.leadsTo.length ? 'Builds toward: ' + state.graph.leadsTo.join('; ') : '', state.graph.related.length ? 'Related: ' + state.graph.related.join('; ') : ''].filter(Boolean).join('\n');
    ...
    return ['You are AlloBot, a context-aware K-12 lesson partner.', mode === 'surprise' ? 'Choose exactly one available action. The choice should feel useful and slightly surprising, but never arbitrary or unsafe.' : 'Choose exactly three distinct available options. Do not execute any option; the teacher will choose later.', 'Use only the action IDs in the catalog. Do not invent an action, standard, prerequisite, or student detail.', 'Current app view: ' + state.activeView, state.gradeLevel ? 'Grade setting: ' + state.gradeLevel : '', state.topic ? 'Current topic or intent: ' + state.topic : '', state.sourceExcerpt ? 'Current source excerpt: ' + state.sourceExcerpt : '', standardLine ? 'Target standard: ' + standardLine : '', state.resourceType ? 'Open resource type: ' + state.resourceType : '', state.dokLevel ? 'Depth target: ' + state.dokLevel : '', state.studentInterests.length ? 'Student interests: ' + state.studentInterests.join(', ') : '', graphLines ? 'Source-provided graph context:\n' + graphLines : 'No graph context is available for this state.', state.latestTitles.length ? 'Recent work titles: ' + state.latestTitles.join('; ') : '', 'AVAILABLE ACTIONS:\n' + catalog, mode === 'surprise' ? 'Return ONLY JSON: {"actionId":"...","title":"...","why":"...","focus":"..."}' : 'Return ONLY JSON: {"options":[{"actionId":"...","title":"...","why":"...","focus":"..."},{"actionId":"...","title":"...","why":"...","focus":"..."},{"actionId":"...","title":"...","why":"...","focus":"..."}]}'].filter(Boolean).join('\n');
  };
  ...
  const _fallbackContextualOptions = state => {
    ...
    return shuffled.slice(0, 3).map(action => Object.assign({}, action, {
      title: action.label,
  ...
  const _runContextualAction = async (option, state) => {
    ...
    const actions = {
      generate_quiz: () => handleGenerate('quiz', null, false, null, {
        rethrowErrors: true
      }),
      generate_glossary: () => handleGenerate('glossary', null, false, null, {
        rethrowErrors: true
      }),
      generate_simplified: () => handleGenerate('simplified', null, false, null, {
        rethrowErrors: true
      }),
      generate_sentence_frames: () => handleGenerate('sentence-frames', null, false, null, {
        rethrowErrors: true
      }),
      generate_analysis: () => handleGenerate('analysis', null, false, null, {
        rethrowErrors: true
      }),
      generate_note_taking: () => handleGenerate('note-taking', null, false, null, {
        rethrowErrors: true
      }),
      generate_anchor_chart: () => handleGenerate('anchor-chart', null, false, null, {
        rethrowErrors: true
      }),
      generate_memory_aid: () => handleGenerate('memory-aid', null, false, null, {
        rethrowErrors: true
      }),
      generate_applied_challenge: () => handleGenerate('applied-challenge', null, false, null, {
        rethrowErrors: true
      }),
      generate_concept_sort: () => handleGenerate('concept-sort', null, false, null, {
        rethrowErrors: true
      }),
      generate_faq: () => handleGenerate('faq', null, false, null, {
        rethrowErrors: true
      }),
      generate_brainstorm: () => handleGenerate('brainstorm', null, false, null, {
        rethrowErrors: true
      })
    };
  ...
  const surpriseMeContextually = async () => {
    ...
    try {
      ...
      const action = state.actions.find(candidate => candidate.actionId === actionId);
      if (action) option = Object.assign({}, action, {
        title: _contextualText(payload.title, 100) || action.label,
        why: _contextualText(payload.why, 220) || action.description,
        focus: _contextualText(payload.focus, 220) || action.focus
      });
    } catch (_) {}
    if (!option) {
      const fallback = _fallbackContextualOptions(state);
  ...
  const suggestContextualNextSteps = async () => {
    ...
    try {
      ...
      options = _fallbackContextualOptions(state);
    }
    if (options.length < 3) options = _normalizeContextualOptions(state, {
      options
    });
    _contextualSuggestionRef.current = {
      options,
      state,
      createdAt: Date.now()
    };
  ...
  const requestTestPrepVoiceControl = (action = 'status') => {
    ...
    return {
      ...
      message: isTestPrepHubOpen ? 'Test Prep voice controls are still loading. Try again in a moment.' : 'Open Test Prep Hub first.'
    };
  };
  const ctx = {
    t,
    addToast,
    callGemini: __live.callGemini,
    commandAudience,
    openSchoolStoreRecognition: () => {
      if (!isTeacherMode || isStudentLinkMode || isParentMode || isIndependentMode) return false;
      if (!normalizeAlloEvaluationPortalUrl(schoolRewardsPortalUrl)) {
        handleOpenSchoolRewards();
        return false;
      }
    ...
    setShowWizard,
    setShowNotebook,
    openTranslateModal: handleSetIsTranslateModalOpenToTrue,
    setShowSessionModal,
    setShowClassAnalytics,
    setIsResearchSuiteOpen,
    setShowExportMenu,
    ...
    openMathFluency: () => {
      ...
      setTimeout(() => {
        try {
          document.getElementById('tour-tool-math')?.scrollIntoView({
            behavior: 'smooth',
            block: 'center'
          });
        } catch (_) {}
    ...
    openBrainstormActivity: mode => {
      ...
      const applyMode = () => {
        ...
        if (typeof setModeBridge === 'function') {
          if (mode) {
            try {
              setModeBridge(mode);
            } catch (_) {}
        ...
        try {
          document.getElementById('tour-tool-brainstorm')?.scrollIntoView({
            behavior: 'smooth',
            block: 'center'
          });
        } catch (_) {}
    ...
    jumpToLatestLessonPlan: latestLessonPlan ? () => {
      try {
        handleRestoreView(latestLessonPlan);
      } catch (_) {}
    } : null,
    ...
    openLeadershipHub: () => {
      setShowEducatorHub(false);
      setShowLearningHub(false);
      if (typeof window.__alloLazyAdminHub === 'function') {
        try {
          window.__alloLazyAdminHub();
        } catch (_) {}
      }
    ...
    setSetupGradeLevel: value => {
      ...
      let next = raw;
      if (/^(k|pre[-\s]?k|kindergarten)$/.test(lower)) next = 'Kindergarten';else if (/graduate/.test(lower)) next = 'Graduate Level';else if (/college|university/.test(lower)) next = 'College';else {
    ...
    zenOn: handleSetIsZenModeToTrue,
    zenOff: handleSetIsZenModeToFalse,
    fontBigger: () => {
      const v = Math.min(48, (sliderFontSize || 16) + 2);
      setSliderFontSize(v);
      return v;
    },
    fontSmaller: () => {
      const v = Math.max(10, (sliderFontSize || 16) - 2);
      setSliderFontSize(v);
      return v;
    },
    resetFontSize,
    ...
    generateQuiz: () => handleGenerate('quiz', null, false, null, {
      rethrowErrors: true
    }),
    generateGlossary: () => handleGenerate('glossary', null, false, null, {
      rethrowErrors: true
    }),
    generateSimplified: cfg => handleGenerate('simplified', null, false, null, Object.assign({
      rethrowErrors: true
    }, cfg || {})),
    generateSentenceFrames: () => handleGenerate('sentence-frames', null, false, null, {
      rethrowErrors: true
    }),
    generateAnalysis: () => handleGenerate('analysis', null, false, null, {
      rethrowErrors: true
    }),
    submitWork: handleSetShowSubmitModalToTrue,
    hasAssignmentDirections: !!assignmentDirections,
    openAssignmentDirections: () => {
      if (assignmentDirections) handleRestoreView(assignmentDirections);
    },
    ...
    openNextAssignmentStep: () => {
      const step = getNextAssignmentStep();
      if (step && step.item) handleRestoreView(step.item);
      return step;
    },
    readAssignmentDirections: () => {
      if (!assignmentDirections) return false;
      handleRestoreView(assignmentDirections);
      setTimeout(() => startReadThisPage(), 80);
      return true;
    },
    getSuccessCriteria,
    getTeacherFeedback,
    sendTeacherSignal: signal => {
      ...
      writeToSession(signalRef, {
        ['roster.' + user.uid + '.signal']: signal,
        ['roster.' + user.uid + '.signalAt']: Date.now()
      }).catch(() => {});
      setShowStudentSignals(false);
      return true;
    },
    ...
    lessonTemplateNames: () => (Array.isArray(lessonTemplates) ? lessonTemplates : []).map(tpl => ({
      id: tpl && tpl.id,
      name: tpl && tpl.name
    })).filter(x => x.id),
    ...
    blueprintStepList: () => {
      ...
      return plan.map((r, i) => ({
        position: i + 1,
        tool: r && (r.tool || r.type),
        uiId: r && (r.uiId || r.stepId)
      })).filter(s => s.uiId);
    },
    runBlueprint: () => {
      setShowUDLGuide(true);
      return handleExecuteBlueprint();
    },
    ...
    rebuildBlueprintStep: position => {
      ...
      if (!row) return null;
      setShowUDLGuide(true);
      return handleRebuildBlueprintStep(row.uiId || row.stepId, {
    ...
    openCommandBlueprintLibrary: () => {
      ...
      setTimeout(() => {
        try {
          handleSendUDLMessage('__allo_plan_library');
        } catch (_) {}
      }, 0);
    },
    ...
    previewStudentAssignment: () => {
      if (!latestStudentPreviewShare) return false;
      openStudentQrPreview(latestStudentPreviewShare.url, 'homework link as a student');
      return true;
    },
    hasResumableWork: !!latestResumableWork,
    resumeLatestWork: () => {
      if (latestResumableWork) handleRestoreView(latestResumableWork);
      return latestResumableWork;
    },
    // Pipeline-aware commands (guarded by `when` in the registry)
    pipelineOpen: !!pdfFixResult,
    getPipelineScore: () => pdfFixResult ? {
      before: Number.isFinite(pdfFixResult.beforeScore) ? pdfFixResult.beforeScore : null,
      after: Number.isFinite(pdfFixResult.afterScore) ? pdfFixResult.afterScore : null,
    ...
    voiceLang: (() => {
      try {
        return __live.getSpeechLangCode(currentUiLanguage) || 'en-US';
      } catch (_) {
        return 'en-US';
      }
    })(),
    voiceSpeakReplies: (() => {
      try {
        return localStorage.getItem('allo_voice_speak_replies') !== 'off';
      } catch (_) {
        return true;
      }
    })(),
    startVoiceLoop: () => {
      ...
      if (!AC || !AC.createVoiceLoop) {
        addToast(t('toasts.voice_module_missing') || 'Voice control is still loading — try again in a moment.', 'info');
        return;
      }
      try {
        if (!window.__alloVoiceLoop) window.__alloVoiceLoop = AC.createVoiceLoop(() => _alloCmdCtxRef.current || _alloCmdCtx(), {
          voiceService: window.AlloFlowVoice,
          callGeminiAudio
        });
    ...
    stopVoiceLoop: () => {
      try {
        if (window.__alloVoiceLoop) window.__alloVoiceLoop.stop();
      } catch (_) {}
    },
    ...
    converse: text => {
      ...
      const drop = () => {
        _voiceConverseWaitersRef.current = _voiceConverseWaitersRef.current.filter(entry => entry !== waiter);
      };
    ...
    whereIs: query => {
      try {
        ...
        document.querySelectorAll('[data-help-key]').forEach(el => {
          ...
          let s = 0;
          if (hay.includes(q)) s = 100;else s = qTokens.filter(w => hay.includes(w)).length * 30;
          if (s > bestScore) {
            bestScore = s;
            best = el;
          }
        ...
        let helpText = '';
        try {
          helpText = (typeof _helpLookup === 'function' ? _helpLookup(best.getAttribute('data-help-key')) || '' : '').split('. ')[0];
        } catch (_) {}
        try {
          best.scrollIntoView({
            behavior: 'smooth',
    ...
    setFontSizeTo: nv => {
      const v = Math.max(10, Math.min(48, Number(nv) || 16));
      setSliderFontSize(v);
      return v;
    },
    prefillTranslateLang: lang => {
      try {
        window.dispatchEvent(new CustomEvent('alloflow:agent-set-translate-lang', {
          detail: {
            lang: String(lang || '')
          }
        }));
    ...
    startPipelineTour: which => {
      try {
        startPipelineTour(which || 'results');
      } catch (_) {}
    },
    ...
    openErrorReporter: () => {
      try {
        if (window.AlloModules && window.AlloModules.ErrorReporter && typeof window.AlloModules.ErrorReporter.openPanel === 'function') {
          window.AlloModules.ErrorReporter.openPanel();
          return;
        }
      } catch (_) {}
      try {
        addToast(t('toasts.error_reporter_loading') || 'The problem reporter is still loading — try again in a moment.', 'info');
      } catch (_) {}
    },
    ...
    openStemTool: id => {
      if (!id) return false;
      _alloRequestStemPlugin(id);
      setStemLabTool(String(id));
      setStemLabTab('explore');
      setShowStemLab(true);
      return true;
    },
    ...
    findReadingBooks: params => {
      ...
      const matches = AC.findReadingMatches(index, params || {}, {
        limit: 4
      });
      if (!matches.length) {
        setIsReadingLibraryOpen(true);
      ...
      if (top.source && top.source.name) details.push(top.source.name);else if (top.sourceId) details.push(top.sourceId);
    ...
    stopReading: (...args) => {
      try {
        if (alloBotRef.current && typeof alloBotRef.current.stopSpeaking === 'function') alloBotRef.current.stopSpeaking('stop-reading-command');
      } catch (_) {}
      stopReadThisPage();
    ...
    cycleReadingTheme: () => {
      const i = READING_THEME_IDS.indexOf(readingTheme);
      const next = READING_THEME_IDS[(i + 1) % READING_THEME_IDS.length];
      setReadingTheme(next);
      return next;
    },
    lineSpacingMore: () => {
      const v = Math.min(2.5, Math.round(((lineHeight || 1.6) + 0.1) * 10) / 10);
      setLineHeight(v);
      return v;
    },
    lineSpacingLess: () => {
      const v = Math.max(1.0, Math.round(((lineHeight || 1.6) - 0.1) * 10) / 10);
      setLineHeight(v);
      return v;
    },
    ...
    toggleCloudSync: () => {
      const wasOn = isCloudSyncEnabled;
      handleCloudToggleClick();
      return wasOn ? 'off' : 'consent';
    },
    ...
    launchFlashcards: () => {
      ...
      launchInteractiveFlashcards('standard');
    },
    ...
    startBingoGame: () => {
      ...
      if (isTeacherMode) handleSetIsBingoGameToTrue();else handleSetIsStudentBingoGameToTrue();
    },
    cycleColorOverlay: () => {
      const order = ['none', 'blue', 'peach', 'yellow'];
      const next = order[(order.indexOf(colorOverlay) + 1) % order.length];
      setColorOverlay(next);
      return next;
    },
    animationsDisabled: disableAnimations,
    adjustVoiceSpeed: delta => {
      const next = Math.round(Math.max(0.5, Math.min(2, (Number(voiceSpeed) || 1) + (Number(delta) || 0))) * 100) / 100;
      setVoiceSpeed(next);
      return next;
    },
    startCrosswordGame: () => {
    ...
    setGlossaryFilterChoice: tier => {
      if (tier === 'academic') handleSetGlossaryFilterToAcademic();else if (tier === 'domain') handleSetGlossaryFilterToDomain();else handleSetGlossaryFilterToAll();
      return tier;
    },
    ...
    generateSourceText: topic => {
      const tp = String(topic || '').trim();
      if (tp) setSourceTopic(tp);
      return handleGenerateSource(tp ? {
        topic: tp
      } : {});
    },
    generateFaq: () => handleGenerate('faq', null, false, null, {
      rethrowErrors: true
    }),
    generateBrainstorm: () => handleGenerate('brainstorm', null, false, null, {
      rethrowErrors: true
    }),
    setGradeLevelChoice: raw => {
      const s = String(raw || '').toLowerCase().trim();
      if (!s) return '';
      if (/^k(indergarten)?$/.test(s)) {
    ...
    toggleContentEditing: () => {
      ...
      if (kind === 'scaffolds' || kind === 'sentence-frames') {
        handleToggleIsEditingScaffolds();
        return 'scaffolds';
      }
      return '';
    ...
    generateNoteTaking: () => handleGenerate('note-taking', null, false, null, {
      rethrowErrors: true
    }),
    generateAnchorChart: () => handleGenerate('anchor-chart', null, false, null, {
      rethrowErrors: true
    }),
    generateMemoryAid: () => handleGenerate('memory-aid', null, false, null, {
      rethrowErrors: true
    }),
    generateAppliedChallenge: () => handleGenerate('applied-challenge', null, false, null, {
      rethrowErrors: true
    }),
    generateConceptSort: () => handleGenerate('concept-sort', null, false, null, {
      rethrowErrors: true
    }),
    contentIsGlossary: !!generatedContent && generatedContent.type === 'glossary',
    resetScaffolds: handleResetScaffolds,
    // self-confirms via setConfirmDialog
    openPersona: handleSetActiveViewToPersona,
    clearWorkspace: handleClearHistory,
    // no internal confirm → registry destructive:true
    restoreLastSettings: restoreIntentSnapshot,
    // self-gates with a "nothing to undo yet" toast
    startNewPdfAudit,
    rerunPipelineFix: () => runAutoFixLoop(),
    // default rounds; runAutoFixLoop has its own re-entry guard
    stopPipelineFix: () => {
      try {
    ...
    pipelineFixRunning: pdfAutoContinueRunning,
    // read-only signal: gates fix-again vs stop
    ...
    spotlightUiLanguage: () => {
      try {
        ...
        try {
          showSpotlight(el, '🌐 ' + (t('cmd.set_ui_language') || 'Interface language'), t('cmd.set_ui_language_hint') || 'Pick your language here.');
        } catch (_) {}
        return true;
      } catch (_) {
        return false;
...
window.AlloModules.buildAlloCommandContext = buildAlloCommandContext;
window.AlloModules.AlloCommandContext = window.AlloModules.buildAlloCommandContext;
console.log('[CDN] AlloCommandContext loaded');
})();
