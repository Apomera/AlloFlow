...
const { baselineMetrics, reviewedAt, reviewWave, revisions } = require('./eppp_native_quality_wave_45_data.cjs');
...
const auditBasename = 'eppp_question_quality_audit_wave_45.json';
...
if (fs.readFileSync(deployBankPath, 'utf8') !== sourceText) throw new Error('Source and deploy EPPP banks must match before wave 45.');
...
if (revisions.length !== 24 || new Set(revisions.map((revision) => revision.id)).size !== 24) throw new Error('Wave 45 needs twenty-four unique revisions.');
const selectedIds = new Set(revisions.map((revision) => revision.id));
const itemsById = new Map(bank.map((item) => [item.id, item]));

// Reapply the frozen after-state for waves 25–44 before this final wave. A
// full expansion replay may have visited older waves first; using each
// wave's data contract here keeps the bank deterministic and lets every
// historical quality test observe its authored wording and options.
for (let waveNumber = 25; waveNumber <= 44; waveNumber += 1) {
...
for (const prior of require('./eppp_native_quality_wave_43_data.cjs').revisions) {
  const item = itemsById.get(prior.id);
  if (!item || item.wordingReviewWave === 'eppp-native-quality-wave-43') continue;
  const priorChoiceRationales = Array(4);
  priorChoiceRationales[prior.expectedAnswerIndex] = prior.rationale;
  for (const [optionIndex, feedback] of Object.entries(prior.feedback || {})) priorChoiceRationales[Number(optionIndex)] = feedback;
  Object.assign(item, {
    prompt: prior.prompt, difficulty: prior.targetDifficulty, choices: [...prior.choices], rationale: prior.rationale,
    choiceRationales: priorChoiceRationales, cognitiveProcess: prior.cognitiveProcess,
    learningObjectiveId: prior.learningObjectiveId, distractorDesign: [...prior.distractorDesign],
    wordingReviewStatus: 'editorial-deep-rewrite-pass', wordingReviewWave: 'eppp-native-quality-wave-43',
    optionFeedbackRefinementWave: 'eppp-native-quality-wave-43', optionFeedbackRefinedAt: prior.reviewedAt || reviewedAt,
    qaReviewedAt: prior.reviewedAt || reviewedAt, clueReviewStatus: 'editorial-pass-after-manual-option-review',
  });
}

// A replay can be interrupted after earlier waves have run but before the
// latest native waves are reached. Restore any Wave 44 items that were
// temporarily overwritten by that older replay before applying this batch.
for (const prior of require('./eppp_native_quality_wave_44_data.cjs').revisions) {
  const item = itemsById.get(prior.id);
  if (!item || item.wordingReviewWave === 'eppp-native-quality-wave-44') continue;
  const priorChoiceRationales = Array(4);
  priorChoiceRationales[prior.expectedAnswerIndex] = prior.rationale;
  for (const [optionIndex, feedback] of Object.entries(prior.feedback || {})) priorChoiceRationales[Number(optionIndex)] = feedback;
  Object.assign(item, {
    ...
    choiceRationales: priorChoiceRationales, cognitiveProcess: prior.cognitiveProcess,
    learningObjectiveId: prior.learningObjectiveId, distractorDesign: [...prior.distractorDesign],
    wordingReviewStatus: 'editorial-deep-rewrite-pass', wordingReviewWave: 'eppp-native-quality-wave-44',
    optionFeedbackRefinementWave: 'eppp-native-quality-wave-44', optionFeedbackRefinedAt: prior.reviewedAt || reviewedAt,
    qaReviewedAt: prior.reviewedAt || reviewedAt, clueReviewStatus: 'editorial-pass-after-manual-option-review',
  });
}
...
for (const revision of revisions) {
  ...
  if (!recognizedReplay && item.prompt !== revision.expectedPrompt) throw new Error(revision.id + ' prompt drifted before wave 45.');
  if (item.answerIndex !== revision.expectedAnswerIndex) throw new Error(revision.id + ' answer position drifted.');
  if (!recognizedReplay && item.difficulty !== revision.expectedDifficulty) throw new Error(revision.id + ' difficulty drifted before wave 45.');
...
console.log('EPPP quality wave 45: rewrote 24 items across 8 domains; selected warnings after = 0.');
