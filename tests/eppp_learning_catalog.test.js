...
describe('EPPP native learning-library catalog', () => {
  ...
  it('catalogs the complete preserved learning library with stable unique IDs', () => {
    expect(catalog.summary).toMatchObject({ chapters: 49, sections: 278, diagrams: 25, diagramPlacements: 52, knowledgeChecks: 91, flashcards: 415, memoryAids: 255 });
  ...
  it('keeps unreviewed content gated while recording complete provenance and learner structure', () => {
    ...
    expect(catalog.chapters.filter((chapter) => chapter.reviewStatus === 'source-reviewed-editorial-pass').map((chapter) => chapter.id)).toEqual(['ch-5', 'ch-6', 'ch-7', 'ch-8', 'ch-9', 'ch-10', 'ch-11', 'ch-12', 'ch-13', 'ch-14', 'ch-15', 'ch-16', 'ch-17', 'ch-18', 'ch-19', 'ch-20', 'ch-21', 'ch-22', 'ch-23', 'ch-24', 'ch-25', 'ch-26', 'ch-27', 'ch-28', 'ch-47', 'ch-49']);
    expect(catalog.chapters.filter((chapter) => chapter.reviewStatus === 'review-required')).toHaveLength(23);
    ...
    expect(qa.summary).toMatchObject({ qaPassedChapters: 0, sourceReviewedChapters: 26, qaPassedFlashcards: 0, qaPassedMemoryAids: 0 });
  });
