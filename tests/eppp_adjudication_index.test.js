...
describe('EPPP cumulative adjudication index', () => {
  it('indexes five batches and fifty unique quarantined candidates', () => {
    ...
    expect(index.summary.batchCount).toBe(5);
    expect(index.summary.adjudicatedCandidates).toBe(50);
    expect(index.summary.minorRevision + index.summary.majorRewrite).toBe(50);
    ...
    expect(index.batches).toHaveLength(5);
    expect(index.items).toHaveLength(50);
  ...
  it('matches the batch reports and excludes every candidate from the native bank', () => {
    const index = read('test_prep/eppp_legacy/adjudication_index.json');
    const reports = ['01', '02', '03', '04', '05'].map((batch) => read(`test_prep/eppp_legacy/adjudication_batch_${batch}.json`));
