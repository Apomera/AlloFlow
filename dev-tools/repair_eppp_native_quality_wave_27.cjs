...
const { baselineMetrics, reviewedAt, reviewWave, revisions } = require('./eppp_native_quality_wave_27_data.cjs');
...
const auditBasename = 'eppp_question_quality_audit_wave_27.json';
...
if (fs.readFileSync(deployBankPath, 'utf8') !== sourceText) throw new Error('Source and deploy EPPP banks must match before wave 27.');
...
if (revisions.length !== 16 || new Set(revisions.map((revision) => revision.id)).size !== 16) throw new Error('Wave 27 needs sixteen unique revisions.');
...
for (const revision of revisions) {
  ...
  if (!recognizedReplay && item.prompt !== revision.expectedPrompt) throw new Error(revision.id + ' prompt drifted before wave 27.');
  if (item.answerIndex !== revision.expectedAnswerIndex) throw new Error(revision.id + ' answer position drifted.');
  if (!recognizedReplay && item.difficulty !== revision.expectedDifficulty) throw new Error(revision.id + ' difficulty drifted.');
...
const audit = {
  ...
  scope: 'Sixteen EPPP items, two per domain, converted from definition or cue-heavy formats into applied or analytic decisions with parallel distractors and concise option-specific feedback.',
  summary: {
...
if (!replayOnly || !fs.existsSync(path.join(outputRoots[0], auditBasename))) {
  for (const outputRoot of outputRoots) writeFile(path.join(outputRoot, auditBasename), auditJson);
}
console.log('EPPP quality wave 27: rewrote 16 items across 8 domains; selected warnings after = 0.');
