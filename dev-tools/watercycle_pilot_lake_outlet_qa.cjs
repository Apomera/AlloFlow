...
// Run: node dev-tools/watercycle_pilot_stream_riffle_qa.cjs
const fs = require('fs');
const path = require('path');
const assert = require('node:assert/strict');
const { chromium } = require('playwright');
const ROOT = process.cwd();
const out = process.argv[2] ? path.resolve(ROOT, process.argv[2]) : path.join(ROOT, 'scratch', 'water-lake-outlet-review');
...
(async () => {
  ...
  try {
    ...
    const outlet=await page.evaluate(()=>{
      const water=pilotReview.scene.getObjectByName('pilot-stream-water'),lake=pilotReview.scene.getObjectByName('pilot-lake-water');
      const path=water.geometry.parameters.path,a=water.geometry.attributes.position;
      const ray=new THREE.Raycaster(),down=new THREE.Vector3(0,-1,0);
      let seamError=0,seamSamples=0,downstreamError=0,downstreamSamples=0;
      const original=new THREE.TubeGeometry(path,96,2.75,8,false),b=original.attributes.position;
      pilotReview.scene.updateMatrixWorld(true);
      for(let i=0;i<=2000;i++){
        const p=path.getPointAt(i/2000),radius=Math.hypot(p.x-268,p.z+104);
        if(radius<34||radius>34.5)continue;
        ray.set(new THREE.Vector3(p.x,500,p.z),down);const hit=ray.intersectObject(water)[0];
        if(hit){seamError=Math.max(seamError,Math.abs(hit.point.y-lake.position.y));seamSamples++;}
      }
      for(let i=0;i<a.count;i++){
        if(Math.hypot(a.getX(i)-268,a.getZ(i)+104)<44)continue;
        const center=path.getPointAt(Math.floor(i/9)/96);
        downstreamError=Math.max(downstreamError,Math.abs(a.getY(i)-(center.y+(b.getY(i)-center.y)*.12)));downstreamSamples++;
      }
      original.dispose();return {seamError,seamSamples,downstreamError,downstreamSamples};
    });
    assert(outlet.seamSamples>2&&outlet.seamError<.012,JSON.stringify(outlet));
    assert(outlet.downstreamSamples>500&&outlet.downstreamError<.00001,JSON.stringify(outlet));
    console.log('Lake outlet measurements',outlet);
    ...
    const crest=await page.evaluate(()=>{
      const scene=pilotReview.scene,r=scene.getObjectByName('pilot-stream-riffles'),water=scene.getObjectByName('pilot-stream-water');scene.updateMatrixWorld(true);
      const a=r.geometry.attributes.position,edge=r.geometry.attributes.riffleEdge,ray=new THREE.Raycaster(),down=new THREE.Vector3(0,-1,0),p=new THREE.Vector3();
      let maxError=0,missing=0,minWidth=Infinity,maxWidth=0;
      for(let i=0;i<a.count;i+=3){p.fromBufferAttribute(a,i).applyMatrix4(r.matrixWorld);ray.set(new THREE.Vector3(p.x,500,p.z),down);const hit=ray.intersectObject(water)[0];if(!hit)missing++;else maxError=Math.max(maxError,Math.abs(p.y-hit.point.y-.085));}
      for(let i=0;i<a.count;i+=2){const width=Math.hypot(a.getX(i+1)-a.getX(i),a.getZ(i+1)-a.getZ(i));minWidth=Math.min(minWidth,width);maxWidth=Math.max(maxWidth,width);}
      return {maxError,missing,minWidth,maxWidth,vertices:a.count,edges:edge.count,
        pairedEdges:Array.from(edge.array).every((v,i)=>v===i%2),
        soft:r.userData.surfaceShader.fragmentShader.includes('sin(clamp(vRiffleEdge, 0.0, 1.0) * 3.14159)'),
        clock:r.userData.surfaceShader.uniforms.pilotStreamTime===water.userData.surfaceShader.uniforms.pilotStreamTime};
    });assert(crest.maxError<.002&&crest.missing===0&&crest.minWidth>.13&&crest.maxWidth-crest.minWidth>.08&&crest.vertices>100&&crest.vertices===crest.edges&&crest.pairedEdges&&crest.soft&&crest.clock,JSON.stringify(crest));
    ...
    await page.evaluate(()=>{
      ...
      renderer.render=(scene,camera)=>{camera.position.set(250,26,-63);camera.lookAt(241,8.6,-85);return render(scene,camera);};
    });
    ...
    const iceContact=await page.evaluate(()=>{
      const ice=pilotReview.scene.getObjectByName('pilot-stream-ice'),water=pilotReview.scene.getObjectByName('pilot-stream-water');
      return {separateGeometry:ice.geometry!==water.geometry,
        aligned:Array.from(ice.geometry.attributes.position.array).every((v,i)=>v===water.geometry.attributes.position.array[i]),
        clearance:ice.position.y-water.position.y};
    });assert.deepEqual(iceContact,{separateGeometry:true,aligned:true,clearance:.025});
