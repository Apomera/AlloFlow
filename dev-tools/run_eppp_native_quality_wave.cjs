...
function runNativeQualityWave({ dataFile, waveNumber }) {
  ...
  const outputBasename = `eppp_native_quality_audit_wave_${waveNumber}`;
  ...
  if (fs.readFileSync(deployPath, 'utf8') !== sourceText) {
    throw new Error(`Source and deploy EPPP banks must match before wave ${waveNumber}.`);
  }
  ...
  if (fs.readFileSync(deployCatalogPath, 'utf8') !== catalogText) {
    throw new Error(`Source and deploy reference catalogs must match before wave ${waveNumber}.`);
  }
...
`;
  ...
  console.log(`EPPP native quality wave ${waveNumber}: ${auditItems.length} rewritten; `
