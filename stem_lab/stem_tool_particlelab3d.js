...
(function () {
  ...
  function advanceParticles(particles, settings, dt) {
    ...
    for (i = 0; i < particles.length; i += 1) {
      ...
      if (isTransportPreset(settings.preset) && settings.membrane && previousX * p.x < 0) {
        ...
        if (transitSignal > effectivePermeability) { p.x = previousX < 0 ? -radius : radius; p.vx = previousX < 0 ? -Math.abs(p.vx) : Math.abs(p.vx); membraneBlocked += 1; }
      ...
      ['x', 'y', 'z'].forEach(function (axis) {
        var vel = 'v' + axis;
        if (p[axis] > half - radius) { impulse += particleMass * Math.abs(p[vel]) * 2; p[axis] = half - radius; p[vel] = -Math.abs(p[vel]); collisions += 1; if (events.length < 8) events.push({ kind: 'wall', axis: axis, side: 1, x: p.x, y: p.y, z: p.z, power: particleMass * Math.abs(p[vel]) }); }
        if (p[axis] < -half + radius) { impulse += particleMass * Math.abs(p[vel]) * 2; p[axis] = -half + radius; p[vel] = Math.abs(p[vel]); collisions += 1; if (events.length < 8) events.push({ kind: 'wall', axis: axis, side: -1, x: p.x, y: p.y, z: p.z, power: particleMass * Math.abs(p[vel]) }); }
      });
    }
  ...
  function diffusionMetrics(particles, boxSize) {
    var leftA = 0, leftB = 0, rightA = 0, rightB = 0, countA = 0, countB = 0, ax = 0, ay = 0, az = 0, bx = 0, by = 0, bz = 0, msdSum = 0;
    ...
    particles.forEach(function (p) {
      ...
      var dx0 = p.x - p.homeX, dy0 = p.y - p.homeY, dz0 = p.z - p.homeZ; msdSum += dx0 * dx0 + dy0 * dy0 + dz0 * dz0;
    ...
    return { mixing: clamp(1 - Math.abs(leftAFraction - rightAFraction), 0, 1), entropy: clamp(entropy, 0, 1), msd: msdSum / total, exchange: (rightA + leftB) / total, leftA: leftAFraction, rightA: rightAFraction, leftCount: leftN, rightCount: rightN, profileA: profileA, profileTotal: profileTotal, centerA: { x: ax, y: ay, z: az }, centerB: { x: bx, y: by, z: bz }, spreadA: Math.sqrt(spreadA2 / Math.max(1, countA)), spreadB: Math.sqrt(spreadB2 / Math.max(1, countB)), solventRight: countA ? rightA / countA : 0, osmoticShift: (countA ? rightA / countA : 0) - 0.5, soluteLeak: countB ? leftB / countB : 0 };
  }
  ...
  window.__alloParticleLabPure = { seeded: seeded, makeParticles: makeParticles, advanceParticles: advanceParticles, systemMoments: systemMoments, diffusionMetrics: diffusionMetrics, compareTrials: compareTrials, speedDistribution: speedDistribution, metrics: metrics, seriesTrend: seriesTrend, speedHeat: speedHeat };
  ...
  window.StemLab.registerTool('particleLab3d', {
    ...
    render: function (ctx) {
      ...
      var [diffusionInfo, setDiffusionInfo] = useState({ mixing: 0, exchange: 0, leftA: 1, rightA: 0, leftCount: 0, rightCount: 0, profileA: new Array(10).fill(0.5), profileTotal: new Array(10).fill(0), entropy: 0, msd: 0, coefficient: 0, elapsed: 0, spreadA: 0, spreadB: 0, solventRight: 0.5, osmoticShift: 0, soluteLeak: 0, milestoneTime: null, flux: 0, fluxA: 0, fluxB: 0, blocked: 0 });
      ...
      useEffect(function () {
        ...
        function animate(now) {
          ...
          if (now - lastUiRef.current > 400) {
            ...
            var diffusion = liveDiffusion; diffusion.elapsed = simulationElapsed; diffusion.coefficient = diffusion.msd / (6 * Math.max(0.001, simulationElapsed)); diffusion.flux = membranePassedTotal / Math.max(0.001, metricElapsed); diffusion.blocked = membraneBlockedTotal / Math.max(0.001, metricElapsed); diffusion.fluxA = membranePassedATotal / Math.max(0.001, metricElapsed); diffusion.fluxB = membranePassedBTotal / Math.max(0.001, metricElapsed); if (transportMilestone == null && ((settingsRef.current.preset === 'diffusion' && diffusion.mixing >= 0.8) || (settingsRef.current.preset === 'osmosis' && diffusion.osmoticShift >= 0.05 && diffusion.soluteLeak < 0.15))) transportMilestone = simulationElapsed; diffusion.milestoneTime = transportMilestone;
            setStats(next); setDistribution(speedDistribution(particles, 12)); setDiffusionInfo(diffusion); setHistory(function (old) { return old.concat([{ temperature: m.temperature, pressure: m.pressure, energy: m.energy, collisions: collisionTotal, mixing: diffusion.mixing, entropy: diffusion.entropy, solventRight: diffusion.solventRight, soluteLeak: diffusion.soluteLeak, elapsed: diffusion.elapsed }]).slice(-36); });
      ...
      function recordTrial() {
        var trial = { id: Date.now(), preset: preset, temperature: stats.temperature, temperatureSetpoint: temperature, pressure: stats.pressure, count: count, boxSize: boxSize, attraction: attraction, gravity: gravity, membrane: membrane, permeability: permeability, membraneSelectivity: membraneSelectivity, massRatioB: massRatioB, mixing: diffusionInfo.mixing, milestoneTime: diffusionInfo.milestoneTime, osmoticShift: diffusionInfo.osmoticShift, soluteLeak: diffusionInfo.soluteLeak, entropy: diffusionInfo.entropy, msd: diffusionInfo.msd, coefficient: diffusionInfo.coefficient, exchange: diffusionInfo.exchange };
      ...
      function modelSummary() {
        ...
        if (preset === 'diffusion' && massRatioB > 1.35 && membraneSelectivity === 'both') return 'Particle B is heavier, so equal-temperature B particles have a lower typical speed. Compare the A and B crossing fluxes and cloud spread.';
        if (preset === 'diffusion' && massRatioB < 0.75 && membraneSelectivity === 'both') return 'Particle B is lighter, so equal-temperature B particles have a higher typical speed. Compare the A and B crossing fluxes and cloud spread.';
          ...
          h('aside', { className: 'space-y-4' },
            ...
            h('div', { className: 'rounded-2xl border border-slate-200 bg-white p-4 shadow-sm' },
              ...
              transportMode && h('div', { className: 'mt-3 rounded-xl border border-violet-200 bg-gradient-to-br from-slate-950 to-violet-950 p-3 text-white', role: 'img', 'aria-label': (preset === 'osmosis' ? 'Osmosis monitor. Net solvent shift ' + Math.round(diffusionInfo.osmoticShift * 100) + ' percent and solute leakage ' + Math.round(diffusionInfo.soluteLeak * 100) + ' percent.' : 'Diffusion monitor. Mixing index ' + Math.round(diffusionInfo.mixing * 100) + ' percent.') + ' Particle B has a relative mass of ' + massRatioB.toFixed(1) + ' times particle A. Particle A concentration is ' + Math.round(diffusionInfo.leftA * 100) + ' percent on the left and ' + Math.round(diffusionInfo.rightA * 100) + ' percent on the right. Particle A crossing flux ' + diffusionInfo.fluxA.toFixed(1) + ' per second and particle B crossing flux ' + diffusionInfo.fluxB.toFixed(1) + ' per second. Spatial mixing entropy is ' + Math.round(diffusionInfo.entropy * 100) + ' percent. Mean squared displacement is ' + diffusionInfo.msd.toFixed(1) + ' square model units. ' + (diffusionInfo.milestoneTime == null ? 'The transport milestone has not been reached.' : 'The transport milestone was reached at ' + diffusionInfo.milestoneTime.toFixed(1) + ' seconds.') },
                ...
                h('div', { className: 'mt-3 grid grid-cols-3 gap-1.5 text-center' }, (preset === 'osmosis' ? [['Solvent shift', (diffusionInfo.osmoticShift >= 0 ? '+' : '') + Math.round(diffusionInfo.osmoticShift * 100) + '%'], ['Solute leak', Math.round(diffusionInfo.soluteLeak * 100) + '%'], ['D*', diffusionInfo.coefficient.toFixed(2)]] : [['Entropy', Math.round(diffusionInfo.entropy * 100) + '%'], ['MSD', diffusionInfo.msd.toFixed(1)], ['D*', diffusionInfo.coefficient.toFixed(2)]]).map(function (item) { return h('div', { key: item[0], className: 'rounded-lg bg-white/[0.06] p-2' }, h('div', { className: 'text-[8px] font-black uppercase tracking-wide text-violet-300' }, item[0]), h('div', { className: 'mt-0.5 font-mono text-xs font-black text-white' }, item[1])); })),
