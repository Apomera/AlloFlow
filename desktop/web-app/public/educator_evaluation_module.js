...
function EducatorEvaluationPanel(props) {
  ...
  const exportEducatorPacket = () => {
    ...
    const obsRows = (packet.observations || []).map((item, index) => '<section class="rec"><h3>Observation ' + aeEsc(aeDate(item.date || item.publishedAt || packet.issuedAt)) + '</h3><label class="lbl" for="ae-refl-' + index + '">Your reflection on this observation (optional)</label><textarea id="ae-refl-' + index + '" data-record="' + aeEsc(item.id) + '" rows="4"></textarea></section>').join("");
    const readable = '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Your evaluation</title><style>body{font:16px/1.6 system-ui,sans-serif;color:#172033;background:#f6f8fb;max-width:780px;margin:0 auto;padding:24px}h1{color:#173e70;font-size:1.6rem}h2{color:#173e70;font-size:1.15rem;margin-top:28px}h3{font-size:1rem;margin:0 0 8px}.card{background:#fff;border:1px solid #d7dee8;border-radius:10px;padding:18px;margin:16px 0}.notice{background:#eef6ff;border:1px solid #93b8e8}.lbl{display:block;font-weight:600;margin:12px 0 6px}textarea{width:100%;font:inherit;padding:10px;border:1px solid #94a3b8;border-radius:8px;min-height:96px}.ackrow{display:flex;align-items:flex-start;gap:12px;margin:14px 0}.ackrow input{width:24px;height:24px;flex:0 0 auto;margin:0}button{font:inherit;font-weight:700;min-height:48px;padding:12px 20px;border:0;border-radius:10px;background:#1d4ed8;color:#fff;cursor:pointer}button:focus-visible,textarea:focus-visible,input:focus-visible{outline:3px solid #b45309;outline-offset:2px}.fine{font-size:.9rem;color:#475569;margin:6px 0 14px}.done{margin-top:12px;font-weight:600;color:#14532d}</style></head><body><h1>Your evaluation</h1><p><strong>' + aeEsc(who) + "</strong> · " + aeEsc(packet.config.organization) + " · " + aeEsc(packet.config.academicYear) + '</p><p class="card notice">This file contains only your own records. You can read it as it is. If you would like to respond, use the form below and send the file it downloads back to your evaluator. Nothing is uploaded from this page, and no account is needed.</p><h2>Add your response</h2><div class="card"><label class="lbl" for="ae-statement">Your statement (in your own words)</label><textarea id="ae-statement" rows="6"></textarea>' + obsRows + '<div class="ackrow"><input type="checkbox" id="ae-ack"><label for="ae-ack">I have read this evaluation.</label></div><p class="fine">Ticking this records that you read it. It is not the signature on the evaluation form: your district’s process still asks you and your evaluator to sign at the conference.</p><button type="button" id="ae-send">Download my response</button><p class="done" id="ae-status" role="status"></p></div><p>Issued ' + aeEsc(aeDateTime(packet.issuedAt)) + '.</p><script type="application/json" id="' + AE_PACKET_SCRIPT_ID + '">' + aePacketEmbed(JSON.stringify(packet)) + "<\/script><script>" + AE_PACKET_FORM_JS + "<\/script></body></html>";
  ...
  const importRubric = (file) => {
    if (!file || file.size > 1024 * 1024) {
      addToast("Rubric import failed: choose a JSON file under 1 MB.", "error");
      return;
    }
    ...
    reader.onload = () => {
      ...
      try {
        ...
        addToast("Rubric import failed: that file is not valid JSON.", "error");
        return;
      }
      if (!rubric) {
