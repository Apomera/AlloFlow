...
const wave = require('../dev-tools/eppp_native_quality_wave_31_data.cjs');
...
describe('EPPP expanded quality wave 31', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_31.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 22, applicationItems: 18, analysisItems: 6, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    expect(audit.summary.distractorWarningsAfter.uniqueKeyStemLexicalLeakageCandidates).toBe(audit.summary.distractorWarningsBefore.uniqueKeyStemLexicalLeakageCandidates - 3);
    expect(audit.summary.distractorWarningsAfter.asymmetricExtremeDistractorCandidates).toBe(audit.summary.distractorWarningsBefore.asymmetricExtremeDistractorCandidates - 2);
    expect(audit.summary.distractorWarningsAfter.advancedDirectRecallCandidates).toBe(audit.summary.distractorWarningsBefore.advancedDirectRecallCandidates);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_31.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_31.json'));
  });

  it('replays after wave 30', () => {
