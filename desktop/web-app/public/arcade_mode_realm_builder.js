(function () {
  ...
  function normalizeClassPalace(raw) {
    ...
    var seenRooms = {};
    var rooms = (Array.isArray(raw.rooms) ? raw.rooms : []).slice(0, CMP_MAX_ROOMS).map(function(room, i) {
      room = room && typeof room === 'object' ? room : {};
      var id = cmpText(room.id, 80) || ('room-' + (i + 1));
      if (seenRooms[id]) id = id + '-' + (i + 1);
      seenRooms[id] = true;
      return { id: id, title: cmpText(room.title, 80) || ('Room ' + (i + 1)), createdBy: cmpText(room.createdBy, 80) };
    });
    if (!rooms.length) rooms.push({ id: 'room-1', title: 'Entrance', createdBy: '' });
    var roomIds = {};
    rooms.forEach(function(room) { roomIds[room.id] = true; });
    var loci = (Array.isArray(raw.loci) ? raw.loci : []).slice(0, CMP_MAX_LOCI).map(function(locus, i) {
      ...
      return {
        ...
        roomId: roomIds[locus.roomId] ? locus.roomId : rooms[0].id,
    ...
    return {
      ...
      rooms: rooms,
      loci: loci,
      submissions: raw.submissions && typeof raw.submissions === 'object' && !Array.isArray(raw.submissions)
        ? raw.submissions : {}
    };
  }

  function buildClassPalaceData(raw) {
    var palace = normalizeClassPalace(raw);
    return {
      main: palace.title,
      structureType: 'Memory Palace',
      branches: palace.rooms.map(function(room) {
  ...
  function ClassMemoryPalaceLauncher(props) {
    ...
    var minutes = 15;
    var cost = Math.ceil(minutes / (ctx.minutesPerToken || 5));
    var canAfford = ctx.tokens >= cost;
    ...
    if (!ctx.sessionCode) {
      ...
      action = h('button', {
        onClick: function() { launch('host'); }, disabled: disabled || !canAfford,
        'aria-label': open ? 'Reopen the class memory palace as teacher' : 'Set up a class memory palace',
        style: cmpButton(palette, true, disabled || !canAfford)
      }, open ? 'Reopen class palace' : 'Set up class palace');
    } else if (open) {
      action = h('button', {
        onClick: function() { launch('student'); }, disabled: disabled || !canAfford,
      ...
      h('div', { style: { display: 'flex', gap: '12px', alignItems: 'center' } },
        ...
        h('div', { style: { flex: 1 } },
          ...
          h('div', { style: { color: palette.textDim || '#cbd5e1', fontSize: '11px', lineHeight: 1.45 } },
            open ? ((shared.title || 'Class Memory Palace') + ' · ' + ((shared.loci || []).length) + ' approved loci')
              : 'Teacher setup, student contributions, one shared 3D recall walk.')
        ),
        action
      )
    );
  ...
  function ClassMemoryPalaceMain(props) {
    ...
    var submissions = rawPalace && rawPalace.submissions && typeof rawPalace.submissions === 'object'
      ? rawPalace.submissions : {};
    var pendingKeys = Object.keys(submissions).filter(function(key) { return !!normalizePalaceSubmission(submissions[key]); }).slice(0, CMP_MAX_PENDING);
    ...
    useEffect(function() {
      if (palace && palace.rooms.length && !roomId) setRoomId(palace.rooms[0].id);
    }, [palace && palace.activityId, palace && palace.rooms.length]);
    ...
    function startPalace() {
      ...
      var roomNames = String(roomsDraft || '').split(/\r?\n/).map(function(v) { return cmpText(v, 80); }).filter(Boolean).slice(0, CMP_MAX_ROOMS);
      if (!title || !topic || !roomNames.length) {
        ctx.addToast('Add a palace title, learning focus, and at least one starter room.');
        return;
      }
      setBusy(true);
      ...
      write({
        ...
        rooms: roomNames.map(function(name, i) { return { id: 'room-' + (i + 1), title: name, createdBy: 'Teacher' }; }),
        loci: [], submissions: {}
      }).then(function() {
    ...
    function submitContribution() {
      ...
      fields.submissions[sub.id] = sub;
      write(fields).then(function() {
        stopVoiceContribution();
        setLabel(''); setMnemonic(''); setNewRoomTitle('');
        ctx.addToast('Contribution sent to your teacher for review.');
      }).catch(function() {
    ...
    function approveSubmission(key, provided, direct) {
      ...
      var targetRoom = sub.roomId && nextRooms.filter(function(room) { return room.id === sub.roomId; })[0];
      if (!targetRoom && sub.newRoomTitle) {
        targetRoom = nextRooms.filter(function(room) { return room.title.toLowerCase() === sub.newRoomTitle.toLowerCase(); })[0];
        if (!targetRoom && nextRooms.length < CMP_MAX_ROOMS) {
          targetRoom = { id: cmpId('room'), title: sub.newRoomTitle, createdBy: sub.contributorNickname };
          nextRooms.push(targetRoom);
        }
      }
      if (!targetRoom) {
        ctx.addToast(nextRooms.length >= CMP_MAX_ROOMS ? 'The palace already has the maximum number of rooms.' : 'That room is no longer available.');
        return;
      }
      if (palace.loci.length >= CMP_MAX_LOCI) {
        ctx.addToast('The palace has reached its 96-locus activity limit.');
        return;
      }
      var nextLoci = palace.loci.concat([{
        id: cmpId('locus'), roomId: targetRoom.id, label: sub.label, mnemonic: sub.mnemonic, role: sub.role,
      ...
      if (!direct) { fields.submissions = {}; fields.submissions[key] = null; }
      write(fields).then(function() {
    ...
    function rejectSubmission(key) {
      if (!isHost || busy) return;
      var fields = { submissions: {} };
      fields.submissions[key] = null;
      setBusy(true);
      write(fields).then(function() { ctx.addToast('Contribution returned without adding it.'); })
    ...
    function walkPalace() {
      if (!palace || !palace.loci.length) {
        ctx.addToast('Approve at least one memory anchor before opening the 3D walk.');
        return;
      }
      ...
      ctx.openMemoryPalaceWalk(buildClassPalaceData(palace), {
        title: palace.title,
        ariaLabel: 'Walk ' + palace.title + ' in 3D',
    ...
    if (isHost && !isOpen) {
      return h('section', { 'aria-label': 'Set up class memory palace', style: { padding: '14px', color: palette.text || '#e2e8f0' } },
        ...
        h('p', { style: { margin: '0 0 14px', color: palette.textDim || '#cbd5e1', fontSize: '12px', lineHeight: 1.5 } },
          'Name the learning focus and starter rooms. Students can then propose anchors for those rooms or suggest a new room.'),
    ...
    return h('section', { 'aria-label': palace.title, style: { padding: '14px', color: palette.text || '#e2e8f0' } },
      h('div', { style: { display: 'flex', gap: '10px', alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: '12px' } },
        h('div', { style: { flex: 1, minWidth: '220px' } },
          ...
          h('div', { role: 'status', 'aria-live': 'polite', style: { marginTop: '4px', color: palette.textMute || '#94a3b8', fontSize: '11px' } },
            palace.rooms.length + ' rooms · ' + palace.loci.length + ' approved loci' + (isHost ? (' · ' + pendingKeys.length + ' awaiting review') : ''))
        ),
        h('button', { onClick: walkPalace, disabled: !palace.loci.length, style: cmpButton(palette, true, !palace.loci.length) }, 'Walk in 3D'),
        isHost
      ...
      h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: '10px', marginBottom: '14px' } },
        palace.rooms.map(function(room) {
          var roomLoci = palace.loci.filter(function(locus) { return locus.roomId === room.id; });
      ...
      h('fieldset', { disabled: busy, style: { border: '1px solid ' + (palette.border || '#334155'), borderRadius: '10px', padding: '12px', margin: '0 0 14px' } },
        h('legend', { style: { fontWeight: 800, padding: '0 6px' } }, isHost ? 'Add an anchor' : 'Propose an anchor'),
        cmpLabeledField(h, palette, 'cmp-role', 'Your group role', h('select', { id: 'cmp-role', value: contributionRole, onChange: function(e) { setContributionRole(e.target.value); }, style: cmpField(palette) },
        ...
        cmpLabeledField(h, palette, 'cmp-room', 'Room', h('select', { id: 'cmp-room', value: roomId, onChange: function(e) { setRoomId(e.target.value); }, style: cmpField(palette) },
          palace.rooms.map(function(room) { return h('option', { key: room.id, value: room.id }, room.title); }),
        ...
        h('div', { style: { display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap', margin: '-2px 0 10px' } },
          h('button', {
            type: 'button', onClick: toggleVoiceContribution, disabled: busy,
            ...
            style: Object.assign({}, cmpButton(palette, false, busy), { minHeight: '40px', padding: '6px 10px', fontSize: '11px' })
        ...
        h('button', { onClick: submitContribution, disabled: busy, style: cmpButton(palette, true, busy) },
          busy ? 'Saving…' : (isHost ? 'Add to palace' : 'Send for teacher review'))
      ),
      ...
      isHost ? h('section', { 'aria-label': 'Contributions awaiting teacher review', style: { border: '1px solid ' + (palette.border || '#334155'), borderRadius: '10px', padding: '12px' } },
        ...
        pendingKeys.length ? pendingKeys.map(function(key) {
          ...
          return h('article', { key: key, style: { padding: '10px', marginBottom: '8px', borderRadius: '8px', background: palette.surface || '#1e293b' } },
            h('div', { style: { fontWeight: 800, fontSize: '12px' } }, sub.label + ' · ' + cmpRoleLabel(sub.role) + ' · ' + sub.contributorNickname),
            ...
            h('div', { style: { display: 'flex', gap: '8px' } },
              ...
              h('button', { onClick: function() { rejectSubmission(key); }, disabled: busy, style: cmpButton(palette, false, busy) }, 'Return')
            )
          );
