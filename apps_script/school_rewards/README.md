...
Staff mint a batch of codes from the Award tab (points, category, reason, count, optional expiry). Each code is a random token id stored in `ClaimTokens`; the printed QR or link is `<web-app>/exec?claim=<token>` and carries nothing else. A signed-in student who opens it is credited once from the row, never from the URL: the token id is the idempotency key, the claim runs through the same intent journal as awards, and a second scan by anyone sees `already_redeemed`. Expired, voided and unknown codes return a state instead of an error. Void a lost printout with `voidSchoolRewardsClaimBatch`; redeemed codes keep their ledger entries. Design note: `docs/SINGLE_USE_CLAIM_QR_DESIGN.md`.
