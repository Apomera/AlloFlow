...
const wave = require('../dev-tools/eppp_native_quality_wave_39_data.cjs');
...
describe('EPPP expanded quality wave 39', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_39.json');
    ...
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(439);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1221);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(587);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(582);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_39.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_39.json'));
  });

  it('replays after wave 38', () => {
    const builder = read('dev-tools/build_eppp_1500_expansion.cjs');
    const i38 = builder.indexOf('repair_eppp_native_quality_wave_38.cjs');
