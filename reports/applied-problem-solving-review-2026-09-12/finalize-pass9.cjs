...
const broad=read('pass9-regression.json'),recheck=read('pass9-final-recheck.json'),browser=read('pass9-browser-results.json');
...
if(!regression.success||regression.numFailedTests||browser.errors.length||browser.states.some(state=>state.violations.length||state.width!==state.pageWidth))throw Error('Validation is incomplete');
...
if(mirrors.some(item=>!item.matches))throw Error('Public assets differ');
const summary={date:'2026-09-19',passed:regression.numPassedTests,suites:regression.testResults.length,browserStates:browser.states.length,pageErrors:browser.errors.length,axeViolations:0,horizontalOverflow:0,mirrors,liveAi:false,liveSearch:false,learnerSessions:0,deployed:false};
fs.writeFileSync(path.join(__dirname,'pass9-validation-summary.json'),JSON.stringify(summary,null,2)+'\n');
const report=`# Pass 9: keep sources separate from reasoning
...
`;
fs.writeFileSync(path.join(__dirname,'PASS9.md'),report);
