        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 sm:p-5 space-y-3 custom-scrollbar min-w-0">
          ...
          {Array.isArray(rosterKey?.sessionHistory) && rosterKey.sessionHistory.length > 0 && (
            <details className="rounded-xl border border-cyan-200 bg-cyan-50/60 p-3">
              ...
              <div role="group" className="mt-3 rounded-lg border border-cyan-100 bg-white p-2" aria-label="Saved session history filters">
                      ...
                      {session.insightBrief && (
                        <div className="rounded-lg border border-indigo-100 bg-indigo-50/70 p-2">
                          ...
                          {Array.isArray(session.insightBrief.evidenceCohorts) && session.insightBrief.evidenceCohorts.length > 0 && (
                            <div role="group" className="mt-2 space-y-1" aria-label="Saved evidence cohorts">
...
const StudentConceptQuestOverlay = React.memo(({ sessionData, user, activeSessionCode, targetAppId, t, playSound }) => {
  ...
  useEffect(() => {
    setAnswerIndex(null); setLocalAction(null); setQuestSyncError(''); setIsSubmitting(false); questWriteRef.current = '';
  }, [actionScope]);
  useEffect(() => {
    if (!myAction) return;
    setAnswerIndex(myAction.answerIndex); setAbilityId(myAction.abilityId);
    setSupportId(myAction.supportId || 'clarify'); setSupportTargetUid(myAction.supportTargetUid || '');
  ...
  const writeChoice = async (field, value) => {
    ...
    questWriteRef.current = requestScope; setQuestSyncError('');
    ...
    try {
      ...
      playSound?.('click');
    } catch (error) { warnLog('Concept Quest action failed:', error); if (currentScopeRef.current === requestScope) setQuestSyncError(tr('choice_sync_failed', 'Your choice could not be saved. Try again when the connection returns.')); }
    finally { if (questWriteRef.current === requestScope) { questWriteRef.current = ''; setIsSubmitting(false); } }
  };
  ...
  return (
    <div className="fixed inset-0 z-[9999] overflow-auto bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 text-white" role="region" aria-labelledby="concept-quest-title">
      {questSyncError && <p role="alert" className="m-3 rounded-lg bg-red-950 p-3 text-red-100">{questSyncError}</p>}
      {escapeState.isPaused && <div ref={pauseFocusRef} tabIndex={-1} role="status" className="sticky top-0 z-30 border-b border-amber-300 bg-amber-100 p-3 text-center font-bold text-amber-900">{tr('paused_notice', 'Quest paused. Your choices are saved while the teacher prepares the next step.')}</div>}
      <header className="sticky top-0 z-20 border-b border-indigo-400/30 bg-slate-950/95 p-3 backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3">
          ...
          <div role="group" className="flex flex-wrap gap-3 text-sm font-bold" aria-label={tr('party_health_aria', 'Party health {current} of {maximum}', { current: quest.party.hp, maximum: quest.party.maxHp })}><span>❤️ {quest.party.hp}/{quest.party.maxHp}</span><span>🛡️ {quest.party.shield || 0}</span><span>✨ {quest.party.xp || 0} XP</span><span>🔷 {(quest.sigils || []).length}/{neededSigils}</span></div>
        </div>
        <div role="progressbar" aria-label={tr('party_hp', 'Party HP')} aria-valuemin={0} aria-valuemax={quest.party.maxHp} aria-valuenow={quest.party.hp} className="mx-auto mt-2 h-2 max-w-6xl overflow-hidden rounded-full bg-slate-700"><div className="h-full bg-emerald-500 transition-all motion-reduce:transition-none" style={{ width: `${partyHpPercent}%` }} /></div>
      </header>
      <main className="mx-auto grid max-w-6xl gap-5 p-4 lg:grid-cols-[1.25fr_.75fr]">
        <section className="rounded-2xl border border-indigo-400/30 bg-slate-900/80 p-4" aria-labelledby="quest-map-heading">
          ...
          <div role="group" className="relative grid grid-cols-2 gap-2 rounded-xl border border-slate-700 bg-slate-950/70 p-2 sm:block sm:h-[360px] sm:p-0" aria-label={tr('map_aria', 'Eight-room game map')}>
            <svg aria-hidden="true" className="pointer-events-none absolute inset-0 hidden h-full w-full sm:block" viewBox="0 0 100 100" preserveAspectRatio="none">{rooms.flatMap(room => (room.neighbors || []).filter(id => id > room.id).map(id => { const other = rooms.find(entry => entry.id === id); return other ? <line key={room.id + id} x1={room.x} y1={room.y} x2={other.x} y2={other.y} stroke="#64748b" strokeWidth="0.5"/> : null; }))}</svg>
            {rooms.map(room => {
          ...
          <div className="mt-3 rounded-xl bg-indigo-950/60 p-3"><p className="font-bold">{currentRoom?.emoji} {currentRoom?.name}</p><p className="text-sm text-indigo-100">{tr('concept_label', 'Concept: {concept}', { concept: currentRoom?.concept || '' })}</p>
            <div className="mt-3"><p className="mb-2 text-sm font-bold">{tr('choose_role', 'Choose a party role')}</p><div className="grid grid-cols-2 gap-2">{(quest.roles || engine.ROLES || []).map(role => <button key={role.id} type="button" disabled={!!myAction || isSubmitting || escapeState.isPaused || !['explore', 'battle'].includes(quest.phase)} onClick={() => { setAbilityId(role.abilityId); writeChoice('questRoles', role.id); }} aria-pressed={myRole === role.id} className={`min-h-14 rounded-lg border p-2 text-left text-xs focus:ring-4 focus:ring-yellow-300 ${myRole === role.id ? 'border-emerald-300 bg-emerald-900' : 'border-slate-600 bg-slate-800'}`}><strong>{role.emoji} {gameLabel(role, 'role')}</strong><span className="mt-1 block text-slate-300">{gameLabel(role, 'role', 'description')}</span></button>)}</div></div>
            {quest.phase === 'explore' && <div className="mt-3"><p className="mb-2 text-sm">{tr('vote_instruction', 'Vote for a connected room. Earn {count} concept sigils to unlock the Mastery Gate.', { count: neededSigils })}</p><div className="flex flex-wrap gap-2">{adjacent.map(room => { const locked = room.kind === 'boss' && (quest.sigils || []).length < (neededSigils); return <button key={room.id} type="button" disabled={isSubmitting || escapeState.isPaused || locked} onClick={() => writeChoice('questVotes', room.id)} className={`min-h-11 rounded-lg px-3 py-2 text-sm font-bold focus:ring-4 focus:ring-yellow-300 disabled:opacity-50 ${myVote === room.id ? 'bg-yellow-400 text-slate-950' : 'bg-indigo-700 hover:bg-indigo-600'}`}>{locked ? '🔒' : room.emoji} {room.name}</button>; })}</div>{myVote && <p role="status" className="mt-2 text-sm text-yellow-200">{tr('vote_recorded', 'Your vote is recorded.')}</p>}</div>}
          </div>
        </section>
        <section className={`space-y-4 lg:col-start-2 lg:row-start-1 ${quest.phase === 'battle' ? 'order-first lg:order-none' : ''}`} aria-label={tr('current_encounter_aria', 'Current encounter')}>
          <h3 ref={encounterFocusRef} tabIndex={-1} className="text-lg font-black focus:outline-none focus:ring-2 focus:ring-yellow-300">{tr('phase_' + quest.phase, quest.phase === 'battle' ? 'Choose your action' : quest.phase === 'explore' ? 'Choose a path together' : quest.phase === 'defeat' ? 'Regroup with your teacher' : 'Quest complete')}</h3>
          {isSubmitting && <p role="status" className="text-sm text-cyan-200">{tr('saving_choice', 'Saving your choice…')}</p>}
          ...
          {quest.phase === 'battle' && currentEnemy && <div className="rounded-2xl border border-fuchsia-400/40 bg-slate-900 p-4">
            ...
            <div role="progressbar" aria-valuemin={0} aria-valuemax={currentEnemy.maxHp} aria-valuenow={currentEnemy.hp} className="my-3 h-3 overflow-hidden rounded-full bg-slate-700" aria-label={tr('enemy_health_aria', 'Enemy health {current} of {maximum}', { current: currentEnemy.hp, maximum: currentEnemy.maxHp })}><div className="h-full bg-fuchsia-500" style={{ width: `${enemyHpPercent}%` }} /></div>
            <fieldset disabled={!!myAction || isSubmitting || escapeState.isPaused} className="space-y-3"><legend className="font-bold">{currentRoom.challenge.prompt}</legend>
              ...
              <button type="button" onClick={() => answerIndex != null && writeChoice('questActions', { abilityId, roleId: myRole || '', answerIndex, submittedAt: Date.now(), ...(quest.actionSchema === 1 ? { turnKey } : {}), ...(supportTargetUid ? { supportId, supportTargetUid } : {}) })} disabled={answerIndex == null} className="min-h-12 w-full rounded-xl bg-emerald-600 px-4 py-2 font-black hover:bg-emerald-500 disabled:opacity-50">{tr('commit_turn', 'Commit turn')}</button>
...
// Leaf writes keep simultaneous teammates from replacing one another's progress.
function alloLiveEscapeProgress(raw = {}, puzzles = []) {
  const solvedPuzzles = [...new Set([...(raw.solvedPuzzles || []), ...Object.values(raw.solved || {}).filter(id => puzzles.some(p => p.id === id))])];
  ...
  return { ...raw, solvedPuzzles, maxLives, lives, wrongAttempts,
...
function alloLiveEscapeSeconds(state, now = Date.now()) {
  ...
  if (!state?.isPaused && Number.isFinite(state?.endsAt)) return Math.max(0, Math.ceil((state.endsAt - now) / 1000));
...
function alloLiveEscapeAnswer(puzzle, answer, type) {
  ...
  if (type === 'matching') return Array.isArray(answer) && answer.length === puzzle.pairs?.length
    && new Set(answer.map(pair => pair.left)).size === answer.length
    && new Set(answer.map(pair => pair.right)).size === answer.length
    && answer.every(pair => puzzle.pairs.some(expected => expected.left === pair.left && expected.right === pair.right));
...
function alloLiveSequenceIndices(puzzle) {
  ...
  const indices = shuffled.map(item => { const at = unused.findIndex(i => items[i] === item); return at < 0 ? -1 : unused.splice(at, 1)[0]; });
  return indices.length === items.length && !indices.includes(-1) ? indices : items.map((_, i) => i);
}
function alloLiveFieldKey(value) { return encodeURIComponent(String(value)).replace(/\./g, '%2E'); }
...
const ClassicStudentEscapeRoomOverlay = React.memo(({ sessionData, user, activeSessionCode, targetAppId, t, playSound, setIsEscapeTimerRunning }) => {
  ...
  const closePuzzleDialog = () => {
    ...
    window.setTimeout(() => { if (!puzzleTriggerRef.current?.disabled) puzzleTriggerRef.current?.focus(); else escapeMainRef.current?.focus(); }, 0);
  };
  ...
  useEffect(() => {
    const escapedTeams = allTeams.filter(team =>
      alloLiveEscapeProgress(escapeState.teamProgress?.[team], puzzles).isEscaped && team !== userTeam
    );
  ...
  useEffect(() => {
    draftsRef.current = {};
    setSelectedPuzzle(null); setUserInput(''); setSequenceOrder([]); setMatchingPairs([]); setMatchingSelected(null); setSyncError('');
  ...
  const handleSubmitAnswer = async (puzzleId, answer, answerType) => {
    ...
    submissionRef.current = true; setIsSubmitting(true); setSyncError('');
    try {
    if (isCorrect) {
      playSound?.('correct');
      try {
    ...
    return (
      <div ref={escapeStateScreenRef} tabIndex={-1} role="alert" aria-labelledby="escape-room-game-over-title" aria-describedby="escape-room-game-over-description" className="fixed top-0 right-0 bottom-0 left-0 z-[9999] bg-gradient-to-br from-red-900 via-slate-900 to-gray-900 flex items-center justify-center focus:outline-none focus:ring-4 focus:ring-inset focus:ring-white">
        <div className="text-center text-white animate-in motion-reduce:animate-none zoom-in duration-500">
          ...
          <p id="escape-room-game-over-description" className="text-2xl text-slate-300 mb-6">{t((escapeState.isGameOver || roomTimedOut) && !teamGameOver ? 'escape_room.time_up' : 'escape_room.life_lost')}</p><p className="text-slate-200 mb-6">{t('escape_room.waiting_host')}</p>
  ...
  if (teamEscaped) {
    const isFirstToEscape = allTeams.filter(team =>
      alloLiveEscapeProgress(escapeState.teamProgress?.[team], puzzles).isEscaped
    ).length === 1;
    return (
  ...
  return (
    <div ref={escapeMainRef} tabIndex={-1} role="region" aria-labelledby="escape-room-active-title" className="fixed top-0 right-0 bottom-0 left-0 z-[9999] bg-gradient-to-br from-slate-900 via-purple-900 to-indigo-900 overflow-auto focus:outline-none focus:ring-4 focus:ring-inset focus:ring-white">
      <div className="sticky top-0 z-50 bg-slate-900/90 backdrop-blur-sm border-b border-purple-500/30 p-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center flex-wrap gap-3">
            ...
            {teamMaxLives < 99 && (
              <div className="flex items-center gap-1 px-2 py-1 rounded-full bg-slate-700/50" role="img" aria-label={`${t('escape_room.lives')}: ${teamLives}/${teamMaxLives}`} title={t('escape_room.lives')} data-help-key="escape_room_lives">
                {Array.from({ length: teamMaxLives }).map((_, i) => (
                  <span key={i} aria-hidden="true" className={`text-sm ${i < teamLives ? 'text-red-500' : 'opacity-30 grayscale'}`}>
                    ❤️
                  </span>
                ))}
              </div>
      ...
      <div className="relative mx-4 mt-4 lg:fixed lg:right-4 lg:top-24 bg-slate-800/80 backdrop-blur-sm rounded-xl p-4 border border-purple-500/30 z-40" data-help-key="escape_room_leaderboard">
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6 mt-4 lg:pr-40">
        <p className="text-center text-purple-200 mb-6 italic">{escapeState.room?.description}</p>
      ...
      {currentPuzzle && !isPaused && !solvedPuzzlesSet.has(currentPuzzle.id) && (
        <div className="fixed inset-0 z-[10000] bg-black/80 flex items-center justify-center p-4" role="presentation">
          ...
          >
            ...
            {syncError && <p role="alert" className="mb-3 rounded-lg bg-red-950 p-3 text-red-100">{syncError}</p>}
            <fieldset disabled={isSubmitting || isPaused} aria-busy={isSubmitting} className="min-w-0">
            <legend className="sr-only">{t('escape_room.submit_answer')}</legend>
            {isSubmitting && <p role="status" className="text-purple-200 mb-3">{t('common.saving', { defaultValue: 'Saving…' })}</p>}
            {currentPuzzle.hint && (
              <div className="mb-4">
                {(teamRevealedHints[alloLiveFieldKey(currentPuzzle.id)] || teamRevealedHints[currentPuzzle.id]) ? (
            ...
            {(currentPuzzle.type === 'cipher' || currentPuzzle.type === 'riddle') && (
              <div className="space-y-4">
                ...
                />
                <button type="button"
                  disabled={!userInput.trim()} onClick={() => handleSubmitAnswer(currentPuzzle.id, userInput, 'cipher')}
              ...
              <div className="space-y-4">
                {currentPuzzle.sentence && (
                  <div className="bg-slate-900 p-4 rounded-lg text-white text-center text-lg">
                    {currentPuzzle.sentence.replace(/_+/g, userInput ? `[${userInput}]` : '______')}
                  </div>
                )}
            ...
            {currentPuzzle.type === 'scramble' && (
              <div className="space-y-4">
                <div className="flex flex-wrap gap-2 justify-center">
                  {(currentPuzzle.displayLetters || Array.from(currentPuzzle.scrambledWord || '')).map((letter, idx) => (
                ...
                />
                <button type="button"
                  disabled={!userInput.trim()} onClick={() => handleSubmitAnswer(currentPuzzle.id, userInput, 'scramble')}
              ...
              <div className="space-y-4" data-help-key="escape_room_sequence_container">
                ...
                <div className="space-y-2" role="list" aria-label={t('escape_room.sequence_list') || 'Sequence items to order'}>
                  {sequenceOrder.length === 0
                    ...
                    : sequenceOrder.map((itemIndex, idx) => (
                        <div key={idx} role="listitem" aria-label={`${t('escape_room.position') || 'Position'} ${idx + 1}: ${currentPuzzle.items[itemIndex]}`} className="flex items-center gap-2 p-3 bg-slate-700 rounded-lg text-white">
                          <span className="w-8 h-8 flex items-center justify-center bg-purple-600 rounded-full font-bold" aria-hidden="true">{idx + 1}</span>
                          <span className="flex-1">{currentPuzzle.items[itemIndex]}</span>
            ...
            {currentPuzzle.type === 'matching' && (
              <div className="space-y-4" data-help-key="escape_room_matching_container">
                <p className="text-sm text-purple-200 italic mb-2">{t('escape_room.matching_instructions')}</p>
                  ...
                  <div className="bg-slate-900 p-3 rounded-lg" role="list" aria-label={t('escape_room.matched_pairs') || 'Matched pairs'}>
                    ...
                    <div className="space-y-1">
                      {matchingPairs.map((pair, idx) => (
                        <div key={idx} role="listitem" className="text-sm text-slate-100 flex items-center justify-between gap-2"><span>{pair.left} ↔ {pair.right}</span><button type="button" className="min-h-11 px-3 rounded bg-slate-700" aria-label={`${t('common.remove', { defaultValue: 'Remove' })}: ${pair.left} ↔ ${pair.right}`} onClick={() => { setMatchingPairs(prev => prev.filter((_, i) => i !== idx)); setMatchingSelected(null); }}>×</button></div>
                      ))}
                    </div>
                  </div>
                )}
...
const ClassicEscapeRoomTeacherControls = React.memo(({ sessionData, activeSessionCode, appId, t, addToast }) => {
  ...
  useEffect(() => { warningsRef.current.clear(); }, [activeSessionCode, escapeState.startedAt]);
  const writeControl = async (patch) => {
    if (controlRef.current) return false;
    controlRef.current = true; setControlBusy(true); setControlError('');
    try {
      const sessionRef = doc(db, 'artifacts', appId, 'public', 'data', 'sessions', activeSessionCode);
      await updateDoc(sessionRef, patch); return true;
    } catch (error) { warnLog('Live escape room control failed:', error); setControlError('The session could not be updated. Check the connection and try again.'); return false; }
    finally { controlRef.current = false; setControlBusy(false); }
  };
  ...
  useEffect(() => {
    if (!escapeState.isActive || escapeState.isPaused || escapeState.isGameOver || !activeSessionCode || !appId) return;
    const sessionRef = doc(db, 'artifacts', appId, 'public', 'data', 'sessions', activeSessionCode);
    if (!Number.isFinite(escapeState.endsAt)) {
      updateDoc(sessionRef, { 'escapeRoomState.endsAt': Date.now() + Math.max(0, escapeState.timeRemaining || 0) * 1000 })
        .catch(error => { warnLog('Live escape clock failed:', error); setControlError('The timer could not start. Check the connection and use Resume to retry.'); });
      return;
    }
    ...
    const tick = async () => {
      ...
      try {
        ...
        stopped = true; addToast?.(t('escape_room.time_up'), 'error');
      } catch (error) { warnLog('Live escape expiry failed:', error); setControlError('Time has ended. Reconnecting to save the result…'); }
      finally { writing = false; }
    };
    tick(); const timer = setInterval(tick, 1000);
    return () => { stopped = true; clearInterval(timer); };
  }, [escapeState.isActive, escapeState.isPaused, escapeState.isGameOver, escapeState.endsAt, activeSessionCode, appId]);
  const isCoopMode = escapeState.isCoopMode || false;
  const isPaused = escapeState.isPaused || false;
  const teamColors = {
  ...
  const handlePauseToggle = async () => {
    ...
    await writeControl({ 'escapeRoomState.isPaused': !isPaused, 'escapeRoomState.timeRemaining': remaining,
      'escapeRoomState.endsAt': isPaused ? Date.now() + remaining * 1000 : null });
  };
  ...
  return (
    <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-2xl p-4 border-2 border-purple-200 shadow-lg mb-4">
      {controlError && <p role="alert" className="mb-3 rounded-lg bg-red-100 p-3 text-red-900">{controlError}</p>}
      {escapeState.isGameOver && <p role="status" className="mb-3 font-bold text-red-800">{t('escape_room.time_up')} · {t('escape_room.end_game')}</p>}
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        ...
        <div className="flex items-center flex-wrap gap-2">
          ...
          <button type="button"
            onClick={handlePauseToggle} disabled={controlBusy || escapeState.isGameOver} aria-pressed={isPaused}
          ...
          return (
            <div key={team} className={`p-3 rounded-xl border-2 ${escaped ? 'border-green-400 bg-green-50' : 'border-slate-200 bg-white'}`}>
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                {progress.isGameOver && <span className="text-red-800 font-bold">{t('escape_room.game_over')}</span>}
        ...
        <div className="mt-4 p-3 bg-green-100 rounded-xl border border-green-300 text-center">
          <span className="text-green-800 font-bold">
            🎉 {escapedTeams.length === 1
              ...
              : escapedTeams.length === allTeams.filter(team => Object.values(escapeState.teams || {}).includes(team)).length ? t('escape_room.all_teams_done') : escapedTeams.join(', ')
            }
          </span>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[9999] bg-black/50 flex items-center justify-center p-4" role="presentation">
          ...
          >
            ...
            <div className="flex gap-3">
              ...
              <button type="button" onClick={handleEndGame} disabled={controlBusy} className="flex-1 min-h-11 px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl transition-colors motion-reduce:transition-none focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
                {t('escape_room.end_game')}
              </button>
            </div>
          </div>
...
const TeacherLiveQuizControls = React.memo(({ sessionData, generatedContent, activeSessionCode, appId, onGenerateImage, onRefineImage, onCreateGroup, onAssignStudent, onSetGroupResource, isPushingResource = {}, onSetGroupLanguage, onSetGroupProfile, onDeleteGroup, onUpdateQuestionRoutingRules, history = [], callGemini, addToast }) => {
    const { t } = useContext(LanguageContext);
    ...
    useEffect(() => { completedWriteRef.current = ''; setQuizError(''); }, [activeSessionCode, quizState.activityId, quizState.roundId, currentQuestionIndex, phase]);
    const writeQuiz = async (sessionRef, patch) => {
      ...
      quizWriteRef.current = true; setQuizBusy(true); setQuizError('');
      try { await updateDoc(sessionRef, patch); completedWriteRef.current = key; return true; }
      catch (error) { warnLog('Live quiz control failed:', error); setQuizError('The live quiz could not be updated. Check the connection and try again.'); return false; }
      finally { quizWriteRef.current = false; setQuizBusy(false); }
    };
    ...
    useEffect(() => { setBossDifficulty(bossStats?.difficulty || 'normal'); }, [bossStats?.difficulty, activeSessionCode]);
    ...
    const bossIdentity = [appId, activeSessionCode, quizState.activityId, mode, bossStats?.battleId || 'legacy'].join(':');
    const bossIdentityRef = useRef(bossIdentity); bossIdentityRef.current = bossIdentity;
    const bossVisualRequestRef = useRef(0);
    useEffect(() => { setBossGmDraft(null); setBossGmPrompt(''); return () => { bossVisualRequestRef.current++; }; }, [bossIdentity]);
    ...
    const battleQuestionCount = (generatedContent?.data?.questions || []).filter(item => quizLiveAggregators?.presentationQuestionIsGameScorable
      ? quizLiveAggregators.presentationQuestionIsGameScorable(item)
      : Array.isArray(item?.options) && item.options.includes(item.correctAnswer)).length;
    ...
    const handleStartQuestion = async () => {
        ...
        const sessionRef = doc(db, 'artifacts', appId, 'public', 'data', 'sessions', activeSessionCode);
        try { await writeQuiz(sessionRef, { "quizState.roundId": Date.now().toString(36) + Math.random().toString(36).slice(2, 8), "quizState.phase": "answering", "quizState.responses": {}, "quizState.responseReceipts": {}, "quizState.currentQuestionIndex": currentQuestionIndex, "quizState.bossStats.lastDamage": 0 }); } catch(e) { warnLog('Firestore sync failed:', e); }
    };
    ...
    const publishBossGmDraft = async () => {
        ...
        if (await writeQuiz(sessionRef, updates)) { setBossGmDraft(null); setBossGmPrompt(''); }
    };
    const handleRevealResults = async () => {
        ...
        try {
            ...
            if (mode === 'boss-battle' && liveQuestionSummary.gameScorable && !(scoredAnsweredCount === 0 && unscoredReceiptCount > 0)) {
                ...
                // A receipt-only delivery is unknown, not a wrong answer.
                ...
                // Budget damage across scorable items only; polls and teacher-review
                // prompts cannot drain the class's opportunities to defeat the boss.
                ...
                updatePayload['quizState.bossStats.roundFeedback'] = { scoringPaused: true, explanation: unscoredReceiptCount > 0 && scoredAnsweredCount === 0
                    ? 'Participation arrived without answer content. No health was lost. Reopen this question after students reconnect to score their answers.'
                    : 'This item is for discussion or teacher review. Battle health is unchanged.' };
                if (!liveQuestionSummary.gameScorable && currentQuestionIndex >= (generatedContent?.data?.questions?.length || 1) - 1) {
                    ...
                    updatePayload['quizState.phase'] = !hasScoredRounds ? 'battle-complete'
                        : (bossStats?.classHP ?? 100) / (bossStats?.classMaxHP || 100) >= (bossStats?.currentHP ?? 1000) / (bossStats?.maxHP || 1000) ? 'boss-defeated' : 'class-defeated';
            ...
            <div className="mt-5 w-full max-w-sm mx-auto text-left bg-black/30 rounded-xl p-3 max-h-44 overflow-y-auto">
                ...
                {log.map((e, i) => (
                    <div key={i} className="flex items-center gap-2 mb-1" title={qText(e.questionIndex)}>
                        ...
                        {log.length > 1 && e === worst && e.accuracy < 70 && (
    ...
    const handleModeChange = async (e, nextDifficulty = bossDifficulty, restart = false) => {
        ...
        if (newMode === 'boss-battle') {
            ...
            const hpMultiplier = nextDifficulty === 'easy' ? 0.5 : nextDifficulty === 'hard' ? 1.5 : 1.0;
    ...
    return (
        <div className="bg-white rounded-2xl shadow-xl border border-slate-400 overflow-hidden mb-6 animate-in motion-reduce:animate-none slide-in-from-top-4 duration-500" aria-busy={quizBusy}>
            {quizError && <p role="alert" className="m-3 rounded-lg bg-red-100 p-3 font-bold text-red-900">{quizError}</p>}
            {quizBusy && <p role="status" className="px-4 py-2 text-indigo-900">Updating the live quiz…</p>}
            <div className="bg-indigo-900 text-white p-4 flex justify-between items-center flex-wrap gap-4">
                ...
                <div className="flex items-center gap-4">
                     <select aria-label={t('common.selection')}
                        value={mode || 'live-pulse'} disabled={quizBusy || phase === 'answering' || answeredCount > 0} title="Choose a mode before starting an unanswered round." data-help-key="quiz_mode_select"
                     ...
                     {mode === 'boss-battle' && (
                         <select aria-label="Battle difficulty"
                     ...
                     {mode !== 'live-pulse' && (
                         <button type="button"
                             aria-label={showLocalStats ? t('quiz.show_game') : t('quiz.show_stats')} aria-pressed={showLocalStats}
                     ...
                     <button type="button" onClick={handleEndQuiz} disabled={quizBusy} className="text-xs bg-red-500 hover:bg-red-600 px-3 py-1.5 rounded-full font-bold transition-colors motion-reduce:transition-none focus:outline-none focus:ring-2 focus:ring-white">
                        {t('quiz.end_quiz')}
                     </button>
                </div>
            </div>
            ...
            <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="flex flex-col justify-between h-full">
                     ...
                     <div className="space-y-3 mt-auto">
                         {phase === 'answering' && <p className="text-sm text-slate-700">Reveal results before moving to another question. Responses stay available until then.</p>}
                         {bossBattleEnded ? <div className="rounded-xl border border-indigo-200 bg-indigo-50 p-3"><p className="mb-2 text-sm text-indigo-900">Review the battle below. Restart begins at question 1 with full health and a new battle history.</p><button type="button" disabled={quizBusy} onClick={() => handleModeChange({target:{value:'boss-battle'}}, bossStats?.difficulty || 'normal', true)} className="min-h-11 w-full rounded-lg bg-indigo-700 px-4 py-3 font-bold text-white disabled:opacity-50">Restart battle</button></div> : phase === 'answering' ? (
                             <button type="button" aria-label={t('quiz.reveal_results')}
                                onClick={handleRevealResults} disabled={quizBusy} data-help-key="quiz_reveal_btn"
                ...
                <div className="bg-slate-50 rounded-xl border border-slate-400 p-6 flex flex-col items-center justify-center min-h-[300px] relative">
                         ...
                         <>
                            {mode === 'boss-battle' && bossStats ? (
                                <div className="w-full h-full flex flex-col items-center justify-center animate-in motion-reduce:animate-none fade-in zoom-in duration-300 relative">
                                     {bossBattleEnded && <section aria-label="Battle result" className="mb-5 w-full rounded-xl border border-indigo-300 bg-slate-900 p-5 text-center text-white">
                                         <h3 className="text-2xl font-black" role="status">{phase === 'boss-defeated' ? 'Class victory!' : phase === 'battle-complete' ? 'Battle complete' : 'A chance to regroup'}</h3>
                                         <p className="mt-2 text-sm text-slate-200">{bossStats.endReason === 'no-scored-items' ? 'This quiz had no scored battle rounds. Discuss the responses together.' : bossStats.endReason === 'questions-complete' ? 'All questions are complete. The result compares the percentage of health remaining; ties favor the class.' : phase === 'boss-defeated' ? 'The class defeated the monster together.' : 'Review the explanations, discuss a strategy, and try another battle.'}</p>
                                         {renderBattleDebrief()}
                                     </section>}
                                     ...
                                     <div className={`w-full max-w-sm bg-slate-300 h-8 rounded-full border-4 relative overflow-hidden shadow-inner mb-2 ${bossStats.currentHP > 0 && (bossStats.currentHP / bossStats.maxHP) < 0.25 ? 'border-red-500 animate-pulse motion-reduce:animate-none' : 'border-slate-400'}`}>
                                         <div
                                            role="progressbar" aria-label="Monster health" aria-valuemin={0} aria-valuemax={bossStats.maxHP || 1} aria-valuenow={Math.min(bossStats.maxHP || 1, Math.max(0, bossStats.currentHP || 0))}
                                     ...
                                     <div className="w-full max-w-sm bg-slate-300 h-6 rounded-full border-4 border-slate-400 relative overflow-hidden shadow-inner mb-2">
                                         <div
                                            role="progressbar" aria-label="Class health" aria-valuemin={0} aria-valuemax={bossStats.classMaxHP || 100} aria-valuenow={Math.min(bossStats.classMaxHP || 100, Math.max(0, bossStats.classHP ?? 100))}
                     ...
                     {(phase === 'revealed' || bossBattleEnded) && !liveQuestionSummary.unscored && liveAnswerGuide && (
