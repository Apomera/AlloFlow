...
const resolveAdventureTtsLanguage = (adventureLanguageMode, unit = null, selectedLanguages = []) => {
  const explicitLanguage = unit && typeof unit === "object" && !Array.isArray(unit) ? String(unit.language || "").trim() : "";
...
const _pkAudioLoadTimeoutMs = () => {
  try {
    ...
    return window.AlloFlowConfig && window.AlloFlowConfig.timeouts && window.AlloFlowConfig.timeouts.audioLoadMs || 3e4;
  } catch (_) {
    return 3e4;
  }
};
...
const _kokoroVoicePrefix = /^(af_|am_|bf_|bm_)/i;
...
const playSequence = async (index, sentences, sessionId, mode = "standard", voiceMap = {}, activeSpeaker = null, preloadedAudio = null, retryCount = 0, speakerName = null, deps, contentId = null) => {
  ...
  try {
    ...
    const fallbackTtsLanguage = mode === "persona" ? resolvePersonaTtsLanguage(currentUiLanguage, leveledTextLanguage) : mode === "adventure" ? resolveAdventureTtsLanguage(adventureLanguageMode, null, selectedLanguages) : leveledTextLanguage || "English";
    const segmentLanguage = mode === "adventure" ? resolveAdventureTtsLanguage(adventureLanguageMode, currentUnit, selectedLanguages) : readAloudUnitLanguage(currentUnit, fallbackTtsLanguage);
    ...
    const shouldJoinAdventureKokoro = mode === "adventure" && _kokoroVoicePrefix.test(String(currentVoice || "")) && _pkIsEnglishTtsLanguage(segmentLanguage);
    ...
    if (preloadedAudio) {
      ...
      if (audio instanceof Promise) {
        try {
          const _tOut = shouldJoinAdventureKokoro ? _pkAudioLoadTimeoutMs() : Math.min(_pkAudioLoadTimeoutMs(), READ_ALOUD_PRELOAD_PROMOTION_MS);
      ...
      if (storedReadAloudUrl) {
        ...
        try {
          const _tOut2 = shouldJoinAdventureKokoro ? _pkAudioLoadTimeoutMs() : Math.min(_pkAudioLoadTimeoutMs(), READ_ALOUD_PRELOAD_PROMOTION_MS);
  ...
  if (contentId && (contentId === "simplified-main" || contentId === "adventure-active" || contentId === "faq-active" || contentId.startsWith("persona-message-"))) {
    ...
    if (mode === "persona") {
      ...
      cleanSentences = createReadAloudDescriptors(cleanSentences, {
        ...
        scope: "adventure",
        occurrenceByText: /* @__PURE__ */ new Map()
      });
...
const executeSaveFile = async (deps, interactionOptions = {}) => {
  ...
  if (_hasVoice || _hasSelText) {
    ...
    const _confirmPrivacy = async () => {
      ...
      if (_ux && typeof _ux.confirm === "function") {
        try {
          ...
          try {
            warnLog("[SaveFile] in-app privacy confirm threw; falling back to the native dialog", _e && _e.message);
          } catch (__) {
          }
        }
      }
      ...
      let _native = false;
      try {
        _native = !!window.confirm(_msg);
      } catch (_e) {
        return { ok: false, route: "native-threw", ms: Date.now() - _t0, suppressed: true };
      }
    ...
    if (!_decision.ok) {
      ...
      try {
        addToast(_decision.suppressed ? _suppressedMsg : t("toasts.save_cancelled") || "Save cancelled.", _decision.suppressed ? "error" : "info");
      } catch (_) {
      }
...
const formatInlineText = (text, enableGlossary = true, isDarkBg = false, deps) => {
  ...
  return parts.map((part, pIdx) => {
    ...
    if (isCode) {
      return /* @__PURE__ */ React.createElement("code", { key: pIdx, className: "bg-slate-100 text-pink-700 px-1 rounded font-mono text-xs border border-slate-400" }, renderedSubParts);
    }
    return /* @__PURE__ */ React.createElement("span", { key: pIdx, className: "whitespace-pre-line" }, renderedSubParts);
  });
};
const autoConfigureSettings = async (text, grade, standards, language, customInput, existingResources = [], targetCount = "Auto", deps) => {
  ...
  try {
    const plannerAudience = isIndependentMode ? `Target Audience: ONE self-directed adult learner working alone at ${grade} level. There is no class and no teacher. Plan for self-testing, self-pacing and self-checking; prefer resources a lone learner can complete and verify unaided. Do NOT plan group protocols, whole-class discussion kits, jigsaw activities or exit tickets - there is nobody to hand them to.` : isParentMode ? `Target Audience: ONE child at home, working with a parent or caregiver at ${grade} level. Plan for a warm one-on-one sitting, not a classroom: shared reading, conversation, hands-on activity and gentle checks for understanding. Favour short resources a caregiver can run without training or prep. Do NOT plan group protocols, whole-class discussion kits, jigsaw activities or exit tickets, and keep formal assessment light.` : `Target Audience: ${grade} students in a class, taught by an educator.`;
    const plannerGuidanceLabel = isIndependentMode ? "LEARNER PACK GUIDANCE" : isParentMode ? "FAMILY PACK GUIDANCE" : "TEACHER PACK GUIDANCE";
    const userCustomBlock = customInput && customInput.trim().length > 0 ? `${plannerGuidanceLabel}: "${customInput}". Use this to shape the resource plan and lessonDNA when it is compatible with the source material, target grade, standards, and already-generated resources. Do not replace or contradict those anchors; if there is a conflict, preserve the anchors and adapt the guidance.` : "";
...
${resourceSummaries.map((s, i) => `  ${i + 1}. ${s}`).join("\n")}`;
    ...
    const VALID_TOOLS_LIST = typeof window !== "undefined" && typeof window.getToolIdsCsv === "function" ? window.getToolIdsCsv() : "analysis, simplified, glossary, outline, image, quiz, sentence-frames, brainstorm, timeline, concept-sort, adventure, faq, persona, dbq, note-taking, anchor-chart, memory-aid, math, lesson-plan, gemini-bridge, alignment-report";
    let countConstraint = "";
    const PLANNER_TOOLS_LIST = VALID_TOOLS_LIST.includes("applied-challenge") ? VALID_TOOLS_LIST : `${VALID_TOOLS_LIST}, applied-challenge`;
