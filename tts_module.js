...
const createTTS = deps => {
  ...
  const callTTS = async (text, voiceName, speed = 1, maxRetriesOrOpts = 2, languageArg) => {
    ...
    if (_isCanvasEnv && _earlyTtsProvider !== 'local') {
      ...
      if (!_isKokoroVoice && !_forceRefresh && state.urlCache.has(canvasCacheKey)) {
        ...
        return _emitResolvedProfile(state.urlCache.get(canvasCacheKey), _resolutionProfile('gemini', 'gemini-tts', _geminiModel, voiceName, 1, {
          cacheHit: true
        }));
      }
      if (_isKokoroVoice) {
        ...
        const fetchCanvasTTSBytes = async () => {
          ...
          if (!entry || isInteractive || _forceRefresh || entryAge > CALLTTS_JOIN_MAX_AGE_MS) {
            ...
            entry = {
              promise: fetchTTSBytes(text, voiceName, speed, _speechProfile, null, _callOpts.priority, _geminiModel),
              startedAt: Date.now()
            };
        ...
        for (let canvasAttempt = 0; canvasAttempt < canvasMaxAttempts; canvasAttempt++) {
          try {
            ...
            if (ttsResult) {
              ...
              return _emitResolvedProfile(url, _resolutionProfile('gemini', 'gemini-tts', _geminiModel, voiceName, 1, {
                cacheHit: false
              }));
            }
    ...
    if (!_forceRefresh && state.urlCache.has(cacheKey)) {
      ...
      return _emitResolvedProfile(state.urlCache.get(cacheKey), _resolutionProfile('gemini', 'gemini-tts', _geminiModel, voiceName, 1, {
        cacheHit: true
      }));
    }
      ...
      if (_isEnglish) {
        try {
          ...
          if (window._kokoroTTS?.ready) {
            ...
            const url = await window._kokoroTTS.speak(cleanTextForLocalTTS(text), localVoice, speed, {
              signal: _signal,
              force: _forceRefresh
            });
            if (url) return _emitResolvedProfile(url, _resolutionProfile('local', 'kokoro-browser', null, localVoice, 1, {
              fallbackFrom: 'gemini'
            }));
          }
        } catch (error) {
  ...
  const callTTSDirect = async (text, voiceName, speed = 1, maxRetries = 2) => {
    ...
    var _botKokoroEligible = typeof voiceName === 'string' && KOKORO_VOICE_PREFIX.test(voiceName) || (_botProvTts === 'local' || _botProvTts === 'auto' && _botProvLocalAI) && typeof voiceName === 'string' && voiceName !== 'browser' || !_isCanvasEnv && !_cloudKeyUsable() && !_botProviderHandles && typeof voiceName === 'string' && voiceName !== 'browser';
