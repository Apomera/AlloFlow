...
describe('Water Cycle Precipitation Lab', () => {
  ...
  it('exposes a dedicated 3D cloud chamber without changing the default 2D lab', () => {
    ...
    WATER_CYCLE_PATHS.forEach((filePath) => {
      ...
      expect(source).toContain('var surfaceImpactMistGroupPrecip3d = new THREE.Group();');
      expect(source).toContain('canvasEl.dataset.precipitationRainShaftRendering');
      expect(source).toContain("'depth-layered-overlapping-drop-curtains'");
      expect(source).toContain('canvasEl.dataset.precipitationSurfaceMistRendering');
      expect(source).toContain("'phase-aware-impact-spray'");
      expect(source).toContain('function makeSurfaceWetnessTexturePrecip3d()');
      expect(source).toContain('var surfaceWetSheenPrecip3d = new THREE.Mesh(');
      expect(source).toContain('var puddleHighlightsPrecip3d = [];');
      expect(source).toContain('var surfaceSplashCrownGroupPrecip3d = new THREE.Group();');
      expect(source).toContain('var surfacePhaseGlintsPrecip3d = new THREE.Points(');
      expect(source).toContain('canvasEl.dataset.precipitationSurfaceMaterialRendering');
      expect(source).toContain("'accumulation-driven-wet-sheen-puddle-reflections-and-cold-phase-glints'");
      expect(source).toContain('canvasEl.dataset.precipitationSplashCrownRendering');
      expect(source).toContain("'radial-impact-crowns-and-ejected-droplets'");
      expect(source).toContain('rainShaftTexturesPrecip3d');
