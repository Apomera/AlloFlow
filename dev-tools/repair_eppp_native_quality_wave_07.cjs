...
if (!Array.isArray(revisions) || revisions.length !== 8) throw new Error('Wave 07 must contain exactly eight revisions.');
...
for (const revision of revisions) {
  ...
  if (revision.choices.some((choice) => extremeCuePattern.test(choice))) throw new Error(revision.id + ' still contains an extreme cue.');
  if (!revision.sourceCheck || revision.sourceCheck.length < 150) throw new Error(revision.id + ' needs a substantive source check.');
  ...
  if (!['application', 'analysis'].includes(revision.cognitiveProcess)) throw new Error(revision.id + ' needs applied or analytic demand.');
  if (!Array.isArray(revision.distractorDesign) || revision.distractorDesign.length !== 3) throw new Error(revision.id + ' needs three distractor rationales.');
...
for (const auditItem of auditItems) {
  auditItem.diagnosticsAfter = [
    ...
    ...(afterByFamily.duplicate.some((pair) => pair.split(' / ').includes(auditItem.id)) ? ['semantic-concept-duplicate-candidate'] : []),
  ];
}
...
const report = {
  ...
  scope: 'Deep-rewrite the first eight unreviewed items in the adjudication-aware distractor action docket while preserving answer positions and full option-level teaching feedback.',
  challengeCriteria: [
  ...
  summary: {
    ...
    warningCountsBefore: { ...diagnosticsBefore.summary },
...
const markdown = `# EPPP native distractor-quality repair - wave 07
...
console.log('EPPP native quality wave 07: ' + auditItems.length + ' rewritten; ' + selectedWarningIdsAfter.length + ' selected items retain diagnostics; ' + report.summary.status + '.');
