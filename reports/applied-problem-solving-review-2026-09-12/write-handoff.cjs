...
const tests=JSON.parse(fs.readFileSync(path.join(__dirname,'implementation-tests.json'),'utf8'));
if(tests.numFailedTests || tests.numPassedTests < 83)throw Error('The final regression pass must succeed before writing the handoff.');
const browser=JSON.parse(fs.readFileSync(path.join(__dirname,'implementation-browser-results.json'),'utf8'));
...
for(const file of mirrors){const bytes=fs.readFileSync(file),target=path.join('desktop/web-app/public',file);if(!bytes.equals(fs.readFileSync(target)))fs.writeFileSync(target,bytes);}
...
const hashes=Object.fromEntries(mirrors.map(file=>[file,crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex')]));fs.writeFileSync(path.join(__dirname,'implementation-build-results.json'),JSON.stringify({compiled,mirrors:hashes},null,2));
const rows=browser.layouts.map(r=>`| ${r.viewport}px | ${r.width}px | ${Math.round(r.firstFieldTop)}px | ${r.visibleFields} |`).join('\n');
const report=`# Applied Problem Solving implementation
...
const review=path.join(__dirname,'README.md');let text=fs.readFileSync(review,'utf8');if(!text.includes('IMPLEMENTATION.md'))text=text.replace('# Refining Applied Problem Solving','# Refining Applied Problem Solving\n\nImplementation completed locally: [changes, validation, and preview](IMPLEMENTATION.md). The findings below describe the original version.');fs.writeFileSync(review,text);
console.log(`${tests.numPassedTests} tests passed; implementation notes and build verification saved.`);
