...
const entry={slug:slug+'_illustrated',title:pack.allopack.title,subject:'Mathematics',grade_level:pack.allopack.gradeLevel,tags:['mathematics','illustrated'],license:'CC-BY-4.0',credit:pack.allopack.author,path:packPath};
...
assert.equal(decoded,pack.history.find(r=>r.type==='glossary').data.length+pack.history.find(r=>r.type==='anchor-chart').data.sections.length+pack.history.find(r=>r.type==='concept-sort').data.items.length+8);
