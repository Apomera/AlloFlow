...
const itemSpecs = [
  ...
  q(1, '1.9', '4.A', 'Two classes have the same median test score. Class A has a smaller IQR than Class B. What does that indicate?', 'The middle 50% of Class A scores are less spread out.', ['Class A must therefore have the larger mean score.', 'Class A has more of its scores below the median.', 'Class B must therefore have a higher maximum score.'], 'The IQR measures the spread of the middle half of a distribution; it does not determine the mean or the extremes.', { cognitiveProcess: 'compare' }),
  q(1, '1.10', '2.B', 'A researcher changes the wording of a survey question after seeing that the first wording produced an unexpected result. Why is that a problem?', 'Changing the question after seeing results can make the study question depend on the observed data.', ['It guarantees that the study will end up with a much larger random sample of people.', 'It turns an observational study into a completely randomized experiment instead.', 'It eliminates all of the response bias from every one of the survey responses given.'], 'A valid investigative question should be determined before the analysis so the study is not retrofitted to a desired or surprising result.', { cognitiveProcess: 'explain' }),
  ...
  q(1, '1.10', '4.B', 'A random sample of 500 app users reports their weekly screen time. Which conclusion is justified if the sample was representative?', 'The sample can support a generalization about the population of similar app users.', ['The sample proves that the app itself caused the reported screen time.', 'The result automatically applies to every single person in the country.', 'The sample statistic is exactly equal to the population parameter itself.'], 'Random sampling supports generalization to the sampled population, but an observational survey does not establish causation and statistics are estimates.', { cognitiveProcess: 'justify' }),
  q(1, '1.9', '3.B', 'Adding the same constant to every observation in a data set changes the mean but leaves which quantity unchanged?', 'The standard deviation', ['The median of the data', 'The minimum of the data', 'The maximum of the data'], 'Adding a constant shifts location summaries but does not change distances among observations, so standard deviation remains unchanged.', { cognitiveProcess: 'explain' }),
  ...
  q(2, '2.1', '4.A', 'A two-way table shows device ownership by grade. Which display is best for comparing device ownership across grades?', 'Conditional relative frequencies within each grade', ['The grand total of the entire two-way table', 'A histogram showing the grade labels only', 'The sample mean of all the device categories'], 'Conditional proportions within each grade put the grade groups on a common basis for comparison.', { cognitiveProcess: 'select' }),
  ...
  q(2, '2.6', '3.C', 'Of 80 library visitors, 32 borrow a book and 20 of those 32 also use a study room. What is P(use a study room | borrow a book)?', '0.625', ['0.25', '0.40', '0.375'], 'Conditional probability uses the borrowers as the denominator: 20/32 = 0.625.', { cognitiveProcess: 'calculate' }),
  ...
  q(2, '2.9', '4.D', 'A game’s expected payout is $2.40 per play. What does that mean?', 'Over many plays, the average payout per play will tend to approach $2.40.', ['Every single player will receive exactly $2.40 on each play.', 'The game must pay exactly $2.40 on its single most common outcome.', 'The probability of winning on any one play is exactly 2.40.'], 'Expected value is a long-run average, not a guarantee for an individual play or the most likely outcome.', { cognitiveProcess: 'interpret' }),
  q(2, '2.10', '4.B', 'Which situation can be modeled by a binomial random variable?', 'The number of defective items in 12 independently inspected items when each has the same defect probability.', ['The number of separate rolls that are needed until a six first appears on a fair six-sided die', 'The total amount of rainfall that is recorded tomorrow at the local weather station in the city', 'The number of heads in a set of flips where the coin changes its bias on every single flip'], 'A binomial count has a fixed number of independent trials, two outcomes, and a constant success probability.', { cognitiveProcess: 'classify' }),
  ...
  q(2, '2.10', '3.C', 'For X ~ Binomial(5, 0.2), which expression gives P(X = 2)?', 'C(5,2)(0.2)^2(0.8)^3', ['C(5,2)(0.2)^2(0.8)^5', 'C(5,2)(0.2)^3(0.8)^2', 'C(5,3)(0.2)^2(0.8)^3'], 'The binomial probability function chooses the two success positions and multiplies by the probability of two successes and three failures.', { cognitiveProcess: 'select' }),
  q(2, '2.10', '3.D', 'If X is binomial with n = 100 and p = 0.60, what is the standard deviation of X?', 'sqrt(24), or about 4.90', ['24, the variance of X', 'sqrt(40), or about 6.32', 'sqrt(60), or about 7.75'], 'The binomial standard deviation is sqrt(np(1-p)) = sqrt(100(0.6)(0.4)) = sqrt(24).', { cognitiveProcess: 'calculate' }),
  ...
  q(2, '2.12', '4.C', 'Why does the Central Limit Theorem matter for sample means?', 'For sufficiently large random samples, the sampling distribution of the sample mean is approximately normal even when the population is not normal.', ['It says that every individual observation in the population is normally distributed whatever the sample size happens to be.', 'It makes the population standard deviation become exactly equal to zero once the sample size is large enough for the study.', 'It eliminates all of the sampling variability whenever the sample is large enough for the procedure to be applied here.'], 'The Central Limit Theorem supports normal modeling of sample means under appropriate conditions, not of every observation and not without variability.', { cognitiveProcess: 'explain' }),
  q(2, '2.12', '4.D', 'A sampling distribution of sample means is centered at 50 with standard deviation 3. What does a sample mean of 56 represent?', 'It is two standard deviations above the center of the sampling distribution.', ['It proves that the population mean must be exactly 56 in this case.', 'It is six standard deviations above the center of the distribution.', 'It must therefore be the most likely sample mean in the distribution.'], 'The standardized distance is (56 − 50)/3 = 2; a sample statistic does not prove the population parameter.', { cognitiveProcess: 'interpret' }),

  // Unit 3: inference for proportions and categorical data.
  q(3, '3.1', '4.B', 'An estimator is unbiased when which statement is true?', 'Its sampling distribution is centered at the population parameter.', ['It has no sampling variability at all in repeated samples.', 'It always equals the population parameter in every sample.', 'It has the smallest possible standard deviation of all.'], 'Unbiasedness means the long-run mean of the estimator equals the parameter; individual estimates can still vary.', { cognitiveProcess: 'identify' }),
  ...
  q(3, '3.6', '4.F', 'What does a p-value of 0.03 mean in a test of a population proportion?', 'Assuming the null hypothesis is true, the probability of a result at least as extreme as the observed result is 0.03.', ['There is only a 3% chance that the null hypothesis is actually true for the whole population here.', 'There is a 97% chance that the alternative hypothesis is actually true for the whole population.', 'The observed sample proportion is wrong by exactly 3 percentage points in this particular study.'], 'A p-value is calculated under the null model and measures how unusual the observed result or more extreme results would be.', { cognitiveProcess: 'interpret' }),
  ...
  q(3, '3.11', '4.G', 'A confidence interval for p1 − p2 is (−0.04, 0.12). What claim is supported?', 'The interval does not provide convincing evidence of a difference between the population proportions.', ['The first population proportion is definitely the larger of the two population proportions.', 'The second population proportion is definitely the larger of the two population proportions.', 'The two sample proportions in the study must be exactly equal to one another here.'], 'Because the interval contains 0, a zero difference remains plausible; this does not prove equality.', { cognitiveProcess: 'justify' }),
  ...
  q(3, '3.13', '4.G', 'A two-proportion test finds p = 0.01. Which conclusion is appropriate at alpha = 0.05?', 'There is convincing evidence that the population proportions differ in the direction of the alternative.', ['The null hypothesis has been proven false in every possible population of interest here.', 'There is no difference between the two proportions because 0.01 is such a small value.', 'The two sample sizes must have been exactly equal for this result to appear in the data.'], 'Since p < alpha, reject the null and describe evidence for the alternative in the population context.', { cognitiveProcess: 'justify' }),
  ...
  q(3, '3.15', '4.F', 'A chi-square test produces a small p-value. What does that suggest?', 'The observed categorical counts would be unusual if the null model were true.', ['Every one of the expected counts in the whole table is exactly zero.', 'The two variables must have a perfect causal relationship here.', 'The sample must necessarily have been a full census of the population.'], 'A small p-value indicates a discrepancy between observed and expected counts that is difficult to attribute to chance under the null model.', { cognitiveProcess: 'interpret' }),
  ...
  q(4, '4.2', '4.C', 'Why is a t-distribution used for a one-sample confidence interval for a population mean?', 'The population standard deviation is usually unknown and is estimated with the sample standard deviation.', ['The data being analyzed are entirely categorical rather than quantitative in nature.', 'The population mean is always exactly zero in every one of the samples that could be drawn.', 'A t-distribution has no tails at all, and so it cannot be used for any interval here.'], 'Replacing unknown sigma with s adds uncertainty; the t family accounts for that extra uncertainty through degrees of freedom.', { cognitiveProcess: 'explain' }),
  ...
  q(4, '4.4', '4.E', 'For a matched-pairs t procedure with fewer than 30 pairs, which condition is especially important?', 'The distribution of the paired differences should not have strong skewness or outliers.', ['The two original variables must both be categorical rather than numeric.', 'The population standard deviation must always be known exactly in advance.', 'Every one of the paired differences must be exactly equal to zero here.'], 'The t model is applied to the differences; with a small sample, their distribution should be reasonably free of strong skewness and outliers.', { cognitiveProcess: 'verify' }),
  ...
  q(4, '4.5', '4.F', 'A one-sample t-test gives t = 2.4 and p = 0.02 for H_a: mu > 0. What does the p-value describe?', 'The probability, assuming mu = 0, of obtaining a test statistic at least as large as 2.4.', ['The probability that the population mean mu is exactly equal to 0.02 here.', 'The chance that the observed sample mean turns out to be exactly 2.4 again.', 'The probability that the alternative hypothesis is actually true in the population.'], 'The p-value is computed under the null model and follows the direction of the alternative.', { cognitiveProcess: 'interpret' }),
  q(4, '4.5', '4.G', 'A test uses alpha = 0.01 and p = 0.02. Which conclusion is correct?', 'Fail to reject the null hypothesis at the 1% significance level.', ['Reject the null hypothesis at the 1% significance level.', 'Accept the null hypothesis as having been proven true.', 'The p-value must be recalculated so that it becomes 0.99.'], 'The p-value exceeds alpha, so the result is not statistically significant at the stated level.', { cognitiveProcess: 'justify' }),
  ...
  q(4, '4.8', '4.G', 'A confidence interval for mu1 − mu2 is (−3.1, 1.4). What conclusion is supported?', 'There is not convincing evidence of a difference between the population means.', ['The first population mean is definitely the smaller of the two means.', 'The second population mean is definitely the smaller of the two means.', 'The two sample means must be exactly equal to one another here.'], 'Because zero is in the interval, a zero population difference remains plausible; the interval does not prove equality.', { cognitiveProcess: 'justify' }),
  ...
  q(4, '4.5', '4.G', 'A confidence interval and a two-sided hypothesis test use the same sample and matched significance level. Which relationship is expected?', 'The null value is rejected when it falls outside the corresponding confidence interval.', ['The null value is rejected only when it falls inside the confidence interval.', 'The interval must contain every individual observation in the whole sample.', 'The test and the interval always use two different population parameters.'], 'For matched procedures, a two-sided test at level alpha rejects the null value when that value is outside the corresponding 1 − alpha confidence interval.', { cognitiveProcess: 'connect' }),
  ...
  q(5, '5.3', '4.D', 'In the model y-hat = 12 + 4x, how should the slope be interpreted?', 'For each one-unit increase in x, the predicted value of y increases by 4 units.', ['Every observed value of y in the data is exactly equal to 4 units.', 'The correlation between x and y must therefore equal exactly 4 units.', 'When x is equal to zero, the predicted response must be exactly 4.'], 'The slope describes the change in predicted response for a one-unit increase in the explanatory variable.', { cognitiveProcess: 'interpret' }),
  q(5, '5.3', '4.D', 'What does the intercept represent in a linear regression model?', 'The predicted response when the explanatory variable equals zero, if that value is meaningful in context.', ['The strength of the association between the two variables in every possible context here', 'The largest observed response value in the whole of the data set that was collected here', 'The residual value for every single observation in the entire set of the collected data'], 'The intercept is a model prediction at x = 0; its contextual interpretation depends on whether x = 0 is reasonable and in the data range.', { cognitiveProcess: 'explain' }),
  ...
  q(5, '5.4', '4.A', 'A residual plot shows a curved pattern rather than random scatter around zero. What does that suggest?', 'A linear model may not adequately describe the relationship.', ['The correlation between the variables must be exactly 1.', 'The explanatory variable must be a categorical variable.', 'The fitted model has no residuals of any kind at all.'], 'Systematic curvature in residuals indicates that the linear form misses structure in the relationship.', { cognitiveProcess: 'interpret' }),
  q(5, '5.4', '4.B', 'A residual plot has one point far from the rest vertically but not far horizontally. How is the point best described?', 'An outlier in the response direction', ['A point with high leverage only', 'A point with a zero residual value', 'A sampling stratum in the data'], 'A large vertical residual indicates an unusual response relative to the model; leverage concerns an unusual explanatory-variable value.', { cognitiveProcess: 'classify' }),
  ...
  q(5, '5.5', '4.D', 'If r-squared = 0.64, what is the best interpretation in context?', 'About 64% of the variation in the response is explained by the linear regression model with the explanatory variable.', ['The model predicts every observed response to within 64 units of its actual observed value here.', 'The correlation between the variables is exactly 0.64 regardless of the direction of the association.', 'There is a 64% chance that the fitted model describes a causal relationship between the two variables.'], 'R-squared describes the proportion of response variability explained by the fitted linear model; it is not a probability of causation.', { cognitiveProcess: 'interpret' }),
  q(5, '5.5', '4.B', 'Which statement about least-squares regression is correct?', 'The least-squares line minimizes the sum of the squared residuals.', ['It minimizes the sum of the residuals without squaring them.', 'It passes through every one of the observed data points.', 'It makes every one of the residuals positive in value.'], 'Squaring residuals prevents cancellation and produces the line with the smallest total squared error.', { cognitiveProcess: 'identify' }),
  ...
  q(5, '5.5', '4.A', 'A residual plot shows roughly equal vertical spread across the x-values and no obvious pattern. What does that support?', 'The constant-variance and linear-form conditions are reasonably plausible.', ['The response variable must be a categorical variable instead.', 'The fitted line must therefore describe a causal relationship.', 'The model predicts every one of the data points exactly right.'], 'Random scatter with similar spread around zero supports, but does not prove, a useful linear model with roughly constant variability.', { cognitiveProcess: 'justify' }),
  q(5, '5.5', '4.G', 'A fitted regression line has a 95% confidence interval for its slope that includes 0. What is a cautious conclusion?', 'The data do not provide convincing evidence of a nonzero population linear association at that confidence level.', ['The two variables have been proven to be independent in every population of interest here.', 'The population slope must therefore be exactly equal to zero everywhere in the population.', 'The fitted model proves that there is a real causal effect between these two variables here.'], 'An interval containing zero means a zero slope remains plausible; it does not prove no association or establish causation.', { cognitiveProcess: 'justify' }),
  q(5, '5.1', '2.B', 'Which feature should be checked before using a linear regression model for a scatterplot?', 'The form, direction, strength, outliers, and influential points of the association', ['Only the largest response value that appears in the whole data set', 'Whether the graph happens to use a pie chart instead of a scatterplot', 'Whether the sample mean of the response happens to be equal to zero'], 'Model selection depends on the observed relationship and unusual points, not on one isolated summary.', { cognitiveProcess: 'select' }),
];

const expandedItemSpecs = [
  // Second angle for every public framework topic.
  q(1, '1.1', '1.B', 'A school wants to estimate the proportion of all enrolled students who regularly use public transportation. What is the parameter of interest?', 'The population proportion of enrolled students who regularly use public transportation.', ['The proportion within the sample of students who answered this survey question.', 'The total number of school buses that are used by the school every day.', 'The mean number of transportation trips that are made by one single student.'], 'A parameter is a numerical summary of the population, while the sample proportion is a statistic used to estimate it.', { cognitiveProcess: 'identify' }),
  ...
  q(1, '1.9', '4.A', 'Two distributions have equal means, but Distribution A has a smaller standard deviation. What is supported?', 'Distribution A has less typical distance from its mean.', ['Distribution A must therefore have a larger median.', 'Distribution A must therefore be skewed to the right.', 'Distribution A has no variability of any kind.'], 'Standard deviation describes typical distance from the mean; equal means do not determine shape or median.', { cognitiveProcess: 'compare' }),
  ...
  q(1, '1.13', '2.C', 'A study randomly assigns volunteers to either a new tutoring program or no program. Which conclusion is supported if the groups differ in scores?', 'The assignment supports a cause-and-effect conclusion about the treatments for the study participants.', ['The result automatically generalizes to every single student in the entire world today.', 'The study proves that the sample mean equals the whole population mean exactly here.', 'The result is only a description of an unassigned observational survey sample here.'], 'Random assignment supports causal comparison, while generalization requires an appropriate sampling design.', { cognitiveProcess: 'justify' }),

  q(2, '2.1', '4.A', 'A two-way table compares breakfast choice for athletes and nonathletes. What should be compared to assess whether choice differs by group?', 'The conditional percentages of breakfast choices within each group.', ['Only the grand total of all of the breakfast choices made.', 'The row labels by themselves without any of the counts.', 'The mean value of all of the categorical labels that are used.'], 'Conditional percentages use a common within-group denominator and reveal differences in the distributions.', { cognitiveProcess: 'select' }),
  ...
  q(2, '2.5', '4.B', 'Which statement distinguishes mutually exclusive events from independent events?', 'Mutually exclusive events cannot occur together, while independent events can occur together but do not change each other\'s probabilities.', ['The two terms always mean exactly the same thing in every probability problem that a student might ever encounter.', 'Independent events must always have exactly equal probabilities as each other in every case that could ever arise.', 'Mutually exclusive events must each have a probability of exactly 0.50 in every problem that might ever be posed.'], 'Mutual exclusivity concerns the intersection; independence concerns whether conditioning changes a probability.', { cognitiveProcess: 'compare' }),
  ...
  q(2, '2.7', '3.C', 'If P(A | B) = 0.42 and P(A) = 0.42, what does this support?', 'The events A and B are independent, assuming P(B) is positive.', ['The events A and B must therefore be mutually exclusive.', 'The probability of the event B must be exactly zero.', 'The events A and B must always occur at the same time.'], 'Independence is supported when conditioning on B does not change the probability of A.', { cognitiveProcess: 'infer' }),
  ...
  q(2, '2.9', '4.D', 'A random variable has expected value 12. Which interpretation is appropriate?', 'The long-run average value over many repetitions tends to approach 12.', ['Every one of the individual outcomes is exactly equal to 12.', 'The most likely single outcome must be exactly equal to 12.', 'The random variable has a standard deviation of exactly 12.'], 'Expected value describes a long-run center and does not guarantee an individual result or identify the mode.', { cognitiveProcess: 'interpret' }),
  ...
  q(2, '2.12', '4.C', 'For a population with standard deviation 18, what happens to the standard deviation of the sample mean when sample size changes from 25 to 100?', 'It is cut in half, from 3.6 to 1.8.', ['It doubles, going from 3.6 to 7.2.', 'It remains exactly 18 in both.', 'It becomes exactly zero here.'], 'The standard error is sigma/sqrt(n), so quadrupling n halves the standard error.', { cognitiveProcess: 'calculate' }),
  ...
  q(3, '3.10', '2.C', 'A confidence interval for p1 - p2 is based on independent random samples from two populations. Which parameter does it estimate?', 'The difference between the two population proportions.', ['The sum of the two sample counts and nothing more.', 'The mean of one single quantitative population only.', 'The correlation between two sets of measurements.'], 'The two-proportion interval targets p1 - p2, the difference in population proportions.', { cognitiveProcess: 'select' }),
  q(3, '3.11', '4.G', 'A confidence interval for p1 - p2 is (0.02, 0.18). What does the sign of the interval imply?', 'The first population proportion is plausibly greater than the second.', ['The second population proportion is definitely the greater one.', 'The two sample proportions must be exactly equal to each other.', 'The interval proves that a causal effect must exist here.'], 'An interval entirely above zero supports a positive difference in the stated order, subject to the design and conditions.', { cognitiveProcess: 'justify' }),
  ...
  q(3, '3.13', '4.G', 'A two-proportion test rejects H0. Which statement should be included in the conclusion?', 'The data provide statistically significant evidence for the stated alternative in the population context.', ['The null hypothesis is mathematically impossible in any possible population of interest.', 'The sample proportions are guaranteed to differ in every one of the future samples taken.', 'The result proves that one treatment caused the difference without a randomized design.'], 'A test conclusion describes evidence relative to the null and must respect the sampling and assignment design.', { cognitiveProcess: 'justify' }),
  ...
  q(4, '4.2', '2.C', 'A study measures each participant before and after an intervention. Which procedure targets the mean change?', 'A matched-pairs t procedure applied to the within-person differences.', ['A one-proportion z-test applied to the paired responses only', 'A chi-square test for independence between the variables', 'A two-sample procedure that ignores the pairing entirely'], 'Paired measurements should be reduced to one difference per participant before using a one-sample t procedure.', { cognitiveProcess: 'select' }),
  q(4, '4.3', '4.F', 'A 95% confidence interval for a population mean is (72, 80). What does the interval estimate?', 'The population mean is plausibly between 72 and 80 under the confidence procedure.', ['About 95% of the individual observations fall between the values 72 and 80.', 'The sample mean changes between 72 and 80 with a probability of 0.95.', 'Every future sample mean must fall inside this same confidence interval.'], 'A confidence interval targets the fixed population mean; it does not describe the middle 95% of individual observations.', { cognitiveProcess: 'interpret' }),
  ...
  q(4, '4.5', '4.G', 'A one-sample t-test gives p = 0.004. What does this indicate when the null model is true?', 'A result at least as extreme as the observed result would be unusual under the null model.', ['There is only a 0.4% chance that the null model is actually true for this population.', 'The observed sample mean is wrong by exactly 0.004 units in this particular study.', 'The population standard deviation must equal exactly 0.004 units in this study.'], 'The p-value is a probability under the null model, not a probability that the null or a measurement is correct.', { cognitiveProcess: 'interpret' }),
  ...
  q(4, '4.7', '2.C', 'Which procedure estimates the difference between two independent population means?', 'A two-sample t-interval for a difference in means.', ['A one-proportion z-interval for a proportion', 'A chi-square test for homogeneity of counts', 'A binomial probability calculation for counts'], 'Two independent quantitative samples with a mean difference as the target call for a two-sample t interval when conditions hold.', { cognitiveProcess: 'select' }),
  q(4, '4.8', '4.G', 'A confidence interval for mu1 - mu2 is (-1.4, 2.1). What is the cautious conclusion?', 'A zero difference remains plausible, so the interval does not show a clear population mean difference.', ['The first population mean is definitely the larger one of the two population means.', 'The two population means have been proven to be exactly equal by this one interval.', 'Every individual measurement has a difference that falls inside that same interval.'], 'Because zero lies in the interval, a zero population difference remains plausible; equality is not proven.', { cognitiveProcess: 'justify' }),
  ...
  q(4, '4.10', '4.G', 'A two-sample t-test has p = 0.23 at alpha = 0.05. What is the correct decision?', 'Fail to reject the null hypothesis because the evidence is not statistically significant at 5%.', ['Reject the null hypothesis because the p-value is not exactly equal to zero here.', 'Accept the null hypothesis as having been proven true by these particular data.', 'Conclude that the two sample means can never differ from one another at all.'], 'A p-value greater than alpha does not provide enough evidence for the alternative and does not prove the null.', { cognitiveProcess: 'justify' }),

  q(5, '5.1', '4.A', 'A scatterplot of hours studied and score shows an upward roughly linear pattern. What is supported?', 'The variables have a positive linear association.', ['The relationship must necessarily be causal.', 'The response variable must be categorical here.', 'The correlation must be exactly equal to zero.'], 'An upward linear pattern indicates positive association, but design and other features determine what conclusions are justified.', { cognitiveProcess: 'describe', difficulty: 'foundational' }),
  ...
  q(5, '5.4', '4.D', 'A regression slope is -2.5 minutes per additional practice session. What does the slope mean?', 'The predicted response decreases by 2.5 minutes for each additional practice session.', ['Every single student loses exactly 2.5 minutes of time in each practice session.', 'The intercept of the fitted model must be exactly -2.5 minutes in this case.', 'The correlation between the variables must therefore be exactly -2.5.'], 'The slope gives the predicted change in response for a one-unit increase in the explanatory variable, with units.', { cognitiveProcess: 'interpret' }),
  ...
  q(1, '1.13', '2.D', 'In a matched-pairs experiment, each participant tries both treatments in random order. What is analyzed?', 'The difference between the two responses for each participant.', ['The grand total of all of the treatment labels that are used.', 'Only the larger of the two responses in each matched pair.', 'The sample proportion within one of the treatment groups.'], 'Matched-pairs analysis reduces each pair to a within-pair difference before inference.', { cognitiveProcess: 'select' }),
  ...
  q(5, '5.4', '4.D', 'A residual plot has a clear curved pattern. What does this suggest about a linear model?', 'The linear form may not adequately describe the relationship.', ['The correlation must therefore be exactly equal to 1.', 'Every one of the residuals must be exactly zero here.', 'The response variable must necessarily be categorical.'], 'Systematic curvature in residuals indicates that the linear form misses structure in the relationship.', { cognitiveProcess: 'interpret' }),
  ...
  q(5, '5.5', '4.G', 'A confidence interval for the slope contains 0. What is a cautious conclusion?', 'The data do not provide convincing evidence of a nonzero population linear slope at that confidence level.', ['The population slope has been proven to be exactly zero in the entire population here.', 'The two variables are proven to be independent in every possible population studied here.', 'The fitted model proves that a real causal effect exists between the two variables here.'], 'An interval containing zero leaves a zero slope plausible; it does not prove no association or establish causation.', { cognitiveProcess: 'justify' }),
  q(3, '3.6', '4.F', 'A p-value of 0.48 is obtained. Which statement is accurate?', 'The observed result is reasonably compatible with the null model.', ['There is a 48% chance that the null model is actually true.', 'The alternative hypothesis has been disproven entirely.', 'The test statistic must therefore equal exactly zero.'], 'A large p-value means the result is not unusual under the null model; it does not assign a probability to the hypothesis.', { cognitiveProcess: 'interpret' }),
  q(3, '3.7', '4.G', 'A test rejects H0 at alpha = 0.10 but not at alpha = 0.05. What does this show?', 'The p-value is between 0.05 and 0.10.', ['The p-value must be exactly 0.05.', 'The null model has been proven true.', 'The sample was not a random one.'], 'Rejecting at the larger alpha but not the smaller one places the p-value between those cutoffs.', { cognitiveProcess: 'infer' }),
  ...
  q(3, '3.15', '4.F', 'A chi-square statistic is large relative to its degrees of freedom. What does that suggest?', 'The observed counts differ substantially from the expected counts under the null model.', ['Every one of the observed counts equals its expected count exactly here.', 'The two variables are proven to be causally related to each other here.', 'The sample size must have been exactly one single observation only here.'], 'The chi-square statistic aggregates squared observed-minus-expected discrepancies scaled by expected counts.', { cognitiveProcess: 'interpret' }),
  ...
  q(4, '4.2', '4.C', 'Why can a t procedure be appropriate for a small sample of quantitative differences?', 'The differences are reasonably free of strong skewness and outliers, and the design supports independence.', ['Small samples never need any one of the conditions to be checked in any way at all here.', 'The response variable must be categorical rather than quantitative in every single case.', 'The paired differences are simply ignored throughout the entire procedure used here now.'], 'For small samples, the shape and outlier conditions for the differences are important to the validity of a t procedure.', { cognitiveProcess: 'justify' }),
  q(4, '4.3', '4.F', 'A 95% confidence interval for mu1 - mu2 is (4, 11) units. What is supported?', 'The first population mean is plausibly greater than the second by 4 to 11 units.', ['The individual observations differ by exactly 4 to 11 units each.', 'The first sample mean must therefore be exactly equal to 4 units.', 'The interval proves that the first treatment caused the whole difference.'], 'The interval targets the population mean difference and is entirely positive; causal language requires an appropriate experiment.', { cognitiveProcess: 'interpret' }),
  ...
  q(4, '4.7', '2.C', 'A study compares the mean completion time for two unrelated random samples. Which feature makes a two-sample t interval appropriate?', 'The response is quantitative and the samples represent independent groups.', ['The response variable is a single categorical label in each group.', 'Each person contributes a single before-and-after pair of values.', 'The two groups were selected by voluntary response alone here.'], 'A two-sample t interval targets a difference in means for independent quantitative groups when conditions hold.', { cognitiveProcess: 'select' }),
  ...
  q(4, '4.10', '4.G', 'A two-sample test rejects H0 for a randomized experiment. What conclusion is justified?', 'There is evidence that the treatment causes a difference in the population represented by the experiment.', ['The result generalizes automatically to all of the people in any possible population here.', 'The p-value is the probability that the treatment actually works as it was intended to.', 'The two sample means will be exactly equal in every one of the future samples taken.'], 'Random assignment supports a causal conclusion for the experimental population, while generalization depends on how participants were sampled.', { cognitiveProcess: 'justify' }),

  q(5, '5.1', '4.A', 'A scatterplot has a strong downward curved pattern. What is the best first response?', 'Describe the form as nonlinear before considering a linear model.', ['Fit a straight line without checking the plot at all.', 'Conclude that the two variables must be independent here.', 'Replace both of the variables with categorical labels.'], 'A curved pattern may require a nonlinear model; the graph should be examined before selecting a linear method.', { cognitiveProcess: 'select' }),
  ...
  q(5, '5.3', '3.B', 'A model predicts monthly cost from number of service calls. What is interpolation?', 'Predicting for a number of service calls within the observed range.', ['Predicting far beyond the observed range of the data values.', 'Changing the measurement units of the response variable.', 'Randomly assigning the service calls across all of the months.'], 'Interpolation uses the model within the range of observed explanatory values and is generally less risky than extrapolation.', { cognitiveProcess: 'classify' }),
  q(5, '5.4', '4.B', 'An observation has a positive residual. What does that mean?', 'The observed response is above the value predicted by the model.', ['The explanatory value must be below its own mean value.', 'The correlation between the variables must be positive.', 'The observed response must be exactly equal to zero here.'], 'A positive residual is observed minus predicted and therefore places the observation above the fitted line.', { cognitiveProcess: 'interpret' }),
  q(5, '5.4', '4.B', 'A point has an unusual x-value but lies close to the fitted line vertically. What is it?', 'A high-leverage point that may or may not be influential.', ['A point with a guaranteed large residual value.', 'A categorical response variable in the whole data set.', 'A sampling error by definition in every possible case.'], 'Unusual explanatory values create leverage; influence depends on how much the fitted model changes when the point is considered.', { cognitiveProcess: 'classify' }),
  ...
  q(5, '5.5', '4.B', 'Why is predicting for x = 250 risky when the observed x-values range from 10 to 80?', 'The prediction is extrapolation far beyond the observed range.', ['The residual value must therefore be exactly zero here.', 'The response variable becomes a categorical variable.', 'The correlation becomes automatically equal to one here.'], 'A model may not preserve its pattern outside the observed range, so extrapolation is risky.', { cognitiveProcess: 'justify' }),
];

const transferItemSpecs = [
  // Transfer set: apply the same reasoning to new contexts.
  q(1, '1.1', '1.A', 'A school wants to know whether its students are satisfied with the lunch schedule. Which question is statistical?', 'What proportion of students would report being satisfied with the lunch schedule?', ['Is one particular named student satisfied with the lunch schedule?', 'What color are the tables in the cafeteria at the school this year?', 'Did the principal personally choose this lunch schedule all alone?'], 'The question anticipates variability across students and asks about a population proportion.', { cognitiveProcess: 'identify' }),
  ...
  q(2, '2.1', '4.A', 'If the conditional percentage of late arrivals is nearly the same for bus riders and walkers, what does that suggest?', 'The two categorical variables may have weak association in the sample.', ['The two categorical variables must be perfectly dependent.', 'The sample mean of all the data must be exactly zero here.', 'The categories must really be quantitative measurements here.'], 'Similar conditional distributions suggest little association, although sampling variation remains possible.', { cognitiveProcess: 'infer' }),
  q(2, '2.3', '3.C', 'A simulation estimates a probability as 0.41. What does repeating the simulation many more times generally do?', 'It tends to make the estimate more stable around the modeled probability.', ['It guarantees that the next repetition will be a success here.', 'It changes the actual event that is being modeled in the simulation.', 'It makes the estimated probability rise above a value of 1.'], 'More repetitions reduce random simulation error but do not guarantee an individual result.', { cognitiveProcess: 'explain' }),
  ...
  q(2, '2.9', '4.D', 'A game has expected payout 5 points and standard deviation 2 points. What does the standard deviation describe?', 'The typical distance of payouts from the long-run average of 5 points.', ['The probability of winning the game must be exactly 2 points.', 'Every single payout must be between 3 and 7 points exactly.', 'The single most common payout must be exactly 5 points here.'], 'Standard deviation measures spread around the expected value; it does not set a guaranteed range or identify the mode.', { cognitiveProcess: 'interpret' }),
  ...
  q(2, '2.12', '4.C', 'A population is strongly skewed, but random samples have size 100. Which statement is most reasonable about the sample mean?', 'Its sampling distribution may be approximately normal because of the Central Limit Theorem.', ['Every single observation in the population becomes normally distributed too.', 'The sample mean has no variability at all across any of the repeated samples.', 'The shape of the population is changed by the sampling process used here.'], 'For sufficiently large random samples, the sampling distribution of the mean can be approximately normal even when the population is skewed.', { cognitiveProcess: 'justify' }),

  q(3, '3.1', '4.B', 'Two estimators are both unbiased. Estimator A has smaller standard deviation. What is supported?', 'Estimator A is less variable while both estimators are centered at the parameter in the long run.', ['Estimator A equals the population parameter exactly in every sample that is taken.', 'Estimator B must therefore necessarily be a biased estimator of the parameter.', 'Both of the estimators have zero sampling variability in all repeated samples.'], 'Unbiasedness concerns center, while standard deviation compares sampling variability.', { cognitiveProcess: 'compare' }),
  ...
  q(3, '3.6', '4.F', 'A p-value is 0.20. Which statement is correct?', 'Results at least as extreme as the observed result are not especially unusual under the null model.', ['There is a 20% probability that the null hypothesis is actually true in this one case.', 'The alternative hypothesis is entirely impossible for this whole population here.', 'The observed sample proportion is wrong by exactly 20 percentage points in this study.'], 'The p-value describes extremeness under the null model, not the probability that a hypothesis is true.', { cognitiveProcess: 'interpret' }),
  ...
  q(3, '3.14', '2.C', 'For a chi-square procedure, why are very small expected counts a concern?', 'The chi-square approximation to the sampling distribution may be unreliable.', ['Small expected counts always prove complete independence here.', 'They make the two variables into quantitative measurements here.', 'They guarantee that the p-value will always be very small in every case.'], 'Chi-square procedures rely on conditions that keep the expected counts sufficiently large for the approximation.', { cognitiveProcess: 'justify' }),
  ...
  q(4, '4.6', '3.D', 'In a paired study, what parameter is estimated by the mean of the within-person differences?', 'The population mean difference between the two paired conditions.', ['The sum of the two separate population means added together.', 'The proportion of all of the participants who improved at all.', 'The correlation coefficient between the two conditions.'], 'Pairing creates one quantitative difference per unit, and the mean difference targets its population mean.', { cognitiveProcess: 'identify' }),
  ...
  q(4, '4.10', '4.G', 'A test of a mean difference produces p = 0.04 at alpha = 0.05. Which wording is best?', 'There is statistically significant evidence of a mean difference in the direction of the alternative.', ['There is only a 4% chance that the null hypothesis is actually true for this population.', 'The sample means will differ by exactly the same amount in every future study done here.', 'The result proves causation regardless of the design of the whole study that was used.'], 'A p-value below alpha supports a conclusion against the null, stated with the direction and design limits.', { cognitiveProcess: 'justify' }),
  ...
  q(5, '5.2', '3.B', 'If all response values are multiplied by 3, what happens to the correlation with the explanatory variable?', 'It remains the same if the multiplier is positive.', ['It triples along with the response values.', 'It becomes exactly zero after the change.', 'It changes its sign after the multiplication.'], 'Correlation is unchanged by positive linear rescaling of either variable.', { cognitiveProcess: 'explain' }),
  ...
  q(5, '5.4', '4.D', 'A regression line has intercept 6. What does the intercept represent?', 'The predicted response when the explanatory variable equals zero, if that value is meaningful in context.', ['The correlation coefficient between the two variables in this one fitted model here.', 'The residual value for every one of the observations in the entire collected data set.', 'The predicted response at the sample mean of the explanatory variable and nowhere else.'], 'The intercept is the model prediction at x = 0, but interpretation requires x = 0 to be meaningful and within scope.', { cognitiveProcess: 'interpret' }),
  ...
  q(5, '5.5', '4.D', 'A model has r-squared = 0.49. Which statement is appropriate?', 'The linear model explains about 49% of the observed variation in the response.', ['The correlation must be exactly 0.49 regardless of its direction.', 'The model is correct for exactly 49% of all of the individuals.', 'There is a 49% chance that the association is truly causal here.'], 'R-squared is the proportion of response variation explained by the fitted linear model, not a probability or accuracy rate.', { cognitiveProcess: 'interpret' }),
...
function buildLibrary(objectiveCatalog) {
  ...
  const quickReference = [
    ['ap-stats-reference-u1', 1, 'Describe the variable and distribution before calculating.', ['Classify the variable as categorical or quantitative.', 'Describe shape, center, spread, and unusual features.', 'Separate random sampling, random assignment, and observational evidence.'], ['z = (value - mean) / standard deviation', 'IQR = Q3 - Q1', 'Outlier fences: Q1 - 1.5(IQR) and Q3 + 1.5(IQR)'], ['Use median and IQR for skewed distributions or outliers.', 'Use a segmented bar chart for conditional categorical comparisons.', 'Generalization needs random sampling; causation needs random assignment.'], ['The mean is not resistant to outliers.', 'A large convenience sample is still vulnerable to bias.']],
    ['ap-stats-reference-u2', 2, 'Define the event or random variable, then verify the model conditions.', ['Write the sample space or event notation.', 'Check the denominator for conditional probability.', 'Verify fixed trials, independence, constant probability, or normal-model conditions.'], ['P(A given B) = P(A and B) / P(B)', 'Expected value = sum of (outcome)(probability)', 'For a binomial variable: mean = np and standard deviation = square root of np(1-p)'], ['Use complements for at least one or none when appropriate.', 'A probability describes long-run behavior, not a guarantee for one outcome.', 'The Central Limit Theorem describes a sampling distribution, not every individual observation.'], ['Mutually exclusive positive-probability events are not independent.', 'Do not call a model binomial when trials or success probabilities change.']],
    ['ap-stats-reference-u3', 3, 'Name the population proportion or difference, check conditions, and interpret evidence.', ['State the parameter and hypotheses in population language.', 'Check randomization, independence, and large-count conditions.', 'Match a one-proportion, two-proportion, or chi-square procedure to the design.'], ['One-proportion standard error under a null: square root of p0(1-p0)/n', 'Two-proportion standard error: square root of p-hat1(1-p-hat1)/n1 + p-hat2(1-p-hat2)/n2', 'Chi-square statistic = sum of (observed - expected)^2 / expected'], ['A p-value is calculated assuming the null model.', 'A confidence interval gives plausible values for a population parameter.', 'A large p-value is not proof that the null is true.'], ['Do not use a sample statistic as the parameter in a hypothesis.', 'Expected counts come from the categorical null model.']],
    ['ap-stats-reference-u4', 4, 'Identify the mean target and data design before selecting a t procedure.', ['Decide between one-sample, paired, and two-sample data.', 'Define the order of a mean difference.', 'Check randomization, independence, shape, outliers, and sample size.'], ['t = (statistic - null value) / standard error', 'One-mean standard error = sample standard deviation / square root of n', 'Confidence interval = statistic plus or minus (critical t)(standard error)'], ['Paired data become one sample of within-pair differences.', 'Use t procedures when population standard deviation is estimated from the sample.', 'Interpret results in the original measurement units.'], ['A t procedure does not repair a biased design.', 'Statistical significance does not automatically imply practical importance.']],
    ['ap-stats-reference-u5', 5, 'Describe the association, inspect residuals, and limit predictions to what the design supports.', ['Describe direction, form, strength, clusters, outliers, and leverage.', 'Interpret slope, intercept, residuals, and r-squared with units.', 'Check residual patterns and the observed x-range before predicting.'], ['Residual = observed response - predicted response', 'r-squared = proportion of response variation explained by the fitted line', 'Predicted response = intercept + slope(explanatory value)'], ['Random residual scatter supports a linear form.', 'Extrapolation is prediction outside the observed explanatory-variable range.', 'Association or r-squared does not establish causation.'], ['The intercept may lack meaning outside the observed x-range.', 'A strong correlation can coexist with lurking variables or a noncausal design.']],
  ].map(([id, unitNumber, purpose, checklist, formulas, decisionRules, cautions]) => {
    const unit = units[unitNumber - 1];
    return {
      id,
      chapterId: 'ap-stats-ch-' + String(unit.number).padStart(2, '0'),
      domainId: unit.id,
      title: unit.shortLabel + ' quick reference',
      purpose,
      checklist,
      formulas,
      decisionRules,
      ...
      reviewStatus: 'source-reviewed-editorial-pass',
      reviewNote: 'Original quick-reference wording; independent AP Statistics subject-expert, accessibility, rights, production, and psychometric review remain pending.',
      releaseEligible: false,
    };
  });
  ...
  return {
    ...
    blueprint: {
      ...
      pilotNote: 'A 240-item foundation sampler across the five revised Fall 2026 units, with two hundred forty original single-choice items, fifteen section-linked practice routes, four guided study routes, five original unit quick references, native chapters, and nine unscored constructed-response planning workshops.',
      skills,
...
function buildPack(library, objectiveCatalog) {
  ...
  return {
    ...
    blueprint: {
      ...
      unitWeights: units.map((unit) => ({ id: unit.id, label: unit.label, officialWeightRange: [unit.officialWeightMin, unit.officialWeightMax] })),
      pilotAlignment: '240-item text-first foundation sampler across all five revised units and all fifty-five public framework topic IDs; forty-eight five-item internal banks; fifteen section-linked practice routes; four guided cumulative routes; transfer practice beyond the two-item topic floor; nine unscored constructed-response planning workshops; no official stimulus sets or FRQ scoring.',
