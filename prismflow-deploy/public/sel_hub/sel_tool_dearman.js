    ...
    render: function(ctx) {
      ...
      function header() {
        return h('div', { className: 'no-print', style: { display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap' } },
          h('button', { onClick: function() { setSelHubTool(null); }, 'aria-label': 'Back to SEL Hub',
            style: { background: 'rgba(255,255,255,0.05)', border: '1px solid #334155', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', color: _deFg('#cbd5e1'), fontSize: 14 } }, '← Back'),
          h('div', { style: { flex: 1, minWidth: 260 } },
            h('h2', { style: { margin: 0, color: _deFg('#93c5fd'), fontSize: 22, fontWeight: 900 } }, '🗣️ DEAR MAN'),
            h('div', { style: { fontSize: 12, color: _deFg('#94a3b8'), marginTop: 4, lineHeight: 1.5 } }, 'A seven-step script for a hard ask.')
          )
        );
      }
      ...
      function softPointer() {
        return h('div', { className: 'no-print',
          style: { marginTop: 16, padding: '8px 12px', borderRadius: 8, background: 'rgba(15,23,42,0.5)', border: '1px solid #334155', fontSize: 11, color: _deFg('#94a3b8'), lineHeight: 1.5, fontStyle: 'italic' }
        },
      ...
      function renderHome() {
        ...
        return h('div', null,
          ...
          h('div', { style: { padding: 14, borderRadius: 10, background: _deBg('#0f172a'), borderTop: '1px solid #1e293b', borderRight: '1px solid #1e293b', borderBottom: '1px solid #1e293b', borderLeft: '3px solid #3b82f6', marginBottom: 12 } },
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 } },
              h('div', null,
                h('label', { htmlFor: 'dm-ask', style: { display: 'block', fontSize: 11, color: _deFg('#93c5fd'), fontWeight: 800, marginBottom: 4 } }, 'What am I asking for, in one sentence?'),
                h('input', { id: 'dm-ask', type: 'text', value: d.ask || '',
                  ...
                  style: { width: '100%', padding: 8, borderRadius: 6, border: '1px solid #334155', background: _deBg('#1e293b'), color: _deFg('#e2e8f0'), fontSize: 13 } })
              ),
              h('div', null,
                h('label', { htmlFor: 'dm-audience', style: { display: 'block', fontSize: 11, color: _deFg('#93c5fd'), fontWeight: 800, marginBottom: 4 } }, 'Who am I asking?'),
          ...
          LETTERS.map(function(L, idx) {
            ...
            return h('div', { key: L.id, style: { padding: 14, borderRadius: 10, background: _deBg('#0f172a'), borderTop: '1px solid #1e293b', borderRight: '1px solid #1e293b', borderBottom: '1px solid #1e293b', borderLeft: '4px solid ' + L.color, marginBottom: 10 } },
              ...
              h('details', { style: { marginBottom: 8 } },
                h('summary', { style: { cursor: 'pointer', fontSize: 11, color: _deFg('#94a3b8'), fontStyle: 'italic' } }, '💡 Example'),
                h('div', { style: { padding: 8, borderRadius: 6, background: _deBg('#1e293b'), fontSize: 12, color: _deFg('#cbd5e1'), marginTop: 6, lineHeight: 1.6, fontStyle: 'italic' } }, '"' + L.example + '"')
              ),
              h('label', { htmlFor: 'dm-' + L.id, style: { display: 'block', fontSize: 11, color: _deFg('#94a3b8'), fontWeight: 700, marginBottom: 4 } }, L.prompt),
              h('textarea', { id: 'dm-' + L.id, value: v,
                placeholder: 'In your own words...',
                onChange: function(e) { setResponse(L.id, e.target.value); },
                style: { width: '100%', minHeight: 60, padding: 8, borderRadius: 6, border: '1px solid #334155', background: _deBg('#1e293b'), color: _deFg('#e2e8f0'), fontSize: 13, fontFamily: 'inherit', lineHeight: 1.6, resize: 'vertical' } })
            );
          }),
      ...
      function renderScript() {
        ...
        return h('div', null,
          ...
          h('div', { style: { padding: 10, borderRadius: 8, background: _deBg('#0f172a'), border: '1px solid #1e293b', marginBottom: 12, fontSize: 12, color: _deFg('#cbd5e1'), lineHeight: 1.6 } },
            d.audience ? h('div', null, h('strong', { style: { color: _deFg('#93c5fd') } }, 'Asking: '), d.audience) : null,
          ...
          h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(59,130,246,0.10)', borderTop: '1px solid rgba(59,130,246,0.3)', borderRight: '1px solid rgba(59,130,246,0.3)', borderBottom: '1px solid rgba(59,130,246,0.3)', borderLeft: '3px solid #3b82f6', marginTop: 12, marginBottom: 8 } },
            ...
            h('div', { style: { fontSize: 12.5, color: _deFg('#dbeafe'), lineHeight: 1.7 } },
              ...
              h('div', null, '• Practice count: ', h('strong', { style: { color: _deFg('#bfdbfe') } }, d.practiceCount || 0), '  ',
                h('button', { onClick: logPractice, 'aria-label': 'Log a practice',
                  style: { padding: '4px 10px', borderRadius: 6, border: '1px solid #3b82f6', background: 'transparent', color: _deFg('#bfdbfe'), cursor: 'pointer', fontSize: 11, fontWeight: 700, marginLeft: 4 } }, '+ Practiced once'))
            )
          ),

          h('div', { style: { display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' } },
            h('button', { onClick: function() { goto('home'); }, 'aria-label': 'Edit script',
              style: { padding: '8px 18px', borderRadius: 8, border: '1px solid #475569', background: _deBg('#1e293b'), color: _deFg('#cbd5e1'), cursor: 'pointer', fontWeight: 700, fontSize: 13 } }, '✏️ Edit'),
      ...
      function renderPrintView() {
        ...
        return h('div', null,
          h('div', { className: 'no-print', style: { display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap', padding: 12, background: 'rgba(59,130,246,0.10)', borderRadius: 8, border: '1px solid rgba(59,130,246,0.3)' } },
            h('div', { style: { flex: 1, minWidth: 200, fontSize: 12.5, color: _deFg('#bfdbfe'), lineHeight: 1.55 } },
          ...
          h('div', {
            ...
            style: { maxWidth: 760, margin: '0 auto', padding: 32, background: _deBg('#fff'), color: _deFg('#0f172a'), borderRadius: 8, border: '1px solid #cbd5e1', fontFamily: '"Helvetica Neue", Arial, sans-serif' }
          },
            h('style', null,
            ...
            h('div', { style: { paddingBottom: 14, marginBottom: 20, borderBottom: '3px solid #2563eb' } },
              h('div', { style: { fontSize: 10, color: _deFg('#64748b'), textTransform: 'uppercase', letterSpacing: 1, fontWeight: 700, marginBottom: 2 } }, 'DEAR MAN · DBT Assertive Ask'),
              h('h1', { style: { margin: 0, fontSize: 22, fontWeight: 900 } }, d.ask || 'My ask'),
              d.audience ? h('div', { style: { fontSize: 13, color: _deFg('#475569'), marginTop: 4 } }, 'For: ' + d.audience) : null
            ),

            // Full script first
            script ? h('div', { style: { padding: 14, background: _deBg('#dbeafe'), borderLeft: '4px solid #2563eb', marginBottom: 18 } },
              h('div', { style: { fontSize: 10, color: _deFg('#64748b'), textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: 800, marginBottom: 4 } }, 'Full script'),
              h('p', { style: { margin: 0, color: _deFg('#0f172a'), fontSize: 13.5, lineHeight: 1.75, whiteSpace: 'pre-wrap' } }, script)
            ) : null,
            ...
            LETTERS.map(function(L) {
              ...
              return h('div', { key: L.id, style: { marginBottom: 12, pageBreakInside: 'avoid' } },
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', borderRadius: 4, marginBottom: 6, background: L.color, color: _deFg('#fff') } },
                ...
                h('p', { style: { margin: '0 8px', color: _deFg('#0f172a'), fontSize: 13, lineHeight: 1.65, whiteSpace: 'pre-wrap' } }, v)
              );
            }),
      ...
      function renderAbout() {
        return h('div', null,
          ...
          h('div', { style: { padding: 16, borderRadius: 12, background: _deBg('#0f172a'), border: '1px solid #1e293b', marginBottom: 12 } },
            h('h3', { style: { margin: '0 0 10px', color: _deFg('#93c5fd'), fontSize: 16 } }, 'What DEAR MAN is'),
            h('p', { style: { margin: '0 0 10px', color: _deFg('#e2e8f0'), fontSize: 13.5, lineHeight: 1.7 } },
            ...
            h('p', { style: { margin: 0, color: _deFg('#e2e8f0'), fontSize: 13.5, lineHeight: 1.7 } },
          ...
          h('div', { style: { padding: 16, borderRadius: 12, background: _deBg('#0f172a'), border: '1px solid #1e293b', marginBottom: 12 } },
            h('h3', { style: { margin: '0 0 10px', color: _deFg('#93c5fd'), fontSize: 16 } }, 'GIVE and FAST: the two modifiers'),
            h('p', { style: { margin: '0 0 10px', color: _deFg('#cbd5e1'), fontSize: 13, lineHeight: 1.7 } },
              'DEAR MAN focuses on getting the result. DBT offers two companion skills you can layer on:'),
            h('div', { style: { padding: 10, borderRadius: 8, background: _deBg('#1e293b'), marginBottom: 8 } },
              h('div', { style: { fontSize: 12, fontWeight: 800, color: _deFg('#22c55e'), marginBottom: 6 } }, '🤝 GIVE — relationship-preserving'),
              h('div', { style: { fontSize: 12, color: _deFg('#cbd5e1'), lineHeight: 1.65 } },
            ...
            h('div', { style: { padding: 10, borderRadius: 8, background: _deBg('#1e293b') } },
              h('div', { style: { fontSize: 12, fontWeight: 800, color: _deFg('#a855f7'), marginBottom: 6 } }, '⚖️ FAST — self-respect-preserving'),
              h('div', { style: { fontSize: 12, color: _deFg('#cbd5e1'), lineHeight: 1.65 } },
          ...
          h('div', { style: { padding: 16, borderRadius: 12, background: _deBg('#0f172a'), border: '1px solid #1e293b', marginBottom: 12 } },
            h('h3', { style: { margin: '0 0 10px', color: _deFg('#93c5fd'), fontSize: 16 } }, 'Where DEAR MAN comes from'),
            h('p', { style: { margin: 0, color: _deFg('#cbd5e1'), fontSize: 13, lineHeight: 1.7 } },
          ...
          h('div', { style: { padding: 16, borderRadius: 12, background: _deBg('#0f172a'), border: '1px solid #1e293b', marginBottom: 12 } },
            h('h3', { style: { margin: '0 0 10px', color: _deFg('#93c5fd'), fontSize: 16 } }, '📚 Sources and learn more'),
            h('div', { style: { fontSize: 12, color: _deFg('#94a3b8'), marginBottom: 10, lineHeight: 1.55 } }, 'Authoritative resources for DEAR MAN and DBT Interpersonal Effectiveness.'),
          ...
          h('div', { style: { padding: 16, borderRadius: 12, background: 'rgba(245,158,11,0.08)', borderTop: '1px solid rgba(245,158,11,0.3)', borderRight: '1px solid rgba(245,158,11,0.3)', borderBottom: '1px solid rgba(245,158,11,0.3)', borderLeft: '3px solid #f59e0b', marginBottom: 12 } },
            h('h3', { style: { margin: '0 0 10px', color: _deFg('#fcd34d'), fontSize: 15 } }, '⚖️ Honest limits'),
            h('ul', { style: { margin: 0, padding: '0 0 0 20px', color: _deFg('#fde68a'), fontSize: 13, lineHeight: 1.75 } },
          ...
          h('div', { style: { padding: 12, borderRadius: 10, background: 'rgba(59,130,246,0.10)', borderTop: '1px solid rgba(59,130,246,0.3)', borderRight: '1px solid rgba(59,130,246,0.3)', borderBottom: '1px solid rgba(59,130,246,0.3)', borderLeft: '3px solid #3b82f6', fontSize: 12.5, color: _deFg('#bfdbfe'), lineHeight: 1.6 } },
      ...
      function sourceCard(authorYear, title, blurb, url) {
        return h('div', { style: { padding: 10, borderRadius: 8, background: _deBg('#1e293b'), border: '1px solid #334155', marginBottom: 8 } },
          h('div', { style: { fontSize: 11, color: _deFg('#93c5fd'), fontWeight: 700, marginBottom: 2 } }, authorYear),
          url
            ? h('a', { href: url, target: '_blank', rel: 'noopener noreferrer',
                style: { fontSize: 13, color: _deFg('#bfdbfe'), fontWeight: 700, textDecoration: 'underline', display: 'block', marginBottom: 4 } }, title + ' ↗')
            : h('div', { style: { fontSize: 13, color: _deFg('#bfdbfe'), fontWeight: 700, marginBottom: 4 } }, title),
          h('div', { style: { fontSize: 12, color: _deFg('#cbd5e1'), lineHeight: 1.55 } }, blurb)
        );
      }
