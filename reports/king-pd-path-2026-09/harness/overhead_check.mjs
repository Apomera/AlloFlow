...
for (let i = 0; i < rounds; i++) {
  for (const variant of (i % 2 ? ['min', 'full'] : ['full', 'min'])) {
    ...
    const lp = await page.waitForFunction((v) => { const x = v === 'full' ? (window.__k4 && window.__k4.marks.launchpad) : (window.__min && window.__min.lp); return x || null; }, variant, { timeout: 180000, polling: 250 }).then((h) => h.jsonValue()).catch(() => null);
...
fs.writeFileSync('C:/tmp/alloflow_dispatch/wave1/K4_scratch/overhead_check.json', JSON.stringify(out, null, 1));
