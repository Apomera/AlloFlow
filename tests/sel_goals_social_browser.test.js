...
const examples = ['elementary','middle','high'].flatMap(band=>library.social.map(item=>({band,item})));
const reports = path.join(root, 'reports/sel-goals-social');
let browser, page;
let errors = [];
...
describe('Goal Setter social depth', () => {
  beforeAll(async () => {
  ...
  it.each(['elementary','middle','high'])('keeps every library example available for %s without creating goals while browsing',async band=>{
    await mount(band);
    for(const [cat,items] of Object.entries(library)) {
      await guide().getByRole('button',{name:'Filter SMART examples by '+({academic:'Academic',social:'Social & Friendship',personal:'Personal Growth',health:'Health & Wellness',creative:'Creative',community:'Community'})[cat],exact:true}).click();
      for(const [index,item] of items.entries()) {
        await chooser().selectOption(item.id||'example-'+index);
        for(const key of 'SMART') expect(await guide().innerText()).toContain(item.smart[key][band]);
      }
    }
    expect((await snapshot()).goals).toBeUndefined();
    expect(await page.evaluate(()=>window.depthXP)).toEqual([]);
    expect(errors).toEqual([]);
  },120000);

  it('remembers selections independently by category and band across closure and serialized restoration',async()=>{
    await mount();
    await chooser().selectOption('boundary');
    await guide().getByRole('button',{name:'Filter SMART examples by Academic',exact:true}).click();
    await chooser().selectOption('example-1');
    await guide().getByRole('button',{name:'Filter SMART examples by Social & Friendship',exact:true}).click();
    expect(await chooser().inputValue()).toBe('boundary');
    await page.evaluate(()=>window.depthSetBand('elementary'));
    expect(await chooser().inputValue()).toBe('invitation');
    await chooser().selectOption('support');
    ...
    expect(await page.getByRole('button',{name:'SMART Goal Examples Library',exact:true}).getAttribute('aria-expanded')).toBe('false');
    await page.getByRole('button',{name:'SMART Goal Examples Library',exact:true}).click();
    expect(await chooser().inputValue()).toBe('boundary');
    await page.evaluate(()=>window.depthSetBand('elementary'));
    expect(await chooser().inputValue()).toBe('support');
    expect(errors).toEqual([]);
  },120000);

  it('falls back safely for malformed saved selections and retains keyboard focus',async()=>{
    await mount('middle','light',1100,{smartExampleCat:'__proto__',smartExampleSelections:[]});
    expect(await chooser().inputValue()).toBe('example-0');
    await mount('middle','light',1100,{smartExampleSelections:{'middle:social':'missing'}});
    expect(await chooser().inputValue()).toBe('invitation');
    ...
    expect(await chooser().inputValue()).toBe('support');
    const category=guide().getByRole('button',{name:'Filter SMART examples by Academic',exact:true});
    await category.focus();await page.keyboard.press('Enter');
    expect(await category.evaluate(node=>node===document.activeElement)).toBe(true);
    expect(errors).toEqual([]);
  },120000);
