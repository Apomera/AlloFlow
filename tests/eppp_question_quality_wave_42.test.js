...
const wave = require('../dev-tools/eppp_native_quality_wave_42_data.cjs');
...
describe('EPPP expanded quality wave 42', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_42.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 21, applicationItems: 12, analysisItems: 12, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    ...
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(431);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1200);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(578);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(570);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_42.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_42.json'));
  });

  it('replays after wave 41', () => {
    ...
    expect(i42).toBeGreaterThan(i41);
  });
});
