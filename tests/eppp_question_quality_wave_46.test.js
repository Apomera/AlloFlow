...
const wave = require('../dev-tools/eppp_native_quality_wave_46_data.cjs');
...
describe('EPPP expanded quality wave 46', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_46.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 24, applicationItems: 9, analysisItems: 15, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    expect(audit.summary.distractorWarningsAfter.uniqueKeyStemLexicalLeakageCandidates).toBe(27);
    expect(audit.summary.distractorWarningsAfter.asymmetricExtremeDistractorCandidates).toBe(31);
    expect(audit.summary.distractorWarningsAfter.advancedDirectRecallCandidates).toBe(1);
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicatePairs).toBe(42);
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicateClusters).toBe(30);
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(391);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1091);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(540);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(501);
    expect(audit.summary.feedbackWarningsAfter.choiceRestatementOptions).toBe(116);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_46.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_46.json'));
  });

  it('replays after wave 45', () => {
    const builder = read('dev-tools/build_eppp_1500_expansion.cjs');
    const i45 = builder.indexOf('repair_eppp_native_quality_wave_45.cjs');
    const i46 = builder.indexOf('repair_eppp_native_quality_wave_46.cjs', i45);
