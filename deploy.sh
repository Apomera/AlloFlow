...
if [[ "${SKIP_RENDER_CHECK:-0}" != "1" ]]; then
  ...
  node dev-tools/check_quarantine.cjs --paths-only
  echo "  ✓ tests/QUARANTINE.txt has no stale paths (a deleted test must leave the known-failure list, or the blocking CI job silently stops covering it)."
  ...
  echo "  ✓ agent_core_* modules match their hand-copied mirrors. These have NO builder, and the two copies have different consumers: desktop/mcp/alloflow-mcp-stdio.cjs requires the ROOT copy while the web app loads desktop/web-app/public. Drift makes an MCP agent and the teacher's UI disagree about the same blueprint contract — the agent validates against one set of rules and the UI executes under another; 2026-07-28."
  node dev-tools/check_da_clinical_isolation.cjs
  echo "  ✓ Dynamic Assessment supports inherit NO ambient lesson context (topic, vocabulary, standards, roster differentiation, student interests). The DA host passed { isolatedContext: true } into the shared dispatcher from the Phase-4 organizer work onward, under a comment promising it suppressed all of that — and the dispatcher never read the flag, so every DA support silently inherited the open lesson. A confounded support invalidates the modifiability measure; 2026-07-28."
  node dev-tools/check_dead_signals.cjs
  echo "  ✓ no dead signals — every offered gameType is emitted, every persisted ledger field has a writer, every AlloHaven badge requires a producible earn source, and every AlloHaven quest counter is incremented somewhere. Three defects of this shape shipped before the check existed: a '+ Word Scramble' goal whose game had no onGameComplete prop (uncheckable for weeks), a timeOnTask ledger saved AND reported to teachers as engagement.timeOnTaskMinutes while nothing ever wrote it (a fabricated zero beside real numbers), and timelineGame/conceptSortGame seed keys nothing emits. Every other gate passes these: the code parses, renders, persists and reports — it is just permanently empty. Verified by a 5-case mutation battery, because two earlier drafts of this very check were themselves silently permissive; 2026-07-28."
  ...
  node dev-tools/check_stem_ratio_claims.cjs
  echo "  ✓ no NEW unreviewed ratio claim in a STEM tool ('about N times', 'N orders of magnitude', 'Nx more'). A ratchet, not a fact-checker: most such claims cite outside facts and cannot be verified mechanically, so the gate only demands that a NEW one is worked out by a person before it is baselined. Three shipped wrong and were found by hand on 2026-09-20 — nuclearLab said its U-238 chain spans '24 orders of magnitude' when its OWN table spans 10^20.9; flightsim said Mach 9.6 is 'about 50 times faster than highway speeds' when the nearest antecedent (SR-71) is ~34x and the X-43 is ~108x, so 50 matched neither reading; nutritionLab said the kidneys filter 'your whole blood volume about 60 times a day' when 60x is the PLASMA figure and stem_tool_anatomy.js already said 40x for blood — two tools contradicting each other on the same physiology. Every other gate passes these: the code parses, renders and tests green, the sentence is just false. 95 claims across 34 tools baselined."
