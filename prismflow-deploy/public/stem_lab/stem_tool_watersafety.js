...
// ═══════════════════════════════════════════════════════════════════════════
// stem_tool_watersafety.js — Water Safety Lab
...
if (!(window.StemLab.isRegistered && window.StemLab.isRegistered('waterSafety'))) {

(function() {
  'use strict';

  window.StemLab.registerTool('waterSafety', {
    name: 'Water Safety Lab',
    icon: '🛟',
    category: 'life-skills',
    description: 'Recognize water hazards, respond to aquatic emergencies, and reduce drowning risk. Cold water shock, rip currents, reach/throw rescue, life jackets, ice safety + self-rescue, hypothermia, pool drain entrapment, and autism-specific water safety. Sources cited inline (CDC, USCG, AAP, NAA, ASAN, DAN, NOAA). Educational only — find a Water Safety Instructor for actual swim training.',
    tags: ['water-safety', 'drowning-prevention', 'cold-water-shock', 'rip-currents', 'ice-safety', 'hypothermia', 'autism-safety', 'maine', 'life-skills', 'safety'],
    ...
    render: function(ctx) {
      ...
      var d = (ctx.toolData && ctx.toolData['waterSafety']) || {};
      var upd = function(key, val) { ctx.update('waterSafety', key, val); };
      var updMulti = function(obj) {
        if (ctx.updateMulti) ctx.updateMulti('waterSafety', obj);
        else Object.keys(obj).forEach(function(k) { upd(k, obj[k]); });
      };
      var addToast = ctx.addToast || function(msg) { console.log('[WaterSafety]', msg); };
      ...
      // ── State schema ──
      var view = d.view || (d.consentAccepted ? 'menu' : 'consent');
      ...
      if (!_wsHydrated.current) {
        ...
        try {
          var winState = (typeof window !== 'undefined' && window.__alloflowWaterSafety) || null;
          var lsState = null;
          try { lsState = JSON.parse(localStorage.getItem('waterSafety.state.v1') || 'null'); } catch (e) {}
      ...
      useEffect(function() {
        try {
          ...
          window.__alloflowWaterSafety = snap;
          localStorage.setItem('waterSafety.state.v1', JSON.stringify(snap));
        } catch (e) {}
      ...
      function renderConsent() {
        return h('div', { style: { padding: 24, maxWidth: 760, margin: '0 auto', color: T.text } },
          h('div', { role: 'region', 'aria-label': 'Water Safety Lab consent and educational scope',
            ...
            h('h2', { style: { margin: '0 0 12px', fontSize: 22, color: T.accentHi } },
              '🛟 Water Safety Lab is an EDUCATIONAL tool.'),
            ...
            h('button', {
              ...
              onClick: function() {
                ...
                awardBadge('water_safety_starter', 'Water Safety Starter');
                wsAnnounce('Consent accepted. Welcome to Water Safety Lab.');
              },
      ...
      function backBar(title) {
        return h('div', { style: { display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14, flexWrap: 'wrap' } },
          h('button', {
            'aria-label': 'Back to Water Safety Lab menu',
      ...
      // ─────────────────────────────────────────
      var MENU_TILES = [
        { id: 'coldShock',    icon: '🥶', label: 'Cold Water Shock & 1-1-10', desc: 'The gasp reflex, mammalian dive response, and the survival timeline that beats panic.', ready: true },
        { id: 'ripCurrents',  icon: '🌊', label: 'Rip Currents',              desc: 'Recognize them from shore. The parallel-to-shore escape. Why fighting the current kills.', ready: true },
        { id: 'reachThrow',   icon: '🪢', label: 'Reach / Throw / Row / Don\'t Go', desc: 'Bystander rescue principles. When NOT to enter the water (almost always).', ready: true },
        { id: 'pfd',          icon: '🦺', label: 'Life Jackets (PFDs)',       desc: 'Types I-V. Fit check that actually works. The 86% statistic. Maine boat law.', ready: true },
        { id: 'iceSafety',    icon: '🧊', label: 'Ice Safety + Self-Rescue',  desc: 'Maine-relevant. Ice color cues. The swim-and-roll. Picks of Life. Snowmobile through-ice.', ready: true },
        { id: 'hypothermia',  icon: '🌡️', label: 'Hypothermia Recognition',   desc: 'The UMBLES rule. Stages. Why rough handling can kill. What NOT to do.', ready: true },
        { id: 'drainEntrap',  icon: '🏊', label: 'Pool Drain Entrapment',     desc: 'The Virginia Graeme Baker Act. Five entrapment types. What to look for in a pool.', ready: true },
        { id: 'autismWater',  icon: '♾️', label: 'Autism + Water Safety',     desc: 'The 160x drowning risk. Defense-in-depth prevention. Adapted swim instruction.', ready: true },
        { id: 'cumulative',   icon: '🎯', label: 'Cumulative Quiz',           desc: '12 questions across all 8 modules. Tracks your mastery.', ready: true },
        { id: 'askLifeguard', icon: '🤖', label: 'Ask the Lifeguard (AI)',    desc: 'Type a water-safety question; AI returns a sourced answer. Educational only.', ready: true },
      ...
      function renderMenu() {
        ...
        return h('div', { style: { padding: 20, maxWidth: 980, margin: '0 auto', color: T.text } },
          ...
          h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 8 } },
            h('h2', { style: { margin: 0, fontSize: 22 } }, '🛟 Water Safety Lab'),
          ...
          h('p', { style: { margin: '0 0 16px', color: T.muted, fontSize: 13, lineHeight: 1.55 } },
            'Pick a module. ',
            h('strong', { style: { color: T.text } }, 'Cold Water Shock'),
            ' is the highest-leverage place to start in Maine. ',
            h('strong', { style: { color: T.text } }, 'Autism + Water Safety'),
            ' is the most-skipped module with the highest stakes.'),
          h('div', { role: 'list',
      ...
      function renderColdShock() {
        return h('div', { style: { padding: 20, maxWidth: 860, margin: '0 auto', color: T.text } },
          backBar('🥶 Cold Water Shock & the 1-1-10 Rule'),
          emergencyBanner(),
      ...
      function renderReachThrow() {
        return h('div', { style: { padding: 20, maxWidth: 860, margin: '0 auto', color: T.text } },
          backBar('🪢 Reach / Throw / Row / Don\'t Go'),
          emergencyBanner(),
          h('p', { style: { margin: '0 0 12px', color: T.muted, fontSize: 14, lineHeight: 1.55 } },
      ...
      function renderIceSafety() {
        return h('div', { style: { padding: 20, maxWidth: 860, margin: '0 auto', color: T.text } },
          backBar('🧊 Ice Safety + Self-Rescue'),
          emergencyBanner(),
          h('p', { style: { margin: '0 0 12px', color: T.muted, fontSize: 14, lineHeight: 1.55 } },
      ...
      function renderHypothermia() {
        return h('div', { style: { padding: 20, maxWidth: 860, margin: '0 auto', color: T.text } },
          backBar('🌡️ Hypothermia Recognition'),
          emergencyBanner(),
          h('p', { style: { margin: '0 0 12px', color: T.muted, fontSize: 14, lineHeight: 1.55 } },
      ...
      function renderCumulative() {
        ...
        if (done) {
          ...
          if (earned && !badges['cumulative_pass']) {
            useEffect(function() { awardBadge('cumulative_pass', 'Water Safety Mastery'); }, []);
          }
          return h('div', { style: { padding: 20, maxWidth: 720, margin: '0 auto', color: T.text } },
            ...
            h('div', { style: { background: T.card, border: '2px solid ' + (earned ? T.ok : T.warn), borderRadius: 14, padding: 24, textAlign: 'center' } },
              ...
              h('p', { style: { margin: '0 0 14px', fontSize: 13, color: T.muted, lineHeight: 1.55 } },
                earned
                  ? 'You have demonstrated mastery across the eight Water Safety Lab modules. The skills you have just rehearsed have saved real lives.'
        ...
        return h('div', { style: { padding: 20, maxWidth: 720, margin: '0 auto', color: T.text } },
          backBar('🎯 Cumulative Water Safety Quiz'),
      ...
      // ─────────────────────────────────────────
      var content;
      if (!consentAccepted) content = renderConsent();
      ...
      return h('div', { style: { background: T.bg, minHeight: '100%', color: T.text, fontFamily: 'system-ui, -apple-system, "Segoe UI", sans-serif' } },
        ...
        console.error('[WaterSafety] render error', err);
        ...
        return React2.createElement('div', { style: { padding: 20, color: '#fde2e2', background: '#7f1d1d', borderRadius: 10, margin: 20 } },
          React2.createElement('strong', null, 'Water Safety Lab error: '),
