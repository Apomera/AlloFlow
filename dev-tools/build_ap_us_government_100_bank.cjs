...
const baseItemSpecs = [
  ...
  q(1, '1.3', '1.A', 'Which claim best represents the Federalist argument that a large republic could protect liberty?', 'A large republic can make it harder for one faction to dominate by multiplying interests and representatives.', ['A large republic eliminates all political factions by requiring unanimous agreement among representatives.', 'A large republic makes state governments unnecessary because local interests disappear as territory expands.', 'A large republic protects liberty only when voters are barred from replacing the officials who represent them.'], 'Federalist No. 10 argued that an extended republic could control faction by making majority combinations more difficult and by filtering public views through representation.', { cognitiveProcess: 'explain' }),
  ...
  q(1, '1.5', '1.C', 'What was the principal effect of the Great Compromise at the Constitutional Convention?', 'It created a bicameral Congress with population-based representation in the House and equal state representation in the Senate.', ['It gave every state an identical number of representatives in both chambers regardless of its population.', 'It replaced congressional representation with direct national voting by citizens on every proposed law.', 'It required the president to be selected by the Supreme Court rather than by electors chosen in the states.'], 'The Great Compromise balanced large-state and small-state interests by combining proportional representation in the House with equal state representation in the Senate.', { cognitiveProcess: 'explain' }),
  ...
  q(1, '1.8', '2.B', 'Which constitutional interpretation did McCulloch v. Maryland support?', 'Congress may use implied powers under the Necessary and Proper Clause, and states may not interfere with valid federal actions.', ['Congress may exercise only the powers listed word-for-word in Article I, so implied powers are never valid.', 'States may tax or veto any federal institution that operates within their borders whenever they object to it.', 'The Supreme Court may review only state cases involving criminal law, leaving federal questions to Congress.'], 'McCulloch supported implied congressional powers and national supremacy by rejecting Maryland’s attempt to tax the national bank.', { cognitiveProcess: 'explain' }),
  q(1, '1.9', '5.C', 'Why can federalism create multiple access points for an interest group seeking a policy change?', 'The group can pursue action through both national institutions and state governments that share policymaking authority.', ['Federalism requires every policy change to be approved separately by every level of government before it takes effect.', 'Federalism removes state governments from policy implementation and leaves national agencies as the only venue.', 'Federalism gives courts exclusive control over public policy, so advocacy outside litigation cannot succeed.'], 'Shared authority creates more than one venue for advocacy, implementation, litigation, and coalition building.', { cognitiveProcess: 'explain' }),
  ...
  q(1, '1.6', '1.C', 'Which example best illustrates separation of powers rather than checks and balances?', 'The Constitution assigns legislative, executive, and judicial functions to different branches.', ['The Senate votes on whether to confirm a presidential nominee to a seat on a federal court.', 'The Supreme Court reviews an act of Congress and declares the statute unconstitutional.', 'Congress votes to override a presidential veto by a two-thirds majority in both chambers.'], 'Separation of powers distributes core governmental functions across branches; the other examples show one branch checking another.', { cognitiveProcess: 'compare' }),
  q(1, '1.7', '1.E', 'Which power is generally reserved to the states by the federal system?', 'Administering local elections and establishing public-school systems subject to constitutional limits', ['Coining national currency and setting the value of money used throughout the United States', 'Negotiating treaties with foreign governments and appointing ambassadors to represent the nation', 'Declaring war on another country and raising the armed forces that would conduct it'], 'State and local governments retain many powers over education and election administration, while currency, treaties, and war are national powers.', { cognitiveProcess: 'classify' }),
  ...
  q(2, '2.9', '2.B', 'What is the significance of judicial review in the American constitutional system?', 'Courts can invalidate government actions that conflict with the Constitution.', ['Courts can write the national budget without any action by Congress.', 'Courts appoint all of the senior members of the executive branch.', 'Courts can remove any elected official from office without legal process.'], 'Judicial review allows courts to determine whether statutes or executive actions violate the Constitution.', { cognitiveProcess: 'identify', difficulty: 'foundational' }),
  q(2, '2.10', '2.A', 'Why might the Supreme Court grant a writ of certiorari?', 'At least four justices may agree that a lower-court decision presents an important legal question for review.', ['The president requests that the Court approve an executive order before it takes effect nationwide.', 'A state legislature automatically forwards every case it considers important to the Court for review.', 'The Court must review the record of every trial in the country before a verdict may be entered.'], 'The rule of four allows four justices to grant certiorari, giving the Court discretion over most of its appellate docket.', { cognitiveProcess: 'explain' }),
  ...
  q(2, '2.12', '1.A', 'What is a central function of the federal bureaucracy?', 'Implementing laws and administering programs created through the political process', ['Replacing national elections with permanent appointments to federal agencies', 'Declaring war on other nations without authorization from any elected institution', 'Writing constitutional amendments that take effect without state ratification'], 'Bureaucratic agencies translate statutes and executive priorities into rules, services, enforcement, and administration.', { cognitiveProcess: 'identify', difficulty: 'foundational' }),
  ...
  q(2, '2.15', '5.C', 'Why do policy advocates often work with Congress, executive agencies, and courts on the same issue?', 'Each branch has different formal and informal powers that can affect policy design, implementation, and review.', ['Only the courts can make policy of any kind, so working with the other branches is a waste of effort.', 'The Constitution requires every advocate to petition all three branches in a fixed sequence.', 'The branches hold identical powers, so approaching more than one of them cannot change an advocacy strategy.'], 'Multiple institutions create distinct access points and leverage, so groups adapt their strategies to lawmaking, implementation, and litigation.', { cognitiveProcess: 'explain' }),
  ...
  q(2, '2.2', '1.B', 'Why are congressional committees important to the legislative process?', 'They divide work, develop expertise, hold hearings, and revise bills before floor consideration.', ['They replace the Constitution’s requirement that a bill pass both chambers before enactment.', 'They allow federal courts to appoint the chairs who lead each standing committee.', 'They prevent constituents from communicating with legislators about pending bills.'], 'Committees organize the large volume of legislative work and provide venues for hearings, investigation, markup, and oversight.', { cognitiveProcess: 'explain' }),
  q(2, '2.3', '5.B', 'A representative supports a farm subsidy because it benefits a narrow district industry even though the policy has diffuse national costs. Which concept best explains the vote?', 'Constituency service and concentrated-interest incentives', ['Judicial activism in reviewing economic regulation', 'Executive privilege over confidential agency advice', 'Selective incorporation of federal rights against states'], 'Members may respond to concentrated local benefits and organized constituents even when the broader costs are widely distributed.', { cognitiveProcess: 'apply' }),
  q(2, '2.4', '1.D', 'Which action most directly illustrates the president’s role as chief executive?', 'Directing executive agencies to implement a statute according to administration priorities', ['Presiding over the annual session of a state legislature and setting its agenda', 'Writing the majority opinion in a Supreme Court case about a federal statute', 'Ratifying a proposed constitutional amendment without any action by the states'], 'The president is chief executive and supervises the executive branch, subject to statutes, appropriations, and judicial review.', { cognitiveProcess: 'identify' }),
  q(2, '2.5', '2.D', 'Which presidential action requires the Senate’s advice and consent?', 'Nominating a federal judge who must be confirmed by the Senate', ['Issuing a pardon to a person convicted of a federal offense', 'Delivering an annual State of the Union address to Congress', 'Recognizing a newly formed foreign government diplomatically'], 'The Senate confirms federal judicial nominations; pardons, communication, and recognition are presidential powers that do not use this confirmation process.', { cognitiveProcess: 'identify' }),
  q(2, '2.6', '5.C', 'Why can a president’s unilateral action be limited even when the action is legally issued?', 'Congress can restrict funding or pass a contrary statute, and courts can review whether the action exceeds executive authority.', ['The president can never direct executive agencies to act without first obtaining a new constitutional amendment.', 'Unilateral actions become automatically permanent once they are published in the Federal Register for public notice.', 'Only state courts, and never Congress or a federal judge, have any authority to review an executive action.'], 'Unilateral action operates within a separation-of-powers system; legal, fiscal, and political checks can reduce or reverse its effect.', { cognitiveProcess: 'explain' }),
  q(2, '2.7', '4.A', 'Which feature of a presidential speech would best help a student identify its persuasive strategy?', 'The values, audience, evidence, and policy frame used to connect the proposal to public concerns', ['The number of Supreme Court justices seated in the audience during the speech', 'The exact population figures for every congressional district named in the speech', 'The private opinions held by every federal agency employee who watched the speech'], 'Source analysis asks readers to identify argument, audience, evidence, and reasoning rather than merely repeat the speaker’s topic.', { cognitiveProcess: 'analyze', stimulus: 'A speech presents a policy proposal as necessary to protect both economic opportunity and public safety.' }),
  ...
  q(2, '2.11', '1.E', 'Which development can limit the practical effect of a Supreme Court decision without changing the written opinion?', 'Congress can change a statute or adjust funding in ways that affect implementation when constitutionally permitted.', ['A single state newspaper can publish an editorial that formally overrules the Court’s holding.', 'The president can order the opinion erased from the United States Reports and from federal law.', 'A lower court can openly ignore a Supreme Court precedent without any consequence for its rulings.'], 'The Court interprets law, but implementation also depends on legislation, funding, executive enforcement, and lower-court compliance.', { cognitiveProcess: 'apply' }),
  ...
  q(2, '2.13', '3.D', 'An agency publishes a proposed rule, receives public comments, and revises the final rule. Which conclusion is best supported?', 'The notice-and-comment process provides an opportunity for affected interests to influence implementation details.', ['Public comments automatically require the agency to adopt whichever suggestion appears most often.', 'The notice-and-comment process transfers all legislative power to the private organizations that comment.', 'The final rule cannot be challenged in any court once the agency has published it in final form.'], 'Notice and comment creates an input channel and record for rulemaking, but the agency remains responsible for the final legally authorized rule.', { cognitiveProcess: 'explain' }),
  q(2, '2.14', '5.D', 'Which response would most directly address an argument that bureaucratic oversight is unnecessary?', 'Oversight can identify implementation failures while preserving the agency’s specialized role in carrying out legislation.', ['Oversight is unnecessary because agency officials are elected directly by voters in a national ballot.', 'Oversight proves that agencies should never rely on technical expertise when they carry out a statute.', 'Oversight makes statutes and judicial review irrelevant to how agencies implement national policy.'], 'A strong rebuttal acknowledges administrative expertise while explaining why monitoring and accountability remain necessary.', { cognitiveProcess: 'argue' }),
  q(2, '2.15', '3.F', 'A graph shows that the number of stakeholders filing comments on a proposed rule rises sharply, but the final rule changes little. What limitation should a student note?', 'The graph shows participation, not whether comments were persuasive or caused particular changes.', ['The graph proves that every participant who commented supported the final rule.', 'The graph proves that the agency ignored every comment that the public submitted.', 'The graph cannot show any information at all about how the rulemaking process worked.'], 'The visual supports a claim about participation volume, but not a causal claim about influence without additional evidence.', { cognitiveProcess: 'evaluate', stimulus: 'The graph reports the number of comments submitted during three rulemaking periods.' }),

  // Unit 3: 15 items, including at least one item for each current topic.
  q(3, '3.1', '1.D', 'What is the principal purpose of the Bill of Rights?', 'To enumerate protections for individual liberties and limit arbitrary government action', ['To establish the procedures for electing every state and local official', 'To give the president the power to suspend the Constitution permanently', 'To replace the Constitution with a set of ordinary congressional statutes'], 'The first ten amendments identify rights and freedoms that constrain government power.', { cognitiveProcess: 'identify', difficulty: 'foundational' }),
  q(3, '3.2', '2.A', 'A public school requires students to participate in a government-written prayer. Which constitutional issue is most directly raised?', 'The Establishment Clause of the First Amendment', ['The Commerce Clause of Article I, Section 8', 'The Takings Clause of the Fifth Amendment', 'The Appointments Clause of Article II, Section 2'], 'Government-sponsored prayer raises the Establishment Clause concern that government may not establish or endorse religion.', { cognitiveProcess: 'apply' }),
  ...
  q(3, '3.4', '4.D', 'A newspaper publishes information about government conduct, and officials seek to stop publication before it occurs. Which constitutional concept is most directly implicated?', 'Prior restraint on the press', ['Double jeopardy after acquittal', 'Equal protection of the laws', 'The right to keep and bear arms'], 'Prior restraint occurs when government attempts to prevent publication in advance, a practice subject to a strong presumption against constitutionality.', { cognitiveProcess: 'identify', stimulus: 'A court is asked to block publication before the public can read the material.' }),
  q(3, '3.5', '2.A', 'Which constitutional interpretation did District of Columbia v. Heller support?', 'The Second Amendment protects an individual right to possess a firearm for lawful purposes such as self-defense in the home.', ['The Second Amendment applies only to organized state militias and protects no individual right at all.', 'The Second Amendment prevents every form of firearm regulation by federal, state, or local government.', 'The Second Amendment applies only to voting rights in federal elections and not to any other activity.'], 'Heller recognized an individual right connected to self-defense while also stating that the right is not unlimited.', { cognitiveProcess: 'explain' }),
  q(3, '3.6', '5.B', 'Why might a court uphold a search rule designed to protect public safety while still recognizing Fourth Amendment interests?', 'Constitutional analysis can balance individual rights against a sufficiently justified government interest under the facts of the case.', ['A government interest in public safety automatically eliminates every constitutional protection a person might raise.', 'Individual rights become relevant only in the cases where the government can identify no interest of its own.', 'The Fourth Amendment applies only to searches by private businesses and never to actions by government officers.'], 'The unit’s liberty-and-order reasoning asks whether the government’s justification and method are constitutionally sufficient, not whether one value always defeats the other.', { cognitiveProcess: 'argue' }),
  q(3, '3.7', '2.B', 'What is selective incorporation?', 'The process by which the Supreme Court applies particular Bill of Rights protections to the states through the Fourteenth Amendment.', ['The process by which Congress decides which of the states may take part in the voting during federal elections.', 'The power of a state supreme court to rewrite the terms of the Bill of Rights as they apply to its residents.', 'The rule that only federal officials, and never state or local officials, must obey constitutional rights.'], 'Selective incorporation uses the Fourteenth Amendment’s due process protection to apply many, but not all at once, Bill of Rights guarantees to the states.', { cognitiveProcess: 'identify' }),
  q(3, '3.8', '2.A', 'Which protection is most directly associated with procedural due process for an accused person?', 'Notice of the accusation and a fair opportunity to be heard through lawful procedures', ['Automatic conviction of the accused whenever police officers make an arrest', 'A guarantee that every criminal case will be decided by a state legislature', 'Permission for officials to use evidence obtained through any search they choose'], 'Procedural due process requires nonarbitrary procedures, including notice, a hearing, and other protections for the accused.', { cognitiveProcess: 'explain' }),
  q(3, '3.9', '2.B', 'Why is privacy often described as an unenumerated constitutional right?', 'The Constitution does not name a general right to privacy in one provision, but the Court has inferred protections from several constitutional guarantees.', ['Privacy appears in the text of the First Amendment as the only right it protects, and no other constitutional provision names a freedom.', 'Privacy exists only when Congress creates it by ordinary statute, and the Court has never inferred it from any constitutional guarantee.', 'Privacy is a power reserved exclusively to state legislatures, so no provision of the federal Constitution has any bearing on it.'], 'The Court has reasoned that privacy interests can arise from the combined implications of specific guarantees and substantive due process.', { cognitiveProcess: 'explain' }),
  q(3, '3.10', '2.C', 'What constitutional principle was central to Brown v. Board of Education?', 'State-sponsored racial segregation in public schools violated the Equal Protection Clause of the Fourteenth Amendment.', ['States could segregate their public schools as long as they also funded a separate school system.', 'The First Amendment required every public school in the country to teach the same religious curriculum.', 'Congress could not pass civil-rights legislation of any kind affecting the public schools in the states.'], 'Brown rejected school segregation as inconsistent with equal protection, overturning the separate-but-equal approach in public education.', { cognitiveProcess: 'identify' }),
  q(3, '3.11', '1.E', 'Which action is an example of the national government responding legislatively to a civil-rights movement?', 'Congress passes a law prohibiting discrimination in public accommodations and employment.', ['A federal court declines to hear a case because it presents no legal issue.', 'A political party changes its campaign logo without changing any policy position.', 'A state weather agency publishes its regular forecast for the coming week.'], 'Civil-rights legislation uses national law to prohibit discriminatory practices and expand equal protection in public life.', { cognitiveProcess: 'apply' }),
  q(3, '3.12', '2.C', 'Why can majority rule be limited in a constitutional democracy?', 'Constitutional rights and judicial review can protect minority liberties from unconstitutional government action.', ['Majorities are never allowed to take part in the elections that decide questions of public policy.', 'Constitutional rights apply only to elected officials and not to the citizens they represent.', 'Courts must approve every private disagreement between citizens before it is allowed to occur.'], 'Constitutional democracy combines majority decision-making with limits that protect individual and minority rights.', { cognitiveProcess: 'explain' }),
  q(3, '3.13', '2.D', 'What constitutional question is central to debates over affirmative-action policies?', 'Whether a policy addressing disparities is consistent with the Equal Protection Clause’s limits on government classifications.', ['Whether Congress may coin additional money in order to fund the operation of state universities and colleges', 'Whether the president may appoint every local school principal without any consent from the state governments', 'Whether the First Amendment bans every form of public education that is operated by a state government'], 'Affirmative-action litigation has focused on how government classifications and remedial goals fit within equal-protection doctrine.', { cognitiveProcess: 'explain' }),
  q(3, '3.3', '5.C', 'Which evidence would best support the claim that student speech restrictions should be limited?', 'Evidence that the expression was peaceful and did not materially disrupt instruction or threaten safety', ['Evidence that the student held a political opinion that school administrators found unpopular', 'Evidence that the school maintains a written dress code unrelated to any disruption', 'Evidence that no one in the surrounding community has ever disagreed about student speech'], 'The relevant evidence connects the expression to the constitutional interest and to the school’s claimed justification, especially disruption or safety.', { cognitiveProcess: 'argue' }),
  ...
  q(4, '4.1', '1.A', 'Which statement best describes political culture in the United States?', 'It includes widely held beliefs about individual rights, equality, civic participation, and the proper role of government.', ['It is identical to the written policy platform of whichever political party currently controls Congress.', 'It consists only of the formal text of the Constitution and of the statutes that Congress has passed.', 'It changes from one day to the next because no political belief held by citizens persists over time.'], 'Political culture refers to broad, durable beliefs and values that influence how people view government and politics.', { cognitiveProcess: 'define', difficulty: 'foundational' }),
  ...
  q(4, '4.4', '4.B', 'How can a major national crisis influence political ideology?', 'It can change how citizens evaluate government responsibility, security, liberty, and policy priorities.', ['It automatically causes every citizen in the country to adopt one identical ideology.', 'It prevents political parties from responding in any way to shifts in public opinion.', 'It affects only local elections and never reaches the national policy debates that follow.'], 'Major events can shift the salience of issues and alter judgments about government’s proper role without producing uniform views.', { cognitiveProcess: 'explain', stimulus: 'A survey conducted before and after a major crisis shows increased concern about national security and executive authority.' }),
  q(4, '4.5', '3.A', 'Which feature most improves the representativeness of a scientific public-opinion poll?', 'A probability sample that gives members of the target population a known chance of selection', ['A voluntary online poll promoted mainly to the strongest supporters of one side', 'A sample made up entirely of the pollster’s own friends, family, and neighbors', 'A question asked only of people who already agree with the poll’s conclusion'], 'Probability sampling reduces selection bias by giving members of the target population a known chance of inclusion.', { cognitiveProcess: 'identify' }),
  q(4, '4.6', '3.E', 'A poll reports that 51 percent of respondents support a policy, with a margin of error of plus or minus 4 percentage points. Which conclusion is most defensible?', 'The population support could plausibly range from about 47 to 55 percent under the poll’s assumptions.', ['Exactly 51 percent of all citizens in the population support the policy as written.', 'The poll proves that the policy will certainly pass in the legislature this session.', 'The reported margin of error shows that the poll included no sampling uncertainty at all.'], 'A margin of error identifies a range of plausible population estimates; it does not guarantee an electoral or legislative outcome.', { cognitiveProcess: 'interpret', stimulus: 'The poll uses a probability sample and reports a 95 percent confidence level.' }),
  q(4, '4.7', '1.E', 'Which statement reflects the broad ideological pattern identified in the current course framework?', 'Democratic platforms generally align more closely with liberal positions, while Republican platforms generally align more closely with conservative positions.', ['The two major parties have no ideological differences in any policy area, so their written platforms are interchangeable documents.', 'Party platforms are written by the Supreme Court rather than by political organizations, and party delegates play no part in them.', 'Ideology is irrelevant to party policy debates, because the two major parties take identical positions on every national issue.'], 'The framework describes broad tendencies while recognizing that party coalitions and platforms can change over time and vary by issue.', { cognitiveProcess: 'apply' }),
  q(4, '4.8', '5.C', 'A policymaker argues that a social issue should be addressed nationally rather than left entirely to states. Which evidence would best support the argument?', 'Evidence that national action would address a cross-state problem through a constitutional power and produce a consistent policy result.', ['A claim that every state already holds identical political preferences, so the choice of governing level cannot matter', 'A statement asserting that ideology never influences public policy at any level of the American federal system', 'A poll of a single respondent that offers no explanation of the mechanism through which the policy would work'], 'A strong policy argument connects the preferred level of government to the problem’s scope, constitutional authority, and expected consequences.', { cognitiveProcess: 'argue' }),
  ...
  q(4, '4.10', '1.E', 'Which policy position is generally associated with a conservative ideology in the framework?', 'Leaving more responsibility for some social issues to state governments rather than expanding national involvement.', ['Requiring the national government to manage every local social program directly from Washington', 'Rejecting all limits on the power of government in every social, economic, and legal context', 'Abolishing the state governments altogether and removing them from the federal system entirely'], 'The framework generally associates conservative ideology with less national involvement in some social issues and greater state responsibility.', { cognitiveProcess: 'apply' }),
  ...
  q(5, '5.3', '1.A', 'Which function do political parties perform in representative democracy?', 'They recruit candidates, organize voters, develop platforms, and help structure governing coalitions.', ['They replace the Constitution with a set of private rules written by party officials.', 'They prevent voters from comparing the policy positions offered by rival candidates.', 'They serve only as temporary administrative agencies appointed by the federal courts.'], 'Parties connect citizens and government by recruiting candidates, aggregating interests, mobilizing voters, and organizing institutions.', { cognitiveProcess: 'identify', difficulty: 'foundational' }),
  q(5, '5.4', '5.C', 'Why might a major political party change its platform after a significant shift in the electorate?', 'Adapting positions can help the party build a winning coalition and respond to changing public priorities.', ['Party platforms cannot change at all because they are formal constitutional documents.', 'Parties change their written platforms only when a federal court orders them to do so.', 'A platform change eliminates the need for the party to run candidates or court voters.'], 'Parties adapt to demographic change, issue salience, electoral incentives, and coalition-building pressures.', { cognitiveProcess: 'explain' }),
  q(5, '5.5', '1.E', 'Why do third-party candidates often face difficulty winning national office?', 'Single-member, plurality elections and ballot-access rules make it difficult to convert support into seats or electoral votes.', ['The Constitution prohibits third parties from appearing on the ballot in any state or federal election.', 'Third parties are barred from raising money of any kind under the federal campaign-finance laws in force.', 'Third-party candidates automatically receive the same number of electoral votes as the major-party nominees.'], 'Electoral rules, ballot access, fundraising, debate access, and strategic voting create structural barriers for third parties.', { cognitiveProcess: 'explain' }),
  q(5, '5.6', '1.E', 'Which activity is a typical form of interest-group lobbying?', 'Providing policy information and communicating with lawmakers about proposed legislation.', ['Selecting the chief justice of the United States without any presidential involvement', 'Counting and certifying every ballot cast in a national general election', 'Issuing a written judicial opinion on behalf of a federal appellate court'], 'Lobbying seeks to influence policymakers through information, relationships, testimony, and organized advocacy.', { cognitiveProcess: 'identify' }),
  q(5, '5.7', '5.B', 'Why can an interest group with a relatively small membership still influence policy?', 'It may have concentrated resources, specialized information, motivated members, or access to decision-makers.', ['Influence is determined only by the total number of people who hold membership in the group.', 'Small groups are constitutionally barred from using the courts or the news media at all.', 'A small group automatically controls the outcome of every election that is held in its state.'], 'Resources, expertise, intensity, organization, and access can matter in addition to membership size.', { cognitiveProcess: 'explain' }),
  q(5, '5.8', '1.C', 'What is the role of the Electoral College in selecting the president?', 'It formally allocates electoral votes to states and determines the winner under the constitutional process.', ['It directly writes the campaign platforms that are adopted by each of the major parties.', 'It selects the members of the House of Representatives according to national popular vote totals.', 'It allows the Supreme Court to choose the president at the close of every national election.'], 'The Electoral College uses state-based electoral votes to select the president, subject to constitutional and statutory rules.', { cognitiveProcess: 'identify' }),
  q(5, '5.9', '1.E', 'Why do incumbents often have an advantage in congressional elections?', 'They may benefit from name recognition, constituent service, fundraising networks, and a record of public visibility.', ['Sitting incumbents cannot be challenged by any other candidate of their party in a primary election.', 'Incumbents receive every vote cast by members of their own party as a constitutional requirement.', 'Incumbents are appointed to their office rather than elected once they have served a first term.'], 'Incumbency provides institutional and electoral resources, although it does not guarantee reelection.', { cognitiveProcess: 'explain' }),
  q(5, '5.10', '4.A', 'Which feature of modern campaigns most directly reflects professionalized election strategy?', 'Campaigns use consultants, targeted communication, fundraising operations, polling, and media planning.', ['Campaigns are conducted without parties, donors, or any communication with the voters.', 'Campaigns require candidates to avoid every public appearance during the entire race.', 'Campaigns are decided by the federal courts long before any of the voters participate.'], 'Modern campaigns rely on specialized organizations and communication strategies, with benefits and costs for democratic participation.', { cognitiveProcess: 'identify' }),
  q(5, '5.11', '1.E', 'What is a central difference between a traditional political action committee and an independent-expenditure-only committee?', 'An independent-expenditure-only committee may spend independently of candidates but cannot coordinate expenditures with them.', ['A traditional political action committee may never communicate with voters or with any federal candidate.', 'An independent-expenditure-only committee operates as an official branch of the Supreme Court itself.', 'Both kinds of organization are entirely exempt from all disclosure requirements and campaign-finance rules.'], 'The distinction concerns coordination and independent expenditures; campaign-finance rules and disclosure requirements can still apply.', { cognitiveProcess: 'compare' }),
  ...
  q(5, '5.13', '3.E', 'What is one possible democratic consequence of increasingly fragmented media choices?', 'Citizens may encounter more information but also more ideologically selective content and reinforcement of existing beliefs.', ['All citizens now receive exactly the same news coverage, because a single national source supplies it.', 'Media fragmentation eliminates political participation entirely by making news too difficult to follow.', 'A larger number of media choices guarantees that every claim that citizens encounter has been verified.'], 'More choices can expand access while also encouraging selective exposure, partisan segmentation, and uneven political knowledge.', { cognitiveProcess: 'explain' }),
  ...
  q(5, '5.2', '1.E', 'Which change would most directly reduce a structural barrier to voting?', 'Making voter registration available through an accessible automatic or same-day process', ['Reducing the number of polling locations available in high-population areas', 'Requiring voters to declare support for a particular party before they may register', 'Limiting all official voting information to paid campaign advertisements'], 'Registration rules, polling access, identification requirements, and election timing can create or reduce opportunities for participation.', { cognitiveProcess: 'apply' }),
  ...
  q(5, '5.4', '3.C', 'A chart shows one party’s support declining among one age cohort while rising among another across several elections. What conclusion is best supported?', 'The party’s coalition changed across the measured cohorts, although the chart alone cannot identify the cause.', ['Every individual voter in the older cohort changed political parties during the measured period.', 'The party’s written platform had no relationship at all to the support that it received.', 'The chart proves that campaign spending is what caused the measured shift in cohort support.'], 'The data show a cohort pattern, but causal explanations require additional evidence about issues, events, mobilization, and demographics.', { cognitiveProcess: 'interpret', stimulus: 'The chart reports party identification by age cohort across three election years.' }),
  q(5, '5.5', '5.C', 'A third-party candidate wins enough votes to deny either major candidate a plurality in a state. What possible effect is illustrated?', 'The candidate may act as a spoiler by changing which major candidate wins the state’s plurality.', ['The candidate automatically receives all of the electoral votes in every state.', 'The candidate eliminates any need for the state to hold a general election at all.', 'The candidate proves that plurality rules favor every third party in the race equally.'], 'In a plurality system, a third-party candidate can alter the distribution of votes enough to affect the major-party winner without winning the office.', { cognitiveProcess: 'apply' }),
  q(5, '5.6', '4.C', 'An interest group files an amicus brief in a Supreme Court case. What is the group attempting to do?', 'Present arguments or information that may influence the Court’s understanding of the legal and policy issues.', ['Replace the sitting justices with elected representatives chosen by organized interest groups', 'Count and certify the ballots that are cast in the next congressional election in every state', 'Write a new constitutional amendment that takes effect without any ratification by the states'], 'An amicus brief allows an organization to offer legal reasoning or relevant information to the Court without being one of the principal parties.', { cognitiveProcess: 'identify', stimulus: 'The brief explains how a proposed ruling could affect the group’s members and cites constitutional principles.' }),
  ...
  q(5, '5.10', '5.A', 'Which claim is most defensible about professional campaign consultants?', 'They can improve a campaign’s communication and fundraising strategy, but their use can also increase costs and reduce candidate autonomy.', ['They guarantee that voters will support the candidate regardless of which policy positions a campaign takes up.', 'They make political parties and organized interest groups entirely irrelevant to the conduct of modern elections.', 'They eliminate any need for the candidate to communicate with citizens at any point during the campaign season.'], 'A defensible claim recognizes both the strategic benefits and the institutional tradeoffs of professionalized campaigns.', { cognitiveProcess: 'argue' }),
  q(5, '5.11', '4.D', 'A political advertisement is funded by an organization that does not disclose its donors. Which source-analysis question is most important?', 'How the funding source and lack of disclosure may affect the advertisement’s perspective and credibility.', ['Whether the advertisement uses the same typeface as a recently published judicial opinion', 'Whether the sponsoring organization has the power to appoint judges to the federal courts', 'Whether the advertisement automatically represents the opinion held by every single voter'], 'Source analysis considers authorship, funding, audience, purpose, evidence, and perspective when evaluating political communication.', { cognitiveProcess: 'analyze', stimulus: 'The advertisement presents a policy claim but identifies only the sponsoring organization, not its donors.' }),
  q(5, '5.12', '1.D', 'When a news organization repeatedly highlights one issue, what media effect may it have on political discussion?', 'Agenda setting by increasing the issue’s perceived importance among citizens and policymakers.', ['Judicial review, by declaring the disputed issue unconstitutional in a written opinion', 'Federalism, by assigning the issue to state governments for their own resolution', 'Cloture, by ending an extended Senate debate over the issue with a formal vote'], 'Agenda setting concerns the media’s influence over which issues receive attention, not necessarily the direction of every opinion.', { cognitiveProcess: 'identify' }),
];
