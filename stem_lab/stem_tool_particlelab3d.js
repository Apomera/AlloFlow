...
(function () {
  ...
  window.StemLab.registerTool('particleLab3d', {
    ...
    render: function (ctx) {
      ...
      // The chamber-controls card sits over the top-left of the stage. Start it folded
      // so the 3D scene reads first; an intentional expansion is persisted for the
      // learner's next session.
      ...
      return h('div', { className: 'max-w-7xl mx-auto space-y-4 animate-in fade-in duration-300' },
        ...
        h('div', { className: 'grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]' },
          h('section', { id: 'particle-stage', ref: stageRef, onKeyDown: onStageKeyDown, className: 'overflow-hidden border border-cyan-500/30 bg-slate-950 shadow-2xl shadow-cyan-950/40 ring-1 ring-white/5 ' + (fsActive ? 'h-screen rounded-none flex flex-col' : 'rounded-[24px]'), style: cssFullscreen ? { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100vh', zIndex: 99990, borderRadius: 0 } : undefined, 'aria-label': 'Three-dimensional particle simulation', 'aria-busy': ready ? 'false' : 'true' },
            ...
            h('div', { className: 'relative min-h-[380px] overflow-hidden bg-slate-950 ' + (fsActive ? 'flex-1' : 'h-[520px]') },
              h('canvas', { ref: canvasRef, tabIndex: ready ? 0 : -1, role: 'application', 'aria-hidden': ready ? undefined : 'true', 'aria-roledescription': 'Interactive 3D particle chamber', 'aria-label': preset + ' particle simulation', 'aria-describedby': 'particle-chamber-help', 'aria-keyshortcuts': 'Space R T V E M G C L F H ? Escape', onKeyDown: onLabKey, onPointerDown: function (event) { if (!ready) return; try { event.currentTarget.focus(); } catch (error) {} }, className: 'h-full w-full saturate-[1.15] contrast-[1.04] focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-[-4px] focus-visible:outline-cyan-200' }),
              ...
              showHud && h('div', { id: 'particle-scene-key', className: 'pointer-events-none absolute left-3 ' + (trace ? 'bottom-[12rem]' : (visualOverlayLabels.length ? 'bottom-[8rem]' : 'bottom-3')) + ' z-10 w-[6rem] max-w-[min(18rem,calc(100%-1.5rem))] break-words rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2 text-[11px] font-bold text-slate-300 shadow-lg backdrop-blur-md sm:w-auto', role: 'img', 'aria-label': sceneKeyLabel }, h('div', { className: 'mb-1 text-[11px] font-black uppercase tracking-[0.18em] text-cyan-300' }, 'Scene key'), h('div', { className: 'flex flex-wrap items-center gap-x-3 gap-y-1' }, h('span', { className: 'inline-flex items-center gap-1.5' }, h('span', { className: 'h-2 w-2 rounded-full bg-cyan-300 shadow-[0_0_8px_rgba(34,211,238,0.7)]' }), transportMode ? 'A / solvent' : 'particles'), transportMode && h('span', { className: 'inline-flex items-center gap-1.5' }, h('span', { className: 'h-2 w-2 rounded-full bg-fuchsia-300 shadow-[0_0_8px_rgba(244,114,182,0.7)]' }), 'B / solute'), trace && h('span', { className: 'inline-flex items-center gap-1.5 text-yellow-200' }, h('span', { className: 'h-2 w-2 rounded-full border-2 border-yellow-200' }), 'tracer')), visualOverlayLabels.length && h('div', { className: 'mt-1 border-t border-white/10 pt-1 text-[10px] font-bold text-slate-400' }, 'Cues: ' + visualOverlayDisplayLabels.join(' • '))),
              ...
              ),              h('div', { id: 'particle-stage-activity', className: 'pointer-events-none absolute ' + (systemProbe || trace || transportMode ? 'bottom-[6rem]' : 'bottom-3') + ' right-3 w-[min(11rem,calc(100%-1.5rem))] sm:w-auto sm:max-w-[min(20rem,calc(100%-1.5rem))] rounded-xl border ' + (running ? 'border-emerald-300/30 bg-emerald-950/75 text-emerald-100' : 'border-white/15 bg-slate-950/75 text-slate-200') + ' px-3 py-2 shadow-lg backdrop-blur', role: 'status', 'aria-live': 'polite', 'aria-label': (running ? 'Live simulation. ' : 'Simulation paused. ') + stageActivityLabel + '. ' + stageActivityDetail }, h('div', { className: 'flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.14em]' }, h('span', { className: 'h-2 w-2 rounded-full ' + (running ? 'animate-pulse bg-emerald-300 shadow-[0_0_10px_2px_rgba(110,231,183,0.7)]' : 'bg-slate-400') }), running ? 'Live simulation' : 'Simulation paused'), h('div', { className: 'mt-1 text-[11px] font-bold' }, stageActivityLabel), h('div', { className: 'mt-0.5 text-[11px] opacity-75' }, stageActivityDetail)),
            ...
            fsActive && showHud && showFullscreenConditions && h('section', { id: 'particle-fullscreen-conditions', className: 'max-h-[38vh] overflow-y-auto border-t border-cyan-400/30 bg-slate-900/95 px-4 py-3 text-white', 'aria-label': 'Fullscreen experiment conditions' },
              h('div', { className: 'mb-2 flex items-center justify-between gap-3' }, h('h3', { className: 'text-xs font-black uppercase tracking-[0.16em] text-cyan-200' }, 'Experiment conditions'), h('button', { type: 'button', onClick: function () { setShowFullscreenConditions(false); }, className: 'min-h-11 rounded-lg border border-slate-600 px-3 py-2 text-xs font-bold text-white hover:bg-slate-800', 'aria-label': 'Close fullscreen experiment conditions' }, 'Close')),
            ...
            showHud && h('div', { className: 'flex flex-wrap items-center gap-2 border-t border-slate-700 p-3' },
              ...
              fsActive && h('button', { type: 'button', onClick: function () { setShowFullscreenConditions(!showFullscreenConditions); }, 'aria-expanded': showFullscreenConditions, 'aria-controls': 'particle-fullscreen-conditions', className: 'rounded-lg border border-cyan-400/40 bg-cyan-950 px-3 py-2 text-sm font-bold text-cyan-100 hover:bg-cyan-900' }, showFullscreenConditions ? 'Hide conditions' : '\u2699 Conditions'),
