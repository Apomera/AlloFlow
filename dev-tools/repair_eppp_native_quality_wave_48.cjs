...
const { baselineMetrics, reviewedAt, reviewWave, revisions } = require('./eppp_native_quality_wave_48_data.cjs');
...
const auditBasename = 'eppp_question_quality_audit_wave_48.json';
...
if (fs.readFileSync(deployBankPath, 'utf8') !== sourceText) throw new Error('Source and deploy EPPP banks must match before wave 48.');
...
if (revisions.length !== 24 || new Set(revisions.map((revision) => revision.id)).size !== 24) throw new Error('Wave 48 needs twenty-four unique revisions.');
const selectedIds = new Set(revisions.map((revision) => revision.id));
const itemsById = new Map(bank.map((item) => [item.id, item]));

// Restore frozen after-states for all prior post-campaign native waves before
// applying this batch. This keeps historical tests stable after any replay.
for (let waveNumber = 25; waveNumber <= 47; waveNumber += 1) {
...
for (const revision of revisions) {
  ...
  if (!recognizedReplay && item.prompt !== revision.expectedPrompt) throw new Error(revision.id + ' prompt drifted before wave 48.');
  if (item.answerIndex !== revision.expectedAnswerIndex) throw new Error(revision.id + ' answer position drifted.');
  if (!recognizedReplay && item.difficulty !== revision.expectedDifficulty) throw new Error(revision.id + ' difficulty drifted before wave 48.');
...
console.log('EPPP quality wave 48: rewrote 24 items across 8 domains; selected warnings after = 0.');
