...
(async()=>{
 ...
 try {
  ...
  await reaction.fill('CH4 + O2 -> CO2 + H2O');
  await reaction.press('Enter');
  await page.getByRole('spinbutton',{name:'CH4 grams available',exact:true}).fill('16.043');
  await page.getByRole('spinbutton',{name:'O2 grams available',exact:true}).fill('31.998');
  await page.getByRole('spinbutton',{name:'Actual grams produced',exact:true}).fill('22.0045');
  const product=page.getByRole('combobox',{name:'Product to calculate',exact:true});
  await product.focus();
  await product.press('End');
  await product.press('Tab');
  assert.equal(await product.inputValue(),'3');
  assert.equal(await page.getByRole('spinbutton',{name:'Actual grams produced',exact:true}).inputValue(),'');
  assert.equal(await page.getByRole('spinbutton',{name:'CH4 grams available',exact:true}).inputValue(),'16.043');
  assert((await page.locator('main').innerText()).includes('Theoretical yield: 18.015 g H2O (1 mol)'));
  await page.getByRole('spinbutton',{name:'Actual grams produced',exact:true}).fill('9.0075');
  assert((await page.locator('main').innerText()).includes('Percent yield: 50.0%'));
  const work=page.locator('summary').filter({hasText:'How this was calculated'});
  await work.focus();
  await work.press('Enter');
  assert.equal(await work.evaluate(n=>n.parentElement.open),true);
  assert((await work.locator('..').innerText()).includes('1 mol O2 × (2 mol H2O / 2 mol O2) = 1 mol H2O'));
  report.checks.push('Keyboard product selection preserves reactants, clears the previous measurement, and updates the yield and accessible calculation explanation.');
  ...
  await page.locator('#chem-yield-qa').screenshot({path:path.join(out,'product-yield-mobile.png')});
 ...
 }finally{fs.writeFileSync(path.join(out,'product-browser-results.json'),JSON.stringify(report,null,2));await browser.close();}
