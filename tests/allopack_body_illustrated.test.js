...
const slug='body_systems_grade6',folder='allopacks/media/'+slug+'/',pack=read('allopacks/illustrated/'+slug+'.allopack.json'),original=read('allopacks/'+slug+'.allopack.json'),manifest=read(folder+'manifest.json');
require('../dev-tools/body_content_refinements.cjs').refine(original);
...
describe('body_systems_grade6 illustrated edition',()=>{
...
for(const tag of ['right-heart-to-lungs','lungs-to-left-heart','left-heart-to-body','body-to-right-heart'])expect(svg('circulation')).toContain('data-flow="'+tag+'"');expect(svg('exchange')).toContain('data-flow="oxygen-to-blood"');expect(svg('exchange')).toContain('data-flow="carbon-dioxide-to-air"');expect(get('simplified').data).toContain('many dietary fats first enter lymph');expect(get('simplified').data).toContain('Pacemaker cells');
});
