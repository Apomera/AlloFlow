...
const slug='place_value_grade2',folder='allopacks/media/'+slug+'/',pack=read('allopacks/illustrated/'+slug+'.allopack.json'),original=read('allopacks/'+slug+'.allopack.json'),manifest=read(folder+'manifest.json');
require('../dev-tools/place_value_content_refinements.cjs').refine(original);
...
describe('place_value_grade2 illustrated edition',()=>{
...
for(const [key,n]of [['245',245],['305',305],['254',254],['four',4],['forty',40],['fourhundred',400],['twohundred',200]])expect((svg(key).match(/data-unit="1"/g)||[]).length).toBe(n);expect(get('applied-challenge').data.supports.parallelExample.move).toContain('ONES column of 230');expect(manifest.assets.find(a=>a.kind==='anchor'&&a.index===2).sourceKey).toBe('twohundred');
});
