...
(function () {
  ...
  window.StemLab.registerTool('timeSchedule', {
    ...
    render: function (ctx) {
      ...
      var TABS = [
        ['clock', 'Clock Link', '🕰️', 'Analog ↔ digital'],
        ['elapsed', 'Elapsed Timeline', '⏱️', 'Count time jumps'],
        ['schedule', 'Schedule Planner', '📅', 'Read and reason'],
        ['challenge', 'Challenge Lab', '🏆', 'Mixed practice']
      ];
      ...
      function formatToggle() {
        return h('div', { className: 'inline-flex rounded-lg border border-slate-300 bg-slate-100 p-1',
          role: 'group', 'aria-label': 'Time display format' },
      ...
      function analog(value, id, compact) {
        ...
        var readable = 'Analog clock showing ' + time12(value) +
          '. Minute hand: ' + minute + ' minutes. Hour hand: ' +
          (minute ? 'past ' : 'at ') + (hour || 12) + '.';
        return h('figure', { className: 'm-0 flex flex-col items-center' },
          h('svg', { viewBox: '0 0 320 320', role: 'img',
            ...
            h('title', { id: id + '-title' }, 'Analog clock: ' + time12(value)),
      ...
      function clockView() {
        ...
        var rule = hour24 === 0 ? '00 becomes 12 AM.' :
          hour24 === 12 ? '12 stays 12 PM.' :
          hour24 > 12 ? 'For PM times after noon, subtract 12 from the hour.' :
          'For AM times, the hour stays the same.';
        return h('section', { className: 'space-y-4',
          'aria-labelledby': 'ts-clock-heading' },
          heading('ts-clock-heading', 'Clock Link',
            'Move one representation and watch analog, 12-hour, and 24-hour time stay linked.'),
          h('div', { className: 'grid grid-cols-1 lg:grid-cols-2 gap-4' },
            ...
            h('div', { className: 'space-y-4' },
              ...
              h('div', { className: 'rounded-xl border border-slate-200 bg-white p-4 space-y-3' },
                h('label', { className: 'block text-xs font-black text-slate-700' }, 'Set an exact time',
                  h('input', { type: 'time', value: time24(clock),
                    ...
                    'aria-label': 'Set exact clock time' })),
                h('div', { className: 'grid grid-cols-3 sm:grid-cols-6 gap-2',
                  role: 'group', 'aria-label': 'Adjust the clock' },
      ...
      function elapsedView() {
        ...
        return h('section', { className: 'space-y-4',
          ...
          heading('ts-elapsed-heading', 'Elapsed-Time Timeline',
            'Break an interval into friendly jumps to the hour, then combine the jumps.'),
          h('div', { className: 'grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-4' },
            h('div', { className: 'rounded-2xl border border-slate-200 bg-white p-4 space-y-4' },
              h('label', { className: 'block text-xs font-black text-slate-700' }, 'Starting time',
              ...
              h('fieldset', null, h('legend', { className: 'text-xs font-black text-slate-700' }, 'Direction'),
                h('div', { className: 'grid grid-cols-2 gap-2 mt-2' },
                  [[1, 'Count forward', 'emerald'], [-1, 'Count backward', 'amber']].map(function (x) {
              ...
              h('div', { className: 'grid grid-cols-2 gap-3' },
                h('label', { className: 'text-xs font-black text-slate-700' }, 'Hours',
                  h('input', { type: 'number', min: 0, max: 12, value: Math.floor(amount / 60),
                    ...
                    className: 'mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 font-bold' })),
                h('label', { className: 'text-xs font-black text-slate-700' }, 'Minutes',
              ...
              h('div', { className: 'rounded-xl bg-slate-950 text-white p-4 text-center' },
                h('p', { className: 'text-[10px] uppercase tracking-widest text-slate-400 font-bold' },
                  direction > 0 ? 'Ending time' : 'Earlier starting time'),
                ...
                h('p', { className: 'text-xs text-slate-400 mt-1' },
                  (rawEnd >= DAY ? 'next day' : rawEnd < 0 ? 'previous day' : 'same day') +
                  ' · ' + durationText(amount)))),
            h('div', { className: 'rounded-2xl border border-cyan-200 bg-gradient-to-b from-cyan-50 to-white p-3 sm:p-5 overflow-hidden' },
              h('svg', { viewBox: '0 0 720 190', className: 'w-full h-auto min-h-[170px]',
                ...
                h('title', { id: 'ts-jump-title' }, 'Elapsed-time jump timeline'),
                h('desc', { id: 'ts-jump-desc' }, 'From ' + time12(start) + ', count ' +
                  (direction > 0 ? 'forward ' : 'backward ') + durationText(amount) + ' to ' +
                  time12(end) + '. Jumps: ' + jumps.map(function (j) { return j.amount + ' minutes'; }).join(', ') + '.'),
                ...
                points.map(function (point, i) {
                  ...
                  return h('g', { key: i },
                    ...
                    i > 0 && h('text', { x: (priorX + x) / 2, y: 32,
                      ...
                      fontWeight: '800' }, (direction > 0 ? '+' : '−') + point.amount + ' min'),
              ...
              h('div', { className: 'rounded-xl border border-cyan-200 bg-white p-3' },
                h('h4', { className: 'text-xs font-black uppercase tracking-wide text-cyan-800' },
                  'Text alternative: jump strategy'),
                jumps.length ? h('ol', { className: 'mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2' },
                  jumps.map(function (jump, i) {
                    return h('li', { key: i, className: 'rounded-lg bg-cyan-50 px-3 py-2 text-xs text-slate-700' },
                      h('strong', { className: 'text-cyan-800' }, 'Jump ' + (i + 1) + ': '),
                      showTime(jump.from, use24) + ' → ' + showTime(jump.to, use24) +
                      ' (' + jump.amount + ' min)');
                  })) : h('p', { className: 'text-xs text-slate-600 mt-2' },
                    'The interval is zero, so start and end are the same.'),
      ...
      function scheduleView() {
        ...
        function check() {
          ...
          if (!result.valid) {
            var formatMessage = sq.answerFormat === '24' ? 'Use 24-hour form such as 13:25.' :
        ...
        return h('section', { className: 'space-y-4',
          ...
          heading('ts-schedule-heading', 'Schedule Planner',
            'Compare event lengths, free-time gaps, and the span of an entire plan.'),
          h('div', { className: 'grid grid-cols-1 lg:grid-cols-[1.2fr_.8fr] gap-4' },
            h('div', { className: 'rounded-2xl border border-violet-200 bg-white overflow-hidden' },
              ...
              h('div', { className: 'overflow-x-auto' },
                h('table', { className: 'w-full text-sm border-collapse' },
                  h('caption', { className: 'sr-only' }, schedule.label + ' event schedule'),
                  h('thead', { className: 'bg-violet-50 text-violet-900' },
                    h('tr', null, ['Event', 'Start', 'End', 'Length', 'Gap after'].map(function (label) {
              ...
              h('div', { className: 'grid grid-cols-3 gap-2 bg-slate-50 border-t p-3 text-center' },
                [[span, 'Total span', 'text-violet-700'], [busy, 'Scheduled', 'text-emerald-700'],
                  [span - busy, 'Free time', 'text-amber-700']].map(function (x) {
            ...
            h('aside', { className: 'rounded-2xl border border-amber-200 bg-gradient-to-b from-amber-50 to-white p-4 space-y-4' },
              ...
              h('details', { className: 'rounded-xl border border-slate-200 bg-white p-3' },
                h('summary', { className: 'cursor-pointer text-xs font-black' }, 'Strategy hint'),
                h('p', { className: 'text-xs text-slate-600 mt-2' },
                  'Mark both times on a number line. Jump to a friendly hour, add whole hours, then finish the remaining minutes.')))));
      }
          ...
          h('div', { className: 'grid grid-cols-1 lg:grid-cols-[.8fr_1.2fr] gap-4' },
              ...
              challenge.clock != null ? analog(challenge.clock, 'ts-challenge-clock-' + chIndex, true) :
                h('div', { className: 'text-center max-w-xs' },
                  ...
                  h('p', { className: 'text-sm text-slate-600 mt-2' },
                    challenge.type === 'time' ? 'Find an exact clock time.' : 'Find the interval length.'))),
            h('div', { className: 'rounded-2xl border border-slate-200 bg-white p-5 sm:p-6 space-y-4 shadow-sm' },
              ...
              h('p', { className: 'text-[11px] text-slate-500' },
                challenge.type === 'duration' ? 'Durations may be entered in total minutes or hours and minutes.' :
                  challenge.answerFormat === '24' ? 'Use two-digit 24-hour form (HH:MM).' :
              ...
              h('details', { className: 'rounded-xl border border-sky-200 bg-sky-50 p-3' },
                ...
                h('ul', { className: 'list-disc pl-5 mt-2 space-y-1 text-xs text-sky-900' },
                  h('li', null, 'Draw jumps to a friendly hour.'),
                  h('li', null, 'Convert each time to minutes after midnight.'),
                  h('li', null, 'For a missing start, count backward from the end.'),
                  h('li', null, 'Crossing midnight begins a new 24-hour cycle.'))))));
      }
      ...
      return h('div', { className: 'max-w-6xl mx-auto space-y-4 pb-6 text-slate-900' },
        h('header', { className: 'rounded-2xl overflow-hidden shadow-lg border border-sky-700/20 bg-gradient-to-br from-sky-700 via-cyan-700 to-indigo-800 text-white' },
          h('div', { className: 'p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4' },
            h('div', { className: 'flex items-center gap-3 min-w-0 flex-1' },
              h('button', { type: 'button', 'aria-label': 'Back to STEM tools',
              ...
              h('div', { className: 'min-w-0' },
                ...
                h('p', { className: 'text-xs sm:text-sm text-sky-100 mt-1' },
                  'See time, count time, plan time, and explain your strategy.'))),
            h('div', { className: 'grid grid-cols-3 gap-2 text-center shrink-0' },
              [[score.correct || 0, 'Correct'], [d.bestStreak || 0, 'Best streak'],
                [Object.keys(d.modesUsed || {}).length, 'Modes tried']].map(function (x) {
                return h('div', { key: x[1],
        ...
        h('div', { className: 'grid grid-cols-2 lg:grid-cols-4 gap-2',
          role: 'tablist', 'aria-label': 'Time and Schedule Lab sections' },
        ...
        h('footer', { className: 'rounded-xl border border-sky-100 bg-sky-50 px-4 py-3 text-xs text-sky-900 flex flex-col sm:flex-row sm:justify-between gap-2' },
          h('span', null, h('strong', null, 'Time tip: '),
            'A clock time and an amount of time are different quantities. Write units with durations.'),
          h('span', { className: 'font-bold' },
            'No timer pressure · keyboard friendly · text alternatives included')));
    }
  });
})();
