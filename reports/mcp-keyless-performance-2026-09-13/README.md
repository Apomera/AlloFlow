...
## Canvas throttle fixes and the review-finding triage (evening)

A Canvas run of an 8-page, 5.4 MB scan (build 3f3ae1896) showed the other slow lane: the app pushed 71 MB through 31 calls, the whole file to three auditors and then two adaptive ones, Canvas throttled the account, and the run spent 25 minutes on twelve minute-long 401s for two successes (each failed call died after 58 to 60 s: the proxy holds a throttled request about a minute before refusing it). Four changes, all committed (0309732e9, 49c1c2138) and none a trade-off:

1. **Image-heavy documents are audited in page slices from the start** (`_alloAuditRoute`: big, long, or more than 300 KB per page above 1.5 MB), so a scan is not uploaded whole to every auditor.
2. **No adaptive extra auditors on a heavy document or into an active throttle** (`_alloAdaptiveAuditAllowed`); the disagreement is reported as the panel.
3. **A proxy-hold policy in the throttle gate:** once three consecutive auth failures each took 45 s or more, the cooldown floor becomes two minutes (growing to the three-minute ceiling) and the first real call after a wait sends a 1 KB probe first, failing cheaply if the proxy still refuses. Instant failures keep the old ladder.
4. **Review-finding triage:** before the final audit, on a healthy lane and for up to ten findings, the model is asked to fix, clear or defer each review-tier finding it can locate; a fix is applied only under a structure-only gate (same words, images and links) and the engines that re-run afterwards decide what cleared. The audit view shows the outcome and the model's reason next to what remains, with an "Ask AI to resolve" button for throttled runs or documents over the cap. Nothing is marked reviewed by the machine.

What is verified: unit and jsdom tests drive the real gate and the real triage function (`tests/pdf_audit_routing_image_heavy.test.js`, `tests/gemini_gate_canvas_hold_signature.test.js`, `tests/review_triage_resolve.test.js`), and the connector gates pass. What is not yet verified: a live Canvas run, which I cannot start; the next real Canvas run's diagnostic bundle (now working in Canvas) will show whether the slices, the skipped escalation and the hold policy behave as designed.
...
`desktop/dist/mcpb/alloflow-remediation.mcpb` was rebuilt twice more. After the palette-prompt fix: v0.11.0, 71,150,584 bytes, SHA-256 `86831fe731fe60e7aa4fc4c3f31e8bb1cb860b00193c1afaf81e941070d09cfd`. After the three enhancements above (a67ae4919), the third-party notice correction (9120638be), the other session's sliced-audit fix (3f3ae1896), the keyless-lane guidance in the bundled skill (f30ee4fa9), the Canvas throttle fixes (0309732e9) and the review-finding triage (49c1c2138): v0.11.0, 70,583,081 bytes, SHA-256 `5dd1ec5aa99731c380b5e77374052d6aa844605924b9413e7d5c0ff0520de5cc` (build verification passed: 41 tools, 59 hashed vendor files, HTTP transport probed). That is the build to hand out; it supersedes every earlier build of 13 September (`7bd8ba60…`, `86831fe7…`, `5ac99487…`, `d6d63f22…`, `1c893366…`). `verify:mcpb-ci` (41 tests), `verify:mcp-parity` (73) and `verify:mcp-calibration` (219) all pass on this tree. Nothing was published or sent anywhere.
