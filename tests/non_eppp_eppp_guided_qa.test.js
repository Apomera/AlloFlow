...
describe('EPPP-guided QA for every non-EPPP test-prep pack', () => {
  ...
  it('fails the shared release build closed on catalog, audit, and suite QA', () => {
    ...
    const identityStampCall = builder.indexOf('execFileSync(process.execPath,[stampLearningLibraryIdentityPath]');
    ...
    expect([correctionCall, postCorrectionQaCall, sourceBindingCall, additionsCall, expansionCall, postExpansionQaCall, identityStampCall, finalBindingCall, catalogCall, assistantAuditCall, suiteQaCall, verifyCall].every((index) => index >= 0)).toBe(true);
    ...
    expect(postExpansionQaCall).toBeLessThan(identityStampCall);
    expect(identityStampCall).toBeLessThan(finalBindingCall);
