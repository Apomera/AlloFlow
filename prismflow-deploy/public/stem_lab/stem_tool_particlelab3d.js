...
(function () {
  ...
  window.StemLab.registerTool('particleLab3d', {
    ...
    render: function (ctx) {
      ...
      var cssFsRef = useRef(false), showKeysRef = useRef(false);
      cssFsRef.current = cssFullscreen; showKeysRef.current = showKeys;
      ...
      useEffect(function () {
        function onLabKey(event) {
          ...
          if (event.key === 'Escape' && (showKeysRef.current || cssFsRef.current)) {
            event.preventDefault(); event.stopPropagation();
            if (showKeysRef.current) { setShowKeys(false); if (ctx.announceToSR) ctx.announceToSR('Closed the keyboard shortcuts panel.'); }
            else { setCssFullscreen(false); if (ctx.announceToSR) ctx.announceToSR('Exited immersive view.'); }
            return;
          }
          ...
          else if (event.key === '?') { event.preventDefault(); setShowKeys(function (value) { return !value; }); }
        }
      ...
      return h('div', { className: 'max-w-7xl mx-auto space-y-4 animate-in fade-in duration-300' },
        ...
        h('div', { className: 'grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]' },
          h('section', { ref: stageRef, className: 'overflow-hidden border border-cyan-500/30 bg-slate-950 shadow-2xl shadow-cyan-950/40 ring-1 ring-white/5 ' + (fsActive ? 'h-screen rounded-none flex flex-col' : 'rounded-[24px]'), style: cssFullscreen ? { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100vh', zIndex: 99990, borderRadius: 0 } : undefined, 'aria-label': 'Three-dimensional particle simulation' },
            ...
            h('div', { tabIndex: 0, role: 'application', 'aria-roledescription': 'Interactive 3D particle chamber', 'aria-label': 'Particle chamber. Click a particle to trace it, drag to orbit, and use the wheel to zoom. Space runs or pauses. R resets. T toggles tracing. V toggles vectors. E toggles speed colors. M toggles the membrane. G toggles gravity. C toggles the showcase camera. L toggles tracer follow. F toggles fullscreen. H hides or shows the simulation controls. The question mark key opens a panel explaining every shortcut.', 'aria-keyshortcuts': 'Space R T V E M G C L F H', onPointerDown: function (event) { try { event.currentTarget.focus(); } catch (error) {} }, className: 'relative min-h-[380px] overflow-hidden bg-slate-950 focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-[-4px] focus-visible:outline-cyan-200 ' + (fsActive ? 'flex-1' : 'h-[520px]') },
              h('canvas', { ref: canvasRef, className: 'h-full w-full saturate-[1.15] contrast-[1.04]', role: 'img', 'aria-label': preset + ' particle simulation in a transparent cubic container. Current measured temperature ' + stats.temperature + ' kelvin, pressure ' + stats.pressure + ' model units, and particle collision diameter ' + particleDiameter.toFixed(2) + ' model units. Gravity field ' + (gravity > 0.01 ? 'active at ' + gravity.toFixed(1) + ' model g' : 'off') + (preset === 'diffusion' ? '. Diffusion mixing index ' + Math.round(diffusionInfo.mixing * 100) + ' percent' : (preset === 'osmosis' ? '. Osmotic solvent shift ' + Math.round(diffusionInfo.osmoticShift * 100) + ' percent and solute leakage ' + Math.round(diffusionInfo.soluteLeak * 100) + ' percent' : '')) + (transportMode ? '. Particle B relative mass ' + massRatioB.toFixed(1) + ' times particle A. Membrane pore openness is ' + Math.round(poreOpennessA * 100) + ' percent for A and ' + Math.round(poreOpennessB * 100) + ' percent for B' : '') + (attraction > 0.6 ? '. Volumetric cluster envelopes show the ' + Math.min(6, clusterInfo.groups.length) + ' largest droplets; the largest contains ' + clusterInfo.largest + ' particles' : '') + '.' }),
              ...
              transportMode && membrane && h('div', { className: 'pointer-events-none absolute right-3 top-24 w-[150px] rounded-xl border border-violet-300/20 bg-slate-950/70 p-2.5 shadow-[0_0_24px_rgba(167,139,250,0.12)] backdrop-blur-md', role: 'img', 'aria-label': 'Live membrane pore lattice. Species A channel openness ' + Math.round(poreOpennessA * 100) + ' percent. Species B channel openness ' + Math.round(poreOpennessB * 100) + ' percent. Cyan and pink bursts show successful crossings by A and B; coral bursts show reflections.' },
                h('div', { className: 'text-[8px] font-black uppercase tracking-[0.18em] text-violet-200' }, 'Pore lattice'),
                [['A', poreOpennessA, 'bg-cyan-300', 'text-cyan-200'], ['B', poreOpennessB, 'bg-fuchsia-300', 'text-fuchsia-200']].map(function (row) { return h('div', { key: row[0], className: 'mt-2' }, h('div', { className: 'flex justify-between font-mono text-[8px] ' + row[3] }, h('span', null, row[0] + ' channels'), h('span', null, Math.round(row[1] * 100) + '%')), h('div', { className: 'mt-1 h-1.5 overflow-hidden rounded-full bg-white/10' }, h('div', { className: 'h-full rounded-full ' + row[2], style: { width: Math.round(row[1] * 100) + '%' } }))); }),
              ...
              showKeys && h('div', { className: 'absolute inset-0 z-30 flex items-center justify-center bg-slate-950/70 p-4', onClick: function () { setShowKeys(false); } },
                h('div', { role: 'dialog', 'aria-modal': 'true', 'aria-label': 'Keyboard shortcuts', onClick: function (event) { event.stopPropagation(); }, className: 'max-h-full w-full max-w-md overflow-y-auto rounded-2xl border border-cyan-300/30 bg-slate-950/95 p-5 shadow-2xl backdrop-blur' },
                  h('div', { className: 'flex items-center justify-between gap-3' }, h('h3', { className: 'text-sm font-black uppercase tracking-wider text-cyan-300' }, 'Keyboard shortcuts'), h('button', { type: 'button', onClick: function () { setShowKeys(false); }, className: 'rounded-lg bg-slate-800 px-3 py-1.5 text-xs font-bold text-white hover:bg-slate-700', 'aria-label': 'Close the keyboard shortcuts panel' }, 'Close')),
                  h('p', { className: 'mt-1 text-[11px] text-slate-400' }, 'Shortcuts work when the chamber has focus (click it once first) and not while typing in a text field.'),
              ...
              trace && h('div', { className: 'pointer-events-none absolute bottom-3 left-3 min-w-[220px] rounded-xl border border-yellow-200/50 bg-yellow-300/90 px-3 py-2 text-slate-950 shadow-[0_0_24px_rgba(253,224,71,0.28)] backdrop-blur', role: 'img', 'aria-label': 'Random walk analyzer for particle ' + (selectedParticle + 1) + ', species ' + (selectedInfo.type ? 'B' : 'A') + '. Speed ' + selectedInfo.speed.toFixed(2) + ' model units per second. Total path ' + selectedInfo.pathLength.toFixed(1) + ' model units, net displacement ' + selectedInfo.displacement.toFixed(1) + ', path efficiency ' + Math.round(selectedInfo.efficiency * 100) + ' percent, ' + selectedInfo.particleHits + ' particle collisions, ' + selectedInfo.wallHits + ' wall collisions, and ' + selectedInfo.membraneHits + ' membrane reflections. Mean free path ' + selectedInfo.meanFreePath.toFixed(1) + ' model units with standard error ' + selectedInfo.meanFreePathError.toFixed(1) + ' from ' + selectedInfo.freeFlightSamples + ' completed free flights, compared with a kinetic theory estimate of ' + currentMeanFreePathTheory.toFixed(1) + ' model units, and particle collision rate ' + selectedInfo.collisionRate.toFixed(2) + ' per second. Glowing rings mark recent tracer collisions, and a dashed color-shifting beam measures the current free flight.' },
                ...
                h('div', { className: 'mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-slate-950/10 pt-1 text-[8px] font-bold' }, h('span', { className: 'inline-flex items-center gap-1.5' }, h('span', { className: 'h-2 w-2 rounded-full border-2 border-cyan-700', 'aria-hidden': 'true' }), 'Rings: recent collisions'), h('span', { className: 'inline-flex items-center gap-1.5' }, h('span', { className: 'w-4 border-t-2 border-dashed border-cyan-700', 'aria-hidden': 'true' }), 'Dashed beam: current free flight'))
              )
            ),
            showHud && h('div', { className: 'flex flex-wrap items-center gap-2 border-t border-slate-700 p-3' },
              ...
              h('button', { type: 'button', onClick: function () { setShowKeys(function (value) { return !value; }); }, 'aria-expanded': showKeys, className: 'rounded-lg bg-slate-800 px-3 py-2 text-sm font-bold text-white hover:bg-slate-700', 'aria-label': 'Show the keyboard shortcuts panel' }, 'Keys (?)'),
              h('div', { className: 'ml-auto flex items-center gap-1 rounded-lg border border-slate-700 bg-slate-900 p-1', role: 'group', 'aria-label': 'Camera views' },
                ...
                h('button', { type: 'button', onClick: function () { var next = !autoCamera; setAutoCamera(next); if (next) setFollowTracer(false); persist({ autoCamera: next, followTracer: next ? false : followTracer }); }, 'aria-pressed': autoCamera, className: 'rounded px-2 py-1 text-[10px] font-black uppercase tracking-wide ' + (autoCamera ? 'bg-violet-300 text-slate-950' : 'text-violet-200 hover:bg-slate-700') }, autoCamera ? '\u25C9 Orbiting' : '\u25CE Showcase'),
                h('button', { type: 'button', disabled: prefersReducedMotion, onClick: function () { var next = !followTracer; setFollowTracer(next); if (next) { setTrace(true); setAutoCamera(false); persist({ followTracer: true, trace: true, traced: true, autoCamera: false }); } else persist({ followTracer: false }); if (ctx.announceToSR) ctx.announceToSR(next ? 'Tracer follow camera enabled.' : 'Tracer follow camera disabled.'); }, 'aria-pressed': followTracer, 'aria-label': prefersReducedMotion ? 'Tracer follow camera unavailable because reduced motion is preferred' : (followTracer ? 'Disable tracer follow camera' : 'Enable tracer follow camera'), className: 'rounded px-2 py-1 text-[10px] font-black uppercase tracking-wide disabled:cursor-not-allowed disabled:opacity-45 ' + (followTracer ? 'bg-yellow-300 text-slate-950' : 'text-yellow-200 hover:bg-slate-700') }, prefersReducedMotion ? '\u25CB Motion safe' : (followTracer ? '\u25C9 Following' : '\u25CE Follow'))
              ),
            ...
            h('div', { className: 'rounded-2xl border border-slate-200 bg-white p-4 shadow-sm' },
              ...
              transportMode && h('div', { className: 'mt-3 rounded-xl border border-violet-200 bg-gradient-to-br from-slate-950 to-violet-950 p-3 text-white', role: 'img', 'aria-label': (preset === 'osmosis' ? 'Osmosis monitor. Net solvent shift ' + Math.round(diffusionInfo.osmoticShift * 100) + ' percent and solute leakage ' + Math.round(diffusionInfo.soluteLeak * 100) + ' percent.' : 'Diffusion monitor. Mixing index ' + Math.round(diffusionInfo.mixing * 100) + ' percent.') + ' Particle B has a relative mass of ' + massRatioB.toFixed(1) + ' times particle A. Particle A concentration is ' + Math.round(diffusionInfo.leftA * 100) + ' percent on the left and ' + Math.round(diffusionInfo.rightA * 100) + ' percent on the right. Particle A crossing flux ' + diffusionInfo.fluxA.toFixed(1) + ' per second and particle B crossing flux ' + diffusionInfo.fluxB.toFixed(1) + ' per second. Spatial mixing entropy is ' + Math.round(diffusionInfo.entropy * 100) + ' percent. Particle A diffusion coefficient is ' + diffusionInfo.coefficientA.toFixed(2) + ' and particle B diffusion coefficient is ' + diffusionInfo.coefficientB.toFixed(2) + '. Mean squared displacement is ' + diffusionInfo.msd.toFixed(1) + ' square model units. ' + (diffusionInfo.milestoneTime == null ? 'The transport milestone has not been reached.' : 'The transport milestone was reached at ' + diffusionInfo.milestoneTime.toFixed(1) + ' seconds.') },
                ...
                preset === 'diffusion' && h('div', { className: 'mt-3 overflow-hidden rounded-xl border border-cyan-300/20 bg-gradient-to-br from-cyan-950/70 via-indigo-950/80 to-fuchsia-950/70 p-3 shadow-[0_0_24px_rgba(34,211,238,0.08)]', role: 'region', 'aria-label': 'Diffusion transport fingerprint. Species A exponent alpha ' + diffusionRegimeA.alpha.toFixed(2) + ', classified as ' + diffusionRegimeA.regime + '. Species B exponent alpha ' + diffusionRegimeB.alpha.toFixed(2) + ', classified as ' + diffusionRegimeB.regime + '. Alpha near one indicates normal diffusion; the dashed line is the normal diffusion guide.' },
                  h('div', { className: 'flex flex-wrap items-start justify-between gap-2' }, h('div', null, h('div', { className: 'text-[8px] font-black uppercase tracking-[0.18em] text-cyan-200' }, 'Transport fingerprint'), h('div', { className: 'mt-0.5 text-[10px] font-bold text-white' }, 'How mean-squared displacement scales with time')), h('span', { className: 'rounded-full border border-white/10 bg-white/5 px-2 py-1 font-mono text-[8px] text-indigo-100' }, 'MSD ∝ time^α')),
                  ...
                  h('div', { className: 'mt-2 grid grid-cols-2 gap-2' }, [['A', diffusionRegimeA, 'border-cyan-300/30 bg-cyan-300/10 text-cyan-100'], ['B', diffusionRegimeB, 'border-fuchsia-300/30 bg-fuchsia-300/10 text-fuchsia-100']].map(function (item) { return h('div', { key: item[0], className: 'rounded-lg border p-2 ' + item[2] }, h('div', { className: 'flex items-baseline justify-between gap-1' }, h('span', { className: 'text-[8px] font-black uppercase' }, 'Species ' + item[0]), h('span', { className: 'font-mono text-sm font-black' }, item[1].samples < 4 ? '—' : 'α ' + item[1].alpha.toFixed(2))), h('div', { className: 'mt-0.5 text-[7px] font-bold uppercase tracking-wide' }, item[1].regime)); })),
                  h('p', { className: 'mt-2 text-[8px] leading-relaxed text-indigo-100' }, diffusionRegimeA.samples < 4 ? 'Run the chamber to reveal whether motion is ballistic, diffusive, slowed, or confined.' : diffusionRegimeA.note + ' In a finite chamber, the curves eventually flatten as particles encounter boundaries.')
                  ...
                  h('div', { className: 'flex items-center justify-between text-[8px] font-black uppercase tracking-wide text-slate-300' }, h('span', null, 'Ensemble collision transport'), h('span', null, 'theory reference ' + currentMeanFreePathTheory.toFixed(1) + ' u')),
                  [['A', ensembleFlights.a, 'from-cyan-300 to-sky-500'], ['B', ensembleFlights.b, 'from-fuchsia-300 to-pink-500']].map(function (row) { var width = clamp(row[1].mean / Math.max(0.001, currentMeanFreePathTheory * 2) * 100, 0, 100); return h('div', { key: row[0], className: 'mt-2' }, h('div', { className: 'flex justify-between gap-2 font-mono text-[9px] text-slate-200' }, h('span', null, row[0] + ' path ' + row[1].mean.toFixed(1) + ' +/- ' + row[1].error.toFixed(1)), h('span', null, row[1].samples + ' flights, ' + row[1].collisionRate.toFixed(2) + '/s')), h('div', { className: 'relative mt-1 h-2 overflow-hidden rounded-full bg-white/10' }, h('div', { className: 'h-full rounded-full bg-gradient-to-r transition-all ' + row[2], style: { width: width + '%' } }), h('span', { className: 'absolute inset-y-0 left-1/2 w-px bg-yellow-200' }))); }),
                  h('p', { className: 'mt-2 text-[8px] leading-relaxed text-slate-400' }, 'Equal-size species should approach a similar mean free path; mass and speed can still change how often collisions occur.')
                ),
              ...
              trace && h('div', { className: 'mt-3 rounded-xl border border-yellow-200 bg-gradient-to-br from-yellow-50 to-amber-50 p-3', role: 'region', 'aria-label': 'Tracer walk comparison notebook' },
                ...
                (activeProtocol === 'mfp' || activeProtocol === 'crosssection') && h('div', { className: 'mt-2 overflow-hidden rounded-xl border border-amber-300 bg-gradient-to-r from-amber-950 via-slate-950 to-violet-950 p-3 text-white shadow-lg', role: 'region', 'aria-label': 'Mean free path protocol progress. ' + mfpStepsComplete + ' of 5 steps complete.' },
                  h('div', { className: 'flex items-center justify-between gap-2' }, h('div', null, h('div', { className: 'text-[8px] font-black uppercase tracking-[0.18em] text-amber-300' }, 'Guided protocol progress'), h('div', { className: 'mt-0.5 text-[10px] font-bold text-white' }, mfpStepsComplete === 5 ? 'Investigation complete' : 'Next: ' + (mfpProtocolSteps.filter(function (step) { return !step.complete; })[0] || { label: 'Review evidence' }).label)), h('span', { className: 'rounded-full border border-amber-300/40 bg-amber-300/10 px-2 py-1 font-mono text-[9px] font-black text-amber-100' }, mfpStepsComplete + '/5')),
                  h('div', { className: 'mt-3 grid grid-cols-5 gap-1' }, mfpProtocolSteps.map(function (step, i) { return h('div', { key: step.label, className: 'min-w-0 text-center' }, h('div', { className: 'mx-auto flex h-6 w-6 items-center justify-center rounded-full border text-[9px] font-black ' + (step.complete ? 'border-emerald-300 bg-emerald-300 text-emerald-950' : 'border-slate-600 bg-slate-800 text-slate-300'), 'aria-hidden': 'true' }, step.complete ? '\u2713' : i + 1), h('div', { className: 'mt-1 text-[7px] font-bold leading-tight ' + (step.complete ? 'text-emerald-200' : 'text-slate-400') }, step.label)); })),
                  h('div', { className: 'mt-3 grid gap-2 sm:grid-cols-[1fr_auto]' }, h('div', { className: 'rounded-lg border border-white/10 bg-white/5 p-2' }, h('div', { className: 'flex items-center justify-between gap-2' }, h('span', { className: 'text-[8px] font-black uppercase tracking-wide text-slate-300' }, 'Sample quality'), h('span', { className: 'text-[9px] font-black ' + tracerSampleQuality.tone.replace('800', '200').replace('700', '200') }, tracerSampleQuality.label)), h('div', { className: 'mt-1.5 h-1.5 overflow-hidden rounded-full bg-slate-700', role: 'progressbar', 'aria-label': 'Free-flight sample progress toward twelve recommended samples', 'aria-valuemin': 0, 'aria-valuemax': 12, 'aria-valuenow': Math.min(12, selectedInfo.freeFlightSamples) }, h('div', { className: 'h-full rounded-full transition-all ' + tracerSampleQuality.fill, style: { width: Math.round(tracerSampleProgress * 100) + '%' } })), h('div', { className: 'mt-1 flex justify-between gap-2 text-[8px] text-slate-300' }, h('span', null, tracerSampleQuality.note), h('span', { className: 'shrink-0 font-mono' }, selectedInfo.freeFlightSamples + '/12 flights'))), liveTracerControl && h('div', { className: 'rounded-lg border px-2 py-1.5 text-center ' + (liveTracerControl.fair ? 'border-emerald-300/40 bg-emerald-300/10 text-emerald-100' : (liveTracerControl.replicate ? 'border-slate-500 bg-slate-700/40 text-slate-200' : 'border-rose-300/40 bg-rose-300/10 text-rose-100')), role: 'status', 'aria-live': 'polite' }, h('div', { className: 'text-[8px] font-black uppercase tracking-wide' }, liveTracerControl.fair ? 'One variable changed' : (liveTracerControl.replicate ? 'Conditions unchanged' : 'Multiple changes')), h('div', { className: 'mt-0.5 max-w-[120px] text-[7px] leading-tight' }, liveTracerControl.fair ? liveTracerControl.changedVariable : (liveTracerControl.replicate ? 'Adjust count or volume.' : liveTracerControl.changed.join(', '))))
                )),
                h('div', { className: 'mt-2 rounded-lg border border-amber-200 bg-white/80 p-2' },
                  h('div', { className: 'flex justify-between text-[8px] font-black uppercase tracking-wide text-amber-900' }, h('span', null, 'Live free-flight distribution'), h('span', null, flightChart.count + ' samples')),
                  ...
                  h('div', { className: 'flex justify-between font-mono text-[8px] text-amber-900' }, h('span', null, 'bars observed'), h('span', null, 'dashed mean'), h('span', null, 'line theory'))
                ),
                (activeProtocol === 'mfp' || activeProtocol === 'crosssection') && h('div', { className: 'mt-2 overflow-hidden rounded-xl border border-cyan-200 bg-gradient-to-br from-slate-950 via-cyan-950 to-indigo-950 p-3 text-white shadow-lg', role: 'img', 'aria-label': recentFlightSequence.length ? 'Flight by flight timeline from oldest to newest. Recent completed flight lengths in model units: ' + recentFlightSequence.map(function (flight) { return flight.toFixed(1); }).join(', ') + '. Kinetic theory reference ' + currentMeanFreePathTheory.toFixed(1) + '. Current unfinished flight ' + selectedInfo.currentFreeFlight.toFixed(1) + '.' : 'Flight by flight timeline waiting for the first completed tracer collision.' },
                  h('div', { className: 'flex flex-wrap items-start justify-between gap-2' }, h('div', null, h('div', { className: 'text-[8px] font-black uppercase tracking-[0.18em] text-cyan-300' }, 'Flight-by-flight timeline'), h('div', { className: 'mt-0.5 text-[10px] font-bold text-white' }, 'Temporal order reveals randomness and rare long flights.')), h('div', { className: 'rounded-lg border border-cyan-300/20 bg-cyan-300/10 px-2 py-1 text-right' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-cyan-200' }, 'Current flight'), h('div', { className: 'font-mono text-xs font-black text-white' }, selectedInfo.currentFreeFlight.toFixed(1) + ' u'))),
                  ...
                  h('div', { className: 'mt-1 grid grid-cols-3 gap-1.5' }, h('div', { className: 'rounded-lg bg-white/5 p-2' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-cyan-300' }, 'Shortest'), h('div', { className: 'mt-0.5 font-mono text-[10px] font-black' }, recentFlightSequence.length ? flightSequenceShortest.toFixed(1) + ' u' : '--')), h('div', { className: 'rounded-lg bg-white/5 p-2' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-violet-300' }, 'Longest'), h('div', { className: 'mt-0.5 font-mono text-[10px] font-black' }, recentFlightSequence.length ? flightSequenceLongest.toFixed(1) + ' u' : '--')), h('div', { className: 'rounded-lg bg-white/5 p-2' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-fuchsia-300' }, 'Long flights'), h('div', { className: 'mt-0.5 font-mono text-[10px] font-black' }, flightSequenceLongCount + ' above 1.5x'))),
                  h('p', { className: 'mt-2 text-[8px] leading-relaxed text-cyan-100' }, 'A random collision process should look irregular: many short flights can be interrupted by occasional long ones. A trend is not required for the mean to be meaningful.')
                ),
                (activeProtocol === 'mfp' || activeProtocol === 'crosssection') && h('div', { className: 'mt-2 rounded-xl border border-violet-200 bg-gradient-to-br from-white via-violet-50 to-cyan-50 p-3 shadow-sm', role: 'img', 'aria-label': selectedInfo.freeFlightSamples < 3 ? 'Observed versus theory agreement monitor waiting for at least three completed free flights.' : 'Observed versus theory agreement monitor. Observed mean free path ' + selectedInfo.meanFreePath.toFixed(1) + ' model units. Kinetic theory estimate ' + currentMeanFreePathTheory.toFixed(1) + '. Approximate 95 percent uncertainty range ' + mfpConfidenceLow.toFixed(1) + ' to ' + mfpConfidenceHigh.toFixed(1) + '. ' + mfpAgreement.label + '.' },
                  h('div', { className: 'flex flex-wrap items-start justify-between gap-2' }, h('div', null, h('div', { className: 'text-[8px] font-black uppercase tracking-[0.16em] text-violet-800' }, 'Observed vs. kinetic theory'), h('div', { className: 'mt-0.5 text-[10px] font-bold text-slate-800' }, mfpAgreement.note)), h('span', { className: 'rounded-full px-2 py-1 text-[8px] font-black uppercase tracking-wide ' + mfpAgreement.tone }, mfpAgreement.label)),
                  ...
                  h('div', { className: 'mt-1 flex justify-between font-mono text-[7px] text-slate-500' }, h('span', null, '0x theory'), h('span', { className: 'font-black text-violet-800' }, '1x theory'), h('span', null, '2x theory')),
                  h('div', { className: 'mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4' },
                    h('div', { className: 'rounded-lg border border-cyan-100 bg-white/80 p-2' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-cyan-800' }, 'Observed mean'), h('div', { className: 'mt-0.5 font-mono text-xs font-black text-slate-950' }, selectedInfo.freeFlightSamples ? selectedInfo.meanFreePath.toFixed(1) + ' u' : '--')),
                    h('div', { className: 'rounded-lg border border-violet-100 bg-white/80 p-2' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-violet-800' }, 'Theory'), h('div', { className: 'mt-0.5 font-mono text-xs font-black text-slate-950' }, currentMeanFreePathTheory.toFixed(1) + ' u')),
                    h('div', { className: 'rounded-lg border border-indigo-100 bg-white/80 p-2' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-indigo-800' }, 'Approx. 95% range'), h('div', { className: 'mt-0.5 font-mono text-[9px] font-black text-slate-950' }, selectedInfo.freeFlightSamples >= 2 ? mfpConfidenceLow.toFixed(1) + '-' + mfpConfidenceHigh.toFixed(1) + ' u' : '--')),
                    h('div', { className: 'rounded-lg border border-fuchsia-100 bg-white/80 p-2' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-fuchsia-800' }, 'Difference'), h('div', { className: 'mt-0.5 font-mono text-xs font-black text-slate-950' }, selectedInfo.freeFlightSamples ? Math.round(mfpAgreementError * 100) + '%' : '--'))
                  )
                ),
                tracerTrials.length ? h('div', { className: 'mt-2 grid grid-cols-2 gap-2' }, tracerTrials.map(function (walk, i) { return h('div', { key: walk.id, className: 'rounded-lg border border-amber-200 bg-white/80 p-2', 'aria-label': 'Walk ' + (i + 1) + ', species ' + walk.species + ', mean free path ' + walk.meanFreePath.toFixed(1) + ', standard error ' + Number(walk.meanFreePathError || 0).toFixed(1) + ' from ' + Number(walk.freeFlightSamples || walk.particleHits || 0) + ' completed flights, kinetic theory estimate ' + Number(walk.theoreticalMeanFreePath || meanFreePathEstimate(walk.count, walk.boxSize, walk.particleDiameter)).toFixed(1) + ', particle collision diameter ' + Number(walk.particleDiameter || 0.58).toFixed(2) + ', collision rate ' + walk.collisionRate.toFixed(2) + ' per second.' }, h('div', { className: 'flex justify-between text-[9px] font-black text-amber-900' }, h('span', null, 'Walk ' + (i + 1) + ' \u2022 ' + walk.species + '-' + walk.particle), h('span', null, walk.temperature + ' K')), h('div', { className: 'mt-1 font-mono text-xs font-black text-slate-900' }, '\u03BB ' + walk.meanFreePath.toFixed(1) + ' u'), h('div', { className: 'mt-0.5 font-mono text-[9px] text-amber-800' }, 'uncertainty +/- ' + Number(walk.meanFreePathError || 0).toFixed(1) + ' u, n=' + Number(walk.freeFlightSamples || walk.particleHits || 0)), h('div', { className: 'mt-0.5 font-mono text-[9px] text-amber-800' }, 'theory ' + Number(walk.theoreticalMeanFreePath || meanFreePathEstimate(walk.count, walk.boxSize, walk.particleDiameter)).toFixed(1) + ' u'), h('div', { className: 'mt-0.5 font-mono text-[9px] text-slate-600' }, walk.collisionRate.toFixed(2) + ' hits/s \u2022 N ' + walk.count + ' \u2022 V ' + Math.round(Math.pow(walk.boxSize, 3)) + ' \u2022 d ' + Number(walk.particleDiameter || 0.58).toFixed(2))); })) : h('p', { className: 'mt-2 text-[10px] leading-relaxed text-amber-900' }, 'Start a new walk, wait for a particle collision, then use Save walk.'),
                mfpPredictionAvailable && h('div', { className: 'mt-2 overflow-hidden rounded-xl border border-indigo-300 bg-gradient-to-r from-indigo-950 via-violet-950 to-fuchsia-950 p-3 text-white shadow-lg', role: 'img', 'aria-label': (tracerTrials.length === 2 ? 'Kinetic theory prediction check.' : 'Kinetic theory prediction preview.') + ' Changed variable ' + mfpPredictionControl.changedVariable + '. Mean free path is predicted to ' + mfpPredictionDirection + ' from ' + Number(tracerTrials[0].meanFreePath || 0).toFixed(1) + ' to ' + mfpPredictedWalk2.toFixed(1) + ' model units. ' + (tracerTrials.length === 2 ? 'Walk 2 observed value ' + Number(tracerTrials[1].meanFreePath || 0).toFixed(1) + ', prediction error ' + Math.round(mfpPredictionErrorPercent) + ' percent.' : 'Walk 2 has not been saved yet.') },
                  h('div', { className: 'flex flex-wrap items-start justify-between gap-2' }, h('div', null, h('div', { className: 'text-[8px] font-black uppercase tracking-[0.18em] text-fuchsia-300' }, tracerTrials.length === 2 ? 'Prediction check' : 'Prediction preview'), h('div', { className: 'mt-0.5 text-[10px] font-bold text-white' }, 'Kinetic theory expects mean free path to ' + mfpPredictionDirection + '.')), h('span', { className: 'rounded-full border border-fuchsia-300/40 bg-fuchsia-300/10 px-2 py-1 text-[8px] font-black uppercase tracking-wide text-fuchsia-100' }, mfpPredictionControl.changedVariable)),
                  h('div', { className: 'mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-2' },
                    h('div', { className: 'rounded-lg border border-white/10 bg-white/5 p-2 text-center' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-indigo-200' }, 'Walk 1 observed'), h('div', { className: 'mt-0.5 font-mono text-lg font-black' }, Number(tracerTrials[0].meanFreePath || 0).toFixed(1)), h('div', { className: 'text-[7px] text-slate-300' }, 'model units')),
                    h('div', { className: 'text-center' }, h('div', { className: 'text-xl font-black text-fuchsia-300', 'aria-hidden': 'true' }, mfpPredictionDirection === 'increase' ? '\u2197' : (mfpPredictionDirection === 'decrease' ? '\u2198' : '\u2192')), h('div', { className: 'mt-0.5 rounded bg-white/10 px-1.5 py-0.5 font-mono text-[8px] font-black text-fuchsia-100' }, (mfpPredictionDeltaPercent >= 0 ? '+' : '') + mfpPredictionDeltaPercent + '%')),
                    h('div', { className: 'rounded-lg border border-fuchsia-300/30 bg-fuchsia-300/10 p-2 text-center' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-fuchsia-200' }, tracerTrials.length === 2 ? 'Walk 2 observed' : 'Walk 2 predicted'), h('div', { className: 'mt-0.5 font-mono text-lg font-black' }, (tracerTrials.length === 2 ? Number(tracerTrials[1].meanFreePath || 0) : mfpPredictedWalk2).toFixed(1)), h('div', { className: 'text-[7px] text-slate-300' }, tracerTrials.length === 2 ? 'predicted ' + mfpPredictedWalk2.toFixed(1) : 'model units'))
                  ),
                  h('div', { className: 'mt-2 flex flex-wrap items-center justify-between gap-2 border-t border-white/10 pt-2 text-[8px] leading-relaxed text-indigo-100' }, h('span', null, mfpPredictionControl.changedVariable === 'particle count' || mfpPredictionControl.changedVariable === 'chamber volume' || mfpPredictionControl.changedVariable === 'particle diameter' ? 'Prediction scales Walk 1 by the change in theoretical mean free path.' : 'At fixed number density, this hard-sphere model predicts little mean-free-path change.'), tracerTrials.length === 2 && h('span', { className: 'rounded bg-white/10 px-2 py-1 font-mono font-black text-white' }, Math.round(mfpPredictionErrorPercent) + '% prediction error'))
                ),
              ...
              (activeProtocol === 'settling' || gravity > 0.01) && h('div', { className: 'mt-3 overflow-hidden rounded-xl border border-amber-200 bg-gradient-to-br from-slate-950 via-indigo-950 to-amber-950 p-3 text-white shadow-lg', role: 'region', 'aria-label': 'Vertical density observatory. Current profile from bottom to top. Band populations: ' + heightInfo.bins.map(function (value) { return Math.round(value * 100) + ' percent'; }).join(', ') + '. Center of mass height ' + heightInfo.centerY.toFixed(2) + ' model units. Bottom to top population ratio ' + heightInfo.bottomTopRatio.toFixed(2) + '. Gravity ' + (gravity > 0.01 ? gravity.toFixed(1) + ' model g' : 'off') + '.' },
                h('div', { className: 'flex items-start justify-between gap-2' }, h('div', null, h('div', { className: 'text-[9px] font-black uppercase tracking-[0.17em] text-amber-300' }, 'Vertical density profile'), h('div', { className: 'mt-0.5 text-[10px] text-indigo-100' }, gravity > 0.01 ? 'Watch particles redistribute from top to bottom.' : 'Microgravity baseline should remain roughly uniform.')), h('span', { className: 'rounded-full px-2 py-1 text-[8px] font-black uppercase tracking-wide ' + (gravity > 0.01 ? 'bg-amber-300 text-amber-950' : 'bg-indigo-200 text-indigo-950') }, gravity > 0.01 ? gravity.toFixed(1) + ' g*' : 'baseline')),
                ...
                h('div', { className: 'mt-1 grid grid-cols-3 gap-1.5 text-center' }, h('div', { className: 'rounded-lg bg-white/5 p-2' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-indigo-200' }, 'Center height'), h('div', { className: 'font-mono text-[10px] font-black' }, (heightInfo.centerY >= 0 ? '+' : '') + heightInfo.centerY.toFixed(2) + ' u')), h('div', { className: 'rounded-lg bg-white/5 p-2' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-amber-200' }, 'Bottom / top'), h('div', { className: 'font-mono text-[10px] font-black' }, heightInfo.bottomTopRatio.toFixed(2) + 'x')), h('div', { className: 'rounded-lg bg-white/5 p-2' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-cyan-200' }, 'Bottom quarter'), h('div', { className: 'font-mono text-[10px] font-black' }, Math.round(heightInfo.bottomFraction * 100) + '%'))),
                h('div', { className: 'mt-3 border-t border-white/10 pt-2' },
                  h('div', { className: 'flex items-center justify-between gap-2' }, h('div', { className: 'text-[8px] font-black uppercase tracking-[0.15em] text-indigo-200' }, 'Gradient evolution'), h('span', { className: 'rounded bg-white/5 px-2 py-1 font-mono text-[7px] text-slate-300' }, gravityTimeline.length + ' samples')),
                  h('div', { className: 'mt-2 grid grid-cols-2 gap-2' },
                    h('div', { className: 'rounded-lg border border-cyan-300/10 bg-white/5 p-2' }, h('div', { className: 'flex justify-between text-[7px] font-black uppercase tracking-wide text-cyan-200' }, h('span', null, 'Center height'), h('span', null, gravityCenterTrend.direction)), h('svg', { viewBox: '0 0 240 64', className: 'mt-1 h-[54px] w-full', role: 'img', 'aria-label': gravityCenters.length > 1 ? 'Center height over time, currently ' + gravityCenterTrend.latest.toFixed(2) + ' model units and ' + gravityCenterTrend.direction + '.' : 'Center height timeline waiting for more samples.' }, gravityCenters.length > 1 ? h('path', { d: sharedScalePath(gravityCenters, gravityCenterMin, gravityCenterMax), fill: 'none', stroke: '#67e8f9', strokeWidth: 3, strokeLinecap: 'round', strokeLinejoin: 'round' }) : h('text', { x: 120, y: 35, textAnchor: 'middle', fontSize: 9, fill: '#a5f3fc' }, 'collecting...')), h('div', { className: 'mt-0.5 flex justify-between font-mono text-[7px] text-slate-400' }, h('span', null, 'start'), h('span', null, (heightInfo.centerY >= 0 ? '+' : '') + heightInfo.centerY.toFixed(2) + ' u'))),
                    h('div', { className: 'rounded-lg border border-amber-300/10 bg-white/5 p-2' }, h('div', { className: 'flex justify-between text-[7px] font-black uppercase tracking-wide text-amber-200' }, h('span', null, 'Bottom / top'), h('span', null, gravityRatioTrend.direction)), h('svg', { viewBox: '0 0 240 64', className: 'mt-1 h-[54px] w-full', role: 'img', 'aria-label': gravityRatios.length > 1 ? 'Bottom to top population ratio over time, currently ' + gravityRatioTrend.latest.toFixed(2) + ' times and ' + gravityRatioTrend.direction + '.' : 'Bottom to top ratio timeline waiting for more samples.' }, gravityRatios.length > 1 ? h('path', { d: sharedScalePath(gravityRatios, 0, gravityRatioMax), fill: 'none', stroke: '#fbbf24', strokeWidth: 3, strokeLinecap: 'round', strokeLinejoin: 'round' }) : h('text', { x: 120, y: 35, textAnchor: 'middle', fontSize: 9, fill: '#fde68a' }, 'collecting...')), h('div', { className: 'mt-0.5 flex justify-between font-mono text-[7px] text-slate-400' }, h('span', null, 'start'), h('span', null, heightInfo.bottomTopRatio.toFixed(2) + 'x')))
                  ),
                  h('div', { className: 'mt-2 rounded-lg border px-2 py-1.5 text-[8px] leading-relaxed ' + (gravityMilestone ? 'border-emerald-300/30 bg-emerald-300/10 text-emerald-100' : 'border-white/10 bg-white/5 text-indigo-100'), role: 'status', 'aria-live': 'polite' }, gravityMilestone ? 'Gradient milestone reached at ' + Number(gravityMilestone.elapsed || 0).toFixed(1) + ' s when bottom/top reached 1.50x.' : (gravity > 0.01 ? 'Waiting for bottom/top population to reach 1.50x.' : 'Record the baseline, then increase gravity to begin the gradient clock.'))
                )
              ),
              (activeProtocol === 'condense' || attraction > 0.6) && h('div', { className: 'mt-3 overflow-hidden rounded-xl border border-emerald-200 bg-gradient-to-br from-slate-950 via-emerald-950 to-cyan-950 p-3 text-white shadow-lg', role: 'region', 'aria-label': 'Condensation observatory. ' + clusterInfo.clusterCount + ' molecular groups detected using a distance cutoff of ' + clusterInfo.cutoff.toFixed(2) + ' model units. Largest cluster contains ' + clusterInfo.largest + ' particles, or ' + Math.round(clusterInfo.largestFraction * 100) + ' percent of the system. ' + Math.round(clusterInfo.bondedFraction * 100) + ' percent of particles have at least one nearby neighbor. ' + coalescenceEvents.length + ' coalescence events have been detected. Largest cluster trend is ' + clusterTrend.direction + '.' },
                h('div', { className: 'flex flex-wrap items-start justify-between gap-2' }, h('div', null, h('div', { className: 'text-[9px] font-black uppercase tracking-[0.17em] text-emerald-300' }, 'Condensation observatory'), h('div', { className: 'mt-0.5 text-[10px] text-cyan-100' }, 'Watch droplets nucleate and merge.')), h('span', { className: 'rounded-full border px-2 py-1 text-[8px] font-black uppercase tracking-wide ' + (clusterInfo.largestFraction >= 0.3 ? 'border-emerald-300/40 bg-emerald-300 text-emerald-950' : 'border-white/10 bg-white/5 text-slate-200') }, clusterInfo.largestFraction >= 0.3 ? 'nucleated' : 'molecular vapor')),
                h('div', { className: 'mt-3 grid grid-cols-2 gap-1.5 text-center sm:grid-cols-4' }, [['Largest droplet', clusterInfo.largest + ' particles'], ['Condensed share', Math.round(clusterInfo.largestFraction * 100) + '%'], ['Neighbor bonded', Math.round(clusterInfo.bondedFraction * 100) + '%'], ['Coalescence', coalescenceEvents.length + ' merges']].map(function (item) { return h('div', { key: item[0], className: 'rounded-lg border border-white/10 bg-white/5 p-2' }, h('div', { className: 'text-[7px] font-black uppercase tracking-wide text-emerald-200' }, item[0]), h('div', { className: 'mt-0.5 font-mono text-[10px] font-black text-white' }, item[1])); })),
                h('div', { className: 'mt-3 rounded-lg border border-white/10 bg-white/[0.04] p-2' }, h('div', { className: 'flex justify-between text-[8px] font-black uppercase tracking-wide text-slate-300' }, h('span', null, 'Cluster population'), h('span', null, clusterInfo.clusterCount + ' groups')), h('svg', { viewBox: '0 0 240 70', className: 'mt-1 h-[70px] w-full', role: 'img', 'aria-label': 'Bubble chart of the largest detected molecular clusters. Cluster sizes: ' + clusterInfo.sizes.slice(0, 10).join(', ') + ' particles.' }, clusterInfo.sizes.slice(0, 10).map(function (size, i) { var radius = clamp(Math.sqrt(size) * 3.5, 3, 15), x = 17 + i * 23, y = 35 + (i % 2 ? 11 : -7); return h('g', { key: i }, h('circle', { cx: x, cy: y, r: radius, fill: i === 0 ? '#34d399' : (i < 4 ? '#22d3ee' : '#818cf8'), opacity: 0.35 + Math.min(0.55, size / Math.max(1, clusterInfo.largest) * 0.55), stroke: i === 0 ? '#a7f3d0' : 'rgba(255,255,255,.35)', strokeWidth: 1 }), size > 1 && h('text', { x: x, y: y + 2.5, textAnchor: 'middle', fontSize: 7, fill: '#ffffff' }, size)); }))),
                h('div', { className: 'mt-3 rounded-lg border border-white/10 bg-white/[0.04] p-2' }, h('div', { className: 'flex justify-between text-[8px] font-black uppercase tracking-wide text-slate-300' }, h('span', null, 'Nucleation timeline'), h('span', null, clusterTrend.direction)), h('svg', { viewBox: '0 0 240 64', className: 'mt-1 h-14 w-full', role: 'img', 'aria-label': clusterFractions.length > 1 ? 'Largest cluster fraction over time, currently ' + Math.round(clusterInfo.largestFraction * 100) + ' percent and ' + clusterTrend.direction + '.' : 'Nucleation timeline waiting for more samples.' }, h('line', { x1: 4, y1: 56 - 0.3 * 48, x2: 236, y2: 56 - 0.3 * 48, stroke: '#fde68a', strokeWidth: 1, strokeDasharray: '4 3' }), clusterFractions.length > 1 ? h('g', null, h('path', { d: percentPath(clusterFractions), fill: 'none', stroke: '#34d399', strokeWidth: 3, strokeLinecap: 'round', strokeLinejoin: 'round' }), clusterTimeline.map(function (sample, i) { if (!sample.clusterMerge) return null; var x = 4 + i / Math.max(1, clusterTimeline.length - 1) * 232, y = 56 - clamp(Number(sample.clusterFraction) || 0, 0, 1) * 48; return h('circle', { key: i, cx: x, cy: y, r: 4, fill: '#fde68a', stroke: '#064e3b', strokeWidth: 1.5 }); })) : h('text', { x: 120, y: 35, textAnchor: 'middle', fontSize: 9, fill: '#a7f3d0' }, 'collecting cluster evidence...')), h('div', { className: 'mt-1 flex justify-between text-[7px] text-slate-400' }, h('span', null, 'vapor'), h('span', { className: 'text-yellow-100' }, '30% nucleation marker'), h('span', null, 'droplet'))),
                h('div', { className: 'mt-2 rounded-lg border px-2 py-1.5 text-[8px] leading-relaxed ' + (clusterMilestone ? 'border-emerald-300/30 bg-emerald-300/10 text-emerald-100' : 'border-white/10 bg-white/5 text-cyan-100'), role: 'status', 'aria-live': 'polite' }, lastCoalescence ? 'Coalescence detected at ' + Number(lastCoalescence.elapsed || 0).toFixed(1) + ' seconds as smaller groups merged into a larger droplet.' : (clusterMilestone ? 'Nucleation milestone reached at ' + Number(clusterMilestone.elapsed || 0).toFixed(1) + ' seconds.' : 'Increase attraction or reduce temperature, then watch small groups merge into a persistent droplet.')),
                h('p', { className: 'mt-2 text-[7px] leading-relaxed text-slate-400' }, 'Model note: clusters are connected groups whose centers fall within ' + clusterInfo.cutoff.toFixed(2) + ' model units; this is an educational proximity criterion, not a molecular phase boundary.')
