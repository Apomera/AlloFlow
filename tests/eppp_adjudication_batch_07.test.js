...
describe('EPPP claim-level adjudication batch 07', () => {
  it('reviews ten high-risk candidates without releasing or repeating them', () => {
    const report = read('quality/eppp_provenance/evidence/adjudication/adjudication_batch_07.json');
    const priorIds = new Set(['01', '02', '03', '04', '05', '06'].flatMap((batch) => read(`quality/eppp_provenance/evidence/adjudication/adjudication_batch_${batch}.json`).items.map((item) => item.legacyId)));
  ...
  it('keeps JSON and Markdown canonical and outside runtime publication', () => {
    expect(fs.existsSync(resolve(root, 'quality/eppp_provenance/evidence/adjudication/adjudication_batch_07.json'))).toBe(true);
    expect(fs.existsSync(resolve(root, 'quality/eppp_provenance/evidence/adjudication/adjudication_batch_07.md'))).toBe(true);
    expect(fs.existsSync(resolve(root, 'desktop/web-app/public/test_prep/eppp_legacy/adjudication_batch_07.json'))).toBe(false);
  });
});
