...
const files = ['lesson_board_module.js', 'desktop/web-app/public/lesson_board_module.js'];
if (process.argv.includes('--check')) {
  for (const file of files) if (!fs.existsSync(file) || fs.readFileSync(file, 'utf8') !== output) throw Error('Board bundle is stale: ' + file);
  console.log('Lesson board bundles match source (' + output.length + ' bytes)');
} else {
  for (const file of files) { fs.writeFileSync(file + '.build-tmp', output); fs.renameSync(file + '.build-tmp', file); }
