...
describe('EPPP claim-level adjudication batch 07', () => {
  it('reviews ten high-risk candidates without releasing or repeating them', () => {
    const report = read('test_prep/eppp_legacy/adjudication_batch_07.json');
    const priorIds = new Set(['01', '02', '03', '04', '05', '06'].flatMap((batch) => read(`test_prep/eppp_legacy/adjudication_batch_${batch}.json`).items.map((item) => item.legacyId)));
  ...
  it('keeps deployment artifacts identical to source artifacts', () => {
    expect(read('prismflow-deploy/public/test_prep/eppp_legacy/adjudication_batch_07.json')).toEqual(read('test_prep/eppp_legacy/adjudication_batch_07.json'));
    expect(fs.readFileSync(resolve(root, 'prismflow-deploy/public/test_prep/eppp_legacy/adjudication_batch_07.md'), 'utf8')).toBe(fs.readFileSync(resolve(root, 'test_prep/eppp_legacy/adjudication_batch_07.md'), 'utf8'));
  });
});
