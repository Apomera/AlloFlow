...
(function (global) {
    ...
    async function initAlloData(config = {}) {
        ...
        const provider = {
            ...
            // Auth helpers
            login:  async (loginId) => {
                const data = await _apiFetch('POST', '/auth/login', { loginId });
                if (data.token) {
                    _token = data.token;
            ...
            createAccount: async (name, loginId, role) => {
                const data = await _apiFetch('POST', '/auth/create-account', { name, loginId, role });
                if (data.token) {
                    _token = data.token;
                    localStorage.setItem('alloflow_local_token', data.token);
                    localStorage.setItem('alloflow_local_user', JSON.stringify(data.user));
                    if (global.__alloShared?.setUser)        global.__alloShared.setUser(data.user);
                    if (global.__alloShared?.setTeacherMode) global.__alloShared.setTeacherMode(data.user.isTeacher || false);
                }
                return data;
            },
            listAccounts: () => _apiFetch('GET', '/auth/accounts'),
    ...
    global.__alloAuth = {
        login:         (loginId)           => global.__alloLocalDB?.login(loginId),
        logout:        ()                  => global.__alloLocalDB?.logout(),
        createAccount: (name, loginId, role) => global.__alloLocalDB?.createAccount(name, loginId, role),
        listAccounts:  ()                  => global.__alloLocalDB?.listAccounts(),
        getToken:      _getToken,
        checkSession:  () => {
            const tok = localStorage.getItem('alloflow_local_token');
            if (!tok) return Promise.resolve(null);
            return fetch(`${_baseUrl}/auth/session`, {
                headers: { Authorization: `Bearer ${tok}` },
                signal: AbortSignal.timeout(3000),
