...
const wave = require('../dev-tools/eppp_native_quality_wave_44_data.cjs');
...
describe('EPPP expanded quality wave 44', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_44.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 22, applicationItems: 12, analysisItems: 12, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    ...
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicatePairs).toBe(50);
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicateClusters).toBe(32);
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(419);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1164);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(578);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(534);
    expect(audit.summary.feedbackWarningsAfter.choiceRestatementOptions).toBe(132);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_44.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_44.json'));
  });

  it('replays after wave 43', () => {
