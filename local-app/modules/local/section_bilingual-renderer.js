...
Return ONLY valid JSON (no markdown): {"term": "suggested term", "reason": "why it should be included (max 15 words)"}`;
  ...
  // Lazily load pdf.js for PDF→image conversion. Prefers the same-origin bundled copy
  // (CSP-safe worker + offline-capable); falls back to the CDN build if missing.
  const _PDFJS_LOCAL = './shared/libs/pdf.min.js';
  const _PDFJS_LOCAL_WORKER = './shared/libs/pdf.worker.min.js';
  const _PDFJS_CDN = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js';
  const _PDFJS_CDN_WORKER = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';
  const _ensurePdfJsForVision = async () => {
    if (window.pdfjsLib) {
      if (window.pdfjsLib.GlobalWorkerOptions) {
        const _ws = window.pdfjsLib.GlobalWorkerOptions.workerSrc || '';
        // Cross-origin worker URLs force pdf.js into a blob: wrapper that CSP blocks — use the bundled one.
        if (!_ws || /^https?:\/\//i.test(_ws)) {
          window.pdfjsLib.GlobalWorkerOptions.workerSrc = _PDFJS_LOCAL_WORKER;
        }
      }
      return;
    }
    const _loadScript = (src) => new Promise((resolve, reject) => {
      ...
      s.src = src;
      s.setAttribute('data-allo-pdfjs', '1');
      s.onload = () => resolve();
      s.onerror = () => reject(new Error('pdf.js failed to load: ' + src));
      document.head.appendChild(s);
    });
    const existing = document.querySelector('script[data-allo-pdfjs]');
    if (existing) {
      await new Promise((resolve, reject) => {
        const wait = setInterval(() => { if (window.pdfjsLib) { clearInterval(wait); resolve(); } }, 100);
        setTimeout(() => { clearInterval(wait); window.pdfjsLib ? resolve() : reject(new Error('pdf.js load timeout')); }, 15000);
      });
    } else {
      try {
        await _loadScript(_PDFJS_LOCAL);
        if (window.pdfjsLib && window.pdfjsLib.GlobalWorkerOptions) {
          window.pdfjsLib.GlobalWorkerOptions.workerSrc = _PDFJS_LOCAL_WORKER;
        }
      } catch (_) {
        await _loadScript(_PDFJS_CDN);
        if (window.pdfjsLib && window.pdfjsLib.GlobalWorkerOptions) {
          window.pdfjsLib.GlobalWorkerOptions.workerSrc = _PDFJS_CDN_WORKER;
        }
      }
    }
  ...
  // Render PDF pages to JPEG data-URIs for vision models that only accept images (NVIDIA NIM).
  // Caps pages and resolution to keep the request payload within API limits.
  const _pdfBase64ToImageDataUris = async (pdfBase64, maxPages = 8, maxWidth = 1280) => {
    await _ensurePdfJsForVision();
    ...
    for (let p = 1; p <= count; p++) {
      ...
      const scale = Math.min(1.5, maxWidth / base.width); // cap width ≈maxWidth px
      const viewport = page.getViewport({ scale });
      const canvas = document.createElement('canvas');
      canvas.width = Math.ceil(viewport.width);
      canvas.height = Math.ceil(viewport.height);
      // intent:'print' renders via setTimeout instead of requestAnimationFrame —
      // avoids an indefinite hang when the tab is backgrounded (rAF throttling).
      await page.render({ canvasContext: canvas.getContext('2d'), viewport, intent: 'print' }).promise;
      uris.push(canvas.toDataURL('image/jpeg', 0.8));
    }
    try { pdf.destroy(); } catch (_) {}
    return { uris, total, rendered: count };
  };
  ...
  const callGeminiVision = async (prompt, base64Data, mimeType) => {
    ...
    if (_aiProv === 'nvidia') {
      ...
      if (_mime === 'application/pdf') {
        // NIM omni models reject application/pdf data-URIs ("cannot identify image file").
        // Render the PDF pages to JPEG images client-side and send those instead.
        const { uris, total, rendered } = await _pdfBase64ToImageDataUris(base64Data);
        if (!uris.length) throw new Error('PDF→image conversion produced no pages');
        const _pageNote = total > rendered
          ? `\n\n[Note: this PDF has ${total} pages; only the first ${rendered} are shown as images. Page ${rendered + 1}+ not included.]`
          : '';
        _content = [
          { type: 'text', text: prompt + _pageNote },
          ...uris.map(u => ({ type: 'image_url', image_url: { url: u } })),
        ];
        console.log(`[callGeminiVision] NVIDIA: converted PDF (${total}p) → ${rendered} JPEG image(s)`);
      } else {
      ...
      const _msg = _nvData.choices?.[0]?.message || {};
      // Reasoning models may leave content empty and put the answer in reasoning_content.
      const _text = _msg.content || _msg.reasoning_content || _msg.reasoning;
      if (!_text) throw new Error('No content returned from NVIDIA omni model');
      return _text;
    }

    // Local OpenAI-compatible path (LM Studio, LocalAI): vision models such as
    // Gemma 3, Qwen-VL, and LLaVA accept OpenAI image_url content. PDFs are
    // rendered to JPEGs client-side (same as the NVIDIA path) since local
    // engines can't ingest application/pdf data URIs.
    const _isLocalProv = !_aiProv || _aiProv === 'lmstudio' || _aiProv === 'localai' || _aiProv === 'llm-engine';
    if (_isLocalProv) {
      const _mime = mimeType || 'image/jpeg';
      if (_mime.startsWith('video/') || _mime.startsWith('audio/')) {
        throw new Error(`Audio/video analysis requires a cloud AI provider (Gemini, Copilot, or NVIDIA). Current provider: ${_aiProv || 'local'}`);
      }
      let _content;
      if (_mime === 'application/pdf') {
        // Smaller renders for local engines: vision tokens count against the
        // loaded context window (often just 4-8K in LM Studio), and Gemma-class
        // encoders downscale to ~896px tiles anyway — larger inputs only burn
        // context on extra tiles without improving fidelity.
        const { uris, total, rendered } = await _pdfBase64ToImageDataUris(base64Data, 8, 896);
        if (!uris.length) throw new Error('PDF→image conversion produced no pages');
        const _pageNote = total > rendered
          ? `\n\n[Note: this PDF has ${total} pages; only the first ${rendered} are shown as images. Page ${rendered + 1}+ not included.]`
          : '';
        ...
        console.log(`[callGeminiVision] LM Studio: converted PDF (${total}p) → ${rendered} JPEG image(s)`);
      } else {
        _content = [
          { type: 'text', text: prompt },
          { type: 'image_url', image_url: { url: `data:${_mime};base64,${base64Data}` } },
        ];
      }
      const _lmBase = (window.__alloLocalConfig?.llmEngineUrl || 'http://localhost:1234').replace(/\/$/, '');
      let _lmModel = window.__alloLocalConfig?.defaultModel || 'default';
      // Newer LM Studio versions reject the placeholder 'default' with 400
      // model_not_found — resolve to a real loaded model via /v1/models.
      if (_lmModel === 'default') {
        if (window.__lmResolvedVisionModel) {
          _lmModel = window.__lmResolvedVisionModel;
        } else {
          try {
            const _mr = await fetch(`${_lmBase}/v1/models`);
            if (_mr.ok) {
              const _md = await _mr.json();
              let _cands = (_md.data || []).filter(x => x.id)
                .filter(x => !/embed|embedding|reranker|whisper/i.test(x.id) && x.type !== 'embeddings');
              // Prefer models already loaded in memory (LM Studio metadata via
              // the proxy) — JIT-loading an unloaded model is slow and can OOM.
              const _loaded = _cands.filter(x => x.state === 'loaded');
              if (_loaded.length) _cands = _loaded;
              // Prefer explicit VLMs, then known vision-capable families
              const _vlms = _cands.filter(x => x.type === 'vlm');
              if (_vlms.length) _cands = _vlms;
              const _ids = _cands.map(x => x.id);
              const _pick = _ids.find(id => /gemma-3|gemma-4|qwen.?vl|llava|vision|pixtral|llama-3\.2/i.test(id)) || _ids[0];
              if (_pick) {
                _lmModel = _pick;
                window.__lmResolvedVisionModel = _pick;
                console.log(`[callGeminiVision] LM Studio: resolved model 'default' → ${_pick}`);
              }
            }
          } catch (_) { /* let the chat call surface the error */ }
        }
      }
      // Local models often run with a small loaded context (e.g. 8192 in LM
      // Studio by default). max_tokens counts against that window alongside
      // the prompt + image tokens, so keep the budget modest and step down
      // on "context size" errors instead of failing the auditor outright.
      const _lmCall = async (_maxTok) => fetch(`${_lmBase}/v1/chat/completions`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          model: _lmModel,
      ...
      // LM Studio divides the model's loaded context across concurrent
      // requests (parallel processing slots), so N simultaneous auditors each
      // get only ~1/N of the context window and fail with "Context size has
      // been exceeded". Serialize local vision calls through a shared queue so
      // every request gets the full context. (Local inference is effectively
      // serial anyway, so this costs no wall-clock time.)
      const _prevTurn = window.__lmVisionQueue || Promise.resolve();
      let _releaseTurn;
      window.__lmVisionQueue = new Promise(r => { _releaseTurn = r; });
      await _prevTurn.catch(() => {});
      try {
        let _lmRes = await _lmCall(2048);
        if (!_lmRes.ok && _lmRes.status === 400) {
          const _eTxt = await _lmRes.text().catch(() => '');
          if (/context/i.test(_eTxt)) {
            console.warn('[callGeminiVision] LM Studio: context exceeded at max_tokens=2048 — retrying with 1024');
            _lmRes = await _lmCall(1024);
          } else {
            throw new Error(`Local vision error 400 — is a vision-capable model loaded in LM Studio at ${_lmBase}? ${_eTxt.slice(0, 200)}`);
          }
        }
        if (!_lmRes.ok) {
          const _e = await _lmRes.text().catch(() => '');
          throw new Error(`Local vision error ${_lmRes.status} — is a vision-capable model loaded in LM Studio at ${_lmBase}? ${_e.slice(0, 200)}`);
        }
        const _lmData = await _lmRes.json();
        const _lmText = _lmData.choices?.[0]?.message?.content;
        if (!_lmText) throw new Error('No content returned from local vision model');
        return _lmText;
      } finally {
        _releaseTurn();
      }
    ...
    // Other non-cloud providers can't do multimodal input.
