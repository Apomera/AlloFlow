...
(function () {
  ...
  function issHullCanvas(kind) {
    ...
    if (kind === 'mli') {
      ...
      g.strokeStyle = 'rgba(88,76,56,.24)'; g.lineWidth = 1.5; g.setLineDash([4, 3]);
      ...
      var pc = 8, pr = 2, pw2 = W / pc, ph2 = H / pr, k;
      for (r = 0; r < pr; r++) for (c = 0; c < pc; c++) {
        var tone = 224 + Math.floor(rnd() * 30);
      ...
      g.strokeStyle = 'rgba(74,86,102,.30)'; g.lineWidth = 1.6;
      for (c = 0; c <= pc; c++) { g.beginPath(); g.moveTo(c * pw2, 0); g.lineTo(c * pw2, H); g.stroke(); }
      // Ring seams repeat along the module, so they carry far less ink than the
      // eight seams that only wrap the hull once.
      g.strokeStyle = 'rgba(92,104,120,.18)'; g.lineWidth = 1.1;
      for (r = 0; r <= pr; r++) { g.beginPath(); g.moveTo(0, r * ph2); g.lineTo(W, r * ph2); g.stroke(); }
      g.fillStyle = 'rgba(86,96,112,.32)';
      for (c = 0; c < pc; c++) for (k = 0; k < 6; k++) {
        g.beginPath(); g.arc(c * pw2 + 4, k * (H / 6) + 10, 1.15, 0, Math.PI * 2); g.fill();
        g.beginPath(); g.arc(c * pw2 + pw2 - 4, k * (H / 6) + 10, 1.15, 0, Math.PI * 2); g.fill();
      }
      // Two pieces of bolted-on hardware per tile. Their only job is to break
      // the repeat: without something irregular the grid reads as a net however
      // faint the seams are.
      g.fillStyle = 'rgba(96,106,120,.34)';
      g.fillRect(pw2 * 1.18, ph2 * 0.28, pw2 * 0.64, ph2 * 0.34);
      g.fillRect(pw2 * 5.22, ph2 * 1.34, pw2 * 0.5, ph2 * 0.3);
      g.fillStyle = 'rgba(255,255,255,.5)';
      g.fillRect(pw2 * 1.18, ph2 * 0.28, pw2 * 0.64, 2);
      g.fillRect(pw2 * 5.22, ph2 * 1.34, pw2 * 0.5, 2);
      for (c = 0; c < pc; c += 2) {
    ...
    render: function (ctx) {
      ...
      function stationCanvasRef(cv) {
        ...
        function doInit(THREE) {
          ...
          camera.lookAt(0, 0, 1);
          var earthShine = new THREE.HemisphereLight(0x9bdcff, 0x081225, 0.65);
          scene.add(earthShine);
          var fillAmbient = new THREE.AmbientLight(0x7288aa, 0.32);
          ...
          // Only when a real surface map is in play: without one the base colour
          // carries the planet's hue and scaling it would shift the colour.
          var earthExposureMat = earthSkin ? earth.material : null;
          earth.position.set(0, -34.5, 0); earth.receiveShadow = false;
            ...
            limbShells.push({
              ...
              update: function (sunDir) {
                for (var i = 0; i < pos.count; i++) {
                  ...
                  var term = Math.exp(-(d / 0.09) * (d / 0.09));
                  var i3 = i * 3;
          ...
          scene.add(atmo);
          var airglow = makeLimbShell(30.3, 0x8fdcff, 0xffb056, 0.32);
          scene.add(airglow);
          ...
          function onKey(e) {
            ...
            else if (e.key === 'Home') { cv._issSetView('overview'); upd({ mapView: 'overview' }); announceToSR(__alloT('stem.spacestation.sr_station_camera_returned_to_overview', 'Station camera returned to overview.')); }
            else handled = false;
          ...
          function animate() {
            ...
            // Fill follows the Sun. The floors are earthshine, not darkness:
            // enough to keep the silhouette and the handrails readable through
            // the 35-minute eclipse, far too little to pass for daylight. Under
            // reduced motion the sweep never runs and daylight01 pins to 1, so
            // the frozen still image is exactly what it was.
            earthShine.intensity = 0.19 + 0.46 * daylight01;
            fillAmbient.intensity = 0.11 + 0.21 * daylight01;
            rim.intensity = 0.26 + 0.54 * daylight01;
            ...
            for (var wi2 = 0; wi2 < wings.length; wi2++) {
              ...
              var glint = daylight01 * Math.pow(incidence, 6);
              var wingMats = wingPanelMats[wi2] || [];
              for (var pi = 0; pi < wingMats.length; pi++) {
                wingMats[pi].emissive.setRGB(0.16 + 0.62 * glint, 0.11 + 0.42 * glint, 0.02 + 0.12 * glint);
              }
            }
            ...
            if (earthExposureMat) earthExposureMat.color.setScalar(1 + 0.46 * lampOn);
      ...
      function interiorCanvasRef(cv) {
        ...
        function doInit(THREE) {
          ...
          // Every in-world text plane, registered for the distance/aim fade below.
          // Declared here because addModulePlacard writes to it and runs long
          // before addWayfindingPlacard is reached.
          var wayfindingSigns = [];
          ...
          function addModulePlacard(def, index) {
            ...
            var placard = new THREE.Mesh(
              ...
              new THREE.MeshBasicMaterial({ map: labelTexture, transparent: true, side: THREE.FrontSide })
            );
          ...
          function addWayfindingPlacard(label, detail, position, lookTarget, color, upVector, signSize) {
            ...
            var sign = new THREE.Mesh(
              ...
              new THREE.MeshBasicMaterial({ map: signTexture, transparent: true, side: THREE.FrontSide })
            );
            sign.position.copy(position);
            if (upVector) sign.up.copy(upVector);
            sign.lookAt(lookTarget);
            scene.add(sign);
            wayfindingSigns.push(sign);
          }
          // Near-fade. Returns true when something actually moved, so the
          // render-on-dirty loop is not woken for a change nobody can see.
          var _signCamPos = new THREE.Vector3();
          var _signCamFwd = new THREE.Vector3();
          var _signToSign = new THREE.Vector3();
          var _signWorld = new THREE.Vector3();
          function updateSignLegibility() {
            if (!wayfindingSigns.length) return false;
            camera.getWorldPosition(_signCamPos);
            camera.getWorldDirection(_signCamFwd);
            var changed = false;
            for (var si = 0; si < wayfindingSigns.length; si++) {
              var sgn = wayfindingSigns[si];
              sgn.getWorldPosition(_signWorld);
              _signToSign.subVectors(_signWorld, _signCamPos);
              var dist = _signToSign.length();
              var near = Math.max(0, Math.min(1, (dist - 0.62) / 0.78));
              // cos 26 deg = .899, cos 44 deg = .719.
              var aim = dist < 0.0001 ? 0 : Math.max(0, Math.min(1, (_signToSign.dot(_signCamFwd) / dist - 0.719) / 0.18));
              var want = Math.min(near, aim);
              if (Math.abs(sgn.material.opacity - want) > 0.01) {
                sgn.material.opacity = want;
                changed = true;
              }
              // Shared materials mean the opacity write above may be a no-op
              // for the second plane of a pair, so visibility is set outside it.
              var vis = want > 0.02;
              if (sgn.visible !== vis) { sgn.visible = vis; changed = true; }
            }
            return changed;
          }
          ...
          addWayfindingPlacard('BULKY TRANSFER BAG', 'CLIP TETHER [B] // CENTER BOTH', new THREE.Vector3(-1.0, 0.8, -9.58), transferStagingPoint, 0xfbbf24, null, [1.18, 0.32]);
          ...
          var airflowStatusLabel = new THREE.Mesh(new THREE.PlaneGeometry(1.15, 0.29), new THREE.MeshBasicMaterial({ map: airflowStatusTexture, transparent: true, side: THREE.FrontSide }));
          airflowStatusLabel.rotation.y = Math.PI / 2;
          airflowStatusLabel.position.set(0.08, -0.7, 0);
          harmonyAirReturn.add(airflowStatusLabel);
          wayfindingSigns.push(airflowStatusLabel);
          ...
          function setCapillaryAction(on) {
            ...
            if (state.transferMode === 'tethered') {
              ...
              announce(__alloT('stem.spacestation.sr_finish_the_bulky_hatch_transfer_before_opening_th', 'Finish the bulky hatch transfer before opening the Destiny water injector.'));
              return false;
            }
            ...
            if (Number(cv._issInteriorResearchStep || 0) < 1) {
              ...
              announce(__alloT('stem.spacestation.sr_secure_the_destiny_plant_sample_before_opening_th', 'Secure the Destiny plant sample before opening the contained water valve.'));
              return false;
            }
            updateCapillaryMetrics();
            if (state.capillaryComplete) {
              setFeedback('PLANT SAMPLE FED // WICK SATURATED', 'safe');
              announce(__alloT('stem.spacestation.sr_the_destiny_plant_sample_has_already_received_its', 'The Destiny plant sample has already received its target capillary dose.'));
              return false;
            }
            if (state.room !== 'destiny') {
              setFeedback('PLANT CHAMBER IN DESTINY // FOLLOW FORWARD ROUTE', 'warn', 1900);
              announce(__alloT('stem.spacestation.sr_the_contained_plant_watering_experiment_is_in_des', 'The contained plant-watering experiment is in Destiny laboratory.'));
              return false;
            }
            if (state.cargoMode === 'held') {
              setFeedback('SECURE POUCH BEFORE SCIENCE', 'warn', 1800);
              announce(__alloT('stem.spacestation.sr_secure_the_cargo_pouch_before_handling_the_fluid', 'Secure the cargo pouch before handling the fluid experiment.'));
              return false;
            }
            ...
            if (state.capillaryAlignment < capillaryAlignmentMinimum) {
              ...
              announce(__alloT('stem.spacestation.sr_face_the_cyan_plant_chamber_ring_more_directly_be', 'Face the cyan plant-chamber ring more directly before opening the water valve.'));
              return false;
            }
            if (!state.capillaryStable) {
              setFeedback('STABILIZE BODY // STOP OR HOLD A RAIL', 'warn', 1900);
              announce(__alloT('stem.spacestation.sr_stop_your_linear_and_rotational_motion_or_hold_a', 'Stop your linear and rotational motion, or hold a handrail, before transferring contained fluid.'));
              return false;
            }
            ...
            announce(__alloT('stem.spacestation.sr_contained_water_valve_open_hold_v_then_release_be', 'Contained water valve open. Hold V, then release between two point seven and three point three milliliters.'));
            dirty = true;
            return true;
          }
          ...
          function updateCapillary(dt, now) {
            ...
            if (state.capillaryActive) {
              ...
              if (!stable) {
                ...
                announce(__alloT('stem.spacestation.sr_fluid_transfer_interrupted_release_v_stabilize_ne', 'Fluid transfer interrupted. Release V, stabilize near the glovebox, and realign before trying again.'));
              } else {
                ...
                if (state.capillaryDose >= capillaryDoseMaximum) {
                  ...
                  announce(__alloT('stem.spacestation.sr_the_valve_auto_closed_at_three_point_three_millil', 'The valve auto-closed at three point three milliliters. A water bead clings inside the sealed glovebox because surface tension dominates in microgravity.'));
                }
              }
            }
            updateCapillaryVisual(now);
          ...
          function setObservationAction(on) {
            if (!on) {
              ...
              if (blurred) {
                ...
                announce(__alloT('stem.spacestation.sr_the_handheld_earth_image_blurred_when_the_camera', 'The handheld Earth image blurred when the camera moved before frame lock. Brace on the rail and hold P for the full training interval.'));
              }
              dirty = true;
              return true;
            }
            if (state.transferMode === 'tethered') {
              setFeedback('BULKY BAG TETHERED // FINISH HATCH TRANSFER', 'warn', 1900);
              announce(__alloT('stem.spacestation.sr_finish_the_bulky_hatch_transfer_before_handling_t', 'Finish the bulky hatch transfer before handling the Cupola camera.'));
              return false;
            }
            ...
            if (state.room !== 'cupola') {
              ...
              announce(__alloT('stem.spacestation.sr_the_earth_observation_camera_station_is_in_cupola', 'The Earth observation camera station is in Cupola.'));
              return false;
            }
            if (state.observationSecured) {
              setFeedback('CUPOLA SECURE // SHUTTERS CLOSED', 'safe');
              announce(__alloT('stem.spacestation.sr_the_cupola_observation_is_complete_and_all_seven', 'The Cupola observation is complete and all seven shutters are closed.'));
              return false;
            }
            ...
            if (state.observationDistance > observationReach) {
              ...
              announce(__alloT('stem.spacestation.sr_move_within_zero_point_nine_five_meters_of_the_cu', 'Move within zero point nine five meters of the Cupola camera control.'));
              return false;
            }
            if (state.observationAlignment < observationAlignmentMinimum) {
              ...
              announce(__alloT('stem.spacestation.sr_center_the_selected_earth_target_inside_the_viole', 'Center the selected Earth target inside the violet Cupola bracket before starting the frame lock.'));
              return false;
            }
            if (!state.observationBraced) {
              setFeedback('BRACE ON RAIL // DAMP CAMERA MOTION', 'warn', 2000);
              announce(__alloT('stem.spacestation.sr_catch_and_hold_the_nearby_rail_then_stop_rotation', 'Catch and hold the nearby rail, then stop rotation before taking the handheld Earth image.'));
              return false;
            }
            ...
            announce(__alloT('stem.spacestation.sr_camera_frame_lock_started_stay_braced_and_hold_p', 'Camera frame lock started. Stay braced and hold P for one point two seconds.'));
            dirty = true;
            return true;
          }
          ...
          function updateObservation(dt, now) {
            ...
            if (state.observationActive) {
              ...
              if (!stable) {
                ...
                announce(__alloT('stem.spacestation.sr_earth_frame_lock_interrupted_by_lost_bracing_reac', 'Earth frame lock interrupted by lost bracing, reach, or alignment. Release P and set up the camera again.'));
              } else {
                ...
                if (state.observationProgress >= observationDuration) {
                  ...
                  announce(__alloT('stem.spacestation.sr_stable_earth_image_captured_release_p_then_press', 'Stable Earth image captured. Release P, then press it again to close all seven Cupola shutters.'));
                }
              }
            }
          ...
          function transferAction() {
            if (state.transferComplete) {
              ...
              announce(__alloT('stem.spacestation.sr_the_bulky_transfer_bag_is_already_docked_in_desti', 'The bulky transfer bag is already docked in Destiny.'));
              return false;
            }
            if (state.transferMode === 'tethered') {
              setFeedback('TETHER CLIPPED // CENTER BODY + BAG', 'info', 1800);
              announce(__alloT('stem.spacestation.sr_the_transfer_tether_is_clipped_keep_both_your_bod', 'The transfer tether is clipped. Keep both your body and the lagging bag centered through the hatch.'));
              return false;
            }
            if (state.cargoMode === 'held' || state.stowHeldId || state.toolHeld || state.capillaryActive || state.observationActive) {
              ...
              announce(__alloT('stem.spacestation.sr_secure_the_item_or_finish_the_current_tool_activi', 'Secure the item or finish the current tool activity before clipping the bulky transfer tether.'));
              return false;
            }
            if (state.room !== 'harmony') {
              setFeedback('TRANSFER BAG STAGED IN HARMONY', 'warn', 1900);
              announce(__alloT('stem.spacestation.sr_return_to_the_harmony_side_of_the_forward_hatch_t', 'Return to the Harmony side of the forward hatch to begin the bulky transfer.'));
              return false;
            }
            ...
            if (attachDistance > transferAttachReach) {
              ...
              announce(__alloT('stem.spacestation.sr_move_within_zero_point_seven_eight_meters_of_the', 'Move within zero point seven eight meters of the staged transfer bag before pressing B.'));
              return false;
            }
            ...
            announce(__alloT('stem.spacestation.sr_tether_clipped_the_twelve_kilogram_bag_follows_wi', 'Tether clipped. The twelve kilogram bag follows with spring and damping lag. Center your body, then allow clearance for the bag before crossing into Destiny.'));
            return true;
          }
          ...
          function updateHatchTransfer(dt) {
            ...
            announce(__alloT('stem.spacestation.sr_bulky_hatch_transfer_complete_both_your_body_and', 'Bulky hatch transfer complete. Both your body and the lagging bag made their own centered crossings before the bag docked in Destiny.'));
          }
          ...
          function setWorksiteAction(on) {
            if (!on) {
              ...
              if (abandonedProgress) {
                ...
                announce(__alloT('stem.spacestation.sr_torque_tool_released_before_the_filter_fastener_w', 'Torque tool released before the filter fastener was secure. Hold it for one point five seconds while braced.'));
              }
              dirty = true;
              return true;
            }
            if (state.transferMode === 'tethered') {
              setFeedback('BULKY BAG TETHERED // FINISH HATCH TRANSFER', 'warn', 1900);
              announce(__alloT('stem.spacestation.sr_finish_the_bulky_hatch_transfer_before_using_the', 'Finish the bulky hatch transfer before using the torque tool.'));
              return false;
            }
            ...
            if (state.worksiteComplete) {
              ...
              announce(__alloT('stem.spacestation.sr_the_tranquility_inlet_filter_is_already_serviced', 'The Tranquility inlet filter is already serviced.'));
              return false;
            }
            if (state.room !== 'tranquility') {
              setFeedback('WORKSITE IN TRANQUILITY // FOLLOW PORT BRANCH', 'warn', 1900);
              announce(__alloT('stem.spacestation.sr_the_filter_maintenance_worksite_is_in_tranquility', 'The filter maintenance worksite is in Tranquility.'));
              return false;
            }
            ...
            if (state.worksiteAlignment < worksiteAlignmentMinimum) {
              ...
              announce(__alloT('stem.spacestation.sr_face_the_filter_worksite_more_directly_before_usi', 'Face the filter worksite more directly before using the torque tool.'));
              return false;
            }
            ...
            if (!state.worksiteBraced) {
              ...
              announce(__alloT('stem.spacestation.sr_the_unbraced_tool_turned_your_body_instead_of_the', 'The unbraced tool turned your body instead of the fastener. Catch a rail, stop the roll, release T, and try again.'));
              dirty = true;
              return false;
            }
            state.toolHeld = true;
            ...
            announce(__alloT('stem.spacestation.sr_torque_tool_engaged_while_braced_hold_t_steadily', 'Torque tool engaged while braced. Hold T steadily for one point five seconds.'));
            dirty = true;
            return true;
          }
          function updateWorksite(dt, now) {
            ...
            if (state.toolHeld) {
              ...
              if (!stable) {
                ...
                announce(__alloT('stem.spacestation.sr_stable_maintenance_interrupted_release_t_catch_th', 'Stable maintenance interrupted. Release T, catch the rail, realign with the filter, and try again.'));
              } else {
                ...
                if (state.worksiteProgress >= worksiteDuration) {
                  ...
                  announce(__alloT('stem.spacestation.sr_filter_serviced_while_braced_reaction_torque_main', 'Filter serviced while braced. Reaction-torque maintenance challenge complete.'));
                }
              }
            }
            updateWorksiteVisual(now);
          ...
          function cargoAction() {
            if (state.transferMode === 'tethered') {
              ...
              announce(__alloT('stem.spacestation.sr_finish_the_bulky_hatch_transfer_before_handling_t_2', 'Finish the bulky hatch transfer before handling the Unity cargo pouch.'));
              return false;
            }
            if (state.cargoMode === 'secured') {
              setFeedback('POUCH SECURED // RESTRAINT LOCKED', 'safe');
              announce(__alloT('stem.spacestation.sr_the_cargo_pouch_is_already_secured_at_the_unity_r', 'The cargo pouch is already secured at the Unity restraint point.'));
              return false;
            }
            if (state.stowHeldId) {
              setFeedback('ONE ITEM AT A TIME // SECURE CABIN ITEM', 'warn', 2100);
              announce(__alloT('stem.spacestation.sr_secure_the_carried_harmony_cabin_item_before_hand', 'Secure the carried Harmony cabin item before handling the Unity cargo pouch.'));
              return false;
            }
            if (state.cargoMode === 'held') {
              ...
              if (state.room !== 'unity') {
                ...
                announce(__alloT('stem.spacestation.sr_carry_the_pouch_back_to_the_marked_restraint_poin', 'Carry the pouch back to the marked restraint point in Unity.'));
                return false;
              }
              ...
              if (!braced) {
                ...
                announce(__alloT('stem.spacestation.sr_brace_on_the_nearby_handrail_or_stop_within_rail', 'Brace on the nearby handrail, or stop within rail reach, before securing the pouch.'));
                return false;
              }
              ...
              announce(__alloT('stem.spacestation.sr_cargo_pouch_secured_at_the_unity_restraint_loose', 'Cargo pouch secured at the Unity restraint. Loose-object management challenge complete.'));
              return true;
            }
          ...
          function stowAction() {
            if (state.transferMode === 'tethered') {
              setFeedback('BULKY BAG TETHERED // HANDS COMMITTED', 'warn', 1900);
              announce(__alloT('stem.spacestation.sr_finish_the_bulky_hatch_transfer_before_catching_a', 'Finish the bulky hatch transfer before catching a loose cabin item.'));
              return false;
            }
            if (state.stowComplete) {
              setFeedback('HARMONY STOW COMPLETE // AIRFLOW CLEAR', 'safe');
              announce(__alloT('stem.spacestation.sr_all_three_harmony_cabin_items_are_already_secured', 'All three Harmony cabin items are already secured and the air return is clear.'));
              return false;
            }
            if (state.cargoMode === 'held') {
              setFeedback('ONE ITEM AT A TIME // SECURE CARGO POUCH', 'warn', 2100);
              announce(__alloT('stem.spacestation.sr_secure_the_unity_cargo_pouch_before_catching_a_ha', 'Secure the Unity cargo pouch before catching a Harmony cabin item.'));
              return false;
            }
            ...
            if (state.room !== 'harmony') {
              ...
              announce(__alloT('stem.spacestation.sr_the_loose_item_cabin_stow_activity_is_in_harmony', 'The loose-item cabin stow activity is in Harmony.'));
              return false;
            }
            var nearest = null;
            ...
            if (!nearest || nearestDistance > stowCatchReach) {
              ...
              announce(__alloT('stem.spacestation.sr_move_within_zero_point_six_five_meters_of_the_nea', 'Move within zero point six five meters of the nearest loose cabin item before pressing X.'));
              return false;
            }
          ...
          function grabRail() {
            if (state.railHeld) {
              ...
              announce(__alloT('stem.spacestation.sr_handrail_released_use_one_push_direction_to_launc', 'Handrail released. Use one push direction to launch away from the rail.'));
              dirty = true;
              return true;
            }
            var speed = state.velocity.length();
          ...
          function updatePhysics(dt, now) {
            ...
            if (state.railHeld) {
              ...
              if (state.rolledFar && !state.orientationDone && Math.abs(state.roll) < 0.14 && Math.abs(state.angularVelocity) < 0.12 && rollInput) {
                ...
                announce(__alloT('stem.spacestation.sr_orientation_recovered_your_agreed_deck_direction', 'Orientation recovered. Your agreed deck direction is level and the roll rate is nearly stopped.'));
              }
            }
            ...
            if (state.cargoMode === 'loose' && state.cargoRoom === state.room) {
              ...
              if (cargoSeparation < 0.53 && !state.cargoContactLatched) {
                ...
                if (now - state.lastCargoEvent > 700) {
                  ...
                  announce(__alloT('stem.spacestation.sr_loose_cargo_contact_momentum_transferred_between', 'Loose cargo contact. Momentum transferred between you and the five kilogram pouch.'));
                }
                dirty = true;
              }
            }
            ...
            if (updateSignLegibility()) dirty = true;
          ...
          function onWebGLContextLost(event) {
            ...
            if (disposed) return;
            announce(__alloT('stem.spacestation.sr_the_3_d_graphics_context_was_lost_the_accessible', 'The 3-D graphics context was lost. The accessible interior diagram remains available.'));
            cleanup();
        ...
        return h('div', { className: 'iss-learning-visual iss-eva-visual' },
          h('svg', { viewBox: '0 0 640 190', role: 'img', 'aria-label': __alloFill(__alloT('stem.spacestation.a11y_spacewalk_route_astronaut_at_tether_a_at_tether', 'Spacewalk route. Astronaut at {value1}. Tether A at {value2}. Tether B at {value3}. Suit consumables {value4} percent. Projected at worksite {value5} percent. {value6}. {value7}.'), { value1: EVA_RAILS[position], value2: EVA_RAILS[evaS.tetherA || 0], value3: EVA_RAILS[evaS.tetherB || 0], value4: oxygen, value5: projectedOxygen, value6: reserveStatus, value7: tetherStatus })},
            h('defs', null, h('linearGradient', { id: 'iss-eva-bg', x1: '0', y1: '0', x2: '0', y2: '1' }, h('stop', { offset: '0%', stopColor: '#020611' }), h('stop', { offset: '100%', stopColor: '#09213a' })),
              ...
              h('filter', { id: 'iss-eva-cloud', x: '-40%', y: '-90%', width: '180%', height: '280%' }, h('feGaussianBlur', { stdDeviation: 3.2 }))),
            ...
            h('g', { opacity: .26, filter: 'url(#iss-eva-cloud)' }, h('ellipse', { cx: 268, cy: 172, rx: 34, ry: 7, fill: '#f8fafc' }), h('ellipse', { cx: 356, cy: 181, rx: 44, ry: 8, fill: '#f8fafc' }), h('ellipse', { cx: 312, cy: 165, rx: 24, ry: 5, fill: '#f8fafc' })),
            ...
            h('rect', { x: 28, y: 86, width: 55, height: 48, rx: 20, fill: '#8ea0b5', stroke: '#e2e8f0', strokeWidth: 2 }),
            ...
            h('text', { x: 587, y: 95, textAnchor: 'middle', fill: '#f8fafc', fontSize: 8, fontWeight: 850 }, 'PUMP'),
            h('text', { x: 587, y: 129, textAnchor: 'middle', fill: '#fbbf24', fontSize: 9, fontWeight: 900 }, (evaS.bolts || 0) + '/4'),
            ...
            routeX.map(function (x, i) { var reached = i <= position; return h('g', { key: labels[i] }, h('circle', { cx: x, cy: routeY[i], r: 7, fill: reached ? '#0ea5e9' : '#172033', stroke: reached ? '#7dd3fc' : '#64748b', strokeWidth: 2 }), h('text', { x: x, y: 163, textAnchor: 'middle', fill: i === position ? '#7dd3fc' : '#cbd5e1', fontSize: 8, fontWeight: i === position ? 900 : 700, style: { paintOrder: 'stroke', stroke: '#020611', strokeWidth: 2.2, strokeOpacity: .85 } }, labels[i])); }),
            h('g', { transform: 'translate(' + astroX + ',' + astroY + ')' }, h('g', { className: 'iss-eva-astronaut' }, h('circle', { cx: 0, cy: -6, r: 9, fill: '#f8fafc', stroke: '#7dd3fc', strokeWidth: 2 }), h('ellipse', { cx: 0, cy: -6.4, rx: 6.2, ry: 5.2, fill: '#c98f1c', stroke: '#f6d67a', strokeWidth: .8, opacity: .93 }), h('ellipse', { cx: -2.1, cy: -8.2, rx: 2.2, ry: 1.3, fill: '#fff7e0', opacity: .55 }), h('rect', { x: -10, y: 4, width: 20, height: 24, rx: 8, fill: '#f1f5f9', stroke: '#334155', strokeWidth: 1.3 }), h('rect', { x: -15, y: 7, width: 6, height: 17, rx: 3, fill: '#b3c0d0', stroke: '#334155', strokeWidth: .8 }), h('line', { x1: -9, y1: 12, x2: -20, y2: 23, stroke: '#e2e8f0', strokeWidth: 4, strokeLinecap: 'round' }), h('line', { x1: 9, y1: 12, x2: 20, y2: 21, stroke: '#e2e8f0', strokeWidth: 4, strokeLinecap: 'round' }))),
            ...
            h('rect', { x: 465, y: 24, width: 92, height: 5, rx: 3, fill: '#263449' }),
            h('rect', { x: 465, y: 24, width: 0.92 * oxygen, height: 5, rx: 3, fill: oxygen > 40 ? '#4ade80' : oxygen > 15 ? '#fbbf24' : '#f87171' }),
            h('rect', { x: 465, y: 35, width: 92, height: 5, rx: 3, fill: '#263449' }),
            h('rect', { x: 465, y: 35, width: 0.92 * projectedOxygen, height: 5, rx: 3, fill: projectedMargin >= 0 ? '#38bdf8' : '#f87171', opacity: .82 }),
            h('line', { x1: 465 + 0.92 * reserveFloor, y1: 21, x2: 465 + 0.92 * reserveFloor, y2: 43, stroke: '#f8fafc', strokeWidth: 1, strokeDasharray: '2 2', opacity: .9 }),
      ...
      function renderCupolaInterior() {
        ...
        var PANES = [[12, 62, 70, 56], [88, 30, 70, 44], [202, 30, 70, 44], [278, 62, 70, 56], [88, 118, 70, 44], [202, 118, 70, 44]];
        function windowPane(x, y, w, hgt, key) {
          return h('g', { key: key },
            h('rect', { x: x, y: y, width: w, height: hgt, rx: 6, fill: shut ? '#39424f' : 'none', stroke: '#94a3b8', strokeWidth: 2 }),
            ...
            shut ? h('line', { x1: x + 4, y1: y + hgt / 2, x2: x + w - 4, y2: y + hgt / 2, stroke: '#556072', strokeWidth: 3 }) : null
          );
        }
        // One view, seen through seven windows. Everything here is drawn across
        // the whole dome and clipped to the panes, so a cloud band or a city
        // web crosses a window frame the way it does from inside the real one.
        function cupolaScenery() {
          if (scene === 'night') return h('g', null,
            h('path', { d: 'M 20 122 L 70 98 L 116 106 L 154 88 L 198 98 L 240 82 L 288 100 L 338 88 M 116 106 L 104 144 L 158 154 M 198 98 L 208 134 L 256 146 M 240 82 L 250 52 L 298 44', stroke: '#fbbf24', strokeWidth: 0.9, fill: 'none', opacity: 0.42 }),
            [[20, 122], [70, 98], [116, 106], [154, 88], [198, 98], [240, 82], [288, 100], [338, 88], [104, 144], [158, 154], [208, 134], [256, 146], [250, 52], [298, 44], [58, 50], [174, 40]].map(function (p, i) {
              return h('circle', { key: i, cx: p[0], cy: p[1], r: i % 3 === 0 ? 2.2 : 1.4, fill: '#fde68a', opacity: 0.95, filter: 'url(#iss-cup-glow)' });
            }),
            h('circle', { cx: 300, cy: 130, r: 8, fill: '#e0f2fe', opacity: 0.5, filter: 'url(#iss-cup-glow)' }));
          if (scene === 'aurora') return h('g', null, [16, 68, 120, 172, 224, 276, 328].map(function (x, i) {
            return h('path', { key: i, className: 'iss-aurora-curtain', d: 'M ' + x + ' 178 Q ' + (x + 15) + ' 106 ' + (x + 5) + ' 30', fill: 'none', stroke: i % 2 ? '#86efac' : '#4ade80', strokeWidth: 7 - (i % 3) * 1.6, opacity: 0.32 + (i % 3) * 0.1, strokeLinecap: 'round', filter: 'url(#iss-cup-glow)' });
          }));
          return h('g', null,
            h('path', { d: 'M 16 128 Q 64 100 112 120 Q 150 136 190 118 Q 238 98 284 116 Q 322 130 350 112', fill: 'none', stroke: '#f2fbff', strokeWidth: 9, opacity: 0.36, strokeLinecap: 'round' }),
            h('path', { d: 'M 30 68 Q 86 48 140 64 Q 190 80 240 60 Q 298 40 348 58', fill: 'none', stroke: '#ffffff', strokeWidth: 6, opacity: 0.28, strokeLinecap: 'round' }),
            // A cyclone: the single thing crews photograph most from this dome.
            h('g', { transform: 'translate(236,140)' },
              h('path', { d: 'M 0 -15 Q 15 -11 13 2 Q 11 13 -2 13 Q -11 12 -11 4', fill: 'none', stroke: '#ffffff', strokeWidth: 4.5, opacity: 0.5, strokeLinecap: 'round' }),
              h('path', { d: 'M 0 15 Q -15 11 -13 -2 Q -11 -13 2 -13 Q 11 -12 11 -4', fill: 'none', stroke: '#ffffff', strokeWidth: 4.5, opacity: 0.5, strokeLinecap: 'round' }),
              h('circle', { r: 2.6, fill: '#0f3f6b', opacity: 0.5 })),
            h('ellipse', { cx: 180, cy: 112, rx: 26, ry: 8, fill: '#e8f4ff', opacity: 0.45 }));
        }
          ...
          h('div', null,
            h('svg', { viewBox: '0 0 360 180', role: 'img', 'aria-label': __alloT('stem.spacestation.cupola_aria', 'View from inside the Cupola dome: six angled windows around a large round center window, looking down at Earth. Scene: ') + sc.label, style: { width: '100%', maxWidth: 480, display: 'block', margin: '0 auto', background: '#101725', borderRadius: 12, border: '1px solid #334155' } },
              h('defs', null,
                ...
                h('clipPath', { id: 'iss-cup-panes' }, h('circle', { cx: 180, cy: 96, r: 34 }), PANES.map(function (p, i) { return h('rect', { key: i, x: p[0], y: p[1], width: p[2], height: p[3], rx: 6 }); })),
              ...
              !shut ? h('g', { clipPath: 'url(#iss-cup-panes)' },
                h('rect', { x: 0, y: 0, width: 360, height: 180, fill: 'url(#' + sc.grad + ')' }),
                cupolaScenery()) : null,
              PANES.map(function (p, i) { return windowPane(p[0], p[1], p[2], p[3], 'w' + i); }),
              h('circle', { cx: 180, cy: 96, r: 34, fill: shut ? '#39424f' : 'none', stroke: '#94a3b8', strokeWidth: 3 }),
              shut ? h('line', { x1: 152, y1: 96, x2: 208, y2: 96, stroke: '#556072', strokeWidth: 3 }) : null,
        ...
        return h('div', null,
          ...
          card('🚀 ' + __alloT('stem.spacestation.mission_dock', 'Mission 1 — Dock the cargo capsule'),
            h('div', null,
              ...
              (typeof d.dockMsg === 'string' && d.dockMsg) ? h('div', { role: 'status', 'aria-live': 'polite', style: { marginTop: 8, padding: 8, borderRadius: 8, background: 'rgba(2,6,23,0.4)', borderLeft: '3px solid ' + (d.dockResult === 'docked' ? '#22c55e' : '#fbbf24'), fontSize: 12, color: TEXT, lineHeight: 1.55 } },
          ...
          card('🧑‍🚀 ' + __alloT('stem.spacestation.mission_eva', 'Mission 2 — Spacewalk: replace the failed pump'),
            h('div', null,
              ...
              h('div', null,
                h('div', { role: 'list', 'aria-label': __alloT('stem.spacestation.a11y_handrail_route', 'Handrail route'), style: { display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 8 } },
      ...
      function renderInterior() {
        ...
        function recordInteriorNavigation(event) {
          ...
          if (event.type === 'hatch' && !navigation.preciseHatch && (event.controlled === true || Number(event.speed) <= 0.35)) announceToSR(__alloT('stem.spacestation.sr_controlled_hatch_challenge_complete', 'Controlled hatch challenge complete.'));
          else if (event.type === 'rail-grab' && (event.controlled === true || (event.controlled == null && Number(event.speed) >= 0.12 && Number(event.speed) <= 0.35))) announceToSR(__alloT('stem.spacestation.sr_controlled_handrail_stop_challenge_complete', 'Controlled handrail stop challenge complete.'));
          else if (event.type === 'cargo-caught') announceToSR(__alloT('stem.spacestation.sr_cargo_pouch_caught_carry_it_to_the_marked_unity_r', 'Cargo pouch caught. Carry it to the marked Unity restraint while braced.'));
          ...
          else if (event.type === 'route-complete') announceToSR(__alloT('stem.spacestation.sr_five_module_free_flight_route_complete', 'Five-module free-flight route complete.'));
        }
        ...
        function chooseInteriorView(mode, event) {
          if (mode === 'diagram') {
            ...
            announceToSR(__alloT('stem.spacestation.sr_accessible_interior_diagram_opened_at_the_saved_f', 'Accessible interior diagram opened at the saved flight room.'));
            return;
          }
          setLabToolData(function (prev) {
            var station = Object.assign({}, (prev && prev.spaceStation) || {});
          ...
          announceToSR(__alloT('stem.spacestation.sr_interactive_3_d_free_flight_view_restored_at_the', 'Interactive 3-D free-flight view restored at the saved flight room.'));
        }
        function renderInteriorSimulation() {
          ...
          function resetNavigation(event) {
            ...
            announceToSR(__alloT('stem.spacestation.sr_free_flight_route_restarted_in_harmony_crew_jobs', 'Free-flight route restarted in Harmony. Crew jobs were kept.'));
          }
          return h('div', { className: 'iss-interior-sim', 'data-iss-interior-sim': 'true' },
            ...
            h('div', { className: 'iss-interior-controls' },
              h('div', { className: 'iss-interior-thrusters', role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_virtual_navigation_push_controls', 'Virtual navigation push controls') },
              ...
              h('div', { className: 'iss-interior-safety', role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_free_flight_safety_assists', 'Free-flight safety assists') },
        ...
        function renderHatchTransferDiagram() {
          ...
          return h('section', {
            ...
            h('div', { role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_choose_a_bulky_hatch_transfer_method', 'Choose a bulky hatch-transfer method'), style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(145px,1fr))', gap: 6 } },
        ...
        function sceneArt() {
          var object = room.id === 'harmony' ?
            h('g', { className: 'iss-float' }, h('rect', { x: 310, y: 78, width: 105, height: 112, rx: 28, fill: 'url(#iss-sleepbag-body)', stroke: '#cbd5e1', strokeWidth: 3 }), h('path', { d: 'M362 116 V182', stroke: '#1e293b', strokeWidth: 3, strokeDasharray: '5 4', opacity: .8 }), [128, 160].map(function (sy) { return h('rect', { key: sy, x: 306, y: sy, width: 113, height: 9, rx: 4, fill: '#334155', stroke: '#94a3b8', strokeWidth: .8, opacity: .9 }); }), h('circle', { cx: 362, cy: 101, r: 15, fill: '#e8d8c3' }), h('path', { d: 'M347 108 Q362 118 377 108', fill: 'none', stroke: '#cbd5e1', strokeWidth: 3 }), h('text', { x: 362, y: 150, textAnchor: 'middle', fill: '#e2e8f0', fontSize: 14 }, 'SLEEP BAG')) :
            room.id === 'destiny' ? h('g', { className: 'iss-float' }, h('rect', { x: 312, y: 98, width: 100, height: 65, rx: 8, fill: '#172554', stroke: '#7dd3fc', strokeWidth: 3 }), h('rect', { x: 318, y: 104, width: 88, height: 7, rx: 3, fill: '#f472b6', opacity: .9, filter: 'url(#iss-light-glow)' }), h('rect', { x: 318, y: 113, width: 88, height: 20, fill: '#f472b6', opacity: .12 }), h('rect', { x: 320, y: 148, width: 84, height: 11, rx: 3, fill: '#3f3222', stroke: '#7c6a4a', strokeWidth: 1 }), h('path', { d: 'M350 150 C338 138 340 126 350 133 C359 123 364 138 350 150', fill: '#4ade80' }), h('path', { d: 'M376 150 C364 138 366 126 376 133 C385 123 390 138 376 150', fill: '#22c55e' }), h('path', { d: 'M350 150 C348 142 349 136 351 131 M376 150 C374 142 375 136 377 131', fill: 'none', stroke: '#14532d', strokeWidth: 1.4, opacity: .75 }), h('text', { x: 362, y: 196, textAnchor: 'middle', fill: '#7dd3fc', fontSize: 12, style: { paintOrder: 'stroke', stroke: '#050a18', strokeWidth: 3.4, strokeOpacity: .85 } }, 'PLANT HABITAT'), h('circle', { cx: 393, cy: 122, r: 7, fill: '#38bdf8', opacity: 0.85 })) :
          ...
          return h('svg', { viewBox: '0 0 724 260', role: 'img', 'aria-label': __alloFill(__alloT('stem.spacestation.a11y_interior_view_of', 'Interior view of {value1}. {value2}'), { value1: room.name, value2: room.scene }), style: { width: '100%', display: 'block', background: '#060b18' } },
            h('defs', null,
              ...
              h('linearGradient', { id: 'iss-sleepbag-body', x1: '0', y1: '0', x2: '1', y2: '1' }, h('stop', { offset: '0%', stopColor: '#7f9ec4' }), h('stop', { offset: '52%', stopColor: '#5b7ca5' }), h('stop', { offset: '100%', stopColor: '#3d5877' })),
              ...
              h('filter', { id: 'iss-window-cloud', x: '-50%', y: '-140%', width: '200%', height: '380%' }, h('feGaussianBlur', { stdDeviation: 5 })),
            ...
            [0.16, 0.33, 0.5, 0.67, 0.84].map(function (t) {
              return h('g', { key: 'seam' + t, stroke: '#cbd5e1', strokeWidth: 1, opacity: .16 },
                h('line', { x1: t * 724, y1: 18, x2: 114 + t * 496, y2: 88 }),
                h('line', { x1: t * 724, y1: 242, x2: 114 + t * 496, y2: 172 }));
            }),
            [0.3, 0.62].map(function (u) {
              return h('g', { key: 'sideseam' + u, stroke: '#cbd5e1', strokeWidth: 1, opacity: .16 },
                h('line', { x1: 0, y1: 18 + u * 224, x2: 114, y2: 88 + u * 84 }),
                h('line', { x1: 724, y1: 18 + u * 224, x2: 610, y2: 88 + u * 84 }));
            }),
            // Two rack fronts per wall, drawn in the plane's own perspective so
            // the tunnel has something to be a tunnel around.
            [[0, 1], [724, -1]].map(function (wall) {
              var edge = wall[0], dir = wall[1];
              return [0.08, 0.52].map(function (u) {
                var u2 = u + 0.34;
                var xa = edge + dir * 46, xb = edge + dir * 92;
                return h('g', { key: 'rack' + edge + u },
                  h('polygon', {
                    points: (xa) + ',' + (18 + u * 224 + (u * 70) * 0.0) + ' ' + (xb) + ',' + (18 + u * 224 + 28) + ' ' +
                            (xb) + ',' + (18 + u2 * 224 + 28) + ' ' + (xa) + ',' + (18 + u2 * 224),
                    fill: '#28374b', stroke: '#8fa1b6', strokeWidth: 1, opacity: .72 }),
                  h('line', { x1: xa, y1: 18 + (u + 0.17) * 224, x2: xb, y2: 18 + (u + 0.17) * 224 + 28, stroke: '#cbd5e1', strokeWidth: 1, opacity: .35 }));
              });
            }),
            ...
            [42, 118, 194, 482, 558, 634].map(function (x, i) { var y = i < 3 ? 42 : 190; return h('g', { key: i }, h('rect', { x: x, y: y, width: 48, height: 18, rx: 4, fill: '#172033', stroke: '#cbd5e1' }), h('rect', { x: x + 2, y: y + 2, width: 44, height: 6, rx: 3, fill: '#ffffff', opacity: .13 }), [0, 1, 2].map(function (v) { return h('line', { key: v, x1: x + 8 + v * 12, y1: y + 11, x2: x + 14 + v * 12, y2: y + 11, stroke: '#7dd3fc', strokeWidth: 1.4, opacity: .6 }); })); }),
        ...
        function renderLowGSimulator() {
          ...
          return h('div', { 'data-iss-maintenance-console': 'true' },
            ...
            h('div', { role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_life_support_telemetry_channels', 'Life-support telemetry channels'), style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(82px,1fr))', gap: 5 } }, sensors.map(function (sensor, i) { var checked = !!checks[i]; return h('button', { key: i, type: 'button', 'aria-pressed': checked, onClick: function () { inspectSensor(i); }, style: { minWidth: 0, padding: 7, borderRadius: 8, textAlign: 'left', border: '1px solid ' + (checked ? room.color : '#475569'), background: checked ? room.color + '16' : 'rgba(2,6,23,.38)', color: TEXT, cursor: 'pointer' } }, h('span', { style: { display: 'block', color: SOFT, fontSize: 8.5, lineHeight: 1.25 } }, (checked ? '✓ ' : '') + sensor[0]), h('strong', { style: { display: 'block', color: sensor[1] === 'NORMAL' ? '#4ade80' : '#fbbf24', fontSize: 11, marginTop: 3 } }, sensor[1])); })),
        ...
        function renderCabinStow() {
          ...
          function secureItem(item) {
            ...
            if (!cabinDiagramEnabled) { announceToSR(__alloT('stem.spacestation.sr_3_d_cabin_stow_is_active_use_x_to_catch_or_secure', '3-D cabin stow is active. Use X to catch or secure the nearest item.')); return; }
          ...
          return h('div', { 'data-iss-cupola-observation': 'true' },
            ...
            h('div', { role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_earth_observation_targets', 'Earth observation targets'), style: { display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 5, marginTop: 7 } }, Object.keys(targets).map(function (id) { var t = targets[id], active = id === targetId; return h('button', { key: id, type: 'button', disabled: roomDone, 'aria-pressed': active, onClick: function () { selectTarget(id); }, style: { minWidth: 0, padding: 6, borderRadius: 7, border: '1px solid ' + (active ? t.color : '#475569'), background: active ? t.color + '18' : 'rgba(2,6,23,.35)', color: active ? '#f8fafc' : SOFT, fontSize: 9.5, fontWeight: 800, cursor: roomDone ? 'default' : 'pointer' } }, t.icon + ' ' + t.label); })),
          ...
          h('div', { style: { display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 8, margin: '-4px 0 12px' } },
            h('div', { role: 'status', 'aria-label': __alloT('stem.spacestation.a11y_crew_shift_progress', 'Crew shift progress'), style: { display: 'flex', flexWrap: 'wrap', gap: 6 } },
            ...
            h('div', { role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_learning_guidance_level', 'Learning guidance level'), style: { display: 'flex', gap: 4, padding: 3, borderRadius: 9, background: '#0f172a', border: '1px solid #334155' } },
      ...
      function renderOpsForecast(samples, eclipseStart, cursorMinute, cursorSample, referenceSamples) {
        ...
        return h('div', { className: 'iss-learning-visual iss-orbit-forecast', 'data-iss-orbit-forecast': 'true' },
          h('svg', { viewBox: '0 0 640 190', role: 'img', 'aria-label': summary },
            h('defs', null,
              h('linearGradient', { id: 'iss-forecast-day', x1: '0', y1: '0', x2: '0', y2: '1' }, h('stop', { offset: '0%', stopColor: '#fbbf24', stopOpacity: .10 }), h('stop', { offset: '100%', stopColor: '#fbbf24', stopOpacity: .02 })),
              h('linearGradient', { id: 'iss-forecast-eclipse', x1: '0', y1: '0', x2: '0', y2: '1' }, h('stop', { offset: '0%', stopColor: '#818cf8', stopOpacity: .10 }), h('stop', { offset: '100%', stopColor: '#818cf8', stopOpacity: .02 }))),
            h('rect', { width: 640, height: 190, fill: '#050b18' }),
            h('rect', { x: left, y: top, width: right - left, height: bottom - top, fill: '#080f1f' }),
            h('rect', { x: left, y: top, width: eclipseX - left, height: bottom - top, fill: 'url(#iss-forecast-day)' }),
            h('rect', { x: eclipseX, y: top, width: right - eclipseX, height: bottom - top, fill: 'url(#iss-forecast-eclipse)' }),
            h('rect', { x: left, y: top - 7, width: eclipseX - left, height: 4, rx: 2, fill: '#fbbf24', opacity: .85 }),
            h('rect', { x: eclipseX, y: top - 7, width: right - eclipseX, height: 4, rx: 2, fill: '#818cf8', opacity: .85 }),
            ...
            h('text', { x: left + 8, y: top + 17, fill: '#fde68a', fontSize: 8.5, fontWeight: 850 }, 'SUNLIGHT'),
            h('text', { x: eclipseX + 8, y: top + 17, fill: '#c7d2fe', fontSize: 8.5, fontWeight: 850 }, 'ECLIPSE'),
      ...
      function opsLogEntry(message, patch) {
        var nextLog = (Array.isArray(d.opsLog) ? d.opsLog : []).concat(['ORBIT ' + ((d.opsRuns || 0) + 1) + ' // ' + message]).slice(-5);
      ...
      function renderOpsNetwork(metrics, focus) {
        ...
        function renderMissionReplay() {
          ...
          return h('div', { className: 'iss-mission-replay', 'data-iss-mission-replay': 'true' },
            ...
            h('div', { className: 'iss-replay-lane', role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_mission_replay_events', 'Mission replay events') }, replayEvents.map(function (event) { var on = Math.round(orbitMinute) === Math.round(event.minute); return h('button', { key: event.label, type: 'button', 'data-iss-replay-event': event.minute, 'aria-pressed': on, onClick: function () { upd({ opsOrbitMinute: event.minute }); } }, h('i', { 'aria-hidden': 'true' }), h('strong', null, 'T+' + event.minute), h('span', null, event.label), h('small', null, event.note)); })));
        ...
        function renderRendezvous() {
          ...
          return h('div', null,
            h('div', { className: 'iss-learning-visual iss-rendezvous-planner', 'data-iss-rendezvous-planner': holdIndex },
              ...
              h('div', { className: 'iss-network-focus', role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_inspect_a_rendezvous_hold_point', 'Inspect a rendezvous hold point') }, holds.map(function (item, i) { var on = i === holdIndex; return h('button', { key: item.range, type: 'button', 'data-iss-rendezvous-hold': i, 'aria-pressed': on, onClick: function () { upd({ opsRendezvousHold: i }); } }, item.range + ' · ' + item.phase.toLowerCase()); })),
        ...
        return h('div',{'data-iss-operations':mode},
          ...
          renderRuleStatus(),
          h('div',{className:'iss-ops-modes',role:'group','aria-label':__alloT('stem.spacestation.a11y_operations_simulation', 'Operations simulation')},modes.map(function(m){var on=mode===m[0];return h('button',{className:'iss-ops-mode',key:m[0],type:'button','aria-pressed':on,onClick:function(){setMode(m[0]);},style:{borderRadius:9,border:'1px solid '+(on?'#38bdf8':'#334155'),background:on?'rgba(14,165,233,.18)':PANEL,color:on?'#bae6fd':TEXT,fontSize:10.5,fontWeight:850,cursor:'pointer'}},m[1]+' '+m[2]);})),content,
          h('div',{className:'iss-ops-debrief'},h('strong',{style:{color:'#7dd3fc',fontSize:11}},'FLIGHT DIRECTOR DEBRIEF'),h('p',{style:{margin:'5px 0 0',color:TEXT,fontSize:11.5,lineHeight:1.55}},health>=75?'All modeled systems retain useful margin. Now increase research load or crew size and find the boundary.':health>=50?'The station is operating with thin margin. Inspect the red or amber subsystem before the next orbit.':'Mission rule violation: one or more life-critical systems require immediate correction.'),(Array.isArray(d.opsLog) ? d.opsLog : []).length?h('div',{className:'iss-ops-log',role:'log'},(Array.isArray(d.opsLog) ? d.opsLog : []).map(function(entry,i){return h('div',{key:i},entry);})):null));
      }
      // ── Tabs ──
      ...
      function renderMap() {
        ...
        return h('div', null,
          ...
          h('div', { className: 'iss-module-picker', role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_station_modules', 'Station modules'), style: { display: 'flex', flexWrap: 'wrap', gap: 6, margin: '10px 0' } },
          ...
          h('svg', { viewBox: '0 0 640 157', role: 'img', 'aria-label': __alloFill(__alloT('stem.spacestation.a11y_twenty_four_hour_gmt_crew_timeline_selected_eve', 'Twenty-four hour GMT crew timeline. Selected event {value1} at {value2}. Daily allocation: 8.5 hours sleep, 1.5 hours preparation, 10.5 hours work and exercise, and 3.5 hours meals and crew time. The selected event is in the {value3} block.'), { value1: slot.label, value2: slot.h, value3: selectedAllocation.label.toLowerCase() })},
            ...
            phases.map(function (phase, phaseIndex) {
              ...
              return h('g', { key: phaseIndex },
                h('rect', { x: x, y: 50, width: width, height: 24, rx: 3, fill: phase.color, opacity: .68 }),
                // Glass sheen on the upper half + a shaded base: enough relief to
                // read the band as a solid block rather than a flat swatch.
                h('rect', { x: x, y: 50, width: width, height: 10, rx: 3, fill: '#ffffff', opacity: .12 }),
                h('rect', { x: x, y: 69, width: width, height: 5, fill: '#020617', opacity: .18 }),
                ...
                isSleep ? h('g', { opacity: .6 }, [0, 1, 2].map(function (s) {
                  ...
                  return starX < x + width - 4 ? h('circle', { key: 's' + s, cx: starX, cy: 56 + (s % 2) * 9, r: .9, fill: '#e0e7ff' }) : null;
                })) : null,
                width > 58 ? h('text', { x: x + width / 2, y: 65, textAnchor: 'middle', fill: '#f8fafc', fontSize: 7.5, fontWeight: 850 }, phase.label) : null);
            }),
            DAY_SCHEDULE.map(function (event, eventIndex) { var eventX = 40 + minuteOf(event.h) / 1440 * 560; var selected = eventIndex === index; return h('g', { key: eventIndex, opacity: selected ? 1 : .68 }, h('line', { x1: eventX, y1: 74, x2: eventX, y2: selected ? 91 : 83, stroke: selected ? '#fbbf24' : '#cbd5e1', strokeWidth: selected ? 2 : 1 }), h('circle', { cx: eventX, cy: 74, r: selected ? 4.5 : 2.5, fill: selected ? '#fbbf24' : '#e2e8f0', stroke: '#07101d', strokeWidth: 1 })); }),
            [0,360,720,1080,1440].map(function (minute, tickIndex) { var x = 40 + minute / 1440 * 560; return h('g', { key: tickIndex }, h('line', { x1: x, y1: 92, x2: x, y2: 97, stroke: '#64748b' }), h('text', { x: x, y: 110, textAnchor: tickIndex === 0 ? 'start' : tickIndex === 4 ? 'end' : 'middle', fill: '#94a3b8', fontSize: 8 }, tickIndex === 4 ? '24:00' : String(Math.floor(minute / 60)).padStart(2, '0') + ':00')); }),
            h('text', { x: 20, y: 125, fill: '#94a3b8', fontSize: 7.5, fontWeight: 850, letterSpacing: 1 }, '24H ALLOCATION'),
      ...
      function renderDayOrbitVisual(slot, index) {
        ...
        return h('div', null,
          ...
          h('div', { className: 'iss-day-strip', role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_daily_schedule', 'Daily schedule'), style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 } },
      ...
      function renderDragReboostVisual() {
        ...
        return h('div', { className: 'iss-learning-visual iss-drag-reboost-visual', 'data-iss-drag-model': solarId, 'data-iss-reboost-day': reboostDay },
          ...
          h('div', { className: 'iss-network-focus', role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_select_solar_activity_for_the_drag_model', 'Select solar activity for the drag model') }, Object.keys(solarModes).map(function (id) { var on = id === solarId; return h('button', { key: id, type: 'button', 'data-iss-solar-mode': id, 'aria-pressed': on, onClick: function () { upd({ orbitSolar: id }); } }, solarModes[id].label.toLowerCase()); })),
          h('div', { className: 'iss-network-focus', role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_select_a_reboost_plan', 'Select a reboost plan') }, [{ day: 0, label: 'No reboost' }, { day: 10, label: 'Reboost day 10' }, { day: 20, label: 'Reboost day 20' }].map(function (plan) { var on = plan.day === reboostDay; return h('button', { key: plan.day, type: 'button', 'data-iss-reboost-plan': plan.day, 'aria-pressed': on, onClick: function () { upd({ orbitReboostDay: plan.day }); } }, plan.label); })),
      ...
      function renderSystems() {
        ...
        return h('div', null,
          ...
          h('div', { className: 'iss-system-tabs', role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_station_systems', 'Station systems'), style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 } },
          ...
          aiOn ? card(__alloT('stem.spacestation.ask_mc', '🎧 Ask Mission Control'),
            h('div', { 'aria-busy': !!d.askLoading },
              h('textarea', {
                ...
                'aria-label': __alloT('stem.spacestation.a11y_question_for_mission_control', 'Question for Mission Control'), 'aria-describedby': 'iss-ask-count',
      ...
      function renderInclinationLab() {
        ...
        return card(__alloT('stem.spacestation.inclination_lab', '🌍 Inclination: which world you fly over'),
          h('div', null,
            ...
            h('div', { role: 'group', 'aria-label': __alloT('stem.spacestation.a11y_inclination_presets', 'Inclination presets'), style: { display: 'flex', flexWrap: 'wrap', gap: 6, margin: '8px 0' } },
      ...
      function renderAssemblyVisual() {
        ...
        return h('div', { className: 'iss-assembly-stage' },
          ...
          h('svg', { viewBox: '0 0 640 245', role: 'img', 'aria-label': __alloFill(__alloT('stem.spacestation.a11y_station_assembly_visualization_at_major_element', 'Station assembly visualization at {value1}. {value2} major elements shown, {value3} of 8 solar array wings installed, {value4} kilowatts available. {value5}.'), { value1: TIMELINE[step].y, value2: visible.length, value3: (arrayWings.length * 2), value4: powerStage, value5: installLabel })},
            ...
            // Readout strip. The module stack grows downward as later modules
            // are added and used to run straight through these numbers, so each
            // one now sits on its own plate over the limb rather than in clear
            // space that stops being clear at the last two milestones.
            h('g', { transform: 'translate(20,209)' }, [['MAJOR ELEMENTS', visible.length, '#7dd3fc'], ['PRESSURIZED VOLUME', volumeStage + ' m³', '#34d399'], ['AVAILABLE POWER', powerStage + ' kW', '#fbbf24'], ['SOLAR WINGS', (arrayWings.length * 2) + ' / 8', '#fde68a']].map(function (metric, i) { return h('g', { key: i, transform: 'translate(' + (i * 152) + ',0)' }, h('rect', { x: -7, y: -13, width: 142, height: 42, rx: 9, fill: 'rgba(4,10,22,.82)', stroke: 'rgba(148,163,184,.22)' }), h('rect', { x: -7, y: -13, width: 3, height: 42, rx: 2, fill: metric[2], opacity: .85 }), h('text', { x: 4, fill: '#94a3b8', fontSize: 8, fontWeight: 800, letterSpacing: .8 }, metric[0]), h('text', { x: 4, y: 22, fill: metric[2], fontSize: 15, fontWeight: 900 }, String(metric[1]))); }))),
        ...
        return h('div', { className: 'iss-learning-visual iss-quiz-debrief', 'data-iss-quiz-debrief': known },
          h('svg', { viewBox: '0 0 640 198', role: 'img', 'aria-label': __alloFill(__alloT('stem.spacestation.a11y_quiz_debrief_score_out_of', 'Quiz debrief. Score {value1} out of {value2}. {value3}. {value4}'), { value1: score, value2: QUIZ.length, value3: outcome.toLowerCase(), value4: detail })},
            ...
            h('g', { transform: 'translate(104,104)' },
              ...
              h('text', { y: 74, textAnchor: 'middle', fill: score >= QUIZ_PASS ? '#86efac' : '#fde68a', fontSize: 8.5, fontWeight: 900, letterSpacing: 1.2 }, outcome)),
