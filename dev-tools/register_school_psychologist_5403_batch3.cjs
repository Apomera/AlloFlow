...
const manifestPath = path.join(
  __dirname,
  'authored',
  'test_prep_independent_additions_manifest.json',
);
const expected = [{
...
const current = manifest.packs.school_psychologist_5403;
if (current && JSON.stringify(current) !== JSON.stringify(expected)) {
  throw new Error('School Psychologist 5403 already has a different manifest registration.');
}
manifest.packs.school_psychologist_5403 = expected;
const output = `${JSON.stringify(manifest, null, 2)}\n`;
if (process.argv.includes('--check')) {
  if (JSON.stringify(current) !== JSON.stringify(expected)) {
    throw new Error('School Psychologist 5403 is not registered exactly.');
  }
  console.log('School Psychologist 5403 authored Batch 3 manifest registration is exact.');
} else if (JSON.stringify(current) === JSON.stringify(expected)) {
  console.log('School Psychologist 5403 authored Batch 3 was already registered exactly.');
} else {
  writeGeneratedFile(manifestPath, output);
  console.log('Registered School Psychologist 5403 authored Batch 3 in the schema-v2 manifest.');
}
