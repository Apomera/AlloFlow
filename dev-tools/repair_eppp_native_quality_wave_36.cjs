...
const { baselineMetrics, reviewedAt, reviewWave, revisions } = require('./eppp_native_quality_wave_36_data.cjs');
...
const auditBasename = 'eppp_question_quality_audit_wave_36.json';
...
if (fs.readFileSync(deployBankPath, 'utf8') !== sourceText) throw new Error('Source and deploy EPPP banks must match before wave 36.');
...
if (revisions.length !== 24 || new Set(revisions.map((revision) => revision.id)).size !== 24) throw new Error('Wave 36 needs twenty-four unique revisions.');
...
for (const revision of revisions) {
  ...
  if (!recognizedReplay && item.prompt !== revision.expectedPrompt) throw new Error(revision.id + ' prompt drifted before wave 36.');
  if (item.answerIndex !== revision.expectedAnswerIndex) throw new Error(revision.id + ' answer position drifted.');
  if (!recognizedReplay && item.difficulty !== revision.expectedDifficulty) throw new Error(revision.id + ' difficulty drifted before wave 36.');
...
const audit = {
  ...
  scope: 'Twenty-four EPPP items, three per domain, rewritten as applied or analytic decisions with parallel distractors and misconception-specific feedback.',
  summary: {
...
console.log('EPPP quality wave 36: rewrote 24 items across 8 domains; selected warnings after = 0.');
