              ...
              return h('div', { className: 'mb-3 bg-white rounded-xl p-2.5 border border-cyan-200' },
                ...
                (npcQuests[npcKey] || []).length > 0 && h('div', { className: 'mt-2 space-y-1' },
                  ...
                  (npcQuests[npcKey] || []).map(function(q) {
                    ...
                    return h('div', { key: q.id, className: 'flex items-center gap-1.5 text-[11px] ' + (q.isCompleted ? 'text-green-600' : isLocked ? 'text-slate-400' : 'text-slate-700') },
                ...
                return h('div', { role: 'button', tabIndex: 0, onKeyDown: function(e) { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.target.click(); } }, key: i, className: 'group relative' },
                  h('button', {
                    ...
                    h('div', { className: 'w-full mt-0.5 flex items-center gap-1' },
                      ...
                      h('span', { className: 'text-[11px] text-slate-400' }, (item.durability || 0) + '/' + (item.maxDurability || 0))
                    )
                  ),
                  // Tooltip on hover
              ...
              h('div', { className: 'flex gap-2 mb-2' },
                [['letterFormation', 'Letters'], ['spacing', 'Spacing'], ['alignment', 'Alignment'], ['neatness', 'Neatness']].map(function(pair) {
                  ...
                  return h('div', { key: pair[0], className: 'flex-1 text-center' },
                    h('div', { className: 'text-sm font-black ' + (val >= 18 ? 'text-green-600' : val >= 12 ? 'text-amber-600' : 'text-slate-500') }, val, h('span', { className: 'text-[11px] opacity-60' }, '/25')),
