...
describe('Dino Lab 3D Field Station accessibility contract', () => {
  it('supports focused keyboard rotation with live status and cleanup', () => {
    ...
    expect(source).toContain('function addVertebralChain(start, end, count, startRadius, endRadius, processScale)');
    expect(source).toContain('var skullLength = Math.max(');
    expect(source).toContain('var temporalOpening = addSkeletonEllipsoid');
    expect(source).toContain('for (var toothIndex = 1; toothIndex <= 7; toothIndex++)');
    expect(source).toContain('for (var gastralIndex = 1; gastralIndex <= 4; gastralIndex++)');
    ...
    expect(source).toContain("var handDigits = /Tyrannosaur/i.test(cladeName) ? 2 : 3;");
    ...
    expect(source).toContain('var cervicalRibCount = isSauropod ? 8 : 4;');
    expect(source).toContain('for (var chevronIndex = 1; chevronIndex <= 7; chevronIndex++)');
    expect(source).toContain('for (var sacralIndex = 0; sacralIndex < 5; sacralIndex++)');
    expect(source).toContain('var furculaCenter = shoulder.clone()');
    expect(source).toContain('var robustFoot = /Sauropod|Ceratops|Ankylosaur|Hadrosaur/i.test(cladeName);');
    expect(source).toContain('var visibleUngual = isTheropod');
