// AlloFlow Section: CAST_LOBBY
// Module: modules/cast
// Variant: local (cloud-stripped)
// Source lines: 16621–23673
// Hash: c7fcb256a14de541
...
// @section CAST_LOBBY — Multi-device cast/lobby system
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                    {characters.map((char, i) => (
                        <div key={i} className="bg-gradient-to-br from-slate-50 to-violet-50 rounded-2xl border border-violet-100 p-4 flex flex-col items-center text-center transition-all hover:shadow-lg hover:border-violet-300 relative group/card">
                            ...
                            {editingField?.idx === i && editingField?.field === 'name' ? (
                                <input type="text" aria-label={t('adventure.edit_name')} value={editFieldValue} onChange={(e) => setEditFieldValue(e.target.value)} onBlur={saveFieldEdit} onKeyDown={(e) => { if (e.key === 'Enter') saveFieldEdit(); if (e.key === 'Escape') { setEditingField(null); setEditFieldValue(''); }}} autoFocus className="font-bold text-slate-800 text-sm text-center w-full px-2 py-0.5 border border-violet-300 rounded-lg focus:ring-2 focus:ring-violet-400 focus:outline-none bg-white"/>
                            ) : (
                            ...
                            {editingField?.idx === i && editingField?.field === 'role' ? (
                                <input type="text" aria-label={t('adventure.edit_role')} value={editFieldValue} onChange={(e) => setEditFieldValue(e.target.value)} onBlur={saveFieldEdit} onKeyDown={(e) => { if (e.key === 'Enter') saveFieldEdit(); if (e.key === 'Escape') { setEditingField(null); setEditFieldValue(''); }}} autoFocus className="text-xs text-violet-600 font-medium text-center w-full px-2 py-0.5 border border-violet-300 rounded-lg focus:ring-2 focus:ring-violet-400 focus:outline-none bg-white mt-0.5"/>
                            ) : (
                            ...
                            {editingField?.idx === i && editingField?.field === 'appearance' ? (
                                <textarea aria-label={t('adventure.edit_appearance')} value={editFieldValue} onChange={(e) => setEditFieldValue(e.target.value)} onBlur={saveFieldEdit} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); saveFieldEdit(); } if (e.key === 'Escape') { setEditingField(null); setEditFieldValue(''); }}} autoFocus rows={3} className="text-[11px] text-slate-600 mt-1 leading-relaxed w-full px-2 py-1 border border-violet-300 rounded-lg focus:ring-2 focus:ring-violet-400 focus:outline-none bg-white resize-none"/>
                            ) : (
                            ...
                            {char.portrait && !char.isGenerating && (
                                <div className="mt-2 flex flex-wrap gap-1.5 justify-center">
                                    ...
                                    <input type="file" accept="image/*" ref={el => portraitFileRefs.current[i] = el} onChange={(e) => handlePortraitFileChange(i, e)} className="hidden" aria-label={t('adventure.upload_portrait') || 'Upload portrait image'} />
                                    ...
                                    {editIdx === i ? (
                                        <div className="w-full flex gap-1 mt-1">
                                            <input type="text" aria-label={t('adventure.refine_portrait_prompt') || 'Refine portrait prompt'} value={editPrompt} onChange={(e) => setEditPrompt(e.target.value)} placeholder="e.g. Add green glasses" className="flex-1 text-xs px-2 py-1 border border-violet-200 rounded-lg focus:ring-2 focus:ring-violet-400 focus:outline-none" onKeyDown={(e) => { if (e.key === 'Enter' && editPrompt.trim()) { onRefinePortrait(i, editPrompt.trim()); setEditIdx(null); setEditPrompt(''); }}}/>
                                            <button onClick={() => { if (editPrompt.trim()) { onRefinePortrait(i, editPrompt.trim()); setEditIdx(null); setEditPrompt(''); }}} className="text-xs px-2 py-1 bg-violet-700 text-white rounded-lg font-bold hover:bg-violet-600">✓</button>
                            ...
                            {!char.portrait && !char.isGenerating && (
                                <div className="mt-2 flex flex-wrap gap-1.5 justify-center">
                                    ...
                                    <input type="file" accept="image/*" ref={el => portraitFileRefs.current['new-' + i] = el} onChange={(e) => handlePortraitFileChange(i, e)} className="hidden" aria-label={t('adventure.upload_portrait') || 'Upload portrait image'} />
                    ...
                    {isAdding ? (
                        <div className="bg-gradient-to-br from-violet-50 to-indigo-50 rounded-2xl border-2 border-dashed border-violet-300 p-4 flex flex-col items-center text-center">
                            ...
                            <input type="text" aria-label={t('adventure.appearance_label') || 'Character appearance'} value={newAppearance} onChange={(e) => setNewAppearance(e.target.value)} placeholder="Appearance (e.g. tall, silver hair, blue robe)" className="w-full text-xs px-3 py-1.5 mb-2 border border-violet-200 rounded-lg focus:ring-2 focus:ring-violet-400 focus:outline-none text-center" onKeyDown={(e) => { if (e.key === 'Enter' && newName.trim()) { onAddCharacter({ name: newName.trim(), role: newRole.trim() || 'Character', appearance: newAppearance.trim() || newName.trim(), portrait: null, isGenerating: false }); setNewName(''); setNewRole(''); setNewAppearance(''); setIsAdding(false); }}}/>
                        ...
                        <button onClick={() => setIsAdding(true)} className="bg-gradient-to-br from-slate-50 to-violet-50 rounded-2xl border-2 border-dashed border-violet-200 p-4 flex flex-col items-center justify-center text-center transition-all hover:shadow-lg hover:border-violet-400 hover:from-violet-50 hover:to-indigo-50 min-h-[180px] cursor-pointer group">
                            ...
                            <span className="text-[11px] text-slate-500 mt-0.5">{t('adventure.create_cast_member')}</span>
                        </button>
                    )}
                </div>
...
const AlloFlowContent = () => {
  ...
  useEffect(() => {
    ...
    const initStorage = async () => {
        try {
            await Promise.all([
                loadScriptSequence([
                    "./shared/libs/lz-string.min.js"
                ], "LZString"),
                loadScriptSequence([
                    "./shared/libs/idb-keyval.min.js"
                ], "idbKeyval"),
                Promise.resolve()
            ]);
            setLzLoaded(true);
  ...
  const [selectedVoice, setSelectedVoice] = useState(() => {
      if (typeof window !== 'undefined') {
          ...
          const isLocalBackend = cfg?.backend === 'ollama' || cfg?.backend === 'lmstudio' || cfg?.backend === 'localai';
          const saved = safeGetItem('allo_voice_preference');
          if (saved) {
  ...
  useEffect(() => {
    // Clear wizard completion on fresh page load so wizard shows every visit
  ...
  // After loading: check if there's an existing session; skip startup if so
  useEffect(() => {
    if (!isAppReady || !window.__alloLocalDB || !window.__alloAuth?.checkSession) return;
    window.__alloAuth.checkSession().then(data => {
      if (data && data.user) { setStartupUser(data.user); setStartupDone(true); }
    });
  }, [isAppReady]);
  ...
  const handleWizardStandardLookup = async (grade, goal, region) => {
      try {
          ...
          const isLocalBackend = ai?.backend === 'ollama' || ai?.backend === 'lmstudio' || ai?.backend === 'localai';
          ...
          if (isLocalBackend) {
              ...
              let searchContext = '';
              try {
                  const searchResults = typeof webSearchProvider !== 'undefined' && webSearchProvider
                      ? await webSearchProvider.search(searchQuery)
                      : null;
...
Return ONLY the hint text as a single paragraph (no JSON, no markdown). Keep it under 2 sentences. Be encouraging.`;
  ...
  React.useEffect(() => {
    const loadModule = (name, url) => {
      console.log('[LOCAL] Loading module ' + name + ' from: ' + url);
      const s = document.createElement('script');
      s.src = url;
      s.onload = () => {
        const found = window.AlloModules && window.AlloModules[name];
        console.log('[LOCAL] ' + name + ': ' + (found ? 'SUCCESS' : 'NOT REGISTERED'));
      };
      s.onerror = (e) => {
        console.warn('[LOCAL] ' + name + ' failed to load from: ' + url);
      };
    ...
    // [LOCAL] Load from local shared/ directory instead of CDN
    (function() {
      const url = './shared/modules/ai_backend_module.js';
      const s = document.createElement('script');
      s.src = url;
      s.onload = () => { console.log('[LOCAL] AIBackend loaded'); };
      s.onerror = () => { console.warn('[LOCAL] AIBackend load failed from:', url); };
      document.head.appendChild(s);
    })();
    loadModule('StemLab', './shared/modules/stem_lab/stem_lab_module.js');
    loadModule('WordSoundsModal', './shared/modules/word_sounds_module.js');
    loadModule('StudentAnalytics', './shared/modules/student_analytics_module.js');
    loadModule('BehaviorLens', './shared/modules/behavior_lens_module.js');
    loadModule('SymbolStudio', './shared/modules/symbol_studio_module.js');
    loadModule('SelHub', './shared/modules/sel_hub/sel_hub_module.js');
    loadModule('GamesBundle', './shared/modules/games_module.js');
    loadModule('QuickStartWizard', './shared/modules/quickstart_module.js');
    loadModule('AlloBot', './shared/modules/allobot_module.js');
    loadModule('TeacherModule', './shared/modules/teacher_module.js');
    loadModule('StoryForge', './shared/modules/story_forge_module.js');
    loadModule('VisualPanelModule', './shared/modules/visual_panel_module.js');
    ...
    // ── DocPipeline module (PDF accessibility audit + remediation) ──
    loadModule('createDocPipeline', './shared/modules/doc_pipeline_module.js');
    // ── Load math.js for graphCalc (lazy, non-blocking) ──
    (function() {
      var s = document.createElement('script');
      s.src = './shared/libs/math.min.js';
      s.async = true;
    ...
    setTimeout(function() {
      var pluginCdnBase = './shared/modules/';
      var toolModules = [
    ...
    if (_isCanvasEnv && !window.__kokoroTTSScriptInjected) {
      ...
      (async () => {
        await loadTTSScript('./shared/kokoro_tts_loader.js');
        loadTTSScript('./shared/piper_tts_loader.js');
...
const handleToggleShowMathAnswers = React.useCallback(() => setShowMathAnswers(prev => !prev), []);
  ...
  useEffect(() => {
      if (wsPreloadedWords.length > 0 && prevWsPreloadedWordsLengthRef.current === 0) {
          debugLog("🚀 Auto-opening Word Sounds mode for", wsPreloadedWords.length, "preloaded words");
