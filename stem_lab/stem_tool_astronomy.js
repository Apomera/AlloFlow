  ...
  window.StemLab.registerTool('astronomy', {
    ...
    render: function(ctx) {
      ...
      if (!labToolData || !labToolData.astronomy) {
        ...
        return h('div', { style: { padding: 24, color: '#94a3b8', textAlign: 'center' } }, t('stem.astronomy.initializing_night_sky_lab', '🔭 Initializing Night Sky Lab...'));
      }
      ...
      // ──────────────────────────────────────────────────────────────
      var TABS = [
        { id: 'tonight',      icon: '🌙', label: t('stem.astronomy.tonight', 'Tonight') },
        { id: 'constellations', icon: '⭐', label: t('stem.astronomy.constellations', 'Constellations') },
        { id: 'moon',         icon: '🌖', label: t('stem.astronomy.moon', 'Moon') },
        { id: 'planets',      icon: '🪐', label: t('stem.astronomy.planets', 'Planets') },
        { id: 'exoplanets',   icon: '🌎', label: t('stem.astronomy.exoplanets', 'Exoplanets') },
        { id: 'seasons',      icon: '🌍', label: t('stem.astronomy.seasons', 'Seasons') },
        { id: 'stars',        icon: '✨', label: t('stem.astronomy.stars', 'Stars') },
        { id: 'galaxies',     icon: '🌌', label: t('stem.astronomy.galaxies', 'Galaxies') },
        { id: 'eclipses',     icon: '🌑', label: t('stem.astronomy.events', 'Events') },
        { id: 'indigenous',   icon: '🪶', label: t('stem.astronomy.sky_traditions', 'Sky Traditions') },
        { id: 'history',      icon: '📜', label: t('stem.astronomy.history', 'History') },
        { id: 'observe',      icon: '🔭', label: t('stem.astronomy.observing', 'Observing') },
        { id: 'quiz',         icon: '📝', label: t('stem.astronomy.quiz', 'Quiz') },
        { id: 'print',        icon: '🖨', label: t('stem.astronomy.print', 'Print') },
        { id: 'hrDiagram',    icon: '⭐', label: t('stem.astronomy.hr_diagram', 'HR Diagram') }
      ];

      var tabBar = h('div', {
        role: 'tablist', 'aria-label': t('stem.astronomy.astronomy_sections', 'Astronomy sections'),
        ...
        return h('div', { style: { padding: 16 } },
          sectionCard('🌙 Right now (' + now.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' }) + ')',
            h('div', null,
              h('div', { style: { fontSize: 13, color: '#cbd5e1', lineHeight: 1.7, marginBottom: 8 } },
                t('stem.astronomy.approximate_moon_phase', 'Approximate moon phase: '),
                h('strong', { style: { color: '#fde68a' } }, moonPhase.name),
                ' (~', Math.round(phaseAge), t('stem.astronomy.days_into_the_lunar_cycle', ' days into the lunar cycle).')
              ),
          ...
          sectionCard('🪐 Naked-eye planets (planning approximation)',
            h('div', { style: { fontSize: 12.5, color: '#cbd5e1', lineHeight: 1.7 } },
              t('stem.astronomy.mercury_venus_mars_jupiter_and_saturn_', 'Mercury, Venus, Mars, Jupiter, and Saturn can all be visible to the naked eye depending on the date. '),
              t('stem.astronomy.venus_is_the_brightest_often_the_morni', 'Venus is the brightest — often the "morning star" before dawn or "evening star" after sunset. '),
              t('stem.astronomy.jupiter_and_saturn_are_visible_most_of', 'Jupiter and Saturn are visible most of each year, near the ecliptic (the path the Sun takes through the sky). '),
              t('stem.astronomy.mars_varies_a_lot_it_can_be_very_brigh', 'Mars varies a lot — it can be very bright and red, or hard to find, depending on where it is in its orbit. '),
              t('stem.astronomy.for_precise_positions_tonight_check_st', 'For precise positions tonight, check Stellarium (free), the NASA Eyes web app, or a sky app like Night Sky or SkyView.')
            )
          ),

          sectionCard('🤖 Ask the sky',
            h('div', null,
              h('div', { style: { fontSize: 12, color: '#94a3b8', marginBottom: 8, lineHeight: 1.55 } }, t('stem.astronomy.ask_anything_what_is_that_bright_thing', 'Ask anything: "What is that bright thing in the south right now?" "How do I find the Pleiades?" "Why does the moon look different colors?"')),
              h('textarea', {
                value: d.askInput || '',
                onChange: function(e) { upd({ askInput: e.target.value }); },
                placeholder: t('stem.astronomy.your_question', 'Your question...'),
                rows: 3,
              ...
              h('div', { style: { marginTop: 8, display: 'flex', gap: 8, alignItems: 'center' } },
                ...
                !callGemini ? h('div', { style: { fontSize: 11, color: '#94a3b8' } }, t('stem.astronomy.ai_unavailable_in_this_session', '(AI unavailable in this session)')) : null
              ),
      ...
      function renderConstellations() {
        ...
        return h('div', { style: { padding: 16 } },
          ...
          selected ? h('div', { style: { padding: 14, borderRadius: 12, background: '#1e293b', border: '1px solid #334155' } },
            ...
            h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.10)', borderLeft: '3px solid ' + INDIGO, marginBottom: 10 } },
              h('div', { style: { fontSize: 11, fontWeight: 800, color: '#a5b4fc', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.astronomy.greek_tradition', 'Greek tradition')),
            ...
            h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(245,158,11,0.10)', borderLeft: '3px solid #f59e0b', marginBottom: 10 } },
              h('div', { style: { fontSize: 11, fontWeight: 800, color: '#fbbf24', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.astronomy.other_traditions', 'Other traditions')),
            ...
            h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.10)', borderLeft: '3px solid #22c55e', marginBottom: 10 } },
              h('div', { style: { fontSize: 11, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.astronomy.how_to_find_it', 'How to find it')),
            ...
            h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(56,189,248,0.10)', borderLeft: '3px solid #38bdf8' } },
              h('div', { style: { fontSize: 11, fontWeight: 800, color: '#7dd3fc', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.astronomy.science_note', 'Science note')),
            ...
            h('div', { style: { marginTop: 12 } },
              ...
              }, t('stem.astronomy.add_to_observing_list', '+ Add to observing list'))
            )
          ) : h('div', { style: { padding: 14, borderRadius: 12, background: '#1e293b', border: '1px dashed #334155', color: '#94a3b8', fontStyle: 'italic', fontSize: 13 } }, t('stem.astronomy.select_a_constellation_to_see_its_stor', 'Select a constellation to see its story across traditions, how to find it, and the science behind it.')),
        ...
        function allConstellations88Section() {
          var ALL_88 = [
            ...
            { abbr: 'CrB', name: t('stem.astronomy.corona_borealis', 'Corona Borealis'), english: 'Northern Crown', area: 179, brightStar: 'Gemma/Alphecca (2.21)', month: 'Jun', hemisphere: 'N', mythology: 'Greek: the crown given to Ariadne by Dionysus when he made her his wife after Theseus abandoned her. A distinctive small semicircular arc of 7 stars. Hosts T Coronae Borealis, the famous recurrent nova due for eruption any year now.', notable: 'T CrB (recurrent nova, predicted ~ 2024-2026 outburst), R CrB (prototype R Coronae Borealis variable)' },
            { abbr: 'Her', name: t('stem.astronomy.hercules', 'Hercules'), english: 'Hercules', area: 1225, brightStar: 'Kornephoros (2.78)', month: 'Jul', hemisphere: 'N', mythology: 'Greek: the great hero, son of Zeus, completer of the Twelve Labors. The shape: a "Keystone" asterism (4 stars in a trapezoid) forms his torso. Hercules contains M13, the brightest globular cluster in the Northern Hemisphere.', notable: 'M13 (Hercules Cluster — brightest northern globular), M92 globular, Hercules Cluster of galaxies' },
            { abbr: 'Per', name: t('stem.astronomy.perseus', 'Perseus'), english: 'Perseus', area: 615, brightStar: 'Mirfak (1.79)', month: 'Dec', hemisphere: 'N', mythology: 'Greek: rescuer of Andromeda from the sea monster Cetus, slayer of Medusa. Algol = "the demon\'s head" (head of Medusa), an eclipsing binary variable. Perseid meteor shower radiates from here every August.', notable: 'Algol (eclipsing binary), Double Cluster (NGC 869/884), California Nebula, Perseid meteor radiant' },
            { abbr: 'And', name: t('stem.astronomy.andromeda', 'Andromeda'), english: 'Chained Maiden', area: 722, brightStar: 'Alpheratz (2.07)', month: 'Nov', hemisphere: 'N', mythology: 'Greek: princess of Ethiopia, daughter of Cassiopeia + Cepheus, chained to a rock + saved by Perseus. Contains the Andromeda Galaxy (M31), the largest galaxy in our Local Group + the most distant object visible to the naked eye.', notable: 'Andromeda Galaxy (M31), Mu Andromedae (multiple star system with confirmed planets)' },
            { abbr: 'Tri', name: t('stem.astronomy.triangulum', 'Triangulum'), english: 'Triangle', area: 132, brightStar: 'Beta Trianguli (3.0)', month: 'Dec', hemisphere: 'N', mythology: 'Small constellation between Andromeda + Aries. Greek "Deltoton" (delta-shape) due to its triangular form. Contains the Triangulum Galaxy (M33), the 3rd-largest member of the Local Group of galaxies.', notable: 'Triangulum Galaxy (M33)' },
            { abbr: 'Peg', name: t('stem.astronomy.pegasus', 'Pegasus'), english: 'Flying Horse', area: 1121, brightStar: 'Enif (2.39)', month: 'Oct', hemisphere: 'N', mythology: 'Greek: the winged horse born from Medusa\'s blood, ridden by Bellerophon. The "Great Square of Pegasus" is the body, formed by 4 bright stars (technically one is shared with Andromeda). Contains the first confirmed exoplanet around a sun-like star (51 Peg b, 1995).', notable: 'Great Square of Pegasus asterism, 51 Pegasi (first sunlike-star exoplanet host)' },
            { abbr: 'Ori', name: t('stem.astronomy.orion', 'Orion'), english: 'Hunter', area: 594, brightStar: 'Rigel (0.18)', month: 'Jan', hemisphere: 'N/S', mythology: 'The most recognizable constellation worldwide. Greek: a giant hunter, killed by the scorpion (placed on opposite side of sky). Orion\'s Belt (Alnitak, Alnilam, Mintaka) is iconic. Betelgeuse = "house of the twin" (Arabic), expected to go supernova in the next 100,000 years (likely).', notable: 'Betelgeuse, Rigel, Orion Nebula (M42), Horsehead Nebula, Trapezium, Orion\'s Belt + Sword' },
            { abbr: 'Aur', name: t('stem.astronomy.auriga', 'Auriga'), english: 'Charioteer', area: 657, brightStar: 'Capella (0.08)', month: 'Jan', hemisphere: 'N', mythology: 'Greek: Erichthonius, son of Hephaestus, inventor of the four-horse chariot. Capella = "she-goat," one of the brightest stars in the Northern Hemisphere. Distinctive pentagon shape.', notable: 'Capella (quadruple star system), M36 + M37 + M38 (rich open clusters)' },
            { abbr: 'CMa', name: t('stem.astronomy.canis_major', 'Canis Major'), english: 'Great Dog', area: 380, brightStar: 'Sirius (-1.46)', month: 'Feb', hemisphere: 'N/S', mythology: 'Greek: Orion\'s hunting dog. Sirius = "scorching" (Greek), the brightest star in the night sky after the Sun. Sirius B is a white dwarf companion, predicted by Bessel in 1844 from astrometric wobble + first observed 1862. Sirius rises with the Sun around late August in the Northern Hemisphere — the original "dog days" of summer.', notable: 'Sirius (brightest night star), Sirius B (first known white dwarf), M41 open cluster' },
            { abbr: 'CMi', name: t('stem.astronomy.canis_minor', 'Canis Minor'), english: 'Small Dog', area: 183, brightStar: 'Procyon (0.34)', month: 'Feb', hemisphere: 'N/S', mythology: 'The smaller of Orion\'s two hunting dogs. Procyon = "before the dog" (it rises before Sirius from northern latitudes). One of the closest naked-eye stars at 11.5 light-years. Procyon B is another nearby white dwarf.', notable: 'Procyon (one of nearest naked-eye stars)' },
            { abbr: 'Lep', name: t('stem.astronomy.lepus', 'Lepus'), english: 'Hare', area: 290, brightStar: 'Arneb (2.58)', month: 'Jan', hemisphere: 'N/S', mythology: 'Greek: a hare hunted by Orion. Direct below Orion in the sky. Small but contains some interesting deep-sky objects + binary stars.', notable: 'M79 (one of few southern-celestial-hemisphere globulars visible at this latitude), R Leporis "Crimson Star"' },
            { abbr: 'Mon', name: t('stem.astronomy.monoceros', 'Monoceros'), english: 'Unicorn', area: 482, brightStar: 'Beta Monocerotis (3.74)', month: 'Feb', hemisphere: 'N/S', mythology: 'Modern constellation (introduced 1612). Contains the Rosette Nebula + Hubble\'s Variable Nebula + the Cone Nebula. Faint to the naked eye but rich in deep-sky targets.', notable: 'Rosette Nebula (NGC 2237), Hubble\'s Variable Nebula, NGC 2244 open cluster' },
            { abbr: 'Cnc', name: t('stem.astronomy.cancer_revisited', 'Cancer (revisited)'), english: '—', area: 506, brightStar: 'Tarf (3.5)', month: 'Mar', hemisphere: 'N', mythology: 'Already listed in zodiac.', notable: 'See zodiac section above' },

            // ─── SOUTHERN HEMISPHERE ───
            { abbr: 'Cru', name: t('stem.astronomy.crux', 'Crux'), english: 'Southern Cross', area: 68, brightStar: 'Acrux (0.77)', month: 'May', hemisphere: 'S', mythology: 'Most iconic Southern Hemisphere constellation. Not visible from northern latitudes above ~ 25°N. Four bright stars + the Coalsack Nebula (a dark molecular cloud) form one of the most recognizable patterns in the southern sky. Featured on flags of Australia, New Zealand, Brazil, Papua New Guinea, Samoa.', notable: 'Smallest of all 88 constellations by area. Coalsack Nebula, Jewel Box Cluster (NGC 4755)' },
            { abbr: 'Cen', name: t('stem.astronomy.centaurus', 'Centaurus'), english: 'Centaur', area: 1060, brightStar: 'Alpha Centauri (-0.27)', month: 'May', hemisphere: 'S', mythology: 'Greek: Chiron the wise centaur, mentor of heroes including Achilles. Alpha Centauri is the closest stellar system to the Sun (4.37 light-years for the binary A+B; Proxima Centauri at 4.24 ly is closer + has confirmed planets). Visible from southern Florida + Hawaii but never high.', notable: 'Alpha Centauri AB (closest naked-eye stars), Proxima Centauri (nearest star), Omega Centauri (largest globular)' },
            { abbr: 'Car', name: t('stem.astronomy.carina', 'Carina'), english: 'Keel', area: 494, brightStar: 'Canopus (-0.74)', month: 'Mar', hemisphere: 'S', mythology: 'Originally part of the giant constellation Argo Navis (the Argonauts\' ship). Lacaille split it into Carina (keel), Vela (sails), + Puppis (poop deck) in 1763. Canopus is the 2nd brightest star in the night sky (after Sirius). Contains the Carina Nebula, one of the largest emission nebulae in our galaxy.', notable: 'Canopus, Carina Nebula (NGC 3372, JWST early-release image), Eta Carinae (variable supergiant)' },
            { abbr: 'Vel', name: t('stem.astronomy.vela', 'Vela'), english: 'Sails', area: 500, brightStar: 'Suhail (1.83)', month: 'Mar', hemisphere: 'S', mythology: 'Another part of the split Argo Navis. Contains the Vela Supernova Remnant + the Vela Pulsar, one of the brightest pulsars in radio + visual.', notable: 'Vela Supernova Remnant, Vela Pulsar' },
            { abbr: 'Pup', name: t('stem.astronomy.puppis', 'Puppis'), english: 'Stern', area: 673, brightStar: 'Naos (2.21)', month: 'Mar', hemisphere: 'S', mythology: 'The third Argo Navis division (poop deck = the stern). Naos is one of the hottest naked-eye stars (~ 40,000 K).', notable: 'Naos (O-class star), M46 + M47 (open clusters)' },
            { abbr: 'Sco', name: t('stem.astronomy.scorpius_revisited', 'Scorpius (revisited)'), english: '—', area: 497, brightStar: 'Antares (1.06)', month: 'Jul', hemisphere: 'N/S', mythology: 'Already listed in zodiac.', notable: 'See zodiac section above' },
            { abbr: 'TrA', name: t('stem.astronomy.triangulum_australe', 'Triangulum Australe'), english: 'Southern Triangle', area: 110, brightStar: 'Atria (1.91)', month: 'Jul', hemisphere: 'S', mythology: 'Modern constellation introduced ~ 1603. Brighter + more distinct than its Northern counterpart Triangulum.', notable: 'NGC 6025 open cluster' },
            { abbr: 'Hyi', name: t('stem.astronomy.hydrus', 'Hydrus'), english: 'Lesser Water Snake', area: 243, brightStar: 'Beta Hydri (2.8)', month: 'Dec', hemisphere: 'S', mythology: 'Introduced by Bayer 1603. Between the two Magellanic Clouds.', notable: 'Near the Small Magellanic Cloud' },
            { abbr: 'Tuc', name: t('stem.astronomy.tucana', 'Tucana'), english: 'Toucan', area: 295, brightStar: 'Alpha Tucanae (2.86)', month: 'Nov', hemisphere: 'S', mythology: 'Introduced by Keyser + de Houtman 1603. Contains the Small Magellanic Cloud + 47 Tucanae (one of the finest globular clusters in the sky, magnitude 4.0).', notable: '47 Tucanae globular cluster, Small Magellanic Cloud (SMC)' },
            { abbr: 'Mus', name: t('stem.astronomy.musca', 'Musca'), english: 'Fly', area: 138, brightStar: 'Alpha Muscae (2.7)', month: 'May', hemisphere: 'S', mythology: 'Originally Apis (Bee), renamed by Lacaille. South of Crux.', notable: 'Small constellation' },
            { abbr: 'Cha', name: t('stem.astronomy.chamaeleon', 'Chamaeleon'), english: 'Chameleon', area: 132, brightStar: 'Alpha Chamaeleontis (4.07)', month: 'Apr', hemisphere: 'S', mythology: 'Introduced 1603. Near the south celestial pole. Faint but contains the Chamaeleon star-forming complex.', notable: 'Chamaeleon dark cloud (active star formation)' },
            { abbr: 'Aps', name: t('stem.astronomy.apus', 'Apus'), english: 'Bird of Paradise', area: 206, brightStar: 'Alpha Apodis (3.83)', month: 'Jul', hemisphere: 'S', mythology: 'Introduced 1603, near south pole. Faint.', notable: '' },
            { abbr: 'Cir', name: t('stem.astronomy.circinus', 'Circinus'), english: 'Compass', area: 93, brightStar: 'Alpha Circini (3.19)', month: 'Jun', hemisphere: 'S', mythology: 'Lacaille\'s drafting-tools constellation, 1763. Adjacent to Centaurus.', notable: 'Circinus Galaxy (Sy 2 AGN)' },
            { abbr: 'Lup', name: t('stem.astronomy.lupus', 'Lupus'), english: 'Wolf', area: 334, brightStar: 'Men (2.30)', month: 'Jun', hemisphere: 'S', mythology: 'Greek: the wolf, sometimes shown impaled on a Centaur\'s spear. Contains rich Milky Way star fields. SN 1006 (the brightest supernova in recorded history, observed worldwide) occurred here.', notable: 'SN 1006 supernova remnant' },
            { abbr: 'Nor', name: t('stem.astronomy.norma', 'Norma'), english: 'Carpenter\'s Square', area: 165, brightStar: 'Gamma Normae (4.0)', month: 'Jul', hemisphere: 'S', mythology: 'Lacaille\'s drafting constellation. Contains the Norma Cluster of galaxies + the Great Attractor (a massive concentration drawing the Local Group toward it).', notable: 'Great Attractor (gravitational anomaly behind the galactic plane)' },
            ...
            { abbr: 'CrA', name: t('stem.astronomy.corona_australis', 'Corona Australis'), english: 'Southern Crown', area: 128, brightStar: 'Alphekka Meridiana (4.1)', month: 'Aug', hemisphere: 'S', mythology: 'Greek: a wreath. South of Sagittarius. Smaller + fainter than Corona Borealis.', notable: 'R Coronae Australis (variable young stellar object)' },
            { abbr: 'Mic', name: t('stem.astronomy.microscopium', 'Microscopium'), english: 'Microscope', area: 210, brightStar: 'Gamma Microscopii (4.67)', month: 'Sep', hemisphere: 'S', mythology: 'Lacaille\'s scientific-instrument constellation. Faint.', notable: '' },
            { abbr: 'PsA', name: t('stem.astronomy.piscis_austrinus', 'Piscis Austrinus'), english: 'Southern Fish', area: 245, brightStar: 'Fomalhaut (1.16)', month: 'Oct', hemisphere: 'S', mythology: 'Greek: a giant fish, parent of the Zodiac Pisces. Fomalhaut = "mouth of the fish" (Arabic). One of the closest A-type main-sequence stars + the host of a famous resolved DEBRIS DISK imaged by Hubble + JWST.', notable: 'Fomalhaut (debris disk visible to Hubble + JWST)' },
            { abbr: 'Gru', name: t('stem.astronomy.grus', 'Grus'), english: 'Crane', area: 366, brightStar: 'Alnair (1.74)', month: 'Oct', hemisphere: 'S', mythology: 'Introduced 1603. Originally part of Piscis Austrinus. The crane is a wading bird, depicted with neck extended.', notable: '' },
            { abbr: 'Pav', name: t('stem.astronomy.pavo', 'Pavo'), english: 'Peacock', area: 378, brightStar: 'Peacock (1.94)', month: 'Aug', hemisphere: 'S', mythology: 'Introduced 1603. Hera\'s sacred bird, often associated with Argus the watcher.', notable: 'NGC 6752 (one of the closer + brighter globular clusters)' },
            { abbr: 'Phe', name: t('stem.astronomy.phoenix', 'Phoenix'), english: 'Phoenix', area: 469, brightStar: 'Ankaa (2.4)', month: 'Nov', hemisphere: 'S', mythology: 'The mythological firebird that rises from its ashes. Introduced 1603.', notable: '' },
            { abbr: 'Eri', name: t('stem.astronomy.eridanus', 'Eridanus'), english: 'River', area: 1138, brightStar: 'Achernar (0.46)', month: 'Dec', hemisphere: 'N/S', mythology: 'Greek: a river — sometimes identified with the Po, sometimes the Nile, sometimes the Euphrates. Achernar = "river\'s end" (Arabic). Very long constellation winding from Orion to the deep south.', notable: 'Achernar (one of the most oblate stars known, spinning rapidly), Eridanus Supervoid' },
            { abbr: 'Hor', name: t('stem.astronomy.horologium', 'Horologium'), english: 'Pendulum Clock', area: 249, brightStar: 'Alpha Horologii (3.85)', month: 'Dec', hemisphere: 'S', mythology: 'Lacaille\'s scientific-instrument constellation. Honoring the pendulum clock invention.', notable: 'Horologium Supercluster' },
            { abbr: 'Ret', name: t('stem.astronomy.reticulum', 'Reticulum'), english: 'Reticle', area: 114, brightStar: 'Alpha Reticuli (3.35)', month: 'Dec', hemisphere: 'S', mythology: 'Lacaille\'s constellation honoring the reticle (crosshairs) used in his measurements of southern star positions.', notable: '' },
            { abbr: 'Dor', name: t('stem.astronomy.dorado', 'Dorado'), english: 'Goldfish / Swordfish', area: 179, brightStar: 'Alpha Doradus (3.30)', month: 'Jan', hemisphere: 'S', mythology: 'Introduced 1603. Sometimes depicted as a swordfish, sometimes the South American mahi-mahi. Contains the LARGE MAGELLANIC CLOUD (LMC) + the Tarantula Nebula.', notable: 'Large Magellanic Cloud (LMC), Tarantula Nebula (30 Doradus)' },
            { abbr: 'Pic', name: t('stem.astronomy.pictor', 'Pictor'), english: 'Painter\'s Easel', area: 247, brightStar: 'Alpha Pictoris (3.27)', month: 'Jan', hemisphere: 'S', mythology: 'Lacaille\'s constellation. Faint. Contains Beta Pictoris, host of an early-resolved debris disk + multiple confirmed exoplanets.', notable: 'Beta Pictoris (resolved debris disk + planets)' },
            { abbr: 'Vol', name: t('stem.astronomy.volans', 'Volans'), english: 'Flying Fish', area: 141, brightStar: 'Beta Volantis (3.77)', month: 'Mar', hemisphere: 'S', mythology: 'Introduced 1603. Faint.', notable: '' },
            { abbr: 'Ant', name: t('stem.astronomy.antlia', 'Antlia'), english: 'Air Pump', area: 239, brightStar: 'Alpha Antliae (4.25)', month: 'Apr', hemisphere: 'S', mythology: 'Lacaille\'s constellation honoring the air pump invented by Boyle. Faint.', notable: 'Antlia Dwarf Galaxy' },
            { abbr: 'Hya', name: t('stem.astronomy.hydra', 'Hydra'), english: 'Water Snake', area: 1303, brightStar: 'Alphard (1.98)', month: 'Apr', hemisphere: 'N/S', mythology: 'Greek: the Lernaean Hydra, the many-headed water serpent slain by Heracles. The LARGEST constellation by area, stretching across ~ 90° of the celestial sphere.', notable: 'M83 (Southern Pinwheel Galaxy), NGC 3242 (Ghost of Jupiter planetary nebula), Alphard ("the solitary")' },
            { abbr: 'Sex', name: t('stem.astronomy.sextans', 'Sextans'), english: 'Sextant', area: 314, brightStar: 'Alpha Sextantis (4.49)', month: 'Apr', hemisphere: 'N/S', mythology: 'Hevelius\' constellation honoring his measuring sextant. Faint.', notable: 'Sextans Dwarf Spheroidal Galaxy' },
            { abbr: 'Crv', name: t('stem.astronomy.corvus', 'Corvus'), english: 'Crow', area: 184, brightStar: 'Gienah (2.59)', month: 'May', hemisphere: 'S', mythology: 'Greek: the crow sent by Apollo to fetch water, but the crow was lazy + brought back a snake (the constellation Hydra) as an excuse. Apollo placed both in the sky as a punishment.', notable: 'Antennae Galaxies (NGC 4038/4039 colliding pair)' },
            { abbr: 'Crt', name: t('stem.astronomy.crater', 'Crater'), english: 'Cup', area: 282, brightStar: 'Delta Crateris (3.56)', month: 'May', hemisphere: 'S', mythology: 'Greek: the cup of Apollo, related to the Corvus story. Faint.', notable: '' },

            // ─── Lacaille\'s scientific-instrument constellations (1763) ───
            { abbr: 'Cae', name: t('stem.astronomy.caelum', 'Caelum'), english: 'Chisel', area: 125, brightStar: 'Alpha Caeli (4.45)', month: 'Jan', hemisphere: 'S', mythology: 'Lacaille\'s sculptor\'s chisel constellation.', notable: '' },
            { abbr: 'Col', name: t('stem.astronomy.columba', 'Columba'), english: 'Dove', area: 270, brightStar: 'Phact (2.65)', month: 'Feb', hemisphere: 'S', mythology: 'Introduced ~ 1592. Sometimes interpreted as Noah\'s dove from the biblical flood. Below Orion + Lepus.', notable: '' },
            { abbr: 'Pyx', name: t('stem.astronomy.pyxis', 'Pyxis'), english: 'Mariner\'s Compass', area: 221, brightStar: 'Alpha Pyxidis (3.68)', month: 'Mar', hemisphere: 'S', mythology: 'Lacaille\'s constellation honoring the ship\'s compass. Originally part of the giant Argo Navis.', notable: 'T Pyxidis (recurrent nova)' },
            { abbr: 'Sct', name: t('stem.astronomy.scutum', 'Scutum'), english: 'Shield', area: 109, brightStar: 'Alpha Scuti (3.85)', month: 'Aug', hemisphere: 'N/S', mythology: 'Introduced by Hevelius 1684, honoring King John III Sobieski\'s shield. Small but contains the Wild Duck Cluster (M11), one of the richest open clusters.', notable: 'Wild Duck Cluster (M11), UY Scuti (one of largest known stars)' },
            { abbr: 'Sge', name: t('stem.astronomy.sagitta', 'Sagitta'), english: 'Arrow', area: 80, brightStar: 'Gamma Sagittae (3.51)', month: 'Aug', hemisphere: 'N', mythology: 'Ancient constellation. Greek: the arrow of various heroes (Heracles, Apollo, Eros). Tiny but recognizable.', notable: 'M71 globular cluster (small but bright)' },
            { abbr: 'Vul', name: t('stem.astronomy.vulpecula', 'Vulpecula'), english: 'Fox', area: 268, brightStar: 'Anser (4.44)', month: 'Sep', hemisphere: 'N', mythology: 'Hevelius\' "Little Fox with Goose." Faint. Contains the Dumbbell Nebula (M27), the first planetary nebula ever discovered.', notable: 'Dumbbell Nebula (M27), first known pulsar (PSR B1919+21, 1967)' },
            { abbr: 'Del', name: t('stem.astronomy.delphinus', 'Delphinus'), english: 'Dolphin', area: 189, brightStar: 'Rotanev (3.63)', month: 'Sep', hemisphere: 'N', mythology: 'Greek: a dolphin honored by Poseidon for finding Amphitrite + persuading her to marry him. Distinctive small diamond shape. "Job\'s Coffin" is its asterism.', notable: 'Distinctive small constellation, easy to find' },
            { abbr: 'Equ', name: t('stem.astronomy.equuleus', 'Equuleus'), english: 'Little Horse', area: 72, brightStar: 'Kitalpha (3.92)', month: 'Sep', hemisphere: 'N', mythology: 'One of the smallest constellations. Greek: a small horse, sometimes Celeris, brother of Pegasus.', notable: 'Faint, second-smallest constellation after Crux' },
            { abbr: 'LMi', name: t('stem.astronomy.leo_minor', 'Leo Minor'), english: 'Little Lion', area: 232, brightStar: 'Praecipua (3.79)', month: 'Apr', hemisphere: 'N', mythology: 'Hevelius\' modern constellation between Leo + Ursa Major.', notable: '' },
            { abbr: 'Lac', name: t('stem.astronomy.lacerta', 'Lacerta'), english: 'Lizard', area: 200, brightStar: 'Alpha Lacertae (3.76)', month: 'Oct', hemisphere: 'N', mythology: 'Hevelius\' modern constellation between Cassiopeia + Pegasus. Faint zigzag of stars.', notable: 'BL Lacertae (prototype of BL Lac AGN objects)' },
            { abbr: 'CVn', name: t('stem.astronomy.canes_venatici', 'Canes Venatici'), english: 'Hunting Dogs', area: 465, brightStar: 'Cor Caroli (2.81)', month: 'May', hemisphere: 'N', mythology: 'Hevelius\' modern constellation. The hunting dogs of Boötes the herdsman. Contains the Whirlpool Galaxy (M51).', notable: 'Whirlpool Galaxy (M51), M3 globular cluster' },
            { abbr: 'Com', name: t('stem.astronomy.coma_berenices', 'Coma Berenices'), english: 'Berenice\'s Hair', area: 386, brightStar: 'Beta Comae Berenices (4.26)', month: 'May', hemisphere: 'N', mythology: 'Faint but recognized since antiquity. Egyptian queen Berenice II offered her hair to the gods for her husband\'s safe return; her hair vanished + was placed in the sky. Contains the COMA CLUSTER of galaxies + many Messier galaxies.', notable: 'Coma Cluster of galaxies, M64 (Black Eye Galaxy), Coma Star Cluster (Mel 111)' },
            { abbr: 'Oph', name: t('stem.astronomy.ophiuchus', 'Ophiuchus'), english: 'Serpent Bearer', area: 948, brightStar: 'Rasalhague (2.08)', month: 'Jul', hemisphere: 'N/S', mythology: 'Greek: Asclepius, the god of medicine, holding the serpent Serpens. The "13th zodiac sign" — the Sun does pass through it (Nov 30 - Dec 17) even though traditional astrology assigns 12. Contains Barnard\'s Star, the second-closest star system at 6 ly + with the largest known proper motion of any star.', notable: 'Barnard\'s Star, M9 + M10 + M12 globular clusters, Kepler\'s Supernova remnant (1604)' },
            { abbr: 'Ser', name: t('stem.astronomy.serpens', 'Serpens'), english: 'Serpent', area: 637, brightStar: 'Unukalhai (2.63)', month: 'Jun', hemisphere: 'N/S', mythology: 'Greek: the serpent held by Ophiuchus. Unique among constellations in being SPLIT INTO TWO PARTS by Ophiuchus passing through: Serpens Caput (head) on the west + Serpens Cauda (tail) on the east.', notable: 'Eagle Nebula (M16, "Pillars of Creation"), Hoag\'s Object (ring galaxy)' },
            { abbr: 'CMa', name: t('stem.astronomy.canis_major_revisited', 'Canis Major (revisited)'), english: '—', area: 380, brightStar: 'Sirius (-1.46)', month: 'Feb', hemisphere: 'N/S', mythology: 'Already listed.', notable: 'See northern section above' },

            // ─── Southern hemisphere (continued) ───
            { abbr: 'Oct', name: t('stem.astronomy.octans', 'Octans'), english: 'Octant', area: 291, brightStar: 'Nu Octantis (3.76)', month: 'Oct', hemisphere: 'S', mythology: 'Lacaille\'s scientific-instrument constellation. Contains the SOUTH CELESTIAL POLE, which has no bright star comparable to Polaris. Sigma Octantis is the dim "south star" at magnitude 5.5.', notable: 'South Celestial Pole (Sigma Octantis is closest visible star)' },
            { abbr: 'Tel', name: t('stem.astronomy.telescopium', 'Telescopium'), english: 'Telescope', area: 252, brightStar: 'Alpha Telescopii (3.51)', month: 'Aug', hemisphere: 'S', mythology: 'Lacaille\'s scientific-instrument constellation honoring the telescope.', notable: '' },
            { abbr: 'Ind', name: t('stem.astronomy.indus', 'Indus'), english: 'Indian', area: 294, brightStar: 'The Persian (3.11)', month: 'Sep', hemisphere: 'S', mythology: 'Introduced 1603. Originally meant to represent a "Native American" — the European voyagers\' imagination of newly-encountered peoples. Honestly: the constellation\'s naming reflects colonial-era European perspectives + has been critiqued.', notable: 'Epsilon Indi (one of nearest stars at 12 ly, ~ similar to Sun)' },
            { abbr: 'Men', name: t('stem.astronomy.mensa', 'Mensa'), english: 'Table Mountain', area: 153, brightStar: 'Alpha Mensae (5.09)', month: 'Jan', hemisphere: 'S', mythology: 'Lacaille\'s constellation honoring Table Mountain near Cape Town, where he made his southern observations. The DIMMEST constellation by brightest-star magnitude. Contains part of the Large Magellanic Cloud.', notable: 'Part of LMC, Mensa Supercluster' },
            { abbr: 'Cae', name: t('stem.astronomy.caelum_revisited', 'Caelum (revisited)'), english: '—', area: 125, brightStar: 'Alpha Caeli (4.45)', month: 'Jan', hemisphere: 'S', mythology: 'Already listed in Lacaille section.', notable: '' },
            { abbr: 'For', name: t('stem.astronomy.fornax', 'Fornax'), english: 'Furnace', area: 398, brightStar: 'Alpha Fornacis (3.85)', month: 'Dec', hemisphere: 'S', mythology: 'Lacaille\'s constellation honoring the chemist\'s furnace. Contains the Fornax Dwarf Galaxy + the Fornax Cluster.', notable: 'Fornax Cluster, NGC 1316 (radio galaxy)' },
            { abbr: 'Scl', name: t('stem.astronomy.sculptor', 'Sculptor'), english: 'Sculptor\'s Studio', area: 475, brightStar: 'Alpha Sculptoris (4.30)', month: 'Nov', hemisphere: 'S', mythology: 'Lacaille\'s constellation honoring the sculptor\'s studio.', notable: 'Sculptor Group (one of closest galaxy groups, ~ 12 million ly)' }
          ];
          ...
          return sectionCard('🗺️ Complete 88-constellation reference',
            h('div', null,
              ...
              h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 10 } },
                ...
                h('input', {
                  type: 'text',
                  placeholder: t('stem.astronomy.search_constellations', 'Search constellations…'),
                  value: search,
                  onChange: function(e) { upd({ constellationSearch: e.target.value }); },
                  'aria-label': t('stem.astronomy.search_constellations_2', 'Search constellations'),
                  className: 'astr-focus',
      ...
      function renderMoon() {
        ...
        return h('div', { style: { padding: 16 } },
          ...
          h('div', { style: { marginBottom: 14 } },
            h('div', { style: { fontSize: 12, color: '#94a3b8', fontWeight: 700, marginBottom: 6 } }, t('stem.astronomy.slide_through_the_lunar_cycle', 'Slide through the lunar cycle:')),
            h('input', {
              type: 'range', min: 0, max: 7, value: d.moonPhaseIdx,
              onChange: function(e) { upd({ moonPhaseIdx: parseInt(e.target.value, 10) }); },
              'aria-label': t('stem.astronomy.moon_phase_position', 'Moon phase position'),
            ...
            h('div', { style: { display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#94a3b8', marginTop: 4 } },
              h('span', null, 'New'),
              h('span', null, t('stem.astronomy.first_quarter', 'First Quarter')),
              h('span', null, t('stem.astronomy.full', 'Full')),
              h('span', null, t('stem.astronomy.last_quarter', 'Last Quarter'))
            )
          ),

          sectionCard('Why the same face always points to us',
            h('div', { style: { fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
              t('stem.astronomy.the_moon_rotates_on_its_own_axis_exact', 'The Moon rotates on its own axis exactly once per orbit around Earth. This "tidal locking" means the same hemisphere always faces us. We never see the "far side" from Earth without a spacecraft. '),
              h('em', { style: { color: '#a5b4fc' } }, t('stem.astronomy.note_it_is_the_far_side_not_the_dark_s', 'Note: it is the far side, NOT the dark side. The far side gets just as much sunlight as the near side — both have day and night as the Moon orbits.'))
            )
          ),

          sectionCard('Earth and Moon to scale',
            h('div', { style: { fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
              t('stem.astronomy.the_moon_is_about_1_4_the_diameter_of_', 'The Moon is about 1/4 the diameter of Earth and 1/80 its mass. At average distance (384,400 km), about 30 Earths would fit between Earth and Moon. The Moon is slowly drifting away — about 3.8 cm per year — and one day will look smaller in our sky than the Sun does, ending total solar eclipses.')
            )
          ),
          ...
          sectionCard('🌖 Features to find on the Moon',
            h('div', null,
              h('p', { style: { margin: '0 0 10px', fontSize: 12.5, color: '#cbd5e1', lineHeight: 1.65 } },
                t('stem.astronomy.a_pair_of_7_50_binoculars_or_any_small', 'A pair of 7×50 binoculars (or any small telescope) reveals enormous detail on the Moon. Best viewing is along the '),
                h('strong', { style: { color: '#fde68a' } }, 'terminator'),
                t('stem.astronomy.the_boundary_between_day_and_night_whe', ' — the boundary between day and night — where shadows make craters dramatic. The terminator is in the middle during first or last quarter. Full moon is too bright + flat (sun overhead = no shadows).')
              ),
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 } },
                [
                  { name: t('stem.astronomy.mare_tranquillitatis', 'Mare Tranquillitatis'), kind: 'Sea (mare)', desc: t('stem.astronomy.the_dark_sea_of_tranquility_easily_vis', 'The dark Sea of Tranquility, easily visible to the naked eye. Apollo 11 landed here in July 1969 at 0.67°N latitude. "One small step" happened on its southwestern shore.'), color: '#94a3b8' },
                  { name: t('stem.astronomy.mare_imbrium', 'Mare Imbrium'), kind: 'Sea (mare)', desc: t('stem.astronomy.the_sea_of_rains_the_largest_dark_feat', 'The Sea of Rains, the largest dark feature on the near side. A basin from a giant impact 3.85 billion years ago, later flooded with basalt lava. About 1,250 km across.'), color: '#94a3b8' },
                  { name: t('stem.astronomy.mare_serenitatis', 'Mare Serenitatis'), kind: 'Sea (mare)', desc: t('stem.astronomy.the_sea_of_serenity_neighboring_tranqu', 'The Sea of Serenity, neighboring Tranquillitatis. Apollo 17 (1972, last crewed lunar mission) landed at its eastern edge.'), color: '#94a3b8' },
                  { name: t('stem.astronomy.oceanus_procellarum', 'Oceanus Procellarum'), kind: 'Sea (mare)', desc: t('stem.astronomy.the_ocean_of_storms_the_largest_mare_o', 'The Ocean of Storms, the largest mare on the Moon. Covers nearly 1/6 of the visible face. Apollo 12 landed here.'), color: '#94a3b8' },
                  { name: t('stem.astronomy.tycho_crater', 'Tycho crater'), kind: 'Bright crater', desc: t('stem.astronomy.young_108_million_years_old_famous_for', 'Young (~108 million years old). Famous for its bright ray system — splash patterns extending 1,500+ km across the surface. Easy to find at full moon in the southern highlands.'), color: '#fde68a' },
                  { name: t('stem.astronomy.copernicus_crater', 'Copernicus crater'), kind: 'Bright crater', desc: t('stem.astronomy.about_800_million_years_old_central_pe', 'About 800 million years old. Central peaks formed by rebound after the impact. Easy binocular target. Often called the "monarch of the Moon" by 19th-century observers.'), color: '#fde68a' },
                  { name: t('stem.astronomy.aristarchus', 'Aristarchus'), kind: 'Brightest spot', desc: t('stem.astronomy.the_brightest_feature_on_the_moon_so_b', 'The brightest feature on the Moon. So bright it can be seen with the naked eye even when shadowed (lit by earthshine). Site of recurring "lunar transient phenomena" — unexplained brief glows.'), color: '#fde68a' },
                  { name: t('stem.astronomy.plato_crater', 'Plato crater'), kind: 'Dark-floored crater', desc: t('stem.astronomy.on_the_northern_shore_of_mare_imbrium_', 'On the northern shore of Mare Imbrium. Unusually dark, smooth floor — flooded with later lava. About 100 km across. A favorite of small-telescope observers.'), color: '#a78bfa' },
                  { name: t('stem.astronomy.lunar_highlands', 'Lunar highlands'), kind: 'Ancient terrain', desc: t('stem.astronomy.the_bright_heavily_cratered_regions_ab', 'The bright, heavily-cratered regions. About 4.4 billion years old — the original lunar crust, predating the maria. Made of anorthosite, an aluminum-rich rock. The "face" you see is partly highlands (white).'), color: '#cbd5e1' }
                ].map(function(f, i) {
          ...
          sectionCard('🌊 Tides + Earth-Moon dynamics',
            (function() {
              var TOPICS = [
                { id: 'why2', name: t('stem.astronomy.why_two_tides_per_day', 'Why TWO tides per day'), color: '#0ea5e9',
                  content: h('div', null,
                    h('p', { style: { margin: '0 0 8px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      t('stem.astronomy.tides_aren_t_caused_by_the_moon_pullin', 'Tides aren\'t caused by the Moon "pulling water up." They\'re caused by DIFFERENTIAL gravity — the Moon pulls slightly harder on the near side of Earth than on the center, and slightly harder on the center than on the far side.')
                    ),
                    h('p', { style: { margin: '0 0 8px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      h('strong', { style: { color: '#7dd3fc' } }, t('stem.astronomy.near_side', 'Near side: ')),
                      t('stem.astronomy.water_pulled_toward_moon_faster_than_t', 'water pulled toward Moon faster than the Earth → bulges out.')
                    ),
                    h('p', { style: { margin: '0 0 8px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      h('strong', { style: { color: '#7dd3fc' } }, t('stem.astronomy.far_side', 'Far side: ')),
                      t('stem.astronomy.water_left_behind_as_earth_itself_gets', 'water left behind as Earth itself gets pulled toward Moon faster → bulges out the other way.')
                    ),
                    h('p', { style: { margin: 0, fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      t('stem.astronomy.earth_rotates_through_both_bulges_in_2', 'Earth rotates THROUGH both bulges in 24 hours, so every point on the equator passes through two tidal highs (and two lows) per day. Tide cycle is actually 24h 50min because the Moon is also moving in its orbit — so the gap between high tides is ~12h 25min.')
                    )
                  )
                },
                { id: 'spring', name: t('stem.astronomy.spring_vs_neap_tides', 'Spring vs neap tides'), color: '#fbbf24',
                  content: h('div', null,
                    h('p', { style: { margin: '0 0 8px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      t('stem.astronomy.the_sun_also_creates_tides_about_half_', 'The Sun also creates tides, about half as strong as the Moon\'s. When Sun + Moon align (new moon OR full moon), their effects add → '),
                      h('strong', { style: { color: '#fbbf24' } }, t('stem.astronomy.spring_tides', 'SPRING TIDES')),
                      t('stem.astronomy.no_relation_to_the_season_from_to_spri', ' (no relation to the season; from "to spring forth"). The highest highs + lowest lows of the month.')
                    ),
                    h('p', { style: { margin: '0 0 8px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      t('stem.astronomy.when_sun_moon_are_at_90_first_or_last_', 'When Sun + Moon are at 90° (first or last quarter), their effects partly cancel → '),
                      h('strong', { style: { color: '#fbbf24' } }, t('stem.astronomy.neap_tides', 'NEAP TIDES')),
                      t('stem.astronomy.the_most_muted_tidal_range_of_the_mont', '. The most muted tidal range of the month.')
                    ),
                    h('p', { style: { margin: 0, fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      t('stem.astronomy.combined_with_moon_s_elliptical_orbit_', 'Combined with Moon\'s elliptical orbit (closer = stronger), spring tides at perigee (perigean spring tides, or "king tides") are the largest of all. Maine\'s Bay of Fundy has tides over 16 m due to its resonant geometry.')
                    )
                  )
                },
                { id: 'locked', name: t('stem.astronomy.tidal_locking', 'Tidal locking'), color: '#a855f7',
                  content: h('div', null,
                    h('p', { style: { margin: '0 0 8px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      t('stem.astronomy.earth_s_gravity_creates_tidal_bulges_i', 'Earth\'s gravity creates tidal bulges in the Moon too (mostly rock, not water). Friction inside the Moon during early history dissipated rotational energy until the Moon\'s rotation period matched its orbital period — about 27.3 days. Now the same face always points to us.')
                    ),
                    h('p', { style: { margin: '0 0 8px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      h('strong', { style: { color: '#d8b4fe' } }, t('stem.astronomy.common_misconception', 'Common misconception: ')),
                      t('stem.astronomy.tidal_locking_does_not_mean_the_moon_d', 'tidal locking does NOT mean the Moon doesn\'t rotate. It rotates exactly once per orbit. We never see the "far side" but the far side gets just as much sunlight as the near side.')
                    ),
                    h('p', { style: { margin: 0, fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      t('stem.astronomy.tidal_locking_is_common_most_large_moo', 'Tidal locking is common — most large moons in the solar system are tidally locked to their planets. Earth-mass planets in red-dwarf habitable zones are likely tidally locked to their stars (one face always lit, one always dark) — creating major climate + biology challenges for habitability.')
                    )
                  )
                },
                { id: 'recede', name: t('stem.astronomy.the_moon_is_leaving', 'The Moon is leaving'), color: '#22c55e',
                  content: h('div', null,
                    h('p', { style: { margin: '0 0 8px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      t('stem.astronomy.earth_s_tidal_bulges_do_not_perfectly_', 'Earth\'s tidal bulges DO not perfectly align with the Moon — Earth\'s rotation drags them ahead. The bulge\'s gravity pulls slightly forward on the Moon, accelerating it. Faster orbit = larger orbit. The Moon recedes by '),
                      h('strong', { style: { color: '#86efac' } }, t('stem.astronomy.3_8_cm_per_year', '~3.8 cm per year')),
                      t('stem.astronomy.measured_precisely_by_laser_ranging_of', ' — measured precisely by laser ranging off retroreflectors the Apollo astronauts placed on the lunar surface.')
                    ),
                    h('p', { style: { margin: '0 0 8px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      h('strong', { style: { color: '#86efac' } }, t('stem.astronomy.conservation_of_angular_momentum', 'Conservation of angular momentum: ')),
                      t('stem.astronomy.whatever_earth_moon_lose_to_friction_h', 'whatever Earth + Moon lose to friction has to go somewhere. Earth\'s rotation is slowing — days are ~1.8 milliseconds longer per century. Hundreds of millions of years ago, days were 22 hours; coral fossils with daily growth bands preserve this record.')
                    ),
                    h('p', { style: { margin: 0, fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      h('strong', { style: { color: '#86efac' } }, t('stem.astronomy.future_consequence', 'Future consequence: ')),
                      t('stem.astronomy.the_moon_will_eventually_appear_too_sm', 'the Moon will eventually appear too small in our sky to cover the Sun. Total solar eclipses will end in ~600 million years. The Moon will recede until Earth\'s rotation matches the lunar orbital period (a "double tidal lock"), but that won\'t happen before the Sun becomes a red giant.')
                    )
                  )
                },
                { id: 'roche', name: t('stem.astronomy.tides_on_other_worlds', 'Tides on other worlds'), color: '#ef4444',
                  content: h('div', null,
                    h('p', { style: { margin: '0 0 8px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      h('strong', { style: { color: '#fca5a5' } }, t('stem.astronomy.jupiter_s_moons', 'Jupiter\'s moons: ')),
                      t('stem.astronomy.io_is_so_tidally_flexed_by_jupiter_tha', 'Io is so tidally flexed by Jupiter that the constant friction makes it the most volcanically active body in the solar system. Hundreds of active volcanoes; surface entirely paved by lava over geological time. Europa\'s subsurface ocean is kept liquid by similar (but smaller) tidal heating.')
                    ),
                    h('p', { style: { margin: '0 0 8px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      h('strong', { style: { color: '#fca5a5' } }, t('stem.astronomy.roche_limit', 'Roche limit: ')),
                      t('stem.astronomy.a_moon_orbiting_too_close_to_its_plane', 'a moon orbiting too close to its planet gets torn apart by tidal forces — the planet\'s gravity pulls harder on the near side than on the far side faster than the moon\'s own gravity can hold it together. Saturn\'s rings may be the remains of a moon that ventured inside the Roche limit, or a small body that never managed to coalesce.')
                    ),
                    h('p', { style: { margin: 0, fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                      h('strong', { style: { color: '#fca5a5' } }, t('stem.astronomy.black_hole_spaghettification', 'Black hole spaghettification: ')),
                      t('stem.astronomy.falling_into_a_small_black_hole_you_wo', 'falling into a small black hole, you would be stretched vertically + compressed horizontally by tidal forces well before reaching the event horizon. For supermassive black holes, the tidal forces at the event horizon are gentle enough that you would cross without immediate notice.')
                    )
                  )
                }
              ];
              ...
              return h('div', null,
                h('p', { style: { margin: '0 0 12px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                  t('stem.astronomy.the_moon_makes_the_tides_but_the_physi', 'The Moon makes the tides — but the physics is subtler than "Moon pulls water." Tidal forces from the Moon shape Earth\'s rotation, its day length, the Moon\'s own orbit, and on a grander scale they sculpt every binary system in the universe.')
                ),
          ...
          sectionCard('Apollo landing sites (all six successful)',
            h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.7 } },
              h('strong', null, t('stem.astronomy.apollo_11', 'Apollo 11 ')), t('stem.astronomy.july_1969_mare_tranquillitatis_armstro', '(July 1969) — Mare Tranquillitatis. Armstrong + Aldrin, the first humans on another world.'), h('br'),
              h('strong', null, t('stem.astronomy.apollo_12', 'Apollo 12 ')), t('stem.astronomy.nov_1969_oceanus_procellarum_conrad_be', '(Nov 1969) — Oceanus Procellarum. Conrad + Bean. Landed within walking distance of the unmanned Surveyor 3 probe; brought pieces back.'), h('br'),
              h('strong', null, t('stem.astronomy.apollo_14', 'Apollo 14 ')), t('stem.astronomy.feb_1971_fra_mauro_highlands_shepard_m', '(Feb 1971) — Fra Mauro highlands. Shepard + Mitchell. (Apollo 13 famously did not land.)'), h('br'),
              h('strong', null, t('stem.astronomy.apollo_15', 'Apollo 15 ')), t('stem.astronomy.jul_aug_1971_hadley_rille_scott_irwin_', '(Jul-Aug 1971) — Hadley Rille. Scott + Irwin. First crewed mission with a lunar rover.'), h('br'),
              h('strong', null, t('stem.astronomy.apollo_16', 'Apollo 16 ')), t('stem.astronomy.apr_1972_descartes_highlands_young_duk', '(Apr 1972) — Descartes highlands. Young + Duke. Highland geology.'), h('br'),
              h('strong', null, t('stem.astronomy.apollo_17', 'Apollo 17 ')), t('stem.astronomy.dec_1972_taurus_littrow_valley_cernan_', '(Dec 1972) — Taurus-Littrow valley. Cernan + Schmitt (only geologist to walk on the Moon). The last people to leave the Moon. The Artemis program plans to return crews starting in the late 2020s.')
            )
          )
        );
      }
      ...
      function renderPlanets() {
        ...
        return h('div', { style: { padding: 16 } },
          h('p', { style: { color: '#cbd5e1', fontSize: 13, marginBottom: 12, lineHeight: 1.6 } },
            t('stem.astronomy.our_solar_system_mercury_venus_mars_ju', 'Our solar system. Mercury, Venus, Mars, Jupiter, and Saturn can all be seen with the naked eye. Uranus is at the edge of naked-eye visibility; Neptune requires a telescope.')
          ),
          ...
          sectionCard('🌀 How the solar system formed — 4.6 billion years ago',
            (function() {
              var STAGES = [
                { id: 'cloud', name: t('stem.astronomy.pre_solar_cloud', 'Pre-solar cloud'), age: '~4.6 Gya', color: '#7c3aed',
                ...
                { id: 'disk', name: t('stem.astronomy.protoplanetary_disk', 'Protoplanetary disk'), age: '~4.6 Gya', color: '#7dd3fc',
                ...
                { id: 'planetesimals', name: t('stem.astronomy.planetesimals', 'Planetesimals'), age: '4.59-4.55 Gya', color: '#fbbf24',
                ...
                { id: 'planets', name: t('stem.astronomy.planet_accretion', 'Planet accretion'), age: '4.55-4.5 Gya', color: '#10b981',
                ...
                { id: 'late', name: t('stem.astronomy.late_heavy_bombardment', 'Late Heavy Bombardment'), age: '~4.1-3.8 Gya', color: '#dc2626',
                ...
                { id: 'now', name: t('stem.astronomy.modern_solar_system', 'Modern solar system'), age: '4.5 Gya — now', color: '#94a3b8',
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 12, borderRadius: 10, background: '#0f172a', borderTop: '1px solid #334155', borderRight: '1px solid #334155', borderBottom: '1px solid #334155', borderLeft: '3px solid ' + sel.color } },
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(56,189,248,0.08)', borderLeft: '3px solid #38bdf8', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#7dd3fc', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.what_happened', 'What happened')),
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(245,158,11,0.08)', borderLeft: '3px solid #f59e0b' } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#fbbf24', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.how_we_know', 'How we know')),
          ...
          sectionCard('🌍 How the Solar System was actually built — 4.6 billion years',
            (function() {
              var SS_TOPICS = [
                ...
                { id: 'differentiate', name: t('stem.astronomy.differentiation_early_earth', 'Differentiation + early Earth'), emoji: '🔥',
                  body: t('stem.astronomy.early_earth_was_molten_heat_sources_gr', 'Early Earth was molten. Heat sources: gravitational potential energy of accretion, radioactive decay of short-lived isotopes (especially ²⁶Al), and giant impacts. The molten Earth DIFFERENTIATED — heavy elements (iron, nickel) sank to form the core; lighter elements (silicates) rose to form the mantle + crust. This took perhaps 30-50 million years. The MAGNETIC FIELD started up as the liquid iron outer core convected. The crust solidified by ~ 4.4 billion years ago (the oldest known zircon mineral grains, from Western Australia, date to that time). Liquid water existed by ~ 4.4 Ga based on oxygen isotope evidence from the same zircons.'),
                ...
                { id: 'lhb', name: t('stem.astronomy.late_heavy_bombardment_nice_model', 'Late Heavy Bombardment + Nice model'), emoji: '☄️',
                  body: t('stem.astronomy.between_4_1_3_8_billion_years_ago_the_', 'Between ~ 4.1-3.8 billion years ago, the inner Solar System was bombarded by a SPIKE of impacts (vs the steady decline that simple accretion would predict). Evidence: lunar crater statistics + Apollo sample impact ages. The NICE MODEL (proposed in 2005 by a team meeting in Nice, France) explains it: Jupiter + Saturn entered a 2:1 orbital resonance, which destabilized the orbits of Uranus + Neptune + sent them outward; this gravitationally scattered the outer-disk planetesimal population, throwing many inward through the inner Solar System. The Late Heavy Bombardment delivered much of Earth\'s water (from icy + water-rich planetesimals) + organic molecules (precursors to life).'),
                ...
                { id: 'asteroids', name: t('stem.astronomy.the_asteroid_belt_kuiper_belt_oort_clo', 'The asteroid belt + Kuiper belt + Oort cloud'), emoji: '🌌',
                  body: t('stem.astronomy.three_populations_of_small_bodies_rema', 'Three populations of small bodies remain from solar-system formation. (1) ASTEROID BELT between Mars + Jupiter — ~ 1 million bodies > 1 km, dominated by silicate + carbon-rich material; Jupiter\'s gravity prevented planet formation here. Most are at 2.2-3.3 AU. (2) KUIPER BELT beyond Neptune (~ 30-50 AU) — icy bodies from the original outer disk, including Pluto + ~ 200 dwarf-planet-class objects + thousands of smaller. The Kuiper Belt is the source of SHORT-PERIOD COMETS (those that orbit Sun in less than 200 years). (3) OORT CLOUD at huge distance (~ 2,000-200,000 AU) — a spherical halo of icy bodies + the source of LONG-PERIOD COMETS. The Oort cloud has never been directly observed; we infer its existence from the orbital characteristics of long-period comets.'),
              ...
              return h('div', null,
                h('div', { style: { fontSize: 12.5, color: '#cbd5e1', lineHeight: 1.65, marginBottom: 12 } },
                  t('stem.astronomy.the_solar_system_took_about_100_millio', 'The Solar System took about 100 million years to mostly form + another billion years to settle. The detailed story has come together from samples (Moon rocks, meteorites, asteroid + comet samples from JAXA + NASA + ESA missions), spacecraft observations (Voyager through New Horizons), modern computer simulations, + observations of protoplanetary disks around other stars. It is a story of physics + chemistry + cosmic accident — and the contingent steps that made Earth habitable.')
                ),
                h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 } },
                ...
                h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.35)' } },
                  ...
                  h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                    h('strong', null, t('stem.astronomy.honest_framing', 'Honest framing: ')), topic.caveat
                  )
                )
              );
            })(),
          ...
          sectionCard('🚀 How we actually visited the Solar System',
            (function() {
              var SC_TOPICS = [
                ...
                { id: 'cassini', name: t('stem.astronomy.cassini_huygens_saturn_1997_2017', 'Cassini-Huygens — Saturn 1997-2017'), emoji: '🪐',
                  body: t('stem.astronomy.cassini_nasa_esa_launched_1997_arrived', 'CASSINI (NASA + ESA, launched 1997, arrived Saturn 2004, ended 2017) was the most successful outer-Solar-System mission ever. It spent 13 YEARS orbiting Saturn, returning 453,000+ images + huge datasets on the planet, rings, moons, magnetosphere. KEY DISCOVERIES: liquid methane lakes on Titan (Huygens probe landing in 2005, the first landing in the outer Solar System); active geysers on Enceladus (water + organics venting from a subsurface ocean — Enceladus is now a top astrobiology target); detailed ring structure + waves; new moons; an enormous hexagonal storm at Saturn\'s north pole. Cassini ended with a deliberate plunge into Saturn\'s atmosphere on September 15, 2017 — to prevent contaminating Enceladus or Titan with Earth microbes if the spacecraft eventually failed + crashed into one of the icy moons.'),
                ...
                { id: 'newhorizons', name: t('stem.astronomy.new_horizons_pluto_beyond', 'New Horizons — Pluto + beyond'), emoji: '🌑',
                  body: t('stem.astronomy.new_horizons_nasa_launched_2006_pluto_', 'NEW HORIZONS (NASA, launched 2006, Pluto flyby July 14 2015) traveled 9 years + 5 billion km to reach Pluto. The mission CHANGED the picture of Pluto from a fuzzy blob into a complex world: smooth ice plains (Sputnik Planitia, the "heart"), 11,000-foot ice mountains, a hazy nitrogen atmosphere, evidence of recent cryovolcanism + possibly a subsurface ocean. Pluto turned out to be GEOLOGICALLY ACTIVE — surprising for a small distant body that should be a frozen dead world. After Pluto, the spacecraft visited the Kuiper Belt Object Arrokoth (2014 MU69, January 2019), a contact-binary "snowman" that gave the first close look at a primordial Kuiper Belt body. New Horizons is now headed out of the Solar System; it is still returning Kuiper Belt data + may study another KBO if one is found along its trajectory.'),
                ...
                { id: 'returns', name: t('stem.astronomy.sample_return_missions', 'Sample-return missions'), emoji: '📦',
                  body: t('stem.astronomy.returning_material_from_another_world_', 'Returning material from another world is the gold standard of Solar System science. SAMPLE RETURNS so far: APOLLO (US, 1969-1972, ~ 382 kg of Moon rocks); LUNA (USSR, 1970-1976, ~ 326 g of Moon material); CHANG\'E 5 (China, 2020, ~ 1.7 kg of Moon rocks from young volcanic terrain); CHANG\'E 6 (2024, FIRST samples from the lunar far side); HAYABUSA (Japan, 2003-2010, microscopic grains from asteroid Itokawa); HAYABUSA 2 (2014-2020, ~ 5g from asteroid Ryugu — including organic molecules + amino acids); OSIRIS-REx (NASA, 2016-2023, ~ 120g from asteroid Bennu — including water + organics); STARDUST (1999-2006, comet Wild 2 dust); GENESIS (2001-2004, solar wind particles).'),
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(225,29,72,0.08)', border: '1px solid rgba(225,29,72,0.35)' } },
                  ...
                  h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.7, marginBottom: 10 } }, topic.body),
                  h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                    h('strong', null, t('stem.astronomy.honest_framing_2', 'Honest framing: ')), topic.caveat
                  )
                ),
        ...
        return h('div', { style: { padding: 16 } },
          sectionCard('🌎 The exoplanet revolution',
            h('div', { style: { fontSize: 13, color: '#e2e8f0', lineHeight: 1.75 } },
              t('stem.astronomy.in_1995_we_knew_of_zero_confirmed_plan', 'In 1995, we knew of zero confirmed planets outside our solar system. As of 2024-2025, we have confirmed roughly '),
              h('strong', { style: { color: '#86efac' } }, EXO_OVERVIEW.knownCount),
              t('stem.astronomy.some_are_larger_than_jupiter_some_are_', '. Some are larger than Jupiter; some are smaller than Earth. Some orbit so close to their stars that the surface is hotter than lava; some orbit in the habitable zone where liquid water could exist. Most stars in our galaxy have at least one planet, and Earth-sized planets in habitable zones are common.')
            ),
            '#22c55e'
          ),
          ...
          sectionCard('🔬 How we find them',
            h('div', { style: { fontSize: 12, color: '#cbd5e1', marginBottom: 10, lineHeight: 1.6 } },
              t('stem.astronomy.exoplanets_are_extraordinarily_hard_to', 'Exoplanets are extraordinarily hard to image directly — even the nearest stars are too dim. Five indirect methods do most of the work:')
            ),
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 } },
              ...
              h('div', { style: { padding: 14, borderRadius: 12, background: '#1e293b', border: '1px solid #334155' } },
                ...
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8, marginBottom: 10 } },
                  h('div', { style: { padding: 8, borderRadius: 6, background: '#0f172a' } },
                    h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' } }, t('stem.astronomy.distance_from_us', 'Distance from us')),
                    h('div', { style: { fontSize: 12, fontWeight: 700, color: '#e2e8f0' } }, selected.distance)
                  ),
                  h('div', { style: { padding: 8, borderRadius: 6, background: '#0f172a' } },
                    h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' } }, t('stem.astronomy.mass', 'Mass')),
                    h('div', { style: { fontSize: 12, fontWeight: 700, color: '#e2e8f0' } }, selected.mass)
                  ),
                  h('div', { style: { padding: 8, borderRadius: 6, background: '#0f172a' } },
                    h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' } }, t('stem.astronomy.year_length', 'Year length')),
                    h('div', { style: { fontSize: 12, fontWeight: 700, color: '#e2e8f0' } }, selected.orbital)
                  ),
                  h('div', { style: { padding: 8, borderRadius: 6, background: '#0f172a' } },
                    h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' } }, t('stem.astronomy.habitability', 'Habitability')),
                ...
                h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.10)', borderLeft: '3px solid #22c55e', marginBottom: 8 } },
                  h('div', { style: { fontSize: 11, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.astronomy.why_it_matters', 'Why it matters')),
                  h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.65 } }, selected.story)
                ),
                h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(56,189,248,0.10)', borderLeft: '3px solid #38bdf8' } },
                  h('div', { style: { fontSize: 11, fontWeight: 800, color: '#7dd3fc', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.astronomy.how_we_know_about_it', 'How we know about it')),
            ...
            (function() {
              ...
              function transitSvg() {
                ...
                return h('svg', { viewBox: '0 0 ' + svgW + ' ' + svgH, width: '100%', height: svgH, role: 'img', 'aria-labelledby': 'transitTitle transitDesc' },
                  h('title', { id: 'transitTitle' }, t('stem.astronomy.exoplanet_transit_simulation', 'Exoplanet transit simulation')),
              ...
              function lightCurveSvg() {
                ...
                return h('svg', { viewBox: '0 0 ' + svgW + ' ' + svgH, width: '100%', height: svgH, role: 'img', 'aria-label': t('stem.astronomy.light_curve_showing_brightness_dips_du', 'Light curve showing brightness dips during transits') },
                  ...
                  // Axis
                  h('text', { x: padX + plotW / 2, y: padY + plotH + 14, textAnchor: 'middle', fill: '#94a3b8', fontSize: 10 }, t('stem.astronomy.time', 'Time →')),
                  // Two transit labels
                  h('text', { x: padX + plotW * 0.25, y: padY + plotH + 14, textAnchor: 'middle', fill: '#fbbf24', fontSize: 9 }, t('stem.astronomy.transit_1', 'transit 1')),
                  h('text', { x: padX + plotW * 0.75, y: padY + plotH + 14, textAnchor: 'middle', fill: '#fbbf24', fontSize: 9 }, t('stem.astronomy.transit_2', 'transit 2'))
                );
              }

              return h('div', null,
                h('p', { style: { margin: '0 0 10px', fontSize: 12.5, color: '#cbd5e1', lineHeight: 1.65 } },
                  t('stem.astronomy.the_transit_method_kepler_tess_75_of_a', 'The transit method (Kepler, TESS, ~75% of all confirmed exoplanets) works by watching a star\'s brightness over time. When a planet passes between us and its star, the star dims by a tiny amount equal to '),
                  h('strong', { style: { color: '#fbbf24' } }, t('stem.astronomy.r_planet_r_star', '(R_planet / R_star)²')),
                  t('stem.astronomy.each_transit_produces_a_characteristic', '. Each transit produces a characteristic flat-bottomed dip. The depth tells us the planet\'s SIZE; the period (time between transits) tells us its ORBIT.')
                ),
                ...
                h('div', { style: { padding: 10, borderRadius: 10, background: '#0a0e1a', border: '1px solid #334155', marginBottom: 12 } },
                  h('div', { style: { fontSize: 11, color: '#94a3b8', marginBottom: 4 } }, t('stem.astronomy.brightness_over_time_two_consecutive_t', 'Brightness over time (two consecutive transits):')),
                  lightCurveSvg()
                ),

                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 10, marginBottom: 12 } },
                  [
                    { label: t('stem.astronomy.planet_size_earth_radii', 'Planet size (Earth radii)'), value: planetR, min: 0.3, max: 12, step: 0.1, key: 'transitPlanetR' },
                    { label: t('stem.astronomy.star_size_sun_radii', 'Star size (Sun radii)'), value: starR, min: 0.3, max: 3.0, step: 0.1, key: 'transitStarR' },
                    { label: t('stem.astronomy.transit_position', 'Transit position'), value: time, min: 0, max: 1, step: 0.02, key: 'transitTime' },
                    { label: t('stem.astronomy.impact_parameter_0_center_1_grazing', 'Impact parameter (0=center, 1=grazing)'), value: impactB, min: 0, max: 1.2, step: 0.05, key: 'transitImpact' }
                  ].map(function(s, i) {
                ...
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 8, marginBottom: 12 } },
                  h('div', { style: { padding: 8, borderRadius: 6, background: '#0f172a', border: '1px solid #334155' } },
                    h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' } }, t('stem.astronomy.transit_depth', 'Transit depth')),
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: '#0f172a', border: '1px solid #334155' } },
                    h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' } }, t('stem.astronomy.naked_eye', 'Naked eye')),
                    h('div', { style: { fontSize: 14, fontWeight: 800, color: detEye ? '#86efac' : '#fca5a5', marginTop: 2 } }, detEye ? '✓ visible' : '✗ undetectable'),
                    h('div', { style: { fontSize: 10, color: '#94a3b8', marginTop: 2 } }, t('stem.astronomy.requires_1_dip', 'requires ≥1% dip'))
                  ),
                  h('div', { style: { padding: 8, borderRadius: 6, background: '#0f172a', border: '1px solid #334155' } },
                    h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' } }, t('stem.astronomy.ground_telescope', 'Ground telescope')),
                    h('div', { style: { fontSize: 14, fontWeight: 800, color: detGround ? '#86efac' : '#fca5a5', marginTop: 2 } }, detGround ? '✓ detectable' : '✗ too small'),
                    h('div', { style: { fontSize: 10, color: '#94a3b8', marginTop: 2 } }, t('stem.astronomy.requires_0_1_1000_ppm', 'requires ≥0.1% / 1000 ppm'))
                  ),
                  h('div', { style: { padding: 8, borderRadius: 6, background: '#0f172a', border: '1px solid #334155' } },
                    h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' } }, t('stem.astronomy.kepler_space_telescope', 'Kepler space telescope')),
                    h('div', { style: { fontSize: 14, fontWeight: 800, color: detKepler ? '#86efac' : '#fca5a5', marginTop: 2 } }, detKepler ? '✓ detectable' : '✗ below precision'),
                    h('div', { style: { fontSize: 10, color: '#94a3b8', marginTop: 2 } }, t('stem.astronomy.requires_20_ppm', 'requires ≥20 ppm'))
                  )
                ),
                ...
                h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.10)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 12, color: '#c7d2fe', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.try_these', 'Try these: ')),
                  h('ul', { style: { margin: '6px 0 0 22px', padding: 0, lineHeight: 1.65 } },
                    h('li', null, t('stem.astronomy.a_jupiter_sized_planet_11_earth_radii_', 'A Jupiter-sized planet (~11 Earth radii) around a Sun-sized star: depth ~1%. Big enough to (barely) see by naked eye.')),
                    h('li', null, t('stem.astronomy.an_earth_sized_planet_around_a_sun_siz', 'An Earth-sized planet around a Sun-sized star: depth ~0.0084% (84 ppm). Kepler detected, ground-based cannot.')),
                    h('li', null, t('stem.astronomy.an_earth_sized_planet_around_a_red_dwa', 'An Earth-sized planet around a red dwarf (R ≈ 0.15 Sun radii): depth ~0.37%. Easily detectable. This is why most habitable-zone Earth-sized exoplanets we know orbit red dwarfs.'))
                  )
                )
              );
            })(),
          ...
          sectionCard('🧬 Astrobiology — the search for life elsewhere',
            (function() {
              var TARGETS = [
                {
                  id: 'mars', name: t('stem.astronomy.mars', 'Mars'), what: 'Best-explored extraterrestrial target.', confidence: 'past life: possible / present: unlikely',
                ...
                {
                  id: 'europa', name: t('stem.astronomy.europa_jupiter_moon', 'Europa (Jupiter moon)'), what: 'Best candidate for present-day life in our solar system.', confidence: 'present-day microbial life: plausible',
                ...
                {
                  id: 'enceladus', name: t('stem.astronomy.enceladus_saturn_moon', 'Enceladus (Saturn moon)'), what: 'Small icy moon with a habitable ocean.', confidence: 'present-day microbial life: plausible',
                ...
                {
                  id: 'titan', name: t('stem.astronomy.titan_saturn_moon', 'Titan (Saturn moon)'), what: 'A world with weather + rivers + seas — of liquid methane.', confidence: 'life as we know it: unlikely / life as we don\'t: possible',
                ...
                {
                  id: 'exo', name: t('stem.astronomy.exoplanet_atmospheres', 'Exoplanet atmospheres'), what: 'Look for "biosignatures" in light from exoplanet atmospheres.', confidence: 'currently inconclusive; technique is rapidly improving',
              ...
              return h('div', null,
                h('p', { style: { margin: '0 0 12px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                  t('stem.astronomy.are_we_alone_it_is_among_the_oldest_hu', 'Are we alone? It is among the oldest human questions and one we now have tools to investigate. Astrobiology is the scientific search for life beyond Earth — past, present, microbial, or technological.')
                ),
                ...
                h('div', { style: { padding: 12, borderRadius: 10, background: '#0f172a', borderTop: '1px solid #334155', borderRight: '1px solid #334155', borderBottom: '1px solid #334155', borderLeft: '3px solid ' + sel.color, marginBottom: 14 } },
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(56,189,248,0.08)', borderLeft: '3px solid #38bdf8', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#7dd3fc', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.why_we_look_here', 'Why we look here')),
                    h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.6 } }, sel.why)
                  ),
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(245,158,11,0.08)', borderLeft: '3px solid #f59e0b' } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#fbbf24', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.missions', 'Missions')),
                ...
                h('div', { style: { padding: 14, borderRadius: 12, background: '#1e1b4b', border: '1px solid #6366f1', marginBottom: 12 } },
                  h('div', { style: { fontSize: 14, fontWeight: 800, color: '#c7d2fe', marginBottom: 8 } }, t('stem.astronomy.the_drake_equation', '🔢 The Drake Equation')),
                  h('p', { style: { margin: '0 0 10px', fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.65 } },
                    t('stem.astronomy.frank_drake_1961_proposed_an_equation_', 'Frank Drake (1961) proposed an equation to estimate the number of communicating civilizations in our galaxy. The equation is a thinking tool, not a measurement — we have huge uncertainty on several factors. Try different values:')
                  ),
                  h('div', { style: { padding: 10, borderRadius: 8, background: '#0a0e1a', textAlign: 'center', fontFamily: 'ui-monospace, monospace', fontSize: 13, color: '#c7d2fe', marginBottom: 10 } },
                    t('stem.astronomy.n_r_f_p_n_e_f_l_f_i_f_c_l', 'N = R★ × f_p × n_e × f_l × f_i × f_c × L')
                  ),
                  h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 8, marginBottom: 10 } },
                    [
                      { label: t('stem.astronomy.f_p_fraction_with_planets', 'f_p: fraction with planets'), value: fp, min: 0, max: 1, step: 0.05, key: 'drakeFp', sub: 'Confirmed: ~1.0' },
                      { label: t('stem.astronomy.n_e_habitable_planets_system', 'n_e: habitable planets/system'), value: ne, min: 0, max: 5, step: 0.1, key: 'drakeNe', sub: 'Kepler: ~0.2-0.6' },
                      { label: t('stem.astronomy.f_l_fraction_that_develop_life', 'f_l: fraction that develop life'), value: fl, min: 0, max: 1, step: 0.05, key: 'drakeFl', sub: 'Unknown! 1 example (us)' },
                      { label: t('stem.astronomy.f_i_fraction_that_go_intelligent', 'f_i: fraction that go intelligent'), value: fi, min: 0, max: 1, step: 0.01, key: 'drakeFi', sub: 'Unknown' },
                      { label: t('stem.astronomy.f_c_fraction_that_communicate', 'f_c: fraction that communicate'), value: fc, min: 0, max: 1, step: 0.05, key: 'drakeFc', sub: 'Unknown' },
                      { label: t('stem.astronomy.l_civ_lifetime_years', 'L: civ. lifetime (years)'), value: dlife, min: 100, max: 1000000, step: 100, key: 'drakeL', sub: 'Unknown — pessimist: 100, optimist: 10⁶+' }
                    ].map(function(s, i) {
                  ...
                  h('div', { style: { padding: 10, borderRadius: 6, background: 'rgba(99,102,241,0.20)', border: '1px solid #6366f1', textAlign: 'center' } },
                    h('div', { style: { fontSize: 11, color: '#94a3b8' } }, t('stem.astronomy.estimated_communicating_civilizations_', 'Estimated communicating civilizations in our galaxy right now:')),
                    ...
                    h('div', { style: { fontSize: 11, color: '#94a3b8' } }, t('stem.astronomy.for_reference_the_milky_way_has_400_bi', 'For reference: the Milky Way has ~400 billion stars'))
                  )
                ),
          ...
          sectionCard('🪐 The habitable zone — and its honest limits',
            h('div', { style: { fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
              h('p', { style: { margin: '0 0 8px' } }, t('stem.astronomy.the_habitable_zone_is_the_orbit_range_', 'The habitable zone is the orbit range where a rocky planet could have liquid water on its surface — given an Earth-like atmosphere. Sometimes called the "Goldilocks zone": not too hot, not too cold.')),
              h('p', { style: { margin: '0 0 8px' } }, t('stem.astronomy.but_the_habitable_zone_is_a_starting_p', 'But the habitable zone is a starting point, not an answer. A planet in the habitable zone might:')),
              h('ul', { style: { margin: 0, padding: '0 0 0 22px', lineHeight: 1.7 } },
                h('li', null, t('stem.astronomy.have_no_atmosphere_like_mercury_mostly', 'Have no atmosphere (like Mercury, mostly).')),
                h('li', null, t('stem.astronomy.have_a_runaway_greenhouse_atmosphere_l', 'Have a runaway greenhouse atmosphere (like Venus).')),
                h('li', null, t('stem.astronomy.be_tidally_locked_to_a_red_dwarf_with_', 'Be tidally locked to a red dwarf, with one side always hot and the other always frozen.')),
                h('li', null, t('stem.astronomy.get_blasted_by_stellar_flares_red_dwar', 'Get blasted by stellar flares (red dwarf hosts are flaring stars).')),
                h('li', null, t('stem.astronomy.have_life_and_we_have_no_good_way_to_k', 'Have life, and we have no good way to know yet.'))
              ),
              h('p', { style: { margin: '8px 0 0' } }, t('stem.astronomy.jwst_and_future_missions_habitable_wor', 'JWST and future missions (Habitable Worlds Observatory, ARIEL) are now probing exoplanet atmospheres for biosignatures: gas combinations like oxygen + methane that, on Earth, only persist because life keeps producing them.'))
            ),
            '#22c55e'
          ),
          ...
          sectionCard('🔬 JWST exoplanet atmosphere spectroscopy',
            (function() {
              ...
              return h('div', null,
                h('div', { style: { fontSize: 12.5, color: '#cbd5e1', lineHeight: 1.65, marginBottom: 12 } },
                  t('stem.astronomy.the_james_webb_space_telescope_jwst_la', 'The James Webb Space Telescope (JWST, launched Dec 25, 2021) was designed largely to study early-universe galaxies — but its biggest impact so far may turn out to be exoplanet atmospheres. By using transmission spectroscopy + thermal emission, JWST is, for the first time, building a real chemical inventory of worlds around other stars.')
                ),
                h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 } },
                ...
                h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.35)' } },
                  ...
                  h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.7, marginBottom: 10 } }, topic.body),
                  h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                    h('strong', null, t('stem.astronomy.honest_framing_3', 'Honest framing: ')), topic.caveat
                  )
                )
              );
            })(),
          ...
          sectionCard('🛸 The Fermi paradox + SETI — where is everybody?',
            (function() {
              var FP_TOPICS = [
                { id: 'fermi', name: t('stem.astronomy.the_fermi_paradox_itself', 'The Fermi paradox itself'), emoji: '❓',
                  body: t('stem.astronomy.in_1950_during_a_lunch_conversation_at', 'In 1950, during a lunch conversation at Los Alamos, physicist Enrico Fermi reportedly asked "But where is everybody?" The argument: the galaxy is ~10 billion years old. Even at a small fraction of the speed of light, a civilization could colonize the entire Milky Way in ~10 million years — a tiny fraction of cosmic time. If there are many intelligent civilizations, the galaxy should be VISIBLY occupied, with engineered megastructures, signals, or visits. Instead we see silence. This contradiction between "there should be many civilizations" + "we see none" is the Fermi paradox.'),
                ...
                { id: 'drake', name: t('stem.astronomy.the_drake_equation_2', 'The Drake equation'), emoji: '📐',
                  body: t('stem.astronomy.frank_drake_1961_wrote_down_an_equatio', 'Frank Drake (1961) wrote down an equation to estimate the number of communicating civilizations in our galaxy: N = R* × f_p × n_e × f_l × f_i × f_c × L. Rate of star formation × fraction with planets × planets per system in habitable zone × fraction that develop life × fraction with intelligence × fraction that develop communicating technology × lifetime of communicating phase. Modern values for the first three factors are reasonably well-known (R* ~ 1-3 stars/yr, f_p ~ 0.5+, n_e ~ 0.1-0.4 from exoplanet surveys). The last four are essentially unknown — they could each be anywhere from 0.001 to ~1. Plug in optimistic values: thousands of civilizations. Pessimistic values: we are alone in the galaxy.'),
                ...
                { id: 'wow', name: t('stem.astronomy.notable_signals', 'Notable signals'), emoji: '📻',
                  body: t('stem.astronomy.the_wow_signal_1977_big_ear_radio_obse', 'The WOW! signal (1977, Big Ear Radio Observatory, Ohio) was a 72-second strong narrow-band radio burst at 1420 MHz (the hydrogen line frequency, often suggested as a natural SETI channel) that has never been explained or repeated. It remains the strongest candidate signal in SETI history. More recent: BLC1 (2019, detected by Breakthrough Listen toward Proxima Centauri, 982 MHz) generated headlines + was eventually traced to terrestrial RFI. Tabby\'s Star / KIC 8462852 (Boyajian 2015) showed unprecedented irregular dimming that briefly invited speculation about Dyson swarms; explanations now favor dust + comet families. Each case has driven new analysis methods + community RFI-mitigation work.'),
                ...
                { id: 'meti', name: t('stem.astronomy.should_we_broadcast_back_meti', 'Should we broadcast back? (METI)'), emoji: '📤',
                  body: t('stem.astronomy.active_meti_messaging_extraterrestrial', 'Active METI (Messaging Extraterrestrial Intelligence) deliberately broadcasts signals INTO space rather than just listening. The Arecibo Message (1974, 3-minute pictogram aimed at globular cluster M13) was the first major active METI attempt. Several METI projects have run since (Cosmic Call 1999 + 2003, A Message From Earth 2008, Lone Signal 2013, others). The METI debate is genuinely contentious. Proponents (some at METI International) argue: silence guarantees we never communicate, and a civilization advanced enough to receive us is advanced enough to find us already. Opponents (Stephen Hawking, the Asilomar SETI Protocol signatories) warn: advertising our presence + location to an unknown audience may be catastrophically unwise; we have no way to assess risk.'),
            ...
            h('div', { style: { padding: 14, borderRadius: 12, background: '#0a0e1a', border: '1px solid #334155', textAlign: 'center' } },
              h('svg', { viewBox: '-130 -130 260 260', width: 260, height: 260, role: 'img', 'aria-labelledby': 'seasonSvgTitle seasonSvgDesc' },
                ...
                h('g', { transform: 'translate(' + earthX + ',' + earthY + ')' },
                  ...
                  h('text', { x: 0, y: 25, textAnchor: 'middle', fill: '#bfdbfe', fontSize: 10 }, t('stem.astronomy.earth', 'Earth'))
                )
              )
            ),
            h('div', null,
              ...
              h('input', {
                ...
                'aria-label': t('stem.astronomy.month_of_year', 'Month of year'),
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: '#1e293b', border: '1px solid #334155', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, color: '#94a3b8', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.astronomy.northern_hemisphere', 'Northern Hemisphere')),
                h('div', { style: { fontSize: 14, fontWeight: 800, color: '#fde68a' } }, nSeason)
              ),
              h('div', { style: { padding: 10, borderRadius: 8, background: '#1e293b', border: '1px solid #334155', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, color: '#94a3b8', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.astronomy.southern_hemisphere', 'Southern Hemisphere')),
          ...
          sectionCard('Why the tilt matters more than the distance',
            h('div', null,
              h('p', { style: { margin: '0 0 8px', color: '#e2e8f0', fontSize: 13, lineHeight: 1.7 } },
                t('stem.astronomy.when_sunlight_hits_earth_at_a_steep_an', 'When sunlight hits Earth at a steep angle (overhead), it concentrates on a small area, heating it more. At a shallow angle, the same sunlight spreads over a larger area, heating each spot less. The 23.5° tilt means each hemisphere takes turns getting the steep-angle sunlight.')
              ),
              h('p', { style: { margin: 0, color: '#e2e8f0', fontSize: 13, lineHeight: 1.7 } },
                t('stem.astronomy.the_tilt_also_affects_day_length_in_su', 'The tilt also affects day length. In summer, the tipped-toward hemisphere has long days; in winter, short days. Near the equator, this effect is small (days stay close to 12 hours year-round). Near the poles, it\'s huge — summer day length goes to 24 hours, winter to zero.')
              )
            )
          )
        );
      ...
      function renderStars() {
        ...
        var HR_STARS = [
          ...
          { name: t('stem.astronomy.spica', 'Spica'),     class: 'B', tempK: 22400, absMag: -3.6, group: 'main' },
          { name: t('stem.astronomy.rigel', 'Rigel'),     class: 'B', tempK: 12100, absMag: -7.0, group: 'super' },
          { name: t('stem.astronomy.vega', 'Vega'),      class: 'A', tempK: 9602,  absMag: 0.58, group: 'main' },
          { name: t('stem.astronomy.sirius_a', 'Sirius A'),  class: 'A', tempK: 9940,  absMag: 1.42, group: 'main' },
          { name: t('stem.astronomy.procyon', 'Procyon'),   class: 'F', tempK: 6530,  absMag: 2.66, group: 'main' },
          { name: 'Sun',       class: 'G', tempK: 5778,  absMag: 4.83, group: 'main' },
          { name: t('stem.astronomy.alpha_cen_a', 'Alpha Cen A'), class: 'G', tempK: 5790, absMag: 4.38, group: 'main' },
          { name: t('stem.astronomy.alpha_cen_b', 'Alpha Cen B'), class: 'K', tempK: 5260, absMag: 5.71, group: 'main' },
          { name: t('stem.astronomy.proxima_cen', 'Proxima Cen'), class: 'M', tempK: 3042, absMag: 15.6, group: 'main' },
          // Red giants (upper right)
          { name: t('stem.astronomy.arcturus', 'Arcturus'),  class: 'K', tempK: 4286,  absMag: -0.30, group: 'giant' },
          { name: t('stem.astronomy.aldebaran', 'Aldebaran'), class: 'K', tempK: 3910,  absMag: -0.63, group: 'giant' },
          // Supergiants (upper)
          { name: t('stem.astronomy.betelgeuse', 'Betelgeuse'),class: 'M', tempK: 3500,  absMag: -5.85, group: 'super' },
          { name: t('stem.astronomy.antares', 'Antares'),   class: 'M', tempK: 3660,  absMag: -5.28, group: 'super' },
          { name: t('stem.astronomy.deneb', 'Deneb'),     class: 'A', tempK: 8525,  absMag: -8.38, group: 'super' },
          // White dwarfs (lower left)
          { name: t('stem.astronomy.sirius_b', 'Sirius B'),  class: 'A', tempK: 25200, absMag: 11.18, group: 'dwarf' },
          { name: t('stem.astronomy.procyon_b', 'Procyon B'), class: 'A', tempK: 7740,  absMag: 13.04, group: 'dwarf' }
        ];
        ...
        function hrDiagram() {
          ...
          return h('svg', { viewBox: '0 0 ' + svgW + ' ' + svgH, width: '100%', height: svgH, role: 'img', 'aria-labelledby': 'hrTitle hrDesc' },
            h('title', { id: 'hrTitle' }, t('stem.astronomy.hertzsprung_russell_diagram', 'Hertzsprung-Russell diagram')),
            h('desc', { id: 'hrDesc' }, t('stem.astronomy.scatter_plot_of_16_real_stars_showing_', 'Scatter plot of 16 real stars showing surface temperature (x-axis, hotter on left) versus absolute magnitude (y-axis, brighter on top). Main sequence stars form a band from upper-left to lower-right. Red giants in upper right, white dwarfs in lower left, supergiants along the top.')),
            // Plot area background
            h('rect', { x: padLeft, y: padTop, width: plotW, height: plotH, fill: '#0a0e1a', stroke: '#334155', strokeWidth: 1 }),
            // Y axis label
            h('text', { x: 16, y: padTop + plotH / 2, transform: 'rotate(-90 16 ' + (padTop + plotH / 2) + ')', textAnchor: 'middle', fill: '#94a3b8', fontSize: 11 }, t('stem.astronomy.absolute_magnitude_brighter', 'Absolute magnitude (brighter ↑)')),
            // X axis label
            h('text', { x: padLeft + plotW / 2, y: svgH - 18, textAnchor: 'middle', fill: '#94a3b8', fontSize: 11 }, t('stem.astronomy.surface_temperature_k_hotter', 'Surface temperature (K) — hotter ←')),
            ...
            // Region labels
            h('text', { x: tx(8000), y: ty(2), textAnchor: 'middle', fill: '#fbbf24', fontSize: 11, fontStyle: 'italic', fontWeight: 700 }, t('stem.astronomy.main_sequence', 'Main sequence')),
            h('text', { x: tx(4000), y: ty(-2), textAnchor: 'middle', fill: '#fda4af', fontSize: 11, fontStyle: 'italic', fontWeight: 700 }, t('stem.astronomy.red_giants', 'Red giants')),
            h('text', { x: tx(5000), y: ty(-7.5), textAnchor: 'middle', fill: '#f87171', fontSize: 11, fontStyle: 'italic', fontWeight: 700 }, t('stem.astronomy.supergiants', 'Supergiants')),
            h('text', { x: tx(12000), y: ty(12), textAnchor: 'middle', fill: '#bfdbfe', fontSize: 11, fontStyle: 'italic', fontWeight: 700 }, t('stem.astronomy.white_dwarfs', 'White dwarfs')),
            // Stars
            HR_STARS.map(function(s, i) {
              ...
              return h('g', { key: 'star_' + i },
                ...
                s.name === 'Sun' ? h('text', { x: x + 8, y: y + 4, fill: '#fde68a', fontSize: 10, fontWeight: 800 }, t('stem.astronomy.sun', '☉ Sun')) : null,
                s.name === 'Betelgeuse' ? h('text', { x: x + 10, y: y + 4, fill: '#fbbf24', fontSize: 9 }, t('stem.astronomy.betelgeuse_2', 'Betelgeuse')) : null,
                s.name === 'Sirius A' ? h('text', { x: x + 8, y: y + 4, fill: '#cbd5e1', fontSize: 9 }, t('stem.astronomy.sirius', 'Sirius')) : null,
                s.name === 'Proxima Cen' ? h('text', { x: x + 6, y: y - 4, fill: '#fbbf24', fontSize: 9 }, t('stem.astronomy.proxima', 'Proxima')) : null,
                s.name === 'Vega' ? h('text', { x: x + 8, y: y - 2, fill: '#cbd5e1', fontSize: 9 }, t('stem.astronomy.vega_2', 'Vega')) : null,
                s.name === 'Arcturus' ? h('text', { x: x + 8, y: y - 2, fill: '#fbbf24', fontSize: 9 }, t('stem.astronomy.arcturus_2', 'Arcturus')) : null,
                s.name === 'Rigel' ? h('text', { x: x + 10, y: y + 4, fill: '#cbd5e1', fontSize: 9 }, t('stem.astronomy.rigel_2', 'Rigel')) : null,
                s.name === 'Deneb' ? h('text', { x: x + 10, y: y + 4, fill: '#cbd5e1', fontSize: 9 }, t('stem.astronomy.deneb_2', 'Deneb')) : null
              );
            })
          );
        }
        ...
        return h('div', { style: { padding: 16 } },
          ...
          sectionCard('🌟 The Hertzsprung-Russell Diagram',
            h('div', null,
              h('p', { style: { margin: '0 0 10px', fontSize: 12.5, color: '#cbd5e1', lineHeight: 1.65 } },
                t('stem.astronomy.when_astronomers_ejnar_hertzsprung_and', 'When astronomers Ejnar Hertzsprung and Henry Norris Russell independently plotted star temperature against brightness around 1910, they discovered stars aren\'t scattered randomly — they cluster into a few distinct groups. The pattern is the foundation of stellar astrophysics. Each region tells you what stage of life a star is in.')
              ),
              hrDiagram(),
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 8, marginTop: 12 } },
                [
                  { name: t('stem.astronomy.main_sequence_2', 'Main Sequence'), desc: t('stem.astronomy.90_of_all_stars_hydrogen_fusion_in_the', '~90% of all stars. Hydrogen fusion in the core. The Sun is here. Hotter = brighter + bigger + shorter-lived.'), color: '#fbbf24' },
                  { name: t('stem.astronomy.red_giants_2', 'Red Giants'), desc: t('stem.astronomy.stars_that_have_exhausted_core_hydroge', 'Stars that have exhausted core hydrogen. Outer layers expand + cool. The Sun will become one in ~5 billion years.'), color: '#fda4af' },
                  { name: t('stem.astronomy.supergiants_2', 'Supergiants'), desc: t('stem.astronomy.the_most_massive_stars_in_their_final_', 'The most massive stars in their final stage. Will end as supernovae. Betelgeuse, Rigel, Deneb.'), color: '#f87171' },
                  { name: t('stem.astronomy.white_dwarfs_2', 'White Dwarfs'), desc: t('stem.astronomy.the_cooling_core_of_a_dead_sun_like_st', 'The cooling core of a dead Sun-like star. Earth-sized but as massive as the Sun. Eventually fades to a black dwarf (none exist yet — the universe is too young).'), color: '#bfdbfe' }
                ].map(function(r, i) {
          ...
          h('div', { style: { padding: 14, borderRadius: 12, background: '#0a0e1a', border: '1px solid #334155', marginBottom: 14, overflowX: 'auto' } },
            h('div', { style: { fontSize: 11, color: '#94a3b8', fontWeight: 700, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 } }, t('stem.astronomy.pick_a_spectral_class_to_learn_more', 'Pick a spectral class to learn more')),
          ...
          selected ? h('div', { style: { padding: 14, borderRadius: 12, background: '#1e293b', border: '1px solid #334155' } },
            ...
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 8, marginBottom: 12 } },
              h('div', { style: { padding: 8, borderRadius: 6, background: '#0f172a' } },
                h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' } }, t('stem.astronomy.surface_temp', 'Surface temp')),
                h('div', { style: { fontSize: 13, fontWeight: 700, color: '#e2e8f0' } }, selected.tempK + ' K')
              ),
              h('div', { style: { padding: 8, borderRadius: 6, background: '#0f172a' } },
                h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' } }, t('stem.astronomy.mass_sun_1', 'Mass (Sun=1)')),
                h('div', { style: { fontSize: 13, fontWeight: 700, color: '#e2e8f0' } }, selected.massSun)
              ),
              h('div', { style: { padding: 8, borderRadius: 6, background: '#0f172a' } },
                h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' } }, t('stem.astronomy.lifespan', 'Lifespan')),
                h('div', { style: { fontSize: 13, fontWeight: 700, color: '#e2e8f0' } }, selected.life)
              ),
              h('div', { style: { padding: 8, borderRadius: 6, background: '#0f172a' } },
                h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' } }, t('stem.astronomy.example', 'Example')),
          ...
          sectionCard('🌈 Spectroscopy — the cipher of starlight',
            (function() {
              ...
              var rest = [
                { name: t('stem.astronomy.h_balmer', 'Hα (Balmer)'), nm: 656.28, color: '#ff6666' },
                { name: 'Hβ', nm: 486.13, color: '#66ccff' },
                { name: 'Hγ', nm: 434.05, color: '#aa88ff' },
                { name: 'Hδ', nm: 410.17, color: '#ddaaff' },
                { name: t('stem.astronomy.na_d_doublet', 'Na D doublet'), nm: 589.30, color: '#ffcc44' },
                { name: t('stem.astronomy.ca_h_k', 'Ca H+K'), nm: 396.85, color: '#ddccff' },
                { name: t('stem.astronomy.mg_b_triplet', 'Mg b triplet'), nm: 517.00, color: '#88dd88' }
              ];
              ...
              return h('div', null,
                h('p', { style: { margin: '0 0 12px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                  t('stem.astronomy.light_from_a_star_spreads_into_a_spect', 'Light from a star spreads into a spectrum when passed through a prism or diffraction grating. The pattern of bright + dark lines tells us EVERYTHING about the star: what it is made of (line positions), how hot it is (line intensities), how it is moving (line shifts), and even its magnetic field (line splitting). Spectroscopy is the single most important technique in modern astrophysics.')
                ),
                h('div', { style: { display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 } },
                  [
                    { id: 'continuous', name: t('stem.astronomy.continuous_hot_solid', 'Continuous (hot solid)'), desc: t('stem.astronomy.a_hot_dense_object_emits_at_all_wavele', 'A hot dense object emits at all wavelengths. The Sun\'s interior or a tungsten light bulb.') },
                    { id: 'emission', name: t('stem.astronomy.emission_hot_thin_gas', 'Emission (hot thin gas)'), desc: t('stem.astronomy.a_hot_gas_under_low_pressure_emits_onl', 'A hot gas under low pressure emits only at specific wavelengths characteristic of the elements present. Neon signs, nebulae.') },
                    { id: 'absorption', name: t('stem.astronomy.absorption_cool_gas_in_front_of_hot', 'Absorption (cool gas in front of hot)'), desc: t('stem.astronomy.the_standard_stellar_spectrum_continuo', 'The standard stellar spectrum: continuous from the hot star, with dark lines where cooler outer atmosphere has absorbed specific wavelengths. The Sun looks like this.') }
                  ].map(function(s) {
                ...
                h('div', { style: { padding: 10, borderRadius: 8, background: '#1e293b', border: '1px solid #334155', marginBottom: 10 } },
                  h('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 6 } },
                    h('span', { style: { fontSize: 11, color: '#94a3b8', fontWeight: 700 } }, t('stem.astronomy.doppler_shift_radial_velocity', 'Doppler shift (radial velocity)')),
                  ...
                  h('input', { type: 'range', min: -300, max: 300, step: 10, value: dopplerKms,
                    ...
                    'aria-label': t('stem.astronomy.doppler_radial_velocity_in_km_s', 'Doppler radial velocity in km/s'),
                    style: { width: '100%', accentColor: '#fbbf24' }
                  })
                ),

                h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.10)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 12, color: '#c7d2fe', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.what_spectroscopy_tells_us_about_a_sta', 'What spectroscopy tells us about a star: ')),
                  h('ul', { style: { margin: '6px 0 0 22px', padding: 0, lineHeight: 1.7 } },
                    h('li', null, h('strong', null, 'Composition: '), t('stem.astronomy.each_element_has_a_unique_pattern_of_s', 'Each element has a unique pattern of spectral lines (its "fingerprint"). Hydrogen, helium, calcium, sodium, iron — all identifiable by line wavelength.')),
                    h('li', null, h('strong', null, 'Temperature: '), t('stem.astronomy.the_continuum_background_spectrum_foll', 'The CONTINUUM (background spectrum) follows Planck\'s blackbody curve. Hotter stars peak at shorter (bluer) wavelengths; cooler stars at longer (redder).')),
                    h('li', null, h('strong', null, t('stem.astronomy.radial_velocity', 'Radial velocity: ')), t('stem.astronomy.a_star_moving_toward_us_blueshifts_its', 'A star moving toward us blueshifts its lines (compressed); moving away redshifts (stretched). Doppler shift Δλ/λ = v/c. This is how exoplanets are found by the radial-velocity method.')),
                    h('li', null, h('strong', null, 'Rotation: '), t('stem.astronomy.a_spinning_star_has_one_limb_moving_to', 'A spinning star has one limb moving toward us, the other away — blueshifted + redshifted at once. Lines BROADEN. Astronomers measure rotation by line width.')),
                    h('li', null, h('strong', null, t('stem.astronomy.magnetic_field', 'Magnetic field: ')), t('stem.astronomy.strong_magnetic_fields_split_a_single_', 'Strong magnetic fields split a single line into multiple components (Zeeman effect). The Sun\'s sunspot magnetic fields show this clearly.'))
                  )
                )
              );
            })(),
          ...
          sectionCard('☁️ Nebulae + star formation — where stars are born',
            (function() {
              var NEBULAE = [
                {
                  id: 'molecular', name: t('stem.astronomy.molecular_cloud_stellar_nursery', 'Molecular cloud (stellar nursery)'), color: '#7c3aed',
                ...
                {
                  id: 'emission', name: t('stem.astronomy.emission_nebula', 'Emission nebula'), color: '#ef4444',
                ...
                {
                  id: 'reflection', name: t('stem.astronomy.reflection_nebula', 'Reflection nebula'), color: '#3b82f6',
                ...
                {
                  id: 'planetary', name: t('stem.astronomy.planetary_nebula', 'Planetary nebula'), color: '#22c55e',
                ...
                {
                  id: 'snr', name: t('stem.astronomy.supernova_remnant', 'Supernova remnant'), color: '#f59e0b',
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 12, background: '#0f172a', borderTop: '1px solid #334155', borderRight: '1px solid #334155', borderBottom: '1px solid #334155', borderLeft: '3px solid ' + sel.color } },
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(56,189,248,0.08)', borderLeft: '3px solid #38bdf8', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#7dd3fc', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.what_it_is', 'What it is')),
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(168,85,247,0.08)', borderLeft: '3px solid #a855f7', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#d8b4fe', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.why_it_exists', 'Why it exists')),
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(34,197,94,0.08)', borderLeft: '3px solid #22c55e', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.size_scale', 'Size + scale')),
                    h('div', { style: { fontSize: 12, color: '#e2e8f0', lineHeight: 1.6 } }, sel.size)
                  ),
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(245,158,11,0.08)', borderLeft: '3px solid #f59e0b' } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#fbbf24', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.how_to_observe', 'How to observe')),
                    h('div', { style: { fontSize: 12, color: '#e2e8f0', lineHeight: 1.6 } }, sel.obs)
                  )
                ),
                h('div', { style: { marginTop: 12, padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.10)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 12, color: '#c7d2fe', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.how_a_star_actually_forms_in_4_steps', 'How a star actually forms (in 4 steps): ')),
                  h('ol', { style: { margin: '6px 0 0 22px', padding: 0, lineHeight: 1.7 } },
                    h('li', null, h('strong', null, 'Trigger: '), t('stem.astronomy.something_disturbs_a_stable_molecular_', 'Something disturbs a stable molecular cloud — a nearby supernova shockwave, two clouds colliding, or compression by spiral-arm density waves.')),
                    h('li', null, h('strong', null, 'Collapse: '), t('stem.astronomy.a_dense_knot_in_the_cloud_begins_gravi', 'A dense knot in the cloud begins gravitational free-fall. It contracts; conserving angular momentum, it flattens into a disk. The center grows hotter.')),
                    h('li', null, h('strong', null, t('stem.astronomy.protostar_phase', 'Protostar phase: ')), t('stem.astronomy.after_100_000_years_the_core_reaches_a', 'After ~100,000 years, the core reaches a few million degrees — but not yet hot enough for fusion. It is "shining" by gravitational contraction. Protoplanetary disk surrounds it. Bipolar jets fire perpendicular to the disk.')),
                    h('li', null, h('strong', null, t('stem.astronomy.main_sequence_ignition', 'Main sequence ignition: ')), t('stem.astronomy.after_10_100_million_years_sun_like_or', 'After ~10-100 million years (Sun-like) or much faster (massive stars), the core reaches ~10 million K. Hydrogen fusion starts. The star is "born" + joins the main sequence. The protoplanetary disk\'s remaining material forms planets.'))
                  )
                ),
                h('div', { style: { marginTop: 8, padding: 10, borderRadius: 8, background: 'rgba(245,158,11,0.10)', border: '1px solid rgba(245,158,11,0.3)', fontSize: 12, color: 'var(--allo-stem-text, #fde68a)', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.the_pillars_of_creation', 'The Pillars of Creation: ')),
                  t('stem.astronomy.hubble_s_famous_1995_image_of_the_eagl', 'Hubble\'s famous 1995 image of the Eagle Nebula shows three towering columns of gas + dust, each several light-years tall, being eroded by ultraviolet radiation from nearby young hot stars. New stars are forming inside the pillars. JWST re-imaged them in 2022 in infrared, revealing the protostars more clearly than ever. The pillars themselves may have already been destroyed by a supernova ~6,000 years ago — but the light hasn\'t reached us yet.')
                )
              );
            })(),
            '#a855f7'
          ...
          sectionCard('☀️ Our Sun — closest, most important star',
            (function() {
              var LAYERS = [
                { id: 'core', name: t('stem.astronomy.core', 'Core'), color: 'var(--allo-stem-text, #fde047)',
                ...
                { id: 'radiative', name: t('stem.astronomy.radiative_zone', 'Radiative zone'), color: '#fbbf24',
                ...
                { id: 'convective', name: t('stem.astronomy.convective_zone', 'Convective zone'), color: '#f97316',
                ...
                { id: 'photosphere', name: t('stem.astronomy.photosphere', 'Photosphere'), color: 'var(--allo-stem-text, #fef3c7)',
                ...
                { id: 'chromosphere', name: t('stem.astronomy.chromosphere', 'Chromosphere'), color: '#dc2626',
                  size: '~2,000 km thick',
                ...
                { id: 'corona', name: t('stem.astronomy.corona', 'Corona'), color: '#7dd3fc',
              ...
              return h('div', null,
                ...
                (function() {
                  ...
                  return h('svg', { viewBox: '0 0 ' + svgW + ' ' + svgH, width: '100%', height: svgH, role: 'img', 'aria-labelledby': 'sunTitle sunDesc' },
                    h('title', { id: 'sunTitle' }, t('stem.astronomy.sun_cross_section_diagram', 'Sun cross-section diagram')),
                    h('desc', { id: 'sunDesc' }, t('stem.astronomy.a_schematic_cross_section_of_the_sun_s', 'A schematic cross-section of the Sun showing nested concentric layers from the central core out to the corona.')),
                    ...
                    h('text', { x: cx, y: cy - 130, textAnchor: 'middle', fill: layerIdx === 5 ? '#7dd3fc' : '#94a3b8', fontSize: 11, fontWeight: layerIdx === 5 ? 800 : 500 }, t('stem.astronomy.corona_2', 'Corona')),
                    // Chromosphere
                    h('circle', { cx: cx, cy: cy, r: 105, fill: '#dc2626', stroke: '#7f1d1d', strokeWidth: layerIdx === 4 ? 3 : 1, opacity: 0.7 }),
                    h('text', { x: cx + 115, y: cy - 25, fill: layerIdx === 4 ? '#fca5a5' : '#94a3b8', fontSize: 10, fontWeight: layerIdx === 4 ? 800 : 500 }, t('stem.astronomy.chromosphere_2', 'Chromosphere')),
                    // Photosphere
                    h('circle', { cx: cx, cy: cy, r: 100, fill: '#fef3c7', stroke: layerIdx === 3 ? '#fbbf24' : '#92400e', strokeWidth: layerIdx === 3 ? 3 : 1, opacity: 0.85 }),
                    h('text', { x: cx + 105, y: cy + 5, fill: layerIdx === 3 ? '#fde68a' : '#94a3b8', fontSize: 10, fontWeight: layerIdx === 3 ? 800 : 500 }, t('stem.astronomy.photosphere_2', 'Photosphere')),
                    // Convective zone
                    h('circle', { cx: cx, cy: cy, r: 80, fill: '#f97316', stroke: '#9a3412', strokeWidth: layerIdx === 2 ? 3 : 1 }),
                    h('text', { x: cx, y: cy + 70, textAnchor: 'middle', fill: layerIdx === 2 ? '#fdba74' : '#94a3b8', fontSize: 10, fontWeight: layerIdx === 2 ? 800 : 500 }, t('stem.astronomy.convective_zone_2', 'Convective zone')),
                    // Radiative zone
                    h('circle', { cx: cx, cy: cy, r: 55, fill: '#fbbf24', stroke: '#92400e', strokeWidth: layerIdx === 1 ? 3 : 1 }),
                    h('text', { x: cx, y: cy + 45, textAnchor: 'middle', fill: layerIdx === 1 ? '#fde68a' : '#94a3b8', fontSize: 9, fontWeight: layerIdx === 1 ? 800 : 500 }, t('stem.astronomy.radiative_zone_2', 'Radiative zone')),
                    // Core
                    h('circle', { cx: cx, cy: cy, r: 25, fill: '#fde047', stroke: '#facc15', strokeWidth: layerIdx === 0 ? 3 : 1 }),
                    h('text', { x: cx, y: cy + 4, textAnchor: 'middle', fill: '#7c2d12', fontSize: 11, fontWeight: 800 }, t('stem.astronomy.core_2', 'Core')),
                ...
                h('div', { style: { padding: 12, borderRadius: 10, background: '#0f172a', borderTop: '1px solid #334155', borderRight: '1px solid #334155', borderBottom: '1px solid #334155', borderLeft: '3px solid ' + selL.color } },
                  ...
                  h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 } },
                    h('div', { style: { padding: 6, borderRadius: 6, background: '#1e293b' } },
                      h('div', { style: { fontSize: 10, color: '#94a3b8' } }, t('stem.astronomy.size', 'Size')),
                      h('div', { style: { fontSize: 11.5, color: '#e2e8f0', fontWeight: 700 } }, selL.size)
                    ),
                    h('div', { style: { padding: 6, borderRadius: 6, background: '#1e293b' } },
                      h('div', { style: { fontSize: 10, color: '#94a3b8' } }, t('stem.astronomy.temperature', 'Temperature')),
                ...
                h('div', { style: { marginTop: 8, padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.10)', border: '1px solid rgba(34,197,94,0.3)', fontSize: 12, color: '#a7f3d0', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.aurora_sun_earth_coupling', 'Aurora — Sun-Earth coupling: ')),
                  t('stem.astronomy.charged_particles_from_solar_wind_cmes', 'Charged particles from solar wind + CMEs follow Earth\'s magnetic field lines toward the poles, colliding with atmospheric atoms at 100-300 km altitude. Excited oxygen emits green (~557 nm) + red (~630 nm); nitrogen emits blue + purple. Strong storms push aurora to lower latitudes — Maine + Vermont saw the May 2024 + October 2024 storms vividly. Best viewing: dark site, look north, allow eyes to dark-adapt. NOAA SWPC (Space Weather Prediction Center) issues aurora forecasts.')
                ),
          ...
          sectionCard('💀 Compact objects — what stars become',
            (function() {
              var COMPACT = [
                {
                  id: 'wd', name: t('stem.astronomy.white_dwarf', 'White Dwarf'), color: '#bfdbfe',
                ...
                {
                  id: 'ns', name: t('stem.astronomy.neutron_star', 'Neutron Star'), color: '#fbbf24',
                ...
                {
                  id: 'sbh', name: t('stem.astronomy.stellar_mass_black_hole', 'Stellar-mass Black Hole'), color: '#a855f7',
                ...
                {
                  id: 'smbh', name: t('stem.astronomy.supermassive_black_hole_smbh', 'Supermassive Black Hole (SMBH)'), color: '#7c3aed',
              ...
              return h('div', null,
                h('p', { style: { margin: '0 0 12px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                  t('stem.astronomy.when_a_star_runs_out_of_fuel_gravity_w', 'When a star runs out of fuel, gravity wins. The kind of corpse it becomes depends on one thing: the mass of the remaining core. Sun-like stars leave white dwarfs. Big stars leave neutron stars or black holes. The boundaries are sharp and rooted in fundamental physics.')
                ),
                h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 } },
                ...
                h('div', { style: { padding: 12, borderRadius: 10, background: '#0f172a', borderTop: '1px solid #334155', borderRight: '1px solid #334155', borderBottom: '1px solid #334155', borderLeft: '3px solid ' + sel.color } },
                  ...
                  h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 8, marginBottom: 10 } },
                    [
                      { label: t('stem.astronomy.mass_2', 'Mass'), value: sel.mass },
                      { label: t('stem.astronomy.size_2', 'Size'), value: sel.size },
                      { label: t('stem.astronomy.density', 'Density'), value: sel.density }
                    ].map(function(p, i) {
                  ...
                  [
                    { title: t('stem.astronomy.origin', 'Origin'), body: sel.origin, color: '#fbbf24' },
                    { title: t('stem.astronomy.physics', 'Physics'), body: sel.physics, color: sel.color },
                    { title: t('stem.astronomy.real_example', 'Real example'), body: sel.example, color: '#38bdf8' },
                    { title: t('stem.astronomy.long_term_fate', 'Long-term fate'), body: sel.fate, color: '#94a3b8' }
                  ].map(function(p, i) {
          ...
          sectionCard('🔆 The magnitude scale: how bright is bright?',
            h('div', null,
              h('p', { style: { margin: '0 0 10px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                t('stem.astronomy.astronomers_measure_brightness_on_the_', 'Astronomers measure brightness on the magnitude scale, invented by the Greek astronomer Hipparchus around 130 BCE. It is '),
                h('strong', { style: { color: '#fbbf24' } }, 'inverted'),
                t('stem.astronomy.smaller_number_brighter_the_historical', ' (smaller number = brighter, the historical convention) and '),
                h('strong', { style: { color: '#fbbf24' } }, 'logarithmic'),
                t('stem.astronomy.each_step_of_5_magnitudes_is_exactly_1', ' (each step of 5 magnitudes is exactly 100× brighter). The Sun at magnitude -27 and the faintest galaxy ever imaged at magnitude +32 differ in brightness by a factor of about 10²³.')
              ),
              ...
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10, marginBottom: 12 } },
                h('div', { style: { padding: 10, borderRadius: 8, background: '#0f172a', borderTop: '1px solid #334155', borderRight: '1px solid #334155', borderBottom: '1px solid #334155', borderLeft: '3px solid #fbbf24' } },
                  h('div', { style: { fontSize: 13, fontWeight: 800, color: '#fbbf24', marginBottom: 4 } }, t('stem.astronomy.apparent_magnitude_m', 'Apparent magnitude (m)')),
                  h('div', { style: { fontSize: 12, color: '#e2e8f0', lineHeight: 1.6 } }, t('stem.astronomy.how_bright_the_object_looks_from_earth', 'How bright the object LOOKS from Earth. Depends on both how much light the object emits AND how far away it is. A bright nearby star and a dim distant star can have the same apparent magnitude.'))
                ),
                h('div', { style: { padding: 10, borderRadius: 8, background: '#0f172a', borderTop: '1px solid #334155', borderRight: '1px solid #334155', borderBottom: '1px solid #334155', borderLeft: '3px solid #c7d2fe' } },
                  h('div', { style: { fontSize: 13, fontWeight: 800, color: '#c7d2fe', marginBottom: 4 } }, t('stem.astronomy.absolute_magnitude_m', 'Absolute magnitude (M)')),
                  h('div', { style: { fontSize: 12, color: '#e2e8f0', lineHeight: 1.6 } }, t('stem.astronomy.how_bright_the_object_would_appear_if_', 'How bright the object would APPEAR if placed at a standard distance of 10 parsecs (32.6 light-years). Lets us compare the actual luminosity of stars regardless of distance.'))
                )
              ),

              // Magnitude reference table
              h('div', { style: { padding: 12, borderRadius: 10, background: '#0a0e1a', border: '1px solid #334155', overflowX: 'auto' } },
                h('div', { style: { fontSize: 12, fontWeight: 700, color: '#cbd5e1', marginBottom: 8 } }, t('stem.astronomy.apparent_magnitude_reference', 'Apparent magnitude reference:')),
          ...
          sectionCard('☀️ Solar physics + space weather',
            (function() {
              var SUN_TOPICS = [
                ...
                { id: 'flares', name: t('stem.astronomy.solar_flares', 'Solar flares'), emoji: '⚡',
                  body: t('stem.astronomy.a_solar_flare_is_a_sudden_bright_flash', 'A solar flare is a sudden bright flash on the Sun caused by magnetic reconnection — twisted field lines in an active region snap into a lower-energy configuration, releasing the stored energy as X-rays + gamma-rays + accelerated particles + heated plasma. Flares are classified A (smallest), B, C, M, X (largest), each class 10× more intense than the previous. The largest flares ever recorded are estimated around X45 (November 2003, modern instruments saturated) + X40 (the May 2024 series). Flare X-rays reach Earth in 8 minutes (the speed of light); they ionize the upper atmosphere, causing HF radio blackouts on the sunlit side of Earth for minutes to hours.'),
                ...
                { id: 'aurora', name: t('stem.astronomy.auroras_explained', 'Auroras explained'), emoji: '🌌',
                  body: t('stem.astronomy.when_solar_wind_particles_enter_earth_', 'When solar-wind particles enter Earth\'s upper atmosphere along magnetic field lines (mostly near the poles), they collide with oxygen + nitrogen atoms + excite them. The atoms emit light at specific wavelengths as they relax: GREEN aurora is excited atomic oxygen at ~100 km altitude (557.7 nm wavelength); RED aurora is excited atomic oxygen at ~250 km (630 nm, lower density + slower decay); BLUE + PURPLE are ionized nitrogen at lower altitudes. The aurora oval expands during geomagnetic storms, pushing visibility to mid-latitudes. Maine sees aurora on average a few nights a year; during the May 10-11 2024 G5 storm, aurora was visible across most of the continental US + parts of Mexico, the strongest such event in 20 years.'),
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(249,115,22,0.08)', border: '1px solid rgba(249,115,22,0.35)' } },
                  ...
                  h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.7, marginBottom: 10 } }, topic.body),
                  h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                    h('strong', null, t('stem.astronomy.honest_framing_5', 'Honest framing: ')), topic.caveat
                  )
                ),
          ...
          sectionCard('⚛️ Where every atom in your body came from',
            (function() {
              var NS_TOPICS = [
                ...
                { id: 'giants', name: t('stem.astronomy.red_giants_agb_stars', 'Red giants + AGB stars'), emoji: '🔴',
                  body: t('stem.astronomy.when_a_star_s_core_runs_out_of_hydroge', 'When a star\'s core runs out of hydrogen, the core contracts + heats up, while the outer envelope expands + cools (a red giant). Helium fusion begins in the core (3 helium-4 → carbon-12 via the triple-alpha process, requiring a key resonance Fred Hoyle predicted in 1953 — the "Hoyle state"). Beyond carbon, helium captures continue to produce oxygen-16, neon-20, magnesium-24. AGB (Asymptotic Giant Branch) stars also drive the s-process — slow neutron capture — building up about half the heavy elements past iron (zirconium, barium, lead). Material from the outer envelope is shed via stellar winds during the red-giant + AGB phases, returning enriched material to the interstellar medium.'),
          ...
          sectionCard('🌠 Star clusters — open + globular',
            (function() {
              var CL_TOPICS = [
                { id: 'overview', name: t('stem.astronomy.clusters_as_galactic_history_records', 'Clusters as galactic-history records'), emoji: '🔭',
                  body: t('stem.astronomy.stars_almost_never_form_alone_almost_e', 'Stars almost never form alone. Almost every star in the Milky Way formed inside a CLUSTER — a group of dozens to millions of stars from the same molecular cloud, born within a few million years of each other. Because the stars share AGE + INITIAL CHEMISTRY, clusters are the cleanest natural laboratories for stellar evolution. They are also fossils of galactic history: their orbits, compositions, + ages tell us about the formation + assembly of the Milky Way itself. The Sun was born in a cluster ~ 4.6 billion years ago; that cluster has long since dispersed + we still cannot identify the Sun\'s siblings with certainty.'),
                ...
                { id: 'globular', name: t('stem.astronomy.globular_clusters_ancient_dense', 'Globular clusters — ancient + dense'), emoji: '🪩',
                  body: t('stem.astronomy.globular_clusters_contain_100_000_to_1', 'Globular clusters contain ~ 100,000 to 10 million stars in a tight spherical ball ~ 50-200 light-years across. They are OLD (10-13 billion years, almost as old as the universe), made of stars from the early Milky Way that have METAL-POOR compositions (only a fraction of the Sun\'s heavy-element content). They are very gravitationally bound + survive for the age of the universe. The Milky Way has ~ 200 known globular clusters, mostly orbiting in a SPHERICAL HALO around the galactic center. Famous examples: Omega Centauri (largest + brightest, naked-eye visible from southern hemisphere — but recent research suggests it may be the remnant of a galaxy that merged with the Milky Way long ago), M13 in Hercules (best northern-hemisphere globular), 47 Tucanae (second-largest), M5, M22, M3, M92.'),
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.35)' } },
                  ...
                  h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.7, marginBottom: 10 } }, topic.body),
                  h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                    h('strong', null, t('stem.astronomy.honest_framing_7', 'Honest framing: ')), topic.caveat
                  )
                ),
                h('div', { style: { marginTop: 12, padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.3)', fontSize: 11.5, color: '#dcfce7', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.try_tonight_with_your_eyes', 'Try tonight, with your eyes: ')),
                  t('stem.astronomy.in_fall_winter_look_for_the_pleiades_a', 'In fall + winter, look for the Pleiades — a tight blue cluster of 6-7 visible stars in Taurus, easily seen even from suburban skies. Use binoculars + you will see dozens more. In summer, find Hercules (between Vega + Arcturus) + look for M13 — a faint smudge to the naked eye, a glittering ball through any telescope. These observations connect you directly to what Harlow Shapley + Edwin Hubble were studying when they were redefining humanity\'s view of the cosmos.')
                )
              );
            })(),
            '#a78bfa'
          ...
          sectionCard('🌟 Variable stars — the changeable sky',
            (function() {
              var VAR_TOPICS = [
                ...
                { id: 'cepheid', name: t('stem.astronomy.cepheids_the_cosmic_distance_ladder', 'Cepheids — the cosmic distance ladder'), emoji: '📏',
                  body: t('stem.astronomy.cepheid_variables_named_for_delta_ceph', 'Cepheid variables (named for delta Cephei, the prototype, discovered 1784) PULSATE radially over periods of 1-100 days with brightness amplitudes of 0.5-2 magnitudes. The crucial fact (Henrietta Leavitt, 1908, at Harvard): their PERIOD predicts their LUMINOSITY. A 30-day Cepheid is intrinsically about 10,000 times brighter than the Sun; a 3-day Cepheid is much less luminous. So if you can MEASURE the period (easy: a few weeks of observation), you can INFER the absolute magnitude, then compare to observed brightness, then DERIVE the distance. Leavitt\'s relation is the rung of the cosmic distance ladder that calibrates everything beyond the local neighborhood. Edwin Hubble used Cepheids in M31 (Andromeda) in 1923-1924 to prove it is a separate GALAXY rather than a nebula in our Milky Way — a finding that redefined the scale of the universe.'),
                ...
                { id: 'rrlyrae', name: t('stem.astronomy.rr_lyrae_globular_cluster_ageometers', 'RR Lyrae — globular-cluster ageometers'), emoji: '⭐',
                  body: t('stem.astronomy.rr_lyrae_variables_named_for_rr_lyrae_', 'RR Lyrae variables (named for RR Lyrae, the prototype) are SHORTER-PERIOD pulsators (0.2-1 day) common in old globular clusters. They are all about the same intrinsic brightness (~ 40 times the Sun), so each one acts as an INDEPENDENT distance indicator. RR Lyrae are particularly useful for distances to old populations + for galactic-archaeology work. They are easy to find (their distinctive sawtooth light curves stand out) + bright enough to be detected throughout the Milky Way halo + into nearby galaxies. Over 50,000 RR Lyrae are now catalogued; current + future surveys (Gaia, LSST) will find millions.'),
                ...
                { id: 'mira', name: t('stem.astronomy.mira_variables_pulsating_red_giants', 'Mira variables — pulsating red giants'), emoji: '🔴',
                  body: t('stem.astronomy.mira_omicron_ceti_was_the_first_non_su', 'Mira (omicron Ceti) was the first non-supernova variable star ever recognized — discovered in 1596 by David Fabricius. The class now bears its name: MIRA VARIABLES are red-giant stars in the late stages of stellar evolution, pulsating over PERIODS of 100-1,000 days with HUGE brightness amplitudes (3-10 magnitudes — a factor of ~ 100 to ~ 10,000 in flux). At maximum, Mira itself is naked-eye easy (magnitude 2-3); at minimum, it requires a telescope (magnitude 8-10). The pulsation mechanism: in the very-extended, cool atmosphere of a red giant, opacity changes with temperature drive a slow-period instability that pumps the outer layers.'),
                ...
                { id: 'cataclysmic', name: t('stem.astronomy.novae_cataclysmic_variables', 'Novae + cataclysmic variables'), emoji: '💥',
                  body: t('stem.astronomy.a_cataclysmic_variable_is_a_binary_sys', 'A CATACLYSMIC VARIABLE is a binary system with a WHITE DWARF accreting matter from a close companion. The matter accumulates on the dwarf\'s surface; when enough builds up, it ignites in a thermonuclear runaway, brightening the system by 100,000× or more in a few days — a CLASSICAL NOVA. The dwarf survives + the cycle can repeat (RECURRENT novae have periods of decades to centuries; the well-known T Coronae Borealis is overdue for its next eruption, predicted any year now — last erupted 1946, predicted ~ 80-year cycle). DWARF NOVAE (a different mechanism) brighten by 5-100× in days from accretion-disk instabilities, repeating every weeks-to-months. Novae deliver ~ 10⁻⁴ solar masses of processed material to the interstellar medium each eruption, contributing to galactic chemical evolution.'),
                ...
                { id: 'survey', name: t('stem.astronomy.modern_surveys_the_future', 'Modern surveys + the future'), emoji: '🛰️',
                  body: t('stem.astronomy.variable_star_astronomy_is_being_trans', 'Variable-star astronomy is being TRANSFORMED by modern time-domain surveys. (1) GAIA (ESA, 2013-present) is measuring positions + brightnesses + spectra of ~ 2 billion stars; expected to catalog millions of variables. (2) ZWICKY TRANSIENT FACILITY (ZTF, Palomar, since 2018) scans the entire Northern Hemisphere sky every 2 nights. (3) ATLAS (Asteroid Terrestrial-impact Last Alert System) does similar work + is finding many new variables. (4) THE VERA C. RUBIN OBSERVATORY (LSST, beginning operations 2025-2026) will image the entire visible Southern Hemisphere sky EVERY THREE NIGHTS for 10 years — producing ~ 20 million variable-star detections + perhaps 10,000 supernovae per year. The data volume + discovery rate will be unprecedented; the analysis is itself a research challenge.'),
        ...
        function namedStarsDatabaseSection() {
          ...
          var STARS = [
            ...
            { name: t('stem.astronomy.sirius_2', 'Sirius'), desig: 'α CMa', con: 'Canis Major', spec: 'A1V', mag: -1.46, dist: 8.6, notes: 'Brightest star in the night sky. Binary system with a white-dwarf companion (Sirius B). The name "scorching" reflects Sirius\'s heat from helical rising during late summer. "Dog Star" of antiquity.' },
            { name: t('stem.astronomy.canopus', 'Canopus'), desig: 'α Car', con: 'Carina', spec: 'A9II', mag: -0.74, dist: 310, notes: 'Second-brightest star in the night sky. F-class supergiant. Used by spacecraft (Voyager, Galileo, others) as a navigation reference because of its brightness + isolation in southern sky.' },
            { name: t('stem.astronomy.arcturus_3', 'Arcturus'), desig: 'α Boo', con: 'Boötes', spec: 'K0III', mag: -0.05, dist: 37, notes: 'Third-brightest star; brightest in northern celestial hemisphere. Red-orange giant. Moving rapidly through the local star neighborhood — has the highest known proper motion of any first-magnitude star.' },
            { name: t('stem.astronomy.alpha_centauri', 'Alpha Centauri'), desig: 'α Cen', con: 'Centaurus', spec: 'G2V+K1V', mag: -0.27, dist: 4.37, notes: 'Closest naked-eye stellar system. Triple system: A (sunlike) + B (orange dwarf) + Proxima Centauri (red dwarf, hosts confirmed planets). Visible only from south + tropical latitudes.' },
            { name: t('stem.astronomy.vega_3', 'Vega'), desig: 'α Lyr', con: 'Lyra', spec: 'A0V', mag: 0.03, dist: 25, notes: 'The astronomical photometric standard (defined magnitude 0). Will be the pole star ~ 12,000 years from now. The "Summer Triangle" star with Deneb + Altair. Hosts a known debris disk.' },
            { name: t('stem.astronomy.capella', 'Capella'), desig: 'α Aur', con: 'Auriga', spec: 'G3III+G0III', mag: 0.08, dist: 42, notes: 'Quadruple star system; 4th brightest star in night sky. Two giant G-type stars + two M dwarfs. "She-goat" (Capra in Latin), held by the charioteer.' },
            { name: t('stem.astronomy.rigel_3', 'Rigel'), desig: 'β Ori', con: 'Orion', spec: 'B8Ia', mag: 0.18, dist: 860, notes: 'Blue supergiant 120,000× the luminosity of the Sun. Will eventually go supernova. Beta designation (despite being brighter than Alpha) because of Bayer ranking choices. Multiple system with at least 4 components.' },
            { name: t('stem.astronomy.procyon_2', 'Procyon'), desig: 'α CMi', con: 'Canis Minor', spec: 'F5IV', mag: 0.34, dist: 11.5, notes: 'One of nearest stars. Binary with a white-dwarf companion (Procyon B). Together with Sirius forms one of the closest pair of bright stars.' },
            { name: t('stem.astronomy.achernar', 'Achernar'), desig: 'α Eri', con: 'Eridanus', spec: 'B6Vpe', mag: 0.46, dist: 140, notes: 'One of the most oblate stars known — rotates so rapidly (250+ km/s equatorial) that its equator bulges to 50% wider than its poles. Visible only from south + low northern latitudes.' },
            { name: t('stem.astronomy.betelgeuse_3', 'Betelgeuse'), desig: 'α Ori', con: 'Orion', spec: 'M1Iab', mag: 0.45, dist: 550, notes: 'Red supergiant — one of the largest known stars (700-1000× the Sun\'s diameter). Variable brightness, with the dramatic 2019-2020 "Great Dimming" event that puzzled astronomers (probably a dust cloud). Will go supernova in ~ 100,000 years (likely).' },
            { name: t('stem.astronomy.hadar', 'Hadar'), desig: 'β Cen', con: 'Centaurus', spec: 'B1III', mag: 0.61, dist: 390, notes: 'Triple system; second-brightest star in Centaurus. Together with Alpha Cen forms a striking visual pair.' },
            { name: t('stem.astronomy.altair', 'Altair'), desig: 'α Aql', con: 'Aquila', spec: 'A7V', mag: 0.77, dist: 17, notes: 'One of closest naked-eye stars. The "third corner" of the Summer Triangle with Vega + Deneb. Rotates rapidly (286 km/s equatorial); has a notably oblate shape resolved by interferometry.' },
            { name: t('stem.astronomy.acrux', 'Acrux'), desig: 'α Cru', con: 'Crux', spec: 'B0.5IV+B1V', mag: 0.77, dist: 320, notes: 'Brightest star in the Southern Cross. Multiple system. Marker for celestial South in southern-hemisphere navigation.' },
            { name: t('stem.astronomy.aldebaran_2', 'Aldebaran'), desig: 'α Tau', con: 'Taurus', spec: 'K5III', mag: 0.85, dist: 65, notes: 'The "Eye of the Bull." Orange giant. Pioneer 10 spacecraft is currently heading roughly toward Aldebaran + will reach its vicinity in about 2 million years.' },
            { name: t('stem.astronomy.antares_2', 'Antares'), desig: 'α Sco', con: 'Scorpius', spec: 'M1.5Iab', mag: 1.06, dist: 550, notes: 'Red supergiant nearly as large as Betelgeuse. Name = "rival of Mars" (anti-Ares) for similar red color. Has a hot blue companion that\'s often lost in glare.' },
            { name: t('stem.astronomy.spica_2', 'Spica'), desig: 'α Vir', con: 'Virgo', spec: 'B1III+B2V', mag: 1.04, dist: 250, notes: 'Spectroscopic binary; the two stars orbit each other in just 4 days. Hot blue stars. Name = "ear of grain" representing the wheat sheaf held by Virgo.' },
            { name: t('stem.astronomy.pollux', 'Pollux'), desig: 'β Gem', con: 'Gemini', spec: 'K0III', mag: 1.14, dist: 34, notes: 'Orange giant. The brighter of the twins (despite Beta designation). Has a confirmed planet (Pollux b).' },
            { name: t('stem.astronomy.fomalhaut', 'Fomalhaut'), desig: 'α PsA', con: 'Piscis Austrinus', spec: 'A4V', mag: 1.16, dist: 25, notes: 'A-class main-sequence star surrounded by a famous debris disk imaged by Hubble + JWST. The disk shows evidence of planet formation.' },
            { name: t('stem.astronomy.deneb_3', 'Deneb'), desig: 'α Cyg', con: 'Cygnus', spec: 'A2Ia', mag: 1.25, dist: 1500, notes: 'White supergiant — intrinsically the most luminous star visible to the naked eye (~ 200,000× the Sun). Looks moderate because it\'s ~ 1500 light-years away. Tail of the swan + corner of Summer Triangle.' },
            { name: t('stem.astronomy.regulus', 'Regulus'), desig: 'α Leo', con: 'Leo', spec: 'B7V+K2V+M4V', mag: 1.40, dist: 79, notes: 'Quadruple system. Spins extremely rapidly (~ 1 rotation in 16 hours, vs the Sun\'s ~ 25 days). "Little king" — close to the ecliptic, so often near the Moon + planets.' },

            // Continuing brightest...
            { name: t('stem.astronomy.adhara', 'Adhara'), desig: 'ε CMa', con: 'Canis Major', spec: 'B2II', mag: 1.50, dist: 430, notes: 'Hot blue giant. ~ 4 million years ago, when it was only ~ 30 ly from Earth, Adhara was the brightest star in our sky at magnitude -3.99 (brighter than Venus). Cosmic timing.' },
            { name: t('stem.astronomy.castor', 'Castor'), desig: 'α Gem', con: 'Gemini', spec: 'A2V', mag: 1.58, dist: 51, notes: 'Sextuple star system — 6 individual stars in 3 binary pairs. The brighter twin to Pollux.' },
            { name: t('stem.astronomy.shaula', 'Shaula'), desig: 'λ Sco', con: 'Scorpius', spec: 'B1.5IV', mag: 1.62, dist: 700, notes: 'The "stinger" of Scorpius. Multiple system.' },
            { name: t('stem.astronomy.bellatrix', 'Bellatrix'), desig: 'γ Ori', con: 'Orion', spec: 'B2III', mag: 1.64, dist: 245, notes: 'The "Amazon Star" — bright blue-white giant. Orion\'s left shoulder.' },
            { name: t('stem.astronomy.alnath', 'Alnath'), desig: 'β Tau', con: 'Taurus', spec: 'B7III', mag: 1.65, dist: 130, notes: 'The "northern horn" of the bull. Shared border star — historically also part of Auriga (where it would be γ Aur).' },
            { name: t('stem.astronomy.miaplacidus', 'Miaplacidus'), desig: 'β Car', con: 'Carina', spec: 'A1IV', mag: 1.69, dist: 110, notes: 'Second-brightest in Carina. Subgiant — beginning post-main-sequence evolution.' },
            { name: t('stem.astronomy.alnilam', 'Alnilam'), desig: 'ε Ori', con: 'Orion', spec: 'B0Ia', mag: 1.69, dist: 1400, notes: 'Central star of Orion\'s Belt. Blue supergiant — among the most luminous stars in the galaxy (~ 500,000× the Sun).' },
            { name: t('stem.astronomy.alnitak', 'Alnitak'), desig: 'ζ Ori', con: 'Orion', spec: 'O9.7Ib', mag: 1.74, dist: 1260, notes: 'Easternmost star of Orion\'s Belt. Hot O-class supergiant. Triple system.' },
            { name: t('stem.astronomy.alioth', 'Alioth'), desig: 'ε UMa', con: 'Ursa Major', spec: 'A1III', mag: 1.76, dist: 81, notes: 'Brightest star in the Big Dipper. Variable magnetic field. Member of the Ursa Major Moving Group.' },
            { name: t('stem.astronomy.mirfak', 'Mirfak'), desig: 'α Per', con: 'Perseus', spec: 'F5Ib', mag: 1.79, dist: 510, notes: 'Yellow-white supergiant. Brightest member of the Alpha Persei Cluster (a moving group of related stars).' },

            // Stars near the Sun (often dim but important for being close)
            { name: t('stem.astronomy.proxima_centauri', 'Proxima Centauri'), desig: 'α Cen C', con: 'Centaurus', spec: 'M5.5V', mag: 11.13, dist: 4.244, notes: 'The closest star to the Sun. Red dwarf, much dimmer than its sister stars Alpha Cen A+B. Hosts at least 3 planets, including Proxima b in the habitable zone (discovered 2016).' },
            { name: t('stem.astronomy.barnard_s_star', 'Barnard\'s Star'), desig: '—', con: 'Ophiuchus', spec: 'M4V', mag: 9.53, dist: 5.96, notes: 'Second-closest stellar system. Red dwarf with the highest known proper motion (10.4 arcseconds/year — moves a full Moon diameter every 180 years).' },
            { name: t('stem.astronomy.wolf_359', 'Wolf 359'), desig: '—', con: 'Leo', spec: 'M6V', mag: 13.50, dist: 7.86, notes: 'Famous as the site of a key Star Trek battle. Real-life: a flare star — dim red dwarf that briefly brightens during flares.' },
            { name: t('stem.astronomy.lalande_21185', 'Lalande 21185'), desig: '—', con: 'Ursa Major', spec: 'M2V', mag: 7.49, dist: 8.31, notes: 'Closest star NOT visible to the unaided eye, despite being only 8 ly away. Has at least 2 planets.' },
            { name: t('stem.astronomy.luyten_726_8', 'Luyten 726-8'), desig: 'A+B', con: 'Cetus', spec: 'M5.5V+M5.5V', mag: 12.5, dist: 8.73, notes: 'Binary pair of dim red dwarfs. UV Ceti (one component) is a prototype flare star — brightens dramatically + briefly.' },
            { name: t('stem.astronomy.ross_154', 'Ross 154'), desig: '—', con: 'Sagittarius', spec: 'M3.5V', mag: 10.43, dist: 9.69, notes: 'Red-dwarf flare star.' },
            { name: t('stem.astronomy.ross_248', 'Ross 248'), desig: '—', con: 'Andromeda', spec: 'M5.5V', mag: 12.29, dist: 10.30, notes: 'Red dwarf. In ~ 30,000 years it will pass within 3 light-years of the Sun, the closest stellar approach in our history.' },
            { name: t('stem.astronomy.epsilon_eridani', 'Epsilon Eridani'), desig: 'ε Eri', con: 'Eridanus', spec: 'K2V', mag: 3.73, dist: 10.5, notes: 'Naked-eye young K-dwarf. Famous for being one of the first stars searched in SETI Project Ozma (1960). Has a debris disk + at least one planet.' },
            { name: t('stem.astronomy.lacaille_9352', 'Lacaille 9352'), desig: '—', con: 'Piscis Austrinus', spec: 'M0V', mag: 7.34, dist: 10.7, notes: 'Red dwarf. Frequently used as a baseline reference for nearby stars.' },
            { name: t('stem.astronomy.ross_128', 'Ross 128'), desig: '—', con: 'Virgo', spec: 'M4V', mag: 11.13, dist: 11.0, notes: 'Has a confirmed planet in the habitable zone (Ross 128 b, 2017). One of closest known potentially habitable exoplanets.' },
            { name: t('stem.astronomy.epsilon_indi', 'Epsilon Indi'), desig: 'ε Ind', con: 'Indus', spec: 'K5V', mag: 4.69, dist: 11.9, notes: 'Naked-eye K-dwarf. Has two brown-dwarf companions + a confirmed planet.' },
            { name: t('stem.astronomy.61_cygni', '61 Cygni'), desig: '61 Cyg', con: 'Cygnus', spec: 'K5V+K7V', mag: 5.2, dist: 11.4, notes: 'First star to have its parallax + distance measured (Bessel, 1838). Binary system. Bessel\'s measurement was a watershed in astronomy.' },
            { name: t('stem.astronomy.tau_ceti', 'Tau Ceti'), desig: 'τ Cet', con: 'Cetus', spec: 'G8V', mag: 3.5, dist: 11.9, notes: 'Closest sunlike star. SETI Project Ozma target. May have 4 planets, including one in the habitable zone.' },

            // Notable for variable / spectroscopic / multiple star reasons
            { name: t('stem.astronomy.algol', 'Algol'), desig: 'β Per', con: 'Perseus', spec: 'B8V+K0IV', mag: 2.12, dist: 90, notes: 'Prototype eclipsing binary. Every 2.87 days, dims from mag 2.1 to 3.4 for ~ 10 hours. Discovered to be eclipsing by Goodricke 1782.' },
            { name: t('stem.astronomy.mira', 'Mira'), desig: 'ο Cet', con: 'Cetus', spec: 'M7III', mag: 'Var 2-10', dist: 420, notes: 'Prototype long-period variable. Period 332 days, dimming from magnitude 2 to 10 (a factor of ~ 1000). Has a vast trailing tail of expelled gas 13 light-years long.' },
            { name: t('stem.astronomy.delta_cephei', 'Delta Cephei'), desig: 'δ Cep', con: 'Cepheus', spec: 'F5Iab', mag: 'Var 3.5-4.4', dist: 890, notes: 'Prototype CEPHEID variable — the basis for Henrietta Leavitt\'s period-luminosity calibration of the cosmic distance ladder.' },
            { name: t('stem.astronomy.polaris', 'Polaris'), desig: 'α UMi', con: 'Ursa Minor', spec: 'F7Ib', mag: 1.97, dist: 433, notes: 'Currently the North Star. Within 1° of celestial pole. Itself a triple system; the main star is a Cepheid variable with a small amplitude.' },
            { name: t('stem.astronomy.eta_carinae', 'Eta Carinae'), desig: 'η Car', con: 'Carina', spec: 'LBV', mag: 'Var 4-7', dist: 7500, notes: 'Luminous blue variable. Underwent the "Great Eruption" of 1843, briefly becoming the 2nd-brightest star in the night sky. Will eventually go supernova (or hypernova).' }
          ];
          ...
          return sectionCard('⭐ Named stars database',
            h('div', null,
              h('p', { style: { fontSize: 12, color: '#94a3b8', lineHeight: 1.55, marginBottom: 10 } }, t('stem.astronomy.bright_notable_named_stars_each_entry_', 'Bright + notable named stars. Each entry: name, Bayer designation, constellation, spectral class, apparent magnitude, distance in light-years, + significance. Search by name, filter by constellation, or restrict to nearest stars.')),

              h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 } },
                h('input', {
                  type: 'text',
                  placeholder: t('stem.astronomy.search_stars_sirius_vega', 'Search stars (Sirius, Vega…)'),
                  value: search,
                  onChange: function(e) { upd({ starsSearch: e.target.value }); },
                  'aria-label': t('stem.astronomy.search_stars', 'Search stars'),
                  className: 'astr-focus',
                ...
                h('select', {
                  ...
                  'aria-label': t('stem.astronomy.filter_by_constellation', 'Filter by constellation'),
                  className: 'astr-focus',
                  style: { padding: '4px 8px', borderRadius: 6, border: '1px solid #334155', background: '#0f172a', color: '#e2e8f0', fontSize: 11 }
                },
                  h('option', { value: '' }, t('stem.astronomy.all_constellations', 'All constellations')),
              ...
              h('div', { style: { marginBottom: 8 } },
                a11ySlider({
                  id: 'astr-stars-dist',
                  label: t('stem.astronomy.max_distance', 'Max distance'),
      ...
      function renderGalaxies() {
        return h('div', { style: { padding: 16 } },
          ...
          sectionCard('🌠 Types of galaxies',
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 } },
              [
                { name: t('stem.astronomy.spiral', 'Spiral'), desc: t('stem.astronomy.like_the_milky_way_and_andromeda_spira', 'Like the Milky Way and Andromeda. Spiral arms of gas, dust, and young stars wrapping around a central bulge. Active star formation continues in the arms.') },
                { name: t('stem.astronomy.elliptical', 'Elliptical'), desc: t('stem.astronomy.football_shaped_or_round_old_red_stars', 'Football-shaped or round. Old, red stars; little star formation. Often the result of galaxy mergers. The largest galaxies in the universe are giant ellipticals.') },
                { name: t('stem.astronomy.irregular', 'Irregular'), desc: t('stem.astronomy.no_clear_shape_often_dwarf_galaxies_ne', 'No clear shape. Often dwarf galaxies near larger ones (like the Magellanic Clouds), or galaxies whose shape has been disrupted by a recent merger.') },
                { name: t('stem.astronomy.active_galactic_nuclei_quasars', 'Active galactic nuclei (quasars)'), desc: t('stem.astronomy.galaxies_with_a_supermassive_black_hol', 'Galaxies with a supermassive black hole actively eating gas, releasing enormous energy. Quasars are the brightest things in the universe.') }
              ].map(function(g, i) {
              ...
              return h('div', null,
                ...
                (function() {
                  ...
                  return h('svg', { viewBox: '0 0 ' + svgW + ' ' + svgH, width: '100%', height: svgH, role: 'img', 'aria-labelledby': 'lensTitle lensDesc' },
                    h('title', { id: 'lensTitle' }, t('stem.astronomy.gravitational_lensing_diagram', 'Gravitational lensing diagram')),
                    ...
                    (function() {
                      if (Math.abs(offset) < 5) {
                        ...
                        return h('g', null,
                          ...
                          h('text', { x: cx, y: cy + ringR + 18, textAnchor: 'middle', fill: '#7dd3fc', fontSize: 11, fontWeight: 700 }, t('stem.astronomy.einstein_ring', 'Einstein ring'))
                        );
                        ...
                        return h('g', null,
                          ...
                          h('text', { x: cx, y: svgH - 16, textAnchor: 'middle', fill: '#7dd3fc', fontSize: 11, fontWeight: 700 }, t('stem.astronomy.lensed_arcs', 'Lensed arcs'))
                        );
                      } else {
                        ...
                        return h('g', null,
                          ...
                          h('ellipse', { cx: cx + ringR - offset / 2, cy: cy, rx: 8, ry: 12, fill: '#7dd3fc', opacity: 0.9 }),
                          h('text', { x: cx - 100, y: cy + 25, textAnchor: 'middle', fill: '#7dd3fc', fontSize: 10 }, t('stem.astronomy.image_1', 'Image 1')),
                          h('text', { x: cx + 100, y: cy + 25, textAnchor: 'middle', fill: '#7dd3fc', fontSize: 10 }, t('stem.astronomy.image_2', 'Image 2'))
                        );
                      }
                    })(),
                    ...
                    h('text', { x: cx, y: cy + lensSize + 22, textAnchor: 'middle', fill: '#fbbf24', fontSize: 11, fontWeight: 700 }, t('stem.astronomy.lens_massive_galaxy_cluster', 'Lens (massive galaxy cluster)')),
                    ...
                    h('g', { transform: 'translate(' + (cx + offset) + ',' + (cy + 60) + ')' },
                      ...
                      h('text', { x: 0, y: 18, textAnchor: 'middle', fill: '#94a3b8', fontSize: 9 }, t('stem.astronomy.true_source_position', 'true source position'))
                    )
                  );
                })(),
                ...
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginTop: 12, marginBottom: 12 } },
                  [
                    { label: t('stem.astronomy.lens_mass_10_m', 'Lens mass (×10¹⁴ M☉)'), value: mass, min: 10, max: 200, step: 5, key: 'lensMass' },
                    { label: t('stem.astronomy.source_offset_alignment', 'Source offset (alignment)'), value: offset, min: -80, max: 80, step: 2, key: 'lensOffset' }
                  ].map(function(s, i) {
                    return h('div', { key: i, style: { padding: 8, borderRadius: 6, background: '#1e293b', border: '1px solid #334155' } },
                      h('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 4 } },
                        h('span', { style: { fontSize: 11, color: '#94a3b8', fontWeight: 700 } }, s.label),
                ...
                h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.10)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 12, color: '#c7d2fe', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.what_we_do_with_lensing', 'What we DO with lensing: ')),
                  h('ul', { style: { margin: '6px 0 0 22px', padding: 0, lineHeight: 1.7 } },
                    h('li', null, h('strong', null, t('stem.astronomy.weigh_galaxy_clusters', 'Weigh galaxy clusters: ')), t('stem.astronomy.the_amount_of_distortion_tells_us_how_', 'The amount of distortion tells us how much mass is doing the bending. Clusters consistently show more lensing than visible matter alone explains — the strongest direct evidence for dark matter.')),
                    h('li', null, h('strong', null, t('stem.astronomy.magnify_the_distant_universe', 'Magnify the distant universe: ')), t('stem.astronomy.a_foreground_galaxy_cluster_magnifies_', 'A foreground galaxy cluster magnifies background galaxies behind it — sometimes by 30× or more. Hubble + JWST routinely point at "lensing clusters" (Abell 1689, MACS J0416) to see otherwise-too-faint objects.')),
                    h('li', null, h('strong', null, t('stem.astronomy.find_exoplanets', 'Find exoplanets: ')), t('stem.astronomy.microlensing_a_star_with_a_planet_pass', 'Microlensing — a star with a planet passing in front of a distant star slightly distorts the light. The planet causes a brief extra blip. Detects planets far from their stars + even rogue (unbound) planets.')),
                    h('li', null, h('strong', null, t('stem.astronomy.test_general_relativity', 'Test general relativity: ')), t('stem.astronomy.every_observed_lens_system_is_a_test_o', 'Every observed lens system is a test of Einstein\'s theory. So far it has passed every test, including in extreme regimes (LIGO BH mergers, EHT BH images).'))
                  )
                ),

                h('div', { style: { marginTop: 8, padding: 10, borderRadius: 8, background: 'rgba(245,158,11,0.10)', border: '1px solid rgba(245,158,11,0.3)', fontSize: 12, color: 'var(--allo-stem-text, #fde68a)', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.the_1919_confirmation', 'The 1919 confirmation: ')),
                  t('stem.astronomy.arthur_eddington_led_an_expedition_to_', 'Arthur Eddington led an expedition to the island of Príncipe to observe a total solar eclipse on May 29 1919. Without the Sun\'s glare, stars near its edge could be photographed. The shift in their apparent positions matched Einstein\'s prediction (~1.75 arcseconds at the solar limb), not Newton\'s (~0.87 arcseconds). News went global; on November 7 1919 the Times of London announced "Revolution in Science." Einstein went from physicist to public figure overnight.')
                )
              );
            })(),
            '#fbbf24'
          ...
          sectionCard('💥 The Big Bang + cosmic microwave background',
            (function() {
              var ERAS = [
                { time: '10⁻⁴³ s', name: t('stem.astronomy.planck_epoch', 'Planck epoch'), desc: t('stem.astronomy.earlier_than_this_our_physics_breaks_d', 'Earlier than this, our physics breaks down: gravity and quantum mechanics are not yet unified. We do not know what happened here.'), color: '#1e293b' },
                { time: '10⁻³⁶ to 10⁻³² s', name: t('stem.astronomy.inflation', 'Inflation'), desc: t('stem.astronomy.the_universe_expanded_faster_than_the_', 'The universe expanded faster than the speed of light by a factor of ~10²⁶. Smoothed out the early universe. Solves the "horizon problem" (why opposite sides of the sky have the same temperature). Quantum fluctuations during inflation became the seeds of all later structure.'), color: '#7c3aed' },
                { time: '10⁻⁶ s', name: t('stem.astronomy.quark_gluon_plasma', 'Quark-gluon plasma'), desc: t('stem.astronomy.free_quarks_and_gluons_in_a_hot_dense_', 'Free quarks and gluons in a hot dense soup. Recreated for tiny fractions of a second in particle accelerators (RHIC, LHC).'), color: '#a855f7' },
                { time: '1 second', name: t('stem.astronomy.hadron_neutrino_freeze_out', 'Hadron + neutrino freeze-out'), desc: t('stem.astronomy.quarks_combine_into_protons_neutrons_n', 'Quarks combine into protons + neutrons. Neutrinos decouple from matter and stream freely. These neutrinos still exist today as the "cosmic neutrino background" — at about 1.95 K. Not yet detected directly.'), color: '#0ea5e9' },
                { time: '3 minutes', name: t('stem.astronomy.big_bang_nucleosynthesis', 'Big Bang nucleosynthesis'), desc: t('stem.astronomy.the_universe_is_finally_cool_enough_1_', 'The universe is finally cool enough (~1 billion K) for protons + neutrons to fuse without immediately being broken up again. Hydrogen, helium-4 (~25% by mass), and trace amounts of deuterium, helium-3, lithium-7 form. ALL the heavier elements come later from stars + supernovae.'), color: '#fbbf24' },
                { time: '380,000 years', name: t('stem.astronomy.recombination_cmb', 'Recombination + CMB'), desc: t('stem.astronomy.the_universe_finally_cools_below_3_000', 'The universe finally cools below 3,000 K. Electrons combine with nuclei to form neutral atoms. Photons that had been ping-ponging through the plasma can now travel freely. Those photons are the cosmic microwave background — the oldest light we can see, redshifted by expansion to 2.725 K today.'), color: '#fbbf24' },
                { time: '~100-200 million years', name: t('stem.astronomy.first_stars', 'First stars'), desc: t('stem.astronomy.the_first_stars_population_iii_form_fr', 'The first stars ("Population III") form from primordial hydrogen + helium. Massive, hot, short-lived. End as supernovae, scattering the first heavier elements. We have not yet directly observed Population III stars — they are a key JWST target.'), color: '#ef4444' },
                { time: '~1 billion years', name: t('stem.astronomy.first_galaxies', 'First galaxies'), desc: t('stem.astronomy.small_galaxies_form_from_the_early_sta', 'Small galaxies form from the early stars + dark-matter halos. Mergers + accretion grow them. JWST is now imaging galaxies older than 13.4 billion years (formed within the first ~400 million years of the universe).'), color: '#22c55e' },
                { time: '9 billion years', name: t('stem.astronomy.solar_system_forms', 'Solar System forms'), desc: t('stem.astronomy.4_6_billion_years_ago_earth_assembles_', '4.6 billion years ago. Earth assembles from the dust + gas left over by previous generations of stars. Every heavy atom in your body was forged in stars that lived + died before our Sun even existed.'), color: '#10b981' },
                { time: '13.8 billion years', name: 'Now', desc: t('stem.astronomy.the_universe_is_still_expanding_accele', 'The universe is still expanding, accelerating (due to dark energy). The cosmic microwave background still surrounds us. About 4.9% ordinary matter, 26.8% dark matter, 68.3% dark energy by content.'), color: '#94a3b8' }
              ];
              return h('div', null,
                h('p', { style: { margin: '0 0 12px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                  t('stem.astronomy.the_big_bang_theory_says_the_universe_', 'The Big Bang theory says the universe began ~13.8 billion years ago as something incredibly hot + dense + small, and has been expanding + cooling ever since. The evidence is overwhelming: the cosmic microwave background, the abundance of light elements, the expansion of space (Hubble), and the cosmic structure we observe. It does NOT say the universe began from "nothing" — physics has no opinion on what came before the Planck epoch.')
                ),
          ...
          sectionCard('🪜 The cosmic distance ladder — how do we know?',
            h('div', null,
              h('p', { style: { margin: '0 0 12px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                t('stem.astronomy.no_single_method_measures_distances_ac', 'No single method measures distances across the universe. Astronomers use a series of overlapping methods, each calibrated by the rung below it. Each successive rung lets us measure farther. If any rung were wrong, every farther distance would be off too — which is why this work is constantly cross-checked.')
              ),
              h('div', { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
                [
                  { rung: '1', name: t('stem.astronomy.radar_lasers', 'Radar + lasers'), range: 'Solar system',
                    ...
                    color: '#22c55e' },
                  { rung: '2', name: t('stem.astronomy.stellar_parallax', 'Stellar parallax'), range: '~1,000 light-years (Gaia: ~30,000+ ly)',
                    ...
                    color: '#fbbf24' },
                  { rung: '3', name: t('stem.astronomy.main_sequence_fitting', 'Main-sequence fitting'), range: '~100,000 light-years',
                    ...
                    color: '#fbbf24' },
                  { rung: '4', name: t('stem.astronomy.cepheid_variables', 'Cepheid variables'), range: '~100 million light-years',
                    ...
                    color: '#fb923c' },
                  { rung: '5', name: t('stem.astronomy.type_ia_supernovae_2', 'Type Ia supernovae'), range: '~10 billion light-years',
                    ...
                    color: '#ef4444' },
                  { rung: '6', name: t('stem.astronomy.hubble_s_law_redshift', 'Hubble\'s law (redshift)'), range: 'Most of the observable universe',
                  ...
                  return h('div', { key: i, style: { padding: 10, borderRadius: 8, background: '#0f172a', borderTop: '1px solid #334155', borderRight: '1px solid #334155', borderBottom: '1px solid #334155', borderLeft: '4px solid ' + r.color } },
                    ...
                    h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.5, fontStyle: 'italic' } },
                      h('strong', null, t('stem.astronomy.why_we_trust_it', 'Why we trust it: ')), r.why
                    )
                  );
                })
              ),
          ...
          sectionCard('🌑 Dark matter + dark energy — the 95% we cannot see',
            (function() {
              var DM_TOPICS = [
                ...
                { id: 'tension', name: t('stem.astronomy.hubble_tension_a_real_problem', 'Hubble tension — a real problem'), emoji: '⚠️',
                  body: t('stem.astronomy.two_independent_ways_to_measure_the_cu', 'Two independent ways to measure the current expansion rate (Hubble constant, H₀) disagree. CMB + ΛCDM model gives ~67.4 km/s/Mpc (Planck 2018). The local distance ladder (Cepheids → Type Ia supernovae, SH0ES collaboration) gives ~73.0 km/s/Mpc. The discrepancy is ~9% and the statistical significance has grown to ~5σ. JWST measurements (2023-2024) confirm the local-ladder value with high precision, so it is not a Cepheid systematic. Either there is unrecognized systematic error somewhere, or our standard cosmological model (ΛCDM) is incomplete. Proposals: early dark energy, evolving Λ, sterile neutrinos, modified gravity — none yet decisive.'),
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(168,85,247,0.08)', border: '1px solid rgba(168,85,247,0.35)' } },
                  ...
                  h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.7, marginBottom: 10 } }, topic.body),
                  h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                    h('strong', null, t('stem.astronomy.what_we_should_not_overstate', 'What we should not overstate: ')), topic.caveat
                  )
                ),
                h('div', { style: { marginTop: 12, padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 11.5, color: '#c7d2fe', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.the_honest_framing', 'The honest framing: ')),
                  t('stem.astronomy.when_you_hear_scientists_have_proven_d', 'When you hear "scientists have proven dark matter," they have not. What is proven is that the data require SOMETHING we have not yet identified. That distinction is not a weakness of science — it is science working as designed, refusing to overstate what the evidence justifies.')
                )
              );
            })(),
            '#a855f7'
          ...
          sectionCard('🌌 Cosmic inflation + the multiverse question',
            (function() {
              var INF_TOPICS = [
                { id: 'whyneed', name: t('stem.astronomy.why_we_needed_inflation', 'Why we needed inflation'), emoji: '❓',
                  body: t('stem.astronomy.the_big_bang_theory_works_beautifully_', 'The Big Bang theory works beautifully — but in the 1970s it had three serious puzzles. (1) The horizon problem: the CMB is almost EXACTLY the same temperature in every direction (to 1 part in 100,000), even between regions that should never have been in causal contact at recombination. How could they reach the same temperature without ever exchanging heat? (2) The flatness problem: spacetime appears geometrically flat to extreme precision. Any tiny initial curvature would have grown over 13.8 billion years; the universe should be wildly curved by now, but it is not. (3) The monopole problem: Grand Unified Theories predict the early universe should have produced huge numbers of magnetic monopoles. We have never found one.'),
                ...
                { id: 'guth', name: t('stem.astronomy.guth_s_proposal_1980', 'Guth\'s proposal (1980)'), emoji: '💡',
                  body: t('stem.astronomy.alan_guth_then_a_young_postdoc_at_corn', 'Alan Guth (then a young postdoc at Cornell, working on monopoles) realized that if the very early universe went through a brief period of exponential expansion — doubling in size many dozens of times in a tiny fraction of a second — all three problems could be solved at once. The horizon problem: regions that look causally separated NOW were once in tiny causal contact BEFORE inflation, then stretched far apart. The flatness problem: exponential expansion smooths out any curvature, the way blowing up a balloon makes its surface look locally flat. The monopole problem: if monopoles formed before inflation, they got diluted to undetectable density. The brief moment of inflation needs to last from about 10⁻³⁶ to 10⁻³² seconds after t=0, expanding space by a factor of at least e^60 (~10²⁶).'),
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(6,182,212,0.08)', border: '1px solid rgba(6,182,212,0.35)' } },
                  ...
                  h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.7, marginBottom: 10 } }, topic.body),
                  h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                    h('strong', null, t('stem.astronomy.what_we_should_not_overstate_2', 'What we should not overstate: ')), topic.caveat
                  )
                )
              );
            })(),
            ...
            (function() {
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(14,165,233,0.08)', border: '1px solid rgba(14,165,233,0.35)' } },
                  ...
                  h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.7, marginBottom: 10 } }, topic.body),
                  h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                    h('strong', null, t('stem.astronomy.what_we_should_not_overstate_3', 'What we should not overstate: ')), topic.caveat
                  )
                )
              );
            })(),
            ...
            (function() {
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(20,184,166,0.08)', border: '1px solid rgba(20,184,166,0.35)' } },
                  ...
                  h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.7, marginBottom: 10 } }, topic.body),
                  h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                    h('strong', null, t('stem.astronomy.what_we_should_not_overstate_4', 'What we should not overstate: ')), topic.caveat
                  )
                )
              );
            })(),
          ...
          sectionCard('📡 Pulsars, magnetars, and fast radio bursts',
            (function() {
              ...
              return h('div', null,
                h('div', { style: { fontSize: 12.5, color: '#cbd5e1', lineHeight: 1.65, marginBottom: 12 } },
                  t('stem.astronomy.neutron_stars_are_the_densest_objects_', 'Neutron stars are the densest objects in the universe short of black holes. Some of them are pulsars, some are magnetars, and some appear to be the sources of fast radio bursts — millisecond flashes of radio energy bright enough to be seen across the universe. Each tells us something different about extreme physics.')
                ),
                h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 } },
                ...
                h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.35)' } },
                  ...
                  h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.7, marginBottom: 10 } }, topic.body),
                  h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                    h('strong', null, t('stem.astronomy.what_we_should_not_overstate_5', 'What we should not overstate: ')), topic.caveat
                  )
                )
              );
            })(),
            ...
            (function() {
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(251,146,60,0.08)', border: '1px solid rgba(251,146,60,0.35)' } },
                  ...
                  h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                    h('strong', null, t('stem.astronomy.on_the_calendar', 'On the calendar: ')), topic.context
                  )
                ),
        ...
        function messierCatalogSection() {
          ...
          var MESSIER = [
            { m: 1, name: t('stem.astronomy.crab_nebula', 'Crab Nebula'), type: 'SNR', con: 'Tau', mag: 8.4, season: 'Winter', notes: 'Supernova remnant from 1054 CE — Chinese records describe the "guest star." Contains the Crab Pulsar (PSR B0531+21, 33-millisecond rotation). The original "Messier object" — Charles Messier mistook it for a comet in 1758.' },
            { m: 2, name: t('stem.astronomy.m2_globular', 'M2 Globular'), type: 'GC', con: 'Aqr', mag: 6.5, season: 'Autumn', notes: 'Large globular cluster, ~ 175 light-years across, 37,500 ly distant. ~ 150,000 stars. Discovered by Maraldi in 1746.' },
            { m: 3, name: t('stem.astronomy.m3_globular', 'M3 Globular'), type: 'GC', con: 'CVn', mag: 6.2, season: 'Spring', notes: 'One of the brightest + most-studied globular clusters. 33,900 ly distant. ~ 500,000 stars. Contains many RR Lyrae variables used to calibrate the cosmic distance ladder.' },
            { m: 4, name: t('stem.astronomy.m4_globular', 'M4 Globular'), type: 'GC', con: 'Sco', mag: 5.6, season: 'Summer', notes: 'Closest globular cluster to Earth at 7,200 ly. Discovered by de Chéseaux 1746. Naked-eye from dark sites. Easy to find — 1.3° west of Antares.' },
            { m: 5, name: t('stem.astronomy.m5_globular', 'M5 Globular'), type: 'GC', con: 'Ser', mag: 5.7, season: 'Summer', notes: 'One of the oldest known globulars at ~ 13 billion years. 24,500 ly distant. Long-period variables in M5 helped calibrate stellar evolution.' },
            { m: 6, name: t('stem.astronomy.butterfly_cluster', 'Butterfly Cluster'), type: 'OC', con: 'Sco', mag: 4.2, season: 'Summer', notes: 'Bright open cluster shaped like a butterfly. ~ 1,600 ly distant, ~ 95 million years old.' },
            { m: 7, name: t('stem.astronomy.ptolemy_cluster', 'Ptolemy Cluster'), type: 'OC', con: 'Sco', mag: 3.3, season: 'Summer', notes: 'Largest + brightest open cluster known to Ptolemy in 130 CE. ~ 980 ly distant. Best southern Scorpius target.' },
            { m: 8, name: t('stem.astronomy.lagoon_nebula', 'Lagoon Nebula'), type: 'EN', con: 'Sgr', mag: 6.0, season: 'Summer', notes: 'Bright emission nebula + star cluster. ~ 4,100 ly distant. Naked-eye visible from dark sites. Contains the Hourglass Region, an active star-forming nursery.' },
            { m: 9, name: t('stem.astronomy.m9_globular', 'M9 Globular'), type: 'GC', con: 'Oph', mag: 7.7, season: 'Summer', notes: 'Small but distant globular near the galactic center. ~ 25,000 ly distant.' },
            { m: 10, name: t('stem.astronomy.m10_globular', 'M10 Globular'), type: 'GC', con: 'Oph', mag: 6.6, season: 'Summer', notes: 'Bright globular cluster. ~ 14,300 ly distant. ~ 200,000 stars.' },
            { m: 11, name: t('stem.astronomy.wild_duck_cluster', 'Wild Duck Cluster'), type: 'OC', con: 'Sct', mag: 5.8, season: 'Summer', notes: 'One of the most beautiful + richest open clusters — looks like a flock of ducks in flight. ~ 6,200 ly distant.' },
            { m: 12, name: t('stem.astronomy.m12_globular', 'M12 Globular'), type: 'GC', con: 'Oph', mag: 6.7, season: 'Summer', notes: 'Loose, sparse globular near M10. ~ 16,000 ly distant.' },
            { m: 13, name: t('stem.astronomy.hercules_cluster', 'Hercules Cluster'), type: 'GC', con: 'Her', mag: 5.8, season: 'Summer', notes: 'Best northern-hemisphere globular cluster. ~ 22,200 ly distant. ~ 300,000 stars. Target of the 1974 Arecibo Message (transmission aimed at it).' },
            { m: 14, name: t('stem.astronomy.m14_globular', 'M14 Globular'), type: 'GC', con: 'Oph', mag: 7.6, season: 'Summer', notes: 'Distant globular near the galactic center, ~ 30,300 ly. ~ 150,000 stars.' },
            { m: 15, name: t('stem.astronomy.m15_globular', 'M15 Globular'), type: 'GC', con: 'Peg', mag: 6.2, season: 'Autumn', notes: 'One of the oldest + densest globulars at ~ 12 Gyr. ~ 33,600 ly. Contains a possible intermediate-mass black hole at its core.' },
            { m: 16, name: t('stem.astronomy.eagle_nebula', 'Eagle Nebula'), type: 'EN', con: 'Ser', mag: 6.0, season: 'Summer', notes: 'Active star-forming region. Hubble\'s 1995 "Pillars of Creation" image of M16\'s central dust columns is one of the most iconic astronomy photographs. ~ 7,000 ly distant.' },
            { m: 17, name: t('stem.astronomy.omega_nebula', 'Omega Nebula'), type: 'EN', con: 'Sgr', mag: 6.0, season: 'Summer', notes: 'Bright emission nebula + active star formation region. Sometimes called the Swan Nebula or Horseshoe Nebula. ~ 5,500 ly distant.' },
            { m: 18, name: t('stem.astronomy.m18_cluster', 'M18 Cluster'), type: 'OC', con: 'Sgr', mag: 6.9, season: 'Summer', notes: 'Small open cluster, ~ 4,900 ly distant.' },
            { m: 19, name: t('stem.astronomy.m19_globular', 'M19 Globular'), type: 'GC', con: 'Oph', mag: 6.8, season: 'Summer', notes: 'One of the most-flattened globulars (due to galactic tidal forces). ~ 28,700 ly distant.' },
            { m: 20, name: t('stem.astronomy.trifid_nebula', 'Trifid Nebula'), type: 'EN+RN', con: 'Sgr', mag: 6.3, season: 'Summer', notes: 'Combination emission + reflection nebula divided into three lobes by dark dust lanes. ~ 5,200 ly distant. Photographically stunning.' },
            { m: 21, name: t('stem.astronomy.m21_cluster', 'M21 Cluster'), type: 'OC', con: 'Sgr', mag: 5.9, season: 'Summer', notes: 'Open cluster near M20. ~ 4,250 ly distant, ~ 4.6 million years old.' },
            { m: 22, name: t('stem.astronomy.sagittarius_cluster', 'Sagittarius Cluster'), type: 'GC', con: 'Sgr', mag: 5.1, season: 'Summer', notes: 'One of the brightest globular clusters in the sky. ~ 10,400 ly distant — relatively close, hence bright. Naked-eye visible.' },
            { m: 23, name: t('stem.astronomy.m23_cluster', 'M23 Cluster'), type: 'OC', con: 'Sgr', mag: 5.5, season: 'Summer', notes: 'Bright + rich open cluster, ~ 2,150 ly distant.' },
            { m: 24, name: t('stem.astronomy.sagittarius_star_cloud', 'Sagittarius Star Cloud'), type: 'SC', con: 'Sgr', mag: 4.6, season: 'Summer', notes: 'Not actually a cluster but a STAR CLOUD — a window through dust to a bright region of the galactic disk. ~ 10,000 ly through it. Among the densest naked-eye star regions.' },
            { m: 25, name: t('stem.astronomy.m25_cluster', 'M25 Cluster'), type: 'OC', con: 'Sgr', mag: 4.6, season: 'Summer', notes: 'Bright open cluster, ~ 2,000 ly distant. Easily naked-eye.' },
            { m: 26, name: t('stem.astronomy.m26_cluster', 'M26 Cluster'), type: 'OC', con: 'Sct', mag: 8.0, season: 'Summer', notes: 'Faint open cluster, ~ 5,000 ly distant.' },
            { m: 27, name: t('stem.astronomy.dumbbell_nebula', 'Dumbbell Nebula'), type: 'PN', con: 'Vul', mag: 7.5, season: 'Summer', notes: 'The first planetary nebula discovered (Messier 1764). Bright + easy through any telescope. ~ 1,360 ly distant. Distinctive double-lobed shape.' },
            { m: 28, name: t('stem.astronomy.m28_globular', 'M28 Globular'), type: 'GC', con: 'Sgr', mag: 6.8, season: 'Summer', notes: 'Bright globular cluster near the galactic center. ~ 17,900 ly distant.' },
            { m: 29, name: t('stem.astronomy.m29_cluster', 'M29 Cluster'), type: 'OC', con: 'Cyg', mag: 7.1, season: 'Summer', notes: 'Small open cluster in the Cygnus star fields. ~ 5,500 ly distant.' },
            { m: 30, name: t('stem.astronomy.m30_globular', 'M30 Globular'), type: 'GC', con: 'Cap', mag: 7.2, season: 'Autumn', notes: 'Compact globular cluster, ~ 26,100 ly distant. Dense core has undergone "core collapse."' },
            { m: 31, name: t('stem.astronomy.andromeda_galaxy', 'Andromeda Galaxy'), type: 'G', con: 'And', mag: 3.4, season: 'Autumn', notes: 'Largest galaxy in the Local Group. ~ 2.5 million ly distant — the FARTHEST object visible to the naked eye. Will collide with the Milky Way in ~ 4 billion years.' },
            { m: 32, name: t('stem.astronomy.m32_dwarf_galaxy', 'M32 Dwarf Galaxy'), type: 'G', con: 'And', mag: 8.1, season: 'Autumn', notes: 'Compact elliptical satellite galaxy of M31. ~ 2.5 million ly distant.' },
            { m: 33, name: t('stem.astronomy.triangulum_galaxy', 'Triangulum Galaxy'), type: 'G', con: 'Tri', mag: 5.7, season: 'Autumn', notes: '3rd-largest member of the Local Group. ~ 2.7 million ly distant. Difficult to see — very LOW SURFACE BRIGHTNESS. Dark skies are essential.' },
            { m: 34, name: t('stem.astronomy.m34_cluster', 'M34 Cluster'), type: 'OC', con: 'Per', mag: 5.2, season: 'Autumn', notes: 'Bright open cluster, ~ 1,500 ly distant, ~ 200 million years old.' },
            { m: 35, name: t('stem.astronomy.m35_cluster', 'M35 Cluster'), type: 'OC', con: 'Gem', mag: 5.1, season: 'Winter', notes: 'Beautiful open cluster, ~ 2,800 ly distant. ~ 200 stars. Often paired with the smaller NGC 2158 just nearby.' },
            { m: 36, name: t('stem.astronomy.pinwheel_cluster_auriga', 'Pinwheel Cluster (Auriga)'), type: 'OC', con: 'Aur', mag: 6.3, season: 'Winter', notes: 'Bright open cluster, ~ 4,100 ly distant. Part of the trio M36/37/38 in Auriga.' },
            { m: 37, name: t('stem.astronomy.m37_cluster', 'M37 Cluster'), type: 'OC', con: 'Aur', mag: 5.6, season: 'Winter', notes: 'The richest open cluster in Auriga. ~ 4,400 ly distant. Hundreds of resolved stars.' },
            { m: 38, name: t('stem.astronomy.starfish_cluster', 'Starfish Cluster'), type: 'OC', con: 'Aur', mag: 6.4, season: 'Winter', notes: 'Bright open cluster. ~ 3,400 ly distant. Pairs nicely with adjacent M37 + M36.' },
            { m: 39, name: t('stem.astronomy.m39_cluster', 'M39 Cluster'), type: 'OC', con: 'Cyg', mag: 4.6, season: 'Summer', notes: 'Loose, bright open cluster in northern Cygnus. ~ 800 ly distant.' },
            { m: 40, name: t('stem.astronomy.winnecke_4', 'Winnecke 4'), type: 'D*', con: 'UMa', mag: 8.4, season: 'Spring', notes: 'A simple DOUBLE STAR — Messier catalogued it as a deep-sky object in error. Often noted as a "stupid Messier mistake" + retained for completeness.' },
            { m: 41, name: t('stem.astronomy.m41_cluster', 'M41 Cluster'), type: 'OC', con: 'CMa', mag: 4.5, season: 'Winter', notes: 'Naked-eye open cluster just south of Sirius. ~ 2,300 ly distant. Aristotle\'s "cloudy spot."' },
            { m: 42, name: t('stem.astronomy.orion_nebula', 'Orion Nebula'), type: 'EN', con: 'Ori', mag: 4.0, season: 'Winter', notes: 'The most famous emission nebula. ~ 1,344 ly distant. Active stellar nursery containing the Trapezium young open cluster.' },
            { m: 43, name: t('stem.astronomy.de_mairan_s_nebula', 'De Mairan\'s Nebula'), type: 'EN', con: 'Ori', mag: 9.0, season: 'Winter', notes: 'Companion nebula to M42, separated by a dark dust lane. Often considered part of the same emission complex.' },
            { m: 44, name: t('stem.astronomy.beehive_cluster', 'Beehive Cluster'), type: 'OC', con: 'Cnc', mag: 3.7, season: 'Spring', notes: 'Bright + nearby open cluster. ~ 600 ly distant. Naked-eye from dark sites. Roman name "Praesepe" (manger). One of the closest open clusters to the Sun.' },
            { m: 45, name: t('stem.astronomy.pleiades', 'Pleiades'), type: 'OC', con: 'Tau', mag: 1.6, season: 'Winter', notes: 'The "Seven Sisters." Brightest + closest open cluster. ~ 444 ly distant. Naked-eye prominent. Surrounded by reflection nebulosity from dust the cluster is passing through.' },
            { m: 46, name: t('stem.astronomy.m46_cluster', 'M46 Cluster'), type: 'OC', con: 'Pup', mag: 6.1, season: 'Winter', notes: 'Rich open cluster with a planetary nebula (NGC 2438) embedded — though the PN is a foreground object, not actually in the cluster. ~ 5,400 ly distant.' },
            { m: 47, name: t('stem.astronomy.m47_cluster', 'M47 Cluster'), type: 'OC', con: 'Pup', mag: 4.4, season: 'Winter', notes: 'Bright young open cluster, ~ 1,600 ly distant.' },
            { m: 48, name: t('stem.astronomy.m48_cluster', 'M48 Cluster'), type: 'OC', con: 'Hya', mag: 5.5, season: 'Spring', notes: 'Bright, scattered open cluster — Messier got the position wrong originally + the modern recovery was done by Oswald Thomas in 1934. ~ 1,500 ly distant.' },
            { m: 49, name: t('stem.astronomy.m49_galaxy', 'M49 Galaxy'), type: 'G', con: 'Vir', mag: 8.4, season: 'Spring', notes: 'Brightest member of the Virgo Cluster of galaxies. Giant elliptical. ~ 56 million ly distant.' },
            { m: 50, name: t('stem.astronomy.m50_cluster', 'M50 Cluster'), type: 'OC', con: 'Mon', mag: 5.9, season: 'Winter', notes: 'Heart-shaped open cluster, ~ 3,200 ly distant.' },
            { m: 51, name: t('stem.astronomy.whirlpool_galaxy', 'Whirlpool Galaxy'), type: 'G', con: 'CVn', mag: 8.4, season: 'Spring', notes: 'Beautiful face-on interacting spiral galaxy with a small companion (NGC 5195) connected by a tidal arm. ~ 31 million ly distant. Lord Rosse identified its spiral structure in 1845.' },
            { m: 52, name: t('stem.astronomy.m52_cluster', 'M52 Cluster'), type: 'OC', con: 'Cas', mag: 6.9, season: 'Autumn', notes: 'Bright open cluster, ~ 5,000 ly distant.' },
            { m: 53, name: t('stem.astronomy.m53_globular', 'M53 Globular'), type: 'GC', con: 'Com', mag: 7.6, season: 'Spring', notes: 'Faint globular cluster, ~ 58,000 ly distant — one of the most-distant Messier globulars.' },
            { m: 54, name: t('stem.astronomy.m54_globular', 'M54 Globular'), type: 'GC', con: 'Sgr', mag: 7.6, season: 'Summer', notes: 'Actually a globular cluster of the SAGITTARIUS DWARF GALAXY (a satellite of the Milky Way currently being absorbed). The first known extragalactic globular cluster (recognized as such in 1994).' },
            { m: 55, name: t('stem.astronomy.m55_globular', 'M55 Globular'), type: 'GC', con: 'Sgr', mag: 6.3, season: 'Summer', notes: 'Bright but low globular for northern observers, ~ 17,300 ly distant.' },
            { m: 56, name: t('stem.astronomy.m56_globular', 'M56 Globular'), type: 'GC', con: 'Lyr', mag: 8.3, season: 'Summer', notes: 'Faint globular near M27, ~ 32,900 ly distant.' },
            { m: 57, name: t('stem.astronomy.ring_nebula', 'Ring Nebula'), type: 'PN', con: 'Lyr', mag: 8.8, season: 'Summer', notes: 'The textbook planetary nebula. Visible smoke ring at any magnification. ~ 2,300 ly distant.' },
            { m: 58, name: t('stem.astronomy.m58_galaxy', 'M58 Galaxy'), type: 'G', con: 'Vir', mag: 9.7, season: 'Spring', notes: 'Barred spiral in the Virgo Cluster. ~ 62 million ly distant.' },
            { m: 59, name: t('stem.astronomy.m59_galaxy', 'M59 Galaxy'), type: 'G', con: 'Vir', mag: 9.6, season: 'Spring', notes: 'Elliptical galaxy in the Virgo Cluster, ~ 60 million ly distant.' },
            { m: 60, name: t('stem.astronomy.m60_galaxy', 'M60 Galaxy'), type: 'G', con: 'Vir', mag: 8.8, season: 'Spring', notes: 'Bright elliptical in the Virgo Cluster, often paired with the smaller M59. ~ 55 million ly distant.' },
            { m: 61, name: t('stem.astronomy.m61_galaxy', 'M61 Galaxy'), type: 'G', con: 'Vir', mag: 9.7, season: 'Spring', notes: 'Face-on spiral in the Virgo Cluster, ~ 52 million ly distant.' },
            { m: 62, name: t('stem.astronomy.m62_globular', 'M62 Globular'), type: 'GC', con: 'Oph', mag: 6.5, season: 'Summer', notes: 'One of the most-distorted globulars (close to galactic center). ~ 22,500 ly distant.' },
            { m: 63, name: t('stem.astronomy.sunflower_galaxy', 'Sunflower Galaxy'), type: 'G', con: 'CVn', mag: 8.6, season: 'Spring', notes: 'Flocculent spiral galaxy. ~ 27 million ly distant. Many tightly-wound spiral arms.' },
            { m: 64, name: t('stem.astronomy.black_eye_galaxy', 'Black Eye Galaxy'), type: 'G', con: 'Com', mag: 8.5, season: 'Spring', notes: 'Spiral galaxy with a striking dark dust lane on one side, like a "black eye." ~ 17 million ly distant.' },
            { m: 65, name: t('stem.astronomy.m65_leo_triplet', 'M65 (Leo Triplet)'), type: 'G', con: 'Leo', mag: 9.3, season: 'Spring', notes: 'Member of the LEO TRIPLET of galaxies along with M66 + NGC 3628. ~ 35 million ly distant.' },
            { m: 66, name: t('stem.astronomy.m66_leo_triplet', 'M66 (Leo Triplet)'), type: 'G', con: 'Leo', mag: 8.9, season: 'Spring', notes: 'Brightest member of the Leo Triplet. ~ 35 million ly distant. Significant tidal distortion.' },
            { m: 67, name: t('stem.astronomy.m67_cluster', 'M67 Cluster'), type: 'OC', con: 'Cnc', mag: 6.1, season: 'Spring', notes: 'One of the OLDEST known open clusters at ~ 4 billion years old. ~ 2,700 ly distant. Used to study stellar evolution in solar-mass stars.' },
            { m: 68, name: t('stem.astronomy.m68_globular', 'M68 Globular'), type: 'GC', con: 'Hya', mag: 7.7, season: 'Spring', notes: 'Distant southern globular, ~ 33,300 ly distant.' },
            { m: 69, name: t('stem.astronomy.m69_globular', 'M69 Globular'), type: 'GC', con: 'Sgr', mag: 7.7, season: 'Summer', notes: 'Faint globular near the galactic center. ~ 29,700 ly distant.' },
            { m: 70, name: t('stem.astronomy.m70_globular', 'M70 Globular'), type: 'GC', con: 'Sgr', mag: 7.9, season: 'Summer', notes: 'Faint globular near the galactic center. ~ 29,400 ly distant. Hale-Bopp comet was discovered while observing M70 in 1995.' },
            { m: 71, name: t('stem.astronomy.m71_globular', 'M71 Globular'), type: 'GC', con: 'Sge', mag: 8.2, season: 'Summer', notes: 'Loose globular cluster — was long considered an intermediate or "transition" between open + globular. ~ 13,000 ly distant.' },
            { m: 72, name: t('stem.astronomy.m72_globular', 'M72 Globular'), type: 'GC', con: 'Aqr', mag: 9.4, season: 'Autumn', notes: 'Distant globular cluster, ~ 54,800 ly distant.' },
            { m: 73, name: t('stem.astronomy.m73_asterism', 'M73 Asterism'), type: 'AS', con: 'Aqr', mag: 8.9, season: 'Autumn', notes: 'NOT a true cluster — just 4 random stars that happen to align from our viewpoint. Catalogued by Messier in error. Some recent disputes about whether to retain it; the IAU keeps it for completeness.' },
            { m: 74, name: t('stem.astronomy.phantom_galaxy', 'Phantom Galaxy'), type: 'G', con: 'Psc', mag: 9.4, season: 'Autumn', notes: 'Beautiful face-on spiral but very LOW SURFACE BRIGHTNESS — challenging visual target. Dramatic in long-exposure photography. ~ 32 million ly distant.' },
            { m: 75, name: t('stem.astronomy.m75_globular', 'M75 Globular'), type: 'GC', con: 'Sgr', mag: 8.5, season: 'Summer', notes: 'Compact, distant globular at ~ 67,500 ly — one of the most-distant Messier globulars.' },
            { m: 76, name: t('stem.astronomy.little_dumbbell_nebula', 'Little Dumbbell Nebula'), type: 'PN', con: 'Per', mag: 10.1, season: 'Autumn', notes: 'A small bipolar planetary nebula similar in shape to M27 but much fainter + more distant. ~ 2,500 ly distant. One of the faintest Messier objects.' },
            { m: 77, name: t('stem.astronomy.m77_galaxy', 'M77 Galaxy'), type: 'G', con: 'Cet', mag: 8.9, season: 'Autumn', notes: 'A Seyfert-2 type AGN (active galactic nucleus) — one of the brightest + closest active galaxies. ~ 47 million ly distant.' },
            { m: 78, name: t('stem.astronomy.m78_nebula', 'M78 Nebula'), type: 'RN', con: 'Ori', mag: 8.3, season: 'Winter', notes: 'Reflection nebula in Orion. ~ 1,600 ly distant. Active region with embedded young stars.' },
            { m: 79, name: t('stem.astronomy.m79_globular', 'M79 Globular'), type: 'GC', con: 'Lep', mag: 7.7, season: 'Winter', notes: 'Unusual: a southern globular cluster that may have been captured from the Canis Major Dwarf Galaxy. ~ 41,000 ly distant.' },
            { m: 80, name: t('stem.astronomy.m80_globular', 'M80 Globular'), type: 'GC', con: 'Sco', mag: 7.3, season: 'Summer', notes: 'Compact globular cluster, ~ 33,000 ly distant. ~ 200,000 stars in a dense core.' },
            { m: 81, name: t('stem.astronomy.bode_s_galaxy', 'Bode\'s Galaxy'), type: 'G', con: 'UMa', mag: 6.9, season: 'Spring', notes: 'A grand-design spiral, one of the brightest galaxies in the sky. ~ 12 million ly distant. Often paired with M82.' },
            { m: 82, name: t('stem.astronomy.cigar_galaxy', 'Cigar Galaxy'), type: 'G', con: 'UMa', mag: 8.4, season: 'Spring', notes: 'A starburst galaxy + irregular interaction partner of M81. The CLOSEST starburst galaxy to Earth. ~ 12 million ly distant. SN 2014J occurred here — closest naked-eye supernova in decades.' },
            { m: 83, name: t('stem.astronomy.southern_pinwheel', 'Southern Pinwheel'), type: 'G', con: 'Hya', mag: 7.5, season: 'Spring', notes: 'A beautiful barred spiral, well-imaged but LOW from Northern Hemisphere latitudes. ~ 15 million ly distant.' },
            { m: 84, name: t('stem.astronomy.m84_galaxy', 'M84 Galaxy'), type: 'G', con: 'Vir', mag: 9.1, season: 'Spring', notes: 'Lenticular galaxy in the Virgo Cluster. Part of Markarian\'s Chain — a string of galaxies including M84 + M86. ~ 60 million ly distant.' },
            { m: 85, name: t('stem.astronomy.m85_galaxy', 'M85 Galaxy'), type: 'G', con: 'Com', mag: 9.1, season: 'Spring', notes: 'Lenticular galaxy at the northern edge of the Virgo Cluster, ~ 60 million ly distant.' },
            { m: 86, name: t('stem.astronomy.m86_galaxy', 'M86 Galaxy'), type: 'G', con: 'Vir', mag: 8.9, season: 'Spring', notes: 'Lenticular giant in the Virgo Cluster, ~ 52 million ly distant. Part of Markarian\'s Chain.' },
            { m: 87, name: t('stem.astronomy.virgo_a', 'Virgo A'), type: 'G', con: 'Vir', mag: 8.6, season: 'Spring', notes: 'Giant elliptical galaxy + the dominant member of the Virgo Cluster. Hosts a SUPERMASSIVE BLACK HOLE (6.5 billion solar masses) — the first directly imaged by the Event Horizon Telescope (2019). ~ 53 million ly distant.' },
            { m: 88, name: t('stem.astronomy.m88_galaxy', 'M88 Galaxy'), type: 'G', con: 'Com', mag: 9.6, season: 'Spring', notes: 'Spiral galaxy in the Virgo Cluster, ~ 47 million ly distant.' },
            { m: 89, name: t('stem.astronomy.m89_galaxy', 'M89 Galaxy'), type: 'G', con: 'Vir', mag: 9.8, season: 'Spring', notes: 'Elliptical galaxy in the Virgo Cluster, ~ 50 million ly distant.' },
            { m: 90, name: t('stem.astronomy.m90_galaxy', 'M90 Galaxy'), type: 'G', con: 'Vir', mag: 9.5, season: 'Spring', notes: 'Spiral galaxy in the Virgo Cluster, ~ 60 million ly distant. Unusual blueshift — approaching us rather than receding.' },
            { m: 91, name: t('stem.astronomy.m91_galaxy', 'M91 Galaxy'), type: 'G', con: 'Com', mag: 10.2, season: 'Spring', notes: 'Barred spiral in the Virgo Cluster. ~ 63 million ly distant. Faint visual target.' },
            { m: 92, name: t('stem.astronomy.m92_globular', 'M92 Globular'), type: 'GC', con: 'Her', mag: 6.4, season: 'Summer', notes: 'Often-overlooked partner to M13. ~ 26,700 ly distant. Comparable in size + age but less dramatic than M13.' },
            { m: 93, name: t('stem.astronomy.m93_cluster', 'M93 Cluster'), type: 'OC', con: 'Pup', mag: 6.2, season: 'Winter', notes: 'Bright open cluster, ~ 3,400 ly distant.' },
            { m: 94, name: t('stem.astronomy.m94_galaxy', 'M94 Galaxy'), type: 'G', con: 'CVn', mag: 8.2, season: 'Spring', notes: 'A "ringed" galaxy with a starburst central region. ~ 16 million ly distant.' },
            { m: 95, name: t('stem.astronomy.m95_galaxy', 'M95 Galaxy'), type: 'G', con: 'Leo', mag: 9.7, season: 'Spring', notes: 'Barred spiral, ~ 33 million ly distant. Part of the Leo I Group.' },
            { m: 96, name: t('stem.astronomy.m96_galaxy', 'M96 Galaxy'), type: 'G', con: 'Leo', mag: 9.2, season: 'Spring', notes: 'Spiral galaxy, ~ 31 million ly distant. Part of the Leo I Group.' },
            { m: 97, name: t('stem.astronomy.owl_nebula', 'Owl Nebula'), type: 'PN', con: 'UMa', mag: 9.9, season: 'Spring', notes: 'Planetary nebula with two "eye-like" dark patches. ~ 2,000 ly distant. Best with an OIII filter.' },
            { m: 98, name: t('stem.astronomy.m98_galaxy', 'M98 Galaxy'), type: 'G', con: 'Com', mag: 10.1, season: 'Spring', notes: 'Edge-on spiral in the Virgo Cluster, ~ 60 million ly distant.' },
            { m: 99, name: t('stem.astronomy.coma_pinwheel', 'Coma Pinwheel'), type: 'G', con: 'Com', mag: 9.9, season: 'Spring', notes: 'Face-on spiral in the Virgo Cluster, ~ 50 million ly distant. Asymmetric arms suggest gravitational interaction.' },
            { m: 100, name: t('stem.astronomy.m100_galaxy', 'M100 Galaxy'), type: 'G', con: 'Com', mag: 9.3, season: 'Spring', notes: 'Bright face-on spiral in the Virgo Cluster. ~ 55 million ly distant. Photogenic.' },
            { m: 101, name: t('stem.astronomy.pinwheel_galaxy', 'Pinwheel Galaxy'), type: 'G', con: 'UMa', mag: 7.9, season: 'Spring', notes: 'Beautiful face-on spiral, one of the largest galaxies in the Local Universe. ~ 21 million ly distant. LARGE — easily fills a long-focal-length field of view.' },
            { m: 102, name: t('stem.astronomy.ngc_5866_spindle', 'NGC 5866 / Spindle'), type: 'G', con: 'Dra', mag: 9.9, season: 'Spring', notes: 'Edge-on lenticular galaxy, ~ 50 million ly distant. (M102 is contested — some scholars argue it was a duplicate of M101.)' },
            { m: 103, name: t('stem.astronomy.m103_cluster', 'M103 Cluster'), type: 'OC', con: 'Cas', mag: 7.4, season: 'Autumn', notes: 'Faint open cluster, ~ 8,500 ly distant.' },
            { m: 104, name: t('stem.astronomy.sombrero_galaxy', 'Sombrero Galaxy'), type: 'G', con: 'Vir', mag: 8.0, season: 'Spring', notes: 'Iconic edge-on spiral with a prominent dust lane. ~ 29 million ly distant. The "sombrero" hat shape from the bright central bulge + the dark equatorial dust ring.' },
            { m: 105, name: t('stem.astronomy.m105_galaxy', 'M105 Galaxy'), type: 'G', con: 'Leo', mag: 9.8, season: 'Spring', notes: 'Elliptical galaxy, ~ 32 million ly distant. Part of the Leo I Group with M95 + M96.' },
            { m: 106, name: t('stem.astronomy.m106_galaxy', 'M106 Galaxy'), type: 'G', con: 'CVn', mag: 8.4, season: 'Spring', notes: 'Spiral galaxy with an active galactic nucleus + a SUPERMASSIVE BLACK HOLE (40 million solar masses). The water-maser distance to M106 was used to calibrate the cosmic distance ladder. ~ 25 million ly distant.' },
            { m: 107, name: t('stem.astronomy.m107_globular', 'M107 Globular'), type: 'GC', con: 'Oph', mag: 7.9, season: 'Summer', notes: 'Loose globular cluster, ~ 20,900 ly distant.' },
            { m: 108, name: t('stem.astronomy.m108_galaxy', 'M108 Galaxy'), type: 'G', con: 'UMa', mag: 10.0, season: 'Spring', notes: 'Edge-on spiral, ~ 46 million ly distant. Near M97 in the same field of view.' },
            { m: 109, name: t('stem.astronomy.m109_galaxy', 'M109 Galaxy'), type: 'G', con: 'UMa', mag: 9.8, season: 'Spring', notes: 'Barred spiral, ~ 84 million ly distant.' },
            { m: 110, name: t('stem.astronomy.m110_dwarf', 'M110 Dwarf'), type: 'G', con: 'And', mag: 8.5, season: 'Autumn', notes: 'Elliptical satellite galaxy of Andromeda (M31). The LAST Messier object — added 1967 (Messier observed it but did not catalog it). ~ 2.7 million ly distant.' }
          ];
            ...
            h('div', null,
              ...
              h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 } },
                h('select', {
                  ...
                  'aria-label': t('stem.astronomy.filter_by_object_type', 'Filter by object type'),
                  className: 'astr-focus',
                  style: { padding: '4px 8px', borderRadius: 6, border: '1px solid #334155', background: '#0f172a', color: '#e2e8f0', fontSize: 11 }
                },
                  h('option', { value: 'all' }, t('stem.astronomy.all_types', 'All types')),
                  h('option', { value: 'G' }, t('stem.astronomy.galaxies_2', 'Galaxies')),
                  h('option', { value: 'GC' }, t('stem.astronomy.globular_clusters', 'Globular clusters')),
                  h('option', { value: 'OC' }, t('stem.astronomy.open_clusters', 'Open clusters')),
                  h('option', { value: 'EN' }, t('stem.astronomy.emission_nebulae', 'Emission nebulae')),
                  h('option', { value: 'PN' }, t('stem.astronomy.planetary_nebulae', 'Planetary nebulae')),
                  h('option', { value: 'SNR' }, t('stem.astronomy.supernova_remnants', 'Supernova remnants')),
                  h('option', { value: 'RN' }, t('stem.astronomy.reflection_nebulae', 'Reflection nebulae'))
                ),
                h('select', {
                  value: seasonFilter,
                  onChange: function(e) { upd({ messierSeason: e.target.value }); },
                  'aria-label': t('stem.astronomy.filter_by_season', 'Filter by season'),
                  className: 'astr-focus',
                  style: { padding: '4px 8px', borderRadius: 6, border: '1px solid #334155', background: '#0f172a', color: '#e2e8f0', fontSize: 11 }
                },
                  h('option', { value: 'all' }, t('stem.astronomy.all_seasons', 'All seasons')),
                  h('option', { value: 'Winter' }, t('stem.astronomy.winter_dec_feb', 'Winter (Dec-Feb)')),
                  h('option', { value: 'Spring' }, t('stem.astronomy.spring_mar_may', 'Spring (Mar-May)')),
                  h('option', { value: 'Summer' }, t('stem.astronomy.summer_jun_aug', 'Summer (Jun-Aug)')),
                  h('option', { value: 'Autumn' }, t('stem.astronomy.autumn_sep_nov', 'Autumn (Sep-Nov)'))
                ),
                h('input', {
                  type: 'text',
                  placeholder: t('stem.astronomy.search_messier_m13_ngc_orion', 'Search Messier (M13, NGC, Orion…)'),
                  value: search,
                  onChange: function(e) { upd({ messierSearch: e.target.value }); },
                  'aria-label': t('stem.astronomy.search_messier_catalog', 'Search Messier catalog'),
                  className: 'astr-focus',
                  style: { flex: 1, minWidth: 140, padding: '5px 10px', borderRadius: 6, border: '1px solid #334155', background: '#0f172a', color: '#e2e8f0', fontSize: 11 }
                })
              ),
      ...
      function renderEvents() {
        return h('div', { style: { padding: 16 } },
          ...
          sectionCard('☄️ Meteorites + comets — visitors from deep space',
            (function() {
              var ITEMS = [
                { id: 'murchison', name: t('stem.astronomy.murchison_meteorite', 'Murchison meteorite'), when: 'Fell Sep 28 1969, Australia', color: '#22c55e',
                ...
                { id: 'allende', name: t('stem.astronomy.allende_meteorite', 'Allende meteorite'), when: 'Fell Feb 8 1969, Mexico', color: '#fbbf24',
                ...
                { id: 'mars', name: t('stem.astronomy.mars_meteorites_300_known', 'Mars meteorites (~300 known)'), when: 'Various', color: '#dc2626',
                ...
                { id: '67p', name: t('stem.astronomy.67p_churyumov_gerasimenko_rosetta', '67P/Churyumov-Gerasimenko (Rosetta)'), when: 'Visited 2014-2016', color: '#0ea5e9',
                ...
                { id: 'oumuamua', name: t('stem.astronomy.oumuamua_1i_2017_u1', 'ʻOumuamua (1I/2017 U1)'), when: 'Discovered Oct 19 2017', color: '#a855f7',
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 12, background: '#0f172a', borderTop: '1px solid #334155', borderRight: '1px solid #334155', borderBottom: '1px solid #334155', borderLeft: '3px solid ' + sel.color } },
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(56,189,248,0.08)', borderLeft: '3px solid #38bdf8', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#7dd3fc', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.what_it_is_2', 'What it is')),
                    h('div', { style: { fontSize: 12, color: '#e2e8f0', lineHeight: 1.6 } }, sel.what)
                  ),
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(245,158,11,0.08)', borderLeft: '3px solid #f59e0b', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#fbbf24', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.why_it_matters_2', 'Why it matters')),
                    h('div', { style: { fontSize: 12, color: '#e2e8f0', lineHeight: 1.6 } }, sel.why)
                  ),
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(168,85,247,0.08)', borderLeft: '3px solid #a855f7' } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#d8b4fe', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.worth_knowing', 'Worth knowing')),
                    h('div', { style: { fontSize: 12, color: '#e2e8f0', lineHeight: 1.6 } }, sel.cool)
                  )
                ),
                h('div', { style: { marginTop: 12, padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.10)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 12, color: '#c7d2fe', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.meteorite_classification_simplified', 'Meteorite classification (simplified): ')),
                  h('ul', { style: { margin: '6px 0 0 22px', padding: 0, lineHeight: 1.7 } },
                    h('li', null, h('strong', null, t('stem.astronomy.chondrites_86_of_falls', 'Chondrites (86% of falls): ')), t('stem.astronomy.stony_primitive_unmelted_contain_chond', 'Stony, primitive, unmelted. Contain chondrules — millimeter-scale glass spheres formed in the early solar nebula. Carbonaceous chondrites are the most pristine (Murchison, Allende).')),
                    h('li', null, h('strong', null, t('stem.astronomy.achondrites_8_of_falls', 'Achondrites (8% of falls): ')), t('stem.astronomy.stony_but_melted_differentiated_came_f', 'Stony but melted + differentiated. Came from bodies large enough to have melted internally. Mars + Moon meteorites are achondrites.')),
                    h('li', null, h('strong', null, t('stem.astronomy.iron_meteorites_5_of_falls', 'Iron meteorites (5% of falls): ')), t('stem.astronomy.metallic_iron_nickel_from_the_cores_of', 'Metallic iron-nickel from the cores of destroyed planetesimals. The Widmanstätten pattern (cooled crystalline pattern) takes millions of years to form + cannot be faked.')),
                    h('li', null, h('strong', null, t('stem.astronomy.stony_irons_1', 'Stony-irons (1%): ')), t('stem.astronomy.mix_of_stone_metal_pallasites_are_the_', 'Mix of stone + metal. Pallasites are the most beautiful — translucent olivine crystals embedded in iron-nickel matrix.'))
                  )
                ),
                h('div', { style: { marginTop: 8, padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.10)', border: '1px solid rgba(34,197,94,0.3)', fontSize: 12, color: '#a7f3d0', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.where_did_earth_s_water_come_from_2', 'Where did Earth\'s water come from? ')),
                  t('stem.astronomy.for_decades_the_leading_theory_was_com', 'For decades the leading theory was: comets delivered water. But Rosetta + other missions showed comet water has DIFFERENT deuterium/hydrogen ratios than Earth\'s oceans. Current best guess: most water came from carbonaceous asteroids during Earth\'s formation; comets contributed perhaps 10%. Active research; still being refined.')
                )
              );
            })(),
            '#22c55e'
          ...
          sectionCard('☄️ Asteroids + planetary defense',
            (function() {
              var IMPACTS = [
                { id: 'tunguska', name: t('stem.astronomy.tunguska_1908', 'Tunguska 1908'), size: '~50-60 m', loc: 'Siberia, Russia', color: '#a16207',
                ...
                { id: 'chelyabinsk', name: t('stem.astronomy.chelyabinsk_2013', 'Chelyabinsk 2013'), size: '~20 m', loc: 'Russia (recorded on dashcams)', color: '#fbbf24',
                ...
                { id: 'apophis', name: t('stem.astronomy.apophis_2029_close_approach', 'Apophis 2029 close approach'), size: '~370 m', loc: 'Will pass 31,000 km from Earth (closer than geostationary satellites)', color: '#ef4444',
                ...
                { id: 'chicxulub', name: t('stem.astronomy.chicxulub_66_mya_dinosaur_killer', 'Chicxulub 66 Mya (dinosaur killer)'), size: '~10-15 km', loc: 'Yucatán, Mexico', color: '#7f1d1d',
                ...
                { id: 'dart', name: t('stem.astronomy.dart_mission_2022_first_deflection_tes', 'DART mission 2022 (first deflection test)'), size: '160 m target', loc: 'Dimorphos asteroid', color: '#22c55e',
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 12, background: '#0f172a', borderTop: '1px solid #334155', borderRight: '1px solid #334155', borderBottom: '1px solid #334155', borderLeft: '3px solid ' + sel.color } },
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(56,189,248,0.08)', borderLeft: '3px solid #38bdf8', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#7dd3fc', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.what_happened_will_happen', 'What happened / will happen')),
                    h('div', { style: { fontSize: 12, color: '#e2e8f0', lineHeight: 1.6 } }, sel.what)
                  ),
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(245,158,11,0.08)', borderLeft: '3px solid #f59e0b', marginBottom: 8 } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#fbbf24', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.how_often', 'How often')),
                  ...
                  h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(34,197,94,0.08)', borderLeft: '3px solid #22c55e' } },
                    h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.preparedness_state', 'Preparedness state')),
                    h('div', { style: { fontSize: 12, color: '#e2e8f0', lineHeight: 1.6 } }, sel.prep)
                  )
                ),
                h('div', { style: { marginTop: 12, padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.10)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 12, color: '#c7d2fe', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.current_detection_programs', 'Current detection programs: ')),
                  t('stem.astronomy.nasa_esa_amateur_observatories_worldwi', 'NASA + ESA + amateur observatories worldwide. NASA\'s Center for NEO Studies tracks ~35,000+ known near-Earth asteroids (as of 2024). The Catalina Sky Survey, Pan-STARRS, ATLAS find dozens of new ones per month. The Vera C. Rubin Observatory (Chile, first light 2025) is expected to find tens of thousands more in its first few years. The Sun-direction "blind spot" remains a major gap; NASA\'s NEO Surveyor space telescope (launching 2027-28) will close it.')
                ),
                h('div', { style: { marginTop: 8, padding: 10, borderRadius: 8, background: 'rgba(245,158,11,0.10)', border: '1px solid rgba(245,158,11,0.3)', fontSize: 12, color: 'var(--allo-stem-text, #fde68a)', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.deflection_options', 'Deflection options: ')),
                  h('ul', { style: { margin: '6px 0 0 22px', padding: 0, lineHeight: 1.65 } },
                    h('li', null, h('strong', null, t('stem.astronomy.kinetic_impactor', 'Kinetic impactor: ')), t('stem.astronomy.crash_a_spacecraft_into_it_dart_approa', 'Crash a spacecraft into it (DART approach). Most mature option. Effective with years of warning.')),
                    h('li', null, h('strong', null, t('stem.astronomy.gravity_tractor', 'Gravity tractor: ')), t('stem.astronomy.park_a_heavy_spacecraft_alongside_the_', 'Park a heavy spacecraft alongside the asteroid + use its tiny gravity to nudge over years. Slow but precise.')),
                    h('li', null, h('strong', null, t('stem.astronomy.nuclear_standoff', 'Nuclear standoff: ')), t('stem.astronomy.detonate_a_nuclear_device_near_not_on_', 'Detonate a nuclear device NEAR (not on) the asteroid; the ablation push deflects it. Theoretically powerful, politically + technically fraught.')),
                    h('li', null, h('strong', null, t('stem.astronomy.last_resort_fragmentation', 'Last-resort fragmentation: ')), t('stem.astronomy.direct_nuclear_hit_if_all_else_fails_i', 'Direct nuclear hit if all else fails + impact is imminent. Risk: fragments could still hit + spread the damage.'))
                  )
                )
              );
            })(),
        ...
        function eclipseSimulatorSection() {
          ...
          function solarEclipseSvg() {
            ...
            return h('svg', {
              ...
              // Annotation lines
              coveragePct === 100 && geometryType === 'total' ? h('text', { x: cx, y: 50, fill: '#fbbf24', fontSize: 14, fontWeight: 700, textAnchor: 'middle' }, t('stem.astronomy.totality', '🌑 TOTALITY 🌑')) : null,
              coveragePct === 100 && geometryType === 'annular' ? h('text', { x: cx, y: 50, fill: '#fdba74', fontSize: 14, fontWeight: 700, textAnchor: 'middle' }, t('stem.astronomy.annularity', '💍 ANNULARITY 💍')) : null
            );
          }
          ...
          function lunarEclipseSvg() {
            ...
            return h('svg', {
              viewBox: '0 0 600 350',
              role: 'img',
              'aria-label': t('stem.astronomy.lunar_eclipse_animation_the_moon_trave', 'Lunar eclipse animation. The Moon traverses Earth\'s shadow.'),
              style: { width: '100%', height: 'auto', display: 'block', maxHeight: 400 }
            },
              h('rect', { x: 0, y: 0, width: 600, height: 350, fill: '#000' }),
              // Background stars
              ...
              h('text', { x: cx, y: cy - 66, fill: '#dc2626', fontSize: 10, textAnchor: 'middle' }, t('stem.astronomy.earth_s_umbra', "Earth's umbra")),
              // Moon
              h('defs', null,
              ...
              moonInUmbra ? h('text', { x: cx, y: 50, fill: '#dc2626', fontSize: 14, fontWeight: 700, textAnchor: 'middle' }, t('stem.astronomy.blood_moon', '🌑 BLOOD MOON 🌑')) : null
            );
          }

          return sectionCard('🌑 Eclipse simulator — Sun-Earth-Moon geometry',
            h('div', null,
              h('p', { style: { fontSize: 12, color: '#94a3b8', lineHeight: 1.55, marginBottom: 10 } }, t('stem.astronomy.eclipses_happen_because_the_moon_s_app', 'Eclipses happen because the Moon\'s apparent size in the sky is almost exactly the same as the Sun\'s — about 0.5° both. This is a cosmic coincidence (the Sun is 400× larger than the Moon, but ~ 400× farther). Adjust the alignment + watch how solar + lunar eclipses appear.')),

              // Type selector
              h('div', { role: 'tablist', 'aria-label': t('stem.astronomy.eclipse_type', 'Eclipse type'), style: { display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' } },
              ...
              h('div', { style: { display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10, flexWrap: 'wrap' } },
                ...
                h('div', { style: { flex: 1, minWidth: 200 } },
                  a11ySlider({
                    ...
                    label: t('stem.astronomy.eclipse_phase', 'Eclipse phase'),
              ...
              eclipseType === 'solar' ? h('div', null,
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10, marginBottom: 10 } },
                  h('div', { style: { padding: 10, borderRadius: 8, background: '#0f172a', borderLeft: '3px solid #fbbf24' } },
                    h('div', { style: { fontSize: 12, fontWeight: 700, color: '#fbbf24', marginBottom: 4 } }, t('stem.astronomy.total_eclipse', 'Total eclipse')),
                    h('div', { style: { fontSize: 11, color: '#e2e8f0', lineHeight: 1.5 } }, t('stem.astronomy.moon_completely_covers_the_sun_only_ha', 'Moon completely covers the Sun. Only happens when the Moon is near perigee (closest to Earth, so larger apparent size). Totality lasts up to 7.5 minutes max; usually 2-4 min. Reveals the Sun\'s corona, prominences, + the diamond-ring effect.'))
                  ),
                  h('div', { style: { padding: 10, borderRadius: 8, background: '#0f172a', borderLeft: '3px solid #fdba74' } },
                    h('div', { style: { fontSize: 12, fontWeight: 700, color: '#fdba74', marginBottom: 4 } }, t('stem.astronomy.annular_eclipse', 'Annular eclipse')),
                    h('div', { style: { fontSize: 11, color: '#e2e8f0', lineHeight: 1.5 } }, t('stem.astronomy.moon_is_near_apogee_farthest_from_eart', 'Moon is near apogee (farthest from Earth, smaller apparent size). Center of Sun is covered but a bright ring of fire ("annulus") remains visible. NOT safe for unprotected viewing — solar filter required throughout.'))
                  ),
                  h('div', { style: { padding: 10, borderRadius: 8, background: '#0f172a', borderLeft: '3px solid #6366f1' } },
                    h('div', { style: { fontSize: 12, fontWeight: 700, color: '#6366f1', marginBottom: 4 } }, t('stem.astronomy.partial_eclipse', 'Partial eclipse')),
                    h('div', { style: { fontSize: 11, color: '#e2e8f0', lineHeight: 1.5 } }, t('stem.astronomy.moon_covers_only_part_of_the_sun_visib', 'Moon covers only part of the Sun. Visible from outside the path of totality even during a "total" event. Most-observed eclipse type. Still requires eclipse glasses — even 99% coverage is dangerously bright.'))
                  )
                )
              ) : h('div', null,
                h('div', { style: { padding: 10, borderRadius: 8, background: '#0f172a', borderLeft: '3px solid #dc2626', marginBottom: 10 } },
                  h('div', { style: { fontSize: 12, fontWeight: 700, color: '#dc2626', marginBottom: 4 } }, t('stem.astronomy.lunar_eclipse_the_blood_moon', 'Lunar eclipse — the "Blood Moon"')),
                  h('div', { style: { fontSize: 11, color: '#e2e8f0', lineHeight: 1.55 } }, t('stem.astronomy.earth_passes_between_sun_moon_casting_', 'Earth passes between Sun + Moon, casting its shadow on the Moon. The Moon turns RED because Earth\'s atmosphere refracts longer-wavelength light into the umbra (essentially every sunrise + sunset on Earth at once illuminating the Moon). Safe to view with naked eye — no equipment needed. Lasts hours, visible from anywhere the Moon is up.'))
                )
              ),
        ...
        function meteorShowerSimulatorSection() {
          var SHOWERS = [
            { id: 'quadrantids', name: t('stem.astronomy.quadrantids', 'Quadrantids'), peak: 'Jan 3-4', zhr: 110, radiantConst: 'Boötes', parent: '2003 EH1 (Marsden group)', speed: 41, bestHr: '4-6 AM local',
            ...
            { id: 'lyrids', name: t('stem.astronomy.lyrids', 'Lyrids'), peak: 'Apr 21-22', zhr: 18, radiantConst: 'Lyra', parent: 'Comet C/1861 G1 Thatcher', speed: 49, bestHr: 'After midnight to dawn',
            ...
            { id: 'etaaqu', name: t('stem.astronomy.eta_aquariids', 'Eta Aquariids'), peak: 'May 5-6', zhr: 50, radiantConst: 'Aquarius', parent: "Comet 1P/Halley", speed: 66, bestHr: 'Pre-dawn (3-5 AM)',
            ...
            { id: 'perseids', name: t('stem.astronomy.perseids', 'Perseids'), peak: 'Aug 12-13', zhr: 100, radiantConst: 'Perseus', parent: 'Comet 109P/Swift-Tuttle', speed: 59, bestHr: 'Pre-dawn (2-5 AM)',
            ...
            { id: 'orionids', name: t('stem.astronomy.orionids', 'Orionids'), peak: 'Oct 21-22', zhr: 25, radiantConst: 'Orion', parent: 'Comet 1P/Halley', speed: 66, bestHr: '11 PM to dawn',
            ...
            { id: 'leonids', name: t('stem.astronomy.leonids', 'Leonids'), peak: 'Nov 17-18', zhr: 15, radiantConst: 'Leo', parent: 'Comet 55P/Tempel-Tuttle', speed: 71, bestHr: 'After midnight',
            ...
            { id: 'geminids', name: t('stem.astronomy.geminids', 'Geminids'), peak: 'Dec 13-14', zhr: 150, radiantConst: 'Gemini', parent: 'Asteroid 3200 Phaethon', speed: 35, bestHr: '10 PM to dawn',
            ...
            { id: 'ursids', name: t('stem.astronomy.ursids', 'Ursids'), peak: 'Dec 22-23', zhr: 10, radiantConst: 'Ursa Minor', parent: 'Comet 8P/Tuttle', speed: 33, bestHr: 'After midnight',
          ...
          return sectionCard('🌠 Meteor shower simulator + observation guide',
            h('div', null,
              ...
              h('div', { role: 'tablist', 'aria-label': t('stem.astronomy.meteor_showers', 'Meteor showers'), style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 } },
              ...
              h('div', { style: { padding: 12, borderRadius: 10, background: '#0f172a', border: '1px solid #334155', marginBottom: 12 } },
                ...
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 6, marginBottom: 8 } },
                  h('div', { style: { padding: 6, borderRadius: 6, background: '#1e293b' } },
                    h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.5 } }, t('stem.astronomy.parent_body', 'Parent body')),
                    h('div', { style: { fontSize: 11, color: '#e2e8f0', marginTop: 2 } }, selected.parent)
                  ),
                  h('div', { style: { padding: 6, borderRadius: 6, background: '#1e293b' } },
                    h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.5 } }, t('stem.astronomy.meteor_speed', 'Meteor speed')),
                    h('div', { style: { fontSize: 11, color: '#e2e8f0', marginTop: 2 } }, selected.speed + ' km/s')
                  ),
                  h('div', { style: { padding: 6, borderRadius: 6, background: '#1e293b' } },
                    h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.5 } }, t('stem.astronomy.best_viewing_hour', 'Best viewing hour')),
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.3)', fontSize: 12, color: '#dcfce7', lineHeight: 1.65, marginBottom: 12 } },
                h('strong', null, t('stem.astronomy.from_maine', '🌲 From Maine: ')), selected.maine
              ),

              // Interactive simulator panel
              h('div', { style: { padding: 12, borderRadius: 10, background: '#1e293b', border: '1px solid #334155' } },
                h('div', { style: { fontSize: 13, fontWeight: 800, color: '#fbbf24', marginBottom: 8 } }, t('stem.astronomy.live_meteor_sky_simulator', '🎬 Live meteor sky simulator')),
                h('p', { style: { margin: '0 0 10px', fontSize: 11.5, color: '#94a3b8', lineHeight: 1.55 } }, t('stem.astronomy.each_frame_shows_1_second_of_sky_meteo', 'Each frame shows ~ 1 second of sky. Meteor streaks point AWAY from the radiant (the shower\'s named constellation). Adjust your sky conditions + radiant altitude to see how many meteors you would realistically observe.')),

                // Sliders
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 10 } },
                  a11ySlider({
                    id: 'astr-sim-zhr',
                    label: t('stem.astronomy.shower_activity_zhr', 'Shower activity (ZHR)'),
                  ...
                  a11ySlider({
                    id: 'astr-sim-bortle',
                    label: t('stem.astronomy.sky_quality_bortle', 'Sky quality (Bortle)'),
                  ...
                  a11ySlider({
                    id: 'astr-sim-alt',
                    label: t('stem.astronomy.radiant_altitude', 'Radiant altitude'),
                ...
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 12, padding: 10, borderRadius: 8, background: '#0f172a', marginBottom: 10, flexWrap: 'wrap' } },
                  h('div', null,
                    h('div', { style: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.5 } }, t('stem.astronomy.effective_rate_at_your_site', 'Effective rate at your site')),
                  ...
                  h('div', { style: { fontSize: 11, color: '#cbd5e1', lineHeight: 1.5, flex: 1, minWidth: 200 } },
                    h('strong', null, 'Why: '),
                    t('stem.astronomy.zhr_is_the_rate_one_observer_would_see', 'ZHR is the rate ONE observer would see if the radiant were AT THE ZENITH under PRISTINE skies (Bortle 1, limiting magnitude 6.5). Most observers see far fewer. Lower altitude = sin(alt) reduction. Higher Bortle = more dim meteors lost.')
                  )
                ),
                ...
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 } },
                  ...
                  _prefersReducedMotion ? h('span', { style: { fontSize: 11, color: '#a5b4fc', fontStyle: 'italic' } }, t('stem.astronomy.reduced_motion_sim_runs_at_slow_refres', '(Reduced motion: sim runs at slow refresh)')) : null,
                  a11yButton({
                    onClick: function() { upd({ simMeteorFrame: animFrame + 1 }); },
                    'aria-label': t('stem.astronomy.advance_one_frame', 'Advance one frame'),
                ...
                h('div', { style: { borderRadius: 10, overflow: 'hidden', border: '1px solid #334155', background: '#000', position: 'relative' } },
                  h('svg', {
                    ...
                    h('text', { x: 10, y: 400 - (simAlt / 90 * 200) - 36, fill: '#64748b', fontSize: 9 }, t('stem.astronomy.horizon', 'Horizon')),
                    // Radiant marker
                    ...
                    h('text', { x: 320, y: 197, fill: '#fbbf24', fontSize: 11, fontWeight: 600 }, t('stem.astronomy.radiant', 'Radiant')),
                ...
                h('p', { style: { margin: '8px 0 0', fontSize: 11, color: '#94a3b8', lineHeight: 1.5 } },
                  h('strong', null, t('stem.astronomy.how_to_read_this_view', 'How to read this view: ')),
                  t('stem.astronomy.the_yellow_dot_at_center_is_the_radian', 'The yellow dot at center is the radiant — the perspective point where meteors APPEAR to come from. Each yellow-white streak is a meteor entering the atmosphere. Earth\'s motion through the comet/asteroid debris stream gives all meteors the same parallel motion through space, but viewed from Earth they appear to RADIATE outward from a single point (the same way parallel railroad tracks appear to converge in the distance).')
                )
              ),

              // Observation tips
              h('div', { style: { marginTop: 12, padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 12, color: '#c7d2fe', lineHeight: 1.65 } },
                h('strong', null, t('stem.astronomy.observation_tips_for_any_meteor_shower', '👁️ Observation tips for any meteor shower: ')),
                h('ol', { style: { margin: '6px 0 0 18px', padding: 0, lineHeight: 1.7 } },
                  h('li', null, h('strong', null, t('stem.astronomy.get_away_from_lights', 'Get away from lights. ')), t('stem.astronomy.even_a_30_minute_drive_to_darker_sky_d', 'Even a 30-minute drive to darker sky doubles or triples your visible meteor count. Bortle 4 is the threshold for serious viewing.')),
                  h('li', null, h('strong', null, t('stem.astronomy.look_60_away_from_the_radiant_not_at_i', 'Look ~ 60° away from the radiant, not AT it. ')), t('stem.astronomy.meteors_near_the_radiant_are_short_for', 'Meteors near the radiant are short (foreshortened); those 60° away are longest + brightest because they cross the most of the field of view perpendicular to your line of sight.')),
                  h('li', null, h('strong', null, t('stem.astronomy.reclining_is_essential', 'Reclining is essential. ')), t('stem.astronomy.a_reclining_lawn_chair_or_sleeping_pad', 'A reclining lawn chair or sleeping pad means you can watch the sky for hours without neck strain. Bring a blanket; nights get cold fast even in summer.')),
                  h('li', null, h('strong', null, t('stem.astronomy.allow_20_30_minutes_for_dark_adaptatio', 'Allow 20-30 minutes for dark adaptation. ')), t('stem.astronomy.eyes_need_time_to_develop_rod_cell_sen', 'Eyes need time to develop rod-cell sensitivity for dim meteors. No phone screens, no white flashlights — use red-light only.')),
                  h('li', null, h('strong', null, t('stem.astronomy.best_hours_are_pre_dawn', 'Best hours are pre-dawn. ')), t('stem.astronomy.after_midnight_earth_s_rotation_puts_y', 'After midnight, Earth\'s rotation puts you on the LEADING edge of its orbit — the side facing the direction Earth is moving. You sweep up more meteors than the trailing side.'))
                )
              )
            ),
            '#fbbf24'
      ...
      function renderIndigenous() {
        ...
        return h('div', { style: { padding: 16 } },
          h('p', { style: { color: '#cbd5e1', fontSize: 13, marginBottom: 12, lineHeight: 1.7 } },
            t('stem.astronomy.every_people_on_earth_has_looked_up_an', 'Every people on Earth has looked up and named what they saw. The Greek-Roman tradition is one tradition among many, and it is not even the oldest. Indigenous astronomical knowledge encodes navigation, seasonal cycles, ceremony, law, and ethics. It is living knowledge, not historical curiosity.')
          ),
          ...
          h('div', { style: { padding: 14, borderRadius: 12, background: '#1e293b', border: '1px solid #334155' } },
            ...
            h('div', { style: { marginTop: 12, padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.10)', border: '1px solid rgba(34,197,94,0.3)' } },
              h('div', { style: { fontSize: 11, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.astronomy.for_deeper_learning', 'For deeper learning')),
        ...
        function crossCulturalTraditionsSection() {
          var TRADS = [
            { id: 'polynesian', name: t('stem.astronomy.polynesian_wayfinding', 'Polynesian wayfinding'), region: 'Pacific Ocean', emoji: '🌊',
            ...
            { id: 'egyptian', name: t('stem.astronomy.ancient_egyptian_astronomy', 'Ancient Egyptian astronomy'), region: 'Nile valley, 3000 BCE-300 CE', emoji: '☥',
            ...
            { id: 'mesopotamian', name: t('stem.astronomy.babylonian_mesopotamian', 'Babylonian + Mesopotamian'), region: 'Modern Iraq, 1800 BCE-100 CE', emoji: '🏛️',
            ...
            { id: 'chinese', name: t('stem.astronomy.chinese_astronomy', 'Chinese astronomy'), region: 'China, 1500 BCE-present', emoji: '🐉',
            ...
            { id: 'maya', name: t('stem.astronomy.maya_astronomy', 'Maya astronomy'), region: 'Mesoamerica, 200-900 CE peak', emoji: '🐆',
            ...
            { id: 'aboriginal', name: t('stem.astronomy.aboriginal_australian', 'Aboriginal Australian'), region: 'Australia, 60000+ BCE-present', emoji: '🌌',
            ...
            { id: 'islamic', name: t('stem.astronomy.islamic_golden_age_astronomy', 'Islamic Golden Age astronomy'), region: 'Cordoba, Baghdad, Damascus, Samarkand, 800-1500 CE', emoji: '☪️',
            ...
            { id: 'inca', name: t('stem.astronomy.inca_astronomy', 'Inca astronomy'), region: 'Andes, 1400-1533 CE peak', emoji: '🏔️',
          ...
          return h('div', { style: { marginTop: 16, padding: 14, borderRadius: 12, background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.25)' } },
            ...
            h('div', { style: { padding: 12, borderRadius: 10, background: '#0f172a', border: '1px solid #334155' } },
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(59,130,246,0.06)', borderLeft: '3px solid #3b82f6', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#93c5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.astronomy.what_the_tradition_knew', 'What the tradition knew')),
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.06)', borderLeft: '3px solid #22c55e', marginBottom: 8 } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.astronomy.evidence_impact', 'Evidence + impact')),
              ...
              h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(168,85,247,0.06)', borderLeft: '3px solid #a78bfa' } },
                h('div', { style: { fontSize: 11, fontWeight: 800, color: '#c4b5fd', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 } }, t('stem.astronomy.honest_limits_framing', 'Honest limits + framing')),
                h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.7 } }, topic.limit)
              )
            ),
            h('div', { style: { marginTop: 12, padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.3)', fontSize: 11.5, color: '#dcfce7', lineHeight: 1.65 } },
              h('strong', null, t('stem.astronomy.why_this_section_matters', 'Why this section matters: ')),
              t('stem.astronomy.reducing_real_astronomy_to_western_tra', 'Reducing "real" astronomy to Western tradition both impoverishes the curriculum + erases knowledge that took millennia to develop. The Polynesian wayfinding revival, the Aboriginal sky-story documentation projects, the Maya descendant calendrical work, and the recovery of Inca + indigenous astronomy globally all matter intellectually + ethically. Wabanaki sky stories are covered separately in this same tab — they are part of THIS continent\'s deep astronomical heritage that students at King Middle can encounter close to home.')
            )
          );
        }
      }
      ...
      function renderHistory() {
        var FIGURES = [
          { id: 'aristarchus', name: t('stem.astronomy.aristarchus_of_samos', 'Aristarchus of Samos'), years: '~310-230 BCE', color: '#7c3aed',
          ...
          { id: 'copernicus', name: t('stem.astronomy.nicolaus_copernicus', 'Nicolaus Copernicus'), years: '1473-1543', color: '#3b82f6',
          ...
          { id: 'tycho', name: t('stem.astronomy.tycho_brahe', 'Tycho Brahe'), years: '1546-1601', color: '#0ea5e9',
          ...
          { id: 'kepler', name: t('stem.astronomy.johannes_kepler', 'Johannes Kepler'), years: '1571-1630', color: '#10b981',
          ...
          { id: 'galileo', name: t('stem.astronomy.galileo_galilei', 'Galileo Galilei'), years: '1564-1642', color: '#fbbf24',
          ...
          { id: 'newton', name: t('stem.astronomy.isaac_newton', 'Isaac Newton'), years: '1643-1727', color: '#f97316',
          ...
          { id: 'herschel', name: t('stem.astronomy.william_caroline_herschel', 'William + Caroline Herschel'), years: '1738-1822 / 1750-1848', color: '#a855f7',
          ...
          { id: 'leavitt', name: t('stem.astronomy.henrietta_swan_leavitt', 'Henrietta Swan Leavitt'), years: '1868-1921', color: '#ec4899',
          ...
          { id: 'hubble', name: t('stem.astronomy.edwin_hubble', 'Edwin Hubble'), years: '1889-1953', color: '#ef4444',
          ...
          { id: 'modern', name: t('stem.astronomy.modern_era', 'Modern era'), years: '1950-now', color: '#94a3b8',
        ...
        return h('div', { style: { padding: 16 } },
          ...
          sectionCard('Ten figures who changed how we understand the universe',
            h('div', null,
              ...
              h('div', { style: { padding: 14, borderRadius: 12, background: '#0f172a', borderTop: '1px solid #334155', borderRight: '1px solid #334155', borderBottom: '1px solid #334155', borderLeft: '3px solid ' + sel.color } },
                ...
                h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(245,158,11,0.08)', borderLeft: '3px solid #f59e0b', marginBottom: 8 } },
                  h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#fbbf24', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.what_they_did', 'What they did')),
                  h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.6 } }, sel.did)
                ),
                h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(56,189,248,0.08)', borderLeft: '3px solid #38bdf8', marginBottom: 8 } },
                  h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#7dd3fc', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.tools_methods', 'Tools + methods')),
                  h('div', { style: { fontSize: 12, color: '#e2e8f0', lineHeight: 1.6 } }, sel.tools)
                ),
                h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(34,197,94,0.08)', borderLeft: '3px solid #22c55e', marginBottom: 8 } },
                  h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#86efac', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.why_it_mattered', 'Why it mattered')),
                  h('div', { style: { fontSize: 12, color: '#e2e8f0', lineHeight: 1.6 } }, sel.why)
                ),
                h('div', { style: { padding: 8, borderRadius: 6, background: 'rgba(168,85,247,0.08)', borderLeft: '3px solid #a855f7' } },
                  h('div', { style: { fontSize: 10.5, fontWeight: 800, color: '#d8b4fe', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 } }, t('stem.astronomy.legacy', 'Legacy')),
          ...
          sectionCard('🧭 Whose names are missing — and why',
            h('div', { style: { fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
              h('p', { style: { margin: '0 0 8px' } },
                t('stem.astronomy.the_great_men_history_of_astronomy_sys', 'The "Great Men" history of astronomy systematically erased many contributors. The Harvard "computers" of the late 19th + early 20th century — including Annie Jump Cannon (classified ~400,000 stars by spectrum, invented the OBAFGKM system), Williamina Fleming, Antonia Maury — did the foundational work that named astronomers got credit for. Cecilia Payne-Gaposchkin\'s 1925 PhD thesis showed that stars are mostly hydrogen — a result so radical it was initially discouraged from publication; later credited.')
              ),
              h('p', { style: { margin: '0 0 8px' } },
                t('stem.astronomy.indigenous_astronomical_traditions_acr', 'Indigenous astronomical traditions across the world (Wabanaki + Lakota + Maori + Aboriginal Australian + many others, covered in the Sky Traditions tab) are at least as old as written-record astronomy + were systematically dismissed by colonial scientists. The Polynesian wayfinding tradition navigated thousands of miles of open ocean using only star positions, swells, and bird flight — a sophisticated practical astronomy mostly absent from Western histories.')
              ),
              h('p', { style: { margin: 0 } },
                t('stem.astronomy.modern_astronomy_is_increasingly_diver', 'Modern astronomy is increasingly diverse but progress remains uneven. Naming this history honestly is part of doing science honestly.')
              )
            ),
            '#94a3b8'
          ),
          ...
          sectionCard('⚖️ Astronomy + ethics — who astronomy serves',
            (function() {
              var ETH_TOPICS = [
                { id: 'overview', name: t('stem.astronomy.astronomy_is_political_ethical', 'Astronomy is political + ethical'), emoji: '🌍',
                  body: t('stem.astronomy.pure_curiosity_driven_knowledge_is_one', 'Pure curiosity-driven knowledge is one part of astronomy. The OTHER parts: who funds research, where telescopes are built, who benefits from the resulting jobs + prestige, whose land is occupied by observatories, whose knowledge traditions are honored or erased, who gets access to telescope time + data, what languages publications appear in, who gets to study the stars + who is excluded. Astronomy looks cosmic + politically neutral — but the practices around it are deeply embedded in colonial, economic, + cultural histories. Honest astronomy education includes the ethics, not just the equations.'),
                ...
                { id: 'pollution', name: t('stem.astronomy.light_pollution_as_environmental_justi', 'Light pollution as environmental justice'), emoji: '🌃',
                  body: t('stem.astronomy.light_pollution_is_unevenly_distribute', 'Light pollution is unevenly distributed. WEALTHIER neighborhoods can afford dark-sky-friendly lighting + private dark properties; POORER + working-class urban neighborhoods get harsh streetlighting from above + adjacent commercial development. INDIGENOUS reservations + rural communities often retain dark skies but lack resources to PROTECT them as development encroaches. The Mauna Kea Observatory complex has produced extensive ARTIFICIAL skyglow that affects Native Hawaiian sky observation; this is a documented additional grievance + an environmental-justice issue. Dark-sky advocacy is therefore not just an aesthetic preservation issue — it intersects with ECOLOGY, HEALTH (sleep, cancer), CULTURE (Indigenous astronomy), + EQUITY (whose ability to see the sky gets protected).'),
                ...
                { id: 'access', name: t('stem.astronomy.telescope_access_scientific_equity', 'Telescope access + scientific equity'), emoji: '🔭',
                  body: t('stem.astronomy.premier_telescope_time_is_heavily_rati', 'Premier telescope time is HEAVILY rationed. JWST receives ~ 1,800 proposals per cycle for ~ 200 awards; Hubble similarly oversubscribed; ALMA, VLT, Keck all 4-10x oversubscribed. Reviewer biases (gender, geography, institutional prestige) have been documented in time-allocation committees. Many studies have shown DOUBLE-BLIND review (reviewers don\'t see proposer identity) IMPROVES allocation equity. Hubble + several other major observatories now use double-blind review. Public archives (MAST, ESO, NASA, Vizier) make most observatory data eventually open to anyone — which has been a major equity gain (researchers in lower-resource institutions can do science with the same data as wealthy university researchers).'),
      ...
      function renderObserve() {
        ...
        function telescopeSim() {
          ...
          function refractorSvg() {
            return h('svg', { viewBox: '0 0 600 220', width: '100%', height: 220, role: 'img', 'aria-labelledby': 'refractorT refractorD' },
              h('title', { id: 'refractorT' }, t('stem.astronomy.refractor_telescope_ray_diagram', 'Refractor telescope ray diagram')),
              ...
              h('text', { x: 6, y: 75, fill: '#fde68a', fontSize: 10 }, t('stem.astronomy.light_from_a_star', 'Light from a star →')),
              // Objective lens (biconvex, at x=80)
              h('ellipse', { cx: 80, cy: 110, rx: 4, ry: 30, fill: '#7dd3fc', opacity: 0.55, stroke: '#0ea5e9', strokeWidth: 1.5 }),
              h('text', { x: 80, y: 60, textAnchor: 'middle', fill: '#bae6fd', fontSize: 9, fontWeight: 700 }, t('stem.astronomy.objective_lens', 'Objective lens')),
              ...
              h('text', { x: 420, y: 75, textAnchor: 'middle', fill: '#fde68a', fontSize: 9, fontWeight: 700 }, t('stem.astronomy.focal_plane', 'Focal plane')),
              ...
              h('text', { x: 460, y: 80, textAnchor: 'middle', fill: '#bbf7d0', fontSize: 9, fontWeight: 700 }, t('stem.astronomy.eyepiece', 'Eyepiece')),
          ...
          function reflectorSvg() {
            ...
            return h('svg', { viewBox: '0 0 600 220', width: '100%', height: 220, role: 'img', 'aria-labelledby': 'reflectorT reflectorD' },
              h('title', { id: 'reflectorT' }, t('stem.astronomy.newtonian_reflector_ray_diagram', 'Newtonian reflector ray diagram')),
              ...
              h('text', { x: 6, y: 75, fill: '#fde68a', fontSize: 10 }, t('stem.astronomy.light_enters', 'Light enters →')),
              ...
              h('text', { x: 505, y: 60, textAnchor: 'middle', fill: '#bfdbfe', fontSize: 9, fontWeight: 700 }, t('stem.astronomy.primary_mirror', 'Primary mirror')),
              ...
              h('text', { x: 202, y: 100, textAnchor: 'middle', fill: '#fde68a', fontSize: 9, fontWeight: 700 }, t('stem.astronomy.secondary', 'Secondary')),
          ...
          return h('div', null,
            ...
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10, marginBottom: 12 } },
              [
                { label: t('stem.astronomy.aperture_mm', 'Aperture (mm)'), value: aperture, min: 50, max: 400, step: 10, key: 'scopeAperture' },
                { label: t('stem.astronomy.focal_length_mm', 'Focal length (mm)'), value: focalLen, min: 300, max: 3000, step: 50, key: 'scopeFocalLen' },
                { label: t('stem.astronomy.eyepiece_focal_length_mm', 'Eyepiece focal length (mm)'), value: eyepieceFl, min: 4, max: 40, step: 1, key: 'eyepieceFl' }
              ].map(function(s, i) {
            ...
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 8 } },
              [
                { label: t('stem.astronomy.magnification', 'Magnification'), value: magnification.toFixed(0) + '×', color: maxOK ? '#86efac' : '#fca5a5', sub: maxOK ? '' : 'Exceeds max useful (' + maxMag.toFixed(0) + '×)' },
                { label: t('stem.astronomy.focal_ratio', 'Focal ratio'), value: 'f/' + focalRatio.toFixed(1), color: '#c7d2fe', sub: focalRatio < 6 ? 'fast (wide field)' : focalRatio > 12 ? 'slow (high mag)' : 'medium' },
                { label: t('stem.astronomy.light_gathering', 'Light gathering'), value: lightGather.toFixed(0) + '× eye', color: '#fde68a', sub: 'vs naked eye' },
                { label: t('stem.astronomy.exit_pupil', 'Exit pupil'), value: exitPupil.toFixed(1) + ' mm', color: exitPupil > 7 ? '#fca5a5' : '#86efac', sub: exitPupil > 7 ? 'too large — wastes light' : exitPupil < 0.5 ? 'too small — dim image' : 'OK' },
                { label: t('stem.astronomy.resolution_dawes', 'Resolution (Dawes)'), value: resolveDawes.toFixed(2) + '″', color: '#c7d2fe', sub: 'arcseconds' },
                { label: t('stem.astronomy.limiting_magnitude', 'Limiting magnitude'), value: '~+' + limMag.toFixed(1), color: '#c7d2fe', sub: 'in dark sky' }
              ].map(function(s, i) {
            ...
            h('div', { style: { marginTop: 10, padding: 10, borderRadius: 8, background: 'rgba(99,102,241,0.10)', border: '1px solid rgba(99,102,241,0.3)', fontSize: 11.5, color: '#c7d2fe', lineHeight: 1.6 } },
              h('strong', null, t('stem.astronomy.key_insight', 'Key insight: ')),
              t('stem.astronomy.aperture_is_everything_magnification_f', 'Aperture is everything. Magnification = focal length / eyepiece focal length, but USEFUL magnification is capped at ~2× aperture in mm. Beyond that, you just magnify the blur. A 100 mm scope tops out around 200×; a 300 mm scope can usefully push to ~600×. The Dawes limit (resolution) and limiting magnitude (faintest visible) both scale with aperture, not magnification. "Bigger eye, not bigger zoom."')
            )
          );
        }
        ...
        return h('div', { style: { padding: 16 } },
          sectionCard('🔭 Tools for observing',
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 } },
              [
                { name: t('stem.astronomy.naked_eye_2', 'Naked eye'), cost: 'Free', see: 'Constellations, the Moon, naked-eye planets (Mercury, Venus, Mars, Jupiter, Saturn), the Milky Way (in dark sky), bright deep-sky objects (Pleiades, Andromeda)', start: 'Best starting point. Learn a few constellations a month. A star chart or sky app (Stellarium is free) helps.' },
                { name: t('stem.astronomy.binoculars', 'Binoculars'), cost: '$50-200 for 7x50 or 10x50', see: 'Moon craters and mare, the four Galilean moons of Jupiter, Saturn (just barely as oval), open clusters, the Andromeda Galaxy as a fuzzy patch, hundreds of times more stars', start: 'A surprisingly underrated astronomy tool. Often better than a cheap telescope for wide views.' },
                { name: t('stem.astronomy.telescope', 'Telescope'), cost: '$200-2000+ for serious entry', see: 'Saturn\'s rings, Jupiter\'s belts, Mars surface details, the Orion Nebula, double stars, distant galaxies', start: 'Start with binoculars first. When you upgrade: a 6-inch or 8-inch Dobsonian reflector gives the most aperture per dollar. Avoid department-store "500x magnification" scopes.' },
                { name: t('stem.astronomy.sky_apps', 'Sky apps'), cost: 'Free to $5', see: 'Real-time planet positions, ISS passes, eclipse predictions, what\'s up tonight', start: 'Stellarium (free desktop + mobile), NASA Eyes (web), SkyView (mobile), Night Sky (iOS).' }
              ].map(function(t, i) {
                return h('div', { key: i, style: { padding: 10, borderRadius: 8, background: '#0f172a', border: '1px solid #334155' } },
                  ...
                  h('div', { style: { fontSize: 12, color: '#e2e8f0', marginBottom: 6, lineHeight: 1.5 } }, h('strong', null, t('stem.astronomy.what_you_can_see', 'What you can see: ')), t.see),
          ...
          sectionCard('🌒 Light pollution: the Bortle scale',
            h('div', null,
              h('p', { style: { margin: '0 0 10px', fontSize: 13, color: '#e2e8f0', lineHeight: 1.7 } },
                t('stem.astronomy.how_dark_is_your_sky_most_people_live_', 'How dark is your sky? Most people live under sky much brighter than they realize. The Bortle scale (1-9) ranks sky darkness; class 1 is true wilderness dark sky, class 9 is inner city. Where do you live?')
              ),
              ...
              h('p', { style: { margin: '10px 0 0', fontSize: 12, color: '#cbd5e1', lineHeight: 1.65 } },
                h('strong', { style: { color: '#fbbf24' } }, t('stem.astronomy.equity_note', 'Equity note: ')),
                t('stem.astronomy.most_us_students_live_under_bortle_5_8', 'most US students live under Bortle 5-8 skies. The actual Milky Way is invisible to about 80% of Americans. Light-pollution dark-sky parks (Acadia in Maine, for example) are intentional preservation of this disappearing resource.')
              )
            )
          ),
          ...
          d.observingList && d.observingList.length > 0 ? sectionCard('⭐ Your observing list',
            h('div', null,
              h('p', { style: { margin: '0 0 8px', fontSize: 12, color: '#94a3b8' } }, t('stem.astronomy.constellations_you_have_marked_to_obse', 'Constellations you have marked to observe. Add more from the Constellations tab.')),
              h('ul', { style: { margin: 0, padding: '0 0 0 22px', color: '#e2e8f0', fontSize: 13, lineHeight: 1.85 } },
          ...
          sectionCard('🌃 Light pollution + dark-sky preservation',
            (function() {
              var LP_TOPICS = [
                ...
                { id: 'shielding', name: t('stem.astronomy.solutions_that_actually_work', 'Solutions that actually work'), emoji: '🔅',
                  body: t('stem.astronomy.engineering_fixes_a_full_cut_off_fixtu', 'Engineering fixes: (a) FULL CUT-OFF fixtures (light directed ONLY downward, no light escaping above horizontal). The IDA-approved "Dark Sky Friendly" certification specifies this. (b) WARMER LED color temperature (≤ 3000 K, ideally 2200 K amber). Cooler/whiter light (4000-6000 K) scatters more in the atmosphere + harms wildlife circadian rhythms. (c) LOWER lumen output (most outdoor lighting is over-lit by 2-5×). (d) MOTION-ACTIVATED + scheduled DIMMING (light only when needed). (e) Avoid uplighting of buildings + landscape features. Phoenix, Tucson, Flagstaff (since 1958 — the FIRST dark-sky city), and parts of Hawaii (around Mauna Kea) have effective lighting ordinances. The International Dark-Sky Association (IDA, founded 1988) certifies "International Dark-Sky Places" — about 200 communities + parks worldwide as of 2024.'),
              ...
              return h('div', null,
                ...
                h('div', { style: { padding: 14, borderRadius: 10, background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.35)' } },
                  ...
                  h('div', { style: { fontSize: 12.5, color: '#e2e8f0', lineHeight: 1.7, marginBottom: 10 } }, topic.body),
                  h('div', { style: { fontSize: 11.5, color: '#cbd5e1', lineHeight: 1.65, padding: 10, borderRadius: 8, background: 'rgba(0,0,0,0.25)', fontStyle: 'italic' } },
                    h('strong', null, t('stem.astronomy.honest_framing_11', 'Honest framing: ')), topic.caveat
                  )
                ),
                h('div', { style: { marginTop: 12, padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.3)', fontSize: 11.5, color: '#dcfce7', lineHeight: 1.65 } },
                  h('strong', null, t('stem.astronomy.citizen_science_you_can_do_tonight', 'Citizen science you can do tonight: ')),
                  t('stem.astronomy.globe_at_night_globeatnight_org_record', 'Globe at Night (globeatnight.org) — record visible constellations from your address, contributing to a global light-pollution map. Loss of the Night app (free, iOS + Android) — same concept. AAVSO variable-star observations (aavso.org) — submit brightness estimates of variable stars. CitizenSky + Zooniverse have many astronomy projects open to students. School psychologists + teachers can use these as low-friction outdoor learning experiences.')
                )
              );
            })(),
            '#8b5cf6'
          ...
          sectionCard('🛰️ The instruments of modern astronomy',
            (function() {
              var INST_TOPICS = [
                { id: 'radio', name: t('stem.astronomy.radio_telescopes', 'Radio telescopes'), emoji: '📡',
                ...
                { id: 'mm', name: t('stem.astronomy.millimeter_submillimeter', 'Millimeter + submillimeter'), emoji: '〰️',
                ...
                { id: 'ir', name: t('stem.astronomy.infrared_telescopes', 'Infrared telescopes'), emoji: '🔥',
                ...
                { id: 'optical', name: t('stem.astronomy.optical_visible_light', 'Optical / visible-light'), emoji: '👁️',
                  range: '~390 to 700 nm',
                ...
                { id: 'uv', name: t('stem.astronomy.ultraviolet', 'Ultraviolet'), emoji: '💜',
                  range: '~10 to 400 nm',
                ...
                { id: 'xray', name: t('stem.astronomy.x_ray_telescopes', 'X-ray telescopes'), emoji: '🩻',
                ...
                { id: 'gamma', name: t('stem.astronomy.gamma_ray_telescopes', 'Gamma-ray telescopes'), emoji: '☢️',
                ...
                { id: 'particles', name: t('stem.astronomy.particle_messengers', 'Particle messengers'), emoji: '⚛️',
                ...
                { id: 'limits', name: t('stem.astronomy.what_we_still_cannot_see', 'What we still cannot see'), emoji: '🚫',
          ...
          sectionCard('📷 Astrophotography — how to capture what you see',
            (function() {
              var AP_TOPICS = [
                { id: 'phone', name: t('stem.astronomy.phone_astrophotography', 'Phone astrophotography'), emoji: '📱',
                ...
                { id: 'dslr', name: t('stem.astronomy.dslr_mirrorless_basics', 'DSLR / mirrorless basics'), emoji: '📸',
                ...
                { id: 'tracker', name: t('stem.astronomy.star_trackers_mounts', 'Star trackers + mounts'), emoji: '⚙️',
                ...
                { id: 'planetary', name: t('stem.astronomy.lunar_planetary_imaging', 'Lunar + planetary imaging'), emoji: '🌙',
                ...
                { id: 'deep', name: t('stem.astronomy.deep_sky_imaging_dsos', 'Deep-sky imaging (DSOs)'), emoji: '🌌',
                ...
                { id: 'narrowband', name: t('stem.astronomy.narrowband_dual_band_filters', 'Narrowband + dual-band filters'), emoji: '🎨',
                ...
                { id: 'process', name: t('stem.astronomy.image_processing_essentials', 'Image processing essentials'), emoji: '🎚️',
                ...
                { id: 'maine', name: t('stem.astronomy.maine_astrophotography', 'Maine astrophotography'), emoji: '🌲',
              ...
              return h('div', null,
                h('div', { style: { fontSize: 12.5, color: '#cbd5e1', lineHeight: 1.65, marginBottom: 12 } },
                  t('stem.astronomy.astrophotography_has_changed_completel', 'Astrophotography has changed completely in the past decade. Phones with night mode capture the Milky Way; $500 star trackers + DSLRs make professional-looking deep-sky images possible from suburban backyards; AI processing tools (StarXTerminator, BlurXTerminator) automate steps that took experts hours. For students at any level, there is a clear progression from phone shots tonight to professional-quality images within a year of focused practice.')
                ),
                h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 } },
          ...
          sectionCard('⏱️ How we tell time — UTC, leap seconds, and GPS relativity',
            (function() {
              var TIME_TOPICS = [
                { id: 'origins', name: t('stem.astronomy.time_from_the_sky', 'Time from the sky'), emoji: '🌅',
                  body: t('stem.astronomy.for_most_of_human_history_time_meant_t', 'For most of human history, time meant the SUN. Local apparent solar time (sundial time) was the only practical reference; each town set its own clocks by when the sun crossed the local meridian. The Earth\'s rotation was the assumed-perfect clock. Mean solar time (averaged to remove Earth\'s orbital eccentricity + axial-tilt wobbles) became standard with mechanical clocks in the 1500s. The MEAN SOLAR SECOND was defined as 1/86,400 of a mean solar day. This worked fine until ~1955.'),
          ...
          sectionCard('🔭 Buying a telescope — the honest student/family guide',
            (function() {
              var SCOPE_TOPICS = [
                ...
                { id: 'maintenance', name: t('stem.astronomy.maintenance_the_long_view', 'Maintenance + the long view'), emoji: '🛠️',
                  body: t('stem.astronomy.a_good_telescope_can_last_50_years_mai', 'A good telescope can last 50+ years. Maintenance is minimal: mirror surfaces should be RECOATED roughly every 15-25 years (~ $200-500 from Spectrum Coatings or Optical Materials Research); lenses need occasional cleaning with proper materials (NEVER paper towels — use lens-cleaning solution + microfiber). Store in a clean dry place — humidity damages coatings. Avoid handling optical surfaces. The biggest care issue is COLLIMATION (alignment of the optical elements) for reflectors + SCTs — every Newtonian or SCT needs occasional collimation adjustment using a Cheshire eyepiece or laser collimator. Refractors stay collimated for years; Dobsonians may need adjustment after travel.'),
        ...
        function glossarySection() {
          ...
          return sectionCard('📖 Astronomy glossary (' + TERMS.length + ' terms)',
            h('div', null,
              h('p', { style: { fontSize: 12, color: '#94a3b8', lineHeight: 1.55, marginBottom: 10 } }, t('stem.astronomy.a_reference_glossary_of_astronomy_term', 'A reference glossary of astronomy terms organized by topic. Search by term or definition, or filter by category.')),

              h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 } },
                h('input', {
                  type: 'text',
                  placeholder: t('stem.astronomy.search_glossary', 'Search glossary…'),
                  value: search,
                  onChange: function(e) { upd({ glossarySearch: e.target.value }); },
                  'aria-label': t('stem.astronomy.search_glossary_2', 'Search glossary'),
                  className: 'astr-focus',
                  style: { flex: 1, minWidth: 140, padding: '5px 10px', borderRadius: 6, border: '1px solid #334155', background: '#0f172a', color: '#e2e8f0', fontSize: 11 }
                }),
                h('select', {
                  ...
                  'aria-label': t('stem.astronomy.filter_glossary_by_category', 'Filter glossary by category'),
                  className: 'astr-focus',
                  style: { padding: '4px 8px', borderRadius: 6, border: '1px solid #334155', background: '#0f172a', color: '#e2e8f0', fontSize: 11, minWidth: 140 }
                },
                  h('option', { value: 'all' }, t('stem.astronomy.all_categories', 'All categories')),
        ...
        function eyepieceViewSimulatorSection() {
          ...
          var TARGETS = [
            { id: 'moon', name: t('stem.astronomy.the_moon', 'The Moon'), type: 'Solar System', mag: -12.7, sizeArcmin: 31, color: '#e0e0e0', spec: 'rocky',
            ...
            { id: 'saturn', name: t('stem.astronomy.saturn', 'Saturn'), type: 'Planet', mag: 0.4, sizeArcmin: 0.7, color: '#fde68a', spec: 'planet',
            ...
            { id: 'jupiter', name: t('stem.astronomy.jupiter', 'Jupiter'), type: 'Planet', mag: -2.5, sizeArcmin: 0.8, color: '#fdba74', spec: 'planet',
            ...
            { id: 'venus', name: t('stem.astronomy.venus', 'Venus'), type: 'Planet', mag: -4.5, sizeArcmin: 0.9, color: '#fef9c3', spec: 'planet',
            ...
            { id: 'mars', name: t('stem.astronomy.mars_2', 'Mars'), type: 'Planet', mag: -0.5, sizeArcmin: 0.2, color: '#fb923c', spec: 'planet',
            ...
            { id: 'orion-nebula', name: t('stem.astronomy.orion_nebula_m42', 'Orion Nebula (M42)'), type: 'Nebula', mag: 4.0, sizeArcmin: 65, color: '#86efac', spec: 'nebula',
            ...
            { id: 'andromeda', name: t('stem.astronomy.andromeda_galaxy_m31', 'Andromeda Galaxy (M31)'), type: 'Galaxy', mag: 3.4, sizeArcmin: 180, color: '#fde68a', spec: 'galaxy',
            ...
            { id: 'pleiades', name: t('stem.astronomy.pleiades_m45', 'Pleiades (M45)'), type: 'Open Cluster', mag: 1.6, sizeArcmin: 110, color: '#bae6fd', spec: 'cluster',
            ...
            { id: 'ring-nebula', name: t('stem.astronomy.ring_nebula_m57', 'Ring Nebula (M57)'), type: 'Planetary Nebula', mag: 8.8, sizeArcmin: 1.4, color: '#fbcfe8', spec: 'planetary',
            ...
            { id: 'hercules-cluster', name: t('stem.astronomy.hercules_cluster_m13', 'Hercules Cluster (M13)'), type: 'Globular Cluster', mag: 5.8, sizeArcmin: 20, color: '#e2e8f0', spec: 'globular',
            ...
            { id: 'double-cluster', name: t('stem.astronomy.double_cluster_ngc_869_884', 'Double Cluster (NGC 869/884)'), type: 'Open Cluster Pair', mag: 4.3, sizeArcmin: 60, color: '#bfdbfe', spec: 'cluster',
            ...
            { id: 'whirlpool', name: t('stem.astronomy.whirlpool_galaxy_m51', 'Whirlpool Galaxy (M51)'), type: 'Galaxy', mag: 8.4, sizeArcmin: 11, color: '#fef3c7', spec: 'galaxy',
            ...
            if (selectedTarget.spec === 'planet' && selectedTargetId === 'jupiter') {
              return h('g', null,
                ...
                magnification < 50 ? null : [
                  { x: -2.5, name: 'Io' }, { x: -1.5, name: t('stem.astronomy.europa', 'Europa') }, { x: 1.7, name: t('stem.astronomy.ganymede', 'Ganymede') }, { x: 2.8, name: t('stem.astronomy.callisto', 'Callisto') }
                ].map(function(m, i) {
          ...
          return sectionCard('🔭 Eyepiece view simulator — what you would see',
            h('div', null,
              h('p', { style: { fontSize: 12, color: '#94a3b8', lineHeight: 1.55, marginBottom: 10 } }, t('stem.astronomy.pick_a_target_adjust_your_telescope_ey', 'Pick a target, adjust your telescope, eyepiece, sky conditions — see what would actually be visible. This simulator approximates real visual appearance: small telescopes show less; light-polluted skies wash out faint objects; higher magnification shrinks the field of view + dims everything.')),

              // Target picker
              h('div', { role: 'tablist', 'aria-label': t('stem.astronomy.observing_targets', 'Observing targets'), style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 } },
                TARGETS.map(function(t) {
              ...
              h('div', { style: { display: 'grid', gridTemplateColumns: 'minmax(280px, 1fr) minmax(240px, 1fr)', gap: 12, alignItems: 'start' } },
                ...
                h('div', { style: { position: 'relative', background: '#000', borderRadius: '50%', overflow: 'hidden', aspectRatio: '1', maxWidth: 500, justifySelf: 'center', width: '100%', border: '6px solid #1e293b' } },
                  h('svg', {
                    ...
                    !targetVisible ? h('text', { x: viewSize / 2, y: viewSize / 2 + 5, fill: '#ef4444', fontSize: 13, textAnchor: 'middle', fontWeight: 700 }, t('stem.astronomy.target_too_dim_for_this_setup', 'Target too dim for this setup')) : null
                  )
                ),
                ...
                h('div', null,
                  ...
                  h('div', { style: { display: 'grid', gap: 10, marginBottom: 10 } },
                    a11ySlider({
                      id: 'astr-ey-ap', label: t('stem.astronomy.aperture', 'Aperture'), value: apMm, min: 50, max: 400, step: 10,
                      valueText: apMm + ' mm (' + (apMm / 25.4).toFixed(1) + '")',
                      onChange: function(v) { upd({ eyApertureMm: v, eyFocalMm: v * 8 }); }, accent: '#f97316'
                    }),
                    a11ySlider({
                      id: 'astr-ey-ep', label: t('stem.astronomy.eyepiece_focal_length', 'Eyepiece focal length'), value: epFlMm, min: 4, max: 40, step: 1,
                      valueText: epFlMm + ' mm',
                      onChange: function(v) { upd({ eyEpFlMm: v }); }, accent: '#f97316'
                    }),
                    a11ySlider({
                      id: 'astr-ey-fld', label: t('stem.astronomy.eyepiece_apparent_field', 'Eyepiece apparent field'), value: epField, min: 40, max: 100, step: 5,
                      valueText: epField + '°',
                      onChange: function(v) { upd({ eyEpField: v }); }, accent: '#f97316'
                    }),
                    a11ySlider({
                      id: 'astr-ey-bort', label: t('stem.astronomy.sky_quality_bortle_2', 'Sky quality (Bortle)'), value: bortleSim, min: 1, max: 9, step: 1,
                  ...
                  h('div', { style: { padding: 10, borderRadius: 8, background: 'rgba(249,115,22,0.10)', border: '1px solid rgba(249,115,22,0.35)', fontSize: 11.5, color: '#fed7aa', lineHeight: 1.7 } },
                    ...
                    h('div', null, h('strong', null, t('stem.astronomy.true_field_of_view', 'True field of view: ')), trueFOVdeg < 1 ? trueFOVarcmin.toFixed(1) + " arcmin" : trueFOVdeg.toFixed(2) + '°'),
                    h('div', null, h('strong', null, t('stem.astronomy.exit_pupil_2', 'Exit pupil: ')), exitPupilMm.toFixed(1) + ' mm', exitPupilMm > 7 ? ' (too large — wasted light)' : (exitPupilMm < 0.5 ? ' (too small — image dim)' : ' (good)')),
                    h('div', null, h('strong', null, t('stem.astronomy.limiting_magnitude_2', 'Limiting magnitude: ')), limitingMag.toFixed(1)),
                    h('div', null, h('strong', null, t('stem.astronomy.light_gathering_2', 'Light gathering: ')), lightGain.toFixed(0) + '× naked eye'),
                    h('div', null, h('strong', null, t('stem.astronomy.dawes_limit', 'Dawes limit: ')), dawesLimit.toFixed(2) + '" (resolution)')
                  )
                )
              ),

              h('div', { style: { marginTop: 12, padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.3)', fontSize: 11.5, color: '#dcfce7', lineHeight: 1.65 } },
                h('strong', null, t('stem.astronomy.what_this_is_like_through_a_real_teles', '👁️ What this is like through a real telescope: ')),
              ...
              targetTooBig ? h('div', { style: { marginTop: 8, padding: 10, borderRadius: 8, background: 'rgba(245,158,11,0.10)', border: '1px solid rgba(245,158,11,0.3)', fontSize: 11.5, color: 'var(--allo-stem-text, #fde68a)', lineHeight: 1.65 } },
                h('strong', null, t('stem.astronomy.target_is_larger_than_your_field_of_vi', '⚠️ Target is larger than your field of view. ')),
                t('stem.astronomy.try_a_longer_focal_length_eyepiece_low', 'Try a longer focal-length eyepiece (lower magnification, wider FOV).')
              ) : null,
              ...
              targetTooSmall ? h('div', { style: { marginTop: 8, padding: 10, borderRadius: 8, background: 'rgba(245,158,11,0.10)', border: '1px solid rgba(245,158,11,0.3)', fontSize: 11.5, color: 'var(--allo-stem-text, #fde68a)', lineHeight: 1.65 } },
                h('strong', null, t('stem.astronomy.target_is_small_relative_to_atmospheri', '⚠️ Target is small relative to atmospheric seeing. ')),
                t('stem.astronomy.use_higher_magnification_wait_for_stea', 'Use higher magnification + wait for steady atmosphere.')
              ) : null
            ),
            '#f97316'
          );
          ...
          return h('div', { style: { padding: 16 } },
            ...
            QUIZ_QUESTIONS.map(function(q, i) {
              ...
              return h('div', { key: i, style: { padding: 12, borderRadius: 10, background: '#0f172a', border: '1px solid ' + (got ? 'rgba(34,197,94,0.4)' : 'rgba(239,68,68,0.4)'), borderLeft: '3px solid ' + (got ? '#22c55e' : '#ef4444'), marginBottom: 10 } },
                ...
                !got ? h('div', { style: { fontSize: 11, color: '#94a3b8', marginBottom: 4 } }, t('stem.astronomy.your_answer', 'Your answer: '), q.choices[answers[i] != null ? answers[i] : 0]) : null,
                h('div', { style: { fontSize: 12, color: '#cbd5e1', lineHeight: 1.6, fontStyle: 'italic' } }, q.explain)
              );
            }),
            h('button', { onClick: reset, style: { padding: '8px 16px', borderRadius: 8, border: 'none', background: INDIGO, color: '#fff', fontWeight: 700, cursor: 'pointer' } }, t('stem.astronomy.retake_quiz', 'Retake quiz'))
          );
        }
      ...
      function renderHRDiagram() {
        ...
        var catMeta = {
          redDwarf:   { label: t('stem.astronomy.red_dwarf', '🔴 Red dwarf'),  color: '#dc2626', bg: '#fef2f2', border: '#fca5a5', desc: t('stem.astronomy.cool_faint_low_mass_most_common_stars_', 'Cool, faint, low-mass. Most common stars in galaxy. Lifespan trillions of years.') },
          sunLike:    { label: t('stem.astronomy.sun_like', '🟡 Sun-like'),   color: '#facc15', bg: '#fefce8', border: '#fde047', desc: t('stem.astronomy.yellow_white_main_sequence_dwarf_fusio', 'Yellow-white main-sequence dwarf. Fusion-stable for billions of years.') },
          redGiant:   { label: t('stem.astronomy.red_giant', '🟠 Red giant'),  color: '#ea580c', bg: '#fff7ed', border: '#fdba74', desc: t('stem.astronomy.evolved_star_hydrogen_shell_burning_bl', 'Evolved star. Hydrogen shell-burning. Bloated outer envelope.') },
          supergiant: { label: t('stem.astronomy.supergiant', '💎 Supergiant'), color: '#3b82f6', bg: '#eff6ff', border: '#93c5fd', desc: t('stem.astronomy.massive_luminous_short_lived_end_in_su', 'Massive, luminous, short-lived. End in supernova explosions.') },
          mainSeq:    { label: t('stem.astronomy.main_sequence_3', '⭐ Main sequence'), color: '#059669', bg: '#ecfdf5', border: '#86efac', desc: t('stem.astronomy.stable_hydrogen_burning_specific_class', 'Stable hydrogen-burning. Specific class depends on mass + temp.') }
        }[category];
        function logObs() {
        ...
        return h('div', { style: { padding: 16 } },
          h('div', { style: { padding: 16, background: '#0f172a', borderRadius: 12, color: '#e2e8f0' } },
            h('h3', { style: { fontSize: 16, fontWeight: 800, color: '#fde047', marginBottom: 6 } }, t('stem.astronomy.hr_diagram_discovery', '⭐ HR diagram discovery')),
            h('p', { style: { fontSize: 12, color: '#cbd5e1', lineHeight: 1.5, marginBottom: 12 } },
              t('stem.astronomy.adjust_stellar_mass_surface_temperatur', 'Adjust stellar mass, surface temperature, and luminosity. The widget classifies your star into one of five discrete categories. No score, no reveal — sweep and notice.')),
            ...
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 12 } },
              [
                { key: 'mass',   label: t('stem.astronomy.mass_m', 'Mass (M☉)'),          val: iq.mass,   min: 0.1, max: 20, step: 0.1 },
                { key: 'tempK',  label: t('stem.astronomy.surface_t_k', 'Surface T (K)'),      val: iq.tempK,  min: 2000, max: 50000, step: 100 },
                { key: 'lumin',  label: t('stem.astronomy.luminosity_l', 'Luminosity (L☉)'),    val: iq.lumin,  min: 0.001, max: 100000, step: 0.1 }
              ].map(function(s) {
            ...
            h('div', { style: { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 12 } },
              h('button', { onClick: logObs, style: { padding: '4px 10px', background: '#1e293b', color: '#cbd5e1', border: '1px solid rgba(100,116,139,0.4)', borderRadius: 4, fontSize: 11, fontWeight: 'bold', cursor: 'pointer' } }, t('stem.astronomy.log', '📋 Log')),
              h('button', { onClick: function() { setIQ({ mass: 1, tempK: 5800, lumin: 1, log: [], hypothesis: '', stuckRevealed: false, understood: false, explanation: '' }); }, style: { padding: '4px 10px', background: 'transparent', color: '#94a3b8', border: '1px solid rgba(100,116,139,0.4)', borderRadius: 4, fontSize: 11, cursor: 'pointer' } }, t('stem.astronomy.reset', '↺ Reset')),
            ...
            h('textarea', { value: iq.hypothesis || '', onChange: function(e) { setIQ({ hypothesis: e.target.value }); }, placeholder: t('stem.astronomy.hypothesis_free_text_what_relationship', 'Hypothesis (free text): What relationships among mass, temp, luminosity define each star type?'),
            ...
            iq.stuckRevealed && h('div', { style: { padding: 10, background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: 4, fontSize: 11, color: '#cbd5e1', marginBottom: 10 } },
              h('ul', { style: { margin: 0, paddingLeft: 18 } },
                h('li', null, t('stem.astronomy.set_mass_1_m_and_vary_temp_luminosity_', 'Set mass = 1 M☉ and vary temp + luminosity. Where does the Sun sit?')),
                h('li', null, t('stem.astronomy.find_two_stars_in_different_categories', 'Find two stars in different categories. What do they have in common?')),
                h('li', null, t('stem.astronomy.real_stars_cluster_on_the_main_sequenc', 'Real stars cluster on the "main sequence." Investigate why.')))),
            h('div', { style: { padding: 10, background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 4 } },
              h('label', { style: { display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 'bold', color: '#34d399', cursor: 'pointer' } },
                ...
                t('stem.astronomy.i_understand_explain_in_own_words', 'I understand — explain in own words')),
              iq.understood && h('textarea', { value: iq.explanation || '', onChange: function(e) { setIQ({ explanation: e.target.value }); }, placeholder: t('stem.astronomy.explain_how_mass_temperature_and_lumin', 'Explain how mass, temperature, and luminosity define a stellar category.'),
            ...
            h('div', { style: { marginTop: 10, padding: 8, background: 'rgba(15,28,47,0.5)', borderRadius: 4, fontSize: 10, fontStyle: 'italic', color: '#94a3b8' } },
              t('stem.astronomy.design_note_discrete_5_category_star_m', 'Design note: discrete 5-category star marker; no luminosity-class score; no reveal — by design.'))
          )
        );
      }
      ...
      function renderPrint() {
        ...
        return h('div', { style: { padding: 16 } },
          h('div', { className: 'no-print', style: { padding: 12, borderRadius: 10, background: 'rgba(99,102,241,0.10)', borderTop: '1px solid rgba(99,102,241,0.4)', borderRight: '1px solid rgba(99,102,241,0.4)', borderBottom: '1px solid rgba(99,102,241,0.4)', borderLeft: '3px solid ' + INDIGO, marginBottom: 12, fontSize: 12.5, color: '#c7d2fe', lineHeight: 1.65 } },
            h('strong', null, t('stem.astronomy.observing_kit', '🖨 Observing kit. ')),
            t('stem.astronomy.a_take_into_the_field_reference_your_o', 'A take-into-the-field reference: your observing list, this season\'s constellation map, the moon phase calendar, safety reminders, and a quick reference for binocular vs naked-eye targets.')
          ),
          h('div', { className: 'no-print', style: { marginBottom: 14, textAlign: 'center' } },
            h('button', { onClick: function() { try { window.print(); } catch (e) {} },
              style: { padding: '8px 18px', borderRadius: 8, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #4f46e5 0%, #6366f1 100%)', color: '#fff', fontWeight: 800, fontSize: 13 } }, t('stem.astronomy.print_save_as_pdf', '🖨 Print / Save as PDF'))
          ),
          h('style', null,
          ...
          h('div', { id: 'astro-print-region', style: { padding: 18, borderRadius: 12, background: '#ffffff', color: '#0f172a', border: '1px solid #e2e8f0' } },
            h('div', { style: { display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', borderBottom: '2px solid #0f172a', paddingBottom: 8, marginBottom: 14 } },
              h('h2', { style: { margin: 0, fontSize: 22, fontWeight: 900, color: '#0f172a' } }, t('stem.astronomy.night_sky_observing_kit', 'Night Sky Observing Kit')),
              h('div', { style: { fontSize: 11, color: '#475569' } }, t('stem.astronomy.ngss_ms_ess1_hs_ess1', 'NGSS MS-ESS1 · HS-ESS1'))
            ),
            ...
            h('div', { style: { padding: 10, background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, marginBottom: 12, fontSize: 12, lineHeight: 1.55, color: '#7f1d1d' } },
              ...
              t('stem.astronomy.never_look_at_the_sun_without_iso_1231', 'Never look at the Sun without ISO 12312-2 solar-eclipse glasses (NOT sunglasses, NOT smoked glass). Lunar eclipses and meteor showers are safe to view directly. Use a red flashlight outside to preserve dark adaptation.')
            ),

            obsList.length > 0 ? h('div', { style: { padding: 12, border: '2px solid #0f172a', borderRadius: 10, marginBottom: 12, pageBreakInside: 'avoid' } },
              h('div', { style: { fontSize: 13, fontWeight: 800, color: '#0f172a', marginBottom: 8 } }, t('stem.astronomy.my_observing_list', 'My observing list')),
            ...
            h('div', { style: { padding: 12, border: '2px solid #0f172a', borderRadius: 10, marginBottom: 12, pageBreakInside: 'avoid' } },
              h('div', { style: { fontSize: 13, fontWeight: 800, color: '#0f172a', marginBottom: 8 } }, t('stem.astronomy.naked_eye_seasonal_sky_northern_hemisp', 'Naked-eye seasonal sky (Northern Hemisphere)')),
              h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: 11.5 } },
                h('thead', null, h('tr', null,
                  h('th', { style: { padding: 6, border: '1px solid #cbd5e1', background: '#f1f5f9', textAlign: 'left' } }, t('stem.astronomy.season', 'Season')),
                  h('th', { style: { padding: 6, border: '1px solid #cbd5e1', background: '#f1f5f9', textAlign: 'left' } }, t('stem.astronomy.what_s_up', 'What\'s up'))
                )),
                h('tbody', null,
                  [
            ...
            h('div', { style: { padding: 12, border: '2px solid #0f172a', borderRadius: 10, marginBottom: 12, pageBreakInside: 'avoid' } },
              h('div', { style: { fontSize: 13, fontWeight: 800, color: '#0f172a', marginBottom: 8 } }, t('stem.astronomy.annual_meteor_showers_set_a_calendar_r', 'Annual meteor showers (set a calendar reminder)')),
              h('ul', { style: { margin: 0, padding: '0 0 0 22px', fontSize: 12, color: '#0f172a', lineHeight: 1.65 } },
                h('li', null, h('strong', null, 'Quadrantids: '), t('stem.astronomy.jan_3_4_strong_but_short_northern_radi', 'Jan 3-4. Strong but short. Northern radiant. Cold; bundle up.')),
                h('li', null, h('strong', null, 'Lyrids: '), t('stem.astronomy.april_22_23_modest_15_20_per_hour', 'April 22-23. Modest, ~15-20 per hour.')),
                h('li', null, h('strong', null, 'Perseids: '), t('stem.astronomy.august_12_13_the_most_popular_shower_w', 'August 12-13. The most popular shower. Warm summer nights, ~60-100 per hour in dark skies.')),
                h('li', null, h('strong', null, 'Orionids: '), t('stem.astronomy.october_21_22_moderate_from_halley_s_c', 'October 21-22. Moderate, from Halley\'s Comet debris.')),
                h('li', null, h('strong', null, 'Leonids: '), t('stem.astronomy.november_17_18_variable', 'November 17-18. Variable.')),
                h('li', null, h('strong', null, 'Geminids: '), t('stem.astronomy.december_13_14_the_strongest_most_reli', 'December 13-14. The strongest, most reliable shower. ~120 per hour in dark skies. Cold but worth it.'))
              )
            ),

            h('div', { style: { padding: 12, border: '2px solid #0f172a', borderRadius: 10, marginBottom: 12, pageBreakInside: 'avoid' } },
              h('div', { style: { fontSize: 13, fontWeight: 800, color: '#0f172a', marginBottom: 8 } }, t('stem.astronomy.my_sky_darkness', 'My sky darkness')),
            ...
            h('div', { style: { padding: 12, border: '2px solid #0f172a', borderRadius: 10, marginBottom: 12, pageBreakInside: 'avoid' } },
              h('div', { style: { fontSize: 13, fontWeight: 800, color: '#0f172a', marginBottom: 8 } }, t('stem.astronomy.tonight_checklist', 'Tonight checklist')),
              h('ul', { style: { margin: 0, padding: '0 0 0 22px', fontSize: 12, color: '#0f172a', lineHeight: 1.75 } },
                h('li', null, t('stem.astronomy.check_moon_phase_look_near_full_moon_t', '□ Check moon phase. Look near full moon? Try the Moon itself. Try meteor showers near new moon.')),
                h('li', null, t('stem.astronomy.let_your_eyes_dark_adapt_for_15_20_min', '□ Let your eyes dark-adapt for 15-20 minutes. Use a red flashlight if you need light.')),
                h('li', null, t('stem.astronomy.pick_one_thing_to_find_tonight_don_t_t', '□ Pick ONE thing to find tonight. Don\'t try to learn everything.')),
                h('li', null, t('stem.astronomy.layer_up_it_s_colder_than_you_think_es', '□ Layer up. It\'s colder than you think, especially in Maine.')),
                h('li', null, t('stem.astronomy.bring_water_a_chair_if_you_ll_be_out_l', '□ Bring water, a chair if you\'ll be out long, binoculars if you have them.')),
                h('li', null, t('stem.astronomy.tell_someone_where_you_re_going_if_you', '□ Tell someone where you\'re going if you\'re going to a dark site alone.'))
              )
            ),

            h('div', { style: { marginTop: 14, padding: 10, borderTop: '2px solid #0f172a', fontSize: 10.5, color: '#94a3b8', lineHeight: 1.5 } },
              t('stem.astronomy.sources_nasa_nasa_gov_international_da', 'Sources: NASA (nasa.gov) · International Dark-Sky Association (darksky.org) · NGSS Lead States, 2013 · Bortle, J. (2001), "Introducing the Bortle Dark-Sky Scale" · Penobscot Nation, Passamaquoddy Tribe, and other Wabanaki sources for the local sky traditions. Printed from AlloFlow STEM Lab.')
            )
          )
        );
      }
      ...
      return h('div', { className: 'selh-astronomy', style: { display: 'flex', flexDirection: 'column', height: '100%', background: BG, color: '#e2e8f0' } },
        ...
        h('div', { style: { padding: '12px 16px', borderBottom: '1px solid #1e293b', background: 'linear-gradient(135deg, #1e1b4b, #0f172a)', display: 'flex', alignItems: 'center', gap: 12 } },
          ...
          h('div', null,
            h('h2', { style: { margin: 0, color: '#c7d2fe', fontSize: 20, fontWeight: 900 } }, t('stem.astronomy.night_sky_astronomy', 'Night Sky & Astronomy')),
            h('div', { style: { fontSize: 12, color: '#94a3b8', marginTop: 2 } }, t('stem.astronomy.earth_space_science_ngss_ms_ess1_hs_es', 'Earth & Space Science · NGSS MS-ESS1 · HS-ESS1'))
          )
        ),
        tabBar,
