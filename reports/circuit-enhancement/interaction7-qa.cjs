...
await page.evaluate(()=>setState({_circuit:{pauseMotion:true,benchView:'3d',mode:'series',voltage:9,components:[{type:'resistor',value:470,id:1},{type:'led',ledColor:'#ef4444',id:2},{type:'switch',closed:true,id:3}]}}));
await page.getByRole('button',{name:'Select led 2 in 3D scene',exact:true}).click();
assert.equal(await page.evaluate(()=>state._circuit.selectedPart),1);
const startCurrent=await page.evaluate(()=>StemLab.solveCircuit(state._circuit).current);
const viewport=page.locator('.circuit-scene-viewport');
await viewport.evaluate(el=>el.scrollIntoView({block:'center'}));
const box=await viewport.boundingBox();
await page.mouse.move(box.x+box.width*.52,box.y+box.height*.2);
await page.mouse.down();await page.mouse.move(box.x+box.width*.65,box.y+box.height*.27,{steps:10});await page.mouse.up();
await page.waitForFunction(()=>state._circuit.cameraYaw!==undefined);
assert.ok(await page.evaluate(()=>state._circuit.cameraYaw>-22&&state._circuit.cameraTilt<48));
assert.equal(await page.evaluate(()=>StemLab.solveCircuit(state._circuit).current),startCurrent);
assert.equal(await page.evaluate(()=>(state._circuit.undo||[]).length),0);
await page.getByRole('button',{name:'Reset camera',exact:true}).click();
await page.getByRole('button',{name:'Select resistor 1 in 3D scene',exact:true}).focus();
await page.keyboard.press('Enter');
assert.equal(await page.evaluate(()=>state._circuit.selectedPart),0);
await page.getByRole('button',{name:'Close-up',exact:true}).click();
assert.equal(await page.locator('.circuit-scene-pin').count(),1);
await page.getByRole('button',{name:'Reset camera',exact:true}).click();
await page.evaluate(()=>{
window.sceneRenders=0;window.originalCircuitCreateElement=React.createElement;
React.createElement=function(type,props,...children){if(type==='section'&&props?.className==='circuit-3d')window.sceneRenders++;return window.originalCircuitCreateElement(type,props,...children);};
setState(prev=>({...prev,_circuit:{...prev._circuit,tick:101}}));
});
await page.waitForFunction(()=>state._circuit.tick===101);
await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
assert.equal(await page.evaluate(()=>sceneRenders),0);
await page.getByLabel('Voltage slider',{exact:true}).fill('12');
assert.ok(await page.evaluate(()=>sceneRenders)>0);
assert.notEqual(await page.evaluate(()=>StemLab.solveCircuit(state._circuit).current),startCurrent);
await page.evaluate(()=>{React.createElement=window.originalCircuitCreateElement;});
assert.equal(await page.locator('svg[aria-label^="Interactive"]').count(),0);
await page.getByRole('button',{name:'Schematic',exact:true}).click();
assert.equal(await page.locator('svg[aria-label^="Interactive"]').count(),1);
await page.getByRole('button',{name:'3D bench',exact:true}).click();
await page.locator('.circuit-3d').screenshot({path:path.join(out,'direct-3d-desktop.png')});
const directAxe=await page.evaluate(async()=>axe.run({include:[['.circuit-3d']]},{runOnly:{type:'tag',values:['wcag2a','wcag2aa','wcag21aa']}}));
assert.deepEqual(directAxe.violations.map(v=>({id:v.id,nodes:v.nodes.map(n=>n.target)})),[]);
await page.setViewportSize({width:390,height:844});
await page.locator('.circuit-3d').screenshot({path:path.join(out,'direct-3d-mobile.png')});
assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));

await page.evaluate(()=>setState({_circuit:{pauseMotion:true,benchView:'3d',selectedPart:7,components:Array.from({length:7},(_,i)=>({type:'resistor',value:100,id:i+1}))}}));
assert.equal(await page.getByRole('button',{name:'Select resistor 7 in 3D scene',exact:true}).getAttribute('aria-pressed'),'true');
assert.equal(await page.getByRole('button',{name:'Inspect 3D part 7 resistor',exact:true}).getAttribute('aria-pressed'),'true');
...
fs.writeFileSync(path.join(out,'interaction7-results.json'),JSON.stringify({errors,axeViolations:axe.violations,checks:['all materials','eight parallel branches','selected close-up','camera preserves physics','reset restores wide view','top view','phone layout','label toggle','empty close-up disabled','pointer camera control','keyboard part selection','camera excluded from undo','static geometry reuse','electrical updates refresh scene','hidden schematic unmounted','direct-scene accessibility','selection remains consistent after removal']},null,2));
