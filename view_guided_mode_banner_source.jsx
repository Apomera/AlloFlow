...
function GuidedModeBanner({
  ...
  const normalizeGuidedPlannerDraft = (value) => {
    ...
    return {
      ...
      refinement: clean(value.refinement, 500), savedName: clean(value.savedName, 80), activeSavedId: clean(value.activeSavedId, 80),
    };
  };
  ...
  React.useEffect(() => { if (_skipUiPersistRef.current) { _skipUiPersistRef.current = false; return; } try { localStorage.setItem('allo_guided_ui_state', JSON.stringify({ collapsed: isCollapsed, showPicker, showAbout: !!showGuidedTip, infoTab })); } catch (_) {} }, [isCollapsed, showPicker, showGuidedTip, infoTab]);
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try { localStorage.setItem('allo_guided_planner_draft', JSON.stringify({ version: 1, goal: aiPlannerGoal, plan: aiPlannerPlan, answers: aiPlannerAnswers, messages: aiPlannerMessages, refinement: aiPlannerRefinement, savedName: savedAiPlanName, activeSavedId: activeSavedAiPlanId, updatedAt: new Date().toISOString() })); } catch (_) {}
    }, 250);
  ...
  React.useEffect(() => {
    ...
    const onKey = (event) => {
      if (event.key === 'Escape') { event.stopPropagation(); if (plannerDirty) setPendingPlannerClose(true); else { setShowAiPlanner(false); setPendingAiPlanApply(false); } return; }
  ...
  const resetPlannerWorkspace = () => {
    setAiPlannerGoal(''); setAiPlannerPlan(null); setAiPlannerError(''); setAiPlannerRefinement(''); setAiPlannerMessages([]); setAiPlannerLastChanges([]); setAiPlannerQuestions([]); setAiPlannerAnswers({}); setSavedAiPlanName(''); setActiveSavedAiPlanId(null); setSavedAiPlanStatus(''); setPendingAiPlanApply(false); setPendingPlannerClose(false); setPlannerRecoveryDraft(null); setPlannerDirty(false);
  };
  const clearPlannerDraft = () => { try { localStorage.removeItem('allo_guided_planner_draft'); } catch (_) {} };
  const persistPlannerDraftNow = () => { try { localStorage.setItem('allo_guided_planner_draft', JSON.stringify({ version: 1, goal: aiPlannerGoal, plan: aiPlannerPlan, answers: aiPlannerAnswers, messages: aiPlannerMessages, refinement: aiPlannerRefinement, savedName: savedAiPlanName, activeSavedId: activeSavedAiPlanId, updatedAt: new Date().toISOString() })); } catch (_) {} };
  const openAiPlanner = () => {
    let recovered = null;
    ...
    setPlannerRecoveryDraft(recovered); setShowAiPlanner(true); setShowPicker(false); setAiPlannerError(''); setSavedAiPlanStatus(''); setPendingPlannerClose(false);
  };
  ...
  const resumePlannerDraft = () => {
    ...
    if (!draft) return;
    setAiPlannerGoal(draft.goal || ''); setAiPlannerPlan(draft.plan ? { ...draft.plan, source: 'draft' } : null); setAiPlannerAnswers(draft.answers || {}); setAiPlannerQuestions([]); setAiPlannerMessages(draft.messages || []); setAiPlannerRefinement(draft.refinement || ''); setSavedAiPlanName(draft.savedName || draft.plan?.title || ''); setActiveSavedAiPlanId(draft.activeSavedId || null); setAiPlannerLastChanges([]); setPlannerRecoveryDraft(null); setPendingPlannerClose(false); setPlannerDirty(true);
  };
  ...
  const requestAiGuidedPlan = async (skipClarification = false) => {
    ...
    try {
      ...
      setAiPlannerPlan(result); setAiPlannerMessages([{ role: 'teacher', text: goal }, ...(contextLines.length ? [{ role: 'teacher', text: (t('guided.ai_plan_context_message') || 'Planning context: {context}').replace('{context}', contextLines.join(' · ')) }] : []), { role: 'planner', text: result.summary || result.title }]); setAiPlannerLastChanges([]); setPlannerDirty(true);
  ...
  const requestAiPlanRefinement = async () => {
    ...
    try {
      ...
      setAiPlannerPlan(result); setPlannerDirty(true);
  ...
  const updateAiPlanStep = (id, enabled) => {
    ...
    setAiPlannerLastChanges([]); setSavedAiPlanStatus(''); setPlannerDirty(true);
  };
  const saveCurrentAiPlan = () => {
    ...
    const saved = { ...aiPlannerPlan, id, source: 'saved', name, title: aiPlannerPlan.title || name, goal: String(aiPlannerPlan.goal || aiPlannerGoal || '').slice(0, 1200), originalGoal: String(aiPlannerGoal || aiPlannerPlan.originalGoal || aiPlannerPlan.goal || '').slice(0, 1200), clarificationAnswers: { ...aiPlannerAnswers }, savedAt: new Date().toISOString() };
  ...
  const loadSavedAiPlan = (saved) => {
    try {
      ...
      setAiPlannerPlan(plan); setAiPlannerGoal(saved.originalGoal || saved.goal || ''); setAiPlannerAnswers(saved.clarificationAnswers || {}); setAiPlannerQuestions([]); setSavedAiPlanName(saved.name || saved.title || ''); setActiveSavedAiPlanId(saved.id);
  ...
  const applyAiPlanNow = () => {
    ...
    setPendingAiPlanApply(false); setPendingPlannerClose(false); setPlannerDirty(false); clearPlannerDraft(); setShowAiPlanner(false); setShowPicker(false); rememberPathChoice();
  };
  ...
  const confirmGuidedJump = () => { if (!pendingJump) return; if (handleGuidedJump) handleGuidedJump(pendingJump.target, pendingJump.bypassedIds); else setGuidedStep(pendingJump.target); setPendingJump(null); };
    ...
    <>
      ...
      <div className="allo-guided-banner" role="region" aria-label={t('guided.indicator_title') || 'Guided mode'} style={{ background: 'linear-gradient(135deg, #312e81, #1e3a5f)', borderRadius: '20px', padding: '16px', marginBottom: '16px', border: '1px solid rgba(99,102,241,0.3)', flexShrink: 0 }}>
        ...
        {showAiPlanner && (
          ...
          <div ref={_aiPlannerDialogRef} className="allo-guided-planning-studio" role="dialog" aria-modal="true" aria-labelledby="guided-ai-planner-title" aria-describedby="guided-ai-planner-intro" tabIndex={-1}>
            <div className="allo-guided-planning-header" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}><div><strong id="guided-ai-planner-title" style={{ display: 'block', color: 'white', fontSize: '20px' }}>✨ {t('guided.ai_plan_studio_title') || 'Guided Planning Studio'}</strong><span id="guided-ai-planner-intro" style={{ display: 'block', color: '#d1fae5', fontSize: '13px', lineHeight: 1.5, marginTop: '4px' }}>{t('guided.ai_plan_studio_intro') || 'Describe the lesson, answer only the context questions that matter, then review the full path before applying it.'}</span></div><button type="button" aria-label={t('common.close') || 'Close'} onClick={requestCloseAiPlanner} style={{ minWidth: '44px', minHeight: '44px', borderRadius: '10px', border: '1px solid rgba(255,255,255,.3)', background: 'rgba(15,23,42,.3)', color: 'white' }}>×</button></div>
            {pendingPlannerClose && <div role="alert" aria-labelledby="guided-planner-close-title" style={{ marginBottom: '12px', padding: '12px', borderRadius: '10px', background: 'rgba(120,53,15,.42)', border: '1px solid rgba(251,191,36,.6)', color: '#fef3c7' }}><strong id="guided-planner-close-title" style={{ display: 'block', color: 'white' }}>{t('guided.ai_plan_unsaved_title') || 'Keep this unfinished plan?'}</strong><span style={{ display: 'block', marginTop: '3px' }}>{t('guided.ai_plan_unsaved_text') || 'Your work can be kept as a private draft on this device, or discarded without affecting saved plans.'}</span><div style={{ display: 'flex', gap: '7px', flexWrap: 'wrap', marginTop: '9px' }}><button type="button" onClick={keepPlannerDraftAndClose} style={{ flex: 1, minWidth: '150px', border: 0, borderRadius: '8px', background: '#10b981', color: '#052e2b', fontWeight: 900 }}>{t('guided.ai_plan_keep_draft') || 'Keep draft and close'}</button><button type="button" onClick={discardPlannerDraftAndClose} style={{ flex: 1, minWidth: '130px', borderRadius: '8px', border: '1px solid rgba(248,113,113,.5)', background: 'transparent', color: '#fecaca', fontWeight: 800 }}>{t('guided.ai_plan_discard_draft') || 'Discard and close'}</button><button type="button" onClick={() => setPendingPlannerClose(false)} style={{ flex: 1, minWidth: '130px', borderRadius: '8px', border: '1px solid rgba(255,255,255,.3)', background: 'transparent', color: 'white' }}>{t('guided.ai_plan_continue_editing') || 'Continue editing'}</button></div></div>}
            {plannerRecoveryDraft ? <div role="region" aria-labelledby="guided-planner-recovery-title" style={{ padding: '18px', borderRadius: '12px', background: 'rgba(79,70,229,.22)', border: '1px solid rgba(165,180,252,.5)' }}><strong id="guided-planner-recovery-title" style={{ display: 'block', color: 'white', fontSize: '16px' }}>{t('guided.ai_plan_recovery_title') || 'Resume your unfinished plan?'}</strong><span style={{ display: 'block', color: '#c7d2fe', marginTop: '5px', lineHeight: 1.5 }}>{(t('guided.ai_plan_recovery_text') || 'A private draft from {date} is available on this device.').replace('{date}', new Date(plannerRecoveryDraft.updatedAt).toLocaleString())}</span>{plannerRecoveryDraft.goal && <div style={{ marginTop: '9px', padding: '9px', borderRadius: '8px', background: 'rgba(15,23,42,.5)', color: '#e0e7ff' }}>{plannerRecoveryDraft.goal}</div>}<div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '12px' }}><button type="button" onClick={resumePlannerDraft} style={{ flex: 1, minWidth: '150px', border: 0, borderRadius: '8px', background: '#10b981', color: '#052e2b', fontWeight: 900 }}>{t('guided.ai_plan_resume_draft') || 'Resume draft'}</button><button type="button" onClick={startFreshPlanner} style={{ flex: 1, minWidth: '150px', borderRadius: '8px', border: '1px solid rgba(255,255,255,.3)', background: 'transparent', color: 'white' }}>{t('guided.ai_plan_start_fresh') || 'Discard draft and start fresh'}</button></div></div> : <>
            {savedAiPlans.length > 0 && <div role="region" aria-labelledby="guided-saved-plans-title" style={{ marginBottom: '9px', padding: '8px', borderRadius: '9px', background: 'rgba(99,102,241,.12)', border: '1px solid rgba(165,180,252,.28)' }}><strong id="guided-saved-plans-title" style={{ display: 'block', color: 'white', fontSize: '12px', marginBottom: '5px' }}>{t('guided.ai_plan_saved_title') || 'Your saved Guided plans'}</strong><div style={{ display: 'grid', gap: '5px' }}>{savedAiPlans.map(saved => <div key={saved.id} style={{ padding: '6px 7px', borderRadius: '7px', background: 'rgba(15,23,42,.42)' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: '7px', alignItems: 'center' }}><span style={{ minWidth: 0 }}><strong style={{ display: 'block', color: '#e0e7ff', fontSize: '11px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{saved.name}</strong><span style={{ display: 'block', color: '#a5b4fc', fontSize: '10px' }}>{saved.stepIds.length + 4} {t('guided.ai_plan_total_steps') || 'total steps'} · {saved.estimatedMinutes} {t('guided.ai_plan_minutes_short') || 'min'}</span></span><span style={{ display: 'flex', gap: '4px', flexShrink: 0 }}><button type="button" onClick={() => loadSavedAiPlan(saved)} disabled={aiPlannerBusy} style={{ minHeight: '34px', padding: '4px 7px', border: 0, borderRadius: '6px', fontWeight: 800 }}>{t('guided.ai_plan_load') || 'Load'}</button><button type="button" aria-label={(t('guided.ai_plan_delete_named') || 'Delete {name}').replace('{name}', saved.name)} onClick={() => setPendingDeleteSavedAiPlanId(saved.id)} disabled={aiPlannerBusy} style={{ minHeight: '34px', padding: '4px 7px', borderRadius: '6px', border: '1px solid rgba(248,113,113,.4)', background: 'transparent', color: '#fecaca' }}>{t('guided.ai_plan_delete') || 'Delete'}</button></span></div>{pendingDeleteSavedAiPlanId === saved.id && <div role="alert" style={{ marginTop: '5px', color: '#fecaca', fontSize: '10px' }}>{t('guided.ai_plan_delete_confirm') || 'Delete this reusable plan?'}<div style={{ display: 'flex', gap: '4px', marginTop: '4px' }}><button type="button" onClick={() => deleteSavedAiPlan(saved.id)} style={{ minHeight: '32px', border: 0, borderRadius: '6px', background: '#b91c1c', color: 'white' }}>{t('guided.ai_plan_delete_now') || 'Delete plan'}</button><button type="button" onClick={() => setPendingDeleteSavedAiPlanId(null)} style={{ minHeight: '32px', borderRadius: '6px', border: '1px solid rgba(255,255,255,.25)', background: 'transparent', color: 'white' }}>{t('common.cancel') || 'Cancel'}</button></div></div>}</div>)}</div></div>}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', marginTop: '3px', color: '#a7f3d0', fontSize: '10px' }}><span>{t('guided.ai_plan_privacy') || 'Do not include student names or sensitive personal information.'}</span><span aria-label={t('guided.ai_plan_character_count') || 'Character count'}>{aiPlannerGoal.length}/1200</span></div>
            ...
            ].map(example => <button type="button" key={example} disabled={aiPlannerBusy} onClick={() => { setAiPlannerGoal(example); setAiPlannerPlan(null); setAiPlannerQuestions([]); setAiPlannerAnswers({}); setAiPlannerLastChanges([]); setPlannerDirty(true); setAiPlannerError(''); }} style={{ minHeight: '34px', padding: '5px 7px', borderRadius: '999px', border: '1px solid rgba(165,180,252,.35)', background: 'rgba(255,255,255,.07)', color: '#e0e7ff', fontSize: '10px' }}>{example}</button>)}</div>
            ...
            {aiPlannerPlan && (
              <div style={{ marginTop: '9px', padding: '10px', borderRadius: '10px', background: 'rgba(15,23,42,.48)', border: '1px solid rgba(165,180,252,.3)' }}>
                ...
                {Array.isArray(aiPlannerPlan.assumptions) && aiPlannerPlan.assumptions.length > 0 && <div style={{ marginTop: '7px' }}><strong style={{ display: 'block', color: '#fef3c7', fontSize: '11px' }}>{t('guided.ai_plan_assumptions') || 'Review these assumptions'}</strong><ul style={{ margin: '3px 0 0', paddingLeft: '18px', color: '#e0e7ff', fontSize: '11px', lineHeight: 1.45 }}>{aiPlannerPlan.assumptions.map((item, index) => <li key={index}>{item}</li>)}</ul></div>}
                ...
                <fieldset style={{ margin: '9px 0 0', padding: 0, border: 0 }}><legend style={{ color: 'white', fontSize: '12px', fontWeight: 900 }}>{t('guided.ai_plan_steps_title') || 'Review and edit the steps'}</legend><div style={{ marginTop: '5px', maxHeight: '190px', overflowY: 'auto', display: 'grid', gap: '4px', paddingRight: '3px' }}>{(allSteps || []).filter(item => !['source-input', 'directions', 'package-deliver', '_final'].includes(item.id)).map(item => { const checked = aiPlannerPlan.stepIds.includes(item.id); const reason = aiPlannerPlan.stepReasons?.[item.id]; return <label key={item.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '7px', padding: '6px 7px', borderRadius: '7px', background: checked ? 'rgba(16,185,129,.12)' : 'rgba(255,255,255,.04)', color: 'white', fontSize: '11px' }}><input type="checkbox" checked={checked} onChange={event => updateAiPlanStep(item.id, event.target.checked)} /><span><strong style={{ display: 'block' }}>{localizeStep(item, 'label')}</strong>{checked && reason && <span style={{ display: 'block', color: '#c7d2fe', marginTop: '2px', lineHeight: 1.35 }}>{reason}</span>}</span></label>; })}</div></fieldset>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '6px', marginTop: '8px' }}><label style={{ color: '#e0e7ff', fontSize: '11px' }}>{t('guided.delivery_setting') || 'Teaching setting'}<select value={aiPlannerPlan.deliverySetting || 'take-home'} onChange={event => { setAiPlannerPlan(previous => ({ ...previous, deliverySetting: event.target.value })); setAiPlannerLastChanges([]); setPlannerDirty(true); setSavedAiPlanStatus(''); }} style={{ display: 'block', width: '100%', marginTop: '3px', padding: '6px', borderRadius: '7px', background: '#172554', color: 'white' }}><option value="take-home">{t('guided.delivery_setting_take_home') || 'Take-home'}</option><option value="print">{t('guided.delivery_setting_print') || 'Print / paper'}</option><option value="live">{t('guided.delivery_setting_live') || 'Live class'}</option><option value="lms">{t('guided.delivery_setting_lms') || 'LMS'}</option></select></label><label style={{ color: '#e0e7ff', fontSize: '11px' }}>{t('guided.delivery_priority') || 'Top priority'}<select value={aiPlannerPlan.deliveryPriority || 'accessible'} onChange={event => { setAiPlannerPlan(previous => ({ ...previous, deliveryPriority: event.target.value })); setAiPlannerLastChanges([]); setPlannerDirty(true); setSavedAiPlanStatus(''); }} style={{ display: 'block', width: '100%', marginTop: '3px', padding: '6px', borderRadius: '7px', background: '#172554', color: 'white' }}><option value="accessible">{t('guided.delivery_priority_accessible') || 'Accessibility'}</option><option value="editable">{t('guided.delivery_priority_editable') || 'Editable'}</option><option value="assessment">{t('guided.delivery_priority_assessment') || 'Assessment'}</option><option value="interactive">{t('guided.delivery_priority_interactive') || 'Interactive'}</option><option value="low-connectivity">{t('guided.delivery_priority_offline') || 'Low connectivity'}</option></select></label></div>
                <div role="region" aria-labelledby="guided-ai-roadmap-title" style={{ marginTop: '9px', padding: '9px', borderRadius: '9px', background: 'rgba(15,23,42,.55)', border: '1px solid rgba(110,231,183,.28)' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: '7px', alignItems: 'baseline' }}><strong id="guided-ai-roadmap-title" style={{ color: 'white', fontSize: '12px' }}>{t('guided.ai_plan_roadmap_title') || 'Before you begin: lesson roadmap'}</strong><span style={{ color: '#a7f3d0', fontSize: '10px' }}>{aiPlanRoadmapSteps.length} {t('guided.ai_plan_total_steps') || 'total steps'} · {aiPlanResourceLabels.length} {t('guided.ai_plan_resources_short') || 'resources'} · {Math.max(5, Number(aiPlannerPlan.estimatedMinutes) || 0)} {t('guided.ai_plan_minutes_short') || 'min'}</span></div><span style={{ display: 'block', color: '#c7d2fe', fontSize: '10px', marginTop: '2px', lineHeight: 1.4 }}>{t('guided.ai_plan_roadmap_hint') || 'This is the path Guided Mode will walk through after you approve it.'}</span><ol aria-label={t('guided.ai_plan_phase_roadmap') || 'Guided phase roadmap'} style={{ listStyle: 'none', margin: '7px 0 0', padding: 0, display: 'grid', gap: '5px' }}>{aiPlanPhaseGroups.map((phase, phaseIndex) => { const phaseKey = 'guided.phase_' + phase.id; const translated = t(phaseKey); const phaseName = translated && translated !== phaseKey ? translated : phase.label; return <li key={phase.id} style={{ display: 'grid', gridTemplateColumns: '22px minmax(0, 1fr)', gap: '6px', alignItems: 'start' }}><span aria-hidden="true" style={{ width: '22px', height: '22px', display: 'grid', placeItems: 'center', borderRadius: '999px', background: '#4f46e5', color: 'white', fontSize: '10px', fontWeight: 900 }}>{phaseIndex + 1}</span><span><strong style={{ display: 'block', color: '#a7f3d0', fontSize: '10px' }}>{phaseName}</strong><span style={{ display: 'block', color: '#e0e7ff', fontSize: '10px', lineHeight: 1.35 }}>{phase.steps.map(item => localizeStep(item, 'label')).join(' → ')}</span></span></li>; })}</ol><div style={{ marginTop: '7px', paddingTop: '7px', borderTop: '1px solid rgba(255,255,255,.1)' }}><strong style={{ display: 'block', color: '#fef3c7', fontSize: '10px' }}>{t('guided.ai_plan_expected_resources') || 'Expected lesson resources'}</strong><span style={{ display: 'block', marginTop: '2px', color: '#e0e7ff', fontSize: '10px', lineHeight: 1.4 }}>{aiPlanResourceLabels.join(' · ')}</span></div>{aiPlanDeliveryRecommendation && <div style={{ marginTop: '7px', padding: '7px', borderRadius: '7px', background: 'rgba(16,185,129,.1)', color: '#d1fae5', fontSize: '10px', lineHeight: 1.4 }}><strong>{t('guided.ai_plan_delivery_preview') || 'Planned delivery'}:</strong> {aiPlanDeliveryRecommendation.primary} <span aria-hidden="true">→</span> <strong>{t('guided.delivery_backup') || 'Backup'}:</strong> {aiPlanDeliveryRecommendation.backup}<span style={{ display: 'block', color: '#a7f3d0', marginTop: '2px' }}>{aiPlanDeliveryRecommendation.why}</span></div>}</div>
                <div role="region" aria-labelledby="guided-ai-readiness-title" style={{ marginTop: '10px', padding: '10px', borderRadius: '10px', background: aiPlanReadinessAttentionCount ? 'rgba(120,53,15,.22)' : 'rgba(6,78,59,.3)', border: '1px solid ' + (aiPlanReadinessAttentionCount ? 'rgba(251,191,36,.5)' : 'rgba(110,231,183,.4)') }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}><strong id="guided-ai-readiness-title" style={{ color: 'white', fontSize: '13px' }}>{t('guided.ai_plan_readiness_title') || 'Plan readiness'}</strong><span style={{ padding: '3px 7px', borderRadius: '999px', background: aiPlanReadinessAttentionCount ? 'rgba(251,191,36,.16)' : 'rgba(16,185,129,.18)', color: aiPlanReadinessAttentionCount ? '#fde68a' : '#a7f3d0', fontWeight: 900 }}>{aiPlanReadinessAttentionCount ? ((t('guided.ai_plan_readiness_attention') || '{count} needs attention').replace('{count}', aiPlanReadinessAttentionCount)) : (t('guided.ai_plan_readiness_ready') || 'Ready to start')}</span></div><ul aria-live="polite" style={{ listStyle: 'none', padding: 0, margin: '7px 0 0', display: 'grid', gap: '5px' }}>{aiPlanReadinessItems.map(item => <li key={item.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '7px', color: item.state === 'attention' ? '#fde68a' : item.state === 'ready' ? '#d1fae5' : '#e0e7ff', lineHeight: 1.45 }}><span aria-hidden="true" style={{ flexShrink: 0 }}>{item.state === 'attention' ? '⚠' : item.state === 'ready' ? '✓' : 'ℹ'}</span><span style={{ flex: 1 }}>{item.label}{item.action === 'quiz' && <button type="button" onClick={() => updateAiPlanStep('quiz', true)} style={{ display: 'block', marginTop: '5px', padding: '5px 8px', minHeight: '34px', border: 0, borderRadius: '7px', background: '#f59e0b', color: '#451a03', fontWeight: 900 }}>{t('guided.ai_plan_add_assess') || 'Add Assess step'}</button>}</span></li>)}</ul></div>
                <div role="group" aria-labelledby="guided-ai-save-title" style={{ marginTop: '8px', padding: '8px', borderRadius: '8px', background: 'rgba(255,255,255,.05)' }}><strong id="guided-ai-save-title" style={{ display: 'block', color: 'white', fontSize: '11px' }}>{t('guided.ai_plan_save_title') || 'Save this path for another lesson'}</strong><div style={{ display: 'flex', gap: '5px', marginTop: '5px' }}><label htmlFor="guided-ai-save-name" style={{ position: 'absolute', width: '1px', height: '1px', padding: 0, margin: '-1px', overflow: 'hidden', clip: 'rect(0, 0, 0, 0)', whiteSpace: 'nowrap', border: 0 }}>{t('guided.ai_plan_save_name') || 'Saved plan name'}</label><input id="guided-ai-save-name" value={savedAiPlanName} maxLength={80} onChange={event => { setSavedAiPlanName(event.target.value); setPlannerDirty(true); setSavedAiPlanStatus(''); }} placeholder={aiPlannerPlan.title || (t('guided.ai_plan_save_name') || 'Saved plan name')} style={{ minWidth: 0, flex: 1, padding: '6px 7px', borderRadius: '7px', border: '1px solid rgba(165,180,252,.35)', background: '#172554', color: 'white', fontSize: '11px' }} /><button type="button" onClick={saveCurrentAiPlan} disabled={aiPlannerBusy} style={{ padding: '6px 8px', border: 0, borderRadius: '7px', background: '#0f766e', color: 'white', fontWeight: 900, fontSize: '10px' }}>{activeSavedAiPlanId ? (t('guided.ai_plan_save_changes') || 'Save changes') : (t('guided.ai_plan_save') || 'Save plan')}</button></div><span style={{ display: 'block', color: '#a5b4fc', fontSize: '9px', marginTop: '3px' }}>{t('guided.ai_plan_save_local') || 'Saved only on this device. Generated lesson content is not included.'}</span>{savedAiPlanStatus && <div role="status" aria-live="polite" style={{ color: '#a7f3d0', fontSize: '10px', marginTop: '4px' }}>{savedAiPlanStatus}</div>}</div>
                <div className="allo-guided-planning-actions">
                {pendingAiPlanApply && <div role="alert" style={{ marginTop: '8px', padding: '8px', borderRadius: '8px', background: 'rgba(120,53,15,.32)', border: '1px solid rgba(251,191,36,.55)', color: '#fef3c7', fontSize: '11px' }}><strong style={{ display: 'block', color: 'white' }}>{t('guided.ai_plan_replace_title') || 'Apply this new path?'}</strong>{t('guided.ai_plan_replace_text') || 'Guided progress will restart at Source Material. Your generated resources remain in History.'}<div style={{ display: 'flex', gap: '6px', marginTop: '7px' }}><button type="button" onClick={applyAiPlanNow} style={{ flex: 1, border: 0, borderRadius: '7px', fontWeight: 900 }}>{t('guided.ai_plan_apply_confirm') || 'Apply and restart path'}</button><button type="button" onClick={() => setPendingAiPlanApply(false)} style={{ flex: 1, borderRadius: '7px', border: '1px solid rgba(255,255,255,.25)', background: 'transparent', color: 'white' }}>{t('common.cancel') || 'Cancel'}</button></div></div>}
                {!pendingAiPlanApply && <button type="button" disabled={!aiPlannerPlan.stepIds.length} onClick={applyAiPlan} style={{ width: '100%', marginTop: '9px', padding: '8px 10px', border: 0, borderRadius: '9px', background: '#10b981', color: '#052e2b', fontWeight: 900 }}>{t('guided.ai_plan_apply') || 'Use this Guided plan'}</button>}
                </div>
              </div>
            )}
            </>}
...
{step.id === 'package-deliver' && Array.isArray(guidedDeliveryGroups) && (
        ...
        {isLast && (
          <div role="region" aria-labelledby="guided-readiness-title" style={{ marginBottom: '10px', padding: '11px 13px', background: 'rgba(59,130,246,.13)', border: '1px solid rgba(147,197,253,.38)', borderRadius: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {isLast && <button type="button" disabled={guidedBusy} onClick={() => { const summary = { completedCount, skippedCount, resourceCount: recapItems.length, readinessCount, readinessTotal, stepLabels: GUIDED_STEPS.map(item => localizeStep(item, 'label')) }; if (typeof handleCompleteGuidedMode === 'function') handleCompleteGuidedMode(summary); else { if (typeof resetGuidedProgress === 'function') resetGuidedProgress(); handleExitGuidedMode(); } }} style={{ flex: 1, padding: '6px 12px', fontSize: '12px', fontWeight: 700, color: 'white', background: 'linear-gradient(135deg, #818cf8, #6366f1)', border: 'none', borderRadius: '10px', cursor: 'pointer', transition: 'all 0.2s' }}>{readinessCount === readinessTotal ? (t('guided.all_done') || 'Finish Guided Mode') : (t('guided.finish_with_checks_remaining') || 'Finish with checks remaining')}</button>}
