...
// Run: node dev-tools/watercycle_pilot_winter_shoreline_qa.cjs
const fs = require('fs');
const path = require('path');
const assert = require('node:assert/strict');
const { chromium } = require('playwright');
const ROOT = process.cwd();
const out = path.join(ROOT, 'scratch', 'water-winter-shoreline-review');
...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(()=>pilotReview.scene.getObjectByName('pilot-lake-shore-ground').material.userData.snowShader);
    const frost=await page.evaluate(()=>{
      const scene=pilotReview.scene,lake=scene.getObjectByName('pilot-lake-water');
      const winter=lake.userData.surfaceShader.uniforms.pilotLakeWinter;
      const mats=['pilot-lake-shore-ground','pilot-lake-shore-stones','pilot-stream-bank-stones'].map(n=>scene.getObjectByName(n).material);
      return {active:winter.value,shared:mats.every(m=>m.userData.snowShader?.uniforms.pilotSurfaceSnow===winter),
        liquid:document.querySelector('#wcPilotCanvas').dataset.parcelForm,
        unchangedRadius:lake.geometry.parameters.radius===34,
        shorelineMask:mats[0].userData.snowShader.fragmentShader.includes('smoothstep(34.6, 35.8'),
        clippedStream:mats[2].userData.snowShader.fragmentShader.includes('vPilotLakeJoinXZ')};
    });assert.deepEqual(frost,{active:1,shared:true,liquid:'liquid',unchangedRadius:true,shorelineMask:true,clippedStream:true});
    assert(Math.abs(await parcelY()-11.8)<.01);
    await page.emulateMedia({reducedMotion:'no-preference'});await page.waitForTimeout(300);
    const winterTime=await ripple();await page.getByRole('button',{name:'Resume the simulation',exact:true}).click();
    await page.waitForFunction(old=>pilotReview.scene.getObjectByName('pilot-lake-water').userData.surfaceShader.uniforms.pilotLakeTime.value>old+.05,winterTime);
    await page.getByRole('button',{name:'Pause the simulation',exact:true}).click();
    const frozenTime=await ripple();await page.waitForTimeout(300);assert.equal(await ripple(),frozenTime);
    assert.equal(await canvas.getAttribute('data-parcel-form'),'liquid');
    ...
    assert.equal(await page.evaluate(()=>pilotReview.scene.getObjectByName('pilot-lake-water').userData.surfaceShader.uniforms.pilotLakeWinter.value),0);
    ...
    await restore();
    assert.equal(await page.evaluate(()=>pilotReview.scene.getObjectByName('pilot-lake-shore-ground').material.userData.snowShader.uniforms.pilotSurfaceSnow.value),0);
    await canvas.screenshot({path:path.join(out,'lake-restored-temperate.jpg'),type:'jpeg',quality:82});
