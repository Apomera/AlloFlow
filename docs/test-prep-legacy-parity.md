...
- Native quality repair wave 01 fixed visible encoding corruption in 661 items, removed brittle answer-letter labels from 189 items (142 contradicted the rotated key), and rewrote 19 severe stacked-modifier or answer-length choice sets plus one newly short rationale. The strengthened QA gate now blocks those defects. A separate 113-item queue still requires content-specific rewriting because mechanically deleting its repeated option prefix would restore severe answer-length clues; those sets are explicitly not represented as repaired.
