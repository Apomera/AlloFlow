...
source+=String.raw`
  ...
  for(const [handleR,drumR] of [[0.05,0.02],[0.45,0.1],[1,0.5],[0.05,0.5],[1,0.02],[0.45,0.25]]){
    await mount('windlass',{windlassHandleR:handleR,windlassDrumR:drumR,shopFocusMechanism:true,shopRotY:72});
  ...
  for(const progress of [0,0.25,0.5,0.75,1]){
    await mount('windlass',{shopMotionProgress:progress,shopFocusMechanism:true,shopRotY:90});await checkPose(progress);
