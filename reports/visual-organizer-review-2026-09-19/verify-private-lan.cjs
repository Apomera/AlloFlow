...
(async()=>{const server=runtime.createServer();await new Promise(r=>server.listen(0,'127.0.0.1',r));let share;const checks=[];let controller;
 ...
 try{
  ...
  const reflection={version:1,resourceId:resource.id,activityId:remote.activityId,fields:[{id:'know',label:'Know',text:'PRIVATE LEARNER WRITING'}]};
  r=await req(student,'/api/lan-sessions/ORGANIZER1/reflections','POST',reflection,token);ok('Private reflection submission accepted',r.status===200);const first=r.data.reflection;ok('Server assigns revision and reference diagram',first.revision===1&&first.diagram.main==='Water');
  r=await req(student,'/api/lan-sessions/ORGANIZER1/reflections','POST',reflection,token);ok('Retry deduplicates saved writing',r.status===200&&r.data.reflection.id===first.id);
  r=await req(teacher,'/api/lan-sessions/ORGANIZER1/reflections');ok('Teacher receives private writing',r.status===200&&r.data.reflections[0].fields[0].text==='PRIVATE LEARNER WRITING');
  r=await req(student,'/api/lan-sessions/ORGANIZER1/reflections','GET',null,other);ok('Peer cannot retrieve writing',r.status===200&&r.data.reflections.length===0);
  r=await req(student,'/api/lan-sessions/ORGANIZER1/reflections');ok('Anonymous reading denied',r.status>=400);
  r=await req(student,'/api/lan-sessions/ORGANIZER1/reflections','PATCH',{id:first.id,text:'Fake teacher',expectedUpdatedAt:0},token);ok('Participant feedback mutation denied',r.status>=400);
  r=await req(teacher,'/api/lan-sessions/ORGANIZER1/reflections','PATCH',{id:first.id,text:'PRIVATE TEACHER FEEDBACK',expectedUpdatedAt:0});ok('Teacher feedback saved',r.status===200);
  r=await req(teacher,'/api/lan-sessions/ORGANIZER1/reflections','PATCH',{id:first.id,text:'Stale',expectedUpdatedAt:0});ok('Stale feedback overwrite rejected',r.status===409);
  r=await req(student,'/api/lan-sessions/ORGANIZER1/reflections','GET',null,token);ok('Reconnected original student retrieves feedback',r.status===200&&r.data.reflections[0].feedback.text==='PRIVATE TEACHER FEEDBACK');
  await req(teacher,'/api/lan-sessions/ORGANIZER1','PATCH',{updates:{resources:[{...resource,data:{...resource.data,main:'Edited diagram'}}]}});
  r=await req(student,'/api/lan-sessions/ORGANIZER1/reflections','POST',{...reflection,fields:[{id:'know',label:'Know',text:'PRIVATE REVISED WRITING'}]},token);ok('Revision keeps launch diagram',r.status===200&&r.data.reflection.revision===2&&r.data.reflection.diagram.main==='Water');
  r=await req(teacher,'/api/lan-sessions/ORGANIZER1');ok('Private writing and feedback absent from shared snapshot',!JSON.stringify(r.data).includes('PRIVATE'));
  const stolenUidToken=await join('student1');r=await req(student,'/api/lan-sessions/ORGANIZER1/reflections','GET',null,stolenUidToken);ok('New token cannot read an existing learner identity',r.status===200&&r.data.reflections.length===0);
  ...
  r=await req(student,'/api/lan-sessions/ORGANIZER1/reflections','POST',{...reflection,fields:[{id:'know',label:'Know',text:'After stop'}]},token);ok('Ended reflection rejects new writing',r.status===409);
  await new Promise(r=>setTimeout(r,80));ok('Private writing and feedback absent from live SSE',!streamText.includes('PRIVATE'));controller.abort();await collect;ok('Live SSE carries retry, ready, and stop updates',streamText.includes('retryUids')&&streamText.includes('"interactiveOrganizer":null'));
  fs.writeFileSync(path.join(output,'private-lan-results.json'),JSON.stringify({checks,passed:checks.length,scope:'Real isolated Desktop LAN HTTP and SSE listeners; teacher private API and two participant tokens; no external classroom changed.'},null,2));console.log(JSON.stringify({passed:checks.length,checks}));
