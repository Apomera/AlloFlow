...
function _mathViz(op, a, b, ans){
  ...
  if(op === '×'){
    ...
    body += txt(gx + gw + 34, gy + gh / 2 + 5, '= ?', GREEN, 16);
  } else if(op === '÷'){
    ...
    body += txt(bx + bw / (seg * 2), by + bh / 2 + 4, '?', GREEN, 12);
    ...
    body += txt(bx + remW / 2, by + bh / 2 + 4, '?', INK, 13);
    ...
    body += txt(bx + bw / 2, by + bh + 22, '?', GREEN, 13);
  }
...
function _parseAiQuestions(text, subject){
  ...
  for(const o of arr){
    ...
    out.push({
      text: String(o.q).trim().slice(0, 240),
      choices: choices.map((c, i) => [String(i), c]),
      correct: String(ci),
      explain: (String(o.explain || '').trim().slice(0, 240)) || 'Good thinking!',
