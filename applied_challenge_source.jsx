...
function normalizeAppliedChallengeCoverage(value) {
  ...
  return { version: 1, workspaceFields: count('workspaceFields', 11), evidenceRows: count('evidenceRows', 12), sourceRecords: count('sourceRecords', 24),
...
function appliedChallengeCoverageText(value, t) {
  ...
  return _apsFill(_apsT(t, 'applied_challenge.coverage.counts', 'Included: {workspaceFields} writing sections, {evidenceRows} evidence rows, {validationChecks} saved checks, and {selfChecks} self-ratings.'), coverage) + ' ' +
    _apsFill(_apsT(t, 'applied_challenge.coverage.sources', '{sourceRecords} saved source records.'), coverage) + ' ' +
...
function appliedChallengeFeedbackContext(value, options) {
  ...
  const make = ceiling => {
    ...
    const context = {
      ...
      studentWork: { workspace, reasoningReferences: data.reasoningReferences.map((ref, index) => ({ part: ref.part, source: ref.source, rowId: ref.rowId, location: fit(ref.location, 'reasoningReferences.' + index), current: appliedReasoningReferenceState(data, ref.part).current, learnerIdentified: true })), linkedWorkAvailable: !!data.workspace.artifactUrl,
    ...
    const coverage = { version: 1, workspaceFields: Object.keys(workspace).length, evidenceRows: rows.length, sourceRecords: data.sourceRecords.length,
...
function normalizeAppliedChallengeSources(value) {
  ...
  for (const item of Array.isArray(value) ? value : []) {
    ...
    ids.add(id);
    result.push({ id, url, rowId, title: typeof item.title === 'string' ? _apsString(item.title, 350) : new URL(url).hostname,
...
function AppliedChallengeSourceNotebook({ sources, rows, editable, onAdd, onUpdate, onRemove, onEdit, session, t }) {
  ...
  const contents = <>
    ...
    {sources.length > 0 && <ol className='mt-3 space-y-3'>{sources.map((source, index) => <li key={source.id} className='min-w-0 rounded-xl border border-slate-200 bg-white p-3'>
      <h4 className='text-sm font-bold'><a href={source.url} target='_blank' rel='noopener noreferrer' className='inline-block min-h-11 break-words py-2 text-blue-800 underline'>{source.title || new URL(source.url).hostname}<span className='sr-only'> {_apsT(t, 'applied_challenge.source_review.new_tab', '(opens in a new tab)')}</span></a></h4>
...
function AppliedReasoningReferenceEditor({ data, item, onChange, t }) {
  ...
  return <details className='mt-2 rounded-xl border border-slate-200 px-3 text-sm'>
    ...
    <div className='space-y-3 pb-3'>
      ...
      {saved && <button type='button' className='aps-button' onClick={event => { event.currentTarget.closest('details')?.querySelector('summary')?.focus(); onChange(null); }}>{tx('remove', 'Remove this reference')}</button>}
    </div>
  </details>;
}
...
function appliedChallengeReviewFollowups(value, t) {
  ...
  data.sourceRecords.forEach((source, index) => { if (!source.reviewNote.trim()) add('source-' + source.id, _apsFill(tx('source_note', 'Record what you checked about source {n}, what remains uncertain, or why you will not use it.'), { n: index + 1 }), { phase: 'possibilities', elementId: 'aps-source-note-' + source.id }); });
  let rowIssues = 0;
...
function appliedChallengeRequestFingerprint(value, purpose, options) {
  ...
  return JSON.stringify({
    ...
    reasoningReferences: data.reasoningReferences,
    sourceRecords: data.sourceRecords,
    feedbackContextVersion: purpose === 'feedback' ? 3 : undefined,
  ...
  return (
    <div>
      <div className='m-3 space-y-4 rounded-2xl border border-orange-200 bg-orange-50/50 p-3'>
        ...
        <details className='rounded-xl border border-orange-200 bg-white p-3'><summary className='min-h-11 cursor-pointer text-sm font-bold'>{tx('applied_challenge.plan.materials_selection', 'Materials and lesson selection')}</summary><div className='mt-3 space-y-3'>
        ...
        <details className='rounded-xl border border-orange-200 bg-white p-3'><summary className='min-h-11 cursor-pointer text-sm font-bold'>{tx('applied_challenge.panel.customize', 'Customize the challenge and support')}</summary><div className='mt-3 space-y-4'>
...
function AppliedChallengeEvidenceSources({ evidence, rowId, t, editable = false, targetId }) {
  ...
  return <div className='min-w-0 space-y-2 lg:col-span-2'>
    ...
    {editable && <details className='applied-challenge-no-print rounded-xl bg-slate-50 px-3 text-sm text-slate-700'>
      ...
      <button type='button' className='aps-button mb-3' onClick={() => document.getElementById(targetId || 'aps-ledger-evidence-' + rowId)?.focus()}>{tx('action', 'Write my source check')}</button>
    </details>}
  </div>;
}
      ...
      {available && state.status === 'done' && <>
        ...
        <ul className='space-y-3'>{state.results.map(result => {
          ...
          return <li key={result.url} className='min-w-0 rounded-xl border border-slate-200 p-3'>
            ...
            {onAddReference && <><button type='button' className='aps-button mt-3' disabled={!check.ok} onClick={() => onAddReference(result, notesMode ? '' : destination)}>{added ? tx('added', 'Reference added') : tx('add', 'Add reference to my evidence')}</button>{check.reason === 'sources-full' && <p className='mt-2 text-sm text-amber-900'>{_apsT(t, 'applied_challenge.sources.full', 'You have 24 saved sources. Remove an unused source before adding another.')}</p>}{check.reason === 'length' && <p className='mt-2 text-sm text-amber-900'>{notesMode ? tx('notes_full', 'Your evidence notes are full. Keep your writing and save the source separately, or make room before adding it.') : tx('length', 'This row has too much writing to add the full reference. Choose another row, create a new one, or edit it first.')}</p>}</>}
          </li>;
        })}</ul>
        {onAddReference && <p className='text-sm text-slate-600'>{sourceRecords && (notesMode || destination || rows.length < 12) ? tx('saved_separately', 'Citation details are saved in Saved outside sources. Add your own source-check note, then explain how the evidence affects your reasoning.') : notesMode ? tx('notes_reference', 'The reference is added to your evidence notes as an unchecked source. Explain what it supports or challenges in your own words.') : !destination && rows.length >= 12 ? tx('full', 'Your evidence table has 12 rows. Choose an existing row above to add a reference.') : tx('reference_note', 'Adding a reference leaves it marked “Needs checking.” Write the claim it informs and explain what you found in your own words.')}</p>}
        <p className='text-xs text-slate-600'>{tx('session', 'You can switch steps and return to these results. Clearing the search, leaving this workspace, or reloading clears them. References you added stay saved with your work.')}</p>
      </>}
    </div>
  </details>;
}
...
function AppliedChallengeView(props) {
  ...
  const renderWorkspacePhase = (phase) => {
    ...
    return <article key={phase.id} className='applied-challenge-section rounded-2xl border border-slate-200 bg-slate-50/60 p-4' aria-labelledby={headingId}>
      ...
      {isTeacherMode && isEditing && !learnerReadOnly
        ...
        : <p id={headingId + '-prompt'} className={data.supports.phasePrompts[phase.id] === label ? 'sr-only' : 'mt-1 text-sm leading-relaxed text-slate-600'}>{data.supports.phasePrompts[phase.id]}</p>}
  ...
  const field = id => phase(id) ? <>{renderWorkspacePhase(phase(id))}{id === 'evidence' && <AppliedChallengeEvidenceSources evidence={data.workspace.evidence} t={t} editable={!learnerReadOnly && !isTeacherMode} targetId='applied-workspace-evidence' />}</> : null;
  ...
  const renderReference = () => <details className='aps-reference min-w-0 rounded-2xl bg-slate-50 p-3' aria-label={tx('applied_challenge.reference.heading', 'Challenge reference')}>
    <summary className='min-h-11 cursor-pointer text-sm font-bold text-slate-900'>{tx('applied_challenge.reference.nearby', 'Lesson ideas and requirements')}</summary>
    <p className='mt-2 text-sm leading-relaxed text-slate-700'>{data.brief.context}</p>
  ...
  return <main id='applied-challenge-print-root' className='applied-challenge-root mx-auto w-full max-w-6xl p-3 sm:p-6' aria-labelledby='applied-challenge-title'>
    ...
    {isTeacherMode ? <>
      ...
      <div className='my-3 flex flex-wrap items-center justify-between gap-2'><p className='text-sm text-slate-600' aria-live='polite'>{reviewOpen ? tx('applied_challenge.review.heading', 'Review my response') : _apsFill(tx('applied_challenge.workspace.stage_progress', '{stage} · Step {number} of 5'), { stage: stageLabel(currentStage), number: stageIndex + 1 })}</p><button type='button' className='aps-button applied-challenge-no-print' aria-pressed={!focusMode} onClick={() => { setFocusMode(!focusMode); setReviewOpen(false); }}>{focusMode ? tx('applied_challenge.focus.show_all', 'Show all steps') : tx('applied_challenge.focus.one_step', 'Focus on one step')}</button></div>
      ...
      <div className='aps-grid'>{renderReference()}<div className='min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5'>
