...
var d = (labToolData.companionPlanting) || {};
          ...
          function renderCommunityGarden() {
            ...
            return h('div', { className: 'space-y-3' },
              ...
              // ── 4x4 Grid ──
              // ── Animated Community Garden Canvas (time-based, cached gradients) ──
              h('canvas', {
                'aria-hidden': 'true',
                style: { width: '100%', height: '140px', borderRadius: '12px', display: 'block' },
                ref: function(cvEl) {
                  if (!cvEl || cvEl._cgCanvasInit) return;
                  cvEl._cgCanvasInit = true;
                  var gctx = cvEl.getContext('2d');
                  var W = cvEl.width = cvEl.offsetWidth * 2;
                  var H = cvEl.height = 280;
                  var startT = performance.now();
                  var groundY = H * 0.38;
                  var cellW = (W - 40) / 4; var cellH = (H - groundY - 20) / 4;
                  ...
                  var skyGrads = [['#87ceeb','#e0f0ff'], ['#4da6ff','#fff8e0'], ['#cc8844','#ffe8cc'], ['#8899aa','#d0dde8']].map(function(c) {
                    var g = gctx.createLinearGradient(0, 0, 0, H * 0.4); g.addColorStop(0, c[0]); g.addColorStop(1, c[1]); return g;
                  });
                  var groundGrad = gctx.createLinearGradient(0, groundY, 0, H);
                  groundGrad.addColorStop(0, '#7a9e5c'); groundGrad.addColorStop(0.3, '#6b8a4e'); groundGrad.addColorStop(1, '#5a7040');
                  // (Emoji cache removed — now using procedural plant graphics)
                  function drawCG() {
                    cvEl._cgAnim = requestAnimationFrame(drawCG);
                    var t = (performance.now() - startT) / 1000; // seconds, smooth
                    // Read season from data attr (set by parent render) to avoid stale closure
                    var sSeason = parseInt(cvEl.getAttribute('data-cg-season') || '0', 10);
                    // Sky (cached gradient)
                    gctx.fillStyle = skyGrads[sSeason] || skyGrads[0]; gctx.fillRect(0, 0, W, H * 0.4);
                    // Sun (smooth time-based)
                    var sunX = W * 0.85 + Math.sin(t * 0.3) * 10;
                    var sunY = 30 + Math.sin(t * 0.2) * 5;
                    gctx.fillStyle = '#ffd700'; gctx.beginPath(); gctx.arc(sunX, sunY, 16, 0, Math.PI * 2); gctx.fill();
                    gctx.globalAlpha = 0.15; gctx.fillStyle = '#ffd700'; gctx.beginPath(); gctx.arc(sunX, sunY, 28, 0, Math.PI * 2); gctx.fill(); gctx.globalAlpha = 1;

                    // Clouds (drifting, fluffy)
                    if (sSeason !== 3) { // no clouds in winter (replaced by snow)
                      gctx.fillStyle = 'rgba(255,255,255,' + (sSeason === 1 ? 0.25 : 0.18) + ')';
                      for (var cl = 0; cl < 3; cl++) {
                        var clx = (t * 8 + cl * W * 0.4) % (W + 100) - 50;
                        var cly = 15 + cl * 12 + Math.sin(t * 0.2 + cl * 2) * 3;
                        var clr = 20 + cl * 8;
                        gctx.beginPath(); gctx.ellipse(clx, cly, clr, clr * 0.4, 0, 0, Math.PI * 2); gctx.fill();
                        gctx.beginPath(); gctx.ellipse(clx + clr * 0.5, cly - 3, clr * 0.7, clr * 0.3, 0, 0, Math.PI * 2); gctx.fill();
                        gctx.beginPath(); gctx.ellipse(clx - clr * 0.4, cly + 2, clr * 0.5, clr * 0.25, 0, 0, Math.PI * 2); gctx.fill();
                      }
                    }

                    // Birds (small V-shapes in spring/summer)
                    if (sSeason < 2) {
                      gctx.strokeStyle = 'rgba(60,60,60,0.3)'; gctx.lineWidth = 1;
                      for (var bd = 0; bd < 3; bd++) {
                        var bdx = (t * 12 + bd * 80) % (W + 60) - 30;
                        var bdy = 20 + bd * 15 + Math.sin(t * 1.5 + bd * 3) * 8;
                        gctx.beginPath();
                        gctx.moveTo(bdx - 4, bdy + 2); gctx.quadraticCurveTo(bdx, bdy - 2 + Math.sin(t * 5 + bd) * 1, bdx + 4, bdy + 2);
                        gctx.stroke();
                      }
                    }

                    // Background hedgerow / tree line (behind garden, above ground)
                    var treeY = groundY - 2;
                    gctx.fillStyle = sSeason === 3 ? 'rgba(100,100,110,0.4)' : sSeason === 2 ? 'rgba(140,100,40,0.35)' : 'rgba(40,100,30,0.35)';
                    for (var tr3 = 0; tr3 < 12; tr3++) {
                      var trx = tr3 * (W / 11) - 5;
                      var trSize = 12 + (tr3 * 7 % 5) * 3;
                      gctx.beginPath(); gctx.arc(trx, treeY - trSize * 0.3, trSize, 0, Math.PI * 2); gctx.fill();
                      // Trunk
                      gctx.fillStyle = 'rgba(80,50,20,0.25)';
                      gctx.fillRect(trx - 1.5, treeY - 3, 3, 6);
                      gctx.fillStyle = sSeason === 3 ? 'rgba(100,100,110,0.4)' : sSeason === 2 ? 'rgba(140,100,40,0.35)' : 'rgba(40,100,30,0.35)';
                    }

                    // Ground (cached gradient)
                    gctx.fillStyle = groundGrad; gctx.fillRect(0, groundY, W, H);

                    // Garden path (gravel walkway down the middle)
                    gctx.fillStyle = 'rgba(180,170,150,0.2)';
                    gctx.fillRect(W / 2 - 6, groundY + 5, 12, H - groundY - 5);
                    // Gravel dots
                    gctx.fillStyle = 'rgba(160,150,130,0.15)';
                    for (var gv = 0; gv < 10; gv++) {
                      gctx.beginPath(); gctx.arc(W / 2 - 3 + (gv % 3) * 3, groundY + 10 + gv * ((H - groundY - 15) / 10), 1, 0, Math.PI * 2); gctx.fill();
                    }
                    // Garden beds + plants (read from data attr)
                    var gridJSON = cvEl.getAttribute('data-cg-grid') || '[]';
                    var grid2; try { grid2 = JSON.parse(gridJSON); } catch(e) { grid2 = []; }
                    // Soil beds with topsoil texture and moisture tint
                    var cgMoistVal = parseInt(cvEl.getAttribute('data-cg-moisture') || '50', 10);
                    for (var gr = 0; gr < 4; gr++) {
                      for (var gc2 = 0; gc2 < 4; gc2++) {
                        var bx3 = 20 + gc2 * cellW + 2; var by3 = groundY + 10 + gr * cellH + 2;
                        var bw3 = cellW - 4; var bh3 = cellH - 4;
                        // Base soil (darker when moist)
                        var soilDark = Math.max(0, 50 - cgMoistVal * 0.3);
                        gctx.fillStyle = 'rgba(' + (120 - soilDark) + ',' + (80 - soilDark * 0.5) + ',' + (40 - soilDark * 0.3) + ',0.4)';
                        gctx.fillRect(bx3, by3, bw3, bh3);
                        // Subtle soil texture (tiny dots)
                        gctx.fillStyle = 'rgba(90,60,30,0.08)';
                        for (var st = 0; st < 4; st++) {
                          gctx.beginPath();
                          gctx.arc(bx3 + (st * 13 + gr * 7) % bw3, by3 + (st * 11 + gc2 * 5) % bh3, 1, 0, Math.PI * 2);
                          gctx.fill();
                        }
                        // Raised bed edge highlight (top edge lighter)
                        gctx.fillStyle = 'rgba(160,120,70,0.12)';
                        gctx.fillRect(bx3, by3, bw3, 1.5);
                      }
                    }
                    // ── Procedural Plant Drawing (no emojis) ──
                    // Plant type → visual appearance: fruit color/shape, leaf style, flower type
                    var PLANT_VISUALS = {
                      tomato: { stemColor: '#3a6b20', fruitColor: '#e53e3e', fruitShape: 'circle', fruitR: 4, leafColor: '#4a8c30', flowerColor: '#fde047' },
                      corn: { stemColor: '#5a8a30', fruitColor: '#f0c040', fruitShape: 'cob', fruitR: 3, leafColor: '#6aaa40', flowerColor: null, tall: true },
                      beans: { stemColor: '#3a7a25', fruitColor: '#558833', fruitShape: 'pod', fruitR: 3, leafColor: '#50a030', flowerColor: '#dda0dd', vine: true },
                      squash: { stemColor: '#4a7a20', fruitColor: '#e08020', fruitShape: 'oval', fruitR: 5, leafColor: '#3a8a20', flowerColor: '#fbbf24', wide: true },
                      lettuce: { stemColor: '#5aaa40', fruitColor: null, fruitShape: null, leafColor: '#70cc50', flowerColor: null, bushy: true },
                      carrot: { stemColor: '#5a8a30', fruitColor: '#f97316', fruitShape: 'root', fruitR: 2, leafColor: '#60a840', flowerColor: null },
                      pepper: { stemColor: '#3a6b20', fruitColor: '#dc2626', fruitShape: 'bell', fruitR: 3, leafColor: '#4a8c30', flowerColor: '#fff' },
                      marigold: { stemColor: '#5a8a30', fruitColor: null, fruitShape: null, leafColor: '#60a840', flowerColor: '#f59e0b', flower: true },
                      sunflower: { stemColor: '#4a7a20', fruitColor: '#7c3a1a', fruitShape: 'circle', fruitR: 4, leafColor: '#4a8a30', flowerColor: '#fbbf24', tall: true, flower: true },
                      strawberry: { stemColor: '#3a7020', fruitColor: '#e53e3e', fruitShape: 'berry', fruitR: 3, leafColor: '#40a030', flowerColor: '#fff', low: true },
                      blueberry: { stemColor: '#4a6830', fruitColor: '#3b5998', fruitShape: 'berry', fruitR: 2, leafColor: '#4a8040', flowerColor: '#ddd' }
                    };
                    var defaultVis = { stemColor: '#4a7a2e', fruitColor: '#888', fruitShape: 'circle', fruitR: 3, leafColor: '#50a030', flowerColor: null };

                    var polCount = 0;
                    for (var gr2 = 0; gr2 < 4; gr2++) {
                      for (var gc3 = 0; gc3 < 4; gc3++) {
                        var ci3 = gr2 * 4 + gc3;
                        var cell3 = grid2[ci3];
                        if (!cell3 || !cell3.plantId || !CG_PLANTS[cell3.plantId]) continue;
                        var pl2 = CG_PLANTS[cell3.plantId];
                        if (pl2.pollinator) polCount++;
                        var growPct2 = Math.min(1, cell3.growthDay / pl2.days);
                        var px2 = 20 + gc3 * cellW + cellW / 2;
                        var py2 = groundY + 10 + gr2 * cellH + cellH - 6;
                        var stemH2 = growPct2 * (cellH * 0.55);
                        var sway = Math.sin(t * 1.2 + ci3 * 0.7) * growPct2 * 2;
                        var vis = PLANT_VISUALS[cell3.plantId] || defaultVis;

                        if (pl2.isStructure) {
                          // Structures: draw as simple geometric shapes
                          if (cell3.plantId === 'bee_hotel') {
                            gctx.fillStyle = '#8B6914'; gctx.fillRect(px2 - 5, py2 - 10, 10, 10);
                            gctx.fillStyle = '#5a4010';
                            for (var bh = 0; bh < 3; bh++) { gctx.beginPath(); gctx.arc(px2 - 3 + bh * 3, py2 - 6, 1.5, 0, Math.PI * 2); gctx.fill(); }
                            gctx.fillStyle = '#6a5020'; gctx.fillRect(px2 - 6, py2 - 12, 12, 2); // roof
                          } else if (cell3.plantId === 'compost_bin') {
                            gctx.fillStyle = '#5a4030'; gctx.fillRect(px2 - 6, py2 - 9, 12, 9);
                            gctx.fillStyle = '#7a6040'; gctx.fillRect(px2 - 7, py2 - 11, 14, 3); // lid
                            gctx.fillStyle = '#3a6020'; gctx.beginPath(); gctx.arc(px2, py2 - 4, 2, 0, Math.PI * 2); gctx.fill(); // compost peek
                          } else if (cell3.plantId === 'rain_barrel') {
                            gctx.fillStyle = '#4a6a8a'; gctx.fillRect(px2 - 5, py2 - 10, 10, 10);
                            gctx.strokeStyle = '#3a5a7a'; gctx.lineWidth = 0.8;
                            gctx.strokeRect(px2 - 5, py2 - 10, 10, 10);
                            gctx.fillStyle = '#6a9abb'; gctx.fillRect(px2 - 4, py2 - 5, 8, 4); // water level
                          }
                        } else {
                          // ── Procedural Plant Rendering ──
                          // Stem with sway
                          gctx.strokeStyle = vis.stemColor; gctx.lineWidth = vis.tall ? 2 + growPct2 : 1 + growPct2;
                          gctx.beginPath(); gctx.moveTo(px2, py2);
                          gctx.quadraticCurveTo(px2 + sway * 0.5, py2 - stemH2 * 0.5, px2 + sway, py2 - stemH2); gctx.stroke();

                          // Leaves (multiple sizes, alternating sides)
                          if (growPct2 > 0.2) {
                            gctx.fillStyle = vis.leafColor;
                            var leafCount = vis.bushy ? 5 : vis.wide ? 3 : 2;
                            for (var lf = 0; lf < leafCount; lf++) {
                              var lfY = py2 - stemH2 * (0.3 + lf * 0.15);
                              var lfSide = lf % 2 === 0 ? -1 : 1;
                              var lfSize = (2 + growPct2 * 3) * (vis.bushy ? 1.5 : vis.wide ? 1.8 : 1);
                              gctx.beginPath();
                              gctx.ellipse(px2 + lfSide * (4 + lf) + sway * (0.5 + lf * 0.1), lfY,
                                lfSize, lfSize * 0.5, lfSide * -0.4, 0, Math.PI * 2);
                              gctx.fill();
                            }
                          }

                          // Flower (appears at 50%+ for flower plants)
                          if (vis.flowerColor && growPct2 > 0.5) {
                            var fTopX = px2 + sway; var fTopY = py2 - stemH2 - 2;
                            gctx.fillStyle = vis.flowerColor;
                            // Petals (5-point flower)
                            for (var pt = 0; pt < 5; pt++) {
                              var pa = pt * Math.PI * 2 / 5 + t * 0.3;
                              gctx.beginPath();
                              gctx.ellipse(fTopX + Math.cos(pa) * 3, fTopY + Math.sin(pa) * 3, 2.5, 1.5, pa, 0, Math.PI * 2);
                              gctx.fill();
                            }
                            // Center
                            gctx.fillStyle = vis.fruitColor || '#8B6914';
                            gctx.beginPath(); gctx.arc(fTopX, fTopY, 1.5, 0, Math.PI * 2); gctx.fill();
                          }

                          // Fruit (appears at 70%+)
                          if (vis.fruitColor && growPct2 > 0.7 && !vis.flower) {
                            var frX = px2 + sway; var frY = py2 - stemH2 * 0.7;
                            var frR = vis.fruitR * growPct2;
                            gctx.fillStyle = vis.fruitColor;
                            if (vis.fruitShape === 'circle' || vis.fruitShape === 'berry') {
                              gctx.beginPath(); gctx.arc(frX, frY, frR, 0, Math.PI * 2); gctx.fill();
                              // Highlight
                              gctx.fillStyle = 'rgba(255,255,255,0.3)';
                              gctx.beginPath(); gctx.arc(frX - frR * 0.3, frY - frR * 0.3, frR * 0.4, 0, Math.PI * 2); gctx.fill();
                            } else if (vis.fruitShape === 'pod') {
                              gctx.beginPath(); gctx.ellipse(frX + 3, frY, frR * 2, frR * 0.6, 0.3, 0, Math.PI * 2); gctx.fill();
                            } else if (vis.fruitShape === 'cob') {
                              gctx.fillRect(frX - 1.5, frY - frR * 2, 3, frR * 4);
                              // Husk
                              gctx.fillStyle = '#8aaa50'; gctx.beginPath();
                              gctx.ellipse(frX + 2, frY, 2, frR * 2.5, 0.2, 0, Math.PI * 2); gctx.fill();
                            } else if (vis.fruitShape === 'bell') {
                              gctx.beginPath(); gctx.moveTo(frX - frR, frY);
                              gctx.quadraticCurveTo(frX - frR, frY - frR * 1.5, frX, frY - frR * 1.8);
                              gctx.quadraticCurveTo(frX + frR, frY - frR * 1.5, frX + frR, frY);
                              gctx.lineTo(frX - frR, frY); gctx.fill();
                            } else if (vis.fruitShape === 'oval') {
                              gctx.beginPath(); gctx.ellipse(frX, frY + 2, frR, frR * 1.3, 0, 0, Math.PI * 2); gctx.fill();
                            } else if (vis.fruitShape === 'root') {
                              // Root vegetable: draw below soil line
                              gctx.fillStyle = vis.fruitColor;
                              gctx.beginPath(); gctx.moveTo(px2 - 2, py2);
                              gctx.lineTo(px2, py2 + frR * 3); gctx.lineTo(px2 + 2, py2); gctx.fill();
                            }
                          }
                        }

                        // Pest dots (batched color)
                        if (cell3.pests > 30) {
                          gctx.fillStyle = 'rgba(200,50,50,0.6)';
                          var pCount = Math.min(4, Math.floor(cell3.pests / 20));
                          for (var pd2 = 0; pd2 < pCount; pd2++) {
                            gctx.beginPath(); gctx.arc(px2 + (pd2 - 1.5) * 4, py2 - stemH2 * 0.3 + Math.sin(t * 3 + pd2) * 2, 1.5, 0, Math.PI * 2); gctx.fill();
                          }
                        }
                      }
                    }
                    // ── Companion interaction glows (green shimmer between companions, red between enemies) ──
                    for (var cg1 = 0; cg1 < 16; cg1++) {
                      if (!grid2[cg1] || !grid2[cg1].plantId) continue;
                      var c1r = Math.floor(cg1 / 4); var c1c = cg1 % 4;
                      var c1x = 20 + c1c * cellW + cellW / 2; var c1y = groundY + 10 + c1r * cellH + cellH / 2;
                      // Check right and down neighbors only (avoid double-drawing)
                      var adjIdx = [cg1 + 1, cg1 + 4];
                      adjIdx.forEach(function(ni2) {
                        if (ni2 >= 16 || !grid2[ni2] || !grid2[ni2].plantId) return;
                        if (cg1 % 4 === 3 && ni2 === cg1 + 1) return; // don't wrap rows
                        var n2r = Math.floor(ni2 / 4); var n2c = ni2 % 4;
                        var n2x = 20 + n2c * cellW + cellW / 2; var n2y = groundY + 10 + n2r * cellH + cellH / 2;
                        // Check if they're companions
                        var compMatch = CG_COMPANIONS.find(function(cp) {
                          return (cp.a === grid2[cg1].plantId && cp.b === grid2[ni2].plantId) ||
                                 (cp.b === grid2[cg1].plantId && cp.a === grid2[ni2].plantId);
                        });
                        if (compMatch) {
                          var glowAlpha = 0.06 + Math.sin(t * 2 + cg1) * 0.03;
                          gctx.strokeStyle = compMatch.bonus > 0 ? 'rgba(34,197,94,' + glowAlpha + ')' : 'rgba(239,68,68,' + glowAlpha + ')';
                          gctx.lineWidth = 3;
                          gctx.beginPath(); gctx.moveTo(c1x, c1y); gctx.lineTo(n2x, n2y); gctx.stroke();
                        }
                      });
                    }

                    // ── Growth sparkles (tiny particles rising from actively growing plants) ──
                    if (sSeason < 3) { // no growth sparkles in winter
                      for (var gs = 0; gs < 16; gs++) {
                        if (!grid2[gs] || !grid2[gs].plantId) continue;
                        var gsP = CG_PLANTS[grid2[gs].plantId];
                        if (!gsP || gsP.isStructure) continue;
                        var gsGrow = grid2[gs].growthDay / gsP.days;
                        if (gsGrow > 0.1 && gsGrow < 0.95) {
                          var gsr = Math.floor(gs / 4); var gsc = gs % 4;
                          var gsx = 20 + gsc * cellW + cellW / 2;
                          var gsy = groundY + 10 + gsr * cellH + cellH * 0.4;
                          // 1-2 sparkle particles
                          for (var sp2 = 0; sp2 < 2; sp2++) {
                            var spx = gsx + Math.sin(t * 3 + gs * 2 + sp2) * 6;
                            var spy = gsy - (t * 8 + gs * 5 + sp2 * 15) % (cellH * 0.5);
                            var spAlpha = 0.3 + Math.sin(t * 5 + sp2 + gs) * 0.2;
                            gctx.fillStyle = 'rgba(200,255,100,' + spAlpha + ')';
                            gctx.beginPath(); gctx.arc(spx, spy, 1, 0, Math.PI * 2); gctx.fill();
                          }
                        }
                        // Harvest glow (golden pulse on mature plants)
                        if (gsGrow >= 1 && grid2[gs].health > 20) {
                          var hgr = Math.floor(gs / 4); var hgc = gs % 4;
                          var hgx = 20 + hgc * cellW + cellW / 2;
                          var hgy = groundY + 10 + hgr * cellH + cellH / 2;
                          gctx.globalAlpha = 0.08 + Math.sin(t * 3 + gs) * 0.05;
                          gctx.fillStyle = '#fbbf24';
                          gctx.beginPath(); gctx.arc(hgx, hgy, cellW * 0.35, 0, Math.PI * 2); gctx.fill();
                          gctx.globalAlpha = 1;
                        }
                      }
                    }

                    // Pollinators (time-based flight paths)
                    for (var bi2 = 0; bi2 < Math.min(5, polCount); bi2++) {
                      var bx2 = (t * 25 + bi2 * 120) % (W + 40) - 20;
                      var by2 = groundY - 10 + Math.sin(t * 1.5 + bi2 * 2) * 15;
                      gctx.fillStyle = bi2 % 2 === 0 ? '#fbbf24' : '#f97316';
                      gctx.beginPath(); gctx.arc(bx2, by2, 2, 0, Math.PI * 2); gctx.fill();
                      gctx.fillStyle = 'rgba(255,255,255,0.5)';
                      var wingAngle = Math.sin(t * 8 + bi2) * 0.5;
                      gctx.beginPath(); gctx.ellipse(bx2 - 2, by2 - 1, 2.5, 1.5, wingAngle, 0, Math.PI * 2); gctx.fill();
                      gctx.beginPath(); gctx.ellipse(bx2 + 2, by2 - 1, 2.5, 1.5, -wingAngle, 0, Math.PI * 2); gctx.fill();
                    }
                    // Seasonal weather (time-based)
                    if (sSeason === 0 || sSeason === 2) {
                      gctx.strokeStyle = 'rgba(100,150,200,0.15)'; gctx.lineWidth = 0.5;
                      for (var ri3 = 0; ri3 < 12; ri3++) {
                        var rx2 = (ri3 * 47 + t * 40) % W;
                        var ry2 = (ri3 * 23 + t * 80) % groundY;
                        gctx.beginPath(); gctx.moveTo(rx2, ry2); gctx.lineTo(rx2 - 1, ry2 + 6); gctx.stroke();
                      }
                    }
                    if (sSeason === 3) {
                      gctx.fillStyle = 'rgba(255,255,255,0.5)';
                      for (var si3 = 0; si3 < 20; si3++) {
                        gctx.beginPath(); gctx.arc((si3 * 37 + t * 8) % W, (si3 * 19 + t * 15) % H, 1 + Math.sin(si3 + t * 0.5) * 0.5, 0, Math.PI * 2); gctx.fill();
                      }
                    }
                    // Fence (static, drawn last)
                    gctx.strokeStyle = '#8a6a4a'; gctx.lineWidth = 2;
                    for (var fi2 = 0; fi2 <= 4; fi2++) {
                      var fx2 = 18 + fi2 * cellW;
                      gctx.beginPath(); gctx.moveTo(fx2, groundY + 5); gctx.lineTo(fx2, groundY - 8); gctx.stroke();
                    }
                    gctx.strokeStyle = '#a0845a'; gctx.lineWidth = 1.5;
                    gctx.beginPath(); gctx.moveTo(18, groundY); gctx.lineTo(18 + 4 * cellW, groundY); gctx.stroke();
                  }
                  drawCG();
              ...
              // Pass garden state to canvas via data attributes (avoids stale closure)
              (function() {
                var cvEl2 = document.querySelector('canvas[data-cg-season]');
                if (!cvEl2) {
                  // Set attrs on next tick after canvas mounts
                  setTimeout(function() {
                    var c = document.querySelector('canvas[aria-hidden="true"]');
                    if (c && !c.hasAttribute('data-cg-season')) {
                      c.setAttribute('data-cg-season', cgSeason);
                      c.setAttribute('data-cg-grid', JSON.stringify(cgGrid));
                      c.setAttribute('data-cg-moisture', Math.round(cgMoisture));
                    }
                  }, 50);
                } else {
                  cvEl2.setAttribute('data-cg-season', cgSeason);
                  cvEl2.setAttribute('data-cg-grid', JSON.stringify(cgGrid));
                  cvEl2.setAttribute('data-cg-moisture', Math.round(cgMoisture));
                }
                return null;
              })(),
