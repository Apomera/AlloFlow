  ...
  window.StemLab.registerTool('particleLab3d', {
    ...
    render: function (ctx) {
      ...
      var particleViewportNode = h('div', { key: 'viewport', id: 'particle-viewport', className: 'relative overflow-hidden bg-slate-950' },
              h('canvas', { ref: canvasRef, tabIndex: ready ? 0 : -1, role: 'application', 'aria-hidden': ready ? undefined : 'true', 'aria-roledescription': 'Interactive 3D particle chamber', 'aria-label': preset + ' particle simulation', 'aria-describedby': 'particle-chamber-help', 'aria-keyshortcuts': 'Space R T V E M G C L F H D ? Escape', onKeyDown: onLabKey, onPointerDown: function (event) { if (!ready) return; try { event.currentTarget.focus(); } catch (error) {} }, className: 'h-full w-full saturate-[1.15] contrast-[1.04] focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-[-4px] focus-visible:outline-cyan-200' }),
          ...
          h('section', { id: 'particle-stage', 'data-fullscreen': fsActive ? 'true' : 'false', ref: stageRef, onKeyDown: onStageKeyDown, className: 'relative min-w-0 overflow-hidden border border-cyan-500/30 bg-slate-950 shadow-2xl shadow-cyan-950/40 ring-1 ring-white/5 ' + (fsActive ? 'h-screen rounded-none flex flex-col' : 'rounded-[24px]'), style: cssFullscreen ? { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100dvh', zIndex: 99990, borderRadius: 0 } : undefined, 'aria-label': 'Three-dimensional particle simulation', 'aria-busy': ready ? 'false' : 'true' },
            ...
            fsActive && showHud && showFullscreenConditions && h('section', { id: 'particle-fullscreen-conditions', className: 'max-h-[38vh] overflow-y-auto border-t border-cyan-400/30 bg-slate-900/95 px-4 py-3 text-white', 'aria-label': 'Fullscreen experiment conditions' },
              ...
              h('div', { className: 'grid gap-x-5 gap-y-3 sm:grid-cols-2 xl:grid-cols-3' },
                ...
                h('label', { className: 'block text-xs font-bold' }, 'Particle count ', h('output', { className: 'text-cyan-300' }, count), h('input', { type: 'range', min: 24, max: 120, step: 8, value: count, onChange: function (e) { var value = Number(e.target.value); setCount(value); setSelectedParticle(function (old) { return Math.min(old, value - 1); }); setRunning(false); setResetKey(function (k) { return k + 1; }); persist({ count: value, selectedParticle: Math.min(selectedParticle, value - 1) }); }, className: 'mt-1 block min-h-11 w-full accent-cyan-400', 'aria-label': 'Fullscreen particle count' })),
                h('label', { className: 'block text-xs font-bold' }, 'Container edge ', h('output', { className: 'text-cyan-300' }, boxSize + ' u'), h('input', { type: 'range', min: 7, max: 18, step: 1, value: boxSize, onChange: function (e) { var value = Number(e.target.value); setBoxSize(value); setRunning(false); setResetKey(function (k) { return k + 1; }); persist({ boxSize: value }); }, className: 'mt-1 block min-h-11 w-full accent-cyan-400', 'aria-label': 'Fullscreen container edge length' })),
                h('label', { className: 'block text-xs font-bold' }, 'Collision diameter ', h('output', { className: 'text-cyan-300' }, particleDiameter.toFixed(2) + ' u'), h('input', { type: 'range', min: 0.36, max: 0.9, step: 0.06, value: particleDiameter, onChange: function (e) { var value = Number(e.target.value); setParticleDiameter(value); setRunning(false); setHistory([]); setResetKey(function (k) { return k + 1; }); persist({ particleDiameter: value }); }, className: 'mt-1 block min-h-11 w-full accent-cyan-400', 'aria-label': 'Fullscreen particle collision diameter' })),
                h('label', { className: 'block text-xs font-bold' }, 'Attraction ', h('output', { className: 'text-cyan-300' }, attraction.toFixed(2)), h('input', { type: 'range', min: 0, max: 1.5, step: 0.05, value: attraction, onChange: function (e) { var value = Number(e.target.value); setAttraction(value); persist({ attraction: value }); }, className: 'mt-1 block min-h-11 w-full accent-cyan-400', 'aria-label': 'Fullscreen interparticle attraction strength' })),
                h('label', { className: 'block text-xs font-bold' }, 'Gravity ', h('output', { className: 'text-amber-300' }, gravity > 0.01 ? gravity.toFixed(1) + ' g*' : 'Off'), h('input', { type: 'range', min: 0, max: 2, step: 0.1, value: gravity, onChange: function (e) { var value = Number(e.target.value); setGravity(value); persist({ gravity: value }); }, className: 'mt-1 block min-h-11 w-full accent-amber-400', 'aria-label': 'Fullscreen downward gravity field strength' })),
