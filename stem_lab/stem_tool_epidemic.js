  ...
  window.StemLab.registerTool('epidemicSim', {
    ...
    render: function(ctx) {
      ...
      function renderSVGChart(data, compartments, width, height) {
        ...
        gridChildren.push(h('text', { key: 'ytitle', x: 11, y: padT + plotH / 2, textAnchor: 'middle', fill: '#64748b', fontSize: 9, fontWeight: 'bold', transform: 'rotate(-90 11 ' + (padT + plotH / 2) + ')' }, t('stem.epidemic.of_population', '% of population')));
        ...
        gridChildren.push(h('text', { key: 'capt', x: padL + 2, y: capY - 3, textAnchor: 'start', fill: '#ef4444', fontSize: 8, fontWeight: 'bold' }, t('stem.epidemic.healthcare_capacity_20', '🏥 Healthcare capacity (~20%)')));
      ...
      return h('div', { className: 'space-y-4' },
        ...
        h('div', { className: glassCard },
          h('div', { className: 'flex items-center justify-between flex-wrap gap-2' },
            h('div', { className: 'flex items-center gap-2' },
              ...
              h('div', null,
                h('h3', { className: 'text-base font-bold text-slate-800' }, t('stem.epidemic.epidemic_modeling_lab', 'Epidemic Modeling Lab')),
        ...
        h('div', { className: 'flex flex-wrap gap-1.5', role: 'tablist', },
          SUBTOOLS.map(function(st) {
            ...
            return h('button', { 'aria-label': t('stem.epidemic.select_intervention_strategy', 'Select intervention strategy'),
              key: st.id,
        ...
        (function() {
          var TAB_META = {
            sir:           { accent: '#0ea5e9', soft: 'rgba(14,165,233,0.10)', icon: '\uD83D\uDCC8', title: t('stem.epidemic.sir_the_foundational_epidemic_model', 'SIR \u2014 the foundational epidemic model'),  hint: t('stem.epidemic.three_compartments_susceptible_infecte', 'Three compartments: Susceptible \u2192 Infected \u2192 Recovered. The simplest model that captures real epidemic dynamics. Peaks when S = 1/R\u2080.') },
            seir:          { accent: '#a855f7', soft: 'rgba(168,85,247,0.10)', icon: '\uD83E\uDDA0', title: t('stem.epidemic.seir_add_the_latent_exposed_phase', 'SEIR \u2014 add the latent (exposed) phase'), hint: t('stem.epidemic.diseases_with_incubation_covid_measles', 'Diseases with incubation (COVID, measles, mumps) need an Exposed compartment. Latent period delays the epidemic peak and changes intervention timing.') },
            r0explorer:    { accent: '#dc2626', soft: 'rgba(220,38,38,0.10)',  icon: '\uD83C\uDF21\uFE0F', title: t('stem.epidemic.r_basic_reproduction_number', 'R\u2080 \u2014 basic reproduction number'), hint: t('stem.epidemic.average_secondary_infections_per_prima', 'Average secondary infections per primary case in a fully susceptible population. R\u2080 > 1 = outbreak; R\u2080 < 1 = dies out. Measles ~15, flu ~1.3, COVID ~2-3.') },
            vaccination:   { accent: '#16a34a', soft: 'rgba(22,163,74,0.10)',  icon: '\uD83D\uDC89', title: t('stem.epidemic.vaccination_herd_immunity', 'Vaccination + herd immunity'),          hint: t('stem.epidemic.herd_immunity_threshold_1_1_r_measles_', 'Herd-immunity threshold = 1 - 1/R\u2080. Measles needs ~94% coverage; flu only ~25%. The math is what makes vaccination a public-health superlever.') },
            interventions: { accent: '#f59e0b', soft: 'rgba(245,158,11,0.10)', icon: '\uD83D\uDE37', title: t('stem.epidemic.non_pharmaceutical_interventions', 'Non-pharmaceutical interventions'),     hint: t('stem.epidemic.masks_distancing_school_closures_conta', 'Masks, distancing, school closures, contact tracing \u2014 each lowers R effective. Stack them: 50% reduction \u00d7 50% \u2192 75% total transmission cut.') },
            outbreak:    { accent: '#15803d', soft: 'rgba(21,128,61,0.10)',  icon: '\uD83C\uDFE5', title: t('stem.epidemic.outbreak_response_26_week_pho_campaign', 'Outbreak Response \u2014 26-week PHO campaign'), hint: 'You are a county Public Health Officer. Four demographic groups. 26 weeks of decisions. Feedback rules tie infections, hospital strain, public trust, and vaccine uptake together. Real pandemic response is about timing and tradeoffs, not formulas.' },
            outbreakmap:   { accent: '#ef4444', soft: 'rgba(239,68,68,0.10)',  icon: '\uD83D\uDDFA\uFE0F', title: t('stem.epidemic.outbreak_map_spatial_spread', 'Outbreak map \u2014 spatial spread'), hint: t('stem.epidemic.disease_moves_through_networks_househo', 'Disease moves through networks (households, schools, transit, workplaces). Travel networks dominate at country scale; close-contact at household scale.') },
            contacttrace:  { accent: '#06b6d4', soft: 'rgba(6,182,212,0.10)',  icon: '\uD83D\uDD17', title: t('stem.epidemic.contact_tracing', 'Contact tracing'),                     hint: t('stem.epidemic.identifies_and_isolates_contacts_of_co', 'Identifies and isolates contacts of confirmed cases before they spread further. Most effective early when case count is low; breaks down when contacts > tracers.') },
            history:       { accent: '#8b5cf6', soft: 'rgba(139,92,246,0.10)', icon: '\uD83D\uDCDC', title: t('stem.epidemic.pandemic_history', 'Pandemic history'),                    hint: t('stem.epidemic.1918_flu_2003_sars_2009_h1n1_2014_ebol', '1918 flu, 2003 SARS, 2009 H1N1, 2014 Ebola, 2019 COVID. Each shaped modern public health \u2014 and surfaced new gaps to fix before the next.') },
            scenarios:     { accent: '#ec4899', soft: 'rgba(236,72,153,0.10)', icon: '\uD83C\uDFAD', title: t('stem.epidemic.scenario_walk_through', 'Scenario walk-through'),               hint: t('stem.epidemic.step_through_historical_and_hypothetic', 'Step through historical and hypothetical outbreaks. Practice intervention sequencing under uncertainty \u2014 the actual job of public health.') },
            challenge:     { accent: '#fbbf24', soft: 'rgba(251,191,36,0.10)', icon: '\uD83C\uDFAF', title: t('stem.epidemic.daily_challenge', 'Daily challenge'),                     hint: t('stem.epidemic.a_new_outbreak_puzzle_every_session_pi', 'A new outbreak puzzle every session: pick interventions, optimize timing, minimize deaths. Streak counter tracks daily wins.') },
            battle:        { accent: '#0d9488', soft: 'rgba(13,148,136,0.10)', icon: '\u2694\uFE0F',  title: t('stem.epidemic.battle_mode_head_to_head', 'Battle mode \u2014 head-to-head'),     hint: t('stem.epidemic.two_players_race_to_contain_identical_', 'Two players race to contain identical outbreaks with limited resources. Tests intervention literacy under speed pressure.') },
            learn:         { accent: '#64748b', soft: 'rgba(100,116,139,0.10)', icon: '\uD83D\uDCDA', title: t('stem.epidemic.reference_glossary', 'Reference + glossary'),               hint: t('stem.epidemic.r_r_effective_attack_rate_case_fatalit', 'R\u2080, R-effective, attack rate, case fatality rate, generation interval \u2014 the glossary you keep coming back to as you read epi papers.') },
            inquiry:       { accent: '#0891b2', soft: 'rgba(8,145,178,0.10)', icon: '\uD83D\uDD2C', title: t('stem.epidemic.inquiry_when_does_an_outbreak_become_o', 'Inquiry \u2014 when does an outbreak become observable?'), hint: t('stem.epidemic.three_sliders_contact_frequency_interv', 'Three sliders: contact frequency, intervention adoption, pathogen transmissibility. Discrete regime: contained / emerging / pandemic. No score, no reveal \u2014 just sweep and observe.') }
          };
        ...
        h('div', { className: glassCard },
          h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase tracking-wide mb-2' }, t('stem.epidemic.disease_presets', 'Disease Presets')),
          h('div', { className: 'flex flex-wrap gap-1.5' },
            PRESETS.map(function(p, idx) {
              var active = selectedPreset === idx;
              return h('button', { 'aria-label': t('stem.epidemic.apply_preset', 'Apply Preset'),
                key: p.name,
        ...
        tab === 'sir' && h('div', { className: 'space-y-4' },
          h('div', {
            ...
            t('stem.epidemic.find_vaccination_and_behavior_settings', 'find vaccination and behavior settings that keep the peak Infected curve below the hospital threshold, or that drive R-effective below 1 entirely. Pick a Disease Preset to load real R0 values; raise vaccination until the Herd Threshold readout flips to Achieved. The Effective R0 stat below the chart tells you whether the epidemic is still spreading or burning out.')
          ),
          // Sliders
          h('div', { className: glassCard + ' space-y-3' },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase tracking-wide' }, t('stem.epidemic.parameters', 'Parameters')),
            ...
            h('button', { 'aria-label': t('stem.epidemic.run_simulation', 'Run Simulation'), onClick: runSim, className: 'w-full py-2 text-sm font-bold bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all shadow-md' }, t('stem.epidemic.run_simulation_2', '\u25B6 Run Simulation'))
          ),
          // Chart
          h('div', { className: glassCard },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase tracking-wide mb-2' }, t('stem.epidemic.sir_curves', 'SIR Curves')),
            renderSVGChart(sirData, ['S', 'I', 'R'])
          ),
          // Stats
          h('div', { className: 'grid grid-cols-2 sm:grid-cols-4 gap-2' },
            [
              { label: t('stem.epidemic.peak_infected', 'Peak Infected'), value: peakI.toFixed(1) + '%', sub: 'Day ' + peakDay, color: '#ef4444' },
              { label: t('stem.epidemic.total_infected', 'Total Infected'), value: totalInf.toFixed(1) + '%', sub: fmtNum(Math.round(totalInf / 100 * popSize)) + ' people', color: '#f59e0b' },
              { label: t('stem.epidemic.effective_r', 'Effective R\u2080'), value: effR0.toFixed(2), sub: effR0 < 1 ? 'Declining' : 'Spreading', color: r0Color(effR0) },
              { label: t('stem.epidemic.herd_threshold', 'Herd Threshold'), value: herdThresh.toFixed(0) + '%', sub: vaccRate >= herdThresh && herdThresh > 0 ? '\u2705 Achieved' : 'Not yet', color: '#6366f1' }
            ].map(function(s) {
          ...
          h('div', { className: glassCard },
            h('div', { className: 'flex items-center justify-between mb-2' },
              h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase tracking-wide' }, t('stem.epidemic.particle_simulation', 'Particle Simulation')),
            ...
            h('canvas', { 'aria-label': t('stem.epidemic.epidemic_visualization', 'Epidemic visualization'), 
              ref: particleRef,
          ...
          h('div', { className: glassCard },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase tracking-wide mb-2' }, t('stem.epidemic.sir_equations', 'SIR Equations')),
            h('div', { className: 'font-mono text-xs text-slate-700 space-y-1 bg-slate-50 rounded-lg p-3' },
              h('p', null, t('stem.epidemic.ds_dt_s_i', 'dS/dt = -\u03B2 \u00D7 S \u00D7 I')),
              h('p', null, t('stem.epidemic.di_dt_s_i_i', 'dI/dt = \u03B2 \u00D7 S \u00D7 I - \u03B3 \u00D7 I')),
              h('p', null, t('stem.epidemic.dr_dt_i', 'dR/dt = \u03B3 \u00D7 I')),
        ...
        tab === 'seir' && h('div', { className: 'space-y-4' },
          h('div', { className: glassCard + ' space-y-3' },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase tracking-wide' }, t('stem.epidemic.seir_parameters', 'SEIR Parameters')),
            slider('R\u2080', r0, 0.5, 12, 0.1, 'r0', function(v) { return v.toFixed(1); }),
            slider('Vaccination Rate (%)', vaccRate, 0, 95, 1, 'vaccRate', function(v) { return v + '%'; }),
            slider('Infectious Period (days)', infectPeriod, 2, 30, 1, 'infectPeriod'),
            slider('Latent Period (days)', latentPeriod, 1, 21, 1, 'latentPeriod'),
            slider('Population', popSize, 1000, 10000000, 1000, 'popSize', fmtNum),
            h('button', { 'aria-label': t('stem.epidemic.run_seir_simulation', 'Run SEIR Simulation'), onClick: runSim, className: 'w-full py-2 text-sm font-bold bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-all shadow-md' }, t('stem.epidemic.run_seir_simulation_2', '\u25B6 Run SEIR Simulation'))
          ),
          h('div', { className: glassCard },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase tracking-wide mb-2' }, t('stem.epidemic.seir_curves', 'SEIR Curves')),
            renderSVGChart(seirData, ['S', 'E', 'I', 'R'])
          ),
          h('div', { className: 'grid grid-cols-2 sm:grid-cols-4 gap-2' },
            [
              { label: t('stem.epidemic.peak_infected_2', 'Peak Infected'), value: peakI.toFixed(1) + '%', sub: 'Day ' + peakDay, color: '#ef4444' },
              { label: t('stem.epidemic.total_infected_2', 'Total Infected'), value: totalInf.toFixed(1) + '%', sub: fmtNum(Math.round(totalInf / 100 * popSize)), color: '#f59e0b' },
              { label: t('stem.epidemic.latent_period', 'Latent Period'), value: latentPeriod + 'd', sub: '\u03C3 = ' + (1/latentPeriod).toFixed(3), color: '#f59e0b' },
              { label: t('stem.epidemic.herd_threshold_2', 'Herd Threshold'), value: herdThresh.toFixed(0) + '%', sub: vaccRate >= herdThresh && herdThresh > 0 ? '\u2705 Achieved' : 'Not yet', color: '#6366f1' }
            ].map(function(s) {
              return h('div', { key: s.label, className: glassCard + ' text-center' },
                h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase' }, s.label),
                h('p', { className: 'text-lg font-bold', style: { color: s.color } }, s.value),
          ...
          (gradeBand === '6-8' || gradeBand === '9-12') &&
          h('div', { className: glassCard },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase tracking-wide mb-2' }, t('stem.epidemic.seir_equations', 'SEIR Equations')),
            h('div', { className: 'font-mono text-xs text-slate-700 space-y-1 bg-slate-50 rounded-lg p-3' },
              h('p', null, t('stem.epidemic.ds_dt_s_i_2', 'dS/dt = -\u03B2 \u00D7 S \u00D7 I')),
              h('p', null, t('stem.epidemic.de_dt_s_i_e', 'dE/dt = \u03B2 \u00D7 S \u00D7 I - \u03C3 \u00D7 E')),
              h('p', null, t('stem.epidemic.di_dt_e_i', 'dI/dt = \u03C3 \u00D7 E - \u03B3 \u00D7 I')),
              h('p', null, t('stem.epidemic.dr_dt_i_2', 'dR/dt = \u03B3 \u00D7 I')),
              h('p', { className: 'pt-1 border-t border-slate-200 text-[11px] text-slate-600' },
        ...
        tab === 'r0explorer' && h('div', { className: 'space-y-4' },
          h('div', { className: glassCard },
            h('h4', { className: 'text-sm font-bold text-slate-700 mb-2' }, t('stem.epidemic.r_explorer', '\uD83C\uDF21\uFE0F R\u2080 Explorer')),
          ...
          h('div', { className: glassCard + ' space-y-3' },
            ...
            h('div', { className: 'flex gap-2' },
              h('button', { onClick: function() { runSim(); addR0Comparison(); }, className: 'flex-1 py-2 text-sm font-bold bg-teal-700 text-white rounded-xl hover:bg-teal-700 transition-all' }, t('stem.epidemic.add_to_comparison', '+ Add to Comparison')),
              h('button', { 'aria-label': t('stem.epidemic.clear', 'Clear'), onClick: function() { upd('r0Compared', []); }, className: 'px-3 py-2 text-sm font-bold bg-slate-100 text-slate-600 rounded-xl' }, t('stem.epidemic.clear_2', 'Clear'))
            )
          ),
          // R0 Visual Scale
          h('div', { className: glassCard },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase mb-2' }, t('stem.epidemic.r_danger_scale', 'R\u2080 Danger Scale')),
            ...
            h('div', { className: 'flex justify-between text-[11px] text-slate-600 mt-1' },
              h('span', null, t('stem.epidemic.dies_out_1', 'Dies out (<1)')),
              h('span', null, t('stem.epidemic.low_1_2', 'Low (1-2)')),
              h('span', null, t('stem.epidemic.moderate_2_4', 'Moderate (2-4)')),
              h('span', null, t('stem.epidemic.high_4_8', 'High (4-8)')),
              h('span', null, t('stem.epidemic.extreme_8', 'Extreme (>8)'))
            )
          ),
          // Comparison table
          r0Compared.length > 0 && h('div', { className: glassCard },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase mb-2' }, t('stem.epidemic.comparison_table', 'Comparison Table')),
            h('div', { className: 'overflow-x-auto' },
              h('table', { className: 'w-full text-[11px]' },
                h('caption', { className: 'sr-only' }, t('stem.epidemic.extreme_8_2', 'Extreme (>8)')), h('thead', null,
        ...
        tab === 'vaccination' && h('div', { className: 'space-y-4' },
          h('div', { className: glassCard },
            h('h4', { className: 'text-sm font-bold text-slate-700 mb-2' }, t('stem.epidemic.vaccination_strategy', '\uD83D\uDC89 Vaccination Strategy')),
          ...
          h('div', { className: glassCard + ' space-y-3' },
            ...
            slider('Vaccination Rate', vaccRate, 0, 95, 1, 'vaccRate', function(v) { return v + '%'; }),
            slider('Infectious Period', infectPeriod, 2, 30, 1, 'infectPeriod'),
            h('button', { 'aria-label': t('stem.epidemic.simulate_vaccination', 'Simulate Vaccination'), onClick: runSim, className: 'w-full py-2 text-sm font-bold bg-teal-700 text-white rounded-xl hover:bg-teal-700 transition-all shadow-md' }, t('stem.epidemic.simulate_vaccination_2', '\uD83D\uDC89 Simulate Vaccination'))
          ),
          // Herd immunity visual
          h('div', { className: glassCard },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase mb-2' }, t('stem.epidemic.herd_immunity_status', 'Herd Immunity Status')),
            ...
            h('div', { className: 'flex justify-between text-[11px] text-slate-600 mt-1' },
              h('span', null, t('stem.epidemic.0_vaccinated', '0% Vaccinated')),
          ...
          h('div', { className: 'grid grid-cols-2 gap-2' },
            h('div', { className: glassCard + ' text-center' },
              h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase' }, t('stem.epidemic.without_vaccination', 'Without Vaccination')),
              ...
              h('p', { className: 'text-[11px] text-slate-600' }, t('stem.epidemic.peak_infected_3', 'Peak Infected'))
            ),
            h('div', { className: glassCard + ' text-center' },
              ...
              h('p', { className: 'text-[11px] text-slate-600' }, t('stem.epidemic.peak_infected_4', 'Peak Infected'))
            )
          )
        ),
        ...
        tab === 'interventions' && h('div', { className: 'space-y-4' },
          h('div', { className: glassCard },
            h('h4', { className: 'text-sm font-bold text-slate-700 mb-2' }, t('stem.epidemic.non_pharmaceutical_interventions_2', '\uD83D\uDE37 Non-Pharmaceutical Interventions')),
            h('p', { className: 'text-xs text-slate-600' }, gradeText(gradeBand,
          ...
          h('div', { className: glassCard + ' space-y-2' },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase tracking-wide mb-2' }, t('stem.epidemic.select_interventions', 'Select Interventions')),
            h('div', { className: 'grid grid-cols-2 sm:grid-cols-4 gap-2' },
              NPI_INTERVENTIONS.map(function(npi) {
                var active = activeNPIs.indexOf(npi.id) >= 0;
                return h('button', { 'aria-label': t('stem.epidemic.subtract', 'Subtract'),
                  key: npi.id,
          ...
          h('div', { className: glassCard + ' space-y-3' },
            ...
            slider('Vaccination (%)', vaccRate, 0, 95, 1, 'vaccRate', function(v) { return v + '%'; }),
            slider('Infectious Period', infectPeriod, 2, 30, 1, 'infectPeriod'),
            h('button', { 'aria-label': t('stem.epidemic.simulate_with_npis', 'Simulate with NPIs'), onClick: runNPISim, className: 'w-full py-2 text-sm font-bold bg-teal-700 text-white rounded-xl hover:bg-teal-700 transition-all shadow-md' }, t('stem.epidemic.simulate_with_npis_2', '\uD83D\uDE37 Simulate with NPIs'))
          ),
          // Reduction summary
          npiResult && h('div', { className: glassCard },
            h('div', { className: 'grid grid-cols-3 gap-3 text-center' },
              h('div', null,
                h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase' }, t('stem.epidemic.reduction', '\u03B2 Reduction')),
              ...
              h('div', null,
                h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase' }, t('stem.epidemic.status', 'Status')),
          ...
          npiResult && npiBaseline && h('div', { className: 'grid grid-cols-1 sm:grid-cols-2 gap-3' },
            h('div', { className: glassCard },
              h('p', { className: 'text-[11px] font-bold text-red-500 uppercase mb-1' }, t('stem.epidemic.without_interventions', 'Without Interventions')),
          ...
          npiResult && h('div', { className: glassCard },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase mb-2' }, t('stem.epidemic.hospital_capacity', '\uD83C\uDFE5 Hospital Capacity')),
        ...
        tab === 'outbreak' && (function() {
          ...
          function outbreakCoachingTip() {
            ...
            return {
              ...
              text: t('stem.epidemic.initial_conditions_look_stable_use_wee', 'Initial conditions look stable. Use Week 1 to read the system: which group infections climb fastest under your specific contact rates and seed events. Active intervention before you have signal can burn trust unnecessarily.')
            };
          }
          ...
          function renderOutbreakDeepDive(id) {
            ...
            return h('div', {
              ...
              h('div', { style: { display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 10 } },
                ...
                h('div', { style: { flex: 1 } },
                  h('div', { style: { fontSize: 11, color: def.color, fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase' } }, t('stem.epidemic.demographic_deep_dive', 'Demographic deep-dive')),
                ...
                h('button', { onClick: closeOutbreakDeepDive,
                  style: { background: 'rgba(15,23,42,0.6)', border: '1px solid var(--allo-stem-border, #334155)', color: 'var(--allo-stem-text, #cbd5e1)', cursor: 'pointer', borderRadius: 8, padding: '6px 12px', fontWeight: 700, fontSize: 13 } }, t('stem.epidemic.close', '✕ Close'))
              ),
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 10 } },
                h('div', { style: { background: 'rgba(15,23,42,0.7)', borderRadius: 10, padding: 12 } },
                  h('div', { style: { fontSize: 11, fontWeight: 700, color: '#86efac', letterSpacing: 0.5, textTransform: 'uppercase', marginBottom: 6 } }, t('stem.epidemic.epidemiology', '📊 Epidemiology')),
                  h('p', { style: { margin: 0, color: 'var(--allo-stem-text, #e2e8f0)', fontSize: 13, lineHeight: 1.55 } }, dd.knowledge)
                ),
                h('div', { style: { background: 'rgba(15,23,42,0.7)', borderRadius: 10, padding: 12 } },
                  h('div', { style: { fontSize: 11, fontWeight: 700, color: '#fbbf24', letterSpacing: 0.5, textTransform: 'uppercase', marginBottom: 6 } }, t('stem.epidemic.case_work', '📰 Case work')),
                  h('p', { style: { margin: 0, color: 'var(--allo-stem-text, #e2e8f0)', fontSize: 13, lineHeight: 1.55 } }, dd.casework)
                ),
                h('div', { style: { background: 'rgba(15,23,42,0.7)', borderRadius: 10, padding: 12 } },
                  h('div', { style: { fontSize: 11, fontWeight: 700, color: '#38bdf8', letterSpacing: 0.5, textTransform: 'uppercase', marginBottom: 6 } }, t('stem.epidemic.maine_context', '🌍 Maine context')),
          ...
          function renderOutbreakTrendChart(weekLog) {
            ...
            return h('div', { style: { background: 'var(--allo-stem-canvas, #0f172a)', borderRadius: 12, padding: 12, marginBottom: 14, border: '1px solid var(--allo-stem-border, #1e293b)' } },
              h('div', { style: { fontSize: 12, fontWeight: 700, color: 'var(--allo-stem-text, #e2e8f0)', marginBottom: 8 } }, t('stem.epidemic.infection_by_demographic_across_all_we', '📈 Infection % by demographic across all weeks')),
              h('svg', { viewBox: '0 0 ' + w + ' ' + hgt, style: { width: '100%', height: 'auto', display: 'block' }, 'aria-label': t('stem.epidemic.week_by_week_infection_trend_chart_by_', 'Week-by-week infection trend chart by demographic group') },
          ...
          function renderOutbreakAIPanel() {
            if (outbreak.aiReadLoading) {
              return h('div', { role: 'status', 'aria-live': 'polite',
                ...
                t('stem.epidemic.ai_public_health_educator_is_reading_y', '⏳ AI public health educator is reading your county data...'));
            }
            if (!outbreak.aiReadResponse) return null;
            return h('div', { role: 'region', 'aria-label': t('stem.epidemic.ai_public_health_reading', 'AI Public Health Reading'),
              ...
              h('div', { style: { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 } },
                ...
                h('strong', { style: { color: '#38bdf8', fontSize: 14 } }, t('stem.epidemic.ai_public_health_reading_2', 'AI Public Health Reading')),
                h('div', { style: { marginLeft: 'auto', display: 'flex', gap: 6 } },
                  h('button', { onClick: readOutbreak,
                    style: { background: 'transparent', border: '1px solid #38bdf8', color: '#38bdf8', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: 11, fontWeight: 700 } }, t('stem.epidemic.re_read', '↻ Re-read')),
              ...
              h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #64748b)', lineHeight: 1.5, paddingTop: 8, borderTop: '1px solid rgba(56,189,248,0.2)', fontStyle: 'italic' } },
                t('stem.epidemic.ai_public_health_educator', 'AI public health educator. '),
                h('strong', null, t('stem.epidemic.it_is_not_a_real_public_health_officer', 'It is not a real Public Health Officer, not a Wabanaki person, and does not speak for any Wabanaki nation, public health agency, or named individual.')),
                t('stem.epidemic.for_authoritative_public_health_voices', ' For authoritative public health voices, consult Maine CDC, Wabanaki Public Health and Wellness, the US CDC, and your local public health department directly.')
              )
            );
          }
          ...
          function endOutbreakWeek() {
            ...
            if (skipRng() < (diff.eventSkip || 0)) {
              ev = { id: 'quietWeek', name: t('stem.epidemic.a_quiet_week', 'A Quiet Week'), icon: '🌤️', desc: t('stem.epidemic.no_major_event_the_system_stayed_withi', 'No major event. The system stayed within its normal noise.'), apply: function() {} };
            } else {
          ...
          function advanceFromOutbreakReview() {
            if (outbreak.week >= outbreak.maxWeeks) {
              ...
              var outcome;
              if (totalDeaths < 15 && avgTrust > 65 && overloads === 0) outcome = { tier: 'mastery', label: t('stem.epidemic.pandemic_response_mastery', 'Pandemic Response Mastery'), color: '#16a34a', icon: '🏆', desc: t('stem.epidemic.low_deaths_high_trust_zero_hospital_ov', 'Low deaths, high trust, zero hospital overload. This is what a well-run public health response looks like, and it required real tradeoffs along the way.') };
              else if (totalDeaths < 35 && avgTrust > 50) outcome = { tier: 'solid', label: t('stem.epidemic.solid_public_health_response', 'Solid Public Health Response'), color: '#22c55e', icon: '🩺', desc: t('stem.epidemic.you_kept_the_curve_flattened_most_of_t', 'You kept the curve flattened most of the time and maintained public engagement. Real-world responses look like this on the good days.') };
              else if (totalDeaths < 60) outcome = { tier: 'mixed', label: t('stem.epidemic.mixed_outcomes', 'Mixed Outcomes'), color: '#f59e0b', icon: '⚖️', desc: t('stem.epidemic.significant_illness_some_hospital_stra', 'Significant illness, some hospital strain, eroded trust. The system held together but at real cost.') };
              else outcome = { tier: 'catastrophic', label: t('stem.epidemic.system_strain', 'System Strain'), color: '#ef4444', icon: '⚠️', desc: t('stem.epidemic.high_deaths_or_sustained_hospital_over', 'High deaths or sustained hospital overload. This is the trajectory communities and health systems work to avoid.') };
            ...
            return h('div', { className: 'space-y-4' },
              ...
              h('div', { style: { padding: 18, borderRadius: 14, background: 'linear-gradient(135deg, rgba(21,128,61,0.18) 0%, rgba(56,189,248,0.06) 100%)', border: '1px solid ' + T_GREEN + '66', borderLeft: '4px solid ' + T_GREEN } },
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 } },
                  ...
                  h('div', null,
                    h('h3', { style: { margin: 0, color: T_GREEN_HI, fontSize: 22 } }, t('stem.epidemic.outbreak_response_26_week_pho_campaign_2', 'Outbreak Response: 26-week PHO campaign')),
                ...
                h('p', { style: { margin: '8px 0 0', color: 'var(--allo-stem-text, #e2e8f0)', fontSize: 14, lineHeight: 1.6 } },
                  t('stem.epidemic.a_novel_respiratory_pathogen_is_circul', 'A novel respiratory pathogen is circulating in your county. You have 26 weeks of decisions. Four demographic groups respond differently. Feedback rules tie '),
                  h('strong', null, t('stem.epidemic.infection_hospital_strain_public_trust', 'infection, hospital strain, public trust, and vaccine uptake')),
                  t('stem.epidemic.together_this_is_pandemic_response_as_', ' together. This is pandemic response as decision-making under uncertainty, not as a formula.')
                )
              ),
              ...
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 10 } },
                DEMOGRAPHIC_GROUPS.map(function(g) {
                  return h('div', { key: g.id, style: { background: 'var(--allo-stem-canvas, #0f172a)', borderLeft: '3px solid ' + g.color, borderRadius: 10, padding: 12 } },
                    ...
                    }, t('stem.epidemic.demographic_deep_dive_2', '📚 Demographic deep-dive →')) : null
                  );
                })
              ),
              ...
              h('div', { style: { background: 'var(--allo-stem-canvas, #0f172a)', borderRadius: 10, padding: 12, border: '1px solid var(--allo-stem-border, #1e293b)' } },
                h('div', { style: { fontSize: 12, color: 'var(--allo-stem-text-soft, #94a3b8)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8, fontWeight: 700 } }, t('stem.epidemic.difficulty', 'Difficulty')),
              ...
              h('button', { onClick: function() { startOutbreak(); },
                style: { width: '100%', padding: '14px 20px', borderRadius: 12, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, ' + T_GREEN + ' 0%, #166534 100%)', color: '#fff', fontWeight: 800, fontSize: 16, boxShadow: '0 6px 14px rgba(21,128,61,0.35)' } }, t('stem.epidemic.begin_26_week_outbreak_response', '🏥 Begin 26-week Outbreak Response'))
            );
          }
            ...
            return h('div', { className: 'space-y-3' },
              ...
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 } },
                h('div', { style: { background: 'var(--allo-stem-canvas, #0f172a)', padding: 12, borderRadius: 10 } },
                  h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)' } }, t('stem.epidemic.cumulative_deaths', 'Cumulative deaths')),
                  h('div', { style: { fontSize: 24, fontWeight: 800, color: '#ef4444' } }, Math.round(outbreak.totalDeaths))
                ),
                h('div', { style: { background: 'var(--allo-stem-canvas, #0f172a)', padding: 12, borderRadius: 10 } },
                  h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)' } }, t('stem.epidemic.average_trust', 'Average trust')),
                  h('div', { style: { fontSize: 24, fontWeight: 800, color: '#86efac' } }, outbreak.avgTrust + '/100')
                ),
                h('div', { style: { background: 'var(--allo-stem-canvas, #0f172a)', padding: 12, borderRadius: 10 } },
                  h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)' } }, t('stem.epidemic.hospital_overload_weeks', 'Hospital overload weeks')),
              ...
              h('div', { style: { padding: 12, borderRadius: 12, background: 'linear-gradient(135deg, rgba(15,23,42,1) 0%, rgba(127,29,29,0.18) 100%)', border: '1px solid rgba(248,113,113,0.4)' } },
                h('strong', { style: { color: '#fecaca', fontSize: 14, display: 'block', marginBottom: 8 } }, t('stem.epidemic.what_if_a_pho_had_done_nothing_for_26_', '↔ What if a PHO had done nothing for 26 weeks?')),
                h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 } },
                  h('div', { style: { background: 'var(--allo-stem-canvas, #0f172a)', padding: 10, borderRadius: 8, borderLeft: '3px solid ' + o2.color } },
                    h('div', { style: { fontSize: 12, fontWeight: 700, color: o2.color, marginBottom: 4 } }, t('stem.epidemic.your_campaign', 'Your campaign')),
                  ...
                  h('div', { style: { background: 'var(--allo-stem-canvas, #0f172a)', padding: 10, borderRadius: 8, borderLeft: '3px solid #ef4444' } },
                    h('div', { style: { fontSize: 12, fontWeight: 700, color: '#fca5a5', marginBottom: 4 } }, t('stem.epidemic.pure_neglect', 'Pure neglect')),
              ...
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap' } },
                h('button', { onClick: resetOutbreak, style: { padding: '10px 16px', borderRadius: 10, border: 'none', cursor: 'pointer', background: 'var(--allo-stem-panel, #1e293b)', color: 'var(--allo-stem-text, #cbd5e1)', fontWeight: 700 } }, t('stem.epidemic.new_campaign', '↻ New campaign')),
                h('button', { onClick: function() { startOutbreak({ seed: outbreak.seed, difficulty: outbreak.difficulty }); }, style: { padding: '10px 16px', borderRadius: 10, border: '1px solid #38bdf8', cursor: 'pointer', background: 'rgba(56,189,248,0.15)', color: '#bae6fd', fontWeight: 700 } }, t('stem.epidemic.replay_same_conditions', '🔁 Replay same conditions'))
              ),
              h('div', { style: { padding: 8, background: 'var(--allo-stem-canvas, #0f172a)', borderRadius: 8, fontSize: 11.5, color: 'var(--allo-stem-text-soft, #94a3b8)', fontFamily: 'ui-monospace, monospace' } },
                h('span', { style: { color: 'var(--allo-stem-text-soft, #64748b)' } }, t('stem.epidemic.campaign_seed', 'Campaign seed: ')),
            ...
            return h('div', { className: 'space-y-3' },
              ...
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 8 } },
                h('div', { style: { background: 'var(--allo-stem-canvas, #0f172a)', padding: 10, borderRadius: 8 } },
                  h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)' } }, t('stem.epidemic.hospital_load', 'Hospital load')),
                ...
                h('div', { style: { background: 'var(--allo-stem-canvas, #0f172a)', padding: 10, borderRadius: 8 } },
                  h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)' } }, t('stem.epidemic.overload_weeks_total', 'Overload weeks total')),
              ...
              (lastSnap2.feedbacks && lastSnap2.feedbacks.length > 0) ? h('div', { style: { padding: 10, borderRadius: 10, background: 'rgba(168,85,247,0.10)', borderLeft: '3px solid #a855f7', fontSize: 13, color: '#e9d5ff' } },
                h('strong', { style: { color: '#a855f7' } }, t('stem.epidemic.feedback_rules_this_week', '🔄 Feedback rules this week')),
              ...
              h('div', { style: { background: 'var(--allo-stem-canvas, #0f172a)', borderRadius: 10, padding: 10 } },
                h('div', { style: { fontWeight: 700, color: 'var(--allo-stem-text, #e2e8f0)', marginBottom: 6, fontSize: 13 } }, t('stem.epidemic.what_changed_this_week', 'What changed this week')),
          ...
          return h('div', { className: 'space-y-3' },
            ...
            coachingTip ? h('div', { role: 'note', style: { padding: '10px 14px', borderRadius: 12, background: 'linear-gradient(135deg, rgba(168,85,247,0.16) 0%, rgba(168,85,247,0.04) 100%)', borderTop: '1px solid rgba(168,85,247,0.6)', borderRight: '1px solid rgba(168,85,247,0.6)', borderBottom: '1px solid rgba(168,85,247,0.6)', borderLeft: '3px solid #a855f7', color: '#e9d5ff', fontSize: 13, lineHeight: 1.55, display: 'flex', alignItems: 'flex-start', gap: 10 } },
              ...
              h('div', { style: { flex: 1 } },
                h('strong', { style: { color: '#a855f7' } }, t('stem.epidemic.week_1_priority', 'Week 1 priority: ')),
                h('span', { style: { color: '#fde68a' } }, coachingTip.priority),
                h('div', { style: { marginTop: 4, color: '#e9d5ff' } }, coachingTip.text)
              ),
              h('button', { onClick: function() { setOutbreak({ firstTipDismissed: true }); }, 'aria-label': t('stem.epidemic.dismiss_tip', 'Dismiss tip'),
            ...
            h('div', { style: { padding: '10px 14px', borderRadius: 12, background: 'linear-gradient(135deg, rgba(21,128,61,0.18) 0%, rgba(15,23,42,0) 100%)', border: '1px solid ' + T_GREEN + '66', borderLeft: '4px solid ' + T_GREEN, display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' } },
              h('div', null,
                h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)' } }, t('stem.epidemic.week', 'Week')),
                h('div', { style: { fontSize: 20, fontWeight: 800, color: T_GREEN_HI } }, outbreak.week + ' / ' + outbreak.maxWeeks)
              ),
              h('div', null,
                h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)' } }, t('stem.epidemic.hours_left', 'Hours left')),
                h('div', { style: { fontSize: 20, fontWeight: 800, color: '#fbbf24' } }, outbreak.hoursLeft + ' / ' + outbreak.hoursPerWeek)
              ),
              h('div', null,
                h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)' } }, t('stem.epidemic.avg_trust', 'Avg trust')),
                h('div', { style: { fontSize: 20, fontWeight: 800, color: avgTrust > 60 ? '#86efac' : avgTrust > 40 ? '#fbbf24' : '#ef4444' } }, avgTrust)
              ),
              h('div', null,
                h('div', { style: { fontSize: 11, color: 'var(--allo-stem-text-soft, #94a3b8)' } }, t('stem.epidemic.total_infected_sum_across_groups', 'Total infected (sum across groups)')),
              ...
              h('div', { style: { marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' } },
                callGemini ? h('button', { onClick: readOutbreak, disabled: outbreak.aiReadLoading,
                  'aria-label': t('stem.epidemic.ask_ai_public_health_educator_to_read_', 'Ask AI public health educator to read your county data'),
                  title: t('stem.epidemic.ai_public_health_educator_reads_your_c', 'AI public health educator reads your current state'),
                ...
                h('button', { onClick: endOutbreakWeek, 'aria-label': t('stem.epidemic.end_this_week', 'End this week'),
                  style: { padding: '10px 16px', borderRadius: 10, border: 'none', cursor: 'pointer', background: '#dc2626', color: '#fff', fontWeight: 700, fontSize: 13 } }, t('stem.epidemic.end_week', 'End Week →'))
              )
            ),
            ...
            (outbreak.activeMods && outbreak.activeMods.length > 0) ? h('div', { style: { padding: 8, borderRadius: 10, background: 'rgba(56,189,248,0.10)', borderLeft: '3px solid #38bdf8', fontSize: 12, color: '#bae6fd' } },
              h('strong', null, t('stem.epidemic.active_interventions_or_mods', '🛠 Active interventions or mods: ')),
              ...
              outbreak.groups.map(function(g) {
                ...
                return h('div', { key: g.id, style: { background: 'var(--allo-stem-canvas, #0f172a)', borderRadius: 12, padding: 12, borderLeft: '3px solid ' + def.color } },
                  h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 } },
                    ...
                    def.deepDive ? h('button', { onClick: function() { openOutbreakDeepDive(g.id); }, 'aria-label': 'Deep-dive for ' + def.name, title: t('stem.epidemic.demographic_deep_dive_3', 'Demographic deep-dive'),
            ...
            h('div', { style: { background: 'var(--allo-stem-canvas, #0f172a)', borderRadius: 12, padding: 12, borderLeft: '3px solid #38bdf8' } },
              h('div', { style: { fontSize: 12, fontWeight: 700, color: '#bae6fd', marginBottom: 8 } }, t('stem.epidemic.county_wide_interventions', '🛠 County-wide interventions')),
            ...
            ) : h('div', { style: { fontSize: 12, color: 'var(--allo-stem-text-soft, #64748b)', fontStyle: 'italic' } }, t('stem.epidemic.no_interventions_yet_this_week_pick_fr', 'No interventions yet this week. Pick from the county-wide row or use Targeted vaccination on a demographic.'))
          );
        })(),
        ...
        tab === 'outbreakmap' && h('div', { className: 'space-y-4' },
          h('div', { className: glassCard },
            h('h4', { className: 'text-sm font-bold text-slate-700 mb-2' }, t('stem.epidemic.outbreak_map', '\uD83D\uDDFA\uFE0F Outbreak Map')),
            h('p', { className: 'text-xs text-slate-600' }, gradeText(gradeBand,
          ...
          h('div', { className: glassCard + ' space-y-3' },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase mb-1' }, t('stem.epidemic.scenario', 'Scenario')),
            h('div', { className: 'flex flex-wrap gap-1.5' },
            ...
            h('div', { className: 'flex gap-2' },
              h('button', { onClick: initMap, className: 'flex-1 py-2 text-sm font-bold bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all' }, t('stem.epidemic.generate_map', '\uD83D\uDDFA\uFE0F Generate Map')),
          ...
          mapGrid && h('div', { className: glassCard + ' space-y-3' },
            h('div', { className: 'flex items-center justify-between' },
              ...
              h('div', { className: 'flex gap-2' },
                h('button', { 'aria-label': t('stem.epidemic.add_zone', '+ Add Zone'),
                  onClick: function() {
                    var sc = MAP_SCENARIOS[mapScenario];
                ...
                }, t('stem.epidemic.add_zone_2', '+ Add Zone')),
                mapQuarantineZones.length > 0 && h('button', { 'aria-label': t('stem.epidemic.clear_3', 'Clear'),
                  onClick: function() { upd('mapQuarantineZones', []); },
                  className: 'px-2 py-1 text-[11px] font-bold bg-slate-100 text-slate-600 rounded-lg'
                }, t('stem.epidemic.clear_4', 'Clear'))
              )
            ),
            mapQuarantineZones.length > 0 && h('p', { className: 'text-[11px] text-amber-600 italic' }, t('stem.epidemic.quarantine_zones_reduce_transmission_b', 'Quarantine zones reduce transmission by 80% and speed recovery by 50% within the zone.')),
            // Hospital capacity bar
            h('div', null,
              h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase mb-1' }, t('stem.epidemic.hospital_capacity_2', '\uD83C\uDFE5 Hospital Capacity')),
              (function() {
                ...
                return h('div', null,
                  ...
                  exceeded && h('p', { className: 'text-[11px] font-bold text-red-600 mt-0.5' }, t('stem.epidemic.hospitals_overwhelmed_mortality_increa', '\u26A0\uFE0F HOSPITALS OVERWHELMED \u2014 mortality increases!'))
                );
              })()
            )
          ),
          ...
          mapHistory && mapHistory.length > 2 && h('div', { className: glassCard },
            h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase mb-2' }, t('stem.epidemic.outbreak_timeline', 'Outbreak Timeline')),
            (function() {
        ...
        tab === 'contacttrace' && h('div', { className: 'space-y-4' },
          h('div', { className: glassCard },
            h('h4', { className: 'text-sm font-bold text-slate-700 mb-2' }, t('stem.epidemic.contact_tracing_2', '\uD83D\uDD17 Contact Tracing')),
            h('p', { className: 'text-xs text-slate-600' }, gradeText(gradeBand,
          ...
          !ctNetwork ? h('div', { className: glassCard + ' text-center space-y-3' },
            ...
            h('p', { className: 'text-sm font-bold text-slate-700' }, t('stem.epidemic.trace_the_infection_chain', 'Trace the infection chain!')),
            h('p', { className: 'text-xs text-slate-600' }, t('stem.epidemic.click_on_people_connected_to_known_cas', 'Click on people connected to known cases to test if they\'re infected.')),
            h('button', { 'aria-label': t('stem.epidemic.start_tracing', 'Start Tracing'), onClick: startContactTrace, className: 'px-6 py-2 text-sm font-bold bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all' }, t('stem.epidemic.start_tracing_2', '\uD83D\uDD17 Start Tracing'))
            ...
            h('div', { className: glassCard + ' flex items-center justify-between' },
              ...
              ctComplete ? h('span', { className: 'text-xs font-bold text-emerald-800 bg-emerald-50 px-2 py-1 rounded-lg' }, t('stem.epidemic.all_found', '\u2705 All Found!')) :
            ...
            // Controls
            h('div', { className: 'flex gap-2' },
              h('button', { onClick: startContactTrace, className: 'px-4 py-2 text-sm font-bold bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all' }, t('stem.epidemic.new_network', '\u21BA New Network')),
            ...
            h('div', { className: 'flex gap-4 text-[11px] text-slate-600 px-2' },
              h('span', null, t('stem.epidemic.patient_zero', '\uD83D\uDD34 Patient Zero')),
              h('span', null, t('stem.epidemic.unknown_clickable_if_connected', '\uD83D\uDD35 Unknown (clickable if connected)')),
              h('span', null, t('stem.epidemic.tested_clear', '\uD83D\uDFE2 Tested Clear')),
              h('span', null, t('stem.epidemic.confirmed_infected', '\uD83D\uDD34 Confirmed Infected'))
            )
          )
        ),
        ...
        tab === 'history' && h('div', { className: 'space-y-4' },
          h('div', { className: glassCard },
            h('h4', { className: 'text-sm font-bold text-slate-700 mb-2' }, t('stem.epidemic.historical_pandemics', '\uD83D\uDCDC Historical Pandemics')),
            h('p', { className: 'text-xs text-slate-600' }, gradeText(gradeBand,
              ...
              h('div', { className: 'space-y-4 pl-10' },
                HISTORICAL_PANDEMICS.map(function(p) {
                  ...
                  return h('div', { key: p.name, className: 'relative' },
                    ...
                    h('button', { 'aria-label': t('stem.epidemic.view_pandemic', 'View Pandemic'),
                      onClick: function() {
                        viewPandemic(p.name);
                    ...
                    expanded && h('div', { className: 'mt-2 p-4 bg-white rounded-xl border border-slate-400 space-y-3' },
                      ...
                      h('div', { className: 'grid grid-cols-2 sm:grid-cols-4 gap-2' },
                        [
                          { label: t('stem.epidemic.deaths', 'Deaths'), value: p.deaths, color: '#ef4444' },
                          { label: t('stem.epidemic.world_pop', '% World Pop'), value: p.pctDeath, color: '#f59e0b' },
                          { label: t('stem.epidemic.r_estimate', 'R\u2080 Estimate'), value: p.r0Est, color: r0Color(parseFloat(p.r0Est) || 3) },
                          { label: t('stem.epidemic.transmission', 'Transmission'), value: p.transmission, color: '#6366f1' }
                        ].map(function(s) {
                      ...
                      // Key facts
                      h('div', null,
                        h('p', { className: 'text-[11px] font-bold text-slate-600 mb-1' }, t('stem.epidemic.key_facts', 'Key Facts:')),
                      ...
                      h('div', { className: 'flex gap-2 pt-2 border-t border-slate-100' },
                        callTTS && h('button', { 'aria-label': t('stem.epidemic.read_aloud', 'Read Aloud'), onClick: function() { callTTS(p.gradeSummary[gradeBand] || p.gradeSummary['3-5']); }, className: 'px-3 py-1.5 text-[11px] font-bold bg-blue-50 text-blue-600 rounded-lg' }, t('stem.epidemic.read_aloud_2', '\uD83D\uDD0A Read Aloud')),
                        h('button', { 'aria-label': t('stem.epidemic.simulate', 'Simulate'), onClick: function() { applyPreset(PRESETS.findIndex(function(pr) { return pr.name === 'COVID-19'; }) || 0); updMulti({ tab: 'sir' }); }, className: 'px-3 py-1.5 text-[11px] font-bold bg-violet-50 text-violet-600 rounded-lg' }, t('stem.epidemic.simulate_2', '\uD83D\uDD2C Simulate'))
                      )
                    )
                  );
                })
        ...
        tab === 'scenarios' && h('div', { className: 'space-y-4' },
          h('div', { className: glassCard },
            h('h4', { className: 'text-sm font-bold text-slate-700 mb-2' }, t('stem.epidemic.outbreak_scenarios', '\uD83C\uDFAD Outbreak Scenarios')),
            h('p', { className: 'text-xs text-slate-600' }, gradeText(gradeBand,
          ...
          !scenarioData && !d.scenarioLoading ? h('div', { className: glassCard + ' text-center space-y-3' },
            h('div', { className: 'text-5xl mb-2' }, '\uD83C\uDFAD'),
            h('p', { className: 'text-sm font-bold text-slate-700' }, t('stem.epidemic.ai_outbreak_scenario_generator', 'AI Outbreak Scenario Generator')),
            h('p', { className: 'text-xs text-slate-600' }, t('stem.epidemic.gemini_will_create_a_unique_fictional_', 'Gemini will create a unique fictional outbreak for you to respond to.')),
            callGemini ? h('button', { 'aria-label': t('stem.epidemic.generate_scenario', 'Generate Scenario'), onClick: generateScenario, className: 'px-6 py-2 text-sm font-bold bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-all' }, t('stem.epidemic.generate_scenario_2', '\uD83E\uDDE0 Generate Scenario')) :
            h('p', { className: 'text-xs text-slate-600 italic' }, t('stem.epidemic.ai_not_available_requires_gemini_integ', 'AI not available \u2014 requires Gemini integration.'))
          ) : d.scenarioLoading ? h('div', { className: glassCard + ' text-center py-6' },
            h('div', { className: 'text-3xl animate-pulse mb-2' }, '\uD83E\uDDE0'),
            h('p', { className: 'text-sm font-bold text-purple-600' }, t('stem.epidemic.generating_outbreak_scenario', 'Generating outbreak scenario...'))
            ...
            h('div', { className: glassCard + ' border-l-4 border-red-500' },
              ...
              h('div', { className: 'grid grid-cols-2 sm:grid-cols-4 gap-2 mt-3' },
                [
                  { label: t('stem.epidemic.origin', 'Origin'), value: scenarioData.origin || 'Unknown', color: '#6366f1' },
                  { label: 'R\u2080', value: (scenarioData.r0 || '?').toString(), color: r0Color(scenarioData.r0 || 3) },
                  { label: t('stem.epidemic.transmission_2', 'Transmission'), value: scenarioData.transmission || 'Unknown', color: '#f59e0b' },
                  { label: t('stem.epidemic.symptoms', 'Symptoms'), value: scenarioData.symptoms || 'Varies', color: '#ef4444' }
                ].map(function(s) {
                  return h('div', { key: s.label, className: 'bg-slate-50 rounded-lg p-2 text-center' },
                    h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase' }, s.label),
                    h('p', { className: 'text-[11px] font-bold', style: { color: s.color } }, s.value)
            ...
            !scenarioChoice && scenarioData.options && h('div', { className: glassCard },
              h('p', { className: 'text-[11px] font-bold text-slate-600 uppercase mb-3' }, t('stem.epidemic.choose_your_response_strategy', 'Choose Your Response Strategy')),
              h('div', { className: 'space-y-2' },
                scenarioData.options.map(function(opt, idx) {
                  var letters = ['A', 'B', 'C'];
                  var colors = ['#3b82f6', '#f59e0b', '#22c55e'];
                  return h('button', { 'aria-label': t('stem.epidemic.choose_scenario', 'Choose Scenario'),
                    key: idx,
            ...
            // Play again
            h('div', { className: 'flex gap-2' },
              h('button', { 'aria-label': t('stem.epidemic.new_scenario', 'New Scenario'), onClick: function() { updMulti({ scenarioData: null, scenarioChoice: null, scenarioResult: null }); generateScenario(); }, className: 'px-4 py-2 text-sm font-bold bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-all' }, t('stem.epidemic.new_scenario_2', '\uD83E\uDDE0 New Scenario')),
              scenarioData.r0 && h('button', { 'aria-label': t('stem.epidemic.simulate_r', 'Simulate R='), onClick: function() { updMulti({ tab: 'sir', r0: scenarioData.r0 }); }, className: 'px-4 py-2 text-sm font-bold bg-indigo-100 text-indigo-600 rounded-xl' }, '\uD83D\uDD2C Simulate R\u2080=' + scenarioData.r0)
            )
          )
        ),
        ...
        tab === 'challenge' && h('div', { className: 'space-y-4' },
          ...
          h('div', { className: glassCard },
            d.chalAILoading ? h('div', { className: 'text-center py-4' },
              ...
              h('p', { className: 'text-xs text-purple-600 font-bold' }, t('stem.epidemic.ai_generating_question', 'AI generating question...'))
            ) : h('div', { className: 'space-y-3' },
              activeChalQ && h('div', null,
                d.chalUseAI != null && h('span', { className: 'px-1.5 py-0.5 bg-purple-100 text-purple-600 text-[11px] font-bold rounded-full mr-2' }, t('stem.epidemic.ai', '\uD83E\uDDE0 AI')),
              ...
              h('input', {
                ...
                placeholder: t('stem.epidemic.type_your_answer', 'Type your answer...'),
                className: 'w-full px-4 py-2 border border-slate-400 rounded-xl text-sm focus:border-indigo-400',
                'aria-label': t('stem.epidemic.challenge_answer', 'Challenge answer')
              }),
              h('div', { className: 'flex gap-2' },
                h('button', { onClick: chalCheck, className: 'px-4 py-2 text-sm font-bold bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all' }, t('stem.epidemic.check', 'Check')),
                h('button', { onClick: function() { upd('chalFeedback', '\uD83D\uDCA1 ' + (activeChalQ.h || 'No hint')); }, className: 'px-3 py-2 text-sm font-bold bg-amber-50 text-amber-600 rounded-xl' }, t('stem.epidemic.hint', '\uD83D\uDCA1 Hint')),
                h('button', { onClick: chalNext, className: 'px-3 py-2 text-sm font-bold bg-slate-100 text-slate-600 rounded-xl' }, t('stem.epidemic.skip', 'Skip \u27A1')),
                callGemini && h('button', { 'aria-label': t('stem.epidemic.ai_next', 'AI Next'), onClick: chalAINext, className: 'px-3 py-2 text-sm font-bold bg-purple-100 text-purple-600 rounded-xl' }, t('stem.epidemic.ai_next_2', '\u2728 AI Next'))
              ),
        ...
        tab === 'battle' && h('div', { className: 'space-y-4' },
          h('div', { className: glassCard },
            h('h4', { className: 'text-sm font-bold text-slate-700 mb-2' }, t('stem.epidemic.outbreak_defense', '\u2694\uFE0F Outbreak Defense')),
            h('p', { className: 'text-xs text-slate-600' }, gradeText(gradeBand,
          ...
          !battleActive ? h('div', { className: glassCard + ' text-center space-y-3' },
            h('div', { className: 'text-5xl mb-2' }, '\uD83E\uDDA0'),
            h('p', { className: 'text-sm font-bold text-red-600' }, t('stem.epidemic.a_virus_is_threatening_the_population', 'A virus is threatening the population!')),
            h('div', { className: 'flex gap-2 justify-center' },
              h('button', { onClick: function() { startBattle(false); }, className: 'px-4 py-2 text-sm font-bold bg-red-600 text-white rounded-xl hover:bg-red-700 transition-all' }, t('stem.epidemic.start_battle', '\u2694\uFE0F Start Battle')),
              callGemini && h('button', { 'aria-label': t('stem.epidemic.ai_battle', 'AI Battle'), onClick: function() { startBattle(true); }, className: 'px-4 py-2 text-sm font-bold bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-all' }, t('stem.epidemic.ai_battle_2', '\uD83E\uDDE0 AI Battle'))
            )
          ) : h('div', { className: glassCard },
            // HP bars
            h('div', { className: 'space-y-2 mb-4' },
              h('div', { className: 'flex items-center gap-2' },
                h('span', { className: 'text-xs font-bold text-emerald-600 w-16' }, t('stem.epidemic.you', '\uD83D\uDEE1\uFE0F You')),
              ...
              h('div', { className: 'flex items-center gap-2' },
                h('span', { className: 'text-xs font-bold text-red-600 w-16' }, t('stem.epidemic.virus', '\uD83E\uDDA0 Virus')),
                h('div', { className: 'flex-1 h-4 bg-slate-200 rounded-full overflow-hidden' },
            ...
            battleOver ? h('div', { className: 'text-center space-y-2 py-4' },
              ...
              h('div', { className: 'flex gap-2 justify-center mt-2' },
                h('button', { onClick: function() { startBattle(false); }, className: 'px-4 py-2 text-sm font-bold bg-red-600 text-white rounded-xl hover:bg-red-700 transition-all' }, t('stem.epidemic.play_again', '\u21BA Play Again')),
                callGemini && h('button', { 'aria-label': t('stem.epidemic.ai_rematch', 'AI Rematch'), onClick: function() { startBattle(true); }, className: 'px-4 py-2 text-sm font-bold bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-all' }, t('stem.epidemic.ai_rematch_2', '\u2728 AI Rematch'))
              )
            ) : h('div', { className: 'space-y-3' },
              (function() {
                if (battleUseAI && d.battleAILoading) {
                  return h('div', { className: 'text-center py-4' },
                    h('div', { className: 'text-2xl animate-pulse mb-2' }, '\uD83E\uDDE0'),
                    h('p', { className: 'text-xs text-purple-600 font-bold' }, t('stem.epidemic.ai_generating_question_2', 'AI generating question...'))
                  );
                }
                var q = getCurrentBattleQ();
                if (!q) return null;
                return h('div', { className: 'space-y-3' },
                  battleUseAI && h('span', { className: 'px-1.5 py-0.5 bg-purple-100 text-purple-600 text-[11px] font-bold rounded-full' }, t('stem.epidemic.ai_generated', '\uD83E\uDDE0 AI-GENERATED')),
                  h('p', { className: 'text-sm font-medium text-slate-700' }, q.q),
                  h('input', { type: 'text', value: battleAnswer, onChange: function(e) { upd('battleAnswer', e.target.value); }, onKeyDown: function(e) { if (e.key === 'Enter') battleAttack(); }, placeholder: t('stem.epidemic.type_your_answer_2', 'Type your answer...'), className: 'w-full px-4 py-2 border border-slate-400 rounded-xl text-sm font-mono focus:border-red-400', 'aria-label': t('stem.epidemic.battle_answer', 'Battle answer') }),
                  h('div', { className: 'flex gap-2' },
                    h('button', { 'aria-label': 'Attack!', onClick: battleAttack, className: 'px-4 py-2 text-sm font-bold bg-red-600 text-white rounded-xl hover:bg-red-700 transition-all' }, t('stem.epidemic.attack', '\u2694\uFE0F Attack!')),
                    h('button', { 'aria-label': t('stem.epidemic.hint_2', 'Hint'), onClick: function() { updMulti({ battleFeedback: '\uD83D\uDCA1 ' + (q.h || 'No hint') }); }, className: 'px-3 py-2 text-sm font-bold bg-amber-50 text-amber-600 rounded-xl' }, t('stem.epidemic.hint_3', '\uD83D\uDCA1 Hint'))
                  ),
        ...
        tab === 'learn' && h('div', { className: 'space-y-4' },
          h('div', { className: glassCard },
            h('h4', { className: 'text-sm font-bold text-slate-700 mb-3' }, t('stem.epidemic.learn_epidemiology_concepts', '\uD83D\uDCDA Learn \u2014 Epidemiology Concepts')),
          ...
          LEARN_TOPICS.map(function(topic) {
            ...
            return h('div', { key: topic.title, className: glassCard + ' space-y-3' },
              ...
              h('div', { className: 'flex gap-2 pt-2 border-t border-slate-100' },
                h('button', { 'aria-label': t('stem.epidemic.try_it', 'Try It'), onClick: function() { markLearnRead(topic.title); updMulti({ tab: topic.tryIt }); announceToSR('Switched to ' + topic.tryIt); }, className: 'px-3 py-1.5 text-[11px] font-bold bg-violet-50 text-violet-600 rounded-lg hover:bg-violet-100 transition-all' }, t('stem.epidemic.try_it_2', '\uD83D\uDD2C Try It')),
                callTTS && h('button', { 'aria-label': t('stem.epidemic.read_aloud_3', 'Read Aloud'), onClick: function() { markLearnRead(topic.title); callTTS(content); }, className: 'px-3 py-1.5 text-[11px] font-bold bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-all' }, t('stem.epidemic.read_aloud_4', '\uD83D\uDD0A Read Aloud'))
              )
            );
          })
        ),
        ...
        tab === 'inquiry' && (function() {
          ...
          var regimeMeta = {
            contained: { label: t('stem.epidemic.contained', '🟢 Contained'), desc: t('stem.epidemic.r_eff_1_outbreak_dies_out_before_becom', 'R_eff < 1: outbreak dies out before becoming detectable. No pandemic.'), color: '#059669', bg: '#ecfdf5', border: '#86efac' },
            emerging:  { label: t('stem.epidemic.emerging', '🟡 Emerging'),  desc: t('stem.epidemic.r_eff_just_above_1_outbreak_grows_slow', 'R_eff just above 1: outbreak grows slowly. Visible to surveillance but not pandemic.'), color: '#d97706', bg: '#fffbeb', border: '#fcd34d' },
            pandemic:  { label: t('stem.epidemic.pandemic', '🔴 Pandemic'),  desc: t('stem.epidemic.r_eff_well_above_1_exponential_growth_', 'R_eff well above 1: exponential growth. Pandemic scale within weeks-months.'), color: '#dc2626', bg: '#fef2f2', border: '#fca5a5' }
          }[regime];
          function logObs() {
          ...
          return h('div', { className: 'space-y-4' },
            h('div', { className: glassCard },
              ...
              (function() {
                ...
                return h('div', { className: 'mb-3 rounded border border-slate-200 bg-slate-50 p-2' },
                  h('svg', { viewBox: '0 0 320 105', className: 'w-full h-28 block' },
                    ...
                    h('text', { x: 160, y: 102, textAnchor: 'middle', fontSize: 9, fill: '#64748b' }, t('stem.epidemic.time', 'time →'))
                  ),
                  h('div', { className: 'text-[10px] italic text-slate-500 text-center' },
                    t('stem.epidemic.sir_trajectories_susceptible_infected_', 'SIR trajectories (Susceptible / Infected / Recovered). Axes unlabeled by design — focus on the shape, not the number.'))
                );
              })(),
              // 3 sliders
              h('div', { className: 'grid grid-cols-1 md:grid-cols-3 gap-3 mb-3' },
                [
                  { key: 'contact', label: t('stem.epidemic.contact_frequency', 'Contact frequency (%)'), val: iq.contact, min: 0, max: 100, step: 1 },
                  { key: 'intervention', label: t('stem.epidemic.intervention_adoption', 'Intervention adoption (%)'), val: iq.intervention, min: 0, max: 100, step: 1 },
                  { key: 'transmissibility', label: t('stem.epidemic.transmissibility_r', 'Transmissibility (R₀)'), val: iq.transmissibility, min: 0.5, max: 8, step: 0.1 }
                ].map(function(s) {
              ...
              h('div', { className: 'flex gap-2 items-center mb-3 flex-wrap' },
                h('button', { onClick: logObs, className: 'px-2 py-1 rounded bg-slate-100 hover:bg-slate-200 text-[11px] font-bold text-slate-700 border border-slate-300' }, t('stem.epidemic.log_observation', '📋 Log observation')),
                h('button', { onClick: function() { setIQ({ contact: 50, intervention: 30, transmissibility: 2.5, log: [], hypothesis: '', stuckRevealed: false, understood: false, explanation: '' }); },
                  className: 'px-2 py-1 rounded bg-white hover:bg-slate-50 text-[11px] font-semibold text-slate-600 border border-slate-300' }, t('stem.epidemic.reset', '↺ Reset')),
              ...
              h('div', { className: 'mb-3' },
                h('label', { htmlFor: 'ep-hypo', className: 'block text-[11px] font-bold text-slate-700 mb-1' },
                  t('stem.epidemic.your_hypothesis_free_text_no_right_ans', 'Your hypothesis (free text — no right answer):')),
                h('textarea', { id: 'ep-hypo', value: iq.hypothesis || '',
                  onChange: function(e) { setIQ({ hypothesis: e.target.value }); },
                  placeholder: t('stem.epidemic.which_slider_is_the_most_efficient_lev', 'Which slider is the most efficient lever to flip a pandemic into emerging or contained? Is there a single combined number that predicts the regime? Type your own theory.'),
              ...
              h('div', { className: 'mb-3' },
                !iq.stuckRevealed && h('button', { onClick: function() { setIQ({ stuckRevealed: true }); },
                  ...
                  t('stem.epidemic.i_m_stuck_show_me_questions_to_think_a', '🤔 I\'m stuck — show me questions to think about (no answers)')),
                iq.stuckRevealed && h('div', { className: 'p-3 rounded bg-amber-50 border border-amber-200 text-[11px] text-slate-700 leading-relaxed' },
                  h('div', { className: 'font-bold text-amber-900 mb-1' }, t('stem.epidemic.open_prompts_investigate_by_manipulati', 'Open prompts — investigate by manipulating:')),
                  h('ul', { className: 'list-disc pl-5 space-y-1' },
                    h('li', null, t('stem.epidemic.hold_two_sliders_steady_move_the_third', 'Hold two sliders steady. Move the third. Watch what happens. Repeat with each.')),
                    h('li', null, t('stem.epidemic.log_observations_from_each_of_the_thre', 'Log observations from each of the three regimes. What patterns do you notice in the table?')),
                    h('li', null, t('stem.epidemic.try_to_find_two_completely_different_s', 'Try to find two completely different slider settings that produce the same regime. What do they share?')),
                    h('li', null, t('stem.epidemic.notice_where_a_small_slider_change_fli', 'Notice where a small slider change flips the regime versus where large changes do nothing. What might explain the difference?')),
                    h('li', null, t('stem.epidemic.look_up_what_r_eff_means_in_real_epide', 'Look up what R_eff means in real epidemiology. Compare it to what you observe here.'))),
                  h('div', { className: 'text-[10px] italic text-amber-700 mt-2' }, t('stem.epidemic.no_answers_no_specific_values_no_direc', 'No answers, no specific values, no directions. Investigate.')))
              ),
              h('div', { className: 'p-3 rounded bg-emerald-50 border border-emerald-200' },
                h('div', { className: 'flex items-center gap-2 mb-2' },
                  ...
                  h('label', { htmlFor: 'ep-und', className: 'text-[12px] font-bold text-emerald-800 cursor-pointer' },
                    t('stem.epidemic.i_think_i_understand_the_trade_offs_le', 'I think I understand the trade-offs — let me explain them in my own words'))),
                iq.understood && h('textarea', { value: iq.explanation || '',
                  onChange: function(e) { setIQ({ explanation: e.target.value }); },
                  placeholder: t('stem.epidemic.explain_in_your_own_words_what_is_the_', 'Explain in your own words: what is the relationship between contact frequency, intervention adoption, transmissibility, and outbreak regime? What single equation could capture it? Why is the threshold at R_eff = 1?'),
