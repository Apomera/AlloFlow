...
for(const [slug,terms,count] of [['materials_grade2',10,14]]){
...
expect(pack.history.find(r=>r.id==='mt-reading').data).toContain('Bending and stretching are different');
expect(pack.history.find(r=>r.id==='mt-reading').data).toContain('bigger patch does not prove');
expect(pack.history.find(r=>r.id==='mt-challenge').data.brief.seedDirection).toContain('fresh dry samples');
expect(pack.history.find(r=>r.id==='mt-faq').data[0].answer).toContain('Hardness and flexibility are different');
expect(pack.history.find(r=>r.id==='mt-quiz').data.questions[4].correctAnswer).toContain('same waiting time');
