// Interaction-DEPTH contrast probe: grade what first-screen sweeps cannot see.
//
//   node dev-tools/axe_tool_depth.cjs <toolFile> [--dark]
//
// WHY. theme_contrast_sweep.cjs and axe_tool_detail.cjs grade the FIRST SCREEN
// only. A sweep only grades what is OPEN: plateTectonics' collapsed Learn panel
// hid 1.6:1 inks from every first-screen instrument. This probe mounts the tool
// exactly like axe_tool_detail.cjs (same white-card dark substrate, same STEM
// palette, same Tailwind build), takes a baseline, then walks interaction
// states and reports only the NEW violations each state exposes:
//
//   1. expand   -- set open on every collapsed <details>
//   2. toggles  -- click each [aria-expanded="false"] control (cumulative)
//   3. tabs     -- click each [role=tab] in turn (axe after each)
//
// It deliberately clicks NOTHING else: arbitrary buttons mutate sim state in
// ways that make counts unreproducible. Dedupe key is fg+bg+html-snippet, so a
// violation visible at baseline is never re-billed to a later state.
'use strict';
...
if (!toolFile || !fs.existsSync(toolFile)) {
  console.error('usage: node dev-tools/axe_tool_depth.cjs <toolFile> [--dark]');
  process.exit(2);
}
...
(async () => {
  ...
  const runAxe = () => page.evaluate(async () => {
    ...
    return {
      v: r.violations.flatMap((x) => x.nodes.map((n) => {
        const msg = (n.any[0] && n.any[0].message) || '';
        const html = String(n.html || '').replace(/\s+/g, ' ').slice(0, 110);
        const m = /contrast of ([\d.]+).*?foreground color: (#[0-9a-f]{6}), background color: (#[0-9a-f]{6})/i.exec(msg);
        return { key: (m ? m[2] + '|' + m[3] : msg.slice(0, 60)) + '|' + html, msg, html };
      })),
  ...
  for (const r of report) {
    ...
    console.log('- ' + r.label + ': total ' + r.total + ', unmeasurable ' + r.incomplete + tag);
    for (const f of r.fresh) {
      const m = /contrast of ([\d.]+).*?foreground color: (#[0-9a-f]{6}), background color: (#[0-9a-f]{6})/i.exec(f.msg);
      console.log('    ' + (m ? m[1] + ':1  fg ' + m[2] + '  bg ' + m[3] : f.msg.slice(0, 100)));
      console.log('    ' + f.html);
    }
  }
  if (!anyFresh && report[0].fresh.length === 0) console.log('CLEAN at depth');
