...
const MAT = {
  ...
  handle:  new THREE.MeshStandardMaterial({ color: 0x557093, metalness: 0.1, roughness: 0.5 }),
...
function syncPointCard(){
  if(selIdx < 0 || selIdx >= design.points.length){
    ptCard.hidden = true; noSel.hidden = false; return;
  }
...
for(const b of rootEl.querySelectorAll('button.tpl')){
  b.addEventListener('click', () => {
    ...
    selIdx = -1;
    fullRebuild(); syncPointCard(); saveDesign();
    banner('Template loaded — press ▶ Test run to ride it.', '', 2600);
  });
}
...
const rq = {
  ...
  score: __clabGet('clab-rqScore'), fill: __clabGet('clab-rqTimerFill'),
  end: __clabGet('clab-rideEnd'), endBody: __clabGet('clab-rideEndBody')
};
...
function startRide(){
  ...
  ride.correct = 0; ride.total = 0; ride.times = [];
  ...
  startRun(false);
  sim.ride = true;
  banner('Ride & Solve — quick questions at the checkpoints. Buckle up!', '', 3000);
}
function pauseForQuestion(){
  ...
  ride.timerId = setInterval(() => {
    ...
    rq.fill.style.width = Math.max(0, left * 100) + '%';
...
function submitRideAnswer(val, instant){
  ...
  if(instant) resume();
  else setTimeout(resume, ok ? 1300 : 2600);
}
...
__clabGet('clab-btnRideClose').addEventListener('click', () => { rq.end.hidden = true; });
...
function animate(){
  ...
  if(!userTouched && !sim.running && camMode === 'orbit' && !REDUCED_MOTION){
