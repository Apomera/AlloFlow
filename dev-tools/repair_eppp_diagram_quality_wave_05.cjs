...
const vm = require('vm');
const { reviewWave, reviewDate, corrections } = require('./eppp_diagram_quality_wave_05_data.cjs');
...
const artifactName = 'eppp_diagram_review_wave_05';
...
function assertPlacement(source, correction, allowCorrected, allowMissingOriginal = false, scopeLabel = 'source') {
  ...
  if (String(diagram.title || '') !== correction.expectedOriginal.title
    ...
    throw new Error(`${correction.placementId}: current ${scopeLabel} diagram differs from both the guarded original and the wave 05 result; refusing to overwrite it.`);
  }
...
const markdown = `# EPPP diagram review wave 05
...
console.log(`EPPP diagram quality wave 05: corrected and source-reviewed ${items.length} inline placements.`);
