...
describe('EPPP claim-level adjudication batch 04', () => {
  it('reviews ten high-risk candidates without releasing or repeating them', () => {
    const report = read('test_prep/eppp_legacy/adjudication_batch_04.json');
    const priorIds = new Set(['01', '02', '03'].flatMap((batch) => read(`test_prep/eppp_legacy/adjudication_batch_${batch}.json`).items.map((item) => item.legacyId)));
  ...
  it('keeps deployment artifacts identical to source artifacts', () => {
    expect(read('prismflow-deploy/public/test_prep/eppp_legacy/adjudication_batch_04.json')).toEqual(read('test_prep/eppp_legacy/adjudication_batch_04.json'));
    expect(fs.readFileSync(resolve(root, 'prismflow-deploy/public/test_prep/eppp_legacy/adjudication_batch_04.md'), 'utf8')).toBe(fs.readFileSync(resolve(root, 'test_prep/eppp_legacy/adjudication_batch_04.md'), 'utf8'));
  });
});
