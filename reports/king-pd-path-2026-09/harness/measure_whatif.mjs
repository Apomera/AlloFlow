...
// What-if: emulate a proposed fix inside the real app, one or more of
// load | link | bytes | vendor (comma-separated). Clicks are still counted from the
// driver's own steps; emulation calls are not clicks.
const WHATIF = new Set((A.whatif || '').split(',').filter(Boolean));
...
const tag = `${TARGET}-${NET}-${FLOW}-r${RUN}${WHATIF.size ? '-whatif-' + [...WHATIF].sort().join('+') : ''}${SHOTS ? '-shots' : ''}`;
...
if (WHATIF.has('bytes')) {
  // Proposed: the closed analytics panel loads no word-audio bank / probe banks / chart.js
  // Abort both the CDN URL and its GitHub-raw fallback.
  // Not the strings: until ui_strings.js lands most labels are blank, so blocking it is
  // not a fix anyone would ship. The strings gate is reported from stringsAt instead.
  const BLOCK = ['word_audio_kokoro_bank.json', 'psychometric_probes.json', 'psychometric_math_probes.json', 'psychometric_literacy_probes.json', 'chart.js@4.4.7'];
  await ctx.route((u) => BLOCK.some((b) => u.href.includes(b)), (rt) => rt.abort('blockedbyclient'));
}
if (WHATIF.has('vendor')) {
  // Proposed: ship vendor/, fonts/ and the logo with the /app/ shell. Fulfilled locally
  // (these bytes skip the throttle: ~70 KB, about 0.4 s on Fast 3G, so this is slightly optimistic).
  const serve = (file, type) => (rt) => rt.fulfill({ status: 200, contentType: type, body: fs.readFileSync(REPO + file) });
  const has = (s) => (u) => u.href.includes(s);
  await ctx.route(has('/app/vendor/lz-string-1.4.4.min.js'), serve('vendor/lz-string-1.4.4.min.js', 'application/javascript'));
  await ctx.route(has('/app/vendor/idb-keyval-6.2.0.umd.min.js'), serve('vendor/idb-keyval-6.2.0.umd.min.js', 'application/javascript'));
  await ctx.route(has('/fonts/Inter-latin.woff2'), serve('desktop/web-app/public/fonts/Inter-latin.woff2', 'font/woff2'));
  await ctx.route(has('/app/rainbow-book.jpg'), serve('rainbow-book.jpg', 'image/jpeg'));
}
...
async function loadPack(openMenu, itemLocator, retryMenu, retryItem) {
  ...
  if (WHATIF.has('load')) {
    // Proposed handleLoadProject: promote the handler module (the host's own
    // __alloEnsureLazyModule path) and await it instead of throwing. Emulated by promoting
    // at the moment the person reaches the menu, then loading once it is registered.
    await page.evaluate(() => { window.__k4.mark('whatif_load_promoted'); try { window.__alloLazyFileIntake(); } catch (e) {} });
    ...
    if (await waitMarks(['pack_loaded'], 60000)) return { attempts: 1, emulatedFix: true };
    throw new Error('what-if load: pack did not load');
  }
  if (openMenu) await click('open-menu', openMenu);
...
async function openToolFromLink() {
  ...
  if (WHATIF.has('link')) {
    // Proposed settle(): a link whose tool is not registered yet requests that tool's module.
    await page.evaluate(() => { try { window.__alloEnsureSelPluginLoaded && window.__alloEnsureSelPluginLoaded('crewProtocols'); } catch (e) {} });
  }
...
const result = { tag, whatif: [...WHATIF], target: TARGET, net: NET, flow: FLOW, run: RUN, cpu: CPU, shots: SHOTS, url: URLS[TARGET], startedAt: new Date(t0wall).toISOString(), wallMs: Date.now() - t0wall,
