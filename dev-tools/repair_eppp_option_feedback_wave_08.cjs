...
const { revisions, wave07WarningSnapshot, wave08WarningSnapshot } = require('./eppp_option_feedback_wave_08_data.cjs');
...
const reviewWave = 'eppp-option-feedback-wave-08';
...
if (Object.keys(revisions).length !== 16) throw new Error('Wave 08 must contain exactly 16 reviewed items.');
...
if (!wave07WarningSnapshot || warningSummaryKeys.some((key) => !Number.isInteger(wave07WarningSnapshot[key]) || wave07WarningSnapshot[key] < 0)) throw new Error('Wave 08 needs a complete explicit wave-07 warning snapshot.');
if (!wave08WarningSnapshot || warningSummaryKeys.some((key) => !Number.isInteger(wave08WarningSnapshot[key]) || wave08WarningSnapshot[key] < 0)) throw new Error('Wave 08 needs a complete explicit wave-08 warning snapshot.');
const baseline = { ...wave07WarningSnapshot };
const auditItems = [];
...
for (const [id, revision] of Object.entries(revisions)) {
  ...
  if (revision.sourceDetails && (item.sourceDetails.length !== item.references.length || item.sourceDetails.some((source) => !item.references.includes(source.url) || source.title.length < 20 || source.credibility.length < 100))) throw new Error(id + ' needs complete source metadata for every strengthened citation.');
...
if (domains.length !== 8) throw new Error('Wave 08 must cover all eight EPPP domains.');
...
if (liveAfter.incorrectOptionsWithWarnings > wave08WarningSnapshot.incorrectOptionsWithWarnings) throw new Error('Wave 08 replay increased the bank-wide incorrect-option warning count beyond its reviewed snapshot.');
const after = { ...wave08WarningSnapshot };
...
if (selectedWarningsAfter !== 0) throw new Error('Wave 08 did not clear every selected incorrect-option warning.');
...
const audit = {
  ...
  sourceCorrections: [
    'Replaced a malformed tardive-dyskinesia DOI with a valid peer-reviewed systematic review.',
    'Added a conditional-probability DIF source, a primary Flynn-reversal study, and the direct future-time formulation of socioemotional selectivity theory.',
    'Added current Cochrane missing-evidence guidance and a direct experimental social-facilitation source.',
    'Aligned the construct-validity stem with the different-method emphasis of the cited multitrait-multimethod framework.',
  ],
  challengeCorrections: [
    'Replaced unrelated VTA distractors with the mesocortical, nigrostriatal, and tuberoinfundibular dopamine pathways.',
    'Replaced three Flynn-effect giveaway claims with aging, retest, and renorming confusions.',
    'Replaced three Principle A giveaway claims with accurate neighboring APA General Principles.',
    'Removed absolute cue wording from DIF, Garcia-Koelling, publication-bias, socioemotional-selectivity, and social-facilitation alternatives.',
    'Changed the Asch key from ally to dissenter so breaking unanimity is not confused with supporting the participant.',
  ],
  summary: {
...
const markdown = `# EPPP incorrect-option explanation repair - wave 08
...
- Replaced one malformed DOI and strengthened source alignment for DIF, Flynn-effect reversal, socioemotional selectivity, publication bias, social facilitation, and multitrait-multimethod validity.
...
for (const outputRoot of outputRoots) {
  writeFileWithRetry(path.join(outputRoot, 'eppp_option_feedback_audit_wave_08.json'), JSON.stringify(audit, null, 2) + '\n');
  writeFileWithRetry(path.join(outputRoot, 'eppp_option_feedback_audit_wave_08.md'), markdown);
}
console.log('EPPP option feedback wave 08: reviewed 16 items, replaced 48 explanations, and refined ' + audit.summary.choicesRefined + ' response options; selected warning count 0.');
