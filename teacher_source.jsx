    ...
    <div className="fixed inset-0 z-[9999] overflow-auto bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 text-white" role="region" aria-labelledby="concept-quest-title">
      ...
      <header className="sticky top-0 z-20 border-b border-indigo-400/30 bg-slate-950/95 p-3 backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-3 text-sm font-bold" aria-label={tr('party_health_aria', 'Party health {current} of {maximum}', { current: quest.party.hp, maximum: quest.party.maxHp })}><span>❤️ {quest.party.hp}/{quest.party.maxHp}</span><span>🛡️ {quest.party.shield || 0}</span><span>✨ {quest.party.xp || 0} XP</span><span>🔷 {(quest.sigils || []).length}/{neededSigils}</span></div>
        </div>
      ...
      <main className="mx-auto grid max-w-6xl gap-5 p-4 lg:grid-cols-[1.25fr_.75fr]">
        <section className="rounded-2xl border border-indigo-400/30 bg-slate-900/80 p-4" aria-labelledby="quest-map-heading">
          ...
          <div className="relative grid grid-cols-2 gap-2 rounded-xl border border-slate-700 bg-slate-950/70 p-2 sm:block sm:h-[360px] sm:p-0" aria-label={tr('map_aria', 'Eight-room game map')}>
