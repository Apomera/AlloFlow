...
const {
  ...
  revisions,
  duplicateRepairs,
} = require('./eppp_application_rewrite_wave_52_data.cjs');
...
const auditBasename = 'eppp_application_rewrite_audit_wave_52.json';
const outputRoots = [
...
if (sourceText === deployText) {
  ...
  if (sourceHasWave === deployHasWave) {
    throw new Error('Source and deploy banks differ without a single wave 52 recovery winner.');
  }
...
if (revisions.length !== 24 || new Set(revisions.map(function (revision) { return revision.id; })).size !== revisions.length) {
  throw new Error('Application rewrite wave 52 requires twenty-four unique revisions.');
}
...
for (const revision of revisions) {
  ...
  if (!item || !prior) throw new Error('Missing wave 52 item: ' + revision.id);
...
for (const repair of duplicateRepairs) {
  const item = itemsById.get(repair.id);
  const prior = beforeById.get(repair.id);
  if (!item || !prior) throw new Error('Missing duplicate-separation item: ' + repair.id);
  if (item.answerIndex !== repair.answerIndex) {
    throw new Error(repair.id + ' duplicate-separation answer position drifted.');
  }
  if (!Array.isArray(repair.choices) || repair.choices.length !== 4) {
    throw new Error(repair.id + ' needs four duplicate-separation choices.');
  }
  if (!Array.isArray(repair.choiceRationales) || repair.choiceRationales.length !== 4) {
    throw new Error(repair.id + ' needs four duplicate-separation explanations.');
  }
  const priorDifficulty = item.duplicatePairCleanupPriorDifficulty || item.difficulty;
  const priorCognitiveProcess = item.duplicatePairCleanupPriorCognitiveProcess || item.cognitiveProcess || null;
  item.prompt = repair.prompt;
  item.choices = repair.choices.slice();
  item.rationale = repair.rationale;
  item.choiceRationales = repair.choiceRationales.slice();
  item.choiceRationales[repair.answerIndex] = repair.rationale;
  item.difficulty = 'intermediate';
  item.cognitiveProcess = 'application';
  item.duplicatePairCleanupWave = reviewWave;
  item.duplicatePairCleanupAt = reviewedAt;
  item.duplicatePairCleanupPriorDifficulty = priorDifficulty;
  item.duplicatePairCleanupPriorCognitiveProcess = priorCognitiveProcess;
  item.wordingReviewStatus = 'editorial-duplicate-separation';
  item.wordingReviewWave = reviewWave;
  item.wordingReviewAt = reviewedAt;
  item.optionFeedbackRefinementWave = reviewWave;
  item.optionFeedbackRefinedAt = reviewedAt;
...
if ((distractorAfter.uniqueKeyStemLexicalLeakage || []).length) {
  throw new Error('Lexical leakage reappeared during wave 52.');
}
if ((distractorAfter.asymmetricExtremeDistractors || []).length) {
  throw new Error('Extreme-word distractors reappeared during wave 52.');
}
if ((distractorAfter.advancedDirectRecall || []).length) {
  throw new Error('Advanced direct-recall candidates remain after wave 52.');
}
if ((distractorAfter.semanticConceptDuplicates && distractorAfter.semanticConceptDuplicates.pairs || []).length) {
  throw new Error('Semantic duplicate pairs reappeared during wave 52.');
}
if ((feedbackAfter.optionFindings || []).length || feedbackAfter.summary && feedbackAfter.summary.itemsWithWarnings) {
  throw new Error('Option-feedback warnings reappeared during wave 52.');
}
if (qaAfter.summary && (qaAfter.summary.status !== 'pass' || qaAfter.summary.passedItems !== 1500)) {
  throw new Error('Native EPPP QA did not pass after wave 52.');
}
...
const audit = {
  ...
  summary: {
    ...
    duplicatePairRepairs: duplicateRows.length,
  ...
  items: resultRows,
  duplicatePairRepairs: duplicateRows,
  limitations: [
...
console.log(
  'EPPP application wave 52: rewrote ' + revisions.length + ' foundation items as applied questions; '
