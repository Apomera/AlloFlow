...
```ts
  ...
  7  |   await page.evaluate(()=>{document.body.className='theme-default';document.getElementById('wrap')!.style.cssText='width:100%;height:auto;display:block;padding:16px;background:white';const w=window as any,p=w.THREE.WebGLRenderer.prototype,render=p.render;w.__earScene=[];p.render=function(scene:any,camera:any){w.__earScene=scene.children.filter((a:any)=>a.visible===true&&a.userData.ears).map((a:any)=>a.userData.ears.map((ear:any)=>({angles:[ear.rotation.y,ear.rotation.z],attached:ear.parent===a.userData.head,parts:ear.children.length})));return render.call(this,scene,camera);};});
