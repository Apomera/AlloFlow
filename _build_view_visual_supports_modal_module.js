...
const SOURCE = path.join(ROOT, 'view_visual_supports_modal_source.jsx');
const OUTPUT = path.join(ROOT, 'view_visual_supports_modal_module.js');
const PUBLIC_OUTPUT = path.join(ROOT, 'prismflow-deploy', 'public', 'view_visual_supports_modal_module.js');
const TMP = path.join(ROOT, '_tmp_visual_supports_modal_entry.jsx');
if (!fs.existsSync(SOURCE)) { console.error('Source not found:', SOURCE); process.exit(1); }
const source = fs.readFileSync(SOURCE, 'utf-8');
const entry = `/* global React */\n\n${source}\n\nwindow.__visualSupportsModalExports = { VisualSupportsModal };\n`;
fs.writeFileSync(TMP, entry, 'utf-8');
console.log('Compiling view_visual_supports_modal_source.jsx...');
try {
...
const compiled = fs.readFileSync(TMP + '.compiled.js', 'utf-8')
    .replace(/\/\*.*global.*\*\/\n/g, '')
    .replace(/window\.__visualSupportsModalExports\s*=\s*\{[^}]+\};?\s*/, '')
    .trim();
fs.unlinkSync(TMP);
