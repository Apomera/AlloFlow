...
const pack = JSON.parse(fs.readFileSync(path.join(root, 'test_prep', 'school_psychologist_5403_pack.json'), 'utf8'));
const sourceItems = JSON.parse(fs.readFileSync(path.join(root, 'test_prep', 'school_psychologist_5403_items.json'), 'utf8'));
...
const expectedDomains = {
  'permeating-practices': { label: 'Professional Practices That Permeate All Aspects of Service Delivery', weight: 0.32, officialQuestions: 40, diagnosticQuestions: 32 },
  'student-level-services': { label: 'Direct and Indirect Services for Children, Families, and Schools: Student-Level Services', weight: 0.23, officialQuestions: 28, diagnosticQuestions: 23 },
  'systems-level-services': { label: 'Direct and Indirect Services for Children, Families, and Schools: Systems-Level Services', weight: 0.20, officialQuestions: 25, diagnosticQuestions: 20 },
  foundations: { label: 'Foundations of School Psychological Service Delivery', weight: 0.25, officialQuestions: 32, diagnosticQuestions: 25 },
};
...
const officialSampleFragments = ['student s family reports that the native language at home is portuguese', 'lenny is a fifth grade student', 'viraag a third grade student'];
const findings = [];
...
if (pack.schemaVersion !== 1 || pack.id !== 'praxis-school-psychologist-5403' || pack.status !== 'ready') add(null, 'blueprint-alignment', 'Invalid pack identity, schema, or release status.');
...
if (pack.simulationItemCount !== 125 || pack.simulationTimeMinutes !== 125) add(null, 'blueprint-alignment', 'The official-length simulation must declare 125 questions and 125 minutes.');
if (pack.learningLibraryUrl !== './test_prep/school_psychologist_5403_learning_library.json' || pack.learningLibraryQaUrl !== './test_prep/school_psychologist_5403_learning_library_qa.json') add(null, 'learning-linkage', 'Learning-library URLs are invalid.');
if (library.libraryId !== 'praxis-school-psychologist-5403-learning-library' || library.packId !== pack.id || (library.skills || []).length !== 12 || (library.chapters || []).length !== 12) add(null, 'learning-linkage', 'Learning-library identity or skill inventory is invalid.');
if (!/125 selected-response questions in 125 minutes/i.test(pack.disclaimer || '') || !/not official or scaled Praxis scores/i.test(pack.disclaimer || '') || !/not affiliated with or endorsed by ETS or NASP/i.test(pack.disclaimer || '') || !/psychological evaluation|diagnosis/i.test(pack.disclaimer || '') || !/mandated-reporting/i.test(pack.disclaimer || '')) add(null, 'provenance', 'The independent-preparation, affiliation, evaluation, and legal-safety disclaimer is incomplete.');
...
if (declaredDomains.size !== 4) add(null, 'blueprint-alignment', 'Exactly four 5403 categories are required.');
...
for (const item of pack.items) {
  ...
  if (!/^sp5403-b[12]-\d{3}$/.test(item.id || '') || !expectedDomains[item.domainId]) add(item, 'blueprint-alignment', 'ID or category is outside the 5403 blueprint.');
...
const report = {
  ...
  packId: pack.id,
  standard: {
    label: 'AlloFlow independent 5403 source, structure, feedback, originality, safety, and linkage QA v1',
    ...
    limitation: 'This is not ETS or NASP approval, independent school-psychologist validation, field testing, psychometric calibration, a scaled-score model, psychological evaluation, diagnosis, mental-health treatment, or legal advice.',
  },
  blueprint: { officialQuestionCount: 125, timeMinutes: 125, selectedResponse: true, categories: Object.fromEntries(Object.entries(expectedDomains).map(([id, entry]) => [id, { questions: entry.officialQuestions, percentage: Math.round(entry.weight * 100) }])) },
  ...
  summary: { totalItems: reports.length, passedItems, reviewRequiredItems: reports.length - passedItems, findings: allFindings.length, status },
  findings,
  items: reports,
};
...
console.log('Praxis School Psychologist 5403 pack QA: ' + status + ' (' + passedItems + '/' + reports.length + ' items passed; ' + allFindings.length + ' findings).');
