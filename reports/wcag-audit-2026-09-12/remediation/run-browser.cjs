...
(async()=>{
 ...
 try{
  for(const route of ['/app/','/catalog.html','/video_studio/video_studio.html']){
   ...
   await page.setViewportSize({width:320,height:800});await audit(id+'-320');
   await page.addStyleTag({content:'* {line-height:1.5!important;letter-spacing:.12em!important;word-spacing:.16em!important} p {margin-bottom:2em!important}'});await audit(id+'-320-spacing');
  }
 }catch(e){results.error=e.stack;console.error(e.stack);process.exitCode=1;}
 results.verification={expectedStates:9,observedStates:results.states.length,passed:!results.error&&results.states.length===9&&results.states.every(s=>s.violations.length===0&&s.scrollWidth<=s.clientWidth)};if(!results.verification.passed)process.exitCode=1;results.blockedOrigins=[...blocked];results.pageErrors=errors;results.finishedAt=new Date().toISOString();fs.writeFileSync(path.join(__dirname,'browser-audit.json'),JSON.stringify(results,null,2));await browser.close();await new Promise(r=>server.close(r));
})();
