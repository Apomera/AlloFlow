...
// Run: node dev-tools/watercycle_pilot_stream_landscape_qa.cjs
const fs = require('fs');
const path = require('path');
const assert = require('node:assert/strict');
const { chromium } = require('playwright');
const ROOT = process.cwd();
const out = path.join(ROOT, 'scratch', 'water-stream-landscape-review');
...
(async () => {
  ...
  try {
    ...
    await page.evaluate(()=>{
      ...
      renderer.render=(scene,camera)=>{camera.position.set(231,24,-46);camera.lookAt(220,5.2,-65);return render(scene,camera);};
    });
    ...
    await page.evaluate(()=>{
      window.streamDisposals={geometry:0,texture:0,channel:0,riffleGeometry:0,riffleMaterial:0};const stones=pilotReview.scene.getObjectByName('pilot-stream-bank-stones');
    ...
    });assert.deepEqual(await page.evaluate(()=>streamDisposals),{geometry:1,texture:1,channel:1,riffleGeometry:1,riffleMaterial:1});assert.deepEqual(errors,[]);
    console.log('PASS: bank-color and cross-channel attributes, finite riffles, shared water/riffle clock, winter suppression, finite stream detail, paused and reduced-motion ripples/flow, both learner views, real runoff-to-water transition, winter ice, desert visibility, mobile accessibility, shared resource disposal, no WebGL errors.');
