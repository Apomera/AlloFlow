...
var d = labToolData || {};
  ...
  window.StemLab.registerTool('geometryProver', {
    ...
    category: 'math',
    render: function(ctx) {
      // geometryProver is the SECOND registerTool in this file and has its own
      // render(ctx), so the __alloT declared for the first tool is out of scope here.
      var __alloT = function (k, fb) { var v; try { v = (typeof ctx.t === 'function') ? ctx.t(k, fb) : null; } catch (e) { v = null; } return (v == null) ? (fb != null ? fb : k) : v; };
      ...
      return (function() {
        ...
        const renderTheoremPanel = () => {
          ...
          if (gpInvestigate&&!gpRevealed) return React.createElement('div',{className:'bg-amber-50 rounded-xl p-3 border-2 border-amber-200 border-dashed'},
            ...
            React.createElement('textarea',{rows:2,placeholder:__alloT('stem.geo.what_theorems_or_rules_do_you_think_apply_he','What theorems or rules do you think apply here?'),'aria-label':__alloT('stem.geo.theorem_prediction','Theorem prediction'),value:gp.prediction||'',onChange:e=>gpUpd('prediction',e.target.value),className:'w-full px-3 py-2 border-2 border-amber-600 rounded-lg text-sm mb-2 outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-300 resize-none'}),
        ...
        return React.createElement('div',{className:'space-y-3 max-w-3xl mx-auto animate-in fade-in duration-200'},
          ...
          // Tab bar
          React.createElement('div',{style:{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))'},className:'gap-1 bg-slate-100 p-1 rounded-xl',role:'tablist','aria-label':__alloT('stem.geo.geometry_prover_sections','Geometry Prover sections')},
          ...
          gpTab==='challenge'&&React.createElement('div',{id:'geometry-prover-panel',role:'tabpanel','aria-labelledby':'geometry-prover-tab-challenge',tabIndex:0,className:'space-y-3'},
            ...
            gpChallenge&&gpChallenge.type!=='theorem_match'&&React.createElement('div',{className:'bg-violet-50 rounded-xl p-4 border-2 border-violet-200'},
              ...
              React.createElement('div',{className:'flex gap-2'},
                React.createElement('input',{type:'text','aria-label':__alloT('stem.geo.challenge_answer_in_degrees','Challenge answer in degrees'),value:gp.challengeAnswer||'',onChange:e=>gpUpd('challengeAnswer',e.target.value),onKeyDown:e=>{if(e.key==='Enter')checkChallenge();},placeholder:__alloT('stem.geo.your_answer',"Your answer (°)"),className:'flex-1 px-3 py-2 border-2 border-violet-600 rounded-lg text-sm font-bold text-center focus:border-violet-500 focus:ring-2 focus:ring-violet-300 outline-none'}),
