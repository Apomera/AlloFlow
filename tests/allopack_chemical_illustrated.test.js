...
const slug='chemical_reactions_grade8',folder='allopacks/media/'+slug+'/',pack=read('allopacks/illustrated/'+slug+'.allopack.json'),original=read('allopacks/'+slug+'.allopack.json'),manifest=read(folder+'manifest.json');
require('../dev-tools/chemical_content_refinements.cjs').refine(original);
...
for(const key of ['reaction','conservation']){const doc=svg(key);const halves=doc.split(/data-side="(?:before|after)"/).slice(1);expect(halves).toHaveLength(2);for(const h of halves){expect((h.match(/data-atom="red"/g)||[]).length).toBe(2);expect((h.match(/data-atom="blue"/g)||[]).length).toBe(2);}}expect((svg('physical').match(/data-molecule="water"/g)||[]).length).toBe(8);expect(get('applied-challenge').data.brief.context).toContain('invented practice');expect(get('quiz').data.questions[6].question).toContain('supplies data');
});
