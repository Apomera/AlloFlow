...
function fixture(motion=true){
  ...
  const sprite=(width,height)=>new THREE.Sprite(new THREE.SpriteMaterial({map:new THREE.CanvasTexture(canvas(width,height)),transparent:true,opacity:0}));
  const npc={data:{name:'Motion Guide',position:[2,0,3],question:{},dialogue:'A test character'},body:new THREE.Mesh(new THREE.BoxGeometry(),new THREE.MeshStandardMaterial()),head:new THREE.Object3D(),label:sprite(640,160),prompt:sprite(320,96),qMark:sprite(128,128),_speechBubble:sprite(512,128),_ring:new THREE.Mesh(new THREE.RingGeometry(),material()),eyeL:new THREE.Object3D(),eyeR:new THREE.Object3D(),_arms:[new THREE.Object3D(),new THREE.Object3D()]};
  ...
  const e={_ambientMotionEnabled:motion,npcs:[npc],_answeredRef:[false],renderer:{domElement:{closest:()=>null}},camera:new THREE.PerspectiveCamera(),clock:{getElapsedTime:()=>1},_particles:[],scene:new THREE.Scene(),velocity:new THREE.Vector3(1,2,3),moveState:{forward:true}};
  e.camera.position.set(20,4,20);e.scene.add(npc.body);e.scene.add(npc._ring);fixtures.push({e,npc});
  const update=new Function('engine','window','answeredNpcs','geometryWorldBlinkScale','dt','THREE','Math',guideSource+'\n'+npcSource);
  function frame(time,random=.5){e.clock.getElapsedTime=()=>time;const math=Object.create(Math);math.random=()=>random;update(e,{THREE,matchMedia:()=>({matches:false})},[],()=>.2,.016,THREE,math);}
  return{e,npc,frame};
}
function snapshot(n){return{body:n.body.position.toArray(),head:n.head.position.toArray(),label:n.label.position.toArray(),bodyRotation:n.body.rotation.toArray(),headRotation:n.head.rotation.toArray(),questionPosition:n.qMark.position.toArray(),questionScale:n.qMark.scale.toArray(),eyes:n._eyeParts.map(e=>e.scale.y),arms:n._arms.map(a=>[a.rotation.x,a.rotation.z]),ringScale:n._ring.scale.toArray(),ringOpacity:n._ring.material.opacity,prompt:n.prompt.position.toArray(),promptOpacity:n.prompt.material.opacity,speech:n._speechBubble.position.toArray(),speechOpacity:n._speechBubble.material.opacity,emissive:n.body.material.emissiveIntensity};}
afterEach(()=>{for(const{e,npc}of fixtures.splice(0)){const seen=new Set();for(const object of [...e.scene.children,npc.label,npc.prompt,npc.qMark,npc._speechBubble])object.traverse(o=>{for(const r of [o.geometry,o.material&&o.material.map,o.material])if(r&&r.dispose&&!seen.has(r)){seen.add(r);r.dispose();}});}vi.restoreAllMocks();});
...
describe('NPC decorative motion and readable feedback',()=>{
  it('holds idle NPCs, markers, eyes and arms still across animation frames',()=>{
    ...
    expect(snapshot(f.npc)).toEqual(before);expect(f.npc.body.position.toArray()).toEqual([2.5,.75,3.5]);expect(f.npc._eyeParts.every(e=>e.scale.y===1)).toBe(true);expect(f.npc._arms.map(a=>a.rotation.x)).toEqual([0,0]);expect(f.npc.qMark.visible).toBe(true);expect(f.npc.label.position.y).toBe(2.45);expect(f.npc.qMark.position.y).toBe(3.05);expect(f.npc.prompt.position.y).toBe(3.05);expect(f.npc._speechBubble.position.y).toBe(3.6);
  });
  ...
  it('keeps proximity cues visible and stable without moving the camera or velocity',()=>{
    ...
    f.e.camera.position.set(2.5,2,8);f.frame(4);expect(f.npc._speechBubble.material.opacity).toBe(.95);expect(f.npc.prompt.material.opacity).toBe(0);const medium=snapshot(f.npc);f.frame(5);expect(snapshot(f.npc)).toEqual(medium);
  });
