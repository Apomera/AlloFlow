...
for(const outputRoot of[path.join(root,'test_prep'),path.join(root,'prismflow-deploy','public','test_prep')]){fs.mkdirSync(outputRoot,{recursive:true});writeGeneratedFile(path.join(outputRoot,'early_childhood_5025_items.json'),JSON.stringify(items,null,2)+'\n');writeGeneratedFile(path.join(outputRoot,'early_childhood_5025_pack.json'),JSON.stringify(pack,null,2)+'\n');}console.log('Built Early Childhood 5025: 200 items; diagnostics 30/25/14/14/17; simulation 36/30/17/17/20.');
