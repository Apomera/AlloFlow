...
<div ref={liveDockPanelRef} tabIndex={-1} role="dialog" aria-modal="true" aria-label={t('live_dock.title') || 'Live Dashboard'} style={{width:'min(1180px, calc(100vw - 2rem))',maxHeight:'calc(100dvh - 2rem)',boxSizing:'border-box',overflowY:'auto',background:'white',borderRadius:16,border:'1px solid #cbd5e1',boxShadow:'0 24px 72px rgba(15,23,42,0.42)',padding:'1.1rem'}}>
                <div style={{position:'sticky',top:'-1.1rem',zIndex:4,display:'flex',justifyContent:'space-between',alignItems:'center',gap:8,margin:'-1.1rem -1.1rem 0.7rem',padding:'0.9rem 1.1rem',background:'rgba(255,255,255,0.97)',borderBottom:'1px solid #e2e8f0',borderRadius:'16px 16px 0 0'}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:8}}>
                    {/* The in-page End Session sits bottom-right, under this dashboard's
                        floating launcher. Here it is always reachable. It opens the
                        same summary + confirm as every other End Session. Older hosts
                        do not pass the handler, so the button waits for one. */}
                    {typeof requestEndLiveSession === 'function' && (
                      <button type="button" data-live-dock-end-session="true" onClick={() => { setShowLiveDock(false); requestEndLiveSession(); }} style={{minHeight:44,background:'#fff1f2',color:'#9f1239',border:'1px solid #fecdd3',borderRadius:8,padding:'0.3rem 0.8rem',cursor:'pointer',fontWeight:700,fontSize:'0.8rem'}}>{t('session.action_end') || 'End Session'}</button>
                    )}
                  ...
                  try {
                    ...
                    return (
                      <button type="button" onClick={() => { try { if (window.__alloOpenDiagnosticsLog) window.__alloOpenDiagnosticsLog('session'); } catch (e) {} }}
                        ...
                        style={{display:'flex',alignItems:'center',gap:6,width:'100%',textAlign:'left',padding:'0.4rem 0.55rem',marginBottom:6,borderRadius:8,cursor:'pointer',fontSize:'0.72rem',fontWeight:700,fontFamily:'inherit',border:'1px solid ' + (problemIsCurrent ? '#fecaca' : lastSync ? '#bbf7d0' : '#cbd5e1'),background:problemIsCurrent ? '#fef2f2' : lastSync ? '#f0fdf4' : '#f8fafc',color:problemIsCurrent ? '#991b1b' : lastSync ? '#166534' : '#475569'}}>
                        <span aria-hidden="true">{problemIsCurrent ? '⚠️' : lastSync ? '🟢' : '○'}</span>
                                ...
                                : organizerProgress.status === 'failed'
                                  ...
                                  : organizerProgress.status === 'ready'
                                    ...
                                    : organizerProgress.status === 'loading'
                                      ...
                                      : 'Organizer working';
                          return (
                            <div key={uid} style={{display:'flex',flexWrap:'wrap',alignItems:'center',gap:6,padding:'0.3rem 0.45rem',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,fontSize:'0.75rem'}}>
                              ...
                              {MailboxImageStatus && _alloMbBridgeActive() && resourcesWithImages.has(targetId || viewing) && (
                                <MailboxImageStatus entry={entry} resourceId={targetId || viewing} resourceAt={targetAt} now={dockNow} mailboxVersion={mailboxImageVersion} onRetry={() => retryMailboxImagesForStudent(uid, targetId || viewing)} />
                              )}
