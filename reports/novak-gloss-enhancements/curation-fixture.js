    ...
    "instructional_context_module.js"(exports, module) {
      (function() {
        ...
        function readingUnit(item) {
          return normalizeReadingUnitId(item && (item.unitId !== void 0 ? item.unitId : item.config && item.config.unitId));
        }
        function normalizeSourceInstructionalText(raw, options) {
          ...
          var profile = normalizeInstructionalText(raw, {
            ...
            form: "original",
            designationSource: opts.designationSource || "workflow-default",
            sourceArtifactId: opts.sourceArtifactId,
            primaryArtifactId: opts.sourceArtifactId,
        ...
        function isAuthorizedAdaptedPrimary(item) {
          ...
          return profile.form === "adapted" && profile.role === "primary" && profile.replacementAuthorization.authorized === true && profile.replacementAuthorization.source === "educator";
        }
        function getSourceInstructionalText(item) {
          var source2 = isObject(item) ? item : {};
          var profile = getInstructionalText(source2);
          var raw = source2.sourceInstructionalText || source2.config && source2.config.sourceInstructionalText;
          if (profile.form !== "adapted") raw = profile;
          var snapshot2 = getSourceSnapshot(source2);
          return normalizeSourceInstructionalText(raw, {
            ...
            role: isAuthorizedAdaptedPrimary(source2) ? "supplemental" : "primary",
            sourceArtifactId: snapshot2 && snapshot2.sourceArtifactId,
            language: snapshot2 && snapshot2.language
          });
        }
        function readingFamilyIds(item) {
          if (!isObject(item)) return [];
          var profile = getInstructionalText(item), snapshot2 = getSourceSnapshot(item);
          ...
          if (snapshot2) values.push(snapshot2.sourceArtifactId);
        ...
        function createSupportedReading(textOrSnapshot, options) {
          ...
          var inherited = opts.instructionalText || opts.sourceInstructionalText || opts.sourceItem && getSourceInstructionalText(opts.sourceItem) || opts.config && opts.config.instructionalText;
          var sourceProfile = normalizeSourceInstructionalText(inherited, {
            ...
            designationSource: opts.designationSource
          });
          ...
          var id = opts.id == null ? "original-" + snapshot2.fingerprint : String(opts.id);
          var familyId = cleanText(opts.sourceFamilyId || opts.sourceItem && getReadingSourceFamilyId(opts.sourceItem) || snapshot2.sourceArtifactId || id, 200);
          var unitId = normalizeReadingUnitId(opts.unitId !== void 0 ? opts.unitId : opts.sourceItem && opts.sourceItem.unitId);
          var reading = {
            id,
            type: "simplified",
            ...
            sourceInstructionalText: sourceProfile,
        ...
        function getReadingRoleLabel(itemOrProfile) {
          var profile = isObject(itemOrProfile) && !itemOrProfile.type && itemOrProfile.role ? normalizeInstructionalText(itemOrProfile) : getInstructionalText(itemOrProfile);
        ...
        function updateInstructionalRole(item, requestedRole, options) {
          ...
          var authorized = current.form === "adapted" && requestedRole === "primary" && (opts.authorizeReplacement === true || isAuthorizedAdaptedPrimary(item));
        ...
        function updateReadingFamilyRole(items, selectedItem, requestedRole, options) {
          var source2 = Array.isArray(items) ? items : [];
          var selected = source2.find(function(item) {
            return item && selectedItem && String(item.id) === String(selectedItem.id);
          }) || selectedItem;
          var updated = updateInstructionalRole(selected, requestedRole, options);
          if (updated === selected) return { items: source2, item: selected, changedIds: [] };
          ...
          var changedIds = [];
          var output = source2.map(function(item) {
            var next = item;
          ...
          return { items: output, item: updated, changedIds };
        }
        function ensureReadingSourcePairs(items, options) {
          ...
          var available = source2.concat(history.filter(function(item) {
            return item && !source2.some(function(current) {
              return current.id === item.id;
            });
          }));
          var originals = source2.filter(isSupportedOriginal), output = [];
          source2.forEach(function(item) {
            var snapshot2 = ["analysis", "simplified"].indexOf(item.type) !== -1 && getInstructionalText(item).form === "adapted" ? getSourceSnapshot(item) : null;
            if (snapshot2 && !originals.some(function(original2) {
              return sameReadingSourceFamily(original2, item) && original2.data === snapshot2.text;
            })) {
              var existing = available.find(function(candidate) {
                return isSupportedOriginal(candidate) && sameReadingSourceFamily(candidate, item) && candidate.data === snapshot2.text;
              });
              ...
              var reading = existing || createSupportedReading(snapshot2, {
                title: (cleanText(item.title, 260) || "Reading") + " \u2014 original",
                config: { language: snapshot2.language, grade: item.config && item.config.grade },
                sourceInstructionalText: role,
                sourceFamilyId: getReadingSourceFamilyId(item),
                unitId: readingUnit(item),
                instructionalContext: item.instructionalContext,
                standardsContext: item.standardsContext
              });
              if (!existing) {
                var baseId = reading.id, suffix = 1;
                while (source2.concat(output).some(function(entry) {
                  return entry.id === reading.id;
        ...
        function resolveReadingSource(options) {
          ...
          var hasScope = opts.unitId !== void 0 && opts.unitId !== "all";
          var scope = normalizeReadingUnitId(opts.unitId);
          var candidates = items.filter(function(item) {
            return item && ["analysis", "simplified"].indexOf(item.type) !== -1 && readableArtifactText(item).trim() && (!hasScope || readingUnit(item) === scope);
          }).map(function(item) {
            var profile = getInstructionalText(item);
            return {
              id: item.id,
              ...
              profile,
              sourceFamilyId: getReadingSourceFamilyId(item),
              eligible: (profile.form !== "same-text-supported" || isSupportedOriginal(item)) && (profile.form !== "adapted" || profile.role !== "primary" || isAuthorizedAdaptedPrimary(item))
            };
          });
          function resolved(artifact, selection, textOverride, suppliedSnapshot) {
            var text = textOverride !== void 0 ? textOverride : readableArtifactText(artifact);
            ...
            var snapshot2 = suppliedSnapshot || artifact && getSourceSnapshot(artifact);
            if (artifact && artifact.type === "analysis" && profile.form !== "adapted" && (!snapshot2 || snapshot2.text !== text || !snapshot2.sourceArtifactId)) {
              snapshot2 = createSourceSnapshot(text, { sourceArtifactId: artifact.id, language: artifact.config && artifact.config.language || opts.language, selection: "saved-analysis" });
            }
            if (!artifact && !snapshot2 && profile.form !== "adapted") snapshot2 = createSourceSnapshot(text, { language: opts.language, selection });
            var unitId = artifact ? readingUnit(artifact) : scope;
            var sourceRole = opts.sourceInstructionalText ? normalizeSourceInstructionalText(opts.sourceInstructionalText) : artifact ? getSourceInstructionalText(artifact) : profile.form === "adapted" ? getSourceInstructionalText({ type: "simplified", instructionalText: profile, sourceSnapshot: snapshot2 }) : normalizeSourceInstructionalText(profile);
            var familyId = cleanText(opts.sourceFamilyId || artifact && getReadingSourceFamilyId(artifact) || snapshot2 && snapshot2.sourceArtifactId || snapshot2 && "input-" + (unitId || "uncategorized") + "-" + snapshot2.fingerprint, 200) || null;
            return {
              status: "resolved",
              selection,
              text,
              ...
              sourceArtifactId: snapshot2 ? snapshot2.sourceArtifactId : null,
              sourceFamilyId: familyId,
              unitId,
              instructionalText: profile,
              sourceInstructionalText: sourceRole,
              sourceSnapshot: snapshot2 || null,
              candidates
            };
          }
          if (opts.sourceArtifactId === "__input__") {
            if (typeof opts.inputText !== "string" || !opts.inputText.trim()) return { status: "missing", reason: "empty-input", candidates, text: "" };
            return resolved(null, "explicit-input", opts.inputText, getSourceSnapshot(opts.sourceSnapshot));
          }
          var explicit = opts.selected || (opts.sourceArtifactId != null && String(opts.sourceArtifactId) ? candidates.find(function(candidate) {
            return String(candidate.id) === String(opts.sourceArtifactId);
          }) : null);
          ...
          if (typeof opts.textOverride === "string") {
            if (!opts.textOverride.trim()) return { status: "missing", reason: "empty-override", candidates, text: "" };
          ...
          if (opts.selected || opts.sourceArtifactId != null && String(opts.sourceArtifactId)) {
            if (!explicit || !readableArtifactText(explicit).trim()) return { status: "missing", reason: "selected-source-unavailable", candidates, text: "" };
            if (getInstructionalText(explicit).form === "same-text-supported" && !isSupportedOriginal(explicit)) {
              return { status: "missing", reason: "unverified-original", candidates, text: "" };
            }
            if (getInstructionalText(explicit).form === "adapted" && getInstructionalText(explicit).role === "primary" && !isAuthorizedAdaptedPrimary(explicit)) {
              return { status: "missing", reason: "unauthorized-adapted-main", candidates, text: "" };
            }
          ...
          if (!hasScope && new Set(eligible.map(function(candidate) {
            return candidate.unitId;
          })).size > 1) {
            return { status: "ambiguous", reason: "choose-lesson-source", candidates, text: "" };
          }
          var mains = eligible.filter(function(candidate) {
            return usablePrimary(candidate.artifact);
          });
          var pool = mains.length ? mains : eligible.filter(function(candidate) {
            return getInstructionalText(candidate.artifact).role !== "primary";
          });
          var distinct = [];
          ...
          return { status: distinct.length ? "ambiguous" : "missing", reason: distinct.length ? "choose-source" : "no-readable-source", candidates, text: "" };
        }
        function summarizeReadingAccess(items, options) {
          var opts = isObject(options) ? options : {};
          var source2 = Array.isArray(items) ? items.filter(isObject) : [];
          var readings = opts.includeSourcePairs === true ? ensureReadingSourcePairs(source2, opts) : source2;
          var primary = readings.filter(usablePrimary);
          var supplemental = readings.filter(function(item) {
            return getInstructionalText(item).role === "supplemental" && readableArtifactText(item).trim();
          });
          var adaptations = readings.filter(function(item) {
            return ["analysis", "simplified"].indexOf(item.type) !== -1 && getInstructionalText(item).form === "adapted";
          });
          var unspecified = adaptations.filter(function(item) {
            return getInstructionalText(item).role === "unspecified";
          });
          var unauthorized = adaptations.filter(function(item) {
            return getInstructionalText(item).role === "primary" && !isAuthorizedAdaptedPrimary(item);
          });
          var missingSource = adaptations.filter(function(item) {
            var snapshot2 = getSourceSnapshot(item);
            return !snapshot2 || !readings.some(function(candidate) {
              return candidate !== item && sameReadingSourceFamily(candidate, item) && getInstructionalText(candidate).form !== "adapted" && readableArtifactText(candidate) === snapshot2.text && (candidate.type === "analysis" || isSupportedOriginal(candidate));
            });
          });
          var missingPrimary = adaptations.filter(function(item) {
            return !primary.some(function(candidate) {
              return candidate === item || sameReadingSourceFamily(candidate, item);
            });
          });
          return {
            items: readings,
            primary,
            supplemental,
            unspecifiedAdapted: unspecified,
            unauthorizedPrimaryAdaptations: unauthorized,
            missingSourceCompanions: missingSource,
            missingPrimaryCompanions: missingPrimary,
            hasPrimary: primary.length > 0,
            hasSupplementalWithoutPrimary: supplemental.some(function(item) {
              return !primary.some(function(candidate) {
                return sameReadingSourceFamily(candidate, item);
              });
            })
          };
        }
        ...
        function readingSupportScope(value) {
          ...
          var snapshot2 = getSourceSnapshot(value);
          return {
            sourceFamilyId: getReadingSourceFamilyId(value),
            unitId: readingUnit(value),
            ownerArtifactId: value.id == null ? null : String(value.id),
            sourceFingerprint: snapshot2 && snapshot2.fingerprint,
            preserved: isSupportedOriginal(value)
          };
        }
        function readingSupportScopeMismatch(scope, envelope) {
          ...
          var samePreservedOwner = scope.preserved && scope.ownerArtifactId && envelope.ownerArtifactId && String(envelope.ownerArtifactId) === scope.ownerArtifactId && envelope.sourceFingerprint === scope.sourceFingerprint && envelope.sourceFamilyId && String(envelope.sourceFamilyId) === String(scope.sourceFamilyId);
          return !!familyMismatch || Object.prototype.hasOwnProperty.call(envelope, "unitId") && normalizeReadingUnitId(envelope.unitId) !== scope.unitId && !samePreservedOwner;
        }
        ...
        function validSupportAnchor(snapshot2, entry) {
          return !!snapshot2 && isObject(entry) && Number.isInteger(entry.start) && Number.isInteger(entry.end) && entry.start >= 0 && entry.end > entry.start && entry.end <= snapshot2.text.length && isSourceTextBoundary(snapshot2.text, entry.start) && isSourceTextBoundary(snapshot2.text, entry.end) && typeof entry.quote === "string" && snapshot2.text.slice(entry.start, entry.end) === entry.quote;
        }
        function validateReadingSupports(snapshotValue, candidates) {
          ...
          var invalidSource = !snapshot2 || envelope.schemaVersion !== void 0 && envelope.schemaVersion !== 1 || envelope.sourceFingerprint && envelope.sourceFingerprint !== snapshot2.fingerprint || readingSupportScopeMismatch(scope, envelope);
          ...
          (Array.isArray(envelope.suppressedAnnotations) ? envelope.suppressedAnnotations : []).forEach(function(entry) {
            if (invalidSource || !validSupportAnchor(snapshot2, entry)) {
              rejectedCount++;
              return;
            }
            if (!suppressedAnnotations.some(function(current) {
              return sameSupportRange(current, entry);
            })) {
              suppressedAnnotations.push({ start: entry.start, end: entry.end, quote: entry.quote });
            }
          });
          entries.forEach(function(entry) {
            var valid = !invalidSource && validSupportAnchor(snapshot2, entry) && typeof entry.text === "string" && !!entry.text.trim() && entry.text.length <= 2400 && ["gloss", "definition", "explanation"].indexOf(entry.kind || "gloss") !== -1;
            var id = valid ? cleanText(entry.id, 160) || "support-" + entry.start + "-" + entry.end : "";
            if (valid && suppressedAnnotations.some(function(removed) {
              return overlappingSupportRanges(removed, entry);
            })) return;
            if (valid && (ids[id] || annotations.some(function(annotation) {
              return overlappingSupportRanges(entry, annotation);
            }))) valid = false;
            if (!valid) {
              rejectedCount++;
              return;
          ...
          suppressedAnnotations.sort(function(left, right) {
            return left.start - right.start;
          });
          ...
          function validatedRanges(value) {
            ...
            value.forEach(function(entry) {
              ...
              if (valid && seenCoverage.some(function(range2) {
                return overlappingSupportRanges(entry, range2);
              })) valid = false;
              if (!valid) {
        ...
        function requireReadingSupportState(snapshotValue, supports) {
          ...
          if (!snapshot2) throw new Error("A complete saved source is required to edit word supports.");
          if (envelope.schemaVersion !== void 0 && envelope.schemaVersion !== 1 || envelope.sourceFingerprint && envelope.sourceFingerprint !== snapshot2.fingerprint || readingSupportScopeMismatch(readingSupportScope(snapshotValue), envelope)) {
        ...
        function upsertReadingSupport(snapshotValue, supports, annotation) {
          ...
          var existing = current.annotations.find(function(entry) {
            return annotation.id ? entry.id === annotation.id : sameSupportRange(entry, annotation);
          });
          ...
          var others = current.annotations.filter(function(entry) {
            return !existing || entry.id !== existing.id;
          });
          if (others.some(function(entry) {
            return entry.id === proposed.id || overlappingSupportRanges(entry, proposed);
          })) {
          ...
          return validateReadingSupports(snapshotValue, Object.assign({}, current, {
            ...
            suppressedAnnotations: current.suppressedAnnotations.filter(function(entry) {
              return !overlappingSupportRanges(entry, proposed);
            }),
        ...
        function removeReadingSupport(snapshotValue, supports, id) {
          var current = requireReadingSupportState(snapshotValue, supports);
          var removed = current.annotations.find(function(entry) {
            return entry.id === id;
          });
          if (!removed) throw new Error("That word support is no longer available. Reopen the support list.");
          return validateReadingSupports(snapshotValue, Object.assign({}, current, {
            annotations: current.annotations.filter(function(entry) {
              return entry.id !== id;
            }),
        ...
        function setReadingSupportPinned(snapshotValue, supports, id, pinned) {
          var current = requireReadingSupportState(snapshotValue, supports);
          var selected = current.annotations.find(function(entry) {
            return entry.id === id;
          });
          if (!selected) throw new Error("That word support is no longer available. Reopen the support list.");
          return validateReadingSupports(snapshotValue, Object.assign({}, current, {
            annotations: current.annotations.map(function(entry) {
              return entry.id === id ? Object.assign({}, entry, { pinned: pinned === true }) : entry;
            })
          }));
        }
        function mergeReadingSupports(snapshotValue, supports, regenerated) {
          ...
          var retained = current.annotations.filter(function(entry) {
            return entry.origin === "educator" || entry.pinned || incoming.status === "unavailable" || incoming.skippedRanges.some(function(range) {
              return overlappingSupportRanges(range, entry);
            });
          });
          var generated = incoming.annotations.filter(function(entry) {
            return !retained.some(function(saved) {
              return overlappingSupportRanges(saved, entry);
            }) && !current.suppressedAnnotations.some(function(removed) {
              return overlappingSupportRanges(removed, entry);
            });
          }).map(function(entry) {
            return Object.assign({}, entry, { origin: "generated", pinned: false });
          });
          return validateReadingSupports(snapshotValue, Object.assign({}, incoming, {
            annotations: retained.concat(generated),
            suppressedAnnotations: current.suppressedAnnotations,
        ...
        function selectReadingSupports(snapshotValue, supports, options) {
          ...
          var breaks = /(?:\r\n|\n|\r(?!\n))[\t ]*(?:\r\n|\n|\r(?!\n))/g, match;
          while (match = breaks.exec(snapshot2.text)) {
            paragraphs.push({ start: paragraphStart, end: match.index });
            paragraphStart = match.index + match[0].length;
          }
          paragraphs.push({ start: paragraphStart, end: snapshot2.text.length });
          var selected = checked.annotations.filter(function(entry) {
            return entry.pinned;
          });
          paragraphs.forEach(function(paragraph) {
            ...
            var text = snapshot2.text.slice(paragraph.start, paragraph.end);
            if (segmenter) for (var word of segmenter.segment(text)) {
              if (word.isWordLike) words.push(paragraph.start + word.index);
            }
            else {
              var pattern = /[\p{L}\p{M}\p{N}]+(?:[’'-][\p{L}\p{M}\p{N}]+)*/gu, found;
              while (found = pattern.exec(text)) words.push(paragraph.start + found.index);
            }
            function wordIndex(offset) {
              var index = 0;
              while (index < words.length && words[index] < offset) index++;
              return index;
            }
            var maximum = Math.max(1, Math.min(3, Math.ceil(words.length / 55)));
            var inParagraph = function(entry) {
              return entry.start >= paragraph.start && entry.start < paragraph.end;
            };
            var candidates = checked.annotations.filter(function(entry) {
              return !entry.pinned && inParagraph(entry);
            });
            candidates.sort(function(left, right) {
              return Number(right.priority === "essential") - Number(left.priority === "essential") || Number(right.origin === "educator") - Number(left.origin === "educator") || left.start - right.start;
            });
        ...
        var API = {
          ...
          getSourceSnapshot,
          normalizeReadingUnitId,
          normalizeSourceInstructionalText,
          getSourceInstructionalText,
          getReadingSourceFamilyId,
          sameReadingSourceFamily,
          sameReadingFamily: sameReadingSourceFamily,
          getReadingRoleLabel,
          updateInstructionalRole,
          updateReadingFamilyRole,
          resolveReadingSource,
          summarizeReadingAccess,
          createSupportedReading,
          isSupportedOriginal,
          getReadingArtifactLabel,
          ensureReadingSourcePairs,
          validateReadingSupports,
          upsertReadingSupport,
          removeReadingSupport,
          setReadingSupportPinned,
          mergeReadingSupports,
          selectReadingSupports,
          fingerprintValue,
  ...
  var require_view_simplified_module = __commonJS({
    "view_simplified_module.js"() {
      (function() {
        ...
        function findSimplifiedReadingSupportOwner(history, owner, snapshot2) {
          var api2 = getInstructionalContextApi();
          if (!owner || !snapshot2 || !api2?.sameReadingSourceFamily || !api2?.isSupportedOriginal || !api2?.getSourceSnapshot) return null;
          if (getSimplifiedInstructionalText(owner).form === "same-text-supported" && !api2.isSupportedOriginal(owner)) return null;
          var candidates = [owner].concat((Array.isArray(history) ? history : []).slice().reverse());
          return candidates.find(function(item) {
            return api2.isSupportedOriginal(item) && api2.sameReadingSourceFamily(item, owner) && item.data === snapshot2.text && sameSimplifiedReadingSnapshot(api2.getSourceSnapshot(item), snapshot2);
          }) || null;
        }
        function findSimplifiedComparisonSupportOwner(history, current, comparison, displayedText) {
          var api2 = getInstructionalContextApi();
          if (!api2 || !comparison || typeof displayedText !== "string") return null;
          var owner = comparison.selection === "captured-source" ? current : comparison.artifact;
          if (!owner) return null;
          var snapshot2 = api2.getSourceSnapshot?.(owner);
          if ((!snapshot2 || snapshot2.text !== comparison.text) && getSimplifiedInstructionalText(owner).form !== "adapted") {
            snapshot2 = api2.createSourceSnapshot?.(comparison.text, {
              sourceArtifactId: owner.id
            });
          }
          if (!snapshot2 || snapshot2.text !== comparison.text || snapshot2.text !== displayedText) return null;
          return findSimplifiedReadingSupportOwner(history, owner, snapshot2);
        }
        function findSimplifiedReadingCompanion(history, original2, snapshot2) {
          var api2 = getInstructionalContextApi();
          if (!original2 || !snapshot2 || !api2?.sameReadingSourceFamily || !api2?.getSourceSnapshot) return null;
          if (getSimplifiedInstructionalText(original2).form === "same-text-supported" && !api2.isSupportedOriginal?.(original2)) return null;
          return (Array.isArray(history) ? history : []).slice().reverse().find(function(item) {
            return item?.type === "simplified" && getSimplifiedInstructionalText(item).form === "adapted" && api2.sameReadingSourceFamily(item, original2) && sameSimplifiedReadingSnapshot(api2.getSourceSnapshot(item), snapshot2);
          }) || null;
        }
        ...
        function findReadingGlossOccurrences(text, query) {
          ...
          while (cursor < text.length && matches.length < 200) {
            ...
            matches.push({
              ...
              context: text.slice(Math.max(0, start - 35), Math.min(text.length, end + 45)).replace(/\s+/g, " ").trim()
            });
            cursor = start + 1;
          }
          return matches;
        ...
        function ReadingGlossEditor({
          ...
          const snapshot2 = getInstructionalContextApi()?.getSourceSnapshot?.(item);
          const sourceText = snapshot2?.text || "";
          const sourceKey = String(item?.id || "") + ":" + (snapshot2?.fingerprint || "");
          const [open, setOpen] = React3.useState(false);
          const [draft, setDraft] = React3.useState(null);
          const [notice, setNotice] = React3.useState("");
          const [error, setError] = React3.useState("");
          const [busy, setBusy] = React3.useState(false);
          const sourceKeyRef = React3.useRef(sourceKey);
          sourceKeyRef.current = sourceKey;
          const termRef = React3.useRef(null), definitionRef = React3.useRef(null), addRef = React3.useRef(null);
          const editRefs = React3.useRef({});
          ...
          const matches = React3.useMemo(() => {
            const entry = draft?.id && entries.find((value) => value.id === draft.id);
            return entry ? [{
              ...entry,
              context: sourceText.slice(Math.max(0, entry.start - 35), Math.min(sourceText.length, entry.end + 45)).replace(/\s+/g, " ").trim()
            }] : findReadingGlossOccurrences(sourceText, draft?.query || "");
          ...
          const updateDraft = (fields) => setDraft((previous) => ({
            ...previous,
            ...fields
          }));
          ...
          const save = async () => {
            ...
            const annotation = {
              ...selected,
              id: draft.id || "gloss-" + selected.start + "-" + selected.end,
            ...
            if (saved) setDraft((previous) => previous && {
              ...previous,
              id: annotation.id,
          ...
          const finish = () => {
            ...
            setDraft(null);
            setError("");
            (editRefs.current[id] || addRef.current)?.focus();
          };
          return /* @__PURE__ */ React3.createElement("section", {
          ...
          }, "Review word supports ", entries.length ? "(" + entries.length + ")" : ""), open && /* @__PURE__ */ React3.createElement("div", {
            id: panelId,
            className: "mt-2 space-y-3"
          }, /* @__PURE__ */ React3.createElement("p", {
            className: "text-sm leading-relaxed text-slate-600"
          }, "Add or adjust explanations beside the original words. Teacher edits and removed supports are kept when AI suggestions are refreshed."), /* @__PURE__ */ React3.createElement("button", {
            ref: addRef,
            type: "button",
          ...
          }, "Add a word or phrase"), draft && /* @__PURE__ */ React3.createElement("div", {
            "data-gloss-draft": true,
            role: "group",
            "aria-label": draft.id ? "Edit word support" : "Add word support",
            className: "space-y-3 rounded-lg border border-indigo-200 bg-indigo-50 p-3"
          }, /* @__PURE__ */ React3.createElement("label", {
            className: "block font-semibold"
          }, "Word or phrase from the original", /* @__PURE__ */ React3.createElement("input", {
            ref: termRef,
            type: "text",
            maxLength: 160,
            value: draft.query,
          ...
          })), !draft.id && matches.length > 1 && /* @__PURE__ */ React3.createElement("label", {
            className: "block font-semibold"
          }, "Which occurrence?", /* @__PURE__ */ React3.createElement("select", {
            value: draft.start,
            onChange: (event) => updateDraft({
            ...
            value: ""
          }, "Choose the word in context"), matches.map((match, index) => /* @__PURE__ */ React3.createElement("option", {
            key: match.start,
            value: String(match.start)
          }, index + 1, ". ", match.context)))), !!draft.query.trim() && !matches.length && /* @__PURE__ */ React3.createElement("p", {
            role: "status",
            className: "text-amber-900"
          }, "No exact match. Copy the word or phrase as it appears in the original."), selected && /* @__PURE__ */ React3.createElement("p", {
            className: "break-words text-slate-600"
          }, /* @__PURE__ */ React3.createElement("span", {
            className: "font-semibold"
          }, "In context: "), selected.context), /* @__PURE__ */ React3.createElement("label", {
            className: "block font-semibold"
          }, "Explanation", /* @__PURE__ */ React3.createElement("textarea", {
            ref: definitionRef,
            rows: 3,
            maxLength: 600,
            value: draft.text,
            ...
            className: "block font-semibold"
          }, "Importance", /* @__PURE__ */ React3.createElement("select", {
            ...
            value: "essential"
          }, "Essential to understanding this passage"), /* @__PURE__ */ React3.createElement("option", {
            value: "helpful"
          }, "Helpful extra explanation"))), /* @__PURE__ */ React3.createElement("label", {
          ...
          }), "Always show in lighter view"), /* @__PURE__ */ React3.createElement("div", {
          ...
          }, busy ? "Saving\u2026" : "Save word support"), /* @__PURE__ */ React3.createElement("button", {
            type: "button",
            disabled: busy,
            onClick: finish,
            className: "min-h-11 rounded-lg border border-slate-300 px-3"
          }, "Done editing"))), error && /* @__PURE__ */ React3.createElement("p", {
            role: "alert",
            className: "rounded bg-red-50 p-2 text-red-900"
          }, error), notice && /* @__PURE__ */ React3.createElement("p", {
            role: "status",
            className: "rounded bg-indigo-50 p-2 text-indigo-900"
          }, notice), !entries.length && !draft && /* @__PURE__ */ React3.createElement("p", {
            className: "text-slate-600"
          }, "No word supports yet. Add your own explanation or generate suggestions."), /* @__PURE__ */ React3.createElement("ul", {
            className: "space-y-2"
          }, entries.map((entry) => /* @__PURE__ */ React3.createElement("li", {
            key: entry.id,
            className: "rounded-lg border border-slate-200 p-3"
          }, /* @__PURE__ */ React3.createElement("p", {
            className: "break-words"
          }, /* @__PURE__ */ React3.createElement("strong", null, entry.quote), " \u2014 ", entry.definition || entry.explanation || entry.text), /* @__PURE__ */ React3.createElement("p", {
            className: "mt-1 text-xs text-slate-600"
          }, entry.origin === "educator" ? "Teacher edited" : "Suggested", entry.priority === "essential" ? " \xB7 Essential" : ""), /* @__PURE__ */ React3.createElement("div", {
            ...
            type: "button",
            disabled: disabled || busy,
            "aria-label": "Edit gloss for " + entry.quote,
            onClick: () => beginEdit(entry),
            className: "min-h-11 rounded-lg border border-indigo-300 px-3 text-indigo-900"
          }, "Edit"), /* @__PURE__ */ React3.createElement("button", {
            type: "button",
            disabled: disabled || busy,
            "aria-label": "Remove gloss for " + entry.quote,
            onClick: async () => {
              const saved = await mutate({
                type: "remove",
                id: entry.id
            ...
            className: "min-h-11 rounded-lg border border-slate-300 px-3"
          }, "Remove"), /* @__PURE__ */ React3.createElement("label", {
            className: "flex min-h-11 items-center gap-2"
          }, /* @__PURE__ */ React3.createElement("input", {
            type: "checkbox",
            "aria-label": "Always show gloss for " + entry.quote + " in lighter view",
            ...
            onChange: (event) => mutate({
              ...
              id: entry.id,
              pinned: event.target.checked
            }, event.target.checked ? "This support will stay visible in lighter view." : "This support now follows the lighter-view selection.")
        ...
        function SimplifiedView(props) {
          ...
          var instructionalRoleControl = generatedContent ? /* @__PURE__ */ React3.createElement("div", {
          ...
          }, /* @__PURE__ */ React3.createElement("strong", null, instructionalFormLabel), /* @__PURE__ */ React3.createElement("span", {
            "aria-hidden": "true"
          }, "\xB7"), /* @__PURE__ */ React3.createElement("span", null, instructionalRoleLabel)), isTeacherMode && /* @__PURE__ */ React3.createElement("div", {
            className: "mt-3 flex flex-wrap items-center gap-3"
          }, /* @__PURE__ */ React3.createElement("label", {
            className: "inline-flex flex-wrap items-center gap-2"
          }, "Use in this lesson", /* @__PURE__ */ React3.createElement("select", {
            ...
            value: "primary"
          }, "Main reading"), /* @__PURE__ */ React3.createElement("option", {
            value: "supplemental"
          }, "Supporting reading"), /* @__PURE__ */ React3.createElement("option", {
            value: "unspecified"
          }, "Not designated"))), instructionalRole === "primary" && instructionalTextProfile.form === "adapted" && !replacementIsEducatorAuthorized && /* @__PURE__ */ React3.createElement("button", {
            type: "button",
          ...
          }, "Review main-reading choice"), props.onSelectReadingSource && (!protectedOriginal || verifiedOriginal) && /* @__PURE__ */ React3.createElement("button", {
            type: "button",
            disabled: isProcessing,
            onClick: () => props.onSelectReadingSource(generatedContent),
            className: "min-h-11 rounded-lg border border-indigo-300 px-3 text-indigo-900"
          }, "Use for activities")), !protectedOriginal && instructionalRole !== "primary" && /* @__PURE__ */ React3.createElement("p", {
            className: "mt-2"
          ...
          var [glossEditorRequest, setGlossEditorRequest] = React3.useState(null);
          ...
          function renderSimplifiedComparison() {
            ...
            var comparisonSupportOwner = findSimplifiedComparisonSupportOwner(history, generatedContent, original2, sourceText);
            ...
            }, part.value)) : renderExactPassage(raw, kind, kind === "source" ? originalLanguage : targetLanguage, kind === "source" ? comparisonGlosses : [], kind === "source" ? comparisonSupportOwner : null);
            return /* @__PURE__ */ React3.createElement("div", {
              ...
              type: "button",
              className: "min-h-11 rounded border border-indigo-300 px-3 text-indigo-900",
              onClick: () => props.onReadOriginal(selected ? original2.artifact : capturedSource ? generatedContent : original2.artifact)
            ...
            }, renderVersion(version.key, version.text)), version.key === "source" && isTeacherMode && comparisonSupportOwner && props.onUpdateReadingSupports && /* @__PURE__ */ React3.createElement(ReadingGlossEditor, {
          ...
          function renderExactPassage(raw, pane, language, annotations, annotationOwner) {
            var cursor = 0;
            return String(raw).split(/(\r\n|\r|\n)/).map(function(line, lineIndex) {
              ...
              var content = parts.map(function(part, index) {
                ...
                }, " (", entry.definition || entry.explanation || entry.text, ")", isTeacherMode && annotationOwner && props.onUpdateReadingSupports && /* @__PURE__ */ React3.createElement("button", {
                  type: "button",
          ...
          var companion = !protectedOriginal ? generatedContent : findSimplifiedReadingCompanion(history, generatedContent, capturedSource);
          ...
          function renderOriginalReading() {
            ...
            }, glossBusy ? "Adding word supports\u2026" : checkedSupports?.annotations?.length ? "Refresh suggested supports" : "Add word supports")), glossNotice && /* @__PURE__ */ React3.createElement("p", {
              role: "status",
              className: "my-3 rounded bg-indigo-50 p-3 text-indigo-900"
            }, glossNotice), isTeacherMode && verifiedOriginal && props.onUpdateReadingSupports && /* @__PURE__ */ React3.createElement(ReadingGlossEditor, {
...
${t("analysis.readability.syllables") || "Syllables"}: ${stats.syllables || "\u2014"}`
  ...
  // reports/novak-gloss-enhancements/curation-fixture.jsx
  ...
  function App() {
    const [history, setHistory] = import_react.default.useState(() => JSON.parse(localStorage.getItem("novak-curation-fixture") || "null") || [original, adapted]);
    const historyRef = import_react.default.useRef(history);
    historyRef.current = history;
    const [itemId, setItemId] = import_react.default.useState(original.id);
    const item = history.find((row) => row.id === itemId);
    const setItem = (next) => setItemId(next.id);
    import_react.default.useEffect(() => localStorage.setItem("novak-curation-fixture", JSON.stringify(history)), [history]);
    window.fixture.history = history;
    const commit = (id, update) => {
      let updated;
      const next = historyRef.current.map((row) => {
    ...
    const onUpdateReadingSupports = (target, action) => {
      const updated = commit(target.id, (row) => {
        const supports = action.type === "upsert" ? api.upsertReadingSupport(row, row.readingSupports, action.annotation) : action.type === "remove" ? api.removeReadingSupport(row, row.readingSupports, action.id) : api.setReadingSupportPinned(row, row.readingSupports, action.id, action.pinned);
        return { ...row, readingSupports: supports };
      });
      return updated.readingSupports;
    };
    const onGenerateReadingSupports = async (target) => {
      await new Promise((resolve) => {
        window.fixture.finishGeneration = resolve;
      });
      const rest = source.indexOf("restless"), persist = source.indexOf("persistent");
      const incoming = api.validateReadingSupports(target, [{ id: "generated-restless", start: rest, end: rest + 8, quote: "restless", text: "moving without settling", priority: "helpful" }, { id: "generated-persistent", start: persist, end: persist + 10, quote: "persistent", text: "AI replacement should not overwrite teacher wording", priority: "essential" }]);
      return commit(target.id, (row) => ({ ...row, readingSupports: api.mergeReadingSupports(row, row.readingSupports, incoming) })).readingSupports;
    };
    const [compare, setCompare] = import_react.default.useState(false), [mode, setMode] = import_react.default.useState("read"), [playing, setPlaying] = import_react.default.useState(false), [playingId, setPlayingId] = import_react.default.useState(null), [theme, setTheme] = import_react.default.useState("light"), [focus, setFocus] = import_react.default.useState(false), [focused, setFocused] = import_react.default.useState(null);
    const stop = () => {
      setPlaying(false);
      setPlayingId(null);
    };
    ...
    return import_react.default.createElement("main", { className: "max-w-6xl mx-auto p-4" }, import_react.default.createElement("h1", { className: "text-2xl font-bold mb-3" }, "The River \u2014 Word supports"), import_react.default.createElement(View, props));
  }
