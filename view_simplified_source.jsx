    ...
    return <div className="space-y-6">{activeReadAloudStatus && <span className="sr-only" role="status" aria-live="polite" aria-atomic="true">{activeReadAloudStatus}</span>}{isImmersiveReaderActive && generatedContent?.immersiveData && <div ref={immersiveDialogRef} role="dialog" aria-modal="true" aria-label={t('immersive.title') || 'Immersive Reader'} tabIndex={-1} onKeyDown={e => containSimplifiedModalFocus(e, immersiveDialogRef.current, handleCloseImmersiveReader)} className="fixed inset-0 z-[200] overflow-y-auto animate-in motion-reduce:animate-none fade-in zoom-in-95 duration-300 motion-reduce:animate-none motion-reduce:transition-none flex flex-col font-sans" style={{
        backgroundColor: immersiveSettings.bgColor || '#fdfbf7'
          ...
          left: Math.min(window.innerWidth - 300, Math.max(20, revisionData.x - 140)) + 'px'
            ...
            {generatedContent.localStats && !generatedContent.levelCheck && (() => {
              ...
              const GRADE_NUMBERS = { 'Kindergarten': 0, '1st Grade': 1, '2nd Grade': 2, '3rd Grade': 3, '4th Grade': 4, '5th Grade': 5, '6th Grade': 6, '7th Grade': 7, '8th Grade': 8, '9th Grade': 9, '10th Grade': 10, '11th Grade': 11, '12th Grade': 12 };
              ...
              const tone = gap === null ? 'bg-slate-50 border-slate-200 text-slate-700'
                : isOver ? 'bg-amber-50 border-amber-200 text-amber-900'
                : isUnder ? 'bg-blue-50 border-blue-200 text-blue-900'
                : 'bg-green-50 border-green-200 text-green-900';
              const verdict = gap === null ? ''
                : isOver ? (t('simplified.measured_above', { grade: targetGrade }) || `Above the ${targetGrade} target`)
                : isUnder ? (t('simplified.measured_below', { grade: targetGrade }) || `Below the ${targetGrade} target`)
                : (t('simplified.measured_on_target', { grade: targetGrade }) || `On target for ${targetGrade}`);
              return <div className={`mb-4 flex flex-wrap items-center gap-x-3 gap-y-1 rounded-lg border px-3 py-2 text-xs ${tone}`}>
                <span className="font-bold uppercase tracking-wider">{t('simplified.measured_level_label') || 'Measured reading level'}</span>
                <span className="font-mono font-bold text-sm" title={`${t('analysis.readability.formula') || 'Flesch-Kincaid'}: (0.39 × ASL) + (11.8 × ASW) - 15.59\n${t('analysis.readability.words') || 'Words'}: ${generatedContent.localStats.words}\n${t('analysis.readability.sentences') || 'Sentences'}: ${generatedContent.localStats.sentences}\n${t('analysis.readability.syllables') || 'Syllables'}: ${generatedContent.localStats.syllables}`}>{generatedContent.localStats.score}</span>
                {verdict && <span className="font-semibold">{verdict}</span>}
                <span className="text-[11px] opacity-80">{t('simplified.measured_note') || 'Flesch-Kincaid, measured on this passage. Use Check Level for a fuller review.'}</span>
              </div>;
