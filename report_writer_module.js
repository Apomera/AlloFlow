...
function classifyScore(score, scoreType) {
  const bands = (scoreType === 't_score') ? T_SCORE_BANDS : STANDARD_SCORE_BANDS;
...
function scoreToPercentile(score, scoreType) {
  let mean, sd;
  if (scoreType === 't_score') { mean = 50; sd = 10; }
  else { mean = 100; sd = 15; }
...
function normalizeScoreEntry(raw) {
  ...
  const scoreType = raw.score_type || raw.scoreType || 'standard';
  return {
...
{"consistent": true/false, "issues": [{"type": "summary-mismatch|recommendation-gap|contradiction|terminology|completeness", "description": "specific issue", "sections": ["Section A", "Section B"], "severity": "critical|moderate|minor"}]}`, true);
        ...
        const handleImportDiscrepancyReport = (event) => {
            ...
            reader.onload = (ev) => {
                try {
                    ...
                    // Audit fix #3 (symmetric): warn if we're overwriting
                    // an inline result with an imported one.
        ...
        const verifyWithPsycheck = useCallback(() => {
            ...
            const sources = validScores.map(s => ({
                ...
                subtest: s.subtest,
                score: s.score,
                scoreType: s.scoreType || s.score_type || 'standard',
        ...
        }, [scoreEntries, reportSections, addToast, studentName, generating, regenSection, adaptingSection, editingSection, discrepancyReport]);
