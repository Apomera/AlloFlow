...
const slug='sound_vibration_grade1';
const panelCount=8;
const root=path.resolve(__dirname,'..'),out=path.join(root,'scratch',slug+'-qa');
const read=p=>fs.readFileSync(path.join(root,p),'utf8');
const packPath='allopacks/illustrated/'+slug+'.allopack.json';
...
const entry={slug:slug+'_illustrated',title:pack.allopack.title,subject:'Science',grade_level:pack.allopack.gradeLevel,tags:['science','illustrated'],license:'CC-BY-4.0',credit:pack.allopack.author,path:packPath};
...
await page.setContent('<html lang="en"><head><title>Illustrated AlloPack integration QA</title></head><body><div id="root"></div><div id="panels"></div></body></html>');
...
const download=await downloadPromise;const saved=path.join(out,slug+'-downloaded.json');await download.saveAs(saved);
...
const urls=h.flatMap(r=>r.type==='glossary'?r.data.map(g=>g.image):r.type==='image'?r.data.visualPlan.panels.map(p=>p.imageUrl):r.type==='anchor-chart'?r.data.sections.map(s=>s.iconUrl):r.type==='concept-sort'?r.data.items.map(s=>s.image):[]);
...
assert.equal(decoded,25);
await page.waitForFunction(count=>document.querySelectorAll('#panels img').length>=count,panelCount);
...
const labelBounds=await page.locator('.visual-label').evaluateAll(ls=>ls.map(l=>{const a=l.getBoundingClientRect(),p=l.parentElement.getBoundingClientRect();return {text:l.textContent,inside:a.left>=p.left&&a.right<=p.right&&a.top>=p.top&&a.bottom<=p.bottom};}));
assert(labelBounds.every(l=>l.inside),JSON.stringify(labelBounds));
const overlaps=await page.locator('.visual-label').evaluateAll(ls=>ls.flatMap((a,i)=>ls.slice(i+1).filter(b=>{if(a.parentElement!==b.parentElement)return false;const x=a.getBoundingClientRect(),y=b.getBoundingClientRect();return Math.min(x.right,y.right)>Math.max(x.left,y.left)&&Math.min(x.bottom,y.bottom)>Math.max(x.top,y.top);}).map(b=>[a.textContent,b.textContent])));
assert.deepEqual(overlaps,[],'Native labels must not overlap on mobile');
await page.screenshot({path:path.join(out,'native-panels-mobile.png'),fullPage:true});
assert.deepEqual(errors,[]);
const report={passed:true,imagesDecodedOffline:decoded,lessonPanelAltsVerified:panelCount,mobileLabelBoundsVerified:labelBounds.length,resources:pack.history.length,productionComponents:['CommunityCatalog','loadProjectFromJson bridge','MiscHandlers.handleLoadProject','hydrateHistory','VisualPanelGrid'],checks:['catalog load','actual catalog download','offline production-loader reopen','history byte/value preservation','all embedded image decoding','native panel alt attributes'],scope:'Component integration harness with local catalog responses; not a live deployment or full signed-in teacher session.'};
