...
(async () => {
 ...
 if(!await page.getByRole('button',{name:'Start Puzzle Challenge',exact:true}).evaluate(el=>el===document.activeElement))throw Error('Replay did not focus Start');
 await page.getByRole('button',{name:'Start Puzzle Challenge',exact:true}).click();
