...
var _lazyIcon = function (name) {
  ...
  function AssessmentPresentationItem(p) {
    ...
    if (type === 'multi-select') {
      var correctAnswers = Array.isArray(q.correctAnswers) ? q.correctAnswers : [];
      body = <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{(q.options || []).map(function (option, index) {
      ...
      answerGuide = <div>Teacher review</div>;
    }
  ...
  function QuizView(props) {
    ...
    React.useEffect(() => { if (!boardSetupOpen || boardSetupReady) return; window.__alloLazyLessonBoard?.(); const timer = setInterval(() => { if (window.AlloModules?.LessonBoardModule) setBoardSetupReady(true); }, 250); return () => clearInterval(timer); }, [boardSetupOpen, boardSetupReady]);
    ...
    React.useEffect(() => {
      ...
      const timer = setInterval(() => { if (window.AlloModules?.ConnectedEscapeRoomModule) setConnectedSetupReady(true); }, 250);
    ...
    return <div className="space-y-6">{boardSetupOpen && (isTeacherMode || !activeSessionCode) && (boardSetupReady && window.AlloModules?.LessonBoardSetup ? <window.AlloModules.LessonBoardSetup callGemini={props.callGemini} inputText={props.inputText} generatedContent={generatedContent} language={leveledTextLanguage === 'All selected languages' ? props.selectedLanguages?.[0] || 'English' : leveledTextLanguage || 'English'} activeSessionCode={activeSessionCode} appId={appId} sessionData={sessionData} user={props.user} allowLive={!!(isTeacherMode && activeSessionCode)} t={t} onClose={() => setBoardSetupOpen(false)} onLaunched={() => setEscapeRoomState(previous => ({ ...previous, isActive: false, isGenerating: false, isPreview: false }))}/> : <div role="status">{t('lesson_board.loading', { defaultValue: 'Loading the lesson board…' })}<button type="button" onClick={() => setBoardSetupOpen(false)}>{t('common.close')}</button></div>)}{connectedSetupOpen && (isTeacherMode || !activeSessionCode) && (connectedSetupReady && window.AlloModules?.ConnectedEscapeRoomSetup ? <window.AlloModules.ConnectedEscapeRoomSetup callGemini={props.callGemini} inputText={props.inputText} generatedContent={generatedContent} language={leveledTextLanguage === 'All selected languages' ? props.selectedLanguages?.[0] || 'English' : leveledTextLanguage || 'English'} activeSessionCode={activeSessionCode} appId={appId} sessionData={sessionData} user={props.user} allowLive={!!(isTeacherMode && activeSessionCode)} t={t} onClose={() => setConnectedSetupOpen(false)} onLaunched={() => setEscapeRoomState(previous => ({ ...previous, isActive: false, isGenerating: false, isPreview: false }))} /> : <div role="status" className="rounded-xl bg-indigo-50 p-4">{t('connected_escape.loading', { defaultValue: 'Loading the connected room…' })}<button type="button" onClick={() => setConnectedSetupOpen(false)}>{t('common.close')}</button></div>)}{classExplainerBanner}{modeBanner}{explainerPanel}{qualityReviewPanel}{deliverySettingsPanel}{draftStatusPanel}{learnerAttemptPanel}{reviewDialog}<div className="bg-teal-50 p-4 rounded-lg border border-teal-100 mb-6 flex justify-between items-center flex-wrap gap-3"><p className="text-sm text-teal-800 flex-grow"><strong>UDL Goal:</strong> Providing options for action and expression. Frequent formative assessments help track progress and adjust instruction.</p><div className="flex items-center gap-2 flex-wrap">{isTeacherMode && activeSessionCode && !sessionData?.quizState?.isActive && <><button type="button" aria-label={t('common.connect')} onClick={handleStartLiveSession} disabled={!!sessionData?.escapeRoomState?.isActive} className={'flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all motion-reduce:transition-none shadow-sm bg-indigo-600 text-white hover:bg-indigo-700 animate-pulse ring-2 ring-indigo-200 ' + quizreducedMotionClass} title={t('quiz.launch_live_tooltip')}><Wifi size={14} /> {t('quiz.launch_live_btn')}</button><div className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 border border-orange-100 rounded-full animate-in motion-reduce:animate-none fade-in duration-300"><Users size={12} className="text-orange-700" /><span className="text-xs font-black text-orange-700">{Object.keys(sessionData?.roster || {}).length} {t('quiz.lobby_waiting') || "Ready"}</span></div><button type="button" aria-label={t('common.locked')} onClick={handleToggleInteractive} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all motion-reduce:transition-none shadow-sm ${sessionData?.forceStatic ? 'bg-slate-800 text-white hover:bg-slate-700' : 'bg-white text-slate-700 border border-slate-400 hover:bg-slate-50'}`} title={t('session.toggle_interactive_title')}>{sessionData?.forceStatic ? <Lock size={12} /> : <Unlock size={12} />}{sessionData?.forceStatic ? t('session.static_only') : t('session.interactive')}</button></>}<button type="button" aria-label={t('common.confirm')} onClick={handleToggleIsPresentationMode} disabled={isReviewGame || isTeacherMode && (sessionData?.quizState?.isActive || sessionData?.escapeRoomState?.isActive)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all motion-reduce:transition-none shadow-sm disabled:opacity-50 disabled:cursor-not-allowed ${isPresentationMode ? 'bg-indigo-600 text-white hover:bg-indigo-700 ring-2 ring-indigo-200' : 'bg-white text-indigo-600 border border-indigo-200 hover:bg-indigo-50'}`} title={t('quiz.presentation')}>{isPresentationMode ? <CheckCircle size={14} /> : <MonitorPlay size={14} />}{isPresentationMode ? t('common.close') : t('quiz.presentation')}</button><button type="button" onClick={handleToggleIsReviewGame} disabled={isPresentationMode || !!sessionData?.escapeRoomState?.isActive} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all motion-reduce:transition-none shadow-sm disabled:opacity-50 disabled:cursor-not-allowed ${isReviewGame ? 'bg-yellow-500 text-indigo-900 hover:bg-yellow-600 ring-2 ring-yellow-200' : 'bg-white text-yellow-600 border border-yellow-200 hover:bg-yellow-50'}`} title={t('quiz.review_game')} aria-label={t('quiz.review_game')}>{isReviewGame ? <XCircle size={14} /> : <Gamepad2 size={14} />}{isReviewGame ? t('common.close') : t('quiz.review_game')}</button><button type="button" onClick={() => {
            if (escapeRoomState.isActive) {
              ...
              return <div key={cIdx} className="flex flex-col gap-4"><div className="bg-slate-800/80 backdrop-blur-sm text-white font-bold text-center py-4 rounded-lg border-b-4 border-blue-600 shadow-lg uppercase tracking-wider text-sm md:text-base flex flex-col items-center gap-1"><CategoryIcon size={20} className={iconColor} />{cat.name}</div>{cat.questions.map((q, qIdx) => {
                  ...
                  return <button type="button" key={qIdx} onClick={() => !isClaimed && handleReviewTileClick(q, q.points)} disabled={isClaimed} aria-label={`Category: ${cat.name}, ${q.points} Points${isClaimed ? ', Claimed' : ''}`} aria-disabled={isClaimed} className={`
                                                        ...
                                                        `}>{!isClaimed && <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:animate-[shimmer_1s_infinite] motion-reduce:animate-none" />}<span className="relative z-10 drop-shadow-md">{isClaimed ? '' : q.points}</span></button>;
                })}</div>;
            })}</div>{reviewGameState.activeQuestion && <div ref={reviewDialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="quiz-review-question-title" onKeyDown={function (e) { _quizHandleDialogKeyDown(e, reviewDialogRef, function () { closeReviewModal(false); }); }} className="absolute inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex items-center justify-center p-4 sm:p-12 animate-in motion-reduce:animate-none zoom-in-95 duration-300"><div className="bg-blue-800 w-full max-w-4xl rounded-2xl border-4 border-yellow-500 shadow-2xl p-8 text-center relative flex flex-col max-h-full overflow-y-auto"><div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-yellow-500 text-blue-900 font-black text-2xl px-6 py-2 rounded-full shadow-lg border-2 border-white">{reviewGameState.activeQuestion.points}</div><button type="button" ref={reviewCloseBtnRef} aria-label={t('common.close')} onClick={() => closeReviewModal(false)} className="absolute top-4 right-4 min-w-11 min-h-11 inline-flex items-center justify-center rounded-lg text-blue-100 hover:text-white hover:bg-blue-700 transition-colors motion-reduce:transition-none"><X size={24} /></button><div className="mt-8 mb-8"><h3 id="quiz-review-question-title" className="text-2xl md:text-4xl font-bold text-white leading-tight drop-shadow-sm">{formatInlineText(reviewGameState.activeQuestion.question, false, true)}</h3>{reviewGameState.activeQuestion.question_en && <p className="text-blue-200 text-lg italic mt-4">{formatInlineText(reviewGameState.activeQuestion.question_en, false, true)}</p>}</div><div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8 text-left">{reviewGameState.activeQuestion.options.map((opt, oIdx) => <div key={oIdx} className={`
                                                        ...
                                                        `}><span className="inline-block w-8 font-bold opacity-50">{String.fromCharCode(65 + oIdx)}.</span> {formatInlineText(opt, false, true)}</div>)}</div><div className="flex flex-col items-center gap-6 mt-auto pt-4 border-t border-blue-700">{!reviewGameState.showAnswer ? <button type="button" aria-label={t('review_game.reveal_answer')} onClick={() => {
                  setReviewGameState(prev => ({
                    ...prev,
                    showAnswer: true
                  }));
                  playSound('reveal');
                }} className="bg-yellow-500 text-blue-900 px-8 py-3 rounded-full font-bold text-lg hover:bg-yellow-400 transition-colors motion-reduce:transition-none shadow-lg hover:shadow-yellow-500/20">{t('review_game.reveal_answer')}</button> : <div className="w-full animate-in motion-reduce:animate-none fade-in slide-in-from-bottom-4"><p className="text-blue-200 text-sm font-bold uppercase tracking-wider mb-3">{t('review_game.who_correct')}</p><div className="flex flex-wrap justify-center gap-3">{gameTeams.map(team => <button type="button" aria-label={`Award ${reviewGameState.activeQuestion.points} points to ${team.name}`} key={team.id} onClick={() => handleAwardPoints(team.id, reviewGameState.activeQuestion.points)} className={`${team.color.replace('bg-', 'bg-')} hover:opacity-90 ${team.color.includes('yellow') ? 'text-indigo-900' : 'text-white'} px-4 py-2 rounded-lg font-bold shadow-md flex items-center gap-2 border-b-4 border-black/20 active:border-b-0 active:translate-y-1 transition-all motion-reduce:transition-none`}><CheckCircle2 size={16} /> {team.name}</button>)}<button type="button" onClick={() => {
                      playSound('incorrect');
                      closeReviewModal(true);
                    }} className="bg-slate-700 text-white hover:bg-slate-600 px-4 py-2 rounded-lg font-bold transition-colors motion-reduce:transition-none">{t('review_game.no_points')}</button></div></div>}</div></div></div>}</div></div> : escapeRoomState.isActive ? window.AlloModules && window.AlloModules.EscapeRoomGameplay ? <window.AlloModules.EscapeRoomGameplay escapeRoomState={escapeRoomState} setEscapeRoomState={setEscapeRoomState} escapeTimeLeft={escapeTimeLeft} isEscapeTimerRunning={isEscapeTimerRunning} setIsEscapeTimerRunning={setIsEscapeTimerRunning} handleSetIsEscapeTimerRunningToTrue={() => setIsEscapeTimerRunning(true)} handlers={{
        generateEscapeRoom,
        handlePuzzleSolved,
        handleSelectObject,
        handleWrongAnswer,
