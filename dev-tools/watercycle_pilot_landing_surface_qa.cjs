...
const out = path.join(ROOT, 'scratch', 'water-landing-surface-review');
...
(async () => {
  ...
  try {
    ...
    const impact=()=>page.evaluate(()=>{
      const g=pilotReview.scene.getObjectByName('pilot-ocean-landing');return {visible:g.visible,position:g.position.toArray(),rings:g.children.filter(o=>o.isMesh).map(o=>({scale:o.scale.toArray(),opacity:o.material.opacity,vertices:Array.from(o.geometry.attributes.position.array)})),spray:g.children.find(o=>o.isPoints).visible,points:Array.from(g.children.find(o=>o.isPoints).geometry.attributes.position.array)};
    });

    const alignment=()=>page.evaluate(()=>{
      const scene=pilotReview.scene,ocean=scene.getObjectByName('pilot-ocean-surface'),g=scene.getObjectByName('pilot-ocean-landing');scene.updateMatrixWorld(true);
      const ray=new THREE.Raycaster(),down=new THREE.Vector3(0,-1,0),p=new THREE.Vector3();let maxError=0,count=0;
      const rings=g.children.filter(o=>o.isMesh);
      for(const ring of rings){const a=ring.geometry.attributes.position;
        for(let i=0;i<a.count;i+=7){p.fromBufferAttribute(a,i).applyMatrix4(ring.matrixWorld);ray.set(new THREE.Vector3(p.x,500,p.z),down);
          const hit=ray.intersectObject(ocean)[0];if(!hit)throw Error('Landing outside ocean');
          maxError=Math.max(maxError,Math.abs(p.y-hit.point.y-.12));count++;
        }
      }
      const alpha=Array.from(rings[0].geometry.attributes.landingEdge.array);
      return {maxError,count,unique:new Set(rings.map(r=>r.geometry)).size,soft:alpha.some(a=>a>.99)&&alpha.some(a=>a<.001)};
    });
    const firstAlignment=await alignment();assert(firstAlignment.count>60&&firstAlignment.maxError<.002&&firstAlignment.unique===3&&firstAlignment.soft,JSON.stringify(firstAlignment));
    ...
    const reduced=await impact();assert.equal(reduced.spray,false);
    assert((await alignment()).maxError<.002,'Reduced motion follows the frozen sea without resetting wave time');await page.waitForTimeout(700);assert.deepEqual(await impact(),reduced);
    
    await canvas.screenshot({path:path.join(out,'landing-reduced.jpg'),type:'jpeg',quality:85});
    await page.emulateMedia({reducedMotion:'no-preference'});
    await page.evaluate(()=>{
      const scene=pilotReview.scene,ocean=scene.getObjectByName('pilot-ocean-surface');
      window.landingMotionCheck={frames:0,maxError:0,lastRadius:0,expanded:false};
      scene.onBeforeRender=()=>{
        const g=scene.getObjectByName('pilot-ocean-landing'),m=landingMotionCheck;
        if(!g.visible||m.frames>=8||waterReviewData.pilot.paused)return;
        const ring=g.children.find(o=>o.isMesh);if(m.lastRadius&&ring.scale.x>m.lastRadius+.001)m.expanded=true;m.lastRadius=ring.scale.x;
        const ray=new THREE.Raycaster(),down=new THREE.Vector3(0,-1,0),p=new THREE.Vector3();
        for(let i=0;i<ring.geometry.attributes.position.count;i+=32){p.fromBufferAttribute(ring.geometry.attributes.position,i).applyMatrix4(ring.matrixWorld);ray.set(new THREE.Vector3(p.x,500,p.z),down);const hit=ray.intersectObject(ocean)[0];if(hit)m.maxError=Math.max(m.maxError,Math.abs(p.y-hit.point.y-.12));else m.maxError=Infinity;}m.frames++;
      };
    });
    ...
    const motion=await page.evaluate(()=>landingMotionCheck);assert(motion.frames>=2&&motion.expanded&&motion.maxError<.002,JSON.stringify(motion));
    ...
    await page.evaluate(()=>{
      window.rainfallDisposals={ring:0,rain:0,allRings:0};const s=pilotReview.scene;
      s.getObjectByName('pilot-ocean-landing').children[0].geometry.addEventListener('dispose',()=>rainfallDisposals.ring++);
      s.getObjectByName('pilot-ocean-landing').children.filter(o=>o.isMesh).forEach(r=>r.geometry.addEventListener('dispose',()=>rainfallDisposals.allRings++));
      s.getObjectByName('pilot-rain-field').geometry.addEventListener('dispose',()=>rainfallDisposals.rain++);
      mountWater({wcMode:'explorer'});
    });
    assert.deepEqual(await page.evaluate(()=>rainfallDisposals),{ring:1,rain:1,allRings:3});assert.deepEqual(errors,[]);
    console.log('PASS: exact rendered-ocean ripple alignment during pause, expansion and reduced motion, soft edges, three independent geometries disposed once, wind-slanted fading rain, sea clipping, paused rain, both camera views, actual ocean landing, held/reduced/expiring ripples, snow and land exclusions, mobile accessibility, resource disposal, no WebGL errors.');
