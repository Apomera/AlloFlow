...
source+=String.raw`
  ...
  for(const pitch of [0.001,0.002,0.005,0.007,0.01,0.02]){
    await mount('screw',{screwPitch:pitch,shopFocusMechanism:true});
    for(const [name,progress] of [['Halfway',0.5],['Full stroke',1]]){
      ...
      const valid=await pg.evaluate(p=>{
        const d=window.__qaShop.mlDemo,points=d.screwThread.geometry.parameters.path.points;
        return Math.abs(d.motion.position.y+(points[32].y-points[0].y)*p)<1e-8 && d.pressBands.length===6;
      },progress);
      if(!valid)errors.push('Thread travel mismatch');
      const label='screw-'+pitch+'-'+Math.round(progress*100);
      await pg.locator('.ml-shop-world').screenshot({path:path.join(OUT,label+'.png')});manifest.push(label);
    }
  }
  for(const radius of [0.05,0.5]){
    await mount('screw',{screwHandleR:radius,shopMotionProgress:0.25,shopFocusMechanism:true});await checkPose(0.25);
    const label='screw-radius-'+radius;await pg.locator('.ml-shop-world').screenshot({path:path.join(OUT,label+'.png')});manifest.push(label);
  }
  await mount('screw',{screwPitch:0.02,shopFocusMechanism:true,shopMotionProgress:0.5,shopRotY:196});await checkPose(0.5);
  await pg.locator('.ml-shop-world').screenshot({path:path.join(OUT,'screw-rear.png')});manifest.push('screw-rear');
  for(const width of [390,320])for(const bench of ['screw']){
