...
const vm = require('vm');
const { reviewWave, reviewDate, corrections } = require('./eppp_diagram_quality_wave_02_data.cjs');
...
function applyGuardedReplacement(source, correctionKey, replacement) {
  ...
  if (!replacement.from || !replacement.to || expectedOccurrences < 1) {
    throw new Error(`${correctionKey}: replacements require non-empty from/to text and a positive expectedOccurrences value.`);
  }
  const oldCount = occurrenceCount(source, replacement.from);
  const newCount = occurrenceCount(source, replacement.to);
  const retainsSourceText = replacement.to.includes(replacement.from);
  if (newCount >= expectedOccurrences && (oldCount === 0 || retainsSourceText)) return source;
  if (oldCount !== expectedOccurrences) {
    throw new Error(`${correctionKey}: expected ${expectedOccurrences} occurrence(s) of guarded source text, found ${oldCount}.`);
  }
  return replacement.replaceAll || expectedOccurrences > 1
    ? source.split(replacement.from).join(replacement.to)
    : source.replace(replacement.from, replacement.to);
}

let source = fs.readFileSync(sourcePath, 'utf8');
for (const correction of corrections) {
  for (const replacement of correction.replacements) source = applyGuardedReplacement(source, correction.key, replacement);
}
for (const correction of corrections) {
  for (const replacement of correction.replacements) {
    const expectedOccurrences = Number(replacement.expectedOccurrences || 1);
    if (!replacement.to.includes(replacement.from) && occurrenceCount(source, replacement.from) !== 0) throw new Error(`${correction.key}: superseded wording remains after repair.`);
    if (occurrenceCount(source, replacement.to) < expectedOccurrences) throw new Error(`${correction.key}: corrected wording is missing after repair.`);
  }
}
// Validate the complete in-memory candidate before writing either mirror.
function loadLearningAssets(diagramSource) {
  ...
  function run(relativePath) {
    ...
    const scriptSource = relativePath === 'js/textbook_diagrams.js' ? diagramSource : fs.readFileSync(target, 'utf8');
    vm.runInContext(scriptSource, context, { filename: relativePath, timeout: 15000 });
  }
  run('js/data.js');
  ...
  return { windowObject, chapterSourceById };
}

function discoverPlacements(diagramSource) {
  const { windowObject, chapterSourceById } = loadLearningAssets(diagramSource);
  ...
  return { placements, diagrams: windowObject._epppDiagrams || {} };
}

const discovered = discoverPlacements(source);
for (const correction of corrections) {
  const placements = discovered.placements.get(correction.key);
  if (placements.length !== correction.expectedPlacementCount) {
    throw new Error(`${correction.key}: expected ${correction.expectedPlacementCount} learner placement(s), found ${placements.length}.`);
  }
  if (!correction.sourceDetails.length || correction.sourceDetails.some((detail) => !detail.title || !detail.organization || !detail.url || !detail.whyReputable)) {
    throw new Error(`${correction.key}: complete named source details are required.`);
  }
  if (correction.references.length !== correction.sourceDetails.length || correction.sourceDetails.some((detail) => !correction.references.includes(detail.url))) {
    throw new Error(`${correction.key}: references must mirror source-detail URLs.`);
  }
}
const eriksonSvg = String(discovered.diagrams.eriksonStages && discovered.diagrams.eriksonStages.svg || '');
if (occurrenceCount(eriksonSvg, 'Ego strength:') !== 8) throw new Error('eriksonStages: the generated SVG must label all eight ego strengths.');
if (occurrenceCount(eriksonSvg, 'font-size:9px;fill:#94a3b8">Ego strength:') !== 8) throw new Error('eriksonStages: all eight ego-strength labels must render at 9px or larger.');
if (/<animate\b/i.test(eriksonSvg)) throw new Error('eriksonStages: the fixed-timetable motion marker must be removed.');
const bareAmpersandPattern = /&(?!amp;|lt;|gt;|quot;|apos;|#\d+;|#x[0-9a-f]+;)/i;
for (const correction of corrections) {
  const diagramSvg = String(discovered.diagrams[correction.key] && discovered.diagrams[correction.key].svg || '');
  if (bareAmpersandPattern.test(diagramSvg)) throw new Error(`${correction.key}: generated SVG contains an unescaped ampersand.`);
}
...
const items = corrections.map((correction, index) => ({
  ...
  placementCount: discovered.placements.get(correction.key).length,
  placements: discovered.placements.get(correction.key),
...
const markdown = `# EPPP diagram review wave 02

Reviewed: ${reviewDate}

Status: **assisted editorial and source review complete; independent qualified expert validation pending**

This wave corrects eight actively used shared diagrams and records each current chapter placement. It does not claim psychometric calibration, clinical guidance, or independent expert validation.
...
${item.sourceDetails.map((detail) => `- [${detail.title}](${detail.url}) — ${detail.organization}. ${detail.whyReputable}`).join('\n')}
...
console.log(`EPPP diagram quality wave 02: corrected ${items.length} templates across ${placementCount} placements.`);
