...
const scene = readFileSync(resolve(root, 'life_skills_transit/life_skills_transit.html'), 'utf8');
const publicScene = readFileSync(resolve(root, 'desktop/web-app/public/life_skills_transit/life_skills_transit.html'), 'utf8');
...
describe('Life Skills 3D transit and street safety lab', () => {
  it('defines six travel-safety targets and three practice modes', () => {
    const literal = scene.match(/var TASKS = (\[[\s\S]*?\]);/);
    expect(literal).not.toBeNull();
    const tasks = Function('return ' + literal[1])();
    expect(tasks.map((task) => task.id)).toEqual(['scan', 'signal', 'route', 'seatbelt', 'visibility', 'help']);
  ...
  it('keeps the 3D view optional and provides an accessible checklist fallback', () => {
    ...
    expect(scene).toContain('transit-interactive');
    expect(scene).toContain('raycaster="objects: .clickable');
    expect(scene).toContain('Accessible transit checklist');
    ...
    expect(scene).toContain('alloflow-life-transit-3d-why-reveal');
    expect(scene).toContain('function pauseForHelp()');
    expect(scene).toContain('alloflow-life-transit-3d-help-pause');
    ...
    expect(scene).toContain('alloflow-life-transit-3d-confidence');
    ...
    expect(scene).toContain('alloflow-life-transit-3d-replay-complete');
    expect(scene).toContain('data-task-id');
    expect(scene).toContain('alloflow-life-transit-3d-next-step');
    expect(scene).toContain('alloflow-life-transit-3d-reflection');
  ...
  it('launches the lab from the existing Transportation tab', () => {
    expect(lifeSkills).toContain('function openLifeSkillsTransit3D()');
    expect(lifeSkills).toContain('/life_skills_transit/life_skills_transit.html');
    expect(lifeSkills).toContain('Open the 3D transit and street safety lab in a new window');
    expect(lifeSkills).toContain('Open 3D transit lab');
  });

  it('copies the transit lab alongside the shared local A-Frame runtime', () => {
    expect(build).toContain("'immersive_geometry'");
    expect(build).toContain("'life_skills_transit'");
  });
});
