...
for (const outputRoot of [path.join(root, 'test_prep'), path.join(root, 'prismflow-deploy', 'public', 'test_prep')]) { fs.mkdirSync(outputRoot, { recursive: true }); fs.writeFileSync(path.join(outputRoot, 'teaching_reading_5205_items.json'), JSON.stringify(items, null, 2) + '\n'); fs.writeFileSync(path.join(outputRoot, 'teaching_reading_5205_pack.json'), JSON.stringify(pack, null, 2) + '\n'); }
