...
for(const out of[path.join(root,'test_prep'),path.join(root,'prismflow-deploy','public','test_prep')]){fs.mkdirSync(out,{recursive:true});fs.writeFileSync(path.join(out,'school_librarian_5312_items.json'),JSON.stringify(items,null,2)+'\n');fs.writeFileSync(path.join(out,'school_librarian_5312_pack.json'),JSON.stringify(pack,null,2)+'\n')}console.log('Built School Librarian 5312: 200 items; diagnostics 20/19/20/29/12; simulation 24/23/24/35/14.');
