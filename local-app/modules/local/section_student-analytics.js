...
// Source lines: 4400–5788
// Hash: 3ea92521f768b65e
...
const fetchAndCleanUrl = async (url, geminiCaller, toastCallback) => {
    ...
    if (isYouTubeUrl(targetUrl)) {
        /* [LOCAL] YouTube transcript extraction requires cloud API — not available locally */
        if (toastCallback) toastCallback("⚠️ YouTube transcript extraction is not available in local mode. Please paste the transcript text directly.", "warning");
        return null;
    }
    try {
...
const translateChunk = async (chunkData, targetLanguage, apiKey) => {
  ...
  /* [LOCAL] Rewritten for OpenAI-compatible API (LM Studio) */
  const ollamaUrl = (window.__alloLocalConfig?.llmEngineUrl || 'http://localhost:1234') + '/v1/chat/completions';
  const model = window.__ollamaModel || 'default';
...
export const LanguageProvider = ({ children }) => {
  ...
  useEffect(() => {
      if (typeof window !== 'undefined' && !window.jsonrepair) {
          ...
          script.src = "./shared/libs/jsonrepair.min.js";
          script.async = true;
