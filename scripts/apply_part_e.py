"""
Apply Part E edits: Add detail view sections for Word Sounds, Game Completions,
Socratic Chat, and Label Challenge to the student detail modal.

Inserts new sections right before the Interview Transcript section.
"""
import sys
import os
...
def apply_edits():
    ...
    # Insert before the Interview Transcript section
    target = """                            {/* Interview Transcript */}
    ...
    replacement = new_sections + target

    if target in content:
        content = content.replace(target, replacement, 1)
        with open(FILE, 'w', encoding='utf-8') as f:
            f.write(content)
        print("✅ Part E: Added 4 detail view sections (Word Sounds, Games, Label Challenge, Socratic Chat)")
        return True
    else:
        # Debug
        idx = content.find('{/* Interview Transcript */}')
        if idx > 0:
            print(f"Found interview transcript comment at offset {idx}")
            print(f"Context: {repr(content[idx:idx+120])}")
        else:
            print("❌ Could not find Interview Transcript section at all")
        return False
