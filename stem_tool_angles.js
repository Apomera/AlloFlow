    ...
    render: function(ctx) {
      ...
      var handleDrag = function(startEvt) {
        var onMove = function(me) {
          var rect = startEvt.target.closest('svg').getBoundingClientRect();
          var dx = me.clientX - rect.left - cx;
          var dy = -(me.clientY - rect.top - cy);
      ...
      return h('div', { className: 'space-y-4 max-w-3xl mx-auto animate-in fade-in duration-200' },
        ...
        h('div', { className: 'bg-white rounded-xl border-2 border-purple-200 p-4 flex justify-center' },
          h('svg', { width: 400, height: 420, className: 'select-none' },
            ...
            // Draggable handle
            h('circle', { cx: rayEndX, cy: rayEndY, r: 10, fill: '#7c3aed', fillOpacity: 0.2, stroke: '#7c3aed', strokeWidth: 2, className: 'cursor-grab', onMouseDown: handleDrag }),
            // Center dot
        ...
        angleChallenge && h('div', { className: 'bg-purple-50 rounded-lg p-3 border border-purple-200' },
          h('p', { className: 'text-sm font-bold text-purple-800 mb-2' }, '\uD83C\uDFAF Create a ' + angleChallenge.target + '\u00B0 angle (within 3\u00B0)'),
