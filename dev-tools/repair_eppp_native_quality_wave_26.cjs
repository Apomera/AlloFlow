...
const { baselineMetrics, reviewedAt, reviewWave, revisions } = require('./eppp_native_quality_wave_26_data.cjs');
...
const auditBasename = 'eppp_question_quality_audit_wave_26.json';
...
if (fs.readFileSync(deployBankPath, 'utf8') !== sourceText) throw new Error('Source and deploy EPPP banks must match before wave 26.');
...
if (revisions.length !== 8 || new Set(revisions.map((revision) => revision.id)).size !== 8) throw new Error('Wave 26 needs eight unique revisions.');
...
for (const revision of revisions) {
  ...
  if (!recognizedReplay && item.prompt !== revision.expectedPrompt) throw new Error(revision.id + ' prompt drifted before wave 26.');
...
const audit = {
  ...
  scope: 'Eight EPPP items, one per domain, converted from definition or cue-heavy formats into applied or analytic decisions with parallel distractors and concise option-specific feedback.',
  summary: {
...
console.log('EPPP quality wave 26: rewrote 8 items across 8 domains; retiered 7; selected warnings after = 0.');
