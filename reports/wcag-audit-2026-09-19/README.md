# AlloFlow WCAG follow-up — September 19, 2026
...
- Native-scroll assertions in the fallback retest: **3 passed**; the same run also passed no-match search but timed out in two full-page search cases. The final focused search-navigation retest is **3 passed, 0 failed, 3 filtered/skipped**, covering all three queries. These are six distinct regression cases with passing evidence across the scoped runs. [Scroll/fallback results](interaction-fallback-retest.json), [final search results](search-final.json), [measured scroll interactions](scroll-fallback-evidence.json), and screenshots in this folder.
...
- Math theme sample: **10 passed, 0 failed, 120 filtered/skipped**. [Results](math-theme-tests.json). The 120 skipped entries are filtered-out cases, not new product failures or test executions.
- Previously repaired header/Quick Start regression files: **37 passed, 0 failed, 0 filtered/skipped**, across the five files recorded in [results](prior-fixes-regressions.json). The requested Video Studio dialog file was absent from that report and was run separately: **1 passed, 0 failed, 0 filtered/skipped**, [result](video-dialog-test.json). Combined, these are 38 passing assertions in six files; no duplicated reruns are counted.
...
- Rebuilt sidebar accessibility/catalog contracts: **18 passed, 0 failed, 0 filtered/skipped**. [Results](sidebar-tests.json).
...
|---|---|---|---|
| Build statistics | 320 / 620 | 0 → 1 | Yes / Yes |
| Circuit schematic | 294 / 600 | 0 → 5 | Yes / Yes |
| Scrollable chart | 286 / 760 | 0 → 3 | Yes / Yes |

### Initial-page browser sample

| State | axe violations | Incomplete rules | Document scroll/client width |
|---|---|---|---|
| app-1280 | 0 | aria-prohibited-attr, color-contrast | 1280/1280 |
| app-320 | 0 | aria-prohibited-attr, color-contrast | 320/320 |
| app-320-spacing | 0 | aria-prohibited-attr, color-contrast | 320/320 |
| catalog-1280 | 0 | 0 | 1280/1280 |
| catalog-320 | 0 | 0 | 320/320 |
| catalog-320-spacing | 0 | 0 | 320/320 |
| video-studio-1280 | 0 | aria-prohibited-attr, color-contrast | 1280/1280 |
| video-studio-320 | 0 | aria-prohibited-attr, color-contrast | 320/320 |
| video-studio-320-spacing | 0 | aria-prohibited-attr, color-contrast | 320/320 |

### Loaded workspace sample

The current workspace runner completed. Its gates are: sourceReady=true, labelInNamePassed=true, quickStartContrastPassed=true, loadedWorkspacePassed=true, backendFocusContained=true, backendFocusRestored=true. [Raw results](browser-label-check.json) retain violations, incomplete rules, page errors and readiness evidence.

- backend-initial-1280: no axe violations; width 1280/1280.
- teacher-workspace-1280: aria-valid-attr-value; width 1280/1280.
- teacher-workspace-after-dialog: aria-valid-attr-value; width 1280/1280.
- teacher-workspace-320: no axe violations; width 320/320.
- teacher-workspace-320-spacing: no axe violations; width 320/320.
- teacher-loaded-1280: no axe violations; width 1280/1280.
- teacher-loaded-320: no axe violations; width 320/320.
- teacher-loaded-320-spacing: no axe violations; width 320/320.
