...
(async()=>{
 ...
 try{
  for(const route of ['/app/']){
   ...
   const teacherRole=page.locator('button[data-help-key="role_teacher"]');await teacherRole.click({timeout:90000});
   await page.getByRole('button',{name:'College',exact:true}).waitFor({state:'visible',timeout:90000});results.modalOverrideLoaded=await page.evaluate(()=>[...document.styleSheets].some(s=>{try{return [...s.cssRules].some(r=>r.selectorText?.includes('.fixed.inset-0')&&r.selectorText?.includes('.text-slate-600'))}catch{return false}}));
   ...
   await page.addStyleTag({content:'* {line-height:1.5!important;letter-spacing:.12em!important;word-spacing:.16em!important} p {margin-bottom:2em!important}'});await audit('teacher-loaded-320-spacing');results.badgeEvidence=await page.locator('span[aria-live="polite"]').filter({hasText:/shown/}).evaluateAll(nodes=>nodes.map(e=>({text:e.innerText,html:e.outerHTML,color:getComputedStyle(e).color,background:getComputedStyle(e).backgroundColor})));
  }
 }catch(e){results.error=e.stack;results.failureState=await page.evaluate(()=>({text:document.body.innerText,buttons:[...document.querySelectorAll('button')].filter(e=>e.getClientRects().length).map(e=>({text:e.innerText,label:e.getAttribute('aria-label')}))}));await page.screenshot({path:path.join(__dirname,'workspace-failure.png'),fullPage:true});console.error(e.stack);process.exitCode=1;}
