...
source+=String.raw`
  ...
  async function solveClue(bench,extra={}){
    ...
    const shown=await bay.evaluate(x=>{const r=x.getBoundingClientRect();return r.top>=-1 && r.bottom<=innerHeight+1;});
    if(!shown)errors.push('Inspect clue did not bring the scene into view: '+bench);
    await checkPose(0.5);
    ...
    await discovery.locator('summary').press('Enter');
