...
const wave = require('../dev-tools/eppp_native_quality_wave_29_data.cjs');
...
describe('EPPP expanded quality wave 29', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_29.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 17, applicationItems: 15, analysisItems: 9, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    ...
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicatePairs).toBe(audit.summary.distractorWarningsBefore.semanticConceptDuplicatePairs - 5);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_29.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_29.json'));
  });

  it('replays after wave 28', () => {
