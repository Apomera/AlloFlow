...
Return ONLY the complete HTML document.`;
    ...
    if (!accessibleHtml.includes('lang=')) { accessibleHtml = accessibleHtml.replace(/<html/, '<html lang="en"'); aiFixCount++; }
    ...
    if (batchMaxFix > 0 && ((curAxeResults && curAxeResults.totalViolations > 0) || bestAiScore < 90)) {
      for (let fp = 0; fp < batchMaxFix; fp++) {
        ...
        if (newAxe === 0 && newAi >= 90) { log('Excellent \u2014 stopping'); break; }
...
"""${chunk}"""`;
  ...
  const fixContrastViolations = (htmlContent) => {
    ...
    fixed = fixed.replace(/opacity:\s*0\.[0-6]\d*/g, () => { fixCount++; return 'opacity:0.85'; });
...
<p style="color:#64748b;font-size:13px">Document: <strong>${fileName}</strong><br>Date: ${date}<br>Standard: WCAG 2.1 Level AA<br>Tool: AlloFlow PDF Accessibility Auditor</p>`;
    ...
    html += `<div class="footer">
      <p>Generated by AlloFlow PDF Accessibility Auditor | ${date}</p>
      <p>Audit methodology: ${audit.auditorCount || 1}-pass AI triangulation with statistical reliability analysis + axe-core (Deque) automated WCAG 2.1 AA verification</p>
      <p>Standard: WCAG 2.1 Level AA | Tool: AlloFlow (open source, GNU AGPL v3)</p>
    </div></body></html>`;
    return html;
  };
