...
source+=String.raw`
  ...
  for(const segments of [1,2,3,4,5,6]){
    await mount('pulley',{pulleySegments:segments,shopFocusMechanism:true});
    for(const [name,progress] of [['Halfway',0.5],['Full stroke',1]]){
      ...
      const valid=await pg.evaluate(n=>{
        const d=window.__qaShop.mlDemo;
        return d.supportCues.length===n && d.supportCues.every((c,i)=>c.position.x===d.supportingRopes[i].position.x && c.position.y===d.supportingRopes[i].position.y);
      },segments);
      if(!valid)errors.push('Wrong supporting-strand cues for '+segments);
      const label='pulley-'+segments+'-'+Math.round(progress*100);
  ...
  for(const width of [390,320])for(const bench of ['pulley']){
    ...
    const pitch=await pg.evaluate(()=>window.__qaShop.data.rotX);if(pitch!==12)errors.push('Wrong Close view tilt for pulley');
