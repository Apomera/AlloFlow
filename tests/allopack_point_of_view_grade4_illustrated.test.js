...
const slug='point_of_view_grade4',folder='allopacks/media/'+slug+'/',pack=read('allopacks/illustrated/'+slug+'.allopack.json'),original=read('allopacks/'+slug+'.allopack.json'),manifest=read(folder+'manifest.json');
require('../dev-tools/five_literacy_content_refinements.cjs').refine(original);
...
describe('Literacy content and artifact audit',()=>{
it('retains valid answers, contextual sort categories and native editable labels',()=>{for(const q of get('quiz').data.questions.filter(q=>q.type==='mcq')){expect(q.options.filter(x=>x===q.correctAnswer)).toHaveLength(1);expect(new Set(q.options).size).toBe(q.options.length);}const cats=new Set(get('concept-sort').data.categories.map(c=>c.id));for(const item of get('concept-sort').data.items)expect(cats.has(item.categoryId)).toBe(true);for(const p of panels)for(const l of p.labels){expect(l.text).toBeTruthy();expect(l.anchorX).toBeGreaterThanOrEqual(0);expect(l.anchorX).toBeLessThanOrEqual(100);}const reading=get('simplified').data;expect(reading).toContain('one isolated pronoun is not enough');expect(reading).toContain('may follow one character closely');expect(get('directions').data.body).toContain('isolated narration excerpts');expect(get('applied-challenge').data.supports.parallelExample.move).toContain('an invention');
});
it('audits each changed source field',()=>{const pristine=read('allopacks/'+slug+'.allopack.json'),changes=read(folder+'content-refinements.json');expect(changes.length).toBe(pack.allopack.contentRefinements.count);for(const c of changes){let old=pristine.history.find(r=>r.id===c.resourceId),now=pack.history.find(r=>r.id===c.resourceId);for(const k of c.path.split('.')){old=old[k];now=now[k];}if(c.resourceId.endsWith('-directions')&&c.path==='data.body')old+='\n\nPicture panels: '+manifest.groups.join('; ')+'. '+manifest.note;expect(old).toEqual(c.from);expect(now).toEqual(c.to);}});});
