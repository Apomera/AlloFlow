...
function llLoad() {
  try {
    return JSON.parse(localStorage.getItem(SI_I18N_KEY)) || {};
  } catch (e) {
    return {};
  }
}
function llStore(v) {
  try {
    localStorage.setItem(SI_I18N_KEY, JSON.stringify(v));
  } catch (e) {
  }
}
...
function siResponseText(value, max) {
  ...
  return text.replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]+/g, " ").trim().slice(0, max || 4e3);
}
function siResponseManifestMap(payload) {
  var p = payload && typeof payload === "object" ? payload : {};
  var raw = p.responseManifest;
  var out = /* @__PURE__ */ Object.create(null);
  if (!raw || typeof raw !== "object" || Array.isArray(raw) || Number(raw.schemaVersion) !== SI_RESPONSE_MANIFEST_SCHEMA_VERSION || !Array.isArray(raw.entries)) return out;
  ...
  list.slice(0, SI_RESPONSE_ENTRY_LIMIT).forEach(function(entry) {
    ...
    var labels = /* @__PURE__ */ Object.create(null);
    var rawLabels = entry.valueLabels && typeof entry.valueLabels === "object" && !Array.isArray(entry.valueLabels) ? entry.valueLabels : {};
...
function siResponseRequiresManualReview(entry) {
  ...
  if (entry.hasManifest === true) {
    ...
    return SI_AI_FREEFORM_RESPONSE_TYPES.indexOf(type) < 0 || SI_AI_FREEFORM_RESPONSE_SUFFIXES.indexOf(suffix) < 0;
  }
...
function siDisplayResponseValue(rawText, valueLabels) {
  var raw = siResponseText(rawText, 12e3);
...
function siResponseEntryModels(payload) {
  ...
  var manifestSupported = !!(rawManifest && typeof rawManifest === "object" && !Array.isArray(rawManifest) && Number(rawManifest.schemaVersion) === SI_RESPONSE_MANIFEST_SCHEMA_VERSION && Array.isArray(rawManifest.entries));
  ...
  if (manifestSupported) Object.keys(manifest).forEach(function(key) {
    if (keys.indexOf(key) < 0) keys.push(key);
  });
  return keys.slice(0, SI_RESPONSE_ENTRY_LIMIT).map(function(key, index) {
    ...
    var rawText = hasStoredResponse ? siResponseText(responses[key], 12e3) : "";
    ...
    var model = {
      ...
      valueLabels: meta && meta.valueLabels || /* @__PURE__ */ Object.create(null),
...
function siAlloSheetAdapter() {
  var adapter = window.AlloSheetTransferAdapter || window.AlloModules && window.AlloModules.AlloSheetTransferAdapter;
  if (!adapter || typeof adapter.column !== "function" || typeof adapter.table !== "function" || typeof adapter.envelope !== "function" || typeof adapter.withinDateRange !== "function") {
...
function siAlloSheetTitle(value) {
  return String(value == null ? "" : value).replace(/[\u0000-\u001f\u007f]+/g, " ").replace(/\s+/g, " ").trim().slice(0, 160);
}
...
function siPrepareAlloSheetSavedSource(input) {
  if (input && input.kind === "submission-inbox-allosheet-source-v1" && Array.isArray(input.entries)) {
    ...
    var copiedEntries = input.entries.slice(0, SI_ALLOSHEET_MAX_SOURCE_ENTRIES).map(function(entry, index) {
      ...
      if (!/^S\d{1,6}$/.test(assignmentKey) || !assignmentLabel || !/^L\d{1,6}$/.test(learnerToken)) {
        preparedInvalid += 1;
        return null;
      }
  ...
  var rawEntries = input && Array.isArray(input.gradebookEntries) ? input.gradebookEntries : Array.isArray(input) ? input : [];
  var assignmentTokens = /* @__PURE__ */ new Map();
  var nextAssignment = 1;
  var learnerTokens = /* @__PURE__ */ new Map();
  var nextLearner = 1;
  var excluded = 0;
...
function siAlloSheetDatedEntries(source, options, adapter, createdTime) {
  ...
  var eligible = source.entries.filter(function(entry) {
    return Number.isFinite(entry.gradedTime) && adapter.withinDateRange(entry.gradedTime, range, createdTime);
  });
  if (siAlloSheetAttemptPolicy(options.attemptPolicy) === "all-saved") {
    return eligible.slice().sort(function(a, b) {
      return a.assignmentLabel.localeCompare(b.assignmentLabel, void 0, { numeric: true, sensitivity: "base" }) || a.assignmentKey.localeCompare(b.assignmentKey) || a.gradedTime - b.gradedTime || a.index - b.index;
    });
  }
  var latest = /* @__PURE__ */ new Map();
  eligible.forEach(function(entry) {
    var key = entry.assignmentKey + "\0" + entry.learnerToken;
    var current = latest.get(key);
    if (!current || entry.gradedTime > current.gradedTime || entry.gradedTime === current.gradedTime && entry.index > current.index) {
...
function siSubmissionInboxAlloSheetOptions(input, options) {
  ...
  var assignments = Array.from(assignmentMap.values()).sort(function(a, b) {
    return a.label.localeCompare(b.label, void 0, { numeric: true, sensitivity: "base" }) || a.key.localeCompare(b.key);
  });
...
function siBuildSubmissionInboxAlloSheetEnvelope(input, options) {
  ...
  var selectedKeys = Array.isArray(opts.assignmentKeys) ? opts.assignmentKeys.map(function(key) {
    return String(key || "");
  }).filter(function(key) {
    return /^S\d{1,6}$/.test(key);
  }) : review.assignments.map(function(item) {
    return item.key;
  });
  var available = new Set(review.assignments.map(function(item) {
    return item.key;
  }));
  var selected = new Set(selectedKeys.filter(function(key) {
  ...
  var assignmentCodes = /* @__PURE__ */ new Map();
  ...
  selectedSorted.forEach(function(item) {
  ...
  if (datasets.submissionSummary !== false) {
    var submissionRows = selectedSorted.map(function(item, index) {
      ...
      var learners = new Set(entries.map(function(entry) {
        return entry.learnerToken;
      }));
      var submittedTimes = entries.map(function(entry) {
        return entry.submittedTime;
      }).filter(function(time) {
        return Number.isFinite(time) && adapter.withinDateRange(time, "all", createdTime);
      }).sort(function(a, b) {
        return a - b;
      });
      var gradedTimes = entries.map(function(entry) {
        return entry.gradedTime;
  ...
  if (datasets.scoreSummary !== false) {
    var scoreRows = selectedSorted.map(function(item, index) {
      ...
      var distributionCounts = bands.concat(Object.keys(statuses).map(function(key) {
        return statuses[key];
      }));
      var reportable = scores.length >= SI_ALLOSHEET_MIN_SCORE_GROUP && !distributionCounts.some(function(count) {
        return count > 0 && count < SI_ALLOSHEET_MIN_SCORE_GROUP;
      });
      if (scores.length > 0 && !reportable) suppressedScoreSummaries += 1;
      var average = reportable ? siRoundScore(scores.reduce(function(sum, score) {
        return sum + score;
      }, 0) / scores.length) : null;
      return {
        ...
        values: {
          ...
          minimum_score_percent: reportable ? scores.reduce(function(minimum, score) {
            return score < minimum ? score : minimum;
          }, scores[0]) : null,
          maximum_score_percent: reportable ? scores.reduce(function(maximum, score) {
          ...
          score_sample_status: scores.length === 0 ? "no_valid_scores" : reportable ? "available" : "suppressed_small_groups",
...
function SubmissionInbox({ isOpen, onClose, rosterKey, t, addToast, onOpenAlloSheet, onOpenInStudio }) {
  ...
  React.useEffect(() => {
    if (!showAlloSheetReview) return void 0;
    ...
    try {
      ...
      if (conflicts.length > 0) {
        overwriteAll = await requestConfirmation({
          ...
          message: conflicts.length + " preset" + (conflicts.length === 1 ? "" : "s") + " already exist with the same name: " + conflicts.slice(0, 5).map((k) => '"' + incoming[k].name + '"').join(", ") + (conflicts.length > 5 ? ", \u2026" : "") + ". Choose overwrite to replace them, or skip to keep the existing presets.",
  ...
  const removeRow = (idx) => {
    ...
    setAnchors((prev) => prev.filter((anchor) => anchor.fromSubmissionIdx !== idx).map((anchor) => Number.isInteger(anchor.fromSubmissionIdx) && anchor.fromSubmissionIdx > idx ? { ...anchor, fromSubmissionIdx: anchor.fromSubmissionIdx - 1 } : anchor));
  ...
  const gradeRow = async (idx, opts) => {
    ...
    const responseEntries = allResponseEntries.filter((entry) => entry.rawText && entry.aiGradable && !anchoredKeys.has(entry.key));
    if (responseEntries.length === 0) {
      if (!opts.silent) addToast && addToast(
        anchoredKeys.size > 0 ? tr("All responses on this submission are already anchored.") : structuredResponseCount > 0 ? tr("No written responses need AI grading. Structured responses are ready for your manual review.") : tr("No responses to grade in this submission."),
        "warn"
      );
      return { ok: 0, fail: 0, skipped: true };
    }
  ...
  const openManualScoreForm = (submissionIdx, entry) => {
    ...
    setPendingAnchor({
      ...
      feedback: hasExistingScore ? String(existing.feedback || "").slice(0, 2e3) : ""
    });
  };
  ...
  const confirmPendingAnchor = () => {
    ...
    const feedback = String(pendingAnchor.feedback || "").trim().slice(0, 2e3);
    if (pendingAnchor.mode === "manual") {
      ...
      setGrades((previous) => ({
        ...
        [pendingAnchor.submissionIdx]: {
          ...
          [pendingAnchor.responseKey]: { score, status, feedback, origin: "teacher-edit" }
        }
      }));
      setReviewedRows((previous) => {
  ...
  const pendingScoreValid = !!(pendingAnchor && String(pendingAnchor.score == null ? "" : pendingAnchor.score).trim() && Number.isFinite(Number(pendingAnchor.score)) && Number(pendingAnchor.score) >= 0 && Number(pendingAnchor.score) <= 100 && (pendingAnchor.mode !== "manual" || pendingAnchor.scoreTouched));
  ...
  const rowGradeSummary = (idx) => {
    ...
    const valid = entries.map((entry) => ({ entry, grade: rowGrades[entry.key] })).filter((item) => {
      const grade = item.grade;
      if (!grade || grade.status === "error" || typeof grade.score !== "number" || !Number.isFinite(grade.score) || grade.score < 0 || grade.score > 100) return false;
  ...
  const _writeRowToGradebook = (idx) => {
    ...
    const savedGrades = /* @__PURE__ */ Object.create(null);
  ...
  const alloSheetReviewOptions = React.useMemo(() => {
    ...
    try {
      ...
      return {
        review: null,
        error: error && error.message ? error.message : "Submission Inbox could not prepare the saved-grade review."
      };
    }
  }, [
    showAlloSheetReview,
  ...
  const alloSheetPreview = React.useMemo(() => {
    ...
    try {
      ...
      return {
        artifact: null,
        error: error && error.message ? error.message : "Submission Inbox could not prepare a bounded AlloSheet preview."
      };
    }
  }, [
    showAlloSheetReview,
  ...
  const openAlloSheetReview = (event) => {
    ...
    try {
      const snapshot = siPrepareAlloSheetSavedSource({ gradebookEntries });
      ...
      alloSheetReturnFocusRef.current = event && event.currentTarget ? event.currentTarget : document.activeElement;
      ...
      setAlloSheetFeedback(review.assignments.length > 0 ? { kind: "", text: "" } : {
        kind: "status",
      ...
      addToast && addToast(
        error && error.message ? error.message : tr("Submission Inbox could not prepare the AlloSheet review."),
        "error"
      );
    }
  };
  const updateAlloSheetWindow = (dateRange, attemptPolicy) => {
    ...
    try {
      ...
      setAlloSheetFeedback({
        kind: "error",
        text: error && error.message ? error.message : "Submission Inbox could not update the saved-grade review."
      });
    }
  };
  ...
  const confirmAlloSheetReview = async () => {
    ...
    try {
      ...
      setAlloSheetFeedback({
        kind: "error",
        text: error && error.message ? error.message : "AlloSheet could not receive the reviewed Submission Inbox summaries."
      });
    } finally {
      alloSheetBusyRef.current = false;
      setAlloSheetBusy(false);
  ...
  const renderAlloSheetReview = () => {
    ...
    const assignmentReviewCodes = /* @__PURE__ */ new Map();
    assignments.filter((assignment) => alloSheetAssignments.includes(assignment.key)).forEach((assignment, index) => {
    ...
    return window.ReactDOM.createPortal(e("div", {
      ...
      e(
        ...
        e(
          ...
          assignments.length === 0 ? e("p", {
            role: "status",
            style: { margin: 0, color: "#92400e", fontWeight: 700 }
          }, tr("No saved assignment/class groups match these choices.")) : e("div", {
            style: {
              display: "grid",
              gap: 7,
              maxHeight: 190,
            ...
            e("input", {
              ...
              onChange: (event) => {
                ...
                setAlloSheetAssignments((previous) => checked ? previous.concat([assignment.key]).filter((title, index, list) => list.indexOf(title) === index) : previous.filter((title) => title !== assignment.key));
                setAlloSheetFeedback({ kind: "", text: "" });
              },
            ...
            e(
              ...
              }, (assignmentReviewCodes.get(assignment.key) ? assignmentReviewCodes.get(assignment.key) + " - " : "") + assignment.label),
              e("span", {
          ...
          }, review.omittedAssignmentCount + " additional assignment option" + (review.omittedAssignmentCount === 1 ? " is" : "s are") + " omitted by the 50-assignment review limit.")
        ),
        e(
          "fieldset",
          {
        ...
        e(
          ...
          artifact && artifact.provenance && artifact.provenance.truncatedSourceEntryCount > 0 && e("p", {
            role: "status",
            style: {
              margin: "9px 0 0",
              padding: 8,
          ...
          }, artifact.provenance.truncatedSourceEntryCount + " saved gradebook record" + (artifact.provenance.truncatedSourceEntryCount === 1 ? " was" : "s were") + " omitted by the 2,000-record source-review safety limit."),
          artifact && artifact.provenance && artifact.provenance.truncatedGradeResultCount > 0 && e("p", {
            role: "status",
            style: {
              margin: "9px 0 0",
              padding: 8,
          ...
          }, artifact.provenance.truncatedGradeResultCount + " grade result" + (artifact.provenance.truncatedGradeResultCount === 1 ? " was" : "s were") + " omitted by the 200-results-per-saved-record safety limit."),
          artifact && artifact.provenance && artifact.provenance.suppressedScoreSummaryCount > 0 && e("p", {
            role: "status",
            style: {
              margin: "9px 0 0",
              padding: 8,
          ...
          }, artifact.provenance.suppressedScoreSummaryCount + " score summary " + (artifact.provenance.suppressedScoreSummaryCount === 1 ? "is" : "rows are") + " privacy-suppressed.")
        ),
        e(
          "div",
          {
      ...
      e(
        ...
        e("button", {
          ...
          disabled: alloSheetBusy || !!alloSheetReviewOptions.error || !!alloSheetPreview.error || !hasRows,
          style: {
            minHeight: 44,
            padding: "8px 17px",
            ...
            fontWeight: 900,
            cursor: alloSheetBusy ? "not-allowed" : "pointer",
            opacity: alloSheetBusy || !!alloSheetReviewOptions.error || !!alloSheetPreview.error || !hasRows ? 0.55 : 1
          }
  ...
  const rosterBadge = (input) => {
    ...
    if (kind === "exact") {
      bg = "#dcfce7";
      color = "#166534";
      label = tr("\u2713 Roster match");
      title = tr("Exact roster match");
    } else if (kind === "fuzzy") {
      bg = "#dcfce7";
      color = "#166534";
      label = "\u2713 " + name;
      ...
      bg = "#fef3c7";
      color = "#92400e";
      label = tr("\u26A0 Unknown name");
      title = tr("No roster student matched this nickname");
    }
    return /* @__PURE__ */ React.createElement("span", {
      title,
      style: {
      ...
      /* @__PURE__ */ React.createElement(
        ...
        /* @__PURE__ */ React.createElement(
            ...
            /* @__PURE__ */ React.createElement(
              ...
              /* @__PURE__ */ React.createElement(
                ...
                privateJwk ? /* @__PURE__ */ React.createElement(
                  "div",
                  { role: "status", "aria-live": "polite", "aria-atomic": "true", style: { fontSize: "0.85rem", color: "#1e40af" } },
                  tr("\u2713 Loaded"),
                  classKeyMeta?.className ? ' for "' + classKeyMeta.className + '"' : "",
                  classKeyMeta?.createdAt ? " (created " + classKeyMeta.createdAt.slice(0, 10) + ")" : ""
                ) : /* @__PURE__ */ React.createElement(
                  "div",
                  { role: "status", "aria-live": "polite", "aria-atomic": "true", style: { fontSize: "0.85rem", color: "#64748b" } },
              ...
              }, privateJwk ? tr("\u{1F501} Load different key") : tr("\u{1F511} Load class key"))
            ),
          ...
          /* @__PURE__ */ React.createElement(
            ...
            /* @__PURE__ */ React.createElement(
              ...
              /* @__PURE__ */ React.createElement(
                ...
                /* @__PURE__ */ React.createElement(
                  "div",
                  { role: "status", "aria-live": "polite", "aria-atomic": "true", style: { fontSize: "0.85rem", color: "#475569" } },
                  queue.length === 0 ? tr("Select the files students saved \u2014 encrypted .alloflow.html (needs the class key) or plain .json (no key needed).") : queue.length + " file" + (queue.length === 1 ? "" : "s") + " loaded \xB7 " + (counts.decrypted || 0) + " decrypted \xB7 " + (counts.pending || 0) + " pending \xB7 " + (counts.error || 0) + " error"
                )
              ),
              /* @__PURE__ */ React.createElement(
                "div",
            ...
            /* @__PURE__ */ React.createElement("input", {
              ...
              style: { display: "none" },
              "aria-label": tr("Submission files")
            })
          ),
          ...
          (counts.decrypted || 0) > 0 && /* @__PURE__ */ React.createElement(
            ...
            /* @__PURE__ */ React.createElement(
              ...
              /* @__PURE__ */ React.createElement(
                ...
                /* @__PURE__ */ React.createElement("button", {
                  type: "button",
                  onClick: () => {
                    if (!bulkGrading) gradeAllDecrypted();
                  },
                  disabled: !bulkGrading && ((counts.decrypted || 0) === 0 || !(globalRubric.rubric || "").trim()),
                  "aria-disabled": bulkGrading ? "true" : void 0,
                  "aria-busy": bulkGrading ? "true" : void 0,
                  "aria-describedby": bulkGrading ? "submission-inbox-bulk-progress-label" : void 0,
                  title: (globalRubric.rubric || "").trim() ? tr("Grade every decrypted submission in the queue with this rubric") : tr("Set a class rubric first"),
                  style: {
                    padding: "8px 16px",
                ...
                bulkGrading && /* @__PURE__ */ React.createElement(
                  "div",
                  { role: "status", "aria-live": "polite", "aria-atomic": "true", style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", fontSize: "0.85rem", color: "#3730a3", fontWeight: 700 } },
            ...
            globalRubricOpen && /* @__PURE__ */ React.createElement(
              "div",
              { id: "submission-inbox-class-rubric-panel", style: { marginTop: 12, paddingTop: 12, borderTop: "1px solid #c7d2fe" } },
              ...
              /* @__PURE__ */ React.createElement(
                ...
                /* @__PURE__ */ React.createElement(
                  ...
                  /* @__PURE__ */ React.createElement("input", {
                    ...
                    style: { display: "none" },
                    "aria-label": tr("Import presets JSON")
                  }),
              ...
              (savedSessionMeta || (globalRubric.rubric || "").trim() || anchors.length > 0) && /* @__PURE__ */ React.createElement(
                ...
                /* @__PURE__ */ React.createElement(
                  "span",
                  null,
                  savedSessionMeta ? "\u{1F4BE} Restored from saved session (" + new Date(savedSessionMeta.savedAt).toLocaleString() + "). Auto-saves on every change." : tr("\u{1F4BE} Auto-saves your rubric + anchors so they persist across browser refreshes.")
                ),
                /* @__PURE__ */ React.createElement("button", {
                  type: "button",
          ...
          /* @__PURE__ */ React.createElement(
            ...
            /* @__PURE__ */ React.createElement(
              ...
              gradebookEntries.length > 0 && /* @__PURE__ */ React.createElement(
                "div",
                { style: { display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", justifyContent: "flex-end", maxWidth: "100%" } },
                /* @__PURE__ */ React.createElement(
                  "div",
                  { role: "group", "aria-label": tr("Group gradebook by"), style: { display: "inline-flex", background: "white", border: "1px solid #86efac", borderRadius: 6, padding: 2, fontSize: "0.74rem", fontWeight: 600 } },
                  /* @__PURE__ */ React.createElement("button", {
                    type: "button",
            ...
            gradebookOpen && /* @__PURE__ */ React.createElement(
              "div",
              { id: "submission-inbox-gradebook-panel", style: { marginTop: 12, paddingTop: 12, borderTop: "1px solid #bbf7d0" } },
              gradebookEntries.length === 0 ? /* @__PURE__ */ React.createElement(
                "div",
                { role: "status", "aria-live": "polite", style: { fontSize: "0.85rem", color: "#166534", fontStyle: "italic" } },
                tr('No saved grades yet. Grade some submissions and click "\u{1F4BE} Save all graded" or the per-row "Save to gradebook" button to populate this list.')
              ) : /* @__PURE__ */ React.createElement(
                "div",
                null,
                /* @__PURE__ */ React.createElement(
          ...
          (counts.decrypted || 0) > 0 && /* @__PURE__ */ React.createElement(
            ...
            /* @__PURE__ */ React.createElement(
              ...
              /* @__PURE__ */ React.createElement(
                ...
                anchors.length === 0 ? tr("Tap \u{1F4CC} on any decrypted response to teach the AI your scoring direction.") : "Each AI grading run will use these " + anchors.length + " anchor" + (anchors.length === 1 ? "" : "s") + " as few-shot examples."
              )
            ),
            anchorsPanelOpen && /* @__PURE__ */ React.createElement(
              "div",
              { id: "submission-inbox-anchors-panel", style: { marginTop: 10, paddingTop: 10, borderTop: "1px solid #fde68a" } },
          ...
          queue.length === 0 ? /* @__PURE__ */ React.createElement(
            "div",
            { role: "status", "aria-live": "polite", style: { padding: "40px 20px", textAlign: "center", color: "#475569", fontSize: "0.95rem", border: "2px dashed #e2e8f0", borderRadius: 12 } },
              ...
              /* @__PURE__ */ React.createElement(
                ...
                queue.map((row, idx) => /* @__PURE__ */ React.createElement(
                  ...
                  expandedRow === idx && row.payload && /* @__PURE__ */ React.createElement(
                    "tr",
                    { id: "submission-inbox-row-details-" + idx },
                    /* @__PURE__ */ React.createElement(
                      "td",
                      ...
                      /* @__PURE__ */ React.createElement(
                        ...
                        siResponseEntryModels(row.payload).length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontStyle: "italic", color: "#475569", fontSize: "0.85rem" } }, tr("No responses captured.")) : siResponseEntryModels(row.payload).map((entry, i) => {
                          const k = entry.key;
                          ...
                          return /* @__PURE__ */ React.createElement(
                            ...
                            /* @__PURE__ */ React.createElement(
                              ...
                              /* @__PURE__ */ React.createElement(
                                ...
                                entry.partLabel && /* @__PURE__ */ React.createElement(
                                  "div",
                                  { style: { color: "#475569", fontSize: "0.74rem", fontWeight: 600, marginTop: 2 } },
                                  entry.partLabel + (entry.responseType ? " \xB7 " + entry.responseType.replace(/-/g, " ") : "")
                                ),
                                ...
                                /* @__PURE__ */ React.createElement(
                                  "div",
                                  { style: { color: "#1e293b", marginTop: 4, whiteSpace: "pre-wrap", overflowWrap: "anywhere" } },
                                  entry.answered ? String(entry.displayValue).slice(0, 12e3) : /* @__PURE__ */ React.createElement("em", { style: { color: "#9a3412", fontWeight: 700 } }, tr("No answer submitted"))
                                ),
                                entry.answered && String(entry.displayValue) !== String(entry.rawText) && /* @__PURE__ */ React.createElement(
                                  "div",
                                  { style: { color: "#64748b", marginTop: 3, fontSize: "0.7rem", overflowWrap: "anywhere" } },
                                  tr("Stored value:") + " ",
                                  /* @__PURE__ */ React.createElement("code", null, String(entry.rawText).slice(0, 12e3))
                                ),
                                /* @__PURE__ */ React.createElement(
                                  "details",
                                  { style: { marginTop: 4, color: "#64748b", fontSize: "0.7rem" } },
                                  ...
                                  /* @__PURE__ */ React.createElement("code", { style: { display: "block", whiteSpace: "pre-wrap", overflowWrap: "anywhere" } }, String(entry.rawText).slice(0, 12e3))
                                )
                              ),
                              entry.requiresManualReview ? /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => openManualScoreForm(idx, entry), title: tr("Score this response yourself. It is not sent to the free-response AI grader."), style: { display: "inline-flex", alignItems: "center", padding: "3px 9px", borderRadius: 999, background: "#e0f2fe", color: "#075985", border: "1px solid #7dd3fc", fontWeight: 700, fontSize: "0.72rem", flexShrink: 0, whiteSpace: "nowrap", cursor: "pointer" } }, g && g.origin === "teacher-edit" ? tr("Edit score") : tr("Score manually")) : entry.answered && (isAnchored ? /* @__PURE__ */ React.createElement(
                                "span",
                        ...
                        Object.keys(row.payload.responses || {}).length > 0 && /* @__PURE__ */ React.createElement(
                          ...
                          /* @__PURE__ */ React.createElement(
                            "div",
                            { style: { fontWeight: 700, color: "#475569", fontSize: "0.8rem", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" } },
                            tr("\u{1F3AF} AI Grade with rubric")
                          ),
                          /* @__PURE__ */ React.createElement(
                            "label",
                            { htmlFor: "submission-inbox-row-rubric-" + idx, style: { display: "block", fontSize: "0.78rem", color: "#475569", fontWeight: 600, marginBottom: 4 } },
                          ...
                          /* @__PURE__ */ React.createElement(
                            "label",
                            { htmlFor: "submission-inbox-row-exemplar-" + idx, style: { display: "block", fontSize: "0.78rem", color: "#475569", fontWeight: 600, marginBottom: 4 } },
                            tr("Quick exemplar (optional) "),
                            /* @__PURE__ */ React.createElement(
                              "span",
                              { style: { fontWeight: 400, color: "#475569", fontSize: "0.75rem" } },
                              anchors.length > 0 ? "\u2014 " + anchors.length + " calibration anchor" + (anchors.length === 1 ? "" : "s") + " active (\u{1F4CC} panel above). Anchors apply across all submissions; this exemplar adds one more locally." : tr("\u2014 or tap \u{1F4CC} on a real student response above to anchor it (multi-anchor calibration).")
                            )
                          ),
                          /* @__PURE__ */ React.createElement("textarea", {
                          ...
                          /* @__PURE__ */ React.createElement(
                            ...
                            Object.keys(grades[idx] || {}).length > 0 && /* @__PURE__ */ React.createElement(
                              "span",
                              { role: "status", "aria-live": "polite", style: { fontSize: "0.78rem", color: "#64748b" } },
                              (() => {
                                const summary = rowGradeSummary(idx);
                                return "Avg: " + (summary.average == null ? "\u2014" : summary.average + "/100") + " \xB7 " + summary.scored + " of " + summary.total + (summary.expectedCountKnown ? " expected" : " captured") + " responses scored";
                              })()
                            )
                          )
                        )
        ...
        /* @__PURE__ */ React.createElement(
          ...
          /* @__PURE__ */ React.createElement(
            ...
            (counts.decrypted || 0) > 0 ? tr('Click "View" on a row to grade it with an AI rubric and save the result to your local gradebook.') : tr("Decrypt the queue to see student responses.")
          ),
          /* @__PURE__ */ React.createElement(
            "div",
            { style: { display: "flex", gap: 8 } },
        ...
        pendingAnchor && /* @__PURE__ */ React.createElement(
          ...
          /* @__PURE__ */ React.createElement(
            ...
            }, style: { background: "white", borderRadius: 14, boxShadow: "0 12px 40px rgba(0,0,0,0.25)", maxWidth: 540, width: "100%", padding: "20px 24px", border: pendingAnchor.mode === "manual" ? "2px solid #7dd3fc" : "2px solid #fde68a" } },
            /* @__PURE__ */ React.createElement("h3", { id: "anchor-dialog-title", style: { margin: "0 0 8px 0", fontSize: "1.05rem", fontWeight: 800, color: pendingAnchor.mode === "manual" ? "#075985" : "#92400e" } }, tr(pendingAnchor.mode === "manual" ? "Score this response" : "\u{1F4CC} Anchor this response")),
            /* @__PURE__ */ React.createElement(
              "p",
              { id: "anchor-dialog-description", style: { margin: "0 0 12px 0", fontSize: "0.85rem", color: "#64748b" } },
              tr(pendingAnchor.mode === "manual" ? "Enter your score for this structured response. It will be saved as a teacher edit." : "Give this response a score. The AI will use it as a calibration example when grading every other response.")
            ),
            /* @__PURE__ */ React.createElement(
              "div",
            ...
            /* @__PURE__ */ React.createElement(
              ...
              /* @__PURE__ */ React.createElement("input", {
                ...
                max: 100,
                step: 5,
                "aria-label": tr(pendingAnchor.mode === "manual" ? "Manual score slider" : "Anchor score slider"),
              ...
              /* @__PURE__ */ React.createElement("input", {
                ...
                min: 0,
                max: 100,
                "aria-label": tr(pendingAnchor.mode === "manual" ? "Manual score" : "Anchor score"),
                value: pendingAnchor.score,
                onChange: (e) => setPendingAnchor({ ...pendingAnchor, score: e.target.value, scoreTouched: true }),
            ...
            /* @__PURE__ */ React.createElement(
              ...
              }, tr(pendingAnchor.mode === "manual" ? "Save score" : "\u{1F4CC} Add anchor"))
            )
          )
        ),
        confirmation && /* @__PURE__ */ React.createElement(
          "div",
          {
            role: "presentation",
          ...
          /* @__PURE__ */ React.createElement(
            "div",
            {
              ...
              "aria-describedby": "submission-confirm-message",
              tabIndex: -1,
              onKeyDown: (event) => {
                event.stopPropagation();
                if (event.key === "Escape") {
