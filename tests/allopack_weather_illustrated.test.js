...
const pack=read('allopacks/illustrated/weather_vs_climate_grade5.allopack.json');
...
describe('illustrated weather and climate accessibility and portability',()=>{
 ...
 it('embeds exactly the reviewed files without network image dependencies',()=>{
  const manifest={assets:read('allopacks/media/weather_vs_climate_grade5/embedded-assets.json')};
  const urls=[...glossary.map(i=>i.image),...panels.map(i=>i.imageUrl)];
  expect(new Set(urls).size).toBe(24);
  for(const a of manifest.assets){
   const expected='data:image/webp;base64,'+fs.readFileSync('allopacks/media/weather_vs_climate_grade5/'+a.file).toString('base64');
 ...
 it('preserves the original activity IDs and corrects key definitions',()=>{
  const original=read('allopacks/weather_vs_climate_grade5.allopack.json');
  for(const r of original.history)expect(pack.history.some(p=>p.id===r.id&&p.type===r.type)).toBe(true);
  expect(glossary.find(g=>g.term==='Atmosphere').def).toContain('troposphere');
  expect(glossary.find(g=>g.term==='Average').def).toContain('dividing');
 });
});
