...
const wave = require('../dev-tools/eppp_native_quality_wave_40_data.cjs');
...
describe('EPPP expanded quality wave 40', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_40.json');
    ...
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(438);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1218);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(587);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(579);
    expect(audit.summary.feedbackWarningsAfter.choiceRestatementOptions).toBe(138);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_40.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_40.json'));
  });

  it('replays after wave 39', () => {
    const builder = read('dev-tools/build_eppp_1500_expansion.cjs');
    const i39 = builder.indexOf('repair_eppp_native_quality_wave_39.cjs');
    const i40 = builder.indexOf('repair_eppp_native_quality_wave_40.cjs');
    expect(i40).toBeGreaterThan(i39);
  });
});
