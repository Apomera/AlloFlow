...
const { baselineMetrics, reviewedAt, reviewWave, revisions } = require('./eppp_native_quality_wave_47_data.cjs');
...
const auditBasename = 'eppp_question_quality_audit_wave_47.json';
...
if (fs.readFileSync(deployBankPath, 'utf8') !== sourceText) throw new Error('Source and deploy EPPP banks must match before wave 47.');
...
if (revisions.length !== 24 || new Set(revisions.map((revision) => revision.id)).size !== 24) throw new Error('Wave 47 needs twenty-four unique revisions.');
...
for (let waveNumber = 25; waveNumber <= 46; waveNumber += 1) {
...
for (const revision of revisions) {
  ...
  if (!recognizedReplay && item.prompt !== revision.expectedPrompt) throw new Error(revision.id + ' prompt drifted before wave 47.');
  if (item.answerIndex !== revision.expectedAnswerIndex) throw new Error(revision.id + ' answer position drifted.');
  if (!recognizedReplay && item.difficulty !== revision.expectedDifficulty) throw new Error(revision.id + ' difficulty drifted before wave 47.');
...
console.log('EPPP quality wave 47: rewrote 24 items across 8 domains; selected warnings after = 0.');
