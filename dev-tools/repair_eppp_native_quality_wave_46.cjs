...
const { baselineMetrics, reviewedAt, reviewWave, revisions } = require('./eppp_native_quality_wave_46_data.cjs');
...
const auditBasename = 'eppp_question_quality_audit_wave_46.json';
...
if (fs.readFileSync(deployBankPath, 'utf8') !== sourceText) throw new Error('Source and deploy EPPP banks must match before wave 46.');
...
if (revisions.length !== 24 || new Set(revisions.map((revision) => revision.id)).size !== 24) throw new Error('Wave 46 needs twenty-four unique revisions.');
const selectedIds = new Set(revisions.map((revision) => revision.id));
const itemsById = new Map(bank.map((item) => [item.id, item]));

// Reapply frozen after-states for waves 25–45 before this batch. The full
// expansion builder can visit historical waves in a different order, so this
// replay contract makes every earlier quality test deterministic.
for (let waveNumber = 25; waveNumber <= 45; waveNumber += 1) {
...
for (const revision of revisions) {
  ...
  if (!recognizedReplay && item.prompt !== revision.expectedPrompt) throw new Error(revision.id + ' prompt drifted before wave 46.');
  if (item.answerIndex !== revision.expectedAnswerIndex) throw new Error(revision.id + ' answer position drifted.');
  if (!recognizedReplay && item.difficulty !== revision.expectedDifficulty) throw new Error(revision.id + ' difficulty drifted before wave 46.');
...
const audit = {
  ...
  scope: 'Twenty-four EPPP items, three per domain, rewritten as applied or analytic decisions with balanced distractors and concise misconception-specific feedback.',
  summary: {
...
console.log('EPPP quality wave 46: rewrote 24 items across 8 domains; selected warnings after = 0.');
