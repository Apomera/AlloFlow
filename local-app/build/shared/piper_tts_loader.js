...
(function () {
    ...
    // ─── Constants ──────────────────────────────────────────────────────
    // Local bundle (served from localhost — avoids CDN tracking prevention blocking IndexedDB)
    const PIPER_LOCAL = './libs/piper-tts-web/piper-tts-web.js';
    // CDN fallback (used if local bundle fails to load, e.g. web/deploy mode)
    ...
    async function _ensureLibLoaded() {
        ...
        _libInitPromise = (async () => {
            try {
                ...
                // Try local bundle first — avoids CDN tracking prevention blocking IndexedDB
                try {
                    _PiperLib = await import(/* webpackIgnore: true */ PIPER_LOCAL);
                    _fireProgress('Piper library loaded (local)', 0.15);
                } catch (localErr) {
                    console.warn('[Piper TTS] Local bundle unavailable, falling back to CDN:', localErr.message);
                    _PiperLib = await import(/* webpackIgnore: true */ PIPER_CDN);
                    _fireProgress('Piper library loaded (CDN)', 0.15);
                }
                return _PiperLib;
            } catch (e) {
