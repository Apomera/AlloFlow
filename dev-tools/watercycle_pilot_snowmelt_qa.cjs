...
const out = path.join(ROOT, 'scratch', 'water-snowmelt-review');
...
(async () => {
  ...
  try {
    ...
    const localMelt=()=>page.evaluate(()=>{
      ...
      return {scale:group.children[0].scale.x,opacity:group.children[0].material.opacity,
    ...
    console.log('PASS: seasonal ridge snow shader, finite height attributes, actual melting and energy cue, paused and reduced-motion transition, first-person proxy, cue expiry and restored rain scale, no fake restored melt, mobile accessibility, shared resource cleanup, no WebGL errors.');
