...
// Lazy Lucide icon resolution from window.AlloIcons (populated by
  ...
  function renderPhonicsDictRow(phonicsData, t, playDictionaryAudio) {
    ...
    if (d.audio) row.push(React.createElement('button', {
      key: 'aud', type: 'button',
      onClick: function () { playDictionaryAudio(d.audio); },
  ...
  function renderFlashcardDictBack(item, t, flashcardDictAudioKey, playDictionaryAudio) {
    ...
    body.push(React.createElement('details', {
      ...
      React.createElement('summary', { className: 'cursor-pointer font-bold text-blue-50 select-none' }, t('common.resource')),
  ...
  function GlossaryView(props) {
    ...
    function activityText(key, fallback) { var value = t(key); return value && value !== key ? value : fallback; }
    ...
    function startGlossaryActivity(name, start) {
      ...
      var data = activityWords.map(item => ({ ...item, translations: item.translations ? { ...item.translations } : item.translations }));
      setActivitySessions(prev => ({ ...prev, [name]: { resourceKey: activityResourceKey, data, boardSize: activityBoardSize === 'standard' ? undefined : Number(activityBoardSize) } }));
      if (name === 'bingo' && typeof setBingoState === 'function') setBingoState({ cards: [], drawPile: [], calledTerms: [], currentCall: null });
      start(data);
    }
    function startScopedWordSearch(language) { startGlossaryActivity('wordsearch', data => generateWordSearch(language, data)); }
    React.useEffect(() => { setActivitySessions({}); setActivityScope('all'); }, [activityResourceKey]);
    var detectiveState = React.useState(false);
    var detectiveOpen = detectiveState[0], setDetectiveOpen = detectiveState[1];
    ...
    React.useEffect(function () { setDetectiveOpen(false); }, [generatedContent && generatedContent.id]);
    ...
    var glossaryToolsState = React.useState({ games: false, teacher: false });
    ...
    var wordSearchActiveState = React.useState({ r: 0, c: 0 });
    ...
    var wordSearchFoundState = React.useState(function () { return new Set(); });
    ...
    var glossaryViewSessionRef = React.useRef({ key: glossaryResourceKey });
    if (glossaryViewSessionRef.current.key !== glossaryResourceKey) glossaryViewSessionRef.current = { key: glossaryResourceKey };
    ...
    var glossaryAudioOwnerRef = React.useRef({ key: glossaryResourceKey, signature: glossaryAudioSignature });
    if (glossaryAudioOwnerRef.current.key !== glossaryResourceKey || glossaryAudioOwnerRef.current.signature !== glossaryAudioSignature) glossaryAudioOwnerRef.current = { key: glossaryResourceKey, signature: glossaryAudioSignature };
    ...
    function isGlossaryViewCurrent(owner) { return glossaryMountedRef.current && glossaryViewSessionRef.current === owner; }
    function isGlossaryAudioCurrent(owner) { return glossaryMountedRef.current && glossaryAudioOwnerRef.current === owner; }
    function playGlossaryDictionaryAudio(url, key) {
      ...
      try {
        ...
        player.addEventListener('error', clear, { once: true });
        var playback = player.play();
        if (playback && typeof playback.catch === 'function') playback.catch(clear);
      } catch (_) { if (isGlossaryAudioCurrent(owner)) setFlashcardDictAudioKey(null); }
    }
    ...
    React.useEffect(function () {
      ...
      setGlossaryToolsOpen({ games: false, teacher: false });
      return function () {
    ...
    React.useEffect(function () {
      setFlashcardDictAudioKey(null);
      setGlossaryAudioPrep({ busy: false, done: 0, total: 0, message: '' });
      setGlossaryAudioEditState({ busyKey: '', message: '' });
      return function () {
    ...
    function containModalFocus(e, container, onEscape) {
      ...
      if (document.activeElement === container) { e.preventDefault(); (e.shiftKey ? last : first).focus(); }
    ...
    function flashcardText(key, fallback) { var value = t(key); return value && value !== key ? value : fallback; }
    React.useEffect(function () {
      setFlashcardSummary(false); setFlashcardRoundKeys(null); setFlashcardReviewState({});
      flashcardNavigationRef.current += 1; clearTimeout(flashcardMoveTimerRef.current);
    ...
    function moveToFlashcardIndex(idx) {
      ...
      clearTimeout(flashcardMoveTimerRef.current);
      flashcardMoveTimerRef.current = setTimeout(function () { if (isGlossaryViewCurrent(owner)) setFlashcardIndex(idx); }, 150);
    }
    function finishFlashcardRound(e) {
      stopFlashcardControl(e); flashcardNavigationRef.current += 1;
      clearTimeout(flashcardMoveTimerRef.current); stopGlossarySavedAudio(true);
      setIsFlashcardEditDrawerOpen(false); setFlashcardSummary(true);
    }
    function goToNextReviewCard(e) {
      stopFlashcardControl(e);
      if (flashcardRoundLast) finishFlashcardRound();
      else moveToFlashcardIndex(flashcardRoundIndices[flashcardRoundPosition + 1]);
    }
    ...
    function markFlashcardConfidence(status, e) {
      ...
      setFlashcardReviewState(prev => ({ ...prev, [currentFlashcardKey]: status }));
      goToNextReviewCard();
    }
    function handleReviewedQuizOption(e, option) {
      ...
      setFlashcardReviewState(prev => ({ ...prev, [currentFlashcardKey]: option === answer ? 'known' : 'learning' }));
      var owner = glossaryViewSessionRef.current, token = ++flashcardNavigationRef.current;
    ...
    function restartFlashcardRound(onlyPractice) {
      ...
      flashcardNavigationRef.current += 1; clearTimeout(flashcardMoveTimerRef.current);
      setFlashcardRoundKeys(keys); setFlashcardSummary(false);
      setFlashcardReviewState(prev => onlyPractice ? Object.fromEntries(Object.entries(prev).filter(([key]) => !keys.includes(key))) : {});
      setFlashcardScore(0); setFlashcardFeedback(null); setQuizSelectedOption(null);
      setIsFlashcardFlipped(false); setFlashcardIndex(keys ? reviewKeys.indexOf(keys[0]) : 0);
    ...
    function renderFlashcardSummary() {
      ...
      return <section data-flashcard-summary="true" className="w-full max-w-2xl rounded-3xl bg-white p-6 sm:p-10 text-slate-900 shadow-xl">
        <h2 ref={flashcardSummaryRef} tabIndex={-1} className="text-2xl font-black rounded focus-visible:ring-2 focus-visible:ring-indigo-600 focus-visible:ring-offset-4">{flashcardText('flashcards.round_complete', 'Study round complete')}</h2>
        <p className="mt-3">{flashcardText('flashcards.summary_help', 'Choose what to practice next. Cards you skipped are counted as not rated.')}</p>
        <dl className="my-6 grid grid-cols-1 sm:grid-cols-3 gap-3 text-center">
          {[[flashcardText('flashcards.known_count', 'Known'), knownCount], [flashcardText('flashcards.practice_count', 'Needs practice'), learningCount], [flashcardText('flashcards.unrated_count', 'Not rated'), reviewKeys.length - knownCount - learningCount]].map(([label, count]) => <div key={label} className="rounded-xl bg-slate-100 p-3"><dt className="text-sm">{label}</dt><dd className="text-2xl font-bold">{count}</dd></div>)}
        </dl>
        <div className="flex flex-wrap gap-3">
          {remaining > 0 && <button type="button" className={buttonClass + ' bg-indigo-700 text-white'} onClick={() => restartFlashcardRound(true)}>{flashcardText('flashcards.review_practice', 'Review cards needing practice')} ({remaining})</button>}
          <button type="button" className={buttonClass} onClick={() => restartFlashcardRound(false)}>{flashcardText('flashcards.start_again', 'Start again')}</button>
          <button type="button" className={buttonClass} onClick={closeInteractiveFlashcards}>{flashcardText('common.done', 'Done')}</button>
        </div>
      </section>;
    }
    ...
    function renderFlashcardProgressDots() {
      ...
      return <div role="group" className="mt-3 flex items-center gap-1.5 overflow-x-auto pb-1 pr-1" aria-label={t('common.progress')}>{flashcardDeck.map(function (item, idx) {
        ...
        return <button key={key + '-' + idx} type="button" onClick={function (e) { handleProgressDotClick(idx, e); }} className={`h-9 w-9 rounded-full shrink-0 flex items-center justify-center transition-all motion-reduce:transition-none  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${isCurrent ? 'bg-white/15 ring-2 ring-white shadow-lg' : 'hover:bg-white/15'}`} aria-current={isCurrent ? 'step' : undefined} aria-label={`${glossaryProgressLabel} ${idx + 1}: ${item?.term || flashcardTermLabel}, ${statusLabel}`} title={`${item?.term || `${flashcardTermLabel} ${idx + 1}`} - ${statusLabel}`}><span className={`h-4 w-4 rounded-full flex items-center justify-center ${fillClass}`}>{status === 'known' ? <CheckCircle2 size={11} aria-hidden="true" /> : status === 'learning' ? <RefreshCw size={10} aria-hidden="true" /> : null}</span></button>;
      })}</div>;
    }
    function renderFlashcardActionBar() {
      ...
      return <div className="sticky bottom-0 z-40 -mx-4 mt-5 border-t border-white/10 bg-slate-950/90 backdrop-blur-xl px-3 py-3 shadow-2xl pb-[calc(env(safe-area-inset-bottom)+0.75rem)]"><div className="mx-auto max-w-4xl space-y-3"><div className="flex flex-wrap items-center justify-center gap-2"><button type="button" aria-pressed={currentFlashcardConfidence === 'known'} onClick={e => markFlashcardConfidence('known', e)} className={`min-h-[44px] inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold border transition-all motion-reduce:transition-none ${focusClass} ${currentFlashcardConfidence === 'known' ? 'bg-emerald-300 text-emerald-950 border-emerald-100 shadow-lg' : 'bg-white/10 text-emerald-100 border-emerald-400/40 hover:bg-emerald-500/20'}`}><CheckCircle2 size={16} aria-hidden="true" /> {flashcardKnownLabel}</button><button type="button" aria-pressed={currentFlashcardConfidence === 'learning'} onClick={e => markFlashcardConfidence('learning', e)} className={`min-h-[44px] inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold border transition-all motion-reduce:transition-none ${focusClass} ${currentFlashcardConfidence === 'learning' ? 'bg-amber-400 text-slate-900 border-amber-200 shadow-lg' : 'bg-white/10 text-amber-100 border-amber-400/40 hover:bg-amber-500/20'}`}><RefreshCw size={16} aria-hidden="true" /> {flashcardLearningLabel}</button><button type="button" aria-pressed={isFlashcardFlipped} onClick={handleFlashcardFlipButton} className={`min-h-[44px] inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold bg-white/10 text-white border border-white/20 hover:bg-white/20 transition-all motion-reduce:transition-none ${focusClass}`}><RefreshCw size={16} aria-hidden="true" /> {isFlashcardFlipped ? flashcardTermLabel : flashcardDefinitionLabel}</button>{isTeacherMode && flashcardMode === 'standard' && <button type="button" ref={flashcardEditButtonRef} aria-expanded={isFlashcardEditDrawerOpen} aria-controls="flashcard-edit-drawer" onClick={handleEditCurrentFlashcard} className={`min-h-[44px] inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold bg-blue-50 text-blue-900 border border-blue-200 hover:bg-white transition-all motion-reduce:transition-none ${focusClass}`}><Pencil size={16} aria-hidden="true" /> {t('glossary.edit_term')}</button>}</div><div className="grid grid-cols-2 sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2"><button type="button" onClick={goToPreviousReviewCard} disabled={flashcardRoundPosition <= 0} className={`min-h-[44px] flex items-center justify-center gap-2 px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 text-white font-bold transition-colors motion-reduce:transition-none disabled:opacity-30 disabled:cursor-not-allowed ${focusClass}`} aria-label={t('common.prev_flashcard')} data-help-key="flashcard_prev"><ArrowDown className="rotate-90" size={20} aria-hidden="true" /> <span className="hidden sm:inline">{t('flashcards.previous')}</span></button><button type="button" onClick={handleCardAudioSequence} className={`col-span-2 order-first sm:col-span-1 sm:order-none justify-center min-h-[52px] px-6 sm:px-8 py-3 rounded-full shadow-[0_0_20px_rgba(234,179,8,0.35)] hover:scale-105 motion-reduce:hover:scale-100 transition-all motion-reduce:transition-none flex items-center gap-3 font-bold text-base sm:text-lg ${focusClass} ${audioActive ? 'bg-red-600 hover:bg-red-700 text-white ring-4 ring-red-300/30' : 'bg-yellow-500 hover:bg-yellow-400 text-slate-900'}`} title={t('common.play_audio_sequence')} aria-label={audioActive ? t('flashcards.stop') : t('flashcards.play_card')} data-help-key="flashcard_play_sequence">{audioActive ? <StopCircle size={24} className="fill-current animate-pulse motion-reduce:animate-none" aria-hidden="true" /> : <Volume2 size={24} className="fill-current" aria-hidden="true" />}{audioActive ? t('flashcards.stop') : t('flashcards.play_card')}</button><button type="button" onClick={e => goToNextReviewCard(e)} disabled={nextDisabled} className={`min-h-[44px] flex items-center justify-center gap-2 px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 text-white font-bold transition-colors motion-reduce:transition-none disabled:opacity-30 disabled:cursor-not-allowed ${focusClass}`} aria-label={flashcardRoundLast ? flashcardText('flashcards.finish_round', 'Finish round') : t('common.next_flashcard')} data-help-key="flashcard_next"><span className={flashcardRoundLast ? '' : 'hidden sm:inline'}>{flashcardRoundLast ? flashcardText('flashcards.finish_round', 'Finish round') : t('flashcards.next')}</span> <ArrowDown className="-rotate-90" size={20} aria-hidden="true" /></button></div></div></div>;
    }
    function renderFlashcardEditDrawer() {
      ...
      return <div className="fixed inset-0 z-[130] pointer-events-none"><button type="button" tabIndex={-1} className="absolute inset-0 bg-slate-950/40 pointer-events-auto" onClick={handleCloseFlashcardEditDrawer} aria-label={t('common.close')} /><aside id="flashcard-edit-drawer" ref={flashcardEditDrawerRef} role="dialog" aria-modal="true" aria-labelledby="flashcard-edit-title" onKeyDown={handleFlashcardEditDrawerKeyDown} className="absolute right-0 top-0 h-full w-full max-w-md bg-white text-slate-800 shadow-2xl pointer-events-auto p-5 overflow-y-auto animate-in slide-in-from-right-8 duration-200 motion-reduce:animate-none motion-reduce:transition-none" onClick={e => e.stopPropagation()} onMouseDown={e => e.stopPropagation()}><div className="flex items-start justify-between gap-3 mb-5"><div><p className="text-xs font-black uppercase tracking-widest text-blue-700">{t('common.edit')}</p><h3 id="flashcard-edit-title" className="text-xl font-black text-slate-900">{t('common.edit')} {flashcardTermLabel.toLowerCase()}</h3></div><button type="button" onClick={handleCloseFlashcardEditDrawer} className={`p-2 rounded-full text-slate-600 hover:text-slate-900 hover:bg-slate-100 ${drawerButtonFocus}`} aria-label={t('common.close')}><X size={20} aria-hidden="true" /></button></div><div className="space-y-4"><label className="block"><span className="block text-xs font-black uppercase tracking-widest text-slate-600 mb-1">{flashcardTermLabel}</span><textarea ref={flashcardEditFirstFieldRef} value={currentFlashcardItem.term || ''} onChange={e => handleGlossaryChange(flashcardIndex, 'term', e.target.value)} rows={getRows(currentFlashcardItem.term, 24)} className={`${fieldClass} text-base font-bold`} /></label><label className="block"><span className="block text-xs font-black uppercase tracking-widest text-slate-600 mb-1">{flashcardDefinitionLabel}</span><textarea value={currentFlashcardItem.def || ''} onChange={e => handleGlossaryChange(flashcardIndex, 'def', e.target.value)} rows={getRows(currentFlashcardItem.def, 42)} className={`${fieldClass} text-sm leading-relaxed`} /></label><label className="block"><span className="block text-xs font-black uppercase tracking-widest text-slate-600 mb-1">{t('glossary.tier2') || t('glossary.label_tier2')}</span><select value={currentFlashcardItem.tier || ''} onChange={e => handleGlossaryChange(flashcardIndex, 'tier', e.target.value)} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-bold  focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 bg-white"><option value="">{t('glossary.edit_tier_placeholder')}</option><option value="Academic">{t('glossary.edit_tier_academic')}</option><option value="Domain-Specific">{t('glossary.edit_tier_domain')}</option></select></label><label className="block"><span className="block text-xs font-black uppercase tracking-widest text-slate-600 mb-1">{t('glossary.etymology_label') || t('glossary.etymology_roots_label')}</span><textarea value={currentFlashcardItem.etymology || ''} onChange={e => handleGlossaryChange(flashcardIndex, 'etymology', e.target.value)} rows={getRows(currentFlashcardItem.etymology, 42)} className={`${fieldClass} text-sm leading-relaxed`} /></label></div><div className="mt-6 flex flex-col sm:flex-row gap-2"><button type="button" onClick={handleCloseFlashcardEditDrawer} className={`min-h-[44px] flex-1 inline-flex items-center justify-center gap-2 rounded-full bg-blue-700 text-white px-4 py-2.5 text-sm font-bold hover:bg-blue-800 ${drawerButtonFocus}`}><CheckCircle2 size={16} aria-hidden="true" /> {t('common.done')}</button><button type="button" onClick={handleOpenCurrentFlashcardInGlossary} className={`min-h-[44px] flex-1 inline-flex items-center justify-center gap-2 rounded-full bg-slate-100 text-slate-800 px-4 py-2.5 text-sm font-bold hover:bg-slate-200 ${drawerButtonFocus}`}><Search size={16} aria-hidden="true" /> {t('glossary.title')}</button></div></aside></div>;
    }
    React.useEffect(function () {
      if (wordSearchPathTimerRef.current) clearTimeout(wordSearchPathTimerRef.current);
      setWordSearchStart(null);
      setWordSearchActive({ r: 0, c: 0 });
      setWordSearchPath([]);
    ...
    function wordSearchPathsMatch(first, second) {
      ...
      var forward = first.every(function (key, index) { return key === second[index]; });
      var reverse = first.every(function (key, index) { return key === second[second.length - index - 1]; });
    ...
    function finishWordSearchPath(start, end) {
      ...
      var match = entries.find(function (entry) { return wordSearchPathsMatch(entry[1], path); });
      ...
      if (match && !wordSearchFoundWords.has(match[0])) {
        ...
        if (typeof addToast === 'function') addToast(t('glossary.word_search_notifications.found', { word: match[0] }), 'success');
        if (typeof handleScoreUpdate === 'function') handleScoreUpdate(5, 'Word Search Find', generatedContent?.id);
        if (complete && typeof handleGameCompletion === 'function') handleGameCompletion('wordSearch', { score: nextFound.size * 5, correctCount: nextFound.size, totalItems: total, isPerfect: true });
      } else if (match) {
      ...
      if (wordSearchPathTimerRef.current) clearTimeout(wordSearchPathTimerRef.current);
      wordSearchPathTimerRef.current = setTimeout(function () { setWordSearchPath([]); }, 650);
    }
    ...
    function handleWordSearchGridKeyDown(event) {
      ...
      var next = { r: wordSearchActive.r, c: wordSearchActive.c };
      if (event.key === 'ArrowRight') next.c = Math.max(0, Math.min(maxCol, next.c + (gameData.isRtl ? -1 : 1)));
      else if (event.key === 'ArrowLeft') next.c = Math.max(0, Math.min(maxCol, next.c + (gameData.isRtl ? 1 : -1)));
      else if (event.key === 'ArrowDown') next.r = Math.min(maxRow, next.r + 1);
      else if (event.key === 'ArrowUp') next.r = Math.max(0, next.r - 1);
      else if (event.key === 'Home') next.c = 0;
      else if (event.key === 'End') next.c = maxCol;
    ...
    function runCurrentGlossaryHealthCheck() {
      if (generatedContent?.type === 'glossary' && Array.isArray(generatedContent?.data)) {
        var sourceText = history.slice().reverse().find(function (item) { return item && item.type === 'analysis'; })?.data?.originalText || inputText || '';
    ...
    function stopGlossarySavedAudio(clearPlaybackState) {
      ...
      if (glossarySavedAudioRef.current) {
        glossarySavedAudioRef.current.pause();
        try { glossarySavedAudioRef.current.currentTime = 0; } catch (_) {}
        glossarySavedAudioRef.current = null;
      }
    ...
    async function handleGlossarySpeak(item, field, spokenText, contentId, language) {
      ...
      if (playingContentId === contentId) {
        if (glossarySavedAudioRef.current) stopGlossarySavedAudio(true);
        else if (typeof stopPlayback === 'function') stopPlayback();
        return;
      }
      stopGlossarySavedAudio(false);
      if (typeof stopPlayback === 'function') stopPlayback();
      ...
      function fallbackToLiveSpeech() {
        ...
        if (glossarySavedAudioRef.current) {
          try { glossarySavedAudioRef.current.pause(); } catch (_) {}
        }
    ...
    async function handleRegenerateGlossaryAudio(item, field, spokenText, contentId, language) {
      ...
      try {
        var url = await regenerate(request, { reason: 'glossary-edit-regenerate', signal: controller.signal });
      ...
      } finally { glossaryAudioControllersRef.current.delete(controller); }
    }
    function renderGlossaryEditAudioTools(item, field, spokenText, contentId, language) {
      ...
      return <div role="group" aria-label={audioReviewLabel + ': ' + fieldLabel + ' ' + (item?.term || '')} className="mt-1 flex flex-wrap items-center justify-center gap-1.5">
        ...
        <button type="button" onClick={() => handleGlossarySpeak(item, field, spokenText, contentId, language)} disabled={!isReady || isBusy || (isGeneratingAudio && !isThisPlaying)} aria-pressed={isThisPlaying} aria-label={(isThisPlaying ? audioStopLabel : audioReviewLabel) + ': ' + fieldLabel + ' ' + (item?.term || '')} title={isReady ? (isThisPlaying ? audioStopLabel : audioReviewLabel) : audioGenerateLabel} className={actionClass + ' border-indigo-200 bg-white text-indigo-700 hover:bg-indigo-50'}>{isThisPlaying ? <StopCircle size={12} aria-hidden="true" /> : <Volume2 size={12} aria-hidden="true" />}<span>{isThisPlaying ? audioStopLabel : audioReviewLabel}</span></button>
        <button type="button" onClick={() => handleRegenerateGlossaryAudio(item, field, spokenText, contentId, language)} disabled={!String(spokenText == null ? '' : spokenText).trim() || anyAudioEditBusy || isGeneratingAudio} aria-label={actionLabel + ': ' + fieldLabel + ' ' + (item?.term || '')} title={actionLabel} className={actionClass + ' border-emerald-200 bg-white text-emerald-700 hover:bg-emerald-50'}>{isBusy ? <RefreshCw size={12} className="animate-spin motion-reduce:animate-none" aria-hidden="true" /> : <Volume2 size={12} aria-hidden="true" />}<span>{isBusy ? (isReady || needsRebuild ? audioRegenerateLabel : audioGenerateLabel) : actionLabel}</span></button>
      </div>;
    }
    async function handlePrepareGlossaryAudio() {
      ...
      if (typeof prepare !== 'function') {
        ...
        setGlossaryAudioPrep({ busy: false, done: 0, total: 0, message: unavailableMessage });
        if (typeof addToast === 'function') addToast(unavailableMessage, 'info');
        return;
      }
      var audioOwner = glossaryAudioOwnerRef.current;
      var controller = new AbortController();
      glossaryAudioControllersRef.current.add(controller);
      setGlossaryAudioPrep({ busy: true, done: 0, total: 0, message: glossaryPreparingAudioLabel });
      ...
      try {
        var result = await prepare({ includeTerms: true, includeDefinitions: includeDefinitions, languages: preparedLanguages }, updateGlossaryAudioProgress, { source: 'glossary-teacher-tools', signal: controller.signal });
        if (!isGlossaryAudioCurrent(audioOwner)) return;
    ...
    function renderGlossaryAudioReviewPanel() {
      ...
      return <div id="glossary-edit-audio-review" role="region" aria-labelledby="glossary-edit-audio-review-title" className="rounded-xl border border-violet-200 bg-white p-3 shadow-sm" data-help-key="glossary_audio_review">
        <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
          <div><h3 id="glossary-edit-audio-review-title" className="text-sm font-black text-violet-950">{t('common.review_and_edit_word_list')}</h3><p className="mt-0.5 text-xs text-slate-600">{t('common.play_audio_sequence')}</p></div>
          <span className="w-fit rounded-full border border-violet-200 bg-violet-50 px-2.5 py-1 text-[10px] font-bold text-violet-800">{t('common.voice_input')} · {t('common.speed')} · {t('common.target_language_selector')}</span>
        </div>
        ...
        <div className="max-h-[34rem] space-y-2 overflow-y-auto pr-1 custom-scrollbar">{generatedContent.data.map(function (item, idx) {
          ...
          return <article key={'audio-review-' + getGlossaryEntryKey(item, idx)} className="rounded-xl border border-slate-200 bg-slate-50/70 p-3">
            <h4 className="mb-2 text-sm font-black text-slate-900">{item.term || t('common.untitled')}</h4>
            <div className="grid gap-2 lg:grid-cols-2">
              <div className="rounded-lg border border-slate-200 bg-white p-2"><p className="text-[10px] font-black uppercase tracking-wide text-slate-500">{flashcardTermLabel}</p><p dir="auto" className="mt-1 break-words text-sm font-bold text-slate-900">{item.term || t('common.untitled')}</p>{renderGlossaryEditAudioTools(item, 'term', item.term, `audio-review-term-${idx}`, item.termLanguage || 'English')}</div>
              <div className="rounded-lg border border-slate-200 bg-white p-2"><p className="text-[10px] font-black uppercase tracking-wide text-slate-500">{flashcardDefinitionLabel}</p><p dir="auto" className="mt-1 break-words text-sm text-slate-800">{item.def || t('common.untitled')}</p>{renderGlossaryEditAudioTools(item, 'definition', item.def, `audio-review-definition-${idx}`, item.definitionLanguage || 'English')}</div>
            </div>
            {selectedLanguages.length > 0 && <details className="mt-2 rounded-lg border border-slate-200 bg-white p-2"><summary className="min-h-11 cursor-pointer py-2 text-xs font-bold text-indigo-800">{t('glossary.edit_translation')}</summary><div className="grid gap-2 pt-2 lg:grid-cols-2">{selectedLanguages.map(function (lang) {
              var translation = item.translations?.[lang] || '';
              return <div key={lang} className="rounded-lg border border-indigo-100 bg-indigo-50/40 p-2"><p className="text-[10px] font-black uppercase tracking-wide text-indigo-700">{lang}</p><p dir={isRtlLang(lang) ? 'rtl' : 'ltr'} className="mt-1 break-words text-sm text-slate-800">{translation || t('common.untitled')}</p>{renderGlossaryEditAudioTools(item, 'translation', translation, `audio-review-translation-${idx}-${lang}`, lang)}</div>;
            })}</div></details>}
          </article>;
        })}</div>
      </div>;
    ...
    function renderGlossaryToolbar() {
      ...
      return <><button type="button" data-glossary-skip onClick={() => document.querySelector('[data-glossary-word-list]')?.focus()} className="sr-only focus:not-sr-only focus:mb-2 focus:inline-block focus:rounded focus:bg-indigo-700 focus:px-3 focus:py-2 focus:font-bold focus:text-white">{t('glossary.skip_to_words') || 'Skip to the word list'}</button><section aria-label={t('glossary.title')} className="mb-4 space-y-4 rounded-xl border border-blue-200 bg-blue-50 p-4 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
          {isTeacherMode && <p data-glossary-udl-goal className="max-w-xl text-sm text-blue-800"><strong>{t('simplified.udl_goal').split(':')[0]}:</strong> {t('glossary.udl_goal_desc')}</p>}
          <div className="relative w-full sm:max-w-sm" data-help-key="glossary_search">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-blue-700" size={16} aria-hidden="true" />
            <input type="search" aria-label={t('glossary.search_placeholder')} placeholder={t('glossary.search_placeholder')} value={glossarySearchTerm} onChange={e => setGlossarySearchTerm(e.target.value)} className="min-h-11 w-full rounded-lg border border-blue-400 bg-white py-2 pl-10 pr-11 text-sm focus:ring-2 focus:ring-indigo-500" />
            {glossarySearchTerm && <button type="button" onClick={() => setGlossarySearchTerm('')} className="absolute right-0 top-0 min-h-11 min-w-11 inline-flex items-center justify-center rounded-r-lg text-blue-700 hover:bg-blue-100 focus-visible:ring-2 focus-visible:ring-indigo-600" aria-label={t('common.clear')}><X size={16} aria-hidden="true" /></button>}
          </div>
        </div>
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
          <div role="group" className="rounded-lg border border-blue-200 bg-white/80 p-3" aria-labelledby="glossary-study-tools-title">
            <h2 id="glossary-study-tools-title" className="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-wider text-blue-800"><MonitorPlay size={16} aria-hidden="true" /> {t('flashcards.practice_mode')}</h2>
            <div className="flex flex-wrap gap-2">
              <button type="button" data-help-key="glossary_standard_flashcards" onClick={() => launchInteractiveFlashcards('standard')} className={toolButton + ' bg-blue-700 text-white border-blue-700 hover:bg-blue-800'} aria-label={t('flashcards.tooltip_launch_standard')}><MonitorPlay size={16} aria-hidden="true" /> {t('flashcards.launch_standard')}</button>
              {selectedLanguages.length > 0 && <button type="button" data-help-key="glossary_language_flashcards" onClick={() => launchInteractiveFlashcards('language')} className={toolButton + ' bg-white text-indigo-700 border-indigo-300 hover:bg-indigo-50'} aria-label={t('flashcards.tooltip_launch_language')}><Languages size={16} aria-hidden="true" /> {t('flashcards.launch_language')}</button>}
            </div>
          </div>
          <div className="flex flex-wrap items-start gap-2">
            <button type="button" onClick={() => toggleGlossaryToolPanel('games')} aria-expanded={glossaryToolsOpen.games} aria-controls="glossary-games-tools" className={toolButton + ' bg-white text-indigo-800 border-indigo-300 hover:bg-indigo-50'}><Gamepad2 size={16} aria-hidden="true" /> {t('common.start_game')} <ChevronDown size={16} aria-hidden="true" className={glossaryToolsOpen.games ? 'rotate-180' : ''} /></button>
            {isTeacherMode && <button type="button" data-help-key="glossary_edit" onClick={handleToggleIsEditingGlossary} aria-pressed={isEditingGlossary} className={toolButton + (isEditingGlossary ? ' bg-blue-700 text-white border-blue-700' : ' bg-white text-blue-800 border-blue-300 hover:bg-blue-50')}><Pencil size={16} aria-hidden="true" /> {isEditingGlossary ? t('common.done') : t('common.edit')}</button>}{isTeacherMode && <button type="button" onClick={() => toggleGlossaryToolPanel('teacher')} aria-expanded={glossaryToolsOpen.teacher} aria-controls="glossary-teacher-tools" className={toolButton + ' bg-white text-slate-800 border-slate-300 hover:bg-slate-50'}><Settings size={16} aria-hidden="true" /> {t('glossary.more_tools')} <ChevronDown size={16} aria-hidden="true" className={glossaryToolsOpen.teacher ? 'rotate-180' : ''} /></button>}
          </div>
        </div>
        {glossaryToolsOpen.games && <div role="group" id="glossary-games-tools" className="rounded-lg border border-indigo-200 bg-indigo-50/70 p-3" aria-labelledby="glossary-games-tools-title"><h2 id="glossary-games-tools-title" className="mb-2 text-xs font-black tracking-wider text-indigo-800">{t('common.start_game')}</h2>
          <div className="my-3 flex flex-wrap items-end gap-3" data-glossary-activity-settings="true">
            <label className="flex min-w-0 flex-col gap-1 text-sm font-semibold">{activityText('glossary.activities.words', 'Words for games')}<select aria-label={activityText('glossary.activities.words', 'Words for games')} value={activityScope} onChange={e => setActivityScope(e.target.value)} aria-describedby="glossary-activity-pool" className="min-h-11 max-w-full rounded-lg border border-slate-400 bg-white px-3 text-slate-800 focus-visible:ring-2 focus-visible:ring-indigo-600"><option value="all">{activityText('glossary.activities.all', 'All glossary words')}</option><option value="filtered">{activityText('glossary.activities.filtered', 'Current filtered words')}</option></select></label>
            <label className="flex min-w-0 flex-col gap-1 text-sm font-semibold">{activityText('glossary.activities.board_size', 'Memory and Matching board size')}<select aria-label={activityText('glossary.activities.board_size', 'Memory and Matching board size')} value={activityBoardSize} onChange={e => setActivityBoardSize(e.target.value)} className="min-h-11 max-w-full rounded-lg border border-slate-400 bg-white px-3 text-slate-800 focus-visible:ring-2 focus-visible:ring-indigo-600"><option value="standard">{activityText('glossary.activities.standard_size', 'Standard')}</option>{[4, 6, 8].map(size => <option key={size} value={String(size)}>{size} {activityText('glossary.activities.pairs', 'pairs')}</option>)}</select></label>
          </div>
          <p id="glossary-activity-pool" role="status" className="mb-2 text-sm text-slate-700">{activityWords.length} {activityText('glossary.activities.available', 'words available for games.')} {activityText('glossary.activities.next_round', 'Changes apply when you start an activity.')}</p>
          {!activityWords.length && <p className="mb-3 text-sm text-slate-700">{activityText('glossary.activities.empty', 'No words are available in this selection. Choose all glossary words or adjust your search and filters.')} <button type="button" onClick={() => setActivityScope('all')} className="min-h-11 rounded px-2 font-bold text-indigo-700 underline focus-visible:ring-2 focus-visible:ring-indigo-600">{activityText('glossary.activities.use_all', 'Use all glossary words')}</button></p>}
<div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          <div className="inline-flex items-stretch rounded-lg border border-slate-300 bg-white">{selectedLanguages.length > 0 && <select aria-label={t('common.target_language_selector')} data-help-key="glossary_puzzle_lang" value={wordSearchLang} onChange={e => { setWordSearchLang(e.target.value); if (gameMode === 'wordsearch') startScopedWordSearch(e.target.value); }} className="min-h-11 rounded-l-lg border-0 border-r border-slate-300 bg-transparent px-3 text-sm font-bold text-teal-800 focus:ring-2 focus:ring-indigo-600"><option value="English">{t('common.english')}</option>{selectedLanguages.map(lang => <option key={lang} value={lang}>{lang}</option>)}</select>}<button type="button" aria-label={(gameMode === 'wordsearch' ? t('common.regenerate') : t('common.start_game')) + ' ' + t('glossary.word_search')} data-help-key="glossary_word_search" disabled={!activityWords.length} onClick={() => startScopedWordSearch(wordSearchLang)} className="min-h-11 inline-flex items-center gap-2 px-3 py-2 text-sm font-bold text-teal-800 hover:bg-teal-50 focus-visible:ring-2 focus-visible:ring-indigo-600"><Gamepad2 size={16} aria-hidden="true" /> {gameMode === 'wordsearch' ? t('common.regenerate') : t('glossary.word_search')}</button></div>
          <button type="button" aria-label={detectiveLabel} data-help-key="glossary_definition_detective" disabled={!activityWords.length} onClick={() => startGlossaryActivity('detective', () => setDetectiveOpen(true))} aria-describedby="glossary-activity-detective-help" className={secondaryButton + ' disabled:opacity-50 disabled:cursor-not-allowed'}><Search size={16} aria-hidden="true" /> <span className="min-w-0 text-left"><span className="block">{detectiveLabel}</span><span id="glossary-activity-detective-help" className="block text-xs font-normal text-slate-600">{activityText('glossary.activities.detective_help', 'Match a word to its meaning')}</span></span></button>
          <button type="button" aria-label={t('glossary.memory_game')} data-help-key="glossary_memory_game" disabled={!activityWords.length} onClick={() => startGlossaryActivity('memory', handleSetIsMemoryGameToTrue)} aria-describedby="glossary-activity-memory-help" className={secondaryButton + ' disabled:opacity-50 disabled:cursor-not-allowed'}><Brain size={16} aria-hidden="true" /> <span className="min-w-0 text-left"><span className="block">{t('glossary.memory_game')}</span><span id="glossary-activity-memory-help" className="block text-xs font-normal text-slate-600">{Math.min(activityMemoryCount, activityBoardSize === 'standard' ? 10 : Number(activityBoardSize))} {activityText('glossary.activities.memory_help', 'pairs: remember words, pictures, or meanings')}</span></span></button>
          <button type="button" aria-label={t('glossary.crossword')} data-help-key="glossary_crossword" disabled={!activityWords.length} onClick={() => startGlossaryActivity('crossword', handleSetIsCrosswordGameToTrue)} aria-describedby="glossary-activity-crossword-help" className={secondaryButton + ' disabled:opacity-50 disabled:cursor-not-allowed'}><Gamepad2 size={16} aria-hidden="true" /> <span className="min-w-0 text-left"><span className="block">{t('glossary.crossword')}</span><span id="glossary-activity-crossword-help" className="block text-xs font-normal text-slate-600">{activityText('glossary.activities.crossword_help', 'Solve clues and spell the words')}</span></span></button>
          <button type="button" aria-label={t('glossary.matching')} data-help-key="glossary_matching" disabled={!activityWords.length} onClick={() => startGlossaryActivity('matching', handleSetIsMatchingGameToTrue)} aria-describedby="glossary-activity-matching-help" className={secondaryButton + ' disabled:opacity-50 disabled:cursor-not-allowed'}><GitMerge size={16} aria-hidden="true" /> <span className="min-w-0 text-left"><span className="block">{t('glossary.matching')}</span><span id="glossary-activity-matching-help" className="block text-xs font-normal text-slate-600">{Math.min(activityMatchingCount, activityBoardSize === 'standard' ? 8 : Number(activityBoardSize))} {activityText('glossary.activities.matching_help', 'pairs: connect words with meanings or pictures')}</span></span></button>
          {/* The card-set generator is for running group Bingo; students play their own card below. */}
          {isTeacherMode && <button type="button" aria-label={t('glossary.bingo')} data-help-key="glossary_bingo" disabled={!activityWords.length} onClick={() => startGlossaryActivity('bingo', handleSetIsBingoGameToTrue)} aria-describedby="glossary-activity-bingo-help" className={secondaryButton + ' disabled:opacity-50 disabled:cursor-not-allowed'}><Gamepad2 size={16} aria-hidden="true" /> <span className="min-w-0 text-left"><span className="block">{t('glossary.bingo')}</span><span id="glossary-activity-bingo-help" className="block text-xs font-normal text-slate-600">{activityText('glossary.activities.bingo_help', 'Create cards for group play')}</span></span></button>}
          <button type="button" aria-label={t('glossary.play_bingo')} data-help-key="glossary_play_bingo" disabled={!activityWords.length} onClick={() => startGlossaryActivity('studentBingo', handleSetIsStudentBingoGameToTrue)} aria-describedby="glossary-activity-studentBingo-help" className={secondaryButton + ' disabled:opacity-50 disabled:cursor-not-allowed'}><Gamepad2 size={16} aria-hidden="true" /> <span className="min-w-0 text-left"><span className="block">{t('glossary.play_bingo')}</span><span id="glossary-activity-studentBingo-help" className="block text-xs font-normal text-slate-600">{activityText('glossary.activities.studentBingo_help', 'Mark the words you hear')}</span></span></button>
          <button type="button" aria-label={t('glossary.scramble')} data-help-key="glossary_scramble" disabled={!activityWords.length} onClick={() => startGlossaryActivity('scramble', handleSetIsWordScrambleGameToTrue)} aria-describedby="glossary-activity-scramble-help" className={secondaryButton + ' disabled:opacity-50 disabled:cursor-not-allowed'}><Shuffle size={16} aria-hidden="true" /> <span className="min-w-0 text-left"><span className="block">{t('glossary.scramble')}</span><span id="glossary-activity-scramble-help" className="block text-xs font-normal text-slate-600">{activityText('glossary.activities.scramble_help', 'Unscramble letters to practice spelling')}</span></span></button>
        </div></div>}
        {isTeacherMode && glossaryToolsOpen.teacher && <div role="group" id="glossary-teacher-tools" className="rounded-lg border border-slate-300 bg-white p-3" aria-labelledby="glossary-teacher-tools-title"><h2 id="glossary-teacher-tools-title" className="mb-2 text-xs font-black tracking-wider text-slate-700">{t('glossary.more_tools')}</h2><div className="flex flex-wrap items-center gap-2">

          <button type="button" data-help-key="glossary_export_standard" onClick={() => handleExportFlashcards('standard')} className={secondaryButton}><GalleryHorizontal size={16} aria-hidden="true" /> {t('flashcards.export_standard')}</button>
          ...
          <label data-help-key="glossary_image_size" className="min-h-11 inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-bold text-slate-700"><ImageIcon size={16} aria-hidden="true" /><span>{t('glossary.image_size_tooltip')}</span><input aria-label={t('glossary.image_size_tooltip')} type="range" min="64" max="300" step="16" value={glossaryImageSize} onChange={e => setGlossaryImageSize(Number(e.target.value))} className="w-24 accent-blue-600" /></label>
          <button type="button" onClick={runCurrentGlossaryHealthCheck} disabled={isRunningHealthCheck || generatedContent?.type !== 'glossary'} className={secondaryButton + ' disabled:opacity-50 disabled:cursor-not-allowed'}>{isRunningHealthCheck ? <RefreshCw size={16} className="animate-spin motion-reduce:animate-none" aria-hidden="true" /> : <CheckCircle2 size={16} aria-hidden="true" />} {isRunningHealthCheck ? t('common.processing') : glossaryHealthCheck ? t('common.refresh') : t('common.check')}</button>
          <label className="min-h-11 inline-flex items-center gap-2 rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-sm font-bold text-violet-900"><span>{t('common.download_audio')}</span><select aria-label={t('common.download_audio')} value={glossaryAudioScope} onChange={e => setGlossaryAudioScope(e.target.value)} disabled={glossaryAudioPrep.busy} className="min-h-9 rounded-md border border-violet-300 bg-white px-2 text-sm focus:ring-2 focus:ring-violet-600"><option value="core">{flashcardTermLabel} + {flashcardDefinitionLabel}</option><option value="terms">{flashcardTermLabel}</option>{selectedLanguages.length > 0 && <option value="all">{flashcardTermLabel} + {flashcardDefinitionLabel} + {t('glossary.edit_translation')}</option>}</select></label>
          <button type="button" onClick={handlePrepareGlossaryAudio} disabled={glossaryAudioPrep.busy} aria-label={glossaryPrepareAudioLabel} aria-describedby="glossary-audio-prep-status" className={toolButton + ' bg-violet-700 text-white border-violet-700 hover:bg-violet-800 disabled:opacity-60'} data-help-key="glossary_prepare_audio">{glossaryAudioPrep.busy ? <RefreshCw size={16} className="animate-spin motion-reduce:animate-none" aria-hidden="true" /> : <Volume2 size={16} aria-hidden="true" />} {glossaryAudioPrep.busy && glossaryAudioPrep.total > 0 ? glossaryAudioPrep.done + '/' + glossaryAudioPrep.total : glossaryPrepareAudioLabel}</button>
        ...
        <div className="flex flex-col gap-2 border-t border-blue-200 pt-3 sm:flex-row sm:items-center sm:justify-between">
          <div role="group" aria-label={t('glossary.title')} className="inline-flex w-fit max-w-full flex-wrap rounded-lg border border-blue-200 bg-white p-1">
            <button type="button" aria-pressed={glossaryFilter === 'all'} aria-label={t('glossary.filter_all')} data-help-key="glossary_filter_all" onClick={handleSetGlossaryFilterToAll} className={'min-h-11 rounded-md px-3 py-2 text-sm font-bold ' + (glossaryFilter === 'all' ? 'bg-blue-100 text-blue-800' : 'text-slate-700 hover:bg-slate-50')}>{t('glossary.filter_all')}</button>
            <button type="button" aria-pressed={glossaryFilter === 'academic'} aria-label={t('glossary.label_tier2')} data-help-key="glossary_filter_tier2" onClick={handleSetGlossaryFilterToAcademic} className={'min-h-11 rounded-md px-3 py-2 text-sm font-bold ' + (glossaryFilter === 'academic' ? 'bg-blue-700 text-white' : 'text-slate-700 hover:bg-slate-50')}>{t('glossary.label_tier2')}</button>
            <button type="button" aria-pressed={glossaryFilter === 'domain'} aria-label={t('glossary.label_tier3')} data-help-key="glossary_filter_tier3" onClick={handleSetGlossaryFilterToDomain} className={'min-h-11 rounded-md px-3 py-2 text-sm font-bold ' + (glossaryFilter === 'domain' ? 'bg-purple-700 text-white' : 'text-slate-700 hover:bg-slate-50')}>{t('glossary.label_tier3')}</button>
          </div>
    ...
    return <><div className="space-y-6">{renderGlossaryToolbar()}{isInteractiveFlashcards && Array.isArray(generatedContent?.data) && generatedContent.data.length > 0 && <div ref={flashcardDialogRef} style={{ margin: 0 }} role="dialog" aria-modal="true" aria-label={t('flashcards.deck_standard') || 'Interactive flashcards'} tabIndex={-1} onKeyDown={e => containModalFocus(e, flashcardDialogRef.current, closeInteractiveFlashcards)} className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-start pt-20 sm:pt-24 p-4 animate-in fade-in duration-300 motion-reduce:animate-none motion-reduce:transition-none overflow-auto"><button ref={flashcardCloseRef} type="button" onClick={closeInteractiveFlashcards} className="absolute top-6 right-6 text-white/70 hover:text-white bg-white/10 hover:bg-white/20 p-2 rounded-full transition-colors z-50  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950" aria-label={t('common.close')}><X size={32} aria-hidden="true" /></button><div id="flashcard-live-status" className="sr-only" aria-live="polite" aria-atomic="true">{flashcardStatusText}</div><div className="absolute top-6 left-6 z-50 hidden sm:flex flex-col gap-2"><button type="button" aria-pressed={showFlashcardImages} onClick={handleToggleShowFlashcardImages} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-bold transition-colors shadow-lg border  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${showFlashcardImages ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-slate-800 text-slate-300 border-slate-700 hover:text-white'}`} title={t('flashcards.tooltip_toggle_images')} aria-label={showFlashcardImages ? t('flashcards.hide_images') : t('flashcards.show_images')}><ImageIcon size={16} aria-hidden="true" /> {showFlashcardImages ? t('flashcards.hide_images') : t('flashcards.show_images')}</button><button type="button" aria-pressed={isFlashcardQuizMode} onClick={handleToggleFlashcardQuizMode} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-bold transition-colors shadow-lg border  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${isFlashcardQuizMode ? 'bg-yellow-500 text-indigo-900 border-yellow-400' : 'bg-slate-800 text-slate-300 border-slate-700 hover:text-white'}`} title={t('flashcards.tooltip_toggle_quiz')} aria-label={glossaryQuizToggleLabel}>{isFlashcardQuizMode ? <CheckCircle2 size={16} aria-hidden="true" /> : <Brain size={16} aria-hidden="true" />}{isFlashcardQuizMode ? t('flashcards.quiz_active') : t('flashcards.practice_mode')}</button></div>{flashcardMode === 'standard' && selectedLanguages.length > 0 && <div className="absolute top-6 left-4 max-w-[calc(100%-6rem)] sm:left-1/2 sm:-translate-x-1/2 z-50"><select aria-label={t('flashcards.deck_standard')} value={standardDeckLang} onChange={e => setStandardDeckLang(e.target.value)} className="max-w-full bg-slate-800 text-white border border-slate-600 rounded-lg px-3 py-2 text-sm font-bold focus:ring-2 focus:ring-indigo-500  shadow-lg"><option value="English Only">{t('languages.english_only')}</option>{selectedLanguages.map(l => <option key={l} value={l}>+ {l}</option>)}</select></div>}{flashcardMode === 'language' && selectedLanguages.length > 1 && <div className="absolute top-6 left-4 max-w-[calc(100%-6rem)] sm:left-1/2 sm:-translate-x-1/2 z-50"><select aria-label={t('flashcards.deck_language')} value={flashcardLang} onChange={e => setFlashcardLang(e.target.value)} className="max-w-full bg-slate-800 text-white border border-slate-600 rounded-lg px-3 py-2 text-sm font-bold focus:ring-2 focus:ring-indigo-500 ">{selectedLanguages.map(l => <option key={l} value={l}>{l}</option>)}</select></div>}{flashcardSummary ? renderFlashcardSummary() : <div className="w-full max-w-4xl min-w-0"><div className="mb-6 px-2"><div className="flex items-center justify-between text-white/80 mb-2"><span className="font-bold text-lg flex items-center gap-2">{flashcardMode === 'standard' ? t('flashcards.deck_standard') : t('flashcards.deck_language', {
                    lang: flashcardLang || 'Language'
                  })}<span className="text-sm font-normal opacity-70">({flashcardRoundPosition + 1}/{flashcardRoundIndices.length})</span></span><div className="hidden md:flex items-center gap-2 text-xs font-bold"><span className="rounded-full bg-emerald-500/15 text-emerald-100 border border-emerald-400/40 px-2 py-0.5">{flashcardKnownLabel} {knownCount}</span><span className="rounded-full bg-amber-500/15 text-amber-100 border border-amber-400/40 px-2 py-0.5">{flashcardLearningLabel} {learningCount}</span>{(isPlaying || flashcardDictAudioKey) && <span className="rounded-full bg-yellow-400 text-slate-900 px-2 py-0.5 animate-pulse motion-reduce:animate-none">{audioReviewLabel}</span>}</div>{isFlashcardQuizMode && <div className="bg-yellow-500 text-indigo-900 px-3 py-0.5 rounded-full text-sm font-black shadow-sm animate-in zoom-in motion-reduce:animate-none">{t('flashcards.score_label')} {flashcardScore}</div>}<div className="sm:hidden flex gap-2"><button type="button" aria-pressed={showFlashcardImages} aria-label={showFlashcardImages ? t('flashcards.hide_images') : t('flashcards.show_images')} onClick={handleToggleShowFlashcardImages} className={`min-h-11 min-w-11 inline-flex items-center justify-center rounded-lg border transition-colors  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${showFlashcardImages ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-800 border-slate-700 text-slate-300'}`}><ImageIcon size={16} aria-hidden="true" /></button><button type="button" aria-pressed={isFlashcardQuizMode} aria-label={glossaryQuizToggleLabel} onClick={handleToggleFlashcardQuizMode} className={`min-h-11 min-w-11 inline-flex items-center justify-center rounded-lg border transition-colors  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${isFlashcardQuizMode ? 'bg-yellow-500 border-yellow-400 text-indigo-900' : 'bg-slate-800 border-slate-700 text-slate-300'}`}><Brain size={16} aria-hidden="true" /></button></div></div>{renderFlashcardProgressDots()}</div><div data-flashcard-body="true" style={{ overflowWrap: 'anywhere' }} className="relative w-full min-w-0 cursor-pointer group" onClick={handleToggleIsFlashcardFlipped}><div key={flashcardTransitionKey} className="w-full min-w-0 shadow-2xl rounded-3xl"><div data-flashcard-face="front" style={{ display: isFlashcardFlipped ? 'none' : 'flex' }} className="relative min-h-80 sm:min-h-[420px] bg-white rounded-3xl p-5 sm:p-8 flex-col items-center justify-center text-center border-4 border-blue-100 shadow-inner">{flashcardMode === 'standard' ? <><div className="self-start mb-4 text-xs font-bold text-blue-700 uppercase tracking-widest">{t('flashcards.front_label_term')}</div>{showFlashcardImages && generatedContent?.data[flashcardIndex].image && <div className="mb-4 w-full flex justify-center"><img loading="lazy" src={generatedContent?.data[flashcardIndex].image} alt={getGlossaryImageAlt(generatedContent?.data[flashcardIndex])} role={getGlossaryImageAlt(generatedContent?.data[flashcardIndex]) ? undefined : "presentation"} className="max-h-48 sm:max-h-64 max-w-full object-contain rounded-lg shadow-sm border border-slate-100" decoding="async" /></div>}<h2 className={`${showFlashcardImages && generatedContent?.data[flashcardIndex].image ? 'text-3xl md:text-5xl' : 'text-4xl md:text-7xl'} font-black text-slate-800 hover:text-blue-600 transition-colors`}><button type="button" onClick={e => {
                      e.stopPropagation();
                      handleGlossarySpeak(generatedContent?.data[flashcardIndex], 'term', generatedContent?.data[flashcardIndex].term, 'fc-front', generatedContent?.data[flashcardIndex].termLanguage || 'English');
                        ...
                        if (term) handleUncachedGlossarySpeak(term, `fc-front-${standardDeckLang}`);
                          ...
                          return "";
                        })()}</button></h3></div>}</> : <><div className="self-start mb-4 text-xs font-bold text-indigo-600 uppercase tracking-widest flex items-center gap-2"><Globe size={14} /> English</div>{showFlashcardImages && generatedContent?.data[flashcardIndex].image && <div className="mb-4 w-full flex justify-center"><img loading="lazy" src={generatedContent?.data[flashcardIndex].image} alt={getGlossaryImageAlt(generatedContent?.data[flashcardIndex])} role={getGlossaryImageAlt(generatedContent?.data[flashcardIndex]) ? undefined : "presentation"} className="max-h-48 sm:max-h-64 max-w-full object-contain rounded-lg shadow-sm border border-slate-100" decoding="async" /></div>}<h2 className={`${showFlashcardImages && generatedContent?.data[flashcardIndex].image ? 'text-3xl md:text-4xl' : 'text-4xl md:text-6xl'} font-black text-slate-800 mb-2 hover:text-indigo-600 transition-colors`}><button type="button" onClick={e => {
                      e.stopPropagation();
                    ...
                    }} aria-label={`${t('common.click_read_aloud')}: ${generatedContent?.data[flashcardIndex].term}`} className="min-h-11 max-w-full inline-flex items-center justify-center appearance-none border-0 bg-transparent p-0 text-inherit [font:inherit] cursor-pointer rounded  focus-visible:ring-2 focus-visible:ring-indigo-700 focus-visible:ring-offset-2">{generatedContent?.data[flashcardIndex].term}</button></h2><p className={`${showFlashcardImages && generatedContent?.data[flashcardIndex].image ? 'text-lg' : 'text-2xl'} text-slate-600 leading-relaxed max-w-2xl hover:text-indigo-500 transition-colors`}><button type="button" onClick={e => {
                      e.stopPropagation();
                      handleGlossarySpeak(generatedContent?.data[flashcardIndex], 'definition', generatedContent?.data[flashcardIndex].def, 'fc-front-def', generatedContent?.data[flashcardIndex].definitionLanguage || 'English');
                    }} aria-label={`${t('common.click_read_aloud')}: ${generatedContent?.data[flashcardIndex].term} definition`} className="min-h-11 max-w-full inline-flex items-center justify-center appearance-none border-0 bg-transparent p-0 text-center text-inherit [font:inherit] cursor-pointer rounded  focus-visible:ring-2 focus-visible:ring-indigo-700 focus-visible:ring-offset-2">{generatedContent?.data[flashcardIndex].def}</button></p></>}<div className="mt-6 text-slate-600 text-xs font-bold uppercase tracking-widest flex items-center gap-1 animate-pulse motion-reduce:animate-none">{t('flashcards.flip_hint')} <RefreshCw size={10} /></div></div><div data-flashcard-face="back" style={{ display: isFlashcardFlipped ? 'flex' : 'none' }} className={`relative min-h-80 sm:min-h-[420px] rounded-3xl p-5 sm:p-8 flex-col items-center justify-center text-center border-4 shadow-inner text-white ${flashcardMode === 'standard' ? 'bg-blue-600 border-blue-400' : 'bg-indigo-600 border-indigo-400'}`}>{isFlashcardQuizMode ? <div className="w-full h-full flex flex-col items-center justify-center animate-in fade-in duration-300 motion-reduce:animate-none"><h2 className="text-2xl md:text-4xl font-black text-white mb-2 drop-shadow-md">{(() => {
                        ...
                        return <button key={idx} onClick={e => handleReviewedQuizOption(e, opt)} disabled={!!quizSelectedOption} className={`p-3 rounded-xl text-xs sm:text-sm font-medium transition-all text-left shadow-sm flex items-center gap-3 ${btnClass}`}>{icon && <span>{icon}</span>}<span className="break-words">{opt}</span></button>;
                      })}</div></div> : flashcardMode === 'standard' ? <><div className="self-start mb-4 text-xs font-bold text-white uppercase tracking-widest">{t('flashcards.back_label_def')}</div><div className="w-full max-w-3xl  px-1 space-y-3 text-left"><div className="rounded-xl bg-white/10 border border-blue-300/40 px-4 py-3 shadow-inner"><p className="text-[10px] font-black text-white uppercase tracking-widest mb-1">{flashcardDefinitionLabel}</p><p className="text-xl md:text-3xl font-medium leading-relaxed hover:text-blue-50 transition-colors cursor-pointer"><button type="button" onClick={e => {
                      e.stopPropagation();
                      handleGlossarySpeak(generatedContent?.data[flashcardIndex], 'definition', generatedContent?.data[flashcardIndex].def, 'fc-back-def', generatedContent?.data[flashcardIndex].definitionLanguage || 'English');
                    }} aria-label={`${t('common.click_read_aloud')}: ${generatedContent?.data[flashcardIndex].term} definition`} title={t('flashcards.tooltip_audio')} className="min-h-11 max-w-full inline-flex items-center justify-center appearance-none border-0 bg-transparent p-0 text-center text-inherit [font:inherit] cursor-pointer rounded  focus-visible:ring-2 focus-visible:ring-yellow-300 focus-visible:ring-offset-blue-600 focus-visible:ring-offset-2">{generatedContent?.data[flashcardIndex].def}</button></p><p className="mt-2 text-[11px] leading-snug text-blue-50">Provenance: generated from this lesson's source text and selected grade level.</p></div>{standardDeckLang !== 'English Only' && <div className="rounded-xl bg-white/10 border border-blue-300/30 px-4 py-3 w-full animate-in fade-in slide-in-from-bottom-2 motion-reduce:animate-none"><p className="text-xs font-bold text-white uppercase mb-2">{standardDeckLang}</p><p className="text-lg md:text-xl font-medium leading-relaxed italic text-blue-50 hover:text-white cursor-pointer"><button type="button" onClick={e => {
                        e.stopPropagation();
                        const fullTrans = generatedContent?.data[flashcardIndex].translations?.[standardDeckLang] || "";
                        ...
                        handleUncachedGlossarySpeak(def, `fc-back-def-${standardDeckLang}`);
                      }} aria-label={`${t('common.read_translated_definition')}: ${standardDeckLang}`} className="min-h-11 max-w-full inline-flex items-center justify-center appearance-none border-0 bg-transparent p-0 text-center text-inherit [font:inherit] cursor-pointer rounded  focus-visible:ring-2 focus-visible:ring-yellow-300 focus-visible:ring-offset-blue-600 focus-visible:ring-offset-2">{(() => {
                          ...
                          return fullTrans;
                        })()}</button></p></div>}{generatedContent?.data[flashcardIndex]?.etymology && <div className="mt-4 pt-3 border-t border-blue-400/50 w-full animate-in fade-in slide-in-from-bottom-2 motion-reduce:animate-none"><p className="text-xs font-bold text-white uppercase mb-1">📜 {t('glossary.etymology_label') || 'Word roots'}</p><p className="text-sm md:text-base text-blue-50 italic leading-relaxed hover:text-white cursor-pointer"><button type="button" onClick={e => {
                        e.stopPropagation();
                        handleUncachedGlossarySpeak(generatedContent.data[flashcardIndex].etymology, 'fc-back-etym');
                      }} aria-label={`${t('common.click_read_aloud')}: ${t('glossary.etymology_label') || 'Word roots'}`} className="min-h-11 max-w-full inline-flex items-center justify-center appearance-none border-0 bg-transparent p-0 text-center text-inherit [font:inherit] cursor-pointer rounded  focus-visible:ring-2 focus-visible:ring-yellow-300 focus-visible:ring-offset-blue-600 focus-visible:ring-offset-2">{generatedContent.data[flashcardIndex].etymology}</button></p></div>}{renderFlashcardDictBack(generatedContent.data[flashcardIndex], t, flashcardDictAudioKey, playGlossaryDictionaryAudio)}</div></> : (() => {
                    ...
                    return <><div className="self-start mb-4 text-xs font-bold text-indigo-200 uppercase tracking-widest flex items-center gap-2"><Globe size={14} /> {flashcardLang}</div>{transTerm && <h2 className="text-4xl md:text-6xl font-black text-white mb-6 hover:text-indigo-200 transition-colors cursor-pointer"><button type="button" onClick={e => {
                        e.stopPropagation();
                        handleUncachedGlossarySpeak(transTerm, 'fc-back-term');
                      }} aria-label={`Read ${flashcardLang} term`} className="min-h-11 max-w-full inline-flex items-center justify-center appearance-none border-0 bg-transparent p-0 text-inherit [font:inherit] cursor-pointer rounded  focus-visible:ring-2 focus-visible:ring-yellow-300 focus-visible:ring-offset-indigo-700 focus-visible:ring-offset-2">{transTerm}</button></h2>}<p className="text-xl md:text-2xl text-indigo-100 leading-relaxed font-serif italic hover:text-white transition-colors cursor-pointer"><button type="button" onClick={e => {
                        e.stopPropagation();
                        handleUncachedGlossarySpeak(transDef, 'fc-back-def');
                      }} aria-label={`${t('common.read_translated_definition')}: ${flashcardLang}`} className="min-h-11 max-w-full inline-flex items-center justify-center appearance-none border-0 bg-transparent p-0 text-center text-inherit [font:inherit] cursor-pointer rounded  focus-visible:ring-2 focus-visible:ring-yellow-300 focus-visible:ring-offset-indigo-700 focus-visible:ring-offset-2">{transDef}</button></p></>;
                  })()}</div></div></div>{renderFlashcardActionBar()}{renderFlashcardEditDrawer()}</div>}</div>}{detectiveOpen && (DefinitionDetectiveGame ? <ErrorBoundary fallbackMessage="Definition Detective encountered an error."><DefinitionDetectiveGame data={activityData('detective')} onClose={() => setDetectiveOpen(false)} playSound={playSound} onScoreUpdate={handleGameScoreUpdate} onGameComplete={handleGameCompletion} /></ErrorBoundary> : <div role="status" className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-amber-900"><p>{t('common.loading')}</p><button type="button" className="min-h-11 p-3 font-bold" onClick={() => detectiveRefresh[1](value => value + 1)}>{t('common.refresh')}</button><button type="button" className="min-h-11 p-3 font-bold" onClick={() => setDetectiveOpen(false)}>{t('common.close')}</button></div>)}{isMemoryGame && <ErrorBoundary fallbackMessage="Memory Game encountered an error."><MemoryGame data={activityData('memory')} roundSize={activitySize('memory')} onClose={closeMemory} onScoreUpdate={handleGameScoreUpdate} onGameComplete={handleGameCompletion} /></ErrorBoundary>}{isCrosswordGame && <ErrorBoundary fallbackMessage="Crossword Puzzle encountered an error."><CrosswordGame data={activityData('crossword')} onClose={closeCrossword} playSound={playSound} onScoreUpdate={handleGameScoreUpdate} onGameComplete={handleGameCompletion} /></ErrorBoundary>}{isMatchingGame && <ErrorBoundary fallbackMessage="Matching Game encountered an error."><MatchingGame data={activityData('matching')} roundSize={activitySize('matching')} onClose={closeMatching} playSound={playSound} onScoreUpdate={handleGameScoreUpdate} onGameComplete={handleGameCompletion} /></ErrorBoundary>}{isBingoGame && <ErrorBoundary fallbackMessage="Bingo Generator encountered an error."><BingoGame data={activityData('bingo')} onClose={closeBingo} settings={bingoSettings} setSettings={setBingoSettings} onGenerate={() => handleGenerateBingo(activityData('bingo'))} bingoState={bingoState} setBingoState={setBingoState} onGenerateAudio={callTTS} selectedVoice={selectedVoice} alloBotRef={alloBotRef} /></ErrorBoundary>}{isStudentBingoGame && <ErrorBoundary fallbackMessage="Bingo Game encountered an error."><StudentBingoGame data={activityData('studentBingo')} onClose={closeStudentBingo} playSound={playSound} onGameComplete={handleGameCompletion} /></ErrorBoundary>}{isWordScrambleGame && <ErrorBoundary fallbackMessage="Word Scramble Game encountered an error."><WordScrambleGame data={activityData('scramble')} onClose={handleCloseWordScramble} playSound={playSound} onScoreUpdate={handleGameScoreUpdate} onGameComplete={handleGameCompletion} /></ErrorBoundary>}{screenerSession && screenerSession.status === 'interstitial' && <div role="presentation" className="fixed inset-0 z-[250] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300 motion-reduce:animate-none"><div role="status" aria-live="polite" aria-atomic="true" aria-label={glossaryProgressLabel} className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center border-4 border-emerald-100"><div className="w-16 h-16 bg-emerald-100 text-emerald-700 rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-emerald-200"><CheckCircle size={32} /></div><h3 className="text-xl font-black text-slate-800 mb-2">{screenerSession.subtests[screenerSession.currentIndex - 1]?.replace(/^./, c => c.toUpperCase())} {glossaryDoneLabel}!</h3><p className="text-slate-600 mb-4">{glossaryNextLabel}:</p><p className="text-2xl font-black text-emerald-600 mb-6">{screenerSession.subtests[screenerSession.currentIndex]?.replace(/^./, c => c.toUpperCase())}</p><div className="w-full bg-slate-100 rounded-full h-2 mb-4" role="progressbar" aria-label={glossaryProgressLabel} aria-valuemin={0} aria-valuemax={screenerSession.subtests.length} aria-valuenow={screenerSession.currentIndex}><div className="bg-emerald-500 h-2 rounded-full transition-all duration-500 motion-reduce:transition-none" style={{
                width: `${Math.round(screenerSession.currentIndex / screenerSession.subtests.length * 100)}%`
              }} /></div><p className="text-xs text-slate-600">{screenerSession.currentIndex} of {screenerSession.subtests.length} {glossaryDoneLabel}</p></div></div>}{screenerSession && screenerSession.status === 'complete' && <div role="presentation" className="fixed inset-0 z-[250] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300 motion-reduce:animate-none"><div ref={screenerDialogRef} role="dialog" aria-modal="true" aria-labelledby="glossary-screener-results-title" tabIndex={-1} onKeyDown={e => containModalFocus(e, screenerDialogRef.current, closeScreenerResults)} className="bg-white rounded-2xl shadow-2xl p-8 max-w-lg w-full border-4 border-violet-100"><div className="text-center mb-6"><div className="w-16 h-16 bg-violet-100 text-violet-600 rounded-full flex items-center justify-center mx-auto mb-3 border-2 border-violet-200"><Award size={32} /></div><h3 id="glossary-screener-results-title" className="text-2xl font-black text-slate-800">{t('common.screening_complete')}</h3><p className="text-slate-600 text-sm mt-1">{screenerSession.student} — Grade {screenerSession.grade} — Form {screenerSession.form}</p></div><div className="space-y-3 mb-6">{screenerSession.results.map((r, idx) => <div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-400"><span className="font-bold text-slate-700 capitalize">{r.activity}</span><div className="flex items-center gap-3"><span className="text-sm text-slate-600">{r.correct}/{r.total}</span><span className={`text-sm font-bold px-2 py-0.5 rounded-full ${r.accuracy >= 80 ? 'bg-emerald-100 text-emerald-700' : r.accuracy >= 60 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>{r.accuracy}%</span>{r.itemsPerMin > 0 && <span className="text-xs text-slate-600">{r.itemsPerMin} items/min</span>}</div></div>)}</div>{(() => {
              ...
              }}><p className="text-3xl mb-1">{risk.emoji}</p><p className="text-sm font-bold" style={{
                  color: risk.color
            ...
            })()}<div className="flex gap-3">{rosterQueue.length > 0 && <button onClick={advanceRoster} className="flex-1 py-3 bg-emerald-700 hover:bg-emerald-800 text-white font-bold rounded-xl shadow-lg transition-colors">▶ {glossaryNextLabel} ({rosterQueue[0]})</button>}<button onClick={closeScreenerResults} className={`${rosterQueue.length > 0 ? 'flex-1' : 'w-full'} py-3 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-xl shadow-lg transition-colors`}>{rosterQueue.length > 0 ? glossarySkipLabel + ' / ' + glossaryDoneLabel : glossaryDoneLabel}</button></div><button onClick={exportScreeningCSV} className="w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-bold rounded-xl transition-colors flex items-center justify-center gap-2 mt-2">📊 Export Screening CSV</button></div></div>}{gameMode === 'wordsearch' && gameData && <div className="mb-6 bg-white p-4 sm:p-6 rounded-xl border-2 border-teal-200 shadow-md animate-in motion-reduce:animate-none fade-in slide-in-from-top-4"><div className="flex flex-wrap justify-between items-center gap-2 mb-4 border-b border-slate-100 pb-2"><div><h3 className="font-bold text-teal-800 text-lg flex items-center gap-2"><Gamepad2 size={20} aria-hidden="true" /> {t('glossary.word_search_title')}</h3><p className="no-print mt-1 text-xs text-slate-600">Select the first and last letter of a word. Use arrow keys inside the grid, then Enter or Space to select.</p></div><div className="flex gap-2">{isTeacherMode && <button type="button" onClick={handleToggleShowWordSearchAnswers} className={'min-h-11 text-xs flex items-center gap-1 px-3 py-2 rounded-full font-bold transition-colors ' + (showWordSearchAnswers ? 'bg-yellow-100 text-yellow-800' : 'bg-slate-100 text-slate-600 hover:bg-slate-200')}>{showWordSearchAnswers ? <Eye size={14} aria-hidden="true" /> : <MousePointerClick size={14} aria-hidden="true" />}{showWordSearchAnswers ? t('glossary.hide_answers') : t('glossary.show_answers')}</button>}{isTeacherMode && <button type="button" onClick={handlePrintGame} className="min-h-11 text-xs flex items-center gap-1 bg-teal-100 text-teal-700 px-3 py-2 rounded-full font-bold hover:bg-teal-200 transition-colors"><Printer size={14} aria-hidden="true" /> {t('glossary.print_puzzle')}</button>}<button type="button" onClick={handleSetGameModeToNull} className="min-h-11 min-w-11 inline-flex items-center justify-center text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-full transition-colors" aria-label={t('common.close')}><X size={18} aria-hidden="true" /></button></div></div><div className="sr-only" role="status" aria-live="polite" aria-atomic="true">{wordSearchAnnouncement}</div><div id="printable-game-area" className="flex flex-col items-center"><h2 className="hidden print-only font-bold text-xl mb-4">{t('glossary.word_search_title')}</h2><div className="max-w-full overflow-x-auto p-1"><div ref={wordSearchGridRef} id="word-search-grid" role="grid" tabIndex={0} dir={gameData.isRtl ? 'rtl' : 'ltr'} aria-label={(t('glossary.word_search_title') || 'Word search') + '. ' + wordSearchFoundWords.size + ' of ' + gameData.words.length + ' words found.'} aria-rowcount={gameData.grid.length} aria-colcount={gameData.grid[0]?.length || 0} aria-activedescendant={'word-search-cell-' + wordSearchActive.r + '-' + wordSearchActive.c} onKeyDown={handleWordSearchGridKeyDown} className="inline-grid gap-0 border-2 border-slate-800 bg-white p-1 focus-visible:ring-4 focus-visible:ring-teal-500 focus-visible:ring-offset-2" style={{ gridTemplateColumns: 'repeat(' + (gameData.grid[0]?.length || 0) + ', minmax(2rem, 2.25rem))' }}>{gameData.grid.map((row, r) => <div key={r} role="row" className="contents">{row.map((char, c) => { var key = wordSearchCellKey(r, c); var active = wordSearchActive.r === r && wordSearchActive.c === c; var inPath = wordSearchPath.includes(key); var foundCell = isWordSearchFoundCell(key); var isAnswer = gameData.solutions && gameData.solutions.includes(key); var highlightClass = showWordSearchAnswers && isAnswer ? 'bg-green-200 text-green-900 font-extrabold' : foundCell ? 'bg-emerald-100 text-emerald-900' : inPath ? 'bg-yellow-200 text-black' : 'bg-white text-slate-700 hover:bg-slate-50'; return <div key={c} id={'word-search-cell-' + r + '-' + c} role="gridcell" aria-rowindex={r + 1} aria-colindex={c + 1} aria-selected={active} aria-label={(t('glossary.word_search_cell_aria') || 'Cell') + ' ' + (r + 1) + '-' + (c + 1) + ': ' + char} onClick={() => { submitWordSearchCell(r, c); wordSearchGridRef.current?.focus({ preventScroll: true }); }} className={'w-8 h-8 sm:w-9 sm:h-9 border border-slate-400 flex items-center justify-center font-mono text-sm sm:text-base font-bold cursor-pointer select-none transition-colors grid-cell ' + (active ? 'ring-2 ring-inset ring-teal-600 z-10 ' : '') + highlightClass}>{char}</div>; })}</div>)}</div></div><div className="mt-4 text-sm font-bold text-teal-800" role="status">Found {wordSearchFoundWords.size} of {gameData.words.length}</div><div className="mt-3 w-full max-w-md"><h4 className="text-sm font-bold text-slate-600 uppercase tracking-wider mb-2 text-center">{t('glossary.word_search_find')}</h4><div className="flex flex-wrap justify-center gap-x-6 gap-y-2 word-list">{gameData.words.map((word, i) => { var isFound = wordSearchFoundWords.has(word) || (foundWords && typeof foundWords.has === 'function' && foundWords.has(word)); return <span key={i} className={'text-sm font-medium px-2 py-1 rounded transition-all flex items-center gap-1 print:bg-transparent print:p-0 ' + (isFound ? 'bg-green-100 text-green-800 line-through opacity-70 decoration-green-600' : 'text-slate-700 bg-slate-100')}>{isFound && <CheckCircle2 size={12} className="inline" aria-hidden="true" />}{word}</span>; })}</div></div></div></div>}{canEditGlossary && <div data-help-key="glossary_add_term" className="flex flex-col sm:flex-row gap-2 mb-4 bg-white p-3 rounded-lg border border-slate-400 shadow-sm sm:items-center animate-in motion-reduce:animate-none fade-in slide-in-from-top-2"><div className="hidden sm:block shrink-0 bg-indigo-100 p-2 rounded-full text-indigo-600"><Plus size={16} /></div><div className="w-full sm:w-1/3 min-w-0 rounded-lg border border-indigo-200 bg-indigo-50/60 p-2"><label className="block text-[11px] font-bold text-indigo-900 mb-1">Image style</label><select aria-label="Glossary image style source" data-help-key="glossary_image_style_mode" value={glossaryStyleMode} onChange={handleGlossaryStyleModeChange} disabled={isAddingTerm} className="w-full rounded border border-indigo-300 bg-white px-2 py-2 text-xs focus:ring-2 focus:ring-indigo-500"><option value="inherit">Use Universal style</option><option value="override">Override for this resource</option></select>{glossaryStyleMode === 'inherit' ? <p className="mt-1 text-[10px] leading-snug text-indigo-800">{String(universalImageStyle || '').trim() ? 'Using Universal style: ' + String(universalImageStyle).trim() : 'No Universal style is set; the app default will be used.'}</p> : <input aria-label={t('common.enter_glossary_image_style')} data-help-key="glossary_image_style" type="text" value={glossaryImageStyle} onChange={e => setGlossaryImageStyle(e.target.value)} placeholder={t('glossary.style_placeholder')} className="mt-2 w-full rounded border border-indigo-300 bg-white px-2 py-2 text-xs placeholder:text-slate-600 focus:ring-2 focus:ring-indigo-500" disabled={isAddingTerm} />}<p className="mt-1 text-[10px] leading-snug text-slate-600">Applies to new or regenerated term images.</p></div><input aria-label={t('common.enter_new_glossary_term')} type="text" value={newGlossaryTerm} onChange={e => setNewGlossaryTerm(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddGlossaryTerm()} placeholder={t('glossary.add_term_placeholder')} className="w-full min-w-0 flex-1 text-sm border border-slate-300 sm:border-none py-2 px-2 focus:ring-2 focus:ring-indigo-500 bg-transparent placeholder:text-slate-600 rounded" disabled={isAddingTerm} /><button onClick={handleAddGlossaryTerm} disabled={!newGlossaryTerm.trim() || isAddingTerm} className="w-full sm:w-auto shrink-0 justify-center text-xs font-bold bg-indigo-600 text-white px-4 py-2 rounded-full hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">{isAddingTerm ? <RefreshCw size={12} className="animate-spin motion-reduce:animate-none" /> : <Sparkles size={12} className="text-yellow-700 fill-current" />}{isAddingTerm ? t('glossary.defining') : t('glossary.add_term')}</button></div>}{isTeacherMode && (glossaryHealthCheck || isRunningHealthCheck) && activeView === 'glossary' && <div className="mb-4 rounded-lg border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 shadow-sm overflow-hidden" data-help-key="glossary_health_check"><div className="w-full flex flex-wrap items-center gap-2 px-2 py-2">
  <button
    type="button"
  ...
  <div className="flex items-center gap-2">
    ...
    ><RefreshCw size={14} aria-hidden="true" /> Re-analyze</button>
    <button
      type="button"
    ...
    ><X size={16} aria-hidden="true" /></button>
...
</div>{showHealthCheckPanel && glossaryHealthCheck && !glossaryHealthCheck.error && <div id="glossary-health-check-details" role="region" aria-labelledby="glossary-health-check-title" className="px-4 pb-4 space-y-3 border-t border-amber-200/50 animate-in slide-in-from-top-2 motion-reduce:animate-none duration-200"><p className="text-sm text-amber-800 italic pt-2">{glossaryHealthCheck.summary}</p>{glossaryHealthCheck.definitionGradeLevel && <div className="flex flex-wrap items-center gap-3"><span className="text-xs font-bold text-slate-600 uppercase w-28 shrink-0">{t('glossary_health.grade_level')}</span><div className="flex flex-wrap items-center gap-2 flex-grow"><span className={`text-sm font-bold px-2.5 py-0.5 rounded-full ${glossaryHealthCheck.gradeAppropriate ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{glossaryHealthCheck.definitionGradeLevel}</span>{glossaryHealthCheck.gradeAppropriate ? <span className="text-xs text-emerald-600">✓ Appropriate</span> : <span className="text-xs text-amber-600">⚠ May need adjustment</span>}</div></div>}{glossaryHealthCheck.tierAudit && <div><span className="text-xs font-bold text-slate-600 uppercase">{t('glossary_health.vocabulary_tiers')}</span><div className="mt-1 flex flex-wrap gap-1.5">{(Array.isArray(glossaryHealthCheck.tierAudit.tier2) ? glossaryHealthCheck.tierAudit.tier2 : []).map((t, i) => <span key={`t2-${i}`} className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-medium">Tier 2: {t}</span>)}{(Array.isArray(glossaryHealthCheck.tierAudit.tier3) ? glossaryHealthCheck.tierAudit.tier3 : []).map((t, i) => <span key={`t3-${i}`} className="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full font-medium">Tier 3: {t}</span>)}</div>{Array.isArray(glossaryHealthCheck.tierAudit.notes) && glossaryHealthCheck.tierAudit.notes.length > 0 && <div className="mt-1.5 space-y-0.5">{glossaryHealthCheck.tierAudit.notes.map((note, i) => <p key={i} className="text-xs text-slate-600 italic">💡 {typeof note === 'string' ? note : JSON.stringify(note)}</p>)}</div>}{typeof glossaryHealthCheck.tierAudit.notes === 'string' && glossaryHealthCheck.tierAudit.notes.length > 0 && <div className="mt-1.5"><p className="text-xs text-slate-600 italic">💡 {glossaryHealthCheck.tierAudit.notes}</p></div>}</div>}{Array.isArray(glossaryHealthCheck.coverageGaps) && glossaryHealthCheck.coverageGaps.length > 0 && <div><span className="text-xs font-bold text-slate-600 uppercase">{t('glossary_health.suggested_terms')}</span><div className="mt-1 space-y-1">{glossaryHealthCheck.coverageGaps.map((gap, i) => <div key={i} className="flex flex-wrap items-center gap-2 bg-white/60 rounded-md px-2.5 py-1.5 border border-amber-100"><span className="text-sm font-semibold text-slate-800">"{gap.term}"</span><span className="min-w-0 text-xs text-slate-600 flex-grow">{gap.reason}</span>{isTeacherMode && <button onClick={async () => {
                  ...
                  }} className="text-[11px] font-bold bg-indigo-100 text-indigo-600 hover:bg-indigo-200 px-2 py-0.5 rounded-full transition-colors flex items-center gap-0.5 shrink-0" title={t('common.add_this_term_to_glossary')}><Plus size={10} /> Add</button>}</div>)}</div></div>}{Array.isArray(glossaryHealthCheck.conceptConnections) && glossaryHealthCheck.conceptConnections.length > 0 && <div><span className="text-xs font-bold text-slate-600 uppercase">{t('glossary_health.concept_web')}</span><div className="mt-1 space-y-1.5">{glossaryHealthCheck.conceptConnections.map((cc, i) => <div key={i} className="bg-white/60 rounded-md px-2.5 py-1.5 border border-amber-100"><div className="flex flex-wrap items-center gap-1.5"><span className="text-sm font-bold text-indigo-700">🔗 {cc.concept}</span><span className="text-xs text-slate-600">→</span><span className="min-w-0 text-xs text-slate-600">{(cc.connectedTerms || []).join(', ')}</span></div>{cc.note && <p className="text-[11px] text-slate-600 mt-0.5">{cc.note}</p>}</div>)}</div></div>}</div>}{showHealthCheckPanel && glossaryHealthCheck && glossaryHealthCheck.error && <div id="glossary-health-check-details" role="region" aria-labelledby="glossary-health-check-title" className="px-4 pb-3 text-sm text-amber-600 italic border-t border-amber-200/50 pt-2">Analysis could not be completed. Click "Re-analyze" to try again.</div>}{showHealthCheckPanel && isRunningHealthCheck && !glossaryHealthCheck && <div id="glossary-health-check-details" role="status" aria-live="polite" aria-labelledby="glossary-health-check-title" className="px-4 pb-3 text-sm text-amber-600 border-t border-amber-200/50 pt-2 flex items-center gap-2"><RefreshCw size={12} className="animate-spin motion-reduce:animate-none" aria-hidden="true" /> Analyzing glossary quality...</div>}</div>}{!isMemoryGame && <div data-help-key="glossary_terms_table" className="overflow-hidden rounded-lg border border-slate-400 shadow-sm"><p id="glossary-table-scroll-hint" className="px-3 py-2 text-xs text-slate-600 bg-slate-50 sm:hidden print:hidden">{t('glossary.table_scroll_hint')}</p><div data-glossary-word-list className="overflow-x-auto focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-indigo-600" role="region" aria-label={t('glossary.table_label')} aria-describedby="glossary-table-scroll-hint" tabIndex={0}><table className="w-full text-center text-sm"><thead className="bg-slate-100 text-slate-600 font-semibold"><tr>{isEditingGlossary && <th className="p-4 border-b border-slate-200 w-12 text-center"><div className="flex flex-col items-center gap-1"><input aria-label={t('common.text_field')} type="checkbox" className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4 cursor-pointer" checked={generatedContent?.data.length > 0 && generatedContent?.data.every(i => i.isSelected !== false)} onChange={handleGlossarySelectAll} title={t('glossary.tooltips.select_all_highlight')} /><span className="text-[11px] font-normal leading-none text-slate-600">{t('glossary.highlight_label')}</span></div></th>}<th className="p-4 border-b border-slate-200 min-w-[150px]">{t('glossary.table_term')}</th><th className="p-4 border-b border-slate-200 min-w-[200px]">{t('glossary.table_def')}</th>{displayLanguages.map(lang => <th key={lang} className="p-4 border-b border-slate-200 min-w-[150px] text-indigo-600 text-center">{lang}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{filteredGlossaryData.map(item => {
                  ...
                  return <tr key={getGlossaryEntryKey(item, idx)} className="group/row allo-vghov-row">{isEditingGlossary && <td className="p-4 border-b border-slate-100 text-center align-top"><input aria-label={t('common.toggle_is_selected_false')} type="checkbox" className="mt-1.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4 cursor-pointer" checked={item.isSelected !== false} onChange={() => handleGlossarySelectionChange(idx)} title={t('glossary.tooltips.select_highlight')} /></td>}<td className="p-4 font-bold text-slate-800 align-top min-w-[200px]"><div className="flex flex-col gap-2 items-center">{!isEditingGlossary && item.tier && <span className={`text-[11px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-full border mb-1 ${isTier2 ? 'bg-blue-50 text-blue-700 border-blue-200' : isTier3 ? 'bg-purple-50 text-purple-700 border-purple-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>{isTier2 ? 'Academic vocabulary' : isTier3 ? 'Subject vocabulary' : item.tier}</span>}{isEditingGlossary ? <div className="w-full flex flex-col gap-2"><select aria-label={(t('glossary.edit_tier_placeholder') || 'Vocabulary type') + ': ' + item.term} value={item.tier || ''} onChange={e => handleGlossaryChange(idx, 'tier', e.target.value)} className="text-[11px] font-bold uppercase tracking-wider w-full border border-slate-400 rounded px-1 py-1  focus:ring-1 focus:ring-indigo-500 bg-slate-50 text-slate-600" data-help-key="glossary_edit_tier"><option value="">{t('glossary.edit_tier_placeholder')}</option><option value="Academic">{t('glossary.edit_tier_academic')}</option><option value="Domain-Specific">{t('glossary.edit_tier_domain')}</option></select><div className="flex items-start gap-2 w-full"><textarea aria-label={t('glossary.edit_term') || 'Edit glossary term'} value={item.term} onChange={e => handleGlossaryChange(idx, 'term', e.target.value)} rows={getRows(item.term, 25)} className="w-full bg-white border border-blue-300 rounded px-2 py-1  focus:ring-2 focus:ring-blue-200 resize-y text-sm font-bold text-center" /><button type="button" aria-label={'Delete term: ' + item.term} onClick={() => handleDeleteGlossaryItem(idx)} className="min-h-11 min-w-11 inline-flex items-center justify-center bg-red-50 text-red-800 hover:bg-red-100 rounded transition-colors shrink-0" title={t('glossary.tooltips.delete_term')}><Trash2 size={14} /></button></div></div> : <div className="px-2 py-1 font-bold text-slate-800 whitespace-pre-wrap text-center">{item.emoji ? <span aria-hidden="true" className="mr-1">{item.emoji}</span> : null}{item.term}</div>}<div className="mt-1 px-1">{item.image ? <div className="flex flex-col gap-2"><div className="relative group/image inline-block w-fit"><img loading="lazy" src={item.image} alt={getGlossaryImageAlt(item)} role={getGlossaryImageAlt(item) ? undefined : "presentation"} style={{
                          ...
                          handlePhonicsClick && <button onClick={ev => handlePhonicsClick(item.term, ev)} className="text-slate-600 hover:text-emerald-700 hover:bg-emerald-50 min-h-11 min-w-11 inline-flex items-center justify-center rounded-full transition-colors flex-shrink-0 font-bold text-[11px] tracking-tight" aria-label={(t('glossary.pronounce_term') || 'Pronounce') + ': ' + item.term} title={t('glossary.pronounce_term_title') || 'Show pronunciation, IPA, and syllables'} data-help-key="glossary_pronounce_term">/əˈ/</button>}<button type="button" aria-label={'Download audio for term: ' + item.term} onClick={() => handleDownloadAudio(item.term, `term-${idx}-audio`, `dl-term-${idx}`)} disabled={downloadingContentId === `dl-term-${idx}`} className="min-h-11 min-w-11 inline-flex items-center justify-center text-slate-600 hover:text-indigo-600 rounded-full transition-colors" data-help-key="glossary_download_audio">{downloadingContentId === `dl-term-${idx}` ? <RefreshCw size={14} className="animate-spin motion-reduce:animate-none" /> : <Download size={14} />}</button></div></div></td><td className="p-4 text-slate-600 align-top"><div className="flex flex-col gap-2 items-center">{isEditingGlossary ? <textarea aria-label={t('glossary.edit_definition') || 'Edit definition'} value={item.def} onChange={e => handleGlossaryChange(idx, 'def', e.target.value)} rows={getRows(item.def, 45)} className="w-full bg-white border border-blue-300 rounded px-2 py-1  focus:ring-2 focus:ring-blue-200 resize-y text-sm text-center" /> : <div className="px-2 py-1 text-slate-600 whitespace-pre-wrap">{currentUiLanguage !== 'English' && <span className="block font-bold text-indigo-900 text-xs mb-1 uppercase tracking-wide">{item.term}</span>}{item.def}</div>}<div className="flex items-center gap-1 "><button type="button" aria-label={'Read definition for ' + item.term} onClick={() => handleGlossarySpeak(item, 'definition', item.def, `def-${idx}`, item.definitionLanguage || 'English')} disabled={isGeneratingAudio && playingContentId !== `def-${idx}`} className={`min-h-11 min-w-11 inline-flex items-center justify-center rounded-full transition-colors flex-shrink-0 ${playingContentId === `def-${idx}` ? 'text-red-700 bg-red-50' : 'text-slate-600 hover:text-indigo-600 hover:bg-indigo-50'}`}>{playingContentId === `def-${idx}` && isGeneratingAudio ? <RefreshCw size={14} className="animate-spin motion-reduce:animate-none" /> : playingContentId === `def-${idx}` ? <StopCircle size={14} /> : <Volume2 size={14} />}</button><button type="button" aria-label={'Download definition audio for ' + item.term} onClick={() => handleDownloadAudio(glossarySpeechText(item, 'definition', item.def), `def-${idx}-audio`, `dl-def-${idx}`)} disabled={downloadingContentId === `dl-def-${idx}`} className="min-h-11 min-w-11 inline-flex items-center justify-center text-slate-600 hover:text-indigo-600 rounded-full transition-colors">{downloadingContentId === `dl-def-${idx}` ? <RefreshCw size={14} className="animate-spin motion-reduce:animate-none" /> : <Download size={14} />}</button></div>{item.etymology ? <details className="mt-2 text-xs w-full" data-help-key="glossary_etymology_info"><summary className="cursor-pointer text-indigo-700 font-medium hover:text-indigo-900 select-none">📜 {t('glossary.etymology_label') || 'Word roots'}</summary>{(() => {
                            ...
                            return <div className="mt-1 pl-4 border-l-2 border-indigo-200 space-y-2">{etyMap && Object.keys(etyMap).length > 1 && <div className="text-[10px] uppercase tracking-wide text-indigo-500 font-bold">{displayLang}</div>}<div className="flex items-start gap-1"><p className="text-slate-700 leading-relaxed italic flex-1">{etymologyProse}</p><button onClick={() => handleUncachedGlossarySpeak(etymologyProse, `etym-${idx}`)} disabled={isGeneratingAudio && playingContentId !== `etym-${idx}`} className={`min-h-11 min-w-11 inline-flex items-center justify-center rounded-full transition-colors flex-shrink-0 ${playingContentId === `etym-${idx}` ? 'text-red-700 bg-red-50' : 'text-slate-600 hover:text-indigo-600 hover:bg-indigo-50'}`} aria-label={t('glossary.etymology_label') || 'Word roots'}>{playingContentId === `etym-${idx}` && isGeneratingAudio ? <RefreshCw size={12} className="animate-spin motion-reduce:animate-none" /> : <Volume2 size={12} />}</button></div>{(() => {
                                ...
                                return <div role="group" className="space-y-1.5" aria-label={t('glossary.etymology_roots_label') || 'Source roots'}><div className="flex flex-wrap gap-1.5 justify-center">{validRoots.map((r, ri) => {
                                      const related = Array.isArray(r.related) ? r.related.filter(w => typeof w === 'string' && w.trim()) : [];
                                      return <button key={ri} type="button" onClick={() => handleUncachedGlossarySpeak(`${r.root}${r.meaning ? ', meaning ' + r.meaning : ''}${related.length > 0 ? '. Related words: ' + related.join(', ') : ''}`, `root-${idx}-${ri}`)} title={`${r.lang || ''}${related.length > 0 ? (r.lang ? ' · ' : '') + 'Related: ' + related.join(', ') : ''}`.trim()} className="inline-flex items-center gap-1 rounded-full bg-indigo-50 border border-indigo-200 px-2 py-0.5 hover:bg-indigo-100 hover:border-indigo-300 transition-colors text-indigo-900" aria-label={`${r.root}${r.lang ? ' (' + r.lang + ')' : ''}${r.meaning ? ', meaning ' + r.meaning : ''}${related.length > 0 ? ', related to ' + related.join(', ') : ''}`}><span className="font-bold">{r.root}</span>{r.lang && <span className="text-[10px] text-indigo-600/80 uppercase tracking-wide">{r.lang}</span>}{r.meaning && <span className="text-slate-600">= {r.meaning}</span>}</button>;
                                    })}</div>{(() => {
                                    const allRelated = [];
                                    const seen = new Set();
                                    ...
                                    return <div className="text-[11px] text-slate-600 leading-snug pl-1"><span className="font-semibold text-slate-700">{t('glossary.related_words_label') || 'Related words:'}</span> {allRelated.slice(0, 6).map((w, wi) => <span key={wi}>{wi > 0 && <span className="text-slate-600">, </span>}<span className="font-medium text-indigo-800">{w}</span></span>)}</div>;
                                  })()}</div>;
                              })()}</div>;
                          })()}</details> : includeEtymology ? <button hidden={!canEditGlossary} style={{ display: canEditGlossary ? undefined : 'none' }} onClick={() => handleGenerateTermEtymology(idx, item.term)} disabled={(isGeneratingEtymology[entryKey] || isGeneratingEtymology[idx])} aria-busy={!!(isGeneratingEtymology[entryKey] || isGeneratingEtymology[idx])} aria-live="polite" className="mt-2 text-xs text-indigo-600 hover:text-indigo-800 disabled:opacity-50 disabled:cursor-wait flex items-center gap-1" data-help-key="glossary_etymology_info" title={t('glossary.etymology_label') || 'Word roots'}>{(isGeneratingEtymology[entryKey] || isGeneratingEtymology[idx]) ? <><RefreshCw size={12} className="animate-spin motion-reduce:animate-none" /> {t('glossary.actions.generating_etymology') || 'Finding roots…'}</> : <>📜 {t('glossary.actions.show_etymology') || 'Show word roots'}</>}</button> : null}</div></td>{displayLanguages.map(lang => <td key={lang} className="p-4 text-slate-600 italic border-l border-slate-50 align-top"><div className="flex flex-col gap-2" dir={isRtlLang(lang) ? 'rtl' : 'ltr'}>{isEditingGlossary ? <textarea aria-label={t('glossary.edit_translation') || 'Edit translation'} value={item.translations?.[lang] || ""} onChange={e => handleGlossaryChange(idx, 'translations', e.target.value, lang)} rows={getRows(item.translations?.[lang], 30)} className="w-full bg-white border border-blue-300 rounded px-2 py-1  focus:ring-2 focus:ring-blue-200 resize-y text-sm italic text-center" /> : <div className="px-2 py-1 text-slate-600 italic whitespace-pre-wrap text-center">{item.translations?.[lang] || ""}</div>}{item.translations?.[lang] && <div className="flex items-center gap-1  justify-center" dir="ltr"><button type="button" aria-label={'Read ' + lang + ' translation of ' + item.term} onClick={() => handleGlossarySpeak(item, 'translation', item.translations[lang], `trans-${idx}-${lang}`, lang)} disabled={isGeneratingAudio && playingContentId !== `trans-${idx}-${lang}`} className={`min-h-11 min-w-11 inline-flex items-center justify-center rounded-full transition-colors flex-shrink-0 ${playingContentId === `trans-${idx}-${lang}` ? 'text-red-700 bg-red-50' : 'text-slate-600 hover:text-indigo-600 hover:bg-indigo-50'}`}>{playingContentId === `trans-${idx}-${lang}` && isGeneratingAudio ? <RefreshCw size={14} className="animate-spin motion-reduce:animate-none" /> : playingContentId === `trans-${idx}-${lang}` ? <StopCircle size={14} /> : <Volume2 size={14} />}</button><button type="button" aria-label={'Download ' + lang + ' translation audio for ' + item.term} onClick={() => handleDownloadAudio(item.translations[lang], `trans-${idx}-${lang}-audio`, `dl-trans-${idx}-${lang}`)} disabled={downloadingContentId === `dl-trans-${idx}-${lang}`} className="min-h-11 min-w-11 inline-flex items-center justify-center text-slate-600 hover:text-indigo-600 rounded-full transition-colors">{downloadingContentId === `dl-trans-${idx}-${lang}` ? <RefreshCw size={14} className="animate-spin motion-reduce:animate-none" /> : <Download size={14} />}</button></div>}{item.translations?.[lang] && (() => {
                          ...
                          if (hasOwnProse) {
                            return <details className="mt-1 text-xs w-full" dir={isRtlLang(lang) ? 'rtl' : 'ltr'}><summary className="cursor-pointer text-indigo-700 font-medium hover:text-indigo-900 select-none text-center">📜 {rootsLabel}</summary><div className="mt-1 pl-3 border-l-2 border-indigo-200 space-y-2"><div className="flex items-start gap-1"><p className="text-slate-700 leading-relaxed italic flex-1 not-italic">{etyLangProse}</p><button onClick={() => handleUncachedGlossarySpeak(etyLangProse, `etym-${idx}-${lang}`)} disabled={isGeneratingAudio && playingContentId !== `etym-${idx}-${lang}`} className={`min-h-11 min-w-11 inline-flex items-center justify-center rounded-full transition-colors flex-shrink-0 ${playingContentId === `etym-${idx}-${lang}` ? 'text-red-700 bg-red-50' : 'text-slate-600 hover:text-indigo-600 hover:bg-indigo-50'}`} aria-label={rootsLabel}>{playingContentId === `etym-${idx}-${lang}` && isGeneratingAudio ? <RefreshCw size={12} className="animate-spin motion-reduce:animate-none" /> : <Volume2 size={12} />}</button></div>{validRootsL.length > 0 && <div role="group" className="space-y-1.5" aria-label={t('glossary.etymology_roots_label') || 'Source roots'} dir="ltr"><div className="flex flex-wrap gap-1.5 justify-center">{validRootsL.map((r, ri) => {
                                      ...
                                      return <button key={ri} type="button" onClick={() => handleUncachedGlossarySpeak(`${r.root}${_meaningLoc ? ', ' + _meaningLoc : ''}${relatedL.length > 0 ? '. ' + relatedLabel + ' ' + relatedL.join(', ') : ''}`, `root-${idx}-${lang}-${ri}`)} title={`${_langLoc || ''}${relatedL.length > 0 ? (_langLoc ? ' · ' : '') + relatedLabel + ' ' + relatedL.join(', ') : ''}`.trim()} className="inline-flex items-center gap-1 rounded-full bg-indigo-50 border border-indigo-200 px-2 py-0.5 hover:bg-indigo-100 hover:border-indigo-300 transition-colors text-indigo-900" aria-label={`${r.root}${_langLoc ? ' (' + _langLoc + ')' : ''}${_meaningLoc ? ', ' + _meaningLoc : ''}${relatedL.length > 0 ? ', ' + relatedLabel + ' ' + relatedL.join(', ') : ''}`}><span className="font-bold">{r.root}</span>{_langLoc && <span className="text-[10px] text-indigo-600/80 uppercase tracking-wide">{_langLoc}</span>}{_meaningLoc && <span className="text-slate-600">= {_meaningLoc}</span>}</button>;
                                    })}</div>{(() => {
                                    const allRelL = [];
                                    const seenL = new Set();
                                    ...
                                    return <div className="text-[11px] text-slate-600 leading-snug pl-1"><span className="font-semibold text-slate-700">{relatedLabel}</span> {allRelL.slice(0, 6).map((w, wi) => <span key={wi}>{wi > 0 && <span className="text-slate-600">, </span>}<span className="font-medium text-indigo-800">{w}</span></span>)}</div>;
                                  })()}</div>}</div></details>;
                          }
                          if (validRootsL.length > 0) {
                            return <details className="mt-1 text-xs w-full" dir={isRtlLang(lang) ? 'rtl' : 'ltr'}><summary className="cursor-pointer text-indigo-700 font-medium hover:text-indigo-900 select-none text-center">📜 {rootsLabel}</summary><div className="mt-1 pl-3 border-l-2 border-indigo-200 space-y-2" dir="ltr"><div className="flex flex-wrap gap-1.5 justify-center">{validRootsL.map((r, ri) => {
                                    const relatedL = Array.isArray(r.related) ? r.related.filter(w => typeof w === 'string' && w.trim()) : [];
                                    const _loc = localizeRoot(r);
                                    const _langLoc = _loc.langLocalized;
                                    const _meaningLoc = _loc.meaningLocalized;
                                    return <button key={ri} type="button" onClick={() => handleUncachedGlossarySpeak(`${r.root}${_meaningLoc ? ', ' + _meaningLoc : ''}${relatedL.length > 0 ? '. ' + relatedLabel + ' ' + relatedL.join(', ') : ''}`, `root-${idx}-${lang}-${ri}`)} title={`${_langLoc || ''}${relatedL.length > 0 ? (_langLoc ? ' · ' : '') + relatedLabel + ' ' + relatedL.join(', ') : ''}`.trim()} className="inline-flex items-center gap-1 rounded-full bg-indigo-50 border border-indigo-200 px-2 py-0.5 hover:bg-indigo-100 hover:border-indigo-300 transition-colors text-indigo-900" aria-label={`${r.root}${_langLoc ? ' (' + _langLoc + ')' : ''}${_meaningLoc ? ', ' + _meaningLoc : ''}${relatedL.length > 0 ? ', ' + relatedLabel + ' ' + relatedL.join(', ') : ''}`}><span className="font-bold">{r.root}</span>{_langLoc && <span className="text-[10px] text-indigo-600/80 uppercase tracking-wide">{_langLoc}</span>}{_meaningLoc && <span className="text-slate-600">= {_meaningLoc}</span>}</button>;
                                  })}</div><button hidden={!canEditGlossary} style={{ display: canEditGlossary ? undefined : 'none' }} onClick={() => handleGenerateTermEtymology(idx, item.term)} disabled={(isGeneratingEtymology[entryKey] || isGeneratingEtymology[idx])} aria-busy={!!(isGeneratingEtymology[entryKey] || isGeneratingEtymology[idx])} className="mt-1 text-[11px] text-indigo-600 hover:text-indigo-800 disabled:opacity-50 disabled:cursor-wait flex items-center gap-1" title={rootsLabel}>{(isGeneratingEtymology[entryKey] || isGeneratingEtymology[idx]) ? <><RefreshCw size={10} className="animate-spin motion-reduce:animate-none" /> {loadingLabel}</> : <>+ {showLabel}</>}</button></div></details>;
                          }
                          if (includeEtymology || hasEnglishEty) {
                            return <button hidden={!canEditGlossary} style={{ display: canEditGlossary ? undefined : 'none' }} onClick={() => handleGenerateTermEtymology(idx, item.term)} disabled={(isGeneratingEtymology[entryKey] || isGeneratingEtymology[idx])} aria-busy={!!(isGeneratingEtymology[entryKey] || isGeneratingEtymology[idx])} className="mt-1 text-xs text-indigo-600 hover:text-indigo-800 disabled:opacity-50 disabled:cursor-wait flex items-center gap-1 justify-center" title={rootsLabel} dir={isRtlLang(lang) ? 'rtl' : 'ltr'}>{(isGeneratingEtymology[entryKey] || isGeneratingEtymology[idx]) ? <><RefreshCw size={12} className="animate-spin motion-reduce:animate-none" /> {loadingLabel}</> : <>📜 {showLabel}</>}</button>;
                          }
                          return null;
                        })()}</div></td>)}</tr>;
        ...
        phonicsData && activeView === 'glossary' && <><div aria-hidden="true" className="fixed inset-0 z-[90]" onClick={closePhonics} /><div ref={phonicsDialogRef} role="dialog" aria-modal="true" aria-labelledby="glossary-phonics-popup-title" tabIndex={-1} onKeyDown={e => containModalFocus(e, phonicsDialogRef.current, closePhonics)} className="fixed z-[100] bg-white allo-popover-solid p-5 rounded-xl shadow-2xl border-2 border-emerald-200 w-72 animate-in zoom-in-95 duration-200 motion-reduce:animate-none motion-reduce:transition-none" style={{
            ...
            left: Math.min(window.innerWidth - 300, (phonicsData.x || 100) - 20) + 'px'
                ...
                }} className="bg-emerald-700 hover:bg-emerald-800 text-white p-2 rounded-full shadow-md transition-transform hover:scale-110 active:scale-95" title={t('glossary.popups.replay')}><Volume2 size={20} className="fill-current" /></button></div>{renderPhonicsDictRow(phonicsData, t, playGlossaryDictionaryAudio)}<div className="grid grid-cols-2 gap-2"><div className="bg-slate-50 p-2 rounded border border-slate-100"><div className="text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1">{t('glossary.popups.ipa')}</div><div className="font-mono text-sm text-slate-600">{phonicsData.data.ipa}</div></div><div className="bg-slate-50 p-2 rounded border border-slate-100"><div className="text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1">{t('glossary.popups.syllables')}</div><div className="flex flex-wrap items-center gap-0.5">{(phonicsData.data.syllables || []).map(function (syl, i) {
                      return <>{i > 0 && <span className="text-emerald-500 font-bold px-0.5" aria-hidden="true">•</span>}<span className="bg-white px-1.5 rounded border border-slate-400 text-sm font-bold text-slate-700 shadow-sm">{glossaryAiText(syl)}</span></>;
