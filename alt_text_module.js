...
async function draftAlts(images, options) {
  ...
  const callMotion = async (image, pair) => {
    ...
    const single = (parseDraftReply(raw, 1) || /* @__PURE__ */ new Map()).get(0);
    results[list.indexOf(image)] = single ? Object.assign({ id: image.id, source: "vision" }, single, single.decorative ? {} : { kind: "animation" }) : planning(image);
  };
  const stills = [];
  const motions = [];
  list.forEach((image) => {
  ...
  for (let i = 0; i < stills.length; i += batchSize) {
    ...
    try {
      ...
      if (error && error.name === "AbortError") throw error;
      stills.slice(i, i + batchSize).forEach((image) => {
