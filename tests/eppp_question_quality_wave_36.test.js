...
const wave = require('../dev-tools/eppp_native_quality_wave_36_data.cjs');
...
describe('EPPP expanded quality wave 36', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_36.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 24, applicationItems: 11, analysisItems: 13, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_36.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_36.json'));
  });

  it('replays after wave 35', () => {
    ...
    expect(i36).toBeGreaterThan(i35);
  });
});
