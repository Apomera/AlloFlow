...
const AllobotEvidence = (() => {
  ...
  function plan(userText) {
    ...
    const query = clean(text.replace(/^(?:please\s+)?(?:can you\s+)?(?:search(?: the web)?(?: for)?|look up|verify|fact[- ]?check|check this claim|find (?:sources|research|evidence)(?: about| on| for)?)[\s:]+/i, ''));
