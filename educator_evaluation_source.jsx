...
function AeLocalOnboarding({ onChoose }) {
  ...
  return <div className="ae-onboarding-overlay" role="dialog" aria-modal="true" aria-labelledby="ae-onboarding-title" aria-describedby="ae-onboarding-description" onKeyDown={trapTab}>
    <section className="ae-onboarding-card">
      ...
      <p id="ae-onboarding-description">Your work stays private on this device — nothing is uploaded. Start your own workspace, or open simulated data first if you would rather see the whole cycle before entering anything real. Connecting your district portal later adds shared, authenticated records.</p>
      <div className="ae-onboarding-options" role="group" aria-label="Choose evaluation workspace starting point">
        <button type="button" ref={firstRef} className="ae-onboarding-option" onClick={() => onChoose('blank')}>
          <strong>Start with a blank workspace</strong>
          <span>Nothing is preloaded. Add your own educators and begin the year’s cycle.</span>
        </button>
      ...
      <div className="ae-onboarding-note"><strong>Where this lives:</strong> in your signed-in profile on this device, never uploaded — protected by your device sign-in rather than by encryption AlloFlow adds. That makes it yours alone: educators cannot see or acknowledge anything here until your district connects the portal, which your LEA authorizes. Keep a backup, and apply your district’s retention rules to evaluation records.</div>
    </section>
  </div>;
}
...
function AeShareQr({ isRemote, standalone, portalUrl }) {
  ...
  return <section className="ae-card ae-span-12" aria-labelledby="ae-share-qr-title"><div className="ae-record-head"><div><h3 id="ae-share-qr-title">Share by QR</h3><p className="ae-sub">{isRemote
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 18, alignItems: 'center', marginTop: 10 }}>
      <figure style={{ margin: 0 }} aria-label={'QR code linking to ' + payload}><div style={{ width: 176, height: 176, background: '#fff', padding: 6, border: '1px solid #ccd5e2', borderRadius: 10 }} dangerouslySetInnerHTML={{ __html: svg }} /></figure>
      <div style={{ minWidth: 220, flex: 1 }}>
        <p className="ae-sub" style={{ wordBreak: 'break-all' }}>{payload}</p>
        <button type="button" className="ae-btn" onClick={() => { try { navigator.clipboard.writeText(payload); setCopied(true); setTimeout(() => setCopied(false), 2000); } catch (err) {} }}>{copied ? 'Link copied' : 'Copy link'}</button>
      </div>
    </div>
  </section>;
}
...
function AeAbout({ workspace, updateConfig, role, isRemote = false, currentUser = null, repository = null, standalone = false, portalUrl = '' }) {
  ...
  return <div className="ae-page">
    ...
    <div className="ae-grid">
      ...
      <AeShareQr isRemote={isRemote} standalone={standalone} portalUrl={portalUrl}/>
...
function EducatorEvaluationPanel(props) {
  ...
  const exportEducatorPacket = () => {
    ...
    const readable = '<!doctype html><html lang="en"><head><meta charset="utf-8">'
      ...
      + '.fine{font-size:.9rem;color:#475569;margin:6px 0 14px}'
      ...
      + obsRows
      + '<div class="ackrow"><input type="checkbox" id="ae-ack"><label for="ae-ack">I have read this evaluation.</label></div><p class="fine">Ticking this records that you read it. It is not the signature on the evaluation form: your district’s process still asks you and your evaluator to sign at the conference.</p>'
  ...
  const body = <div ref={dialogRef} tabIndex={-1} className="ae-workspace" role={standalone ? undefined : 'dialog'} aria-modal={standalone ? undefined : 'true'} aria-labelledby="ae-title">
    <header className="ae-top">
      ...
      <div className="ae-top-actions">{!isRemote && <div className="ae-role" aria-label="View this workspace as evaluator or educator"><button type="button" aria-pressed={role === 'evaluator'} onClick={() => setRole('evaluator')}>Evaluator</button><button type="button" aria-pressed={role === 'teacher'} onClick={() => setRole('teacher')}>Teacher</button></div>}<a className="ae-btn" data-help-key="ae_manual_link" /* Extensionless, matching the Setup-tab link and the district Portal. That
