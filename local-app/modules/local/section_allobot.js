      ...
      if (!isSilent && soundEnabledRef.current && !authFailedRef.current) {
          ...
          if (onGenerateAudio) {
              ...
              try {
                  ...
                  if (!audioUrl) {
                      console.warn("[TTS] Local TTS attempt 1 timed out or returned null — retrying after delay...");
                      await new Promise(r => setTimeout(r, 1500));
                      if (myGenId !== speechGenerationRef.current) return;
                      audioUrl = await attemptTTS(2);
                  }
                  if (!audioUrl) {
                      console.warn("[TTS] ⚠️ Both Local TTS attempts failed — no audio available");
                  } else {
