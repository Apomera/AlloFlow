...
const out = path.join(ROOT, 'scratch', 'water-beach-profile-review');
...
(async () => {
  ...
  try {
    ...
    const coast=()=>page.evaluate(()=>{
      const scene=pilotReview.scene,foam=scene.getObjectByName('pilot-shore-foam'),shelf=scene.getObjectByName('pilot-coastal-shallows');
      return {positions:[shelf,...foam.children].map(m=>Array.from(m.geometry.attributes.position.array)),
        opacity:shelf.material.opacity,time:foam.children[0].material.userData.surfaceShader.uniforms.pilotCoastTime.value};
    });
    async function checkCoast(){
      const result=await page.evaluate(()=>{
        const scene=pilotReview.scene,ocean=scene.getObjectByName('pilot-ocean-surface'),shelf=scene.getObjectByName('pilot-coastal-shallows'),foam=scene.getObjectByName('pilot-shore-foam');scene.updateMatrixWorld(true);
        const ray=new THREE.Raycaster(),down=new THREE.Vector3(0,-1,0);let maxError=0,samples=0,finite=true;
        for(const mesh of [shelf,...foam.children]){
          const p=mesh.geometry.attributes.position;finite&&=Array.from(p.array).every(Number.isFinite);
          for(let i=0;i<p.count;i+=137){const point=mesh.localToWorld(new THREE.Vector3().fromBufferAttribute(p,i));ray.set(new THREE.Vector3(point.x,20,point.z),down);
            const hit=ray.intersectObject(ocean)[0];if(!hit)throw Error('Coastal detail outside ocean');
            maxError=Math.max(maxError,Math.abs(point.y-hit.point.y-mesh.position.y));samples++;
          }
        }
        const fade=Array.from(shelf.geometry.attributes.coastalFade.array);
        return {maxError,samples,finite,bands:foam.children.length,meshes:foam.children.every(m=>m.isMesh),
          fade:fade.every(v=>v>=0&&v<=1)&&fade.some(v=>v===0)&&fade.some(v=>v===1),
          sharedClock:foam.children.every(m=>m.material.userData.surfaceShader.uniforms.pilotCoastTime===foam.children[0].material.userData.surfaceShader.uniforms.pilotCoastTime)};
      });assert(result.samples>30&&result.maxError<.002,JSON.stringify(result));assert(result.finite&&result.meshes&&result.fade&&result.sharedClock);assert.equal(result.bands,3);
    }
    async function checkBeach(){
      const result=await page.evaluate(()=>{
        const scene=pilotReview.scene,meshes=['pilot-dry-sand-slope','pilot-wet-sand-slope'].map(n=>scene.getObjectByName(n));
        const ray=new THREE.Raycaster(new THREE.Vector3(),new THREE.Vector3(0,-1,0));let seamError=0;
        const checks=meshes.map(mesh=>{
          const g=mesh.geometry,p=g.attributes.position,n=g.attributes.normal,profile=g.userData.beachProfile;
          let maxRise=0,minNormal=1;
          for(let ring=0;ring<profile.rings;ring++)for(let col=0;col<profile.columns;col++){
            const i=profile.base+ring*profile.columns+col;minNormal=Math.min(minNormal,n.getY(i));
            if(col)maxRise=Math.max(maxRise,p.getY(i)-p.getY(i-1));
          }
          return {finite:Array.from(p.array).every(Number.isFinite)&&Array.from(n.array).every(Number.isFinite),maxRise,minNormal,
            top:p.getY(profile.base),bottom:p.getY(profile.base+10),gradient:g.attributes.color.getX(profile.base)>g.attributes.color.getX(profile.base+10)};
        });
        scene.updateMatrixWorld(true);const p=meshes[0].geometry.attributes.position,profile=meshes[0].geometry.userData.beachProfile;
        for(let ring=0;ring<profile.rings;ring+=9){const i=profile.base+ring*11+10;ray.ray.origin.set(p.getX(i),20,p.getZ(i));
          const hit=ray.intersectObject(meshes[1])[0];if(!hit)throw Error('Gap beneath dry/wet sand join');seamError=Math.max(seamError,Math.abs(hit.point.y-p.getY(i)));
        }
        return {checks,seamError};
      });assert(result.checks.every(c=>c.finite&&c.maxRise===0&&c.minNormal>.1&&c.gradient),JSON.stringify(result));
      assert(result.seamError<.12,JSON.stringify(result));assert(Math.abs(result.checks[0].top-4.85)<.001);assert(result.checks[1].bottom<-1.8);
    }
    ...
    await page.waitForFunction(()=>pilotReview.scene.getObjectByName('pilot-shore-foam').children.every(m=>m.material.userData.surfaceShader));
    await checkBeach();await checkCoast();const pausedCoast=await coast();await page.waitForTimeout(500);assert.deepEqual(await coast(),pausedCoast);
    ...
    assert.notDeepEqual((await coast()).positions,pausedCoast.positions,'Foam and shallows follow animated waves');await checkCoast();
    await page.emulateMedia({reducedMotion:'reduce'});
    await restore();await page.waitForFunction(()=>document.querySelector('#wcPilotCanvas').dataset.ambientCloudMotion==='still');
    const stillCoast=await coast();const still=await patch();await page.getByRole('button',{name:'Resume the simulation',exact:true}).click();
    await page.waitForFunction(()=>waterReviewData.pilot.snapshot.elapsed>.5);
    await page.getByRole('button',{name:'Pause the simulation',exact:true}).click();
    assert.deepEqual(await patch(),still,'Reduced motion keeps the highlight on the frozen surface');await checkSurface();assert.deepEqual(await coast(),stillCoast,'Reduced motion freezes all coastal detail');await checkCoast();
    ...
    await restore(-28,30);
    await canvas.screenshot({path:path.join(out,'coast-follow.jpg'),type:'jpeg',quality:82});
    await page.getByRole('button',{name:'Water view',exact:true}).click();
    await canvas.screenshot({path:path.join(out,'coast-water-view.jpg'),type:'jpeg',quality:82});
    await page.evaluate(()=>{const renderer=pilotReview.renderer,render=renderer.render.bind(renderer);
      renderer.render=(scene,camera)=>{camera.position.set(-38,16,74);camera.lookAt(5,1,53);return render(scene,camera);};});
    await page.waitForTimeout(500);await canvas.screenshot({path:path.join(out,'coast-detail.jpg'),type:'jpeg',quality:82});
    ...
    await page.evaluate(()=>{
      window.sunlightDisposals={ring:0,fill:0,texture:0,shelfGeometry:0,shelfMaterial:0,foamGeometry:0,foamMaterial:0,beachGeometry:0,beachMaterial:0,wetGeometry:0,wetMaterial:0};const scene=pilotReview.scene;
      ...
      const shelf=scene.getObjectByName('pilot-coastal-shallows');shelf.geometry.addEventListener('dispose',()=>sunlightDisposals.shelfGeometry++);shelf.material.addEventListener('dispose',()=>sunlightDisposals.shelfMaterial++);
      scene.getObjectByName('pilot-shore-foam').children.forEach(m=>{m.geometry.addEventListener('dispose',()=>sunlightDisposals.foamGeometry++);m.material.addEventListener('dispose',()=>sunlightDisposals.foamMaterial++);});
      const beach=scene.getObjectByName('pilot-dry-sand-slope'),wet=scene.getObjectByName('pilot-wet-sand-slope');
      beach.geometry.addEventListener('dispose',()=>sunlightDisposals.beachGeometry++);beach.material.addEventListener('dispose',()=>sunlightDisposals.beachMaterial++);
      wet.geometry.addEventListener('dispose',()=>sunlightDisposals.wetGeometry++);wet.material.addEventListener('dispose',()=>sunlightDisposals.wetMaterial++);
      mountWater({wcMode:'explorer'});
    });assert.deepEqual(await page.evaluate(()=>sunlightDisposals),{ring:1,fill:1,texture:1,shelfGeometry:1,shelfMaterial:1,foamGeometry:3,foamMaterial:3,beachGeometry:1,beachMaterial:1,wetGeometry:1,wetMaterial:1});assert.deepEqual(errors,[]);
    console.log('PASS: continuous downward beach profiles, upward normals, bounded sand join gap, wet-edge color gradient, submerged beach foot, beach cleanup, coastal foam and shelf follow actual ocean triangles, fading shelf, shared foam clock, coastal pause and reduced motion, finite mesh ribbons, coastal resource disposal, exact ocean-triangle alignment, inside/outside feedback, unchanged evaporation rate ratio, pause and reduced motion, both camera views, vapor visibility, mobile accessibility, geometry/texture cleanup, no browser errors.');
