...
describe('expanded STEM contrast coverage',()=>{
  it('keeps literal gradient endpoints AA in every state',()=>{const failures=[];for(const name of files()){const s=readFileSync(resolve(process.cwd(),'stem_lab',name),'utf8');for(const m of s.matchAll(/(["'\x60])([^"'\x60\r\n]*bg-gradient-to-[^"'\x60\r\n]*)\1/g)){const w=m[2].split(/\s+/),tt=w.find(x=>!x.includes(':')&&color(x,'text'));if(!tt)continue;for(const x of w.filter(v=>/^(?:(?:hover|active|focus|group-hover):)?(?:from|via|to)-[a-z]+-\d+$/.test(v))){const k=x.replace(/^(?:hover|active|focus|group-hover):/,'').split('-')[0],bg=color(x,k);if(bg&&ratio(color(tt,'text'),bg)<4.5)failures.push(name+': '+tt+' on '+x);}}}expect(failures).toEqual([]);});
