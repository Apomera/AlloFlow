...
const wave = require('../dev-tools/eppp_native_quality_wave_35_data.cjs');
...
describe('EPPP expanded quality wave 35', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_35.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 23, applicationItems: 14, analysisItems: 10, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    expect(audit.summary.distractorWarningsAfter.uniqueKeyStemLexicalLeakageCandidates).toBe(39);
    ...
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicateClusters).toBe(37);
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(457);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1271);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(587);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(630);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_35.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_35.json'));
  });

  it('replays after wave 34', () => {
    ...
    const i35 = builder.indexOf('repair_eppp_native_quality_wave_35.cjs');
