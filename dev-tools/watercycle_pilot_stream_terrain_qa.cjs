...
// Run: node dev-tools/watercycle_pilot_stream_terrain_qa.cjs
const fs = require('fs');
const path = require('path');
const assert = require('node:assert/strict');
const { chromium } = require('playwright');
const ROOT = process.cwd();
const out = path.join(ROOT, 'scratch', 'water-stream-terrain-review');
...
(async () => {
  ...
  try {
    ...
    await page.evaluate(()=>{
      window.streamDisposals={geometry:0,texture:0,channel:0,riffleGeometry:0,riffleMaterial:0,cascadeGeometry:0,cascadeMaterial:0,terrainGeometry:0,terrainMaterial:0,terrainMask:0};const stones=pilotReview.scene.getObjectByName('pilot-stream-bank-stones');
