...
describe('EPPP claim-level adjudication batch 02', () => {
  ...
  it('keeps deployment artifacts identical to their source artifacts', () => {
    expect(read('prismflow-deploy/public/test_prep/eppp_legacy/adjudication_batch_02.json')).toEqual(read('test_prep/eppp_legacy/adjudication_batch_02.json'));
    expect(fs.readFileSync(resolve(root, 'prismflow-deploy/public/test_prep/eppp_legacy/adjudication_batch_02.md'), 'utf8')).toBe(fs.readFileSync(resolve(root, 'test_prep/eppp_legacy/adjudication_batch_02.md'), 'utf8'));
  });
});
