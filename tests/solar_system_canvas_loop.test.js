...
describe('solar system main 3D canvas loop', () => {
  ...
  it('exposes elapsed Earth time and hover target cues in the Orrery', () => {
    SOLAR_SYSTEM_PATHS.forEach((filePath) => {
      ...
      expect(source).toContain('@media (min-width:900px) and (max-width:1199px)');
      expect(source).toContain('role: "region", tabIndex: 0');
      expect(source).toContain('id: "orrery-stage-key-scroll-hint"');
      expect(source).toContain('className: "orr-stage-key-shell"');
      expect(source).toContain('"data-key-items": stageKeyItemCount');
    });
  });
  it('offers disclosed marker-size modes and collision-safe Orrery labels', () => {
    SOLAR_SYSTEM_PATHS.forEach((filePath) => {
      ...
      expect(source).toContain('function placeOrbitLabel(sx, sy, dotR, pillW, pillH, required)');
      expect(source).toContain('var labelRequests = [];');
      expect(source).toContain('if (!best || (bestScore > 0 && !required)) return null;');
      expect(source).toContain('labelRequests.sort(function(a, b) { return a.priority - b.priority; });');
      expect(source).toContain('drawOrbitLabel(labelRequests[labelRequestIndex]);');
      expect(source).toContain('cv._orreryLabelLayout = {');
      expect(source).toContain('selectedOrbitScreenRadius >= Math.min(90, viewW * 0.18)');
      expect(source).toContain('labelBoxes.push({ x: anchorLabelX - 3');
      expect(source).toContain('var hideCompactPlanetLabel = zoomMode === "full" && viewW < 480');
      expect(source).toContain('b.type !== "planet" && selBody !== b.id && !isHoveredBody');
  ...
  it('prevents drag-release clicks and exposes canvas keyboard shortcuts', () => {
    SOLAR_SYSTEM_PATHS.forEach((filePath) => {
      ...
      expect(source).toContain('function resizeCanvasBuffer(preserveCamera)');
      expect(source).toContain('var cssWidth = responsiveCanvas ? Math.max(1, Math.round(measuredWidth)) : props.width;');
      expect(source).toContain('var backingWidth = Math.max(1, Math.round(cssWidth * nextDpr));');
      expect(source).toContain('return Math.min(2, Math.max(1, rawDpr));');
      expect(source).toContain('resizeObserver = new window.ResizeObserver(function()');
      expect(source).toContain('resizePending = true;');
      expect(source).toContain('resizeCanvasBuffer(true);');
      expect(source).toContain('if (resizeObserver) resizeObserver.disconnect();');
      expect(source).toContain('if (intersectionObserver) intersectionObserver.disconnect();');
      expect(source).toContain('if (fallbackResizeHandler) window.removeEventListener("resize", fallbackResizeHandler);');
      const observerStart = source.indexOf('resizeObserver = new window.ResizeObserver(function()');
      const observerEnd = source.indexOf('resizeObserver.observe(cv);', observerStart);
      const observerSlice = source.slice(observerStart, observerEnd);
      expect(observerStart, filePath + ' should install ResizeObserver').toBeGreaterThan(-1);
      expect(observerEnd, filePath + ' should observe the responsive canvas').toBeGreaterThan(observerStart);
      expect(observerSlice).toContain('resizePending = true;');
      expect(observerSlice).toContain('scheduleFrame();');
      expect(observerSlice).not.toContain('resizeCanvasBuffer');
      const drawStart = source.indexOf('function draw(t) {', observerEnd);
      const drawResize = source.indexOf('resizeCanvasBuffer(true);', drawStart);
      const drawPaint = source.indexOf('drawRef.current(ctx, cv, st, t);', drawStart);
      expect(drawResize, filePath + ' should resize inside the animation frame').toBeGreaterThan(drawStart);
      expect(drawPaint, filePath + ' should paint after applying a pending resize').toBeGreaterThan(drawResize);
      ...
      expect(source).toContain('cv._logicalWidth = cssWidth;');
      expect(source).toContain('cv._designWidth = props.width;');
      expect(source).toContain('st.viewportWidth = cssWidth;');
      expect(source).toContain('st.canvasHitRadius = st.coarsePointer ? 22 : 14;');
      expect(source).toContain('responsive: true,');
      expect(source).toContain('responsiveMinHeight: 340');
      expect(source).toContain('responsiveMaxHeight: 620');
      ...
      expect(source).toContain('if (!running || document.hidden || !canvasInViewport || raf.current) return;');
