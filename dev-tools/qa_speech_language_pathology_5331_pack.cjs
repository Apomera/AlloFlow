...
const report = { schemaVersion: 1, generatedAt, packId: pack.id, standard: { label: 'AlloFlow independent 5331 source, structure, feedback, originality, safety, and linkage QA v1', meaning: 'A pass confirms exact near-equal diagnostic allocation, balanced answer positions, independently authored prompts, official-sample fragment screening, authoritative-source allowlisting, substantive feedback, stable identifiers, and complete learning links.', limitation: 'This is not ETS or ASHA approval, independent licensed-SLP validation, field testing, psychometric calibration, a scaled-score model, clinical evaluation, diagnosis, medical or swallowing-safety decision, treatment, supervision, or legal advice.' }, blueprint: { officialQuestionCount: 132, timeMinutes: 150, selectedResponse: true, categories: Object.fromEntries(Object.entries(expectedDomains).map(([id, entry]) => [id, { questions: entry.officialQuestions, percentage: 33.333333 }])) }, diagnosticBatch: { batchCount: 2, batchSize: 100, categories: Object.fromEntries(Object.entries(expectedDomains).map(([id, entry]) => [id, entry.diagnosticQuestions])), batches: diagnosticBatches }, summary: { totalItems: reports.length, passedItems, reviewRequiredItems: reports.length - passedItems, findings: allFindings.length, status }, findings, items: reports };
...
for (const outputRoot of [path.join(root, 'test_prep'), path.join(root, 'prismflow-deploy', 'public', 'test_prep')]) { fs.mkdirSync(outputRoot, { recursive: true }); fs.writeFileSync(path.join(outputRoot, 'speech_language_pathology_5331_native_qa.json'), JSON.stringify(report, null, 2) + '\n', 'utf8'); fs.writeFileSync(path.join(outputRoot, 'speech_language_pathology_5331_native_qa.md'), markdown, 'utf8'); }
