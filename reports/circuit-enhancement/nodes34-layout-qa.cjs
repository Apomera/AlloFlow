...
const root=process.cwd(),out=path.join(root,'reports/circuit-enhancement/nodes34-layout-qa');
(async()=>{
...
function Host(){
const [data,setData]=React.useState({_circuit:{pauseMotion:true},_circuitNetwork:{boardOverlay:"energy",boardNodeMode:"inspect",inspectNode:"0"}});
...
await click('Inspect network nodes');await page.evaluate(()=>setState(prev=>({...prev,_circuitNetwork:{...prev._circuitNetwork,analysis:'dc',components:[{id:1,type:'voltage',a:'A',b:'0',value:5},...Array.from({length:15},(_,i)=>({id:i+2,type:i%3===0?'capacitor':'resistor',a:['A','B','C','D','E','F','G'][i%7],b:'0',value:1000}))],selected:4}})));
