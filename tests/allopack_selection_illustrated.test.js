...
const slug='natural_selection_grade8',folder='allopacks/media/'+slug+'/',pack=read('allopacks/illustrated/'+slug+'.allopack.json'),original=read('allopacks/'+slug+'.allopack.json'),manifest=read(folder+'manifest.json');
require('../dev-tools/selection_content_refinements.cjs').refine(original);
...
const frequency=svg('frequency');expect([...frequency.matchAll(/data-dark-count="([0-9]+)"/g)].map(m=>Number(m[1]))).toEqual([2,4,6]);expect((frequency.match(/data-total="8"/g)||[]).length).toBe(3);const resistance=svg('resistance');expect((resistance.match(/data-variant="dark"/g)||[]).length).toBe(12);expect((resistance.match(/data-variant="light"/g)||[]).length).toBe(6);expect(resistance).toContain('data-flow="reproduction"');expect(get('simplified').data).toContain('grow, learn and acclimate');expect(get('quiz').data.questions[8].expectedAnswer).toContain('help cause their spread');
});
