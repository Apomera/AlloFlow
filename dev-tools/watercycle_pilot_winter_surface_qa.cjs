...
// Run: node dev-tools/watercycle_pilot_winter_surface_qa.cjs
const fs = require('fs');
const path = require('path');
const assert = require('node:assert/strict');
const { chromium } = require('playwright');
const ROOT = process.cwd();
const out = path.join(ROOT, 'scratch', 'water-winter-surface-review');
...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(()=>pilotReview.scene.getObjectByName('pilot-forest-snow').material.userData.snowShader);
    const surfaceSnow=await page.evaluate(()=>{
      const scene=pilotReview.scene,snow=scene.getObjectByName('pilot-forest-snow'),upper=scene.getObjectByName('pilot-forest-upper-crowns'),rocks=scene.getObjectByName('pilot-landscape-rocks');
      const a=snow.geometry.attributes.position,b=upper.geometry.attributes.position,m=new THREE.Matrix4(),n=new THREE.Matrix4(),p=new THREE.Vector3(),q=new THREE.Vector3();let maxClearance=0;
      for(let i=0;i<snow.count;i++){snow.getMatrixAt(i,m);upper.getMatrixAt(i,n);for(let j=0;j<a.count;j++){p.fromBufferAttribute(a,j).applyMatrix4(m);q.fromBufferAttribute(b,j).applyMatrix4(n);maxClearance=Math.max(maxClearance,p.distanceTo(q));}}
      return {aligned:a.count===b.count&&Array.from(a.array).every((v,i)=>v===b.array[i]),maxClearance,
        enabled:snow.material.userData.snowShader.uniforms.pilotSurfaceSnow.value,
        shared:snow.material.userData.snowShader.uniforms.pilotSurfaceSnow===rocks.material.userData.snowShader.uniforms.pilotSurfaceSnow,
        offset:snow.material.polygonOffset,depthWrite:snow.material.depthWrite};
    });assert(surfaceSnow.aligned&&surfaceSnow.maxClearance<.08&&surfaceSnow.maxClearance>.03);assert.equal(surfaceSnow.enabled,1);assert(surfaceSnow.shared&&surfaceSnow.offset&&!surfaceSnow.depthWrite);
    ...
    await page.evaluate(()=>{const scene=pilotReview.scene,rocks=scene.getObjectByName('pilot-landscape-rocks'),m=new THREE.Matrix4(),p=new THREE.Vector3();rocks.getMatrixAt(Math.min(6,rocks.count-1),m);p.setFromMatrixPosition(m);
      scene.onBeforeRender=(r,s,camera)=>{camera.position.set(p.x+14,p.y+8,p.z+17);camera.lookAt(p.x,p.y-1,p.z);camera.updateMatrixWorld();};
    });await page.waitForTimeout(400);await canvas.screenshot({path:path.join(out,'snow-rocks.jpg'),type:'jpeg',quality:82});
    await restore('runoff',.45,'desertBasin');
    assert.equal(await page.evaluate(()=>pilotReview.scene.getObjectByName('pilot-landscape-rocks').material.userData.snowShader.uniforms.pilotSurfaceSnow.value),0);
    ...
    assert.equal(await page.evaluate(()=>pilotReview.scene.getObjectByName('pilot-landscape-rocks').material.userData.snowShader.uniforms.pilotSurfaceSnow.value),0);
    await canvas.screenshot({path:path.join(out,'temperate-rocks.jpg'),type:'jpeg',quality:82});
