                ...
                ref: function(cvEl) {
                  ...
                  function drawDescent() {
                    ...
                    if (landed) {
                      ...
                      ctx.fillText('\uD83C\uDF15 "The Eagle has landed!"', W * 0.5, H * 0.2);
                      ctx.font = '12px system-ui';
                      ctx.fillStyle = '#e2e8f0';
                      ctx.fillText('Touchdown! V: ' + Math.abs(vVel).toFixed(1) + ' m/s \u2022 Fuel remaining: ' + fuel.toFixed(0) + '%', W * 0.5, H * 0.26);
                      ctx.fillStyle = '#94a3b8'; ctx.font = '10px system-ui';
                      ctx.fillText('Click "Begin EVA" to walk on the Moon!', W * 0.5, H * 0.32);
                    }
        ...
        phase >= 10 && h('div', { className: 'space-y-3' },
          h('div', { className: 'bg-gradient-to-br from-indigo-900 via-purple-900 to-slate-900 rounded-xl p-5 text-white text-center' },
            ...
            h('div', { className: 'bg-white/5 rounded-lg p-3 border border-white/10 text-left mb-3' },
              ...
              // Sample manifest
              (d.lunarSamples || []).length > 0 && h('div', { className: 'mt-1 space-y-1' },
                h('p', { className: 'text-[9px] text-slate-500 font-bold' }, 'SAMPLE MANIFEST:'),
                (d.lunarSamples || []).map(function(s, i) {
                  return h('p', { key: i, className: 'text-[9px] text-slate-400' }, s.icon + ' ' + s.name + ' \u2014 ' + s.fact);
                })
              )
            ),
            h('button', {
