...
const createGeminiAPI = deps => {
  ...
  const _classifyGeminiError = err => {
    ...
    if (msg.includes('Content Blocked') || msg.includes('finishReason: OTHER') || msg.includes('Refusal') || msg.includes('Generation Stopped') || msg.includes('Generation Blocked')) {
      return {
        kind: 'refusal',
        userMessage: 'Safety filter blocked the response.',
        model: null
      };
    }
    ...
    if (msg.includes('429') || lower.includes('resource_exhausted') || lower.includes('quota exceeded')) {
      ...
      const userMessage = perMinHint ? 'Gemini API per-minute rate limit hit — usually clears in 60 seconds.' : perDayHint ? 'Gemini API daily quota reached — resolves at midnight Pacific time.' : 'Gemini API rate or quota limit hit. May be a per-minute burst (clears in seconds) or a daily quota (resolves at midnight Pacific) — try again in a minute first.';
      return {
        kind: 'quota',
        userMessage,
        model: null
    ...
    if (msg.includes('401') || lower.includes('unauthenticated') || lower.includes('api key not valid') || lower.includes('api_key_invalid') || lower.includes('permission_denied') || msg.includes('403')) {
      ...
      if (lower.includes('quota') || lower.includes('rate')) {
        return {
          kind: 'quota',
          userMessage: 'Gemini API rate or quota limit hit — try again in a minute first; if it persists, you may have hit the daily quota.',
          model: null
        };
      }
      ...
      const _authMsg = _isCanvasEnv ? 'The AI service didn’t accept that request. This is almost always a brief rate-limit or hiccup — not a real key problem (this app manages the AI key for you). It usually clears on its own — wait a moment and try again.' : 'The Gemini API key looks invalid, expired, or missing. If it was working recently, heavy usage can cause a temporary 401 — wait a few minutes before regenerating it.';
      return {
        kind: 'auth',
        userMessage: _authMsg,
        model: null
      };
    }
    // Config: model not found / unsupported / 404 / INVALID_ARGUMENT.
    if (msg.includes('404') || lower.includes('model not found') || lower.includes('not found for api') || lower.includes('models/') && lower.includes('not found') || lower.includes('invalid_argument') || lower.includes('is not supported')) {
      const modelMatch = msg.match(/models\/([a-z0-9.\-]+)/i);
      return {
        kind: 'config',
        userMessage: 'Gemini model name is not recognized by the API (deploy-side configuration error).',
    ...
    if (msg.match(/HTTP 5\d\d/) || lower.includes('failed to fetch') || lower.includes('networkerror') || lower.includes('timed out') || lower.includes('etimedout') || lower.includes('unexpected end of input') || lower.includes('empty response body') || lower.includes('truncated/invalid json response body') || msg.includes('408')) {
      return {
        kind: 'transient',
        userMessage: 'Gemini API temporarily unavailable.',
        model: null
      };
    }
    return {
      kind: 'other',
      userMessage: msg || 'Unknown Gemini API error.',
      model: null
    };
  };
  ...
  const _showQuotaBanner = classification => {
    ...
    if (classification.kind === 'auth') {
      ...
      if (_authFailStreak < _AUTH_BANNER_THRESHOLD) {
        try {
          if (typeof warnLog === 'function') warnLog('[GeminiAPI] transient auth failure ' + _authFailStreak + '/' + _AUTH_BANNER_THRESHOLD + ' — likely a brief rate-limit; not alarming the user yet.');
        } catch (_) {}
        return;
      }
    }
    try {
      ...
      if (classification.kind === 'quota') {
        ...
        window.__alloGeminiQuotaHits.push({
          at: typeof Date !== 'undefined' ? Date.now() : 0,
          model: classification.model || '(unknown)',
          message: String(classification.userMessage || '').slice(0, 160)
        });
      ...
      if (!banner) {
        ...
        banner.style.cssText = ['position:fixed', 'top:0', 'left:0', 'right:0', 'z-index:2147483647', 'background:#7c1d1d', 'color:#ffffff', 'padding:12px 16px', 'font:600 14px/1.4 system-ui,-apple-system,Segoe UI,Roboto,sans-serif', 'box-shadow:0 2px 8px rgba(0,0,0,0.4)', 'display:flex', 'align-items:center', 'justify-content:space-between', 'gap:12px'].join(';');
        ...
        closeBtn.onclick = () => {
          try {
            if (window.sessionStorage) sessionStorage.setItem('__alloflowQuotaBannerDismissed', '1');
          } catch (_) {}
          banner.remove();
        };
      ...
      if (msgEl) {
        ...
        const prefix = classification.kind === 'auth' ? '🔑 Auth error (HTTP 401 — a key/sign-in problem, NOT your quota): ' : classification.kind === 'config' ? '⚙ Configuration error: ' : '🛑 Gemini quota limit (HTTP 429): ';
        ...
        const trailing = classification.kind === 'quota' ? ' AI-dependent steps (Vision OCR, rewrite, alt-text) will fail meanwhile. Deterministic fixes still run — try again in about a minute. If the message recurs immediately, you have likely hit the daily quota and will need to wait until midnight Pacific or upgrade the Gemini tier.' : classification.kind === 'auth' ? _isCanvasEnv ? ' The basic (non-AI) fixes still ran. The AI steps (reading images, rewriting, alt text) keep retrying — if it clears, you’ll see a green “responding again” note. If this banner keeps showing for a few minutes, the AI service is heavily rate-limited; wait and retry.' : ' AI steps (image reading, rewriting, alt text) will fail until the key is fixed; the basic fixes still ran. (A key that worked recently may just be hitting a temporary 401 from heavy usage — wait a few minutes before regenerating.)' : ' AI-dependent steps (Vision OCR, rewrite, alt-text) will fail until this is resolved. Deterministic fixes still run.';
        msgEl.textContent = prefix + classification.userMessage + trailing;
      }
      try {
        window.dispatchEvent(new CustomEvent('alloflow:quota-exhausted', {
          detail: window.__alloflowQuotaState
        }));
  ...
  const _noteApiSuccess = () => {
    ...
    try {
      ...
      try {
        window.dispatchEvent(new CustomEvent('alloflow:quota-recovered', {}));
      } catch (_) {}
      // Re-arm the dismissal so a LATER genuine error can show again.
      try {
        if (window.sessionStorage) sessionStorage.removeItem('__alloflowQuotaBannerDismissed');
      } catch (_) {}
      if (bannerEl) {
        ...
        setTimeout(() => {
          try {
            const b = document.getElementById('alloflow-quota-banner');
            if (b) b.remove();
          } catch (_) {}
        }, 6000);
      }
  ...
  const listAvailableModels = async () => {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        return {
          error: cls.userMessage + ' (HTTP ' + r.status + ')',
          models: [],
          reachable: true,
          httpStatus: r.status,
          classification: cls
  ...
  const _throwClassified = err => {
    ...
    if (cls.kind === 'quota' || cls.kind === 'auth' || cls.kind === 'config') {
      ...
      const out = new Error(cls.kind === 'quota' ? 'API_QUOTA_EXHAUSTED' : cls.kind === 'auth' ? 'API_AUTH_FAILED' : 'API_MODEL_NOT_FOUND');
      ...
      try {
        ...
        if (shouldFallback && GEMINI_MODELS.fallback && GEMINI_MODELS.fallback !== GEMINI_MODELS.default) {
          ...
          try {
            ...
            throw cls.kind === 'quota' || cls.kind === 'auth' || cls.kind === 'config' ? primaryErr : fbCls.kind === 'quota' || fbCls.kind === 'auth' || fbCls.kind === 'config' ? fbErr : fbErr;
          }
        } else {
          throw primaryErr;
        }
