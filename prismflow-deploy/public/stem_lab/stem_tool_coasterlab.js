...
scene.add(trackGroup, supportGroup, markerGroup, safetyGroup, handleGroup);
...
function predictSafetyFindings(){
  ...
  if(a.C && a.ans.vC < a.ans.vCmin) add('loop', 'bad', a.C.idx, 'Loop apex is too slow',
    'Lower or shrink the loop, or add more starting energy so the wheels stay loaded.');
...
function renderSafetyCoach(){
  ...
  summary.textContent = safetyFindings.length
    ...
    : 'Ideal preview looks comfortable';
...
for(const b of elementButtons) b.addEventListener('click', () => insertTrackElement(b.dataset.element));
