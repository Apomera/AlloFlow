...
function AllobotSearchSettings({ t }) {
  ...
  return <section aria-label={tx('chat_guide.search_settings', 'Allobot web sources')} className="border-t border-slate-200 pt-4 space-y-2 text-slate-800">
    ...
    <label className="flex items-start gap-2 text-sm">
      <input type="checkbox" className="mt-1" checked={enabled} onChange={event => { setEnabled(event.target.checked); save({ allobotWebSearch: event.target.checked }); }} />
    ...
    <p className="text-xs leading-relaxed">{tx('chat_guide.search_privacy_help', 'Serper is outside your district Google environment. External search accepts approved public topics and standard codes only; other queries are blocked. A personal key does not change this restriction. Your selected AI still receives conversation and lesson context: use a district-approved connection for student information. A Gemini API key alone does not establish district coverage. Local AI requires this lookup option to be enabled.')}</p>
  </section>;
}
