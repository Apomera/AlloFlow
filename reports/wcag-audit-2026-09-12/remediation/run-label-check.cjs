...
(async()=>{
 ...
 async function audit(name){
  ...
  results.states.push({name,...result});fs.writeFileSync(path.join(__dirname,'browser-label-check.json'),JSON.stringify(results,null,2));console.log(name+': '+result.violations.length+' axe rules; width '+result.scrollWidth+'/'+result.clientWidth);
 }
 try{
  for(const route of ['/app/']){
   ...
   await page.waitForFunction(()=>[...document.styleSheets].some(s=>{try{return [...s.cssRules].some(r=>r.selectorText?.includes('.fixed.inset-0')&&r.selectorText?.includes('.text-slate-600'))}catch{return false}}),null,{timeout:120000});
   await page.getByRole('button',{name:'College',exact:true}).hover();
   await page.waitForTimeout(2000);
   results.collegeStyle=await page.getByRole('button',{name:'College',exact:true}).evaluate(e=>({html:e.outerHTML,color:getComputedStyle(e).color,background:getComputedStyle(e).backgroundColor,rules:[...document.styleSheets].flatMap(s=>{try{return [...s.cssRules].filter(r=>r.selectorText&&e.matches(r.selectorText)&&r.style.color).map(r=>r.cssText)}catch{return []}})}));await audit('teacher-workspace-1280');
   ...
   try{await page.waitForFunction(()=>!!document.getElementById('tour-input-panel'),null,{timeout:180000});results.sourcePanelReady=true;}catch(e){results.sourcePanelReady=false;results.sourcePanelReadinessError=e.message;}
   await audit('teacher-loaded-1280');
   results.labelInName = await page.evaluate(async()=>axe.run(document,{runOnly:{type:'rule',values:['label-content-name-mismatch']},rules:{'label-content-name-mismatch':{enabled:true}}}));
   results.headerLabelEvidence=await page.locator('button').filter({hasText:'More information'}).evaluateAll(elements=>elements.map(e=>({html:e.outerHTML,visibleText:e.innerText,ariaLabel:e.getAttribute('aria-label'),labelledby:e.getAttribute('aria-labelledby'),inert:!!e.closest('[inert]'),hidden:e.getAttribute('aria-hidden')})));
   await page.evaluate(()=>{for(const style of document.querySelectorAll('style'))if(style.textContent==='* {line-height:1.5!important;letter-spacing:.12em!important;word-spacing:.16em!important} p {margin-bottom:2em!important}')style.remove();});
   await page.setViewportSize({width:320,height:800});await audit('teacher-loaded-320');
   await page.addStyleTag({content:'* {line-height:1.5!important;letter-spacing:.12em!important;word-spacing:.16em!important} p {margin-bottom:2em!important}'});await audit('teacher-loaded-320-spacing');
  }
 }catch(e){results.error=e.stack;console.error(e.stack);process.exitCode=1;}
 results.verification={sourceReady:results.sourcePanelReady===true,labelInNamePassed:results.labelInName?.violations.length===0,quickStartContrastPassed:results.states.filter(s=>s.name.startsWith('teacher-workspace')).length===4&&results.states.filter(s=>s.name.startsWith('teacher-workspace')).every(s=>!s.violations.some(v=>v.id==='color-contrast')),loadedWorkspacePassed:results.states.filter(s=>s.name.startsWith('teacher-loaded')).length===3&&results.states.filter(s=>s.name.startsWith('teacher-loaded')).every(s=>s.violations.length===0&&s.scrollWidth<=s.clientWidth),backendFocusContained:results.backendKeyboard?.length===45&&results.backendKeyboard.every(s=>s.inDialog),backendFocusRestored:results.backendAfterEscape?.dialogCount===0&&results.backendAfterEscape?.focus==='AI Backend Settings'};if(results.error||!Object.values(results.verification).every(Boolean))process.exitCode=1;results.blockedOrigins=[...blocked];results.pageErrors=errors;results.finishedAt=new Date().toISOString();fs.writeFileSync(path.join(__dirname,'browser-label-check.json'),JSON.stringify(results,null,2));await browser.close();await new Promise(r=>server.close(r));
})();
