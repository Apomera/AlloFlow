...
function LessonTeachingScriptView(props) {
  ...
  return <LessonTeachingScriptPanel key={String(props.generatedContent.id)} {...props} />;
}
function LessonTeachingScriptPanel(props) {
  ...
  const edit = () => {
    ...
    setDraftBase(JSON.stringify(sourceSteps)); setLocalError(''); setNotice('');
  };
  ...
  const saveEdits = async () => {
    ...
    try {
      const result = await onUpdateTeachingScript(plan.id, version.id, draft.map(step => ({ ...step, minutes: Number(step.minutes) })));
    ...
    <section className="rounded-xl border border-indigo-200 bg-white shadow-sm" aria-labelledby={id + '-title'} data-teaching-script-plan={planId}>
      <div className="p-4 sm:p-5">
        <h3 id={id + '-title'} className="text-lg font-black text-indigo-950">
          <button type="button" className="flex min-h-11 w-full items-center justify-between gap-3 text-left focus-visible:ring-2 focus-visible:ring-indigo-600 rounded-lg" aria-expanded={expanded} aria-controls={id + '-panel'} onClick={() => setExpanded(!expanded)}>
      ...
      {expanded && <div id={id + '-panel'} className="space-y-5 border-t border-indigo-100 p-4 sm:p-5">
        ...
        <form onSubmit={generate} className="space-y-4 no-print">
          ...
          <fieldset className="space-y-2 rounded-lg border border-slate-300 p-3" disabled={busy || saving}>
            ...
            {materials.length ? materials.map((material, index) => <label key={String(material.id)} className="flex min-h-11 cursor-pointer items-start gap-3 rounded-lg px-2 py-2 hover:bg-indigo-50">
              <input type="checkbox" className="mt-1 h-4 w-4" checked={materialIds.includes(String(material.id))} onChange={() => toggleMaterial(String(material.id))} />
              <span className="text-sm text-slate-800">{_ltsText(material.title || material.data?.title) || tr('untitled_resource', 'Lesson resource') + ' ' + (index + 1)} <span className="text-xs text-slate-600">({_ltsText(material.type).replace(/-/g, ' ')})</span></span>
          ...
          <div>
            ...
            {!canResearch && <p className="text-sm text-slate-700">{tr('research_unavailable', 'Live research is unavailable here. Turn off research to generate from the lesson, or try again when research is available.')}</p>}
          </div>
        ...
        {version && <div className="space-y-5 border-t border-slate-200 pt-5">
          <div className="flex flex-wrap items-end gap-3 no-print">
            ...
            {!draft && typeof onUpdateTeachingScript === 'function' && <button type="button" className={buttonClass} disabled={busy || saving} onClick={edit}>{tr('edit', 'Edit script')}</button>}
          ...
          <ol className="space-y-4">
            {steps.map((step, index) => <li key={step.id || index} className="space-y-3 rounded-xl border border-slate-300 p-4" data-teaching-step={step.id || index}>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {stepFields.map(([key, label]) => <div key={key} className={key === 'teacherSays' ? 'sm:col-span-2' : ''}>
                  {draft ? <label className="block text-sm font-bold text-slate-900">{label} · {index + 1}<textarea className={fieldClass + ' mt-1'} rows={3} value={step[key] || ''} maxLength={5000} disabled={saving} onChange={event => updateStep(index, key, event.target.value)} /></label> : <><h6 className="text-xs font-black uppercase tracking-wide text-slate-600">{label}</h6><p className="mt-1 whitespace-pre-wrap text-sm text-slate-900">{_ltsText(step[key]) || '—'}</p></>}
                </div>)}
              </div>
          ...
          {draft && <div className="space-y-3 rounded-lg border border-indigo-300 bg-indigo-50 p-3 no-print">
            ...
            <div className="flex flex-wrap gap-2">
              <button type="button" className={buttonClass} disabled={saving || invalidTiming || invalidDraftText || staleDraft} onClick={saveEdits}>{saving ? tr('saving', 'Adding edits…') : tr('save', 'Save edits')}</button>
              <button type="button" className={buttonClass} disabled={saving} onClick={() => { setDraft(null); setDraftBase(''); setLocalError(''); }}>{tr('discard', 'Discard edits')}</button>
            </div>
          </div>}
