      ...
      return h('div', { className: 'space-y-4 animate-in fade-in duration-200' },
              ...
              h('div', { className: 'flex-1 space-y-2' },
                h('div', { className: 'grid grid-cols-3 gap-2' },
                  ...
                  h('div', null,
                    ...
                    ctxGradeLevel && !playerGradeLevel && h('div', { className: 'text-[8px] text-violet-500 mt-0.5' }, '📋 Set by teacher: ' + ctxGradeLevel)
                  )
                ),
                h('div', null,
        ...
        selectedWorld && room && h('div', { className: 'space-y-4' },
          ...
          h('div', { className: 'bg-white rounded-xl border border-slate-200 p-3' },
            h('div', { className: 'text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2' }, '🗺️ Connected Locations'),
          ...
          h('div', { className: 'bg-gradient-to-r from-violet-50 to-indigo-50 rounded-2xl border-2 border-violet-200 p-5' },
            h('div', { className: 'flex items-center justify-between mb-2' },
              ...
              h('div', { className: 'flex items-center gap-2' },
                h('span', { className: 'text-[10px] font-bold px-2 py-1 rounded-full', style: { background: getQualityTier(writingPower).color + '22', color: getQualityTier(writingPower).color } },
          ...
          actionResult && (actionResult.qualityScore >= 0 || actionResult.finalScore >= 0) && h('div', { className: 'space-y-3' },
            ...
            h('div', { className: 'bg-white rounded-2xl border-2 shadow-lg overflow-hidden', style: { borderColor: getQualityTier(actionResult.finalScore || actionResult.qualityScore).color + '66' } },
              ...
              h('div', { className: 'px-4 pb-3 flex flex-wrap gap-2' },
                actionResult.xpEarned && h('span', { className: 'text-[10px] font-bold bg-amber-100 text-amber-700 px-2 py-1 rounded-full' }, '+' + actionResult.xpEarned + ' XP'),
                actionResult.damageDealt && h('span', { className: 'text-[10px] font-bold bg-red-100 text-red-700 px-2 py-1 rounded-full' }, '💥 ' + actionResult.damageDealt + ' damage'),
                actionResult.powerChange > 0 && h('span', { className: 'text-[10px] font-bold bg-violet-100 text-violet-700 px-2 py-1 rounded-full' }, '+' + actionResult.powerChange + ' Power'),
                actionResult.discoveredItem && h('span', { className: 'text-[10px] font-bold bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full' }, '🎁 Found: ' + actionResult.discoveredItem),
                (actionResult.vocabHighlights || []).map(function(v, vi) {
                  return h('span', { key: vi, className: 'text-[10px] font-bold bg-blue-100 text-blue-700 px-2 py-1 rounded-full' }, '📖 ' + v);
                })
              )
            ),
          ...
          h('details', { className: 'bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl border-2 border-amber-200' },
            ...
            h('div', { className: 'px-4 pb-4 space-y-3' },
              ...
              h('div', { className: 'text-[10px] font-bold text-amber-500 uppercase tracking-widest mb-1' }, 'Or create a custom character:'),
              h('textarea', {
