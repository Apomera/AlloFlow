...
// Run: node dev-tools/watercycle_pilot_rain_contact_qa.cjs
const fs = require('fs');
const path = require('path');
const assert = require('node:assert/strict');
const { chromium } = require('playwright');
const ROOT = process.cwd();
const out = path.join(ROOT, 'scratch', 'water-rain-contact-review');
...
(async () => {
  ...
  try {
    ...
    const glints=()=>page.evaluate(()=>{const g=pilotReview.scene.getObjectByName('pilot-water-surface-glints');return {positions:Array.from(g.geometry.attributes.position.array),opacity:g.material.opacity};});
    const glintContact=()=>page.evaluate(()=>{
      const scene=pilotReview.scene,g=scene.getObjectByName('pilot-water-surface-glints'),ocean=scene.getObjectByName('pilot-ocean-surface');scene.updateMatrixWorld(true);
      const a=g.geometry.attributes.position,ray=new THREE.Raycaster(),p=new THREE.Vector3(),down=new THREE.Vector3(0,-1,0);let max=0;
      for(let i=0;i<a.count;i++){p.fromBufferAttribute(a,i).applyMatrix4(g.matrixWorld);ray.set(new THREE.Vector3(p.x,500,p.z),down);const hit=ray.intersectObject(ocean)[0];if(!hit)throw Error('Missing glint surface');max=Math.max(max,Math.abs(p.y-hit.point.y-.08));}
      return {max,count:a.count};
    });
    const glintStart=await glints();assert.deepEqual((await glintContact()).count,124);assert((await glintContact()).max<.002);
    await page.getByRole('button',{name:'Resume the simulation',exact:true}).click();
    await page.waitForFunction(()=>Number(document.querySelector('#wcPilotCanvas').dataset.parcelElapsed)>.25);
    await page.getByRole('button',{name:'Pause the simulation',exact:true}).click();
    const glintMoving=await glints(),rainMoving=await rain();assert.notDeepEqual(glintMoving,glintStart);assert((await glintContact()).max<.002);
    await page.emulateMedia({reducedMotion:'reduce'});await page.waitForTimeout(350);
    assert.deepEqual(await glints(),glintMoving,'Enabling reduced motion preserves glint positions and opacity');
    assert.deepEqual(await rain(),rainMoving,'Enabling reduced motion preserves the current rain streaks');
    const tails=rainMoving.points.filter((_,i)=>i%12===1);
    await page.getByRole('button',{name:'Resume the simulation',exact:true}).click();
    const reducedElapsed=await canvas.getAttribute('data-parcel-elapsed');await page.waitForFunction(v=>document.querySelector('#wcPilotCanvas').dataset.parcelElapsed!==v,reducedElapsed);
    await page.getByRole('button',{name:'Pause the simulation',exact:true}).click();
    assert.deepEqual(await glints(),glintMoving,'Reduced motion holds glints while the kernel runs');
    assert.deepEqual((await rain()).points.filter((_,i)=>i%12===1),tails,'Reduced motion holds the local rain phase as the parcel moves');
    assert((await glintContact()).max<.002);
    await canvas.screenshot({path:path.join(out,'rain-wave-contact.jpg'),type:'jpeg',quality:85});
    ...
    console.log('PASS: ray-verified rain/wave contact and surface glints, moving and reduced-motion surface continuity, wind-slanted fading rain, sea clipping, paused rain, both camera views, actual ocean landing, held/reduced/expiring ripples, snow and land exclusions, mobile accessibility, resource disposal, no WebGL errors.');
