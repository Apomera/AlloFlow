...
Available properties: `text`, `visible`, `exposed`, `name`, `role`, `disabled`, `value`, `formData`, `checked`, `selected`, `href`, `targetText`, `language`, and `direction`. `candidateSelector` may identify the corresponding candidate element when its ID changes. `targetText` supports HTML and SVG same-document links to element IDs, including SVG `xlink:href` and encoded fragments. It uses the same destination resolution and SVG attribute precedence as `href`; external destinations, missing targets, and undecodable fragments remain unavailable. Pair it with `href` when exact destination preservation is required.
...
For forms whose submission payload must survive a repair, add a checkpoint such as:

```json
{ "id": "response-payload", "sourceSelector": "#response-form", "properties": ["formData"] }
```

`formData` inspects a native HTML form with `new FormData(form)` in each requested rendering profile. Native DOM access preserves evidence even when form controls have names or IDs such as `tagName`, `textContent`, or `attributes`, which otherwise shadow browser properties; these names do not need to be changed. It retains the ordered name/value entries, including duplicate names, external form-associated controls, direction fields, and CSS-dependent newlines from hard-wrapped textareas. Font or width changes require review only if the resulting payload changes; unchanged payloads remain valid. No form submission or network navigation occurs. This checkpoint is opt-in: `value` continues to compare a control's DOM value and cannot establish preservation of rendered hard wrapping.

No submitter is chosen, so submit-button values and submitter-specific overrides are outside this payload checkpoint. It does not test validation, navigation destinations, or server-side processing. Non-form checkpoints and payloads containing files report unavailable because this static inspection does not establish uploaded bytes. Text entries are compared exactly, including whitespace and newline positions, and the existing bounded-observation limit applies. Use the standard `sourceFidelity.checkpoints` export contract to require this check before an automated export report can pass.

`href` resolves HTML and SVG anchors, including SVG `xlink:href` fallback and authored base URLs. Missing or unresolvable destinations remain unavailable. `language` follows effective inherited language, respects namespaced SVG `xml:lang` and explicit empty resets, and treats language-tag capitalization as equivalent. When ancestors declare no language, it observes Chromium’s `content-language` metadata fallback, including applicable declaration precedence and literal empty, whitespace, or comma-list values.
