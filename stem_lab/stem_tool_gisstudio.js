      ...
      function Studio() {
        ...
        function tableTwin() {
          ...
          return h('section', { 'aria-labelledby': 'gis-table-heading', style: Object.assign({}, panel, { overflow: 'hidden' }) },
            h('h2', { id: 'gis-table-heading', style: { margin: '0 0 4px', fontSize: 15, color: '#f0fdfa' } }, __alloT('stem.gisstudio.ui_accessible_data_table_twin', 'Accessible data-table twin')),
            h('p', { style: { margin: '0 0 10px', color: '#a7c7d8', fontSize: 11, lineHeight: 1.5 } }, gisFillTemplate(__alloT('stem.gisstudio.ui_table_twin_intro', '{count} records carry the same values as the map without relying on colour or position.'), { count: records.length })),
            h('div', { role: 'search', 'aria-label': __alloT('stem.gisstudio.a11y_data_explorer_controls', 'Data Explorer controls'), style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(165px,1fr))', gap: 9, padding: 10, marginBottom: 9, borderRadius: 9, border: '1px solid #2d5868', background: '#081d29' } },
              h('label', { style: { display: 'grid', gap: 4, color: '#dbeafe', fontSize: 11, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_search_locations', 'Search locations'),
                h('input', { type: 'search', value: tableQuery, onChange: function (event) { setTableQuery(event.target.value); }, placeholder: __alloT('stem.gisstudio.ui_name_contains', 'Name contains...'), 'aria-controls': 'gis-data-explorer-table', 'aria-describedby': 'gis-data-explorer-note', style: control })),
              h('label', { style: { display: 'grid', gap: 4, color: '#dbeafe', fontSize: 11, fontWeight: 700 } }, gisFillTemplate(__alloT('stem.gisstudio.ui_minimum_of', 'Minimum {attribute}'), { attribute: metricLabel }),
              ...
              h('label', { style: { display: 'grid', gap: 4, color: '#dbeafe', fontSize: 11, fontWeight: 700 } }, gisFillTemplate(__alloT('stem.gisstudio.ui_maximum_of', 'Maximum {attribute}'), { attribute: metricLabel }),
              ...
              h('label', { style: { display: 'grid', gap: 4, color: '#dbeafe', fontSize: 11, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_sort_table_rows', 'Sort table rows'),
                h('select', { value: tableSort, onChange: function (event) { setTableSort(event.target.value); }, 'aria-controls': 'gis-data-explorer-table', style: control },
                  h('option', { value: 'value-desc' }, __alloT('stem.gisstudio.ui_value_high_to_low', 'Value: high to low')),
                  h('option', { value: 'value-asc' }, __alloT('stem.gisstudio.ui_value_low_to_high', 'Value: low to high')),
                  h('option', { value: 'name-asc' }, __alloT('stem.gisstudio.ui_location_a_to_z', 'Location: A to Z')),
                  h('option', { value: 'name-desc' }, __alloT('stem.gisstudio.ui_location_z_to_a', 'Location: Z to A')))),
              h('label', { style: { display: 'flex', alignItems: 'center', gap: 7, alignSelf: 'end', minHeight: 36, color: '#dbeafe', fontSize: 11, fontWeight: 700 } },
                ...
                __alloT('stem.gisstudio.ui_only_spatial_analysis_selection', 'Only spatial-analysis selection')),
              h('button', { type: 'button', onClick: resetTableExplorer, disabled: !tableExplorerActive, style: Object.assign({}, control, { alignSelf: 'end', cursor: tableExplorerActive ? 'pointer' : 'not-allowed', opacity: tableExplorerActive ? 1 : 0.55 }) }, __alloT('stem.gisstudio.ui_reset_table_view', 'Reset table view'))),
            ...
            h('p', { id: 'gis-data-explorer-note', style: { margin: '0 0 10px', color: '#9fb6c5', fontSize: 10, lineHeight: 1.45 } }, __alloT('stem.gisstudio.ui_these_controls_change_only_this_table_view_the', 'These controls change only this table view. The map, analysis, project, and exports retain the complete mapped dataset.')),
            exploredRecords.length > 0
              ? h('div', { role: 'region', 'aria-label': __alloT('stem.gisstudio.a11y_scrollable_gis_data_table', 'Scrollable GIS data table'), tabIndex: 0, style: { overflowX: 'auto', outlineOffset: 3 } },
                  h('table', { id: 'gis-data-explorer-table', style: { width: '100%', borderCollapse: 'collapse', fontSize: 12 } },
                    h('caption', { style: { position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0,0,0,0)' } }, gisFillTemplate(__alloT('stem.gisstudio.ui_table_caption', '{attribute} by location. {summary}'), { attribute: metricLabel, summary: resultMessage })),
        ...
        function schematicMap(options) {
          ...
          if (!proj) {
            ...
            }, __alloT('stem.gisstudio.ui_no_mappable_coordinates_yet_import_points_or_a', 'No mappable coordinates yet. Import points or a GeoJSON layer, or use the data table below.'));
          }
          ...
          kids.push(h('text', { key: 'schem', x: 8, y: 16, fill: '#7dd3fc', fontSize: 10, fontWeight: 700 }, __alloT('stem.gisstudio.ui_schematic_not_a_projected_navigation_map', 'Schematic — not a projected navigation map')));
        ...
        function analysisControls() {
          return h('section', { 'aria-labelledby': 'gis-analysis-controls-heading', style: { marginTop: 15, paddingTop: 14, borderTop: '1px solid #28516a' } },
            h('h3', { id: 'gis-analysis-controls-heading', style: { margin: '0 0 8px', color: '#fde68a', fontSize: 13 } }, __alloT('stem.gisstudio.ui_spatial_analysis_workbench', 'Spatial analysis workbench')),
            h('label', { style: { display: 'grid', gap: 5, fontSize: 12, marginBottom: 9 } },
              h('span', { style: { fontWeight: 700 } }, __alloT('stem.gisstudio.ui_map_click_tool', 'Map-click tool')),
              h('select', { value: analysisMode, onChange: function (event) {
                setAnalysisMode(event.target.value); clearAnalysis();
              }, style: control },
                h('option', { value: 'distance' }, __alloT('stem.gisstudio.ui_measure_path', 'Measure path')),
                h('option', { value: 'buffer' }, __alloT('stem.gisstudio.ui_radius_buffer', 'Radius buffer')),
                h('option', { value: 'nearest' }, __alloT('stem.gisstudio.ui_find_nearest_point', 'Find nearest point')))),
            h('label', { style: { display: 'grid', gap: 5, fontSize: 12, marginBottom: 9 } },
              h('span', { style: { fontWeight: 700 } }, __alloT('stem.gisstudio.ui_units', 'Units')),
              h('select', { value: analysisUnit, onChange: function (event) { setAnalysisUnit(event.target.value); persist('gisAnalysisUnit', event.target.value); }, style: control },
                h('option', { value: 'metric' }, __alloT('stem.gisstudio.ui_metric_km', 'Metric (km)')),
                h('option', { value: 'imperial' }, __alloT('stem.gisstudio.ui_u_s_customary_mi', 'U.S. customary (mi)')))),
            analysisMode === 'buffer' && h('label', { style: { display: 'grid', gap: 5, fontSize: 12, marginBottom: 9 } },
              h('span', { style: { fontWeight: 700 } }, __alloT('stem.gisstudio.ui_buffer_radius', 'Buffer radius')),
            ...
            h('button', { type: 'button', onClick: clearAnalysis, style: Object.assign({}, control, { width: '100%', cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_clear_map_analysis', 'Clear map analysis')),
            h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8, marginBottom: 2 } },
              h('button', { type: 'button', onClick: undoAnalysis, disabled: !analysisHistory.length, 'aria-label': __alloT('stem.gisstudio.a11y_undo_last_spatial_analysis_change', 'Undo last spatial analysis change'), style: Object.assign({}, control, { cursor: analysisHistory.length ? 'pointer' : 'not-allowed', opacity: analysisHistory.length ? 1 : 0.55 }) }, __alloT('stem.gisstudio.ui_undo_analysis', 'Undo analysis')),
              h('button', { type: 'button', onClick: redoAnalysis, disabled: !analysisFuture.length, 'aria-label': __alloT('stem.gisstudio.a11y_redo_spatial_analysis_change', 'Redo spatial analysis change'), style: Object.assign({}, control, { cursor: analysisFuture.length ? 'pointer' : 'not-allowed', opacity: analysisFuture.length ? 1 : 0.55 }) }, __alloT('stem.gisstudio.ui_redo_analysis', 'Redo analysis'))),
            ...
            h('p', { style: { margin: '3px 0 0', color: '#8aa9bb', fontSize: 10 } }, __alloT('stem.gisstudio.ui_keyboard_ctrl_cmd_z_to_undo_ctrl_cmd_shift_z_o', 'Keyboard: Ctrl/Cmd+Z to undo; Ctrl/Cmd+Shift+Z or Ctrl/Cmd+Y to redo.')),
            geoFeatures.length > 0 && h('div', { style: { marginTop: 13, paddingTop: 12, borderTop: '1px solid #28516a' } },
              h('label', { style: { display: 'grid', gap: 5, fontSize: 12, marginBottom: 8 } },
                h('span', { style: { fontWeight: 700 } }, __alloT('stem.gisstudio.ui_geojson_feature_to_measure', 'GeoJSON feature to measure')),
              ...
              h('button', { type: 'button', onClick: analyzeBoundary, disabled: selectedGeometryType !== 'Polygon' && selectedGeometryType !== 'MultiPolygon', style: Object.assign({}, primary, { width: '100%', opacity: selectedGeometryType === 'Polygon' || selectedGeometryType === 'MultiPolygon' ? 1 : 0.55 }) }, __alloT('stem.gisstudio.ui_select_points_inside_boundary', 'Select points inside boundary'))));
        }
        ...
        function analysisResults() {
          ...
          return h('section', { 'aria-labelledby': 'gis-analysis-results-heading', style: panel },
            h('div', { style: { display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', flexWrap: 'wrap' } },
              h('div', null,
                h('p', { style: { margin: 0, color: '#fde68a', fontSize: 10, fontWeight: 900, letterSpacing: '.08em' } }, __alloT('stem.gisstudio.ui_measure_buffer_select', 'MEASURE • BUFFER • SELECT')),
                h('h2', { id: 'gis-analysis-results-heading', style: { margin: '3px 0 0', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_spatial_analysis_results', 'Spatial analysis results'))),
              h('button', { type: 'button', onClick: sonifySelection, disabled: !selectedRecords.length, style: Object.assign({}, primary, { background: '#083344', border: '1px solid #22d3ee', opacity: selectedRecords.length ? 1 : 0.55 }) }, __alloT('stem.gisstudio.ui_sonify_selection', '♫ Sonify selection'))),
              h('button', { type: 'button', onClick: copyAnalysisSummary, disabled: !hasAnalysisState(), 'aria-label': __alloT('stem.gisstudio.a11y_copy_the_current_spatial_analysis_summary', 'Copy the current spatial analysis summary'), style: Object.assign({}, control, { cursor: hasAnalysisState() ? 'pointer' : 'not-allowed', opacity: hasAnalysisState() ? 1 : 0.55 }) }, __alloT('stem.gisstudio.ui_copy_analysis_summary', 'Copy analysis summary')),
            ...
            selectedGeoFeature && h('div', { style: { display: 'flex', gap: 9, flexWrap: 'wrap', padding: 10, borderRadius: 9, background: '#071827', color: '#dbeafe', fontSize: 11 } },
              (selectedGeometryType === 'Polygon' || selectedGeometryType === 'MultiPolygon') && h('strong', { style: { color: '#86efac' } }, gisFillTemplate(__alloT('stem.gisstudio.ui_area_is', 'Area: {value}'), { value: formatArea(measuredFeature.areaSquareKm) })),
              (selectedGeometryType === 'Polygon' || selectedGeometryType === 'MultiPolygon') && h('span', null, gisFillTemplate(__alloT('stem.gisstudio.ui_perimeter_is', 'Perimeter: {value}'), { value: formatDistance(measuredFeature.perimeterKm) })),
        ...
        function mapView() {
          return h('div', { style: { display: 'grid', gap: 14 } },
            h('details', { open: true, style: Object.assign({}, panel, { padding: 14 }) },
              h('summary', { style: { cursor: 'pointer', color: '#f0fdfa', fontSize: 15, fontWeight: 900 } }, __alloT('stem.gisstudio.ui_start_a_first_investigation', 'Start a first investigation')),
              h('p', { style: { margin: '9px 0', color: '#b7d2df', fontSize: 12, lineHeight: 1.5 } }, __alloT('stem.gisstudio.ui_use_this_short_path_to_move_from_a_map_pattern', 'Use this short path to move from a map pattern to a defensible evidence handoff.')),
              h('ol', { style: { margin: '0 0 11px', paddingLeft: 22, color: '#dbeafe', fontSize: 11, lineHeight: 1.65 } },
                h('li', null, __alloT('stem.gisstudio.ui_choose_a_sample_or_import_your_own_coordinate', 'Choose a sample or import your own coordinate data.')),
                h('li', null, __alloT('stem.gisstudio.ui_describe_a_visible_pattern_and_check_the_table', 'Describe a visible pattern and check the table twin.')),
                h('li', null, __alloT('stem.gisstudio.ui_compare_layers_name_a_limitation_and_save_the', 'Compare layers, name a limitation, and save the evidence packet.'))),
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap' } },
                h('button', { type: 'button', onClick: function () { go('import'); }, style: Object.assign({}, control, { cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_import_data', 'Import data')),
                h('button', { type: 'button', onClick: function () { go('compare'); }, style: Object.assign({}, control, { cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_compare_layers', 'Compare layers')),
                h('button', { type: 'button', onClick: function () { go('packet'); }, style: Object.assign({}, primary, { cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_build_evidence_packet', 'Build evidence packet')))),
            h('section', { 'aria-labelledby': 'gis-map-heading', style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 12 } },
              h('div', { style: panel },
                h('h2', { id: 'gis-map-heading', style: { margin: '0 0 5px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_layer_workspace', 'Layer workspace')),
                h('p', { style: { margin: '0 0 13px', color: '#a7c7d8', fontSize: 11, lineHeight: 1.5 } }, __alloT('stem.gisstudio.ui_a_gis_combines_geometry_attributes_and_layers', 'A GIS combines geometry, attributes, and layers.')),
                h('fieldset', { style: { border: 0, padding: 0, margin: '0 0 13px' } },
                  h('legend', { style: { color: '#67e8f9', fontWeight: 800, fontSize: 11, marginBottom: 7 } }, __alloT('stem.gisstudio.ui_data_source', 'DATA SOURCE')),
                  h('label', { style: { display: 'block', fontSize: 12, marginBottom: 6 } }, h('input', { type: 'radio', name: 'gis-source', checked: !imported, onChange: function () { resetDatasetDerivedState(); setSource('sample'); } }), __alloT('stem.gisstudio.ui_sample_region_pack', ' Sample / region pack')),
                  h('label', { style: { display: 'block', fontSize: 12, color: importedRows.length ? '#dbeafe' : '#7d97ab' } }, h('input', { type: 'radio', name: 'gis-source', checked: imported, disabled: !importedRows.length, onChange: function () { resetDatasetDerivedState(); setSource('import'); } }), __alloT('stem.gisstudio.ui_my_csv', ' My CSV'))),
                !imported && h('div', { style: { display: 'grid', gap: 5, fontSize: 12, marginBottom: 13 } },
                  h('label', { style: { display: 'grid', gap: 5 } },
                    h('span', { style: { fontWeight: 700 } }, __alloT('stem.gisstudio.ui_sample_region_pack_2', 'Sample region pack')),
                ...
                h('details', {
                  ...
                  h('summary', { style: { cursor: 'pointer', color: '#67e8f9', fontWeight: 800, fontSize: 11 } }, __alloT('stem.gisstudio.ui_geographic_coverage_lens', 'Geographic coverage lens')),
                  ...
                  h('p', { style: { margin: '5px 0', color: '#b7d2df', fontSize: 10, lineHeight: 1.45 } },
                    h('strong', { style: { color: '#f0fdfa' } }, __alloT('stem.gisstudio.ui_hemispheres', 'Hemispheres: ')), geographicCoverage.hemispheres.length ? geographicCoverage.hemispheres.join(', ') : 'Not available'),
                  geographicCoverage.represented.length > 0 && h('p', { style: { margin: '5px 0', color: '#b7d2df', fontSize: 10, lineHeight: 1.45 } },
                    h('strong', { style: { color: '#86efac' } }, __alloT('stem.gisstudio.ui_represented_areas', 'Represented areas: ')), geographicCoverage.represented.join('; ')),
                  geographicCoverage.gaps.length > 0 && h('p', { style: { margin: '5px 0', color: '#fde68a', fontSize: 10, lineHeight: 1.45 } },
                    h('strong', null, __alloT('stem.gisstudio.ui_known_gaps', 'Known gaps: ')), geographicCoverage.gaps.join('; ')),
                ...
                geoKeys.length > 0 && h('label', { style: { display: 'grid', gap: 5, fontSize: 12, marginBottom: 10 } },
                  h('span', { style: { fontWeight: 700 } }, __alloT('stem.gisstudio.ui_choropleth_attribute', 'Choropleth attribute')),
                ...
                geoFeatures.length > 0 && h('label', { style: { display: 'grid', gap: 5, fontSize: 12, marginBottom: 10 } },
                  h('span', { style: { fontWeight: 700 } }, __alloT('stem.gisstudio.ui_classification_method', 'Classification method')),
                  h('select', { value: classification, onChange: function (event) { setClassification(event.target.value); persist('gisClassification', event.target.value); }, style: control },
                    h('option', { value: 'quantile' }, __alloT('stem.gisstudio.ui_quantiles', 'Quantiles')),
                    h('option', { value: 'equal' }, __alloT('stem.gisstudio.ui_equal_intervals', 'Equal intervals')),
                    h('option', { value: 'jenks' }, __alloT('stem.gisstudio.ui_natural_breaks_jenks', 'Natural breaks (Jenks)')),
                    h('option', { value: 'custom' }, __alloT('stem.gisstudio.ui_custom_thresholds', 'Custom thresholds')))),
                geoFeatures.length > 0 && classification !== 'custom' && h('label', { style: { display: 'grid', gap: 5, fontSize: 12, marginBottom: 10 } },
                  h('span', { style: { fontWeight: 700 } }, gisFillTemplate(__alloT('stem.gisstudio.ui_classes_count', 'Classes: {count}'), { count: classCount })),
                  h('input', { type: 'range', min: 3, max: 7, value: classCount, onChange: function (event) { setClassCount(Number(event.target.value)); } })),
                geoFeatures.length > 0 && classification === 'custom' && h('label', { style: { display: 'grid', gap: 5, fontSize: 12, marginBottom: 10 } },
                  h('span', { style: { fontWeight: 700 } }, __alloT('stem.gisstudio.ui_thresholds_comma_separated', 'Thresholds, comma separated')),
                ...
                h('fieldset', { style: { border: 0, padding: 0, display: 'grid', gap: 8, margin: '0 0 13px' } },
                  h('legend', { style: { color: '#67e8f9', fontWeight: 800, fontSize: 11, marginBottom: 7 } }, __alloT('stem.gisstudio.ui_visible_layers', 'Visible layers')),
                  toggle('points', __alloT('stem.gisstudio.ui_thematic_points', 'Thematic points')), geoFeatures.length > 0 && toggle('polygons', __alloT('stem.gisstudio.ui_geojson_choropleth', 'GeoJSON choropleth')), !imported && activeRegionPack.id === 'maine' && toggle('coast', __alloT('stem.gisstudio.ui_coastal_guide', 'Coastal guide')), toggle('grid', __alloT('stem.gisstudio.ui_coordinate_grid', 'Coordinate grid'))),
                h('button', { type: 'button', onClick: sonify, style: Object.assign({}, primary, { width: '100%', background: '#083344', border: '1px solid #22d3ee' }) }, __alloT('stem.gisstudio.ui_sonify_values', '♫ Sonify values')),
                h('p', { style: { color: '#9fb6c5', fontSize: 10, lineHeight: 1.4 } }, __alloT('stem.gisstudio.ui_low_values_use_lower_pitches_the_table_is_the', 'Low values use lower pitches. The table is the equivalent non-audio view.')),
                analysisControls()),
              h('div', null,
                offlineMap
                  ? schematicMap()
                ...
                leafletBlocked && basemap !== 'none' && h('aside', { role: 'status', style: { marginTop: 8, padding: 10, borderLeft: '4px solid #38bdf8', borderRadius: 8, background: '#102c3b', color: '#bae6fd', fontSize: 11, lineHeight: 1.45 } },
                  h('strong', null, __alloT('stem.gisstudio.ui_online_basemap_unavailable', 'Online basemap unavailable. ')), __alloT('stem.gisstudio.ui_the_offline_schematic_is_active', 'The offline schematic is active. '),
                  h('button', { type: 'button', onClick: retryLeaflet, style: Object.assign({}, control, { margin: '7px 6px 0 0', cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_try_online_basemap_again', 'Try online basemap again')),
                  h('button', { type: 'button', onClick: function () { setBasemap('none'); persist('gisBasemap', 'none'); }, style: Object.assign({}, control, { marginTop: 7, cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_keep_offline', 'Keep offline'))),
                (imported || geoFeatures.length > 0) && basemap !== 'none' && h('p', { role: 'status', style: { margin: '7px 2px 0', color: '#fde68a', fontSize: 10, lineHeight: 1.45 } }, __alloT('stem.gisstudio.ui_online_basemap_privacy_the_tile_service_can_in', 'Online basemap privacy: the tile service can infer the area being viewed. Choose "No basemap - offline schematic" before working with sensitive classroom locations.')),
                (imported || activePackIsCustom || geoFeatures.length > 0) && (privacyAssessment.highPrecision > 0 || privacyAssessment.identifierWarnings > 0) && h('aside', { role: 'alert', style: { marginTop: 8, padding: 10, borderLeft: '4px solid #f59e0b', borderRadius: 8, background: '#2b2617', color: '#fde68a', fontSize: 11, lineHeight: 1.45 } },
                  h('strong', null, __alloT('stem.gisstudio.ui_privacy_check_before_sharing', 'Privacy check before sharing. ')), gisFillTemplate(__alloT('stem.gisstudio.ui_privacy_precise_summary', '{precise} of the mapped features or point rows use highly precise coordinates, and {named} have identifier-like labels. Review the Project privacy controls and round or aggregate sensitive locations.'), { precise: privacyAssessment.highPrecision, named: privacyAssessment.identifierWarnings })),
                imported && (importDiagnostics.invalidRows > 0 || importDiagnostics.truncatedRows > 0) && h('p', { role: 'status', style: { margin: '7px 2px 0', padding: 9, borderLeft: '4px solid #f59e0b', borderRadius: 6, background: '#2b2617', color: '#fde68a', fontSize: 11 } }, gisFillTemplate(__alloT('stem.gisstudio.ui_import_review_skipped', 'Import review: {count} rows were skipped because coordinates or values were invalid.'), { count: importDiagnostics.invalidRows }) + (importDiagnostics.truncatedRows > 0 ? ' ' + importDiagnostics.truncatedRows + ' additional valid row' + (importDiagnostics.truncatedRows === 1 ? '' : 's') + ' were not loaded; the 250-row limit applies.' : '')),
                geoValues.length > 0 ? h('div', { role: 'list', 'aria-label': classification + ' choropleth legend for ' + geoMetric, style: { display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap', color: '#b7d2df', fontSize: 10 } },
                  legendBounds.slice(0, -1).map(function (lower, index) {
                    ...
                    return h('span', { key: index, role: 'listitem', style: { display: 'inline-flex', alignItems: 'center', gap: 4 } },
                      ...
                      label);
                  })) : h('div', { role: 'img', 'aria-label': __alloT('stem.gisstudio.a11y_point_values_run_from_low_teal_to_high_rose', 'Point values run from low teal to high rose'), style: { color: '#b7d2df', fontSize: 10 } }, __alloT('stem.gisstudio.ui_point_legend_low_teal_high_rose', 'Point legend: low teal → high rose')))),
            analysisResults(),
            basemap === 'satellite' && h('section', { 'aria-labelledby': 'gis-imagery-heading', style: panel },
              h('h2', { id: 'gis-imagery-heading', style: { margin: '0 0 5px', color: '#f0fdfa', fontSize: 15 } }, __alloT('stem.gisstudio.ui_satellite_imagery_reading_routine', 'Satellite imagery reading routine')),
              h('p', { style: { margin: '0 0 9px', color: '#b7d2df', fontSize: 12, lineHeight: 1.5 } }, __alloT('stem.gisstudio.ui_observe_before_interpreting_look_for_color_tex', 'Observe before interpreting. Look for color, texture, pattern, shape, shadow, and context; then switch to the street map to test your idea.')),
              h('label', { style: { display: 'grid', gap: 5, color: '#e6fffb', fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_evidence_based_observation', 'Evidence-based observation'),
                h('textarea', { value: imageryNote, onChange: function (event) { setImageryNote(event.target.value); }, rows: 3, placeholder: __alloT('stem.gisstudio.ui_i_notice_i_think_it_may_be_because', 'I notice... I think it may be... because...'), style: { width: '100%', boxSizing: 'border-box', padding: 9, borderRadius: 8, border: '1px solid #3f6b82', background: '#071827', color: '#fff' } }))),
            h('section', { 'aria-labelledby': 'gis-pattern-heading', style: panel },
              h('div', { style: { display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' } },
                h('h2', { id: 'gis-pattern-heading', style: { margin: 0, fontSize: 15, color: '#f0fdfa' } }, __alloT('stem.gisstudio.ui_spatial_pattern_coach', 'Spatial pattern coach')),
            ...
            geoFeatures.length > 0 && h('section', { 'aria-labelledby': 'gis-geo-table-heading', style: Object.assign({}, panel, { overflow: 'hidden' }) },
              h('h2', { id: 'gis-geo-table-heading', style: { margin: '0 0 4px', fontSize: 15, color: '#f0fdfa' } }, __alloT('stem.gisstudio.ui_choropleth_feature_table_twin', 'Choropleth feature-table twin')),
              h('p', { style: { margin: '0 0 10px', color: '#a7c7d8', fontSize: 11 } }, gisFillTemplate(__alloT('stem.gisstudio.ui_geojson_features_mapped_by', '{count} GeoJSON features mapped by {attribute}.'), { count: geoFeatures.length, attribute: geoMetric })),
              h('div', { style: { overflowX: 'auto' } },
                h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: 12 } },
                  h('caption', { style: { position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0,0,0,0)' } }, __alloT('stem.gisstudio.ui_geojson_feature_values_for_the_choropleth', 'GeoJSON feature values for the choropleth')),
          ...
          return h('div', { style: { marginTop: 14, padding: 14, borderRadius: 12, background: '#071827', border: '1px solid #3f6b82', display: 'grid', gap: 12 } },
            h('div', null,
              ...
              h('p', { role: 'status', style: { margin: '7px 0 0', color: '#67e8f9', fontSize: 11 } },
                gisFillTemplate(__alloT('stem.gisstudio.ui_import_preview_summary', '{rows} source rows \u00B7 {crs} \u00B7 {format}'), { rows: display.number(importPreview.totalRows, { maximumFractionDigits: 0 }), crs: importPreview.crs, format: csvFormatSummary(importPreview) }))),
        ...
        function geoImportReviewPanel() {
          ...
          return h('section', {
            ...
            (inspection.highPrecisionCoordinates > 0 || inspection.identifierWarnings > 0) && h('p', { role: 'alert', style: { margin: 0, padding: 9, borderLeft: '4px solid #f59e0b', borderRadius: 6, background: '#2b2617', color: '#fde68a', fontSize: 11 } },
              gisText.vectorPrivacyWarning + ' ' + gisFillTemplate(__alloT('stem.gisstudio.ui_vector_privacy_counts', '{precise} high-precision {coordinates}; {named} identifier-like {features}.'), { precise: inspection.highPrecisionCoordinates, coordinates: gisText.vectorCoordinates, named: inspection.identifierWarnings, features: gisText.vectorFeatures })),
          ...
          return h('div', { style: { maxWidth: 980, margin: '0 auto', display: 'grid', gap: 14 } },
            h('section', { 'aria-labelledby': 'gis-import-heading', style: Object.assign({}, panel, { padding: 18 }) },
              ...
              h('div', { style: { display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' } },
                ...
                h('button', { type: 'button', onClick: downloadImportReport, disabled: !importDiagnostics.invalidRows && !importDiagnostics.truncatedRows, style: Object.assign({}, control, { cursor: importDiagnostics.invalidRows || importDiagnostics.truncatedRows ? 'pointer' : 'not-allowed', opacity: importDiagnostics.invalidRows || importDiagnostics.truncatedRows ? 1 : 0.55 }) }, __alloT('stem.gisstudio.ui_download_import_review', 'Download import review')),
            ...
            h('section', { 'aria-labelledby': 'gis-join-heading', style: Object.assign({}, panel, { padding: 18 }) },
              h('p', { style: { margin: 0, color: '#67e8f9', fontSize: 11, fontWeight: 800 } }, __alloT('stem.gisstudio.ui_attribute_join', 'ATTRIBUTE JOIN')),
              h('h2', { id: 'gis-join-heading', style: { color: '#f0fdfa', margin: '5px 0 8px' } }, __alloT('stem.gisstudio.ui_join_a_csv_to_map_boundaries', 'Join a CSV to map boundaries')),
              h('p', { style: { color: '#b7d2df', lineHeight: 1.6, fontSize: 13 } }, __alloT('stem.gisstudio.ui_match_a_csv_row_to_each_geojson_feature_using', 'Match a CSV row to each GeoJSON feature using a shared identifier such as county name or FIPS code. Keys are compared without case, punctuation, or extra spaces. Review every mismatch before applying.')),
              !geoData && h('p', { role: 'status', style: { padding: 10, borderRadius: 8, background: '#172d3c', color: '#bae6fd', fontSize: 12 } }, __alloT('stem.gisstudio.ui_build_or_load_a_geojson_layer_above_before_pre', 'Build or load a GeoJSON layer above before preparing a join.')),
              h('label', { style: { display: 'grid', gap: 6, margin: '12px 0', fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_choose_attribute_csv', 'Choose attribute CSV'),
              ...
              h('label', { style: { display: 'grid', gap: 6, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_or_paste_attribute_csv', 'Or paste attribute CSV'),
              ...
              h('div', { style: { display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' } },
                h('button', { type: 'button', onClick: readJoinColumns, disabled: !geoData, style: Object.assign({}, primary, { opacity: geoData ? 1 : 0.55 }) }, __alloT('stem.gisstudio.ui_1_read_csv_columns', '1. Read CSV columns')),
                h('button', { type: 'button', onClick: function () { setJoinText(EXAMPLE_JOIN_CSV); setJoinTable(null); setJoinPreview(null); setJoinError(''); }, disabled: !geoData, style: Object.assign({}, control, { cursor: geoData ? 'pointer' : 'not-allowed' }) }, __alloT('stem.gisstudio.ui_restore_join_example', 'Restore join example'))),
              joinTable && h('div', { style: { marginTop: 14, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 10 } },
                h('label', { style: { display: 'grid', gap: 5, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_geojson_match_field', 'GeoJSON match field'),
                ...
                h('label', { style: { display: 'grid', gap: 5, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_csv_match_field', 'CSV match field'),
                ...
                h('label', { style: { display: 'grid', gap: 5, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_numeric_value_to_map', 'Numeric value to map'),
              ...
              joinTable && h('button', { type: 'button', onClick: previewJoinData, disabled: !joinValueKey, style: Object.assign({}, primary, { marginTop: 12 }) }, __alloT('stem.gisstudio.ui_2_preview_matches', '2. Preview matches')),
              ...
              joinPreview && h('div', { style: { marginTop: 14, padding: 12, borderRadius: 10, background: '#071827', border: '1px solid #3f6b82' } },
                h('h3', { style: { margin: '0 0 8px', color: '#f0fdfa', fontSize: 14 } }, __alloT('stem.gisstudio.ui_join_preview', 'Join preview')),
                h('div', { role: 'status', style: { display: 'flex', gap: 8, flexWrap: 'wrap', fontSize: 12 } },
                  h('strong', { style: { color: '#86efac' } }, gisFillTemplate(__alloT('stem.gisstudio.ui_join_matched_count', '{count} matched'), { count: joinPreview.matched })),
                  h('span', { style: { color: joinPreview.unmatchedCSV.length ? '#fde68a' : '#86efac' } }, gisFillTemplate(__alloT('stem.gisstudio.ui_join_unmatched_csv_count', '{count} CSV rows unmatched'), { count: joinPreview.unmatchedCSV.length })),
                  h('span', { style: { color: joinPreview.unmatchedGeo.length ? '#fde68a' : '#86efac' } }, gisFillTemplate(__alloT('stem.gisstudio.ui_join_unmatched_boundaries_count', '{count} boundaries unmatched'), { count: joinPreview.unmatchedGeo.length })),
                  h('span', { style: { color: joinPreview.duplicates.length ? '#fca5a5' : '#86efac' } }, gisFillTemplate(__alloT('stem.gisstudio.ui_join_duplicate_keys_count', '{count} duplicate keys'), { count: joinPreview.duplicates.length }))),
                (joinPreview.unmatchedCSV.length > 0 || joinPreview.unmatchedGeo.length > 0 || joinPreview.duplicates.length > 0) && h('details', { style: { marginTop: 9, color: '#cfe8f3', fontSize: 12 } },
                  h('summary', { style: { cursor: 'pointer', fontWeight: 700 } }, __alloT('stem.gisstudio.ui_review_unmatched_and_duplicate_keys', 'Review unmatched and duplicate keys')),
                  joinPreview.unmatchedCSV.length > 0 && h('p', null, gisFillTemplate(__alloT('stem.gisstudio.ui_join_csv_only', 'CSV only: {names}'), { names: joinPreview.unmatchedCSV.join(', ') })),
                  joinPreview.unmatchedGeo.length > 0 && h('p', null, gisFillTemplate(__alloT('stem.gisstudio.ui_join_geojson_only', 'GeoJSON only: {names}'), { names: joinPreview.unmatchedGeo.join(', ') })),
                  joinPreview.duplicates.length > 0 && h('p', null, gisFillTemplate(__alloT('stem.gisstudio.ui_join_duplicate_csv_keys', 'Duplicate CSV keys: {names}'), { names: joinPreview.duplicates.join(', ') }))),
                h('button', { type: 'button', onClick: applyJoinData, disabled: !joinPreview.matched, style: Object.assign({}, primary, { marginTop: 10 }) }, gisFillTemplate(__alloT('stem.gisstudio.ui_apply_join_step', '3. Apply the join and map {field}'), { field: joinValueKey })))),
            h('aside', { style: { padding: 12, borderLeft: '4px solid #f59e0b', background: '#2b2617', color: '#fde68a', borderRadius: 8, fontSize: 12, boxSizing: 'border-box' } },
              h('strong', null, __alloT('stem.gisstudio.ui_data_ethics', 'Data ethics: ')), __alloT('stem.gisstudio.ui_do_not_map_student_home_addresses_or_sensitive', 'Do not map student home addresses or sensitive locations. Aggregate, blur, or suppress identifiable points.')));
        }
        ...
        function comparisonTable(series, side) {
          ...
          return h('section', { 'aria-labelledby': 'gis-compare-' + side + '-table-heading', style: Object.assign({}, panel, { overflow: 'hidden' }) },
            ...
            h('p', { style: { margin: '0 0 9px', color: '#a7c7d8', fontSize: 11 } },
              (stats.mean == null
                ? gisFillTemplate(__alloT('stem.gisstudio.ui_series_count_only', '{label}: {count} records.'), { label: series.label, count: stats.count })
                : gisFillTemplate(__alloT('stem.gisstudio.ui_series_with_range', '{label}: {count} records, ranging from {min} to {max}, mean {mean}.'), { label: series.label, count: stats.count, min: display.number(stats.min), max: display.number(stats.max), mean: display.number(stats.mean, 1) }))),
            h('div', { style: { overflowX: 'auto', maxHeight: 320, overflowY: 'auto' } },
              h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: 11 } },
                h('caption', { style: { position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0,0,0,0)' } }, gisFillTemplate(__alloT('stem.gisstudio.ui_side_map_values_caption', '{side} map values for {label}'), { side: side, label: series.label })),
        ...
        function comparisonView() {
          ...
          function sideControls(side, choice, setChoice, base, setBase) {
            return h('fieldset', { style: Object.assign({}, panel, { margin: 0 }) },
              ...
              h('label', { style: { display: 'grid', gap: 5, fontSize: 12, marginBottom: 9 } }, __alloT('stem.gisstudio.ui_data_layer', 'Data layer'),
              ...
              h('label', { style: { display: 'grid', gap: 5, fontSize: 12 } }, __alloT('stem.gisstudio.ui_basemap', 'Basemap'),
                h('select', { value: base, onChange: function (event) { setBase(event.target.value); persist(side === 'Left' ? 'gisCompareLeftBasemap' : 'gisCompareRightBasemap', event.target.value); persist('gisCompared', true); }, style: control },
                  base === 'none' && h('option', { value: 'none' }, __alloT('stem.gisstudio.ui_unavailable_saved_provider', 'Unavailable saved provider')),
                  h('option', { value: 'street' }, __alloT('stem.gisstudio.ui_street_map', 'Street map')),
                  h('option', { value: 'satellite' }, __alloT('stem.gisstudio.ui_satellite_imagery', 'Satellite imagery')))));
          }
          return h('div', { style: { display: 'grid', gap: 14 } },
            h('section', { 'aria-labelledby': 'gis-compare-heading', style: panel },
              h('p', { style: { margin: 0, color: '#fde68a', fontSize: 10, fontWeight: 900, letterSpacing: '.09em' } }, __alloT('stem.gisstudio.ui_same_place_different_lens', 'SAME PLACE • DIFFERENT LENS')),
              h('h2', { id: 'gis-compare-heading', style: { margin: '4px 0 6px', color: '#f0fdfa', fontSize: 19 } }, __alloT('stem.gisstudio.ui_synchronized_map_comparison', 'Synchronized map comparison')),
              h('p', { style: { margin: 0, color: '#b7d2df', fontSize: 12, lineHeight: 1.5 } }, __alloT('stem.gisstudio.ui_change_either_layer_or_basemap_pan_and_zoom_on', 'Change either layer or basemap. Pan and zoom one map; the other follows so scale and extent stay comparable.')),
            ...
            h('section', { 'aria-label': __alloT('stem.gisstudio.a11y_synchronized_comparison_maps', 'Synchronized comparison maps'), style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(330px,100%),1fr))', gap: 12 } },
              h('div', null,
                h('h3', { style: { margin: '0 0 6px', color: '#67e8f9', fontSize: 13 } }, gisFillTemplate(__alloT('stem.gisstudio.ui_left_layer', 'Left: {label}'), { label: comparisonLabel(leftChoice) })),
                ...
                h('p', { style: { color: '#a7c7d8', fontSize: 10 } }, gisFillTemplate(__alloT('stem.gisstudio.ui_legend_and_count', 'Legend: low teal to high rose. {count} records.'), { count: leftStats.count }))),
              h('div', null,
                h('h3', { style: { margin: '0 0 6px', color: '#67e8f9', fontSize: 13 } }, gisFillTemplate(__alloT('stem.gisstudio.ui_right_layer', 'Right: {label}'), { label: comparisonLabel(rightChoice) })),
                ...
                h('p', { style: { color: '#a7c7d8', fontSize: 10 } }, gisFillTemplate(__alloT('stem.gisstudio.ui_legend_and_count', 'Legend: low teal to high rose. {count} records.'), { count: rightStats.count })))),
            ...
            h('section', { 'aria-labelledby': 'gis-evidence-heading', style: panel },
              h('h2', { id: 'gis-evidence-heading', style: { margin: '0 0 6px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_evidence_builder', 'Evidence builder')),
              h('label', { style: { display: 'grid', gap: 6, color: '#e6fffb', fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_observation_or_claim', 'Observation or claim'),
                h('textarea', { value: comparisonObservation, onChange: function (event) { setComparisonObservation(event.target.value); }, rows: 3, placeholder: __alloT('stem.gisstudio.ui_i_observe_evidence_from_the_left_map_evidence', 'I observe... Evidence from the left map... Evidence from the right map... A limitation is...'), style: { width: '100%', boxSizing: 'border-box', padding: 10, borderRadius: 8, border: '1px solid #3f6b82', background: '#071827', color: '#fff' } })),
              h('p', { style: { margin: '8px 0', color: '#fcd34d', fontSize: 10 } }, __alloT('stem.gisstudio.ui_describe_what_the_maps_show_treat_explanations', 'Describe what the maps show. Treat explanations as hypotheses unless other evidence supports causation.')),
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap' } },
                h('button', { type: 'button', onClick: downloadEvidenceReport, style: primary }, __alloT('stem.gisstudio.ui_download_accessible_report', 'Download accessible report')),
                h('button', { type: 'button', onClick: printEvidenceReport, style: Object.assign({}, primary, { background: '#155e75' }) }, __alloT('stem.gisstudio.ui_print_or_save_as_pdf', 'Print or save as PDF')))),
        ...
        function remoteSensingView() {
          ...
          return h('div', { style: { display: 'grid', gap: 14 } },
            h('section', { 'aria-labelledby': 'gis-remote-heading', style: panel },
              h('p', { style: { margin: 0, color: '#fde68a', fontSize: 10, fontWeight: 900, letterSpacing: '.09em' } }, __alloT('stem.gisstudio.ui_observe_measure_verify', 'OBSERVE • MEASURE • VERIFY')),
              h('h2', { id: 'gis-remote-heading', style: { margin: '4px 0 6px', color: '#f0fdfa', fontSize: 20 } }, __alloT('stem.gisstudio.ui_remote_sensing_lab', 'Remote Sensing Lab')),
              h('p', { style: { margin: 0, color: '#b7d2df', fontSize: 12, lineHeight: 1.55 } },
                __alloT('stem.gisstudio.ui_compare_matched_season_multispectral_scenes_in', 'Compare matched-season multispectral scenes, inspect spectral signatures, mask clouds, calculate change area, and separate observations from causal explanations.')),
              ...
              h('aside', { style: { marginTop: 10, padding: 10, borderLeft: '4px solid #38bdf8', borderRadius: 8, background: '#082f49', color: '#bae6fd', fontSize: 11, lineHeight: 1.5 } },
                h('strong', null, __alloT('stem.gisstudio.ui_instructional_data_notice', 'Instructional-data notice: ')), remoteScene.source)),
            h('section', { 'aria-labelledby': 'gis-remote-controls-heading', style: panel },
              h('h2', { id: 'gis-remote-controls-heading', style: { margin: '0 0 10px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_imagery_and_index_controls', 'Imagery and index controls')),
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 10 } },
                h('label', { style: { display: 'grid', gap: 5, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_display', 'Display'),
                  h('select', { value: remoteSensing.viewMode, onChange: function (event) { updateRemoteSensing('viewMode', event.target.value); }, style: control },
                    h('option', { value: 'trueColor' }, __alloT('stem.gisstudio.ui_true_color', 'True color')),
                    h('option', { value: 'falseColor' }, __alloT('stem.gisstudio.ui_color_infrared', 'Color infrared')),
                    h('option', { value: 'ndvi' }, __alloT('stem.gisstudio.ui_ndvi_vegetation_index', 'NDVI vegetation index')),
                    h('option', { value: 'ndwi' }, __alloT('stem.gisstudio.ui_ndwi_water_index', 'NDWI water index')),
                    h('option', { value: 'ndbi' }, __alloT('stem.gisstudio.ui_ndbi_built_up_index', 'NDBI built-up index')))),
                h('label', { style: { display: 'grid', gap: 5, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_analysis_index', 'Analysis index'),
                  h('select', { value: remoteSensing.analysisIndex, onChange: function (event) { updateRemoteSensing('analysisIndex', event.target.value); }, style: control },
                    h('option', { value: 'ndvi' }, __alloT('stem.gisstudio.ui_ndvi_vegetation', 'NDVI — vegetation')),
                    h('option', { value: 'ndwi' }, __alloT('stem.gisstudio.ui_ndwi_water_and_moisture', 'NDWI — water and moisture')),
                    h('option', { value: 'ndbi' }, __alloT('stem.gisstudio.ui_ndbi_built_up_surfaces', 'NDBI — built-up surfaces')))),
            ...
            h('section', { 'aria-labelledby': 'gis-remote-swipe-heading', style: panel },
              ...
              h('div', {
                ...
                h('span', { 'aria-hidden': 'true', style: { position: 'absolute', top: 8, left: 8, padding: '4px 7px', borderRadius: 6, background: 'rgba(7,24,39,.85)', color: '#fff', fontSize: 10, fontWeight: 900 } }, __alloT('stem.gisstudio.ui_before', 'BEFORE')),
                h('span', { 'aria-hidden': 'true', style: { position: 'absolute', top: 8, right: 8, padding: '4px 7px', borderRadius: 6, background: 'rgba(7,24,39,.85)', color: '#fff', fontSize: 10, fontWeight: 900 } }, __alloT('stem.gisstudio.ui_after', 'AFTER')),
            ...
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 14 } },
              h('section', { 'aria-labelledby': 'gis-remote-inspector-heading', style: panel },
                ...
                h('label', { style: { display: 'grid', gap: 5, marginBottom: 10, color: '#dbeafe', fontSize: 11, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_selected_pixel', 'Selected pixel'),
                ...
                h('div', { style: { marginTop: 10, display: 'grid', gap: 7, fontSize: 11 } },
                  h('p', { style: { margin: 0 } }, h('strong', { style: { color: '#86efac' } }, __alloT('stem.gisstudio.ui_before_estimate', 'Before estimate: ')), remoteBeforeClass.label + ' — ' + remoteBeforeClass.evidence + '.'),
                  h('p', { style: { margin: 0 } }, h('strong', { style: { color: '#fde68a' } }, __alloT('stem.gisstudio.ui_after_estimate', 'After estimate: ')), remoteAfterClass.label + ' — ' + remoteAfterClass.evidence + '.'),
                  h('p', { style: { margin: 0, color: '#a7c7d8' } }, __alloT('stem.gisstudio.ui_threshold_classifications_are_clues_not_ground', 'Threshold classifications are clues, not ground truth. Mixed pixels and atmospheric effects can change the signature.')))),
              h('section', { 'aria-labelledby': 'gis-remote-quality-heading', style: panel },
                ...
                h('aside', { style: { marginTop: 10, padding: 10, borderRadius: 8, background: '#2b2617', color: '#fde68a', fontSize: 10, lineHeight: 1.5 } },
                  h('strong', null, __alloT('stem.gisstudio.ui_a_change_map_answers_where_and_how_much', 'A change map answers “where and how much?” ')), __alloT('stem.gisstudio.ui_it_does_not_by_itself_answer_why_use_field_obs', 'It does not by itself answer “why?” Use field observations, additional dates, metadata, and local knowledge before naming a cause.')))),
            h('section', { 'aria-labelledby': 'gis-remote-table-heading', style: Object.assign({}, panel, { overflow: 'hidden' }) },
              h('h2', { id: 'gis-remote-table-heading', style: { margin: '0 0 5px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_accessible_raster_table_twin', 'Accessible raster-table twin')),
              h('p', { style: { margin: '0 0 9px', color: '#a7c7d8', fontSize: 10 } }, __alloT('stem.gisstudio.ui_every_visual_pixel_has_an_equivalent_row_cloud', 'Every visual pixel has an equivalent row. Cloud-obscured values are excluded from change statistics.')),
              h('div', { style: { overflowX: 'auto', maxHeight: 380, overflowY: 'auto' } },
                h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: 10 } },
                  h('caption', { style: { position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0,0,0,0)' } }, __alloT('stem.gisstudio.ui_remote_sensing_before_and_after_pixel_values', 'Remote sensing before-and-after pixel values')),
            ...
            h('section', { 'aria-labelledby': 'gis-remote-evidence-heading', style: panel },
              ...
              h('p', { style: { margin: '0 0 9px', color: '#cfe8f3', fontSize: 11, lineHeight: 1.5 } },
                __alloT('stem.gisstudio.ui_prompt_what_changed_between_the_two_matched_se', 'Prompt: What changed between the two matched-season scenes? Cite at least two pixels or summary measures, explain what the selected index contributes, and name one limitation.')),
              h('label', { style: { display: 'grid', gap: 5, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_evidence_based_interpretation', 'Evidence-based interpretation'),
                h('textarea', {
                  ...
                  placeholder: __alloT('stem.gisstudio.ui_i_observe_the_change_table_shows_the_index_sug', 'I observe... The change table shows... The index suggests... This does not prove... One limitation is...'),
                  style: Object.assign({}, control, { resize: 'vertical' })
                })),
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 10 } },
                h('button', { type: 'button', onClick: downloadRemoteSensingReport, style: primary }, __alloT('stem.gisstudio.ui_download_remote_sensing_evidence_report', 'Download remote-sensing evidence report')),
                h('button', { type: 'button', onClick: printRemoteSensingReport, style: Object.assign({}, primary, { background: '#155e75' }) }, __alloT('stem.gisstudio.ui_print_or_save_as_pdf', 'Print or save as PDF')),
                h('button', { type: 'button', onClick: function () { setBasemap('satellite'); persist('gisBasemap', 'satellite'); setTab('map'); persist('gisTab', 'map'); }, style: Object.assign({}, control, { cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_open_live_satellite_basemap_for_context', 'Open live satellite basemap for context'))),
              h('p', { style: { margin: '9px 0 0', color: '#a7c7d8', fontSize: 10 } },
                __alloT('stem.gisstudio.ui_the_live_basemap_is_contextual_imagery_from_es', 'The live basemap is contextual imagery from Esri and does not provide the dated band values used in this illustrative learning scene.'))));
        }
        ...
        function composerView() {
          ...
          return h('div', { style: { display: 'grid', gap: 14 } },
            h('section', { 'aria-labelledby': 'gis-composer-heading', style: panel },
              h('p', { style: { margin: 0, color: '#fde68a', fontSize: 10, fontWeight: 900, letterSpacing: '.09em' } }, __alloT('stem.gisstudio.ui_design_explain_share', 'DESIGN • EXPLAIN • SHARE')),
              h('h2', { id: 'gis-composer-heading', style: { margin: '4px 0 6px', color: '#f0fdfa', fontSize: 20 } }, __alloT('stem.gisstudio.ui_accessible_map_composer', 'Accessible Map Composer')),
              h('p', { style: { margin: 0, color: '#b7d2df', fontSize: 12, lineHeight: 1.55 } },
                __alloT('stem.gisstudio.ui_turn_the_current_point_layer_into_a_classroom', 'Turn the current point layer into a classroom-ready evidence map. The export keeps the visual map, description, annotations, data table, method, source, and limitations together in one HTML file.'))),
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 14, alignItems: 'start' } },
              h('section', { 'aria-labelledby': 'gis-composer-controls-heading', style: panel },
                h('h3', { id: 'gis-composer-controls-heading', style: { margin: '0 0 10px', color: '#67e8f9', fontSize: 15 } }, __alloT('stem.gisstudio.ui_map_text_and_legend', 'Map text and legend')),
                [
                  ['title', 'Map title', 200],
                ...
                h('label', { style: { display: 'grid', gap: 5, marginBottom: 9, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_evidence_based_takeaway', 'Evidence-based takeaway'),
                  h('textarea', { value: composer.claim, maxLength: 2000, rows: 4, onChange: function (event) { updateComposer('claim', event.target.value); }, placeholder: __alloT('stem.gisstudio.ui_state_a_claim_cite_the_mapped_pattern_and_name', 'State a claim, cite the mapped pattern, and name a limitation.'), style: Object.assign({}, control, { resize: 'vertical' }) })),
                h('label', { style: { display: 'grid', gap: 5, marginBottom: 8, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_map_description_for_screen_readers', 'Map description for screen readers'),
                  h('textarea', { value: composer.altText, maxLength: 2000, rows: 5, onChange: function (event) { updateComposer('altText', event.target.value); }, placeholder: __alloT('stem.gisstudio.ui_describe_the_spatial_pattern_highest_and_lowes', 'Describe the spatial pattern, highest and lowest values, and notable annotations.'), style: Object.assign({}, control, { resize: 'vertical' }) })),
                h('button', { type: 'button', onClick: generateComposerDescription, style: Object.assign({}, control, { cursor: 'pointer', marginBottom: 10 }) }, __alloT('stem.gisstudio.ui_draft_description_from_data', 'Draft description from data')),
                h('label', { style: { display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, fontWeight: 700 } },
                  ...
                  __alloT('stem.gisstudio.ui_show_the_legend_in_the_composed_map', 'Show the legend in the composed map'))),
              h('section', { 'aria-labelledby': 'gis-composer-preview-heading', style: Object.assign({}, panel, { background: '#f8fafc', color: '#172033' }) },
                h('p', { style: { margin: 0, color: '#0f766e', fontSize: 10, fontWeight: 900, letterSpacing: '.09em' } }, __alloT('stem.gisstudio.ui_live_accessible_preview', 'LIVE ACCESSIBLE PREVIEW')),
            ...
            h('section', { 'aria-labelledby': 'gis-annotation-heading', style: panel },
              h('h2', { id: 'gis-annotation-heading', style: { margin: '0 0 6px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_evidence_annotations', 'Evidence annotations')),
              h('p', { style: { margin: '0 0 10px', color: '#a7c7d8', fontSize: 11 } }, __alloT('stem.gisstudio.ui_add_up_to_20_numbered_callouts_coordinates_are', 'Add up to 20 numbered callouts. Coordinates are included in the export and should not identify private locations.')),
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 8, alignItems: 'end' } },
                h('label', { style: { display: 'grid', gap: 4, fontSize: 11, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_callout_text', 'Callout text'),
                ...
                h('label', { style: { display: 'grid', gap: 4, fontSize: 11, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_latitude', 'Latitude'),
                ...
                h('label', { style: { display: 'grid', gap: 4, fontSize: 11, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_longitude', 'Longitude'),
                ...
                h('button', { type: 'button', onClick: addComposerAnnotation, style: primary }, __alloT('stem.gisstudio.ui_add_callout', 'Add callout'))),
                  ...
                  return h('li', { key: annotation.id, style: { display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', padding: 8, borderRadius: 8, background: '#071827', fontSize: 11 } },
                    h('span', null, h('strong', { style: { color: '#fde047' } }, 'A' + (index + 1) + ' '), annotation.label + ' (' + display.coordinate(annotation.lat, 4, 'lat') + ', ' + display.coordinate(annotation.lon, 4, 'lon') + ')',
                      strayAnnotations.outside.some(function (item) { return item.index === index; })
                        ...
                        : null),
                    h('button', { type: 'button', onClick: function () { removeComposerAnnotation(index); }, 'aria-label': 'Remove annotation A' + (index + 1), style: Object.assign({}, control, { cursor: 'pointer', padding: '5px 8px' }) }, __alloT('stem.gisstudio.ui_remove', 'Remove')));
                }))),
            h('section', { 'aria-labelledby': 'gis-cartography-coach-heading', style: panel },
              h('div', { style: { display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'start', flexWrap: 'wrap' } },
                h('div', null,
                  h('p', { style: { margin: 0, color: '#fde68a', fontSize: 10, fontWeight: 900, letterSpacing: '.08em' } }, __alloT('stem.gisstudio.ui_cartography_coach', 'CARTOGRAPHY COACH')),
            ...
            h('section', { 'aria-labelledby': 'gis-composer-export-heading', style: panel },
              ...
              h('p', { style: { margin: '0 0 10px', color: '#b7d2df', fontSize: 11, lineHeight: 1.5 } },
                __alloT('stem.gisstudio.ui_the_self_contained_html_package_includes_the_s', 'The self-contained HTML package includes the schematic map, screen-reader description, legend, callout key, complete point-data table, provenance, method, limitations, and print styles.')),
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap' } },
                h('button', { type: 'button', onClick: downloadComposerPackage, style: primary }, __alloT('stem.gisstudio.ui_download_accessible_map_package', 'Download accessible map package')),
                h('button', { type: 'button', onClick: printComposerPackage, style: Object.assign({}, primary, { background: '#155e75' }) }, __alloT('stem.gisstudio.ui_print_or_save_as_pdf', 'Print or save as PDF')),
                h('button', { type: 'button', onClick: function () { setTab('project'); persist('gisTab', 'project'); }, style: Object.assign({}, control, { cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_review_project_provenance', 'Review project provenance'))),
        ...
        function storyMapView() {
          return h('div', { style: { display: 'grid', gap: 14 } },
            h('section', { 'aria-labelledby': 'gis-story-heading', style: panel },
              h('p', { style: { margin: 0, color: '#fde68a', fontSize: 10, fontWeight: 900, letterSpacing: '.09em' } }, __alloT('stem.gisstudio.ui_sequence_explain_share', 'SEQUENCE • EXPLAIN • SHARE')),
              h('h2', { id: 'gis-story-heading', style: { margin: '4px 0 6px', color: '#f0fdfa', fontSize: 20 } }, __alloT('stem.gisstudio.ui_story_map_studio', 'Story Map Studio')),
              h('p', { style: { margin: 0, color: '#b7d2df', fontSize: 12, lineHeight: 1.55 } }, __alloT('stem.gisstudio.ui_sequence_views_from_gis_studio_into_an_accessi', 'Sequence views from GIS Studio into an accessible claim → evidence → limitation narrative. Every frame keeps its text, source, and limitation beside the visual view.')),
              h('label', { style: { display: 'grid', gap: 5, marginTop: 12, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_story_title', 'Story title'), h('input', { type: 'text', value: storyMap.title, maxLength: 200, onChange: function (event) { updateStoryMapField('title', event.target.value); }, style: control })),
              h('label', { style: { display: 'grid', gap: 5, marginTop: 9, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_guiding_question_or_subtitle', 'Guiding question or subtitle'), h('input', { type: 'text', value: storyMap.subtitle, maxLength: 300, onChange: function (event) { updateStoryMapField('subtitle', event.target.value); }, style: control })),
              h('div', { style: { display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 12 } },
                ['claim', 'evidence', 'limitation'].map(function (key) { return h('label', { key: key, style: { display: 'flex', alignItems: 'center', gap: 7, fontSize: 12, fontWeight: 800 } }, h('input', { type: 'checkbox', checked: storyMap.checks[key], onChange: function (event) { updateStoryMapField(key, event.target.checked); } }), ({
                  claim: __alloT('stem.gisstudio.ui_story_check_claim', 'Claim check'),
                  evidence: __alloT('stem.gisstudio.ui_story_check_evidence', 'Evidence check'),
                  limitation: __alloT('stem.gisstudio.ui_story_check_limitation', 'Limitation check')
                }[key])); })),
            ...
            h('section', { 'aria-labelledby': 'gis-story-capture-heading', style: panel },
              ...
              h('p', { style: { margin: '0 0 10px', color: '#b7d2df', fontSize: 11, lineHeight: 1.5 } }, __alloT('stem.gisstudio.ui_open_a_map_comparison_timeline_composer_or_rem', 'Open a map, comparison, timeline, Composer, or Remote Sensing view, then use “Add current view to story.” You can also add a frame from scratch here.')),
              h('button', { type: 'button', onClick: addCurrentViewToStory, style: primary }, __alloT('stem.gisstudio.ui_add_current_view_to_story', 'Add current view to story')),
              ...
              h('button', { type: 'button', onClick: addCustomStoryFrame, style: Object.assign({}, control, { cursor: 'pointer', marginTop: 10 }) }, __alloT('stem.gisstudio.ui_add_custom_evidence_frame', 'Add custom evidence frame'))),
            h('section', { 'aria-labelledby': 'gis-story-trail-heading', style: panel },
              h('h2', { id: 'gis-story-trail-heading', style: { margin: '0 0 6px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_accessible_evidence_trail', 'Accessible evidence trail')),
              storyMap.slides.length ? h('ol', { style: { margin: 0, paddingLeft: 22, display: 'grid', gap: 10 } }, storyMap.slides.map(function (frame, index) {
                return h('li', { key: frame.id }, h('article', { style: { background: '#071827', border: '1px solid #36586b', borderRadius: 10, padding: 12 } },
                  h('div', { style: { display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap', alignItems: 'start' } }, h('div', null, h('strong', { style: { color: '#67e8f9' } }, frame.title), h('p', { style: { margin: '3px 0 0', color: '#8fb4c4', fontSize: 10 } }, frame.view + (frame.metric ? ' • ' + frame.metric : ''))), h('div', { style: { display: 'flex', gap: 5 } }, h('button', { type: 'button', disabled: index === 0, onClick: function () { moveStoryFrame(index, -1); }, 'aria-label': 'Move ' + frame.title + ' earlier', style: Object.assign({}, control, { cursor: index === 0 ? 'not-allowed' : 'pointer', padding: '5px 8px' }) }, '↑'), h('button', { type: 'button', disabled: index === storyMap.slides.length - 1, onClick: function () { moveStoryFrame(index, 1); }, 'aria-label': 'Move ' + frame.title + ' later', style: Object.assign({}, control, { cursor: index === storyMap.slides.length - 1 ? 'not-allowed' : 'pointer', padding: '5px 8px' }) }, '↓'), h('button', { type: 'button', onClick: function () { removeStoryFrame(index); }, 'aria-label': 'Remove ' + frame.title, style: Object.assign({}, control, { cursor: 'pointer', padding: '5px 8px' }) }, __alloT('stem.gisstudio.ui_remove', 'Remove')))),
                  h('p', { style: { margin: '9px 0 0', fontSize: 11, lineHeight: 1.5 } }, h('strong', { style: { color: '#fde68a' } }, __alloT('stem.gisstudio.ui_observation', 'Observation: ')), frame.narrative || 'Not recorded.'),
                  h('p', { style: { margin: '5px 0 0', fontSize: 11, lineHeight: 1.5 } }, h('strong', { style: { color: '#86efac' } }, __alloT('stem.gisstudio.ui_evidence', 'Evidence: ')), frame.evidence || 'Not recorded.'),
                  h('p', { style: { margin: '5px 0 0', fontSize: 11, lineHeight: 1.5 } }, h('strong', { style: { color: '#fca5a5' } }, __alloT('stem.gisstudio.ui_limitation', 'Limitation: ')), frame.limitation || 'Not recorded.')));
              })) : h('p', { style: { padding: 12, borderRadius: 9, background: '#071827', color: '#a7c7d8' } }, __alloT('stem.gisstudio.ui_no_frames_yet_capture_the_current_gis_view_or', 'No frames yet. Capture the current GIS view or add a custom frame above.'))),
            h('section', { 'aria-labelledby': 'gis-story-export-heading', style: panel },
              h('h2', { id: 'gis-story-export-heading', style: { margin: '0 0 6px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_export_the_story_map', 'Export the story map')),
              h('p', { style: { margin: '0 0 10px', color: '#b7d2df', fontSize: 11, lineHeight: 1.5 } }, __alloT('stem.gisstudio.ui_the_self_contained_html_report_includes_an_ord', 'The self-contained HTML report includes an ordered, screen-reader-friendly evidence trail, reflection checks, a data-table twin, and method limitations. It prints cleanly to PDF.')),
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap' } }, h('button', { type: 'button', onClick: downloadStoryMapReport, style: primary }, __alloT('stem.gisstudio.ui_download_story_map_report', 'Download story map report')), h('button', { type: 'button', onClick: printStoryMapReport, style: Object.assign({}, primary, { background: '#155e75' }) }, __alloT('stem.gisstudio.ui_print_or_save_as_pdf', 'Print or save as PDF')))));
        }
        ...
        function qualityReviewView() {
          ...
          return h('div', { style: { display: 'grid', gap: 14 } },
            h('section', { 'aria-labelledby': 'gis-quality-heading', style: panel },
              h('p', { style: { margin: 0, color: '#fde68a', fontSize: 10, fontWeight: 900, letterSpacing: '.09em' } }, __alloT('stem.gisstudio.ui_check_explain_share', 'CHECK • EXPLAIN • SHARE')),
            ...
            h('section', { 'aria-labelledby': 'gis-quality-table-heading', style: panel },
              h('h2', { id: 'gis-quality-table-heading', style: { margin: '0 0 6px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_accessible_quality_table', 'Accessible quality table')),
              h('div', { style: { overflowX: 'auto' } }, h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: 11 } }, h('caption', { style: { textAlign: 'left', padding: '6px 0', color: '#67e8f9', fontWeight: 800 } }, __alloT('stem.gisstudio.ui_quality_checks_findings_and_next_actions', 'Quality checks, findings, and next actions')), h('thead', null, h('tr', null, ['Check', 'Status', 'Finding', 'Next action'].map(function (item) { return h('th', { key: item, scope: 'col', style: { textAlign: 'left', padding: 7, borderBottom: '1px solid #3f6b82', color: '#a7c7d8' } }, item); }))), h('tbody', null, qualityReview.checks.map(function (item) { var color = item.status === 'error' ? '#fca5a5' : item.status === 'warning' ? '#fde68a' : '#86efac'; return h('tr', { key: item.id }, h('th', { scope: 'row', style: { textAlign: 'left', padding: 7, borderBottom: '1px solid #234456' } }, item.label), h('td', { style: { padding: 7, borderBottom: '1px solid #234456', color: color, fontWeight: 900 } }, item.status.toUpperCase()), h('td', { style: { padding: 7, borderBottom: '1px solid #234456' } }, item.message), h('td', { style: { padding: 7, borderBottom: '1px solid #234456', color: '#b7d2df' } }, item.recommendation)); }))))),
            h('section', { 'aria-labelledby': 'gis-quality-export-heading', style: panel },
              h('h2', { id: 'gis-quality-export-heading', style: { margin: '0 0 6px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_export_the_quality_review', 'Export the quality review')),
              h('p', { style: { margin: '0 0 10px', color: '#b7d2df', fontSize: 11, lineHeight: 1.5 } }, __alloT('stem.gisstudio.ui_the_report_preserves_the_score_review_checklis', 'The report preserves the score, review checklist, quality table, and plain-language limits for a teacher handoff or story-map appendix.')),
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap' } }, h('button', { type: 'button', onClick: downloadDataQualityReport, style: primary }, __alloT('stem.gisstudio.ui_download_quality_review', 'Download quality review')), h('button', { type: 'button', onClick: printDataQualityReport, style: Object.assign({}, primary, { background: '#155e75' }) }, __alloT('stem.gisstudio.ui_print_or_save_as_pdf', 'Print or save as PDF')))));
        }
        ...
        function investigationPacketView() {
          ...
          return h('div', { style: { display: 'grid', gap: 14 } },
            h('section', { 'aria-labelledby': 'gis-packet-heading', style: panel },
              h('p', { style: { margin: 0, color: '#fde68a', fontSize: 10, fontWeight: 900, letterSpacing: '.09em' } }, __alloT('stem.gisstudio.ui_assemble_hand_off_reflect', 'ASSEMBLE • HAND OFF • REFLECT')),
              h('h2', { id: 'gis-packet-heading', style: { margin: '4px 0 6px', color: '#f0fdfa', fontSize: 20 } }, __alloT('stem.gisstudio.ui_investigation_packet', 'Investigation Packet')),
              h('p', { style: { margin: 0, color: '#b7d2df', fontSize: 12, lineHeight: 1.55 } }, __alloT('stem.gisstudio.ui_combine_the_story_map_quality_review_mapped_ta', 'Combine the Story Map, Quality Review, mapped table, and provenance into one accessible teacher handoff. This is the natural final artifact for a GIS investigation.')),
            ...
            h('section', { 'aria-labelledby': 'gis-packet-contents-heading', style: panel },
              ...
              h('ul', { style: { margin: 0, paddingLeft: 21, color: '#dbeafe', fontSize: 12, lineHeight: 1.8 } }, h('li', null, __alloT('stem.gisstudio.ui_working_claim_and_investigation_question', 'Working claim and investigation question')), h('li', null, gisFillTemplate(__alloT('stem.gisstudio.ui_ordered_story_frames', '{count} ordered Story Map evidence frames'), { count: storyProgress.frames })), h('li', null, __alloT('stem.gisstudio.ui_quality_and_uncertainty_table_with_next_action', 'Quality and uncertainty table with next actions')), h('li', null, gisFillTemplate(__alloT('stem.gisstudio.ui_mapped_records_table_twin', '{count} mapped records in a data-table twin'), { count: composerRows.length })), h('li', null, __alloT('stem.gisstudio.ui_provenance_method_limitations_and_handoff_ques', 'Provenance, method, limitations, and handoff questions'))),
            ...
            h('section', { 'aria-labelledby': 'gis-packet-export-heading', style: panel },
              ...
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap' } }, h('button', { type: 'button', onClick: downloadInvestigationPacket, style: primary }, __alloT('stem.gisstudio.ui_download_investigation_packet', 'Download Investigation Packet')), h('button', { type: 'button', onClick: printInvestigationPacket, style: Object.assign({}, primary, { background: '#155e75' }) }, __alloT('stem.gisstudio.ui_print_or_save_as_pdf', 'Print or save as PDF'))),
        ...
        function investigationPlannerView() {
          ...
          return h('div', { style: { display: 'grid', gap: 14 } },
            h('section', { 'aria-labelledby': 'gis-planner-heading', style: panel },
              h('p', { style: { margin: 0, color: '#fde68a', fontSize: 10, fontWeight: 900, letterSpacing: '.09em' } }, __alloT('stem.gisstudio.ui_ask_test_revise', 'ASK • TEST • REVISE')),
              h('h2', { id: 'gis-planner-heading', style: { margin: '4px 0 6px', color: '#f0fdfa', fontSize: 20 } }, __alloT('stem.gisstudio.ui_investigation_planner', 'Investigation Planner')),
              h('p', { style: { margin: 0, color: '#b7d2df', fontSize: 12, lineHeight: 1.55 } }, __alloT('stem.gisstudio.ui_plan_the_reasoning_before_you_interpret_the_ma', 'Plan the reasoning before you interpret the map. A strong spatial investigation names a question, a claim, the evidence needed, an alternative explanation, and a next step.')),
              h('label', { style: { display: 'grid', gap: 5, marginTop: 13, fontSize: 12, fontWeight: 800 } }, __alloT('stem.gisstudio.ui_question_type', 'Question type'), h('select', { value: inquiryPlan.template, onChange: function (event) { chooseInquiryTemplate(event.target.value); }, style: control }, templateOptions)),
              ...
              h('div', { style: { display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginTop: 13 } }, h('progress', { max: inquiryProgress.total, value: inquiryProgress.complete, style: { width: 190, accentColor: '#5eead4' }, 'aria-label': __alloT('stem.gisstudio.a11y_investigation_planning_progress', 'Investigation planning progress') }), h('strong', { style: { color: inquiryProgress.ready ? '#86efac' : '#fde68a' } }, gisFillTemplate(__alloT('stem.gisstudio.ui_planning_checks_complete', '{done} of {total} planning checks complete'), { done: inquiryProgress.complete, total: inquiryProgress.total }))),
            ...
            h('section', { 'aria-labelledby': 'gis-planner-next-heading', style: panel },
              h('h2', { id: 'gis-planner-next-heading', style: { margin: '0 0 6px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_continue_the_investigation', 'Continue the investigation')),
              h('p', { style: { margin: '0 0 10px', color: '#b7d2df', fontSize: 11, lineHeight: 1.5 } }, __alloT('stem.gisstudio.ui_your_plan_is_saved_with_the_project_and_includ', 'Your plan is saved with the project and included in the Investigation Packet. Move between workspaces without losing the reasoning trail.')),
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap' } }, h('button', { type: 'button', onClick: function () { setTab('map'); persist('gisTab', 'map'); }, style: primary }, __alloT('stem.gisstudio.ui_open_map_workspace', 'Open Map Workspace')), h('button', { type: 'button', onClick: function () { setTab('story'); persist('gisTab', 'story'); }, style: Object.assign({}, control, { cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_build_story_map', 'Build Story Map')), h('button', { type: 'button', onClick: function () { setTab('packet'); persist('gisTab', 'packet'); }, style: Object.assign({}, control, { cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_review_investigation_packet', 'Review Investigation Packet')))));
        }
        ...
        function teacherReviewView() {
          ...
          return h('div', { style: { display: 'grid', gap: 14 } },
            h('section', { 'aria-labelledby': 'gis-review-heading', style: panel },
              h('p', { style: { margin: 0, color: '#fde68a', fontSize: 10, fontWeight: 900, letterSpacing: '.09em' } }, __alloT('stem.gisstudio.ui_notice_respond_revise', 'NOTICE • RESPOND • REVISE')),
              h('h2', { id: 'gis-review-heading', style: { margin: '4px 0 6px', color: '#f0fdfa', fontSize: 20 } }, __alloT('stem.gisstudio.ui_teacher_review', 'Teacher Review')),
              h('p', { style: { margin: 0, color: '#b7d2df', fontSize: 12, lineHeight: 1.55 } }, __alloT('stem.gisstudio.ui_use_the_rubric_to_make_the_investigation_stron', 'Use the rubric to make the investigation stronger. Feedback is saved with the project and carried into the Investigation Packet.')),
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 10, marginTop: 13 } },
                h('label', { style: { display: 'grid', gap: 5, fontSize: 11, fontWeight: 800 } }, __alloT('stem.gisstudio.ui_reviewer', 'Reviewer'), h('input', { type: 'text', value: teacherReview.reviewer, maxLength: 160, onChange: function (event) { updateTeacherReviewField('reviewer', event.target.value); }, style: control })),
                h('label', { style: { display: 'grid', gap: 5, fontSize: 11, fontWeight: 800 } }, __alloT('stem.gisstudio.ui_review_status', 'Review status'), h('select', { value: teacherReview.status, onChange: function (event) { updateTeacherReviewField('status', event.target.value); }, style: control }, h('option', { value: 'draft' }, __alloT('stem.gisstudio.ui_draft', 'Draft')), h('option', { value: 'revise' }, __alloT('stem.gisstudio.ui_revise_and_resubmit', 'Revise and resubmit')), h('option', { value: 'ready' }, __alloT('stem.gisstudio.ui_ready_to_share', 'Ready to share'))))),
              h('div', { style: { display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginTop: 12 } }, h('strong', { style: { color: statusColor, fontSize: 26 } }, teacherProgress.points + '/' + teacherProgress.max), h('span', { style: { color: statusColor, fontSize: 11, fontWeight: 800 } }, (teacherProgress.ready
                ? gisFillTemplate(__alloT('stem.gisstudio.ui_rubric_rated_complete', '{rated} of {total} rubric areas rated • review complete'), { rated: teacherProgress.rated, total: teacherProgress.total })
                : gisFillTemplate(__alloT('stem.gisstudio.ui_rubric_rated', '{rated} of {total} rubric areas rated'), { rated: teacherProgress.rated, total: teacherProgress.total })))),
            ...
            h('section', { 'aria-labelledby': 'gis-review-rubric-heading', style: panel },
              h('h2', { id: 'gis-review-rubric-heading', style: { margin: '0 0 6px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_accessible_review_rubric', 'Accessible review rubric')),
              h('div', { style: { display: 'grid', gap: 9 } }, GIS_TEACHER_RUBRIC.map(function (item) {
                return h('div', { key: item.id, style: { display: 'grid', gridTemplateColumns: 'minmax(170px,1fr) minmax(120px,160px)', gap: 10, alignItems: 'center', padding: 10, borderRadius: 9, background: '#071827' } },
                  ...
                  h('label', { style: { display: 'grid', gap: 4, fontSize: 10, fontWeight: 800 } }, __alloT('stem.gisstudio.ui_rating_0_3', 'Rating (0-3)'),
                    h('select', { value: teacherReview.ratings[item.id], onChange: function (event) { updateTeacherRating(item.id, event.target.value); }, style: control },
                      h('option', { value: 0 }, __alloT('stem.gisstudio.ui_0_not_yet_reviewed', '0 - not yet reviewed')),
                      h('option', { value: 1 }, __alloT('stem.gisstudio.ui_1_emerging', '1 - emerging')),
                      h('option', { value: 2 }, __alloT('stem.gisstudio.ui_2_solid', '2 - solid')),
                      h('option', { value: 3 }, __alloT('stem.gisstudio.ui_3_strong', '3 - strong'))
                    )
                  )
                );
              })),
              h('label', { style: { display: 'grid', gap: 5, marginTop: 12, fontSize: 11, fontWeight: 800 } }, __alloT('stem.gisstudio.ui_overall_feedback', 'Overall feedback'), h('textarea', { value: teacherReview.overall, maxLength: 2000, rows: 5, onChange: function (event) { updateTeacherReviewField('overall', event.target.value); }, placeholder: __alloT('stem.gisstudio.ui_name_the_strongest_reasoning_move_and_one_thin', 'Name the strongest reasoning move and one thing to revise.'), style: Object.assign({}, control, { resize: 'vertical' }) })),
              h('label', { style: { display: 'grid', gap: 5, marginTop: 9, fontSize: 11, fontWeight: 800 } }, __alloT('stem.gisstudio.ui_next_revision', 'Next revision'), h('textarea', { value: teacherReview.nextRevision, maxLength: 1500, rows: 3, onChange: function (event) { updateTeacherReviewField('nextRevision', event.target.value); }, placeholder: __alloT('stem.gisstudio.ui_what_should_the_learner_check_add_or_reconside', 'What should the learner check, add, or reconsider next?'), style: Object.assign({}, control, { resize: 'vertical' }) }))),
            h('section', { 'aria-labelledby': 'gis-review-export-heading', style: panel },
              h('h2', { id: 'gis-review-export-heading', style: { margin: '0 0 6px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_export_and_continue', 'Export and continue')),
              h('p', { style: { margin: '0 0 10px', color: '#b7d2df', fontSize: 11, lineHeight: 1.5 } }, __alloT('stem.gisstudio.ui_the_standalone_review_is_useful_for_a_conferen', 'The standalone review is useful for a conference; the Investigation Packet carries the same review beside the evidence and quality safeguards.')),
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap' } }, h('button', { type: 'button', onClick: downloadTeacherReview, style: primary }, __alloT('stem.gisstudio.ui_download_teacher_review', 'Download teacher review')), h('button', { type: 'button', onClick: printTeacherReview, style: Object.assign({}, primary, { background: '#155e75' }) }, __alloT('stem.gisstudio.ui_print_or_save_as_pdf', 'Print or save as PDF')), h('button', { type: 'button', onClick: function () { setTab('packet'); persist('gisTab', 'packet'); }, style: Object.assign({}, control, { cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_open_investigation_packet', 'Open Investigation Packet')))));
        }
        ...
        function projectView() {
          ...
          return h('div', { style: { display: 'grid', gap: 14 } },
            ...
            h('section', { 'aria-labelledby': 'gis-project-heading', style: panel },
              h('p', { style: { margin: 0, color: '#fde68a', fontSize: 10, fontWeight: 900, letterSpacing: '.09em' } }, __alloT('stem.gisstudio.ui_portable_classroom_work', 'PORTABLE CLASSROOM WORK')),
              h('h2', { id: 'gis-project-heading', style: { margin: '4px 0 6px', color: '#f0fdfa', fontSize: 20 } }, __alloT('stem.gisstudio.ui_save_reopen_and_recover_projects', 'Save, reopen, and recover projects')),
              h('p', { style: { margin: 0, color: '#b7d2df', fontSize: 12, lineHeight: 1.55 } }, __alloT('stem.gisstudio.ui_project_files_preserve_normalized_datasets_lay', 'Project files preserve normalized datasets, layers, analysis settings, timelines, mission work, and provenance. Raw pasted CSV text is intentionally excluded.')),
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(230px,1fr))', gap: 10, marginTop: 13 } },
                h('label', { style: { display: 'grid', gap: 5, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_project_name', 'Project name'),
                ...
                h('label', { style: { display: 'grid', gap: 5, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_open_gis_studio_project', 'Open GIS Studio project'),
              ...
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 12 } },
                h('button', { type: 'button', onClick: downloadProjectFile, style: primary }, __alloT('stem.gisstudio.ui_download_project_file', 'Download project file')),
                h('button', { type: 'button', onClick: downloadMappedCSV, style: Object.assign({}, control, { cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_download_mapped_csv', 'Download mapped CSV')),
                h('button', { type: 'button', onClick: downloadGeoJSONLayer, disabled: !geoData, style: Object.assign({}, control, { cursor: geoData ? 'pointer' : 'not-allowed', opacity: geoData ? 1 : 0.55 }) }, __alloT('stem.gisstudio.ui_download_geojson_layer', 'Download GeoJSON layer')),
                h('button', { type: 'button', onClick: function () { setTab('map'); persist('gisTab', 'map'); }, style: Object.assign({}, control, { cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_return_to_map', 'Return to map'))),
            ...
            h('section', { 'aria-labelledby': 'gis-provenance-heading', style: panel },
              h('h2', { id: 'gis-provenance-heading', style: { margin: '0 0 5px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_data_provenance_manifest', 'Data provenance manifest')),
              h('p', { style: { margin: '0 0 11px', color: '#a7c7d8', fontSize: 11 } }, __alloT('stem.gisstudio.ui_these_fields_travel_with_the_project_and_help', 'These fields travel with the project and help another reader judge whether the data and comparisons are appropriate.')),
            ...
            h('section', { 'aria-labelledby': 'gis-project-inventory-heading', style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 12 } },
              h('div', { style: panel },
                h('h2', { id: 'gis-project-inventory-heading', style: { margin: '0 0 8px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_project_inventory', 'Project inventory')),
                ...
                h('h3', { style: { margin: '13px 0 6px', color: '#67e8f9', fontSize: 12 } }, __alloT('stem.gisstudio.ui_recorded_transformations', 'Recorded transformations')),
                h('ul', { style: { margin: 0, paddingLeft: 18, color: '#dbeafe', fontSize: 11, lineHeight: 1.7 } },
              ...
              h('div', { style: panel },
                ...
                h('label', { style: { display: 'grid', gap: 5, fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_round_point_coordinates_to', 'Round point coordinates to'),
                  h('select', { value: privacyDigits, onChange: function (event) { setPrivacyDigits(Number(event.target.value)); }, style: control },
                    ...
                    h('option', { value: 3 }, __alloT('stem.gisstudio.ui_3_decimal_places_about_100_m', '3 decimal places (about 100 m)')),
                    h('option', { value: 4 }, __alloT('stem.gisstudio.ui_4_decimal_places_about_10_m', '4 decimal places (about 10 m)')))),
                h('button', { type: 'button', onClick: roundPrivateCoordinates, disabled: !importedRows.length && !timeDataset.rows.length, style: Object.assign({}, primary, { marginTop: 10, opacity: importedRows.length || timeDataset.rows.length ? 1 : 0.55 }) }, __alloT('stem.gisstudio.ui_round_imported_timeline_points', 'Round imported + timeline points')),
                h('p', { style: { margin: '10px 0 0', color: '#fcd34d', fontSize: 10, lineHeight: 1.45 } }, __alloT('stem.gisstudio.ui_rounding_is_applied_to_the_current_in_memory_p', 'Rounding is applied to the current in-memory point datasets and will be reflected in future autosaves and downloads. GeoJSON boundaries are not changed. Review names and attributes separately.')),
                ...
                h('div', { style: { marginTop: 12, paddingTop: 10, borderTop: '1px solid #2c5468' } },
                  h('h3', { style: { margin: '0 0 5px', color: '#f0fdfa', fontSize: 13 } }, __alloT('stem.gisstudio.ui_what_leaves_this_device', 'What leaves this device')),
            ...
            h('aside', { style: { padding: 12, borderLeft: '4px solid #f59e0b', background: '#2b2617', color: '#fde68a', borderRadius: 8, fontSize: 11, lineHeight: 1.5 } },
              h('strong', null, __alloT('stem.gisstudio.ui_before_sharing', 'Before sharing: ')), __alloT('stem.gisstudio.ui_project_files_may_contain_precise_locations_na', 'Project files may contain precise locations, names, notes, and joined attributes. Confirm consent, remove identifiers, aggregate where possible, and open the saved file once to verify its contents.')));
        }
        ...
        function timelineView() {
          ...
          return h('div', { style: { display: 'grid', gap: 14 } },
            h('section', { 'aria-labelledby': 'gis-timeline-heading', style: panel },
              h('p', { style: { margin: 0, color: '#fde68a', fontSize: 10, fontWeight: 900, letterSpacing: '.09em' } }, __alloT('stem.gisstudio.ui_change_over_time', 'CHANGE OVER TIME')),
              h('h2', { id: 'gis-timeline-heading', style: { margin: '4px 0 6px', color: '#f0fdfa', fontSize: 20 } }, __alloT('stem.gisstudio.ui_time_series_change_lab', 'Time-Series Change Lab')),
              ...
              h('p', { style: { margin: 0, color: '#b7d2df', fontSize: 12, lineHeight: 1.55 } }, __alloT('stem.gisstudio.ui_compare_the_same_locations_across_years_keep_t', 'Compare the same locations across years. Keep the baseline fixed, move or play the focus year, and distinguish measured change from explanations that require more evidence.')),
              h('details', { style: { marginTop: 11, color: '#cfe8f3', fontSize: 11 } },
                h('summary', { style: { cursor: 'pointer', fontWeight: 800, color: '#67e8f9' } }, __alloT('stem.gisstudio.ui_import_a_time_series_csv', 'Import a time-series CSV')),
                h('p', null, __alloT('stem.gisstudio.ui_required_headers_name_latitude_longitude_year', 'Required headers: name, latitude, longitude, year, value. Optional: unit, source, method. Up to 3,000 valid rows stay in this browser session.')),
                h('label', { style: { display: 'grid', gap: 5, margin: '9px 0', fontWeight: 700 } }, __alloT('stem.gisstudio.ui_choose_csv_file', 'Choose CSV file'),
                ...
                h('label', { style: { display: 'grid', gap: 5, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_or_paste_time_series_csv', 'Or paste time-series CSV'),
                ...
                h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 9 } },
                  h('button', { type: 'button', onClick: loadTimeSeries, style: primary }, __alloT('stem.gisstudio.ui_load_time_series', 'Load time series')),
            ...
            h('section', { 'aria-labelledby': 'gis-time-controls-heading', style: panel },
              ...
              h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 11 } },
                h('button', { type: 'button', onClick: function () { setTimeFocusYear(timeYears[Math.max(0, focusIndex - 1)]); setTimePlaying(false); }, disabled: focusIndex <= 0, style: Object.assign({}, control, { cursor: focusIndex <= 0 ? 'not-allowed' : 'pointer' }) }, __alloT('stem.gisstudio.ui_previous_year', 'Previous year')),
                h('button', { type: 'button', onClick: toggleTimelinePlayback, style: primary }, timePlaying ? 'Pause animation' : 'Play timeline'),
                h('button', { type: 'button', onClick: function () { setTimeFocusYear(timeYears[Math.min(timeYears.length - 1, focusIndex + 1)]); setTimePlaying(false); }, disabled: focusIndex >= timeYears.length - 1, style: Object.assign({}, control, { cursor: focusIndex >= timeYears.length - 1 ? 'not-allowed' : 'pointer' }) }, __alloT('stem.gisstudio.ui_next_year', 'Next year')),
                h('button', { type: 'button', onClick: sonifyTemporalChange, disabled: !temporalComplete.length, style: Object.assign({}, primary, { background: '#083344', border: '1px solid #22d3ee', opacity: temporalComplete.length ? 1 : 0.55 }) }, __alloT('stem.gisstudio.ui_sonify_changes', '♫ Sonify changes'))),
            ...
            h('section', { 'aria-labelledby': 'gis-change-summary-heading', style: panel },
              h('div', { style: { display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'flex-start', flexWrap: 'wrap' } },
                h('div', null,
                  h('h2', { id: 'gis-change-summary-heading', style: { margin: '0 0 5px', color: '#f0fdfa', fontSize: 16 } }, __alloT('stem.gisstudio.ui_accessible_change_summary', 'Accessible change summary')),
                  h('p', { style: { margin: 0, color: '#cfe8f3', fontSize: 12, lineHeight: 1.5 } }, temporalSummary)),
                h('button', { type: 'button', onClick: downloadTimeEvidence, style: primary }, __alloT('stem.gisstudio.ui_download_change_evidence_report', 'Download change evidence report'))),
              ...
              h('label', { style: { display: 'grid', gap: 6, marginTop: 12, color: '#e6fffb', fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_change_claim_or_observation', 'Change claim or observation'),
            ...
            h('aside', { style: { padding: 12, borderLeft: '4px solid #f59e0b', background: '#2b2617', color: '#fde68a', borderRadius: 8, fontSize: 11, lineHeight: 1.5 } },
              h('strong', null, __alloT('stem.gisstudio.ui_compatibility_check', 'Compatibility check: ')), __alloT('stem.gisstudio.ui_a_numeric_difference_is_meaningful_only_when_y', 'A numeric difference is meaningful only when years use compatible definitions, units, geography, collection methods, and coverage. Missing records are excluded from ranked change.')));
        }
          ...
          return h('div', { style: { display: 'grid', gap: 14 } },
            ...
            h('section', { role: 'tabpanel', 'aria-labelledby': 'gis-active-mission-heading', style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 14 } },
              h('div', { style: Object.assign({}, panel, { padding: 18 }) },
                ...
                h('label', { style: { display: 'grid', gap: 6, marginTop: 15, color: '#e6fffb', fontSize: 12, fontWeight: 700 } }, __alloT('stem.gisstudio.ui_evidence_response', 'Evidence response'),
                  ...
                  h('textarea', {
                    ...
                    rows: 6, placeholder: __alloT('stem.gisstudio.ui_claim_evidence_reasoning_limitation', 'Claim... Evidence... Reasoning... Limitation...'),
                ...
                h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 11 } },
                  h('button', { type: 'button', onClick: function () { downloadMissionEvidence(activeMission); }, style: primary }, __alloT('stem.gisstudio.ui_download_mission_evidence_report', 'Download mission evidence report')),
                  h('button', { type: 'button', onClick: function () { setTab('missions'); persist('gisTab', 'missions'); }, style: Object.assign({}, control, { cursor: 'pointer' }) }, __alloT('stem.gisstudio.ui_stay_with_mission_guide', 'Stay with mission guide')))),
              h('aside', { 'aria-labelledby': 'gis-teacher-lens-heading', style: Object.assign({}, panel, { alignSelf: 'start' }) },
                h('h2', { id: 'gis-teacher-lens-heading', style: { margin: '0 0 7px', color: '#86efac', fontSize: 15 } }, __alloT('stem.gisstudio.ui_teacher_lens', 'Teacher lens')),
                h('p', { style: { color: '#cfe8f3', fontSize: 11, lineHeight: 1.55 } }, activeMission.teacherNote),
                h('h3', { style: { color: '#67e8f9', fontSize: 12, margin: '13px 0 6px' } }, __alloT('stem.gisstudio.ui_curricular_practices', 'Curricular practices')),
                ...
                h('h3', { style: { color: '#67e8f9', fontSize: 12, margin: '13px 0 6px' } }, __alloT('stem.gisstudio.ui_quick_evidence_rubric', 'Quick evidence rubric')),
                h('ol', { style: { margin: 0, paddingLeft: 18, color: '#dbeafe', fontSize: 11, lineHeight: 1.6 } },
                  h('li', null, __alloT('stem.gisstudio.ui_names_a_defensible_spatial_pattern', 'Names a defensible spatial pattern.')),
                  h('li', null, __alloT('stem.gisstudio.ui_cites_mapped_or_tabular_evidence', 'Cites mapped or tabular evidence.')),
                  h('li', null, __alloT('stem.gisstudio.ui_explains_a_method_or_spatial_relationship', 'Explains a method or spatial relationship.')),
                  h('li', null, __alloT('stem.gisstudio.ui_names_a_meaningful_limitation', 'Names a meaningful limitation.'))),
                h('p', { style: { margin: '13px 0 0', padding: 9, borderLeft: '4px solid #f59e0b', background: '#2b2617', color: '#fde68a', fontSize: 10, lineHeight: 1.45 } },
                  __alloT('stem.gisstudio.ui_the_included_values_and_simplified_regions_are', 'The included values and simplified regions are for learning. Do not use them for public policy or resource-allocation decisions.')))));
        }
        ...
        function projectionView() {
          ...
          return h('section', { 'aria-labelledby': 'gis-projection-heading', style: Object.assign({}, panel, { maxWidth: 930, margin: '0 auto', padding: 18 }) },
            h('p', { style: { margin: 0, color: '#67e8f9', fontSize: 11, fontWeight: 800 } }, __alloT('stem.gisstudio.ui_coordinates_distortion', 'COORDINATES + DISTORTION')),
            h('h2', { id: 'gis-projection-heading', style: { color: '#f0fdfa', margin: '5px 0 8px' } }, __alloT('stem.gisstudio.ui_projection_lab', 'Projection lab')),
            h('p', { style: { color: '#b7d2df', lineHeight: 1.6, fontSize: 13 } }, __alloT('stem.gisstudio.ui_earth_is_curved_screens_are_flat_every_project', 'Earth is curved; screens are flat. Every projection preserves some relationships and distorts others.')),
            h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 18 } },
              h('div', null,
                h('label', { style: { display: 'grid', gap: 5, fontSize: 12, fontWeight: 700, marginBottom: 15 } }, __alloT('stem.gisstudio.ui_projection', 'Projection'),
              ...
              h('svg', { viewBox: '0 0 560 300', role: 'img', 'aria-label': projection + ' distortion diagram at ' + display.coordinate(latitude, 0, 'lat') + '. Area factor ' + display.number(factor, 2), style: { width: '100%', minHeight: 280, background: '#071827', borderRadius: 12 } },
                ...
                h('text', { x: 280, y: 286, textAnchor: 'middle', fill: '#a7c7d8', fontSize: 12 }, __alloT('stem.gisstudio.ui_tissot_indicatrix_one_equal_patch_of_ground', 'Tissot indicatrix: one equal patch of ground')))),
            h('p', { style: { color: '#b7d2df', fontSize: 12, lineHeight: 1.5 } },
              h('strong', { style: { color: '#86efac' } }, __alloT('stem.gisstudio.ui_choose_intentionally', 'Choose intentionally: ')),
              __alloT('stem.gisstudio.ui_mercator_for_local_direction_equirectangular_f', 'Mercator for local direction, equirectangular for simple coordinate grids, and equal-area for choropleth comparisons.')));
        }
