...
const examples = ['elementary','middle','high'].flatMap(band=>library.creative.map(item=>({band,item})));
const reports = path.join(root, 'reports/sel-goals-creative');
let browser, page;
let errors = [];
async function mount(band = 'middle', theme = 'light', width = 1100, initial = {}) {
  ...
  await page.evaluate(({ band, theme, initial }) => {
    ...
    function App() {
      const [data, setData] = R.useState({ goals_tool: { soundOn: false, tab: 'smart', showSmartExamples: true, smartExampleCat: 'creative', ...initial } });
...
describe('Goal Setter creative depth', () => {
  beforeAll(async () => {
    ...
    await page.route('**/*', route => route.request().url().startsWith('http://sel-depth.test/')
      ? route.fulfill({ contentType: 'text/html', body: '<!doctype html><html lang="en"><head><title>Goal Setter creative planning</title></head><body><div id="root"></div></body></html>' }) : route.abort());
  }, 60000);
  ...
  it.each(examples)('$band / $item.id explains context and creates an editable independent copy',async({band,item})=>{
    ...
    const beforeFeedback=await snapshot();
    const beforeXP=await page.evaluate(()=>window.depthXP);
    const feedback=guide().getByText('Ask for useful feedback (optional)',{exact:true});
    await feedback.focus();await page.keyboard.press('Enter');
    expect(await guide().innerText()).toContain(item.feedback[band].question);
    expect(await guide().innerText()).toContain(item.feedback[band].response);
    expect(await snapshot()).toEqual(beforeFeedback);
    expect(await page.evaluate(()=>window.depthXP)).toEqual(beforeXP);
  ...
  it('preserves old creative goals and saved example selections while browsing revised plans', async()=>{
    const oldGoal={...legacyGoal,category:'creative',text:'My saved writing goal',smart:{...legacyGoal.smart,S:'My previously chosen writing plan'}};
    await mount('middle','light',1100,{goals:[oldGoal],smartExampleSelections:{'middle:creative':'example-2','elementary:creative':'example-1'}});
  ...
  it('resets feedback disclosure and uses the chosen example and grade without leaking into other categories',async()=>{
    await mount();
    const feedback=()=>guide().getByText('Ask for useful feedback (optional)',{exact:true});
    await feedback().click();
    expect(await guide().innerText()).toContain(library.creative[0].feedback.middle.question);
    await chooser().selectOption('example-1');
    expect(await feedback().evaluate(node=>node.parentElement.open)).toBe(false);
    await feedback().click();
    expect(await guide().innerText()).toContain(library.creative[1].feedback.middle.question);
    expect(await guide().innerText()).not.toContain(library.creative[0].feedback.middle.question);
    await page.evaluate(()=>window.depthSetBand('elementary'));
    expect(await feedback().evaluate(node=>node.parentElement.open)).toBe(false);
    await feedback().click();
    expect(await guide().innerText()).toContain(library.creative[0].feedback.elementary.question);
    await guide().getByRole('button',{name:'Filter SMART examples by Health & Wellness',exact:true}).click();
  ...
  it.each(['light','dark','contrast'])('supports readable phone plans and accessible controls in %s',async theme=>{
    ...
    await guide().getByText('Ask for useful feedback (optional)',{exact:true}).click();
    ...
    await guide().getByText('Ask for useful feedback (optional)',{exact:true}).evaluate(node=>node.scrollIntoView({block:'start'}));
    await page.screenshot({path:path.join(reports,theme+'-feedback-phone.png')});
