...
const scene = readFileSync(resolve(root, 'life_skills_laundry/life_skills_laundry.html'), 'utf8');
const publicScene = readFileSync(resolve(root, 'desktop/web-app/public/life_skills_laundry/life_skills_laundry.html'), 'utf8');
...
describe('Life Skills 3D laundry and clothing care lab', () => {
  it('defines six clothing-care targets and three practice modes', () => {
    const literal = scene.match(/var TASKS = (\[[\s\S]*?\]);/);
    expect(literal).not.toBeNull();
    const tasks = Function('return ' + literal[1])();
    expect(tasks.map((task) => task.id)).toEqual(['sort', 'label', 'detergent', 'stain', 'cycle', 'lint']);
  ...
  it('keeps the 3D view optional and provides an accessible checklist fallback', () => {
    ...
    expect(scene).toContain('laundry-interactive');
    expect(scene).toContain('raycaster="objects: .clickable');
    expect(scene).toContain('Accessible laundry checklist');
    ...
    expect(scene).toContain('alloflow-life-laundry-3d-why-reveal');
    expect(scene).toContain('function pauseForHelp()');
    expect(scene).toContain('alloflow-life-laundry-3d-help-pause');
    ...
    expect(scene).toContain('alloflow-life-laundry-3d-confidence');
    ...
    expect(scene).toContain('alloflow-life-laundry-3d-replay-complete');
    expect(scene).toContain('data-task-id');
    expect(scene).toContain('alloflow-life-laundry-3d-next-step');
    expect(scene).toContain('alloflow-life-laundry-3d-reflection');
  ...
  it('launches the lab from the existing Laundry tab', () => {
    expect(lifeSkills).toContain('function openLifeSkillsLaundry3D()');
    expect(lifeSkills).toContain('/life_skills_laundry/life_skills_laundry.html');
    expect(lifeSkills).toContain('Open the 3D laundry and clothing care lab in a new window');
    expect(lifeSkills).toContain('Open 3D laundry lab');
  });

  it('copies the laundry lab alongside the shared local A-Frame runtime', () => {
    expect(build).toContain("'immersive_geometry'");
    expect(build).toContain("'life_skills_laundry'");
  });
});
