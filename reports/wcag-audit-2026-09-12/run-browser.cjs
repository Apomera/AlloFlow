...
(async()=>{
 ...
 results.blockedOrigins=[...blocked];results.pageErrors=errors;results.finishedAt=new Date().toISOString();fs.writeFileSync(path.join(__dirname,'browser-audit.json'),JSON.stringify(results,null,2));await browser.close();await new Promise(r=>server.close(r));
})();
