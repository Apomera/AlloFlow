...
(async () => {
  ...
  const result = await page.evaluate(async ({ html, words, png }) => {
    ...
    try {
      ...
      const fixResult = {
        isScanned: true, accessibleHtml: html,
        groundTruthPages: [{ pageNum: 1, pageW: 612, pageH: 792, text: words.map((w) => w.text).join(' '), words: words }],
      };
  ...
  }, { html: ACCESSIBLE_HTML, words: WORDS, png: PNG_1x1 });
