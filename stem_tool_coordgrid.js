...
// Extracted from stem_lab_module.js L8632-8931
// ═══════════════════════════════════════════
  ...
  window.StemLab.registerTool('coordinate', {
    ...
    render: function(ctx) {
      ...
      var handleGridClick = function(e) {
        ...
        var x = fromSvg(e.clientX - rect.left, 'x');
        var y = fromSvg(e.clientY - rect.top, 'y');
      ...
      var lineElements = gridLines.map(function(ln, li) {
        return h(React.Fragment, { key: 'ln' + li },
          ...
          h('line', { x1: toSvg(ln.to.x, 'x'), y1: toSvg(ln.from.y, 'y'), x2: toSvg(ln.to.x, 'x'), y2: toSvg(ln.to.y, 'y'), stroke: '#ef4444', strokeWidth: 1.5, strokeDasharray: '3,2', opacity: 0.5 }),
          h('line', { x1: toSvg(ln.from.x, 'x'), y1: toSvg(ln.from.y, 'y'), x2: toSvg(ln.to.x, 'x'), y2: toSvg(ln.from.y, 'y'), stroke: '#3b82f6', strokeWidth: 1.5, strokeDasharray: '3,2', opacity: 0.5 }),
          h('rect', { x: (toSvg(ln.from.x, 'x') + toSvg(ln.to.x, 'x')) / 2 - 24, y: (toSvg(ln.from.y, 'y') + toSvg(ln.to.y, 'y')) / 2 - 10, width: 48, height: 18, rx: 5, fill: '#6366f1', opacity: 0.9 }),
          h('text', { x: (toSvg(ln.from.x, 'x') + toSvg(ln.to.x, 'x')) / 2, y: (toSvg(ln.from.y, 'y') + toSvg(ln.to.y, 'y')) / 2 + 3, textAnchor: 'middle', fill: '#fff', style: { fontSize: '9px', fontWeight: 'bold' } }, 'm=' + ln.slope.display)
        );
      });
      ...
      var slopeChallengeElements = slopeChallenge && gridChallenge.p1
        ? h(React.Fragment, null,
            h('line', { x1: toSvg(gridChallenge.p1.x, 'x'), y1: toSvg(gridChallenge.p1.y, 'y'), x2: toSvg(gridChallenge.p2.x, 'x'), y2: toSvg(gridChallenge.p2.y, 'y'), stroke: '#f59e0b', strokeWidth: 2.5 }),
            h('circle', { cx: toSvg(gridChallenge.p1.x, 'x'), cy: toSvg(gridChallenge.p1.y, 'y'), r: 6, fill: '#f59e0b', stroke: '#fff', strokeWidth: 2 }),
            h('circle', { cx: toSvg(gridChallenge.p2.x, 'x'), cy: toSvg(gridChallenge.p2.y, 'y'), r: 6, fill: '#f59e0b', stroke: '#fff', strokeWidth: 2 })
          )
        : null;
      ...
      return h('div', { className: 'space-y-4 max-w-3xl mx-auto animate-in fade-in duration-200' },
        ...
        h('div', { className: 'bg-white rounded-xl border-2 border-cyan-200 p-4 flex justify-center' },
          h('svg', { width: gridW, height: gridH, onClick: handleGridClick, className: 'cursor-crosshair', style: { background: '#f8fafc' } },
            gridElements,
            lineElements,
        ...
        connectMode && h('div', { className: 'bg-indigo-50 rounded-lg p-3 border border-indigo-200' },
          ...
          h('p', { className: 'text-xs text-indigo-600' }, connectFirst != null ? 'Click a second point to draw a line.' : 'Click a point to start.'),
          gridLines.length > 0 && h('div', { className: 'mt-2 space-y-1' },
            gridLines.map(function(ln, li) {
              return h('div', { key: li, className: 'flex items-center gap-2 text-[10px] bg-white rounded px-2 py-1 border' },
                ...
                h('span', { className: 'ml-auto font-bold text-indigo-800' }, 'm=' + ln.slope.display)
              );
            })
          ),
