'use strict';
// Browser acceptance check for navigation through recorded changes.
// Run: node dev-tools/watercycle_pilot_review_navigation_qa.cjs
const fs = require('fs');
const path = require('path');
const assert = require('node:assert/strict');
const { chromium } = require('playwright');
const ROOT = process.cwd();
const out = path.join(ROOT, 'scratch', 'water-review-navigation');
...
(async () => {
  ...
  try {
    ...
    const earlier=page.getByRole('button',{name:'Earlier recorded observation',exact:true});
    const later=page.getByRole('button',{name:'Later recorded observation',exact:true});
    assert(await earlier.isDisabled());assert(await later.isEnabled());
    const origin=(await data()).pilot.reviewReturnId;
    const reviewTime=await canvas.getAttribute('data-parcel-elapsed');
    await later.click();
    await page.waitForFunction(()=>document.activeElement.id==='wcPilotNoticeTitle');
    ...
    assert((await page.locator('.wc-pilot-review-nav').innerText()).includes('2 of 2'));
    assert(await later.isDisabled());assert(await earlier.isEnabled());
    assert.equal((await data()).pilot.reviewReturnId,origin);
    assert.equal(await canvas.getAttribute('data-parcel-elapsed'),reviewTime,'Browsing records does not advance physics');
    assert.deepEqual((await data()).pilot.lastChange,before.lastChange);
    assert.deepEqual((await data()).pilot.notebookChanges,before.notebookChanges);
    await earlier.click();
    await page.waitForFunction(()=>document.activeElement.id==='wcPilotNoticeTitle');
    assert.equal(await notice.getAttribute('data-change-id'),'review-old');
    assert((await notice.innerText()).includes('Energy released'));
    assert((await page.locator('.wc-pilot-review-nav').innerText()).includes('1 of 2'));
    ...
    for(const [width,dark] of [[1280,false],[390,false],[320,false],[1280,true]]) {
      ...
      const nav=await page.locator('.wc-pilot-review-nav').boundingBox();
      assert(reading.y+reading.height<=nav.y+1,'Navigation does not cover evidence');
      assert(nav.y+nav.height<=actions.y+1,'Review actions remain below navigation');
      assert(await page.locator('.wc-pilot-notice-reading').evaluate(n=>n.scrollHeight<=n.clientHeight+1),'The complete example explanation and prompt fit without scrolling');
      for(const name of ['Earlier recorded observation','Later recorded observation']) assert((await page.getByRole('button',{name,exact:true}).boundingBox()).height>=44,'44px navigation target');
    ...
    console.log('PASS: earlier/later controls, boundary states, original focus return, frozen physics and evidence, older/current recorded review, original climate and measurements, detached evidence, real pause/resume, close focus return, unchanged history, reset cleanup, 320/390px controls, and light/dark accessibility.');
