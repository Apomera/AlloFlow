// GIS Studio β€” the offline schematic map, and the egress it exists to avoid.
//
...
// pan or zoom tells a third party which area the class is viewing β€” which for
...
describe('GIS Studio β€” offline schematic projection', () => {
  ...
  it('corrects longitude by cos(latitude) rather than stretching high-latitude data', () => {
    ...
    // one degree of longitude covers far less ground at 60Β°N, so it must occupy less
...
describe('GIS Studio β€” no-basemap mode is genuinely no-egress', () => {
  beforeEach(() => resetStemLab());
  ...
  it('draws the schematic instead, labelled honestly and described for screen readers', () => {
    ...
    expect(html).toMatch(/Schematic β€” not a projected navigation map/);
  ...
  it('keeps the choropleth honest offline β€” same classes and colours as the Leaflet path', () => {
    const t = load();
...
describe('GIS Studio"ιέyψ§yΤ schematic geometry decomposition', () => {
  beforeEach(() => resetStemLab());
