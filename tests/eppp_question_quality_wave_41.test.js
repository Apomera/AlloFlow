...
const wave = require('../dev-tools/eppp_native_quality_wave_41_data.cjs');
...
describe('EPPP expanded quality wave 41', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_41.json');
    ...
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(435);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1210);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(585);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(573);
    expect(audit.summary.feedbackWarningsAfter.choiceRestatementOptions).toBe(136);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_41.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_41.json'));
  });

  it('replays after wave 40', () => {
    const builder = read('dev-tools/build_eppp_1500_expansion.cjs');
    const i40 = builder.indexOf('repair_eppp_native_quality_wave_40.cjs');
    const i41 = builder.indexOf('repair_eppp_native_quality_wave_41.cjs');
