...
const handleSubmitLiveAnswer = async (payload) => {
      if (payload && payload.requireConfirmation === true) {
          ...
          return guest.sendConfirmedResponse(payload.questionIdx, {
              ...
              timestamp: payload.timestamp || Date.now(), submitted: true, assessmentActivityId: activityId,
...
const handleQuizQuestionAction = async (qIndex, action, payload = null) => {
    ...
    if (action === 'release-feedback') {
        ...
        const release = { resourceId: String(__d.generatedContent.id || ''), signature: payload.signature, releasedAt: Date.now() };
        if (__d.activeSessionCode) {
        ...
        const data = { ...__d.generatedContent.data, deliverySettings: { ...__d.generatedContent.data.deliverySettings, feedbackReleasedFor: payload.signature } };
        __d.setGeneratedContent({ ...__d.generatedContent, data });
        __d.setHistory(prev => prev.map(item => item.id === __d.generatedContent.id ? { ...item, data } : item));
        return true;
    }
    const newData = { ...__d.generatedContent.data };
