...
(async()=>{
 ...
 await page.screenshot({path:'reports/video-studio-enhancement/start-desktop.png',fullPage:false});
 ...
 await page.screenshot({path:'reports/video-studio-enhancement/editor-desktop.png'});
 ...
 await download.saveAs('reports/video-studio-enhancement/export-fixture.webm');
 ...
 await page.screenshot({path:'reports/video-studio-enhancement/editor-mobile.png'});
 await page.locator('#trimStart').scrollIntoViewIfNeeded();await page.screenshot({path:'reports/video-studio-enhancement/trim-mobile.png'});
 assert.deepEqual(errors,[]);
 fs.writeFileSync('reports/video-studio-enhancement/browser-results.json',JSON.stringify({passed:true,checks:['hidden recovery banners','specialist workflow disclosure','decimal trim typing','empty states','source-dependent camera controls','recording before advanced tools','real WebM import','per-take trims','bounded trim input','trim at playhead and reset','draft recovery after reload','narration and visual workflow visibility','prepare and download actual video','mobile overflow','no page errors'],errors},null,2));
