...
beforeAll(() => {
  ...
  pack = Hub.listPacks()
    .find((candidate) => candidate.id === 'praxis-school-psychologist-5403');
});
...
describe('Praxis School Psychologist 5403 diagnostic bank', () => {
  it('registers five ready 100-item batches with the exact ETS 32/23/20/25 diagnostic blueprint', () => {
    ...
    expect(pack).toMatchObject({
      status: 'ready',
      batchSize: 100,
      simulationItemCount: 125,
      simulationTimeMinutes: 125,
    ...
    // Honest tiers: 200 source-reviewed items, 100 independently authored
    // and reviewed items, and 200 guided transformations.
    expect(pack.items.filter((item) => item.qaStatus === 'qa-passed')).toHaveLength(200);
    expect(pack.items.filter(
      (item) => item.qaStatus === 'qa-passed-independent-practice-item',
    )).toHaveLength(100);
    expect(pack.items.filter(
      (item) => item.qaStatus === 'structural-qa-passed-guided-practice-only',
    )).toHaveLength(200);
    expect(pack.items.filter(
      (item) => item.qaStatus === 'structural-qa-passed-guided-practice-only',
    ).every(
      (item) => item.reviewStatus === 'assistant-reviewed-guided-practice-only',
    )).toBe(true);
    expect(Object.fromEntries(pack.domains.map((domain) => [domain.id, domain.weight])))
      .toEqual({
    ...
    for (let batchIndex = 0; batchIndex < 3; batchIndex += 1) {
      ...
      for (const [domainId, count] of Object.entries(expected)) {
        expect(batch.filter((item) => item.domainId === domainId)).toHaveLength(count);
      }
      expect(batch.reduce((counts, item) => {
        counts[item.answerIndex] += 1;
        return counts;
      }, [0, 0, 0, 0])).toEqual([25, 25, 25, 25]);
    }
    expect(Hub.batchMeta(pack, 100)).toMatchObject({
      batchNumber: 2,
      batchCount: 5,
      position: 1,
      startIndex: 100,
  ...
  it('keeps every activity distinct, fully explained, candidly reviewed, and linked to one compatible chapter', () => {
    const prompts = pack.items
      .map((item) => item.prompt.toLowerCase().replace(/\s+/g, ' ').trim());
    ...
    for (const item of pack.items) {
      ...
      ]).toContain(item.reviewStatus + ' | ' + item.qaStatus);
  ...
  it('builds domain and confidence diagnostics without inventing a scaled score or credential result', () => {
    ...
    const answers = Object.fromEntries(
      firstBatch.map((item) => [item.id, item.answerIndex]),
    );
    const confidence = Object.fromEntries(
      firstBatch.map((item) => [item.id, 'sure']),
    );
    const missed = [
    ...
    const rows = Object.fromEntries(
      diagnostic.domainRows.map((row) => [row.id, row]),
    );
    ...
    expect(rows['permeating-practices']).toMatchObject({
      correct: 31,
      total: 32,
      missed: 1,
    });
    expect(rows['student-level-services']).toMatchObject({
      correct: 22,
      total: 23,
      missed: 1,
    });
    expect(diagnostic.confidentMissQuestionNumbers)
      .toEqual(missed.map((item) => pack.items.indexOf(item) + 1));
  ...
  it('uses safety-first and legally cautious guidance for high-risk and evaluation scenarios', () => {
    const selfHarm = pack.items
      .find((item) => item.prompt.includes('specific suicide plan'));
    const iee = pack.items
      .find((item) => item.prompt.includes('independent educational evaluation'));
    const evaluation = pack.items
      .find((item) => item.prompt.includes('IDEA require of evaluation procedures'));
    expect(selfHarm.choices[selfHarm.answerIndex]).toContain('Maintain supervision');
    expect(selfHarm.choices[selfHarm.answerIndex])
      .toContain('activate the current school crisis protocol');
    expect(iee.choices[iee.answerIndex]).toContain('Without unnecessary delay');
    expect(iee.choices[iee.answerIndex])
      .toContain('provide the IEE at public expense or file due process');
    expect(evaluation.choices[evaluation.answerIndex])
      .toContain('never rely on one measure as the sole criterion');
  });
  ...
  it('publishes zero-finding QA and exact deployment mirrors', () => {
    ...
    expect(qa.summary).toMatchObject({
      ...
      newIndependentItemsNeeded: 300,
      findings: 0,
      status: 'pass',
    });
    expect(qa.blueprint).toMatchObject({
    ...
    for (const name of [
      'school_psychologist_5403_items.json',
      'school_psychologist_5403_pack.json',
      'school_psychologist_5403_native_qa.json',
      'school_psychologist_5403_native_qa.md',
    ]) {
      expect(read('desktop/web-app/public/test_prep/' + name))
        .toBe(read('test_prep/' + name));
    }
  }, 20000);
});
