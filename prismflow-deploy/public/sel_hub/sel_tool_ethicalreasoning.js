    ...
    render: function(ctx) {
      ...
      return h('div', { className: 'space-y-4 animate-in fade-in duration-200' },
        ...
        tab === 'branching' && h('div', { className: 'space-y-4' },
          ...
          d.branchingId && (function() {
            ...
            return h('div', { className: 'space-y-4' },
              // Back button
              h('button', { 'aria-label': 'Back to all scenarios', onClick: function() { updMulti({ branchingId: null, branchChoice: null, branchReflection: '', branchAIDiscussion: null }); }, className: 'text-xs text-slate-600 hover:text-slate-600 font-bold' }, '\u2190 All Scenarios'),
              ...
              h('div', { className: 'bg-gradient-to-r from-emerald-50 to-teal-50 rounded-2xl border-2 border-emerald-200 p-5' },
                ...
                callTTS && h('button', { 'aria-label': 'Read scenario aloud', onClick: function() { callTTS(sc.scenario); }, className: 'mt-2 text-[10px] text-emerald-600 hover:text-emerald-800 font-bold' }, '\uD83D\uDD0A Read Aloud')
              ),
              ...
              !d.branchChoice && h('div', { className: 'space-y-2' },
                ...
                sc.choices.map(function(ch, idx) {
                  return h('button', { 'aria-label': 'Select this choice', key: idx, onClick: function() {
        ...
        tab === 'explore' && selectedDilemma && h('div', { className: 'space-y-4' },
          ...
          h('div', { className: 'bg-gradient-to-r from-slate-50 to-indigo-50 rounded-2xl border-2 border-slate-200 p-5' },
            h('div', { className: 'flex items-center gap-2 mb-3' },
              ...
              h('button', { 'aria-label': 'Back to all dilemmas', onClick: function() { updMulti({ dilemmaId: null, tab: 'dilemmas', frameworkAnalysis: null }); }, className: 'ml-auto text-xs text-slate-600 hover:text-slate-600 font-bold' }, '\u2190 All Dilemmas')
            ),
            ...
            callTTS && h('button', { 'aria-label': 'Read dilemma aloud', onClick: function() { callTTS(gradeBand === 'elementary' && selectedDilemma.gradeBands.elementary ? selectedDilemma.gradeBands.elementary : selectedDilemma.scenario); }, className: 'mt-2 text-[10px] text-indigo-500 hover:text-indigo-700 font-bold' }, '\uD83D\uDD0A Read Aloud')
          ),
        ...
        tab === 'explore' && !selectedDilemma && h('div', { className: 'bg-slate-50 border-2 border-dashed border-slate-300 rounded-2xl p-8 text-center' },
          ...
          h('button', { 'aria-label': 'Go to dilemmas', onClick: function() { upd('tab', 'dilemmas'); }, className: 'mt-3 px-4 py-2 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700' }, 'Go to Dilemmas')
        ),
          ...
          d.kohlbergStarted && !d.kohlbergComplete && (function() {
            ...
            return h('div', { className: 'space-y-4' },
              ...
              h('div', { className: 'bg-white rounded-2xl border-2 border-purple-200 p-5' },
                ...
                callTTS && h('button', { 'aria-label': 'Read scenario aloud', onClick: function() { callTTS(sc.scenario); }, className: 'text-[10px] text-purple-500 hover:text-purple-700 font-bold mb-3 block' }, '\uD83D\uDD0A Read Aloud'),
        ...
        tab === 'debate' && h('div', { className: 'space-y-4' },
          ...
          d.debateTopicObj && h('div', { className: 'space-y-4' },
            ...
            !d.debateSide && h('div', { className: 'flex gap-3' },
              h('button', { 'aria-label': 'Argue for this position', onClick: function() { upd('debateSide', 'for'); ctx.awardXP(3); },
            ...
            d.debateSide && !d.debateFeedback && h('div', { className: 'space-y-4' },
              h('div', { className: 'bg-white rounded-xl border border-slate-200 p-4' },
                h('div', { className: 'flex items-center gap-2 mb-2' },
                  ...
                  h('button', { 'aria-label': 'Switch debate side', onClick: function() { upd('debateSide', d.debateSide === 'for' ? 'against' : 'for'); },
        ...
        tab === 'values' && h('div', { className: 'space-y-4' },
          ...
          d.valuesStarted && !d.valuesComplete && !(d.valuesRanked && d.valuesRanked.length === 5) && (function() {
            ...
            return h('div', { className: 'space-y-4' },
              ...
              selected.length === 5 && h('button', { 'aria-label': 'Rank my values', onClick: function() {
          ...
          d.dtStarted && !d.decisionTreeComplete && (function() {
            ...
            return h('div', { className: 'space-y-4' },
              ...
              h('div', { className: 'flex gap-2' },
                ...
                h('div', { className: 'flex-1' }),
                step < steps.length - 1 && h('button', { 'aria-label': 'Next step', onClick: function() {
                ...
                step === steps.length - 1 && d[currentStep.field] && d[currentStep.field].length > 10 && h('button', { 'aria-label': 'Complete exercise', onClick: function() {
                  // Generate summary
