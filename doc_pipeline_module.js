...
Return ONLY the complete HTML document.`;
    ...
    if (batchMaxFix > 0 && (curAxeResults && curAxeResults.totalViolations > 0 || bestAiScore < 90)) {
      for (let fp = 0; fp < batchMaxFix; fp++) {
        ...
        const [rv1, rv2, rv3, rAxe] = await Promise.all([auditOutputAccessibility(accessibleHtml), auditOutputAccessibility(accessibleHtml), auditOutputAccessibility(accessibleHtml), runAxeAudit(accessibleHtml)]);
        ...
        const rv = [rv1, rv2, rv3].filter(Boolean).sort((a, b) => (b.score || 0) - (a.score || 0))[0] || null;
        if (rv) {
          rv.score = newAi;
          rv._sem = rvSEM;
          rv._sd = rvSD;
    ...
    return {
      ...
      autoFixPasses,
      needsExpertReview,
      axeViolations: curAxeResults ? curAxeResults.totalViolations : 0,
      elapsed,
...
Return ONLY the complete fixed HTML.`, true);
  ...
  const fixAndVerifyPdf = async (batchOverrides = null) => {
    ...
    if (!_base64) {
      console.warn('[fixAndVerifyPdf] EARLY EXIT: no _base64');
      addToast('⚠️ Cannot fix: PDF data not found in memory. Please re-upload the PDF.', 'error');
      return null;
    }
    if (!_isBatch && !_auditResult) {
      console.warn('[fixAndVerifyPdf] EARLY EXIT: no _auditResult');
      addToast('⚠️ Cannot fix: No audit results found. Please run the audit first.', 'error');
      return null;
    }
    if (!_isBatch) {
