...
(async () => {
    ...
    for (const config of [
      ...
      await page.evaluate(({ config, strings, propNames }) => {
        ...
        Object.assign(props, {
          ...
          isSocialStoryMode: !!config.social, socialStoryFocus: 'Resolving disagreements', enableFactionResources: !!config.systems,
      ...
      await page.screenshot({ path: path.join(__dirname, config.name + '.png'), fullPage: true });
      results.push({ name: config.name, errors, ...data }); fs.writeFileSync(path.join(__dirname, config.name + '.json'), JSON.stringify(results.at(-1), null, 2));
  ...
  } finally { await browser.close(); fs.writeFileSync(path.join(__dirname, 'findings.json'), JSON.stringify(results, null, 2)); }
