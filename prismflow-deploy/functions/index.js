/**
 * Optional AlloFlow Firebase search service.
 *
 * This file intentionally exports only searchProxy. Institutional endpoints
...
async function enforceRateLimit(uid) {
  ...
  const ref = db.collection("_alloflowRateLimits").doc("search_" + key);
  const now = Date.now();
  await db.runTransaction(async (transaction) => {
    ...
    if (now - windowStart < RATE_WINDOW_MS && count >= SEARCH_REQUESTS_PER_MINUTE) {
