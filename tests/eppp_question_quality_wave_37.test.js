...
const wave = require('../dev-tools/eppp_native_quality_wave_37_data.cjs');
...
describe('EPPP expanded quality wave 37', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_37.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 24, applicationItems: 12, analysisItems: 12, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    ...
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicatePairs).toBe(52);
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicateClusters).toBe(34);
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(455);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1265);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(587);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(624);
    expect(audit.summary.feedbackWarningsAfter.choiceRestatementOptions).toBe(145);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_37.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_37.json'));
  });

  it('replays after wave 36', () => {
    const builder = read('dev-tools/build_eppp_1500_expansion.cjs');
    const i36 = builder.indexOf('repair_eppp_native_quality_wave_36.cjs');
    const i37 = builder.indexOf('repair_eppp_native_quality_wave_37.cjs');
    expect(i37).toBeGreaterThan(i36);
  });
});
