...
{prereqGaps && prereqGaps.missing.length > 0 && <div className="mt-2 rounded border border-amber-300 bg-amber-50 p-2 text-xs" role="note" aria-label="Prerequisite gaps from the knowledge graph">
  ...
  function EngagementSection(p) {
    ...
    return <ComprehensiveSection id="audit-engagement" icon="🎯" title="Engagement variety" status={e.status}> // Top stat row
      ...
      // LLM review
      e.llmReview && <div className="p-3 bg-indigo-50 border border-indigo-200 rounded mb-3"><div className="text-xs font-semibold text-indigo-900 mb-2 flex items-center gap-2"><span aria-hidden="true">🤖</span> UDL + DOK review (AI)</div>{auditText(e.llmReview.narrative) && <p className="text-sm text-indigo-900 mb-2">{auditText(e.llmReview.narrative)}</p>}{e.llmReview.dokAssessment && <p className="text-sm text-indigo-900 mb-2 italic">{'"' + e.llmReview.dokAssessment + '"'}</p>}{e.llmReview.formatGaps && e.llmReview.formatGaps.length > 0 && <div><div className="text-xs font-semibold text-indigo-800 mb-1">Suggested format additions:</div><ul className="list-disc ml-5 text-sm text-indigo-900 space-y-1">{e.llmReview.formatGaps.map(function (g, i) {
  ...
  function AccuracySection(p) {
    ...
    return <ComprehensiveSection id="audit-accuracy" icon="✅" title="Content accuracy" status={a.status}> // Top stat row
      ...
      a.llmReview && <div className="p-3 bg-indigo-50 border border-indigo-200 rounded mb-3"><div className="text-xs font-semibold text-indigo-900 mb-2 flex items-center gap-2"><span aria-hidden="true">🤖</span> Fact-checker review (AI)</div>{auditText(a.llmReview.narrative) && <p className="text-sm text-indigo-900 mb-3">{auditText(a.llmReview.narrative)}</p>}{a.llmReview.claimsToVerify && a.llmReview.claimsToVerify.length > 0 && <div className="mb-3"><div className="text-xs font-semibold text-indigo-800 mb-1">Specific claims to double-check:</div><ul className="list-disc ml-5 text-sm text-indigo-900 space-y-1">{a.llmReview.claimsToVerify.map(function (c, i) {
  ...
  function DifferentiationSection(p) {
    ...
    return <ComprehensiveSection id="audit-differentiation" icon="🎚️" title="Differentiation coverage" status={d.status}><div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4"><div className="p-3 bg-slate-50 rounded text-center"><div className="text-2xl font-bold text-slate-800">{d.coverage + '%'}</div><div className="text-xs text-slate-600">Access-path coverage</div></div><div className="p-3 bg-slate-50 rounded text-center"><div className="text-2xl font-bold text-slate-800">{d.presentCount + '/' + d.totalAccommodationTypes}</div><div className="text-xs text-slate-600">Scored access paths</div></div><div className="p-3 bg-emerald-50 rounded text-center border border-emerald-200"><div className="text-2xl font-bold text-emerald-800">{d.presentCount}</div><div className="text-xs text-emerald-700">Present</div></div><div className={'p-3 rounded text-center ' + (d.missing && d.missing.length > 0 ? 'bg-rose-50 border border-rose-200' : 'bg-slate-50 border border-slate-200')}><div className={'text-2xl font-bold ' + (d.missing && d.missing.length > 0 ? 'text-rose-800' : 'text-slate-800')}>{d.missing && d.missing.length || 0}</div><div className={'text-xs ' + (d.missing && d.missing.length > 0 ? 'text-rose-700' : 'text-slate-600')}>Missing</div></div></div> // Inventory grid
      <div className="grid grid-cols-2 md:grid-cols-3 gap-1.5 mb-3">{Object.keys(flags).map(function (k) {
          ...
          return <div key={k} className={'flex items-center gap-2 text-xs px-2 py-1.5 rounded border ' + (on ? 'bg-emerald-50 border-emerald-200 text-emerald-900' : 'bg-slate-50 border-slate-200 text-slate-500')}><span aria-hidden="true" className="flex-shrink-0">{on ? '✓' : '○'}</span><span className="min-w-0 break-words">{labelMap[k] || k}</span></div>;
            ...
            return <li key={i}>{auditText(r)}</li>;
          })}</ul></div>}{d.llmReview && <div className="p-3 bg-indigo-50 border border-indigo-200 rounded mb-3"><div className="text-xs font-semibold text-indigo-900 mb-2 flex items-center gap-2"><span aria-hidden="true">🤖</span> UDL specialist review (AI)</div>{auditText(d.llmReview.narrative) && <p className="text-sm text-indigo-900 mb-2">{auditText(d.llmReview.narrative)}</p>}{d.llmReview.priorityAdditions && d.llmReview.priorityAdditions.length > 0 && <div className="mb-2"><div className="text-xs font-semibold text-indigo-800 mb-1">Priority additions:</div><ul className="list-disc ml-5 text-sm text-indigo-900 space-y-1">{d.llmReview.priorityAdditions.map(function (r, i) {
              return <li key={i}>{auditText(r)}</li>;
  ...
  function CognitiveLoadSection(p) {
    ...
    return <ComprehensiveSection id="audit-cognitiveLoad" icon="⏱️" title="Cognitive load / pacing" status={c.status}><div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4"><div className="p-3 bg-slate-50 rounded text-center"><div className="text-2xl font-bold text-slate-800">{c.claimedTotalMinutes + 'm'}</div><div className="text-xs text-slate-600">Claimed</div></div><div className="p-3 bg-slate-50 rounded text-center"><div className="text-2xl font-bold text-slate-800">{c.estimatedTotalMinutes + 'm'}</div><div className="text-xs text-slate-600">Estimated</div></div><div className={'p-3 rounded text-center bg-' + ratioColor + '-50 border border-' + ratioColor + '-200'}><div className={'text-2xl font-bold text-' + ratioColor + '-800'}>{c.ratio === null ? 'n/a' : c.ratio + 'x'}</div><div className={'text-xs text-' + ratioColor + '-700'}>Estimated / Claimed</div></div><div className="p-3 bg-slate-50 rounded text-center"><div className="text-xs font-bold text-slate-800 leading-tight pt-1">{'R: ' + b.reading + 'm · Q: ' + b.quiz + 'm · A: ' + b.activities + 'm'}</div><div className="text-[10px] text-slate-500 mt-1">{'Reading at ' + b.wpmAssumption + ' wpm'}</div></div></div>{
      ...
      Array.isArray(c.perSegment) && c.perSegment.length > 0 && <div className="mb-3"><div className="text-xs font-semibold text-slate-700 mb-1">Lesson plan segments:</div><ul className="space-y-0.5 text-sm text-slate-700">{c.perSegment.map(function (s, i) {
            return <li key={i} className="flex items-center justify-between gap-2"><span>{s.label}</span><span className={'font-mono text-xs ' + (s.claimedMinutes !== null ? 'text-slate-800' : 'text-slate-600 italic')}>{s.claimedMinutes !== null ? s.claimedMinutes + ' min' : '(no time given)'}</span></li>;
  ...
  function ExecutiveSummary(p) {
    ...
    return <div className="p-5 rounded-2xl border-2 mb-6 shadow-md max-w-4xl mx-auto" style={{
      ...
      borderColor: statusClr.ring
        ...
        }} className="flex-shrink-0 px-4 py-2 rounded-lg bg-amber-700 hover:bg-amber-800 text-white text-sm font-bold shadow-md hover:shadow-lg transition-all flex items-center gap-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-700 focus-visible:ring-offset-2" title="Generate a Pre-Check Quiz that probes the prerequisites the audit identified as gaps. Students take the quiz before the lesson; missed concepts get just-in-time AI explainers."><span aria-hidden="true">🎯</span> Pre-Check from gaps</button>}</div>{
  ...
  function computeAuditFreshness(generatedContent, history) {
    ...
    var present = {};
    var added = [], modified = [], unverified = 0;
    var fingerprints = comp.auditScope?.artifactFingerprints || {};
    (comp.auditScope?.includedArtifacts || []).forEach(item=>{ if (item.contentFingerprint && !fingerprints[item.id]) fingerprints = { ...fingerprints, [item.id]:item.contentFingerprint }; });
    history.forEach(function (item) {
      if (!item || !item.type || FRESHNESS_IGNORED_TYPES[item.type]) return;
      var id = String(item.id || '');
      if (id) present[id] = true;
      if (id && included[id]) {
        if (fingerprints[id] && auditResourceFingerprint(item)) { if (fingerprints[id] !== auditResourceFingerprint(item)) modified.push(item.title || item.type); }
        else { const changedAt=Date.parse(item.updatedAt || item.timestamp || ''); if (!isNaN(changedAt) && changedAt>generatedAt) modified.push(item.title || item.type); else unverified++; }
        return;
      }
