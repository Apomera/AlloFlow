...
const createTTS = deps => {
  ...
  const callTTSDirect = async (text, voiceName = "Puck", speed = 1, maxRetries = 2) => {
    ...
    var _botKokoroEligible = typeof voiceName === 'string' && KOKORO_VOICE_PREFIX.test(voiceName) || !_isCanvasEnv && !apiKey && !_botProviderHandles && typeof voiceName === 'string' && voiceName !== 'browser';
