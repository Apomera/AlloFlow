...
for(const theme of ['light','dark','contrast'])for(const id of ['openBim','organismId']){
 const file=id==='openBim'?'openbim':'organismid',selector=id==='openBim'?'[data-design-trials]':'[data-taxonomy-observation]';
 ...
 try{
  ...
  if(id==='openBim'){
   ...
   assert(text.includes('Next evidence to seek:'));assert(text.includes('Evidence review: Your key decisions match'));
  }
  await page.locator(selector).scrollIntoViewIfNeeded();
  await page.screenshot({path:path.join(OUT,file+'-'+theme+'-desktop.png'),animations:'disabled'});
  const sizes=[];
  for(const width of [375,320]){
   await page.setViewportSize({width,height:812});await page.locator(selector).scrollIntoViewIfNeeded();
