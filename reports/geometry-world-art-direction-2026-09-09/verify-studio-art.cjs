...
harness+='('+(async function(){
 ...
 try{
   ...
   for(const config of[{width:1180,height:860,look:'Studio',postFx:false,file:'studio-art-desktop.png'},{width:390,height:844,look:'Studio',postFx:true,file:'studio-art-phone.png'}]){
     ...
     await page.screenshot({path:path.join(out,config.width>500?'studio-art-ui-desktop.png':'studio-art-ui-phone.png'),timeout:60000});
   ...
   assert.equal(stage.resources,8);assert.ok(stage.footprints>0);assert.equal(stage.shaders.length,0);results.checks.stage=stage;
 ...
 }catch(e){results.failure=e.stack;process.exitCode=1;await page.screenshot({path:path.join(out,'studio-art-failure.png'),timeout:60000}).catch(()=>{});}
 finally{writeFile(path.join(out,'studio-art-results.json'),Buffer.from(JSON.stringify(results,null,2)));console.log(JSON.stringify(results,null,2));await browser.close();await new Promise(r=>server.close(r));}
