...
const slug='proportional_relationships_grade7',folder='allopacks/media/'+slug+'/',pack=read('allopacks/illustrated/'+slug+'.allopack.json'),original=read('allopacks/'+slug+'.allopack.json'),manifest=read(folder+'manifest.json');
require('../dev-tools/proportional_content_refinements.cjs').refine(original);
...
describe('proportional_relationships_grade7 illustrated edition',()=>{
...
expect(svg('taco')).toContain('data-slope="3"');expect(svg('taco')).toContain('data-intercept="0"');expect(svg('offset')).toContain('data-intercept="3"');expect(get('math').data.problems[4].steps[2].explanation).toContain('y = 3x + 5');expect(get('quiz').data.questions[0].correctAnswer).toContain('nonzero');expect(get('simplified').data).toContain('do not calculate 0 divided by 0');
});
