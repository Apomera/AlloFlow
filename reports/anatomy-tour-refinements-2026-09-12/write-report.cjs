...
const tests=JSON.parse(fs.readFileSync(__dirname+'/full-tests.json','utf8')),browser=JSON.parse(fs.readFileSync(__dirname+'/browser-results.json','utf8')),validation=JSON.parse(fs.readFileSync(__dirname+'/validation-summary.json','utf8'));
assert.equal(tests.success,true);assert.equal(tests.numFailedTests,0);assert.equal(browser.axe.flatMap(r=>r.violations).length,0);assert.equal(browser.axe.flatMap(r=>r.incomplete).length,0);assert.equal(browser.errors.length,0);
const root='C:/Users/cabba/OneDrive/Desktop/UDL-Tool-Updated/',here=root+'reports/anatomy-tour-refinements-2026-09-12/',link=(title,file)=>'['+title+']('+here+file+')';
const report=`# Anatomy guided-tour refinements
...
Each step now has a concise, authored explanation and a direct OpenStax Anatomy and Physiology 2e reference. The 42 steps link to 30 distinct sections. The ${link('content and source index','tours.json')} records the structure ID, narration, recall clue, and source slug for each step.
...
Final source SHA-256: \`${validation.sha256}\`.

${link('Full test results','full-tests.json')} · ${link('Browser results','browser-results.json')} · ${link('Content and translation validation','validation-summary.json')}

${link('Phone tour step','phone-390-step.png')} · ${link('Endocrine thyroid step','desktop-thyroid-step.png')} · ${link('Arabic dark-theme recap','phone-arabic-dark-complete.png')}
...
`;
fs.writeFileSync(__dirname+'/README.md',report);console.log('Wrote verified tour refinement report.');
