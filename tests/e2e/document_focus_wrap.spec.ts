...
test.use({ viewport: { width: 700, height: 350 } });
...
async function focusedFacts(page: any, html: string, media: any = {}) {
...
test.describe('generated keyboard focus remains visible across document surfaces', () => {
  for (const [_name, background] of [
    ['light', '#ffffff'],
    ['image-control slate', '#475569'],
    ['dark', '#0f172a'],
    ['middle gray', '#777777'],
  ]) test('provides a solid contrasting focus band on ' + _name, async ({ page }) => {
    const facts = await focusedFacts(page, fixture(background));
  ...
  for (const colorScheme of ['light', 'dark']) test('retains a visible system-color outline in forced-colors ' + colorScheme, async ({ page }) => {
    const facts = await focusedFacts(page, fixture('#475569'), { forcedColors: 'active', colorScheme });
  ...
  test('upgrades the previous generated blue ring and remains idempotent', async ({ page }) => {
    ...
    const facts = await focusedFacts(page, twice);
    const surface = rgb(facts.background);
    const bands = [rgb(facts.outlineColor), ...(facts.boxShadow.match(/rgba?\([^)]+\)/g) || []).map(rgb)];
    expect(Math.max(...bands.map(color => contrast(composite(color, surface), surface)))).toBeGreaterThanOrEqual(3);
  });
...
test.describe('generated document footnotes reflow', () => {
  for (const width of [320, 390]) test('wraps the source footnote URLs inside a ' + width + 'px viewport', async ({ page }) => {
