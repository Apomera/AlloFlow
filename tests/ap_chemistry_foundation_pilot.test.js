...
describe('AP Chemistry internal foundation pilot', () => {
  it('crosswalks the current nine-unit blueprint without presenting itself as official', () => {
    ...
    expect(pack.version).toBe('0.5.0-internal-preview');
  ...
  it('contains a balanced 700-item pilot across all units and science practices', () => {
    ...
    expect(pack.items).toHaveLength(700);
    expect(new Set(pack.items.map((item) => item.id)).size).toBe(700);
    expect(Object.values(unitCounts)).toEqual([72, 70, 95, 70, 74, 80, 82, 91, 66]);
    expect(Object.keys(practiceCounts).sort()).toEqual(['SP1', 'SP2', 'SP3', 'SP4', 'SP5', 'SP6']);
    expect(answerCounts).toEqual({ 0: 175, 1: 175, 2: 175, 3: 175 });
    expect(pack.sections).toHaveLength(140);
  ...
  it('adds a 100-item capstone layer centered on data interpretation and argumentation', () => {
    const capstone = pack.items.filter((item) => {
      const itemNumber = Number(item.id.match(/-(\d{3})$/)?.[1]);
  ...
  it('adds a 100-item synthesis layer centered on model translation and method design', () => {
    const synthesis = pack.items.filter((item) => {
      const itemNumber = Number(item.id.match(/-(\d{3})$/)?.[1]);
      return itemNumber >= 601 && itemNumber <= 700;
    });
    const synthesisPractices = countsBy(synthesis.map((item) => item.practiceId));

    expect(synthesis).toHaveLength(100);
    expect(synthesisPractices).toEqual({ SP1: 50, SP2: 50 });
    expect(synthesis.every((item) => item.stimulus && item.editorialChecks.scenarioBased)).toBe(true);
  });
  ...
  it('provides nine native chapters with structured lessons and study aids', () => {
    ...
    expect(library.contentMigration).toMatchObject({ contentVersion: 'ap-chemistry-foundation-v4', richLessonPrototypes: 9 });
    expect(library.blueprint.pilotAlignment).toMatch(/700-item/);
  ...
  it('passes deterministic QA and binds byte-identical deployment mirrors', () => {
    ...
    expect(qa.metrics).toMatchObject({ itemCount: 700, unitCount: 9, topicCount: 91, chapterCount: 9, richLessonPrototypeCount: 9, nearDuplicatePairCount: 0 });
    ...
    expect(entry).toMatchObject({ loadMode: 'lazy', visibility: 'internal', itemCount: 700, domainCount: 9 });
