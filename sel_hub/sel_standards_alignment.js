...
(function() {
  ...
  var ALIGNMENTS = {
    ...
    'somaticReset': {
      ...
      other: [
        { framework: 'Trauma-Informed Practice', area: 'choice, eyes-open options, a compact native visual picker, a one-cycle motion preview before timing begins, a predictable direction-marked linear visual, a two-part cyclic guide with non-color phase patterns, active stroke-weight emphasis, a phase-shaped direction marker, and a handoff marker, screen-reader phase cues, a quiet focus view with a native visual start/pause control, a phase-only cue, optional countdown and guidance-word display, user-controlled motion, still alternatives, optional ratings, no-count and no-visual modes, and opt-out' },
      ...
      crewPrompt: 'Use the compact visual picker to keep every guide available without a dense button grid. The Breath Orbit pairs a solid inhale arc with a dotted exhale arc, makes the active phase bolder, changes its traveling marker from a circle on inhale to a diamond on exhale, and adds a clockwise chevron plus handoff marker, so the cycle does not depend on color alone. Offer full, phase-only, or hidden guidance words. Let students sample one breath of visual motion before timing begins, or choose Still. Quiet View makes the enlarged visual a direct start/pause control. Offer the direction-marked linear guide and screen-reader phase cues, plus hidden-countdown, hidden-guidance, still-motion, natural-breathing, no-visual, no-number, and gentle movement options. Students practice privately and may share one word or pass.'
    },
    'quietQuestions': {
