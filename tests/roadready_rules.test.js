...
describe('RoadReady rules-of-road content', () => {
  ...
  it('uses current Maine §2074 defaults for open-world speed-limit signs and enforcement', () => {
    for (const relPath of ROADREADY_FILES) {
      ...
      expect(src).toContain("addRoadSign('rect', 0xffffff, '15\\nMPH', 0x000000, 3);");
      expect(src).not.toContain("addRoadSign('rect', 0xffffff, '20\\nMPH', 0x000000, 3);");
...
describe('RoadReady road-local traffic invariants', () => {
  it.each(ROADREADY_FILES)('%s keeps curved-road traffic decisions in authored lane coordinates', (relPath) => {
    ...
    expect(src).toContain('var aiRoadStation = typeof t._roadStation === \'number\'');
    expect(src).toContain('var aiRoadHeading = aiwSp.headingAt(aiRoadStation);');
...
describe('RoadReady visual geometry invariants', () => {
  ...
  it.each(ROADREADY_FILES)('%s keeps roadside visuals outside the authored paved edge', (relPath) => {
    ...
    expect(src).toContain('function guardrailSideForRoad(spline, worldY)');
    expect(src).toContain('var generatedGuardrailSide = guardrailSideForRoad(spline, guardrailWorldY);');
    expect(src).toContain('chunk.cells[gry][generatedGuardrailX] = 9;');
    expect(src).toContain('var grSide = guardrailSideForRoad(iw.spline, grGridY);');
    expect(src).toContain('grRail.rotation.y = grHd;');
    expect(src).not.toContain('if (Math.abs(grHd) < 0.18) continue;');
  ...
  it.each(ROADREADY_FILES)('%s renders and samples a shared crowned road cross-section', (relPath) => {
    ...
    expect(src).toContain('function roadProfileAt(world, worldY, fallbackProfile)');
    expect(src).toContain('function createFreeExploreWorld(seed, roadType)');
    expect(src).toContain("if (selected === 'roundabout') return null;");
    ...
    expect(src).toContain('function roadRelativeTarget(world, observer, target)');
    ...
    expect(src).not.toContain('var sameLaneX = Math.abs((tc.x || 0) - car.x)');
    expect(src).not.toContain('var ahead = Math.abs(s.y - car.y)');
