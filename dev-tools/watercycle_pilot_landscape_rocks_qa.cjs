...
// Run: node dev-tools/watercycle_pilot_landscape_rocks_qa.cjs
const fs = require('fs');
const path = require('path');
const assert = require('node:assert/strict');
const { chromium } = require('playwright');
const ROOT = process.cwd();
const out = path.join(ROOT, 'scratch', 'water-landscape-rocks-review');
...
(async () => {
  ...
  try {
    ...
    const rocks=await page.evaluate(()=>{
      const scene=pilotReview.scene,rocks=scene.getObjectByName('pilot-landscape-rocks'),surfaces=[];
      scene.traverse(o=>{if(o.material?.pilotStreamChannelTexture===scene.getObjectByName('pilot-watershed-stream').userData.terrainMask)surfaces.push(o);});
      scene.updateMatrixWorld(true);const ray=new THREE.Raycaster(new THREE.Vector3(),new THREE.Vector3(0,-1,0));
      const matrix=new THREE.Matrix4(),p=new THREE.Vector3(),v=new THREE.Vector3();let floating=0,buried=0,missing=0,inLake=0;const sizes=[];
      for(let i=0;i<rocks.count;i++){
        rocks.getMatrixAt(i,matrix);p.setFromMatrixPosition(matrix);ray.ray.origin.set(p.x,500,p.z);const hits=ray.intersectObjects(surfaces,false);
        if(!hits.length){missing++;continue;}let min=Infinity,max=-Infinity;const vertices=rocks.geometry.attributes.position;
        for(let j=0;j<vertices.count;j++){v.fromBufferAttribute(vertices,j).applyMatrix4(matrix);min=Math.min(min,v.y);max=Math.max(max,v.y);}
        if(min>=hits[0].point.y)floating++;if(max<=hits[0].point.y)buried++;sizes.push(max-min);
        if(Math.hypot(p.x-268,p.z+104)<38)inLake++;
      }
      window.summerRockColor=rocks.material.color.getHex();
      return {count:rocks.count,floating,buried,missing,inLake,finite:Array.from(rocks.instanceMatrix.array).every(Number.isFinite),
        detailRange:Math.max(...sizes)/Math.min(...sizes),colors:!!rocks.instanceColor,sharedTexture:rocks.material.map===scene.getObjectByName('pilot-stream-bank-stones').material.map};
    });assert(rocks.count>40&&rocks.count<=114,JSON.stringify(rocks));assert.equal(rocks.floating,0);assert.equal(rocks.buried,0);assert.equal(rocks.missing,0);assert.equal(rocks.inLake,0);assert(rocks.finite&&rocks.colors&&rocks.sharedTexture&&rocks.detailRange>3);
    ...
    await page.evaluate(()=>{const renderer=pilotReview.renderer,render=renderer.render.bind(renderer);
      const rocks=pilotReview.scene.getObjectByName('pilot-landscape-rocks'),m=new THREE.Matrix4(),p=new THREE.Vector3();
      rocks.getMatrixAt(Math.min(6,rocks.count-1),m);p.setFromMatrixPosition(m);
      const nativeRender=THREE.WebGLRenderer.prototype.render;
      // The renderer's existing wrapper supplies pilotReview; set the camera last by wrapping its scene callback.
      window.rockDetailTarget=p.clone();
    });
    await page.evaluate(()=>{const renderer=pilotReview.renderer;
      // Use the scene's before-render hook after the fixed stream camera is applied.
      pilotReview.scene.onBeforeRender=(r,scene,camera)=>{const p=rockDetailTarget;camera.position.set(p.x+14,p.y+8,p.z+17);camera.lookAt(p.x,p.y-1,p.z);camera.updateMatrixWorld();};
    });
    await page.waitForTimeout(500);await canvas.screenshot({path:path.join(out,'rock-cluster-detail.jpg'),type:'jpeg',quality:82});
    ...
    assert.equal(await page.evaluate(()=>pilotReview.scene.getObjectByName('pilot-landscape-rocks').material.color.getHex()!==summerRockColor),true);
    ...
    assert.equal(await page.evaluate(()=>pilotReview.scene.getObjectByName('pilot-landscape-rocks').visible),false);
    ...
    await page.evaluate(()=>{
      window.streamDisposals={geometry:0,texture:0,channel:0,riffleGeometry:0,riffleMaterial:0,cascadeGeometry:0,cascadeMaterial:0,terrainGeometry:0,terrainMaterial:0,terrainMask:0,rockGeometry:0,rockMaterial:0};const stones=pilotReview.scene.getObjectByName('pilot-stream-bank-stones');
      ...
      const rocks=pilotReview.scene.getObjectByName('pilot-landscape-rocks');
      rocks.geometry.addEventListener('dispose',()=>streamDisposals.rockGeometry++);rocks.material.addEventListener('dispose',()=>streamDisposals.rockMaterial++);
      mountWater({wcMode:'explorer'});
    });assert.deepEqual(await page.evaluate(()=>streamDisposals),{geometry:1,texture:1,channel:1,riffleGeometry:1,riffleMaterial:1,cascadeGeometry:1,cascadeMaterial:1,terrainGeometry:1,terrainMaterial:1,terrainMask:1,rockGeometry:1,rockMaterial:1});assert.deepEqual(errors,[]);
    console.log('PASS: embedded terrain rocks, no floating or buried rocks, mixed rock sizes and colors, shared texture, seasonal palette, subsurface hiding, rock cleanup, terrain-bank edges match sampled land, finite geometry and normals, distinct wider terrain mask, seasonal bank colors, groundwater cutaway, terrain resource cleanup, cascade mesh aligned with water, quiet and whitewater reaches, bounded opacity strength, shared clock, cascade resource cleanup, bank-color and cross-channel attributes, finite riffles, shared water/riffle clock, winter suppression, finite stream detail, paused and reduced-motion ripples/flow, both learner views, real runoff-to-water transition, winter ice, desert visibility, mobile accessibility, shared resource disposal, no WebGL errors.');
