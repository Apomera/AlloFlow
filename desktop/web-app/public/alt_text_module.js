...
const OPEN_IMAGE_MAX_BYTES = 3e6;
...
function _oiPlainText(html, max) {
  const raw = _atString(html, 2e4).replace(/<[^>]*>/g, " ").replace(/&(#x[0-9a-f]+|#\d+|[a-z]+|#39);/gi, (whole, name) => {
...
const _oiHttps = (value) => /^https:\/\//i.test(_atString(value, 2e3)) ? _atString(value, 2e3) : "";
...
function normalizeCommonsPage(page) {
  ...
  return {
    ...
    categories: _atString(value("Categories"), 4e3).split("|").map((s) => s.trim()).filter(Boolean),
...
async function searchOpenImages(query, options) {
  ...
  let json;
  try {
    const response = await fetchImpl(OPEN_IMAGE_API + "?" + params.join("&"), opts.signal ? { signal: opts.signal } : void 0);
  ...
  pages.forEach((page) => {
    ...
    if (!candidate) return;
    if (!isClassroomLicense(candidate.license)) {
      excludedLicense++;
      return;
    }
    if (commonsMetadataBlocked(candidate)) {
      blockedByMetadata++;
      return;
    }
...
async function _oiFetchDataUrl(url, fetchImpl, signal) {
  const response = await fetchImpl(url, signal ? { signal } : void 0);
...
async function screenOpenImages(candidates, query, options) {
  ...
  const vision = typeof opts.callGeminiVision === "function" ? opts.callGeminiVision : typeof window !== "undefined" && typeof window.callGeminiVision === "function" ? window.callGeminiVision : null;
  ...
  try {
    for (let i = 0; i < list.length; i += batchSize) {
      ...
      for (let index = 0; index < batch.length; index++) {
        if (batchVerdicts) {
          verdictFor.push(batchVerdicts.get(index));
          continue;
        }
      ...
      batch.forEach((item, index) => {
        ...
        out.images.push({
          ...
          alt: verdict.alt,
          altSource: verdict.alt ? "vision" : "",
          attribution: item.attribution,
          creditLine: openImageCreditLine(item.attribution),
          source: "wikimedia"
        });
      });
    }
...
async function fetchClassroomImage(image, options) {
  const opts = options || {};
  const screened = _atString(image && image.dataUrl, 5e7);
  const fetchImpl = typeof opts.fetchImpl === "function" ? opts.fetchImpl : typeof fetch === "function" ? fetch : null;
  const vision = typeof opts.callGeminiVision === "function" ? opts.callGeminiVision : typeof window !== "undefined" && typeof window.callGeminiVision === "function" ? window.callGeminiVision : null;
  ...
  let dataUrl;
  try {
    dataUrl = await _oiFetchDataUrl(larger, fetchImpl, opts.signal);
  } catch (error) {
    if (error && error.name === "AbortError") throw error;
    return screened;
  }
...
function creditBandLines(ctx, width, attribution, creditLine) {
  const wrap = (text) => {
    ...
    _atString(text, 800).split(/\s+/).filter(Boolean).forEach((word) => {
      ...
      if (line && ctx.measureText(next).width > width - 16) {
        lines.push(line);
        line = word;
      } else line = next;
    });
...
async function bakeCreditBand(dataUrl, creditLine, options) {
  const opts = options || {};
  const src = _atString(dataUrl, 5e7);
...
async function cropImageBottom(dataUrl, pixels) {
  ...
  canvas.height = img.naturalHeight - band;
  const ctx = canvas.getContext("2d");
  if (!ctx) throw new Error("The picture could not be opened.");
...
async function shrinkImageDataUrl(dataUrl, maxSide, options) {
  ...
  const scale = opts.fitWidth ? Math.min(1, side / naturalWidth, side * 2 / naturalHeight) : Math.min(1, side / Math.max(naturalWidth, naturalHeight));
  const canvas = document.createElement("canvas");
...
async function searchMulberrySymbols(query, options) {
  ...
  let rows;
  try {
    const response = await fetchImpl(url, opts.signal ? { signal: opts.signal } : void 0);
...
function ClassroomImagePicker(props) {
  ...
  const stopWork = () => {
    epoch.current++;
    [controller, chooseController].forEach((ref) => {
  ...
  const run = async () => {
    ...
    const common = { language, fetchImpl, callGeminiVision, signal: current ? current.signal : void 0 };
    let result;
    try {
      result = tab === "photos" ? await findClassroomImages(q, common) : await searchMulberrySymbols(q, common).then((r) => ({ images: r.symbols, withheld: 0, unchecked: 0, error: r.error }));
    } catch (error) {
  ...
  const choose = async (item) => {
    ...
    try {
      const dataUrl = item.source === "mulberry" ? await _oiFetchDataUrl(item.svgUrl, fetchImpl || fetch, signal).catch((error) => {
        if (error && error.name === "AbortError") throw error;
        return item.svgUrl;
  ...
  const status = state.status === "checking" ? tr("images_checking", "Searching and checking each photo for classroom safety...") : state.status === "searching" ? tr("images_searching", "Searching...") : state.status !== "done" ? "" : state.error === "unavailable" ? tr("images_unavailable", "Photos need the AI safety check, which is not available right now, so none are shown.") : state.error === "network" ? tr("images_network", "Could not reach the image library. Check the connection and try again.") : state.error === "timeout" && state.items.length ? state.items.length === 1 ? tr("images_timeout_partial_one", "The search took too long, so it stopped early. Showing the 1 photo checked so far.") : tr("images_timeout_partial", "The search took too long, so it stopped early. Showing the {count} photos checked so far.", { count: state.items.length }) : state.error === "timeout" ? tr("images_timeout", "The search took too long, so it was stopped. Try again, or try a simpler word.") : !state.items.length ? tr("images_none", "No usable results. Try a simpler or more common word.") : state.items.length === 1 ? tr("images_found_one", "1 result.") : tr("images_found", "{count} results.", { count: state.items.length });
  const held = state.status === "done" && tab === "photos" && state.withheld + state.unchecked > 0 ? state.withheld + state.unchecked === 1 ? tr("images_withheld_one", "1 photo was not shown because it could not be confirmed as classroom-appropriate.") : tr("images_withheld", "{count} photos were not shown because they could not be confirmed as classroom-appropriate.", { count: state.withheld + state.unchecked }) : "";
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "rounded-xl border border-slate-300 bg-white p-3", "data-classroom-image-picker": "" }, allowed.length > 1 && /* @__PURE__ */ React.createElement("div", { role: "group", "aria-label": tr("images_source_label", "Image source"), className: "mb-2 flex flex-wrap gap-2" }, allowed.map((source) => /* @__PURE__ */ React.createElement("button", { key: source, type: "button", "aria-pressed": tab === source, onClick: () => {
    stopWork();
    setTab(source);
    setState({ status: "idle", items: [], withheld: 0, unchecked: 0, error: "" });
  }, className: tabClass(tab === source) }, source === "photos" ? tr("images_tab_photos", "Photos (Wikimedia Commons)") : tr("images_tab_symbols", "Symbols (Mulberry)")))), /* @__PURE__ */ React.createElement("form", { className: "flex flex-wrap items-end gap-2", onSubmit: (event) => {
    event.preventDefault();
    run();
  } }, /* @__PURE__ */ React.createElement("label", { htmlFor: base + "-query", className: "text-xs font-black text-slate-800" }, tab === "photos" ? tr("images_search_photos", "Search photos") : tr("images_search_symbols", "Search symbols")), /* @__PURE__ */ React.createElement(
    "input",
    {
      id: base + "-query",
      type: "search",
    ...
    {
      type: "submit",
      disabled: !query.trim() || state.status === "checking" || state.status === "searching",
  ...
  )), tab === "photos" && /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-[11px] leading-snug text-slate-600" }, tr("images_photos_help", "Only freely licensed photos are searched. Each one is checked by AI for classroom safety before it appears, and that check writes its description. Review before sharing with students.")), /* @__PURE__ */ React.createElement("p", { role: "status", "aria-live": "polite", className: "mt-2 text-xs font-bold text-slate-700" }, state.chooseError ? state.chooseError + " " : "", status, held ? " " + held : ""), state.items.length > 0 && /* @__PURE__ */ React.createElement("ul", { className: "mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3", "aria-label": tr("images_results", "Results") }, state.items.map((item) => /* @__PURE__ */ React.createElement("li", { key: item.id, className: "flex flex-col rounded-xl border border-slate-200 p-2" }, /* @__PURE__ */ React.createElement("img", { src: item.source === "mulberry" ? item.svgUrl : item.dataUrl, alt: item.alt || "", className: "h-28 w-full rounded-lg bg-slate-50 object-contain" }), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-[11px] leading-snug text-slate-700" }, item.creditLine), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      onClick: () => choose(item),
      disabled: !!choosing,
      "aria-busy": choosing === item.id,
