...
(async()=>{
 ...
 async function audit(name){
  ...
  results.states.push({name,...result});fs.writeFileSync(path.join(__dirname,'browser-workflows.json'),JSON.stringify(results,null,2));console.log(name+': '+result.violations.length+' axe rules; width '+result.scrollWidth+'/'+result.clientWidth);
 }
 try{
  for(const route of ['/app/']){
   ...
   await page.setViewportSize({width:320,height:800});await audit('full-platform-320');
   await page.addStyleTag({content:'* {line-height:1.5!important;letter-spacing:.12em!important;word-spacing:.16em!important} p {margin-bottom:2em!important}'});await audit('full-platform-320-spacing');
  }
 }catch(e){results.error=e.stack;console.error(e.stack);process.exitCode=1;}
 results.blockedOrigins=[...blocked];results.pageErrors=errors;results.finishedAt=new Date().toISOString();fs.writeFileSync(path.join(__dirname,'browser-workflows.json'),JSON.stringify(results,null,2));await browser.close();await new Promise(r=>server.close(r));
})();
