...
const SOURCE = path.join(ROOT, 'research_lane_humanities_source.jsx');
const OUTPUT = path.join(ROOT, 'research_lane_humanities_module.js');
const MIRROR = path.join(ROOT, 'prismflow-deploy', 'public', 'research_lane_humanities_module.js');

if (!fs.existsSync(SOURCE)) { console.error('Source not found:', SOURCE); process.exit(1); }

console.log('[ResearchLaneHumanities] Compiling ...');
try {
...
console.log(`[ResearchLaneHumanities] Built ${OUTPUT} (${cleaned.split('\n').length} lines)`);
console.log(`[ResearchLaneHumanities] Mirrored to ${MIRROR}`);
