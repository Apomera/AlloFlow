// Inject Chunk Read mood keyframes once. Reduced-motion media query disables
  ...
  function SimplifiedView(props) {
    ...
    return <div className="space-y-6">{activeReadAloudStatus && <span className="sr-only" role="status" aria-live="polite" aria-atomic="true">{activeReadAloudStatus}</span>}{isImmersiveReaderActive && generatedContent?.immersiveData && <div ref={immersiveDialogRef} role="dialog" aria-modal="true" aria-label={t('immersive.title') || 'Immersive Reader'} tabIndex={-1} onKeyDown={e => containSimplifiedModalFocus(e, immersiveDialogRef.current, handleCloseImmersiveReader)} className="fixed inset-0 z-[200] overflow-y-auto animate-in motion-reduce:animate-none fade-in zoom-in-95 duration-300 motion-reduce:animate-none motion-reduce:transition-none flex flex-col font-sans" style={{
        backgroundColor: immersiveSettings.bgColor || '#fdfbf7'
        ...
        }} />}{revisionData && <div ref={revisionDialogRef} role="dialog" aria-modal="true" aria-labelledby="simplified-revision-title" tabIndex={-1} onKeyDown={e => containSimplifiedModalFocus(e, revisionDialogRef.current, closeRevision)} className={`fixed ${_popupZ} bg-white p-4 rounded-xl shadow-2xl border border-indigo-200 w-72 max-h-[50vh] overflow-y-auto custom-scrollbar animate-in motion-reduce:animate-none zoom-in-95 duration-200 motion-reduce:animate-none motion-reduce:transition-none`} style={{
          ...
          left: Math.min(window.innerWidth - 300, Math.max(20, revisionData.x - 140)) + 'px'
            ...
            {!generatedContent.levelCheck && simplifiedComplexityDisplay.measuredGrade !== null && (() => {
          ...
          })()}<SourceReferencesPanel referencesText={simplifiedContentParts.references} />{isProcessing && <div className="mt-6 flex items-center justify-center gap-2 text-indigo-500 text-xs font-bold uppercase tracking-wider animate-pulse motion-reduce:animate-none opacity-80"><RefreshCw size={12} className="animate-spin motion-reduce:animate-none" /> Generating more...</div>}</div> : <div className={`w-full min-h-[500px] text-lg font-medium leading-relaxed font-sans prose prose-p:my-2 max-w-none ${cursorStyles[interactionMode]} transition-all duration-500 ease-in-out ${isLineFocusMode ? 'bg-slate-950 text-slate-600 p-8 rounded-2xl shadow-inner prose-invert' : 'text-slate-800 prose-headings:text-orange-900 prose-strong:text-orange-900'} ${getContentDirection(generatedContent?.config?.language || leveledTextLanguage) === 'rtl' ? 'text-right' : 'text-left'}`} style={{ maxWidth: 'min(72ch, 100%)', marginLeft: 'auto', marginRight: 'auto' }} dir={getContentDirection(generatedContent?.config?.language || leveledTextLanguage)}>{generatedContent?.data ? <div className="space-y-4">{(() => {
            ...
            })()}</div> : null}{isProcessing && <div className="mt-4 flex items-center gap-2 text-indigo-500 text-xs font-bold uppercase tracking-wider animate-pulse motion-reduce:animate-none opacity-80"><RefreshCw size={12} className="animate-spin motion-reduce:animate-none" /> Generating more...</div>}</div>}</div></div>;
  }
