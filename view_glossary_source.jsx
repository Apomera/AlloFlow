// Lazy Lucide icon resolution from window.AlloIcons (populated by
  ...
  function GlossaryView(props) {
    ...
    function renderGlossaryToolbar() {
      ...
      return <section aria-label="Glossary tools" className="mb-4 space-y-4 rounded-xl border border-blue-200 bg-blue-50 p-4 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
          ...
          <div className="relative w-full sm:max-w-sm" data-help-key="glossary_search">
            ...
            {glossarySearchTerm && <button type="button" onClick={() => setGlossarySearchTerm('')} className="absolute right-0 top-0 min-h-11 min-w-11 inline-flex items-center justify-center rounded-r-lg text-blue-700 hover:bg-blue-100 focus-visible:ring-2 focus-visible:ring-indigo-600" aria-label="Clear glossary search"><X size={16} aria-hidden="true" /></button>}
          </div>
        </div>
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
          <div className="rounded-lg border border-blue-200 bg-white/80 p-3" aria-labelledby="glossary-study-tools-title">
            <h2 id="glossary-study-tools-title" className="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-wider text-blue-800"><MonitorPlay size={16} aria-hidden="true" /> Study</h2>
            <div className="flex flex-wrap gap-2">
              <button type="button" data-help-key="glossary_standard_flashcards" onClick={() => launchInteractiveFlashcards('standard')} className={toolButton + ' bg-blue-700 text-white border-blue-700 hover:bg-blue-800'} aria-label="Study standard flashcards"><MonitorPlay size={16} aria-hidden="true" /> {t('flashcards.launch_standard')}</button>
              {selectedLanguages.length > 0 && <button type="button" data-help-key="glossary_language_flashcards" onClick={() => launchInteractiveFlashcards('language')} className={toolButton + ' bg-white text-indigo-700 border-indigo-300 hover:bg-indigo-50'} aria-label="Study language flashcards"><Languages size={16} aria-hidden="true" /> {t('flashcards.launch_language')}</button>}
            </div>
          </div>
          <div className="flex flex-wrap items-start gap-2">
            <button type="button" onClick={() => toggleGlossaryToolPanel('games')} aria-expanded={glossaryToolsOpen.games} aria-controls="glossary-games-tools" className={toolButton + ' bg-white text-indigo-800 border-indigo-300 hover:bg-indigo-50'}><Gamepad2 size={16} aria-hidden="true" /> Games <ChevronDown size={16} aria-hidden="true" className={glossaryToolsOpen.games ? 'rotate-180' : ''} /></button>
            {isTeacherMode && <button type="button" onClick={() => toggleGlossaryToolPanel('teacher')} aria-expanded={glossaryToolsOpen.teacher} aria-controls="glossary-teacher-tools" className={toolButton + ' bg-white text-slate-800 border-slate-300 hover:bg-slate-50'}><Pencil size={16} aria-hidden="true" /> Teacher tools <ChevronDown size={16} aria-hidden="true" className={glossaryToolsOpen.teacher ? 'rotate-180' : ''} /></button>}
          </div>
        </div>
        {glossaryToolsOpen.games && <div id="glossary-games-tools" className="rounded-lg border border-indigo-200 bg-indigo-50/70 p-3" aria-labelledby="glossary-games-tools-title"><h2 id="glossary-games-tools-title" className="mb-2 text-xs font-black uppercase tracking-wider text-indigo-800">Choose a game</h2><div className="flex flex-wrap gap-2">
          <div className="inline-flex items-stretch rounded-lg border border-slate-300 bg-white">{selectedLanguages.length > 0 && <select aria-label="Word search language" data-help-key="glossary_puzzle_lang" value={wordSearchLang} onChange={e => { setWordSearchLang(e.target.value); if (gameMode === 'wordsearch') generateWordSearch(e.target.value); }} className="min-h-11 rounded-l-lg border-0 border-r border-slate-300 bg-transparent px-3 text-sm font-bold text-teal-800 focus:ring-2 focus:ring-indigo-600"><option value="English">{t('common.english')}</option>{selectedLanguages.map(lang => <option key={lang} value={lang}>{lang}</option>)}</select>}<button type="button" aria-label={(gameMode === 'wordsearch' ? 'Regenerate' : 'Play') + ' word search'} data-help-key="glossary_word_search" onClick={() => generateWordSearch(wordSearchLang)} className="min-h-11 inline-flex items-center gap-2 px-3 py-2 text-sm font-bold text-teal-800 hover:bg-teal-50 focus-visible:ring-2 focus-visible:ring-indigo-600"><Gamepad2 size={16} aria-hidden="true" /> {gameMode === 'wordsearch' ? t('common.regenerate') : t('glossary.word_search')}</button></div>
          <button type="button" aria-label={'Play ' + t('glossary.memory_game')} data-help-key="glossary_memory_game" onClick={handleSetIsMemoryGameToTrue} className={secondaryButton}><Brain size={16} aria-hidden="true" /> {t('glossary.memory_game')}</button>
          <button type="button" aria-label={'Play ' + t('glossary.crossword')} data-help-key="glossary_crossword" onClick={handleSetIsCrosswordGameToTrue} className={secondaryButton}><Gamepad2 size={16} aria-hidden="true" /> {t('glossary.crossword')}</button>
          <button type="button" aria-label={'Play ' + t('glossary.matching')} data-help-key="glossary_matching" onClick={handleSetIsMatchingGameToTrue} className={secondaryButton}><GitMerge size={16} aria-hidden="true" /> {t('glossary.matching')}</button>
          <button type="button" aria-label={'Create ' + t('glossary.bingo') + ' cards'} data-help-key="glossary_bingo" onClick={handleSetIsBingoGameToTrue} className={secondaryButton}><Gamepad2 size={16} aria-hidden="true" /> {t('glossary.bingo')}</button>
          <button type="button" aria-label={'Play ' + t('glossary.play_bingo')} data-help-key="glossary_play_bingo" onClick={handleSetIsStudentBingoGameToTrue} className={secondaryButton}><Gamepad2 size={16} aria-hidden="true" /> {t('glossary.play_bingo')}</button>
          <button type="button" aria-label={'Play ' + t('glossary.scramble')} data-help-key="glossary_scramble" onClick={handleSetIsWordScrambleGameToTrue} className={secondaryButton}><Shuffle size={16} aria-hidden="true" /> {t('glossary.scramble')}</button>
        </div></div>}
        {isTeacherMode && glossaryToolsOpen.teacher && <div id="glossary-teacher-tools" className="rounded-lg border border-slate-300 bg-white p-3" aria-labelledby="glossary-teacher-tools-title"><h2 id="glossary-teacher-tools-title" className="mb-2 text-xs font-black uppercase tracking-wider text-slate-700">Teacher tools</h2><div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex flex-col gap-2 border-t border-blue-200 pt-3 sm:flex-row sm:items-center sm:justify-between">
          <div role="group" aria-label="Vocabulary type" className="inline-flex w-fit max-w-full flex-wrap rounded-lg border border-blue-200 bg-white p-1">
            <button type="button" aria-pressed={glossaryFilter === 'all'} aria-label="Show all glossary terms" data-help-key="glossary_filter_all" onClick={handleSetGlossaryFilterToAll} className={'min-h-11 rounded-md px-3 py-2 text-sm font-bold ' + (glossaryFilter === 'all' ? 'bg-blue-100 text-blue-800' : 'text-slate-700 hover:bg-slate-50')}>{t('glossary.filter_all')}</button>
            <button type="button" aria-pressed={glossaryFilter === 'academic'} aria-label="Show academic vocabulary" data-help-key="glossary_filter_tier2" onClick={handleSetGlossaryFilterToAcademic} className={'min-h-11 rounded-md px-3 py-2 text-sm font-bold ' + (glossaryFilter === 'academic' ? 'bg-blue-700 text-white' : 'text-slate-700 hover:bg-slate-50')}>Academic vocabulary</button>
            <button type="button" aria-pressed={glossaryFilter === 'domain'} aria-label="Show subject vocabulary" data-help-key="glossary_filter_tier3" onClick={handleSetGlossaryFilterToDomain} className={'min-h-11 rounded-md px-3 py-2 text-sm font-bold ' + (glossaryFilter === 'domain' ? 'bg-purple-700 text-white' : 'text-slate-700 hover:bg-slate-50')}>Subject vocabulary</button>
          </div>
