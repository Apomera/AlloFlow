...
describe('LitLab durable narration', () => {
  ...
  it('retains played narration for Save progress and leaves the adapted-text lane untouched',async()=>{
    ...
    await clickAsync(dialog.querySelector('[aria-label="Read line 1 aloud"]'));
    await eventually(()=>expect(dialog.textContent).toContain('Narration has unsaved changes'));
  ...
  it('regenerates only the selected line with a fresh provider request and saves it',async()=>{
    const fixture={...playbackFixture,lines:[...playbackFixture.lines,{...playbackFixture.lines[0],id:'l2'}]};
    const dialog=await openPerformance({onCallTTS},fixture);
    await clickAsync(findButton(dialog,'Prepare narration'));
    await eventually(()=>expect(dialog.textContent).toContain('Narration saved with this script'));
    const before=JSON.parse(localStorage.getItem('alloLitLabScripts'))[0];
    onCallTTS.mockClear();
    await clickAsync(dialog.querySelector('[aria-label="Regenerate audio for line 2"]'));
    await eventually(()=>expect(dialog.textContent).toContain('The new take was saved with this script.'));
    expect(onCallTTS).toHaveBeenCalledTimes(1);
    expect(onCallTTS.mock.calls[0][3].force).toBe(true);
    const after=JSON.parse(localStorage.getItem('alloLitLabScripts'));
    expect(after).toHaveLength(1);expect(after[0].id).toBe(before.id);
    const first=payload=>Object.values(payload.karaokeAudio.entries).find(entry=>entry.identity.segmentId==='l1');
    expect(first(after[0])).toEqual(first(before));
    expect(dialog.textContent).toContain('2 of 2 narration lines ready');
  });
  it('removes one line from the durable save while muted and preserves other clips on reopen',async()=>{
    const fixture={...playbackFixture,lines:[...playbackFixture.lines,{...playbackFixture.lines[0],id:'l2'}]};
    let dialog=await openPerformance({onCallTTS},fixture);
    await clickAsync(findButton(dialog,'Prepare narration'));
    await eventually(()=>expect(dialog.textContent).toContain('Narration saved with this script'));
    onCallTTS.mockClear();window.__alloIsGlobalMuted=()=>true;
    await clickAsync(dialog.querySelector('[aria-label="Remove audio for line 1"]'));
    await eventually(()=>expect(dialog.textContent).toContain('removed from the saved script'));
    expect(onCallTTS).not.toHaveBeenCalled();
    const entries=Object.values(JSON.parse(localStorage.getItem('alloLitLabScripts'))[0].karaokeAudio.entries);
    expect(entries.map(entry=>entry.identity.segmentId)).toEqual(['l2']);
    dialog=await reopen(onCallTTS);
    expect(dialog.textContent).toContain('1 of 2 narration lines ready');
    expect(dialog.querySelector('[aria-label="Remove audio for line 1"]')).toBeNull();
    expect(dialog.querySelector('[aria-label="Remove audio for line 2"]')).not.toBeNull();
  });
  it('preserves the saved take when regeneration fails or is cancelled',async()=>{
    const dialog=await openPerformance({onCallTTS});
    await clickAsync(findButton(dialog,'Prepare narration'));
    await eventually(()=>expect(dialog.textContent).toContain('Narration saved with this script'));
    const before=localStorage.getItem('alloLitLabScripts');
    onCallTTS.mockRejectedValueOnce(new Error('Offline'));
    await clickAsync(dialog.querySelector('[aria-label="Regenerate audio for line 1"]'));
    await eventually(()=>expect(dialog.textContent).toContain('Existing saved audio has not been replaced'));
    expect(localStorage.getItem('alloLitLabScripts')).toBe(before);
    let finish;
    onCallTTS.mockImplementationOnce(()=>new Promise(resolve=>{finish=resolve;}));
    await clickAsync(dialog.querySelector('[aria-label="Regenerate audio for line 1"]'));
    const signal=onCallTTS.mock.calls.at(-1)[3].signal;
    await clickAsync(findButton(dialog,'Cancel preparation'));
    expect(signal.aborted).toBe(true);
    await act(async()=>finish('blob:late-replacement'));
    expect(localStorage.getItem('alloLitLabScripts')).toBe(before);
    expect(dialog.textContent).toContain('1 of 1 narration lines ready');
  });
  it('saves reflection edits made while preparation is pending',async()=>{
    let finish;
    onCallTTS.mockImplementationOnce(()=>new Promise(resolve=>{finish=resolve;}));
    const {dialog}=await openAnalysis({onCallTTS});
    await clickAsync(findButton(dialog,'Prepare narration'));
    changeValue(dialog.querySelector('[aria-label="Theme analysis"]'),'An edit made during audio preparation.');
    await act(async()=>finish('blob:prepared-after-edit'));
    await eventually(()=>expect(dialog.textContent).toContain('Narration saved with this script'));
    const saved=JSON.parse(localStorage.getItem('alloLitLabScripts'))[0];
    expect(saved.progress.analysisResponses.theme).toBe('An edit made during audio preparation.');
  });
  it('reports a failed removal save without replacing the previously saved audio',async()=>{
    const dialog=await openPerformance({onCallTTS});
    await clickAsync(findButton(dialog,'Prepare narration'));
    await eventually(()=>expect(dialog.textContent).toContain('Narration saved with this script'));
    const before=localStorage.getItem('alloLitLabScripts');
    ...
    vi.spyOn(Storage.prototype,'setItem').mockImplementation(function(key,value){if(key==='alloLitLabScripts')throw new Error('QuotaExceededError');return original.call(this,key,value);});
    await clickAsync(dialog.querySelector('[aria-label="Remove audio for line 1"]'));
    await eventually(()=>expect(dialog.textContent).toContain('The audio change is ready but could not be saved'));
    expect(localStorage.getItem('alloLitLabScripts')).toBe(before);
    expect(dialog.textContent).toContain('0 of 1 narration lines ready');
  });
