...
if (!window.StemLab.isRegistered('ecosystem')) {
(function() {
  'use strict';
    ...
    render: function(ctx) {
      ...
      var buildPopSVG = function() {
        ...
        return h('svg', { viewBox: '0 0 ' + W + ' ' + H, className: 'w-full', style: { maxHeight: 200 } },
          ...
          // End labels
          h('text', { x: sx(data.length - 1) + 6, y: sy(lastData.prey) + 3, fill: '#22c55e', fontSize: 9, fontWeight: 'bold' }, lastData.prey),
          h('text', { x: sx(data.length - 1) + 6, y: sy(lastData.pred) + 3, fill: '#ef4444', fontSize: 9, fontWeight: 'bold' }, lastData.pred),
          // Axis labels
          h('text', { x: W / 2, y: H - 4, textAnchor: 'middle', fill: '#94a3b8', fontSize: 9 }, 'Time Steps'),
          h('text', { x: 8, y: H / 2, textAnchor: 'middle', fill: '#94a3b8', fontSize: 9, transform: 'rotate(-90, 8, ' + (H / 2) + ')' }, 'Population')
        );
      };
      ...
      var buildPhaseSVG = function() {
        ...
        return h('div', null,
          h('svg', { viewBox: '0 0 ' + W + ' ' + H, className: 'w-full', style: { maxHeight: 200 } },
            ...
            h('text', { x: 8, y: H / 2, textAnchor: 'middle', fill: '#94a3b8', fontSize: 9, transform: 'rotate(-90, 8, ' + (H / 2) + ')' }, 'Predator Population')
          ),
          h('div', { className: 'flex gap-3 text-xs text-slate-400 mt-1 justify-center' },
      ...
      var buildEnvSVG = function() {
        ...
        if (!hist || hist.length < 2) return h('p', { className: 'text-xs text-slate-400 text-center py-4' }, 'Run the canvas simulation to see environment data.');
        ...
        return h('svg', { viewBox: '0 0 ' + W + ' ' + H, className: 'w-full', style: { maxHeight: 200 } },
          ...
          h('text', { x: 8, y: H / 2, textAnchor: 'middle', fill: '#94a3b8', fontSize: 9, transform: 'rotate(-90, 8, ' + (H / 2) + ')' }, 'Level (0\u20131)')
        );
      };
      ...
      return h('div', { className: 'space-y-3 pb-4' },
        ...
        h('div', { className: 'flex gap-1 bg-slate-100 dark:bg-slate-800 rounded-lg p-1' },
          ...
          eventHistory.length > 0 && h('div', { className: 'bg-slate-50 dark:bg-slate-800 rounded-lg p-2 border border-slate-200 dark:border-slate-700' },
            ...
            h('div', { className: 'space-y-0.5 max-h-20 overflow-y-auto' },
              eventHistory.slice(-5).reverse().map(function(ev, idx) {
                ...
                return h('div', {
                  key: 'eh' + idx,
                  className: 'flex items-center gap-2 text-[9px] text-slate-500 dark:text-slate-400'
                },
                  h('span', null, eventIcons[ev.name] || '\u26A1'),
                  h('span', { className: 'font-semibold' }, ev.name),
                  h('span', { className: 'ml-auto text-slate-400' }, timeLabel)
                );
              })
            ),
            h('div', { className: 'flex items-center gap-2 mt-1 pt-1 border-t border-slate-200 dark:border-slate-600' },
              h('span', { className: 'text-[9px] text-slate-400' }, 'Total events: ' + eventHistory.length),
              h('span', { className: 'text-[9px] text-slate-400 ml-auto' },
          ...
          h('div', { className: 'bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden' },
            ...
            ecoGraphOpen && h('div', { className: 'px-3 pb-3 space-y-2' },
              ...
              ecoGraphView === 'population' ? (
                ...
                })() : h('p', { className: 'text-xs text-slate-400 text-center py-4' }, 'Canvas simulation is generating live data...')
              ) : buildEnvSVG(),
              ...
              ECO_CHALLENGES.map(function(ch) {
                ...
                return h('div', {
                  ...
                  h('div', { className: 'flex items-center gap-1 mb-1' },
                    ...
                    done && h('span', { className: 'text-[9px] text-green-500 font-bold ml-auto' }, '\u2714')
                  ),
                  h('p', { className: 'text-[9px] text-slate-500 dark:text-slate-400 mb-1' }, ch.desc),
                  h('p', { className: 'text-[9px] font-bold ' + (done ? 'text-green-600' : 'text-amber-600') },
        ...
        tab === 'sandbox' && h('div', { className: 'space-y-3' },
          ...
          h('div', { className: 'space-y-1' },
            ...
            h('div', { className: 'flex gap-1 flex-wrap' },
              eventDefs.map(function(ev) {
                return h('button', {
                  key: 'sb-' + ev.id,
                  className: 'flex-1 min-w-[55px] px-1 py-1 text-[9px] font-bold rounded-lg text-white transition-all hover:opacity-90 active:scale-95 ' + ev.color,
                  onClick: function() { triggerEvent(ev.id); },
                  title: ev.label
                }, ev.icon + ' ' + ev.label);
              })
            ...
            (function() {
              ...
              return [
                h('div', { key: 'sbp', className: 'bg-green-50 dark:bg-green-900/20 rounded p-1.5' },
                  h('div', { className: 'text-[9px] text-slate-500' }, '\uD83D\uDC07 Prey'),
                ...
                h('div', { key: 'sbd', className: 'bg-red-50 dark:bg-red-900/20 rounded p-1.5' },
                  h('div', { className: 'text-[9px] text-slate-500' }, '\uD83E\uDD8A Predators'),
                ...
                h('div', { key: 'sbv', className: 'bg-emerald-50 dark:bg-emerald-900/20 rounded p-1.5' },
                  h('div', { className: 'text-[9px] text-slate-500' }, '\uD83C\uDF3F Vegetation'),
          ...
          h('div', { className: 'bg-slate-50 dark:bg-slate-800 rounded-lg p-3 text-xs text-slate-600 dark:text-slate-400 space-y-1' },
            ...
            h('p', { className: 'text-[10px] italic text-slate-400' }, 'Tip: Pause the simulation first for precise placement!')
          ),
            ...
            BADGES.map(function(b) {
              ...
              return h('div', {
                ...
                h('div', null,
                  ...
                  h('p', { className: 'text-[9px] text-slate-500' }, b.desc),
        ...
        !tutorialDismissed && tutorialStep < 5 && h('div', {
          ...
          h('div', { className: 'bg-white dark:bg-slate-800 rounded-2xl p-5 max-w-sm mx-4 shadow-2xl border-2 border-emerald-400' },
            h('div', { className: 'flex items-center justify-between mb-3' },
              ...
              h('button', { className: 'text-slate-400 hover:text-slate-600 text-sm', onClick: dismissTutorial }, '\u2715')
            ),
