...
source+=String.raw`
  ...
  for(const bench of ['windlass','screw']){
    await mount(bench);
  ...
  for(const bench of ['windlass','screw'])for(const radius of [0.05,0.5]) {
    await mount(bench,{windlassHandleR:radius,screwHandleR:radius});
    const slider=inspector.getByRole('slider');await slider.press('Home');
    for(let step=0;step<25;step++)await slider.press('ArrowRight');
    await checkPose(0.25);
    await pg.locator('.ml-shop-world').screenshot({path:path.join(OUT,bench+'-quarter-'+radius+'.png')});
    manifest.push(bench+'-quarter-'+radius);
  }
  for(const width of [390,320])for(const bench of ['windlass','screw']){
