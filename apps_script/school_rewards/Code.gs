...
function mailMessageForDelivery_(book, delivery, recipient) {
  ...
  if (delivery.kind === 'STUDENT_STATEMENT') {
    ...
    return {
      to: recipient.email, subject: (config.schoolName || 'School') + ' rewards update',
...
function validatePendingCoreJournal_(book, key, operation, journal) {
  ...
  var kind = String(journal.kind || ''), intent = object_(journal.intent), actorEmail = normalizeEmail_(intent.actorEmail), actorRole = String(intent.actorRole || ''), allowedRoles = { award: ['admin', 'staff'], reverse: ['admin'], checkout: ['admin', 'cashier'], refund: ['admin'], catalog: ['admin'] };
