...
const { baselineMetrics, reviewedAt, reviewWave, revisions } = require('./eppp_native_quality_wave_34_data.cjs');
...
const auditBasename = 'eppp_question_quality_audit_wave_34.json';
...
if (fs.readFileSync(deployBankPath, 'utf8') !== sourceText) throw new Error('Source and deploy EPPP banks must match before wave 34.');
...
if (revisions.length !== 24 || new Set(revisions.map((revision) => revision.id)).size !== 24) throw new Error('Wave 34 needs twenty-four unique revisions.');
...
for (const revision of revisions) {
  ...
  if (!recognizedReplay && item.prompt !== revision.expectedPrompt) throw new Error(revision.id + ' prompt drifted before wave 34.');
...
console.log('EPPP quality wave 34: rewrote 24 items across 8 domains; selected warnings after = 0.');
