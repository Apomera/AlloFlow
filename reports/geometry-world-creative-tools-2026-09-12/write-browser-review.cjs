...
const files=['browser-creative.json','browser-final.json','browser-resolved.json','browser-stable-layout.json'];
const omitted=new Set(['Recolor applies material to six selected blocks','Front frames selection','Side frames selection','Top frames selection','Orbit control changes camera','Free view returns free camera','View printer bed reaches front preset and changes camera','Print guide Return camera is available','Activity portfolio control reachable at 390','Activity portfolio control reachable at 320']);
const map=new Map(),runs=files.map(file=>({file,data:JSON.parse(fs.readFileSync(path.join(d,file),'utf8'))}));
for(const run of runs)for(const a of run.data.assertions)if(!omitted.has(a.name))map.set(a.name,{...a,source:run.file});
const assertions=[...map.values()],failed=assertions.filter(a=>!a.pass),errors=runs.flatMap(run=>(run.data.errors||[]).concat(run.data.consoleErrors||[]));
if(failed.length||errors.length||runs.at(-1).data.failure)throw Error('Final browser checks still fail: '+JSON.stringify({failed,errors,layoutFailure:runs.at(-1).data.failure}));
const summary={scope:'Unique named assertions across the broad browser pass and targeted corrections; superseded fixture-only assertions are excluded. Not a full application E2E suite.',passed:assertions.length,failed:0,runtimeErrors:errors.length,sources:files,excludedAssertionNames:[...omitted],assertions};
fs.writeFileSync(path.join(d,'browser-verification-summary.json'),JSON.stringify(summary,null,2));
const layouts=runs.at(-1).data.layouts;
const report=`# Geometry World creative tools — browser review

Verified ${assertions.length} distinct named browser assertions with no remaining failed checks and no JavaScript or console errors in the reviewed runs. See [browser-verification-summary.json](browser-verification-summary.json) for the accepted checks and their original evidence files.
...
- Free Build panels remain navigable at390 and320 pixels. Corrected touch Position is tappable. Drawing HUD and expanded Position do not overlap at390 × 844,320 × 700, or800 × 500; the crosshair stays clear. Settled journal layouts have document scrollWidth/clientWidth ${layouts[390].scrollWidth}/${layouts[390].clientWidth} and ${layouts[320].scrollWidth}/${layouts[320].clientWidth}; snapshot cards fit and the portfolio button passes hit testing.
...
`;
fs.writeFileSync(path.join(d,'BROWSER-REVIEW.md'),report);console.log(JSON.stringify({passed:assertions.length,failed:0,runtimeErrors:errors.length,review:path.join(d,'BROWSER-REVIEW.md')}));
