...
test.describe('Raptor cinematic simulator rendering', () => {
  ...
  for (const scene of [
    ...
    test(`renders ${scene.label} with working scenic view and a pause-aware atmosphere`, async ({ page }) => {
      ...
      if (scene.label === 'night-low') await page.addStyleTag({ content: '#wrap{width:420px}' });
      ...
      }, { phase: scene.phase, lake: scene.label === 'lake' });
      ...
      if (scene.label === 'night-low') { expect(before.reducedMotion).toBe(true); expect(before.sceneryTime).toBe(0); }
