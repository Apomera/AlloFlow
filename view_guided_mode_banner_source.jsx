...
function GuidedModeBanner({
  ...
  const normalizeReadinessChecks = value => Object.fromEntries(['directions', 'delivery', 'learner-view', 'answer-keys', 'accessibility', 'backup'].filter(id => value?.[id] === true).map(id => [id, true]));
  const readinessContextKey = React.useMemo(() => {
    const ids = Array.from(new Set((guidedCreatedHistoryIds || []).filter(id => typeof id === 'string' && id))).sort();
    const resources = ids.map(id => (history || []).find(item => item?.id === id));
    // Wait for saved resources to hydrate before replacing a saved confirmation.
  ...
  const [readinessState, setReadinessState] = React.useState(() => {
    try {
      ...
      if (value?.version === 2 && typeof value.contextKey === 'string') return { contextKey: value.contextKey, checks: normalizeReadinessChecks(value.checks) };
    } catch (_) {}
    // Unscoped confirmations from older versions cannot verify this lesson.
    return { contextKey: readinessContextKey, checks: {} };
  });
  const [readinessChanged, setReadinessChanged] = React.useState(false);
  const [readinessSaveFailed, setReadinessSaveFailed] = React.useState(false);
  const readinessChecks = readinessContextKey && readinessState.contextKey === readinessContextKey ? readinessState.checks : {};
  const setReadinessChecks = update => {
    setReadinessChanged(false);
    setReadinessState(previous => {
      const current = previous.contextKey === readinessContextKey ? previous.checks : {};
      return { contextKey: readinessContextKey, checks: normalizeReadinessChecks(typeof update === 'function' ? update(current) : update) };
    });
  };
  React.useEffect(() => {
    if (readinessContextKey && readinessState.contextKey !== readinessContextKey) {
      if (Object.keys(readinessState.checks).length) setReadinessChanged(true);
      setReadinessState({ contextKey: readinessContextKey, checks: {} });
    }
  }, [readinessContextKey, readinessState.contextKey]);
  ...
  React.useEffect(() => {
    if (!readinessContextKey) return;
    try {
      localStorage.setItem('allo_guided_readiness_checks', JSON.stringify({ version: 2, contextKey: readinessContextKey, checks: readinessChecks }));
      setReadinessSaveFailed(false);
    } catch (_) {
      setReadinessSaveFailed(true);
      // Do not leave an older checked state behind when an uncheck cannot save.
      try { localStorage.removeItem('allo_guided_readiness_checks'); } catch (_) {}
    }
  }, [readinessContextKey, readinessState]);
  ...
  const _errorLower = String(guidedErrorText).toLowerCase();
  const guidedErrorStatus = String(guidedStepError?.status || guidedStepError?.statusCode || guidedStepError?.code || '').toLowerCase();
  const guidedErrorSignal = guidedErrorStatus + ' ' + _errorLower;
  const guidedErrorGuidance = /\b(401|403)\b|unauthenticated|unauthorized|permission.denied|api[ _-]?key|missing.key|invalid.key/.test(guidedErrorSignal)
    ? (t('guided.error_access') || 'Check your AI connection and access settings, then retry. A missing or invalid key cannot be fixed by retrying alone.')
    : /quota|insufficient[ _-]?credits|billing|usage.limit/.test(guidedErrorSignal)
      ? (t('guided.error_quota') || 'Check your AI provider usage allowance, or choose another configured provider before retrying.')
      : /\b429\b|rate[ _-]?limit|too many requests|resource.exhausted/.test(guidedErrorSignal)
        ? (t('guided.error_rate_limit') || 'The service is busy. Wait a moment, then retry.')
        : /network|fetch|connection|offline|timed?\s*out|timeout/.test(guidedErrorSignal)
          ? (t('guided.error_network') || 'Check your connection, then try again.')
          : step.id === 'image' ? (t('guided.error_image') || 'Check the image settings or try a simpler visual request.') : (t('guided.error_default') || 'Review the source and settings, then retry. Your completed work is safe.');
  ...
  // Only show a result from this run, or one that just arrived on this step.
  // The host may register its Guided ID on the following render. Never fall
  // back to an unrelated lesson merely because the run has no tracked IDs yet.
  const currentResultItem = _matchTypes ? [...(history || [])].reverse().find(item => item && _matchTypes.includes(item.type) && (_createdIdSet.has(item.id) || _newItems.includes(item))) : null;
  ...
  const resultPreviewText = (() => { const item = currentResultItem; const data = item?.data && typeof item.data === 'object' ? item.data : {}; const candidates = [typeof item?.data === 'string' ? item.data : null, data.simplifiedText, data.text, data.content, item?.text, item?.content, item?.summary, data.summary, data.description]; const value = candidates.find(entry => typeof entry === 'string' && entry.trim()); return value ? value.replace(/\s+/g, ' ').trim().slice(0, 260) : ''; })();
  ...
  return (
    <>
      ...
      <div data-help-key="guided_workflow" className="allo-guided-banner" role="region" aria-label={t('guided.indicator_title') || 'Guided mode'} style={{ background: 'linear-gradient(135deg, #312e81, #1e3a5f)', borderRadius: '20px', padding: '16px', marginBottom: '16px', border: '1px solid rgba(99,102,241,0.3)', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', marginBottom: isCollapsed ? 0 : '8px' }}>
          <div style={{ minWidth: 0 }}><span style={{ fontSize: '13px', fontWeight: 800, color: 'white', display: 'block' }}>{t('guided.indicator_title')}</span><span data-help-key="guided_progress_storage" className="allo-guided-progress-save" data-state={progressSaveStatus} role={progressSaveStatus === 'error' ? 'alert' : undefined} aria-label={progressSaveLabel} title={progressSaveLabel}><span aria-hidden="true">{progressSaveStatus === 'error' ? '!' : progressSaveStatus === 'saved' ? '✓' : '↻'}</span>{isCollapsed ? (progressSaveStatus === 'error' ? (t('guided.progress_not_saved_short') || 'Not saved') : progressSaveStatus === 'saved' ? (t('guided.progress_saved_short') || 'Saved') : (t('guided.progress_saving_short') || 'Saving')) : progressSaveLabel}</span>{isCollapsed && <span style={{ display: 'block', fontSize: '12px', color: '#c7d2fe', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{step.label || 'Complete!'}</span>}</div>
        ...
        {showInitialPath && !hasGuidedProgress && Array.isArray(guidedPresets) && (
          <div role="region" data-help-key="guided_path_choice" aria-labelledby="guided-path-title" style={{ marginBottom: '11px', padding: '11px', borderRadius: '12px', background: 'rgba(15,23,42,.45)', border: '1px solid rgba(167,243,208,.4)' }}>
            ...
            <span style={{ display: 'block', color: '#c7d2fe', fontSize: '12px', lineHeight: 1.45, marginBottom: '8px' }}>{t('guided.choose_goal_hint') || 'Choose a focused lesson path, or keep all available steps.'}</span>
            <button type="button" data-help-key="guided_ai_plan" onClick={openAiPlanner} style={{ width: '100%', padding: '10px', marginBottom: '8px', textAlign: 'left', borderRadius: '10px', border: '1px solid rgba(110,231,183,.55)', background: 'linear-gradient(135deg, rgba(16,185,129,.2), rgba(99,102,241,.2))', color: 'white' }}><strong style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px' }}><span aria-hidden="true">✨</span>{t('guided.ai_plan_button') || 'Plan with AI'}</strong><span style={{ display: 'block', marginTop: '3px', color: '#d1fae5', fontSize: '12px', lineHeight: 1.4 }}>{t('guided.ai_plan_button_hint') || 'Describe your lesson goals, timing, learners, and delivery needs. Review the proposed path before applying it.'}</span></button>
        ...
        {showAiPlanner && (
          ...
          <div ref={_aiPlannerDialogRef} className="allo-guided-planning-studio" role="dialog" aria-modal="true" aria-labelledby="guided-ai-planner-title" aria-describedby="guided-ai-planner-intro" aria-busy={aiPlannerBusy ? 'true' : 'false'} tabIndex={-1}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', marginTop: '3px', color: '#a7f3d0', fontSize: '10px' }}><span>{t('guided.ai_plan_privacy') || 'Do not include student names or sensitive personal information.'}</span><span role="group" aria-label={t('guided.ai_plan_character_count') || 'Character count'}>{aiPlannerGoal.length}/1200</span></div>            <details className="allo-guided-collapsible" style={{ marginTop: '10px' }}>
        ...
        {showResumeCheckpoint && hasGuidedProgress && !isLast && <section data-help-key="guided_resume_card" className="allo-guided-resume-card" role="status" aria-live="polite" aria-labelledby="guided-resume-title">
          ...
          <span>{(t('guided.resume_saved_step') || 'Your saved step: {step}').replace('{step}', localizeStep(step, 'label'))}</span>
          <div><button type="button" disabled={guidedBusy} onClick={() => { setShowResumeCheckpoint(false); if (typeof focusGuidedTarget === 'function') requestAnimationFrame(focusGuidedTarget); }}>{t('guided.resume_continue') || 'Continue where I left off'}</button>{latestGuidedResourceItem && openGuidedHistoryItem && <button type="button" disabled={guidedBusy} onClick={() => openGuidedHistoryItem(latestGuidedResourceItem)}>{t('guided.resume_review') || 'Review latest result'}</button>}{typeof generateGuidedPlanFromGoal === 'function' && <button type="button" disabled={guidedBusy} onClick={() => { setShowResumeCheckpoint(false); openAiPlanner(); }}>{t('guided.resume_adjust') || 'Adjust what remains'}</button>}</div>
        ...
        {/* N1 (2026-08-16): the single "do this now" instruction now LEADS the banner body.
            It used to render at position 11, below a three-box journey grid, an estimate
            chip row and four collapsed disclosures — so a novice met a wall of options
            before the one sentence telling them what to click. Everything else still
            exists; it just renders after this card. Nothing here animates: the only
            attention cue in Guided Mode is the pulsing ring the host puts on the tool
            itself (.allo-guided-target), so the eye has exactly one place to go. */}
        {step.action && (
          <div role="status" style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', background: stepDone ? 'rgba(34,197,94,0.16)' : 'rgba(99,102,241,0.26)', border: '1px solid ' + (stepDone ? 'rgba(74,222,128,0.45)' : 'rgba(129,140,248,0.6)'), borderRadius: '12px', padding: '12px 13px', marginBottom: '8px' }}>
            <span aria-hidden="true" style={{ fontSize: '16px', lineHeight: '1.35' }}>{stepDone ? '✅' : '👉'}</span>
            <span style={{ minWidth: 0 }}>
              <span style={{ display: 'block', fontSize: '11px', fontWeight: 900, letterSpacing: '.06em', textTransform: 'uppercase', color: stepDone ? '#6ee7b7' : '#a5b4fc', marginBottom: '2px' }}>{stepDone ? (t('guided.step_done_label') || 'Done') : (t('guided.do_this_now') || 'Do this now')}</span>
              <span style={{ display: 'block', fontSize: '14px', color: 'white', fontWeight: 600, lineHeight: '1.5' }}>{stepDone ? (step.success || step.action) : step.action}</span>
            </span>
          </div>
        )}
        ...
        {/* N1: the journey grid ("Why now / You'll create / Next") and the estimate chips
            moved from always-on into this disclosure. They are useful orientation, not a
            next action, and three boxes plus three chips above the instruction were a
            large part of what Aaron described as "a lot at once". */}
        <details data-help-key="guided_plan_navigation" className="allo-guided-focus-more allo-guided-collapsible" open={showFocusDetails} onToggle={event => setShowFocusDetails(event.currentTarget.open)}><summary>{t('guided.focus_more') || 'Plan & navigation'} <span>{currentPhaseIndex + 1}/{activePhaseDefinitions.length}</span></summary><div className="allo-guided-focus-more-body">
        {(stepWhyNow || expectedStepOutput || upNextLabel) && <section className="allo-guided-journey-context" aria-label={t('guided.journey_context') || 'Current Guided journey context'}>{stepWhyNow && <div><span>{t('guided.journey_why') || 'Why now'}</span><strong>{stepWhyNow}</strong></div>}{expectedStepOutput && <div><span>{t('guided.journey_creates') || 'You’ll create'}</span><strong>{expectedStepOutput}</strong></div>}{upNextLabel && <div><span>{t('guided.journey_next') || 'Next'}</span><strong>{upNextLabel}</strong></div>}</section>}
        ...
        {!isLast && typeof generateGuidedPlanFromGoal === 'function' && <details className="allo-guided-ask allo-guided-collapsible"><summary>✨ {t('guided.quick_guide_title') || 'Ask Guide to adjust what remains'}</summary><div><span>{t('guided.quick_guide_hint') || 'Describe a change. You will review the updated path before anything is applied.'}</span><div className="allo-guided-prompt-chips"><button type="button" onClick={() => setQuickGuideText(t('guided.quick_guide_shorter') || 'Make the remaining path shorter.')}>{t('guided.quick_guide_shorter_chip') || 'Shorter'}</button><button type="button" onClick={() => setQuickGuideText(t('guided.quick_guide_discussion') || 'Add more discussion and collaboration.')}>{t('guided.quick_guide_discussion_chip') || 'More discussion'}</button><button type="button" onClick={() => setQuickGuideText(t('guided.quick_guide_paper') || 'Optimize the remaining path for paper-first delivery.')}>{t('guided.quick_guide_paper_chip') || 'Paper-first'}</button></div><label htmlFor="guided-quick-guide">{t('guided.quick_guide_label') || 'What should change?'}</label><textarea id="guided-quick-guide" maxLength={400} rows={3} value={quickGuideText} disabled={quickGuideBusy} onChange={event => { setQuickGuideText(event.target.value); setQuickGuideStatus(''); }} placeholder={t('guided.quick_guide_placeholder') || 'Example: keep the quiz, add partner discussion, and finish in 25 minutes.'} /><button type="button" disabled={quickGuideBusy || quickGuideText.trim().length < 3} onClick={askGuideAboutRemaining}>{quickGuideBusy ? (t('guided.quick_guide_working') || 'Adjusting…') : (t('guided.quick_guide_preview') || 'Preview adjustment')}</button>{quickGuideStatus && <span role="status" aria-live="polite">{quickGuideStatus}</span>}</div></details>}        {phaseCheckpointReady && <section data-help-key="guided_phase_review" ref={_phaseCheckpointRef} tabIndex={-1} role="status" aria-live="polite" className="allo-guided-phase-checkpoint" aria-labelledby="guided-phase-checkpoint-title" style={{ marginBottom: '10px', padding: '12px', borderRadius: '12px', background: 'linear-gradient(135deg, rgba(16,185,129,.2), rgba(79,70,229,.2))', border: '1px solid rgba(167,243,208,.45)' }}>
          ...
          <div style={{ display: 'flex', gap: '7px', flexWrap: 'wrap', marginTop: '10px' }}>
            ...
            {typeof callGemini === 'function' && <button type="button" disabled={guidedBusy} data-help-key="guided_ai_plan" onClick={openAiPlanner} style={{ flex: 1, minWidth: '145px', borderRadius: '9px', border: '1px solid rgba(165,180,252,.4)', background: 'rgba(255,255,255,.08)', color: '#e0e7ff', fontWeight: 800 }}>{t('guided.phase_checkpoint_adjust') || 'Adjust remaining path'}</button>}
            <button type="button" disabled={guidedBusy} data-help-key="guided_next_step" onClick={() => handleGuidedSkip(false)} style={{ flex: 1, minWidth: '180px', border: 0, borderRadius: '9px', background: 'linear-gradient(135deg, #10b981, #6366f1)', color: 'white', fontWeight: 900 }}>{(t('guided.phase_checkpoint_continue') || 'Continue to {phase}').replace('{phase}', nextPhaseLabel)}</button>
          </div>
        </section>}
{step.id === 'package-deliver' && Array.isArray(guidedDeliveryGroups) && (
          <div role="region" aria-labelledby="guided-delivery-title" style={{ marginBottom: '10px', padding: '11px 12px', borderRadius: '12px', background: 'rgba(15,23,42,.38)', border: '1px solid rgba(167,243,208,.35)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '6px', marginTop: '9px' }}>
              {typeof openGuidedDocumentBuilder === 'function' && <button type="button" disabled={guidedBusy} data-help-key="guided_document_builder" onClick={openGuidedDocumentBuilder} style={{ padding: '7px 8px', border: 0, borderRadius: '8px', background: '#4f46e5', color: 'white', fontWeight: 800, fontSize: '12px' }}>{t('guided.open_builder') || 'Document Builder'}</button>}
              {typeof createGuidedHomeworkShare === 'function' && <button type="button" disabled={guidedBusy} data-help-key="guided_homework_share" onClick={createGuidedHomeworkShare} style={{ padding: '7px 8px', border: '1px solid rgba(165,180,252,.45)', borderRadius: '8px', background: 'rgba(255,255,255,.08)', color: 'white', fontWeight: 800, fontSize: '12px' }}>{t('guided.create_homework_qr') || 'Homework QR'}</button>}
              {typeof startGuidedLiveSession === 'function' && <button type="button" disabled={guidedBusy} data-help-key="guided_live_session" onClick={startGuidedLiveSession} style={{ padding: '7px 8px', border: '1px solid rgba(110,231,183,.45)', borderRadius: '8px', background: 'rgba(16,185,129,.14)', color: '#d1fae5', fontWeight: 800, fontSize: '12px' }}>{t('guided.start_live_session') || 'Live session'}</button>}
              {typeof previewGuidedStudentAssignment === 'function' && <button type="button" disabled={guidedBusy || !canPreviewGuidedStudentAssignment} data-help-key="guided_learner_preview" onClick={previewGuidedStudentAssignment} title={!canPreviewGuidedStudentAssignment ? (t('guided.preview_requires_share') || 'Create a student link first') : undefined} style={{ padding: '7px 8px', border: '1px solid rgba(252,211,77,.45)', borderRadius: '8px', background: 'rgba(245,158,11,.12)', color: '#fef3c7', fontWeight: 800, fontSize: '12px' }}>{t('guided.test_student_link') || 'Test student link'}</button>}
            </div>
          </div>
        )}
        ...
        {guidedStepError && (
          <div role="alert" style={{ marginBottom: '10px', padding: '11px 12px', background: 'rgba(127,29,29,.35)', border: '1px solid rgba(248,113,113,.65)', borderRadius: '12px' }}>
            <div style={{ color: 'white', fontSize: '13px', fontWeight: 800 }}>{t('guided.error_title') || 'This attempt did not finish'}</div>
            <div style={{ color: '#fecaca', fontSize: '12px', lineHeight: 1.5, margin: '4px 0 8px' }}>{guidedErrorGuidance}</div>
            <div style={{ display: 'flex', gap: '7px', flexWrap: 'wrap' }}>{typeof retryGuidedStep === 'function' && <button type="button" disabled={guidedBusy} onClick={retryGuidedStep} style={{ minHeight: '40px', padding: '7px 11px', borderRadius: '8px', border: '1px solid rgba(255,255,255,.25)', background: '#b91c1c', color: 'white', fontWeight: 800, opacity: guidedBusy ? .65 : 1 }}>{guidedBusy ? (t('guided.retrying') || 'Retrying…') : (t('guided.retry') || 'Retry')}</button>}{typeof openUniversalSettings === 'function' && <button type="button" disabled={guidedBusy} onClick={openUniversalSettings} style={{ minHeight: '40px', padding: '7px 11px', borderRadius: '8px', border: '1px solid rgba(255,255,255,.25)', background: 'transparent', color: 'white' }}>{t('guided.error_open_settings') || 'Review settings'}</button>}{guidedStep > 0 && <button type="button" disabled={guidedBusy} data-help-key="guided_skip_step" onClick={() => handleGuidedSkip(true)} style={{ minHeight: '40px', padding: '7px 11px', borderRadius: '8px', border: '1px solid rgba(255,255,255,.25)', background: 'rgba(255,255,255,.08)', color: 'white', opacity: guidedBusy ? .65 : 1 }}>{t('guided.skip_for_now') || 'Skip for now'}</button>}<button type="button" aria-expanded={showErrorDetails} onClick={() => setShowErrorDetails(value => !value)} style={{ minHeight: '40px', padding: '7px 11px', borderRadius: '8px', border: '1px solid rgba(255,255,255,.25)', background: 'transparent', color: '#fecaca' }}>{showErrorDetails ? (t('guided.hide_details') || 'Hide details') : (t('guided.view_details') || 'View details')}</button></div>
        ...
        {isLast && (
          <div data-help-key="guided_readiness" ref={_readinessRegionRef} tabIndex={-1} role="region" aria-labelledby="guided-readiness-title" style={{ marginBottom: '10px', padding: '11px 13px', background: 'rgba(59,130,246,.13)', border: '1px solid rgba(147,197,253,.38)', borderRadius: '12px' }}>
        ...
        <div className="allo-guided-primary-actions" style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {guidedStep > 0 && <button type="button" disabled={guidedBusy} data-help-key="guided_back_step" onClick={() => setGuidedStep(s => Math.max(0, s - 1))} aria-label={t('guided.back') || 'Back'} title={t('guided.back') || 'Back'} style={{ padding: '6px 10px', fontSize: '12px', fontWeight: 800, color: '#c7d2fe', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '10px', cursor: 'pointer', transition: 'all 0.2s', flexShrink: 0 }}>{t('guided.back') || '← Back'}</button>}
          ...
          {!isLast && stepDone && !phaseCheckpointReady && <button type="button" disabled={guidedBusy} data-help-key="guided_next_step" onClick={() => handleGuidedSkip(false)} style={{ flex: 1, padding: '6px 12px', fontSize: '12px', fontWeight: 800, color: 'white', background: 'linear-gradient(135deg, #818cf8, #6366f1)', border: '1px solid rgba(129,140,248,0.45)', borderRadius: '10px', cursor: 'pointer', transition: 'all 0.2s' }}>{t('guided.next_step') || 'Next step →'}{nextStep && <span style={{ display: 'block', fontSize: '11px', fontWeight: 500, marginTop: '2px', overflowWrap: 'anywhere' }}>{localizeStep(nextStep, 'label')}</span>}</button>}
          {!isLast && !stepDone && !phaseCheckpointReady && guidedStep > 0 && <button type="button" disabled={guidedBusy} data-help-key="guided_skip_step" onClick={() => handleGuidedSkip(true)} style={{ flex: 1, padding: '6px 12px', fontSize: '12px', fontWeight: 800, color: '#c7d2fe', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '10px', cursor: 'pointer', transition: 'all 0.2s' }}>{t('guided.skip_step') || t('guided.skip') || 'Skip step'}</button>}
          {isLast && <button type="button" disabled={guidedBusy} data-help-key="guided_finish" onClick={requestFinishGuidedRun} style={{ flex: 1, padding: '6px 12px', fontSize: '12px', fontWeight: 700, color: 'white', background: 'linear-gradient(135deg, #818cf8, #6366f1)', border: 'none', borderRadius: '10px', cursor: 'pointer', transition: 'all 0.2s' }}>{readinessCount === readinessTotal ? (t('guided.all_done') || 'Finish Guided Mode') : (t('guided.finish_with_checks_remaining') || 'Finish with checks remaining')}</button>}
          {toggleGuidedStepId && <button type="button" disabled={guidedBusy} data-help-key="guided_customize" onClick={() => setShowPicker(p => !p)} aria-label={t('guided.customize') || 'Choose which steps to include'} aria-expanded={showPicker} aria-controls="guided-step-picker" title={t('guided.customize') || 'Choose which steps to include'} style={{ padding: '6px 10px', fontSize: '12px', fontWeight: 700, color: showPicker ? 'white' : '#c7d2fe', background: showPicker ? 'rgba(99,102,241,0.4)' : 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '10px', cursor: 'pointer', transition: 'all 0.2s' }}><span aria-hidden="true">⊙ </span>{t('guided.customize_short') || 'Customize'}</button>}
          <button type="button" data-help-key="guided_step_help" onClick={() => setShowGuidedTip(p => !p)} aria-expanded={showGuidedTip} aria-controls="guided-about-panel" style={{ padding: '6px 12px', fontSize: '12px', fontWeight: 700, color: showGuidedTip ? 'white' : '#c7d2fe', background: showGuidedTip ? 'rgba(99,102,241,0.4)' : 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '10px', cursor: 'pointer', transition: 'all 0.2s' }}>{showGuidedTip ? '✕' : 'ℹ️'} {t('guided.about')}</button>
          <button type="button" disabled={guidedBusy} data-help-key="guided_resume_later" onClick={requestResumeLater} title={progressSaveStatus === 'error' ? (t('guided.resume_later_unsaved_hint') || 'Progress is not saved; review before exiting') : (t('guided.resume_later_hint') || 'Save your place and return from Setup')} style={{ padding: '6px 12px', fontSize: '12px', fontWeight: 700, color: '#fde68a', background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: '10px', cursor: 'pointer', transition: 'all 0.2s' }}>{t('guided.resume_later') || 'Resume later'}</button>
        </div>
        {showPicker && toggleGuidedStepId && (
          <div data-help-key="guided_customize" id="guided-step-picker" role="group" aria-label={t('guided.choose_steps') || 'Choose which steps to include'} style={{ marginTop: '10px', padding: '10px 12px', background: 'rgba(255,255,255,0.06)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', maxHeight: '340px', overflowY: 'auto', animation: 'fadeIn 0.3s ease-out' }}>
