...
(function() {
  ...
  window.SelHub.registerTool('path', {
    ...
    render: function(ctx) {
      ...
      var _paT = (ctx && ctx.theme) || {};
      var _paHC = !!_paT.isContrast, _paL = !_paHC && !_paT.isDark;
      var _pa_BGL = {'#1e293b':'#ffffff','#0f172a':'#f8fafc'}, _pa_BGH = {'#1e293b':'#000000','#0f172a':'#000000','#fff':'#000000'};
      var _pa_FGL = {'#cbd5e1':'#334155','#a5b4fc':'#3730a3','#94a3b8':'#64748b','#e0e7ff':'#3730a3','#fde68a':'#92400e','#e2e8f0':'#1e293b','#fcd34d':'#78350f'}, _pa_FGH = {'#cbd5e1':'#ffff00','#a5b4fc':'#ffff00','#94a3b8':'#ffff00','#e0e7ff':'#ffff00','#fde68a':'#ffff00','#e2e8f0':'#ffff00','#fff':'#ffff00','#0f172a':'#ffff00','#64748b':'#ffff00','#475569':'#ffff00','#fcd34d':'#ffff00'};
      var _pa_BDL = {'#334155':'#e2e8f0','#1e293b':'#e5e7eb','#475569':'#cbd5e1'}, _pa_BDH = {'#334155':'#ffff00','#818cf8':'#ffff00','#fbbf24':'#ffff00','#1e293b':'#ffff00','#475569':'#ffff00','#cbd5e1':'#ffff00','#4f46e5':'#ffff00','#f59e0b':'#ffff00'};
      var _paBg = function(h){ return _paHC ? (_pa_BGH[h]||h) : (_paL ? (_pa_BGL[h]||h) : h); };
      var _paFg = function(h){ return _paHC ? (_pa_FGH[h]||h) : (_paL ? (_pa_FGL[h]||h) : h); };
      var _paBd = function(h){ return _paHC ? (_pa_BDH[h]||h) : (_paL ? (_pa_BDL[h]||h) : h); };
      // accent text on the dark shell needs the 300/400 weight (1.4.3)
      var _paInk = function(c){ return _paHC || _paL ? c : ({'#6366f1':'#818cf8','#4f46e5':'#818cf8','#a855f7':'#c084fc','#9333ea':'#c084fc','#7c3aed':'#a78bfa','#8b5cf6':'#a78bfa','#3b82f6':'#60a5fa','#2563eb':'#60a5fa','#ef4444':'#f87171','#dc2626':'#f87171','#059669':'#34d399','#10b981':'#34d399','#16a34a':'#4ade80','#0891b2':'#22d3ee','#0284c7':'#38bdf8','#0d9488':'#2dd4bf','#ec4899':'#f472b6','#db2777':'#f472b6','#64748b':'#94a3b8','#475569':'#94a3b8','#a16207':'#fbbf24','#b45309':'#fbbf24'}[String(c).toLowerCase()] || c); };
      // white text needs a 700-weight fill (1.4.3)
      var _paSolid = function(c){ return _paHC ? c : ({'#0ea5e9':'#0369a1','#38bdf8':'#0369a1','#0284c7':'#0369a1','#f59e0b':'#b45309','#fbbf24':'#b45309','#d97706':'#b45309','#22c55e':'#15803d','#16a34a':'#15803d','#4ade80':'#15803d','#10b981':'#047857','#059669':'#047857','#ef4444':'#b91c1c','#f87171':'#b91c1c','#dc2626':'#b91c1c','#fb7185':'#be123c','#3b82f6':'#1d4ed8','#60a5fa':'#1d4ed8','#6366f1':'#4338ca','#818cf8':'#4338ca','#a855f7':'#7e22ce','#a78bfa':'#6d28d9','#8b5cf6':'#6d28d9','#ec4899':'#be185d','#f472b6':'#be185d','#14b8a6':'#0f766e','#0d9488':'#0f766e','#06b6d4':'#0e7490','#0891b2':'#0e7490','#f97316':'#c2410c'}[String(c).toLowerCase()] || c); };
      var React = ctx.React;
      ...
      function header() {
        return h('div', { className: 'no-print', style: { display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap' } },
          h('button', { onClick: function() { setSelHubTool(null); }, 'aria-label': 'Back to SEL Hub',
            style: { background: 'rgba(255,255,255,0.05)', border: '1px solid #334155', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', color: _paFg('#cbd5e1'), fontSize: 14 } }, '← Back'),
          h('div', { style: { flex: 1, minWidth: 260 } },
            h('h2', { style: { margin: 0, color: _paFg('#a5b4fc'), fontSize: 22, fontWeight: 900 } }, '🧭 PATH'),
            h('div', { style: { fontSize: 12, color: _paFg('#94a3b8'), marginTop: 4, lineHeight: 1.5 } }, 'Planning Alternative Tomorrows with Hope: a futures-planning visual.')
          )
        );
      }
          ...
          tabs.map(function(t) {
            ...
            return h('button', { key: t.id, onClick: function() { goto(t.id); },
              ...
              style: { padding: '6px 12px', borderRadius: 8, border: '1px solid ' + (active ? '#818cf8' : '#334155'),
                background: active ? 'rgba(129,140,248,0.18)' : _paBg('#1e293b'),
                color: active ? _paFg('#e0e7ff') : _paFg('#cbd5e1'), cursor: 'pointer', fontSize: 12, fontWeight: 700 } },
              t.icon + ' ' + t.label);
          })
        );
      }
      ...
      function softPointer() {
        return h('div', { className: 'no-print',
          style: { marginTop: 16, padding: '8px 12px', borderRadius: 8, background: 'rgba(15,23,42,0.5)', border: '1px solid #334155', fontSize: 11, color: _paFg('#94a3b8'), lineHeight: 1.5, fontStyle: 'italic' }
        },
      ...
      function renderOverview() {
        ...
        return h('div', null,
          h('div', { style: { padding: 14, borderRadius: 12, background: 'linear-gradient(135deg, rgba(129,140,248,0.16) 0%, rgba(15,23,42,0.4) 60%)', border: '1px solid rgba(129,140,248,0.4)', marginBottom: 14 } },
            h('div', { style: { display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' } },
              h('div', { style: { flex: 1, minWidth: 200 } },
                h('div', { style: { fontSize: 22, fontWeight: 900, color: _paFg('#e0e7ff') } }, d.name ? (d.name + '’s PATH') : 'My PATH'),
                h('div', { style: { fontSize: 12, color: _paFg('#94a3b8'), marginTop: 4 } },
          ...
          h('div', { style: { padding: 12, borderRadius: 8, background: 'rgba(251,191,36,0.10)', borderTop: '1px solid rgba(251,191,36,0.3)', borderRight: '1px solid rgba(251,191,36,0.3)', borderBottom: '1px solid rgba(251,191,36,0.3)', borderLeft: '3px solid #fbbf24', marginBottom: 14, fontSize: 12.5, color: _paFg('#fde68a'), lineHeight: 1.65 } },
          ...
          h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 10 } },
            STAGES.map(function(s, i) {
              ...
              return h('button', { key: s.key,
                ...
                style: { textAlign: 'left', padding: 14, borderRadius: 10, border: '1px solid ' + (done ? s.color + 'aa' : '#334155'), borderLeft: '4px solid ' + s.color, background: done ? s.color + '14' : '#0f172a', cursor: 'pointer', color: _paFg('#e2e8f0') } },
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 } },
                  h('span', { style: { fontSize: 22 } }, s.icon),
                  h('div', null,
                    h('div', { style: { fontSize: 10, color: _paFg('#94a3b8'), fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 } }, 'Stage ' + (i + 1)),
                    h('div', { style: { fontSize: 14, fontWeight: 800, color: done ? s.color : _paFg('#e0e7ff') } }, s.label)
                  ),
                  done ? h('span', { style: { marginLeft: 'auto', fontSize: 10, color: s.color, fontWeight: 700 } }, '✓') : null
                ),
                h('div', { style: { fontSize: 12, color: _paFg('#94a3b8'), lineHeight: 1.55 } },
          ...
          h('div', { style: { display: 'flex', gap: 10, marginTop: 14, flexWrap: 'wrap' } },
            h('button', { onClick: function() { goto('print'); }, 'aria-label': 'Print my PATH',
              style: { padding: '10px 20px', borderRadius: 10, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #4f46e5 0%, #6366f1 100%)', color: _paFg('#fff'), fontWeight: 800, fontSize: 14 } },
              '🖨 Print my PATH')
          ),
          softPointer()
        );
      ...
      function renderEdit() {
        ...
        return h('div', null,
          h('div', { style: { padding: 10, borderRadius: 8, background: _paBg('#0f172a'), border: '1px solid #1e293b', marginBottom: 12 } },
            h('label', { htmlFor: 'path-name', style: { display: 'block', fontSize: 11, color: _paFg('#94a3b8'), fontWeight: 700, marginBottom: 4 } }, 'Whose PATH is this?'),
            h('input', { id: 'path-name', type: 'text', value: d.name || '',
              placeholder: 'Your name',
              onChange: function(e) { setPATH({ name: e.target.value }); },
              style: { width: '100%', padding: 8, borderRadius: 6, border: '1px solid #334155', background: _paBg('#1e293b'), color: _paFg('#e2e8f0'), fontSize: 13 } })
          ),
          ...
          h('div', { role: 'tablist', 'aria-label': 'PATH stages', style: { display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 12 } },
            STAGES.map(function(s, i) {
              ...
              return h('button', { key: s.key, role: 'tab', 'aria-selected': act,
                ...
                style: { padding: '6px 10px', borderRadius: 6, border: '1px solid ' + (act ? s.color : '#334155'), background: act ? s.color + '22' : _paBg('#1e293b'), color: act ? _paInk(s.color) : (done ? _paFg('#cbd5e1') : _paFg('#94a3b8')), cursor: 'pointer', fontSize: 11, fontWeight: 700 } },
                (done ? '✓' : (i + 1)) + ' · ' + s.label
              );
            })
          ),
          ...
          h('div', { style: { padding: 18, borderRadius: 12, background: 'linear-gradient(135deg, ' + active.color + '14 0%, rgba(15,23,42,0.4) 60%)', border: '1px solid ' + active.color + '66', borderLeft: '4px solid ' + active.color, marginBottom: 12 } },
            h('div', { style: { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 } },
              ...
              h('div', null,
                h('div', { style: { fontSize: 10, color: _paFg('#94a3b8'), fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 } }, 'Stage ' + (activeIdx + 1) + ' of ' + STAGES.length),
                h('h3', { style: { margin: 0, color: active.color, fontSize: 20, fontWeight: 900 } }, active.label)
              )
            ),
            h('p', { style: { margin: '0 0 12px', color: _paFg('#e2e8f0'), fontSize: 13.5, lineHeight: 1.7 } }, active.blurb),

            h('label', { htmlFor: 'path-resp', style: { display: 'block', fontSize: 11, color: _paFg('#94a3b8'), fontWeight: 700, marginBottom: 4 } }, active.label),
            h('textarea', { id: 'path-resp', value: resp,
              ...
              style: { width: '100%', minHeight: 180, padding: 12, borderRadius: 8, border: '1px solid #334155', background: _paBg('#1e293b'), color: _paFg('#e2e8f0'), fontSize: 14, fontFamily: 'inherit', lineHeight: 1.7, resize: 'vertical' } })
          ),
          ...
          h('div', { style: { display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' } },
            ...
            h('button', { onClick: goNext, disabled: activeIdx === STAGES.length - 1, 'aria-label': 'Next stage',
              style: { padding: '8px 14px', borderRadius: 8, border: 'none', background: activeIdx === STAGES.length - 1 ? _paBg('#1e293b') : _paSolid(active.color), color: activeIdx === STAGES.length - 1 ? _paFg('#475569') : '#fff', cursor: activeIdx === STAGES.length - 1 ? 'not-allowed' : 'pointer', fontWeight: 800, fontSize: 13 } }, 'Next →')
          ),
      ...
      function renderPrintView() {
        return h('div', null,
          h('div', { className: 'no-print', style: { display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap', padding: 12, background: 'rgba(129,140,248,0.10)', borderRadius: 8, border: '1px solid rgba(129,140,248,0.3)' } },
            h('div', { style: { flex: 1, minWidth: 200, fontSize: 12.5, color: _paFg('#e0e7ff'), lineHeight: 1.55 } },
          ...
          h('div', {
            ...
            style: { maxWidth: 760, margin: '0 auto', padding: 32, background: _paBg('#fff'), color: _paFg('#0f172a'), borderRadius: 8, border: '1px solid #cbd5e1', fontFamily: '"Helvetica Neue", Arial, sans-serif' }
          },
            h('style', null,
            ...
            h('div', { style: { paddingBottom: 14, marginBottom: 20, borderBottom: '3px solid #4f46e5' } },
              h('div', { style: { fontSize: 10, color: _paFg('#64748b'), textTransform: 'uppercase', letterSpacing: 1, fontWeight: 700, marginBottom: 2 } }, 'PATH · Planning Alternative Tomorrows with Hope'),
              h('h1', { style: { margin: 0, fontSize: 28, fontWeight: 900 } }, d.name ? (d.name + '’s PATH') : 'My PATH'),
              d.lastUpdated ? h('div', { style: { fontSize: 12, color: _paFg('#475569'), marginTop: 4 } }, 'Updated ' + d.lastUpdated) : null
            ),

            STAGES.map(function(s, i) {
              var v = (d.responses || {})[s.key] || '';
              return h('div', { key: s.key, style: { marginBottom: 18, pageBreakInside: 'avoid' } },
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderRadius: 4, marginBottom: 8, background: _paSolid(s.color), color: _paFg('#fff') } },
                  h('span', { style: { fontSize: 18 } }, s.icon),
                  h('span', { style: { fontSize: 11, fontWeight: 700 } }, 'Stage ' + (i + 1) + ' ·'),
                  h('span', { style: { fontSize: 14, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 0.5 } }, s.label)
                ),
                v ? h('p', { style: { margin: '0 8px', color: _paFg('#0f172a'), fontSize: 13, lineHeight: 1.7, whiteSpace: 'pre-wrap' } }, v)
                  : h('div', { style: { margin: '0 8px', padding: 8, fontSize: 11, color: _paFg('#475569'), fontStyle: 'italic' } }, '(not filled in)')
              );
            }),

            h('div', { style: { marginTop: 20, paddingTop: 12, borderTop: '1px solid #cbd5e1', fontSize: 9, color: _paFg('#475569'), textAlign: 'center', lineHeight: 1.5 } },
      ...
      function renderAbout() {
        return h('div', null,
          ...
          h('div', { style: { padding: 16, borderRadius: 12, background: _paBg('#0f172a'), border: '1px solid #1e293b', marginBottom: 12 } },
            h('h3', { style: { margin: '0 0 10px', color: _paFg('#a5b4fc'), fontSize: 16 } }, 'What PATH is'),
            h('p', { style: { margin: '0 0 10px', color: _paFg('#e2e8f0'), fontSize: 13.5, lineHeight: 1.7 } },
            ...
            h('p', { style: { margin: 0, color: _paFg('#e2e8f0'), fontSize: 13.5, lineHeight: 1.7 } },
          ...
          h('div', { style: { padding: 16, borderRadius: 12, background: _paBg('#0f172a'), border: '1px solid #1e293b', marginBottom: 12 } },
            h('h3', { style: { margin: '0 0 10px', color: _paFg('#a5b4fc'), fontSize: 16 } }, 'PATH and MAPS'),
            h('p', { style: { margin: 0, color: _paFg('#cbd5e1'), fontSize: 13, lineHeight: 1.7 } },
          ...
          // Sources
          h('div', { style: { padding: 16, borderRadius: 12, background: _paBg('#0f172a'), border: '1px solid #1e293b', marginBottom: 12 } },
            h('h3', { style: { margin: '0 0 10px', color: _paFg('#a5b4fc'), fontSize: 16 } }, '📚 Sources and learn more'),
            h('div', { style: { fontSize: 12, color: _paFg('#94a3b8'), marginBottom: 10, lineHeight: 1.55 } }, 'Authoritative resources you can use to verify the evidence base for PATH or to learn more.'),
          ...
          h('div', { style: { padding: 16, borderRadius: 12, background: 'rgba(245,158,11,0.08)', borderTop: '1px solid rgba(245,158,11,0.3)', borderRight: '1px solid rgba(245,158,11,0.3)', borderBottom: '1px solid rgba(245,158,11,0.3)', borderLeft: '3px solid #f59e0b', marginBottom: 12 } },
            h('h3', { style: { margin: '0 0 10px', color: _paFg('#fcd34d'), fontSize: 15 } }, '⚖️ Honest limits'),
            h('ul', { style: { margin: 0, padding: '0 0 0 20px', color: _paFg('#fde68a'), fontSize: 13, lineHeight: 1.75 } },
          ...
          h('div', { style: { padding: 12, borderRadius: 10, background: 'rgba(129,140,248,0.10)', borderTop: '1px solid rgba(129,140,248,0.3)', borderRight: '1px solid rgba(129,140,248,0.3)', borderBottom: '1px solid rgba(129,140,248,0.3)', borderLeft: '3px solid #818cf8', fontSize: 12.5, color: _paFg('#e0e7ff'), lineHeight: 1.6 } },
      ...
      function sourceCard(authorYear, title, blurb, url) {
        return h('div', { style: { padding: 10, borderRadius: 8, background: _paBg('#1e293b'), border: '1px solid #334155', marginBottom: 8 } },
          h('div', { style: { fontSize: 11, color: _paFg('#a5b4fc'), fontWeight: 700, marginBottom: 2 } }, authorYear),
          url
            ? h('a', { href: url, target: '_blank', rel: 'noopener noreferrer',
                style: { fontSize: 13, color: _paFg('#e0e7ff'), fontWeight: 700, textDecoration: 'underline', display: 'block', marginBottom: 4 } }, title + ' ↗')
            : h('div', { style: { fontSize: 13, color: _paFg('#e0e7ff'), fontWeight: 700, marginBottom: 4 } }, title),
          h('div', { style: { fontSize: 12, color: _paFg('#cbd5e1'), lineHeight: 1.55 } }, blurb)
        );
      }
