...
(async()=>{
 ...
 async function audit(name){
  ...
  results.states.push({name,...result});fs.writeFileSync(path.join(__dirname,'browser-loaded-workspace.json'),JSON.stringify(results,null,2));console.log(name+': '+result.violations.length+' axe rules; width '+result.scrollWidth+'/'+result.clientWidth);
 }
 try{
 ...
 results.blockedOrigins=[...blocked];results.pageErrors=errors;results.finishedAt=new Date().toISOString();fs.writeFileSync(path.join(__dirname,'browser-loaded-workspace.json'),JSON.stringify(results,null,2));await browser.close();await new Promise(r=>server.close(r));
})();
