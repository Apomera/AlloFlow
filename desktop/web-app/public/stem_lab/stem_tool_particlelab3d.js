        ...
        h('div', { className: 'rounded-2xl border border-cyan-200 bg-gradient-to-r from-slate-950 via-cyan-950 to-slate-900 p-5 text-white shadow-xl' },
          h('div', { className: 'flex flex-wrap items-start justify-between gap-3' },
            ...
            h('div', { className: 'flex flex-wrap items-center justify-end gap-2' },
              ...
              h('a', { href: '#particle-stage', className: 'inline-flex min-h-11 items-center rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-3 py-2 text-xs font-black text-cyan-100 transition hover:bg-cyan-300/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200 sm:min-h-9', 'aria-label': 'Jump to the 3D particle chamber' }, 'Enter 3D chamber')
            )
          )
        ),
...
,
        ...
        h('div', { className: 'grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]' },
          h('section', { id: 'particle-stage', 'data-fullscreen': fsActive ? 'true' : 'false', ref: stageRef, tabIndex: -1, onKeyDown: onStageKeyDown, className: 'relative min-w-0 overflow-hidden border border-cyan-500/30 bg-slate-950 shadow-2xl shadow-cyan-950/40 ring-1 ring-white/5 ' + (fsActive ? 'h-screen rounded-none flex flex-col' : 'rounded-[24px]'), style: cssFullscreen ? { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100dvh', zIndex: 99990, borderRadius: 0 } : undefined, 'aria-label': 'Three-dimensional particle simulation', 'aria-busy': chamberLive ? 'false' : 'true' },
            ...
            showHud && !fsActive && h('div', { id: 'particle-experiment-runway', role: 'region', 'aria-label': 'Experiment loop', className: 'border-b border-slate-800 bg-slate-900/80 px-4 py-2.5' },
              ...
              h('div', { className: 'mt-2 grid grid-cols-3 gap-1.5', role: 'list', 'aria-label': 'Experiment steps' },
            ...
            fsActive && showHud && showFullscreenConditions && h('section', { id: 'particle-fullscreen-conditions', className: 'max-h-[38vh] overflow-y-auto border-t border-cyan-400/30 bg-slate-900/95 px-4 py-3 text-white', 'aria-label': 'Fullscreen experiment conditions' },
              h('div', { className: 'mb-2 flex items-center justify-between gap-3' }, h('h3', { className: 'text-xs font-black uppercase tracking-[0.16em] text-cyan-200' }, 'Experiment conditions'), h('button', { type: 'button', onClick: function () { setShowFullscreenConditions(false); if (conditionsToggleRef.current) conditionsToggleRef.current.focus(); }, className: 'min-h-11 rounded-lg border border-slate-600 px-3 py-2 text-xs font-bold text-white hover:bg-slate-800', 'aria-label': 'Close fullscreen experiment conditions' }, 'Close')),
              h('div', { className: 'grid gap-x-5 gap-y-3 sm:grid-cols-2 xl:grid-cols-3' },
                h('label', { className: 'block text-xs font-bold' }, 'Temperature ', h('output', { className: 'text-cyan-300' }, temperature + ' K'), h('input', { type: 'range', min: 40, max: 900, step: 10, value: temperature, onChange: function (e) { setTemp(e.target.value); }, className: 'mt-1 block min-h-11 w-full accent-cyan-400', 'aria-label': 'Fullscreen temperature in kelvin' })),
                ...
                h('label', { className: 'block text-xs font-bold' }, 'Particle count ', h('output', { className: 'text-cyan-300' }, countDraft), h('input', { type: 'range', min: 24, max: 120, step: 8, value: countDraft, onChange: function (e) { var value = Number(e.target.value); setCountDraft(value); scheduleRebuild(function () { setCount(value); setSelectedParticle(function (old) { return Math.min(old, value - 1); }); setRunning(false); setResetKey(function (k) { return k + 1; }); persist({ count: value, selectedParticle: Math.min(selectedParticle, value - 1) }); }); }, className: 'mt-1 block min-h-11 w-full accent-cyan-400', 'aria-label': 'Fullscreen particle count' })),
                h('div', { className: 'flex flex-wrap items-end gap-2', role: 'group', 'aria-label': 'Fullscreen pump and lid' }, pumpButton('fs-a', 0), transportMode && pumpButton('fs-b', 1), releaseButton('fs')),
                h('label', { className: 'block text-xs font-bold' }, 'Container edge ', h('output', { className: 'text-cyan-300' }, boxSize + ' u'), h('input', { type: 'range', min: 7, max: 18, step: 1, value: boxSize, 'aria-disabled': holdPressure ? 'true' : undefined, onChange: function (e) { if (holdPressure) { announce('The container is under pressure hold; turn the hold off to move the walls by hand.'); return; } var value = Number(e.target.value); setBoxSize(value); persistLater({ boxSize: value }); }, className: 'mt-1 block min-h-11 w-full accent-cyan-400', 'aria-label': 'Fullscreen container edge length' })),
                h('label', { className: 'block text-xs font-bold' }, 'Collision diameter ', h('output', { className: 'text-cyan-300' }, particleDiameterDraft.toFixed(2) + ' u'), h('input', { type: 'range', min: 0.36, max: 0.9, step: 0.06, value: particleDiameterDraft, onChange: function (e) { var value = Number(e.target.value); setParticleDiameterDraft(value); scheduleRebuild(function () { setParticleDiameter(value); setRunning(false); setHistory([]); setResetKey(function (k) { return k + 1; }); persist({ particleDiameter: value }); }); }, className: 'mt-1 block min-h-11 w-full accent-cyan-400', 'aria-label': 'Fullscreen particle collision diameter' })),
                h('label', { className: 'block text-xs font-bold' }, 'Attraction ', h('output', { className: 'text-cyan-300' }, attraction.toFixed(2)), h('input', { type: 'range', min: 0, max: 1.5, step: 0.05, value: attraction, onChange: function (e) { var value = Number(e.target.value); setAttraction(value); persistLater({ attraction: value }); }, className: 'mt-1 block min-h-11 w-full accent-cyan-400', 'aria-label': 'Fullscreen interparticle attraction strength' })),
                h('label', { className: 'block text-xs font-bold' }, 'Gravity ', h('output', { className: 'text-amber-300' }, gravity > 0.01 ? gravity.toFixed(1) + ' g*' : 'Off'), h('input', { type: 'range', min: 0, max: 2, step: 0.1, value: gravity, onChange: function (e) { var value = Number(e.target.value); setGravity(value); persistLater({ gravity: value }); }, className: 'mt-1 block min-h-11 w-full accent-amber-400', 'aria-label': 'Fullscreen downward gravity field strength' })),
                transportMode && h('label', { className: 'block text-xs font-bold' }, 'Membrane permeability ', h('output', { className: 'text-violet-300' }, Math.round(permeability * 100) + '%'), h('input', { type: 'range', min: 0, max: 1, step: 0.05, value: permeability, disabled: !membrane, onChange: function (e) { var value = Number(e.target.value); setPermeability(value); persistLater({ permeability: value }); }, className: 'mt-1 block min-h-11 w-full accent-violet-400 disabled:opacity-40', 'aria-label': 'Fullscreen membrane permeability' }))
              )
            ),
            showHud && h('div', { id: 'particle-secondary-row', className: 'flex flex-wrap items-center gap-3 border-t border-slate-700 p-3' },
              ...
              h('button', { type: 'button', onClick: function () { setAudio(!audioCues); }, 'aria-pressed': audioCues, 'aria-label': 'Audio cues. A soft tone follows pressure, wall hits click, and the pump whooshes.', className: 'rounded-lg px-3 py-2 text-sm font-bold ' + (audioCues ? 'bg-emerald-300 text-slate-950' : 'bg-slate-800 text-white hover:bg-slate-700') }, '\uD83D\uDD0A Audio cues')
              ),
          ...
          h('aside', { className: 'space-y-4' },
            h('div', { className: 'rounded-2xl border border-slate-200 bg-white p-4 shadow-sm' },
              ...
              h('div', { className: 'mt-3 rounded-xl border border-cyan-200 bg-cyan-50 p-3' },
                ...
                h('div', { className: 'mt-2 flex flex-wrap gap-2', role: 'group', 'aria-label': 'Pump and lid' }, pumpButton('side-a', 0), transportMode && pumpButton('side-b', 1), releaseButton('side')),
              ...
              h('input', { type: 'range', min: 7, max: 18, step: 1, value: boxSize, 'aria-disabled': holdPressure ? 'true' : undefined, onChange: function (e) { if (holdPressure) { announce('The container is under pressure hold; turn the hold off to move the walls by hand.'); return; } var value = Number(e.target.value); setBoxSize(value); persistLater({ boxSize: value }); }, className: 'mt-1 w-full accent-cyan-600', 'aria-label': 'Container edge length and volume' }),
              ...
              h('input', { type: 'range', min: 0, max: 1.5, step: 0.05, value: attraction, onChange: function (e) { var value = Number(e.target.value); setAttraction(value); persistLater({ attraction: value }); }, className: 'mt-1 w-full accent-cyan-600', 'aria-label': 'Interparticle attraction strength' }),
              ...
              h('input', { type: 'range', min: 0, max: 2, step: 0.1, value: gravity, onChange: function (e) { var value = Number(e.target.value); setGravity(value); persistLater({ gravity: value }); }, className: 'mt-1 w-full accent-amber-500', 'aria-label': 'Downward gravity field strength', 'aria-valuetext': gravity > 0.01 ? gravity.toFixed(1) + ' model g' : 'off' }),
              ...
              transportMode && h('div', { className: 'mt-4 rounded-xl border border-violet-200 bg-violet-50 p-3' },
                ...
                h('input', { type: 'range', min: 0, max: 1, step: 0.05, value: permeability, disabled: !membrane, onChange: function (e) { var value = Number(e.target.value); setPermeability(value); persistLater({ permeability: value }); }, className: 'mt-1 w-full accent-violet-600 disabled:opacity-40', 'aria-label': 'Membrane permeability', 'aria-valuetext': Math.round(permeability * 100) + ' percent' }),
                h('label', { className: 'mt-3 block text-xs font-bold text-slate-700' }, 'Species preference'),
                h('select', { value: membraneSelectivity, disabled: !membrane, onChange: function (e) { var value = e.target.value; setMembraneSelectivity(value); persist({ membraneSelectivity: value }); }, className: 'mt-1 w-full rounded-lg border border-violet-500 bg-white px-3 py-2 text-xs text-slate-800 disabled:opacity-40', 'aria-label': 'Membrane species preference' }, h('option', { value: 'both' }, 'A and B equally'), h('option', { value: 'a' }, 'Prefer particle A'), h('option', { value: 'b' }, 'Prefer particle B')),
                ...
                h('input', { type: 'range', min: 0.5, max: 3, step: 0.1, value: massRatioBDraft, onChange: function (e) { var value = Number(e.target.value); setMassRatioBDraft(value); scheduleRebuild(function () { setMassRatioB(value); setRunning(false); setHistory([]); setResetKey(function (k) { return k + 1; }); persist({ massRatioB: value }); }); }, className: 'mt-1 w-full accent-fuchsia-600', 'aria-label': 'Particle B mass relative to particle A', 'aria-valuetext': massRatioBDraft.toFixed(1) + ' times particle A mass' }),
            ...
            h('div', { className: 'rounded-2xl border border-slate-200 bg-white p-4 shadow-sm' },
              h('div', { className: 'flex flex-wrap items-center justify-between gap-2' }, h('h3', { className: 'font-black text-slate-900' }, 'Live measurements'), h('button', { type: 'button', onClick: describeChamber, className: 'min-h-11 rounded-lg border border-slate-400 bg-white px-3 py-2 text-xs font-black text-slate-800 hover:bg-slate-100 sm:min-h-9', 'aria-label': 'Describe the chamber. Reads a summary of what is in the chamber right now.' }, '🗣 Describe the chamber')),
              ...
              trialComparison && h('div', { className: 'mt-3 overflow-hidden rounded-xl border ' + (trialComparison.fair ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50') },
                ...
                trialGasLaw && !isTransportPreset(trials[0].preset) && h('div', { id: 'particle-gas-law-check', className: 'border-t border-white/70 px-3 py-2', role: 'group', 'aria-label': 'Ideal gas law check' },
