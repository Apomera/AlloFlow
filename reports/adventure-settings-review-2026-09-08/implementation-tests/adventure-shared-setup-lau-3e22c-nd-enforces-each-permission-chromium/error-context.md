...
- Name: adventure-shared-setup.spec.ts >> launch retains allowed student edits and enforces each permission
...
```ts
  ...
  4  | 
  5  | async function mount(page: any, surface = 'launch', teacher = false, permissions: any = {}, mode = 'choice') {
  ...
  9  |   await page.evaluate(({ surface, teacher, permissions, mode }) => {
  ...
  20 |   },{surface,teacher,permissions,mode});
  21 | }
  22 | 
...
> 26 |     await expect(page.getByLabel('Interaction mode',{exact:true})).toBeDisabled();
  ...
  66 |   await mount(page,'launch',true,{},'system');
  67 |   // The shared surface supplies native styles independently of Tailwind loading.
  68 |   if(theme!=='light')await page.locator('[data-adventure-settings]').first().evaluate((el:any,theme:string)=>{
  69 |     for(const [key,value]of Object.entries({'--as-ink':'#f8fafc','--as-muted':'#cbd5e1','--as-bg':theme==='contrast'?'#000':'#0f172a','--as-wash':theme==='contrast'?'#000':'#1e293b','--as-line':theme==='contrast'?'#fff':'#94a3b8','--as-accent':theme==='contrast'?'#fde047':'#a5b4fc'}))el.style.setProperty(key,value);
  70 |   },theme);
  71 |   await page.locator('details').evaluateAll((nodes:any[])=>nodes.forEach(n=>n.open=true));
  72 |   for(const width of [375,1200]){
  ...
  75 |     expect(await page.evaluate(async()=> (await (window as any).axe.run(document.querySelector('main'),{runOnly:{type:'tag',values:['wcag2a','wcag2aa','wcag22aa']}})).violations.map((v:any)=>({id:v.id,nodes:v.nodes.map((n:any)=>n.target)})))).toEqual([]);
  76 |     await page.screenshot({path:info.outputPath('shared-'+theme+'-'+width+'.png'),fullPage:true});
  77 |   }
  78 | });
  79 | 
```