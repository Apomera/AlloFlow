...
await page.waitForFunction(()=>state._circuit.cameraPanX>0);
assert.equal(await page.evaluate(()=>StemLab.solveCircuit(state._circuit).current),beforeCurrent);
assert.equal(await page.evaluate(()=>state._circuit.cameraPanY),0);
const yaw=await page.evaluate(()=>state._circuit.cameraPanX),scroll=await page.evaluate(()=>scrollY);
...
assert.equal(await page.evaluate(()=>state._circuit.cameraPanX),yaw);
assert.equal(await page.locator('.circuit-scene-viewport').getAttribute('data-dragging'),'false');
assert.deepEqual(errors,[]);
fs.writeFileSync(path.join(out,'framing15-touch-results.json'),JSON.stringify({checks:['horizontal touch pans','vertical touch scrolls page','touch preserves circuit readings','pointer cancellation ends drag'],errors},null,2));
console.log('Touch pan and page-scroll checks passed.');
