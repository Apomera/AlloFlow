...
// Run: node dev-tools/watercycle_pilot_stream_vegetation_qa.cjs
const fs = require('fs');
const path = require('path');
const assert = require('node:assert/strict');
const { chromium } = require('playwright');
const ROOT = process.cwd();
const out = path.join(ROOT, 'scratch', 'water-stream-vegetation-review');
...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(()=>pilotReview.scene.getObjectByName('pilot-stream-bank-grasses').userData.surfaceShader);
    const planting=await page.evaluate(()=>{
      const scene=pilotReview.scene,grass=scene.getObjectByName('pilot-stream-bank-grasses');
      const surfaces=[scene.getObjectByName('pilot-stream-wet-banks'),scene.getObjectByName('pilot-stream-terrain-banks')];
      const ray=new THREE.Raycaster(new THREE.Vector3(),new THREE.Vector3(0,-1,0)),matrix=new THREE.Matrix4(),position=new THREE.Vector3();
      let rootError=0,missing=0,inLake=0;
      for(let i=0;i<grass.count;i++){
        grass.getMatrixAt(i,matrix);position.setFromMatrixPosition(matrix);
        ray.ray.origin.set(position.x,500,position.z);const hits=ray.intersectObjects(surfaces,false);
        if(!hits.length)missing++;else rootError=Math.max(rootError,Math.abs(position.y-(hits[0].point.y-.035)));
        if(Math.hypot(position.x-268,position.z+104)<36)inLake++;
      }
      let floating=0,buried=0,checked=0;
      for(const name of ['pilot-stream-bank-stones','pilot-stream-bank-gravel']){
        const mesh=scene.getObjectByName(name),vertices=mesh.geometry.attributes.position,vertex=new THREE.Vector3();
        for(let i=0;i<mesh.count;i++){
          mesh.getMatrixAt(i,matrix);position.setFromMatrixPosition(matrix);
          if(Math.hypot(position.x-268,position.z+104)<35)continue;
          ray.ray.origin.set(position.x,500,position.z);const hits=ray.intersectObjects(surfaces,false);if(!hits.length)continue;
          let min=Infinity,max=-Infinity;for(let j=0;j<vertices.count;j++){vertex.fromBufferAttribute(vertices,j).applyMatrix4(matrix);min=Math.min(min,vertex.y);max=Math.max(max,vertex.y);}
          if(min>hits[0].point.y+.01)floating++;if(max<hits[0].point.y+.01)buried++;checked++;
        }
      }
      window.bankGrassSummer=grass.material.color.getHex();
      return {count:grass.count,rootError,missing,inLake,floating,buried,checked,
        finite:Array.from(grass.instanceMatrix.array).every(Number.isFinite)&&Array.from(grass.geometry.attributes.normal.array).every(Number.isFinite),
        anchoredBases:Array.from(grass.geometry.attributes.reedFlex.array).every((v,i)=>grass.geometry.attributes.position.getY(i)!==0||v===0),
        sharedTime:grass.userData.surfaceShader.uniforms.pilotStreamTime===scene.getObjectByName('pilot-stream-water').userData.surfaceShader.uniforms.pilotStreamTime};
    });assert(planting.count>=48&&planting.count<=72);assert(planting.rootError<.001,JSON.stringify(planting));assert.equal(planting.missing,0);assert.equal(planting.inLake,0);assert.equal(planting.floating,0);assert.equal(planting.buried,0);assert(planting.checked>150&&planting.finite&&planting.anchoredBases&&planting.sharedTime);
    ...
    assert.equal(await page.evaluate(()=>pilotReview.scene.getObjectByName('pilot-stream-bank-grasses').material.color.getHex()!==bankGrassSummer),true);
    ...
    assert.equal(await page.evaluate(()=>pilotReview.scene.getObjectByName('pilot-stream-bank-grasses').visible),false);
    ...
    await page.evaluate(()=>{
      window.streamDisposals={geometry:0,texture:0,channel:0,riffleGeometry:0,riffleMaterial:0,cascadeGeometry:0,cascadeMaterial:0,terrainGeometry:0,terrainMaterial:0,terrainMask:0,grassGeometry:0,grassMaterial:0};const stones=pilotReview.scene.getObjectByName('pilot-stream-bank-stones');
      ...
      const grass=pilotReview.scene.getObjectByName('pilot-stream-bank-grasses');
      grass.geometry.addEventListener('dispose',()=>streamDisposals.grassGeometry++);
      grass.material.addEventListener('dispose',()=>streamDisposals.grassMaterial++);
      mountWater({wcMode:'explorer'});
    });assert.deepEqual(await page.evaluate(()=>streamDisposals),{geometry:1,texture:1,channel:1,riffleGeometry:1,riffleMaterial:1,cascadeGeometry:1,cascadeMaterial:1,terrainGeometry:1,terrainMaterial:1,terrainMask:1,grassGeometry:1,grassMaterial:1});assert.deepEqual(errors,[]);
    console.log('PASS: grass roots meet bank surface, no lake planting, rocks intersect bank rather than float or disappear, shared grass clock and anchored blade bases, seasonal vegetation and subsurface visibility, grass cleanup, terrain-bank edges match sampled land, finite geometry and normals, distinct wider terrain mask, seasonal bank colors, groundwater cutaway, terrain resource cleanup, cascade mesh aligned with water, quiet and whitewater reaches, bounded opacity strength, shared clock, cascade resource cleanup, bank-color and cross-channel attributes, finite riffles, shared water/riffle clock, winter suppression, finite stream detail, paused and reduced-motion ripples/flow, both learner views, real runoff-to-water transition, winter ice, desert visibility, mobile accessibility, shared resource disposal, no WebGL errors.');
