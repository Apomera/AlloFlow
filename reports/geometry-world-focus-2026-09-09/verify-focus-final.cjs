...
harness+='('+(async function verifyFocusCreation(){
  ...
  const screenshot=name=>page.screenshot({path:path.join(out,'focus-final-'+name+'.png'),timeout:60000});
  ...
  async function frame(){return page.evaluate(()=>{
    ...
    const modelRect={left:canvas.left+(minX+1)*canvas.width/2,right:canvas.left+(maxX+1)*canvas.width/2,top:canvas.top+(1-maxY)*canvas.height/2,bottom:canvas.top+(1-minY)*canvas.height/2};
    const obstacleOverlaps=[];for(const selector of ['.gw-hotbar','.gw-shape-tray','.gw-action-bar','.gw-touch-actions','.gw-touch-joystick','.gw-touch-look-panel','.gwe-builder-dock','.gwe-focus-return']){const node=document.querySelector(selector);if(!node)continue;const s=getComputedStyle(node),r=node.getBoundingClientRect();if(s.display==='none'||s.visibility==='hidden'||!r.width||!r.height)continue;const width=Math.min(modelRect.right,r.right)-Math.max(modelRect.left,r.left),height=Math.min(modelRect.bottom,r.bottom)-Math.max(modelRect.top,r.top);if(width>1&&height>1)obstacleOverlaps.push({selector,width,height});}
    const rectOf=node=>{if(!node)return null;const r=node.getBoundingClientRect(),s=getComputedStyle(node);return {x:r.x,y:r.y,width:r.width,height:r.height,visible:s.display!=='none'&&s.visibility!=='hidden'};};
    const returnRect=rectOf(document.querySelector('.gwe-focus-return')),lookRect=rectOf(document.querySelector('.gw-touch-look-panel'));
    const returnLookOverlap=!!(returnRect?.visible&&lookRect?.visible&&Math.min(returnRect.x+returnRect.width,lookRect.x+lookRect.width)-Math.max(returnRect.x,lookRect.x)>1&&Math.min(returnRect.y+returnRect.height,lookRect.y+lookRect.height)-Math.max(returnRect.y,lookRect.y)>1);
    return {count:selected.measurement.count,vertices:points.length,obstacleOverlaps,returnRect,lookRect,returnLookOverlap,minX,maxX,minY,maxY,minZ:Math.min(...points.map(p=>p[2])),maxZ:Math.max(...points.map(p=>p[2])),canvas:{x:canvas.x,y:canvas.y,width:canvas.width,height:canvas.height},camera:__focusPose(),safeRect:en._showcase?null:en._creationFocus?.frame?.rect,manual:!!en._creationFocus?.manual,transitioning:!!en._creationFocus?.transition,dockCollapsed:!!__ctx.toolData.geometryWorld.sandboxDockCollapsed,overflow:document.documentElement.scrollWidth>innerWidth,engineValid:!en._destroyed&&!en._runtimeFailed,activeElement:document.activeElement?.id||document.activeElement?.getAttribute('aria-label')};
  });}
  function checkFrame(row,label){check(row.minX>-0.99&&row.maxX<0.99&&row.minY>-0.99&&row.maxY<0.99&&row.minZ>-1&&row.maxZ<1,label+': every actual mesh vertex fits the camera frustum');check(!row.overflow&&row.engineValid,label+': no page overflow or engine teardown');check(!row.obstacleOverlaps.length,label+': model bounding rectangle does not intersect visible controls');check(!row.returnLookOverlap,label+': Previous view bar does not overlap Look speed');if(row.safeRect){const r=row.safeRect;check(row.minX>=r.left-1e-5&&row.maxX<=r.right+1e-5&&row.minY>=r.bottom-1e-5&&row.maxY<=r.top+1e-5,label+': every selected vertex fits the clear area between controls');}}
  ...
  try{
    ...
    // The last corrected races use actual production animation/render loops.
    // Seed only their starting state in the capture phase of the real Focus click.
    await page.setViewportSize({width:1440,height:900});await stable();await expand(true);
    result.races={baseline:await page.evaluate(async()=>{
      const e=__geoWorldEngine,p=__focusFixture.aim,mesh=e.placeBlock(p.x+1,6,p.z+1,'gold','quarter',3);
      if(!mesh)throw Error('Race fixture block was not placed');window.__focusPopMesh=mesh;
      e.blocksPlaced++;__ctx.updateMulti('geometryWorld',{blocksPlaced:e.blocksPlaced,autoCycle:true});
      return __focusSignature();
    })};
    const raceFocus=page.getByRole('button',{name:actionNames.focus,exact:true});await raceFocus.scrollIntoViewIfNeeded();
    await raceFocus.evaluate(button=>button.addEventListener('click',()=>{
      const e=__geoWorldEngine,m=__focusPopMesh,f=e.scene.fog,b=e.scene.background;
      m.scale.setScalar(.7);m.userData._popT=0;e._popBlocks=(e._popBlocks||[]).filter(x=>x!==m);e._popBlocks.push(m);e._ambientMotionEnabled=true;
      e._envStart=Object.assign({},e._envStart,{fog:[f.color.r,f.color.g,f.color.b],fogNear:f.near,fogFar:f.far,sky:[b.r,b.g,b.b]});
      e._envTarget=Object.assign({},e._envTarget,{fogNear:1,fogFar:5});e._envTransition=.25;e._envDone=false;e._cycleTimer=60.01;
      window.__focusRaceSeed={progress:e._envTransition,timer:e._cycleTimer,environment:e._currentEnv,scale:m.scale.toArray(),fogNear:f.near,fogFar:f.far};
    },{capture:true,once:true}));
    await raceFocus.click();await stable();result.races.frame=await frame();checkFrame(result.races.frame,'Just-placed fractional block');
    result.races.animation=await page.evaluate(()=>{const e=__geoWorldEngine;return {seed:__focusRaceSeed,progress:e._envTransition,timer:e._cycleTimer,environment:e._currentEnv,scale:__focusPopMesh.scale.toArray(),pendingPop:e._popBlocks.includes(__focusPopMesh),fogNear:e.scene.fog.near,depthFar:e._creationFocus.frame.depthFar};});
    check(result.races.animation.progress===.25&&result.races.animation.timer===60.01&&result.races.animation.environment===result.races.animation.seed.environment,'Active environment fade and overdue auto-cycle stay paused during framing');
    check(result.races.animation.scale.every(v=>v===1)&&!result.races.animation.pendingPop,'New fractional block finishes its normal pop animation');
    check(result.races.animation.fogNear>result.races.animation.depthFar,'Fog stays behind the entire selected creation');exact(result.races.baseline,await signature(),'Pop completion preserves exact canonical STL and authored state');
    await page.setViewportSize({width:320,height:700});await stable();result.races.resized=await frame();checkFrame(result.races.resized,'Active focus desktop to phone resize');check(!result.races.resized.manual,'Resize automatically refits until intentional navigation');
    await screenshot('pop-phone');
    await page.evaluate(()=>{const e=__geoWorldEngine;e._envDone=true;e._envTransition=1;e._cycleTimer=0;e._ambientMotionEnabled=false;__ctx.updateMulti('geometryWorld',{autoCycle:false});});
    await clickAction(actionNames.restore);await stable();
    ...
    const target=path.join(out,'focus-final-results.json'),content=JSON.stringify(result,null,2);if(fs.existsSync(target)){const fd=fs.openSync(target,'r+');fs.writeFileSync(fd,content);fs.ftruncateSync(fd,Buffer.byteLength(content));fs.closeSync(fd);}else fs.writeFileSync(target,content);
