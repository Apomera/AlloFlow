...
const InteractiveBlueprintCard = React.memo(({ config, run, isRunning, onStopRun, onRebuildStep, onOpenErrorLog, onCopyDiagnostics, onDownloadDiagnostics, summarizeFailureReason, onPreviewStep, onSaveTemplate, onUpdate, onConfirm, onCancel }) => {
  ...
  const getPlanInstructionalText = (type, existing) => {
    ...
    const defaults = type === 'simplified'
      ...
      : (type === 'analysis' ? { role: 'primary', form: 'original' } : {});
    const candidate = {
      schemaVersion: 1,
    ...
    return contextModule && typeof contextModule.normalizeInstructionalText === 'function'
      ? contextModule.normalizeInstructionalText(candidate)
      : candidate;
  };
  ...
  const runtimeMatrixUnavailable = !!(run && (
      run.generationMatrixUnavailable === true
      || (run.generationMatrixUnavailable === undefined && Object.values(run.rows || {}).some(row =>
          row && row.generationMatrixUnavailable === true))
  ));
  ...
  const blueprintMatrixUnavailable = run && typeof run.generationMatrixUnavailable === 'boolean'
      ? runtimeMatrixUnavailable
      : (plannedMatrixUnavailable || !blueprintMatrixModuleReady);
  const blueprintMatrixReady = !blueprintMatrixUnavailable && items.length > 0
  ...
  const configuredLanguages = blueprintSettings.leveledTextLanguage === 'All Selected Languages'
      ? Array.from(new Set(['English', ...(Array.isArray(blueprintSettings.selectedLanguages) ? blueprintSettings.selectedLanguages : [])]))
      : [blueprintSettings.leveledTextLanguage || blueprintSettings.language || 'English'];
  const resolvedTranslationTarget = blueprintSettings.resolvedTranslationTarget || blueprintSettings.translationTarget || null;
  const embeddedGlossaryLanguages = items.some(item => item && item.type === 'glossary')
      ? Array.from(new Set(Array.isArray(blueprintSettings.selectedLanguages) ? blueprintSettings.selectedLanguages.filter(Boolean) : []))
      : [];
  ...
  const readableSettingName = (field) => ({
      gradeLevel: 'grade', language: 'primary language', selectedLanguages: 'output languages',
      studentInterests: 'student interests', targetStandards: 'standards', translationMode: 'translation policy',
      currentUiLanguage: 'interface language', differentiationRange: 'differentiation range',
      differentiationTypes: 'differentiated resources', differentiationCustomGrades: 'custom grades',
      dokLevel: 'DOK level', useEmojis: 'emoji preference', textFormat: 'text format',
  ...
  return (
    <div data-help-key="blueprint_card_panel" className="bg-white border-2 border-indigo-100 rounded-xl p-4 my-2 shadow-lg animate-in zoom-in duration-300 w-full max-w-2xl">
      ...
      {blueprintMatrixUnavailable && (
          <div data-testid="bp-matrix-unavailable-warning" role="alert" className="mb-3 rounded-lg border border-amber-400 bg-amber-50 p-2.5 text-xs leading-relaxed text-amber-950">
              <div className="font-bold">{t('blueprint.matrix_unavailable_title') || 'Generation planning is still loading.'}</div>
              <div className="mt-0.5">
                  {t('blueprint.matrix_unavailable_detail') || 'Blueprint is paused: exact duplicate checks and grade/language versions must be resolved before any resources can run. Nothing is generated while this warning is shown.'}
              </div>
              {matrixRetryPending && (
                  <div className="mt-1 font-semibold" data-testid="bp-matrix-retry-guidance">
                      {t('blueprint.matrix_unavailable_retry') || 'Choose Generate again to retry after generation planning finishes loading.'}
                  </div>
              )}
          </div>
      )}
      <div data-testid="bp-generation-matrix-summary" role="status" aria-live="polite" className="mb-3 rounded-lg border border-sky-200 bg-sky-50 p-2.5 text-[11px] leading-relaxed text-sky-950">
          ...
          <div className="mt-0.5">
              {blueprintMatrixUnavailable
                  ...
                  : blueprintMatrixReady
                  ? `${blueprintExpectedCalls} ${t('blueprint.new_generations') || 'new generations'} · ${blueprintReuseCount} ${t('blueprint.reused_outputs') || 'existing outputs reused'}`
                  : (t('blueprint.matrix_refresh_pending') || 'The exact reuse and variant matrix will be refreshed before generation because this plan was edited.')}
          </div>
          {blueprintSettings.translationMode && (
      ...
      {run?.settingsStale && (
          <div data-testid="bp-settings-stale-notice" role="status" className="mb-3 rounded-lg border border-amber-300 bg-amber-50 p-2 text-xs text-amber-950">
              ...
              {t('blueprint.settings_changed_since_review') || 'Universal Settings changed after this Blueprint was reviewed'}
              {staleSettingNames.length ? `: ${staleSettingNames.map(readableSettingName).join(', ')}.` : '.'}
          </div>
      )}
      {sourceSelection?.divergentFromLatestAnalysis && (
          <div data-testid="bp-source-choice-notice" role="status" className="mb-3 rounded-lg border border-violet-200 bg-violet-50 p-2 text-xs text-violet-950">
              <span className="font-bold">{t('blueprint.current_source_selected') || 'Current source selected.'}</span>{' '}
              {t('blueprint.source_differs_from_analysis') || 'It differs from the latest analyzed original, so Blueprint uses the current source and rechecks reuse before generation.'}
          </div>
      )}
          ...
          <div data-help-key="blueprint_resource_list_review" className="space-y-3 mb-6">
              {items.map((item, idx) => {
                  ...
                  const _statusStyle = {
                      ...
                      partial:     { label: t('blueprint.status_partial') || 'Partial',         cls: 'bg-amber-50 text-amber-900 border-amber-300' },
                  ...
                  const _claimedArtifactIds = _rowRun && Array.isArray(_rowRun.resourceIds) && _rowRun.resourceIds.length
                      ? _rowRun.resourceIds.filter(Boolean)
                      : (_rowRun && _rowRun.resourceId ? [_rowRun.resourceId] : []);
                  const _missingArtifactIds = new Set((_rowRun && Array.isArray(_rowRun.missingResourceIds)
                      ? _rowRun.missingResourceIds : []).map(value => String(value)));
                  const _survivingArtifactIds = _claimedArtifactIds.filter(value => !_missingArtifactIds.has(String(value)));
                  const _allSurvivingArtifactsAudited = _survivingArtifactIds.length > 0
                      && _survivingArtifactIds.every(value => _auditIds && _auditIds.indexOf(value) !== -1);
                  const _auditBadge = _missing
                      ...
                      : (!_auditIds || _isAuditRow || (_status !== 'landed' && _status !== 'partial')) ? null
                      : _allSurvivingArtifactsAudited
                  ...
                  const _rowMatrixUnavailable = _rowRun && typeof _rowRun.generationMatrixUnavailable === 'boolean'
                      ? _rowRun.generationMatrixUnavailable
                      : (!!(item && item.generationMatrixUnavailable === true) || !blueprintMatrixModuleReady);
                  const _missingRuntimeResourceIds = new Set((_rowRun && Array.isArray(_rowRun.missingResourceIds)
                      ? _rowRun.missingResourceIds : []).map(value => String(value)));
                  ...
                  const _successfulRuntimeVariants = _runtimeVariants.filter(variant => variant && variant.status === 'landed'
                      && _runtimeArtifactId(variant) && !_runtimeArtifactMissing(variant));
                  ...
                  <div key={item.id} className="flex gap-3 items-start p-3 bg-slate-50 rounded-lg border border-slate-100 border-l-4" style={_st ? { borderLeftColor: _st.stroke } : undefined}>
                      ...
                      <div className="flex-grow">
                          ...
                          {typeof onPreviewStep === 'function' && (_status === 'landed' || _status === 'partial') && !_missing
                              && _runtimeVariants.length <= 1 && (
                              <button
                                  type="button"
                                  data-testid="bp-preview-btn"
                                  data-help-key="blueprint_preview_step_btn"
                                  onClick={() => {
                                      const selection = _successfulRuntimeVariants.length
                                          ? _previewSelection(_successfulRuntimeVariants[0])
                                          : _previewSelection({
                                          status: 'landed',
                          ...
                          {(_status === 'partial' || _status === 'failed' || _status === 'interrupted') && _safeFailure && (
                          ...
                          >
                              {_rowMatrixUnavailable
                                  ? <span data-testid="bp-row-matrix-unavailable">{t('blueprint.row_matrix_unavailable') || 'Waiting for generation planning. This row will not run until exact duplicate and audience-version checks are available.'}</span>
                                  : _generationVariants.length
                              ...
                              <div data-testid="bp-variant-results" className="mt-2 rounded border border-slate-200 bg-white p-2">
                                  ...
                                  {_missingRuntimeVariants.length > 0 && (
                                      <div className="mb-1 text-[10px] font-semibold text-amber-800">
                                          {_missingRuntimeVariants.length} {t('blueprint.variant_unavailable') || 'successful version unavailable in the workspace'}
                                      </div>
                                  )}
                                          ...
                                          return (
                                              ...
                                              >
                                                  <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1">
                                                      <span className="font-bold">{isSuccessful
                                                          ? (t('blueprint.status_landed') || 'Done')
                                                          : (isMissingArtifact
                                                              ? (t('blueprint.resource_missing') || 'Resource gone')
                                                              : (variant && variant.status === 'interrupted'
                                                  ...
                                                  {!isSuccessful && (
                                                      <div className="mt-0.5 opacity-85">
                                                          {isMissingArtifact
                                                              ? (t('blueprint.variant_missing') || 'This successful version is no longer in the workspace. Rebuild the step to create it again.')
                                                              : (safeVariantReason && safeVariantReason.summary)
                                                                  || (t('blueprint.variant_failure_safe') || 'This variant did not finish; technical details remain in the on-device error log.')}
                                                      </div>
                                                  )}
                                              </li>
                                          );
