...
describe('EPPP native content QA gate', () => {
  it('requires every current native item to pass every named QA check', () => {
    expect(report.summary).toMatchObject({ totalItems: 416, passedItems: 416, reviewRequiredItems: 0, status: 'pass' });
    expect(report.items).toHaveLength(416);
  ...
  it('preserves an honest boundary between content QA and expert validation', () => {
    ...
    expect(eppp.contentReview).toContain('416/416 native items passed content QA');
