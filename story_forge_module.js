...
(function() {
  ...
  var Redo2 = _icons.Redo2 || function() { return null; };
...
const comicDialogueHasBubbles = (dialogue = {}) => Boolean(
  String(dialogue.speech || "").trim() || String(dialogue.thought || "").trim()
);
...
const getComicExportProof = (pages = [], context = {}) => {
  ...
  const rows = pageList.map((page) => {
    ...
    const panels = Array.isArray(page.panels) ? page.panels : [];
    const targetPanels = (predicate) => panels.map(({ paragraph }, panelIndex) => ({ paragraph, number: panelIndex + 1 })).filter(({ paragraph, number }) => predicate(paragraph, number)).map(({ paragraph, number }) => ({ id: paragraph?.id, number })).filter((target) => target.id);
    ...
    issues.forEach((issue) => {
      issue.panelTargets = panelTargetsByIssue[issue.key] || [];
    });
    return {
      page: page.page,
...
const getComicContinuityAudit = (paragraphs = [], context = {}) => {
  ...
  const panelRows = list.map((paragraph, index) => {
    const dialogue = (context.panelDialogue || {})[paragraph?.id] || {};
    const direction = (context.panelDirections || {})[paragraph?.id] || {};
    const searchText = [paragraph?.text, paragraph?.scaffoldFrame, dialogue.speech, dialogue.thought].map((value) => String(value || "").trim()).filter(Boolean).join(" ");
    return {
      id: paragraph?.id || "",
      panel: index + 1,
  ...
  const ignoredSpeakers = /* @__PURE__ */ new Set(["narrator", "narration", "voiceover", "voice-over", "sfx"]);
  ...
  if (references.length > 0) {
    ...
    const usedReferences = references.filter((reference) => panelRows.some((row) => matchesReference(row.speaker, reference) || textMentionsReference(row.searchText, reference)));
  ...
  const sameDirection = (left, right) => Boolean(
    left && right && left.shot && left.angle && left.mood && left.shot === right.shot && left.angle === right.angle && left.mood === right.mood
  );
  ...
  const usedReferenceCount = references.filter((reference) => panelRows.some((row) => matchesReference(row.speaker, reference) || textMentionsReference(row.searchText, reference))).length;
  return {
    ...
    usedReferenceCount,
    referenceNames,
    summary: issues.length ? "Review cast naming and repeated visual setups before locking final panel art." : references.length ? "Cast references and the current visual sequence are internally consistent." : "Add named cast references to unlock speaker continuity checks.",
...
const getStoryForgeProjectReadiness = (context = {}) => {
  ...
  const narratedSections = paragraphs.filter((p) => {
    ...
    return Boolean(
      segment.studentAudioBase64 || segment.studentAudioUrl || segment.aiAudioUrl || Array.isArray(segment.sentenceAudios) && segment.sentenceAudios.some(Boolean)
    );
  }).length;
  ...
  if (layoutMode === "comic") {
    ...
    const targetWords2 = Math.max(80, totalSections * 25);
    if (contentSections > 0 && totalWords < targetWords2) {
      addIssue(warnings, "write", "short-story-draft", "Develop the story draft", `The current ${totalWords}-word draft is below the ${targetWords2}-word production target.`);
    }
  }
  const safeRatio = (value, total) => total > 0 ? Math.max(0, Math.min(1, value / total)) : 0;
  const setupScore = Math.round(
    (storyCueReady ? 40 : 0) + (vocabTotal > 0 ? 30 : 0) + (context.genre ? 15 : 0) + (context.layoutMode ? 15 : 0)
  );
  const targetWords = layoutMode === "comic" ? Math.max(20, totalSections * 8) : Math.max(80, totalSections * 25);
  const writeScore = Math.round(
    safeRatio(writtenSections, totalSections) * 70 + Math.min(1, totalWords / targetWords) * 20 + (vocabTotal ? safeRatio(vocabUsed, vocabTotal) * 10 : 10)
  );
  ...
  const percent = Math.round(
    setupScore * 0.15 + writeScore * 0.35 + illustrateScore * 0.15 + effectiveNarrateScore * 0.1 + reviewScore * 0.1 + exportScore * 0.15
  );
  ...
  return {
    ...
    summary: blockers.length ? `${blockers.length} production blocker${blockers.length === 1 ? "" : "s"} to resolve` : warnings.length ? `Ready to export with ${warnings.length} refinement${warnings.length === 1 ? "" : "s"} available` : "Production-ready"
  };
};
const sanitizeParagraphs = (arr) => {
  ...
  const cleaned = arr.slice(0, MAX_DRAFT_PARAGRAPHS).map((p, i) => {
    ...
    return {
      id,
      text: p && typeof p.text === "string" ? p.text.slice(0, 5e4) : "",
      scaffoldFrame: p && typeof p.scaffoldFrame === "string" ? p.scaffoldFrame.slice(0, 6e3) : "",
...
const sanitizeAudioSegments = (obj) => {
  ...
  Object.keys(obj).slice(0, MAX_DRAFT_PARAGRAPHS).forEach((k) => {
    ...
    const clean = {};
    const safeAudio = (url) => typeof url === "string" && /^(data:audio\/|https?:\/\/)/i.test(url) && url.length <= 2e7 ? url : "";
    const aiAudioUrl = safeAudio(value.aiAudioUrl);
    const studentAudioBase64 = typeof value.studentAudioBase64 === "string" && value.studentAudioBase64.length <= 2e7 ? value.studentAudioBase64 : "";
...
const sanitizeContinuityReferences = (arr) => Array.isArray(arr) ? arr.slice(0, 12).map((item, index) => {
  ...
  return {
    ...
    imageUrl: safeImageUrl(value.imageUrl).slice(0, 15e6)
  };
...
const sanitizeVocabTerms = (arr) => Array.isArray(arr) ? arr.slice(0, 64).filter((v) => v && typeof v.term === "string" && v.term.trim()).map((v) => ({
...
const getStoryForgeReviewSignature = (value = {}) => {
  ...
  const serialized = JSON.stringify({
    ...
    rubricText: typeof source.rubricText === "string" ? source.rubricText.slice(0, 12e3) : "",
...
const useAudioRecorder = () => {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      console.warn("Microphone access denied:", err);
      const message = err?.name === "NotAllowedError" ? "Microphone access is blocked. Allow microphone access in your browser or device settings, then retry." : "The microphone could not start. Check that another app is not using it, then retry.";
      setError(message);
      setIsRecording(false);
      return { ok: false, error: message };
    }
...
const normalizeStoryForgeModeSelection = (value = {}) => {
  ...
  const artifactType = hasOwnOption(ARTIFACT_TYPES, source.artifactType) ? source.artifactType : legacy === "comic" ? "comic" : "story";
...
const validateStoryForgeImport = (value) => {
  ...
  const version = rawVersion === void 0 && value._storyForgePackage === "project" ? 1 : Number(rawVersion);
  ...
  const comicPages = layoutMode === "comic" ? buildComicPageGroups(paragraphs, snapshot.comicPageComposer, snapshot.comicPageLayout) : [];
...
const mergeStoryForgeVocabulary = (existing, incoming) => {
  ...
  for (const item of sanitizeVocabTerms(incoming) || []) {
    ...
    if (!known.has(key(item.term))) {
      result.push(item);
      known.add(key(item.term));
    }
  }
  return result;
};
const storyForgeSectionHasWork = (section, maps = []) => Boolean(section && ([section.text, section.scaffoldFrame, section.plotBeat].some((value) => typeof value === "string" && value.trim()) || maps.some((map) => map && map[section.id] != null)));
...
const normalizeStoryForgePlan = (data, comic = false) => {
  ...
  return items.map((item) => {
    ...
    if (typeof caption !== "string" || !caption.trim() || caption.length > 5e3) throw new Error("Invalid plan section");
...
const sanitizeStoryForgeDraft = (value = {}) => {
  ...
  const parsedSavedAt = typeof source.savedAt === "string" && Number.isFinite(Date.parse(source.savedAt)) ? new Date(source.savedAt).toISOString() : "";
  return {
    ...
    customArtStyle: cleanString(source.customArtStyle, 1e3),
    storyPrompt: cleanString(source.storyPrompt, 6e3),
    rubricText: cleanString(source.rubricText, 12e3),
...
const getStoryForgeRestoredPhase = (value = {}) => {
  ...
  const reviewIsCurrent = Boolean(
    typeof draft.reviewedDraftSignature === "string" && draft.reviewedDraftSignature && draft.reviewedDraftSignature === getStoryForgeReviewSignature(draft)
  );
...
const getStoryForgeRecoverySummary = (value = {}, sourceTopic = "") => {
  ...
  const savedAt = typeof draft.savedAt === "string" && Number.isFinite(Date.parse(draft.savedAt)) ? new Date(draft.savedAt).toISOString() : "";
  return {
...
const sanitizeComicContinuityAudit = (value) => {
  ...
  const rows = Array.isArray(source.rows) ? source.rows.slice(0, 12).map((row) => {
    const item = row && typeof row === "object" && !Array.isArray(row) ? row : {};
    const panelTargets = Array.isArray(item.panelTargets) ? item.panelTargets.slice(0, MAX_DRAFT_PARAGRAPHS).map((target) => {
...
const sanitizeComicFlowReport = (value) => {
  ...
  const cleanTextList = (input, maxItems, maxLength) => Array.isArray(input) ? input.slice(0, maxItems).map((item) => cleanString(item, maxLength)).filter(Boolean) : [];
  const cleanNotes = (input, maxItems) => Array.isArray(input) ? input.slice(0, maxItems).map((note) => {
  ...
  const cleanChecks = Array.isArray(source.checks) ? source.checks.slice(0, 32).map((check) => {
...
const STORYFORGE_CLASS_SHARE_MAX_SCENE_TEXT = 5e3;
const STORYFORGE_CLASS_SHARE_MAX_IMAGE_URL = 15e6;
const isStoryForgeClassShareRecord = (value) => Boolean(value && typeof value === "object" && !Array.isArray(value));
const cleanStoryForgeClassShareLabel = (value, maxLength) => typeof value === "string" ? value.replace(/\s+/g, " ").trim().slice(0, maxLength) : "";
const cleanStoryForgeClassShareText = (value, maxLength) => typeof value === "string" ? value.replace(/\r\n?/g, "\n").trim().slice(0, maxLength) : "";
const getStoryForgeClassShareImageUrl = (value) => {
  const raw = typeof value === "string" ? value : isStoryForgeClassShareRecord(value) && typeof value.imageUrl === "string" ? value.imageUrl : "";
...
const getStoryForgeClassShareFingerprint = (value) => {
  ...
  for (const symbol of input) {
    ...
    second = Math.imul(second ^ codePoint, 2246822507);
    second ^= second >>> 13;
  }
...
const buildStoryForgeClassSharePayload = (project = {}, options = {}) => {
  ...
  const title = cleanStoryForgeClassShareLabel(settings.title, 120) || cleanStoryForgeClassShareLabel(source.storyTitle || source.title, 120) || fallbackTitle;
  const penName = cleanStoryForgeClassShareLabel(settings.penName, 80) || cleanStoryForgeClassShareLabel(source.penName, 80) || "Creative Writer";
  const genre = cleanStoryForgeClassShareLabel(settings.genre || settings.genreLabel, 80) || cleanStoryForgeClassShareLabel(source.genreLabel || source.genre, 80) || "Creative Writing";
  ...
  const maxSceneTextLength = Number.isFinite(requestedTextLimit) ? Math.max(1, Math.min(STORYFORGE_CLASS_SHARE_MAX_SCENE_TEXT, Math.round(requestedTextLimit))) : 500;
  const rawScenes = (Array.isArray(source.paragraphs) ? source.paragraphs : Array.isArray(source.scenes) ? source.scenes : Array.isArray(source.fullStory) ? source.fullStory : []).slice(0, STORYFORGE_CLASS_SHARE_MAX_SCENES);
  ...
  const scenes = rawScenes.map((rawScene, index) => {
    ...
    const sourceText = cleanStoryForgeClassShareText(scene.text, 5e4);
    wordCount += countStoryForgeClassShareWords(sourceText);
    const sourceId = typeof scene.id === "string" ? scene.id : "";
    const mappedIllustration = sourceId && isStoryForgeClassShareRecord(illustrationMap[sourceId]) ? illustrationMap[sourceId].imageUrl : isStoryForgeClassShareRecord(illustrationMap[String(index)]) ? illustrationMap[String(index)].imageUrl : isStoryForgeClassShareRecord(illustrationMap[`p-${index}`]) ? illustrationMap[`p-${index}`].imageUrl : "";
    const illustration = getStoryForgeClassShareImageUrl(scene.illustration) || getStoryForgeClassShareImageUrl(scene.imageUrl) || getStoryForgeClassShareImageUrl(mappedIllustration);
    const sharedScene = {};
...
const getStoryForgeClassSharePreview = (payload = {}) => {
  ...
  return {
    ...
    wordCount: cleanCount(counts.words, STORYFORGE_CLASS_SHARE_MAX_SCENES * 5e4),
...
const isStoryForgeDraftMeaningful = (value = {}) => {
  ...
  const panelProductionHasContent = [draft.panelDialogue, draft.panelDirections, draft.panelThumbnails, draft.panelLayouts, draft.panelStickers].some((item) => Object.keys(item).length > 0);
  return Boolean(
    draft.storyTitle.trim() || draft.storyPrompt.trim() || draft.rubricText.trim() || draft.customArtStyle.trim() || draft.customLanguage.trim() || draft.vocabTerms.length || draft.paragraphs.some((item) => String(item.text || item.scaffoldFrame || item.plotBeat || "").trim()) || draft.genre !== "free" || draft.artStyle !== "storybook" || draft.language !== "en" || draft.storyShape || draft.scaffoldsGenerated || draft.draftCount > 1 || draft.layoutMode !== "prose" || draft.comicPageLayout !== "grid" || composerChanged || printSafetyChanged || continuityHasContent || panelProductionHasContent || Object.keys(draft.valenceByPara).length > 0
  );
};
...
const StoryForge = React.memo(({
  ...
  const optLabel = (family, value, fallback) => {
    const slug = value == null || value === "" ? "placeholder" : String(value).toLowerCase().replace(/[^a-z0-9]+/g, "_");
    const v = t("a11y.storyforge_opt_" + family + "_" + slug);
    return typeof v === "string" ? v : fallback ?? "";
  };
  const phaseLabel = (i) => {
    const v = t("a11y.storyforge_phase_name_" + (PHASES[i] || "configure"));
    return typeof v === "string" ? v : PHASE_LABELS[i] || "";
  };
  const shapeLabel = (key) => {
    const v = t("a11y.storyforge_shape_" + key);
    return typeof v === "string" ? v : STORY_SHAPES[key]?.label || "";
  };
  const shapeDesc = (key) => {
    const v = t("a11y.storyforge_shape_" + key + "_desc");
    return typeof v === "string" ? v : STORY_SHAPES[key]?.desc || "";
  };
  ...
  const layoutMode = artifactType === "comic" ? "comic" : WRITING_VIEWS[writingView]?.layoutMode || "prose";
  ...
  const selectArtifactType = (nextType) => {
    ...
    const hasArtifactContent = paragraphs.some((item) => String(item.text || "").trim()) || Object.keys(panelDialogue || {}).length > 0 || Object.keys(panelDirections || {}).length > 0 || Object.keys(panelThumbnails || {}).length > 0 || Object.values(illustrations || {}).some((item) => Boolean(item?.imageUrl)) || Object.values(audioSegments || {}).some((item) => Boolean(item?.studentAudioUrl || item?.aiAudioUrl));
  ...
  const updateContinuityReference = (referenceId, field, value) => {
    const maxLength = field === "imageUrl" ? 15e6 : field === "name" ? 80 : field === "aliases" ? 240 : field === "role" ? 120 : field === "appearance" ? 360 : 240;
  ...
  const handleContinuityReferenceImage = (referenceId, event) => {
    ...
    const reader = new FileReader();
    reader.onload = () => {
      const imageUrl = safeImageUrl(String(reader.result || "")).slice(0, 15e6);
      if (!imageUrl) return;
      setComicContinuity((prev) => sanitizeComicContinuity({
        ...prev,
    ...
    reader.onerror = () => {
      if (addToast) addToast(ta("a11y.storyforge_toast_reference_art_could_not_be_read"), "error");
    };
  ...
  useEffect(() => {
    if (!comicProofFocus || comicProofFocus.page !== comicPreviewPage || typeof document === "undefined") return void 0;
  ...
  const getComicConsistencyReference = (idx) => {
    const castReference = layoutMode === "comic" ? sanitizeComicContinuity(comicContinuity).references.find((reference) => getImageBase64Payload(reference.imageUrl)) : null;
  ...
  const commitPendingComicHistory = () => {
    const history = comicHistoryRef.current;
    if (history.timer) clearTimeout(history.timer);
    const pending = history.pending || (history.currentKey !== comicProductionSnapshotKey ? { snapshot: comicProductionSnapshot, key: comicProductionSnapshotKey } : null);
    history.timer = null;
    history.pending = null;
  ...
  const applyComicHistorySnapshot = (snapshot, actionLabel) => {
    ...
    setBubbleDrag(null);
    setIsDirty(true);
    sfAnnounce(ta(cleanParagraphs.length === 1 ? "a11y.storyforge_panel_in_sequence_one" : "a11y.storyforge_panel_in_sequence_many").replace("{0}", actionLabel).replace("{1}", cleanParagraphs.length));
  };
  ...
  const draftSaveLabel = draftHydrationState === "awaiting" ? "Review draft" : draftHydrationState !== "ready" || showRestorePrompt ? "Save paused" : draftSaveState === "saving" ? "Saving" : draftSaveState === "error" ? "Save failed" : draftSaveState === "saved" ? "Saved" : "Save draft";
  const draftSaveDescription = draftSaveError || (lastDraftSavedAt ? `${vaultStorageMode === "vault" ? "Project vault" : "Browser storage"} \xB7 Last saved at ${new Date(lastDraftSavedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : draftHydrationState !== "ready" || showRestorePrompt ? "Choose whether to restore the saved draft first" : "Save this project in the browser vault");
...
IMPORTANT: Respond entirely in ${langLabel}. All text output must be in ${langLabel}.` : "";
  ...
  const hasPriorReviewOutput = Boolean(
    gradingResult || revisionPlan || comicFlowReport || selfAssessmentSubmitted && Object.keys(selfAssessment).length > 0
  );
  ...
  const comicExportProof = useMemo(
    () => layoutMode === "comic" ? getComicExportProof(comicPageGroups, { panelDialogue, panelDirections, panelThumbnails, panelLayouts, illustrations, comicPrintSafety }) : null,
    [layoutMode, comicPageGroups, panelDialogue, panelDirections, panelThumbnails, panelLayouts, illustrations, comicPrintSafety]
  );
  const comicContinuityAudit = useMemo(
    () => layoutMode === "comic" ? getComicContinuityAudit(paragraphs, { panelDialogue, panelDirections, comicContinuity, comicPages: comicPageGroups }) : null,
    ...
    (async () => {
      ...
      if (Array.isArray(vaultRecord?.revisions)) {
        const cleanRevisions = vaultRecord.revisions.slice(0, 12).filter((item) => item && item.snapshot).map((item) => ({
          ...
          savedAt: typeof item.savedAt === "string" && Number.isFinite(Date.parse(item.savedAt)) ? new Date(item.savedAt).toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
  ...
  const saveAsSubmission = () => {
    ...
    const receiptText = receipt ? ta(isComic ? "a11y.storyforge_saved_comic_to_portfolio_full" : "a11y.storyforge_saved_story_to_portfolio_full") : ta(isComic ? "a11y.storyforge_comic_saved_to_portfolio" : "a11y.storyforge_story_saved_to_portfolio");
  ...
  const getPhaseRequirement = () => {
    ...
    if (phase === "configure") return hasStoryCue ? ta("a11y.storyforge_phase_plan_ready") : ta("a11y.storyforge_phase_need_title");
    if (phase === "write") {
      ...
      return isComic ? pick(n, "a11y.storyforge_phase_panels_ready_one", "a11y.storyforge_phase_panels_ready_many") : pick(n, "a11y.storyforge_phase_scenes_ready_one", "a11y.storyforge_phase_scenes_ready_many");
    }
    if (phase === "review") return hasCompletedReview ? ta("a11y.storyforge_phase_review_complete") : isReviewStale ? ta("a11y.storyforge_phase_need_review_stale") : ta("a11y.storyforge_phase_need_selfcheck");
    if ((phase === "illustrate" || phase === "narrate") && !hasCompletedReview) {
      return ta("a11y.storyforge_phase_need_review_forward");
    }
    if (phase === "illustrate") return projectReadiness.metrics.illustratedSections > 0 ? pick(projectReadiness.metrics.illustratedSections, "a11y.storyforge_phase_sections_designed_one", "a11y.storyforge_phase_sections_designed_many") : ta("a11y.storyforge_phase_optional_visuals");
    if (phase === "narrate") return projectReadiness.metrics.narratedSections > 0 ? pick(projectReadiness.metrics.narratedSections, "a11y.storyforge_phase_sections_audio_one", "a11y.storyforge_phase_sections_audio_many") : ta("a11y.storyforge_phase_optional_audio");
    if (!hasCompletedReview) return ta("a11y.storyforge_phase_need_review_publish");
    return projectReadiness.blockers.length > 0 ? ta("a11y.storyforge_phase_blocker").replace("{0}", projectReadiness.blockers[0].detail) : ta("a11y.storyforge_phase_ready_to_publish");
  };
  ...
  const changePhase = (newPhase, focusTargetId = "") => {
    ...
    if (PHASES.indexOf(newPhase) > phaseIdx && !canEnterPhase(newPhase)) {
      const requiredPhase = !hasStoryCue ? "configure" : !hasDraftContent ? "write" : !hasCompletedReview ? "review" : phase;
  ...
  const handlePanelSequenceKeyDown = (event, idx) => {
    ...
    event.preventDefault();
    event.stopPropagation();
    const target = event.key === "Home" ? 0 : event.key === "End" ? paragraphs.length - 1 : idx + (event.key === "ArrowLeft" || event.key === "ArrowUp" ? -1 : 1);
    movePanelToIndex(idx, Math.max(0, Math.min(paragraphs.length - 1, target)));
  };
  const generateScaffolds = async () => {
    if (!onCallGemini) {
      sfAnnounce(ux("ai_unavailable", "AI tools are unavailable. You can keep writing and use the self-check."));
      return;
    }
...
"""`;
      ...
      if (window.WebSearchProvider && keywords) {
        try {
          const genreLabel3 = GENRE_TEMPLATES[genre]?.label || "";
          const searchQuery = `${keywords} ${genreLabel3 ? genreLabel3 + " " : ""}famous public domain short story excerpt gutenberg`;
   ...
   ${String(r.snippet || "").slice(0, 220)}`
      ...
      const genreLabel2 = GENRE_TEMPLATES[genre]?.label || "creative";
  ...
  const buildComicFlowSnapshot = () => {
    ...
    const panelRows = paragraphs.map((p, idx) => {
      ...
      return {
        ...
        frameLabel: `${getComicPanelFrameLabel(layoutFrame.frame)} \xB7 ${getComicPanelSpanLabel(layoutFrame, comicPageLayout, idx)}`,
    ...
    const issuePenalty = missingCaptionPanels.length * 12 + missingDirectionPanels.length * 6 + missingTransitionPanels.length * 4 + missingThumbnailPanels.length * 3 + missingPageTurns.length * 4 + unsafeLetteringPanels.length * 4 + gutterRiskPanels.length * 4 + (bleedReady ? 0 : 3) + missingImagePanels.length * 4 + heavyBubblePanels.length * 5 + (shotSet.size <= 1 && total > 2 ? 10 : 0) + (transitionSet.size <= 1 && total > 3 ? 6 : 0) + continuityAudit.issueCount * 3 + (continuityFields < 2 ? 8 : 0);
...
Return ONLY JSON:
{
  ...
  const ensureReadyToPublish = () => {
    if (!isCurrentDraftReviewed) {
      const message2 = ta("a11y.storyforge_review_before_publishing");
      if (addToast) addToast(message2, "error");
      sfAnnounce(message2);
...
Return ONLY JSON:
{
  ...
  const exportStorybook = async () => {
    ...
    if (isComic) {
      ...
      chaptersHtml += `<header class="comic-page-heading"><span>Page ${firstPage.page}</span><strong>${escapeHtml(getComicPageLayoutLabel(firstPage.layout))}</strong><em>Panels ${firstPage.startPanel}-${firstPage.endPanel} \xB7 ${escapeHtml(getComicPrintFormatLabel(storybookPrintSafety.format))}</em></header>`;
      chaptersHtml += `<div class="comic-reading-guide">${escapeHtml(getComicReadingOrderLabel(firstPage.layout))} \xB7 Follow the numbered panels</div>`;
    ...
    paragraphs.forEach((p, idx) => {
        ...
        if (nextPage) {
          ...
          if (prevPage) {
            ...
            if (prevTurnLabel || prevNote) {
              chaptersHtml += `<div class="comic-page-turn"><strong>Page turn:</strong> ${prevTurnLabel ? escapeHtml(prevTurnLabel) : "Production note"}${prevNote ? ` \xB7 ${prevNote}` : ""}</div>`;
            }
          }
          ...
          chaptersHtml += `<header class="comic-page-heading"><span>Page ${nextPage.page}</span><strong>${escapeHtml(getComicPageLayoutLabel(nextPage.layout))}</strong><em>Panels ${nextPage.startPanel}-${nextPage.endPanel} \xB7 ${escapeHtml(getComicPrintFormatLabel(storybookPrintSafety.format))}</em></header>`;
          chaptersHtml += `<div class="comic-reading-guide">${escapeHtml(getComicReadingOrderLabel(nextPage.layout))} \xB7 Follow the numbered panels</div>`;
      ...
      if (isComic) {
        ...
        if (hasOverlayBubble) {
          ...
          chaptersHtml += overlayBubbleKind === "thought" ? `<div class="dialogue-thought" aria-label="Inner thought">&#128173; ${safeThought}</div></div></div>` : `<div class="dialogue-speech">${safeSpeech}</div></div></div>`;
        }
        ...
        if (safeThought && !(hasOverlayBubble && !safeSpeech)) chaptersHtml += `<div class="thought-bubble" aria-label="Inner thought">\u{1F4AD} ${safeThought}</div>`;
        ...
        const beatLabel2 = (PLOT_BEATS.find((b) => b.value === p.plotBeat) || {}).label;
        if (beatLabel2 && p.plotBeat) {
          chaptersHtml += `<div class="beat-label" aria-label="${escapeHtml(t("a11y.narrative_beat", { label: beatLabel2 }))}">${escapeHtml(beatLabel2)}</div>`;
        }
    ...
    if (isComic) {
      ...
      if (lastPage) {
        ...
        if (lastTurnLabel || lastNote) {
          chaptersHtml += `<div class="comic-page-turn"><strong>Page note:</strong> ${lastTurnLabel ? escapeHtml(lastTurnLabel) : "Production note"}${lastNote ? ` \xB7 ${lastNote}` : ""}</div>`;
        }
      }
      chaptersHtml += "</section>";
    }
...
<header class="cover" role="banner">
  ...
  <p class="meta">${escapeHtml(date)} \xB7 ${escapeHtml(GENRE_TEMPLATES[genre]?.label || "Creative Writing")} \xB7 Art style: ${escapeHtml(artStyle)}${isComic ? ` \xB7 Layout: ${escapeHtml(COMIC_PAGE_LAYOUTS[comicLayout]?.label || "Grid")} \xB7 Pages: ${escapeHtml(exportedComicPages.length || 1)} \xB7 Print: ${escapeHtml(getComicPrintFormatLabel(storybookPrintSafety.format))} \xB7 ${escapeHtml(getComicReadingOrderLabel(comicLayout))}` : ""}</p>
</header>
...
<footer class="colophon" role="contentinfo">Created with StoryForge \xB7 AlloFlow</footer>
  ...
  const exportComicScript = async () => {
    ...
    const continuityHtml = continuityRows.length || continuityReferencesHtml ? `<section class="continuity-sheet"><h2>Continuity Sheet</h2><dl>${continuityRows.map(([label, value]) => `<dt>${escapeHtml(label)}</dt><dd>${escapeHtml(value).replace(/\n/g, "<br/>")}</dd>`).join("")}</dl>${continuityReferencesHtml}</section>` : "";
    ...
    const pagePlanHtml = `<section class="page-plan"><h2>Page Composer</h2>${scriptPages.map((page) => {
      ...
      return `<div class="page-row">
        ...
        <span>${escapeHtml(getComicPageLayoutLabel(page.layout))} \xB7 Panels ${page.startPanel}-${page.endPanel}</span>
        <em>${turnLabel ? escapeHtml(turnLabel) : page.page < scriptPages.length ? "Turn unset" : "Final page"}${page.note ? ` \xB7 ${escapeHtml(page.note)}` : ""}</em>
      </div>`;
    }).join("")}</section>`;
    const panelsHtml = paragraphs.map((p, idx) => {
      ...
      return `<section class="script-panel">
        <header><h2>Panel ${idx + 1}</h2>${beatLabel2 ? `<span>${escapeHtml(beatLabel2)}</span>` : ""}</header>
        <dl>
          <dt>Caption</dt><dd>${escapeHtml(caption).replace(/\n/g, "<br/>") || "<em>Not written yet</em>"}</dd>
          <dt>Frame</dt><dd>${escapeHtml(getComicPanelFrameLabel(layoutFrame.frame))} \xB7 ${escapeHtml(getComicPanelSpanLabel(layoutFrame, panelPage?.layout || comicLayout, panelPageIndex))}</dd>
...
<h1>${title}</h1>
<div class="meta">Comic script by ${author} \xB7 Layout: ${layoutLabel} \xB7 Pages: ${escapeHtml(scriptPages.length || 1)} \xB7 Print: ${escapeHtml(getComicPrintFormatLabel(scriptPrintSafety.format))} \xB7 ${escapeHtml(getComicReadingOrderLabel(comicLayout))} \xB7 ${escapeHtml((/* @__PURE__ */ new Date()).toLocaleDateString())}</div>
${continuityHtml}
${pagePlanHtml}
${panelsHtml}
</body></html>`;
  ...
  const exportComicProductionPack = async () => {
    ...
    const continuityAuditHtml = continuityAudit.rows.map((row) => {
      const targets = row.panelTargets.length ? "Panels " + row.panelTargets.map((target) => target.number).join(", ") : "Comic-wide";
      return '<div class="proof-row review"><div><strong>' + escapeHtml(row.label) + "</strong><span>" + escapeHtml(targets) + "</span></div><b>" + escapeHtml(row.severity === "low" ? "Polish" : "Review") + "</b><p>" + escapeHtml(row.detail) + "</p></div>";
    }).join("");
    const pageProofHtml = proof.rows.map((row) => {
      const issueText = row.issues.length ? row.issues.slice(0, 3).map((issue) => issue.label).join(" - ") : "All page checks clear.";
    ...
    const globalProofHtml = proof.globalIssues.length ? `<div class="proof-global">${proof.globalIssues.map((issue) => `<div><strong>${escapeHtml(issue.label)}</strong><span>${escapeHtml(issue.detail)}</span></div>`).join("")}</div>` : "";
    const pageProofVisualHtml = packPages.map((page) => {
      ...
      const panelVisualHtml = (page.panels || []).map(({ paragraph, idx: panelIdx }, pageIndex) => {
        ...
        const bubbleMarkup = bubbleText ? '<div class="' + escapeHtml(bubbleClass) + '" style="' + escapeHtml(bubbleStyle) + '">' + (bubbleKind === "speech" && dialogue.speaker ? "<strong>" + escapeHtml(dialogue.speaker) + ":</strong> " : "") + (bubbleKind === "thought" ? '<span aria-hidden="true">Thought: </span>' : "") + escapeHtml(bubbleText) + "</div>" : letteringSpace ? '<div class="page-proof-lettering-reserve space-' + escapeHtml(letteringSpace) + '">Lettering space</div>' : "";
        const panelMarkup = '<article class="page-proof-panel ' + escapeHtml(frameClass) + '" style="' + escapeHtml(panelStyle) + '" aria-label="' + escapeHtml("Panel " + (panelIdx + 1)) + '">\n<span class="page-proof-number" aria-hidden="true">' + (panelIdx + 1) + "</span>" + (imageUrl ? '<img class="page-proof-art" src="' + escapeHtml(imageUrl) + '" alt="' + escapeHtml(altText) + '" />' : '<div class="page-proof-empty">Art pending</div>') + bubbleMarkup + (panelStickers[p.id] ? '<span class="page-proof-sticker" aria-hidden="true">' + escapeHtml(panelStickers[p.id]) + "</span>" : "") + (dialogue.sfx ? '<span class="page-proof-sfx" aria-label="Sound effect">' + escapeHtml(dialogue.sfx) + "</span>" : "") + (caption ? '<div class="page-proof-caption">' + escapeHtml(caption).replace(/\n/g, "<br/>") + "</div>" : "") + "</article>";
        return panelMarkup;
      }).join("");
      const issueSummary = row.issues.length ? row.issues.slice(0, 3).map((issue) => issue.label).join(" - ") : "All page checks clear.";
      const guideMarkup = (packPrintSafety.showGuides ? '<div class="page-proof-guide page-proof-safe" aria-hidden="true"></div>' : "") + (packPrintSafety.showGuides && packPrintSafety.includeBleed && packPrintSafety.format !== "digital" ? '<div class="page-proof-guide page-proof-bleed" aria-hidden="true"></div>' : "") + (packPrintSafety.showGuides && gutterSide ? '<div class="page-proof-guide page-proof-gutter page-proof-gutter-' + escapeHtml(gutterSide) + '" aria-hidden="true"></div>' : "");
      return '<article class="page-proof-sheet" data-page-proof-status="' + escapeHtml(row.status) + '">\n<div class="page-proof-header"><div><h3>Page ' + escapeHtml(page.page) + "</h3><span>" + escapeHtml(getComicPageLayoutLabel(page.layout)) + " - Panels " + escapeHtml(page.startPanel) + "-" + escapeHtml(page.endPanel) + " - " + escapeHtml(getComicReadingOrderLabel(page.layout)) + '</span></div><strong class="page-proof-status ' + (row.status === "Ready" ? "ready" : "review") + '">' + escapeHtml(row.status) + '</strong></div>\n<div class="page-proof-canvas format-' + escapeHtml(packPrintSafety.format) + " layout-" + escapeHtml(page.layout) + '">' + (panelVisualHtml || '<div class="page-proof-empty-page">No panels assigned</div>') + guideMarkup + '</div>\n<div class="page-proof-footer"><span>' + escapeHtml(getComicPrintFormatLabel(packPrintSafety.format)) + (gutterSide ? " - " + escapeHtml(gutterSide) + " gutter" : "") + "</span><span>" + escapeHtml(issueSummary) + "</span></div>\n</article>";
    }).join("");
      ...
      return `
        <section class="panel-card">
          <header>
            <div>
              <h2>Panel ${idx + 1}</h2>
              ${beatLabel2 ? `<span class="beat">${escapeHtml(beatLabel2)}</span>` : ""}
            </div>
...
<div class="footer">Created with StoryForge - AlloFlow</div>
  ...
  const exportSlideshow = () => {
    ...
    paragraphs.forEach((p, idx) => {
      ...
      const dialogueHtml = artifactType === "comic" && (dialogue.speech || dialogue.thought || dialogue.sfx) ? `<div class="comic-dialogue">${dialogue.speech ? `<div class="comic-speech">${dialogue.speaker ? `<strong>${escapeHtml(dialogue.speaker)}:</strong> ` : ""}${escapeHtml(dialogue.speech)}</div>` : ""}${dialogue.thought ? `<div class="comic-thought">Thought: ${escapeHtml(dialogue.thought)}</div>` : ""}${dialogue.sfx ? `<div class="comic-sfx">${escapeHtml(dialogue.sfx)}</div>` : ""}</div>` : "";
      const beatLabel2 = (PLOT_BEATS.find((b) => b.value === p.plotBeat) || {}).label;
      const beatHtml = beatLabel2 && p.plotBeat ? `<div class="beat-label" aria-label="${escapeHtml(t("a11y.narrative_beat", { label: beatLabel2 }))}">${escapeHtml(beatLabel2)}</div>` : "";
...
<\/script></body></html>`;
  ...
  const exportStoryForgeProject = async () => {
    ...
    const comicPages = snapshot.layoutMode === "comic" ? buildComicPageGroups(snapshot.paragraphs, snapshot.comicPageComposer, snapshot.comicPageLayout) : [];
    const proof = snapshot.layoutMode === "comic" ? getComicExportProof(comicPages, {
    ...
    const continuityAudit = snapshot.layoutMode === "comic" ? getComicContinuityAudit(snapshot.paragraphs, {
      panelDialogue: snapshot.panelDialogue,
      panelDirections: snapshot.panelDirections,
  ...
  const applyImportedPackage = (validated) => {
    ...
    if (addToast) {
      addToast(importedPhase === "review" ? `Student progress loaded from ${summary.exportedBy || "student"} \u2014 review their work!${packageDetail}` : `Draft loaded from ${summary.exportedBy || "classmate"} \u2014 keep writing!${packageDetail}`, "success");
    }
    ...
    input.onchange = (e) => {
      ...
      reader.onload = async (ev) => {
        try {
          ...
          if (!validated.valid) {
            if (addToast) addToast(validated.code === "newer-version" ? `This package uses Story Forge format v${validated.version}, newer than this editor supports.` : t("toasts.invalid_storyforge_file"), "error");
  ...
  const phaseGuides = {
    ...
    write: {
      ...
      outcome: artifactType === "comic" ? "Build the sequence one panel at a time with concise captions and dialogue." : "Build the story one scene at a time. Focus on the ideas before visual polish."
    },
    review: {
    ...
    illustrate: {
      ...
      outcome: artifactType === "comic" ? "Add art, arrange panels, and check that lettering is easy to follow." : "Add optional illustrations and choose the final visual treatment."
    },
    narrate: {
  ...
  const artifactTypeLocked = paragraphs.some((item) => String(item.text || "").trim()) || Object.keys(panelDialogue || {}).length > 0 || Object.keys(panelDirections || {}).length > 0 || Object.keys(panelThumbnails || {}).length > 0 || Object.values(illustrations || {}).some((item) => Boolean(item?.imageUrl)) || Object.values(audioSegments || {}).some((item) => Boolean(item?.studentAudioUrl || item?.aiAudioUrl));
  const comicAltCoverageLabel = layoutMode === "comic" ? projectReadiness.comicStats.artPanels > 0 ? `${Math.max(0, projectReadiness.comicStats.artPanels - projectReadiness.comicStats.missingAltText)}/${projectReadiness.comicStats.artPanels}` : "0/0" : "n/a";
  const renderComicPreviewPanel = (p, idx, previewLayout = comicPageLayout, pageIndex = idx, pageForPanel = null, editable = false) => {
    ...
    return /* @__PURE__ */ React.createElement("div", { key: p.id, id: "sf-comic-preview-panel-" + p.id, tabIndex: -1, "data-sf-comic-panel-proof-target": "true", className: `sf-comic-page-panel bg-white rounded-lg overflow-hidden shadow-md relative ${getComicPanelFramePreviewClass(layoutFrame.frame)} ${!normalizeComicPanelFrame(layoutFrame.frame) && previewLayout === "splash" && pageIndex === 0 ? "col-span-2" : ""}`, style: { ...getComicPanelGridStyle(layoutFrame, previewLayout, pageIndex), border: "3px solid #1e293b", direction: "ltr" } }, /* @__PURE__ */ React.createElement("div", { className: `absolute top-2 ${mangaFlow ? "right-2" : "left-2"} z-20 w-7 h-7 rounded-full bg-slate-950 text-white border-2 border-white shadow-md flex items-center justify-center text-xs font-black` }, idx + 1), printSafety.showGuides && /* @__PURE__ */ React.createElement(React.Fragment, null, printSafety.includeBleed && printSafety.format !== "digital" && /* @__PURE__ */ React.createElement("div", { className: "absolute inset-0 z-10 border-4 border-amber-300/30 pointer-events-none", "aria-hidden": "true" }), /* @__PURE__ */ React.createElement("div", { className: "absolute inset-3 z-10 rounded-md border border-dashed border-emerald-300/90 pointer-events-none", "aria-hidden": "true" }), gutterSide && /* @__PURE__ */ React.createElement("div", { className: `absolute top-0 bottom-0 ${gutterSide === "left" ? "left-0" : "right-0"} z-10 w-3 bg-rose-400/25 pointer-events-none`, "aria-hidden": "true" })), illustrations[p.id]?.imageUrl && (() => {
      ...
      const letteringBubble = showPlacedBubble ? /* @__PURE__ */ React.createElement(
        "div",
        {
        ...
        placedBubbleKind === "speech" && dialogue.speaker && /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-bold text-blue-600 mb-0.5" }, dialogue.speaker, ":"),
        placedBubbleKind === "thought" && /* @__PURE__ */ React.createElement("span", { "aria-hidden": "true" }, "\u{1F4AD}", " "),
        placedBubbleText,
        editable && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(
          "button",
          {
            ...
            "aria-label": ta("a11y.storyforge_aria_move_bubble_for_panel_use_arrow_keys").replace("{0}", placedBubbleKind).replace("{1}", idx + 1),
          ...
          "button",
          {
            ...
            "aria-label": ta("a11y.storyforge_aria_resize_bubble_for_panel_use_left_and").replace("{0}", placedBubbleKind).replace("{1}", idx + 1),
      ...
      return /* @__PURE__ */ React.createElement("div", { className: "relative", "data-sf-comic-art-layer": "true" }, /* @__PURE__ */ React.createElement("img", { src: illustrations[p.id].imageUrl, alt: panelThumbnails[p.id]?.altText || `Panel ${idx + 1}`, className: `w-full object-cover ${isComicPanelWideFrame(layoutFrame, previewLayout, pageIndex) ? "aspect-video" : "aspect-square"}` }), showPlacedBubble && (customLetteringPosition ? /* @__PURE__ */ React.createElement("div", { className: "absolute inset-2 z-10 pointer-events-none" }, letteringBubble) : /* @__PURE__ */ React.createElement("div", { className: `absolute inset-2 z-10 flex pointer-events-none ${getComicLetteringPreviewFlexClass(space)}` }, letteringBubble)), space && !placedBubbleText && /* @__PURE__ */ React.createElement("div", { className: `absolute inset-2 z-10 flex pointer-events-none ${getComicLetteringPreviewFlexClass(space)}` }, /* @__PURE__ */ React.createElement("div", { className: "border-2 border-dashed border-teal-300 bg-white/70 text-teal-700 rounded-xl px-2 py-1 text-[10px] font-black uppercase tracking-widest" }, "Bubble space")));
    })(), panelStickers[p.id] && /* @__PURE__ */ React.createElement("div", { className: `absolute ${mangaFlow ? "top-11 right-2" : "top-2 right-2"} text-3xl drop-shadow-lg select-none pointer-events-none`, style: { transform: "rotate(12deg)" } }, panelStickers[p.id]), (panelDialogue[p.id] || {}).sfx && /* @__PURE__ */ React.createElement("div", { className: `absolute ${mangaFlow ? "top-3 left-3" : "top-11 left-3"} font-black text-red-500 text-lg drop-shadow-lg select-none pointer-events-none`, style: { transform: "rotate(-8deg)", textShadow: "2px 2px 0 #fff, -1px -1px 0 #fff" } }, panelDialogue[p.id].sfx), editable && /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
  ...
  return /* @__PURE__ */ React.createElement("div", { ref: modalRootRef, tabIndex: -1, className: `sf-modal-root theme-${hostTheme} fixed inset-0 z-[200] ${hostTheme === "default" ? "bg-slate-100/95" : "bg-slate-900/95"} backdrop-blur-sm flex flex-col ${animClass}`, role: "dialog", "aria-modal": "true", "aria-label": t("a11y.story_forge_studio") }, /* @__PURE__ */ React.createElement("div", { className: "allo-docsuite", style: { display: "contents" } }, /* @__PURE__ */ React.createElement("audio", { ref: audioRef, onEnded: handleAudioEnded, onError: handleAudioPlaybackError, className: "hidden" }), /* @__PURE__ */ React.createElement("div", { "aria-live": "polite", "aria-atomic": "true", className: "sr-only" }, playbackIdx >= 0 && paragraphs[playbackIdx] ? `Now reading ${artifactType === "comic" ? "panel" : "scene"} ${playbackIdx + 1}${audioSegments[paragraphs[playbackIdx].id]?.sentences?.[sentenceIdx] ? ": " + audioSegments[paragraphs[playbackIdx].id].sentences[sentenceIdx] : ""}` : ""), /* @__PURE__ */ React.createElement("div", { id: "allo-live-storyforge", "aria-live": "polite", "aria-atomic": "true", className: "sr-only" }), /* @__PURE__ */ React.createElement("style", null, `
      ...
      `), showRestorePrompt && /* @__PURE__ */ React.createElement(
    "div",
    {
      ref: restorePromptDialogRef,
      tabIndex: -1,
      "data-sf-restore-dialog": true,
    ...
    /* @__PURE__ */ React.createElement("div", { className: "sf-dialog-card w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl" }, /* @__PURE__ */ React.createElement("div", { className: "text-center" }, /* @__PURE__ */ React.createElement("div", { className: "text-3xl mb-3", "aria-hidden": "true" }, "\u{1F4D6}"), /* @__PURE__ */ React.createElement("h3", { id: "sf-restore-title", className: "text-lg font-black text-slate-800 mb-2" }, t("ui_common.continue_where_left")), /* @__PURE__ */ React.createElement("p", { id: "sf-restore-description", className: "text-sm text-slate-600" }, ta("a11y.storyforge_ui_a_saved_project_is_waiting_review"))), /* @__PURE__ */ React.createElement("dl", { id: "sf-restore-summary", "data-sf-restore-summary": true, className: "mt-4 grid grid-cols-1 gap-2 rounded-xl border border-rose-100 bg-rose-50/50 p-3 text-left text-xs sm:grid-cols-2" }, /* @__PURE__ */ React.createElement("div", { className: "min-w-0 sm:col-span-2" }, /* @__PURE__ */ React.createElement("dt", { className: "font-bold uppercase tracking-wide text-rose-700" }, ta("a11y.storyforge_ui_saved_project")), /* @__PURE__ */ React.createElement("dd", { "data-sf-recovery-project-title": true, className: "mt-1 truncate text-sm font-black text-slate-900" }, recoverySummary.title)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("dt", { className: "font-bold uppercase tracking-wide text-rose-700" }, ta("a11y.storyforge_ui_artifact")), /* @__PURE__ */ React.createElement("dd", { "data-sf-recovery-artifact-label": true, className: "mt-1 font-bold text-slate-800" }, recoverySummary.artifactLabel)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("dt", { className: "font-bold uppercase tracking-wide text-rose-700" }, ta("a11y.storyforge_ui_resume_safely_at")), /* @__PURE__ */ React.createElement("dd", { "data-sf-recovery-phase-label": true, className: "mt-1 font-bold text-slate-800" }, recoverySummary.resumePhaseLabel)), /* @__PURE__ */ React.createElement("div", { className: "sm:col-span-2" }, /* @__PURE__ */ React.createElement("dt", { className: "font-bold uppercase tracking-wide text-rose-700" }, ta("a11y.storyforge_ui_last_saved")), /* @__PURE__ */ React.createElement("dd", { className: "mt-1 font-bold text-slate-800" }, /* @__PURE__ */ React.createElement("time", { "data-sf-recovery-saved-at": true, dateTime: recoverySummary.savedAt || void 0 }, recoverySummary.savedAt ? new Date(recoverySummary.savedAt).toLocaleString() : "Time unavailable")))), /* @__PURE__ */ React.createElement("div", { className: "mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end" }, /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, "data-sf-request-discard-draft": true, onClick: requestDiscardDraftConfirmation, className: "min-h-11 rounded-lg border border-rose-300 bg-white px-4 py-2 text-sm font-bold text-rose-700 transition-colors hover:bg-rose-50" }, "Delete saved project & start new"), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, "data-sf-restore-draft": true, onClick: restoreDraft, className: "min-h-11 rounded-lg bg-rose-600 px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-rose-700" }, ta("a11y.storyforge_ui_restore_and_resume_at").replace("{0}", recoverySummary.resumePhaseLabel))))
  ), showDiscardDraftConfirm && /* @__PURE__ */ React.createElement("div", { role: "presentation", className: "fixed inset-0 z-[220] flex items-center justify-center bg-black/70 p-4" }, /* @__PURE__ */ React.createElement(
    "div",
    {
      ...
      "data-sf-discard-draft-confirmation": true,
      className: "sf-dialog-card w-full max-w-md rounded-2xl border-2 border-rose-300 bg-white p-6 shadow-2xl"
    },
    /* @__PURE__ */ React.createElement("h3", { id: "sf-discard-draft-title", className: "text-lg font-black text-slate-900" }, ta("a11y.storyforge_ui_delete_saved_project")),
    /* @__PURE__ */ React.createElement("p", { id: "sf-discard-draft-description", className: "mt-2 text-sm leading-relaxed text-slate-700" }, "This permanently removes \u201C", recoverySummary.title, "\u201D and its saved checkpoints from this browser. This cannot be undone."),
    /* @__PURE__ */ React.createElement("div", { className: "mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end" }, /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, "data-sf-cancel-discard-draft": true, onClick: cancelDiscardDraftConfirmation, className: "min-h-11 rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50" }, ta("a11y.storyforge_ui_keep_saved_project")), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, "data-sf-confirm-discard-draft": true, onClick: () => void confirmDiscardDraft(), className: "min-h-11 rounded-lg bg-rose-700 px-4 py-2 text-sm font-bold text-white hover:bg-rose-800" }, "Delete & start new"))
  )), showCloseConfirm && /* @__PURE__ */ React.createElement("div", { ref: closeConfirmDialogRef, tabIndex: -1, className: "fixed inset-0 z-[210] bg-black/60 flex items-center justify-center motion-safe:animate-in motion-safe:fade-in motion-safe:duration-200", role: "dialog", "aria-modal": "true", "aria-labelledby": "sf-close-confirm-title", "aria-describedby": "sf-close-confirm-description" }, /* @__PURE__ */ React.createElement("div", { className: "sf-dialog-card bg-white rounded-2xl p-6 max-w-sm mx-4 shadow-2xl text-center" }, /* @__PURE__ */ React.createElement("div", { className: "text-3xl mb-3" }, "\u270F\uFE0F"), /* @__PURE__ */ React.createElement("h3", { id: "sf-close-confirm-title", className: "text-lg font-black text-slate-800 mb-2" }, t("ui_common.unsaved_changes")), /* @__PURE__ */ React.createElement("p", { id: "sf-close-confirm-description", className: "text-sm text-slate-600 mb-4" }, ta("a11y.storyforge_ui_your_latest_changes_have_not_finished")), draftSaveError && /* @__PURE__ */ React.createElement("p", { role: "alert", className: "mb-4 rounded-lg border border-red-200 bg-red-50 p-2 text-xs font-bold text-red-700" }, draftSaveError), /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row gap-3 justify-center" }, /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, onClick: () => setShowCloseConfirm(false), className: "px-4 py-2 bg-slate-200 text-slate-700 rounded-lg text-sm font-bold hover:bg-slate-300 transition-colors" }, t("ui_common.keep_working")), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, "data-sf-save-close": true, onClick: async () => {
    if (await persistDraftToStorage({ announce: true, allowDuringHydration: true })) {
      setShowCloseConfirm(false);
      onClose();
    }
  }, className: "px-4 py-2 bg-amber-700 text-white rounded-lg text-sm font-bold hover:bg-amber-800 transition-colors" }, t("ui_common.save_draft_close")), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, onClick: () => {
    setShowCloseConfirm(false);
    onClose();
  }, className: "px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-bold hover:bg-red-700 transition-colors" }, t("ui_common.close_anyway"))))), exportConsent && /* @__PURE__ */ React.createElement("div", { role: "presentation", className: "fixed inset-0 z-[230] bg-black/70 flex items-center justify-center p-4" }, /* @__PURE__ */ React.createElement("div", { ref: exportConsentDialogRef, role: "alertdialog", "aria-modal": "true", "aria-labelledby": "sf-export-consent-title", "aria-describedby": "sf-export-consent-message", tabIndex: -1, className: "sf-dialog-card w-full max-w-lg rounded-2xl border-2 border-cyan-300 bg-white p-6 shadow-2xl" }, /* @__PURE__ */ React.createElement("h3", { id: "sf-export-consent-title", className: "text-lg font-black text-slate-900" }, exportConsent.title), /* @__PURE__ */ React.createElement("p", { id: "sf-export-consent-message", className: "mt-2 text-sm leading-relaxed text-slate-700" }, exportConsent.message), /* @__PURE__ */ React.createElement("div", { className: "mt-5 flex flex-wrap justify-end gap-3" }, /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, onClick: () => finishExportConsent(false), className: "rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50" }, ta("a11y.storyforge_ui_cancel")), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, onClick: () => finishExportConsent(true), className: "rounded-lg bg-cyan-700 px-4 py-2 text-sm font-bold text-white hover:bg-cyan-800" }, exportConsent.confirmLabel || "Export file")))), importConfirmation && /* @__PURE__ */ React.createElement("div", { role: "presentation", className: "fixed inset-0 z-[230] bg-black/70 flex items-center justify-center p-4" }, /* @__PURE__ */ React.createElement("div", { ref: importConfirmationDialogRef, role: "alertdialog", "aria-modal": "true", "aria-labelledby": "sf-import-confirm-title", "aria-describedby": "sf-import-confirm-message", tabIndex: -1, className: "sf-dialog-card w-full max-w-lg rounded-2xl border-2 border-amber-300 bg-white p-6 shadow-2xl" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-start gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "rounded-full bg-amber-100 p-2 text-amber-700", "aria-hidden": "true" }, /* @__PURE__ */ React.createElement(RefreshCw, { size: 18 })), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h3", { id: "sf-import-confirm-title", className: "text-lg font-black text-slate-900" }, ta("a11y.storyforge_ui_replace_current_project")), /* @__PURE__ */ React.createElement("p", { id: "sf-import-confirm-message", className: "mt-2 text-sm leading-relaxed text-slate-700" }, ta("a11y.storyforge_ui_importing_this_file_replaces_the_current")))), /* @__PURE__ */ React.createElement("dl", { className: "mt-5 grid gap-2 rounded-xl border border-amber-200 bg-amber-50/60 p-3 text-xs sm:grid-cols-2" }, /* @__PURE__ */ React.createElement("div", { className: "min-w-0" }, /* @__PURE__ */ React.createElement("dt", { className: "font-bold uppercase tracking-wide text-amber-800" }, ta("a11y.storyforge_ui_incoming_project")), /* @__PURE__ */ React.createElement("dd", { className: "mt-1 truncate font-black text-slate-900" }, importConfirmation.title)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("dt", { className: "font-bold uppercase tracking-wide text-amber-800" }, ta("a11y.storyforge_ui_format")), /* @__PURE__ */ React.createElement("dd", { className: "mt-1 font-bold text-slate-800" }, "v", importConfirmation.version || STORYFORGE_PROJECT_VERSION, " \xB7 ", importConfirmation.layoutMode === "comic" ? "Comic" : "Story")), importConfirmation.layoutMode === "comic" && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("dt", { className: "font-bold uppercase tracking-wide text-amber-800" }, ta("a11y.storyforge_ui_pages")), /* @__PURE__ */ React.createElement("dd", { className: "mt-1 font-bold text-slate-800" }, importConfirmation.pageCount)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("dt", { className: "font-bold uppercase tracking-wide text-amber-800" }, ta("a11y.storyforge_ui_panels")), /* @__PURE__ */ React.createElement("dd", { className: "mt-1 font-bold text-slate-800" }, importConfirmation.panelCount))), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("dt", { className: "font-bold uppercase tracking-wide text-amber-800" }, ta("a11y.storyforge_ui_source")), /* @__PURE__ */ React.createElement("dd", { className: "mt-1 truncate font-bold text-slate-800" }, importConfirmation.exportedBy || "Not provided")), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("dt", { className: "font-bold uppercase tracking-wide text-amber-800" }, ta("a11y.storyforge_ui_exported")), /* @__PURE__ */ React.createElement("dd", { className: "mt-1 font-bold text-slate-800" }, importConfirmation.exportedAt ? new Date(importConfirmation.exportedAt).toLocaleDateString() : "Not provided"))), /* @__PURE__ */ React.createElement("div", { className: "mt-5 flex flex-wrap justify-end gap-3" }, /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, onClick: () => finishImportConfirmation(false), className: "rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50" }, ta("a11y.storyforge_ui_keep_current_project")), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, onClick: () => finishImportConfirmation("checkpoint"), className: "rounded-lg border border-amber-500 bg-white px-4 py-2 text-sm font-bold text-amber-800 hover:bg-amber-50" }, "Save checkpoint & import"), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, "data-sf-import-confirm-action": "replace", onClick: () => finishImportConfirmation(true), className: "rounded-lg bg-amber-700 px-4 py-2 text-sm font-bold text-white hover:bg-amber-800" }, ta("a11y.storyforge_ui_replace_and_import"))))), /* @__PURE__ */ React.createElement("div", { className: "sf-project-header bg-gradient-to-r from-rose-700 to-pink-700 p-3 sm:p-4 text-white flex justify-between items-center gap-2 shadow-lg shrink-0" }, /* @__PURE__ */ React.createElement("div", { className: "flex min-w-0 flex-1 items-center gap-3" }, /* @__PURE__ */ React.createElement(BookOpen, { size: 24 }), /* @__PURE__ */ React.createElement("div", { className: "min-w-0" }, /* @__PURE__ */ React.createElement("h2", { className: "truncate text-lg font-black sm:text-xl" }, t("headings.story_forge")), /* @__PURE__ */ React.createElement("p", { className: "max-w-[58vw] truncate text-[11px] font-semibold text-rose-100 sm:max-w-[42vw] sm:text-xs", title: `${storyTitle.trim() || `Untitled ${artifactType === "comic" ? "comic" : "story"}`} - ${phaseLabel(phaseIdx)} - ${draftSaveLabel}` }, /* @__PURE__ */ React.createElement("span", { "data-sf-header-project-title": true }, storyTitle.trim() || `Untitled ${artifactType === "comic" ? "comic" : "story"}`), /* @__PURE__ */ React.createElement("span", { "aria-hidden": "true" }, " \xB7 "), /* @__PURE__ */ React.createElement("span", { "data-sf-header-artifact": true }, artifactType === "comic" ? "Comic" : "Story"), /* @__PURE__ */ React.createElement("span", { "aria-hidden": "true" }, " \xB7 "), /* @__PURE__ */ React.createElement("span", null, phaseLabel(phaseIdx)), /* @__PURE__ */ React.createElement("span", { "aria-hidden": "true" }, " \xB7 "), /* @__PURE__ */ React.createElement("span", { "data-sf-header-save-state": true }, draftSaveLabel)))), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-1 sm:gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "hidden md:flex bg-white/20 px-3 py-1 rounded-full text-xs font-bold items-center gap-2", title: `${xpData.totalXP} XP \xB7 ${levelLabel(currentLevel)}${xpData.streak > 1 ? ` \xB7 ${xpData.streak}-day streak` : ""}` }, /* @__PURE__ */ React.createElement("span", null, currentLevel.emoji, " ", levelLabel(currentLevel)), /* @__PURE__ */ React.createElement("span", { className: "text-rose-200" }, xpData.totalXP, " XP"), xpData.streak > 1 && /* @__PURE__ */ React.createElement("span", { className: "text-amber-700" }, "\u{1F525}", xpData.streak), nextLevel && /* @__PURE__ */ React.createElement("div", { className: "w-12 h-1.5 bg-white/20 rounded-full overflow-hidden" }, /* @__PURE__ */ React.createElement("div", { className: "h-full bg-amber-300 rounded-full transition-all", style: { width: `${Math.min(100, (xpData.totalXP - currentLevel.min) / (nextLevel.min - currentLevel.min) * 100)}%` } }))), totalWords > 0 && /* @__PURE__ */ React.createElement("div", { className: "hidden lg:flex bg-white/20 px-3 py-1 rounded-full text-xs font-bold items-center gap-2" }, /* @__PURE__ */ React.createElement("span", null, totalWords, " words"), /* @__PURE__ */ React.createElement("span", null, "\xB7"), /* @__PURE__ */ React.createElement("span", null, vocabUsedCount, "/", vocabTerms.length, " terms"), readingLevel && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", null, "\xB7"), /* @__PURE__ */ React.createElement("span", null, ta("a11y.storyforge_ui_grade").replace("{0}", readingLevel.grade)))), /* @__PURE__ */ React.createElement("details", { "data-sf-project-menu": true, className: "relative" }, /* @__PURE__ */ React.createElement(
    "summary",
    {
      "data-sf-focusable": true,
      "data-sf-project-menu-trigger": true,
    ...
    /* @__PURE__ */ React.createElement(BookOpen, { size: 15, "aria-hidden": "true" }),
    /* @__PURE__ */ React.createElement("span", null, ta("a11y.storyforge_ui_project"))
  ), /* @__PURE__ */ React.createElement("div", { className: "sf-project-menu-panel absolute right-0 z-[260] mt-2 w-72 max-w-[calc(100vw-1.5rem)] rounded-2xl border border-slate-200 bg-white p-3 text-slate-800 shadow-2xl", role: "group", "aria-label": ta("a11y.storyforge_attr_project_actions") }, /* @__PURE__ */ React.createElement("div", { className: "border-b border-slate-200 px-2 pb-3" }, /* @__PURE__ */ React.createElement("p", { className: "truncate text-sm font-black", "data-sf-project-menu-title": true }, storyTitle.trim() || `Untitled ${artifactType === "comic" ? "comic" : "story"}`), /* @__PURE__ */ React.createElement("p", { className: "mt-0.5 text-xs text-slate-500" }, ta(artifactType === "comic" ? "a11y.storyforge_ui_comic" : "a11y.storyforge_ui_story"), " \xB7 ", phaseLabel(phaseIdx), " \xB7 ", draftSaveLabel)), draftHydrationState === "awaiting" ? /* @__PURE__ */ React.createElement("div", { className: "mt-3 space-y-2" }, /* @__PURE__ */ React.createElement("p", { className: "px-2 text-xs leading-relaxed text-amber-800" }, ta("a11y.storyforge_ui_a_saved_project_is_waiting_restore")), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-project-menu-restore": true, onClick: restoreDraft, className: "flex min-h-10 w-full items-center gap-2 rounded-lg bg-amber-700 px-3 py-2 text-left text-xs font-bold text-white hover:bg-amber-800" }, /* @__PURE__ */ React.createElement(RefreshCw, { size: 14, "aria-hidden": "true" }), " ", ta("a11y.storyforge_ui_restore_saved_project")), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-project-menu-recovery-options": true, onClick: () => setShowRestorePrompt(true), className: "min-h-10 w-full rounded-lg border border-amber-300 bg-white px-3 py-2 text-left text-xs font-bold text-amber-900 hover:bg-amber-50" }, ta("a11y.storyforge_ui_review_recovery_options"))) : /* @__PURE__ */ React.createElement("div", { className: "mt-3 grid gap-2" }, /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-project-menu-save": true, onClick: () => void persistDraftToStorage({ announce: true }), disabled: projectMutationBusy || draftHydrationState !== "ready" || draftSaveState === "saving", className: "flex min-h-10 w-full items-center gap-2 rounded-lg bg-rose-600 px-3 py-2 text-left text-xs font-bold text-white hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-60" }, /* @__PURE__ */ React.createElement(Save, { size: 14, "aria-hidden": "true" }), " ", ta("a11y.storyforge_ui_save_project_now")), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-project-menu-export": true, onClick: () => void exportStoryForgeProject(), disabled: draftHydrationState !== "ready", className: "flex min-h-10 w-full items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-left text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60" }, /* @__PURE__ */ React.createElement(Download, { size: 14, "aria-hidden": "true" }), " ", ta("a11y.storyforge_ui_export_backup")), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-project-menu-import": true, onClick: importDraftJSON, disabled: projectMutationBusy || draftHydrationState !== "ready", className: "flex min-h-10 w-full items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-left text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60" }, /* @__PURE__ */ React.createElement(RefreshCw, { size: 14, "aria-hidden": "true" }), " ", ta("a11y.storyforge_ui_import_project")), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-project-menu-checkpoint": true, onClick: () => void saveRevisionCheckpoint(), disabled: projectMutationBusy || draftHydrationState !== "ready" || draftSaveState === "saving", className: "flex min-h-10 w-full items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-left text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60" }, /* @__PURE__ */ React.createElement(Star, { size: 14, "aria-hidden": "true" }), " ", ta("a11y.storyforge_ui_save_checkpoint"))), draftHydrationState === "ready" && /* @__PURE__ */ React.createElement("section", { className: "mt-3 border-t border-slate-200 pt-3", "aria-labelledby": "sf-menu-checkpoints-title" }, /* @__PURE__ */ React.createElement("h3", { id: "sf-menu-checkpoints-title", className: "text-xs font-bold text-slate-800" }, ux("checkpoints", "Checkpoints")), /* @__PURE__ */ React.createElement("label", { className: "mt-2 block text-xs text-slate-700" }, ux("checkpoint_name", "Name the next checkpoint"), /* @__PURE__ */ React.createElement("input", { "data-sf-checkpoint-name": true, value: revisionLabel, onChange: (e) => setRevisionLabel(e.target.value.slice(0, 100)), className: "mt-1 w-full rounded-lg border border-slate-400 p-2 text-sm", placeholder: ux("checkpoint_example", "For example: before the ending") })), /* @__PURE__ */ React.createElement("p", { className: "mt-2 text-xs text-slate-600" }, ux("checkpoint_restore_help", "Restoring saves your current version first, so you can undo it.")), revisionHistory.length === 0 ? /* @__PURE__ */ React.createElement("p", { className: "mt-2 text-xs text-slate-600" }, ux("no_checkpoints", "No checkpoints yet. Name one above, then choose Save checkpoint.")) : /* @__PURE__ */ React.createElement("ul", { className: "mt-2 space-y-1" }, revisionHistory.slice(0, 6).map((revision) => /* @__PURE__ */ React.createElement("li", { key: revision.id }, /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-menu-checkpoint": revision.id, disabled: projectMutationBusy || isProcessing, onClick: (event) => {
    const menu = event.currentTarget.closest("details");
    if (menu) menu.open = false;
    void restoreRevisionCheckpoint(revision);
  }, className: "min-h-11 w-full rounded-lg border border-slate-300 px-3 py-2 text-left text-xs text-slate-800 hover:bg-slate-100 disabled:opacity-50" }, /* @__PURE__ */ React.createElement("span", { className: "block font-bold" }, revision.label), /* @__PURE__ */ React.createElement("span", { className: "block mt-1" }, new Date(revision.savedAt).toLocaleString())))))), /* @__PURE__ */ React.createElement("p", { className: "mt-3 px-2 text-[11px] leading-relaxed text-slate-500" }, ta("a11y.storyforge_ui_backups_and_checkpoints_preserve_work_before")))), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      "data-sf-focusable": true,
      "data-sf-draft-save": true,
    ...
    draftSaveState === "saving" ? /* @__PURE__ */ React.createElement(RefreshCw, { size: 15, "aria-hidden": "true" }) : draftSaveState === "saved" ? /* @__PURE__ */ React.createElement(CheckCircle2, { size: 15, "aria-hidden": "true" }) : /* @__PURE__ */ React.createElement(Save, { size: 15, "aria-hidden": "true" }),
    /* @__PURE__ */ React.createElement("span", { className: "hidden sm:inline" }, draftSaveLabel)
  ), /* @__PURE__ */ React.createElement("span", { className: "sr-only", "aria-live": "polite", "data-sf-draft-save-live": true }, draftSaveState === "error" ? draftSaveError : draftSaveState === "saving" ? "Saving project" : draftSaveState === "saved" ? "Project saved" : ""), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      "data-sf-focusable": true,
  ...
  ), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, onClick: safeClose, className: "hover:bg-white/20 p-2 rounded-full transition-colors", "aria-label": t("a11y.close_story_forge") }, /* @__PURE__ */ React.createElement(X, { size: 24 })))), draftHydrationState === "awaiting" && !showRestorePrompt && !showDiscardDraftConfirm && /* @__PURE__ */ React.createElement(
    "section",
    {
      "data-sf-recovery-banner": true,
      "data-sf-recovery-title": recoverySummary.title,
      "data-sf-recovery-artifact": recoverySummary.artifactType,
      "data-sf-recovery-resume-phase": recoverySummary.resumePhase,
    ...
    /* @__PURE__ */ React.createElement("div", { className: "mx-auto flex max-w-4xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between" }, /* @__PURE__ */ React.createElement("div", { className: "min-w-0" }, /* @__PURE__ */ React.createElement("h2", { id: "sf-recovery-banner-title", className: "text-sm font-black text-amber-950" }, ta("a11y.storyforge_ui_saved_project_waiting"), " ", /* @__PURE__ */ React.createElement("span", { className: "break-words" }, recoverySummary.title)), /* @__PURE__ */ React.createElement("p", { className: "mt-0.5 text-xs text-amber-900" }, recoverySummary.artifactLabel, " \xB7 ", ta("a11y.storyforge_ui_resume_safely_at"), " ", recoverySummary.resumePhaseLabel, recoverySummary.savedAt ? " \xB7 Saved " + new Date(recoverySummary.savedAt).toLocaleString() : "")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-col gap-2 sm:flex-row" }, /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, "data-sf-banner-restore-draft": true, onClick: restoreDraft, className: "min-h-11 rounded-lg bg-amber-700 px-4 py-2 text-xs font-bold text-white hover:bg-amber-800" }, ta("a11y.storyforge_ui_restore_project")), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-focusable": true, "data-sf-open-recovery-options": true, onClick: () => setShowRestorePrompt(true), className: "min-h-11 rounded-lg border border-amber-400 bg-white px-4 py-2 text-xs font-bold text-amber-900 hover:bg-amber-100" }, ta("a11y.storyforge_ui_review_recovery_options"))))
  ), projectActionUndo && /* @__PURE__ */ React.createElement("div", { className: "sf-undo-notice flex shrink-0 flex-wrap items-center gap-2 border-b border-indigo-200 bg-indigo-50 px-3 py-2 text-xs text-indigo-900" }, /* @__PURE__ */ React.createElement("span", { role: "status", className: "flex-1" }, ux("previous_saved", "Your previous version is saved.")), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-undo-project-edit": true, disabled: projectMutationBusy || isProcessing, onClick: () => void restoreRevisionCheckpoint(projectActionUndo), className: "min-h-11 rounded-lg border border-indigo-400 px-3 py-2 font-bold disabled:opacity-50" }, ux("undo_last_change", "Undo last change")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setProjectActionUndo(null), className: "min-h-11 rounded-lg px-2 underline" }, ux("dismiss", "Dismiss"))), /* @__PURE__ */ React.createElement("div", { className: "sf-mobile-workflow" }, /* @__PURE__ */ React.createElement("label", { htmlFor: "sf-mobile-step", className: "sr-only" }, ux("choose_step", "Choose a step")), /* @__PURE__ */ React.createElement("select", { id: "sf-mobile-step", value: phase, onChange: (e) => changePhase(e.target.value) }, PHASES.map((p, i) => /* @__PURE__ */ React.createElement("option", { key: p, value: p, disabled: !canEnterPhase(p) }, i + 1, ". ", phaseLabel(i)))), /* @__PURE__ */ React.createElement("button", { type: "button", "aria-expanded": showProjectHealth, "aria-controls": "sf-project-health", onClick: () => setShowProjectHealth((v) => !v) }, ux("checklist", "Checklist"))), /* @__PURE__ */ React.createElement("div", { className: "sf-workflow-dashboard bg-white border-b border-slate-200 shrink-0" }, /* @__PURE__ */ React.createElement("nav", { className: "px-2 sm:px-6 pt-3 pb-2 flex items-center justify-start sm:justify-center gap-1 overflow-x-auto", role: "navigation", "aria-label": t("a11y.story_creation_phases") }, PHASES.map((p, i) => {
    ...
    const tone = isCurrent ? "bg-rose-600 text-white border-rose-600 shadow-md" : phaseHealth.status === "ready" ? "bg-emerald-50 text-emerald-800 border-emerald-200 hover:bg-emerald-100" : phaseHealth.status === "blocked" ? "bg-rose-50 text-rose-800 border-rose-200 hover:bg-rose-100" : phaseHealth.status === "progress" ? "bg-blue-50 text-blue-800 border-blue-200 hover:bg-blue-100" : phaseHealth.status === "optional" ? "bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100" : "bg-amber-50 text-amber-800 border-amber-200 hover:bg-amber-100";
    ...
    return /* @__PURE__ */ React.createElement(React.Fragment, { key: p }, i > 0 && /* @__PURE__ */ React.createElement("div", { className: `w-2 sm:w-7 h-0.5 shrink-0 ${isReady ? "bg-emerald-300" : phaseHealth.status === "blocked" ? "bg-rose-300" : "bg-slate-200"}`, "aria-hidden": "true" }), /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        "data-sf-focusable": true,
        ...
        "aria-label": ta("a11y.storyforge_aria_percent").replace("{0}", phaseLabel(i)).replace("{1}", statusLabel).replace("{2}", phaseHealth.score).replace("{3}", phaseHealth.detail),
      ...
      /* @__PURE__ */ React.createElement("span", null, phaseLabel(i)),
      /* @__PURE__ */ React.createElement("span", { className: `hidden sm:block w-1.5 h-1.5 rounded-full ${isCurrent ? "bg-white" : isReady ? "bg-emerald-500" : phaseHealth.status === "blocked" ? "bg-rose-500" : phaseHealth.status === "progress" ? "bg-blue-500" : "bg-amber-500"}`, "aria-hidden": "true" })
    ));
  })), /* @__PURE__ */ React.createElement("div", { id: "sf-project-health", "data-expanded": showProjectHealth, className: "sf-project-health px-3 sm:px-6 pb-3" }, /* @__PURE__ */ React.createElement("div", { className: "max-w-4xl mx-auto flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4" }, /* @__PURE__ */ React.createElement("div", { className: "min-w-0 flex-1" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-3 text-[11px] font-black uppercase text-slate-600" }, /* @__PURE__ */ React.createElement("span", null, ta("a11y.storyforge_ui_build_checklist")), /* @__PURE__ */ React.createElement("span", null, projectReadiness.readyCount, "/", PHASES.length, " ", ta("a11y.storyforge_ui_steps_ready"))), /* @__PURE__ */ React.createElement("div", { className: "mt-1 h-2 overflow-hidden rounded-full bg-slate-200", role: "progressbar", "aria-label": ta("a11y.storyforge_attr_artifact_production_readiness"), "aria-valuemin": "0", "aria-valuemax": "100", "aria-valuenow": projectReadiness.percent }, /* @__PURE__ */ React.createElement("div", { className: `h-full rounded-full transition-all ${projectReadiness.blockers.length ? "bg-rose-500" : projectReadiness.warnings.length ? "bg-amber-500" : "bg-emerald-500"}`, style: { width: `${projectReadiness.percent}%` } })), /* @__PURE__ */ React.createElement("div", { className: "mt-1 text-[11px] font-medium text-slate-500" }, phase === "configure" && !hasStoryCue ? ux("start_hint", "Choose Story or Comic, then add a title or starting idea.") : projectReadiness.summary)), primaryReadinessIssue && !(phase === "configure" && !hasStoryCue) && /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      "data-sf-focusable": true,
    ...
    /* @__PURE__ */ React.createElement("span", { className: "min-w-0 flex-1" }, /* @__PURE__ */ React.createElement("span", { className: "block text-[9px] uppercase tracking-widest" }, projectReadiness.blockers.length ? "Required" : "Recommended"), /* @__PURE__ */ React.createElement("span", { className: "block truncate" }, primaryReadinessIssue.label)),
    /* @__PURE__ */ React.createElement(ArrowRight, { size: 14, className: "shrink-0", "aria-hidden": "true" })
  )))), /* @__PURE__ */ React.createElement("section", { className: "sf-step-guide bg-slate-50 border-b border-slate-200 px-3 sm:px-6 py-3 shrink-0", "aria-labelledby": "sf-current-step-title" }, /* @__PURE__ */ React.createElement("div", { className: "max-w-4xl mx-auto flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-5" }, /* @__PURE__ */ React.createElement("div", { className: "min-w-0 flex-1" }, /* @__PURE__ */ React.createElement("div", { className: "sf-step-kicker text-[10px] font-black uppercase tracking-widest text-rose-700" }, ta("a11y.storyforge_ui_step_of").replace("{0}", phaseIdx + 1).replace("{1}", PHASES.length), " \xB7 ", optLabel("artifact", artifactType, ARTIFACT_TYPES[artifactType].label)), /* @__PURE__ */ React.createElement("h2", { id: "sf-current-step-title", className: "text-base font-black text-slate-800" }, currentPhaseGuide.title), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-600 mt-0.5" }, currentPhaseGuide.outcome)), /* @__PURE__ */ React.createElement(
    "div",
    {
      id: "sf-phase-requirements",
      "data-sf-phase-requirements": true,
      className: "sm:max-w-sm rounded-lg border px-3 py-2 text-xs font-bold " + (canGoNext() || phase === "export" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-900")
    },
    currentPhaseRequirement
  ))), /* @__PURE__ */ React.createElement("div", { className: "flex-grow overflow-y-auto", ref: phaseContentRef, tabIndex: -1, role: "region", "aria-label": ta("a11y.storyforge_aria_phase").replace("{0}", PHASE_LABELS[phaseIdx]) }, /* @__PURE__ */ React.createElement("div", { className: "max-w-4xl mx-auto px-3 py-4 sm:p-6" }, phase === "configure" && /* @__PURE__ */ React.createElement("div", { className: `space-y-6 ${animClass}` }, /* @__PURE__ */ React.createElement("div", { className: "text-center mb-6" }, /* @__PURE__ */ React.createElement("h3", { className: "text-2xl font-black text-slate-800" }, "Plan your ", artifactType === "comic" ? "comic" : "story"), /* @__PURE__ */ React.createElement("p", { className: "text-slate-600 text-sm mt-1" }, "Name your ", artifactType === "comic" ? "comic" : "story", ", choose a genre, and set vocabulary goals"), /* @__PURE__ */ React.createElement("div", { className: "flex gap-2 justify-center mt-2 flex-wrap" }, /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-import-draft": true, onClick: importDraftJSON, className: "px-3 py-1 bg-cyan-100 text-cyan-700 rounded-full text-[11px] font-bold hover:bg-cyan-200 transition-colors inline-flex items-center gap-1" }, /* @__PURE__ */ React.createElement(Plus, { size: 10 }), " Import classmate's draft"))), lessonImportProposal && /* @__PURE__ */ React.createElement("section", { ref: lessonImportPreviewRef, tabIndex: -1, "data-sf-lesson-preview": true, "aria-labelledby": "sf-lesson-preview-title", className: "rounded-2xl border-2 border-indigo-300 bg-indigo-50 p-4" }, /* @__PURE__ */ React.createElement("h3", { id: "sf-lesson-preview-title", className: "font-bold text-indigo-900" }, ux("lesson_preview", "Preview lesson import"), ": ", lessonImportProposal.title), /* @__PURE__ */ React.createElement("p", { className: "mt-2 text-sm text-indigo-900" }, ux("lesson_preserves", "Your scenes, writing, dialogue, and artwork stay intact. Applying saves a checkpoint first.")), lessonImportProposal.kind === "plan" && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("p", { className: "mt-2 text-sm text-slate-800" }, ux("lesson_prompts", "These planning prompts replace the prompts for matching sections. Extra authored sections are kept.")), /* @__PURE__ */ React.createElement("ol", { className: "my-3 list-decimal space-y-2 pl-5 text-sm text-slate-800" }, lessonImportProposal.suggestions.map((item, index) => /* @__PURE__ */ React.createElement("li", { key: index }, item.scaffoldFrame))), lessonImportProposal.total > maxParagraphs && /* @__PURE__ */ React.createElement("p", { className: "text-sm font-bold text-amber-900" }, ux("lesson_limit", "Only the first {0} sections fit in this project.").replace("{0}", maxParagraphs))), lessonImportProposal.kind === "vocabulary" && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("p", { className: "mt-2 text-sm text-slate-800" }, ux("lesson_vocab", "New goals are added. Existing goals and definitions are kept; duplicates are skipped. Projects support up to 64 goals.")), /* @__PURE__ */ React.createElement("ul", { className: "my-3 list-disc pl-5 text-sm text-slate-800" }, lessonImportProposal.terms.map((item, index) => /* @__PURE__ */ React.createElement("li", { key: index }, item.term)))), lessonImportProposal.kind === "prompt" && /* @__PURE__ */ React.createElement(React.Fragment, null, storyPrompt && /* @__PURE__ */ React.createElement("p", { className: "mt-3 text-sm text-slate-700" }, /* @__PURE__ */ React.createElement("strong", null, ux("current_idea", "Current starting idea"), ": "), storyPrompt), /* @__PURE__ */ React.createElement("p", { className: "my-3 text-sm text-slate-800" }, /* @__PURE__ */ React.createElement("strong", null, ux("new_idea", "New starting idea"), ": "), lessonImportProposal.prompt)), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2" }, /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-apply-lesson": true, onClick: () => void applyLessonImport(), disabled: projectMutationBusy || isProcessing, className: "min-h-11 rounded-lg bg-indigo-700 px-4 py-2 font-bold text-white disabled:opacity-50" }, ux("apply_lesson", "Apply lesson resource")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setLessonImportProposal(null), className: "min-h-11 rounded-lg border border-slate-400 bg-white px-4 py-2 font-bold text-slate-800" }, ux("cancel_lesson", "Keep current project")))), lessonResources && lessonResources.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "bg-gradient-to-r from-indigo-50 to-violet-50 border-2 border-indigo-200 rounded-2xl p-4" }, /* @__PURE__ */ React.createElement("h4", { className: "text-[11px] font-bold text-indigo-600 uppercase tracking-widest mb-2 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(BookOpen, { size: 12 }), " Import from Lesson Resources"), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2" }, lessonResources.filter((r) => ["glossary", "simplified", "sentence-frames", "lesson-plan", "timeline"].includes(r.type)).map((r, ri) => /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      key: ri,
    ...
    r.title || r.type
  ))), /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-indigo-400 mt-1.5" }, ta("a11y.storyforge_ui_click_to_auto_fill_vocabulary_prompts"))), /* @__PURE__ */ React.createElement("div", { "data-sf-artifact-picker": true, className: "bg-white rounded-2xl border-2 border-blue-100 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-blue-700 uppercase tracking-wider mb-3 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Type, { size: 16 }), " What are you making?"), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-600 mb-3" }, ta("a11y.storyforge_ui_choose_this_before_drafting_story_and")), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-2" }, Object.entries(ARTIFACT_TYPES).map(([key, m]) => /* @__PURE__ */ React.createElement(
    "button",
    {
      key,
      type: "button",
      ...
      "aria-label": optLabel("artifact", key, m.label) + ": " + optLabel("artifactdesc", key, m.desc),
      title: artifactTypeLocked && artifactType !== key ? ta("a11y.storyforge_ui_artifact_type_locked") : optLabel("artifactdesc", key, m.desc),
      className: `p-3 rounded-xl border-2 text-center text-xs font-bold transition-all disabled:opacity-45 disabled:cursor-not-allowed ${artifactType === key ? "border-blue-500 bg-blue-50 text-blue-700 shadow-md" : "border-slate-200 text-slate-600 hover:border-blue-300"}`
    },
    /* @__PURE__ */ React.createElement("span", { className: "block text-sm" }, optLabel("artifact", key, m.label)),
    /* @__PURE__ */ React.createElement("span", { className: "block text-[11px] font-medium leading-snug mt-1" }, m.desc)
  ))), artifactTypeLocked && /* @__PURE__ */ React.createElement("p", { className: "mt-3 text-xs font-bold text-blue-800 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2" }, "Artifact type is locked because drafting has started. Your current Story or Comic structure is protected."), artifactType === "story" && /* @__PURE__ */ React.createElement("div", { "data-sf-writing-view-picker": true, className: "mt-4 pt-4 border-t border-blue-100" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-blue-600 uppercase tracking-widest mb-1" }, ta("a11y.storyforge_ui_writing_view")), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-500 mb-2" }, ta("a11y.storyforge_ui_this_only_changes_how_the_draft")), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-3 gap-2" }, Object.entries(WRITING_VIEWS).map(([key, item]) => /* @__PURE__ */ React.createElement(
    "button",
    {
      key,
      type: "button",
      ...
      title: item.desc,
      className: "p-2.5 rounded-xl border-2 text-left transition-all " + (writingView === key ? "border-blue-500 bg-blue-50 text-blue-700 shadow-sm" : "border-slate-200 text-slate-600 hover:border-blue-300")
    },
    /* @__PURE__ */ React.createElement("div", { className: "text-xs font-black" }, item.label),
    /* @__PURE__ */ React.createElement("div", { className: "text-[10px] leading-snug mt-0.5" }, item.desc)
  )))), layoutMode === "comic" && /* @__PURE__ */ React.createElement("div", { className: "mt-4 pt-4 border-t border-blue-100" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-blue-600 uppercase tracking-widest mb-2" }, ta("a11y.storyforge_ui_comic_page_layout")), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 sm:grid-cols-4 gap-2" }, Object.entries(COMIC_PAGE_LAYOUTS).map(([key, item]) => /* @__PURE__ */ React.createElement(
    "button",
    {
      key,
      type: "button",
    ...
    /* @__PURE__ */ React.createElement("div", { className: "text-xs font-black" }, item.label),
    /* @__PURE__ */ React.createElement("div", { className: "text-[10px] leading-snug mt-0.5" }, item.desc)
  ))))), /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border-2 border-slate-200 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("label", { htmlFor: "sf-title", className: "block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1" }, artifactType === "comic" ? "Comic title" : "Story title"), /* @__PURE__ */ React.createElement(
    "input",
    {
      id: "sf-title",
      type: "text",
  ...
  )), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("label", { className: "block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1" }, t("labels.pen_name")), /* @__PURE__ */ React.createElement("div", { className: "w-full text-sm p-2.5 border border-slate-400 rounded-lg bg-slate-50 font-bold text-slate-700 flex items-center gap-2" }, /* @__PURE__ */ React.createElement("span", { className: "text-base" }, "\u270D\uFE0F"), " ", authorName), /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-slate-500 mt-1" }, ta("a11y.storyforge_ui_your_codename_is_your_pen_name"))))), /* @__PURE__ */ React.createElement("details", { "data-sf-genre-options": true, className: "bg-white rounded-2xl border-2 border-indigo-100 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("summary", { className: "text-sm font-bold text-indigo-700 uppercase tracking-wider mb-3 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(BookOpen, { size: 16 }), " Genre"), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 sm:grid-cols-4 gap-2" }, Object.entries(GENRE_TEMPLATES).map(([key, g]) => /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      key,
    ...
    genreLabel(key)
  )))), /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border-2 border-rose-100 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-rose-700 uppercase tracking-wider mb-3 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(BookOpen, { size: 16 }), " Vocabulary Goals (", vocabTerms.length, " terms)"), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2 mb-4" }, vocabTerms.map((v, i) => /* @__PURE__ */ React.createElement("div", { key: i, className: "bg-rose-50 border border-rose-200 rounded-full px-3 py-1 text-sm font-bold text-rose-800 flex items-center gap-2 group" }, /* @__PURE__ */ React.createElement("span", null, v.term), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => removeVocabTerm(i), className: "text-rose-700 hover:text-rose-600 opacity-60 group-hover:opacity-100 focus:opacity-100 transition-opacity", "aria-label": ta("a11y.storyforge_aria_remove").replace("{0}", v.term) }, /* @__PURE__ */ React.createElement(X, { size: 12 })))), vocabTerms.length === 0 && /* @__PURE__ */ React.createElement("p", { className: "text-slate-500 text-sm italic" }, ta("a11y.storyforge_ui_no_vocabulary_terms_yet_add_some"))), /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row gap-2" }, /* @__PURE__ */ React.createElement(
    "input",
    {
  ...
  ), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: addVocabTerm, disabled: !newTerm.trim(), className: "w-full sm:w-auto justify-center px-4 py-2 bg-rose-600 text-white rounded-lg text-sm font-bold hover:bg-rose-700 disabled:opacity-50 transition-colors flex items-center gap-1" }, /* @__PURE__ */ React.createElement(Plus, { size: 14 }), " Add"))), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
    ...
    /* @__PURE__ */ React.createElement("span", { className: "flex items-center gap-2 text-left" }, /* @__PURE__ */ React.createElement(Palette, { size: 16 }), " Optional setup & assignment"),
    /* @__PURE__ */ React.createElement("span", { className: `transition-transform ${showAdvancedConfig ? "rotate-180" : ""}` }, "\u25BC")
  ), /* @__PURE__ */ React.createElement("p", { className: "-mt-4 px-2 text-xs text-slate-500" }, ta("a11y.storyforge_ui_story_shape_art_language_starting_idea")), showAdvancedConfig && /* @__PURE__ */ React.createElement("div", { className: "space-y-6" }, /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border-2 border-violet-100 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-violet-700 uppercase tracking-wider mb-1 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Sparkles, { size: 16 }), " ", ta("a11y.storyforge_ui_story_shape"), " ", /* @__PURE__ */ React.createElement("span", { className: "text-[10px] font-medium text-slate-500 normal-case tracking-normal" }, "(optional \u2014 the emotional ups & downs)")), /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-slate-500 mb-3" }, ta("a11y.storyforge_ui_pick_the_shape_of_your_character")), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 sm:grid-cols-3 gap-2" }, Object.entries(STORY_SHAPES).map(([key, sh]) => {
    ...
    return /* @__PURE__ */ React.createElement(
      "button",
      {
        ...
        "aria-pressed": active,
        "aria-label": ta("a11y.storyforge_aria_story_shape").replace("{0}", shapeLabel(key)).replace("{1}", shapeDesc(key)),
      ...
      /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-1 mb-1" }, /* @__PURE__ */ React.createElement("span", { className: `text-xs font-bold ${active ? "text-violet-700" : "text-slate-600"}` }, sh.emoji, " ", shapeLabel(key)), /* @__PURE__ */ React.createElement("svg", { viewBox: "0 0 48 18", width: "48", height: "18", "aria-hidden": "true", className: "shrink-0" }, /* @__PURE__ */ React.createElement(
        "polyline",
        {
          fill: "none",
  ...
  }))), /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border-2 border-purple-100 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-purple-700 uppercase tracking-wider mb-3 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Palette, { size: 16 }), " Art Style"), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-3 sm:grid-cols-6 gap-2" }, Object.keys(ART_STYLE_MAP).map((style) => /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      key: style,
  ...
  ), language !== "en" && /* @__PURE__ */ React.createElement("p", { className: "mt-2 text-[11px] text-teal-500 font-medium" }, ta("a11y.storyforge_ui_ai_scaffolds_coaching_grading_and_dictation").replace("{0}", langLabel))), /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border-2 border-amber-100 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-amber-700 uppercase tracking-wider mb-3 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Sparkles, { size: 16 }), " Starting Idea (Optional)"), /* @__PURE__ */ React.createElement(
    "textarea",
    {
      value: storyPrompt,
  ...
  ), genre !== "free" && STORY_STARTERS[genre] && /* @__PURE__ */ React.createElement("div", { className: "mt-3 pt-3 border-t border-amber-100" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-amber-500 uppercase tracking-widest mb-2" }, "\u{1F4A1} ", genreLabel(genre), " ", ta("a11y.storyforge_ui_story_starters_click_to_use")), /* @__PURE__ */ React.createElement("div", { className: "space-y-2" }, STORY_STARTERS[genre].map((starter, si) => /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      key: si,
    ...
    starter,
    '"'
  ))))), /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border-2 border-emerald-100 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-emerald-700 uppercase tracking-wider mb-3 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Star, { size: 16 }), " Custom Rubric (Optional)"), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-600 mb-2" }, ta("a11y.storyforge_ui_paste_a_custom_grading_rubric_if")), /* @__PURE__ */ React.createElement(
    "textarea",
    {
      id: "sf-rubric",
      value: rubricText,
  ...
  )), onSaveConfig && /* @__PURE__ */ React.createElement("div", { className: "flex justify-end rounded-2xl border-2 border-indigo-100 bg-indigo-50/50 p-4" }, /* @__PURE__ */ React.createElement("button", { type: "button", onClick: saveAsConfig, disabled: vocabTerms.length === 0, className: "inline-flex w-full sm:w-auto items-center justify-center gap-1 rounded-lg bg-indigo-100 px-4 py-2 text-xs font-bold text-indigo-700 transition-colors hover:bg-indigo-200 disabled:opacity-40" }, /* @__PURE__ */ React.createElement(Download, { size: 12 }), " Save as Assignment")))), phase === "write" && /* @__PURE__ */ React.createElement("div", { className: `space-y-4 ${animClass}` }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h3", { className: "text-2xl font-black text-slate-800" }, artifactType === "comic" ? "Draft your comic" : "Draft your story"), /* @__PURE__ */ React.createElement("p", { className: "text-slate-600 text-sm mt-1" }, artifactType === "comic" ? "Build one clear panel beat at a time" : "Build one clear scene at a time", revisionSnapshot && /* @__PURE__ */ React.createElement("span", { className: "text-indigo-500 ml-2" }, "Draft #", draftCount, " \u2014 revising!"))), /* @__PURE__ */ React.createElement("div", { className: "flex gap-2 items-center flex-wrap sm:justify-end" }, /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
    ...
    " ",
    layoutMode === "comic" ? scaffoldsGenerated ? "Regenerate Panel Plan" : "Generate Panel Plan" : scaffoldsGenerated ? "Regenerate Scene Plan" : "Generate Scene Plan"
  ), /* @__PURE__ */ React.createElement("button", { type: "button", "aria-pressed": focusMode, onClick: () => {
    setFocusMode(!focusMode);
    setFocusParagraphIdx(0);
  }, className: "min-h-11 rounded-full border border-indigo-300 px-4 py-2 text-xs font-bold text-indigo-800" }, focusMode ? ux("all_scenes", "All scenes") : ux("focus_writing", "Focus on writing")), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
    ...
    /* @__PURE__ */ React.createElement("div", { className: `grid gap-4 ${layoutMode === "comic" && onCallGemini ? "lg:grid-cols-3" : "md:grid-cols-2"}` }, /* @__PURE__ */ React.createElement(
      "section",
      {
        "aria-labelledby": "sf-writing-setup-heading",
        className: "rounded-xl border border-rose-100 bg-rose-50/40 p-3"
      },
      /* @__PURE__ */ React.createElement("h4", { id: "sf-writing-setup-heading", className: "text-xs font-black uppercase tracking-wider text-rose-800" }, "Writing setup"),
      /* @__PURE__ */ React.createElement("div", { className: "mt-3 flex flex-wrap items-center gap-2" }, timerActive ? /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2 bg-rose-100 border border-rose-300 rounded-full px-3 py-1" }, /* @__PURE__ */ React.createElement("span", { className: `text-xs font-black tabular-nums ${timerDuration - timerSeconds <= 30 ? "text-red-600 animate-pulse motion-reduce:animate-none" : "text-rose-700"}` }, formatTime(timerSeconds)), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => {
        setTimerActive(false);
        clearTimeout(timerRef.current);
      }, className: "text-[11px] font-bold text-rose-500 hover:text-rose-700" }, t("ui_common.stop"))) : /* @__PURE__ */ React.createElement("div", { className: "flex bg-slate-100 rounded-full p-0.5" }, [3, 5, 10].map((min) => /* @__PURE__ */ React.createElement("button", { type: "button", key: min, onClick: () => startTimer(min), className: "px-2 py-1 rounded-full text-[11px] font-bold text-slate-600 hover:text-rose-600 hover:bg-white transition-all", title: `${min}-minute writing sprint` }, min, "m"))), artifactType === "story" ? /* @__PURE__ */ React.createElement("div", { "data-sf-writing-view-picker": true, className: "flex flex-wrap bg-slate-100 rounded-full p-0.5" }, Object.entries(WRITING_VIEWS).map(([key, item]) => /* @__PURE__ */ React.createElement(
        "button",
        {
          type: "button",
          key,
          onClick: () => selectWritingView(key),
          "aria-pressed": writingView === key,
          className: `px-3 py-1 rounded-full text-[11px] font-bold transition-all ${writingView === key ? "bg-white text-slate-800 shadow-sm" : "text-slate-600 hover:text-slate-700"}`,
          "aria-label": item.desc,
          title: item.desc
        },
        item.label
      ))) : /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => changePhase("configure"), className: "px-3 py-1 rounded-full text-[11px] font-bold bg-blue-100 text-blue-800 hover:bg-blue-200", title: ta("a11y.storyforge_attr_artifact_type_is_chosen_in_plan") }, "Comic \xB7 Change in Plan"))
      ...
      /* @__PURE__ */ React.createElement("h4", { id: "sf-comic-helpers-heading", className: "text-xs font-black uppercase tracking-wider text-blue-800" }, "Comic helpers"),
      /* @__PURE__ */ React.createElement("div", { className: "mt-3 flex flex-wrap items-center gap-2" }, layoutMode === "comic" && onCallGemini && /* @__PURE__ */ React.createElement(
        "button",
        {
          type: "button",
      ...
      /* @__PURE__ */ React.createElement("h4", { id: "sf-focus-feedback-heading", className: "text-xs font-black uppercase tracking-wider text-indigo-800" }, "Focus & feedback"),
      /* @__PURE__ */ React.createElement("div", { className: "mt-3 flex flex-wrap items-center gap-2" }, /* @__PURE__ */ React.createElement(
        "button",
        {
          type: "button",
          onClick: () => {
          ...
          title: focusMode ? "Show all " + (artifactType === "comic" ? "panels" : "scenes") + " at once" : "Focus on one " + (artifactType === "comic" ? "panel" : "scene") + " at a time"
        },
    ...
    return /* @__PURE__ */ React.createElement("div", { key: i, className: "relative group" }, /* @__PURE__ */ React.createElement(
      "button",
      {
        ...
        "aria-label": ta("a11y.storyforge_aria_copy_term_to_paste_into_your_draft").replace("{0}", v.term).replace("{1}", used ? " \u2014 used" : " \u2014 not yet used"),
        ...
        onClick: async () => {
          if (!navigator.clipboard?.writeText) {
            if (addToast) addToast(ta("a11y.storyforge_toast_copy_manually_clipboard_unavailable").replace("{0}", v.term), "error");
            return;
          }
          try {
            const ok = window.alloCopyText ? await window.alloCopyText(v.term) : false;
            if (!ok) throw new Error("copy unavailable");
            if (addToast) addToast(ta("a11y.storyforge_toast_copied_paste_into_your_draft").replace("{0}", v.term), "success");
          } catch (err) {
            console.warn("Clipboard write failed:", err);
            if (addToast) addToast(ta("a11y.storyforge_toast_couldn_t_copy_please_copy_manually").replace("{0}", v.term), "error");
          }
        }
      },
      ...
      v.term
    ), /* @__PURE__ */ React.createElement("div", { id: `sf-vocab-tip-${i}`, role: "tooltip", className: "absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 bg-slate-800 text-white rounded-xl p-3 shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible group-focus-within:opacity-100 group-focus-within:visible transition-all z-50 pointer-events-none" }, /* @__PURE__ */ React.createElement("div", { className: "text-xs font-bold text-amber-300 mb-1" }, v.term), v.definition && /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-200 leading-relaxed mb-1" }, v.definition), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-300 italic" }, "Click to copy \xB7 Paste into your ", artifactType === "comic" ? "panel" : "scene"), /* @__PURE__ */ React.createElement("div", { "aria-hidden": "true", className: "absolute top-full left-1/2 -translate-x-1/2 -mt-1 w-0 h-0 border-l-[6px] border-r-[6px] border-t-[6px] border-transparent border-t-slate-800" })));
    ...
    return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-3 mb-3" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-black text-blue-700 uppercase tracking-widest" }, ta("a11y.storyforge_ui_storyboard_board")), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-500 mt-0.5" }, ta("a11y.storyforge_ui_panel_pacing_camera_bubbles_and_readiness"))), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-black text-blue-700 bg-blue-50 border border-blue-100 rounded-full px-3 py-1" }, readyCount, "/", paragraphs.length, " production-ready")), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2" }, panelSummaries.map(({ p, idx, direction, lettering, hasCaption, hasDirection, hasRough, hasBubble, hasImage, ready }) => {
      ...
      return /* @__PURE__ */ React.createElement(
        ...
        /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-1 mt-2" }, [
          ...
          ["Bubble", hasBubble],
          ["Art", hasImage]
        ].map(([label, ok]) => /* @__PURE__ */ React.createElement("span", { key: label, className: `text-[9px] font-black rounded-full px-1.5 py-0.5 ${ok ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-600"}` }, label)))
      );
    })));
  })()), focusMode && /* @__PURE__ */ React.createElement("div", { "data-sf-focus-navigation": true, className: "flex flex-wrap items-center justify-between gap-2 bg-indigo-50 border-2 border-indigo-200 rounded-2xl p-3" }, /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
    ...
    "\u2190 Previous"
  ), /* @__PURE__ */ React.createElement("div", { className: "sf-focus-summary text-center" }, /* @__PURE__ */ React.createElement("div", { className: "text-xs font-bold text-indigo-700" }, layoutMode === "comic" ? "Panel" : "Scene", " ", focusParagraphIdx + 1, " of ", paragraphs.length), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-indigo-700 mt-0.5" }, paragraphs[focusParagraphIdx]?.scaffoldFrame ? paragraphs[focusParagraphIdx].scaffoldFrame.substring(0, 60) + (paragraphs[focusParagraphIdx].scaffoldFrame.length > 60 ? "..." : "") : "Free write"), /* @__PURE__ */ React.createElement("div", { className: "sf-focus-jumps flex flex-wrap justify-center gap-1 mt-1.5", role: "group", "aria-label": ux("choose_section", "Choose a section") }, paragraphs.map((pp, pi) => /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      key: pi,
      onClick: () => setFocusParagraphIdx(pi),
      className: `sf-focus-jump rounded-lg border font-bold transition-colors ${pi === focusParagraphIdx ? "bg-indigo-700 text-white border-indigo-700" : "bg-white text-indigo-900 border-indigo-300"}`,
    ...
    "button",
    {
      ...
      "data-sf-focus-next": true,
      disabled: focusParagraphIdx >= paragraphs.length - 1 && paragraphs.length >= maxParagraphs,
    ...
    focusParagraphIdx >= paragraphs.length - 1 ? paragraphs.length >= maxParagraphs ? ux("section_limit", "Section limit reached") : artifactType === "comic" ? "+ New panel" : "+ New scene" : "Next \u2192"
  )), paragraphs.map((p, idx) => focusMode && idx !== focusParagraphIdx ? null : /* @__PURE__ */ React.createElement(React.Fragment, { key: p.id }, /* @__PURE__ */ React.createElement(
    "div",
    {
      id: "sf-para-" + p.id,
      ...
      className: `sf-panel-sequence-card rounded-2xl border-2 shadow-sm overflow-hidden transition-colors ${panelSequenceDrag?.from === idx ? "sf-panel-dragging" : panelSequenceDrag?.over === idx ? "sf-panel-drop-target" : ""} ${focusMode ? "border-indigo-300 shadow-lg ring-2 ring-indigo-100" : layoutMode === "dark" ? "bg-slate-800 border-slate-600 text-slate-100" : layoutMode === "journal" ? "bg-amber-50 border-amber-200" : "bg-white border-slate-200 hover:border-rose-200"}`
    },
    /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between px-4 py-2 bg-slate-50 border-b border-slate-100" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2" }, layoutMode === "comic" && !focusMode && /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        draggable: "true",
        ...
        style: { cursor: "grab" },
        "aria-label": ta("a11y.storyforge_aria_reorder_panel_use_arrow_keys_home_or").replace("{0}", idx + 1),
        title: ta("a11y.storyforge_attr_drag_to_reorder_panel"),
        "data-sf-panel-drag-handle": p.id,
        "data-sf-focusable": true
      },
      /* @__PURE__ */ React.createElement(Move, { size: 15, "aria-hidden": "true" })
    ), /* @__PURE__ */ React.createElement("span", { className: "text-xs font-bold text-slate-600" }, layoutMode === "comic" ? "Panel" : "Scene", " ", idx + 1), /* @__PURE__ */ React.createElement("div", { className: "flex gap-0.5" }, /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => moveParagraph(idx, -1), disabled: idx === 0, className: "text-slate-500 hover:text-slate-700 disabled:opacity-20 p-0.5 rounded text-[11px] font-bold transition-colors", "aria-label": ta(layoutMode === "comic" ? "a11y.storyforge_aria_move_panel_earlier" : "a11y.storyforge_aria_move_scene_earlier").replace("{0}", idx + 1), title: t("ui_common.move_up") }, "\u25B2"), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => moveParagraph(idx, 1), disabled: idx === paragraphs.length - 1, className: "text-slate-500 hover:text-slate-700 disabled:opacity-20 p-0.5 rounded text-[11px] font-bold transition-colors", "aria-label": ta(layoutMode === "comic" ? "a11y.storyforge_aria_move_panel_later" : "a11y.storyforge_aria_move_scene_later").replace("{0}", idx + 1), title: t("ui_common.move_down") }, "\u25BC"))), /* @__PURE__ */ React.createElement("div", { className: "flex gap-2" }, /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
      ...
      " Help Me"
    ), paragraphs.length > 1 && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => void removeParagraph(idx), disabled: projectMutationBusy || isProcessing, className: "text-slate-500 hover:text-red-500 focus:text-red-500 p-1 rounded transition-colors", "aria-label": ta(layoutMode === "comic" ? "a11y.storyforge_aria_remove_panel" : "a11y.storyforge_aria_remove_scene").replace("{0}", idx + 1) }, /* @__PURE__ */ React.createElement(Trash2, { size: 14 })))),
    p.scaffoldFrame && /* @__PURE__ */ React.createElement("div", { className: "px-4 py-2 bg-amber-50 border-b border-amber-100 text-xs text-amber-700 italic flex items-center gap-2" }, /* @__PURE__ */ React.createElement(HelpCircle, { size: 12, className: "shrink-0" }), " ", p.scaffoldFrame),
    genre !== "free" && /* @__PURE__ */ React.createElement("div", { className: "px-4 py-2 bg-indigo-50/60 border-b border-indigo-100 flex items-center gap-2" }, /* @__PURE__ */ React.createElement("label", { htmlFor: `sf-beat-${p.id}`, className: "text-[11px] font-bold text-indigo-700 uppercase tracking-widest shrink-0" }, "\u{1F4CD} Plot Beat"), /* @__PURE__ */ React.createElement(
      "select",
      {
        id: `sf-beat-${p.id}`,
        value: p.plotBeat || "",
      ...
      PLOT_BEATS.map((b) => /* @__PURE__ */ React.createElement("option", { key: b.value || "none", value: b.value }, beatLabel(b.value)))
    ), p.plotBeat && /* @__PURE__ */ React.createElement("span", { className: "text-[11px] text-indigo-600 italic" }, "tagged")),
    helpMeParagraphIdx === idx && helpMeResult && /* @__PURE__ */ React.createElement("div", { className: "px-4 py-3 bg-gradient-to-r from-amber-50 to-yellow-50 border-b border-amber-100" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-amber-600 uppercase tracking-widest mb-2 flex items-center gap-1" }, /* @__PURE__ */ React.createElement(Sparkles, { size: 10 }), " ", ta("a11y.storyforge_ui_writing_coach_suggestions")), /* @__PURE__ */ React.createElement("div", { className: "space-y-1.5" }, helpMeResult.map((s, si) => /* @__PURE__ */ React.createElement("div", { key: si, className: "text-xs text-amber-800 flex items-start gap-2" }, /* @__PURE__ */ React.createElement("span", { className: "text-amber-400 mt-0.5" }, "\u{1F4A1}"), /* @__PURE__ */ React.createElement("span", null, s)))), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => {
      setHelpMeParagraphIdx(-1);
      setHelpMeResult(null);
    }, className: "mt-2 text-[11px] text-amber-500 hover:text-amber-700 font-bold" }, t("ui_common.dismiss"))),
    layoutMode === "comic" ? (
      ...
      /* @__PURE__ */ React.createElement("div", { className: "p-3 space-y-2 bg-gradient-to-b from-slate-50 to-white" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-2" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-black text-slate-700 uppercase tracking-widest" }, "Panel ", idx + 1, " Bubbles"), onCallGemini && /* @__PURE__ */ React.createElement(
        ...
        /* @__PURE__ */ React.createElement(Sparkles, { size: 10 }),
        " Draft"
      )), /* @__PURE__ */ React.createElement("div", { className: "rounded-lg border border-slate-200 bg-white p-2" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-2 mb-1" }, /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-black text-slate-600 uppercase tracking-widest" }, ta("a11y.storyforge_ui_panel_direction")), onCallGemini && /* @__PURE__ */ React.createElement(
        "button",
        {
          type: "button",
        ...
        "select",
        {
          ...
          "aria-label": ta("a11y.storyforge_aria_panel").replace("{0}", idx + 1).replace("{1}", label.toLowerCase())
        },
        options.map((opt) => /* @__PURE__ */ React.createElement("option", { key: opt.value || `${field}-none`, value: opt.value }, optLabel(field, opt.value, opt.label)))
      ))))), /* @__PURE__ */ React.createElement("div", { className: "rounded-lg border border-teal-100 bg-teal-50/40 p-2" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-2 mb-2" }, /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-black text-teal-700 uppercase tracking-widest" }, ta("a11y.storyforge_ui_thumbnail_rough")), onCallGemini && /* @__PURE__ */ React.createElement(
        "button",
        {
          type: "button",
        ...
        "input",
        {
          ...
          "aria-label": ta("a11y.storyforge_aria_panel_focal_point").replace("{0}", idx + 1)
        }
      ), /* @__PURE__ */ React.createElement(
        "select",
        {
          ...
          "aria-label": ta("a11y.storyforge_aria_panel_lettering_space").replace("{0}", idx + 1)
        },
        COMIC_LETTERING_SPACE_OPTIONS.map((opt) => /* @__PURE__ */ React.createElement("option", { key: opt.value || "lettering-space-none", value: opt.value }, optLabel("lettering", opt.value, opt.label)))
      )), /* @__PURE__ */ React.createElement(
        "textarea",
        {
          ...
          "aria-label": ta("a11y.storyforge_aria_panel_composition_rough").replace("{0}", idx + 1)
        }
      ), /* @__PURE__ */ React.createElement(
        "textarea",
        {
          ...
          "aria-label": ta("a11y.storyforge_aria_panel_thumbnail_sketch_note").replace("{0}", idx + 1)
        }
        ...
        "textarea",
        {
          ...
          "aria-label": ta("a11y.storyforge_aria_panel_narration").replace("{0}", idx + 1)
        }
      ), /* @__PURE__ */ React.createElement(
        "textarea",
        {
          ...
          "aria-label": ta("a11y.storyforge_aria_panel_accessibility_description").replace("{0}", idx + 1)
        }
        ...
        "input",
        {
          ...
          "aria-label": ta("a11y.storyforge_aria_panel_speaker_name").replace("{0}", idx + 1)
        }
      ), /* @__PURE__ */ React.createElement(
        "textarea",
        {
          ...
          "aria-label": ta("a11y.storyforge_aria_panel_speech").replace("{0}", idx + 1)
        }
        ...
        "textarea",
        {
          ...
          "aria-label": ta("a11y.storyforge_aria_panel_thought").replace("{0}", idx + 1)
        }
      )), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2" }, /* @__PURE__ */ React.createElement("label", { className: "text-[11px] font-bold text-red-500 uppercase tracking-widest" }, ta("a11y.storyforge_ui_sfx")), /* @__PURE__ */ React.createElement(
        "input",
        {
          type: "text",
          ...
          "aria-label": ta("a11y.storyforge_aria_panel_sound_effect").replace("{0}", idx + 1)
        }
      )), (() => {
        ...
        return /* @__PURE__ */ React.createElement("div", { id: `sf-lettering-${p.id}`, tabIndex: -1, "data-sf-focusable": true, className: "rounded-lg border border-slate-200 bg-white p-2 focus:ring-2 focus:ring-amber-400", "aria-label": ta("a11y.storyforge_aria_panel_lettering_budget").replace("{0}", idx + 1) }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-center justify-between gap-2 mb-1" }, /* @__PURE__ */ React.createElement("span", { className: "text-[10px] font-black text-slate-600 uppercase tracking-widest" }, ta("a11y.storyforge_ui_lettering_budget")), onCallGemini && lettering.words > COMIC_BUBBLE_WORD_WARNING && /* @__PURE__ */ React.createElement(
          "button",
          {
            type: "button",
          ...
          "Tighten"
        ), /* @__PURE__ */ React.createElement("span", { className: `text-[10px] font-black ${textColor}` }, lettering.words, "/", lettering.limit, " ", ta("a11y.storyforge_ui_words_2").replace("{0}", lettering.label))), /* @__PURE__ */ React.createElement("div", { className: "h-2 rounded-full bg-slate-100 overflow-hidden" }, /* @__PURE__ */ React.createElement("div", { className: `h-full rounded-full ${color}`, style: { width: `${pct}%` } })), lettering.words > 0 && /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-[10px] text-slate-500 leading-snug" }, lettering.detail));
      })())
    ) : (
      ...
      /* @__PURE__ */ React.createElement(
        "textarea",
        {
          ...
          onChange: (e) => updateParagraph(idx, e.target.value),
          dir: "auto",
          className: `w-full p-4 text-sm resize-none transition-colors ${layoutMode === "dark" ? "bg-slate-800 text-slate-100 placeholder:text-slate-600 focus:bg-slate-700 caret-cyan-400" : layoutMode === "journal" ? "bg-amber-50 text-amber-900 placeholder:text-amber-800 focus:bg-amber-100/50" : "focus:bg-rose-50/30"}`,
          style: {
            minHeight: "120px",
          ...
          "aria-label": ta("a11y.storyforge_aria_scene_text").replace("{0}", idx + 1)
        }
      )
    ),
    onCallGeminiVision && /* @__PURE__ */ React.createElement("div", { className: `px-4 py-1.5 border-t flex items-center gap-2 flex-wrap ${layoutMode === "dark" ? "bg-slate-900 border-slate-700" : "bg-white border-slate-100"}` }, /* @__PURE__ */ React.createElement(
      "label",
      {
        ...
        "aria-label": ta("a11y.storyforge_aria_snap_or_upload_handwriting_for_scene").replace("{0}", idx + 1)
      },
      /* @__PURE__ */ React.createElement(
        "input",
        {
      ...
      hwLoading && hwTargetParagraph === idx ? /* @__PURE__ */ React.createElement("span", { className: "animate-spin motion-reduce:animate-none" }, "\u23F3") : "\u{1F4F7}",
      ...
      "button",
      {
        ...
        "aria-label": ta(hwPenmanshipOn ? "a11y.storyforge_aria_disable_penmanship" : "a11y.storyforge_aria_enable_penmanship"),
    ...
    hwResult?.penmanship && hwTargetParagraph === idx && (() => {
      ...
      return /* @__PURE__ */ React.createElement("div", { className: `px-4 py-3 border-t ${layoutMode === "dark" ? "bg-slate-800 border-slate-700" : "bg-gradient-to-r from-violet-50 to-fuchsia-50 border-violet-200"}`, role: "region", "aria-label": ta("a11y.storyforge_attr_penmanship_feedback") }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between mb-1" }, /* @__PURE__ */ React.createElement("span", { className: `text-[11px] font-bold uppercase tracking-widest ${layoutMode === "dark" ? "text-cyan-400" : "text-violet-600"}` }, ta("a11y.storyforge_ui_penmanship_feedback")), /* @__PURE__ */ React.createElement("span", { className: "text-xs font-black px-2 py-0.5 rounded-full text-white", style: { background: bandColor } }, pm.band)), /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-slate-500 mb-2" }, pm.auditorCount > 1 ? `AI estimate \xB7 ~${pm.score}/100 (likely ${pm.ci[0]}\u2013${pm.ci[1]}) \xB7 averaged across ${pm.auditorCount} reviewers \xB7 ${pm.agreement} agreement` : `AI estimate \xB7 ~${pm.score}/100 (single pass)`), /* @__PURE__ */ React.createElement("div", { className: "flex gap-2 mb-2" }, [["letterFormation", "Letters"], ["spacing", "Spacing"], ["alignment", "Alignment"], ["neatness", "Neatness"]].map(([key, label]) => /* @__PURE__ */ React.createElement("div", { key, className: "flex-1 text-center" }, /* @__PURE__ */ React.createElement("div", { className: `text-sm font-black ${(pm[key] || 0) >= 18 ? "text-green-600" : (pm[key] || 0) >= 12 ? "text-amber-600" : "text-slate-600"}` }, pm[key] || 0, /* @__PURE__ */ React.createElement("span", { className: "text-[11px] opacity-60" }, "/25")), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-500 font-bold uppercase" }, label)))), pm.strengths && /* @__PURE__ */ React.createElement("p", { className: "text-xs text-green-700 font-medium mb-1" }, "\u{1F4AA} ", pm.strengths), pm.tips && /* @__PURE__ */ React.createElement("p", { className: `text-xs font-medium ${layoutMode === "dark" ? "text-cyan-400" : "text-violet-600"}` }, "\u{1F4A1} ", pm.tips), /* @__PURE__ */ React.createElement("p", { className: "text-[10px] text-slate-500 italic mt-1" }, ta("a11y.storyforge_ui_formative_ai_feedback_to_guide_practice")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setHwResult(null), className: "text-[11px] text-slate-500 hover:text-slate-600 font-bold mt-1", "aria-label": t("a11y.dismiss_penmanship_feedback") }, t("ui_common.dismiss")));
    })(),
    p.text.length > 0 && /* @__PURE__ */ React.createElement("div", { className: `px-4 py-1.5 border-t flex flex-wrap items-center gap-3 text-[11px] font-medium ${layoutMode === "dark" ? "bg-slate-900 border-slate-700 text-slate-300" : "bg-slate-50 border-slate-100 text-slate-500"}` }, /* @__PURE__ */ React.createElement("span", null, paragraphStats[idx]?.wordCount || 0, " words"), /* @__PURE__ */ React.createElement("span", null, "\xB7"), /* @__PURE__ */ React.createElement("span", null, paragraphStats[idx]?.sentenceCount || 0, " sentences"), /* @__PURE__ */ React.createElement("span", null, "\xB7"), /* @__PURE__ */ React.createElement("span", { className: paragraphStats[idx]?.vocabUsed > 0 ? "text-green-500" : "text-slate-500" }, paragraphStats[idx]?.vocabUsed || 0, " ", ta("a11y.storyforge_ui_vocab_terms")), overusedWords.length > 0 && p.text.toLowerCase().split(/\s+/).some((w) => overusedWords.includes(w.replace(/[^a-z'-]/g, ""))) && /* @__PURE__ */ React.createElement("span", { className: "text-amber-500", title: `Overused: ${overusedWords.join(", ")}` }, ta("a11y.storyforge_ui_repeated_words")), sentenceVariety[idx] && !sentenceVariety[idx].varied && /* @__PURE__ */ React.createElement("span", { className: "text-orange-500", title: sentenceVariety[idx].issues.join("; ") }, ta("a11y.storyforge_ui_vary_sentences"))),
    vocabTerms.length > 0 && (() => {
        ...
        "button",
        {
          ...
          onClick: async () => {
            ...
            try {
              const ok = window.alloCopyText ? await window.alloCopyText(v.term) : false;
              if (!ok) throw new Error("copy unavailable");
              if (addToast) addToast(ta("a11y.storyforge_toast_copied").replace("{0}", v.term), "success");
            } catch (err) {
              console.warn("Clipboard write failed:", err);
              if (addToast) addToast(ta("a11y.storyforge_toast_couldn_t_copy_please_copy_manually").replace("{0}", v.term), "error");
            }
    ...
    'Tip: Start the next scene with "',
    suggestTransition(idx + 1),
    '"'
  )))), !focusMode && paragraphs.length < maxParagraphs && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: addParagraph, className: "w-full p-3 border-2 border-dashed border-slate-300 rounded-2xl text-slate-500 font-bold text-sm hover:border-rose-400 hover:text-rose-500 transition-colors flex items-center justify-center gap-2" }, /* @__PURE__ */ React.createElement(Plus, { size: 16 }), " ", artifactType === "comic" ? "Add Panel" : "Add Scene")), phase === "illustrate" && /* @__PURE__ */ React.createElement("div", { className: `space-y-4 ${animClass}` }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h3", { className: "text-2xl font-black text-slate-800" }, "Design your ", artifactType === "comic" ? "comic" : "story"), /* @__PURE__ */ React.createElement("p", { className: "text-slate-600 text-sm mt-1" }, layoutMode === "comic" ? "Add consistent artwork, then open the production workbench to build pages and lettering" : "Add optional artwork to any scene that benefits from a visual")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2 justify-end" }, /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
    ...
    " ",
    isProcessing ? "Generating..." : artifactType === "comic" ? "Design All Panels" : "Illustrate All Scenes"
  ))), (coverArt || coverArtLoading) && /* @__PURE__ */ React.createElement("div", { className: "bg-gradient-to-r from-purple-50 to-pink-50 border-2 border-purple-200 rounded-2xl p-4 text-center" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-purple-500 uppercase tracking-widest mb-2" }, ta("a11y.storyforge_ui_cover_art")), coverArtLoading ? /* @__PURE__ */ React.createElement("div", { className: "w-48 h-48 mx-auto bg-purple-100 rounded-xl flex items-center justify-center border-2 border-dashed border-purple-300" }, /* @__PURE__ */ React.createElement(RefreshCw, { size: 32, className: "text-purple-700 animate-spin motion-reduce:animate-none" })) : coverArt && /* @__PURE__ */ React.createElement("img", { src: coverArt, alt: "Artifact cover art", className: "max-w-xs mx-auto rounded-xl shadow-lg border-2 border-purple-200" })), layoutMode === "comic" && /* @__PURE__ */ React.createElement("div", { id: "sf-comic-continuity", tabIndex: -1, "data-sf-focusable": true, className: "bg-white rounded-2xl border-2 border-purple-100 shadow-sm p-4 focus:ring-2 focus:ring-purple-400" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-3 mb-3" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-purple-600 uppercase tracking-widest" }, ta("a11y.storyforge_ui_comic_continuity")), onCallGemini && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: draftComicContinuity, disabled: isProcessing || !paragraphs.some((p) => (p.text || p.scaffoldFrame || "").trim().length > 0), className: "px-3 py-1.5 bg-purple-100 text-purple-700 rounded-full text-[11px] font-bold hover:bg-purple-200 transition-colors disabled:opacity-50 flex items-center gap-1" }, /* @__PURE__ */ React.createElement(Sparkles, { size: 12 }), " Draft Notes")), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-3" }, [
  ...
  ].map(({ field, label, placeholder }) => /* @__PURE__ */ React.createElement("label", { key: field, className: "block" }, /* @__PURE__ */ React.createElement("span", { className: "block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1" }, label), /* @__PURE__ */ React.createElement("textarea", { value: comicContinuity[field] || "", onChange: (e) => updateComicContinuity(field, e.target.value), placeholder, className: "w-full h-20 p-2 text-xs rounded-lg border border-purple-100 bg-purple-50/40 text-slate-700 focus:border-purple-400 resize-none", "aria-label": ta("a11y.storyforge_aria_comic_continuity").replace("{0}", label.toLowerCase()) })))), /* @__PURE__ */ React.createElement("div", { className: "mt-4 border-t border-purple-100 pt-4", "data-sf-cast-references": true }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-2 mb-2" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-black text-slate-500 uppercase tracking-widest" }, ta("a11y.storyforge_ui_cast_references")), /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-slate-500 mt-1" }, ta("a11y.storyforge_ui_lock_recurring_appearance_wardrobe_props_and"))), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: addContinuityReference, className: "px-3 py-1.5 rounded-lg border border-purple-200 bg-purple-50 text-[11px] font-bold text-purple-700 hover:bg-purple-100", "aria-label": ta("a11y.storyforge_attr_add_cast_reference") }, /* @__PURE__ */ React.createElement(Plus, { size: 12, "aria-hidden": "true" }), " ", ta("a11y.storyforge_ui_add_cast"))), comicContinuity.references.length === 0 ? /* @__PURE__ */ React.createElement("div", { className: "rounded-lg border border-dashed border-purple-200 bg-purple-50/30 p-3 text-[11px] text-slate-500" }, ta("a11y.storyforge_ui_add_a_cast_reference_when_a")) : /* @__PURE__ */ React.createElement("div", { className: "space-y-3" }, comicContinuity.references.map((reference, referenceIndex) => /* @__PURE__ */ React.createElement("div", { key: reference.id, className: "rounded-xl border border-purple-100 bg-purple-50/30 p-3" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-2 mb-2" }, /* @__PURE__ */ React.createElement("span", { className: "text-[10px] font-black text-purple-700 uppercase tracking-widest" }, ta("a11y.storyforge_ui_cast").replace("{0}", referenceIndex + 1)), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => removeContinuityReference(reference.id), className: "p-1 rounded-md text-rose-800 hover:bg-rose-50", "aria-label": ta("a11y.storyforge_aria_remove_cast_reference").replace("{0}", referenceIndex + 1), title: ta("a11y.storyforge_attr_remove_cast_reference") }, /* @__PURE__ */ React.createElement(Trash2, { size: 13, "aria-hidden": "true" }))), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-2" }, [["name", "Name", "Mina"], ["aliases", "Also known as", "Min, Captain M"], ["role", "Role", "Explorer"], ["appearance", "Appearance", "Round glasses, warm brown skin, short curls"], ["wardrobe", "Wardrobe", "Red jacket with a silver compass"], ["props", "Props", "Brass compass and canvas satchel"]].map(([field, label, placeholder]) => /* @__PURE__ */ React.createElement("label", { key: field, className: field === "appearance" || field === "wardrobe" || field === "props" ? "sm:col-span-2" : "" }, /* @__PURE__ */ React.createElement("span", { className: "block text-[10px] font-bold text-slate-500 mb-1" }, label), field === "name" || field === "aliases" || field === "role" ? /* @__PURE__ */ React.createElement("input", { value: reference[field] || "", onChange: (e) => updateContinuityReference(reference.id, field, e.target.value), placeholder, className: "w-full px-2 py-1.5 text-xs rounded-md border border-purple-100 bg-white text-slate-700", "aria-label": ta("a11y.storyforge_aria_cast_reference").replace("{0}", label.toLowerCase()) }) : /* @__PURE__ */ React.createElement("textarea", { value: reference[field] || "", onChange: (e) => updateContinuityReference(reference.id, field, e.target.value), placeholder, className: "w-full h-14 px-2 py-1.5 text-xs rounded-md border border-purple-100 bg-white text-slate-700 resize-y", "aria-label": ta("a11y.storyforge_aria_cast_reference").replace("{0}", label.toLowerCase()) }))), /* @__PURE__ */ React.createElement("div", { className: "sm:col-span-2 rounded-lg border border-purple-100 bg-white p-2" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-end gap-2" }, /* @__PURE__ */ React.createElement("label", { className: "flex-1 min-w-[180px]" }, /* @__PURE__ */ React.createElement("span", { className: "block text-[10px] font-bold text-slate-500 mb-1" }, ta("a11y.storyforge_ui_reference_art_url")), /* @__PURE__ */ React.createElement("input", { type: "url", value: reference.imageUrl || "", onChange: (e) => updateContinuityReference(reference.id, "imageUrl", e.target.value), placeholder: "https://...", className: "w-full px-2 py-1.5 text-xs rounded-md border border-purple-100 bg-white text-slate-700", "aria-label": ta("a11y.storyforge_aria_cast_reference_art_url").replace("{0}", referenceIndex + 1) })), /* @__PURE__ */ React.createElement("label", { className: "inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border border-purple-200 bg-purple-50 text-[11px] font-bold text-purple-700 hover:bg-purple-100 cursor-pointer" }, /* @__PURE__ */ React.createElement(ImageIcon, { size: 12, "aria-hidden": "true" }), " Upload art", /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", className: "sr-only", onChange: (e) => handleContinuityReferenceImage(reference.id, e), "aria-label": ta("a11y.storyforge_aria_upload_cast_reference_art").replace("{0}", referenceIndex + 1) })), reference.imageUrl && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => updateContinuityReference(reference.id, "imageUrl", ""), className: "inline-flex items-center gap-1 px-2.5 py-1.5 rounded-md border border-rose-200 bg-rose-50 text-[11px] font-bold text-rose-700 hover:bg-rose-100", "aria-label": ta("a11y.storyforge_aria_remove_cast_reference_art").replace("{0}", referenceIndex + 1) }, /* @__PURE__ */ React.createElement(X, { size: 12, "aria-hidden": "true" }), " Remove art")), safeImageUrl(reference.imageUrl) && /* @__PURE__ */ React.createElement("img", { src: safeImageUrl(reference.imageUrl), alt: `Reference art for ${reference.name || "cast " + (referenceIndex + 1)}`, className: "mt-2 h-20 w-20 rounded-md border border-purple-100 object-cover" })))))))), layoutMode === "comic" && comicContinuityAudit && /* @__PURE__ */ React.createElement("div", { "data-sf-continuity-audit": true, className: "mt-4 bg-white rounded-2xl border-2 border-cyan-100 shadow-sm p-4" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-black text-cyan-700 uppercase tracking-widest" }, ta("a11y.storyforge_ui_continuity_audit")), /* @__PURE__ */ React.createElement("div", { className: "text-xs text-slate-500 mt-1" }, comicContinuityAudit.referenceCount ? comicContinuityAudit.usedReferenceCount + "/" + comicContinuityAudit.referenceCount + " cast references surfaced" : "Add cast references to check speaker continuity", comicContinuityAudit.issueCount ? " - " + comicContinuityAudit.issueCount + " review note" + (comicContinuityAudit.issueCount === 1 ? "" : "s") : "")), /* @__PURE__ */ React.createElement("span", { className: "self-start sm:self-auto rounded-full border px-2.5 py-1 text-[10px] font-black uppercase " + (comicContinuityAudit.status === "Clear" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700") }, comicContinuityAudit.status)), /* @__PURE__ */ React.createElement("p", { className: "mt-3 text-[11px] leading-relaxed text-slate-600" }, comicContinuityAudit.summary), comicContinuityAudit.rows.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2" }, comicContinuityAudit.rows.map((row) => {
    ...
    return /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-2 text-[11px] font-black text-cyan-900" }, /* @__PURE__ */ React.createElement("span", null, row.label), target && /* @__PURE__ */ React.createElement("span", { className: "shrink-0 text-cyan-700" }, ta("a11y.storyforge_ui_panel").replace("{0}", target.number))),
      /* @__PURE__ */ React.createElement("div", { className: "mt-1 text-[10px] leading-relaxed text-cyan-800" }, row.detail)
    );
  }))), layoutMode === "comic" && /* @__PURE__ */ React.createElement(
    "details",
    {
      id: "sf-comic-production-workbench",
      "data-sf-comic-production-workbench": true,
      "data-sf-focusable": true,
      tabIndex: -1,
    ...
    /* @__PURE__ */ React.createElement("summary", { id: "sf-comic-production-summary", "data-sf-focusable": true, className: "cursor-pointer list-none rounded-xl focus-visible:ring-2 focus-visible:ring-fuchsia-500 focus-visible:ring-offset-2 [&::-webkit-details-marker]:hidden" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2 text-sm font-black text-fuchsia-900" }, /* @__PURE__ */ React.createElement(Move, { size: 16, "aria-hidden": "true" }), " ", ta("a11y.storyforge_ui_comic_production_workbench")), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-xs text-fuchsia-800" }, ta("a11y.storyforge_ui_build_pages_set_print_safety_reorder"))), /* @__PURE__ */ React.createElement("span", { className: "self-start rounded-full border border-fuchsia-200 bg-white px-2.5 py-1 text-[10px] font-black uppercase text-fuchsia-700 sm:self-auto" }, comicPageGroups.length, " page", comicPageGroups.length === 1 ? "" : "s", " \xB7 ", Object.keys(sanitizePanelLayouts(panelLayouts)).length, " ", ta("a11y.storyforge_ui_custom_layout"), Object.keys(sanitizePanelLayouts(panelLayouts)).length === 1 ? "" : "s"))),
    /* @__PURE__ */ React.createElement("div", { className: "mt-4 space-y-4", "data-sf-comic-production-controls": true }, layoutMode === "comic" && /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap justify-center gap-2 mb-4" }, Object.entries(COMIC_PAGE_LAYOUTS).map(([key, item]) => /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        key,
      ...
      item.label
    ))), layoutMode === "comic" && /* @__PURE__ */ React.createElement("div", { id: "sf-comic-page-composer", "data-sf-comic-page-composer": true, tabIndex: -1, "data-sf-focusable": true, className: "sf-comic-tool-card bg-white border-2 border-blue-100 rounded-2xl p-4 shadow-sm focus:ring-2 focus:ring-blue-400" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-black text-blue-700 uppercase tracking-widest flex items-center gap-2" }, /* @__PURE__ */ React.createElement(BookOpen, { size: 14 }), " Page Composer"), /* @__PURE__ */ React.createElement("div", { className: "text-xs text-slate-500 mt-1" }, comicPageGroups.length, " page", comicPageGroups.length === 1 ? "" : "s", " \xB7 ", paragraphs.length, " panel", paragraphs.length === 1 ? "" : "s")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-center gap-2" }, /* @__PURE__ */ React.createElement("span", { className: "text-[11px] font-bold text-slate-500 uppercase tracking-widest" }, ta("a11y.storyforge_ui_panels_page")), COMIC_PANELS_PER_PAGE_OPTIONS.map((value) => /* @__PURE__ */ React.createElement(
      "button",
      {
        key: value,
        type: "button",
      ...
      value
    )), /* @__PURE__ */ React.createElement("select", { value: "", onChange: (e) => applyComicPagePreset(e.target.value), className: "px-3 py-1.5 rounded-full text-[11px] font-black border border-blue-200 bg-white text-blue-700", "aria-label": ta("a11y.storyforge_attr_comic_page_preset") }, /* @__PURE__ */ React.createElement("option", { value: "" }, ta("a11y.storyforge_ui_page_preset")), Object.entries(COMIC_PAGE_PRESETS).map(([key, preset]) => /* @__PURE__ */ React.createElement("option", { key, value: key }, optLabel("pagepreset", key, preset.label)))), "                      ", /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
      ...
      return /* @__PURE__ */ React.createElement("div", { key: page.page, className: "sf-comic-page-row border border-blue-100 rounded-lg p-3 bg-blue-50/40" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-center justify-between gap-2 mb-2" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "font-black text-slate-800 text-sm" }, ta("a11y.storyforge_ui_page").replace("{0}", page.page)), /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-bold text-slate-500" }, ta("a11y.storyforge_ui_panels"), " ", page.startPanel, "-", page.endPanel)), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-center gap-1.5" }, /* @__PURE__ */ React.createElement("div", { className: `text-[11px] font-black px-2 py-1 rounded-full border ${pageStats.status === "Review" ? "bg-rose-50 border-rose-200 text-rose-700" : pageStats.status === "Ready" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-blue-50 border-blue-200 text-blue-700"}` }, pageStats.status), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded-full" }, ta("a11y.storyforge_ui_art"), " ", pageStats.artPanels, "/", pageStats.total), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-fuchsia-700 bg-white border border-fuchsia-100 px-2 py-1 rounded-full" }, ta("a11y.storyforge_ui_lettering"), " ", pageStats.placedBubbles, "/", pageStats.bubblePanels), pageStats.attention > 0 && /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-rose-700 bg-rose-50 border border-rose-100 px-2 py-1 rounded-full" }, pageStats.attention, " review"))), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-2" }, /* @__PURE__ */ React.createElement(
        "select",
        {
          ...
          "aria-label": ta("a11y.storyforge_aria_page_layout").replace("{0}", page.page)
        },
        /* @__PURE__ */ React.createElement("option", { value: "" }, ta("a11y.storyforge_ui_use_global"), getComicPageLayoutLabel(comicPageLayout), ")"),
        Object.entries(COMIC_PAGE_LAYOUTS).map(([key, item]) => /* @__PURE__ */ React.createElement("option", { key, value: key }, optLabel("pagelayout", key, item.label)))
      ), /* @__PURE__ */ React.createElement(
        "select",
        {
          value: pageMeta.turn || "",
          ...
          className: "px-3 py-2 rounded-lg border border-blue-100 bg-white text-xs font-bold text-slate-700 focus:border-blue-400",
          "aria-label": ta("a11y.storyforge_aria_page_turn").replace("{0}", page.page)
        },
        COMIC_PAGE_TURN_OPTIONS.map((option) => /* @__PURE__ */ React.createElement("option", { key: option.value || "unset", value: option.value }, optLabel("pageturn", option.value, option.label)))
      ), /* @__PURE__ */ React.createElement(
        "input",
        {
          value: pageMeta.note || "",
          ...
          "aria-label": ta("a11y.storyforge_aria_page_note").replace("{0}", page.page)
        }
      ...
      return /* @__PURE__ */ React.createElement("div", { id: "sf-comic-print-safety", "data-sf-comic-print-safety": true, tabIndex: -1, "data-sf-focusable": true, className: "sf-comic-tool-card bg-white border-2 border-emerald-100 rounded-2xl p-4 shadow-sm focus:ring-2 focus:ring-emerald-400" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-black text-emerald-700 uppercase tracking-widest flex items-center gap-2" }, /* @__PURE__ */ React.createElement(CheckCircle2, { size: 14 }), " Print Safety"), /* @__PURE__ */ React.createElement("div", { className: "text-xs text-slate-500 mt-1" }, COMIC_PRINT_FORMATS[printSafety.format]?.trim || "Screen", " \xB7 ", COMIC_PRINT_FORMATS[printSafety.format]?.safe || "Safe area")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-center gap-2" }, /* @__PURE__ */ React.createElement(
      ...
      ))), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-3" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1" }, ta("a11y.storyforge_ui_format")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2" }, Object.entries(COMIC_PRINT_FORMATS).map(([key, item]) => /* @__PURE__ */ React.createElement(
        "button",
        {
          key,
          type: "button",
          ...
          title: `${item.trim} \xB7 ${item.safe}`
        },
        item.label
      )))), /* @__PURE__ */ React.createElement("label", { className: "block" }, /* @__PURE__ */ React.createElement("span", { className: "text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 block" }, ta("a11y.storyforge_ui_gutter")), /* @__PURE__ */ React.createElement(
        "select",
        {
        ...
        Object.entries(COMIC_PRINT_GUTTERS).map(([key, item]) => /* @__PURE__ */ React.createElement("option", { key, value: key }, optLabel("gutter", key, item.label), " ", item.width !== "none" ? `(${item.width})` : ""))
      ))));
    })(), layoutMode === "comic" && /* @__PURE__ */ React.createElement("div", { id: "sf-comic-layout-studio", "data-sf-comic-layout-studio": true, tabIndex: -1, "data-sf-focusable": true, className: "sf-comic-tool-card bg-white border-2 border-fuchsia-100 rounded-2xl p-4 shadow-sm focus:ring-2 focus:ring-fuchsia-400" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-black text-fuchsia-700 uppercase tracking-widest flex items-center gap-2" }, /* @__PURE__ */ React.createElement(ImageIcon, { size: 14 }), " Layout Studio"), /* @__PURE__ */ React.createElement("div", { className: "text-xs text-slate-500 mt-1" }, Object.keys(sanitizePanelLayouts(panelLayouts)).length, " ", ta("a11y.storyforge_ui_custom_layout_2").replace("{0}", Object.keys(sanitizePanelLayouts(panelLayouts)).length === 1 ? "" : "s"))), /* @__PURE__ */ React.createElement("div", { className: "sf-comic-toolbar flex items-center gap-1.5 self-start", role: "toolbar", "aria-label": ta("a11y.storyforge_attr_comic_edit_history") }, /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        ...
        "data-sf-comic-undo": true,
        "data-sf-focusable": true
      },
      /* @__PURE__ */ React.createElement(Undo2, { size: 16, "aria-hidden": "true" })
    ), /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        ...
        "data-sf-comic-redo": true,
        "data-sf-focusable": true
      },
      /* @__PURE__ */ React.createElement(Redo2, { size: 16, "aria-hidden": "true" })
    ))), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-3" }, paragraphs.map((p, idx) => {
      const layoutFrame = panelLayouts[p.id] || {};
      ...
      return /* @__PURE__ */ React.createElement(
        ...
        /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-2" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "font-black text-slate-800 text-sm" }, ta("a11y.storyforge_ui_panel").replace("{0}", idx + 1)), /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-bold text-slate-500" }, ta("a11y.storyforge_ui_sequence"), " ", idx + 1, " of ", paragraphs.length)), /* @__PURE__ */ React.createElement("div", { className: "sf-comic-toolbar flex flex-wrap items-center gap-1.5", role: "toolbar", "aria-label": ta("a11y.storyforge_aria_panel_layout_actions").replace("{0}", idx + 1) }, /* @__PURE__ */ React.createElement(
          "button",
          {
            type: "button",
            draggable: "true",
          ...
          /* @__PURE__ */ React.createElement(Move, { size: 14, "aria-hidden": "true" })
        ), /* @__PURE__ */ React.createElement(
          "button",
          {
            type: "button",
            ...
            "aria-label": ta("a11y.storyforge_aria_move_panel_earlier").replace("{0}", idx + 1),
          ...
          "button",
          {
            ...
            className: "sf-comic-action px-2 py-1 rounded-md border border-fuchsia-100 bg-white text-[10px] font-black text-slate-700 hover:border-fuchsia-300 disabled:opacity-40 disabled:hover:border-fuchsia-100 flex items-center gap-1",
            "aria-label": ta("a11y.storyforge_aria_move_panel_later").replace("{0}", idx + 1),
          ...
          "button",
          {
            ...
            "aria-label": ta("a11y.storyforge_aria_duplicate_panel").replace("{0}", idx + 1),
          ...
          " Duplicate"
        ), /* @__PURE__ */ React.createElement("div", { className: "sf-comic-status-pill text-[11px] font-bold text-fuchsia-700 bg-white border border-fuchsia-100 px-2 py-1 rounded-full" }, getComicPanelFrameLabel(frame), " \xB7 ", getComicPanelSpanLabel(layoutFrame, comicPageLayout, idx)))),
        /* @__PURE__ */ React.createElement("div", { className: "sf-comic-frame-group flex flex-wrap gap-1.5 mb-2", role: "group", "aria-label": ta("a11y.storyforge_aria_panel_frame_preset").replace("{0}", idx + 1) }, COMIC_PANEL_FRAME_OPTIONS.map((option) => /* @__PURE__ */ React.createElement(
          "button",
          {
          ...
          optLabel("frame", option.value, option.label)
        ))),
        /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-2 mb-2" }, /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-bold text-slate-500" }, ta("a11y.storyforge_ui_drag_the_preview_corner_to_resize")), hasCustomSpans && /* @__PURE__ */ React.createElement(
          "button",
          {
            type: "button",
        ...
        /* @__PURE__ */ React.createElement("label", { className: "block" }, /* @__PURE__ */ React.createElement("span", { className: "text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 block" }, ta("a11y.storyforge_ui_bubble_anchor")), /* @__PURE__ */ React.createElement(
          "select",
          {
            ...
            "aria-label": ta("a11y.storyforge_aria_panel_bubble_anchor").replace("{0}", idx + 1)
          },
          COMIC_LETTERING_SPACE_OPTIONS.map((option) => /* @__PURE__ */ React.createElement("option", { key: option.value || "unset", value: option.value }, optLabel("lettering", option.value, option.label)))
        )),
        /* @__PURE__ */ React.createElement("label", { className: "mt-3 block" }, /* @__PURE__ */ React.createElement("span", { className: "flex items-center justify-between gap-2 text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1" }, /* @__PURE__ */ React.createElement("span", null, ta("a11y.storyforge_ui_bubble_width")), /* @__PURE__ */ React.createElement("span", null, hasCustomBubbleWidth ? `${customBubbleWidth}%` : "Auto")), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2" }, /* @__PURE__ */ React.createElement(
          "input",
          {
            type: "range",
            min: "28",
            ...
            "aria-label": ta("a11y.storyforge_aria_panel_bubble_width").replace("{0}", idx + 1),
          ...
          "button",
          {
            ...
            "aria-label": ta("a11y.storyforge_aria_reset_panel_bubble_width_to_auto").replace("{0}", idx + 1),
            "data-sf-focusable": true
          },
    ...
    }))), /* @__PURE__ */ React.createElement("div", { "data-sf-comic-design-preview": true, className: "sf-comic-preview-shell overflow-hidden rounded-2xl border-2 border-slate-700 bg-slate-950 text-white shadow-lg" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col gap-2 border-b border-slate-700 px-3 py-3 sm:flex-row sm:items-center sm:justify-between" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-xs font-black uppercase tracking-widest" }, ta("a11y.storyforge_ui_interactive_page_preview")), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-[11px] text-slate-300" }, ta("a11y.storyforge_ui_drag_panel_corners_and_bubble_handles"))), /* @__PURE__ */ React.createElement("div", { role: "group", className: "flex flex-wrap items-center gap-1.5", "aria-label": ta("a11y.storyforge_attr_design_preview_page_navigation") }, /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setComicPreviewPage((prev) => Math.max(1, prev - 1)), disabled: comicPreviewPage <= 1, className: "sf-page-nav-button h-9 rounded-md border border-slate-600 px-3 text-xs font-black text-white hover:bg-slate-800 disabled:opacity-40" }, ta("a11y.storyforge_ui_previous")), comicPageGroups.map((page) => /* @__PURE__ */ React.createElement(
      "button",
      {
      ...
      page.page
    )), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setComicPreviewPage((prev) => Math.min(comicPageGroups.length, prev + 1)), disabled: comicPreviewPage >= comicPageGroups.length, className: "sf-page-nav-button h-9 rounded-md border border-slate-600 px-3 text-xs font-black text-white hover:bg-slate-800 disabled:opacity-40" }, ta("a11y.storyforge_ui_next")))), focusedComicPreviewPage && /* @__PURE__ */ React.createElement("div", { className: `grid gap-3 p-3 ${focusedComicPreviewPage.layout === "strip" ? "grid-cols-1" : "grid-cols-2"}`, style: { direction: focusedComicPreviewPage.layout === "manga" ? "rtl" : "ltr" } }, focusedComicPreviewPage.panels.map(({ paragraph, idx: panelIdx }, pageIndex) => renderComicPreviewPanel(paragraph, panelIdx, focusedComicPreviewPage.layout, pageIndex, focusedComicPreviewPage, true)))))
  ), promptPreview && /* @__PURE__ */ React.createElement("div", { className: "bg-purple-50 border-2 border-purple-300 rounded-2xl p-5 shadow-lg" }, /* @__PURE__ */ React.createElement("div", { className: "text-xs font-bold text-purple-600 uppercase tracking-widest mb-2 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Eye, { size: 14 }), " Preview Image Prompt \u2014 ", layoutMode === "comic" ? "Panel" : "Scene", " ", promptPreview.idx + 1), /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-slate-600 mb-2" }, ta("a11y.storyforge_ui_edit_the_prompt_below_before_generating")), /* @__PURE__ */ React.createElement(
    "textarea",
    {
  ...
  ), /* @__PURE__ */ React.createElement("div", { className: "flex gap-2 mt-3" }, /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => confirmIllustration(), className: "px-4 py-2 bg-purple-600 text-white rounded-lg text-xs font-bold hover:bg-purple-700 transition-colors flex items-center gap-2" }, /* @__PURE__ */ React.createElement(ImageIcon, { size: 14 }), " Generate Image"), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setPromptPreview(null), className: "px-4 py-2 bg-slate-200 text-slate-600 rounded-lg text-xs font-bold hover:bg-slate-300 transition-colors" }, "Cancel"))), paragraphs.map((p, idx) => /* @__PURE__ */ React.createElement("div", { key: p.id, id: `sf-illustrate-${p.id}`, tabIndex: -1, "data-sf-focusable": true, className: "bg-white rounded-2xl border-2 border-purple-100 shadow-sm p-5 focus:ring-2 focus:ring-purple-400" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row items-start gap-4" }, /* @__PURE__ */ React.createElement("div", { className: "flex-1" }, /* @__PURE__ */ React.createElement("div", { className: "text-xs font-bold text-purple-600 mb-1" }, layoutMode === "comic" ? "Panel" : "Scene", " ", idx + 1), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-slate-700 leading-relaxed" }, p.text || p.scaffoldFrame || /* @__PURE__ */ React.createElement("span", { className: "italic text-slate-500" }, ta("a11y.storyforge_ui_no_draft_text_yet"))), illustrations[p.id]?.prompt && !illustrations[p.id]?.isLoading && /* @__PURE__ */ React.createElement("div", { className: "mt-2 text-[11px] text-purple-700 italic truncate", title: illustrations[p.id].prompt }, "Prompt: ", illustrations[p.id].prompt.substring(0, 80), "..."), layoutMode === "comic" && onCallGemini && /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
  ...
  }, disabled: !imageEditState.prompt.trim(), className: "flex-1 text-[11px] font-bold bg-teal-700 text-white rounded py-1 hover:bg-teal-800 disabled:opacity-40" }, t("ui_common.apply")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setImageEditState(null), className: "text-[11px] font-bold bg-slate-200 text-slate-600 rounded py-1 px-2 hover:bg-slate-300" }, t("ui_common.cancel")))), /* @__PURE__ */ React.createElement("label", { className: "mt-2 block rounded-lg border border-purple-100 bg-purple-50/70 px-2 py-1.5 text-[10px] font-bold text-purple-700" }, /* @__PURE__ */ React.createElement("span", { className: "flex items-center justify-between" }, /* @__PURE__ */ React.createElement("span", null, ta("a11y.storyforge_ui_image_size")), /* @__PURE__ */ React.createElement("span", null, clampStoryIllustrationWidth(illustrations[p.id]?.displayWidth), "px")), /* @__PURE__ */ React.createElement("input", { type: "range", min: 240, max: 720, step: 10, value: clampStoryIllustrationWidth(illustrations[p.id]?.displayWidth), onChange: (e) => updateIllustrationDisplayWidth(p.id, e.target.value), className: "mt-1 w-full accent-purple-600", "aria-label": "Image width for " + (artifactType === "comic" ? "panel " : "scene ") + (idx + 1) }))) : illustrations[p.id]?.error ? /* @__PURE__ */ React.createElement("div", { className: "w-full h-28 bg-red-50 rounded-xl flex flex-col items-center justify-center border-2 border-dashed border-red-200 gap-1" }, /* @__PURE__ */ React.createElement("span", { className: "text-red-600 text-lg" }, "\u26A0\uFE0F"), /* @__PURE__ */ React.createElement("span", { className: "text-[11px] font-bold text-red-500" }, t("ui_common.generation_failed")), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      onClick: () => {
  ...
  ))))))), phase === "narrate" && /* @__PURE__ */ React.createElement("div", { className: `space-y-4 ${animClass}` }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h3", { className: "text-2xl font-black text-slate-800" }, "Add audio to your ", artifactType === "comic" ? "comic" : "story"), /* @__PURE__ */ React.createElement("p", { className: "text-slate-600 text-sm mt-1" }, artifactType === "comic" ? "AI reads each panel caption aloud \u2014 or record your own voice" : "AI reads each scene aloud \u2014 or record your own voice")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2 items-center" }, /* @__PURE__ */ React.createElement("button", { type: "button", onClick: narrateAll, disabled: isProcessing, className: "px-4 py-2 bg-indigo-600 text-white rounded-full text-xs font-bold hover:bg-indigo-700 transition-colors disabled:opacity-50 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Volume2, { size: 14 }), " ", isProcessing ? "Narrating..." : artifactType === "comic" ? "Narrate All Captions" : "Narrate All Scenes"), isNarrating && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: stopStoryNarration, className: "px-4 py-2 bg-amber-700 text-white rounded-full text-xs font-bold hover:bg-amber-800 transition-colors flex items-center gap-2" }, /* @__PURE__ */ React.createElement(X, { size: 14 }), " Stop narration"), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      onClick: () => {
  ...
  ))), /* @__PURE__ */ React.createElement("details", { "data-sf-narration-settings": true, className: "rounded-2xl border border-indigo-200 bg-indigo-50/70 p-4" }, /* @__PURE__ */ React.createElement("summary", { className: "cursor-pointer rounded-lg text-sm font-black text-indigo-800 focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2" }, "Narration settings ", /* @__PURE__ */ React.createElement("span", { className: "ml-1 font-medium text-indigo-600" }, ta("a11y.storyforge_ui_voice_and_character_voices"))), /* @__PURE__ */ React.createElement("div", { className: "mt-4 space-y-3" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-center gap-3" }, /* @__PURE__ */ React.createElement("label", { htmlFor: "sf-voice", className: "text-[11px] font-bold text-indigo-600 uppercase" }, ta("a11y.storyforge_ui_narrator_voice")), /* @__PURE__ */ React.createElement(
    "select",
    {
      id: "sf-voice",
      value: narratorVoice,
  ...
  ), characters.length === 0 && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: detectCharacters, disabled: isProcessing, className: "w-full sm:w-auto justify-center px-4 py-2 bg-white text-indigo-700 border border-indigo-200 rounded-full text-xs font-bold hover:bg-indigo-100 transition-colors disabled:opacity-50 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Eye, { size: 14 }), " Detect character voices")), characters.length > 0 && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-indigo-500 uppercase tracking-widest mb-2" }, ta("a11y.storyforge_ui_character_voices")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-3" }, characters.map((c, i) => /* @__PURE__ */ React.createElement("div", { key: i, className: "bg-white border border-indigo-200 rounded-xl px-3 py-2 text-xs" }, /* @__PURE__ */ React.createElement("span", { className: "font-bold text-indigo-800" }, c.name), /* @__PURE__ */ React.createElement("span", { className: "text-slate-500 ml-2" }, ta("a11y.storyforge_ui_voice").replace("{0}", c.voice)))))))), recordingError && /* @__PURE__ */ React.createElement("div", { "data-sf-microphone-error": true, role: "alert", className: "rounded-2xl border-2 border-rose-200 bg-rose-50 p-4 text-sm text-rose-900" }, /* @__PURE__ */ React.createElement("div", { className: "font-black" }, ta("a11y.storyforge_ui_microphone_unavailable")), /* @__PURE__ */ React.createElement("p", { className: "mt-1" }, recordingError.message, " ", ta("a11y.storyforge_ui_you_can_also_use_ai_narration")), /* @__PURE__ */ React.createElement("div", { className: "mt-3 flex flex-wrap gap-2" }, recordingError.paragraphId && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => startRecordingParagraph(recordingError.paragraphId), className: "rounded-full bg-rose-600 px-4 py-2 text-xs font-bold text-white hover:bg-rose-700" }, "Retry microphone"), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => {
    setRecordingError(null);
    recorder.clearError();
  }, className: "rounded-full border border-rose-300 bg-white px-4 py-2 text-xs font-bold text-rose-700 hover:bg-rose-100" }, "Dismiss"))), paragraphs.map((p, idx) => {
    ...
    return /* @__PURE__ */ React.createElement("div", { key: p.id, className: `bg-white rounded-2xl border-2 shadow-sm p-4 sm:p-5 transition-colors ${isCurrentPlayback ? "border-green-400 bg-green-50/30" : "border-indigo-100"}` }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-3" }, /* @__PURE__ */ React.createElement("span", { className: "text-xs font-bold text-indigo-600" }, artifactType === "comic" ? "Panel" : "Scene", " ", idx + 1, " ", isCurrentPlayback && "\u25B6 Playing"), /* @__PURE__ */ React.createElement("div", { role: "group", className: "flex w-full flex-wrap items-center gap-2 rounded-xl bg-indigo-50 px-3 py-2 sm:w-auto", "aria-label": ta("a11y.storyforge_attr_narration_and_voice_practice_actions") }, /* @__PURE__ */ React.createElement("span", { className: "w-full text-[10px] font-black uppercase tracking-wider text-indigo-500" }, ta("a11y.storyforge_ui_create_narration")), hasSentenceAudio ? /* @__PURE__ */ React.createElement("span", { className: "text-xs text-green-600 font-bold flex items-center gap-1" }, /* @__PURE__ */ React.createElement(CheckCircle2, { size: 12 }), " AI Narrated (", seg.sentenceAudios.filter(Boolean).length, " sentences)") : seg?.aiLoading ? /* @__PURE__ */ React.createElement("span", { className: "text-xs text-indigo-400 flex items-center gap-1" }, /* @__PURE__ */ React.createElement(RefreshCw, { size: 12, className: "animate-spin motion-reduce:animate-none" }), " ", ta("a11y.storyforge_ui_generating")) : /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => narrateParagraph(p.id, p.text), className: "text-xs font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-1" }, /* @__PURE__ */ React.createElement(Volume2, { size: 12 }), " Narrate"), hasSentenceAudio && /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        onClick: () => {
    ...
    ), /* @__PURE__ */ React.createElement("span", { className: "mt-1 w-full border-t border-indigo-200 pt-2 text-[10px] font-black uppercase tracking-wider text-rose-500" }, ta("a11y.storyforge_ui_practice_or_record_my_voice")), /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
    ...
    })), seg?.studentAudioUrl && /* @__PURE__ */ React.createElement("div", { className: "mt-2" }, /* @__PURE__ */ React.createElement("audio", { controls: true, src: seg.studentAudioUrl, className: "w-full h-8" })), fluencyResult && fluencyResult.paragraphId === p.id && /* @__PURE__ */ React.createElement("div", { className: "mt-3 bg-gradient-to-r from-teal-50 to-emerald-50 border border-teal-200 rounded-xl p-4" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-3 mb-2" }, /* @__PURE__ */ React.createElement("div", { className: "text-center" }, /* @__PURE__ */ React.createElement("div", { className: `text-2xl font-black ${fluencyResult.accuracy >= 90 ? "text-green-600" : fluencyResult.accuracy >= 70 ? "text-amber-600" : "text-red-600"}` }, fluencyResult.accuracy || 0, "%"), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-600 font-bold" }, ta("a11y.storyforge_ui_accuracy"))), /* @__PURE__ */ React.createElement("div", { className: "text-center" }, /* @__PURE__ */ React.createElement("div", { className: "text-2xl font-black text-indigo-600" }, fluencyResult.wcpm || 0), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-600 font-bold" }, "WCPM")), fluencyResult.confidence && /* @__PURE__ */ React.createElement("div", { className: "text-center" }, /* @__PURE__ */ React.createElement("div", { className: `text-2xl font-black ${fluencyResult.confidence.overall >= 7 ? "text-green-600" : fluencyResult.confidence.overall >= 4 ? "text-amber-600" : "text-red-600"}` }, fluencyResult.confidence.overall, "/10"), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-600 font-bold" }, ta("a11y.storyforge_ui_confidence"))), fluencyResult.prosody && /* @__PURE__ */ React.createElement("div", { className: "flex gap-2 ml-auto" }, [{ k: "pacing", l: "Pace" }, { k: "expression", l: "Expr" }, { k: "phrasing", l: "Phrase" }].map(({ k, l }) => /* @__PURE__ */ React.createElement("div", { key: k, className: "text-center" }, /* @__PURE__ */ React.createElement("div", { className: "text-sm font-bold text-slate-700" }, fluencyResult.prosody[k], "/5"), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-500" }, l))))), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-500 italic mb-1" }, ta("a11y.storyforge_ui_ai_estimate_from_one_read_aloud")), fluencyResult.wordData && /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-1 mt-2" }, fluencyResult.wordData.map((w, wi) => /* @__PURE__ */ React.createElement(
      "span",
      {
        key: wi,
        title: w.said ? `Said: "${w.said}"${w.lowConfidence ? " (\u26A0 uncertain)" : ""}` : w.lowConfidence ? "\u26A0 AI uncertain" : "",
        className: `px-1 py-0.5 rounded text-xs font-medium ${w.lowConfidence ? "ring-1 ring-amber-400 " : ""}${w.status === "correct" ? "text-green-700 bg-green-100" : w.status === "missed" ? "text-white bg-red-600" : w.status === "stumbled" ? "text-amber-800 bg-amber-100" : w.status === "self_corrected" ? "text-blue-700 bg-blue-100" : w.status === "mispronounced" ? "text-red-700 bg-red-100" : "text-slate-600"}`
      },
      w.word
    ))), fluencyResult.confidence?.note && /* @__PURE__ */ React.createElement("div", { className: "mt-2 text-[11px] text-slate-600 italic" }, fluencyResult.confidence.note), fluencyResult.confidence?.accentDetected && /* @__PURE__ */ React.createElement("div", { className: "mt-1 text-[11px] text-teal-600 font-medium" }, ta("a11y.storyforge_ui_accent_patterns_detected_scores_adjusted_conservatively")), fluencyResult.feedback && /* @__PURE__ */ React.createElement("div", { className: "mt-2 text-xs text-teal-800 bg-white rounded-lg p-2 border border-teal-200" }, fluencyResult.feedback), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setFluencyResult(null), className: "mt-2 text-[11px] text-slate-500 hover:text-slate-600 font-bold" }, t("ui_common.dismiss"))));
  })), phase === "review" && /* @__PURE__ */ React.createElement("div", { id: "sf-review-tools", tabIndex: -1, "data-sf-focusable": true, className: `space-y-6 focus:ring-2 focus:ring-indigo-400 ${animClass}` }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h3", { className: "text-2xl font-black text-slate-800" }, t("headings.review_feedback")), /* @__PURE__ */ React.createElement("p", { className: "text-slate-600 text-sm mt-1" }, "Draft #", draftCount, " \xB7 Reflect, get feedback, and revise before visual design")), /* @__PURE__ */ React.createElement("div", { className: "flex gap-2 flex-wrap sm:justify-end" }, /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
    ...
    "button",
    {
      ...
      onClick: () => {
        ...
        sfAnnounce(ux("selfcheck_return", "You can return to the self-check at any time."));
      },
      className: "text-xs text-violet-700 hover:text-violet-800 font-bold underline shrink-0 min-h-11"
    },
    "Skip self-assessment"
  )), /* @__PURE__ */ React.createElement("div", { className: "space-y-2" }, getRubricCriteria().map((c, index) => /* @__PURE__ */ React.createElement("div", { key: c, className: "flex flex-col sm:flex-row sm:items-center gap-2 bg-white border border-violet-100 rounded-xl px-3 py-2" }, /* @__PURE__ */ React.createElement("label", { htmlFor: `sf-self-${index}`, className: "text-xs font-bold text-violet-800 flex-1 min-w-0 break-words" }, c), /* @__PURE__ */ React.createElement(
    "select",
    {
      id: `sf-self-${index}`,
      "data-sf-self-rating": true,
    ...
    /* @__PURE__ */ React.createElement("option", { value: "1" }, ux("rating_1", "1 \u2014 I need help getting started")),
    /* @__PURE__ */ React.createElement("option", { value: "2" }, ux("rating_2", "2 \u2014 I am beginning")),
    /* @__PURE__ */ React.createElement("option", { value: "3" }, ux("rating_3", "3 \u2014 I am developing this")),
    /* @__PURE__ */ React.createElement("option", { value: "4" }, ux("rating_4", "4 \u2014 I can do this well")),
    /* @__PURE__ */ React.createElement("option", { value: "5" }, ux("rating_5", "5 \u2014 I can explain my choices"))
  )))), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
  ...
  )), sensesResult && /* @__PURE__ */ React.createElement("div", { className: "bg-white border-2 border-rose-200 rounded-2xl p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between mb-3" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-rose-700 uppercase tracking-wider flex items-center gap-2" }, ta("a11y.storyforge_ui_senses_imagery")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setSensesResult(null), className: "text-[11px] text-slate-500 hover:text-slate-700 font-bold", "aria-label": t("a11y.dismiss_senses_result") }, t("ui_common.dismiss"))), (() => {
    const counts = sensesResult.counts || {};
    const max = Math.max(1, ...Object.values(counts).map((n) => Number(n) || 0));
    const SENSE_EMOJI = { sight: "\u{1F441}\uFE0F", sound: "\u{1F442}", smell: "\u{1F443}", taste: "\u{1F445}", touch: "\u270B", motion: "\u{1F3C3}", emotion: "\u{1F497}" };
    return /* @__PURE__ */ React.createElement("div", { className: "space-y-1.5" }, Object.entries(SENSE_EMOJI).map(([k, emoji]) => {
      const n = Number(counts[k]) || 0;
      const pct = n / max * 100;
      const isStrongest = sensesResult.strongest === k;
      const isMissing = sensesResult.missing === k;
      return /* @__PURE__ */ React.createElement("div", { key: k, className: "flex items-center gap-2" }, /* @__PURE__ */ React.createElement("div", { className: "text-xs font-bold text-slate-700 w-24 shrink-0" }, emoji, " ", ta("a11y.storyforge_sense_" + k)), /* @__PURE__ */ React.createElement("div", { className: "flex-1 h-4 bg-slate-100 rounded-full overflow-hidden" }, /* @__PURE__ */ React.createElement("div", { className: `h-full rounded-full transition-all ${isStrongest ? "bg-teal-500" : isMissing ? "bg-amber-400" : "bg-rose-300"}`, style: { width: `${pct}%` } })), /* @__PURE__ */ React.createElement("div", { className: "text-xs text-slate-700 font-bold w-8 text-right" }, n), isStrongest && /* @__PURE__ */ React.createElement("span", { className: "text-[10px] font-bold text-teal-700 bg-teal-100 px-1.5 py-0.5 rounded-full" }, "strongest"), isMissing && /* @__PURE__ */ React.createElement("span", { className: "text-[10px] font-bold text-amber-700 bg-amber-100 px-1.5 py-0.5 rounded-full" }, "missing"));
    }));
  })(), sensesResult.suggestion && /* @__PURE__ */ React.createElement("div", { className: "mt-3 bg-yellow-50 border border-yellow-200 rounded-xl p-3 text-xs text-yellow-900 leading-relaxed" }, /* @__PURE__ */ React.createElement("strong", null, ta("a11y.storyforge_ui_try_this")), " ", sensesResult.suggestion)), mentorMatch && /* @__PURE__ */ React.createElement("div", { role: "region", "aria-label": t("a11y.mentor_story_analysis"), className: "bg-white border-2 border-fuchsia-200 rounded-2xl p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between mb-3" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-fuchsia-700 uppercase tracking-wider flex items-center gap-2" }, ta("a11y.storyforge_ui_mentor_match")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setMentorMatch(null), className: "text-[11px] text-slate-500 hover:text-slate-700 font-bold", "aria-label": t("a11y.dismiss_mentor_match") }, t("ui_common.dismiss"))), mentorMatch.error && /* @__PURE__ */ React.createElement("p", { className: "text-xs text-red-600 italic" }, mentorMatch.error), !mentorMatch.error && /* @__PURE__ */ React.createElement("div", { className: "space-y-3" }, mentorMatch.mentor && /* @__PURE__ */ React.createElement("article", { className: "bg-fuchsia-50/40 border border-fuchsia-100 rounded-xl p-4" }, /* @__PURE__ */ React.createElement("div", { className: "mb-2" }, /* @__PURE__ */ React.createElement("h5", { className: "text-base font-black text-fuchsia-900" }, mentorMatch.mentor.title || "Untitled"), /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-slate-600 italic mt-0.5" }, "\u2014 ", mentorMatch.mentor.author || "Unknown", mentorMatch.mentor.year ? `, ${mentorMatch.mentor.year}` : "", " ", ta("a11y.storyforge_ui_public_domain"))), mentorMatch.mentor.uncertain ? /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-700 italic leading-relaxed" }, mentorMatch.mentor.text || "Excerpt withheld \u2014 open the source link to read in context.") : /* @__PURE__ */ React.createElement("pre", { className: "whitespace-pre-wrap font-serif text-sm text-slate-800 leading-relaxed bg-white border border-fuchsia-100 rounded-lg p-3" }, mentorMatch.mentor.text || ""), mentorMatch.mentor.sourceUrl && /* @__PURE__ */ React.createElement("p", { className: "text-[11px] mt-2" }, /* @__PURE__ */ React.createElement("a", { href: mentorMatch.mentor.sourceUrl, target: "_blank", rel: "noopener noreferrer", className: "text-fuchsia-700 hover:text-fuchsia-900 font-bold underline", "aria-label": ta("a11y.storyforge_aria_open_mentor_source").replace("{0}", mentorMatch.mentor.title || ta("a11y.storyforge_aria_mentor_story_fallback")) }, "Read the full story \u2197")), mentorMatch._grounding && /* @__PURE__ */ React.createElement("p", { className: "text-[10px] text-slate-500 italic mt-2" }, mentorMatch._grounding.searchUsed ? `\u2713 Verified via web search (${mentorMatch._grounding.resultCount} candidates considered, keywords: "${mentorMatch._grounding.keywords}")` : "\u26A0 No web search available \u2014 recommendation comes from the model's memory, please double-check.")), mentorMatch.sharedTheme && /* @__PURE__ */ React.createElement("div", { className: "bg-white border border-fuchsia-100 rounded-xl p-3" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-fuchsia-600 uppercase tracking-widest mb-1" }, ta("a11y.storyforge_ui_shared_theme")), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-800 leading-relaxed" }, mentorMatch.sharedTheme)), mentorMatch.craftToBorrow && /* @__PURE__ */ React.createElement("div", { className: "bg-amber-50 border border-amber-200 rounded-xl p-3" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-amber-700 uppercase tracking-widest mb-1" }, ta("a11y.storyforge_ui_craft_to_borrow")), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-amber-900 leading-relaxed" }, mentorMatch.craftToBorrow)), mentorMatch.studentEcho && /* @__PURE__ */ React.createElement("div", { className: "bg-green-50 border border-green-200 rounded-xl p-3" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-green-700 uppercase tracking-widest mb-1" }, ta("a11y.storyforge_ui_you_re_already_doing_this")), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-green-900 leading-relaxed" }, mentorMatch.studentEcho)))), showTellResult && /* @__PURE__ */ React.createElement("div", { className: "bg-white border-2 border-emerald-200 rounded-2xl p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between mb-3" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-emerald-700 uppercase tracking-wider flex items-center gap-2" }, ta("a11y.storyforge_ui_show_vs_tell")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setShowTellResult(null), className: "text-[11px] text-slate-500 hover:text-slate-700 font-bold", "aria-label": t("a11y.dismiss_show_vs_tell") }, t("ui_common.dismiss"))), showTellResult.summary && /* @__PURE__ */ React.createElement("p", { className: "text-xs text-emerald-800 italic mb-3 leading-relaxed" }, showTellResult.summary), (showTellResult.tellings || []).length === 0 ? /* @__PURE__ */ React.createElement("div", { className: "bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-xs text-emerald-900 leading-relaxed" }, "\u2728 Strong showing throughout \u2014 keep it up!") : /* @__PURE__ */ React.createElement("div", { className: "space-y-3" }, showTellResult.tellings.map((t2, i) => /* @__PURE__ */ React.createElement("div", { key: i, className: "bg-emerald-50/40 border border-emerald-100 rounded-xl p-3" }, /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-bold text-amber-700 uppercase tracking-widest mb-1" }, ta("a11y.storyforge_ui_telling")), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-slate-800 italic leading-relaxed mb-2" }, '"', t2.telling, '"'), /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-bold text-emerald-700 uppercase tracking-widest mb-1" }, ta("a11y.storyforge_ui_try_showing")), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-emerald-900 leading-relaxed" }, '"', t2.showing, '"'), t2.why && /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-slate-600 mt-2 italic" }, t2.why))))), arcReport && /* @__PURE__ */ React.createElement("div", { className: "bg-white border-2 border-sky-200 rounded-2xl p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between mb-3" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-sky-700 uppercase tracking-wider flex items-center gap-2" }, ta("a11y.storyforge_ui_character_arcs")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setArcReport(null), className: "text-[11px] text-slate-500 hover:text-slate-700 font-bold", "aria-label": t("a11y.dismiss_character_arcs") }, t("ui_common.dismiss"))), arcReport.summary && /* @__PURE__ */ React.createElement("p", { className: "text-xs text-sky-800 italic mb-3 leading-relaxed" }, arcReport.summary), (arcReport.characters || []).length === 0 ? /* @__PURE__ */ React.createElement("div", { className: "bg-sky-50 border border-sky-200 rounded-xl p-3 text-xs text-sky-900 leading-relaxed" }, "No named characters yet. If you'd like to track arcs, give your main character a name and try again.") : /* @__PURE__ */ React.createElement("div", { className: "space-y-3" }, arcReport.characters.map((c, i) => {
    const beatOrder = [
    ...
    const beatColor = (status) => status === "strong" ? "bg-green-100 border-green-300 text-green-800" : status === "partial" ? "bg-amber-100 border-amber-300 text-amber-800" : "bg-slate-100 border-slate-300 text-slate-600";
    ...
    })), c.suggestion && /* @__PURE__ */ React.createElement("div", { className: "bg-amber-50 border border-amber-200 rounded-lg p-2 text-xs text-amber-900 leading-relaxed" }, /* @__PURE__ */ React.createElement("strong", { className: "text-amber-700" }, ta("a11y.storyforge_ui_try_this")), " ", c.suggestion));
  }))), dialogueReport && /* @__PURE__ */ React.createElement("div", { className: "bg-white border-2 border-orange-200 rounded-2xl p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between mb-3" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-orange-700 uppercase tracking-wider flex items-center gap-2" }, ta("a11y.storyforge_ui_dialogue_tune_up")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setDialogueReport(null), className: "text-[11px] text-slate-500 hover:text-slate-700 font-bold", "aria-label": t("a11y.dismiss_dialogue_tuneup") }, t("ui_common.dismiss"))), dialogueReport.summary && /* @__PURE__ */ React.createElement("p", { className: "text-xs text-orange-800 italic mb-3 leading-relaxed" }, dialogueReport.summary), dialogueReport.tagCounts && Object.keys(dialogueReport.tagCounts).length > 0 && /* @__PURE__ */ React.createElement("div", { className: "bg-orange-50/40 border border-orange-100 rounded-xl p-3 mb-3" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-orange-700 uppercase tracking-widest mb-2" }, ta("a11y.storyforge_ui_tag_usage")), /* @__PURE__ */ React.createElement("div", { className: "space-y-1.5" }, (() => {
  ...
  })())), (dialogueReport.issues || []).length > 0 ? /* @__PURE__ */ React.createElement("div", { className: "space-y-2" }, dialogueReport.issues.map((iss, i) => /* @__PURE__ */ React.createElement("div", { key: i, className: "bg-orange-50/40 border border-orange-100 rounded-xl p-3" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2 mb-1" }, /* @__PURE__ */ React.createElement("span", { className: "text-[10px] font-bold text-orange-700 bg-orange-100 px-2 py-0.5 rounded-full uppercase tracking-widest" }, iss.type === "tag-swap" ? "Tag swap" : iss.type === "missing-tag" ? "Add tag" : iss.type)), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-700 italic leading-relaxed mb-1.5" }, '"', iss.line, '"'), /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-bold text-emerald-700 uppercase tracking-widest" }, ta("a11y.storyforge_ui_try")), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-emerald-900 leading-relaxed" }, iss.suggestion), iss.why && /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-slate-600 mt-1 italic" }, iss.why)))) : !dialogueReport.tagCounts || Object.keys(dialogueReport.tagCounts).length === 0 ? null : /* @__PURE__ */ React.createElement("div", { className: "bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-xs text-emerald-900 leading-relaxed" }, "\u2728 Dialogue mechanics look strong \u2014 no specific suggestions.")), comicFlowReport && layoutMode === "comic" && /* @__PURE__ */ React.createElement("div", { className: "bg-white border-2 border-blue-200 rounded-2xl p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between mb-3" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-blue-700 uppercase tracking-wider flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Eye, { size: 14 }), " ", ta("a11y.storyforge_ui_comic_flow_audit")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setComicFlowReport(null), className: "text-[11px] text-slate-500 hover:text-slate-700 font-bold", "aria-label": ta("a11y.storyforge_attr_dismiss_comic_flow_audit") }, t("ui_common.dismiss"))), /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row gap-4 mb-4" }, /* @__PURE__ */ React.createElement("div", { className: "shrink-0 w-24 h-24 rounded-2xl bg-blue-600 text-white flex flex-col items-center justify-center shadow-md" }, /* @__PURE__ */ React.createElement("div", { className: "text-3xl font-black" }, Math.round(Number(comicFlowReport.score) || 0)), /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-bold uppercase tracking-widest" }, ta("a11y.storyforge_ui_flow"))), /* @__PURE__ */ React.createElement("div", { className: "flex-1 min-w-0" }, /* @__PURE__ */ React.createElement("p", { className: "text-sm text-blue-900 leading-relaxed font-medium" }, comicFlowReport.summary), comicFlowReport.strengths && comicFlowReport.strengths.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-1.5 mt-3" }, comicFlowReport.strengths.map((s, i) => /* @__PURE__ */ React.createElement("span", { key: i, className: "text-[10px] font-bold text-green-700 bg-green-100 border border-green-200 rounded-full px-2 py-0.5" }, s))))), comicFlowReport.metrics && /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2 mb-4" }, [
  ...
  })())), /* @__PURE__ */ React.createElement("details", { "data-sf-analytics": true, className: "bg-white rounded-2xl border-2 border-slate-200 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("summary", { className: "cursor-pointer text-sm font-bold text-slate-700 uppercase tracking-wider mb-3" }, t("headings.writing_analytics")), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 sm:grid-cols-5 gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "text-center p-3 bg-slate-50 rounded-xl" }, /* @__PURE__ */ React.createElement("div", { className: "text-2xl font-black text-slate-800" }, totalWords), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-600 font-bold" }, ta("a11y.storyforge_ui_words"))), /* @__PURE__ */ React.createElement("div", { className: "text-center p-3 bg-slate-50 rounded-xl" }, /* @__PURE__ */ React.createElement("div", { className: "text-2xl font-black text-slate-800" }, authoredText.split(/[.!?]+/).filter((text) => text.trim()).length), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-600 font-bold" }, ta("a11y.storyforge_ui_sentences"))), /* @__PURE__ */ React.createElement("div", { className: "text-center p-3 bg-slate-50 rounded-xl" }, /* @__PURE__ */ React.createElement("div", { className: "text-2xl font-black text-slate-800" }, paragraphs.length), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-600 font-bold" }, artifactType === "comic" ? "Panels" : "Scenes")), /* @__PURE__ */ React.createElement("div", { className: "text-center p-3 bg-slate-50 rounded-xl" }, /* @__PURE__ */ React.createElement("div", { className: "text-2xl font-black text-slate-800" }, vocabUsedCount, "/", vocabTerms.length), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-600 font-bold" }, ta("a11y.storyforge_ui_vocab_used"))), /* @__PURE__ */ React.createElement("div", { className: "text-center p-3 bg-slate-50 rounded-xl" }, /* @__PURE__ */ React.createElement("div", { className: `text-2xl font-black ${readingLevel ? "text-indigo-600" : "text-slate-600"}` }, readingLevel ? `${readingLevel.grade}` : "\u2014"), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-600 font-bold" }, ux("reading_estimate", "Text complexity estimate")))), readingLevel && /* @__PURE__ */ React.createElement("div", { className: "mt-3 text-xs text-slate-600" }, "Avg ", readingLevel.avgWordsPerSentence, " ", ta("a11y.storyforge_ui_words_sentence_flesch_kincaid_grade_level"), " ", readingLevel.grade, /* @__PURE__ */ React.createElement("span", null, " \xB7 ", ux("estimate_note", "Approximate English text complexity, not a writing quality score."))), !readingLevel && /* @__PURE__ */ React.createElement("p", { className: "mt-2 text-xs text-slate-600" }, ux("estimate_threshold", "A complexity estimate is available for English drafts with at least 100 words.")), /* @__PURE__ */ React.createElement("div", { className: "mt-4 pt-4 border-t border-slate-100" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between mb-2" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-slate-600 uppercase tracking-widest" }, ta("a11y.storyforge_ui_narrative_arc"), " ", /* @__PURE__ */ React.createElement("span", { className: "normal-case tracking-normal text-slate-500 font-medium" }, ta("a11y.storyforge_ui_fortune_over_time"))), onCallGemini && /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      "data-sf-focusable": true,
    ...
    valenceLoading ? /* @__PURE__ */ React.createElement("span", { className: "animate-spin motion-reduce:animate-none" }, "\u23F3") : /* @__PURE__ */ React.createElement(Sparkles, { size: 12 }),
    " ",
    ...
    return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("svg", { viewBox: `0 0 ${W} ${H}`, width: "100%", height: H, role: "img", "aria-label": ta(artifactType === "comic" ? "a11y.storyforge_aria_arc_across_panels" : "a11y.storyforge_aria_arc_across_scenes"), className: "overflow-visible" }, /* @__PURE__ */ React.createElement("line", { x1: pad, y1: py(0), x2: W - pad, y2: py(0), stroke: "#e2e8f0", strokeWidth: "1", strokeDasharray: "3 3" }), /* @__PURE__ */ React.createElement("text", { x: pad, y: py(5) + 2, fontSize: "7", fill: "#94a3b8" }, ta("a11y.storyforge_ui_good")), /* @__PURE__ */ React.createElement("text", { x: pad, y: py(-5), fontSize: "7", fill: "#94a3b8" }, ta("a11y.storyforge_ui_bad")), /* @__PURE__ */ React.createElement("polyline", { fill: "none", stroke: "#7c3aed", strokeWidth: "2.5", strokeLinecap: "round", strokeLinejoin: "round", points: pts }), vals.map((v, i) => /* @__PURE__ */ React.createElement("circle", { key: i, cx: px(i), cy: py(v), r: "3", fill: "#7c3aed" }))), /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between text-[11px] text-slate-500 mt-0.5" }, /* @__PURE__ */ React.createElement("span", null, ta("a11y.storyforge_ui_beginning")), /* @__PURE__ */ React.createElement("span", null, ta("a11y.storyforge_ui_end"))), /* @__PURE__ */ React.createElement("div", { className: "mt-2 space-y-1" }, paragraphs.map((p, i) => {
      ...
      return /* @__PURE__ */ React.createElement("div", { key: p.id, className: "flex items-center gap-2" }, /* @__PURE__ */ React.createElement("span", { className: "text-[11px] text-slate-500 font-bold w-7 shrink-0" }, "P", i + 1), /* @__PURE__ */ React.createElement(
        "input",
        {
          ...
          "aria-label": ta("a11y.storyforge_aria_fortune_for_paragraph_from_5_bad_to").replace("{0}", i + 1),
          className: "flex-1 accent-violet-600"
        }
      ), /* @__PURE__ */ React.createElement("span", { className: "text-[11px] text-slate-500 w-6 text-right tabular-nums" }, v > 0 ? "+" + v : v));
    })), anySet && match ? /* @__PURE__ */ React.createElement("div", { className: "mt-2 text-[11px] text-violet-700 bg-violet-50 border border-violet-200 rounded-lg px-2 py-1.5" }, match.weak ? "Closest shape (loosely): " : "Your story looks like a ", /* @__PURE__ */ React.createElement("span", { className: "font-black" }, match.emoji, " ", match.label), match.weak ? "" : "!", " ", /* @__PURE__ */ React.createElement("span", { className: "text-slate-500 font-medium" }, ta("a11y.storyforge_ui_a_craft_lens_not_a_rule"))) : /* @__PURE__ */ React.createElement("div", { className: "mt-2 text-[11px] text-slate-500 italic" }, `Drag a point or tap "Suggest arc" to map your story's emotional ups & downs.`));
  })()), wordFrequency.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border-2 border-slate-200 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-slate-700 uppercase tracking-wider mb-3" }, t("headings.word_frequency")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2" }, wordFrequency.slice(0, 12).map(([word, count]) => /* @__PURE__ */ React.createElement("div", { key: word, className: `px-3 py-1.5 rounded-full text-xs font-bold border-2 ${count >= 4 ? "bg-amber-100 border-amber-300 text-amber-800" : "bg-slate-50 border-slate-200 text-slate-600"}`, title: `"${word}" used ${count} times` }, word, " ", /* @__PURE__ */ React.createElement("span", { className: "text-[11px]" }, "\xD7", count)))), overusedWords.length > 0 && /* @__PURE__ */ React.createElement("p", { className: "mt-2 text-[11px] text-amber-600 font-medium" }, "Tip: Try varying your word choice \u2014 ", /* @__PURE__ */ React.createElement("strong", null, overusedWords.join(", ")), " ", overusedWords.length === 1 ? "appears" : "appear", " 4+ times. Use synonyms for variety!"))), !gradingResult && !isProcessing && onCallGemini && /* @__PURE__ */ React.createElement("div", { className: "bg-slate-50 border-2 border-dashed border-slate-300 rounded-2xl p-12 text-center" }, /* @__PURE__ */ React.createElement(Star, { size: 48, className: "text-slate-600 mx-auto mb-4" }), /* @__PURE__ */ React.createElement("p", { className: "text-slate-600 font-bold" }, 'Click "Get Feedback" to receive AI-powered Glow & Grow feedback on your draft')), isProcessing && /* @__PURE__ */ React.createElement("div", { className: "bg-indigo-50 border-2 border-indigo-200 rounded-2xl p-12 text-center" }, /* @__PURE__ */ React.createElement(RefreshCw, { size: 48, className: "text-indigo-400 mx-auto mb-4 animate-spin motion-reduce:animate-none" }), /* @__PURE__ */ React.createElement("p", { className: "text-indigo-600 font-bold" }, ta("a11y.storyforge_ui_reading_your_draft_and_preparing_feedback"))), gradingResult && /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, /* @__PURE__ */ React.createElement("div", { className: "text-center" }, /* @__PURE__ */ React.createElement("div", { className: "inline-block bg-gradient-to-r from-indigo-700 to-purple-700 text-white px-8 py-3 rounded-2xl text-2xl font-black shadow-lg", title: ta("a11y.storyforge_attr_ai_generated_estimate_draft_feedback_not") }, gradingResult.totalScore), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-500 mt-1.5 font-medium" }, ta("a11y.storyforge_ui_ai_estimate_draft_feedback_not_a"))), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4" }, /* @__PURE__ */ React.createElement("div", { className: "bg-green-50 border-2 border-green-200 rounded-2xl p-5" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-green-700 uppercase tracking-wider mb-2 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(CheckCircle2, { size: 16 }), " Glow"), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-green-800 leading-relaxed" }, gradingResult.feedback?.glow)), /* @__PURE__ */ React.createElement("div", { className: "bg-amber-50 border-2 border-amber-200 rounded-2xl p-5" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-amber-700 uppercase tracking-wider mb-2 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(HelpCircle, { size: 16 }), " Grow"), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-amber-800 leading-relaxed" }, gradingResult.feedback?.grow))), gradingResult.scores && /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border-2 border-slate-200 overflow-hidden" }, /* @__PURE__ */ React.createElement("div", { className: "px-4 py-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-slate-700" }, t("headings.score_breakdown")), Object.keys(selfAssessment).length > 0 && /* @__PURE__ */ React.createElement("div", { className: "text-[11px] text-slate-500 flex items-center gap-3" }, /* @__PURE__ */ React.createElement("span", { className: "flex items-center gap-1" }, /* @__PURE__ */ React.createElement("span", { className: "inline-block w-2 h-2 rounded-full bg-violet-400" }), " ", ta("a11y.storyforge_ui_you")), /* @__PURE__ */ React.createElement("span", { className: "flex items-center gap-1" }, /* @__PURE__ */ React.createElement("span", { className: "inline-block w-2 h-2 rounded-full bg-indigo-500" }), " AI"))), /* @__PURE__ */ React.createElement("div", { className: "divide-y divide-slate-100" }, gradingResult.scores.map((s, i) => {
    const aiScoreNum = (() => {
  ...
  }))), gradingResult.vocabScores && /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border-2 border-slate-200 overflow-hidden" }, /* @__PURE__ */ React.createElement("div", { className: "px-4 py-3 bg-slate-50 border-b border-slate-200" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-slate-700" }, t("headings.vocab_usage"))), /* @__PURE__ */ React.createElement("div", { className: "p-4 flex flex-wrap gap-2" }, gradingResult.vocabScores.map((vs, i) => /* @__PURE__ */ React.createElement("div", { key: i, className: `px-3 py-1.5 rounded-full text-xs font-bold border-2 ${vs.status === "correct" ? "bg-green-100 border-green-300 text-green-800" : vs.status === "partial" ? "bg-amber-100 border-amber-300 text-amber-800" : "bg-red-100 border-red-300 text-red-800"}`, title: vs.comment }, vs.status === "correct" ? "\u2713" : vs.status === "partial" ? "~" : "\u2717", " ", vs.term)))))), phase === "export" && /* @__PURE__ */ React.createElement("div", { className: `space-y-6 ${animClass}` }, /* @__PURE__ */ React.createElement("div", { className: "text-center mb-8" }, /* @__PURE__ */ React.createElement("h3", { className: "text-2xl font-black text-slate-800" }, !isCurrentDraftReviewed ? "Review the latest draft before publishing" : publishBlocked ? "Finish required items before publishing" : projectReadiness.warnings.length ? "Your " + ARTIFACT_TYPES[artifactType].label.toLowerCase() + " is ready to export as a draft" : "Your " + ARTIFACT_TYPES[artifactType].label.toLowerCase() + " is ready to publish"), /* @__PURE__ */ React.createElement("p", { className: "text-slate-600 text-sm mt-1" }, layoutMode === "comic" ? "Review the proof and finished comic, then choose how to save it. Structural production settings stay in Design." : "Preview the finished story and choose how to share or save it."), /* @__PURE__ */ React.createElement("div", { className: "inline-flex mt-3 px-3 py-1 rounded-full bg-slate-100 text-slate-700 text-xs font-bold" }, ta("a11y.storyforge_ui_artifact_label"), " ", optLabel("artifact", artifactType, ARTIFACT_TYPES[artifactType].label), artifactType === "story" ? " \xB7 " + WRITING_VIEWS[writingView].label + " writing view" : "")), layoutMode === "comic" && /* @__PURE__ */ React.createElement("div", { "data-sf-publish-design-route": true, className: "flex flex-col gap-3 rounded-2xl border border-fuchsia-200 bg-fuchsia-50 p-4 sm:flex-row sm:items-center sm:justify-between" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-sm font-black text-fuchsia-900" }, ta("a11y.storyforge_ui_need_to_change_pages_panel_order")), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-xs text-fuchsia-800" }, ta("a11y.storyforge_ui_make_structural_changes_in_design_then"))), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      "data-sf-edit-comic-production": true,
      onClick: () => {
        setShowComicProduction(true);
    ...
    /* @__PURE__ */ React.createElement(ArrowLeft, { size: 14, "aria-hidden": "true" }),
    " Edit production in Design"
  )), /* @__PURE__ */ React.createElement("section", { className: `sf-export-preflight overflow-hidden rounded-lg border-2 bg-white shadow-sm ${projectReadiness.blockers.length ? "border-rose-200" : projectReadiness.warnings.length ? "border-amber-200" : "border-emerald-200"}`, "aria-labelledby": "sf-export-preflight-title" }, /* @__PURE__ */ React.createElement("div", { className: `px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 ${projectReadiness.blockers.length ? "bg-rose-50" : projectReadiness.warnings.length ? "bg-amber-50" : "bg-emerald-50"}` }, /* @__PURE__ */ React.createElement("div", { className: "min-w-0" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2" }, projectReadiness.blockers.length ? /* @__PURE__ */ React.createElement(HelpCircle, { size: 18, className: "text-rose-700", "aria-hidden": "true" }) : /* @__PURE__ */ React.createElement(CheckCircle2, { size: 18, className: projectReadiness.warnings.length ? "text-amber-700" : "text-emerald-700", "aria-hidden": "true" }), /* @__PURE__ */ React.createElement("h4", { id: "sf-export-preflight-title", className: "text-sm font-black text-slate-900" }, ta("a11y.storyforge_ui_export_preflight")), /* @__PURE__ */ React.createElement("span", { className: `rounded-full border px-2 py-0.5 text-[10px] font-black uppercase ${projectReadiness.blockers.length ? "border-rose-300 bg-white text-rose-700" : projectReadiness.warnings.length ? "border-amber-300 bg-white text-amber-700" : "border-emerald-300 bg-white text-emerald-700"}` }, projectReadiness.blockers.length ? `${projectReadiness.blockers.length} blocker${projectReadiness.blockers.length === 1 ? "" : "s"}` : projectReadiness.warnings.length ? `${projectReadiness.warnings.length} refinement${projectReadiness.warnings.length === 1 ? "" : "s"}` : "Ready")), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-xs font-medium text-slate-600" }, projectReadiness.summary)), /* @__PURE__ */ React.createElement("div", { role: "group", className: "grid grid-cols-2 sm:grid-cols-5 gap-x-4 gap-y-1 text-[10px] font-bold text-slate-600", "aria-label": ta("a11y.storyforge_attr_production_coverage") }, /* @__PURE__ */ React.createElement("span", null, "Words ", /* @__PURE__ */ React.createElement("strong", { className: "text-slate-900" }, projectReadiness.metrics.totalWords)), /* @__PURE__ */ React.createElement("span", null, ta("a11y.storyforge_ui_art"), " ", /* @__PURE__ */ React.createElement("strong", { className: "text-slate-900" }, projectReadiness.metrics.illustratedSections, "/", projectReadiness.metrics.totalSections)), /* @__PURE__ */ React.createElement("span", null, ta("a11y.storyforge_ui_audio"), " ", /* @__PURE__ */ React.createElement("strong", { className: "text-slate-900" }, projectReadiness.metrics.narratedSections, "/", projectReadiness.metrics.totalSections)), /* @__PURE__ */ React.createElement("span", null, ta("a11y.storyforge_ui_review"), " ", /* @__PURE__ */ React.createElement("strong", { className: "text-slate-900" }, projectReadiness.metrics.reviewSignalCount ? "Done" : "Open")), artifactType === "comic" && /* @__PURE__ */ React.createElement("span", null, "Alt ", /* @__PURE__ */ React.createElement("strong", { className: "text-slate-900" }, comicAltCoverageLabel)))), exportReadinessIssues.length > 0 ? /* @__PURE__ */ React.createElement("div", { className: "divide-y divide-slate-200" }, exportReadinessIssues.slice(0, 7).map((issue) => {
    const isBlocker = projectReadiness.blockers.some((item) => item.code === issue.code);
    return /* @__PURE__ */ React.createElement("div", { key: issue.code, className: "sf-preflight-issue px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-3" }, /* @__PURE__ */ React.createElement("span", { className: `w-2 h-2 rounded-full shrink-0 ${isBlocker ? "bg-rose-500" : "bg-amber-500"}`, "aria-hidden": "true" }), /* @__PURE__ */ React.createElement("div", { className: "min-w-0 flex-1" }, /* @__PURE__ */ React.createElement("div", { className: "text-xs font-black text-slate-800" }, issue.label), /* @__PURE__ */ React.createElement("div", { className: "mt-0.5 text-[11px] leading-relaxed text-slate-500" }, issue.detail)), /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        "data-sf-focusable": true,
      ...
      " ",
      /* @__PURE__ */ React.createElement(ArrowRight, { size: 12, "aria-hidden": "true" })
    ));
  }), exportReadinessIssues.length > 7 && /* @__PURE__ */ React.createElement("div", { className: "px-4 py-2 text-[11px] font-bold text-slate-500" }, exportReadinessIssues.length - 7, " additional refinement", exportReadinessIssues.length - 7 === 1 ? "" : "s", " tracked in the phase dashboard.")) : /* @__PURE__ */ React.createElement("div", { className: "px-4 py-4 text-sm font-bold text-emerald-800 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(CheckCircle2, { size: 18, "aria-hidden": "true" }), " Narrative structure, media, review, and production checks are clear.")), layoutMode === "comic" && comicExportProof && /* @__PURE__ */ React.createElement("div", { role: "group", "data-sf-comic-export-proof": true, className: "mt-4 border-t-2 border-blue-100 pt-4", "aria-label": ta("a11y.storyforge_attr_comic_export_proof") }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-black text-blue-700 uppercase tracking-widest flex items-center gap-2" }, comicExportProof.status === "Ready" ? /* @__PURE__ */ React.createElement(CheckCircle2, { size: 14, "aria-hidden": "true" }) : /* @__PURE__ */ React.createElement(HelpCircle, { size: 14, "aria-hidden": "true" }), " Export proof"), /* @__PURE__ */ React.createElement("div", { className: "text-xs text-slate-500 mt-1" }, comicExportProof.readyPages, "/", comicExportProof.pageCount, " pages clear \xB7 ", comicExportProof.issueCount, " proof note", comicExportProof.issueCount === 1 ? "" : "s")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-center gap-2 self-start sm:self-auto" }, /* @__PURE__ */ React.createElement("span", { className: `rounded-full border px-2.5 py-1 text-[10px] font-black uppercase ${comicExportProof.status === "Ready" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700"}` }, comicExportProof.status), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      onClick: () => {
    ...
    "Edit in Design"
  ))), comicExportProof.issueCount > 0 ? /* @__PURE__ */ React.createElement("div", { className: "mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2" }, comicExportProof.globalIssues.map((issue) => /* @__PURE__ */ React.createElement("div", { key: issue.key, className: "rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] text-amber-900" }, /* @__PURE__ */ React.createElement("div", { className: "font-black" }, issue.label), /* @__PURE__ */ React.createElement("div", { className: "mt-0.5 leading-relaxed" }, issue.detail))), comicExportProof.rows.filter((row) => row.issues.length > 0).slice(0, 6).map((row) => /* @__PURE__ */ React.createElement("div", { key: "proof-page-" + row.page, className: "rounded-lg border border-rose-200 bg-rose-50 px-3 py-2" }, /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
    ...
    /* @__PURE__ */ React.createElement("span", { className: "text-[11px] font-black text-rose-800" }, ta("a11y.storyforge_ui_page").replace("{0}", row.page)),
    /* @__PURE__ */ React.createElement("span", { className: "text-[10px] font-black text-rose-700" }, row.issues.length, " note", row.issues.length === 1 ? "" : "s")
  ), /* @__PURE__ */ React.createElement("div", { className: "mt-2 space-y-1" }, row.issues.slice(0, 3).map((issue) => {
    ...
    return /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement("span", null, issue.label),
      target && /* @__PURE__ */ React.createElement("span", { className: "shrink-0 font-black text-rose-600" }, ta("a11y.storyforge_ui_panel").replace("{0}", target.number))
    );
  }))))) : /* @__PURE__ */ React.createElement("div", { className: "mt-3 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-[11px] font-bold text-emerald-800" }, "Every page clears the current art, lettering, accessibility, pacing, and print checks.")), /* @__PURE__ */ React.createElement("div", { className: "sf-comic-preview-shell bg-gradient-to-b from-amber-50 to-white border-2 border-amber-200 rounded-2xl overflow-hidden shadow-lg" }, /* @__PURE__ */ React.createElement("div", { className: "text-center p-8 border-b border-amber-200 bg-gradient-to-r from-amber-100/50 to-rose-100/50" }, coverArt && /* @__PURE__ */ React.createElement("img", { src: coverArt, alt: "Artifact cover art", className: "max-w-[200px] mx-auto rounded-xl shadow-lg mb-4 border-2 border-amber-200" }), /* @__PURE__ */ React.createElement("h3", { className: "text-3xl font-black text-amber-900" }, storyTitle || storyPrompt || sourceTopic || (artifactType === "comic" ? "My Comic" : "My Story")), authorName && /* @__PURE__ */ React.createElement("p", { className: "text-amber-800 text-sm mt-1 font-bold" }, "By ", authorName), /* @__PURE__ */ React.createElement("p", { className: "text-amber-700 text-sm mt-1 italic" }, genreLabel(genre) || "Creative Writing", " \xB7 ", vocabTerms.length, " ", ta("a11y.storyforge_ui_vocabulary_terms"))), layoutMode === "comic" ? (
    /* Page-aware Comic Preview */
    /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { className: "sf-page-navigator bg-slate-950 text-white px-3 py-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-slate-700" }, /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-black uppercase tracking-widest" }, "Page ", focusedComicPreviewPage?.page || 1, " of ", comicPageGroups.length, " ", /* @__PURE__ */ React.createElement("span", { className: "text-slate-400" }, ta("a11y.storyforge_ui_follow_the_numbered_panels"))), /* @__PURE__ */ React.createElement("div", { role: "group", className: "flex items-center gap-1.5 max-w-full", "aria-label": ta("a11y.storyforge_attr_comic_preview_page_navigation") }, /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        "data-sf-focusable": true,
      ...
      /* @__PURE__ */ React.createElement(ArrowLeft, { size: 15, "aria-hidden": "true" })
    ), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-1 overflow-x-auto", role: "tablist", "aria-label": ta("a11y.storyforge_attr_comic_pages") }, comicPageGroups.map((page) => /* @__PURE__ */ React.createElement(
      "button",
      {
        ...
        "aria-label": ta("a11y.storyforge_aria_show_comic_page").replace("{0}", page.page)
      },
      page.page
    ))), /* @__PURE__ */ React.createElement(
      "button",
      ...
      /* @__PURE__ */ React.createElement(ArrowRight, { size: 15, "aria-hidden": "true" })
    ))), /* @__PURE__ */ React.createElement("div", { className: "p-3 sm:p-4 bg-slate-900", "aria-live": "polite" }, focusedComicPreviewPage && (() => {
      ...
      return /* @__PURE__ */ React.createElement("section", { id: `sf-comic-preview-page-${page.page}`, role: "tabpanel", className: "overflow-hidden rounded-lg border border-slate-700 bg-slate-950/75 shadow-lg", "aria-label": ta("a11y.storyforge_aria_comic_page").replace("{0}", page.page) }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-3 py-2 bg-slate-950 text-white border-b border-slate-700" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "text-xs font-black uppercase tracking-widest" }, "Page ", page.page, " \xB7 ", getComicPageLayoutLabel(page.layout)), /* @__PURE__ */ React.createElement("div", { className: "text-[10px] text-slate-300 font-bold" }, ta("a11y.storyforge_ui_panels"), " ", page.startPanel, "-", page.endPanel, " \xB7 ", getComicReadingOrderLabel(page.layout))), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-center gap-1.5 text-[10px] font-black uppercase tracking-widest" }, /* @__PURE__ */ React.createElement("span", { className: `rounded-full border px-2 py-1 ${previewPageStats.status === "Review" ? "border-rose-300/50 bg-rose-400/20 text-rose-100" : previewPageStats.status === "Ready" ? "border-emerald-300/50 bg-emerald-400/20 text-emerald-100" : "border-white/20 bg-white/10 text-white"}` }, previewPageStats.status), /* @__PURE__ */ React.createElement("span", { className: "rounded-full border border-white/20 bg-white/10 px-2 py-1" }, ta("a11y.storyforge_ui_art"), " ", previewPageStats.artPanels, "/", previewPageStats.total), /* @__PURE__ */ React.createElement("span", { className: "rounded-full border border-white/20 bg-white/10 px-2 py-1" }, ta("a11y.storyforge_ui_lettering"), " ", previewPageStats.placedBubbles, "/", previewPageStats.bubblePanels), /* @__PURE__ */ React.createElement("span", { className: "rounded-full border border-white/20 bg-white/10 px-2 py-1" }, getComicPrintFormatLabel(printSafety.format)), gutterSide && /* @__PURE__ */ React.createElement("span", { className: "rounded-full border border-rose-300/40 bg-rose-400/20 px-2 py-1" }, gutterSide, " gutter"))), /* @__PURE__ */ React.createElement("div", { className: `p-3 grid gap-3 ${page.layout === "strip" ? "grid-cols-1" : "grid-cols-2"}`, style: { direction: page.layout === "manga" ? "rtl" : "ltr" } }, page.panels.map(({ paragraph, idx: panelIdx }, pageIndex) => renderComicPreviewPanel(paragraph, panelIdx, page.layout, pageIndex, page))), previewPageStats.attention > 0 && /* @__PURE__ */ React.createElement("div", { className: "px-3 py-2 bg-rose-950/40 text-rose-100 border-t border-rose-900/60 text-[11px] font-bold flex flex-wrap gap-x-3 gap-y-1" }, previewPageStats.unplacedBubbles > 0 && /* @__PURE__ */ React.createElement("span", null, previewPageStats.unplacedBubbles, " ", ta("a11y.storyforge_ui_lettering_anchor"), previewPageStats.unplacedBubbles === 1 ? "" : "s", " needed"), previewPageStats.gutterRiskPanels > 0 && /* @__PURE__ */ React.createElement("span", null, previewPageStats.gutterRiskPanels, " ", ta("a11y.storyforge_ui_gutter_conflict").replace("{0}", previewPageStats.gutterRiskPanels === 1 ? "" : "s")), previewPageStats.crowdedBubbles > 0 && /* @__PURE__ */ React.createElement("span", null, previewPageStats.crowdedBubbles, " ", ta("a11y.storyforge_ui_crowded_bubble_panel").replace("{0}", previewPageStats.crowdedBubbles === 1 ? "" : "s")), previewPageStats.emptyPanels > 0 && /* @__PURE__ */ React.createElement("span", null, previewPageStats.emptyPanels, " ", ta("a11y.storyforge_ui_empty_panel").replace("{0}", previewPageStats.emptyPanels === 1 ? "" : "s"))), (turnLabel || page.note) && /* @__PURE__ */ React.createElement("div", { className: "px-3 py-2 bg-slate-900 text-slate-200 border-t border-slate-700 text-[11px] font-bold" }, turnLabel && /* @__PURE__ */ React.createElement("span", null, ta("a11y.storyforge_ui_page_turn").replace("{0}", turnLabel)), page.note && /* @__PURE__ */ React.createElement("span", null, turnLabel ? " \xB7 " : "", page.note)));
    })()))
  ) : (
    /* ── Prose Layout ── */
  ...
  )), /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border-2 border-amber-200 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-amber-700 uppercase tracking-wider mb-3 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Star, { size: 16 }), " ", ta("a11y.storyforge_ui_achievements"), earnedCount, "/", achievements.length, ")"), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 sm:grid-cols-5 gap-2" }, achievements.map((a) => /* @__PURE__ */ React.createElement("div", { key: a.id, className: `text-center p-2.5 rounded-xl border-2 transition-all ${a.earned ? "bg-amber-50 border-amber-300 shadow-sm" : "bg-slate-50 border-slate-200 opacity-50"}`, title: ta("a11y.storyforge_ach_" + a.id + "_desc") }, /* @__PURE__ */ React.createElement("div", { className: "text-2xl" }, a.icon), /* @__PURE__ */ React.createElement("div", { className: "text-[11px] font-bold text-slate-700 mt-1" }, ta("a11y.storyforge_ach_" + a.id + "_name")))))), /* @__PURE__ */ React.createElement("div", { className: "flex justify-center" }, /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
    ...
    " Export ",
    artifactType === "comic" ? "Comic" : "Storybook"
  )), /* @__PURE__ */ React.createElement("details", { "data-sf-publish-more": true, className: "bg-white rounded-2xl border border-slate-200 p-4" }, /* @__PURE__ */ React.createElement("summary", { className: "cursor-pointer text-sm font-black text-slate-700 focus-visible:ring-2 focus-visible:ring-rose-500 rounded-lg" }, "More output options"), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-xs text-slate-500" }, ta("a11y.storyforge_ui_choose_a_presentation_production_handoff_or")), /* @__PURE__ */ React.createElement("div", { className: "mt-3 flex flex-col sm:flex-row sm:flex-wrap gap-3 items-center" }, layoutMode === "comic" && /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
    ...
    " Save to Portfolio"
  ))), /* @__PURE__ */ React.createElement("p", { className: "text-slate-600 text-sm text-center" }, ux("output_help", "Download a finished story to read or print. Use an editable backup below to continue working on another device.")), /* @__PURE__ */ React.createElement("p", { className: "text-slate-500 text-xs text-center" }, artifactType === "comic" ? "Comic" : "Storybook", " and slideshow exports open in new tabs \xB7 print or save as PDF"), /* @__PURE__ */ React.createElement("details", { "data-sf-project-tools": true, className: "bg-white rounded-2xl border border-slate-200 p-4" }, /* @__PURE__ */ React.createElement("summary", { className: "cursor-pointer rounded-lg text-sm font-black text-slate-700 focus-visible:ring-2 focus-visible:ring-rose-500" }, /* @__PURE__ */ React.createElement("span", { className: "inline-flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Save, { size: 16, "aria-hidden": "true" }), " ", ux("project_files", "Editable backups and collaboration"))), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-xs text-slate-500" }, ta("a11y.storyforge_ui_export_or_import_a_portable_project")), /* @__PURE__ */ React.createElement("div", { className: "mt-3 space-y-4" }, /* @__PURE__ */ React.createElement("div", { "data-sf-project-vault": true, className: "bg-white rounded-2xl border-2 border-emerald-200 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-emerald-700 uppercase tracking-wider mb-2 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Save, { size: 16 }), " Project Vault"), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-600 mb-3" }, vaultStorageMode === "vault" ? "Your writing, artwork, cover, continuity notes, production audits, and durable narration data are saved in this browser." : "Save a portable project package to preserve artwork and narration across browsers or devices."), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2 items-end" }, /* @__PURE__ */ React.createElement("button", { type: "button", onClick: exportStoryForgeProject, className: "px-4 py-2 bg-emerald-700 text-white rounded-lg text-xs font-bold hover:bg-emerald-800 transition-colors flex items-center gap-2", "aria-label": ta("a11y.storyforge_attr_export_full_story_forge_project") }, /* @__PURE__ */ React.createElement(Download, { size: 14 }), " Export .storyforge"), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-import-draft": true, onClick: importDraftJSON, className: "px-4 py-2 bg-emerald-100 text-emerald-700 rounded-lg text-xs font-bold hover:bg-emerald-200 transition-colors flex items-center gap-2", "aria-label": ta("a11y.storyforge_attr_import_story_forge_project") }, /* @__PURE__ */ React.createElement(Plus, { size: 14 }), " Import project"), /* @__PURE__ */ React.createElement("label", { className: "flex-1 min-w-[180px]" }, /* @__PURE__ */ React.createElement("span", { className: "sr-only" }, ta("a11y.storyforge_ui_checkpoint_name")), /* @__PURE__ */ React.createElement("input", { value: revisionLabel, onChange: (e) => setRevisionLabel(e.target.value.slice(0, 100)), placeholder: ta("a11y.storyforge_attr_checkpoint_name"), className: "w-full px-3 py-2 text-xs rounded-lg border border-emerald-200 bg-emerald-50/40 text-slate-700", "aria-label": ta("a11y.storyforge_attr_checkpoint_name") })), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => void saveRevisionCheckpoint(), disabled: projectMutationBusy || draftHydrationState !== "ready" || draftSaveState === "saving", className: "px-4 py-2 bg-slate-900 text-white rounded-lg text-xs font-bold hover:bg-slate-800 transition-colors flex items-center gap-2 disabled:opacity-50", "aria-label": ta("a11y.storyforge_attr_save_revision_checkpoint") }, /* @__PURE__ */ React.createElement(Save, { size: 14 }), " Checkpoint")), revisionHistory.length > 0 && /* @__PURE__ */ React.createElement("div", { role: "group", className: "mt-4 border-t border-emerald-100 pt-3", "aria-label": ta("a11y.storyforge_attr_saved_project_checkpoints") }, /* @__PURE__ */ React.createElement("div", { className: "text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2" }, ta("a11y.storyforge_ui_recent_checkpoints")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2" }, revisionHistory.slice(0, 6).map((revision) => /* @__PURE__ */ React.createElement("button", { key: revision.id, type: "button", "data-sf-restore-checkpoint": revision.id, onClick: () => restoreRevisionCheckpoint(revision), className: "px-3 py-1.5 rounded-full border border-emerald-200 bg-emerald-50 text-[11px] font-bold text-emerald-800 hover:bg-emerald-100", title: `Restore ${revision.label}` }, revision.label, " \xB7 ", new Date(revision.savedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })))))), /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl border-2 border-cyan-200 p-5 shadow-sm" }, /* @__PURE__ */ React.createElement("h4", { className: "text-sm font-bold text-cyan-700 uppercase tracking-wider mb-2 flex items-center gap-2" }, /* @__PURE__ */ React.createElement(RefreshCw, { size: 16 }), " Pass the Torch"), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-600 mb-3" }, ta("a11y.storyforge_ui_export_your_draft_as_a_file")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row sm:flex-wrap gap-3" }, /* @__PURE__ */ React.createElement("button", { type: "button", onClick: exportDraftJSON, className: "px-4 py-2 bg-cyan-700 text-white rounded-lg text-xs font-bold hover:bg-cyan-800 transition-colors flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Download, { size: 14 }), " Export Draft (.json)"), /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-import-draft": true, onClick: importDraftJSON, className: "px-4 py-2 bg-cyan-100 text-cyan-700 rounded-lg text-xs font-bold hover:bg-cyan-200 transition-colors flex items-center gap-2" }, /* @__PURE__ */ React.createElement(Plus, { size: 14 }), " Import Classmate's Draft")))))))), /* @__PURE__ */ React.createElement("div", { className: "bg-white border-t border-slate-200 p-3 sm:p-4 flex flex-wrap sm:flex-nowrap justify-between items-center gap-2 shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]" }, /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
    ...
    " Back"
  ), /* @__PURE__ */ React.createElement("div", { "data-sf-step-summary": true, className: "order-first sm:order-none w-full sm:w-auto sm:flex-1 text-center text-xs text-slate-500 font-medium" }, PHASE_LABELS[phaseIdx], " ", ta("a11y.storyforge_ui_step"), " ", phaseIdx + 1, " of ", PHASES.length), phaseIdx < PHASES.length - 1 ? /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      "data-sf-primary-cta": true,
    ...
    nextActionLabels[phase],
    " ",
    /* @__PURE__ */ React.createElement(ArrowRight, { size: 16 })
  ) : /* @__PURE__ */ React.createElement("button", { type: "button", "data-sf-footer-action": "done", onClick: safeClose, className: "min-h-11 flex-1 sm:flex-none px-4 sm:px-5 py-2.5 rounded-full text-sm font-bold bg-slate-600 text-white hover:bg-slate-700 transition-colors flex items-center justify-center gap-2" }, "Done ", /* @__PURE__ */ React.createElement(CheckCircle2, { size: 16 })))));
});
