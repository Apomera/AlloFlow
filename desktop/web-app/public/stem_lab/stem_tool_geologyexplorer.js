...
(function () {
  ...
  var FOSSILS = {
    ...
    sandstone: { icon: '🌿', name: 'Plant fossils',                  tells: 'Plant fossils show land was nearby. Ripple marks beside them are not fossils: moving water or wind shaped the sand. Together they point to rivers, deltas or beaches.' },
    shale:     { icon: '🦐', name: 'Trilobites & graptolites',       tells: 'Trilobites crawled on the sea floor and graptolites drifted in the water above; their remains sank into calm water where fine mud settled. Both are index fossils that help date layers.' },
    limestone: { icon: '🐚', name: 'Brachiopods, crinoids & coral',  tells: 'Shelly sea-floor life. Many limestones are made mostly of these shells, and they mark a warm, shallow sea.' }
  };
  ...
  // ── Sub-voxel digging (pure). A cell the explorer digs into splits into DIG_SUB³ small
  // voxels and each strike carves a ragged crater of them. The geology stays per CELL (rock
  // type, fossils, cores, depth), so layer contacts stay crisp while the hole looks dug. ──
  var DIG_SUB = 4, DIG_SUB3 = 64, DIG_HISTORY_MAX = 160, DIG_HISTORY_KEEP = 120;
  // Crater radius (in small voxels) and swing time per strike: soft rock goes in big fast
  // bites, hard and crystalline rock in small slow chips — hardness you can feel.
  var DIG_STRIKE = { Loose: { r: 1.95, ms: 150 }, Layered: { r: 1.65, ms: 210 }, Dense: { r: 1.5, ms: 250 }, Crystalline: { r: 1.4, ms: 270 }, Hard: { r: 1.4, ms: 320 } };
  function digStrike(label, tool) {
    var s = DIG_STRIKE[label] || DIG_STRIKE.Dense;
    return tool === 'drill' ? { r: Math.round(s.r * 85) / 100, ms: Math.max(70, Math.round(s.ms * 0.45)) } : { r: s.r, ms: s.ms };
  }
  function digSubIndex(lx, ly, lz) { return lx + DIG_SUB * (ly + DIG_SUB * lz); }
  function digMaskHex(mask) {
    var out = '';
    for (var i = 0; i < DIG_SUB3; i += 4) out += (mask[i] | (mask[i + 1] << 1) | (mask[i + 2] << 2) | (mask[i + 3] << 3)).toString(16);
    return out;
  }
  function digMaskFromHex(hex) {
    var mask = new Uint8Array(DIG_SUB3), s = String(hex || '');
    for (var i = 0; i < DIG_SUB3 / 4; i++) {
      var n = parseInt(s.charAt(i), 16) || 0;
      mask[i * 4] = n & 1; mask[i * 4 + 1] = (n >> 1) & 1; mask[i * 4 + 2] = (n >> 2) & 1; mask[i * 4 + 3] = (n >> 3) & 1;
    }
    return mask;
  }
  function digCellState(mask) {                                // 'F' intact, '0' dug out, else the 16-hex mask
    if (!mask) return 'F';
    var n = 0; for (var i = 0; i < DIG_SUB3; i++) n += mask[i];
    return n === DIG_SUB3 ? 'F' : (n === 0 ? '0' : digMaskHex(mask));
  }
  function digNoise(gx, gy, gz) {                              // deterministic 0..1 per small voxel
    var h = Math.imul(gx | 0, 374761393) ^ Math.imul(gy | 0, 668265263) ^ Math.imul(gz | 0, 1442695041);
    h = Math.imul(h ^ (h >>> 13), 1274126177); h ^= h >>> 16;
    return (h >>> 0) / 4294967296;
  }
  // Small voxels (global sub-grid coords) one strike removes: a sphere whose radius wobbles
  // ±20% per voxel, so the rim breaks unevenly. present(gx,gy,gz) gates what can go.
  function digCrater(cx, cy, cz, r, present) {
    var out = [], span = Math.ceil(r * 1.2) + 1, bx = Math.floor(cx), by = Math.floor(cy), bz = Math.floor(cz);
    for (var gz = bz - span; gz <= bz + span; gz++) for (var gy = by - span; gy <= by + span; gy++) for (var gx = bx - span; gx <= bx + span; gx++) {
      var dx = gx + 0.5 - cx, dy = gy + 0.5 - cy, dz = gz + 0.5 - cz, reach = r * (0.8 + 0.4 * digNoise(gx, gy, gz));
      if (dx * dx + dy * dy + dz * dz <= reach * reach && present(gx, gy, gz)) out.push([gx, gy, gz]);
    }
    return out;
  }
  // Digging down under your own feet: a body-wide shaft bite from the row under the feet down
  // `rows` rows, its edge ragged like a crater's. A sphere centred on the aim point leaves a
  // rim under one foot that holds the explorer above their own hole.
  function digShaft(cx, cz, gyTop, rows, r, present) {
    var out = [], span = Math.ceil(r * 1.2) + 1, bx = Math.floor(cx), bz = Math.floor(cz);
    for (var gy = gyTop; gy < gyTop + rows; gy++) for (var gz = bz - span; gz <= bz + span; gz++) for (var gx = bx - span; gx <= bx + span; gx++) {
      var dx = gx + 0.5 - cx, dz = gz + 0.5 - cz, reach = r * (0.85 + 0.3 * digNoise(gx, gy, gz));
      if (dx * dx + dz * dz <= reach * reach && present(gx, gy, gz)) out.push([gx, gy, gz]);
    }
    return out;
  }
  // Grid ray march (Amanatides–Woo) in sub-grid units: the first solid small voxel along the
  // ray within maxT, with the face it was entered through. PURE — replaces a per-frame
  // raycast against thousands of instances.
  function digRayMarch(ox, oy, oz, dx, dy, dz, maxT, solid) {
    var gx = Math.floor(ox), gy = Math.floor(oy), gz = Math.floor(oz);
    if (solid(gx, gy, gz)) return { g: [gx, gy, gz], t: 0, normal: [0, 0, 0] };
    ...
    var tmx = sx > 0 ? (gx + 1 - ox) / dx : (sx < 0 ? (ox - gx) / -dx : Infinity);
    var tmy = sy > 0 ? (gy + 1 - oy) / dy : (sy < 0 ? (oy - gy) / -dy : Infinity);
    var tmz = sz > 0 ? (gz + 1 - oz) / dz : (sz < 0 ? (oz - gz) / -dz : Infinity);
    for (var steps = 0; steps < 4096; steps++) {
      var t, n;
      if (tmx < tmy && tmx < tmz) { gx += sx; t = tmx; tmx += tdx; n = [-sx, 0, 0]; }
      else if (tmy < tmz) { gy += sy; t = tmy; tmy += tdy; n = [0, -sy, 0]; }
      else { gz += sz; t = tmz; tmz += tdz; n = [0, 0, -sz]; }
      if (!(t <= maxT)) return null;
      if (solid(gx, gy, gz)) return { g: [gx, gy, gz], t: t, normal: n };
    }
    return null;
  }
  ...
  function digEntryStates(entry) {
    ...
    if (entry && entry.b) return entry.b;
    var out = {}; if (entry && entry.c) Object.keys(entry.c).forEach(function (id) { out[id] = entry.c[id][1]; });
    return out;
  }
  function digReplay(history) {
    var state = {};
  ...
  function specimenForCell(sceneId, key, x, y, z, keyAt) {
    if (sceneId === 'crust' && SED_FOSSIL[key] && hasFossilAt(x, y, z)) {
      var fossil = FOSSILS[key];
  ...
  function specimenSubOf(x, y, z) { var i = Math.floor(digNoise(x + 101, y + 211, z + 307) * 8); return [1 + (i & 1), 1 + ((i >> 1) & 1), 1 + ((i >> 2) & 1)]; }
  // r128's setColorAt sizes the colour buffer from the CURRENT count, so an instanced mesh that
  // starts at count 0 got a zero-length buffer and drew every instance black. Allocate it at capacity.
  function withInstanceColors3d(instanced3d, capacity3d) {
    var T = window.THREE;
    instanced3d.instanceColor = new T.InstancedBufferAttribute(new Float32Array(capacity3d * 3).fill(1), 3);
  ...
  // ── Landing search: where "Drop in" actually puts the walker ────────────────
  // fpSeedPose names a fixed column, but the learner can hide that column (cutaway,
  // focus lens, play-history stage, or having dug it out) and hidden voxels are not
  // solid ground — so dropping in there fell for ever. Walk outward from the seed
  // column ring by ring and take the nearest column whose probe returns a landing.
  // `probe(x, z)` is the engine's physics check; here it is only a predicate so the
  // search itself is jsdom-testable. Ties inside a ring go to the Euclidean-nearest.
  function fpLandingSearch(cx, cz, nx, nz, probe) {
    var maxRing = Math.max(nx, nz);
    for (var ring = 0; ring <= maxRing; ring++) {
      ...
      for (var dx = -ring; dx <= ring; dx++) for (var dz = -ring; dz <= ring; dz++) {
        ...
        var landing = probe(x, z);
        if (!landing) continue;
        var d2 = dx * dx + dz * dz;
        if (!best || d2 < best.d2) best = { x: x, z: z, ring: ring, d2: d2, landing: landing };
      }
      if (best) return best;
    }
    return null;
  }
  // ── Layer extent + neighbours: the "where does this layer sit" facts a highlight shows ──
  // Scanned from the scene generator (not the engine's excavation state), so the numbers
  ...
  function layerExtent(key) {
    ...
    // scene's order (outer → inner) is the honest neighbour list there.
    var shellIdx = radial && SCENE.order ? SCENE.order.indexOf(key) : -1;
    var aboveKey = shellIdx >= 0 ? (SCENE.order[shellIdx - 1] || null) : top(above);
    var belowKey = shellIdx >= 0 ? (SCENE.order[shellIdx + 1] || null) : top(below);
    return {
      key: key, count: count, total: total, share: total ? count / total : 0,
      minY: minY, maxY: maxY, rows: count ? maxY - minY + 1 : 0,
      topKm: count ? minY * KM_PER_VOXEL : null, bottomKm: count ? (maxY + 1) * KM_PER_VOXEL : null,
      thicknessKm: count ? (maxY - minY + 1) * KM_PER_VOXEL : null,
  ...
  var FP_BUST = {
    ...
    intrusion: 'Myth-bust: this granite never erupted — it cooled slowly deep underground, which is why its crystals grew big enough to see.',
    marble: 'Marble IS limestone, recrystallised by the pluton’s heat — same chemistry, a new interlocking texture, fossils erased.',
    hornfels: 'Hornfels IS shale baked hard beside the pluton — contact metamorphism needs heat, not deep burial.',
    magma: 'Myth-bust: magma underground is not a vast liquid lake — most reservoirs are crystal mush with pockets of melt. It is called lava only once it erupts.',
    limestone: 'Limestone forms in warm shallow seas from shells and carbonate mud — finding it here means this land was once under the sea.'
  };
  ...
  function initEngine(container, opts) {
    ...
    // removed: cells fully dug out. damaged: cells partly dug, as a DIG_SUB³ mask (1 = still rock).
    var removed = {}, damaged = {}, excavationHistory = [], excavationRedo = [];
    var SUBV = VOXEL / DIG_SUB, SUBNX = NX * DIG_SUB, SUBNY = NY * DIG_SUB, SUBNZ = NZ * DIG_SUB;
    function cellAt(x, y, z) { return (x < 0 || y < 0 || z < 0 || x >= NX || y >= NY || z >= NZ) ? null : voxels[(y * NX + x) * NZ + z]; }
    function applyCellState(id, state) {                       // 'F' intact · '0' dug out · 16-hex partial mask
      if (state === '0') { removed[id] = 1; delete damaged[id]; }
      else if (state === 'F' || !state) { delete removed[id]; delete damaged[id]; }
      else { delete removed[id]; damaged[id] = digMaskFromHex(state); }
    }
    ...
    initialHistory.forEach(function (entry) {
      if (typeof entry === 'string') {
        ...
        if (voxel && voxel.key !== 'void' && !removed[entry]) { removed[entry] = 1; delete damaged[entry]; excavationHistory.push(entry); }
        return;
      }
    ...
    // Specimens hidden in this world (see specimenForCell): one small voxel each, looked up by
    // its sub-grid coordinates when a strike takes it.
    var specimens3d = [], specimenByG3d = {}, specimenByCell3d = {};
    for (var spi = 0; spi < voxels.length; spi++) {
      var spCell = voxels[spi], spInfo = specimenForCell(SCENE.id, spCell.key, spCell.x, spCell.y, spCell.z, function (nx, ny, nz) { var nb = cellAt(nx, ny, nz); return nb ? nb.key : null; });
      if (!spInfo) continue;
      var spLocal = specimenSubOf(spCell.x, spCell.y, spCell.z);
    ...
    // World-aligned texture coordinates: every face samples the rock texture by its position in
    // the block, not 0..1 per cube. Neighbouring cubes join into one continuous surface (no
    // repeating tile grid) and a dug small voxel shows the same grain as the cell it came from.
    mat.onBeforeCompile = function (shader3d) {
      shader3d.uniforms.geoUvScale = { value: 1 / VOXEL };
      shader3d.vertexShader = 'uniform float geoUvScale;\n' + shader3d.vertexShader.replace('#include <uv_vertex>', [
        '#include <uv_vertex>',
        '#if defined( USE_UV ) && defined( USE_INSTANCING )',
        '\tvec4 geoWp = modelMatrix * instanceMatrix * vec4( position, 1.0 );',
        '\tvec3 geoN = abs( normal );',
        '\tvec2 geoUv = geoN.x > 0.5 ? geoWp.zy : ( geoN.y > 0.5 ? geoWp.xz : geoWp.xy );',
        '\tvUv = ( uvTransform * vec3( geoUv * geoUvScale, 1.0 ) ).xy;',
        '#endif'
      ].join('\n'));
    };
    var mesh = new THREE.InstancedMesh(geo, mat, voxels.length);
    mesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
    mesh.frustumCulled = false;   // instances span the block; the base cube's bounds at the origin would cull the lot from inside it
    // Partly-dug cells draw as their remaining small voxels on a second instanced mesh.
    var subGeo3d = new THREE.BoxGeometry(SUBV, SUBV, SUBV), subCapacity3d = 2048, instanceToSub3d = [];
    function makeSubMesh3d(capacity3d) {
      var subMeshNew3d = withInstanceColors3d(new THREE.InstancedMesh(subGeo3d, mat, capacity3d), capacity3d);
      subMeshNew3d.instanceMatrix.setUsage(THREE.DynamicDrawUsage); subMeshNew3d.count = 0; subMeshNew3d.frustumCulled = false;
      subMeshNew3d.castShadow = geologyHighDetail3d; subMeshNew3d.receiveShadow = geologyHighDetail3d;
      return subMeshNew3d;
    }
    var subMesh3d = makeSubMesh3d(subCapacity3d);
    ...
    Object.keys(specimenGeos3d).forEach(function (shape3d) {
      var capacity3d = Math.max(1, specimens3d.filter(function (sp) { return sp.info.shape === shape3d; }).length);
      var specimenMesh3d = withInstanceColors3d(new THREE.InstancedMesh(specimenGeos3d[shape3d], specimenMats3d[shape3d], capacity3d), capacity3d);
      specimenMesh3d.instanceMatrix.setUsage(THREE.DynamicDrawUsage); specimenMesh3d.count = 0; specimenMesh3d.frustumCulled = false;
      specimenMeshes3d[shape3d] = specimenMesh3d; scene.add(specimenMesh3d);
    });
    ...
    function rebuildSpecimens3d() {
      ...
      for (var si = 0; si < specimens3d.length; si++) {
        ...
        var open = null;
        for (var fi = 0; fi < 6 && !open; fi++) { var f = SPECIMEN_FACES3d[fi]; if (!subSolidG(g[0] + f[0], g[1] + f[1], g[2] + f[2])) open = f; }
        if (!open) continue;                                    // still buried: nothing to see yet
        var shape = sp.info.shape, target = specimenMeshes3d[shape], n = counts3d[shape];
        if (n >= target.instanceMatrix.count) continue;
        var c = subCentreWorld(g[0], g[1], g[2]), spin = digNoise(g[0] + 7, g[1] + 7, g[2] + 7) * 6.28;
        // poke out through the open face (sub-grid y points down, world y up)
        specimenDummy3d.position.set(c[0] + open[0] * SUBV * 0.32, c[1] - open[1] * SUBV * 0.32, c[2] + open[2] * SUBV * 0.32);
        if (shape === 'fossil') {                               // a flat imprint facing the open face
          specimenDummy3d.rotation.set(open[2] ? Math.PI / 2 : 0, spin, open[0] ? Math.PI / 2 : 0);
          specimenDummy3d.scale.set(SUBV * 1.3, SUBV * 0.42, SUBV * 1.0);
        } else if (shape === 'prism') { specimenDummy3d.rotation.set(0.5 * (open[2] || 1), spin, 0.4 * (open[0] || 0)); specimenDummy3d.scale.set(SUBV * 0.8, SUBV * 1.15, SUBV * 0.8); }
        else { specimenDummy3d.rotation.set(spin, spin * 0.7, spin * 0.3); specimenDummy3d.scale.setScalar(SUBV * 0.9); }
        specimenDummy3d.updateMatrix(); target.setMatrixAt(n, specimenDummy3d.matrix); target.setColorAt(n, specimenColor3d.setHex(sp.info.color));
        counts3d[shape] = n + 1;
      }
      Object.keys(specimenMeshes3d).forEach(function (shape3d) {
        var specimenMesh3d = specimenMeshes3d[shape3d];
        specimenMesh3d.count = counts3d[shape3d]; specimenMesh3d.instanceMatrix.needsUpdate = true; specimenMesh3d.instanceColor.needsUpdate = true;
      });
    }
    // A strike or whole-cell dig that takes a specimen's voxel collects it.
    function collectSpecimen3d(sp) {
      if (!sp || sp.taken) return false;
      sp.taken = true;
      var c = subCentreWorld(sp.g[0], sp.g[1], sp.g[2]);
      geoSfxFind(sp.info.shape); spawnDebris3d([c], sp.info.color, 3, SUBV * 0.3);
      if (window._alloHaptic) { try { window._alloHaptic('achieve'); } catch (e) {} }
      var found = Object.assign({ sceneId: SCENE.id, depthKm: +(sp.cell.y * KM_PER_VOXEL).toFixed(1) }, sp.info);
      if (opts.onSpecimen) opts.onSpecimen(found);
      else if (sp.info.crustFossil && opts.onUncover) opts.onUncover(sp.info.key);
      return true;
    }
    ...
    function spawnExcavationBurst3d(voxel3d, at3d, size3d) {
      if (!voxel3d) return;
      var burstPosition3d = at3d || worldPos(voxel3d), burstSize3d = size3d || 1;
      ...
      excavationFlashBox3d.scale.setScalar(burstSize3d); excavationFlashBox3d.userData.size = burstSize3d;
    ...
    // Time-based (k = frames at 60 fps this step): a frame-counted puff hung for seconds on a slow device.
    function updateExcavationEffects3d(dt3d) {
      if (excavationBurstLife3d <= 0) return;
      var k = Math.min(3, Math.max(0.1, (Number(dt3d) || 0.016) * 60)), dustDrag = Math.pow(0.96, k), dustLift = Math.pow(0.95, k), chipDrag = Math.pow(0.985, k);
      excavationBurstLife3d = Math.max(0, excavationBurstLife3d - 0.024 * k);
      excavationFlashMaterial3d.opacity = excavationBurstLife3d > 0
        ? (reducedMotion3d ? 0.72 : excavationBurstLife3d * 0.82) : 0;
      excavationFlashBox3d.scale.setScalar((excavationFlashBox3d.userData.size || 1) * (reducedMotion3d ? 1.04 : 1.02 + (1 - excavationBurstLife3d) * 0.24));
      if (!reducedMotion3d) {
        for (var dustUpdateIndex3d = 0; dustUpdateIndex3d < excavationDustCount3d; dustUpdateIndex3d++) {
          excavationDustPositions3d[dustUpdateIndex3d * 3] += excavationDustVelocity3d[dustUpdateIndex3d * 3] * k;
          excavationDustPositions3d[dustUpdateIndex3d * 3 + 1] += excavationDustVelocity3d[dustUpdateIndex3d * 3 + 1] * k;
          excavationDustPositions3d[dustUpdateIndex3d * 3 + 2] += excavationDustVelocity3d[dustUpdateIndex3d * 3 + 2] * k;
          excavationDustVelocity3d[dustUpdateIndex3d * 3] *= dustDrag;
          excavationDustVelocity3d[dustUpdateIndex3d * 3 + 1] *= dustLift;
          excavationDustVelocity3d[dustUpdateIndex3d * 3 + 2] *= dustDrag;
        }
        for (var chipUpdateIndex3d = 0; chipUpdateIndex3d < excavationChipCount3d; chipUpdateIndex3d++) {
          excavationChipPositions3d[chipUpdateIndex3d * 3] += excavationChipVelocity3d[chipUpdateIndex3d * 3] * k;
          excavationChipPositions3d[chipUpdateIndex3d * 3 + 1] += excavationChipVelocity3d[chipUpdateIndex3d * 3 + 1] * k;
          excavationChipPositions3d[chipUpdateIndex3d * 3 + 2] += excavationChipVelocity3d[chipUpdateIndex3d * 3 + 2] * k;
          excavationChipVelocity3d[chipUpdateIndex3d * 3 + 1] -= 0.007 * k;
          excavationChipVelocity3d[chipUpdateIndex3d * 3] *= chipDrag;
          excavationChipVelocity3d[chipUpdateIndex3d * 3 + 2] *= chipDrag;
        }
        excavationDustGeometry3d.attributes.position.needsUpdate = true;
        excavationChipGeometry3d.attributes.position.needsUpdate = true;
    ...
    // Rubble: a strike knocks loose a few chunks of the rock it broke. They tumble, land on the
    // small voxels below (or the floor of the hole), settle, then shrink away. Skipped under
    // reduced motion (the crater and flash still show what happened).
    var DEBRIS_MAX3d = 40, debris3d = [], debrisScratch3d = new THREE.Object3D(), debrisColor3d = new THREE.Color();
    var debrisGeo3d = new THREE.BoxGeometry(1, 1, 1), debrisMat3d = new THREE.MeshStandardMaterial({ color: 0xffffff, map: rockSurfaceTexture3d, roughness: 0.94, metalness: 0.02 });
    var debrisMesh3d = withInstanceColors3d(new THREE.InstancedMesh(debrisGeo3d, debrisMat3d, DEBRIS_MAX3d), DEBRIS_MAX3d);
    debrisMesh3d.instanceMatrix.setUsage(THREE.DynamicDrawUsage); debrisMesh3d.frustumCulled = false; debrisMesh3d.count = 0;
    scene.add(debrisMesh3d);
    function spawnDebris3d(points3d, colorHex3d, count3d, spread3d, away3d) {
      if (reducedMotion3d || !points3d || !points3d.length) return;
      var ax = away3d ? away3d.x : 0, az = away3d ? away3d.z : 0, al = Math.sqrt(ax * ax + az * az) || 1;
      for (var di = 0; di < count3d; di++) {
        if (debris3d.length >= DEBRIS_MAX3d) debris3d.shift();
        var src3d = points3d[(excavationRandom3d() * points3d.length) | 0], spread = spread3d || 0;
        debris3d.push({
          x: src3d[0] + (excavationRandom3d() - 0.5) * spread, y: src3d[1] + (excavationRandom3d() - 0.5) * spread, z: src3d[2] + (excavationRandom3d() - 0.5) * spread,
          vx: (excavationRandom3d() - 0.5) * VOXEL * 0.9 + (away3d ? ax / al * VOXEL * (0.4 + excavationRandom3d() * 0.6) : 0),
          vy: VOXEL * (away3d ? 0.1 + excavationRandom3d() * 0.35 : 0.25 + excavationRandom3d() * 0.8),
          vz: (excavationRandom3d() - 0.5) * VOXEL * 0.9 + (away3d ? az / al * VOXEL * (0.4 + excavationRandom3d() * 0.6) : 0),
          rx: excavationRandom3d() * 6.28, ry: excavationRandom3d() * 6.28, spin: (excavationRandom3d() - 0.5) * 9,
          s: SUBV * (0.3 + excavationRandom3d() * 0.4), life: 0, settled: false, c: colorHex3d, tint: 0.72 + excavationRandom3d() * 0.3
        });
      }
    }
    function debrisSolid3d(wx, wy, wz) { var g = worldToSubG(wx, wy, wz); return subSolidG(g[0], g[1], g[2]); }
    function updateDebris3d(dt) {
      if (!debris3d.length) { debrisMesh3d.count = 0; return; }
      var n = 0, grav = VOXEL * 16;
      for (var di = debris3d.length - 1; di >= 0; di--) { if ((debris3d[di].life += dt) > 2.6 || debris3d[di].y < -WORLD.h) debris3d.splice(di, 1); }
      var eye3d = camera.position, nearEye3d = VOXEL * 0.5;
      for (var dj = 0; dj < debris3d.length; dj++) {
        var d = debris3d[dj], half = d.s * 0.5;
        if (!d.settled) {
          d.vy -= grav * dt;
          var nx = d.x + d.vx * dt, nz = d.z + d.vz * dt;
          if (debrisSolid3d(nx, d.y, nz)) { d.vx *= -0.3; d.vz *= -0.3; } else { d.x = nx; d.z = nz; }   // glance off a wall
          var ny = d.y + d.vy * dt;
          if (d.vy < 0 && debrisSolid3d(d.x, ny - half, d.z)) {
            var gUnder = worldToSubG(d.x, ny - half, d.z);
            d.y = (NY / 2 - gUnder[1] / DIG_SUB) * VOXEL + half;   // rest on the top face of the small voxel below
            d.vy = Math.abs(d.vy) > VOXEL * 1.4 ? -d.vy * 0.28 : 0; d.vx *= 0.55; d.vz *= 0.55;
            if (!d.vy && Math.abs(d.vx) + Math.abs(d.vz) < VOXEL * 0.25) d.settled = true;
          } else d.y = ny;
          d.rx += d.spin * dt; d.ry += d.spin * 0.7 * dt;
        }
        // a chunk right at a first-person eye would fill the view: skip drawing it there
        if (fp.active && Math.abs(d.x - eye3d.x) < nearEye3d && Math.abs(d.y - eye3d.y) < nearEye3d && Math.abs(d.z - eye3d.z) < nearEye3d) continue;
        var fade = d.life > 2.0 ? Math.max(0, 1 - (d.life - 2.0) / 0.6) : 1;
        debrisScratch3d.position.set(d.x, d.y, d.z); debrisScratch3d.rotation.set(d.rx, d.ry, 0); debrisScratch3d.scale.setScalar(Math.max(0.001, d.s * fade));
        debrisScratch3d.updateMatrix(); debrisMesh3d.setMatrixAt(n, debrisScratch3d.matrix);
        debrisMesh3d.setColorAt(n, debrisColor3d.setHex(d.c).multiplyScalar(d.tint)); n++;
      }
      debrisMesh3d.count = n; debrisMesh3d.instanceMatrix.needsUpdate = true; if (debrisMesh3d.instanceColor) debrisMesh3d.instanceColor.needsUpdate = true;
    }
    ...
    function cellColor3d(v, out) {
      out.setHex((SCENE.palette[v.key] || ROCKS[v.key] || { color: 0x888888 }).color);
      out.multiplyScalar(v.j || 1);                     // per-voxel grain → natural, non-plastic rock texture
      ...
      out.multiplyScalar(ao * depthShade);
      if ((SCENE.palette[v.key] || {}).glow) {
        ...
        out.getHSL(glowHsl3d);
        out.setHSL(glowHsl3d.h, Math.min(1, glowHsl3d.s * 1.04), Math.min(0.64, glowHsl3d.l * 1.5));
      }
      if (SCENE.id === 'collision' && v.key === 'summitLimestone') out.lerp(WHITE, 0.5);   // snow line: the summit reads white from any distance
      ...
      if (highlightKey && !fp.active && !(SCENE.id === 'deepEarth' && geologyScienceStage3d > 0)) { if (v.key === highlightKey) out.lerp(WHITE, 0.42); else out.multiplyScalar(0.5); }
      return out;
    }
    // A small voxel is solid when its cell is shown and (if partly dug) its mask bit is set.
    // Reads visibleNow3d, which every state change refreshes through rebuild().
    function subSolidG(gx, gy, gz) {
      if (gx < 0 || gy < 0 || gz < 0 || gx >= SUBNX || gy >= SUBNY || gz >= SUBNZ) return false;
      var cx = (gx / DIG_SUB) | 0, cy = (gy / DIG_SUB) | 0, cz = (gz / DIG_SUB) | 0;
      if (!visibleNow3d[(cy * NX + cx) * NZ + cz]) return false;
      var m = damaged[cellAt(cx, cy, cz).id];
      return m ? m[digSubIndex(gx - cx * DIG_SUB, gy - cy * DIG_SUB, gz - cz * DIG_SUB)] === 1 : true;
    }
    function worldToSubG(wx, wy, wz) { return [Math.floor((wx / VOXEL + NX / 2) * DIG_SUB), Math.floor((NY / 2 - wy / VOXEL) * DIG_SUB), Math.floor((wz / VOXEL + NZ / 2) * DIG_SUB)]; }
    function subCentreWorld(gx, gy, gz) { return [((gx + 0.5) / DIG_SUB - NX / 2) * VOXEL, (NY / 2 - (gy + 0.5) / DIG_SUB) * VOXEL, ((gz + 0.5) / DIG_SUB - NZ / 2) * VOXEL]; }
    // Remaining small voxels of every partly-dug cell. Buried ones (all six sides rock) are
    // skipped; crater walls under an overhang shade darker and exposed edges catch light.
    var subColor3d = new THREE.Color();
    function rebuildSubs3d() {
      var ids = Object.keys(damaged), need = ids.length * DIG_SUB3;
      if (need > subCapacity3d) {
        scene.remove(subMesh3d); if (subMesh3d.dispose) subMesh3d.dispose();
        subCapacity3d = Math.min(Math.max(need, subCapacity3d * 2), 262144);
        subMesh3d = makeSubMesh3d(subCapacity3d); scene.add(subMesh3d);
      }
      var n = 0; instanceToSub3d.length = 0;
      for (var di = 0; di < ids.length && n < subCapacity3d; di++) {
        var cell = voxelByKey[ids[di]]; if (!cell || !visibleNow3d[(cell.y * NX + cell.x) * NZ + cell.z]) continue;
        var m = damaged[ids[di]], base = cellColor3d(cell, subColor3d), br = base.r, bgc = base.g, bb = base.b;
        var bx = cell.x * DIG_SUB, by = cell.y * DIG_SUB, bz = cell.z * DIG_SUB;
        for (var lz = 0; lz < DIG_SUB; lz++) for (var ly = 0; ly < DIG_SUB; ly++) for (var lx = 0; lx < DIG_SUB; lx++) {
          if (!m[digSubIndex(lx, ly, lz)] || n >= subCapacity3d) continue;
          var gx = bx + lx, gy = by + ly, gz = bz + lz;
          var around = (subSolidG(gx + 1, gy, gz) ? 1 : 0) + (subSolidG(gx - 1, gy, gz) ? 1 : 0) + (subSolidG(gx, gy + 1, gz) ? 1 : 0) +
            (subSolidG(gx, gy - 1, gz) ? 1 : 0) + (subSolidG(gx, gy, gz + 1) ? 1 : 0) + (subSolidG(gx, gy, gz - 1) ? 1 : 0);
          if (around === 6) continue;
          var sp = subCentreWorld(gx, gy, gz); dummy.position.set(sp[0], sp[1], sp[2]); dummy.updateMatrix();
          subMesh3d.setMatrixAt(n, dummy.matrix);
          var shade = (subSolidG(gx, gy - 1, gz) ? 0.84 : 1) * (around <= 3 ? 1.08 : (around === 4 ? 1.04 : 1)) * (0.94 + digNoise(gx, gy, gz) * 0.12);
          subMesh3d.setColorAt(n, subColor3d.setRGB(br * shade, bgc * shade, bb * shade));
          instanceToSub3d[n] = { cell: cell, g: [gx, gy, gz] }; n++;
        }
      }
      subMesh3d.count = n; subMesh3d.instanceMatrix.needsUpdate = true; if (subMesh3d.instanceColor) subMesh3d.instanceColor.needsUpdate = true;
      rebuildSpecimens3d();
    }
    // A tree or boulder stands while the small voxels under its middle are still rock.
    function surfacePropStanding3d(x, z) {
      var id = x + ',0,' + z; if (removed[id]) return false;
      var m = damaged[id]; if (!m) return true;
      return !!(m[digSubIndex(1, 0, 1)] || m[digSubIndex(2, 0, 1)] || m[digSubIndex(1, 0, 2)] || m[digSubIndex(2, 0, 2)]);
    }
    function rebuild(lightRebuild3d) {
      ...
      for (var k = 0; k < voxels.length; k++) {
        var v = voxels[k]; if (!visibleNow3d[k] || damaged[v.id]) continue;   // partly-dug cells draw as small voxels below
        var p = worldPos(v); dummy.position.set(p[0], p[1], p[2]); dummy.updateMatrix();
        mesh.setMatrixAt(i, dummy.matrix);
        mesh.setColorAt(i, cellColor3d(v, col));
      ...
      for (var ti = 0; ti < treeMeshes.length; ti++) { var tu = treeMeshes[ti].userData; treeMeshes[ti].visible = !focusLens && (tu.z < NZ - sliceZ) && surfacePropStanding3d(tu.x, tu.z) && (FORMED_AT.soil <= showStage); }
      if (lightRebuild3d) { updateUndoPreview(); return; }    // a strike changes only rock; skip the scene-wide effect passes
      ...
      updateUndoPreview();
      if (fpPhysicsReady) { updateLandingMarker3d(); updateHighlightAnchor3d(); }
    }
    rebuild();
    ...
    function fpVoxelAtWorld(wx, wy, wz) {
      ...
      if (!voxel || !visible(voxel)) return null;
      if (damaged[voxel.id]) { var g = worldToSubG(wx, wy, wz); if (!subSolidG(g[0], g[1], g[2])) return null; }   // a dug-out small voxel is air
      return voxel;
    }
    ...
    function fpGroundEyeY(wx, wz, maxFeetY) {
      ...
      for (var oi = 0; oi < offsets.length; oi++) {
        ...
        var col = fpWorldToVoxel(sx, 0, sz), sub = null;
        for (var vy = 0; vy < NY; vy++) {
          var voxel = cellAt(col.x, vy, col.z);
          if (!voxel || !visible(voxel) || fpMaterialPhysics(voxel.key).kind !== 'solid') continue;
          var top = worldPos(voxel)[1] + VOXEL * 0.5, mask = damaged[voxel.id];
          if (mask) {                                           // partly dug: stand on the small voxels under this foot sample
            if (!sub) sub = worldToSubG(sx, 0, sz);
            var lx = fpClampN(sub[0] - col.x * DIG_SUB, 0, DIG_SUB - 1), lz = fpClampN(sub[2] - col.z * DIG_SUB, 0, DIG_SUB - 1);
            for (var ly = 0; ly < DIG_SUB; ly++) {
              if (!mask[digSubIndex(lx, ly, lz)]) continue;
              var subTop = top - ly * SUBV;
              if (subTop <= maxFeetY + FP_STEP && (best == null || subTop > best)) best = subTop;
            }
            continue;
          }
    ...
    // ── Landing pad + marker: the validated spot "Drop in" puts the walker on ─────
    var fpPhysicsReady = true;                                 // hoisted → falsy during the init-time rebuild() above; true once this block has run
    var landingState3d = null, landingReportKey = '', landingPinAt3d = 0, highlightAnchor3d = null, calloutAt3d = 0;
    var landingRingGeo3d = new THREE.RingGeometry(VOXEL * 0.42, VOXEL * 0.6, 40);
    var landingRingMat3d = new THREE.MeshBasicMaterial({ color: 0xfbbf24, transparent: true, opacity: 0.92, side: THREE.DoubleSide, depthTest: false, depthWrite: false });
    var landingMarker3d = new THREE.Mesh(landingRingGeo3d, landingRingMat3d);
    landingMarker3d.rotation.x = -Math.PI / 2; landingMarker3d.renderOrder = 6; landingMarker3d.visible = false; scene.add(landingMarker3d);
    var landingBeamGeo3d = new THREE.CylinderGeometry(VOXEL * 0.05, VOXEL * 0.12, VOXEL * 2.6, 10, 1, true);
    var landingBeamMat3d = new THREE.MeshBasicMaterial({ color: 0xfde68a, transparent: true, opacity: 0.3, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide });
    var landingBeam3d = new THREE.Mesh(landingBeamGeo3d, landingBeamMat3d);
    landingBeam3d.renderOrder = 6; landingBeam3d.visible = false; scene.add(landingBeam3d);
    function fpLandingAt(wx, wz, maxFeetY) {                  // physics check for one standing spot; null when it cannot be stood on
      var floorEyeY = fpGroundEyeY(wx, wz, maxFeetY);
      if (floorEyeY == null) return null;
      if (fpBodyBlocked(wx, floorEyeY, wz)) return null;
      if (fpMediumAt(wx, floorEyeY, wz).kind === 'hazard') return null;
      var medium = fpMediumAt(wx, floorEyeY, wz);
      return { x: wx, y: floorEyeY, z: wz, hazard: fpHazardNearby(wx, floorEyeY, wz), fluidKey: medium.kind === 'fluid' && medium.voxel ? medium.voxel.key : null };   // standable-but-next-to-magma is a last resort, not a first choice
    }
    function fpLandingPose(sceneId) {
      var seed = fpSeedPose(sceneId);
      if (fpExplorerMode(sceneId) === 'fly') return { pos: { x: seed.pos.x, y: seed.pos.y, z: seed.pos.z }, yaw: seed.yaw, pitch: seed.pitch, ok: true, relocated: false, cause: null, ring: 0 };
      var col = fpWorldToVoxel(seed.pos.x, 0, seed.pos.z);
      var seedFeetY = seed.pos.y - FP_EYE_HEIGHT;
      function floorProbe(x, floorEyeY, z) {                  // the capsule has a radius: report the SOLID voxel it actually stands on, not water at its centre
        var offsets = [[0, 0], [FP_RADIUS, 0], [-FP_RADIUS, 0], [0, FP_RADIUS], [0, -FP_RADIUS]], fallback = null;
        for (var oi = 0; oi < offsets.length; oi++) {
          var here = null; try { here = fpProbe(x + offsets[oi][0], floorEyeY - FP_EYE_HEIGHT - VOXEL * 0.5, z + offsets[oi][1]); } catch (e) { here = null; }
          if (!here) continue;
          if (fpMaterialPhysics(here.key).kind === 'solid') return here;
          if (!fallback) fallback = here;
        }
        return fallback;
      }
      function finish(spot, x, z, startY, ring, relocated, cause) {
        var floorEyeY = spot.y, here = floorProbe(x, floorEyeY, z);
        var fluid = spot.fluidKey ? (SCENE.palette[spot.fluidKey] || ROCKS[spot.fluidKey] || null) : null;
        return { pos: { x: x, y: startY, z: z }, yaw: seed.yaw, pitch: seed.pitch, ok: true, relocated: relocated, cause: cause, ring: ring, floorY: floorEyeY, layerName: here ? here.layerName : null, layerKey: here ? here.key : null, depthKm: here ? here.depthKm : null, tempC: here ? here.tempC : null, fluidName: fluid ? fluid.name : null };
      }
      // The seed itself, at its exact world point (behaviour-preserving for a whole block).
      var hidden = sliceZ > 0 && col.z >= NZ - sliceZ;
      var seedSpot = hidden ? null : fpLandingAt(seed.pos.x, seed.pos.z, seedFeetY);
      if (seedSpot && !seedSpot.hazard) return finish(seedSpot, seed.pos.x, seed.pos.z, Math.max(seed.pos.y, seedSpot.y), 0, false, null);
      var cause = hidden ? 'cutaway' : (seedSpot ? 'hazard' : (focusLens && highlightKey ? 'lens' : (showStage < 99 ? 'history' : 'excavation')));
      // Pass 1 drops from the seed height (the geode seed sits inside the hollow, so this keeps
      // the landing on the crystal floor); pass 2 accepts any standable top in the block.
      var passes = [seedFeetY, WORLD.h * 0.5 + VOXEL];
      for (var pi = 0; pi < passes.length; pi++) {
        var maxFeetY = passes[pi];
        var found = fpLandingSearch(col.x, col.z, NX, NZ, function (cx, cz) {
          var spot = fpLandingAt((cx - (NX - 1) / 2) * VOXEL, (cz - (NZ - 1) / 2) * VOXEL, maxFeetY);
          return spot && !spot.hazard ? spot : null;
        });
        // A hazard-adjacent seed is a property of the scene, not something the learner did: nudge silently.
        if (found) return finish(found.landing, found.landing.x, found.landing.z, found.landing.y + VOXEL * 1.4, found.ring, cause !== 'hazard', cause);
      }
      if (seedSpot) return finish(seedSpot, seed.pos.x, seed.pos.z, Math.max(seed.pos.y, seedSpot.y), 0, false, 'hazard');   // next to magma beats nowhere at all
      return { pos: null, ok: false, relocated: false, cause: cause, ring: -1, layerName: null, layerKey: null };
    }
    function updateLandingMarker3d() {
      if (!fpPhysicsReady) return;
      var show = !fp.active && fpExplorerMode(SCENE.id) === 'mine';
      var landing = show ? fpLandingPose(SCENE.id) : null;
      landingState3d = landing;
      var visible3d = !!(landing && landing.ok);
      landingMarker3d.visible = visible3d; landingBeam3d.visible = visible3d;
      if (visible3d) {
        var floorY = landing.floorY - FP_EYE_HEIGHT + 0.02;
        landingMarker3d.position.set(landing.pos.x, floorY, landing.pos.z);
        landingBeam3d.position.set(landing.pos.x, floorY + VOXEL * 1.3, landing.pos.z);
      }
      var report = show ? { ok: visible3d, relocated: !!(landing && landing.relocated), cause: landing ? landing.cause : null, layerName: landing ? landing.layerName : null, layerKey: landing ? landing.layerKey : null, depthKm: landing ? landing.depthKm : null, tempC: landing ? landing.tempC : null, fluidName: landing ? (landing.fluidName || null) : null } : null;
      var reportKey = JSON.stringify(report);
      if (reportKey !== landingReportKey) { landingReportKey = reportKey; if (opts.onFpLanding) { try { opts.onFpLanding(report); } catch (e) {} } }
    }
    function hudNode3d(selector) {
      var root = container.parentNode;
      try { return root && root.querySelector ? root.querySelector(selector) : null; } catch (e) { return null; }
    }
    function projectToOverlay3d(wx, wy, wz) {                 // world → CSS px inside the viewport, or null when behind the camera
    ...
    function updateLandingPin3d() {                            // "you'll land here" label follows the ring as the camera orbits
      var node = hudNode3d('[data-geology-landing-pin]'); if (!node) return;
      if (fp.active || !landingMarker3d.visible) { node.style.display = 'none'; return; }
      var at = projectToOverlay3d(landingMarker3d.position.x, landingMarker3d.position.y + VOXEL * 1.9, landingMarker3d.position.z);
      var cta = hudNode3d('[data-geology-drop-in-cta]');
      placeOverlayNode3d(node, at, -(node.offsetWidth || 120) / 2, -(node.offsetHeight || 28), cta ? cta.offsetTop + cta.offsetHeight + 6 : 4);   // never stacked under the call to action
    }
    function updateHighlightAnchor3d() {                       // topmost front-face voxel of the highlighted layer (distinct per shell in radial scenes)
      highlightAnchor3d = null;
      if (!highlightKey) return;
      var best = null;
      for (var k = 0; k < voxels.length; k++) {
        var v = voxels[k]; if (v.key !== highlightKey || !visible(v)) continue;
        if (!best || v.z > best.z || (v.z === best.z && v.y < best.y)) best = v;
      }
      if (!best) return;
      var xs = [];
      for (var j = 0; j < voxels.length; j++) { var u = voxels[j]; if (u.key === highlightKey && u.z === best.z && u.y === best.y && visible(u)) xs.push(u.x); }
      xs.sort(function (a, b) { return a - b; });
      var p = worldPos({ x: xs[Math.floor(xs.length / 2)], y: best.y, z: best.z });
      highlightAnchor3d = { x: p[0], y: p[1] + VOXEL * 0.5, z: p[2] + VOXEL * 0.5 };
    }
    ...
    function updateHoverCard3d(v, e) {                          // orbit-mode peek: name · type · depth · temperature, next to the pointer
      var node = hudNode3d('[data-geology-hover-card]'); if (!node) return;
      if (!v || fp.active) { if (node.style.display !== 'none') node.style.display = 'none'; hoverCardKey3d = ''; return; }
      var cardKey = v.key + '@' + v.y;
      if (cardKey !== hoverCardKey3d) {
        hoverCardKey3d = cardKey;
        var f = rockFacts(v.key, v.y), R = f.R || {};
        ...
        if (typeNode) typeNode.textContent = R.type || '';
        if (metaNode) metaNode.textContent = (f.measurements && f.measurements.length) ? f.measurements.map(function (row) { return row.label + ' ' + row.value; }).join(' · ') : ('Depth ≈ ' + f.depthKm + ' km · ' + temperatureValue(f.tempC));
        node.dataset.hoverKey = v.key;
      }
      var rect = cnv.getBoundingClientRect();
      placeOverlayNode3d(node, { x: e.clientX - rect.left, y: e.clientY - rect.top }, 16, 14);
    }
    function fpBail(reason) {
    ...
    function fpReseatIfUnsupported(cause) {
      // A cutaway / lens / history change while walking can delete the floor underfoot.
      if (!fp.active || fp.mode !== 'mine') return;
      if (fpGroundEyeY(fp.pos.x, fp.pos.z, WORLD.h * 0.5 + VOXEL) != null && !fpBodyBlocked(fp.pos.x, fp.pos.y, fp.pos.z)) return;
      fp.safePose = null;
    ...
    function fpWalkStep(dt, fwd) {
      ...
      if (fp.pos.y < -WORLD.h * 0.5 - VOXEL * 3) {
        // Every respawn lands on validated ground, so a third straight fall-out without touching
        // ground means the world has no floor left for us: leave rather than loop.
        fp.fallLoopCount = (fp.fallLoopCount || 0) + 1;
    ...
    function fpSetMiningProgress(progress, active) {
      ...
      var boxScale = (fp.mining && fp.mining.plan) ? fp.mining.plan.boxScale : (fp.targetBoxScale || 1);
      hoverBox.scale.setScalar(boxScale * (active ? 1 + p * 0.035 : 1)); hoverBox.material.opacity = active ? 0.72 + p * 0.25 : 0.85;
      miningCrackBox.visible = !!(active && !fp.reduced && fp.mining);
      if (miningCrackBox.visible) {
        var wp = fp.mining.plan ? fp.mining.plan.world : worldPos(fp.mining.voxel), stage = Math.max(1, fpMiningStage(p, miningCrackPattern.length));
        miningCrackBox.position.set(wp[0], wp[1], wp[2]); miningCrackBox.scale.setScalar(boxScale);
    ...
    // Where a strike bites: a little way into the rock along the face the reticle hit, sized
    // by the rock's hardness and the tool. Sub-grid coordinates plus the world point.
    function fpStrikePlan(hit, v, tool) {
      ...
      if (n[1] === -1 && fp.active && fp.mode === 'mine') {
        // Striking the ground inside your own footprint digs DOWN (see digShaft).
        var footR = FP_RADIUS / SUBV, px = (fp.pos.x / VOXEL + NX / 2) * DIG_SUB, pz = (fp.pos.z / VOXEL + NZ / 2) * DIG_SUB;
        var feetRow = Math.floor((NY / 2 - (fp.pos.y - FP_EYE_HEIGHT - 0.001) / VOXEL) * DIG_SUB);
        if ((c[0] - px) * (c[0] - px) + (c[2] - pz) * (c[2] - pz) <= (footR + 1.2) * (footR + 1.2) && hit.g[1] - feetRow <= 4) {
          // from the feet down past the aimed row, so a strike made mid-drop still bites rock
          // bite depth follows hardness: loose/layered 3 rows, dense 2, hard or crystalline 1
          shaft = { cx: px, cz: pz, top: feetRow, rows: Math.max(strike.r >= 1.6 ? 3 : (strike.r >= 1.45 ? 2 : 1), hit.g[1] + 2 - feetRow), r: footR + 0.9 };
          c = [px, feetRow + shaft.rows / 2, pz]; r = shaft.r;
        }
      }
      return { centre: c, radius: r, shaft: shaft, ms: strike.ms, world: [(c[0] / DIG_SUB - NX / 2) * VOXEL, (NY / 2 - c[1] / DIG_SUB) * VOXEL, (c[2] / DIG_SUB - NZ / 2) * VOXEL], boxScale: Math.max(0.3, Math.min(1.15, (r * 2 + 0.4) / DIG_SUB)) };
    }
    // The reticle marches the small-voxel grid from the eye; the first rock within reach is the
    // target (water is looked through, as before). fp.targetHit keeps the exact small voxel and
    // entry face so a strike lands where the player aimed.
    function fpTargetSolid(gx, gy, gz) {
      if (!subSolidG(gx, gy, gz)) return false;
      return fpMaterialPhysics(cellAt((gx / DIG_SUB) | 0, (gy / DIG_SUB) | 0, (gz / DIG_SUB) | 0).key).kind !== 'fluid';
    }
    function fpTargetVoxel() {
      ...
      var o = raycaster.ray.origin, dir = raycaster.ray.direction, k = DIG_SUB / VOXEL;
      ...
      if (v) {
        ...
        hoverBox.position.set(plan.world[0], plan.world[1], plan.world[2]); hoverBox.scale.setScalar(plan.boxScale); hoverBox.visible = true;
    ...
    function coreRigSupportTop3d(wx3d, wz3d, maxTop3d) {
      ...
      for (var supportY3d = 0; supportY3d < NY; supportY3d++) {
        ...
        if (!supportVoxel3d || removed[vkey(supportVoxel3d)] || damaged[vkey(supportVoxel3d)] || !visible(supportVoxel3d) || fpMaterialPhysics(supportVoxel3d.key).kind !== 'solid') continue;
    ...
    function coreRigStablePad3d(cx3d, cz3d, maxTop3d, yaw3d) {
      ...
      if (!centreSupport3d || removed[vkey(centreSupport3d)] || damaged[vkey(centreSupport3d)] || !visible(centreSupport3d) || fpMaterialPhysics(centreSupport3d.key).kind !== 'solid') return null;
...
function updateCoreRig3d(dt3d) {
    ...
    function fpCompleteMining() {
      ...
      if (mining.tool === 'drill' && mining.duration === 0) {
        fp.drillHeat = Math.min(1, fp.drillHeat + fpDrillHeatRate(mining.profile) * mining.plan.ms / 1000);
      }
      var result = carveStrike3d(mining.plan, mining.voxel);
      if (result) { geoSfxStrike(mining.profile.label, mining.tool === 'drill' ? 0.6 : 1); geoSfxCrumble(mining.profile.label, Math.min(1, result.subs / 18)); }
    ...
    function fpUpdateMining(dt) {
      if (!fp.mining) return;
      // Keep swinging at the same spot while the aim stays near it (the eye bobs); aiming
      // somewhere else or at another block cancels the swing.
      var aimHit = fp.targetHit, g0 = fp.mining.g;
      if (!aimHit || Math.abs(aimHit.g[0] - g0[0]) + Math.abs(aimHit.g[1] - g0[1]) + Math.abs(aimHit.g[2] - g0[2]) > 3) { fpCancelMining(); return; }
    ...
    function fpMineAtCrosshair(instant, chained) {
      ...
      var plan = fpStrikePlan(fp.targetHit, v, fp.tool);
      fp.drillNextAt = now + Math.max(90, plan.ms) + (fp.tool === 'pick' ? 70 : 0);   // a held pick keeps a swing rhythm
      fp.mining = { id: 'g' + fp.targetHit.g.join(','), g: fp.targetHit.g.slice(), plan: plan, voxel: v, elapsed: 0, duration: (instant || fp.reduced) ? 0 : plan.ms, profile: profile, tool: fp.tool };
    ...
    function undoableCount() { var n = 0; for (var hi = 0; hi < excavationHistory.length; hi++) { if (!(excavationHistory[hi] && excavationHistory[hi].b)) n++; } return n; }
    var notifyTimer3d = null, notifyAt3d = 0;
    function notifyExcavationChange() {
      if (notifyTimer3d) { clearTimeout(notifyTimer3d); notifyTimer3d = null; }
      notifyAt3d = (window.performance && performance.now) ? performance.now() : Date.now();
      if (opts.onExcavateChange) opts.onExcavateChange(undoableCount(), { history: excavationHistory.slice(), redo: excavationRedo.slice() });
    }
    // Strikes land several times a second; tell the host (a full React render + save) at most
    // every 400 ms, always ending on the latest state.
    function notifyExcavationChangeSoon() {
      var now = (window.performance && performance.now) ? performance.now() : Date.now();
      if (!notifyTimer3d && now - notifyAt3d > 400) { notifyExcavationChange(); return; }
      if (!notifyTimer3d) notifyTimer3d = setTimeout(function () { notifyTimer3d = null; if (!eng.disposed) notifyExcavationChange(); }, 400);
    }
    function updateUndoPreview() {
      var target = null;
      if (undoPreviewRequested && !fp.active) {
        // strikes preview the first cell they touched; a partly-dug cell counts as dug
        var previewIds = [], touched = Object.assign({}, removed);
        Object.keys(damaged).forEach(function (id) { touched[id] = 1; });
        excavationHistory.forEach(function (entry) { if (typeof entry === 'string') previewIds.push(entry); else if (entry && entry.c) previewIds.push(Object.keys(entry.c)[0]); });
        target = undoPreviewTarget(previewIds, voxelByKey, touched, sliceZ, focusLens, showStage, FORMED_AT, NZ);
      }
    ...
    function pushHistory3d(entry) {
      excavationHistory.push(entry); excavationRedo = [];
      if (excavationHistory.length > DIG_HISTORY_MAX) excavationHistory = digFoldHistory(excavationHistory, DIG_HISTORY_MAX, DIG_HISTORY_KEEP);
    }
    function excavateVoxel(v, method) {
      if (!v || v.key === 'void') return null;
      var id = vkey(v); if (removed[id]) return null;
      var partial = damaged[id] ? digMaskHex(damaged[id]) : null;   // a whole-cell dig of a half-dug cell must undo back to the half
      spawnExcavationBurst3d(v); spawnDebris3d([worldPos(v)], (SCENE.palette[v.key] || ROCKS[v.key] || { color: 0xb7a58f }).color, 5, VOXEL * 0.5);
      removed[id] = 1; delete damaged[id];
      if (partial) { var wholeEntry = { c: {} }; wholeEntry.c[id] = [partial, '0']; pushHistory3d(wholeEntry); } else pushHistory3d(id);
      ...
      var cellSpecimen = specimenByCell3d[id];                 // digging the whole cell takes whatever it held (if not already dug out)
      if (cellSpecimen && !cellSpecimen.taken && (!partial || digMaskFromHex(partial)[digSubIndex(cellSpecimen.g[0] - v.x * DIG_SUB, cellSpecimen.g[1] - v.y * DIG_SUB, cellSpecimen.g[2] - v.z * DIG_SUB)])) collectSpecimen3d(cellSpecimen);
    ...
    // One first-person strike: carve a ragged crater of small voxels around plan.centre.
    // Only solid, mineable rock that is currently shown can go (never water, magma or a
    // cutaway-hidden cell). Every touched cell records its before/after mask as one undo step.
    function carvableSub3d(gx, gy, gz) {
      if (!subSolidG(gx, gy, gz)) return false;
    ...
    function carveStrike3d(plan, aimedCell) {
      ...
      // A specimen still buried when the swing lands is left standing in the fresh wall, so the
      // explorer always sees a find before the next strike digs it free.
      bites = bites.filter(function (g) {
        var sp = specimenByG3d[g.join(',')]; if (!sp || sp.taken) return true;
        return SPECIMEN_FACES3d.some(function (f) { return !subSolidG(g[0] + f[0], g[1] + f[1], g[2] + f[2]); });
      });
      if (!bites.length) return null;
      var touched = {}, order = [], points = [], takenSpecimens = [];
      bites.forEach(function (g) {
        var sp = specimenByG3d[g.join(',')]; if (sp && !sp.taken) takenSpecimens.push(sp);
        var cx = (g[0] / DIG_SUB) | 0, cy = (g[1] / DIG_SUB) | 0, cz = (g[2] / DIG_SUB) | 0, cell = cellAt(cx, cy, cz), id = cell.id;
        if (!touched[id]) { touched[id] = { cell: cell, before: digCellState(damaged[id]) }; order.push(id); if (!damaged[id]) { damaged[id] = new Uint8Array(DIG_SUB3); damaged[id].fill(1); } }
        damaged[id][digSubIndex(g[0] - cx * DIG_SUB, g[1] - cy * DIG_SUB, g[2] - cz * DIG_SUB)] = 0;
        if (points.length < 24) points.push(subCentreWorld(g[0], g[1], g[2]));
      });
      var entry = { c: {} }, firstTouch = [], cleared = [];
      order.forEach(function (id) {
        var after = digCellState(damaged[id]);
        if (after === '0') { delete damaged[id]; removed[id] = 1; cleared.push(touched[id].cell); }
        entry.c[id] = [touched[id].before, after];
        if (touched[id].before === 'F') firstTouch.push(touched[id].cell);
      });
      pushHistory3d(entry);
      rebuild(true); notifyExcavationChangeSoon();
      var aimedColor = (SCENE.palette[aimedCell.key] || ROCKS[aimedCell.key] || { color: 0xb7a58f }).color;
      spawnExcavationBurst3d(aimedCell, plan.world, plan.boxScale);
      spawnDebris3d(points, aimedColor, Math.min(5, 2 + Math.round(bites.length / 10)), SUBV * 0.6, fp.active ? fpForward(fp.yaw, fp.pitch) : null);
      takenSpecimens.forEach(collectSpecimen3d);               // a fossil or crystal that broke free with this bite
      // A cell broken into for the first time yields a field sample (field runs, journal).
      firstTouch.forEach(function (cell) {
        var name = (SCENE.palette[cell.key] || ROCKS[cell.key] || { name: cell.key }).name;
        if (opts.onExcavate) opts.onExcavate({ removedKey: cell.key, removedY: cell.y, name: name, exposedKey: cell.key, count: undoableCount(), firstPerson: !!fp.active, method: 'strike' });
      });
      cleared.forEach(function (cell) { if (surveyVoxelKey === cell.id) { surveyBox.visible = false; surveyVoxelKey = null; } });
      if (cleared.length && opts.onSelect) {                    // a cell dug right through: show what lies beneath it
        var last = cleared[cleared.length - 1], below = shallowest(last.x, last.z), belowCell = below == null ? null : cellAt(last.x, below, last.z);
        if (belowCell) opts.onSelect(rockFacts(belowCell.key, belowCell.y));
      }
    ...
    function applyEntry3d(entry, undo) {                         // returns the first cell it touched
      ...
      var first = null;
      Object.keys(entry.c).forEach(function (id) { if (!voxelByKey[id]) return; applyCellState(id, entry.c[id][undo ? 0 : 1]); if (!first) first = voxelByKey[id]; });
      return first;
    }
    function undoExcavation() {
      while (excavationHistory.length) {
        var entry = excavationHistory[excavationHistory.length - 1];
        if (entry && entry.b) break;                           // a folded snapshot is the floor of undo
        excavationHistory.pop();
        if (typeof entry === 'string' && !removed[entry]) continue;   // stale legacy id
        var restored = applyEntry3d(entry, true);
        if (!restored) continue;
        excavationRedo.push(entry); rebuild(); notifyExcavationChange();
        var material = SCENE.palette[restored.key] || ROCKS[restored.key] || { name: restored.key || 'material' };
        return { key: restored.key, y: restored.y, name: material.name, remaining: undoableCount(), redoCount: excavationRedo.length };
      }
      notifyExcavationChange(); return null;
    }
    function redoExcavation() {
      while (excavationRedo.length) {
        var entry = excavationRedo.pop();
    ...
    function onMoveHover(e) {
      ...
      if (down) { if (hoverBox.visible) hoverBox.visible = false; updateHoverCard3d(null); return; }
      ...
      raycaster.setFromCamera(pointer, camera);
      var v = orbitHitCell3d();
      if (v) { var p = worldPos(v); hoverBox.position.set(p[0], p[1], p[2]); hoverBox.scale.setScalar(1); hoverBox.visible = true; if (e.pointerType !== 'touch') updateHoverCard3d(v, e); return; }
      hoverBox.visible = false; updateHoverCard3d(null);
    }
    // The cell under the pointer, whether it is drawn whole or as its remaining small voxels.
    function orbitHitCell3d() {
      var hits = raycaster.intersectObjects(subMesh3d.count ? [mesh, subMesh3d] : [mesh], false); if (!hits.length) return null;
      var hit = hits[0];
      if (hit.object === subMesh3d) { var sub = instanceToSub3d[hit.instanceId]; return sub ? sub.cell : null; }
      return instanceToVoxel[hit.instanceId] || null;
    }
    function onLeaveHover() { hoverBox.visible = false; updateHoverCard3d(null); if (fp.active) { down = null; fpPrev = null; } }   // dropping the look-drag off the canvas ends it cleanly
    ...
    function exitFP(silentRigAbort3d) {
      ...
      if (notifyTimer3d) notifyExcavationChange();             // the orbit view's undo count reflects every strike
    ...
    function applyFP(dt) {
      ...
      if (now - fp.lastHud > 110) {                           // ~9Hz HUD sample; only push on a layer change
        fp.lastHud = now;
        // Walking: the rock underfoot (matches the depth badge; in a dug shaft the eye is still up
        // near the old surface). Swimming or flying: where the eye is.
        var hudY = (fp.mode === 'mine' && fp.medium !== 'fluid') ? fp.pos.y - FP_EYE_HEIGHT - VOXEL * 0.5 : fp.pos.y;
        var p = fpProbe(fp.pos.x, hudY, fp.pos.z);
    ...
    function loop() {
      ...
      updateExcavationEffects3d(fpDt);
      try { updateDebris3d(fpDt); } catch (e) {}
      ...
      if (fpFrameAt - landingPinAt3d > 50) { landingPinAt3d = fpFrameAt; try { updateLandingPin3d(); updateLayerCallout3d(); } catch (e) {} }
    ...
    eng.setSlice = function (z) {
      ...
      rebuild(); fpReseatIfUnsupported('cutaway');
    };
    ...
    eng.digDebug = function (id) { return { damagedIds: Object.keys(damaged), mask: id && damaged[id] ? digMaskHex(damaged[id]) : null, targetHit: fp.targetHit ? { g: fp.targetHit.g.slice(), t: fp.targetHit.t, normal: fp.targetHit.normal.slice() } : null }; };   // read-only, for the real-browser harness
    eng.fpDebug = function () { return { active: !!fp.active, mode: fp.mode, pos: { x: fp.pos.x, y: fp.pos.y, z: fp.pos.z }, onGround: !!fp.onGround, medium: fp.medium, intro: !!fp.intro, safePose: fp.safePose ? { x: fp.safePose.pos.x, y: fp.safePose.pos.y, z: fp.safePose.pos.z } : null }; };   // test/probe seam: read-only walker state
    eng._fpExit = exitFP;
    eng.dispose = function () {
      ...
      geoSfxDrill(false);
      if (notifyTimer3d) { try { notifyExcavationChange(); } catch (e) {} }   // save the last strikes before this world goes away
      ...
      try {
        ...
        landingRingGeo3d.dispose(); landingRingMat3d.dispose(); landingBeamGeo3d.dispose(); landingBeamMat3d.dispose();
  ...
  window.StemLab.registerTool('geologyExplorer', {
    ...
    render: function (ctx) {
      ...
      // Template form: one key per SENTENCE, values substituted by name, so a translator can
      // reorder the words. Falls back to the English template with the same substitution.
      var tf = function (k, fb, vars) {
        var s = t(k, fb);
        if (vars) for (var name in vars) if (Object.prototype.hasOwnProperty.call(vars, name)) s = s.split('{' + name + '}').join(String(vars[name]));
        return s;
      };
      ...
      var specimensRef = React.useRef(d.specimensFound || {});   // per-scene tally of fossils and crystals dug free
      ...
      function saveCoreRigReport(sceneId, report) {
        ...
        announce(tf('stem.geology.sr.core_log_saved', 'Core log saved. Grade {grade}, {score} points. {samples} samples across {materials} materials. ', { grade: evaluation.grade, score: evaluation.score, samples: summary.sampleCount, materials: summary.uniqueMaterials }) + (evaluation.integrityPercent != null ? tf('stem.geology.sr.core_integrity_percent', '{percent} percent core integrity. ', { percent: evaluation.integrityPercent }) : '') + tf('stem.geology.sr.bore_brief_complete', 'Bore Brief {met} of 3 complete. ', { met: cleanReport.boreBrief.metCount }) + certificationSpeech + completionSpeech + (cleanReport.totalReward ? tf('stem.geology.sr.experience_earned', '{xp} experience earned. ', { xp: cleanReport.totalReward }) : t('stem.geology.sr.no_new_experience', 'No new experience this run. ')) +
          (cleanReport.comparison ? tf('stem.geology.sr.paired_finding', 'Paired finding: {percent} percent sequence match. ', { percent: cleanReport.comparison.similarityPct }) : '') +
          (cleanReport.nextExperiment ? tf('stem.geology.sr.next_experiment_ready', 'Next experiment ready. {question}', { question: cleanReport.nextExperiment.question }) : ''));
        return cleanReport;
      }
      ...
      function surveyFieldTarget(sceneId) {
        var book = fieldBookRef.current || {}, entry = book.byScene && book.byScene[sceneId];
        if (!entry || !entry.active) { announce(t('stem.geology.sr.start_a_field_run_before_using_the_specimen_su', 'Start a Field Run before using the specimen survey.')); return null; }
        if (entry.ready) { announce(t('stem.geology.sr.the_specimen_set_is_complete_return_to_the_ent', 'The specimen set is complete. Return to the entry point to bank it.')); return null; }
        var contract = fieldExpeditionFor(sceneId, entry.contractIndex);
        ...
        if (!result) { announce(t('stem.geology.sr.the_specimen_survey_is_available_while_explori', 'The specimen survey is available while exploring in first person.')); return null; }
        if (result.cooldown) { announce(t('stem.geology.sr.the_survey_pulse_is_recharging', 'The survey pulse is recharging.')); return result; }
        if (!result.found) {
          addToast('Survey could not find an unexcavated ' + fieldSpecimenName(sceneId, targetKey) + ' specimen in this cutaway.', 'info');
          announce(tf('stem.geology.sr.no_unexcavated_specimen_was_found_in_this_cuta', 'No unexcavated {target_key} specimen was found in this cutaway.', { target_key: fieldSpecimenName(sceneId, targetKey) }));
          return result;
        }
        var reading = result.name + ' is about ' + result.distanceBlocks + ' block' + (result.distanceBlocks === 1 ? '' : 's') + ' ' + result.direction + ', ' + result.vertical + '.';
        addToast('Survey pulse: ' + reading + ' Turning you to face it.', 'info'); announce(tf('stem.geology.sr.survey_pulse_your_view_now_faces_it', 'Survey pulse. {reading} Your view now faces it.', { reading: reading }));
        return result;
      }
      function coreRigAction(action, value) {
        var engine = window[ENGINE_KEY];
        if (!engine || !coreRigSupported(scene)) { announce(t('stem.geology.sr.the_directional_core_rig_is_available_in_surfa', 'The directional core rig is available in surface Walk and Dig expeditions.')); return null; }
      ...
      function reviewCoreRigReport(report) {
        ...
        announce(tf('stem.geology.sr.reviewing_grade_bore_points_degrees_and_interv', 'Reviewing grade {grade} bore, {score} points, {angle} degrees and {target_depth} intervals.', { grade: evaluation.grade, score: evaluation.score, angle: coreRigAngleDegrees(report.angle), target_depth: report.targetDepth }));
        return true;
      }
      function applyCoreRigTrajectory(angle, depth) {
        ...
        if (engineState && (engineState.running || engineState.stage === 'deploying')) {
          announce(t('stem.geology.sr.end_the_active_bore_before_loading_a_new_traje', 'End the active bore before loading a new trajectory.'));
          return null;
        }
      ...
      function loadCoreRigProgram(program, experiment) {
        ...
        if (!catalogProgram) {
          announce(t('stem.geology.sr.that_core_rig_certification_program_is_unavail', 'That core rig certification program is unavailable.'));
          return false;
        }
      ...
      function browserReadAloud(text, expectedSession) {
        ...
        if (!window.speechSynthesis || typeof window.SpeechSynthesisUtterance !== 'function') {
          setTtsSpeaking(false); announce(t('stem.geology.sr.read_aloud_is_not_available_in_this_browser', 'Read aloud is not available in this browser.'));
          return;
        }
        try {
        ...
        } catch (e) { setTtsSpeaking(false); announce(t('stem.geology.sr.read_aloud_could_not_start', 'Read aloud could not start.')); }
      }
      ...
      function readAloudButton(text, key, label) {
        return h('button', { key: key, type: 'button', 'data-geology-read-aloud': key, 'aria-label': ttsSpeaking ? 'Stop reading aloud' : label, onClick: function () { if (ttsSpeaking) stopReadAloud(); else if (ttsAvailable) readAloud(text); else announce(t('stem.geology.sr.read_aloud_is_not_available_in_this_browser', 'Read aloud is not available in this browser.')); }, className: 'shrink-0 rounded-md border px-2 py-1 text-[10px] font-bold ' + btnIdle }, ttsSpeaking ? 'Stop reading' : 'Read aloud');
      }
      React.useEffect(function () {
      ...
      function setEvidenceRole(itemId, role, itemLabel) {
        ...
        announce(clearing ? t('stem.geology.sr.evidence_mapping_cleared', 'Evidence mapping cleared.') : tf('stem.geology.sr.mapping_saved_for', '{role} mapping saved for {item}.', { role: roleDef.label, item: (itemLabel || itemId) }));
      }
      ...
      function takeCore(site) {
        ...
        announce(tf('stem.geology.sr.core_sample_top_to_bottom', 'Core sample, {label}. Top to bottom: {join}. {blurb}', { label: site.label, join: segs.map(function (s) { return ROCKS[s.key].name; }).join(', '), blurb: site.blurb }));
      }
      ...
      React.useEffect(function () {
        ...
        function set(e, on) {
          ...
          if (on && (key === '1' || key === '2')) {
            ...
            announce(nextTool === 'drill' ? t('stem.geology.sr.powered_drill_selected', 'Powered drill selected.') : t('stem.geology.sr.pickaxe_selected', 'Pickaxe selected.'));
            return;
          }
          if (on && key === 'z') {
            e.preventDefault(); if (e.repeat) return;
      ...
      function toggleFocusLens() {
        ...
        announce(next ? tf('stem.geology.sr.focus_lens_on', 'Focus lens on. Showing {name} apart from surrounding materials.{paused}', { name: selected.R.name, paused: pausedDigging ? ' ' + t('stem.geology.sr.focus_lens_paused_digging', 'Excavation was turned off while surrounding layers are hidden.') : '' }) : t('stem.geology.sr.focus_lens_off', 'Focus lens off. Full scene context restored.'));
      }
      ...
      function layerSitsLines(f) {
        if (!f || !f.key) return [];
        var ext = layerExtent(f.key), R = f.R || {}, lines = [], palette = SCENE.palette || ROCKS;
        var nameOf = function (k) { return (palette[k] && palette[k].name) || (ROCKS[k] && ROCKS[k].name) || k; };
        var geodeScale = SCENE.id === 'geode';
        var fmt = function (km) { return geodeScale ? ((km * 1000).toFixed(2) + ' m') : (km.toFixed(1) + ' km'); };
        ...
        if (ext.count) lines.push(tf('stem.geology.sits_share', 'About {pct}% of this block', { pct: Math.max(1, Math.round(ext.share * 100)) }));
        if (ext.radial && (ext.above || ext.below)) {
          lines.push(ext.above && ext.below
            ? tf('stem.geology.sits_shell_between', 'Just inside {above} · wraps around {below}', { above: nameOf(ext.above), below: nameOf(ext.below) })
            : (ext.above ? tf('stem.geology.sits_shell_inner', 'The innermost shell, inside {above}', { above: nameOf(ext.above) }) : tf('stem.geology.sits_shell_outer', 'The outermost shell, wrapped around {below}', { below: nameOf(ext.below) })));
        } else if (ext.above || ext.below) {
          lines.push(ext.above && ext.below
            ? tf('stem.geology.sits_between', 'Sits under {above} and over {below}', { above: nameOf(ext.above), below: nameOf(ext.below) })
            : (ext.above ? tf('stem.geology.sits_under', 'Sits under {above}', { above: nameOf(ext.above) }) : tf('stem.geology.sits_over', 'At the surface, over {below}', { below: nameOf(ext.below) })));
        } else if (ext.count && !ext.radial) lines.push(t('stem.geology.sits_surface', 'Exposed at the surface'));
        return lines;
      }
      function infoPanel() {
        ...
        var sitsLines = layerSitsLines(f), bust = fpBust(f.key);
        var revealCut = revealCutawayFor(SCENE.id, f.key), revealNeeded = revealCut > 0 && slice < revealCut && threeReady && !webglError;
        ...
        return h('div', { className: 'p-3 rounded-xl border ' + cardBg, style: { borderLeft: '3px solid ' + tc }, role: 'region', 'aria-label': t('stem.geology.a11y.selected_rock_details', 'Selected rock details') },
          ...
          sitsLines.length ? h('div', { className: 'mt-2 rounded-lg border px-2 py-1.5 text-[11.5px] ' + (isDark ? 'border-slate-700 bg-slate-900/60' : 'border-slate-200 bg-slate-50') + ' ' + ink, 'data-geology-layer-sits': f.key },
            h('div', { className: 'text-[10px] font-extrabold uppercase tracking-wide ' + muted }, '📐 ' + t('stem.geology.sits_title', 'Where it sits')),
            sitsLines.map(function (line, i) { return h('div', { key: 'sits-' + i }, line); }),
            revealNeeded ? h('button', { type: 'button', 'data-geology-reveal-layer': f.key, onClick: function () { setSlice(revealCut); try { if (window[ENGINE_KEY]) window[ENGINE_KEY].setSlice(revealCut); } catch (e) {} announce(tf('stem.geology.sr.revealed', 'Cut the block to {pct}% so {name} is on screen.', { pct: cutawayReadout(revealCut, NZ).percent, name: R.name })); }, className: 'mt-1 rounded-md border px-2 py-0.5 text-[10.5px] font-bold ' + (isDark ? 'border-cyan-500/60 bg-cyan-950/60 text-cyan-100 hover:bg-cyan-900/70' : 'border-cyan-500 bg-cyan-50 text-cyan-900 hover:bg-cyan-100') }, '🔍 ' + t('stem.geology.reveal_layer', 'Hidden inside the block — cut it open to see this layer')) : null) : null,
          h('div', { className: 'mt-2 text-[11.5px] ' + (isDark ? 'text-amber-200' : 'text-amber-800') }, '🕓 ' + R.age),
          bust ? h('div', { className: 'mt-1 text-[11.5px] font-semibold ' + (isDark ? 'text-amber-200' : 'text-amber-800'), 'data-geology-layer-bust': f.key }, '💡 ' + bust) : null,
          feat.fossils
            ? (F
          ...
          return h('div', { key: side, className: 'min-w-0 rounded-lg border p-2 ' + (isDark ? 'border-slate-700 bg-slate-900/40' : 'border-slate-200 bg-slate-50'), role: 'group', 'aria-label': tf('stem.geology.a11y.visual_story', '{title} visual story', { title: title })}, [
            ...
            h('div', { key: 'lane-steps', className: 'relative mt-2 space-y-1.5' }, [
              ...
              journey.map(function (item, index) {
                ...
                return h('button', { key: item.key, type: 'button', 'aria-pressed': on ? 'true' : 'false', 'aria-label': tf('stem.geology.a11y.stage', '{title} stage {index}: {label}', { title: title, index: (index + 1), label: item.label }), 'data-geology-comparison-stage': side + ':' + index, onClick: function () { setCompareStage(index); announce(tf('stem.geology.sr.comparing_stage_with_the_matching_stage_in_the', 'Comparing stage {index}: {label} with the matching stage in the other scene.', { index: (index + 1), label: item.label })); }, className: 'relative flex w-full items-start gap-2 rounded-md border px-2 py-1.5 text-left transition-colors ' + (on ? ('border-' + accent + '-600 bg-' + accent + '-700 text-white') : (isDark ? 'border-slate-700 bg-slate-950/40 text-slate-200 hover:border-' + accent + '-400' : 'border-slate-200 bg-white text-slate-700 hover:border-' + accent + '-400')) }, [
      ...
      function historyBar() {
        ...
        return h('div', { className: 'p-2.5 rounded-xl border-2 ' + (isDark ? 'bg-amber-950/40 border-amber-700/60' : 'bg-amber-50 border-amber-300'), role: 'region', 'aria-label': tf('stem.geology.a11y.formation_history_step_of', 'Formation history, step {hist_stage} of {length}', { hist_stage: (histStage + 1), length: HISTORY.length })},
      ...
      function eruptionBar() {
        ...
        return h('div', { className: 'p-2.5 rounded-xl border-2 ' + (isDark ? 'bg-orange-950/40 border-orange-700/60' : 'bg-orange-50 border-orange-300'), role: 'region', 'aria-label': tf('stem.geology.a11y.volcanic_eruption_step_of', 'Volcanic eruption, step {erupt_stage} of {length}', { erupt_stage: (eruptStage + 1), length: ERUPT.length })},
          h('div', { className: 'flex items-center justify-between gap-2 mb-1.5' },
      ...
      function selectSequenceCard(key) {
        ...
        if (!sequenceTapKey) {
          ...
          announce(tf('stem.geology.sr.selected_tap_place_here_on_another_card_to_mov', 'Selected {label}. Tap Place here on another card to move it before that card.', { label: item.label }));
          return;
        }
        if (sequenceTapKey === key) {
          setSequenceTapKey(null);
          announce(t('stem.geology.sr.touch_reorder_canceled', 'Touch reorder canceled.'));
          return;
        }
        ...
        setSequenceTapKey(null);
        invalidateSequenceCompletion();
        announce(tf('stem.geology.sr.moved_before', 'Moved {label} before {label_2}.', { label: moving.label, label_2: item.label }));
      }
      ...
      function resetSequenceOrder() {
        ...
        invalidateSequenceCompletion();
        announce(t('stem.geology.sr.sequence_order_reset_arrange_the_cards_from_ea', 'Sequence order reset. Arrange the cards from earliest to latest.'));
      }
          ...
          evidence.length
            ? h('div', { key: 'items', className: 'mt-2 space-y-2' }, evidence.map(function (item) {
                ...
                return h('div', { key: item.id, className: 'rounded-lg border p-2 ' + (isDark ? 'border-slate-700' : 'border-slate-200'), 'data-geology-evidence-item': item.id }, [
                  ...
                  h('div', { key: 'roles', className: 'mt-1.5 flex flex-wrap gap-1', role: 'group', 'aria-label': tf('stem.geology.a11y.map', 'Map {label}', { label: item.label })}, EVIDENCE_MAP_ROLES.map(function (role) {
        ...
        function coreStrip(laneId, label, core) {
          ...
          return h('div', {
            ...
            bands.length ? h('div', { key: 'scroll', className: 'overflow-x-auto pb-1' },
              h('ol', {
                className: 'flex gap-1', 'aria-label': tf('stem.geology.a11y.recovered_formations', '{label} recovered formations', { label: label }),
                ...
                'aria-label': tf('stem.geology.a11y.has_fewer_recovered_intervals_on_the_shared', '{label} has {remainder_intervals} fewer recovered intervals on the shared scale', { label: label, remainder_intervals: remainderIntervals })}, [
                h('span', {
      ...
      function fieldJournalPanel() {
        ...
        function toggleJournal() {
          ...
          announce(next ? t('stem.geology.sr.specimen_journal_expanded', 'Specimen journal expanded.') : t('stem.geology.sr.specimen_journal_collapsed', 'Specimen journal collapsed.'));
        }
                        ...
                        return h('tr', { key: angle }, [
                          ...
                          return h('td', { key: programKey, className: 'p-0.5' },
                            h('button', {
                              ...
                              'aria-label': tf('stem.geology.a11y.degree_interval_certification_program', '{angle_degrees} degree, {depth} interval certification program. {spoken_status}.', { angle_degrees: cell.angleDegrees, depth: cell.depth, spoken_status: spokenStatus }),
                              title: spokenStatus,
                              onClick: function () {
                                setCoreRigProgramSelection(programKey);
                                announce(tf('stem.geology.sr.degree_interval_program_selected', '{angle_degrees} degree, {depth} interval program selected. {spoken_status}.', { angle_degrees: cell.angleDegrees, depth: cell.depth, spoken_status: spokenStatus }));
                              },
      ...
      function setCameraView(view) {
        ...
        setCameraViewState(next); announce(tf('stem.geology.sr.camera_view', 'Camera view: {next}.', { next: cameraViewLabel(next) }));
        ...
        return h('section', { className: 'rounded-xl border ' + cardBg, role: 'region', 'aria-label': t('stem.geology.a11y.formation_timeline', 'Formation timeline'), 'data-geology-formation-timeline': 'true' },
          h('div', { className: 'p-3' }, [
            ...
            h('div', { key: 'stages', className: 'mt-2 grid grid-cols-3 gap-1.5' }, timeline.map(function (item, index) {
              var on = index === activeIndex;
              return h('button', { key: item.key, type: 'button', 'aria-pressed': on ? 'true' : 'false', 'aria-label': tf('stem.geology.a11y.timeline_stage', 'Timeline stage {index}: {cue_label}', { index: (index + 1), cue_label: item.cueLabel }), 'data-geology-timeline-stage': item.key, onClick: function () { chooseStage(index); }, className: 'min-w-0 rounded-lg border px-1.5 py-2 text-center transition-colors ' + (on ? 'border-sky-600 bg-sky-700 text-white shadow-sm' : btnIdle) }, [
        ...
        return h('section', { className: 'rounded-xl border ' + cardBg, role: 'region', 'aria-label': t('stem.geology.a11y.camera_orientation', 'Camera orientation'), 'data-geology-camera-orientation': 'true' },
          h('div', { className: 'flex flex-wrap items-center justify-between gap-2 p-3' }, [
            ...
            h('div', { key: 'controls', className: 'flex flex-wrap gap-1.5', role: 'group', 'aria-label': t('stem.geology.a11y.camera_orientation_choices', 'Camera orientation choices') }, views.map(function (item) {
              ...
              return h('button', { key: item[0], type: 'button', 'aria-pressed': on ? 'true' : 'false', 'aria-label': tf('stem.geology.a11y.set_camera_view', 'Set camera view: {item}', { item: item[1] }), onClick: function () { setCameraView(item[0]); }, className: 'rounded-md border px-2 py-1 text-[10px] font-bold ' + (on ? 'border-sky-600 bg-sky-700 text-white' : btnIdle) }, item[1]);
            }))
          ]));
      }
      ...
      function activateBeacon(beacon) {
        ...
        announce(tf('stem.geology.sr.beacon_focus', '{label}. {detail}{cut}', { label: beacon.label, detail: beacon.detail, cut: beaconReveal ? ' ' + t('stem.geology.sr.cut_back_to_expose', 'Cut back to expose it.') : '' }));
      }
      ...
      function startBeaconTour() {
        ...
        announce(tf('stem.geology.sr.beacon_tour_started_stop_1_of', 'Beacon tour started. Stop 1 of {length}: {label}.', { length: items.length, label: items[0].label }));
      }
      function moveBeaconTour(delta) {
        ...
        announce(tf('stem.geology.sr.beacon_tour_stop_of', 'Beacon tour stop {next} of {length}: {label}.', { next: (next + 1), length: items.length, label: items[next].label }));
      }
      function exitBeaconTour() { setBeaconTourOn(false); setBeaconTourStep(0); announce(t('stem.geology.sr.beacon_tour_closed_you_can_still_choose_any_la', 'Beacon tour closed. You can still choose any landmark.')); }
      ...
      function viewport() {
        ...
        function fpAction(name) {
          ...
          try {
            ...
            if (name === 'mine' && E.fpMine) result = E.fpMine(false);
              ...
              setFpTool(nextTool); upd('fpTool', nextTool); announce(nextTool === 'drill' ? t('stem.geology.sr.powered_drill_selected', 'Powered drill selected.') : t('stem.geology.sr.pickaxe_selected', 'Pickaxe selected.'));
            }
            ...
            h('div', { key: 'body', className: 'p-2' }, [
              h('header', { key: 'head', className: 'relative sticky top-0 z-20 -mx-2 -mt-2 flex flex-wrap items-center justify-between gap-2 border-b border-white/10 bg-slate-950/95 px-2 pb-1.5 pt-3 backdrop-blur-md' }, [
                ...
                h('ol', {
                  ...
                  return h('li', {
                    ...
                    'aria-label': tf('stem.geology.a11y.phase', '{phase} phase, {phase_state}', { phase: phase[1], phase_state: phaseState }),
              ...
              rigPreview ? h('div', { key: 'config', 'data-geology-core-phase-surface': 'setup', className: 'mt-2 grid grid-cols-1 gap-2 sm:grid-cols-[1fr_auto]' }, [
                h('div', { key: 'angles', role: 'group', 'aria-label': t('stem.geology.a11y.bore_angle', 'Bore angle'), className: 'grid grid-cols-3 gap-1' }, [
                  ...
                  return h('button', { key: angleOption[0], type: 'button', disabled: rigLocked, 'data-geology-core-rig-angle': angleOption[0], 'aria-pressed': angleActive ? 'true' : 'false', 'aria-label': tf('stem.geology.a11y.bore', '{angle_option} {angle_option_2} bore', { angle_option: angleOption[2], angle_option_2: angleOption[0] }), onClick: function () { coreRigAction('angle', angleOption[0]); }, className: 'min-h-11 rounded-md border px-1 text-[11px] font-extrabold transition disabled:cursor-not-allowed disabled:opacity-45 ' + (angleActive ? 'border-cyan-300 bg-cyan-400/25 text-cyan-50 shadow-[0_0_10px_rgba(34,211,238,.2)]' : 'border-slate-600 bg-slate-900/70 text-slate-300 hover:border-cyan-500') }, angleOption[1] + ' ' + angleOption[2]);
                })),
                h('div', { key: 'depths', role: 'group', 'aria-label': t('stem.geology.a11y.target_core_depth', 'Target core depth'), className: 'grid grid-cols-3 gap-1' }, CORE_RIG_DEPTHS.map(function (depthOption) {
                  ...
                  return h('button', { key: depthOption, type: 'button', disabled: rigLocked, 'data-geology-core-rig-depth': depthOption, 'aria-pressed': depthActive ? 'true' : 'false', 'aria-label': tf('stem.geology.a11y.interval_target_depth', '{depth_option} interval target depth', { depth_option: depthOption }), onClick: function () { coreRigAction('depth', depthOption); }, className: 'min-h-11 min-w-8 rounded-md border px-1 text-[11px] font-extrabold disabled:cursor-not-allowed disabled:opacity-45 ' + (depthActive ? 'border-amber-300 bg-amber-400/25 text-amber-100' : 'border-slate-600 bg-slate-900/70 text-slate-300') }, String(depthOption));
                }))
              ]) : null,
        ...
        // Prominent drop-in pad. It reads the engine's landing report so the learner knows WHERE
        // they will land before they commit, and when nothing is standable it offers the fix
        // (reset the cutaway / lens) instead of a silent fall.
        function dropInCta() {
          if (fpOn) return null;
          var landing = fpLanding, ok = !landing || landing.ok !== false, relocated = !!(landing && landing.relocated);
          var landName = landing && landing.layerName ? landing.layerName : null, cause = landing ? landing.cause : null;
          ...
          else if (firstTime && scene === 'geode') sub = tf('stem.geology.fp_first_sub_geode', 'Land on {name} inside the cavity and mine the crystal lining', { name: landName || t('stem.geology.fp_floor', 'floor') });
          else if (firstTime) sub = landName ? tf('stem.geology.fp_first_sub_on', 'Land on {name}, dig down through the layers, feel the heat rise', { name: landName }) : t('stem.geology.fp_first_sub', 'Land on the surface, dig down through the layers, feel the heat rise');
          else if (landName) sub = tf('stem.geology.fp_lands_on', 'Lands on {name}', { name: landName });
          var fix = !ok ? (cause === 'cutaway' ? 'cutaway' : (cause === 'lens' ? 'lens' : null)) : null;
          var visibleLabel = !ok && fix ? (fpWalkScene ? '⛏️ ' : '🛰️ ') + t('stem.geology.fp_reset_and_enter', fix === 'lens' ? 'Turn off the lens & drop in' : 'Reset the cutaway & drop in') : (fpWalkScene ? '⛏️ ' : '🛰️ ') + label;
          function go() {
            if (fix === 'cutaway') { var openCut = sceneOpeningCutaway(scene, NZ); setSlice(openCut); try { if (window[ENGINE_KEY]) window[ENGINE_KEY].setSlice(openCut); } catch (e) {} }
            else if (fix === 'lens' && focusLensOn) toggleFocusLens();
            if (!d.fpUsed) upd('fpUsed', true);
            setFpOn(true);
            setTimeout(function () { try { if (containerRef.current) containerRef.current.focus(); } catch (e) {} }, 0);
          }
          var tone = !ok
            ? 'border-rose-300/80 bg-rose-950/85 text-rose-50 shadow-[0_0_18px_rgba(244,63,94,.28)]'
            : (relocated ? 'border-amber-300/80 bg-amber-950/85 text-amber-50 shadow-[0_0_18px_rgba(251,191,36,.32)]'
              : 'border-amber-200 bg-gradient-to-br from-amber-400 to-amber-500 text-amber-950 shadow-[0_0_20px_rgba(251,191,36,.42)]');   // dark ink: amber-50 on this gradient was ~2:1
          return h('div', { className: 'absolute top-2 left-1/2 z-10 -translate-x-1/2', 'data-geology-drop-in-cta': !ok ? 'blocked' : (relocated ? 'relocated' : 'ready') },
            h('button', { ref: fpToggleRef, type: 'button', 'aria-pressed': 'false', 'aria-label': visibleLabel.replace(/^\S+\s/, '') + (sub ? '. ' + sub : '') + ' (first-person explorer)', onClick: go,
              className: 'min-h-11 max-w-[min(56vw,340px)] rounded-xl border-2 px-3.5 py-1.5 text-left transition-transform active:scale-[0.97] hover:-translate-y-px ' + tone },
              h('span', { className: 'block text-[13px] font-black tracking-tight' }, visibleLabel),
              sub ? h('span', { className: 'block text-[10.5px] font-semibold leading-snug' }, sub) : null));
        }
        ...
        return h('div', Object.assign({ ref: fsRef, className: (isFs ? 'fixed inset-0 z-[9999] overflow-hidden bg-[#060913]' : 'relative rounded-xl overflow-hidden border ' + (isDark ? 'border-slate-700' : 'border-slate-300')) }, isFs ? { role: 'dialog', 'aria-modal': 'true', 'aria-labelledby': 'geology-fullscreen-title', tabIndex: -1, 'data-geology-fullscreen': 'true' } : {}),
          ...
          h('div', { className: 'absolute top-2 left-2 z-10 flex gap-1' },
            [['iso', '3D'], ['front', 'Front'], ['top', 'Top']].map(function (vw) {
              ...
              fpOn ? h('button', { key: 'fp', ref: fpToggleRef, type: 'button', 'aria-pressed': 'true', 'aria-label': 'Exit first-person explorer', onClick: function () { setFpOn(false); }, className: 'min-h-9 transition-colors active:scale-[0.97] text-[10px] font-bold px-2 py-1 rounded-md border bg-emerald-500 border-emerald-400 text-emerald-950' }, '🚪 ' + t('stem.geology.fp_exit', 'Exit')) : null
            ])),
          dropInCta(),
          // "You'll land here" pin: the engine projects it onto the landing ring every frame
          (!fpOn && fpWalkScene) ? h('div', { 'data-geology-landing-pin': 'true', className: 'pointer-events-none absolute z-10 whitespace-nowrap rounded-full border border-amber-300/70 bg-amber-950/85 px-2 py-0.5 text-[10px] font-extrabold text-amber-100 shadow-lg', style: { display: 'none' }, 'aria-hidden': 'true' }, '⛏ ' + (fpLanding && fpLanding.fluidName ? tf('stem.geology.splash_here', 'You splash down here · {name}', { name: fpLanding.fluidName }) : (fpLanding && fpLanding.layerName ? tf('stem.geology.land_here_on', 'You land here · {name}', { name: fpLanding.layerName }) : t('stem.geology.land_here', 'You land here')))) : null,
          // orbit-mode peek card: the engine fills name/type/meta on hover and positions it by the pointer
          !fpOn ? h('div', { 'data-geology-hover-card': 'true', className: 'pointer-events-none absolute z-20 max-w-[230px] rounded-lg border px-2.5 py-1.5 text-[11px] shadow-xl ' + (isDark ? 'bg-slate-900/92 border-slate-600 text-slate-100' : 'bg-white/95 border-slate-300 text-slate-800'), style: { display: 'none' }, 'aria-hidden': 'true' },
            h('div', { 'data-hover-name': 'true', className: 'text-[12px] font-extrabold' }),
            h('div', { 'data-hover-type': 'true', className: 'text-[10px] font-bold ' + muted }),
            h('div', { 'data-hover-meta': 'true', className: 'mt-0.5' }),
            h('div', { className: 'mt-0.5 text-[10px] ' + muted }, t('stem.geology.hover_hint', 'Click to select · full profile in the side panel'))) : null,
          // layer callout: anchored by the engine to the highlighted layer's exposed face
          (!fpOn && selected) ? (function () {
        ...
        h('div', { className: 'grid gap-3 lg:grid-cols-2 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]',
          ...
          h('div', { className: 'space-y-2' },
            ...
            h('div', { className: 'flex flex-wrap items-center gap-2' },
              ...
              inInvestigation && digCount > 0 && h('button', { type: 'button', disabled: histStage >= 0 || eruptStage >= 0 || !threeReady || webglError || focusLensOn, 'data-geology-undo-excavation': 'true', 'data-geology-undo-preview-control': 'true', 'aria-label': tf(digCount === 1 ? 'stem.geology.a11y.undo_excavation_one' : 'stem.geology.a11y.undo_excavation_other', digCount === 1 ? 'Undo last excavation. {n} block removed.' : 'Undo last excavation. {n} blocks removed.', { n: digCount }), title: focusLensOn ? 'Turn off the Focus lens to restore the last block' : 'Hover or focus to preview the block; activate to restore it', onMouseEnter: function () { setUndoPreviewIntent('hover', true); }, onMouseLeave: function () { setUndoPreviewIntent('hover', false); }, onFocus: function () { setUndoPreviewIntent('focus', true); }, onBlur: function () { setUndoPreviewIntent('focus', false); }, onClick: undoLastExcavation, className: btn + (isDark ? 'border-amber-500/70 bg-amber-950/40 text-amber-100 hover:bg-amber-900/60' : 'border-amber-300 bg-amber-50 text-amber-900 hover:bg-amber-100') }, '↶ Undo dig (' + digCount + ')'),
              inInvestigation && redoCount > 0 && h('button', { type: 'button', disabled: histStage >= 0 || eruptStage >= 0 || !threeReady || webglError || focusLensOn, 'data-geology-redo-excavation': 'true', 'aria-label': tf(redoCount === 1 ? 'stem.geology.a11y.redo_excavation_one' : 'stem.geology.a11y.redo_excavation_other', redoCount === 1 ? 'Redo last excavation. {n} block available.' : 'Redo last excavation. {n} blocks available.', { n: redoCount }), title: 'Reapply the most recently undone excavation', onClick: redoLastExcavation, className: btn + (isDark ? 'border-sky-500/70 bg-sky-950/40 text-sky-100 hover:bg-sky-900/60' : 'border-sky-300 bg-sky-50 text-sky-900 hover:bg-sky-100') }, '↷ Redo dig (' + redoCount + ')'),
