...
const { revisions, wave09WarningSnapshot, wave10WarningSnapshot } = require('./eppp_option_feedback_wave_10_data.cjs');
...
const reviewWave = 'eppp-option-feedback-wave-10';
...
if (Object.keys(revisions).length !== 16) throw new Error('Wave 10 must contain exactly 16 reviewed items.');
...
if (!wave09WarningSnapshot || warningSummaryKeys.some((key) => !Number.isInteger(wave09WarningSnapshot[key]) || wave09WarningSnapshot[key] < 0)) throw new Error('Wave 10 needs a complete explicit wave-09 warning snapshot.');
if (!wave10WarningSnapshot || warningSummaryKeys.some((key) => !Number.isInteger(wave10WarningSnapshot[key]) || wave10WarningSnapshot[key] < 0)) throw new Error('Wave 10 needs a complete explicit wave-10 warning snapshot.');
const baseline = { ...wave09WarningSnapshot };
const auditItems = [];
...
if (domains.length !== 8) throw new Error('Wave 10 must cover all eight EPPP domains.');
...
for (const key of warningSummaryKeys) {
  if (liveAfter[key] > wave10WarningSnapshot[key]) throw new Error('Wave 10 replay increased ' + key + ' beyond its reviewed snapshot (' + liveAfter[key] + ' > ' + wave10WarningSnapshot[key] + ').');
}
const after = { ...wave10WarningSnapshot };
...
if (selectedWarningsAfter !== 0) throw new Error('Wave 10 did not clear every selected incorrect-option warning.');

const audit = {
  schemaVersion: 1,
  reviewWave,
  reviewedAt,
  scope: 'Two source-checked questions per EPPP domain, prioritizing inaccurate sampling claims, mismatched explanations or citations, and conspicuous answer cues while replacing every selected incorrect-option explanation.',
  feedbackCriteria: [
  ...
  sourceCorrections: [
    'Replaced mismatched sources for the Halstead-Reitan battery, default-mode network, episodic buffer, and Internal Family Systems with sources that directly describe the tested construct.',
    'Added primary or authoritative evidence for stratified-sampling allocation, Prolonged Exposure, the Dusky competence standard, moral disengagement, and door-in-the-face compliance.',
    'Qualified historically bounded claims about Gilligan and boundary crossings so the keyed distinctions do not imply stronger consensus or legal generality than the sources support.',
  ],
  challengeCorrections: [
    'Corrected two stratified-sampling items so stratification no longer falsely guarantees proportional allocation.',
    'Reframed direct-definition and completion prompts as application or interpretation tasks with plausible neighboring constructs.',
    'Repaired the episodic-buffer option feedback so each explanation addresses its current response option.',
    'Removed answer-word leakage, duplicated alternatives, and asymmetric extreme cues while preserving every expected answer position.',
  ],
  summary: {
...
const markdown = `# EPPP incorrect-option explanation repair - wave 10
...
for (const outputRoot of outputRoots) {
  writeFileWithRetry(path.join(outputRoot, 'eppp_option_feedback_audit_wave_10.json'), JSON.stringify(audit, null, 2) + '\n');
  writeFileWithRetry(path.join(outputRoot, 'eppp_option_feedback_audit_wave_10.md'), markdown);
}
console.log('EPPP option feedback wave 10: reviewed 16 items, replaced 48 explanations, and refined ' + audit.summary.choicesRefined + ' response options; selected warning count 0.');
