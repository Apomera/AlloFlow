...
const wave = require('../dev-tools/eppp_native_quality_wave_28_data.cjs');
...
describe('EPPP expanded quality wave 28', () => {
  ...
  it('clears clue diagnostics and replaces all 72 flagged distractor explanations', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_28.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 6, applicationItems: 12, analysisItems: 12, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicatePairs).toBeLessThan(audit.summary.distractorWarningsBefore.semanticConceptDuplicatePairs);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_28.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_28.json'));
  });

  it('replays after wave 27', () => {
    ...
    const i28 = builder.indexOf('repair_eppp_native_quality_wave_28.cjs');
