...
const slug='linear_equations_grade8',folder='allopacks/media/'+slug+'/',pack=read('allopacks/illustrated/'+slug+'.allopack.json'),original=read('allopacks/'+slug+'.allopack.json'),manifest=read(folder+'manifest.json');
require('../dev-tools/linear_content_refinements.cjs').refine(original);
...
expect(svg('slope')).toContain('data-rise-run="true"');expect(svg('slope')).toContain('data-point="2,10"');expect(svg('cross')).toContain('data-crossing="20,800"');expect(40*20).toBe(25*20+300);for(const a of manifest.assets.filter(a=>a.kind==='sort')){const m=Number(svg(a.sourceKey).match(/data-slope="(-?[0-9.]+)"/)[1]);expect(get('concept-sort').data.items[a.index].categoryId).toBe(m>0?'le-pos':m<0?'le-neg':'le-zero');}expect(get('quiz').data.questions[5].expectedAnswer).toContain('0 ≤ x ≤ 10');
});
