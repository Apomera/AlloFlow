...
const wave = require('../dev-tools/eppp_native_quality_wave_33_data.cjs');
...
describe('EPPP expanded quality wave 33', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_33.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 21, applicationItems: 17, analysisItems: 7, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    expect(audit.summary.distractorWarningsAfter.uniqueKeyStemLexicalLeakageCandidates).toBe(40);
    expect(audit.summary.distractorWarningsAfter.asymmetricExtremeDistractorCandidates).toBe(72);
    ...
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(470);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1303);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(616);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_33.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_33.json'));
  });

  it('replays after wave 32', () => {
    ...
    expect(i33).toBeGreaterThan(i32);
  });
});
