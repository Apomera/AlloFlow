...
const hydrateResourcesManifest = async (appId, resources) => {
  ...
  for (const item of source) {
    ...
    let manifestItems;
    try { manifestItems = JSON.parse(manifestJson); }
    catch (error) { throw createSessionAssetHydrationError(manifestId, 'invalid-manifest-json', error); }
...
const hydrateSessionAssets = async (appId, resources) => {
  ...
  await Promise.all(hydrated.map(async (item, index) => {
    ...
    if (payload.kind === 'sessionResourceChunks' && Array.isArray(payload.chunks)) {
      ...
      try { hydrated[index] = JSON.parse(json); }
      catch (error) { throw createSessionAssetHydrationError(resourceId, 'invalid-resource-json', error); }
      return;
    }
  ...
  const restoreField = (obj, key) => {
    ...
    if (val && typeof val === 'string' && val.startsWith('ref::')) {
      ...
      fetchPromises.push((async () => {
        ...
        const payload = snap.data() || {};
        const restored = payload.kind === 'sessionImageChunks' && Array.isArray(payload.chunks)
          ? await loadChunkedAssetString(appId, payload)
          : payload.data;
  ...
  const restoreJsonField = (obj, key) => {
    ...
    if (val && typeof val === 'string' && val.startsWith('jsonref::')) {
      ...
      fetchPromises.push((async () => {
        ...
        try { obj[key] = JSON.parse(json || 'null'); }
        catch (error) { throw createSessionAssetHydrationError(assetId, 'invalid-json-asset', error); }
      })());
    }
  };
