...
// Run: node dev-tools/check_glossary_games_review.cjs
...
const OUT = path.join(ROOT, 'reports/glossary-games-review');
...
(async () => {
  ...
  try {
    ...
    for(const viewport of [{width:1280,height:900},{width:390,height:844}]) {
      ...
      for(const name of ['MemoryGame','MatchingGame','CrosswordGame','SyntaxScramble','BingoGame','StudentBingoGame','WordScrambleGame']) {
        await page.evaluate(name=>renderGame(name),name);
        await page.waitForFunction(name=>document.body.textContent.length>100 && (name!=='WordScrambleGame'||document.querySelector('input')),name);
    ...
    console.log('Browser checks passed: 14 game/viewport combinations; caller, marks, keyboard focus, and two additional themes. Screenshots: '+OUT);
