...
(async()=>{
 const css=(await postcss([tailwind({...require(path.join(ROOT,'desktop/web-app/tailwind.config.js')),content:[{raw:fs.readFileSync(path.join(ROOT,'view_simplified_source.jsx'),'utf8')+fs.readFileSync(path.join(OUT,'host-focus-exit.js'),'utf8'),extension:'jsx'}]})]).process('@tailwind base; @tailwind components; @tailwind utilities;',{from:undefined})).css;
 ...
 try {for(const width of [320,390,1280]){
  ...
  await page.addScriptTag({path:path.join(OUT,'host-focus-exit.js')});
  ...
  await page.evaluate(()=>updateReader({theme:'light',readingTheme:'warm',isZenMode:false}));
  const focusToggle=page.locator('[data-reader-focus-view]');
  await page.locator('select[aria-label="Reading width"]').selectOption('40');
  await focusToggle.focus();await focusToggle.press('Enter');
  assert.equal(await focusToggle.getAttribute('aria-pressed'),'true');
  assert.equal(await focusToggle.textContent(),'Exit focus view');
  const exit=page.getByRole('button',{name:'Exit focus view (Esc)',exact:true});
  await exit.waitFor();
  const bounds=await exit.boundingBox(),readerBounds=await page.locator('[data-adapted-reader]').boundingBox();
  assert.ok(bounds.height>=44);assert.ok(bounds.y+bounds.height<=readerBounds.y,'Exit does not overlap content');
  assert.equal(await picker.inputValue(),'warm');
  assert.equal(await page.locator('select[aria-label="Reading width"]').inputValue(),'40');
  await page.screenshot({path:path.join(OUT,'focus-view-'+width+'.png'),fullPage:true});
  await exit.click();
  assert.equal(await focusToggle.getAttribute('aria-pressed'),'false');
  assert.equal(await focusToggle.evaluate(el=>el===document.activeElement),true,'Global exit restores reader focus');
  await focusToggle.press('Space');
  assert.equal(await focusToggle.getAttribute('aria-pressed'),'true');
  await focusToggle.press('Space');
  assert.equal(await focusToggle.getAttribute('aria-pressed'),'false');
  assert.equal(await focusToggle.evaluate(el=>el===document.activeElement),true);
  await page.evaluate(()=>document.documentElement.style.fontSize='24px');
  await focusToggle.click();
  assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false,'Large text fits');
  assert.deepEqual(await page.evaluate(async()=>{const r=await axe.run(document.querySelector('[data-adapted-reader]'));return r.violations.filter(v=>['serious','critical'].includes(v.impact)).map(v=>({id:v.id,targets:v.nodes.map(n=>n.target)}));}),[]);
