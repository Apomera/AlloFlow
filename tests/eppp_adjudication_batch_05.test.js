...
describe('EPPP claim-level adjudication batch 05', () => {
  it('reviews ten high-risk candidates without releasing or repeating them', () => {
    const report = read('test_prep/eppp_legacy/adjudication_batch_05.json');
    const priorIds = new Set(['01', '02', '03', '04'].flatMap((batch) => read(`test_prep/eppp_legacy/adjudication_batch_${batch}.json`).items.map((item) => item.legacyId)));
