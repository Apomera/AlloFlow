...
(function (global) {
    ...
    class LocalAIProvider {
        ...
        /**
         * Resolve the placeholder model name 'default' to a real loaded model.
         * Newer LM Studio versions (0.3.x+) reject unknown model identifiers with
         * 400 model_not_found, so we query /v1/models once and cache the result.
         * Prefers a chat/vision model over embedding models.
         * @returns {Promise<string>} A usable model id
         */
        async _resolveModel() {
            const m = this.model;
            ...
            try {
                ...
                if (r.ok) {
                    ...
                    let cands = (d.data || []).filter(x => x.id)
                        .filter(x => !/embed|embedding|reranker|whisper/i.test(x.id) && x.type !== 'embeddings');
                    // Prefer already-loaded models when the proxy exposes
                    // LM Studio state metadata (avoids slow/risky JIT loads).
                    const loaded = cands.filter(x => x.state === 'loaded');
                    if (loaded.length) cands = loaded;
                    const ids = cands.map(x => x.id);
                    const pick = ids[0];
                    if (pick) {
                        this._resolvedModel = pick;
                        this._debugLog(`[LocalAI] resolved model 'default' → ${pick}`);
                        return pick;
                    }
                }
            } catch (_) { /* fall through */ }
            return m; // let the server report the error
        }
        ...
        async generateText(prompt, { json = false, temperature = null, maxTokens = 8192 } = {}) {
            ...
            const _model = await this._resolveModel();
            ...
            this._debugLog(`[LocalAI] generateText: model=${_model}, json=${json}`);
