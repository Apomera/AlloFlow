  ...
  function GlossaryView(props) {
    ...
    function renderGlossaryToolbar() {
      ...
      return <section aria-label={t('glossary.title')} className="mb-4 space-y-4 rounded-xl border border-blue-200 bg-blue-50 p-4 shadow-sm">
        ...
        {glossaryToolsOpen.games && <div id="glossary-games-tools" className="rounded-lg border border-indigo-200 bg-indigo-50/70 p-3" aria-labelledby="glossary-games-tools-title"><h2 id="glossary-games-tools-title" className="mb-2 text-xs font-black tracking-wider text-indigo-800">{t('common.start_game')}</h2><div className="flex flex-wrap gap-2">
          <div className="inline-flex items-stretch rounded-lg border border-slate-300 bg-white">{selectedLanguages.length > 0 && <select aria-label={t('common.target_language_selector')} data-help-key="glossary_puzzle_lang" value={wordSearchLang} onChange={e => { setWordSearchLang(e.target.value); if (gameMode === 'wordsearch') generateWordSearch(e.target.value); }} className="min-h-11 rounded-l-lg border-0 border-r border-slate-300 bg-transparent px-3 text-sm font-bold text-teal-800 focus:ring-2 focus:ring-indigo-600"><option value="English">{t('common.english')}</option>{selectedLanguages.map(lang => <option key={lang} value={lang}>{lang}</option>)}</select>}<button type="button" aria-label={(gameMode === 'wordsearch' ? t('common.regenerate') : t('common.start_game')) + ' ' + t('glossary.word_search')} data-help-key="glossary_word_search" onClick={() => generateWordSearch(wordSearchLang)} className="min-h-11 inline-flex items-center gap-2 px-3 py-2 text-sm font-bold text-teal-800 hover:bg-teal-50 focus-visible:ring-2 focus-visible:ring-indigo-600"><Gamepad2 size={16} aria-hidden="true" /> {gameMode === 'wordsearch' ? t('common.regenerate') : t('glossary.word_search')}</button></div>
          <button type="button" aria-label={detectiveLabel} data-help-key="glossary_definition_detective" onClick={() => setDetectiveOpen(true)} className={secondaryButton}><Search size={16} aria-hidden="true" /> {detectiveLabel}</button>
          <button type="button" aria-label={t('glossary.memory_game')} data-help-key="glossary_memory_game" onClick={handleSetIsMemoryGameToTrue} className={secondaryButton}><Brain size={16} aria-hidden="true" /> {t('glossary.memory_game')}</button>
          <button type="button" aria-label={t('glossary.crossword')} data-help-key="glossary_crossword" onClick={handleSetIsCrosswordGameToTrue} className={secondaryButton}><Gamepad2 size={16} aria-hidden="true" /> {t('glossary.crossword')}</button>
          <button type="button" aria-label={t('glossary.matching')} data-help-key="glossary_matching" onClick={handleSetIsMatchingGameToTrue} className={secondaryButton}><GitMerge size={16} aria-hidden="true" /> {t('glossary.matching')}</button>
          <button type="button" aria-label={t('glossary.bingo')} data-help-key="glossary_bingo" onClick={handleSetIsBingoGameToTrue} className={secondaryButton}><Gamepad2 size={16} aria-hidden="true" /> {t('glossary.bingo')}</button>
          <button type="button" aria-label={t('glossary.play_bingo')} data-help-key="glossary_play_bingo" onClick={handleSetIsStudentBingoGameToTrue} className={secondaryButton}><Gamepad2 size={16} aria-hidden="true" /> {t('glossary.play_bingo')}</button>
          <button type="button" aria-label={t('glossary.scramble')} data-help-key="glossary_scramble" onClick={handleSetIsWordScrambleGameToTrue} className={secondaryButton}><Shuffle size={16} aria-hidden="true" /> {t('glossary.scramble')}</button>
        </div></div>}
    ...
    return <><div className="space-y-6">{renderGlossaryToolbar()}{isInteractiveFlashcards && Array.isArray(generatedContent?.data) && generatedContent.data.length > 0 && <div ref={flashcardDialogRef} role="dialog" aria-modal="true" aria-label={t('flashcards.deck_standard') || 'Interactive flashcards'} tabIndex={-1} onKeyDown={e => containModalFocus(e, flashcardDialogRef.current, closeInteractiveFlashcards)} className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-start pt-20 sm:pt-24 p-4 animate-in fade-in duration-300 motion-reduce:animate-none motion-reduce:transition-none overflow-auto"><button ref={flashcardCloseRef} type="button" onClick={closeInteractiveFlashcards} className="absolute top-6 right-6 text-white/70 hover:text-white bg-white/10 hover:bg-white/20 p-2 rounded-full transition-colors z-50  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950" aria-label={t('common.close')}><X size={32} aria-hidden="true" /></button><div id="flashcard-live-status" className="sr-only" aria-live="polite" aria-atomic="true">{flashcardStatusText}</div><div className="absolute top-6 left-6 z-50 hidden sm:flex flex-col gap-2"><button type="button" aria-pressed={showFlashcardImages} onClick={handleToggleShowFlashcardImages} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-bold transition-colors shadow-lg border  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${showFlashcardImages ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-slate-800 text-slate-300 border-slate-700 hover:text-white'}`} title={t('flashcards.tooltip_toggle_images')} aria-label={showFlashcardImages ? t('flashcards.hide_images') : t('flashcards.show_images')}><ImageIcon size={16} aria-hidden="true" /> {showFlashcardImages ? t('flashcards.hide_images') : t('flashcards.show_images')}</button><button type="button" aria-pressed={isFlashcardQuizMode} onClick={handleToggleFlashcardQuizMode} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-bold transition-colors shadow-lg border  focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${isFlashcardQuizMode ? 'bg-yellow-500 text-indigo-900 border-yellow-400' : 'bg-slate-800 text-slate-300 border-slate-700 hover:text-white'}`} title={t('flashcards.tooltip_toggle_quiz')} aria-label={glossaryQuizToggleLabel}>{isFlashcardQuizMode ? <CheckCircle2 size={16} aria-hidden="true" /> : <Brain size={16} aria-hidden="true" />}{isFlashcardQuizMode ? t('flashcards.quiz_active') : t('flashcards.practice_mode')}</button></div>{flashcardMode === 'standard' && selectedLanguages.length > 0 && <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50"><select aria-label={t('flashcards.deck_standard')} value={standardDeckLang} onChange={e => setStandardDeckLang(e.target.value)} className="bg-slate-800 text-white border border-slate-600 rounded-lg px-3 py-2 text-sm font-bold focus:ring-2 focus:ring-indigo-500  shadow-lg"><option value="English Only">{t('languages.english_only')}</option>{selectedLanguages.map(l => <option key={l} value={l}>+ {l}</option>)}</select></div>}{flashcardMode === 'language' && selectedLanguages.length > 1 && <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50"><select aria-label={t('flashcards.deck_language')} value={flashcardLang} onChange={e => setFlashcardLang(e.target.value)} className="bg-slate-800 text-white border border-slate-600 rounded-lg px-3 py-2 text-sm font-bold focus:ring-2 focus:ring-indigo-500 ">{selectedLanguages.map(l => <option key={l} value={l}>{l}</option>)}</select></div>}<div className="w-full max-w-4xl perspective-1000"><div className="mb-6 px-2"><div className="flex items-center justify-between text-white/80 mb-2"><span className="font-bold text-lg flex items-center gap-2">{flashcardMode === 'standard' ? t('flashcards.deck_standard') : t('flashcards.deck_language', {
                    lang: flashcardLang || 'Language'
                      ...
                      })}</div></div> : flashcardMode === 'standard' ? <><div className="absolute top-6 left-6 text-xs font-bold text-white uppercase tracking-widest">{t('flashcards.back_label_def')}</div><div className="w-full max-w-3xl max-h-[82%] overflow-y-auto custom-scrollbar px-1 space-y-3 text-left"><div className="rounded-xl bg-white/10 border border-blue-300/40 px-4 py-3 shadow-inner"><p className="text-[10px] font-black text-white uppercase tracking-widest mb-1">{flashcardDefinitionLabel}</p><p className="text-xl md:text-3xl font-medium leading-relaxed hover:text-blue-50 transition-colors cursor-pointer"><button type="button" onClick={e => {
                      e.stopPropagation();
