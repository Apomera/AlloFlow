  ...
  const callTTS = async (text, voiceName, speed = 1, maxRetriesOrOpts = 2, languageArg) => {
    ...
    if (_isCanvasEnv) {
      ...
      if (_isKokoroVoice) {
        ...
        const canvasMaxAttempts = Math.max(1, (Number.isFinite(maxRetries) ? Math.max(0, Math.floor(maxRetries)) : 2) + 1);
        canvasLastErr = null;
      ...
      if (ttsLang === 'en') {
        try {
          if (window._kokoroTTS) {
            ...
            if (url) {
              _ttsTrace('calltts:kokoro-fallback-ok', null);
              return url;
            }
          }
        } catch (e) {
