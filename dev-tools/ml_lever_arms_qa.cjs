...
source+=String.raw`
  ...
  for(const [effortArm,loadArm] of [[2,1],[2,2],[4,0.2],[0.2,4]]){
    await mount('lever',{leverEffortArm:effortArm,leverLoadArm:loadArm,shopFocusMechanism:true});
    for(const [name,progress] of [['Halfway',0.5],['Full stroke',1]]){
      ...
      const ratio=await pg.evaluate(()=>-window.__qaShop.mlDemo.effortContact.x/window.__qaShop.mlDemo.loadContact.x);
      if(Math.abs(ratio-effortArm/loadArm)>1e-8)errors.push('Wrong contact-arm ratio');
      const label='lever-'+effortArm+'-'+loadArm+'-'+Math.round(progress*100);
      await pg.locator('.ml-shop-world').screenshot({path:path.join(OUT,label+'.png')});manifest.push(label);
    }
  }
  await mount('lever',{shopFocusMechanism:true,shopMotionProgress:0.5,shopRotY:196});await checkPose(0.5);
  await pg.locator('.ml-shop-world').screenshot({path:path.join(OUT,'lever-rear.png')});manifest.push('lever-rear');
  for(const width of [390,320])for(const bench of ['lever']){
    ...
    const pitch=await pg.evaluate(()=>window.__qaShop.data.rotX);if(pitch!==12)errors.push('Wrong Close view tilt for ramp');
  ...
  await pg.emulateMedia({reducedMotion:'reduce'});await mount('lever');
  await inspector.getByRole('button',{name:'Halfway',exact:true}).click();await checkPose(0.5);
  const before=await pg.evaluate(()=>window.__qaShop.mlDemo.motion.rotation.z);
  await pg.waitForTimeout(300);const after=await pg.evaluate(()=>window.__qaShop.mlDemo.motion.rotation.z);
  if(before!==after || Math.abs(after-(await pg.evaluate(()=>window.__qaShop.mlDemo.baseRotation+window.__qaShop.mlDemo.leverTravel*0.5)))>1e-8)errors.push('Reduced-motion inspection drifted');
  await pg.locator('.ml-shop-world').screenshot({path:path.join(OUT,'reduced-motion-lever.png')});
