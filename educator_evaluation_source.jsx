...
function AeReleaseReview({ state, onCancel, onConfirm }) {
  ...
  return <div className="ae-onboarding-overlay" role="dialog" aria-modal="true" aria-labelledby="ae-release-title" aria-describedby="ae-release-description" onKeyDown={trapFocus}>
    <section className="ae-onboarding-card ae-release-review">
      ...
      <div className="ae-release-actions"><button type="button" className="ae-btn" disabled={busy} onClick={onCancel}>{t("educator_evaluation.cancel_ew9em3", "Cancel")}</button><button type="button" className="ae-btn ae-btn-primary" disabled={!confirmed || busy} onClick={onConfirm}>{busy ? t("educator_evaluation.confirming_access_5itq6j", 'Confirming accessâ€¦') : (review.action === 'verify_existing' ? t("educator_evaluation.confirm_and_verify_access_jas4xj", 'Confirm and verify access') : t("educator_evaluation.confirm_and_grant_access_1cd5aju", 'Confirm and grant access'))}</button></div>
    </section>
  </div>;
}
...
function AeFrameworkReference() {
  return <div className="ae-card">
    ...
    {AE_DOMAINS.map((domain) => <details className="ae-domain" key={domain.id}>
      <summary>{t("educator_evaluation.domain_1gqyxox", "Domain")} {domain.code} · {aeRubricDisplayLabel(domain.label)} <span className="ae-chip ae-chip-neutral">{domain.weight}{t("educator_evaluation.of_o_and_p_eiuk1e", "% of O&P")}</span></summary>
...
function AeRatingComposer({ teacher, role, updateTeacher, evidenceFindings, aiReflectionEnabled, askForReflection, reflection }) {
  ...
  return <div className="ae-card">
    ...
    <div className="ae-rating-grid" style={{ marginTop: 12 }}>
      {AE_DOMAINS.map((domain) => <div className="ae-rating-card" key={domain.id} style={{ borderTop: t("educator_evaluation.4px_solid_121zz6w", '4px solid ') + domain.color }}>
        <h4>{domain.code}. {aeRubricDisplayLabel(domain.label)} <span className="ae-chip ae-chip-neutral">{domain.weight}{t("educator_evaluation.of_o_and_p_eiuk1e", "% of O&P")}</span></h4>
  ...
  return <div className="ae-page">
    ...
    <div className="ae-grid">
      ...
      {isEvaluator && (workload.overdue > 0 || workload.soon > 0 || workload.month > 0) && <section className="ae-card ae-span-12" aria-labelledby="ae-workload-title"><h3 id="ae-workload-title">{t("educator_evaluation.coming_due_ry9pkk", "Coming due")}</h3><p className="ae-sub">{t("educator_evaluation.open_cycles_by_due_date_a_band_is_triage_for_your_calendar_15zfrim", "Open cycles by due date. A band is triage for your calendar, not a judgment about anyone.")}</p><div className="ae-grid" style={{ marginTop: 10 }}>
        ...
        <div className="ae-span-4 ae-stat"><strong>{workload.month}</strong><span>{t("educator_evaluation.due_in_15_30_days_1974v1z", "due in 15–30 days")}</span></div>
      </div></section>}
...
function AeTrendChart({ points, metric, label }) {
  ...
  return <div>
    <p className="ae-sub">{values.length} {t("educator_evaluation.finalized_point_1punnkr", "finalized point")}{values.length === 1 ? '' : 's'} {t("educator_evaluation.fixed_0_3_scale_missing_ratings_are_not_connected_sm6tpy", "· fixed 0–3 scale. Missing ratings are not connected.")}</p>
...
function EducatorEvaluationPanel(props) {
  ...
  const exportWorkspace = () => { const payload = Object.assign({}, workspace, { kind: AE_EXPORT_KIND, exportedAt: aeNow() }); aeDownload(t("educator_evaluation.alloflow_evaluation_1rx8rx5", 'alloflow-evaluation-') + aeToday() + '.json', 'application/json', JSON.stringify(payload, null, 2)); commit(() => {}, { event: 'EXPORTED', summary: t("educator_evaluation.workspace_json_exported_rs18wj", 'Workspace JSON exported'), entityType: 'workspace', entityId: 'workspace' }, 'Workspace export created'); };
  const exportCsv = () => { const rows = workspace.teachers.map((teacher) => ({ staff_code: teacher.code, educator: teacher.name, building: teacher.building, assignment: teacher.assignment, employee_type: teacher.employeeType, evaluation_status: aeTeacherStatus(teacher), due_date: teacher.dueDate, evaluator: teacher.evaluator, walkthroughs: workspace.walkthroughs.filter((item) => item.teacherId === teacher.id && item.publishedAt).length, formal_observation: workspace.observations.some((item) => item.teacherId === teacher.id && item.finalizedAt) ? 'finalized' : (workspace.observations.some((item) => item.teacherId === teacher.id) ? 'in_progress' : 'not_started'), spm_status: (workspace.spms.find((item) => item.teacherId === teacher.id) || {}).status || 'not_started' })); aeDownload(t("educator_evaluation.evaluation_status_12p2q0f", 'evaluation-status-') + aeToday() + '.csv', 'text/csv;charset=utf-8', '\uFEFF' + aeCsv(rows)); commit(() => {}, { event: 'EXPORTED', summary: t("educator_evaluation.evaluation_status_csv_exported_8n3zje", 'Evaluation status CSV exported'), entityType: 'workspace', entityId: 'workspace' }, 'Status CSV created'); };
  ...
  const exportEducatorPacket = () => {
    ...
    const walkthroughRows = (packet.walkthroughs || []).map((item, index) => t("educator_evaluation.section_class_card_rec_aria_labelledby_ae_walk_title_hulocm", '<section class="card rec" aria-labelledby="ae-walk-title-') + index + '">'
      ...
      + ackControl('walkthroughs', item, t("educator_evaluation.ae_walk_ack_19j5d5y", 'ae-walk-ack-') + index, 'I acknowledge receipt of this published walkthrough.')
  ...
  const exportRubric = () => {
    aeDownload(t("educator_evaluation.evaluation_rubric_mxt02e", 'evaluation-rubric-') + (AE_ACTIVE_FW.versionTag || 'current') + '.json', 'application/json',
      JSON.stringify({
...
href="https://alloflow-cdn.pages.dev/educator-evaluation-manual" target="_blank" rel="noopener noreferrer" aria-label={t("educator_evaluation.user_manual_opens_in_a_new_tab_1t3w65", "User manual (opens in a new tab)")} title={t("educator_evaluation.how_to_run_a_full_evaluation_cycle_set_up_the_district_por_162ju8s", "How to run a full evaluation cycle, set up the district portal, and read the released summary")}>{t("educator_evaluation.manual_1wu7r43", "Manual")}</a>{!standalone && <button type="button" className="ae-close" onClick={onClose} aria-label={t("educator_evaluation.close_educator_growth_and_evaluation_1d9f8pc", "Close Educator Growth and Evaluation")}>×</button>}</div>
    ...
    <main className="ae-main" id="ae-panel" role="tabpanel" tabIndex={-1} aria-labelledby={t("educator_evaluation.ae_tab_yr2u0k", 'ae-tab-') + tab} aria-busy={remoteState.inFlight ? 'true' : undefined} aria-disabled={isRemote && ['error', 'conflict'].includes(remoteState.status) ? 'true' : undefined} onClickCapture={blockRemoteMutation} onChangeCapture={blockRemoteMutation} onInputCapture={blockRemoteMutation} onSubmitCapture={blockRemoteMutation}>
