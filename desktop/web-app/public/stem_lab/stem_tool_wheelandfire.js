...
(function () {
  ...
  var __alloWheelCtx = null;
  var __alloT = function (k, fb) {
    var v;
    try { v = (__alloWheelCtx && typeof __alloWheelCtx.t === "function") ? __alloWheelCtx.t(k, fb) : null; } catch (e) { v = null; }
  ...
  function analyzeFailureContributors(vessel, settings) {
    ...
    if (structural) {
      ...
      if (settings.method === 'coil' && stats.worstCoilMoistureMismatch !== null && stats.worstCoilMoistureMismatch >= 18) add('coil-moisture-gap', 'Coil-to-body moisture gap', Math.round(stats.worstCoilMoistureMismatch) + ' moisture points at ring ' + (stats.worstCoilMoistureMismatchRing + 1) + ', with the coil recorded as ' + stats.worstCoilMoistureDirection + ' than the body at placement.', 'Compare a more closely matched coil while holding support, pressure, and consolidation constant.', stats.worstCoilMoistureMismatch * 2.2);
    }
    if (openingFailure) {
      var openingFloor = analyzeOpeningFloor(vessel);
    ...
    if (drying) {
      ...
      if (defects.indexOf('coil separation') >= 0) add('coil-separation', 'Drying-sensitive coil joint', (typeof vessel.coilSeparationRing === 'number' ? 'The tracked joint at ring ' + (vessel.coilSeparationRing + 1) : (stats.weakestCoilJointRing === null ? Math.round(stats.coilBond) + '% whole-form bond proxy' : 'The weakest tracked joint at ring ' + (stats.weakestCoilJointRing + 1))) + ' was carried into drying.' + (stats.worstCoilMoistureMismatch !== null ? ' Largest recorded seam moisture gap: ' + Math.round(stats.worstCoilMoistureMismatch) + ' points at ring ' + (stats.worstCoilMoistureMismatchRing + 1) + '.' : ''), typeof vessel.coilSeparationRing === 'number' ? 'Compare stronger consolidation and a closer moisture match at ring ' + (vessel.coilSeparationRing + 1) + ' before the next drying trial.' : 'Compress each joint and compare moisture match before the next drying trial.', 100 - stats.weakestCoilJoint + finite(stats.worstCoilMoistureMismatch, 0) * 0.35);
    }
    if (thermal) {
    ...
    render: function (ctx) {
      ...
      function applyActive(index, options) {
        if (vessel.stage !== 'wet' && !(vessel.stage === 'leather-hard' && activeTool === 'trim')) {
          announce(__alloT('stem.wheelandfire.sr_shaping_is_unavailable_after_the_clay_has_dried', 'Shaping is unavailable after the clay has dried.'));
          return;
        }
      ...
      function undo() {
        ...
        if (!history.length) { announce(__alloT('stem.wheelandfire.sr_nothing_to_undo', 'Nothing to undo.')); return; }
        ...
        announce(__alloT('stem.wheelandfire.sr_pottery_action_undone', 'Pottery action undone.'));
      }
      function redo() {
        var future = copyArray(data.future);
        if (!future.length) { announce(__alloT('stem.wheelandfire.sr_nothing_to_redo', 'Nothing to redo.')); return; }
        ...
        announce(__alloT('stem.wheelandfire.sr_pottery_action_redone', 'Pottery action redone.'));
      }
      ...
      function experienceModeControl() {
        ...
        return h('section', { className: 'rounded-xl border border-stone-300 bg-white p-2 flex flex-wrap items-center gap-2', 'aria-labelledby': 'wheel-fire-experience-mode-title' },
          ...
          h('div', { className: 'flex flex-wrap gap-1', role: 'group', 'aria-label': __alloT('stem.wheelandfire.a11y_pottery_workspace_depth', 'Pottery workspace depth') }, modes.map(function (mode) {
      ...
      function restoreLastSafeCheckpoint() {
        ...
        if (index < 0) { announce(__alloT('stem.wheelandfire.sr_no_earlier_safe_checkpoint_is_available_load_fres', 'No earlier safe checkpoint is available. Load fresh clay or a saved journal record.')); return; }
        ...
        announce(__alloT('stem.wheelandfire.sr_restored_the_last_checkpoint_before_modeled_failu', 'Restored the last checkpoint before modeled failure. Change one input before repeating the trial.'));
      }
      function failureAutopsy() {
        ...
        return h('section', { className: 'rounded-2xl border border-rose-400 bg-rose-50 p-3 space-y-3', 'aria-labelledby': 'wheel-fire-autopsy-title' },
          ...
          h('ol', { className: 'grid md:grid-cols-3 gap-2 text-xs text-rose-950', 'aria-label': __alloT('stem.wheelandfire.a11y_modeled_failure_chain', 'Modeled failure chain') },
          ...
          h('div', { className: 'flex flex-wrap gap-2' },
            h('button', { type: 'button', onClick: function () { patchData({ view: 'shape', workRing: failureReport.criticalRing }); announce(__alloT('stem.wheelandfire.sr_focused_the_highest_risk_ring_in_shape', 'Focused the highest-risk ring in Shape.')); }, className: 'rounded-lg border border-rose-400 bg-white px-3 py-2 text-xs font-black text-rose-900' }, 'Inspect ring ' + (failureReport.criticalRing + 1)),
      ...
      function stageStrip() {
        ...
        return h('div', { className: 'wheel-fire-stage-line', role: 'list', 'aria-label': __alloT('stem.wheelandfire.a11y_pottery_lifecycle', 'Pottery lifecycle') }, STAGES.map(function (stage, index) {
          ...
          h('g', { 'data-wheel-fire-clay-orbit': method === 'wheel' ? wheelWobble.motionState : undefined, 'data-wheel-fire-speed-load': method === 'wheel' ? wheelWobble.loadState : undefined, 'data-wheel-fire-wobble-cycle-seconds': method === 'wheel' ? (wheelWobble.cycleSeconds === null ? 'stopped' : wheelWobble.cycleSeconds.toFixed(3)) : undefined, className: method === 'wheel' && wheelWobble.motionState === 'orbiting' ? 'wheel-fire-wobble-motion' : undefined, style: method === 'wheel' ? { '--wheel-fire-wobble': wobblePx.toFixed(1) + 'px', '--wheel-fire-wobble-neg': (-wobblePx).toFixed(1) + 'px', '--wheel-fire-wobble-diag': wheelWobble.diagonalAmplitudePx.toFixed(1) + 'px', '--wheel-fire-wobble-diag-neg': (-wheelWobble.diagonalAmplitudePx).toFixed(1) + 'px', '--wheel-fire-wobble-depth': wheelWobble.depthAmplitudePx.toFixed(1) + 'px', '--wheel-fire-wobble-depth-neg': (-wheelWobble.depthAmplitudePx).toFixed(1) + 'px', '--wheel-fire-wobble-depth-diag': wheelWobble.diagonalDepthPx.toFixed(1) + 'px', '--wheel-fire-wobble-depth-diag-neg': (-wheelWobble.diagonalDepthPx).toFixed(1) + 'px', animationDuration: wheelWobble.cycleSeconds === null ? undefined : wheelWobble.cycleSeconds.toFixed(3) + 's', transform: wheelWobble.motionState === 'stationary-offset' ? 'translate(' + wobblePx.toFixed(1) + 'px, 0px)' : undefined } : undefined },
            ...
            method === 'coil' ? h('g', { 'data-wheel-fire-coil-joint-map': 'true', 'data-wheel-fire-coil-joint-count': coilJointProfile.jointCount, 'data-wheel-fire-coil-joint-preview-count': showPredictedCoilJoints ? formingPreviewCoilJointChanges.length : 0, pointerEvents: 'none', 'aria-hidden': 'true' },
              coilJointProfile.joints.map(function (joint) {
                ...
                var jointRy = Math.max(3, jointRadius * (0.038 + perspectiveDepth * 0.038));
                return h('g', { key: 'coil-joint-' + joint.ring },
                  h('ellipse', { 'data-wheel-fire-coil-joint': joint.ring + 1, 'data-wheel-fire-coil-joint-state': joint.status, 'data-wheel-fire-coil-joint-strength': joint.strengthPct.toFixed(1), 'data-wheel-fire-coil-joint-moisture-state': joint.moistureStatus, 'data-wheel-fire-coil-joint-moisture-gap': typeof joint.moistureMismatchPct === 'number' ? joint.moistureMismatchPct.toFixed(1) : undefined, 'data-wheel-fire-coil-joint-moisture-direction': joint.moistureDirection, cx: geometry.center, cy: jointY, rx: jointRadius, ry: jointRy, fill: 'none', stroke: coilJointColor(joint), strokeWidth: coilJointWidth(joint), strokeDasharray: coilJointDash(joint), strokeLinecap: 'round', opacity: .98 }),
                  joint.moistureStatus === 'watch' || joint.moistureStatus === 'high' ? h('ellipse', { 'data-wheel-fire-coil-moisture-gap': joint.ring + 1, 'data-wheel-fire-coil-moisture-gap-state': joint.moistureStatus, 'data-wheel-fire-coil-moisture-gap-pct': joint.moistureMismatchPct.toFixed(1), 'data-wheel-fire-coil-moisture-gap-direction': joint.moistureDirection, cx: geometry.center, cy: jointY, rx: jointRadius + 5, ry: jointRy + 2.5, fill: 'none', stroke: coilMoistureGapColor(joint), strokeWidth: coilMoistureGapWidth(joint), strokeDasharray: '2 5', strokeLinecap: 'round', opacity: .96 }) : null
                );
              }),
              showPredictedCoilJoints ? formingPreviewCoilJointChanges.filter(function (change) { return typeof change.after === 'number'; }).map(function (change) {
                ...
                return h('ellipse', { key: 'preview-coil-joint-' + change.ring, 'data-wheel-fire-coil-joint-preview': change.ring + 1, 'data-wheel-fire-coil-joint-preview-strength': (change.after * 100).toFixed(1), 'data-wheel-fire-coil-joint-preview-state': change.state, 'data-wheel-fire-coil-joint-preview-load': change.loadId, 'data-wheel-fire-coil-joint-preview-moisture-state': change.moistureStatus, 'data-wheel-fire-coil-joint-preview-moisture-gap': typeof change.moistureMismatchPct === 'number' ? change.moistureMismatchPct.toFixed(1) : undefined, 'data-wheel-fire-coil-joint-preview-moisture-direction': change.moistureDirection, cx: formingPreviewGeometry.center, cy: previewJointY, rx: previewJointRadius, ry: Math.max(4, previewJointRadius * (0.041 + perspectiveDepth * 0.041)), fill: 'none', stroke: '#facc15', strokeWidth: 2.5, strokeDasharray: '4 4', strokeLinecap: 'round', opacity: .96 });
              }) : null
            ) : null,
            visibleCoilJointChange ? h('g', { 'data-wheel-fire-coil-joint-load-cue': visibleCoilJointChange.state, 'data-wheel-fire-coil-joint-load-kind': visibleCoilJointChange.loadId, 'data-wheel-fire-coil-joint-load-ring': visibleCoilJointChange.ring + 1, 'data-wheel-fire-coil-joint-load-moisture-gap': typeof visibleCoilJointChange.moistureMismatchPct === 'number' ? visibleCoilJointChange.moistureMismatchPct.toFixed(1) : undefined, pointerEvents: 'none', 'aria-hidden': 'true' },
      ...
      function shapePanel() {
        ...
        function useSaferTouchSetup() {
          ...
          patchData(safer);
          announce(__alloT('stem.wheelandfire.sr_loaded_a_safer_comparative_touch_setup_review_the', 'Loaded a safer comparative touch setup. Review the new forecast before applying the tool.'));
        }
        ...
        return h('section', { id: 'wheel-fire-panel-shape', role: 'tabpanel', 'aria-labelledby': 'wheel-fire-tab-shape', className: 'space-y-3' },
          h('div', { className: 'wheel-fire-main' },
            h('div', { className: 'space-y-2' }, vesselSvg(),
              ...
              formingForecast(),
              h('div', { className: 'wheel-fire-stats', role: 'group', 'aria-label': __alloT('stem.wheelandfire.a11y_live_vessel_measurements', 'Live vessel measurements') },
            ...
            h('div', { className: 'space-y-3' },
              h('div', { className: 'rounded-xl border border-amber-300 bg-amber-50 p-3' },
                ...
                h('div', { className: 'grid grid-cols-2 gap-2', role: 'group', 'aria-label': __alloT('stem.wheelandfire.a11y_forming_method', 'Forming method') },
              ...
              h('div', { className: 'rounded-xl border border-teal-300 bg-teal-50 p-3' },
                ...
                h('div', { className: 'grid grid-cols-2 gap-2', role: 'group', 'aria-label': __alloT('stem.wheelandfire.a11y_clay_shaping_tools', 'Clay shaping tools') }, tools.map(function (tool) {
      ...
      function sciencePanel() {
        ...
        function removeMaterialScenario(id) {
          ...
          announce(__alloT('stem.wheelandfire.sr_material_scenario_removed_from_the_comparison_she', 'Material scenario removed from the comparison shelf.'));
        }
        ...
        function logTrial() {
          ...
          announce(__alloT('stem.wheelandfire.sr_measurement_trial_logged', 'Measurement trial logged.'));
        }
        ...
        function startTrialSeries() {
          ...
          announce(__alloT('stem.wheelandfire.sr_new_mechanics_experiment_series_started_earlier_t', 'New mechanics experiment series started. Earlier trials remain saved in the journal.'));
        }
        ...
        function trialComparison() {
          ...
          compareInput('coilMoisture', 'new-coil moisture', function (value) { return value === undefined ? 'not recorded' : Math.round(finite(value, 0)) + '%'; });
          var deltas = [];
          ...
          if (reference.worstCoilMoistureMismatch !== undefined || current.worstCoilMoistureMismatch !== undefined) comparisonMetrics.push(['worstCoilMoistureMismatch', 'largest seam moisture gap', ' pts']);
        ...
        function seriesEvidenceSummary() {
          ...
          return h('div', { className: 'rounded-xl border border-violet-300 bg-violet-50 p-3 space-y-2', role: 'region', 'aria-label': __alloT('stem.wheelandfire.a11y_series_evidence_trail', 'Series evidence trail') },
        ...
        function studyProtocol() {
          ...
          return h('div', { className: 'rounded-xl border border-sky-300 bg-sky-50 p-3 space-y-2', role: 'region', 'aria-label': __alloT('stem.wheelandfire.a11y_study_protocol', 'Study protocol') },
            h('div', { className: 'flex flex-wrap items-baseline justify-between gap-2' },
        ...
        function trialSeriesControl() {
          ...
          return h('div', { className: 'rounded-xl border border-teal-600 bg-white p-3 space-y-2' },
            h('div', { className: 'grid md:grid-cols-3 gap-2' },
              ...
              h('label', { htmlFor: 'wheel-fire-trial-series-name', className: 'block text-xs font-bold text-slate-700' }, 'Series name (optional)', h('input', { id: 'wheel-fire-trial-series-name', maxLength: 60, value: data.trialSeriesName || '', onChange: function (event) { patchData({ trialSeriesName: event.target.value }); }, placeholder: __alloT('stem.wheelandfire.e_g_speed_and_rim_stability','e.g. speed and rim stability'), className: 'block w-full mt-1 rounded-lg border border-teal-300 p-2 font-normal' })),
        ...
        return h('section', { id: 'wheel-fire-panel-science', role: 'tabpanel', 'aria-labelledby': 'wheel-fire-tab-science', className: 'space-y-3' },
          h('div', { className: 'rounded-2xl border border-cyan-300 bg-cyan-50 p-4' },
            ...
            h('ol', { className: 'grid md:grid-cols-3 gap-2 mt-3 text-xs text-cyan-950', 'aria-label': __alloT('stem.wheelandfire.a11y_one_variable_experiment_loop', 'One-variable experiment loop') },
            ...
            h('div', { className: 'wheel-fire-stats mt-3' },
              ...
              metricCard('Coil continuity', percent(stats.coilBond), method === 'coil' ? (stats.trackedCoilJointCount ? 'Weakest ring ' + (stats.weakestCoilJointRing + 1) + ' · ' + percent(stats.weakestCoilJoint) + (stats.worstCoilMoistureMismatch === null ? '' : ' · largest moisture gap ' + Math.round(stats.worstCoilMoistureMismatch) + ' pts at ring ' + (stats.worstCoilMoistureMismatchRing + 1)) : 'No locally tracked seams yet') : 'Not currently limiting'),
          ...
          h('div', { className: 'rounded-xl border border-rose-300 bg-rose-50 p-3 space-y-3' },
            ...
            h('div', { className: 'grid grid-cols-2 md:grid-cols-3 gap-2', role: 'list', 'aria-label': __alloT('stem.wheelandfire.a11y_local_ring_stress_zones', 'Local ring stress zones') }, ringZones.map(function (zone) {
              var peak = zone.peak;
          ...
          h('div', { className: 'wheel-fire-advanced rounded-xl border border-amber-300 bg-amber-50 p-3 space-y-3' },
            ...
            h('div', { className: 'grid md:grid-cols-2 gap-3' },
              h('label', { htmlFor: 'wheel-fire-recipe-label', className: 'block text-xs font-bold text-slate-700' }, 'Study label', h('input', { id: 'wheel-fire-recipe-label', maxLength: 48, value: recipeDraft.label, onChange: function (event) { updateRecipe('label', event.target.value); }, placeholder: __alloT('stem.wheelandfire.e_g_coarse_temper_trial','e.g. coarse temper trial'), className: 'block w-full mt-1 rounded-lg border border-amber-600 p-2 bg-white font-normal' })),
          ...
          h('div', { className: 'grid md:grid-cols-2 gap-3' },
            h('div', { className: 'rounded-xl border border-slate-300 bg-white p-3 space-y-3' },
              ...
              h('label', { htmlFor: 'wheel-fire-trial-observation', className: 'block text-xs font-bold text-slate-700' }, 'Studio observation (optional)', h('textarea', { id: 'wheel-fire-trial-observation', rows: 3, maxLength: 240, value: data.trialObservation || '', onChange: function (event) { patchData({ trialObservation: event.target.value }); }, placeholder: __alloT('stem.wheelandfire.what_did_you_feel_hear_see_or_notice_the_mod','What did you feel, hear, see, or notice? The model cannot sense drag, wobble, sound, cracks, or touch.'), className: 'block w-full mt-1 rounded-lg border border-slate-400 p-2 font-normal' })),
            ...
            h('div', { className: 'wheel-fire-advanced rounded-xl border border-slate-300 bg-white p-3' },
              ...
              h('ul', { className: 'list-disc pl-5 text-xs text-slate-700 space-y-2' },
                ...
                h('li', null, 'New coils begin with local joints and preserve their coil-to-body moisture gap at placement. Paddling or smoothing strengthens a contacted seam; reshaping across it can add outward tension, vertical stretch, opening shear, or inward compression.'),
                h('li', null, 'Moisture-match bands are comparative, not universal instructions. Clay bodies, joining methods, environments, and community practices can call for different decisions.'),
      ...
      function traditionsPanel() {
        ...
        function applyStudy(study) {
          ...
          commitVessel(next, 'Technique study materials configured for ' + study.name + '. The resulting work remains your own study, not an authentic community object.', {
            method: exp.method, activeTool: exp.method === 'coil' ? 'add-coil' : 'center', clayBody: exp.clayBody, rpm: exp.rpm, coilMoisture: next.moisture * 100, kilnType: exp.kilnType, kilnTemp: exp.kilnTemp, glazeId: exp.glaze, selectedTradition: study.id, visitedTraditions: nextVisited, studyLabel: study.name + ' materials study', recipeDraft: null
          });
        }
        function saveCulturalComparison() {
          ...
          if (!similarity || !difference || !evidence) { announce(__alloT('stem.wheelandfire.sr_complete_all_three_comparison_reflections_before', 'Complete all three comparison reflections before saving.')); return; }
          ...
          announce(__alloT('stem.wheelandfire.sr_contextual_pottery_comparison_saved_both_process', 'Contextual pottery comparison saved. Both process studies are marked visited.'));
        }
      ...
      function kilnPanel() {
        ...
        function removeFiringSchedule(id) {
          ...
          announce(__alloT('stem.wheelandfire.sr_firing_schedule_removed_from_the_comparison_shelf', 'Firing schedule removed from the comparison shelf.'));
        }
        ...
        function dryingHistory() {
          ...
          return h('section', { className: 'rounded-xl border border-sky-200 bg-white p-3 space-y-3', 'aria-labelledby': 'wheel-fire-drying-history-title' },
            ...
            history.ready ? h('div', { className: 'space-y-2', 'aria-label': __alloT('stem.wheelandfire.a11y_modeled_drying_history_steps', 'Modeled drying history steps') }, history.segments.map(function (segment) {
            ...
            history.hotspots.length ? h('div', { className: 'rounded-lg border border-sky-200 bg-sky-50 p-3 space-y-2', 'aria-label': __alloT('stem.wheelandfire.a11y_drying_hotspots_to_inspect', 'Drying hotspots to inspect') },
        ...
        function dimensionalHistory() {
          ...
          function saveDimensionMeasurement() {
            ...
            if (!hasValue) { announce(__alloT('stem.wheelandfire.sr_enter_at_least_one_positive_measured_dimension_be', 'Enter at least one positive measured dimension before logging the checkpoint.')); return; }
          ...
          function clearDimensionTargets() {
            ...
            announce(__alloT('stem.wheelandfire.sr_dimensional_targets_cleared', 'Dimensional targets cleared.'));
          }
          ...
          return h('section', { className: 'wheel-fire-advanced rounded-xl border border-indigo-200 bg-indigo-50 p-3 space-y-3', 'aria-labelledby': 'wheel-fire-dimensional-history-title' },
            ...
            h('div', { role: 'region', tabIndex: 0, 'aria-label': __alloT('stem.wheelandfire.a11y_projected_dimensional_checkpoints_table', 'Projected dimensional checkpoints table'), className: 'overflow-x-auto rounded-lg border border-indigo-200 bg-white' },
              h('table', { className: 'w-full text-xs border-collapse' },
            ...
            h('div', { className: 'rounded-lg border border-indigo-500 bg-white p-3 space-y-3' },
              ...
              h('div', { className: 'grid sm:grid-cols-2 gap-2' },
                ...
                h('label', { className: 'block text-xs font-bold text-slate-700' }, 'Measurement note (optional)', h('input', { value: inputValue('dimensionMeasureNote'), maxLength: 240, onChange: function (event) { patchData({ dimensionMeasureNote: event.target.value }); }, className: 'block w-full mt-1 rounded-lg border border-indigo-500 p-2 bg-white', placeholder: __alloT('stem.wheelandfire.e_g_calipers_after_glaze_firing','e.g. calipers after glaze firing') }))
              ),
              h('div', { className: 'rounded-lg border border-indigo-200 bg-indigo-50 p-2 space-y-2' },
                ...
                h('div', { className: 'grid sm:grid-cols-2 lg:grid-cols-4 gap-2' },
                  h('label', { className: 'block text-xs font-bold text-slate-700' }, 'Height +/- (cm)', h('input', { type: 'number', min: '0', step: '0.01', value: inputValue('dimensionUncertaintyHeight'), onChange: function (event) { patchData({ dimensionUncertaintyHeight: event.target.value }); }, className: 'block w-full mt-1 rounded-lg border border-indigo-500 p-2 bg-white', placeholder: __alloT('stem.wheelandfire.e_g_0_1','e.g. 0.1') })),
                  h('label', { className: 'block text-xs font-bold text-slate-700' }, 'Diameter +/- (cm)', h('input', { type: 'number', min: '0', step: '0.01', value: inputValue('dimensionUncertaintyDiameter'), onChange: function (event) { patchData({ dimensionUncertaintyDiameter: event.target.value }); }, className: 'block w-full mt-1 rounded-lg border border-indigo-500 p-2 bg-white', placeholder: __alloT('stem.wheelandfire.e_g_0_1_2','e.g. 0.1') })),
                  h('label', { className: 'block text-xs font-bold text-slate-700' }, 'Capacity +/- (mL)', h('input', { type: 'number', min: '0', step: '1', value: inputValue('dimensionUncertaintyCapacity'), onChange: function (event) { patchData({ dimensionUncertaintyCapacity: event.target.value }); }, className: 'block w-full mt-1 rounded-lg border border-indigo-500 p-2 bg-white', placeholder: __alloT('stem.wheelandfire.e_g_5','e.g. 5') })),
                  h('label', { className: 'block text-xs font-bold text-slate-700' }, 'Min wall +/- (cm)', h('input', { type: 'number', min: '0', step: '0.01', value: inputValue('dimensionUncertaintyMinWall'), onChange: function (event) { patchData({ dimensionUncertaintyMinWall: event.target.value }); }, className: 'block w-full mt-1 rounded-lg border border-indigo-500 p-2 bg-white', placeholder: __alloT('stem.wheelandfire.e_g_0_02','e.g. 0.02') }))
                )
              ),
        ...
        function thermalHistory() {
          ...
          return h('section', { className: 'rounded-xl border border-orange-600 bg-white p-3 space-y-3', 'aria-labelledby': 'wheel-fire-thermal-history-title' },
            ...
            h('div', { className: 'space-y-2', 'aria-label': __alloT('stem.wheelandfire.a11y_modeled_thermal_history_segments', 'Modeled thermal history segments') }, history.segments.map(function (segment) {
              return h('div', { key: segment.id },
                h('div', { className: 'flex flex-wrap items-center justify-between gap-2 text-xs font-bold text-slate-700' },
        ...
        return h('section', { id: 'wheel-fire-panel-kiln', role: 'tabpanel', 'aria-labelledby': 'wheel-fire-tab-kiln', className: 'space-y-3' },
          ...
          h('div', { className: 'grid grid-cols-1 md:grid-cols-2 gap-3' },
            ...
            h('div', { className: 'rounded-xl border border-orange-300 bg-white p-3 space-y-3' },
              ...
              h('label', { className: 'block text-xs font-bold text-slate-700' }, 'Schedule label', h('input', { maxLength: 48, value: scheduleLabel, onChange: function (event) { patchData({ scheduleLabel: event.target.value }); }, placeholder: __alloT('stem.wheelandfire.e_g_slow_stoneware_test','e.g. slow stoneware test'), className: 'block w-full mt-1 rounded-lg border border-slate-400 p-2 font-normal' })),
      ...
      function performancePanel() {
        ...
        function runPerformanceTest() {
          if (!preview.ready) { announce(__alloT('stem.wheelandfire.sr_fire_the_piece_before_running_a_function_test', 'Fire the piece before running a function test.')); return; }
        ...
        function logSensitivitySweep() {
          if (testType !== 'cycles' || !preview.ready) { announce(__alloT('stem.wheelandfire.sr_fire_the_piece_and_select_repeated_wet_dry_cycles', 'Fire the piece and select repeated wet-dry cycles before logging a sensitivity sweep.')); return; }
        ...
        function cycleSensitivityExplainer() {
          ...
          return h('section', { className: 'wheel-fire-advanced rounded-xl border border-slate-300 bg-slate-50 p-3 space-y-3', 'aria-labelledby': 'wheel-fire-sensitivity-title' },
            ...
            h('div', { className: 'rounded-lg border border-slate-200 bg-white p-3 space-y-2', 'aria-label': __alloT('stem.wheelandfire.a11y_sensitivity_band_contributors', 'Sensitivity band contributors') },
        ...
        function cycleSensitivitySweep() {
          ...
          return h('section', { className: 'wheel-fire-advanced rounded-xl border border-teal-300 bg-teal-50 p-3 space-y-3', 'aria-labelledby': 'wheel-fire-sensitivity-sweep-title' },
            ...
            h('div', { className: 'rounded-lg border border-teal-200 bg-white p-3 space-y-2' },
              h('label', { htmlFor: 'wheel-fire-sensitivity-observation', className: 'block text-xs font-bold text-slate-700' }, 'Sensitivity observation (optional)', h('textarea', { id: 'wheel-fire-sensitivity-observation', rows: 3, maxLength: 240, value: sensitivityObservation, onChange: function (event) { patchData({ sensitivityObservation: event.target.value }); }, placeholder: __alloT('stem.wheelandfire.record_what_changed_in_the_real_piece_when_y','Record what changed in the real piece when you varied one condition. Keep this separate from the model result.'), className: 'block w-full mt-1 rounded-lg border border-slate-400 p-2 font-normal' })),
        ...
        function cycleProgression() {
          ...
          return h('section', { className: 'rounded-xl border border-amber-300 bg-amber-50 p-3 space-y-3', 'aria-labelledby': 'wheel-fire-cycle-progression-title' },
            ...
            h('div', { className: 'space-y-2', 'aria-label': __alloT('stem.wheelandfire.a11y_visual_damage_progression', 'Visual damage progression') }, points.map(function (point) {
            ...
            h('div', { className: 'rounded-lg border border-amber-200 bg-white p-3 space-y-2', 'aria-label': __alloT('stem.wheelandfire.a11y_relative_modeled_damage_drivers', 'Relative modeled damage drivers') },
              h('div', null,
        ...
        function removeCycleProtocol(id) {
          ...
          announce(__alloT('stem.wheelandfire.sr_reuse_protocol_removed_from_the_shelf', 'Reuse protocol removed from the shelf.'));
        }
        function cycleProtocolShelf() {
          ...
          return h('section', { className: 'wheel-fire-advanced rounded-xl border border-violet-300 bg-violet-50 p-3 space-y-3', 'aria-labelledby': 'wheel-fire-cycle-shelf-title' },
            ...
            h('div', { className: 'flex flex-wrap gap-2 items-end' },
              h('label', { className: 'block text-xs font-bold text-slate-700 flex-1 min-w-[190px]' }, 'Protocol label', h('input', { id: 'wheel-fire-cycle-protocol-label', value: cycleProtocolLabel, maxLength: 48, onChange: function (event) { patchData({ cycleProtocolLabel: event.target.value }); }, placeholder: __alloT('stem.wheelandfire.e_g_slow_studio_rinse','e.g. slow studio rinse'), className: 'block w-full mt-1 rounded-lg border border-slate-500 p-2 bg-white' })),
        ...
        return h('section', { id: 'wheel-fire-panel-performance', role: 'tabpanel', 'aria-labelledby': 'wheel-fire-tab-performance', className: 'space-y-3' },
          ...
          h('div', { className: 'grid md:grid-cols-[minmax(240px,.72fr)_minmax(0,1.28fr)] gap-3' },
            h('div', { className: 'rounded-xl border border-slate-300 bg-white p-3 space-y-3' },
              ...
              h('label', { htmlFor: 'wheel-fire-test-observation', className: 'block text-xs font-bold text-slate-700' }, 'Observed note (optional)', h('textarea', { id: 'wheel-fire-test-observation', rows: 3, maxLength: 240, value: data.testObservation || '', onChange: function (event) { patchData({ testObservation: event.target.value }); }, placeholder: __alloT('stem.wheelandfire.record_what_you_actually_saw_measured_or_not','Record what you actually saw, measured, or noticed. Keep it separate from the model result.'), className: 'block w-full mt-1 rounded-lg border border-slate-500 p-2 font-normal bg-white' })),
      ...
      function journalPanel() {
        ...
        function savePiece() {
          ...
          if (!name) { announce(__alloT('stem.wheelandfire.sr_name_the_piece_before_saving_it', 'Name the piece before saving it.')); return; }
        ...
        return h('section', { id: 'wheel-fire-panel-journal', role: 'tabpanel', 'aria-labelledby': 'wheel-fire-tab-journal', className: 'space-y-3' },
          ...
          h('div', { className: 'grid md:grid-cols-2 gap-3' },
            h('div', { className: 'rounded-xl border border-slate-300 bg-white p-3 space-y-3' },
              ...
              h('label', { className: 'block text-xs font-bold text-slate-700' }, 'Artist statement and process reflection', h('textarea', { rows: 6, value: data.artistStatement || '', onChange: function (event) { patchData({ artistStatement: event.target.value }); }, placeholder: __alloT('stem.wheelandfire.what_did_you_intend_which_material_evidence','What did you intend? Which material evidence changed your decisions? If you studied a named tradition, how will you credit it without claiming authenticity?'), className: 'block w-full mt-1 rounded-lg border border-slate-400 p-2 font-normal' })),
      ...
      return h('div', { className: 'wheel-fire-shell space-y-3 rounded-2xl bg-gradient-to-br from-amber-50 via-orange-50 to-stone-100 p-3 sm:p-4', 'data-wheel-fire-lab': 'true', 'data-experience-mode': experienceMode },
        ...
        h('nav', { role: 'tablist', 'aria-label': __alloT('stem.wheelandfire.a11y_wheel_and_fire_sections', 'Wheel and Fire sections'), 'aria-orientation': 'horizontal', className: 'flex flex-wrap gap-2 rounded-xl border border-amber-600 bg-white p-2' },
