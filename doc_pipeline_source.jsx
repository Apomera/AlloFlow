      ...
      if (extractedImages.length > 0) {
        ...
        bodyContent = bodyContent.replace(/<figure data-img-placeholder="true"[\s\S]*?<\/figure>/gi, (match) => {
          ...
          const altText = captionMatch ? captionMatch[1].replace(/<[^>]*>/g, '').trim() : 'Image';
          {
            ...
            const _insertFn2 = `function(c, dataUrl, altText){var target=c.querySelector('img');if(target){target.src=dataUrl;if(altText)target.alt=altText;}else{target=document.createElement('img');target.src=dataUrl;target.alt=altText||'Image';target.style.cssText='max-width:100%;border-radius:8px;border:1px solid #e2e8f0';c.appendChild(target);}c.style.background='none';c.style.border='none';c.style.padding='0';c.style.minHeight='0';Array.from(c.children).forEach(function(ch){if(ch!==target)ch.remove();});c.removeAttribute('ondragover');c.removeAttribute('ondragleave');c.removeAttribute('ondrop');}`;
