...
const out = path.join(ROOT, 'scratch', 'water-surface-review');
...
(async () => {
  ...
  try {
    ...
    await page.evaluate(()=>{const ocean=pilotReview.scene.getObjectByName('pilot-ocean-surface');window.surfaceDisposal={material:0,geometry:0};ocean.material.addEventListener('dispose',()=>surfaceDisposal.material++);ocean.geometry.addEventListener('dispose',()=>surfaceDisposal.geometry++);mountWater({wcMode:'explorer'});});
    await page.waitForFunction(()=>surfaceDisposal.material===1&&surfaceDisposal.geometry===1);
    assert.deepEqual(errors,[],'No JavaScript or WebGL shader errors');
    console.log('PASS four climates, live shader compilation, paused/reduced-motion clocks, water-level/phone views, exactly-once surface disposal');
