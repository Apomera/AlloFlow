...
describe('Ecosystem tutorial dialog accessibility', () => {
  let host;
  let reactRoot;
  let config;
  ...
  async function renderHarness() {
    function Harness() {
      const [toolData, setToolData] = React.useState({
        ecosystem: { tutorialDismissed: false, tutorialStep: 0 },
      });
  ...
  it('opens as a named modal with safe initial focus and inert background', async () => {
    ...
    expect(dialog.getAttribute('aria-labelledby')).toBe('ecosystem-tutorial-title');
    expect(dialog.getAttribute('aria-describedby')).toBe('ecosystem-tutorial-description');
    expect(document.activeElement.getAttribute('aria-label')).toBe('Dismiss Tutorial');
    expect(host.querySelector('[data-ecosystem-tool="true"] > [inert]')).not.toBeNull();
  });
  ...
  it('contains focus, dismisses with Escape, restores focus, and removes isolation', async () => {
    ...
    expect(host.querySelector('[data-ecosystem-tool="true"] > [inert]')).toBeNull();
  });

  it('updates the named step without closing the dialog', async () => {
    await renderHarness();
    await settle();
    ...
    expect(host.querySelector('#ecosystem-tutorial-title').textContent).toContain('Step 2 of 5');
    expect(Array.from(host.querySelectorAll('[role="dialog"] button')).some((button) => button.textContent.includes('Back'))).toBe(true);
  });

  it('keeps accessible dialog behavior in source and deploy copies', () => {
