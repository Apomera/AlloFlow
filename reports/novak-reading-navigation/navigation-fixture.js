    ...
    "instructional_context_module.js"(exports, module) {
      (function() {
        ...
        function isSupportedOriginal(item) {
          ...
          return !!snapshot && getInstructionalText(item).form === "same-text-supported" && item.data === snapshot.text;
        }
        ...
        function getSourceInstructionalText(item) {
          ...
          var snapshot = getSourceSnapshot(source2);
        ...
        function createSupportedReading(textOrSnapshot, options) {
          var opts = isObject(options) ? options : {};
          var snapshot = typeof textOrSnapshot === "string" ? createSourceSnapshot(textOrSnapshot, opts) : getSourceSnapshot(textOrSnapshot);
        ...
        function ensureReadingSourcePairs(items, options) {
          ...
          source2.forEach(function(item) {
            ...
            if (snapshot && !originals.some(function(original2) {
              return sameReadingSourceFamily(original2, item) && original2.data === snapshot.text;
            })) {
        ...
        function resolveReadingSource(options) {
          ...
          function resolved(artifact, selection, textOverride, suppliedSnapshot) {
            ...
            if (!artifact && !snapshot && profile.form !== "adapted") snapshot = createSourceSnapshot(text, { language: opts.language, selection });
            var unitId = artifact ? readingUnit(artifact) : scope;
            var sourceRole = opts.sourceInstructionalText ? normalizeSourceInstructionalText(opts.sourceInstructionalText) : artifact ? getSourceInstructionalText(artifact) : profile.form === "adapted" ? getSourceInstructionalText({ type: "simplified", instructionalText: profile, sourceSnapshot: snapshot }) : normalizeSourceInstructionalText(profile);
            var familyId = cleanText(opts.sourceFamilyId || artifact && getReadingSourceFamilyId(artifact) || snapshot && snapshot.sourceArtifactId || snapshot && "input-" + (unitId || "uncategorized") + "-" + snapshot.fingerprint, 200) || null;
            return {
              status: "resolved",
              selection,
              text,
              ...
              sourceArtifactId: snapshot ? snapshot.sourceArtifactId : null,
              sourceFamilyId: familyId,
              unitId,
              instructionalText: profile,
        ...
        function summarizeReadingAccess(items, options) {
          ...
          var missingSource = adaptations.filter(function(item) {
            ...
            return !snapshot || !readings.some(function(candidate) {
              return candidate !== item && sameReadingSourceFamily(candidate, item) && getInstructionalText(candidate).form !== "adapted" && readableArtifactText(candidate) === snapshot.text && (candidate.type === "analysis" || isSupportedOriginal(candidate));
            });
          });
        ...
        function validSupportAnchor(snapshot, entry) {
          return !!snapshot && isObject(entry) && Number.isInteger(entry.start) && Number.isInteger(entry.end) && entry.start >= 0 && entry.end > entry.start && entry.end <= snapshot.text.length && isSourceTextBoundary(snapshot.text, entry.start) && isSourceTextBoundary(snapshot.text, entry.end) && typeof entry.quote === "string" && snapshot.text.slice(entry.start, entry.end) === entry.quote;
        }
        function validateReadingSupports(snapshotValue, candidates) {
          ...
          var invalidSource = !snapshot || envelope.schemaVersion !== void 0 && envelope.schemaVersion !== 1 || envelope.sourceFingerprint && envelope.sourceFingerprint !== snapshot.fingerprint || readingSupportScopeMismatch(scope, envelope);
          ...
          (Array.isArray(envelope.suppressedAnnotations) ? envelope.suppressedAnnotations : []).forEach(function(entry) {
            if (invalidSource || !validSupportAnchor(snapshot, entry)) {
              rejectedCount++;
              return;
            }
          ...
          entries.forEach(function(entry) {
            var valid = !invalidSource && validSupportAnchor(snapshot, entry) && typeof entry.text === "string" && !!entry.text.trim() && entry.text.length <= 2400 && ["gloss", "definition", "explanation"].indexOf(entry.kind || "gloss") !== -1;
          ...
          function validatedRanges(value) {
            ...
            value.forEach(function(entry) {
              var valid = !invalidSource && isObject(entry) && Number.isInteger(entry.start) && Number.isInteger(entry.end) && entry.start >= 0 && entry.end > entry.start && entry.end <= snapshot.text.length && isSourceTextBoundary(snapshot.text, entry.start) && isSourceTextBoundary(snapshot.text, entry.end);
        ...
        function requireReadingSupportState(snapshotValue, supports) {
          ...
          if (envelope.schemaVersion !== void 0 && envelope.schemaVersion !== 1 || envelope.sourceFingerprint && envelope.sourceFingerprint !== snapshot.fingerprint || readingSupportScopeMismatch(readingSupportScope(snapshotValue), envelope)) {
        ...
        function measureSourceComplexity(artifactText, calculateReadability) {
          ...
          if (!snapshot) return null;
          var hasCounts = snapshot.words !== null && snapshot.words > 0 && snapshot.sentences !== null && snapshot.sentences > 0 && snapshot.syllables !== null && snapshot.syllables >= 0;
          ...
          var rawGrade = hasCounts ? 0.39 * averageSentenceLength + 11.8 * averageSyllablesPerWord - 15.59 : _finiteNumber(snapshot.score);
    ...
    "view_simplified_module.js"() {
      (function() {
        ...
        function resolveSimplifiedCompareSource(history, adaptedItem, fallbackText) {
          var safeHistory = Array.isArray(history) ? history : [];
          var api2 = getInstructionalContextApi();
          var snapshot = api2 && api2.getSourceSnapshot && api2.getSourceSnapshot(adaptedItem);
          if (snapshot) return {
            text: snapshot.text,
            artifact: {
          ...
          for (var linkIndex = 0; linkIndex < linkedIds.length; linkIndex += 1) {
            for (var historyIndex = safeHistory.length - 1; historyIndex >= 0; historyIndex -= 1) {
              ...
              if (!api2?.sameReadingSourceFamily?.(candidate, adaptedItem)) continue;
        ...
        function findSimplifiedReadingSupportOwner(history, owner, snapshot) {
          var api2 = getInstructionalContextApi();
          if (!owner || !snapshot || !api2?.sameReadingSourceFamily || !api2?.isSupportedOriginal || !api2?.getSourceSnapshot) return null;
          ...
          return candidates.find(function(item) {
            return api2.isSupportedOriginal(item) && api2.sameReadingSourceFamily(item, owner) && item.data === snapshot.text && sameSimplifiedReadingSnapshot(api2.getSourceSnapshot(item), snapshot);
          }) || null;
        }
        function findSimplifiedComparisonSupportOwner(history, current, comparison, displayedText) {
          ...
          var snapshot = api2.getSourceSnapshot?.(owner);
          if ((!snapshot || snapshot.text !== comparison.text) && getSimplifiedInstructionalText(owner).form !== "adapted") {
            snapshot = api2.createSourceSnapshot?.(comparison.text, {
        ...
        function findSimplifiedReadingCompanion(history, original2, snapshot) {
          var api2 = getInstructionalContextApi();
          if (!original2 || !snapshot || !api2?.sameReadingSourceFamily || !api2?.getSourceSnapshot) return null;
          ...
          return (Array.isArray(history) ? history : []).slice().reverse().find(function(item) {
            return item?.type === "simplified" && getSimplifiedInstructionalText(item).form === "adapted" && api2.sameReadingSourceFamily(item, original2) && sameSimplifiedReadingSnapshot(api2.getSourceSnapshot(item), snapshot);
          }) || null;
        }
        ...
        function getReadingNavigationSections(text) {
          const source2 = typeof text === "string" ? text : "";
          const sections = [];
          const breaks = /(?:\r\n|\n|\r(?!\n)|\u2028)[\t ]*(?:\r\n|\n|\r(?!\n)|\u2028)(?:[\t ]*(?:\r\n|\n|\r(?!\n)|\u2028))*|\u2029+/g;
          let start = 0, match;
          const append = (end) => {
            const sectionText = source2.slice(start, end);
          ...
          while (match = breaks.exec(source2)) {
            append(match.index);
            start = match.index + match[0].length;
          }
          append(source2.length);
          return sections;
        }
        function findRelatedOriginalPassage(sourceText, adaptedSectionText, options = {}) {
          ...
          if (sourceText.length > 12e4 || adaptedSectionText.length > 12e3) return unavailable("passage-too-long");
          ...
          let segmenter;
          try {
            if (typeof Intl !== "undefined" && Intl.Segmenter) segmenter = new Intl.Segmenter(void 0, {
              granularity: "word"
            });
          } catch (_) {
          }
          ...
          if (totalWords > 24e3) return unavailable("passage-too-long");
          ...
          const queryPhrases = /* @__PURE__ */ new Set();
          ...
          for (let start = 0; start < prepared.length; start++) {
            for (let count = 1; count <= 3 && start + count <= prepared.length; count++) {
              ...
              if (queryCoverage < 0.55 || sourceCoverage < 0.2) continue;
              ...
              if (distinctive.length < 2 && !(phrases.size >= 2 && queryCoverage >= 0.9)) continue;
              if (!phrases.size && (shared.length < 5 || queryCoverage < 0.65)) continue;
              const score = queryCoverage * 0.72 + Math.min(1, sourceCoverage / 0.55) * 0.22 + Math.min(0.1, phrases.size * 0.025) - (count - 1) * 0.045;
              candidates.push({
                start: group[0].start,
          ...
          const alternatives = candidates.slice(1).filter((candidate) => !nested(best, candidate) && (candidate.score >= best.score * 0.9 || best.score - candidate.score < 0.08));
          ...
          return {
            ...
            confidence: best.evidence.queryCoverage >= 0.8 && best.evidence.sharedTerms.length >= 6 ? "high" : "moderate"
          };
        }
        function findReadingGlossOccurrences(text, query) {
          const source2 = String(text || ""), word = String(query || "").trim(), matches = [];
          ...
          let segments;
          try {
            segments = typeof Intl.Segmenter === "function" ? new Intl.Segmenter(void 0, {
              granularity: "word"
            }).segment(source2) : null;
          } catch (_) {
          }
          const boundary = (offset, first) => {
            ...
            const adjacent = first ? Array.from(source2.slice(Math.max(0, offset - 2), offset)).pop() : Array.from(source2.slice(offset, offset + 2))[0];
            if (adjacent && letters.test(adjacent)) return false;
            if (adjacent && /['’]/u.test(adjacent)) {
              const beyond = first ? Array.from(source2.slice(Math.max(0, offset - 3), offset - 1)).pop() : Array.from(source2.slice(offset + 1, offset + 3))[0];
              if (beyond && letters.test(beyond)) return false;
            }
            return true;
          };
          let cursor = 0;
          while (cursor < source2.length) {
            const start = source2.indexOf(word, cursor);
            if (start < 0) break;
            ...
            matches.push({
              ...
              quote: source2.slice(start, end),
              context: source2.slice(Math.max(0, start - 35), Math.min(source2.length, end + 45)).replace(/\s+/g, " ").trim()
            });
          }
          return finish(false);
        }
        function ReadingGlossEditor({
          ...
          const [baseline, setBaseline] = React3.useState("");
          const [pendingAction, setPendingAction] = React3.useState(null);
          ...
          const termRef = React3.useRef(null), definitionRef = React3.useRef(null), addRef = React3.useRef(null), toggleRef = React3.useRef(null), keepRef = React3.useRef(null);
          ...
          return /* @__PURE__ */ React3.createElement("section", {
          ...
          }, "Add a word or phrase"), pendingAction && /* @__PURE__ */ React3.createElement("div", {
            role: "alertdialog",
            ...
            className: "font-bold"
          }, "Discard unsaved word support changes?"), /* @__PURE__ */ React3.createElement("p", {
            id: panelId + "-discard-description"
          }, "Your explanation has not been saved. Keep editing to preserve this draft, or discard it to continue."), /* @__PURE__ */ React3.createElement("div", {
          ...
          }, "Keep editing"), /* @__PURE__ */ React3.createElement("button", {
            type: "button",
          ...
          }, "Discard changes"))), draft && /* @__PURE__ */ React3.createElement("fieldset", {
            disabled: disabled || busy || !!pendingAction,
            ...
            ref: termRef,
            type: "text",
            maxLength: draft.anchor ? void 0 : 160,
            value: draft.query,
          ...
          })), !draft.anchor && matches.length > 1 && /* @__PURE__ */ React3.createElement("label", {
            className: "block font-semibold"
          ...
          }, index + 1, ". Text position ", match.start + 1, "\u2013", match.end, ": ", match.context)))), matches.truncated && /* @__PURE__ */ React3.createElement("p", {
            role: "status",
            className: "text-amber-900"
          }, "Showing the first 200 matching occurrences. Use a longer phrase to find a later occurrence."), !!draft.query.trim() && !matches.length && /* @__PURE__ */ React3.createElement("p", {
            role: "status",
            className: "text-amber-900"
          }, "No exact whole-word or phrase match. Copy the word or phrase as it appears in the original."), selected && /* @__PURE__ */ React3.createElement("p", {
          ...
          })), draft.text.length > 2400 && /* @__PURE__ */ React3.createElement("p", {
            className: "text-amber-900"
          }, "This saved explanation is longer than the current limit. Shorten it to 2,400 characters or fewer to save an edit."), /* @__PURE__ */ React3.createElement("label", {
            className: "block font-semibold"
          ...
          }, occurrenceLabel(entry)), /* @__PURE__ */ React3.createElement("p", {
        ...
        function SimplifiedView(props) {
          ...
          var getReadAloudSentenceEntriesForText = function(rawText) {
            ...
            if (parts) {
              ...
              var marker2 = "--- ENGLISH TRANSLATION ---";
              var markerIndex = text.indexOf(marker2);
              if (markerIndex >= 0) {
                sourceList = splitBlock(text.slice(0, markerIndex));
                targetList = splitBlock(text.slice(markerIndex + marker2.length));
              } else {
          ...
          var readerSurfaceRef = React3.useRef(null);
          var navigationRef = React3.useRef({
            positions: /* @__PURE__ */ new Map(),
            companions: [],
            pending: null
          });
          var [comparisonPreferences, setComparisonPreferences] = React3.useState({});
          var [comparisonReturn, setComparisonReturn] = React3.useState(null);
          var [linkPassages, setLinkPassages] = React3.useState(false);
          var [linkedSection, setLinkedSection] = React3.useState(0);
          var [relatedPassage, setRelatedPassage] = React3.useState(null);
          var readingFingerprint = (value) => readingContract?.fingerprintText ? readingContract.fingerprintText(String(value || "")) : String(value || "");
          ...
          var comparisonPreferenceKey = JSON.stringify([companion?.id || generatedContent?.id, capturedSource?.fingerprint, readingFingerprint(companion?.data)]);
          ...
          function openOriginalReading(item, expectedText, focusPassage) {
            ...
            if (isCompareMode) setComparisonReturn({
              ...
              expectedText: expectedText || capturedSource?.text || "",
              sourceItem: item
            });
            if (typeof stopPlayback === "function") stopPlayback();
          ...
          function renderSimplifiedComparison() {
            ...
            var sameLanguage = !!originalLanguage && !!targetLanguage && !!simplifiedLanguageTag(originalLanguage) && !!simplifiedLanguageTag(targetLanguage) && !mismatch && !/[+&/]|bilingual|multilingual/i.test(originalLanguage + targetLanguage);
            ...
            return /* @__PURE__ */ React3.createElement("div", {
              ...
              value: "english"
            }, simplifiedEnglishTranslationLabel))), /* @__PURE__ */ React3.createElement("label", {
              className: "inline-flex min-h-11 items-center gap-2"
            }, /* @__PURE__ */ React3.createElement("input", {
              type: "checkbox",
            ...
            }), "Link passages")), linkPassages && /* @__PURE__ */ React3.createElement("div", {
            ...
            }, "Choose a passage from the adaptation to look for related wording in the original. A link suggests a place to read; it does not verify the adaptation."), showDiff ? /* @__PURE__ */ React3.createElement("p", {
              role: "status"
            }, "Turn off Show changes to link passages.") : !sameLanguage ? /* @__PURE__ */ React3.createElement("p", {
              role: "status"
            }, "Passage links need two versions in the same known language.") : /* @__PURE__ */ React3.createElement("div", {
              className: "flex flex-wrap items-end gap-3"
            }, /* @__PURE__ */ React3.createElement("label", {
              className: "min-w-0 flex-1 text-sm font-semibold"
            }, "Adapted passage", /* @__PURE__ */ React3.createElement("select", {
              ...
              className: "mt-1 block w-full min-w-0 rounded border border-slate-300 p-2"
            }, sections.map((section) => /* @__PURE__ */ React3.createElement("option", {
              key: section.index,
              value: section.index
            }, section.index + 1, ". ", section.text.replace(/\s+/g, " ").slice(0, 85))))), /* @__PURE__ */ React3.createElement("button", {
              type: "button",
              ...
              type: "button",
              className: "min-h-11 rounded border border-indigo-300 px-3 text-indigo-900",
              onClick: () => openOriginalReading(selected ? original2.artifact : capturedSource ? generatedContent : original2.artifact, original2.text, true)
          ...
          function renderExactPassage(raw, pane, language, annotations, annotationOwner, relatedRange) {
            ...
            return String(raw).split(/(\r\n|\r|\n)/).map(function(line, lineIndex) {
              ...
              return /* @__PURE__ */ React3.createElement("span", {
                ...
                "data-related-original": isRelated ? "true" : void 0,
          ...
          var returnComparisonItem = protectedOriginal && comparisonReturn && generatedContent.data === comparisonReturn.expectedText && readingContract?.sameReadingSourceFamily?.(generatedContent, comparisonReturn.sourceItem) && (history || []).find((item) => item.id === comparisonReturn.id && readingFingerprint(item.data) === comparisonReturn.fingerprint && getSimplifiedInstructionalText(item).form === "adapted");
          ...
          }, "Both")), returnComparisonItem && /* @__PURE__ */ React3.createElement("button", {
            type: "button",
            className: "min-h-11 rounded-lg border border-indigo-300 px-3",
            onClick: () => switchReadingVersion(returnComparisonItem, true)
          }, "Back to comparison"), protectedOriginal && isTeacherMode && props.onCreateAdaptedCompanion && /* @__PURE__ */ React3.createElement("button", {
            type: "button",
            disabled: isProcessing,
...
${t("analysis.readability.syllables") || "Syllables"}: ${stats.syllables || "\u2014"}`
  ...
  // reports/novak-reading-navigation/navigation-fixture.jsx
  ...
  var extension = " Field researchers return during another season to compare their notes carefully. They measure the same location, record the weather, and explain any uncertainty before drawing a conclusion.";
  var source = "  COASTAL FIELD NOTES\r\n\r\n" + marine.map((text, i) => i < 3 ? text : text + extension).join("\r\n\r\n") + "\r\n";
  var olderText = marine.map((text, i) => i === 1 ? "Young crabs hide among mangrove roots that trap sediment along the coast." : text + (i > 3 ? " The observers record this habitat in their field journal." : "")).join("\n\n");
  var newerText = "NEWER COMPANION\n\n" + marine.slice().reverse().map((text) => text + " This is the newer companion.").join("\n\n");
  ...
  function makeAdapted(id, text, original2, language = "English") {
    return {
      id,
      type: "simplified",
      title: id,
      data: text,
      sourceSnapshot: original2.sourceSnapshot,
      sourceFamilyId: original2.sourceFamilyId,
      unitId: original2.unitId,
      sourceInstructionalText: api.getSourceInstructionalText(original2),
      instructionalText: { role: "supplemental", form: "adapted", sourceArtifactId: original2.sourceSnapshot.sourceArtifactId },
      config: { language, grade: "5" }
    };
  }
  var original = makeOriginal("original-coast", source, "coastal-source");
  var older = makeAdapted("adapted-older", olderText, original);
  var newer = makeAdapted("adapted-newer", newerText, original);
  var alternate = makeOriginal(
    "alternate-original",
  ...
  var ambiguousOriginal = makeOriginal("ambiguous-original", [marine[1], marine[0], marine[1]].join("\r\n\r\n"), "ambiguous-source", "ambiguous-lesson");
  var ambiguous = makeAdapted("adapted-ambiguous", marine[1], ambiguousOriginal);
  var weak = makeAdapted("adapted-unavailable", "Astronauts inspect lunar rocks and record crater measurements.", original);
  var spanish = [
    "Los peces de colores encuentran refugio entre los corales.",
    "Los cangrejos j\xF3venes se esconden entre las ra\xEDces de los manglares que atrapan sedimentos junto a la costa.",
    "Las tortugas verdes comen las hojas largas de las praderas marinas."
  ].join("\n\n");
  var bilingual = makeAdapted("adapted-bilingual", spanish + "\n\n" + marker + "\n\n" + olderText, original, "Spanish");
  var initialHistory = [original, older, newer, alternate, ambiguousOriginal, ambiguous, weak, bilingual];
  var noop = () => {
  };
  ...
  function App() {
    ...
    const [history, setHistory] = import_react.default.useState(initialHistory);
    const [item, setItem] = import_react.default.useState(() => initialHistory.find((row) => row.id === params.get("item")) || older);
    const [compare, setCompare] = import_react.default.useState(params.get("compare") === "1");
    const [mode, setMode] = import_react.default.useState("read");
    const [playing, setPlaying] = import_react.default.useState(false);
    const [playingId, setPlayingId] = import_react.default.useState(null);
    const [focused, setFocused] = import_react.default.useState(null);
    const [lineFocus, setLineFocus] = import_react.default.useState(false);
    const [selectionMenu, setSelectionMenu] = import_react.default.useState(null);
    const [theme, setTheme] = import_react.default.useState("default");
    const editorRef = import_react.default.useRef(null);
    const events = window.navigationFixture.events;
    const stop = () => {
      events.push({ type: "stop", at: performance.now() });
      setPlaying(false);
      setPlayingId(null);
    };
    ...
    const readOriginal = (target) => {
      ...
      if (!snapshot) {
        events.push({ type: "missing-original" });
        return;
      }
      ...
      if (found) open(found, false);
        ...
        const created = api.createSupportedReading(snapshot, { sourceFamilyId: api.getReadingSourceFamilyId(target), unitId: target.unitId });
        setHistory((rows) => [...rows, created]);
        open(created, false);
      }
    };
    Object.assign(window.navigationFixture, {
      ...
      history,
      open: (id, both) => open(history.find((row) => row.id === id), both),
      clearEvents: () => {
        events.length = 0;
      }
    });
    ...
    const props = {
      ...
      inputText: "Ambient input must never become the source.",
      history,
      setHistory,
      gradeLevel: "5",
      leveledTextLanguage: item.config?.language || "English",
      studentInterests: [],
      selectedVoice: "Kore",
      voiceSpeed: 1,
      isTeacherMode: true,
      ...
      isProcessing: false,
      isPlaying: playing,
      playingContentId: playingId,
      interactionMode: mode,
      setInteractionMode: setMode,
      setIsCompareMode: setCompare,
      setIsFluencyMode: noop,
      setGeneratedContent: setItem,
      ...
      setPhonicsData: noop,
      closeDefinition: noop,
      closePhonics: noop,
      closeRevision: noop,
      handleToggleIsEditingLeveledText: noop,
      splitTextToSentences: (text) => pure.splitTextToSentences(text, {}),
      getSideBySideContent: bilingualParts,
      ...
      handleQuickAddGlossary: noop,
      isLineFocusMode: lineFocus,
      setIsLineFocusMode: setLineFocus,
      focusedParagraphIndex: focused,
      setFocusedParagraphIndex: setFocused,
      ...
      renderFormattedText: (text) => import_react.default.createElement("div", null, text),
      formatInteractiveText: (text, cloze) => phase.formatInteractiveText(text, cloze, false, {
        highlightGlossaryTerms: (value) => value,
        latestGlossary: [],
        MathSymbol: ({ text: text2 }) => text2
      }),
      SourceReferencesPanel: () => null,
      playbackState: { currentIdx: -1 },
      handleTextMouseUp: noop,
      highlightGlossaryTerms: (value) => value,
      latestGlossary: [],
      onReadOriginal: readOriginal,
      onOpenReadingArtifact: open,
      readingTheme: theme,
      setReadingTheme: setTheme,
      ComplexityGauge: () => null,
      setComplexityLevel: noop,
      setSaveOriginalOnAdjust: noop,
      handleToggleIsZenMode: noop
    };
    return /* @__PURE__ */ import_react.default.createElement("main", { className: "fixture-shell" }, /* @__PURE__ */ import_react.default.createElement("header", { className: "fixture-header" }, /* @__PURE__ */ import_react.default.createElement("h1", null, "Reading navigation QA"), /* @__PURE__ */ import_react.default.createElement("p", null, "Selected: ", /* @__PURE__ */ import_react.default.createElement("output", { "data-fixture-current": true }, item.id), " \xB7 ", compare ? "Both" : api.isSupportedOriginal(item) ? "Original" : "Adapted")), /* @__PURE__ */ import_react.default.createElement("div", { "data-allo-anno-host": "true", "data-fixture-scroller": true, className: "fixture-scroller" }, /* @__PURE__ */ import_react.default.createElement(View, { ...props }), /* @__PURE__ */ import_react.default.createElement("footer", { className: "fixture-footer" }, "End of the selected reader view.")));
  }
