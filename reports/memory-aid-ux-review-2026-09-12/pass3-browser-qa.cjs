...
(async()=>{
 ...
 try{
  ...
  const audit=async(name,screenshot=false)=>{
   ...
   if(screenshot)await page.screenshot({path:path.join(out,'pass3-'+name+'.png'),fullPage:true});
  };
  ...
  assert.equal(result.errors.length,0);fs.writeFileSync(path.join(out,'pass3-browser-qa.json'),JSON.stringify(result,null,2));console.log(JSON.stringify({states:result.states.length,mobileRecall:result.mobileRecall,checks:result.checks,errors:result.errors}));
 }catch(e){result.failure=e.stack;fs.writeFileSync(path.join(out,'pass3-browser-qa.json'),JSON.stringify(result,null,2));throw e;}finally{await browser.close();}
