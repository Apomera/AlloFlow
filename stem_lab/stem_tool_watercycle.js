...
const d = labToolData.waterCycle;
          ...
          const canvasRef = function (canvasEl) {
            ...
            // Click handler for journey decisions
            canvasEl.addEventListener('click', function(e) {
              ...
              // Three choice zones
              if (mx < 0.4 && my > 0.55 && my < 0.75) {
                // River runoff (left area)
                journey.state = 'river_runoff'; journey.progress = 0; journey.particleTrail = [];
              } else if (mx > 0.25 && mx < 0.5 && my > 0.65) {
                // Infiltration (center-low area)
                journey.state = 'infiltrating'; journey.progress = 0; journey.particleTrail = [];
              } else if (mx > 0.45 && my > 0.45 && my < 0.70) {
                // Plant absorption (right area near trees)
                journey.state = 'plant_absorb'; journey.progress = 0; journey.particleTrail = [];
              }
            });
            ...
            function draw() {
              ...
              if (jState !== 'idle') {
                ...
                if (jState === 'ground_choice') {
                  ...
                  // Choice highlight zones
                  var choices = [
                    { x: 0.20, y: 0.63, w: 0.18, h: 0.08, label: '\uD83C\uDF0A River Path', color: 'rgba(59,130,246,0.2)', border: 'rgba(59,130,246,0.6)' },
                    { x: 0.28, y: 0.72, w: 0.18, h: 0.08, label: '\uD83E\uDEB4 Go Underground', color: 'rgba(120,53,15,0.2)', border: 'rgba(120,53,15,0.6)' },
                    { x: 0.50, y: 0.56, w: 0.18, h: 0.08, label: '\uD83C\uDF3F Enter Plant', color: 'rgba(34,197,94,0.2)', border: 'rgba(34,197,94,0.6)' }
                  ];
