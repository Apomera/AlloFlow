...
function createWordSoundsCore() {
  ...
  const validWordFamilyBoard = (board, word) => {
    ...
    return normalize(word).endsWith(board.rime) && normalize(word).length > board.rime.length && board.options.every((v) => v.endsWith(board.rime) && v.length > board.rime.length) && board.distractors.every((v) => !v.endsWith(board.rime));
  };
...
const WordSoundsGenerator = React.memo(({ glossaryTerms, onStartGame, onClose, callGemini, callImagen, callTTS, gradeLevel, t: tProp, preloadedWords = [], onShowReview, onMinimize, onExpand, isProbeMode, probeActivity, probeGradeLevel, probeForm, selectedVoice, setSelectedVoice, isCanvasEnv, ttsSpeed, onRequestKokoroOffer, wordSoundsLanguage, probeStudentNames = [], universalImageStyle = "" }) => {
  ...
  const compileActivityItems = (items) => {
    ...
    items.forEach((item) => {
      ...
      const rime = suppliedRime && (teacherFamily || word.endsWith(suppliedRime)) ? suppliedRime : packIsEnglish ? (word.match(/[aeiou][a-z]*$/) || [""])[0] : packRimeOf(word);
      ...
      const familyOptions = teacherFamily ? familySource.slice(0, 8) : shuffleForPack(familySource.length ? familySource : wordPool.filter((value) => value !== word && rime && value.endsWith(rime) && value.length > rime.length)).slice(0, word.length <= 3 ? 3 : 5);
      const familyDistractors = teacherFamily ? familyWords(familySpec.distractors).slice(0, 8) : shuffleForPack(wordPool.filter((value) => value !== word && !value.endsWith(rime))).slice(0, word.length <= 3 ? 2 : 4);
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-2 sm:p-4 animate-in motion-reduce:animate-none fade-in" }, /* @__PURE__ */ React.createElement("div", { ref: setupDialogRef, role: "dialog", "aria-modal": "true", "aria-label": tf("word_sounds.setup_dialog_label", "Word Sounds setup"), tabIndex: -1, className: "bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-5xl max-h-[96vh] sm:max-h-[90vh] flex flex-col overflow-hidden border border-slate-400 focus:outline-none" }, /* @__PURE__ */ React.createElement("div", { className: "bg-gradient-to-r from-violet-700 to-indigo-700 p-4 sm:p-6 flex flex-wrap justify-between items-center gap-3 text-white shrink-0" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-4" }, /* @__PURE__ */ React.createElement("div", { className: "bg-white/20 p-3 rounded-2xl backdrop-blur-md" }, /* @__PURE__ */ React.createElement(Sparkles, { size: 32, className: "text-yellow-300 animate-pulse motion-reduce:animate-none" })), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h2", { className: "text-3xl font-black tracking-tight" }, isProbeMode ? `\u{1F4CA} ${(probeActivity || "").charAt(0).toUpperCase() + (probeActivity || "").slice(1)} Probe` : tf("word_sounds.title", "Word Sounds Studio")), /* @__PURE__ */ React.createElement("p", { className: "text-indigo-100 font-medium opacity-90" }, tf("word_sounds.subtitle", "Design your phonics lesson"), " \u2022 ", gradeLevel || "K-2"))), /* @__PURE__ */ React.createElement("div", { className: "flex gap-2" }, /* @__PURE__ */ React.createElement("button", { type: "button", "data-help-key": "ws_gen_minimize", onClick: () => {
    ...
    "button",
    {
      ...
      className: `px-8 py-4 rounded-2xl font-black text-xl shadow-xl transition-all flex items-center gap-3 ${selectedIndices.size > 0 && !isProcessing ? "bg-gradient-to-r from-violet-700 to-indigo-700 text-white hover:scale-105 active:scale-95 hover:shadow-2xl hover:brightness-110" : isProcessing ? "bg-violet-600 text-white cursor-wait" : "bg-slate-100 text-slate-600 cursor-not-allowed"}`
    },
