...
const d = labToolData.solarSystem;
  ...
  var accent = "#4a90d9";
  var border = isDark ? "#444" : "#ccc";
  var cardBg = isDark ? "#2a2a3e" : "#f0f4f8";
      ...
      if (sb) {
        ...
        bodyInfoCard = card([
          h("div", { key: "hdr", style: { display: "flex", justifyContent: "space-between", alignItems: "center" } },
            h("strong", null, sb.emoji + " " + sb.name),
            h("button", {
              onClick: function() { upd("orr_sel", null); },
              style: { background: "none", border: "none", cursor: "pointer", color: fg, fontSize: "16px" }
            }, "\u2716")
          ),
          h("p", { key: "desc", style: { fontSize: "12px", color: mutedFg, margin: "4px 0" } }, sb.desc),
          h("div", { key: "stats", style: { fontSize: "12px", lineHeight: "1.6" } },
            h("div", null, "Semi-major axis: " + fmt(sb.a, 3) + " AU"),
            h("div", null, "Eccentricity: " + fmt(sb.e, 4)),
            h("div", null, "Period: " + fmt(sb.T, 2) + " yr"),
            h("div", null, "Inclination: " + fmt(sb.i, 1) + "\u00b0"),
            h("div", null, "Perihelion: " + fmt(peri, 3) + " AU (" + fmt(vPeri, 1) + " km/s)"),
            h("div", null, "Aphelion: " + fmt(aph, 3) + " AU (" + fmt(vAph, 1) + " km/s)"),
            h("div", null, "Current r: " + fmt(pos.r, 3) + " AU"),
            h("div", null, "Current v: " + fmt(spd, 2) + " km/s"),
            sb.m > 0 ? h("div", null, "Mass: " + fmt(sb.m, 4) + " M\u2295") : null,
            sb.R > 100 ? h("div", null, "Radius: " + fmt(sb.R, 0) + " km") : null
          )
        ], { marginTop: "8px" });
      }
    }
  ...
  function buildKeplerIIITab() {
    ...
    var tableRows = OB.map(function(b) {
      ...
      var a3 = b.a * b.a * b.a;
      var ratio = T2 / a3;
      return h("tr", { key: "k3r-" + b.id },
        h("td", { style: { padding: "4px 10px", borderBottom: "1px solid " + border } }, b.emoji + " " + b.name),
        h("td", { style: { padding: "4px 10px", borderBottom: "1px solid " + border, textAlign: "right" } }, fmt(b.a, 3)),
        h("td", { style: { padding: "4px 10px", borderBottom: "1px solid " + border, textAlign: "right" } }, fmt(b.T, 2)),
        h("td", { style: { padding: "4px 10px", borderBottom: "1px solid " + border, textAlign: "right" } }, fmt(T2, 2)),
        h("td", { style: { padding: "4px 10px", borderBottom: "1px solid " + border, textAlign: "right" } }, fmt(a3, 2)),
        h("td", { style: { padding: "4px 10px", borderBottom: "1px solid " + border, textAlign: "right", fontWeight: 700, color: Math.abs(ratio - 1) < 0.05 ? "#27ae60" : "#e74c3c" } }, fmt(ratio, 4))
      );
    });
    ...
    var verTable = h("div", { style: { overflowX: "auto", marginTop: "10px" } },
      h("table", { style: { borderCollapse: "collapse", fontSize: "12px", color: fg, width: "100%" } },
        h("thead", null,
          h("tr", null,
            h("th", { style: { padding: "6px 10px", borderBottom: "2px solid " + border, textAlign: "left" } }, "Body"),
            h("th", { style: { padding: "6px 10px", borderBottom: "2px solid " + border, textAlign: "right" } }, "a (AU)"),
            h("th", { style: { padding: "6px 10px", borderBottom: "2px solid " + border, textAlign: "right" } }, "T (yr)"),
            h("th", { style: { padding: "6px 10px", borderBottom: "2px solid " + border, textAlign: "right" } }, "T\u00b2"),
            h("th", { style: { padding: "6px 10px", borderBottom: "2px solid " + border, textAlign: "right" } }, "a\u00b3"),
            h("th", { style: { padding: "6px 10px", borderBottom: "2px solid " + border, textAlign: "right" } }, "T\u00b2/a\u00b3")
          )
        ),
    ...
    return h("div", { className: "space-y-3" },
      ...
      card([
        ...
        h("div", { key: "d1", style: { fontSize: "13px", lineHeight: "1.8", color: fg } },
          h("div", null, "Period T = " + fmt(T, 3) + " yr  (" + fmt(T * 365.25, 1) + " days)"),
          h("div", null, "Perihelion = " + fmt(peri, 4) + " AU  (" + fmt(peri * AU_KM, 0) + " km)"),
          h("div", null, "Aphelion = " + fmt(aph, 4) + " AU  (" + fmt(aph * AU_KM, 0) + " km)"),
          h("div", null, "v at perihelion = " + fmt(vPeri, 2) + " km/s"),
          h("div", null, "v at aphelion = " + fmt(vAph, 2) + " km/s"),
          h("div", null, "Escape v at perihelion = " + fmt(vEsc, 2) + " km/s"),
          h("div", {
            style: {
              marginTop: "6px",
              padding: "6px 10px",
              borderRadius: "6px",
              background: habitable ? "#27ae6022" : "#e74c3c22",
              border: "1px solid " + (habitable ? "#27ae60" : "#e74c3c"),
              fontWeight: 700
            }
          }, habitable
      ...
      card([
        ...
        h("div", { key: "bt", style: { overflowX: "auto" } },
          h("table", { style: { borderCollapse: "collapse", fontSize: "12px", color: fg, width: "100%" } },
            h("thead", null,
              h("tr", null,
                h("th", { style: { padding: "6px 10px", borderBottom: "2px solid " + border, textAlign: "left" } }, "Destination"),
                h("th", { style: { padding: "6px 10px", borderBottom: "2px solid " + border, textAlign: "right" } }, "\u0394v\u2081 (km/s)"),
                h("th", { style: { padding: "6px 10px", borderBottom: "2px solid " + border, textAlign: "right" } }, "\u0394v\u2082 (km/s)"),
                h("th", { style: { padding: "6px 10px", borderBottom: "2px solid " + border, textAlign: "right" } }, "Total (km/s)"),
                h("th", { style: { padding: "6px 10px", borderBottom: "2px solid " + border, textAlign: "right" } }, "Transit")
              )
            ),
  ...
  function buildChallengesTab() {
    ...
    liveSection = card([
      ...
      !liveCorrect[liveIdx] ? h("div", { key: "linput", style: { display: "flex", gap: "8px", alignItems: "center" } },
        h("input", {
          ...
          style: { width: "120px", padding: "6px", borderRadius: "4px", border: "1px solid " + border, background: bg, color: fg }
        }),
    ...
    return h("div", { className: "space-y-3" },
      ...
      card([
        h("p", { key: "cq", style: { fontSize: "14px", color: fg, marginBottom: "10px", lineHeight: "1.6" } }, prob.q),

        // Hints
        hintCount > 0 ? h("div", { key: "hints", style: { marginBottom: "10px" } },
          prob.hints.slice(0, hintCount).map(function(ht, hi) {
            return h("div", { key: "h-" + hi, style: { fontSize: "12px", color: "#e67e22", padding: "3px 0" } }, "\ud83d\udca1 Hint " + (hi + 1) + ": " + ht);
          })
        ) : null,
        ...
        !answered ? h("div", { key: "hinbtn", style: { marginBottom: "10px" } },
          hintCount < prob.hints.length ? h("button", {
            ...
            style: { padding: "4px 12px", borderRadius: "4px", border: "1px solid #e67e22", background: "transparent", color: "#e67e22", cursor: "pointer", fontSize: "12px" }
        ...
        ) : h("div", { key: "acorr", style: { color: "#27ae60", fontWeight: 700, fontSize: "16px", padding: "8px 0" } },
  ...
  function buildTrueFalseTab() {
    ...
    return h("div", { className: "space-y-3" },
      ...
      card([
        h("div", { key: "stmt", style: { fontSize: "16px", color: fg, marginBottom: "16px", lineHeight: "1.6", fontStyle: "italic" } },
          "\u201c" + mc.q + "\u201d"
        ),

        !answered ? h("div", { key: "btns", style: { display: "flex", gap: "12px" } },
          h("button", {
            onClick: function() {
            ...
            style: { padding: "10px 28px", borderRadius: "8px", border: "2px solid #27ae60", background: "#27ae6022", color: "#27ae60", cursor: "pointer", fontWeight: 700, fontSize: "16px" }
          }, "\u2714 TRUE"),
          h("button", {
            onClick: function() {
              var newA = Object.assign({}, tf_ans);
            ...
            style: { padding: "10px 28px", borderRadius: "8px", border: "2px solid #e74c3c", background: "#e74c3c22", color: "#e74c3c", cursor: "pointer", fontWeight: 700, fontSize: "16px" }
          }, "\u2718 FALSE")
        ) : h("div", { key: "result" },
          h("div", {
            style: {
              ...
              borderRadius: "8px",
              marginBottom: "10px",
              background: correct ? "#27ae6022" : "#e74c3c22",
              border: "2px solid " + (correct ? "#27ae60" : "#e74c3c"),
              fontWeight: 700,
              fontSize: "15px",
  ...
  /* ====================================================================
   *  13. GRAVITY ASSIST TAB
   * ==================================================================== */
  function buildGravityAssistTab() {
    var W = 700, H = 560;
    var ga_planet = d.orr_ga_planet || "jupiter";
    var ga_angle = d.orr_ga_angle !== undefined ? d.orr_ga_angle : 45;
    var ga_speed = d.orr_ga_speed !== undefined ? d.orr_ga_speed : 10; // km/s approach
    var planet = OB.filter(function(b) { return b.id === ga_planet; })[0] || OB[5];
    var vPlanet = visViva(planet.a, planet.a); // circular speed
    var animRef = React.useRef(0);

    var cvPanel = h(CanvasPanel, {
      key: "ga-cv",
      width: W, height: H, panZoom: false,
      redrawKey: ga_planet + "_" + ga_angle + "_" + ga_speed,
      draw: function(ctx) {
        animRef.current = (animRef.current + 0.005) % 1;
        ctx.clearRect(0, 0, W, H);
        ctx.fillStyle = isDark ? "#0a0a1a" : "#f0f4ff";
        ctx.fillRect(0, 0, W, H);

        var cx = W / 2, cy = H / 2 - 40;
        var pR = 30; // planet visual radius
        var angleRad = ga_angle * PI / 180;

        // Planet
        ctx.beginPath(); ctx.arc(cx, cy, pR, 0, TAU);
        ctx.fillStyle = planet.color; ctx.fill();
        ctx.font = "bold 14px sans-serif"; ctx.fillStyle = fg; ctx.textAlign = "center";
        ctx.fillText(planet.emoji + " " + planet.name, cx, cy - pR - 10);

        // Sphere of influence (Hill sphere approximation)
        var soiR = 120;
        ctx.beginPath(); ctx.arc(cx, cy, soiR, 0, TAU);
        ctx.strokeStyle = planet.color + "44"; ctx.lineWidth = 1; ctx.setLineDash([6, 4]); ctx.stroke(); ctx.setLineDash([]);
        ctx.font = "10px sans-serif"; ctx.fillStyle = mutedFg; ctx.textAlign = "left";
        ctx.fillText("Sphere of Influence", cx + soiR + 4, cy - 4);

        // Planet velocity arrow (horizontal right)
        var vpLen = vPlanet * 3;
        ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(cx + vpLen, cy);
        ctx.strokeStyle = planet.color; ctx.lineWidth = 2.5; ctx.stroke();
        ctx.beginPath(); ctx.moveTo(cx + vpLen, cy);
        ctx.lineTo(cx + vpLen - 8, cy - 5); ctx.lineTo(cx + vpLen - 8, cy + 5); ctx.closePath();
        ctx.fillStyle = planet.color; ctx.fill();
        ctx.font = "11px sans-serif"; ctx.fillStyle = planet.color;
        ctx.fillText("v_planet = " + fmt(vPlanet, 1) + " km/s", cx + vpLen + 5, cy - 5);

        // Spacecraft approach trajectory (from left, at angle)
        var approachDist = 200;
        var entryX = cx - approachDist * Math.cos(angleRad);
        var entryY = cy + approachDist * Math.sin(angleRad);
        // Hyperbolic deflection: simplified — bend the path around the planet
        var v_inf = ga_speed; // approach speed relative to planet
        var deflection = 2 * Math.asin(1 / (1 + (v_inf * v_inf * soiR * 0.01))); // simplified
        deflection = Math.min(deflection, PI * 0.8);
        var exitAngle = angleRad - deflection;
        var exitX = cx + approachDist * Math.cos(exitAngle);
        var exitY = cy - approachDist * Math.sin(exitAngle);

        // Draw approach path
        ctx.beginPath(); ctx.moveTo(entryX, entryY);
        // Curve through closest approach
        var cpDist = pR + 15;
        var cpAngle = (angleRad + (angleRad - deflection)) / 2 + PI;
        var cpX = cx + cpDist * Math.cos(cpAngle);
        var cpY = cy - cpDist * Math.sin(cpAngle);
        ctx.quadraticCurveTo(cpX, cpY, exitX, exitY);
        ctx.strokeStyle = "#e74c3c"; ctx.lineWidth = 2; ctx.stroke();

        // Animated spacecraft
        var t = animRef.current;
        var scX = entryX + (exitX - entryX) * t + (cpX - (entryX + exitX) / 2) * 4 * t * (1 - t);
        var scY = entryY + (exitY - entryY) * t + (cpY - (entryY + exitY) / 2) * 4 * t * (1 - t);
        ctx.beginPath(); ctx.arc(scX, scY, 4, 0, TAU);
        ctx.fillStyle = "#e74c3c"; ctx.fill();
        ctx.font = "14px sans-serif";
        ctx.fillText("\uD83D\uDE80", scX - 7, scY - 8);

        // Velocity vectors: approach v_in (relative to Sun), exit v_out
        var v_in_sun = Math.sqrt(v_inf * v_inf + vPlanet * vPlanet + 2 * v_inf * vPlanet * Math.cos(angleRad));
        var v_out_sun = Math.sqrt(v_inf * v_inf + vPlanet * vPlanet + 2 * v_inf * vPlanet * Math.cos(exitAngle));
        var deltaV = v_out_sun - v_in_sun;

        // Info panel at bottom
        var iY = H - 150;
        ctx.fillStyle = isDark ? "rgba(10,10,26,0.85)" : "rgba(240,244,255,0.9)";
        ctx.fillRect(0, iY, W, H - iY);
        ctx.strokeStyle = border; ctx.lineWidth = 0.5;
        ctx.beginPath(); ctx.moveTo(0, iY); ctx.lineTo(W, iY); ctx.stroke();

        ctx.font = "bold 13px sans-serif"; ctx.fillStyle = accent; ctx.textAlign = "left";
        ctx.fillText("Gravity Assist Analysis", 15, iY + 20);

        ctx.font = "12px monospace"; ctx.fillStyle = fg;
        var lines = [
          "Approach speed (v\u221E) = " + fmt(v_inf, 1) + " km/s relative to " + planet.name,
          planet.name + " orbital speed = " + fmt(vPlanet, 1) + " km/s",
          "Approach angle = " + fmt(ga_angle, 0) + "\u00b0   Deflection \u2248 " + fmt(deflection * 180 / PI, 1) + "\u00b0",
          "v_in (Sun frame) \u2248 " + fmt(v_in_sun, 1) + " km/s   v_out \u2248 " + fmt(v_out_sun, 1) + " km/s",
          "\u0394v (free!) = " + (deltaV > 0 ? "+" : "") + fmt(deltaV, 2) + " km/s" +
            (deltaV > 0 ? " \u2014 spacecraft GAINED speed" : deltaV < -0.5 ? " \u2014 spacecraft LOST speed (braking)" : " \u2014 minimal change")
        ];
        for (var i = 0; i < lines.length; i++) {
          ctx.fillText(lines[i], 15, iY + 40 + i * 18);
        }

        // Key insight
        ctx.font = "bold 11px sans-serif"; ctx.fillStyle = "#2ecc71";
        ctx.fillText("Key: Speed is conserved in the planet's frame, but the planet is moving \u2014 so the Sun-frame speed changes!", 15, H - 12);

        // Title
        ctx.font = "bold 14px sans-serif"; ctx.fillStyle = fg; ctx.textAlign = "center";
        ctx.fillText("Gravity Assist (Slingshot Maneuver)", W / 2, 20);
      }
    });

    return h("div", { className: "space-y-3" },
      card([
        h("strong", { key: "t" }, "\uD83C\uDF0B Gravity Assist \u2014 Free \u0394v from Planets"),
        h("p", { key: "p", style: { fontSize: "12px", color: mutedFg, margin: "4px 0" } },
          "A spacecraft can gain (or lose) speed by flying past a planet. In the planet's reference frame, speed is conserved (like a ball bouncing off a moving wall). But since the planet is moving, the Sun-frame speed changes. Voyager 2 used this at Jupiter, Saturn, and Uranus!")
      ]),
      h("div", { key: "ctrls", style: { display: "flex", flexWrap: "wrap", gap: "12px", alignItems: "center" } },
        h("label", null, "Planet: "),
        h("select", {
          value: ga_planet,
          onChange: function(ev) { upd("orr_ga_planet", ev.target.value); },
          style: { padding: "4px 8px", borderRadius: "4px", border: "1px solid " + border, background: cardBg, color: fg }
        }, OB.filter(function(b) { return b.type === "planet" && b.a > 0.5; }).map(function(b) {
          return h("option", { key: b.id, value: b.id }, b.emoji + " " + b.name);
        })),
        h("label", null, "Approach angle: "),
        h("input", { type: "range", min: "10", max: "170", step: "1", value: ga_angle,
          onChange: function(ev) { upd("orr_ga_angle", parseInt(ev.target.value)); },
          style: { width: "120px" } }),
        h("span", { style: { fontSize: "12px", color: mutedFg } }, ga_angle + "\u00b0"),
        h("label", null, "Approach v\u221E: "),
        h("input", { type: "range", min: "2", max: "30", step: "0.5", value: ga_speed,
          onChange: function(ev) { upd("orr_ga_speed", parseFloat(ev.target.value)); },
          style: { width: "120px" } }),
        h("span", { style: { fontSize: "12px", color: mutedFg } }, fmt(ga_speed, 1) + " km/s")
      ),
      cvPanel,
      card([
        h("div", { key: "c1", style: { fontSize: "12px", lineHeight: "1.7" } },
          h("div", null, "\u2022 The spacecraft's speed relative to the planet is unchanged (energy conserved in planet's frame)."),
          h("div", null, "\u2022 But the planet orbits at " + fmt(vPlanet, 1) + " km/s \u2014 this velocity gets 'added' or 'subtracted'."),
          h("div", null, "\u2022 Trailing approach: speed boost. Leading approach: speed reduction (used for orbital capture)."),
          h("div", null, "\u2022 Voyager 2 gained ~20 km/s total from Jupiter+Saturn+Uranus gravity assists."),
          h("div", null, "\u2022 The energy comes from the planet's orbital motion (planet slows by an immeasurably tiny amount).")
        )
      ])
    );
  }
  ...
  /* ====================================================================
   *  14. RETROGRADE MOTION TAB
   * ==================================================================== */
  function buildRetrogradeTab() {
    var W = 700, H = 550;
    var animRef = React.useRef(0);
    var trailRef = React.useRef([]);
    var rt_speed = d.orr_rt_speed || 1;
    ...
    var cvPanel = h(CanvasPanel, {
      ...
      redrawKey: rt_speed,
      draw: function(ctx) {
        animRef.current += 0.003 * rt_speed;
        ctx.clearRect(0, 0, W, H);
        ctx.fillStyle = isDark ? "#0a0a1a" : "#f0f4ff";
        ctx.fillRect(0, 0, W, H);

        // Left panel: top-down orbits
        var lcx = W * 0.32, lcy = H * 0.4;
        var eScale = 100; // Earth orbit radius px
        var mScale = eScale * 1.524; // Mars

        // Sun
        ctx.beginPath(); ctx.arc(lcx, lcy, 6, 0, TAU);
        ctx.fillStyle = "#ffdd44"; ctx.fill();

        // Orbits
        ctx.beginPath(); ctx.arc(lcx, lcy, eScale, 0, TAU);
        ctx.strokeStyle = "#3498db44"; ctx.lineWidth = 1; ctx.stroke();
        ctx.beginPath(); ctx.arc(lcx, lcy, mScale, 0, TAU);
        ctx.strokeStyle = "#e74c3c44"; ctx.lineWidth = 1; ctx.stroke();

        // Earth and Mars positions (Earth faster than Mars)
        var t = animRef.current;
        var eAngle = t * TAU; // Earth completes 1 orbit per t unit
        var mAngle = t * TAU / 1.881; // Mars is slower
        var eX = lcx + eScale * Math.cos(eAngle);
        var eY = lcy - eScale * Math.sin(eAngle);
        var mX = lcx + mScale * Math.cos(mAngle);
        var mY = lcy - mScale * Math.sin(mAngle);

        ctx.beginPath(); ctx.arc(eX, eY, 5, 0, TAU); ctx.fillStyle = "#3498db"; ctx.fill();
        ctx.beginPath(); ctx.arc(mX, mY, 4, 0, TAU); ctx.fillStyle = "#e74c3c"; ctx.fill();
        ctx.font = "10px sans-serif"; ctx.fillStyle = fg; ctx.textAlign = "center";
        ctx.fillText("\uD83C\uDF0D", eX, eY - 10);
        ctx.fillText("\uD83D\uDD34", mX, mY - 10);

        // Sightline from Earth to Mars
        ctx.beginPath(); ctx.moveTo(eX, eY); ctx.lineTo(mX, mY);
        ctx.strokeStyle = "#f1c40f88"; ctx.lineWidth = 1; ctx.setLineDash([4, 4]); ctx.stroke(); ctx.setLineDash([]);

        // Right panel: apparent motion on sky (projected onto distant "celestial sphere")
        var rcx = W * 0.75, rcy = H * 0.42;
        var skyW = W * 0.4, skyH = H * 0.5;

        // Sky background
        ctx.fillStyle = isDark ? "#06061a" : "#0a0a2e";
        ctx.fillRect(rcx - skyW / 2, rcy - skyH / 2, skyW, skyH);
        ctx.strokeStyle = "#ffffff22"; ctx.lineWidth = 0.5;
        ctx.strokeRect(rcx - skyW / 2, rcy - skyH / 2, skyW, skyH);
        // Stars
        for (var si = 0; si < 30; si++) {
          ctx.fillStyle = "rgba(255,255,255," + (0.1 + 0.15 * Math.sin(si * 3.7)) + ")";
          ctx.fillRect(rcx - skyW / 2 + (si * 47 + 13) % skyW, rcy - skyH / 2 + (si * 71 + 23) % skyH, 1, 1);
        }

        // Project Mars direction as seen from Earth onto the "sky strip"
        var dx = mX - eX, dy = mY - eY;
        var marsAngle = Math.atan2(-dy, dx); // angle on sky
        // Map angle to x-position on sky strip
        var skyX = rcx + (marsAngle / PI) * skyW * 0.4;
        var skyY = rcy; // keep centered vertically

        // Maintain trail
        var trail = trailRef.current;
        trail.push({ x: skyX, y: skyY, t: t });
        ...
        // Current Mars position on sky
        ctx.beginPath(); ctx.arc(skyX, skyY, 4, 0, TAU);
        ctx.fillStyle = "#e74c3c"; ctx.fill();
        ctx.font = "10px sans-serif"; ctx.fillStyle = "#fff"; ctx.textAlign = "center";
        ctx.fillText("Mars", skyX, skyY - 10);

        // Detect retrograde: is Mars moving "backwards" on the sky?
        var isRetrograde = false;
        if (trail.length > 10) {
          var recent = trail[trail.length - 1].x - trail[trail.length - 10].x;
          // If Mars normally moves left-to-right, retrograde is right-to-left (or vice versa)
          var longTerm = trail.length > 50 ? trail[trail.length - 1].x - trail[Math.floor(trail.length / 2)].x : recent;
          isRetrograde = (recent * longTerm < 0); // opposite directions = retrograde
        }

        // Labels
        ctx.font = "bold 13px sans-serif"; ctx.fillStyle = fg; ctx.textAlign = "center";
        ctx.fillText("Top-Down View (reality)", lcx, 20);
        ctx.fillText("Earth's Sky View (what we see)", rcx, rcy - skyH / 2 - 10);

        if (isRetrograde) {
          ctx.font = "bold 14px sans-serif"; ctx.fillStyle = "#e74c3c";
          ctx.fillText("\u21BA RETROGRADE MOTION!", rcx, rcy + skyH / 2 + 25);
        } else {
          ctx.font = "12px sans-serif"; ctx.fillStyle = mutedFg;
          ctx.fillText("Prograde motion (normal)", rcx, rcy + skyH / 2 + 25);
        }

        // Info at bottom
        ctx.font = "12px sans-serif"; ctx.fillStyle = fg; ctx.textAlign = "left";
        ctx.fillText("Earth overtakes Mars (inner planet is faster) \u2192 Mars appears to move backward on the sky", 15, H - 35);
        ctx.fillText("This 'retrograde motion' puzzled ancient astronomers and inspired Copernicus!", 15, H - 18);
      }
    });

    return h("div", { className: "space-y-3" },
      card([
        h("strong", { key: "t" }, "\u21BA Apparent Retrograde Motion"),
        h("p", { key: "p", style: { fontSize: "12px", color: mutedFg, margin: "4px 0" } },
          "Mars normally drifts east across the stars. But every ~26 months, Earth overtakes Mars in its orbit, and Mars appears to reverse direction for several weeks. This isn't real backward motion \u2014 it's a parallax effect from Earth's faster orbit. The ancient Greeks called these 'wandering stars' (planetes).")
      ]),
      h("div", { key: "ctrls", style: { display: "flex", gap: "12px", alignItems: "center" } },
        h("label", null, "Speed: "),
        h("input", { type: "range", min: "0.2", max: "5", step: "0.1", value: rt_speed,
          onChange: function(ev) { upd("orr_rt_speed", parseFloat(ev.target.value)); },
          style: { width: "150px" } }),
        h("span", { style: { fontSize: "12px", color: mutedFg } }, fmt(rt_speed, 1) + "x"),
        h("button", { onClick: function() { trailRef.current = []; }, style: { padding: "4px 10px", borderRadius: "4px", border: "1px solid " + border, background: cardBg, color: fg, cursor: "pointer", fontSize: "12px" } }, "Clear trail")
      ),
      cvPanel,
      card([
        h("div", { key: "info", style: { fontSize: "12px", lineHeight: "1.7" } },
          h("div", null, "\u2022 Retrograde occurs when Earth passes Mars (opposition) \u2014 Mars is closest and brightest."),
          h("div", null, "\u2022 All outer planets show retrograde; it's more pronounced for nearby Mars."),
          h("div", null, "\u2022 Ptolemy explained it with epicycles; Copernicus showed it's just perspective from a moving Earth."),
          h("div", null, "\u2022 The red trail on the sky view shows Mars's apparent path \u2014 watch for the loop!")
        )
      ])
    );
  }
  ...
  /* ====================================================================
   *  15. LAGRANGE POINTS TAB
   * ==================================================================== */
  function buildLagrangeTab() {
    var W = 700, H = 500;
    var lg_system = d.orr_lg_sys || "sun_earth";
    var systems = {
      sun_earth: { name: "Sun\u2013Earth", m1: 1.989e30, m2: 5.972e24, dist: 1.0, emoji: "\u2609\uD83C\uDF0D", color1: "#ffdd44", color2: "#3498db" },
      earth_moon: { name: "Earth\u2013Moon", m1: 5.972e24, m2: 7.342e22, dist: 0.00257, emoji: "\uD83C\uDF0D\uD83C\uDF19", color1: "#3498db", color2: "#aaa" },
      sun_jupiter: { name: "Sun\u2013Jupiter", m1: 1.989e30, m2: 1.898e27, dist: 5.203, emoji: "\u2609\uD83E\uDE90", color1: "#ffdd44", color2: "#f97316" }
    };
    var sys = systems[lg_system];
    var mu = sys.m2 / (sys.m1 + sys.m2); // mass ratio

    // L-point positions (approximate for small mu)
    var L1_frac = 1 - Math.pow(mu / 3, 1 / 3);
    var L2_frac = 1 + Math.pow(mu / 3, 1 / 3);
    var L3_frac = -(1 + 5 * mu / 12);
    // L4, L5 at equilateral triangle points

    return h("div", { className: "space-y-3" },
      card([
        h("strong", { key: "t" }, "\u2696 Lagrange Points \u2014 Gravitational Balance"),
        h("p", { key: "p", style: { fontSize: "12px", color: mutedFg, margin: "4px 0" } },
          "In any two-body system, there are 5 special points where a small object can maintain a stable position relative to both bodies. L1, L2, L3 are unstable (saddle points); L4 and L5 are stable (objects collect there). JWST orbits at Sun\u2013Earth L2!")
      ]),
      h("div", { key: "ctrls", style: { display: "flex", gap: "12px", alignItems: "center" } },
        h("label", null, "System: "),
        h("select", {
          value: lg_system,
          onChange: function(ev) { upd("orr_lg_sys", ev.target.value); },
          style: { padding: "4px 8px", borderRadius: "4px", border: "1px solid " + border, background: cardBg, color: fg }
        },
          h("option", { value: "sun_earth" }, "\u2609 Sun\u2013Earth"),
          h("option", { value: "earth_moon" }, "\uD83C\uDF0D Earth\u2013Moon"),
          h("option", { value: "sun_jupiter" }, "\u2609 Sun\u2013Jupiter")
        )
      ),
      h(CanvasPanel, {
        key: "lg-cv", width: W, height: H, panZoom: false,
        redrawKey: lg_system,
        draw: function(ctx) {
          ctx.clearRect(0, 0, W, H);
          ctx.fillStyle = isDark ? "#0a0a1a" : "#f0f4ff";
          ...
          var cx = W * 0.42, cy = H * 0.45;
          var orbitR = 200; // pixels for the orbital distance

          // Body 1 (larger, at origin or slightly offset)
          var b1x = cx - mu * orbitR;
          var b1y = cy;
          var b2x = cx + (1 - mu) * orbitR;
          var b2y = cy;

          // Orbit of body 2
          ctx.beginPath(); ctx.arc(b1x, b1y, orbitR, 0, TAU);
          ctx.strokeStyle = sys.color2 + "33"; ctx.lineWidth = 1; ctx.stroke();

          // Body 1
          var b1R = Math.max(8, 20 * (1 - mu));
          ctx.beginPath(); ctx.arc(b1x, b1y, b1R, 0, TAU);
          ctx.fillStyle = sys.color1; ctx.fill();

          // Body 2
          var b2R = Math.max(5, 12 * Math.pow(mu, 0.3));
          ctx.beginPath(); ctx.arc(b2x, b2y, b2R, 0, TAU);
          ctx.fillStyle = sys.color2; ctx.fill();

          // L1: between bodies
          var l1x = b1x + L1_frac * orbitR, l1y = cy;
          // L2: beyond body 2
          var l2x = b1x + L2_frac * orbitR, l2y = cy;
          // L3: opposite side of body 1
          var l3x = b1x + L3_frac * orbitR, l3y = cy;
          // L4: 60° ahead
          var l4x = cx + orbitR * Math.cos(PI / 3);
          var l4y = cy - orbitR * Math.sin(PI / 3);
          // L5: 60° behind
          var l5x = cx + orbitR * Math.cos(-PI / 3);
          var l5y = cy - orbitR * Math.sin(-PI / 3);

          // Draw L-points
          var lPts = [
            { x: l1x, y: l1y, label: "L1", stable: false, note: "Between bodies" },
            { x: l2x, y: l2y, label: "L2", stable: false, note: "Beyond smaller body" },
            { x: l3x, y: l3y, label: "L3", stable: false, note: "Opposite side" },
            { x: l4x, y: l4y, label: "L4", stable: true, note: "60\u00b0 ahead" },
            { x: l5x, y: l5y, label: "L5", stable: true, note: "60\u00b0 behind" }
          ];

          lPts.forEach(function(lp) {
            // Stability zone for L4/L5
            if (lp.stable) {
              ctx.beginPath(); ctx.arc(lp.x, lp.y, 18, 0, TAU);
              ctx.fillStyle = "#2ecc7122"; ctx.fill();
              ctx.strokeStyle = "#2ecc7144"; ctx.lineWidth = 1; ctx.stroke();
            }

            ctx.beginPath(); ctx.arc(lp.x, lp.y, 5, 0, TAU);
            ctx.fillStyle = lp.stable ? "#2ecc71" : "#e74c3c"; ctx.fill();
            ctx.font = "bold 12px sans-serif"; ctx.fillStyle = fg; ctx.textAlign = "center";
            ctx.fillText(lp.label, lp.x, lp.y - 12);
            ctx.font = "9px sans-serif"; ctx.fillStyle = mutedFg;
            ctx.fillText(lp.note, lp.x, lp.y + 18);
          });

          // Equilateral triangle lines (L4/L5)
          ctx.beginPath(); ctx.moveTo(b1x, b1y); ctx.lineTo(l4x, l4y); ctx.lineTo(b2x, b2y); ctx.closePath();
          ctx.strokeStyle = "#2ecc7133"; ctx.lineWidth = 1; ctx.setLineDash([4, 4]); ctx.stroke(); ctx.setLineDash([]);
          ctx.beginPath(); ctx.moveTo(b1x, b1y); ctx.lineTo(l5x, l5y); ctx.lineTo(b2x, b2y); ctx.closePath();
          ctx.strokeStyle = "#2ecc7133"; ctx.stroke();

          // Labels for bodies
          ctx.font = "bold 12px sans-serif"; ctx.textAlign = "center";
          ctx.fillStyle = sys.color1; ctx.fillText(sys.name.split("\u2013")[0], b1x, b1y + b1R + 16);
          ctx.fillStyle = sys.color2; ctx.fillText(sys.name.split("\u2013")[1], b2x, b2y + b2R + 16);

          // Legend
          ctx.font = "11px sans-serif"; ctx.textAlign = "left"; ctx.fillStyle = fg;
          var ly = H - 80;
          ctx.fillStyle = "#e74c3c"; ctx.fillText("\u25cf Unstable (L1, L2, L3) \u2014 objects drift away, need stationkeeping", 15, ly);
          ctx.fillStyle = "#2ecc71"; ctx.fillText("\u25cf Stable (L4, L5) \u2014 objects naturally collect here (Trojan asteroids)", 15, ly + 18);
          ctx.fillStyle = mutedFg; ctx.font = "11px sans-serif";
          ctx.fillText("Mass ratio \u03bc = " + mu.toExponential(3) + "   Distance = " + fmt(sys.dist, 3) + " AU", 15, ly + 40);

          // Title
          ctx.font = "bold 14px sans-serif"; ctx.fillStyle = fg; ctx.textAlign = "center";
          ctx.fillText("Lagrange Points: " + sys.emoji + " " + sys.name + " System", W / 2, 20);
        }
      }),
      card([
        h("div", { key: "info", style: { fontSize: "12px", lineHeight: "1.7" } },
          h("div", null, "\u2022 L1 (Sun\u2013Earth): SOHO solar observatory. Always between Sun and Earth."),
          h("div", null, "\u2022 L2 (Sun\u2013Earth): James Webb Space Telescope. Shielded from Sun by Earth."),
          h("div", null, "\u2022 L4/L5 (Sun\u2013Jupiter): Over 10,000 Trojan asteroids collected there!"),
          h("div", null, "\u2022 L4/L5 are stable because Coriolis force in the rotating frame pushes objects back."),
          h("div", null, "\u2022 L1/L2/L3 are unstable \u2014 JWST uses tiny thrust corrections to stay at L2.")
        )
      ])
    );
  }
  ...
  /* ====================================================================
   *  16. ORBITAL RESONANCE TAB
   * ==================================================================== */
  function buildResonanceTab() {
    var W = 700, H = 520;
    var animRef = React.useRef(0);
    var res_speed = d.orr_res_speed || 1;
    var res_system = d.orr_res_sys || "jupiter_moons";

    var RESONANCE_SYSTEMS = {
      jupiter_moons: {
        name: "Jupiter's Galilean Moons (1:2:4)",
        bodies: [
          { name: "Io", color: "#f1c40f", orbitR: 60, period: 1.769, emoji: "\uD83D\uDFE1" },
          { name: "Europa", color: "#67e8f9", orbitR: 95, period: 3.551, emoji: "\u26AA" },
          { name: "Ganymede", color: "#a1a1aa", orbitR: 135, period: 7.155, emoji: "\u26AB" }
        ],
        central: { name: "Jupiter", color: "#f97316", R: 25, emoji: "\uD83E\uDE90" },
        desc: "For every 1 Ganymede orbit, Europa orbits 2\u00D7, Io orbits 4\u00D7. This 1:2:4 resonance pumps tidal heating into Io (most volcanic body in the solar system) and keeps Europa's subsurface ocean liquid.",
        ratio: "1 : 2 : 4"
      },
      pluto_neptune: {
        name: "Pluto\u2013Neptune (2:3)",
        bodies: [
          { name: "Neptune", color: "#6366f1", orbitR: 80, period: 164.8, emoji: "\uD83D\uDD35" },
          { name: "Pluto", color: "#a78bfa", orbitR: 110, period: 247.9, emoji: "\u2B50" }
        ],
        central: { name: "Sun", color: "#ffdd44", R: 12, emoji: "\u2609" },
        desc: "Pluto completes 2 orbits for every 3 of Neptune. This resonance prevents them from ever colliding, even though Pluto's orbit crosses Neptune's! They are never closer than 17 AU.",
        ratio: "2 : 3"
      },
      saturn_rings: {
        name: "Saturn's Ring Gaps (Kirkwood-like)",
        bodies: [
          { name: "Mimas", color: "#d4d4d8", orbitR: 60, period: 0.942, emoji: "\u26AA" },
          { name: "Ring particle", color: "#eab308", orbitR: 42, period: 0.471, emoji: "\u25CB" }
        ],
        central: { name: "Saturn", color: "#eab308", R: 22, emoji: "\uD83E\uDE90" },
        desc: "Ring particles in 2:1 resonance with Mimas get repeatedly kicked at the same orbital phase, clearing the Cassini Division. Same physics as Kirkwood gaps in the asteroid belt!",
        ratio: "1 : 2"
      }
    };

    var sys = RESONANCE_SYSTEMS[res_system];

    var cvPanel = h(CanvasPanel, {
      key: "res-cv", width: W, height: H, panZoom: false,
      redrawKey: res_system + "_" + res_speed,
      draw: function(ctx) {
        animRef.current += 0.002 * res_speed;
        ctx.clearRect(0, 0, W, H);
        ctx.fillStyle = isDark ? "#0a0a1a" : "#f0f4ff";
        ctx.fillRect(0, 0, W, H);

        var cx = W * 0.38, cy = H * 0.45;

        // Central body
        ctx.beginPath(); ctx.arc(cx, cy, sys.central.R, 0, TAU);
        ctx.fillStyle = sys.central.color; ctx.fill();

        // Saturn's rings (if saturn)
        if (res_system === "saturn_rings") {
          ctx.beginPath(); ctx.ellipse(cx, cy, sys.central.R * 2.5, sys.central.R * 0.8, 0, 0, TAU);
          ctx.strokeStyle = "#eab30844"; ctx.lineWidth = 12; ctx.stroke();
          // Cassini division
          ctx.beginPath(); ctx.ellipse(cx, cy, 42, 42 * 0.32, 0, 0, TAU);
          ctx.strokeStyle = isDark ? "#0a0a1a" : "#f0f4ff"; ctx.lineWidth = 4; ctx.stroke();
        }

        var t = animRef.current;
        var conjunctions = []; // Track conjunction points

        sys.bodies.forEach(function(b, bi) {
          // Orbit
          ctx.beginPath(); ctx.arc(cx, cy, b.orbitR, 0, TAU);
          ctx.strokeStyle = b.color + "44"; ctx.lineWidth = 1; ctx.stroke();

          // Body position
          var angle = (t * TAU * 365.25 / b.period) % TAU;
          var bx = cx + b.orbitR * Math.cos(angle);
          var by = cy - b.orbitR * Math.sin(angle);

          ctx.beginPath(); ctx.arc(bx, by, bi === 0 ? 6 : 5, 0, TAU);
          ctx.fillStyle = b.color; ctx.fill();
          ctx.font = "10px sans-serif"; ctx.fillStyle = fg; ctx.textAlign = "center";
          ctx.fillText(b.name, bx, by - 12);

          conjunctions.push({ x: bx, y: by, angle: angle, name: b.name });
        });

        // Draw conjunction lines when moons are aligned
        if (conjunctions.length >= 2) {
          for (var i = 0; i < conjunctions.length - 1; i++) {
            var angleDiff = Math.abs(conjunctions[i].angle - conjunctions[i + 1].angle) % TAU;
            if (angleDiff > PI) angleDiff = TAU - angleDiff;
            if (angleDiff < 0.15) { // Near conjunction
              ctx.beginPath(); ctx.moveTo(conjunctions[i].x, conjunctions[i].y);
              ctx.lineTo(conjunctions[i + 1].x, conjunctions[i + 1].y);
              ctx.strokeStyle = "#e74c3c88"; ctx.lineWidth = 2; ctx.setLineDash([4, 4]); ctx.stroke(); ctx.setLineDash([]);
            }
          }
        }

        // Right side: orbit counter
        var rcx = W * 0.78, rcy = 80;
        ctx.font = "bold 13px sans-serif"; ctx.fillStyle = fg; ctx.textAlign = "center";
        ctx.fillText("Orbit Count", rcx, rcy - 10);
        ctx.font = "12px monospace"; ctx.textAlign = "left";
        sys.bodies.forEach(function(b, bi) {
          var orbits = (t * 365.25 / b.period);
          var fullOrbits = Math.floor(orbits);
          ctx.fillStyle = b.color;
          ctx.fillText(b.emoji + " " + b.name + ": " + fullOrbits + " orbits", rcx - 60, rcy + 20 + bi * 22);
          // Progress bar
          var frac = orbits - fullOrbits;
          ctx.fillStyle = b.color + "44";
          ctx.fillRect(rcx - 60, rcy + 24 + bi * 22, 120, 4);
          ctx.fillStyle = b.color;
          ctx.fillRect(rcx - 60, rcy + 24 + bi * 22, 120 * frac, 4);
        });

        // Ratio display
        ctx.font = "bold 16px sans-serif"; ctx.fillStyle = accent; ctx.textAlign = "center";
        ctx.fillText("Resonance: " + sys.ratio, rcx, rcy + 20 + sys.bodies.length * 22 + 20);

        // Description at bottom
        ctx.font = "11px sans-serif"; ctx.fillStyle = mutedFg; ctx.textAlign = "left";
        var lines = sys.desc.match(/.{1,85}(\s|$)/g) || [sys.desc];
        lines.forEach(function(line, li) {
          ctx.fillText(line.trim(), 15, H - 50 + li * 16);
        });

        // Title
        ctx.font = "bold 14px sans-serif"; ctx.fillStyle = fg; ctx.textAlign = "center";
        ctx.fillText("Orbital Resonance: " + sys.name, W / 2, 20);
      }
    });

    return h("div", { className: "space-y-3" },
      card([
        h("strong", { key: "t" }, "\uD83C\uDFB5 Orbital Resonance \u2014 Gravitational Harmony"),
        h("p", { key: "p", style: { fontSize: "12px", color: mutedFg, margin: "4px 0" } },
          "When orbital periods form simple ratios (1:2, 2:3, 1:2:4), bodies repeatedly interact at the same points, creating stable long-term patterns. Like musical harmonics, these ratios shape the architecture of solar systems.")
      ]),
      h("div", { key: "ctrls", style: { display: "flex", gap: "12px", alignItems: "center", flexWrap: "wrap" } },
        h("label", null, "System: "),
        h("select", {
          value: res_system,
          onChange: function(ev) { upd("orr_res_sys", ev.target.value); animRef.current = 0; },
          style: { padding: "4px 8px", borderRadius: "4px", border: "1px solid " + border, background: cardBg, color: fg }
        },
          h("option", { value: "jupiter_moons" }, "\uD83E\uDE90 Jupiter Moons (1:2:4)"),
          h("option", { value: "pluto_neptune" }, "\u2B50 Pluto\u2013Neptune (2:3)"),
          h("option", { value: "saturn_rings" }, "\uD83E\uDE90 Saturn Ring Gaps (1:2)")
        ),
        h("label", null, "Speed: "),
        h("input", { type: "range", min: "0.2", max: "5", step: "0.1", value: res_speed,
          onChange: function(ev) { upd("orr_res_speed", parseFloat(ev.target.value)); },
          style: { width: "120px" } }),
        h("span", { style: { fontSize: "12px", color: mutedFg } }, fmt(res_speed, 1) + "x")
      ),
      cvPanel,
      card([
        h("div", { key: "info", style: { fontSize: "12px", lineHeight: "1.7" } },
          h("div", null, "\u2022 Io-Europa-Ganymede: 1:2:4 resonance drives intense tidal heating on Io (\u2265400 active volcanoes)."),
          h("div", null, "\u2022 Pluto-Neptune 2:3: Pluto never comes closer than 17 AU to Neptune despite crossing its orbit."),
          h("div", null, "\u2022 Kirkwood gaps: Asteroids in resonance with Jupiter get kicked out, creating empty bands."),
          h("div", null, "\u2022 Resonances can be stabilizing (Pluto) or destabilizing (Kirkwood gaps) depending on geometry."),
          h("div", null, "\u2022 TRAPPIST-1's 7 planets form a chain of resonances: 2:3:4:6:9:15:24!")
        )
      ])
    );
  }
  ...
  /* ====================================================================
   *  17. RENDER — dispatch to active tab
   * ==================================================================== */
  var content;
  switch (stab) {
    ...
    case 8: content = buildGravityAssistTab(); break;
    case 9: content = buildRetrogradeTab(); break;
    case 10: content = buildLagrangeTab(); break;
    case 11: content = buildResonanceTab(); break;
