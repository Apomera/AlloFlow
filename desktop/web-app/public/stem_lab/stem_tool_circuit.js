...
(function() {
  ...
  CIRCUIT_NETWORK_EXAMPLES.push(
    {id:'opamp-bandwidth',name:'Bandwidth and phase lag',duration:.003,scopeSource:1,selected:2,question:'The follower has a 1 kHz gain-bandwidth product and a 1 kHz input. Predict the amplitude loss and phase lag. Disable timing to compare with the idealized response.',components:[{id:1,type:'voltage',a:'A',b:'0',value:0,waveform:{shape:'sine',amplitude:.2,frequency:1000}},{id:2,type:'opamp',a:'B',b:'0',value:100000,control:{positive:'A',negative:'B'},opamp:{lower:-5,upper:5,timing:{enabled:true,gbw:1000,slew:1,initial:0}}},{id:3,type:'resistor',a:'B',b:'0',value:1000}]},
    {id:'opamp-gain-bandwidth',name:'Gain trades for bandwidth',duration:.03,scopeSource:1,selected:2,question:'R3 and R4 set a gain near ten. With a 1 kHz gain-bandwidth product, expect a closed-loop bandwidth near 100 Hz. Compare this 100 Hz response with the follower example.',components:[{id:1,type:'voltage',a:'A',b:'0',value:0,waveform:{shape:'sine',amplitude:.1,frequency:100}},{id:2,type:'opamp',a:'B',b:'0',value:100000,control:{positive:'A',negative:'C'},opamp:{lower:-5,upper:5,timing:{enabled:true,gbw:1000,slew:1,initial:0}}},{id:3,type:'resistor',a:'B',b:'C',value:9000},{id:4,type:'resistor',a:'C',b:'0',value:1000},{id:5,type:'resistor',a:'B',b:'0',value:10000}]},
    {id:'opamp-slew',name:'When a sine wave becomes a ramp',duration:.0015,scopeSource:1,selected:2,question:'This 3 V peak, 2 kHz sine needs about 0.038 V/µs at its steepest point. U2 allows only 0.02 V/µs. Inspect rising and falling slopes, then double the slew rate.',components:[{id:1,type:'voltage',a:'A',b:'0',value:0,waveform:{shape:'sine',amplitude:3,frequency:2000}},{id:2,type:'opamp',a:'B',b:'0',value:100000,control:{positive:'A',negative:'B'},opamp:{lower:-5,upper:5,timing:{enabled:true,gbw:1000000,slew:.02,initial:0}}},{id:3,type:'resistor',a:'B',b:'0',value:1000}]},
    {id:'opamp-startup',name:'Slew, settle, and reach a DC level',duration:.006,scopeSource:1,selected:2,question:'The input starts at 3 V while U2 starts at 0 V. Its 0.001 V/µs slew limit bounds the fastest rise. Why does a curved settling tail follow the straight ramp? Compare with DC equilibrium.',components:[{id:1,type:'voltage',a:'A',b:'0',value:3},{id:2,type:'opamp',a:'B',b:'0',value:100000,control:{positive:'A',negative:'B'},opamp:{lower:-5,upper:5,timing:{enabled:true,gbw:1000,slew:.001,initial:0}}},{id:3,type:'resistor',a:'B',b:'0',value:1000}]}
  );
  ...
  function circuitNetworkOpAmpTiming(p){var raw=p.opamp&&p.opamp.timing||{};return {enabled:raw.enabled===true,gbw:circuitNumber(raw.gbw,1000000,10,100000000),slew:circuitNumber(raw.slew,.5,.000001,1000),initial:circuitNumber(raw.initial,0,-24,24)};}
  function circuitNetworkTimedOpAmp(p){return p.type==='opamp'&&circuitNetworkOpAmpTiming(p).enabled;}
  function circuitNetworkStateful(p){return p.type==='capacitor'||p.type==='inductor'||circuitNetworkTimedOpAmp(p);}
  function circuitNetworkOpAmpModel(p){if(p.type!=='opamp')return null;var m=p.opamp||{},timing=circuitNetworkOpAmpTiming(p),result={model:timing.enabled?'dominant-pole-slew':'finite-gain-clamp',lower:circuitNumber(m.lower,-5,-24,24),upper:circuitNumber(m.upper,5,-24,24)};if(m.timing)result.timing=timing;return result;}
  function circuitNetworkOpAmpProfile(p,dynamic){
    var m=p.opamp,t=circuitNetworkOpAmpTiming(p);if(!dynamic||!t.enabled)return {gain:p.value,offset:0,lower:m.lower,upper:m.upper};
    var previous=dynamic.states&&Object.prototype.hasOwnProperty.call(dynamic.states,p.id)?dynamic.states[p.id]:t.initial;
    if(!Number.isFinite(previous)||previous<m.lower-1e-8||previous>m.upper+1e-8)return {invalid:true};
    if(dynamic.initial)return {gain:0,offset:previous,lower:previous,upper:previous};
    var tau=p.value/(2*Math.PI*t.gbw),dt=dynamic.dt;if(!(dt>0))return {invalid:true};
    return {gain:p.value*dt/(tau+dt),offset:previous*tau/(tau+dt),lower:Math.max(m.lower,previous-t.slew*1e6*dt),upper:Math.min(m.upper,previous+t.slew*1e6*dt)};
  }
  function circuitNetworkOpAmpMotion(p,input,output){
    var m=p.opamp,t=circuitNetworkOpAmpTiming(p),raw=(p.value*input-output)*2*Math.PI*t.gbw/p.value,limit=t.slew*1e6,rate=Math.max(-limit,Math.min(limit,raw)),region=raw>=limit?'slew-up':raw<=-limit?'slew-down':'tracking';
    if(output>=m.upper-1e-8&&raw>=0){region='upper';rate=0;}else if(output<=m.lower+1e-8&&raw<=0){region='lower';rate=0;}return {rate:rate,region:region};
  }
  ...
  function circuitNetworkOperatingPoint(matrix,rhs,parts,index,constraints,nodeCount,seed,dynamic){
    ...
    for(var candidate=0;candidate<candidates;candidate++){
      ...
      amps.forEach(function(p,i){var mode=code%3,profile=profiles[i];modes.push(mode);code=Math.floor(code/3);var k=nodeCount+constraints.indexOf(p);if(mode===0){var plus=index(p.control.positive),minus=index(p.control.negative);if(plus>=0)a[k][plus]-=profile.gain;if(minus>=0)a[k][minus]+=profile.gain;b[k]=profile.offset;}else b[k]=mode===1?profile.lower:profile.upper;});
      var solved=circuitNetworkNonlinear(a,b,parts,index,seed);iterations+=solved.nonlinearIterations||0;if(!solved.ok)continue;
      var valid=true,values=[],unresolved=false;amps.forEach(function(p,i){var input=solved.measure([[index(p.control.positive),1],[index(p.control.negative),-1]]),output=solved.measure([[index(p.a),1],[index(p.b),-1]]);if(input==null||output==null){unresolved=true;return;}var profile=profiles[i],unlimited=profile.offset+profile.gain*input,target=Math.max(profile.lower,Math.min(profile.upper,unlimited));if(modes[i]===0&&(unlimited<profile.lower||unlimited>profile.upper))valid=false;if(Math.abs(output-target)>1e-7+1e-8*Math.max(Math.abs(output),Math.abs(target)))valid=false;values.push(input,output);});
  ...
  window.StemLab.circuitNetworkOpAmpTiming=circuitNetworkOpAmpTiming;
  ...
  function solveNetworkCircuit(state,dynamic) {
    ...
    var linear=missingSense?{ok:false,message:circuitNetworkLabel(missingSense)+' needs an existing voltage-output source to sense. Choose a source, or insert a 0 V voltage source in series with the branch to measure its current.'}:circuitNetworkOperatingPoint(matrix,rhs,parts,index,constraints,used.length,dynamic&&dynamic.diodeVoltages,dynamic),difference=function(a,b){if(a===b)return 0;if(!linear.ok||a!=='0'&&!used.includes(a)||b!=='0'&&!used.includes(b))return null;return linear.measure([[index(a),1],[index(b),-1]]);};
    ...
    rows.forEach(function(row){if(!row.component.opamp)return;var p=row.component,m=p.opamp,law=row.controlValue==null?null:circuitNetworkOpAmpLaw(p,row.controlValue);row.opampTimingActive=!!dynamic&&circuitNetworkTimedOpAmp(p);var motion=row.opampTimingActive&&law&&row.voltage!=null?circuitNetworkOpAmpMotion(p,row.controlValue,row.voltage):null;row.opampRate=motion?motion.rate:null;row.opampStepRate=motion&&dynamic&&!dynamic.initial?(row.voltage-dynamic.states[p.id])/dynamic.dt:null;row.opampRegion=linear.ok&&law?(motion?motion.region:law.region):'unknown';row.opampRequested=linear.ok&&law?law.requested:null;row.opampHeadroom=row.voltage==null?null:Math.max(0,Math.min(row.voltage-m.lower,m.upper-row.voltage));});
  ...
  function circuitNetworkTransient(state){
    var design=circuitNetworkDesign(state),duration=circuitNetworkDuration(state),reactive=design.components.filter(circuitNetworkStateful),initial=solveNetworkCircuit(design,{initial:true,time:0}),plan=circuitNetworkSignalPlan(design,duration),switchPlan=circuitNetworkSwitchPlan(design,duration),switchStates={},switchIndex=0,steps=0,rejected=0,eventIndex=0;
    ...
    if(!initial.ok)return fail(initial.message+' Capacitor voltage, inductor current, and timed op-amp output cannot jump instantly. Add a resistive path or use compatible starting conditions.');
    var statesOf=function(s){var values={};reactive.forEach(function(p){var row=s.rows.find(function(r){return r.component.id===p.id;});values[p.id]=p.type==='inductor'?row.current:row.voltage;});return values;};
    ...
    if(!known(states))return fail('The starting state is undetermined. Define capacitor voltage, inductor current, and timed op-amp output before running.');
    Object.keys(stateScale).forEach(function(id){stateScale[id]=Math.abs(stateScale[id]);});
    var timedAmps=design.components.filter(circuitNetworkTimedOpAmp),fastStep=timedAmps.reduce(function(dt,p){return Math.min(dt,1/(32*Math.PI*circuitNetworkOpAmpTiming(p).gbw));},plan.maxStep),frames=[circuitNetworkSnapshot(initial,0)],time=0,dt=fastStep,maxError=0;
    ...
    while(time<duration){
      ...
      var error=reactive.reduce(function(max,p){var absolute=p.type==='inductor'?1e-10:1e-7,scale=Math.max(stateScale[p.id],Math.abs(next[p.id]),Math.abs(middle[p.id]));return Math.max(max,Math.abs(next[p.id]-coarse[p.id])/(absolute+2e-5*scale));},0);
      ...
      var restartTiming=error<=1&&timedAmps.length&&((switchEvent&&time===switchEvent.time)||plan.events.some(function(event){return Math.abs(event-time)<duration*1e-12;}));
      dt=Math.min(restartTiming?fastStep:plan.maxStep,dt*Math.min(2,Math.max(.15,error===0?2:.85/Math.sqrt(error))));
    }
  ...
  function circuitNetworkOpAmpTimingColumns(p,row){if(!p.opamp)return ['','','','','','',''];var t=circuitNetworkOpAmpTiming(p);return [t.enabled,t.gbw,t.slew,t.initial,row.opampTimingActive||false,row.opampRate==null?'':row.opampRate,row.opampStepRate==null?'':row.opampStepRate];}
  ...
  function CircuitNetworkDiagram(props) {
    ...
    s.rows.forEach(function(row,i){var part=row.component,a=layout.nodes[part.a],b=layout.nodes[part.b],c=layout.centers[i],selected=props.selected===part.id,color=selected?'#f6df9d':row.current==null?'#93a2b3':Math.abs(row.current)<1e-12?'#7194a0':'#8bd8c0',direction=row.current!=null&&Math.abs(row.current)>1e-12?(row.current>0?1:-1):0,route=geometry.routes[i];
      ...
      if(part.type==='resistor'){
        ...
        var limitColor=row.voltage==null?'#95a3b4':row.opampRegion==='linear'?'#a5e6d2':row.opampRegion==='tracking'?'#9bdcf0':row.opampRegion==='slew-up'||row.opampRegion==='slew-down'?'#dfbdf3':'#efbb7f';
  ...
  function circuitNetworkSlewText(rate){return rate==null?'Undetermined':(rate>0?'+':'')+Number((rate/1e6).toPrecision(4))+' V/µs';}
  function CircuitNetworkOpAmpTimingEditor(props){
    var h=props.React.createElement,p=props.component,t=circuitNetworkOpAmpTiming(p),change=function(values){props.change(Object.assign({},t,values));};
    return h('section',{className:'circuit-network-opamp-timing','aria-label':'Op-amp timing settings'},h('div',{className:'circuit-network-opamp-timing-title'},h('span',{className:'circuit-eyebrow'},'HOW FAST CAN IT FOLLOW?'),h('strong',null,t.enabled?'Bandwidth + slew rate':'Idealized timing')),
      h('button',{type:'button','aria-pressed':t.enabled,onClick:function(){change({enabled:!t.enabled});}},t.enabled?'Disable bandwidth and slew':'Enable bandwidth and slew'),
      h('p',{className:'circuit-help'},t.enabled?(props.timed?'Time response carries the amplifier output from its starting value.':'Timing is enabled for Time response. DC equilibrium uses the settled gain and output limits only.'):'The output follows its finite gain and voltage limits immediately. Enable timing to investigate lag, rounded edges, and ramps.'),
      t.enabled&&h(props.React.Fragment,null,
        h(CircuitActiveControl,{React:props.React,field:'network-opamp-gbw',label:'Gain-bandwidth product (Hz)',value:t.gbw,min:10,max:100000000,step:1,logarithmic:true,onChange:function(value){change({gbw:value});}}),
        h('p',{className:'circuit-help'},'Open-loop pole: '+Number((t.gbw/p.value).toPrecision(5))+' Hz. In a simple negative-feedback circuit, closed-loop bandwidth is approximately gain-bandwidth product ÷ noise gain.'),
        h(CircuitActiveControl,{React:props.React,field:'network-opamp-slew',label:'Slew rate (V/µs)',value:t.slew,min:.000001,max:1000,step:.001,logarithmic:true,onChange:function(value){change({slew:value});}}),
        h('div',{className:'circuit-network-opamp-shortcuts'},h('button',{type:'button',disabled:t.slew<=.000001,onClick:function(){change({slew:Math.max(.000001,t.slew/2)});}},'Halve slew rate'),h('button',{type:'button',disabled:t.slew>=1000,onClick:function(){change({slew:Math.min(1000,t.slew*2)});}},'Double slew rate')),
        h('p',{className:'circuit-help'},'A sine wave with output peak Vp and frequency f needs a maximum slope of 2πfVp. Slew rate limits this slope in both directions; it is different from output-voltage clipping.'),
        h(CircuitActiveControl,{React:props.React,field:'network-opamp-start',label:'Starting output A − B (V)',value:t.initial,min:-24,max:24,step:.1,onChange:function(value){change({initial:value});}}),
        h('p',{className:'circuit-help'},'The starting output must lie inside the configured output window. It applies only in Time response. Stored timing settings are retained when timing is disabled.')));
  }
  function CircuitNetworkOpAmpReadout(props){
    ...
    return h('section',{className:'circuit-network-opamp-state','data-region':row.opampRegion||'unknown','aria-label':props.scope?'Op-amp scope operating point':'Op-amp operating point'},
      ...
      row.opampTimingActive&&h('div',{className:'circuit-network-slew-readout'},h('span',null,'Output slope'),h('strong',null,circuitNetworkSlewText(row.opampRate)),h('span',null,'Slew limit · ±'+Number(circuitNetworkOpAmpTiming(p).slew.toPrecision(4))+' V/µs')),
      !row.opampTimingActive&&circuitNetworkTimedOpAmp(p)&&h('p',{className:'circuit-help'},'DC equilibrium · timing is inactive here.'),
      h('p',{className:'circuit-help'},!known?'Repair the circuit to see the operating point.':row.opampRegion==='slew-up'||row.opampRegion==='slew-down'?'The output is changing at the configured slew limit. Reduce the signal amplitude or frequency, or increase the slew rate.':row.opampRegion==='tracking'?'The one-pole response follows the input over time. Its slope is below the slew limit at this sample; bandwidth can still make the waveform smaller and later.':row.opampRegion==='linear'?'With negative feedback, high gain keeps the input difference small while the output has room to move. The two inputs are not physically shorted.':'The output has reached a configured limit. It cannot follow the requested gain, so the input error can grow. Reduce the signal or gain, or widen the output window.'));
  }
  function CircuitNetworkOpAmpEditor(props){
    ...
    return h('section',{className:'circuit-network-opamp-editor','aria-label':'Op-amp controls'},h('span',{className:'circuit-eyebrow'},'FOLLOW THE FEEDBACK'),h('h4',null,'Gain, error, and output limits'),
      ...
      h('p',{className:'circuit-help'},'Open-loop gain multiplies the input difference. The surrounding feedback circuit sets the closed-loop gain. The DC target is Vout = clamp(gain × (V+ − V−), lower, upper). Timing, when enabled, determines how the output approaches its response.'),
      ...
      h(CircuitNetworkOpAmpTimingEditor,{React:props.React,component:p,timed:props.timed,change:function(timing){change({timing:timing});}}),
      h(CircuitNetworkOpAmpReadout,{React:props.React,row:props.row}),
      h('details',null,h('summary',null,'What this op-amp models'),h('p',{className:'circuit-help'},'Finite positive open-loop gain and hard output limits relative to output reference B. Inputs draw zero current and the output has zero impedance with no current limit. The output is internally powered; these limits are settings, not connected supply terminals. Optional timing adds one dominant pole and a symmetric slew rate limit to the output state. Supply current, input common-mode restrictions, input offset/bias, additional poles, and overload-storage recovery are not modeled.'),h('p',{className:'circuit-help'},'Up to four op-amps can be checked together. Multiple operating points, undefined input differences, or inconsistent output constraints stop the run. DC equilibrium and idealized timing do not choose a latch state. The timing mode is a simplified voltage-feedback model, not a validated comparator, hysteresis, or stability-margin model.')));
  }
  ...
  function CircuitNetworkScope(props){
    ...
    var time=s.time||0,sideText=s.side?' · '+s.side+' switching':'',carriedMetric=selected&&(selected.component.type==='capacitor'||circuitNetworkTimedOpAmp(selected.component)?'voltage':selected.component.type==='inductor'?'current':null),before=s.event&&carriedMetric?circuitNetworkFrame(run,time,'before'):null,after=s.event&&carriedMetric?circuitNetworkFrame(run,time,'after'):null;
    return h('section',{className:'circuit-active-panel circuit-network-scope','aria-label':'Connected circuit time response'},
      ...
      run.ok&&selected&&h(React.Fragment,null,
        ...
        (run.events||[]).length>0&&h('section',{className:'circuit-network-event-panel','aria-label':'Switch event timeline'},
          ...
          before&&after&&h('div',{className:'circuit-network-continuity'},h('strong',null,selected.label+' · '+(carriedMetric==='voltage'?'Voltage':'Current')+' carried through'),h('span',null,(carriedMetric==='voltage'?circuitPreciseVoltageText:circuitCurrentText)(before.rows.find(function(r){return r.component.id===selected.component.id;})[carriedMetric])+' → '+(carriedMetric==='voltage'?circuitPreciseVoltageText:circuitCurrentText)(after.rows.find(function(r){return r.component.id===selected.component.id;})[carriedMetric])),h('p',{className:'circuit-help'},circuitNetworkTimedOpAmp(selected.component)?'The timed op-amp output stays continuous at the switch. Its output slope and current can change immediately.':carriedMetric==='voltage'?'The capacitor keeps its voltage and stored energy at the switch. Its current can change immediately.':'The inductor keeps its current and stored energy at the switch. Its voltage can change immediately.'))),
  ...
  function CircuitNetworkWorkbench(props) {
    ...
    if(!s)s=Object.assign({},dc,{ok:false,time:0,rows:dc.rows.map(function(r){return Object.assign({},r,{voltage:null,current:null,power:null,energy:null,controlValue:null,opampRegion:'unknown',opampRequested:null,opampHeadroom:null,opampRate:null,opampStepRate:null,opampTimingActive:timed&&circuitNetworkTimedOpAmp(r.component)});}),nodes:dc.nodes.map(function(n){return Object.assign({},n,{voltage:n.id==='0'?0:null});}),balances:dc.balances.map(function(b){return {id:b.id,current:null};}),absorbed:null,delivered:null,storedEnergy:null,message:run.message});
      ...
      h('section',{className:'circuit-active-panel'},h('div',{className:'circuit-active-toolbar'},h('h3',null,'02 · Make the connections'),h('span',null,d.components.length+'/16 components · 8 named nodes')),
        ...
        h('div',{className:'circuit-network-editor'},h('div',{className:'circuit-network-part-list',role:'group','aria-label':'Network components'},s.rows.map(function(row){return h('button',{type:'button',key:row.component.id,'aria-label':'Edit '+row.label,'aria-pressed':!!selected&&selected.component.id===row.component.id,onClick:function(){patch({selected:row.component.id});}},h('strong',null,row.label+' · '+CIRCUIT_NETWORK_TYPES[row.component.type].name),h('span',null,row.component.a+' → '+row.component.b+' · '+circuitNetworkValue(row.component,row.closed)));}),!s.rows.length&&h('p',null,'Add a component or load an example to begin.')),
          selected&&h('div',{className:'circuit-network-inspector'},h('h4',null,selected.label+' · '+CIRCUIT_NETWORK_TYPES[selected.component.type].name),h('div',{className:'circuit-network-terminals'},nodeSelect(selected.component.opamp?'Output node A':'Terminal A'+(selected.component.type==='voltage'?' (+)':selected.component.type==='diode'?' (anode)':''),selected.component.a,function(id){edit({a:id});}),nodeSelect(selected.component.opamp?'Output reference B':'Terminal B'+(selected.component.type==='voltage'?' (−)':selected.component.type==='diode'?' (cathode)':''),selected.component.b,function(id){edit({b:id});})),
            ...
            selected.component.opamp&&h(CircuitNetworkOpAmpEditor,{key:selected.component.id,React:React,row:selected,edit:edit,timed:timed,measure:function(red,black){patch({probeRed:red,probeBlack:black});}}),
  ...
  if (!document.getElementById('circuit-css-anims')) {
    ...
    circStyle.textContent += ".circuit-network-opamp-timing{margin:17px 0;padding:13px;border:1px solid #7e8dad;border-radius:12px;background:linear-gradient(140deg,#293551,#142e41)}.circuit-network-opamp-timing-title{display:grid;gap:7px;margin-bottom:13px}.circuit-network-opamp-timing-title strong{color:#c9e7f2;font-size:16px}.circuit-network-opamp-timing>button{width:100%;min-height:44px}.circuit-network-slew-readout{display:grid;grid-template-columns:1fr auto;gap:7px 12px;margin-top:13px;padding-top:12px;border-top:1px solid #6d829d;color:#d5e5ef;font-size:12px}.circuit-network-slew-readout strong{font:600 14px ui-monospace,monospace;color:#e6cbf7}.circuit-network-slew-readout span:last-child{grid-column:1/-1;font-size:11px;color:#cbd9e7}.circuit-network-opamp-state[data-region=tracking]{background:linear-gradient(125deg,#163b4a,#1b3048);border-color:#7db6c9}.circuit-network-opamp-state[data-region=slew-up],.circuit-network-opamp-state[data-region=slew-down]{background:linear-gradient(125deg,#403651,#23344a);border-color:#b9a0d3}.circuit-network-opamp-state[data-region=slew-up] .circuit-network-opamp-state-title strong,.circuit-network-opamp-state[data-region=slew-down] .circuit-network-opamp-state-title strong{color:#e3c8f9}@media(max-width:480px){.circuit-network-opamp-timing{padding:10px}.circuit-network-slew-readout{grid-template-columns:1fr}.circuit-network-slew-readout strong{font-size:15px}}";
