...
3. **Each audit slice gets its own pages.** Being implemented in another session on this machine at the time of writing (the driver's page-window selection and page-range bounding of the sliced audit, with their own tests); left to that session so the two do not collide.
