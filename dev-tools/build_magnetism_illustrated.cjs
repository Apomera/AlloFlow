const fs=require('fs'),{execFileSync}=require('child_process'),{refine}=require('./magnetism_content_refinements.cjs');
execFileSync(process.execPath,['dev-tools/build_math_illustrated.cjs','magnetism_grade6'],{stdio:'inherit'});
const dir='allopacks/media/magnetism_grade6/',f='allopacks/illustrated/magnetism_grade6.allopack.json',p=JSON.parse(fs.readFileSync(f)),changes=refine(p);
