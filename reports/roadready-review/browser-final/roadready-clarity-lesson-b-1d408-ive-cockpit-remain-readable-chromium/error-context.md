...
- Name: roadready-clarity.spec.ts >> lesson briefing and live cockpit remain readable
- Location: tests\e2e\roadready-clarity.spec.ts:15:5
...
Locator: getByRole('button', { name: 'Fasten seatbelt' })
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: 'Fasten seatbelt' })

```

```yaml
- 'img "Three-dimensional driving scene: road, traffic, scenery, weather conditions, and other vehicles ahead. The scene updates as you drive."'
- 'application "RoadReady driving HUD: shows current speed in mph, score, elapsed time, fuel, and active warnings (seatbelt, speed limit, hazards). Controls: W or up arrow to accelerate, S or down arrow to brake, A and D or left and right arrows to steer, C to cycle camera, L to toggle high beams, Space to pause, Escape to end the drive."'
- button "End drive and open debrief": End
- button "Pause drive": ⏸ Pause
- 'button "Cycle driving HUD preset. Current: calm"': "HUD: Calm"
- button "More"
- text: 🔔 Fasten Your Seatbelt Press B or any movement key to fasten. The car will not move until you do.
- status: SAFE BUFFER. Keep scanning ahead · 25 mph limit.
```
