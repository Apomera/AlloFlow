...
window.React = React;
import * as icons from '../../desktop/web-app/node_modules/lucide-react';
window.AlloIcons = icons;
const translate = key => key.split('.').reduce((value, part) => value?.[part], window.fixtureStrings) || key;
window.AlloLanguageContext = React.createContext({ t: translate });
require('../../immersive_reader_module.js');
require('../../instructional_context_module.js');
require('../../pure_helpers_module.js');
require('../../phase_n_misc_helpers_module.js');
require('../../view_simplified_module.js');
...
const olderText = '## Discovering the coast\n\n**Reading the field notes**\n\n' + marine.map((text, i) => i === 1
...
const spanish = ['## La costa', '**Notas de campo**',
...
const spanishOnly = makeAdapted('adapted-spanish', spanish, original, 'Spanish');
const bilingual = makeAdapted('adapted-bilingual', spanish + '\n\n' + marker + '\n\n' + olderText, original, 'Spanish');
const initialHistory = [spanishOnly, original, older, newer, alternate, ambiguousOriginal, ambiguous, weak, bilingual];
const noop = () => {};
window.audioRequests = [];
window.__alloResolveReadAloudAudio = async (text, options) => { window.audioRequests.push({text, language:options.language, profile:options.profile}); return new URLSearchParams(location.search).has('fallback') ? null : '/tone.wav'; };
...
function App() {
  ...
  const props = {
    t, KaraokeReaderOverlay: window.AlloModules.KaraokeReaderOverlay, generatedContent: item, inputText: 'Ambient input must never become the source.', history, setHistory,
