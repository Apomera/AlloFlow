...
if(!summary.pass||!aim.pass)throw Error('Final browser checks must pass before writing the review');
const detail=aim.checks.find(check=>check.detail&&typeof check.detail.contrast==='number').detail;
fs.writeFileSync(path.join(out,'FREE-BUILD-BROWSER-REVIEW.md'),`# Free Build browser refinement review — September 12, 2026

**${summary.checks.length} recorded browser assertions passed.** The broad pass captured ${summary.interactionStates} interaction states; focused keyboard, aiming, feedback, and alignment checks supplement those snapshots. Each run used one real React/Three WebGL page at a time. All Chromium instances were closed, with no page or console errors in the final passes.
...
- Looking above the ground produces neutral aiming guidance. The Aim target measures ${detail.width.toFixed(1)}×${detail.height}px, with nominal text contrast ${detail.contrast.toFixed(2)}:1. Aim returns to a valid placement target without camera translation or geometry changes.
