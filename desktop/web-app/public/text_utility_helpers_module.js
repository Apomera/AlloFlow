...
const _extractRepairCitationMarkers = (value) => {
  ...
  for (let pieceIndex = 0; pieceIndex < pieces.length; pieceIndex += 2) {
    ...
    for (let cursor = 0; cursor < line.length; ) {
      ...
      if (!label || line[label.end] !== "(" || !/^\u207d[\u2070\u00b9\u00b2\u00b3\u2074\u2075\u2076\u2077\u2078\u2079]+\u207e$/.test(label.content)) {
        cursor = start + 1;
        continue;
      }
...
const repairGeneratedText = async (originalText, issue, targetLength, context, preserveCitations = false, deps) => {
  ...
  const citationRule = preserveCitations ? "\n                - CRITICAL: Preserve all citation markers in the format [\u207D\xB9\u207E](url). Keep them exactly as-is \u2014 do not remove, merge, or reformat them. They are important hyperlinks." : "";
  ...
  try {
    ...
    if (preserveCitations) {
      ...
      const preservedExactly = originalCitationSequence.length === repairedSequence.length && originalCitationSequence.every((marker, index) => marker === repairedSequence[index]);
      if (!preservedExactly) {
...
const generateHelpfulHint = async (type, text, shouldSpeak = false, deps) => {
  ...
  try {
    ...
    if (type === "simplified") {
      ...
      console.log("[handleGenerate] \u{1F5BC}\uFE0F Image generation triggered!");
      contextInstruction = `
    ...
    setHelpfulHint(hint);
    setHintHistory((prev) => [
      { id: Date.now(), text: hint, tool: toolName, timestamp: /* @__PURE__ */ new Date() },
      ...prev
    ]);
...
const generateWordSearch = (targetLang, deps, activityData) => {
  ...
  const candidates = (Array.isArray(activityData) ? activityData : Array.isArray(generatedContent.data) ? generatedContent.data : []).map((item) => {
    if (!item) return null;
    let text = item.term;
  ...
  const placedWords = [];
  const solutionSet = /* @__PURE__ */ new Set();
...
try {
  if (typeof document !== "undefined" && !window.__alloDeviceStorageProbeArmed) {
    ...
    const openDeviceStorageProbe = () => {
      const ready = () => {
        try {
          window.alloDeviceStorage.__openProbePanel();
        } catch (e) {
          console.warn("[DeviceStorage] probe panel failed:", e);
        }
      };
