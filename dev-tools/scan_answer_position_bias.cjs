...
for (const f of files) {
  ...
  const reA = /(?:options|opts|choices|a):\s*\[([^\]]{4,600})\]\s*,\s*(?:correctIdx|correct|answer|ans):\s*(\d+)/g;
  let m;
  ...
  const reA2 = /(?:correctIdx|correct|answer|ans):\s*(\d+)\s*,\s*(?:options|opts|choices|a):\s*\[([^\]]{4,600})\]/g;
