...
Generated 2026-09-26T06:40:50.058Z by `/tmp/claude-0/-home-user-AlloFlow/3efddaaf-b151-5c02-b4cf-dbf152bb6493/scratchpad/stemsel/summarize.cjs` from `out/*.json` (per-tool raw) — harness `probe.cjs`.
...
- Clean (no violations of any kind, no harness errors): 133
- Clean of WCAG A/AA (no contrast, no A/AA rule; best-practice findings allowed): 199 (127 STEM, 72 SEL)
...
- Tools with ≥1 color-contrast violation: 21
...
|---|---|---|---|---|
| heading-order | best-practice | best-practice | 78 | 47 |
...
- state: `tab "🧪 SMART Lab"` [light+dark] — src: `sel_hub/sel_tool_howl.js:14359` (via style #fbbf24|fontSize: 14)
...
- state: `tab "📚 Goals Library"` [light+dark] — src: `sel_hub/sel_tool_howl.js:7512` (via text ""By end of quarter, I will answer at lea")
  ...
  - Fix any of the following: Heading order invalid
- state: `tab "⚖️ Climate Scenarios"` [light+dark] — src: `sel_hub/sel_tool_howl.js:15115` (via text "How to navigate any of these")
...
- state: `tab "🖨 Print view"` [light+dark] — src: `sel_hub/sel_tool_costbenefit.js:372` (via aria-label "Time frame")
...
- state: `tab "📋 Data"` [light+dark] — src: `stem_lab/stem_tool_statslab.js:90` (via style #94a3b8)
  - `<th scope="col" style="padding: 6px; color: var(--allo-stem-text-soft, #94a3b8); text-align: left;"></th>`
  - Fix any of the following: Element does not have text that is visible to screen readers
- state: `tab "📋 Data"` [light+dark] — src: not found (tag th)
  - `<th scope="col"></th>`
  - Fix any of the following: Element does not have text that is visible to screen readers
...
- state: `tab "🖨 Print view"` [light+dark] — src: `sel_hub/sel_tool_onepageprofile.js:441` (via style #0f172a|fontSize: 28)
...
**stem_lab/stem_tool_fireecology.js** — 115 nodes
...
- #fbbf24 on #ffffff = 1.66 (need 4.5:1) ×9 — first at `tab "🌺Smoke & Seeds"` [dark] — src: `stem_lab/stem_tool_fireecology.js:4046`
  - `<div style="font-weight: 700; color: rgb(251, 191, 36); margin-bottom: 10px; font-size: 15px;">🧪 Smoke Chemicals That Trigger Life</div>`
...
- … 36 more colour pairs in aggregate.json
...
**stem_lab/stem_tool_dna.js** — 23 nodes
...
- #0f172a on #0f253d = 1.14 (need 4.5:1) ×1 — first at `tab "📝Transcribe"` [dark] — src: `stem_lab/stem_tool_dna.js:4299`
  - `<h3 style="color: var(--allo-stem-text, #172033); font-size: 15px; font-weight: 900; margin: 2px 0px 0px; line-height: 1.2;">Transcription - DNA → mRNA</h3>`
- #475569 on #0f253d = 2.04 (need 4.5:1) ×1 — first at `tab "📝Transcribe"` [dark] — src: `stem_lab/stem_tool_dna.js:4332`
  - `<p style="margin: 3px 0px 0px; color: var(--allo-stem-text-soft, #475569); font-size: 10px; line-height: 1.45;">`
- #0f172a on #11282f = 1.16 (need 4.5:1) ×1 — first at `tab "🔬Translate"` [dark] — src: `stem_lab/stem_tool_dna.js:4300`
  - `<h3 style="color: var(--allo-stem-text, #172033); font-size: 15px; font-weight: 900; margin: 2px 0px 0px; line-height: 1.2;">Translation - mRNA → protein</h3>`
- #475569 on #11282f = 2.02 (need 4.5:1) ×1 — first at `tab "🔬Translate"` [dark] — src: `stem_lab/stem_tool_dna.js:4300`
  - `<p style="margin: 3px 0px 0px; color: var(--allo-stem-text-soft, #475569); font-size: 10px; line-height: 1.45;">Ribosome reads codons (3 bases at a time). Each codon → one amino acid. The genetic code…`
- #0f172a on #262527 = 1.17 (need 4.5:1) ×1 — first at `tab "🦠Mutate"` [dark] — src: `stem_lab/stem_tool_dna.js:382`
  - `<h3 style="color: var(--allo-stem-text, #172033); font-size: 15px; font-weight: 900; margin: 2px 0px 0px; line-height: 1.2;">Mutations</h3>`
- … 13 more colour pairs in aggregate.json
...
**stem_lab/stem_tool_anatomy.js** — 20 nodes
...
**stem_lab/stem_tool_playlab.js** — 19 nodes

- #a78bfa on #f6f3ff = 2.48 (need 4.5:1) ×6 — first at `baseline` [dark] — src: `stem_lab/stem_tool_playlab.js:223`
  - `<span style="color: rgb(167, 139, 250); font-style: italic;">🎙️ Show me what you got</span>`
...
- #a5b4fc on #eff0fe = 1.76 (need 4.5:1) ×1 — first at `tab "Scout"` [dark] — src: not found (tag button)
...
**stem_lab/stem_tool_epidemic.js** — 10 nodes
...
- #a7f3d0 on #d0e6d8 = 1.02 (need 4.5:1) ×1 — first at `tab "🏥 Outbreak Response"` [dark] — src: `stem_lab/stem_tool_epidemic.js:1231`
  - `<div style="font-size: 11px; color: rgb(167, 243, 208); margin-top: 2px; line-height: 1.4;">12 hrs/week, standard events. Default.</div>`

**stem_lab/stem_tool_echolocation.js** — 9 nodes
...
- #475569 on #191e44 = 2.11 (need 4.5:1) ×1 — first at `tab "🚨Doppler Effect"` [dark] — src: `stem_lab/stem_tool_echolocation.js:3681`
  - `<span class="text-[0.6875rem] font-black" style="color: var(--allo-stem-text-soft, #94a3b8);">Stationary</span>`
