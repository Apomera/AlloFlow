...
const wave = require('../dev-tools/eppp_native_quality_wave_48_data.cjs');
...
describe('EPPP expanded quality wave 48', () => {
  ...
  it('records challenge gains and synchronized source, deploy, and runtime artifacts', () => {
    const audit = json('test_prep/eppp_question_quality_audit_wave_48.json');
    expect(audit.summary).toMatchObject({ rewrittenItems: 24, domainsCovered: 8, difficultyRetieredItems: 23, applicationItems: 12, analysisItems: 12, keyPositionsPreserved: 24, optionSpecificExplanations: 96, selectedDistractorWarningsAfter: 0, selectedFeedbackWarningsAfter: 0, status: 'pass' });
    expect(audit.summary.distractorWarningsAfter.uniqueKeyStemLexicalLeakageCandidates).toBe(13);
    expect(audit.summary.distractorWarningsAfter.asymmetricExtremeDistractorCandidates).toBe(25);
    expect(audit.summary.distractorWarningsAfter.advancedDirectRecallCandidates).toBe(0);
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicatePairs).toBe(39);
    expect(audit.summary.distractorWarningsAfter.semanticConceptDuplicateClusters).toBe(27);
    expect(audit.summary.feedbackWarningsAfter.itemsWithWarnings).toBe(367);
    expect(audit.summary.feedbackWarningsAfter.incorrectOptionsWithWarnings).toBe(1031);
    expect(audit.summary.feedbackWarningsAfter.insufficientDetailOptions).toBe(499);
    expect(audit.summary.feedbackWarningsAfter.genericTemplateOptions).toBe(482);
    expect(audit.summary.feedbackWarningsAfter.choiceRestatementOptions).toBe(111);
    expect(audit.summary.feedbackWarningsAfter.fullKeyEchoOptions).toBe(58);
    ...
    expect(read('desktop/web-app/public/test_prep/eppp_question_quality_audit_wave_48.json')).toBe(read('test_prep/eppp_question_quality_audit_wave_48.json'));
  });

  it('replays after wave 47', () => {
    const builder = read('dev-tools/build_eppp_1500_expansion.cjs');
    const i47 = builder.indexOf('repair_eppp_native_quality_wave_47.cjs');
    const i48 = builder.indexOf('repair_eppp_native_quality_wave_48.cjs', i47);
    expect(i48).toBeGreaterThan(i47);
  });
});
