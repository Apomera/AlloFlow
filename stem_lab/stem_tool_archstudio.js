        ...
        el('aside', { id: 'arch-studio-tools', hidden: sidebarCollapsed, tabIndex: -1, className: 'arch-studio-sidebar' + (showDesign ? ' arch-studio-workbench' : '') + (showBOM ? ' arch-studio-schedule' : '') + (showTemplates ? ' arch-studio-templates' : '') + (showReplay ? ' arch-studio-replay' : '') + (showFilter ? ' arch-studio-filtering' : '') + (showSlice ? ' arch-studio-sections' : '') + (showFloorPlans ? ' arch-studio-floors' : '') + (showStats ? ' arch-studio-composition' : ''), 'aria-label': __alloAST('stem.archstudio.a11y_architecture_tools', 'Architecture tools'), style: { width: showDesign ? 'clamp(280px,28vw,360px)' : 'clamp(224px,21vw,252px)', flexShrink: 0, background: 'linear-gradient(180deg,var(--allo-stem-panel, #1e293b),rgba(15,23,42,.98))', padding: '11px 10px', overflowY: 'auto', borderRight: '1px solid var(--allo-stem-border, #334155)', display: 'flex', flexDirection: 'column', gap: 10 } },
          ...
          selectedBlock && el('section', {
            ...
            el('div', { style: { display: 'flex', alignItems: 'center', gap: 7, marginBottom: 7 } },
              ...
              el('button', { type: 'button', 'aria-label': __alloAST('stem.archstudio.a11y_clear_selected_block', 'Clear selected block'), title: t('stem.archstudio.deselect_escape','Deselect (Escape)'), onClick: function () { clearSelectedBlock(true); }, style: { minWidth: 28, minHeight: 28, border: '1px solid transparent', borderRadius: 6, background: 'transparent', color: '#fca5a5', cursor: 'pointer', padding: 4, fontSize: 13 } }, '\u2715')
            ),
            ...
            el('div', { 'data-arch-inspector-actions': 'true', role: 'group', 'aria-label': __alloAST('stem.archstudio.a11y_selected_object_actions', 'Selected object actions'), style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 } },
              ...
              el('button', { type: 'button', 'aria-label': __alloAST('stem.archstudio.a11y_delete_selected_block', 'Delete selected block'), title: t('stem.archstudio.delete_selected_block_delete','Delete selected block (Delete)'), disabled: showReplay, onClick: function () { commitSelectedAction({ type: 'delete' }); }, style: { minHeight: 34, padding: '7px 4px', borderRadius: 7, border: '1px solid #ef4444', background: 'rgba(239,68,68,.1)', color: showReplay ? '#475569' : '#fca5a5', cursor: showReplay ? 'default' : 'pointer', fontSize: 10, fontWeight: 750 } }, '\uD83D\uDDD1 Delete')
            )
          ),
