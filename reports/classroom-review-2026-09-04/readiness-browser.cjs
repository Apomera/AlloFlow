...
(async () => {
  ...
  try {
    ...
    await page.evaluate(()=>window.__setReadingReviewFixture({...window.__reviewFixture,guidedStep:6,guidedStepError:null}));
    const readiness=page.getByRole('region',{name:'Student-ready preflight',exact:true});
    await readiness.waitFor();
    const accessibilityCheck=readiness.getByRole('checkbox',{name:/Accessibility and reading order/});
    await accessibilityCheck.focus(); await page.keyboard.press('Space');
    if(!await accessibilityCheck.isChecked())throw new Error('Keyboard confirmation failed');
    const saved=await page.evaluate(()=>JSON.parse(localStorage.getItem('allo_guided_readiness_checks')));
    if(saved.version!==2||saved.checks.accessibility!==true)throw new Error('Scoped check was not saved');
    await page.evaluate(()=>window.__setReadingReviewFixture({...window.__reviewFixture,guidedStep:6,guidedStepError:null,inputText:'Changed classroom passage about rainfall.'}));
    await page.getByText('The lesson changed. Please confirm the manual checks again.',{exact:true}).waitFor();
    if(await accessibilityCheck.isChecked())throw new Error('Changed source inherited confirmation');
    await page.getByRole('button',{name:'Finish with checks remaining',exact:true}).click();
    await page.getByText('Finish Guided Mode with open checks?',{exact:true}).waitFor();
    const readinessAxe=await page.evaluate(async()=>{
      const result=await axe.run({include:[document.querySelector('[aria-labelledby="guided-readiness-title"]')]},{runOnly:{type:'tag',values:['wcag2a','wcag2aa','wcag21aa','wcag22aa']}});
      return {violations:result.violations.map(v=>({id:v.id,impact:v.impact,nodes:v.nodes.map(n=>n.failureSummary)})),passes:result.passes.length};
    });
    fs.writeFileSync(path.join(__dirname,'readiness-axe.json'),JSON.stringify(readinessAxe,null,2));
    if(readinessAxe.violations.length)throw new Error('Readiness accessibility issues: '+JSON.stringify(readinessAxe.violations));
    await readiness.screenshot({path:path.join(__dirname,'readiness-mobile.png')});
    await accessibilityCheck.click();
    await page.evaluate(()=>{
      window.__realReadinessSetItem=Storage.prototype.setItem;
      Storage.prototype.setItem=function(key,value){if(key==='allo_guided_readiness_checks')throw new Error('Quota exceeded');return window.__realReadinessSetItem.call(this,key,value);};
    });
    await accessibilityCheck.click();
    await page.getByText(/These checks could not be saved on this device/).waitFor();
    if(await accessibilityCheck.isChecked())throw new Error('Uncheck was lost in session');
    if(await page.evaluate(()=>localStorage.getItem('allo_guided_readiness_checks'))!==null)throw new Error('Stale check persisted after save failure');
    await page.evaluate(()=>{Storage.prototype.setItem=window.__realReadinessSetItem;});
    if(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth))throw new Error('Readiness layout overflows mobile width');
    if(errors.length) throw new Error(errors.join('\n'));
    const result={passed:true,scope:'Real GuidedModeBanner + GuidedModeConfig in an isolated browser harness; not full application E2E',checks:['7-step preset visible before selection','preset applied','short draft protected','keyboard sample load','next-step navigation','390px viewport without horizontal overflow','actual adapted text preview','correct reading opened','settings opened by keyboard','retry callback','old lesson excluded','generation error guidance','targeted automated accessibility scan','readiness confirmation by keyboard','scoped readiness persistence','source change invalidation','finish gate remains open','storage failure reported and stale check removed'],pageErrors:errors};
    fs.writeFileSync(path.join(__dirname,'readiness-browser.json'),JSON.stringify(result,null,2));
