...
// The stem_lab/ mirrors that ship AS SOURCE. The first entry is canonical:
// tests read it by that exact path (tests/roadready_rules.test.js,
// stem_position_bias_*), so it is the copy to fix toward, not away from.
//
// desktop/web-app/build and desktop/app-build are deliberately NOT here. They
// are build OUTPUT -- the first is regenerated by the web build, the second is
// gitignored -- so both lag source between builds as a matter of course.
// Including them made this gate report 62 meaningless findings alongside 21
// real ones, which is why it sat unrun. Part A below still sweeps all three
// roots for resurrected fossils, because a fossil in a build output is a real
// (if harmless) signal that something regenerated one.
const MIRROR_DIRS = [
  'stem_lab',
