...
**Slice** opens the Cross-section explorer near the top of the sidebar and moves focus to its heading. The front overview combines all depths; choose a **Depth (Z)** for a drawing through that section. **Previous section** and **Next section** move between occupied depths, including negative coordinates, while the depth profile shows where blocks are concentrated. **All depths** restores the full depth of the 3D view without closing the panel.

The drawing and depth counts follow the current replay frame. A selected depth that is empty in an earlier frame stays selected, with an explanation and navigation to other depths. The drawing includes the complete section, while a separate count reports what remains visible in 3D after floor, material, and shape filters. The Floor Grid continues to show its full editing floor. Drawings represent occupied cells, including curved and sloped shapes, and retain the frame's overall extents for comparison.

Expand **Block coordinates** for an accessible table of X, Y, shape, and material. Large sections are paginated in groups of 48 rows. Changing depth or displayed geometry resets the coordinate page. Reaching a navigation boundary keeps keyboard focus inside the panel. Close, or press Escape outside the depth selector, to return focus to Slice and restore all depths; reopening resumes the selected section. Browsing sections does not change geometry, project details, or undo history.

Section explorer refinement (September 9): all 577 Architecture Studio unit checks across 20 suites passed across scoped runs and final rechecks. Nine local Chromium workflows passed, covering negative-depth navigation, replay and empty restored selections, preview versus filtered viewport scope, renderer and project-state preservation, coordinate pagination, keyboard boundary focus, phone targets/model reachability, and existing drawing/filter/workspace behavior. The drawing and expanded coordinate table passed selected axe WCAG 2 A/AA, 2.1 AA, and 2.2 AA checks in light, dark, and high-contrast themes. Forced-color and reduced-motion keyboard use were exercised. Desktop and 320px phone drawing screenshots were visually reviewed. No deployment was performed.

## Explore floor plans

**Floor Plans** opens a floor explorer with a larger plan, material counts, and a floor selector. **Previous floor** and **Next floor** move between occupied floors, skipping empty gaps. The drawing retains the whole frame's X/Z extents, so floors are easy to compare. It represents occupied cells, including curved and sloped shapes.

Plans and counts follow the current replay frame and include cells hidden by 3D filters. A saved floor that is empty in a historical frame stays selected with an explanation. Only the selected floor is drawn, avoiding the old per-floor allocation of empty grid cells for large, sparse models.

Browsing plans leaves the model view unchanged. **View this floor in 3D** applies that floor filter and focuses the model. **Open this floor in the grid** selects the editing floor and focuses an occupied cell, including at distant coordinates; an empty floor starts at X=0, Z=0. Replay grids remain read-only. **Show all floors in 3D** clears only the floor filter and retains the previewed floor. Other filters continue to apply in 3D.

The compact **Floor visibility** card has a 44px slider with counts and bounds from the displayed frame. A restored empty floor remains within the slider's range. It changes the 3D floor filter without changing the editing floor, and **Explore floor plans** opens the explorer directly. Closing the explorer keeps view settings and restores focus to Floor Plans. Escape closes it from controls other than the native floor selector.

Floor explorer refinement (September 19): 159 focused unit checks across five suites passed, including 43 floor checks repeated after the final navigation adjustment. Eight local Chromium workflows passed, covering preview and view separation, replay and empty-floor selection, historical visibility bounds, keyboard focus, editing and undo, renderer/model/history preservation, and existing workspace/replay/section behavior. The explorer passed selected axe WCAG 2 A/AA, 2.1 AA, and 2.2 AA checks in light, dark, and high-contrast themes. Phone controls and reachability at 320px, forced colors, and reduced-motion keyboard use were checked. Desktop and phone captures were visually reviewed. No push or deployment was performed.

## Inspect build composition

**Stats** opens Build composition near the top of the sidebar and focuses its heading. Four readable totals show blocks, materials, shapes, and occupied floors. Counts follow the current replay frame and include blocks hidden by 3D filters. Empty builds and empty replay steps explain how to continue.

Switch between **Materials** and **Shapes**, then order groups by block count or displayed name. Each row combines a labeled texture or shape diagram with its count, percentage of the entire frame, and proportional bar. Percentages use one decimal place when needed; shares below 0.1% remain explicitly nonzero. **Model span** reports the outer occupied-cell dimensions in X, Y, and Z, including gaps, independently of the world origin.

Inspecting a row opens Block filters with that material or shape selected and clears the other block filter. Floor and section settings remain in place, with the filter panel explaining how to reveal hidden matches. Geometry, project details, editing floor, and undo history remain unchanged. Grouping and ordering persist when reopening Stats. Close, or press Escape outside the native order selector, to return focus to Stats. Controls have 44px targets and a scrollable phone layout.

Composition refinement (September 19): 141 focused unit checks across five suites and eight distinct Chromium workflows passed across the main run and focused rechecks. Coverage includes frame totals and shares, grouping/order, filter inspection, preserved model/history/renderer, replay and empty frames, editing and undo, and existing workspace/filter/floor workflows. Both grouping modes passed selected axe WCAG 2 A/AA, 2.1 AA, and 2.2 AA checks in light, dark, and high-contrast themes. Forced colors, reduced motion, 320px control sizes, row reachability, and keyboard focus were exercised. Desktop and phone screenshots were reviewed; a shorter phone panel was then verified to leave the canvas center visible and reachable. One initial browser fixture was corrected to explicitly restore 3D mode. All four existing module copies match. No push or deployment was performed.
