# AlloFlow School Rewards - Google Education pilot (schema v7)
...
- User manual (illustrated, for leaders, staff, and families): `school-rewards-manual.html` at the repository root, published at https://apomera.github.io/AlloFlow/school-rewards-manual.html and https://alloflow-cdn.pages.dev/school-rewards-manual.
- Printable quick cards for staff, cashiers, and students/families: `school-rewards-quick-cards.html`.
- Practice page (the portal on a fictional ledger, bilingual, with a five-minute demo guide): `school-rewards-practice.html`, generated by `dev-tools/build_school_rewards_practice.py`.
- Inside the portal, the **Help** button in the header opens role-specific answers with links to the above; every string is in the Spanish pack.
- This README is the technical and operational reference; the pathway matrix is in `docs/school_rewards_readiness.md`.
...
1. In AlloFlow's **STEM Lab -> 3D Print Lab**, the student builds a constrained primitive recipe or locally opens an allowlisted GLB/STL. Three other STEM Lab tools hand work into Print Lab locally, without a file: **Geometry World** sends a selected block build as STL with its editable block source; **Architecture Studio** (all of its styles, including Architect) sends its block structure as STL (added 2026-09-05); and **Art Studio** (sculpt mode) sends its primitive-shape sculpture as the same editable recipe (added 2026-09-05). Nothing else in AlloFlow produces printable geometry; other tools' 'print' features are for paper. The bridges are bidirectional: Print Lab's Design tab offers Start in Geometry World / Architecture Studio / Art Studio when nothing was handed in, and each source card has a return button. Geometry World and Art Studio receive the editable model back (`window.__alloGeometryWorldPendingBuild`, `window.__alloArtStudioPendingSculpt`); Architecture Studio's return is navigation only because the studio keeps its own block save and Print Lab holds only the STL.
...
Fresh setup creates schema v7 in one reviewed operation: the `ClaimTokens` table for single-use claim codes, schema-v5 `Catalog.InventoryVersion` and `InventoryMovements`, plus `MailRuns`, `MailOutbox`, the private mail-delivery signing material in Script Properties, and the signed recurring hourly mail-safety sweep. The first setup may ask the owner to authorize Sheets, Drive, mail sending, and trigger management. Private Print Lab folders are created lazily. Uploaded binaries remain private and quarantined from quotation until an authorized staff member records the separate verification decision.

## Upgrading an existing schema v4 or v5 pilot to v6

Use the ordered `v4 -> v5 -> v6 -> v7` path. Setup creates a new v7 repository, but rerunning setup against an existing repository deliberately preserves its recorded schema version; it is not a migration shortcut.
...
5b. From a verified schema-v6 repository, run the idempotent `migrateSchoolRewardsRepositoryV7()` as an active administrator. It adds the `ClaimTokens` sheet and writes `schemaVersion: 7`. Until it runs, `mintSchoolRewardsClaimTokens`, `voidSchoolRewardsClaimBatch` and `claimSchoolRewardsToken` refuse with `claim_migration_required`; everything else keeps working.
6. Run the read-only integrity report, aggregate reconciliation, and audit-chain verification. Confirm inventory chains remain valid, mail-run/outbox checks are clean, no unresolved delivery exists, and `/exec?api=health` reports version `7`.
...
## Single-use claim codes (schema v7)

Staff mint a batch of codes from the Award tab (points, category, reason, count, optional expiry). Each code is a random token id stored in `ClaimTokens`; the printed QR or link is `<web-app>/exec?claim=<token>` and carries nothing else. A signed-in student who opens it is credited once from the row, never from the URL: the token id is the idempotency key, the claim runs through the same intent journal as awards, and a second scan by anyone sees `already_redeemed`. Expired, voided and unknown codes return a state instead of an error. Reopening a code you already redeemed says it is already in your balance (the result is flagged `replayed`), never a second credit. When many students scan at once and the script lock is busy, the portal retries the claim a few times with backoff, which is safe because a claim is idempotent per code and student. Staff who open a claim link see that it is a student code; nothing is redeemed. Minting takes an optional `idempotencyKey`: the portal sends one per form, so a double-click or a retry after a lost response returns the same code sheet instead of printing a second set of live codes. The Award tab prints the batch as cut-out coupons (title, points, reason, a message the teacher types, and a QR of the claim link, and the expiry when one is set). The QR is drawn by a copy of AlloFlow's `qrcode.js` embedded in `Portal.html`: the portal runs as the signed-in administrator or staff member, so it never loads executable code from the CDN. After changing `qrcode.js`, run `node dev-tools/embed_school_rewards_qr.cjs`; the claim portal tests fail if the embedded copy draws anything different. Teachers can also pick a coupon background: a PNG or JPEG from Art Studio, a scanned drawing or an AI image generator. The portal redraws it through a canvas as a JPEG (at most 1400 px on the long edge, which also drops photo metadata), keeps it only in that browser, and prints it behind every coupon with the text on a readable panel and the QR on its own white tile in one of three positions. The art is never sent to the server and never carries the code: the QR is always encoded by the portal, because image generators cannot draw a scannable one. The link base is the saved `webAppUrl`, so set it during setup or the coupons show the bare code. The Award tab also lists recent batches (`listSchoolRewardsClaimBatches`: staff see the batches they minted, administrators every batch) with unused/used/cancelled/expired counts, reprints only the unused codes, and voids a lost printout with `voidSchoolRewardsClaimBatch` (own batches for staff, any for administrators); redeemed codes keep their ledger entries. The integrity report checks every token against its ledger row (`CLAIM_TOKEN_*`, `DUPLICATE_CLAIM_LEDGER`) and every completed claim journal against both. Design note: `docs/SINGLE_USE_CLAIM_QR_DESIGN.md`.
