  ...
  try {
    ...
    const speakViaBrowserFallback = (reason) => {
      ...
      try {
        if (!("speechSynthesis" in window)) {
          playSequence2(index + 1, sentences, sessionId, mode, voiceMap, nextSpeaker, null, 0, speakerName, deps);
          return;
        }
    ...
    const handlePlaybackError = (err) => {
      ...
      if (playbackSessionRef.current === sessionId) {
        ...
        if (retryCount < 3) {
          ...
          setTimeout(() => {
            if (playbackSessionRef.current === sessionId) {
              playSequence2(index, sentences, sessionId, mode, voiceMap, activeSpeaker, null, retryCount + 1, speakerName, deps);
            }
          }, backoffMs);
    ...
    if (preloadedAudio) {
      ...
      if (audio instanceof Promise) {
        try {
          const _tOut = window._kokoroTTS ? 9e4 : 15e3;
      ...
      if (audioBufferRef.current[bufferKey]) {
        try {
          const _tOut2 = window._kokoroTTS ? 9e4 : 15e3;
        ...
        try {
          const _tOut3 = window._kokoroTTS ? 9e4 : 15e3;
    ...
    audio.onended = async () => {
      ...
      playSequence2(index + 1, sentences, sessionId, mode, voiceMap, nextSpeaker, nextPreloadedAudio, 0, speakerName, deps);
    };
    audio.onerror = (e) => {
      if (watchdogTimer) clearTimeout(watchdogTimer);
      handlePlaybackError(e);
    ...
    if (playbackSessionRef.current === sessionId) {
      if (err.message && err.message.includes("finishReason: OTHER")) {
        ...
        if (retryCount < 2) {
          ...
          playSequence2(index + 1, sentences, sessionId, mode, voiceMap, activeSpeaker, null, 0, speakerName, deps);
        }
      }
    }
  }
...
const autoConfigureSettings = async (text, grade, standards, language, customInput, existingResources = [], targetCount = "Auto", deps) => {
  ...
  try {
    const userCustomBlock = customInput && customInput.trim().length > 0 ? `USER'S SPECIAL REQUEST: "${customInput}" (Prioritize this over general analysis)` : "";
