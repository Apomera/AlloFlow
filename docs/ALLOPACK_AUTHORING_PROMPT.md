...
Every pack: types 1-7 below plus 8-10. Also legal when the topic earns them: `timeline`, `outline`, `math`, `note-taking` (see docs/ALLOPACK_FORMAT_SPEC.md).
...
8. **anchor-chart** (classroom reference): `data = { "chartType": "reference", "title": "<SHORT ALL-CAPS TITLE>", "sections": [{"label": "<CAPS>", "bullets": ["..."]}] }`. 3-5 sections; the chart is the one-page version of the reading.
9. **memory-aid** (Memory Aid Studio): `data = { "schemaVersion": 2, "title", "instructions", "selectionMode": "manual", "selectedTypes": [...], "authorshipMode": "scaffolded", "reflectionLevel": "quick", "reasoningRequired": false, "sourceExcerpt": "<verbatim sentences from the reading>", "lessonRef": {"resourceId": "<reading id>", "title"}, "cards": [ { "id", "target", "essentialFacts": [2-4 facts quoted or closely paraphrased from the reading], "factLocked": true, "factVerified": true, "type": "acronym-acrostic"|"rhyme-rhythm"|"chunking"|"story-chain"|"keyword-association"|"visual-association"|"analogy-pattern"|"sequence-cue", "mode": "scaffolded", "aiExample": "<a finished aid a student could have made>", "mapping": "<how each part of the aid leads back to a fact>", "scaffoldStarter": "<the aid with blanks>", "scaffoldSteps": [3 steps], "coachPrompts": [1-2], "studentPrompt", "reasoningPrompt", "hookFact": {"text": "<a surprising fact from the reading>", "sourceTitle": "<reading title> (this lesson)"} } ] }`. 3 cards, three different types, each card one concept cluster (the sequence, the rule, the trap).
10. **applied-challenge** (Applied Challenge Studio): `data = { "schemaVersion": 6, "title", "instructions", "selectionMode": "manual", "family": "investigate"|"design"|"decide"|"propose"|"explore", "fitReason": "<why this family fits this lesson>", "agencyMode": "co-framed", "scope": "compact"|"standard", "brief": { "family": <same>, "context": "<a real situation with real stakes, 2-4 sentences>", "role", "audience", "drivingQuestion", "seedDirection", "lockedLessonFacts": [3-5 facts from the reading], "openQuestions": [3-4 things the student must find out], "stakeholders": [3-5], "criteria": [3-5, one requiring glossary words], "constraints": [3-4], "deliverable": "<one concrete product>", "factLocked": true, "factVerified": true }, "supports": { "parallelExample": {"context", "move", "whyItHelps"}, "frameStarter": "<sentence with ____ blanks>", "frameChoices": [4 directions a student could take], "coachPrompts": [3 questions] }, "coachHint": "<one sentence>", "sourceExcerpt": "<verbatim from the reading>", "lessonRef": {"resourceId", "title"} }`. Grade 3-4: scope compact, two-cup / one-change investigations. Grade 6-8: standard, with a real audience who could push back.

Place the memory-aid right after the anchor chart (or glossary) and the applied-challenge last; give each a numbered step in the directions.
