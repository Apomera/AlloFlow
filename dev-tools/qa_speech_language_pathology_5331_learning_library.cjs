...
for (const outputRoot of [path.join(root, 'test_prep'), path.join(root, 'prismflow-deploy', 'public', 'test_prep')]) { fs.mkdirSync(outputRoot, { recursive: true }); fs.writeFileSync(path.join(outputRoot, 'speech_language_pathology_5331_learning_library_qa.json'), JSON.stringify(report, null, 2) + '\n', 'utf8'); fs.writeFileSync(path.join(outputRoot, 'speech_language_pathology_5331_learning_library_qa.md'), markdown, 'utf8'); }
