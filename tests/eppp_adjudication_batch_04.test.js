...
describe('EPPP claim-level adjudication batch 04', () => {
  it('reviews ten high-risk candidates without releasing or repeating them', () => {
    const report = read('quality/eppp_provenance/evidence/adjudication/adjudication_batch_04.json');
    const priorIds = new Set(['01', '02', '03'].flatMap((batch) => read(`quality/eppp_provenance/evidence/adjudication/adjudication_batch_${batch}.json`).items.map((item) => item.legacyId)));
  ...
  it('keeps JSON and Markdown canonical and outside runtime publication', () => {
    expect(fs.existsSync(resolve(root, 'quality/eppp_provenance/evidence/adjudication/adjudication_batch_04.json'))).toBe(true);
    expect(fs.existsSync(resolve(root, 'quality/eppp_provenance/evidence/adjudication/adjudication_batch_04.md'))).toBe(true);
    expect(fs.existsSync(resolve(root, 'desktop/web-app/public/test_prep/eppp_legacy/adjudication_batch_04.json'))).toBe(false);
  });
});
