...
describe('AP Biology internal foundation pilot', () => {
  ...
  it('contains a balanced 50-item pilot across all units and science practices', () => {
    ...
    expect(new Set(pack.items.map((item) => item.id)).size).toBe(50);
    expect(Object.values(unitCounts).every((count) => count >= 5)).toBe(true);
    expect(Object.keys(practiceCounts).sort()).toEqual(['SP1', 'SP2', 'SP3', 'SP4', 'SP5', 'SP6']);
    expect(answerCounts).toEqual({ 0: 13, 1: 13, 2: 12, 3: 12 });
  ...
  it('provides eight native chapters with two richer lesson prototypes and study aids', () => {
    expect(library.chapters).toHaveLength(8);
    expect(library.chapters.filter((chapter) => chapter.foundationPrototype).length).toBe(2);
    ...
    expect(library.summary).toMatchObject({ chapters: 8, sections: 8, knowledgeChecks: 8, richLessonPrototypes: 2 });
    expect(library.chapters.slice(0, 2).every((chapter) => {
  ...
  it('passes deterministic QA and binds byte-identical deployment mirrors', () => {
    ...
    expect(qa.metrics).toMatchObject({ itemCount: 50, unitCount: 8, chapterCount: 8, richLessonPrototypeCount: 2 });
    ...
    expect(entry).toMatchObject({ loadMode: 'lazy', visibility: 'internal', itemCount: 50, domainCount: 8 });
