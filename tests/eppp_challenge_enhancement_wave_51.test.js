...
const data = createRequire(import.meta.url)('../dev-tools/eppp_challenge_enhancement_wave_51_data.cjs');

const readJson = (filePath) => JSON.parse(fs.readFileSync(filePath, 'utf8'));
const words = (text) => String(text || '').trim().split(/\s+/).filter(Boolean).length;

describe('EPPP challenge enhancement wave 51', () => {
  it('reframes all twenty-four selected foundation items as applied items', () => {
    ...
    for (const revision of data.revisions) {
      ...
      expect(item.challengeEnhancementWave).toBe(data.reviewWave);
      expect(item.challengeEnhancementPriorDifficulty).toBe('foundation');
      expect(item.difficulty).toBe('intermediate');
      expect(item.cognitiveProcess).toBe('application');
    }
  });

  it('keeps incorrect explanations concise while clearing the live warning queues', () => {
    const bank = readJson(sourceBankPath);
  ...
  it('keeps the source and deploy banks plus wave audit synchronized', () => {
    const source = fs.readFileSync(sourceBankPath, 'utf8');
    expect(fs.readFileSync(deployBankPath, 'utf8')).toBe(source);
    const audit = readJson(path.join(root, 'test_prep/eppp_challenge_enhancement_audit_wave_51.json'));
