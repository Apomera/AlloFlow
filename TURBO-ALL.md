...
Syncs `AlloFlowANTI.txt` → `desktop/web-app/src/App.jsx`, builds the React bundle, stamps the service worker, and deploys to Firebase Hosting.
...
5. `npm run build` in `desktop/web-app/`.
6. Stamp `build/sw.js` with a unique timestamp (node one-liner in deploy.md).
7. `npx firebase deploy --only hosting` in `desktop/web-app/`.
