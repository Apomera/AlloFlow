...
// Source lines: 9602–10584
// Hash: 0bc5073397579e45
...
const analyzeFluencyWithGemini = async (audioBase64, mimeType, referenceText) => {
  ...
  `;
  /* [LOCAL] Audio analysis not available — LM Studio cannot process audio */
  const url = (window.__alloLocalConfig?.llmEngineUrl || 'http://localhost:1234') + '/v1/chat/completions';
  warnLog('[AudioAnalysis] Audio analysis via AI is not available in local mode (requires multimodal audio support).');
  return null;
};
const BRIDGE_MODES = [
  { id: "react" },
