        ...
        return blocks.map((block, blockIdx) => {
          ...
          switch (block.type) {
            ...
            case 'image': {
              ...
              const _insertFn = `function(c, dataUrl, altText){var ex=c.querySelector('img');if(ex){ex.src=dataUrl;if(altText)ex.alt=altText;}else{c.style.background='none';c.style.border='none';c.style.padding='0';c.style.minHeight='0';var ni=document.createElement('img');ni.src=dataUrl;ni.alt=altText||'Image';ni.style.cssText='max-width:100%;border-radius:8px;border:1px solid #e2e8f0';c.insertBefore(ni,c.firstChild);}var pk=c.querySelector('[data-alloflow-picker]');if(pk)pk.remove();}`;
              ...
              return `<figure id="${_imgId}-figure" data-img-placeholder="true" style="margin:1em 0">`
                ...
                + `<label style="display:inline-flex;align-items:center;gap:6px;padding:8px 14px;background:#1d4ed8;color:#ffffff;border:1px solid #1e3a8a;border-radius:6px;font-size:12px;font-weight:700;cursor:pointer;text-shadow:0 1px 2px rgba(0,0,0,0.25)">📷 Upload image<input type="file" accept="image/*" style="display:none" onchange="${_uploadHandler}"></label>`
                + `<button type="button" onclick="${_pickHandler}" style="display:inline-flex;align-items:center;gap:6px;padding:8px 14px;background:#7c3aed;color:#ffffff;border:1px solid #5b21b6;border-radius:6px;font-size:12px;font-weight:700;cursor:pointer;text-shadow:0 1px 2px rgba(0,0,0,0.25)" aria-label="Pick from extracted images">🖼 Pick extracted</button>`
                + `</div>`
                + `</div>`
      ...
      if (extractedImages.length > 0) {
        ...
        bodyContent = bodyContent.replace(/<figure data-img-placeholder="true"[\s\S]*?<\/figure>/gi, (match) => {
          ...
          const altText = captionMatch ? captionMatch[1].replace(/<[^>]*>/g, '').trim() : 'Image';
          {
            ...
            const _insertFn2 = `function(c, dataUrl, altText){var ex=c.querySelector('img');if(ex){ex.src=dataUrl;if(altText)ex.alt=altText;}else{c.style.background='none';c.style.border='none';c.style.padding='0';c.style.minHeight='0';var ni=document.createElement('img');ni.src=dataUrl;ni.alt=altText||'Image';ni.style.cssText='max-width:100%;border-radius:8px;border:1px solid #e2e8f0';c.insertBefore(ni,c.firstChild);}var pk=c.querySelector('[data-alloflow-picker]');if(pk)pk.remove();}`;
