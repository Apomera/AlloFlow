...
const tests=JSON.parse(fs.readFileSync(path.join(__dirname,'final-validation.json'),'utf8'));
assert.equal(tests.numFailedTests,0);assert.equal(tests.numPassedTests,186);
const browser=JSON.parse(fs.readFileSync(path.join(__dirname,'followthrough-browser-results.json'),'utf8'));
for(const row of browser.filter(row=>row.width)){assert.equal(row.errors.length,0);assert.equal(row.a11y.length,0);}
const report=`# Memory Aid improvements completed
...
`;
fs.writeFileSync(path.join(__dirname,'IMPLEMENTATION.md'),report);
const readme=path.join(__dirname,'README.md');let original=fs.readFileSync(readme,'utf8');
original=original.replace(/^Implementation[^\n]+/, 'The product recommendations, including the follow-through features, are implemented locally. See [the completed implementation and validation](IMPLEMENTATION.md) and [classroom evaluation protocol](CLASSROOM-PILOT.md). The analysis below records the original review.');
fs.writeFileSync(readme,original);
const temporary=['complete-report.cjs','followthrough-components.jsx','implement-followthrough.cjs','refine-followthrough.cjs','followthrough-polish.cjs','personalization-finish.cjs','static-print-finish.cjs','final-copy.cjs','format-final.cjs'];
for(const name of temporary){const target=path.resolve(__dirname,name);assert.equal(path.dirname(target),__dirname);if(fs.existsSync(target))fs.unlinkSync(target);}
console.log('Saved completion report and classroom protocol. '+tests.numPassedTests+' tests passed.');
