...
(function (root) {
  ...
  function ogPlayNote(engine, event, when, durationSec) {
    ...
    var osc = ctx.createOscillator();
    var filter = ctx.createBiquadFilter();
    var env = connectEnvelope(ctx, engine.master, start, dur, event && event.velocity, { attack: 0.006, decay: 0.08, sustain: 0.55, release: 0.12 });
    osc.type = 'sawtooth';
  ...
  function ogDecodeAndRegisterSample(engine, assetId, blob) {
    ...
    return blob.arrayBuffer().then(function (arrayBuffer) {
      var decoded = engine.ctx.decodeAudioData(arrayBuffer.slice ? arrayBuffer.slice(0) : arrayBuffer);
      return Promise.resolve(decoded);
    }).then(function (buffer) {
      return ogRegisterSampleBuffer(assetId, buffer, {
  ...
  function ogPlaySample(engine, assetId, when, velocity) {
    ...
    var gain = connectEnvelope(ctx, engine.master, start, Math.max(0.03, buffer.duration || 0.2), vel, { attack: 0.001, decay: 0.01, sustain: 0.98, release: 0.03 });
    source.connect(gain);
    source.start(start);
    source.stop(start + Math.max(0.04, buffer.duration || 0.2) + 0.05);
    return true;
  }
  ...
  function ogSchedulePlan(engine, plan, options) {
    ...
    plan.forEach(function (item) {
      ...
      if (item.type === 'drumHit') {
        if (item.assetId && ogPlaySample(engine, item.assetId, when, item.velocity)) {
          scheduled += 1;
          return;
        }
        ...
        scheduled += ogPlayNote(engine, item.event || item, when, item.durationSec) ? 1 : 0;
      }
    });
    return scheduled;
  }
