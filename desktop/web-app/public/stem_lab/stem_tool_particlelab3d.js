        ...
        h('div', { className: 'grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]' },
          h('section', { id: 'particle-stage', ref: stageRef, onKeyDown: onStageKeyDown, className: 'overflow-hidden border border-cyan-500/30 bg-slate-950 shadow-2xl shadow-cyan-950/40 ring-1 ring-white/5 ' + (fsActive ? 'h-screen rounded-none flex flex-col' : 'rounded-[24px]'), style: cssFullscreen ? { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100vh', zIndex: 99990, borderRadius: 0 } : undefined, 'aria-label': 'Three-dimensional particle simulation' },
            ...
            h('div', { className: 'relative min-h-[380px] overflow-hidden bg-slate-950 ' + (fsActive ? 'flex-1' : 'h-[520px]') },
              ...
              showHud && h('div', { id: 'particle-scene-key', className: 'pointer-events-none absolute left-3 ' + (trace ? 'bottom-[12rem]' : 'bottom-3') + ' z-10 max-w-[min(18rem,calc(100%-1.5rem))] rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2 text-[11px] font-bold text-slate-300 shadow-lg backdrop-blur-md', role: 'img', 'aria-label': transportMode ? 'Scene key: cyan particles are species A, pink particles are species B, and a yellow ring marks the traced particle.' : 'Scene key: cyan particles are the model particles, and a yellow ring marks the traced particle.' }, h('div', { className: 'mb-1 text-[11px] font-black uppercase tracking-[0.18em] text-cyan-300' }, 'Scene key'), h('div', { className: 'flex flex-wrap items-center gap-x-3 gap-y-1' }, h('span', { className: 'inline-flex items-center gap-1.5' }, h('span', { className: 'h-2 w-2 rounded-full bg-cyan-300 shadow-[0_0_8px_rgba(34,211,238,0.7)]' }), transportMode ? 'A / solvent' : 'particles'), transportMode && h('span', { className: 'inline-flex items-center gap-1.5' }, h('span', { className: 'h-2 w-2 rounded-full bg-fuchsia-300 shadow-[0_0_8px_rgba(244,114,182,0.7)]' }), 'B / solute'), trace && h('span', { className: 'inline-flex items-center gap-1.5 text-yellow-200' }, h('span', { className: 'h-2 w-2 rounded-full border-2 border-yellow-200' }), 'tracer'))),
