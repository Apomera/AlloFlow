...
var processMathHTML = window.processMathHTML || function (t) {
  return t;
};
...
var createDocPipeline = function (deps) {
  ...
  var t = deps.t;
  var isRtlLang = deps.isRtlLang || function () {
    return false;
  };
  ...
  var _bindState = function () {
    ...
    exportPreviewMode = s.exportPreviewMode;
    ...
    pendingPdfBase64 = s.pendingPdfBase64;
    pendingPdfFile = s.pendingPdfFile;
    pdfFixResult = s.pdfFixResult;
    pdfAuditResult = s.pdfAuditResult;
    pdfAutoFixPasses = s.pdfAutoFixPasses;
    pdfPolishPasses = s.pdfPolishPasses;
    pdfAuditorCount = s.pdfAuditorCount;
    pdfPreviewTheme = s.pdfPreviewTheme;
    pdfPreviewFontSize = s.pdfPreviewFontSize;
    pdfPreviewA11yInspect = s.pdfPreviewA11yInspect;
    pdfBatchQueue = s.pdfBatchQueue;
    pdfExperimentMode = s.pdfExperimentMode;
    pdfExperimentRuns = s.pdfExperimentRuns;
    customExportCSS = s.customExportCSS;
    ...
    setPdfFixLoading = s.setPdfFixLoading;
    setPdfFixStep = s.setPdfFixStep;
    setPendingPdfBase64 = s.setPendingPdfBase64;
    setPendingPdfFile = s.setPendingPdfFile;
    setPdfBatchQueue = s.setPdfBatchQueue;
    setPdfBatchProcessing = s.setPdfBatchProcessing;
    setPdfBatchCurrentIndex = s.setPdfBatchCurrentIndex;
    setPdfBatchStep = s.setPdfBatchStep;
    setPdfBatchSummary = s.setPdfBatchSummary;
    setIsGeneratingStyle = s.setIsGeneratingStyle;
    setCustomExportCSS = s.setCustomExportCSS;
...
Return ONLY valid JSON:
{
  ...
  "hasForms": true/false
      ...
      const allVariants = [auditPrompt, auditPrompt.replace('Analyze this PDF', 'Perform an independent accessibility analysis of this PDF'), auditPrompt.replace('Analyze this PDF', 'As a fresh auditor with no prior context, evaluate this PDF'), auditPrompt.replace('Analyze this PDF', 'Conduct a strict WCAG compliance review of this PDF'), auditPrompt.replace('Analyze this PDF', 'From the perspective of a screen reader user, assess this PDF'), auditPrompt.replace('Analyze this PDF', 'As a document remediation specialist, evaluate this PDF'), auditPrompt.replace('Analyze this PDF', 'As a disability rights advocate, critically assess this PDF'), auditPrompt.replace('Analyze this PDF', 'Using Section 508 federal standards, audit this PDF'), auditPrompt.replace('Analyze this PDF', 'As an assistive technology expert testing with JAWS/NVDA, evaluate this PDF'), auditPrompt.replace('Analyze this PDF', 'As a university compliance officer reviewing for Title II ADA, assess this PDF')];
      ...
      const parsedAudits = auditResults.filter(Boolean).map(r => {
        try {
          return parseAudit(r);
        } catch {
          return null;
      ...
      while (parsedAudits.length < numAuditors && parsedAudits.length > 0 && retryRound < MAX_RETRY_ROUNDS) {
        ...
        const retryPool = allVariants.slice(numAuditors).concat(allVariants.slice(0, numAuditors).map(p => p + `\n\n(Attempt ${retryRound + 1} — be especially thorough.)`));
        const retryVariants = retryPool.slice(0, shortfall);
        // Run retries sequentially if rate limited, parallel otherwise
        const retryResults = retryRound > 1 ? await (async () => {
          const res = [];
          for (const p of retryVariants) {
            try {
              res.push(await callGeminiVision(p, base64Data, 'application/pdf'));
            } catch {
              res.push(null);
            }
      ...
      const triangulated = {
        ...
        needsAdditionalAnalysis: false,
        // High variance on pre-remediation is expected — flag only post-remediation
...
Return ONLY valid JSON (no markdown, no backticks): {"score":N,"summary":"1-2 sentence overview","critical":[{"issue":"description","wcag":"SC number"}],"serious":[{"issue":"...","wcag":"..."}],"moderate":[{"issue":"...","wcag":"..."}],"minor":[{"issue":"...","wcag":"..."}],"passes":["what's already accessible"],"pageCount":N,"hasSearchableText":true/false,"hasImages":true/false,"hasTables":true/false,"hasForms":true/false}`;
    ...
    const batchParsedAudits = batchAuditResults.filter(Boolean).map(r => {
      try {
    ...
    if (_brandMode === 'upload' && _brandOverride) {
      ...
      try {
        const styleRes = await callGeminiVision(`Analyze the visual design of this PDF. Extract the exact color scheme.\n\nReturn ONLY JSON:\n{"headingColor":"hex","accentColor":"hex","bgColor":"hex","headerBg":"hex","headerText":"hex","tableBg":"hex","tableBorder":"hex"}`, base64Data, 'application/pdf');
        if (styleRes) {
...
Return ONLY the complete HTML document (<!DOCTYPE html> to </html>).`;
    ...
    accessibleHtml = accessibleHtml.replace(/<h([1-6])[^>]*>\s*<\/h\1>/gi, () => {
      aiFixCount++;
      return '';
    });
    ...
    const batchIdCounts = {};
    batchIdMatches.forEach(m => {
      batchIdCounts[m[1]] = (batchIdCounts[m[1]] || 0) + 1;
    });
    ...
    if (batchMaxFix > 0 && (curAxeResults && curAxeResults.totalViolations > 0 || bestAiScore < 90)) {
      for (let fp = 0; fp < batchMaxFix; fp++) {
        ...
        const axeIns = [].concat((curAxeResults ? curAxeResults.critical || [] : []).map(v => `CRITICAL (axe-core): ${v.description} (${v.id}, ${v.nodes} elements)`)).concat((curAxeResults ? curAxeResults.serious || [] : []).map(v => `SERIOUS (axe-core): ${v.description} (${v.id}, ${v.nodes} elements)`)).concat((curAxeResults ? curAxeResults.moderate || [] : []).map(v => `MODERATE (axe-core): ${v.description} (${v.id})`));
        const aiIns = curVerification && curVerification.issues ? curVerification.issues.map(i => `AI-FLAGGED: ${i.issue || i}`) : [];
        ...
        if (postListFix.fixCount > 0) {
          ...
          log(`  Deterministic: fixed ${postListFix.fixCount} list issues`);
        }
        const postContrastFix = fixContrastViolations(accessibleHtml);
        if (postContrastFix.fixCount > 0) {
        ...
        if (newAxe >= bestAxeViolations && newAi <= bestAiScore + bMinDet && fp > 0) {
          log(`Plateau (SEM\u00b1${rvSEM.toFixed(1)}, threshold=${bMinDet}) \u2014 stopping`);
          break;
        }
      }
    }
    ...
    const remediationFooter = `<footer role="contentinfo" style="margin-top:48px;padding:16px 20px;border-top:2px solid ${batchDocStyle.accentColor || '#6366f1'};font-family:system-ui,sans-serif;font-size:11px;color:#64748b;page-break-inside:avoid">` + `<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">` + `<span>\u267f Accessibility remediated by AlloFlow \u2014 Score: ${finalScore || '?'}/100 | ${axeViolationCount} axe violation${axeViolationCount !== 1 ? 's' : ''} | ${autoFixPasses} fix pass${autoFixPasses !== 1 ? 'es' : ''}</span>` + `<span>${new Date().toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'short',
      day: 'numeric'
    })}</span>` + `</div></footer>`;
  ...
  const runPdfBatchRemediation = async () => {
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      setPdfBatchQueue(prev => prev.map((q, idx) => idx === i ? {
        ...q,
        status: 'processing'
      } : q));
      try {
    ...
    setPdfBatchSummary({
      ...
      avgBefore: function () {
        var valid = done.filter(q => {
          var _q$result;
          return ((_q$result = q.result) === null || _q$result === void 0 ? void 0 : _q$result.beforeScore) != null;
        });
        return valid.length ? Math.round(valid.reduce((s, q) => s + q.result.beforeScore, 0) / valid.length) : null;
      }(),
      avgAfter: function () {
        var valid = done.filter(q => {
          var _q$result2;
          return ((_q$result2 = q.result) === null || _q$result2 === void 0 ? void 0 : _q$result2.afterScore) != null;
        });
        return valid.length ? Math.round(valid.reduce((s, q) => s + q.result.afterScore, 0) / valid.length) : null;
      }(),
      avgImprovement: function () {
        var valid = done.filter(q => {
          var _q$result3, _q$result4;
          return ((_q$result3 = q.result) === null || _q$result3 === void 0 ? void 0 : _q$result3.afterScore) != null && ((_q$result4 = q.result) === null || _q$result4 === void 0 ? void 0 : _q$result4.beforeScore) != null;
        });
        return valid.length ? Math.round(valid.reduce((s, q) => s + (q.result.afterScore - q.result.beforeScore), 0) / valid.length) : null;
      }(),
  ...
  const downloadBatchResults = async () => {
    ...
    const telemetry = {
      ...
      timestamp: new Date().toISOString(),
      pipelineConfig: {
        auditorCount: pdfAuditorCount,
        autoFixPasses: pdfAutoFixPasses,
        polishPasses: pdfPolishPasses,
        verificationSamples: 3
      },
      ...
      files: pdfBatchQueue.map(f => ({
        ...
        error: f.error || null,
        result: f.result ? {
          beforeScore: f.result.beforeScore,
          afterScore: f.result.afterScore,
          improvement: (f.result.afterScore || 0) - (f.result.beforeScore || 0),
          autoFixPasses: f.result.autoFixPasses,
          axeViolations: f.result.axeViolations,
          needsExpertReview: f.result.needsExpertReview,
  ...
  const runTestRetestExperiment = async (base64Data, fileName, numRuns) => {
    ...
    for (let r = 0; r < numRuns; r++) {
      ...
      try {
        ...
        runs.push({
          ...
          autoFixPasses: result.autoFixPasses,
    ...
    const stats = {
      runs,
      n,
      afterMean: Math.round(mean * 10) / 10,
      afterSD: Math.round(sd * 10) / 10,
      afterSEM: Math.round(sem * 10) / 10,
      afterCV: cv,
      afterRange: range,
      beforeMean: Math.round(bMean * 10) / 10,
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `experiment_${fileName.replace(/\.pdf$/i, '')}_${numRuns}runs.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
...
Return ONLY the extracted text.`;
    try {
      ...
      if (pendingPdfFile && pendingPdfFile.size > 5 * 1024 * 1024) {
        ...
        const sectionPrompts = [`You are an OCR expert. Extract all text from the FIRST HALF of this document. Preserve structure using markdown. Describe images in [brackets]. Preserve tables as markdown. Return ONLY the text.`, `You are an OCR expert. Extract all text from the SECOND HALF of this document. Preserve structure using markdown. Describe images in [brackets]. Preserve tables as markdown. Return ONLY the text.`];
        const chunks = [];
      ...
      if (extractedText && extractedText.length > 100) {
        ...
        try {
          const verifyResult = await callGeminiVision(`Compare the original PDF document with this extracted text. Rate the extraction quality 1-10 and note any significant missing content, misread sections, or structural errors. Be brief (2-3 sentences).

Extracted text (first 2000 chars): """${extractedText.substring(0, 2000)}"""

Return ONLY JSON: {"quality": N, "issues": "description or null", "missingContent": true/false}`, pendingPdfBase64, 'application/pdf');
          try {
            let vCleaned = verifyResult.trim();
            if (vCleaned.indexOf('```') !== -1) {
              const parts = vCleaned.split('```');
              vCleaned = parts[1] || parts[0];
      ...
      if (extractedText && pendingPdfBase64) {
        ...
        try {
          const imageResult = await callGeminiVision(`List any significant images, diagrams, charts, or figures in this PDF document. For each, provide:
...
If there are no significant images, return: {"images": [], "totalImages": 0}`, pendingPdfBase64, 'application/pdf');
          try {
            let iCleaned = imageResult.trim();
            if (iCleaned.indexOf('```') !== -1) {
              const parts = iCleaned.split('```');
              iCleaned = parts[1] || parts[0];
...
"""${chunk}"""`;
      ...
      chunkResults.forEach(cr => {
        ...
        (cr.passes || []).forEach(p => {
          ...
          const passKey = passText.toLowerCase().replace(/\b(the|a|an|is|are|was|were|no|not|none|found|present|detected|this|that|in|of|for|to|and|or|with|without|section|block|specific|text|content|provided|been|has|have|all)\b/g, '').replace(/[^a-z\s]/g, '').trim().split(/\s+/).filter(w => w.length > 2).sort().slice(0, 5).join('_');
      ...
      if (!window.axe) {
        await new Promise((resolve, reject) => {
          if (document.querySelector('script[data-axe-core]')) {
            ...
            setTimeout(() => {
              clearInterval(wait);
              reject(new Error('axe-core load timeout'));
            }, 10000);
            return;
          }
...
.bg-gray-800 *,.bg-slate-800 *,.bg-gray-900 *,.bg-slate-900 *,.bg-black *{color:#e2e8f0}
  ...
  const fixListViolations = htmlContent => {
    ...
    fixed = fixed.replace(/<(ul|ol)(\s[^>]*)?>[\s\S]*?<\/\1>/gi, (listBlock, tag) => {
      ...
      result = result.replace(new RegExp('(<' + tag + '(?:\\s[^>]*)?>)([\\s\\S]*?)(<\\/' + tag + '>)', 'i'), (m, open, content, close) => {
        ...
        newContent = newContent.replace(/(<(?:ul|ol)(?:\s[^>]*)?>|<\/li>)(\s*)(<(?!li\b|\/li|ul\b|ol\b|\/ul|\/ol|script|template)[a-z][a-z0-9]*\b[^>]*>[\s\S]*?<\/(?!li|ul|ol)[a-z][a-z0-9]*>)(\s*)(?=<li|<\/(?:ul|ol)>)/gi, (m, prefix, ws1, inner, ws2) => {
          fixCount++;
          return prefix + '<li>' + inner + '</li>';
        });
        // Also wrap self-closing elements like <hr/>, <br/>, <img/>
        newContent = newContent.replace(/(<(?:ul|ol)(?:\s[^>]*)?>|<\/li>)(\s*)(<(?:hr|img|br|input)\b[^>]*\/?>)(\s*)(?=<li|<\/(?:ul|ol)>)/gi, (m, prefix, ws1, inner, ws2) => {
          fixCount++;
          return prefix + '<li>' + inner + '</li>';
        });
  ...
  const autoFixAxeViolations = async (htmlContent, axeResult, maxPasses = 2) => {
    if (!callGemini || !axeResult || axeResult.totalViolations === 0) return {
      html: htmlContent,
      axe: axeResult,
      passes: 0
    };
    ...
    while (currentAxe.totalViolations > 0 && passCount < maxPasses) {
      ...
      const violationInstructions = [].concat(currentAxe.critical.map(v => `CRITICAL: ${v.description} (${v.id}, ${v.nodes} elements) — ${v.wcag}`)).concat(currentAxe.serious.map(v => `SERIOUS: ${v.description} (${v.id}, ${v.nodes} elements) — ${v.wcag}`)).concat(currentAxe.moderate.map(v => `MODERATE: ${v.description} (${v.id}, ${v.nodes} elements)`)).concat(currentAxe.minor.map(v => `MINOR: ${v.description} (${v.id})`)).join('\n');
      try {
...
"""`, true);
  ...
  const fixAndVerifyPdf = async (batchOverrides = null) => {
    ...
    if (!_base64) {
      addToast('Cannot fix: PDF data not found in memory. Please re-upload the PDF.', 'error');
      return null;
    }
    if (!_isBatch && !_auditResult) {
      addToast('Cannot fix: No audit results found. Please run the audit first.', 'error');
      return null;
    }
    if (!_isBatch) {
    ...
    try {
      ...
      if (numChunks <= 1) {
        ...
        for (let i = 0; i < numChunks; i++) {
          ...
          chunkPromises.push(callGeminiVision(`Extract ALL text content from pages ${startPage} through ${endPage} of this document.\n\nRULES:\n- Use # for titles, ## for sections, ### for subsections\n- Preserve tables as markdown tables with | pipes and --- dividers\n- Describe images as: [Image: detailed description]\n- Use * for bullet lists, 1. for numbered lists\n- LINKS: Preserve ALL hyperlinks as [link text](URL). If you can see blue/underlined text with a visible URL, include it. If the URL destination isn't visible, use [link text](#).\n- Keep ALL content — do not skip any paragraphs or details\n\nIMPORTANT: Return ONLY plain text with markdown formatting. Do NOT wrap in JSON. Do NOT use \\n escape sequences — use actual line breaks. Just the document text.`, _base64, _mimeType).catch(err => {
            warnLog(`[PDF Fix] Chunk ${i + 1} extraction failed:`, err);
            return null;
          }));
        }
        ...
        const chunks = chunkResults.map((chunk, i) => {
          ...
          return chunk.trim().replace(/^\s*```[\w]*\n?/g, '').replace(/\n?```\s*$/g, '').replace(/^\s*\{[\s\S]*?"(?:text|content)":\s*"/g, '').replace(/"\s*\}\s*$/g, '').replace(/\\n/g, '\n').replace(/\\"/g, '"').replace(/\\t/g, '\t');
        });
        extractedText = chunks.join('\n\n---\n\n');
      }
      if (!extractedText || extractedText.length < 20) {
      ...
      extractedText = extractedText.replace(/^\s*\[\s*\{[^}]*"(?:html_content|text|content|section)":\s*"/gm, '') // strip JSON wrapper starts
      ...
      let extractedImages = [];
      try {
        const imgResult = await callGeminiVision(`Identify and extract ALL images from this PDF document. For each image:\n1. Describe it in detail (what it shows, any text in the image, educational purpose)\n2. Note its approximate location (page number, position)\n3. Indicate if it's decorative (borders, backgrounds) or meaningful (diagrams, photos, charts)\n\nReturn ONLY JSON:\n{"images": [{"id": 1, "description": "detailed description", "page": 1, "position": "top/middle/bottom", "type": "photo|diagram|chart|illustration|logo|decorative", "educationalPurpose": "what it teaches or communicates"}]}`, _base64, _mimeType);
        if (imgResult) {
            ...
            try {
              ...
              for (const [pageNum, imgs] of Object.entries(pageGroups)) {
                try {
                    ...
                    for (const img of imgs) {
                      ...
                      if (pos && pos.w > 20 && pos.h > 20) {
                        try {
                          ...
                          warnLog(`[PDF Fix] Image crop failed:`, cropErr);
                        }
                      }
                    }
                  }
                  ...
                  warnLog(`[PDF Fix] Page ${pageNum} extraction failed:`, pgErr);
                }
              }
            } catch (pdfJsErr) {
              ...
              if (callImagen && extractedImages.length <= 5) {
                for (let imgI = 0; imgI < extractedImages.length; imgI++) {
                  ...
                  try {
                    ...
                    const imgUrl = await callImagen(`Recreate this image for an educational document: ${extractedImages[imgI].description}. Clean, professional style. No text overlays.`, 300, 0.8);
                    if (imgUrl) {
                      extractedImages[imgI].generatedSrc = imgUrl;
                      extractedImages[imgI].isRegenerated = true;
                    }
                  } catch (genErr) {}
                }
              }
      ...
      try {
        ...
        warnLog('[PDF Fix] Style extraction failed (using defaults):', styleErr);
      }
      ...
      const renderJsonToHtml = blocks => {
        ...
        return blocks.map((block, blockIdx) => {
          ...
          try {
            ...
            if (block.text) {
              block.text = block.text.replace(/\\n/g, ' ').replace(/\n?id:\s*[a-z0-9-]+\n?/gi, '').replace(/^(description|alt|title|subtitle|caption|type|text|items|headers|rows):\s*/gim, '') // strip leaked JSON field names
            ...
            if (!block.id && block.text) {
              ...
              if (idMatch) {
                block.id = idMatch[1];
                block.text = block.text.replace(idMatch[0], '').trim();
              }
            }
            const id = block.id ? ` id="${block.id}"` : '';
            switch (block.type) {
              case 'h1':
                return `<h1${id} style="color:${docStyle.headingColor};font-size:1.75rem;font-weight:bold;border-bottom:3px solid ${docStyle.accentColor};padding-bottom:0.5rem;margin:1.5em 0 0.5em">${block.text}</h1>`;
              case 'h2':
                return `<h2${id} style="color:${docStyle.headingColor};font-size:1.35rem;font-weight:bold;margin:1.5em 0 0.5em;${docStyle.hasSidebarAccents ? 'border-left:4px solid ' + docStyle.accentColor + ';padding-left:12px;' : ''}">${block.text}</h2>`;
              case 'h3':
                return `<h3${id} style="color:${docStyle.headingColor};font-size:1.1rem;font-weight:bold;margin:1.2em 0 0.4em">${block.text}</h3>`;
              case 'p':
                return `<p style="margin:0.6em 0;line-height:1.7">${block.text}</p>`;
              case 'ul':
                return `<ul style="margin:0.6em 0;padding-left:1.5em">${(Array.isArray(block.items) ? block.items : [block.text || '']).filter(Boolean).map(i => `<li style="margin:0.3em 0">${sanitizeField(i)}</li>`).join('')}</ul>`;
              case 'ol':
                return `<ol style="margin:0.6em 0;padding-left:1.5em">${(Array.isArray(block.items) ? block.items : [block.text || '']).filter(Boolean).map(i => `<li style="margin:0.3em 0">${sanitizeField(i)}</li>`).join('')}</ol>`;
              case 'table':
                {
              ...
              case 'link':
                return `<a href="${block.url || '#'}" style="color:${docStyle.accentColor}">${block.text}</a>`;
              case 'blockquote':
                return `<blockquote style="border-left:4px solid ${docStyle.accentColor};padding:12px 16px;margin:1em 0;background:${docStyle.bgColor === '#ffffff' ? '#f8fafc' : docStyle.bgColor};border-radius:0 8px 8px 0;font-style:italic">${block.text}</blockquote>`;
              case 'hr':
                return `<hr style="border:none;border-top:2px solid ${docStyle.sectionBorderColor};margin:2em 0">`;
              case 'banner':
                return `<div style="background:${docStyle.headerBg};color:${docStyle.headerText};padding:24px 28px;border-radius:12px;margin-bottom:24px"><div style="font-size:1.5em;font-weight:bold">${block.title || ''}</div>${block.subtitle ? '<div style="font-size:0.9em;opacity:0.85;margin-top:4px">' + block.subtitle + '</div>' : ''}</div>`;
              case 'rawhtml':
                return block.html || '';
              default:
                return `<div style="margin:0.6em 0">${block.text || ''}</div>`;
            }
      ...
      if (pageCount <= 8) {
        ...
        try {
          ...
          const repairSingle = raw => {
            ...
            if (lb > 0 && !r.endsWith(']')) r = r.substring(0, lb + 1) + ']';
            try {
              return JSON.parse(r);
            } catch (e) {}
          ...
          try {
            ...
            if (chunkResult) {
              ...
              const repairAndParseJson = raw => {
                ...
                let repaired = s.replace(/,\s*([}\]])/g, '$1') // trailing commas: ,} or ,]
              ...
              if (parsed && Array.isArray(parsed)) {
                ...
                if (objectMatches && objectMatches.length > 0) {
                  ...
                  try {
                    const fallbackHtml = await callGeminiVision(`Transform pages ${startPg}-${endPg} of this PDF into accessible HTML. Use headings, tables with th scope, lists, links. Include ALL content. Return ONLY HTML body content.`, _base64, _mimeType);
            ...
            return [{
              type: 'p',
              text: `[Pages ${startPg}-${endPg}] ` + extractedText.substring(fallbackStart, fallbackEnd).substring(0, 3000)
            }];
          }
          return [];
        };
        ...
        for (let batch = 0; batch < transformChunks; batch += 5) {
          ...
          const batchResults = await Promise.all(Array.from({
            length: batchEnd - batch
          }, (_, j) => processJsonChunk(batch + j)));
        ...
        window.__retryPdfChunk = async chunkIdx => {
          try {
            ...
            if (retryBlocks && retryBlocks.length > 0) {
              ...
              if (failIdx >= 0) {
                allBlocks.splice(failIdx, 1, {
                  type: 'rawhtml',
                  html: '<div data-chunk-start="' + chunkIdx + '" style="display:none"></div>'
                }, ...retryBlocks, {
                  type: 'rawhtml',
                  html: '<div data-chunk-end="' + chunkIdx + '" style="display:none"></div>'
                });
              }
              // Re-render full body
              bodyContent = renderJsonToHtml(allBlocks);
              ...
              if (storedResult && storedResult.set) storedResult.set(prev => ({
                ...prev,
                accessibleHtml: prev.accessibleHtml.replace(/<body[^>]*>[\s\S]*<\/body>/, '<body>' + bodyContent + '</body>')
              }));
...
Return ONLY the polished HTML body content.`, true);
              if (polished && polished.length > bodyContent.length * 0.5) {
                bodyContent = polished.trim().replace(/^\s*```[\w]*\n?/g, '').replace(/\n?```\s*$/g, '').replace(/^[\s\S]*?(<[a-zA-Z])/m, '$1');
              }
            } catch (polishErr) {
      ...
      if (bodyContent) {
        bodyContent = bodyContent.trim().replace(/^\s*```[\w]*\n?/g, '').replace(/\n?```\s*$/g, '').replace(/^[\s\S]*?(<[a-zA-Z])/m, '$1') // strip any preamble before first HTML tag
        .replace(/\\n\\n/g, '</p><p>') // literal \n\n → paragraph break
        .replace(/\\n/g, ' ') // literal \n → space
...
${hasSrc ? `<img src="${imgInfo.generatedSrc}" alt="${desc.replace(/"/g, '&quot;')}" style="max-width:100%;border-radius:8px;border:1px solid #e2e8f0">` : `<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg>
...
If no errors found, return: {"corrections": [], "totalErrors": 0}`, true);
        ...
        warnLog('[PDF Fix] Grammar check failed (non-blocking):', gramErr);
      }
      ...
      if (maxFixPasses > 0 && (axeResults && axeResults.totalViolations > 0 || bestAiScore < 90)) {
        for (let fixPass = 0; fixPass < maxFixPasses; fixPass++) {
          ...
          const axeInstructions = [].concat((axeResults ? axeResults.critical || [] : []).map(v => `CRITICAL (axe-core): ${v.description} (${v.id}, ${v.nodes} elements)`)).concat((axeResults ? axeResults.serious || [] : []).map(v => `SERIOUS (axe-core): ${v.description} (${v.id}, ${v.nodes} elements)`)).concat((axeResults ? axeResults.moderate || [] : []).map(v => `MODERATE (axe-core): ${v.description} (${v.id})`));
          const aiInstructions = verification && verification.issues ? verification.issues.map(i => `AI-FLAGGED: ${i.issue || i}`) : [];
          ...
          if (_postListFix.fixCount > 0) {
            accessibleHtml = _postListFix.html;
            warnLog(`  Deterministic: fixed ${_postListFix.fixCount} list issues`);
          }
          const _postContrastFix = fixContrastViolations(accessibleHtml);
          if (_postContrastFix.fixCount > 0) {
            accessibleHtml = _postContrastFix.html;
            warnLog(`  Deterministic: fixed ${_postContrastFix.fixCount} contrast issues`);
          }
          ...
          accessibleHtml = accessibleHtml.replace(/role="([^"]*)"/gi, (m, role) => {
            ...
            if (__roleCorrections[lower]) {
              __roleFixCount++;
              return `role="${__roleCorrections[lower]}"`;
            }
            __roleFixCount++;
            return '';
          });
          ...
          const [reVerify1, reVerify2, reVerify3, reAxe] = await Promise.all([auditOutputAccessibility(accessibleHtml), auditOutputAccessibility(accessibleHtml), auditOutputAccessibility(accessibleHtml), runAxeAudit(accessibleHtml)]);
          ...
          const reVerify = [reVerify1, reVerify2, reVerify3].filter(Boolean).sort((a, b) => (b.score || 0) - (a.score || 0))[0] || null;
          if (reVerify) {
            reVerify.score = newAiScore;
            reVerify._sem = reSEM;
            reVerify._sd = reSD;
            reVerify._scores = reScores;
          }
      ...
      const needsExpertReview = axeFailed ||
      // axe-core failed entirely — can't verify
      ...
      const _result = {
        ...
        needsExpertReview,
        docStyle,
        // extracted color palette for auto brand match in preview
        pageCount,
  ...
  const generateAuditReportHtml = (auditData, fileName, isBeforeAfter = false) => {
    ...
    const issueRows = (issues, severity) => (issues || []).map(i => {
      ...
      let issueText = (i.issue || '').replace(/\s*\(?\s*(?:WCAG\s*)?\d+\.\d+\.\d+\s*\)?\s*$/gi, '') // strip trailing WCAG code
...
<p style="color:#64748b;font-size:13px">Document: <strong>${fileName}</strong><br>Date: ${date}<br>Standards: WCAG 2.1 Level AA &bull; ADA Title II &bull; Section 508 &bull; EN 301 549<br>Methodology: AI multi-auditor triangulation + axe-core (Deque) automated verification<br>Tool: AlloFlow Document Accessibility Pipeline</p>`;
  ...
  const downloadAccessiblePdf = (htmlContent, filename) => {
    ...
    if (!printWindow) {
      addToast('Pop-up blocked — allow pop-ups to download PDF', 'error');
      return;
    }
      ...
      if (!exportAuditResult) {
        setTimeout(async () => {
          try {
            ...
            const [aiR, axeR] = await Promise.all([auditOutputAccessibility(html), runAxeAudit(html).catch(() => null)]);
            const combined = aiR || {
              score: 0,
              summary: '',
              issues: [],
            ...
            if (axeR) {
              combined.axeViolations = axeR.totalViolations;
              combined.axePasses = axeR.totalPasses;
            }
...
Return ONLY the CSS — no explanation, no markdown fences, just pure CSS.`);
  ...
  const generateResourceHTML = (item, isTeacher, responses = {}, config = null) => {
    ...
    const tv = typeVisuals[item.type] || {
      icon: '📄',
      color: '#475569',
      bg: '#f8fafc',
      label: ''
    ...
    if (item.type === 'simplified') {
      ...
      if (type === 'Venn Diagram') {
        const setA = branches[0] || {
          title: 'Set A',
          items: []
        };
        const setB = branches[1] || {
          title: 'Set B',
          items: []
        };
        const shared = branches[2] || {
          title: 'Shared',
          items: []
        };
      ...
      const reflectionHtml = Array.isArray(item.data.reflections) ? item.data.reflections.map(r => {
      ...
      const heatmapHtml = wordData.map(w => {
        ...
        if (w.status === 'correct') colorStyle = "color: #15803d; font-weight: bold;";else if (w.status === 'missed') colorStyle = "color: #ef4444; text-decoration: line-through;";else if (w.status === 'stumbled') colorStyle = "color: #d97706; border-bottom: 2px dotted #f59e0b;";
  ...
  const EXPORT_THEMES = {
    ...
    colorful: {
      name: 'Colorful Elementary',
      emoji: '🌈',
      bodyFont: "'Comic Sans MS', 'Lexend', sans-serif",
      headingColor: '#7c3aed',
      accentColor: '#ec4899',
      bgColor: '#fefce8',
      cardBg: '#ffffff',
      cardBorder: '#fbbf24',
      headerBg: 'linear-gradient(135deg, #7c3aed, #ec4899, #f59e0b)',
      headerText: '#ffffff',
      borderRadius: '16px',
      extraCSS: '.section { border-left: 5px solid #ec4899; border-radius: 12px; padding: 1.5rem; background: white; box-shadow: 0 2px 8px rgba(0,0,0,0.06); } h2 { color: #7c3aed; } .resource-header { background: linear-gradient(135deg, #faf5ff, #fdf2f8); border-left: 4px solid #a855f7; }'
    },
    minimal: {
      name: 'Minimalist',
      emoji: '✨',
    ...
    highContrast: {
      ...
      headerBg: '#000000',
      headerText: '#ffff00',
      extraCSS: 'body { font-size: 1.1rem; } a { color: #0000ff; text-decoration: underline; } .section { border: 2px solid #000; } th { background: #000; color: #fff; }'
    },
    nature: {
      name: 'Nature & Calm',
      emoji: '🌿',
      bodyFont: "'Lexend', system-ui, sans-serif",
      headingColor: '#166534',
      accentColor: '#15803d',
      bgColor: '#f0fdf4',
      cardBg: '#ffffff',
      ...
      headerText: '#ffffff',
      extraCSS: '.section { border-left: 4px solid #86efac; border-radius: 8px; background: white; } .resource-header { background: #f0fdf4; color: #166534; }'
    },
    print: {
      ...
      headerBg: '#ffffff',
      headerText: '#000000',
      extraCSS: 'body { font-size: 12pt; } .section { page-break-inside: avoid; } @media screen { body { max-width: 700px; } }'
    }
  };
  const generateFullPackHTML = (historyItems, topic, isWorksheet = false, responses = {}, config = null) => {
    ...
    return `
      ...
      'Haitian Creole': 'ht',
      'Somali': 'so'
    }[currentUiLanguage] || 'en'}" dir="${direction}">
      <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
