...
> **Counts and names are generated, not remembered.** This narrative goes stale
> between audits; the registry layer under it does not: see
> [`docs/FEATURE_REGISTRY_INVENTORY.md`](docs/FEATURE_REGISTRY_INVENTORY.md),
> regenerated by `node dev-tools/build_feature_registry_inventory.cjs` and kept
> current by `tests/feature_registry_inventory.test.js`. Quote counts from there.
