...
const useGameDialogFocus = (dialogRef, initialFocusRef, onClose, active = true) => {
  ...
  useEffect(() => {
    ...
    const onKeyDown = event => {
      // Nested selection/victory dialogs own Escape and Tab. Native bubbling
      // reaches this workspace before React's delegated dialog handlers.
...
// Thin delegator to window.AlloSpeechPlayer (defined in AlloFlowANTI.txt).
// The player handles: Gemini → Kokoro → browser cascade, mute respect,
// "currently speaking" state for UI feedback, language/rate plumbing, and
// error toasts when every fallback fails. We keep this wrapper so callers
// inside games_source can speakText('foo') without knowing about the player.
//
// When the player hasn't loaded yet (cold boot, before monolith eval) we
// silently no-op rather than crashing — the call sites are user-initiated
// (button clicks) so the next click after boot will work.
...
const SpeakButton = ({ text, size = 13, className = "" }) => {
  ...
  // Visual playing-state: this specific instance shows "Stop" when the player
  // is currently speaking THIS button's text. Match by text rather than by
  // session id so the button works even if the parent doesn't pass an id.
  // Multiple SpeakButtons rendering the same text will all flip — that's fine
  // (rare in practice and the behaviour is intuitive).
  ...
  React.useEffect(() => {
    ...
    // Initial sync: state event only fires on change, so check current state on mount
...
const gameMessage = (t, key, fallback) => { const value = t(key); return value && value !== key ? value : fallback; };
...
const GameReviewScreen = ({ score, title, items, onPlayAgain, onClose, t, onPractice, practiceCount = 0, playAgainLabel, focusOnMount = false }) => {
  ...
  useEffect(() => { if (focusOnMount) reviewTitleRef.current?.focus(); }, [focusOnMount]);
  ...
  return (
    <div role="region" aria-labelledby={reviewTitleId} className={`mt-4 bg-white rounded-2xl border-2 border-indigo-100 shadow-lg overflow-hidden${useReducedMotion() ? '' : ' animate-in fade-in slide-in-from-bottom-2 duration-300'}`}>
      <div className="bg-gradient-to-r from-indigo-700 to-purple-700 p-4 text-white text-center">
        <h3 ref={reviewTitleRef} tabIndex={-1} id={reviewTitleId} aria-describedby={reviewTitleId + '-summary'} className="text-xl font-black">{title || "Review"}</h3>
        <div id={reviewTitleId + '-summary'} className="flex flex-wrap items-center justify-center gap-3 mt-2">
          <span className="bg-white/20 px-3 py-1 rounded-full text-sm font-bold">{score} pts</span>
          <span className="bg-white/20 px-3 py-1 rounded-full text-sm font-bold">{correct}/{total} {gameMessage(t, 'common.correct', 'correct')}</span>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {items.map((item, idx) => (
            <div key={idx} className={`flex items-center gap-3 p-3 rounded-xl border-2 transition-all ${
              ...
              <span className="sr-only">{item.status === 'correct' ? gameMessage(t, 'common.correct', 'Correct') : item.status === 'unanswered' ? gameMessage(t, 'games.review.unanswered', 'Not answered') : gameMessage(t, 'common.incorrect', 'Incorrect')}</span>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-sm text-slate-800 break-words">{item.label}</div>
                {item.detail && <div className="text-sm leading-relaxed text-slate-600 break-words">{item.detail}</div>}
      ...
      <div className="p-3 border-t border-slate-200 flex flex-wrap gap-2 justify-center">
        {onPractice && practiceCount > 0 && <button type="button" data-game-review-practice onClick={onPractice} className="min-h-11 px-5 py-2 rounded-full text-xs font-bold bg-amber-100 text-amber-900 border border-amber-300 hover:bg-amber-200 focus:ring-2 focus:ring-amber-600 focus:ring-offset-2">{gameMessage(t, 'games.review.practice_words', 'Practice these words')} ({practiceCount})</button>}
        <button type="button" onClick={onPlayAgain} className="min-h-11 px-5 py-2 rounded-full text-xs font-bold bg-indigo-600 text-white hover:bg-indigo-700 transition-colors flex items-center gap-2 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
          <RefreshCw size={14} aria-hidden="true" /> {playAgainLabel || gameMessage(t, 'memory.play_again', 'Play Again')}
        </button>
...
const MemoryGame = React.memo(({ data, onClose, onScoreUpdate, onGameComplete, roundSize = 10 }) => {
  ...
  const initializeGame = () => {
    ...
    const deck = gameItems.flatMap((item, index) => {
      ...
      switch (strategy) {
          case 'term-def':
              content1 = item.term; type1 = 'text'; isTerm1 = true; accessibleLabel1 = `Term: ${item.term}`;
              content2 = item.def; type2 = 'text'; isTerm2 = false; accessibleLabel2 = `Definition: ${item.def}`;
              break;
          case 'image-def':
              content1 = item.image; type1 = 'image'; isTerm1 = false; accessibleLabel1 = meaningfulImageLabel;
              content2 = item.def; type2 = 'text'; isTerm2 = false; accessibleLabel2 = `Definition: ${item.def}`;
              break;
          case 'term-image':
          default:
              content1 = item.term; type1 = 'text'; isTerm1 = true; accessibleLabel1 = `Term: ${item.term}`;
              content2 = item.image; type2 = 'image'; isTerm2 = false; accessibleLabel2 = meaningfulImageLabel;
              break;
      }
      const card1 = {
        id: `p${index}-1`,
  ...
  const handleCardClick = (index) => {
    if (
      !cards[index] || flippedIndices.includes(index) ||
  ...
  return (
    ...
    >
      ...
      {cards.length === 0 && <p role="status" className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-amber-900">{gameMessage(t, 'games.memory.empty', 'Add glossary terms with a definition or picture to play Memory.')}</p>}
      ...
      {isWon ? (() => {
          ...
          <div role="status" className={`flex flex-col items-center justify-center py-8 px-4 text-center${useReducedMotion() ? '' : ' animate-in zoom-in duration-300'}`}>
            ...
            <div role="group" className={`flex items-center gap-1 mb-4 ${!useReducedMotion() ? 'animate-in zoom-in duration-500' : ''}`} aria-label={`${stars} out of 3 stars`}>
              {[0, 1, 2].map(i => (
                <Star
                  key={i}
                  size={32}
                ...
                <div className={`w-full h-full ${!useReducedMotion() ? 'transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)]' : ''} transform-style-3d rounded-xl border-2 ${isFlipped ? `rotate-y-180 ${isMismatch ? 'border-red-400 shadow-lg shadow-red-200' : isMatched ? 'border-green-400 shadow-lg shadow-green-200' : 'border-indigo-300 shadow-md'}` : 'rotate-y-0 border-slate-200 bg-white shadow-sm group-hover:shadow-md'}`}>
                  ...
                  <div className={`absolute inset-0 backface-hidden rotate-y-180 rounded-xl flex items-center justify-center p-2 text-center overflow-hidden ${isMismatch ? 'bg-red-50' : isMatched ? 'bg-green-50' : 'bg-white'}`}>
                    {card.type === 'image' ? (
                      ...
                      <div className="w-full h-full flex justify-center overflow-y-auto custom-scrollbar px-1">
                          <p className={`font-bold w-full my-auto leading-tight ${isMismatch ? 'text-red-900' : isMatched ? 'text-green-900' : 'text-slate-800'} ${card.isTerm ? 'text-xs sm:text-sm' : 'text-[11px] sm:text-[11px] font-normal text-slate-600 leading-snug'}`}>
                            {card.content}
                          </p>
                      </div>
                    )}
...
// Ignore descriptions belonging to an earlier image, just as the glossary does.
function matchingImageDescription(item) {
  ...
  if (item.imageAltHash) {
    ...
    let hash = 0x811c9dc5;
    const mix = code => { hash ^= code; hash = Math.imul(hash, 0x01000193) >>> 0; };
...
function MatchingClue({ clue, pictureMode, t }) {
  ...
  if (!pictureMode) return <span>{clue.text}</span>;
  return <figure className="w-full min-w-0 flex flex-col items-center justify-center gap-1">
    {clue.image && failedImage !== clue.image
      ? <img src={clue.image} alt={clue.text} draggable={false} onError={() => setFailedImage(clue.image)} style={{ width: '100%', height: 104, objectFit: 'contain' }} />
      : <div><span className="block text-[10px] font-semibold text-slate-500">{clue.image ? gameMessage(t, 'matching.picture_unavailable', 'Picture unavailable') : gameMessage(t, 'matching.definition_clue', 'Definition clue')}</span><span>{clue.definition || clue.text}</span></div>}
    {clue.image && clue.credit && <figcaption className="text-[9px] leading-tight text-slate-500 break-words">{clue.credit}</figcaption>}
  </figure>;
}
const MatchingGame = React.memo(({ data, onClose, playSound, onScoreUpdate, onGameComplete, roundSize = 8 }) => {
  ...
  useEffect(() => {
    ...
    const validItems = fisherYatesShuffle((Array.isArray(data) ? data : []).filter(d => d && typeof d.term === 'string' && d.term.trim() && ((typeof d.def === 'string' && d.def.trim()) || (clueMode === 'pictures' && typeof d.image === 'string' && d.image.trim())))).slice(0, Math.max(1, Math.min(8, Math.floor(Number(roundSize)) || 8))).map((item, i) => ({
        ...
        credit: item.imageAttribution ? [item.imageAttribution.set, item.imageAttribution.author, item.imageAttribution.license].filter(value => typeof value === 'string' && value.trim()).join(' · ') : ''
    }));
  ...
  useEffect(() => {
    if (isChecked) { if (scrollRef.current) scrollRef.current.scrollTop = 0; return; }
  ...
  const handleMouseDown = (e, termId) => {
      ...
      const cancelDrag = () => { cleanup(); setTempLine(null); setSnapTarget(null); };
      const cleanup = () => {
          window.removeEventListener('mousemove', onMove);
          window.removeEventListener('mouseup', onUp);
          window.removeEventListener('touchmove', onMove);
  ...
  // Remove the meaning actually matched, even when two rows share a term label.
  ...
  return (
    <div ref={matchingDialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="matching-game-title" className={`allo-glossary-print-dialog glossary-matching-dialog fixed inset-0 z-[100] bg-slate-50 flex flex-col overflow-hidden${useReducedMotion() ? '' : ' animate-in fade-in duration-300'}`}>
        ...
        <section className="glossary-matching-sheet">
            ...
            <p>{clueMode === 'pictures' ? gameMessage(t, 'matching.picture_print_instructions', 'Write the letter of the matching picture or definition beside each word.') : t('matching.print_instructions')}</p>
            <table><tbody>{items.map((item, index) => <tr key={item.id}>
                <td><strong>{index + 1}. {item.term}</strong> <span>_____</span></td>
                <td><strong>{String.fromCharCode(65 + index)}.</strong> {rightCol[index] && <MatchingClue clue={rightCol[index]} pictureMode={clueMode === 'pictures'} t={t} />}</td>
            </tr>)}</tbody></table>
        </section>
        <div className="sr-only" role="status" aria-live="polite">{announcement}</div>
        <div className="bg-white border-b border-slate-200 p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 shadow-sm no-print z-20 relative">
            <div>
                 <h3 id="matching-game-title" className="font-bold text-lg flex items-center gap-2 text-indigo-900">
                     <GitMerge size={20} className="text-orange-500"/> {t('matching.title')}
                 </h3>
                 <p className="text-xs text-slate-600">{clueMode === 'pictures' ? gameMessage(t, 'matching.picture_instructions', 'Choose a word, then its picture. Words without pictures use definitions.') : t('matching.instructions')}</p>
                 <label className="flex flex-wrap items-center gap-2 mt-2 text-xs font-semibold text-slate-700">
                     {gameMessage(t, 'matching.match_with', 'Match words with')}
                     <select aria-label={gameMessage(t, 'matching.match_with', 'Match words with')} aria-describedby="matching-mode-help" value={clueMode} onChange={event => setClueMode(event.target.value)} className="min-h-11 max-w-full rounded-lg border border-slate-400 bg-white px-3 py-2 text-slate-900">
                         <option value="definitions">{gameMessage(t, 'matching.definitions', 'Definitions')}</option>
                         <option value="pictures">{gameMessage(t, 'matching.pictures', 'Pictures')}</option>
                     </select>
                 </label>
                 {isPracticeRound && <p className="mt-1 text-xs font-bold text-amber-800" data-game-practice-round>{gameMessage(t, 'games.review.practice_round', 'Practice round')} · {items.length}/{originalRoundRef.current.length} {gameMessage(t, 'matching.pairs', 'pairs')}</p>}
                 <p id="matching-mode-help" className="text-xs text-slate-600 mt-1">{gameMessage(t, 'matching.mode_help', 'Changing this starts a new round.')}</p>
                 {clueMode === 'pictures' && <p className="text-xs text-slate-600 mt-1" role="status">{gameMessage(t, 'matching.pictures', 'Pictures')}: {items.filter(item => item.image).length}/{items.length}. {gameMessage(t, 'matching.picture_fallback', 'Missing or unavailable pictures use text clues.')}</p>}
                 <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
            ...
            <div className="flex flex-wrap justify-end items-center gap-1 p-1 rounded-full bg-slate-50 border border-slate-400 shadow-sm self-end sm:self-auto">
                ...
                >
                    ...
                    <span className="hidden sm:inline">{t('a11y.read_aloud') || 'Audio hints'}</span>
        ...
        {isChecked && (
          <div className="px-6 pb-2">
            {items.length > 0 && score === items.length * 25 && !useReducedMotion() && <ConfettiExplosion />}
            <GameReviewScreen
              score={score}
              ...
              items={connections.filter(isCorrectConnection).map(connection => {
                ...
                return {
                  ...
                  note: !attempted ? gameMessage(t, 'games.review.unanswered', 'Not answered') : undefined
                };
              }))}
              onPlayAgain={reset}
              playAgainLabel={isPracticeRound ? gameMessage(t, 'matching.restart_all', 'Restart all pairs') : undefined}
            ...
            >
                <div className="hidden print:block mb-8 text-center">
                    ...
                    <div className="flex justify-between text-sm border-b-2 border-slate-800 pb-2 mb-4">
                        ...
                        <span>{t('matching.print_score')}: _______ / {items.length * 25}</span>
                    </div>
                ...
                >
                    ...
                    {items.length === 0 && <p role="status" className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-amber-900">{gameMessage(t, 'games.matching.empty', 'Add glossary terms and definitions to play Matching.')}</p>}
                    <div className="flex justify-between gap-3 sm:gap-12 h-full">
                        <div className="w-2/5 sm:w-1/3 min-w-0 space-y-6">
                            {items.map((item) => (
                                <div key={item.id} className={`flex items-center justify-between ${clueMode === 'pictures' ? 'h-44' : 'h-28 sm:h-24'} group relative z-20`}>
                                    <div
                                        onClick={() => {
                                            if (isChecked) return;
                                    ...
                                    <div
                                        ...
                                        aria-label={`${t('matching.select_term_aria')}: ${item.term}`}
                                        aria-pressed={keyboardSelectedTerm === item.id}
                                        className={`w-6 h-6 shrink-0 bg-white border-4 rounded-full cursor-pointer touch-none hover:scale-110 transition-transform ms-1 sm:ms-4 print:bg-black print:border-black print:w-4 print:h-4 print:scale-100 focus:ring-4 focus:ring-indigo-300 ${
                        ...
                        <div className="w-3/5 sm:w-2/3 min-w-0 space-y-6">
                            {rightCol.map((def) => (
                                <div key={def.id} className={`flex items-center justify-between ${clueMode === 'pictures' ? 'h-44' : 'h-28 sm:h-24'} relative z-20`}>
                                    <div
                                        data-id={def.id}
                                    ...
                                    >
                                        <MatchingClue clue={def} pictureMode={clueMode === 'pictures'} t={t} />
                                    </div>
                                </div>
                            ))}
                        </div>
...
const ConceptSortGame = React.memo(({ data, onClose, playSound, onGenerateItem, onScoreUpdate, onGameComplete, onExplainIncorrect, imageScale, onImageScaleChange, allowAddItems = true }) => {
  ...
  // Cards confirmed correct on the last check. They stay where they are and
  // cannot be moved while the learner fixes the rest, so a retry never undoes
  ...
  // After a check with mistakes: keep every correct card locked in its bucket
  // and return only the incorrect ones to the deck. Before this the only way
  // forward was Reset, which sent all cards back, correct ones included.
  const retryIncorrect = () => {
    ...
    setIsChecked(false);
    setAttempts(prev => prev + 1);
    setExplanations(prev => { const next = { ...prev }; returned.forEach(i => { delete next[i.id]; }); return next; });
    setKeyboardSelectedItemId(null);
    const tmpl = t('concept_sort.announce.retry_incorrect');
    setAnnouncement(tmpl
      ? tmpl.replace('{kept}', String(correctIds.size)).replace('{returned}', String(returned.length))
      : `${correctIds.size} correct card${correctIds.size === 1 ? '' : 's'} kept in place. ${returned.length} returned to the deck to sort again.`);
    if (playSound) playSound('click');
  };
  ...
  const renderCard = (item) => {
    ...
    return (
      <div
        ...
        onClick={() => { if (!isChecked && !locked) { if (keyboardSelectedItemId === item.id) { setKeyboardSelectedItemId(null); } else { setKeyboardSelectedItemId(item.id); if (playSound) playSound('click'); } } }}
        tabIndex={isChecked || locked ? -1 : 0}
        role="button"
        aria-pressed={keyboardSelectedItemId === item.id}
        aria-label={locked
          ? `${item.content}. ${t('concept_sort.locked_aria') || 'Correct and locked in place.'}`
          : `${item.content}. ${item.currentContainer === 'deck' ? t('concept_sort.unsorted_aria') : t('concept_sort.sorted_aria')}. ${t('concept_sort.move_aria')}`}
        className={`
               ...
               <div className="max-w-6xl mx-auto relative">
                   <div className="flex justify-between items-start mb-2">
                       ...
                       <div className="flex gap-2 items-center">
                           ...
                           {isChecked && incorrectPlacedCount > 0 && (
                               <button type="button"
...
// Shared anti-score-farming economics for drag-sort games. A first-try correct
// placement earns 20; a correction after a revealed hint earns 5, offsetting
// the prior -5; already-awarded items cannot award again.
const makeSortScoreTracker = () => {
  const missed = new Set();
  const awarded = new Set();
  return {
    correct(id) { if (awarded.has(id)) return 0; awarded.add(id); return missed.has(id) ? 5 : 20; },
    incorrect(id) { missed.add(id); return -5; },
  };
};
  ...
  return (
    <div ref={gameContainerRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="multi-bucket-game-title" className={`fixed inset-0 z-[200] bg-slate-50 flex flex-col${reducedMotion ? '' : ' animate-in zoom-in-95'}` }>
      ...
      <div className="flex-grow relative overflow-y-auto">
        ...
        {isWon && (
          <div role="presentation" className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div ref={multiBucketWinRef} role="dialog" aria-modal="true" aria-labelledby="mb-victory-title" aria-describedby="mb-victory-description"
              onKeyDown={event => {
                event.stopPropagation();
                if (event.key === 'Escape') { event.preventDefault(); onClose(); return; }
                if (event.key !== 'Tab') return;
        ...
        {lastHint && lastHintLabel && (
          <div className={`absolute top-4 left-1/2 -translate-x-1/2 z-40 bg-amber-100 border-2 border-amber-400 text-amber-800 px-5 py-2 rounded-full shadow-lg font-bold text-sm flex items-center gap-2${reducedMotion ? '' : ' animate-in fade-in slide-in-from-top-2 duration-300'}`}>
            <HelpCircle size={16} aria-hidden="true" /> {t('games.ce_sort.hint_try') || 'Try'}: {lastHintLabel}
          </div>
        )}
        {keyboardSelectedItemId && (
...
// ── ProblemSolutionSortGame — match details to source-grounded solution paths ──
const ProblemSolutionSortGame = React.memo(({ data, onClose, playSound, onScoreUpdate, onGameComplete, topicTitle = "" }) => {
  const { t } = useContext(LanguageContext);
  const adapted = useMemo(() => {
    const branches = Array.isArray(data?.branches) ? data.branches : [];
    // Match the diagram's actual solution paths. Array order is not a defensible priority ranking.
...
const PipelineBuilderGame = React.memo(({ data, onClose, playSound, onScoreUpdate, onGameComplete, topicTitle = "" }) => {
  ...
  const rewardedConnectionKeysRef = useRef(new Set());
  ...
  const getStepTargets = useCallback((origIdx) => {
    ...
    if (Array.isArray(rawTargets)) {
      const validTargets = [...new Set(rawTargets
        .map(target => Number(target))
        .filter(target => Number.isInteger(target) && target >= 0 && target < stepCount && target !== origIdx))];
      if (validTargets.length > 0) return validTargets;
    }
    return origIdx < stepCount - 1 ? [origIdx + 1] : [];
  }, []);

  // Serialize all game-relevant data so parent renders do not reset the board,
  // while genuine edits to labels, details, or branch destinations do.
  const dataFingerprint = useMemo(() => {
    if (!data?.steps?.length) return '';
    return JSON.stringify(data.steps.map((s, i) => typeof s === 'string'
      ? { title: s, items: [], connectsTo: i < data.steps.length - 1 ? [i + 1] : [] }
      : {
          title: s.title || '',
  ...
  useEffect(() => {
    ...
    setChecked(false);
    setNodePositions({});
    rewardedConnectionKeysRef.current = new Set();
  ...
  const addConnection = useCallback((fromId, toId, requiredOut) => {
    setConnections(prev => {
      // Keep a single copy of an edge, but allow several branches to converge
      // on the same target node. A merge is a valid flow-chart relationship.
  ...
  const handleNodeClick = (nodeId) => {
    ...
    if (connectingFrom === null) {
      ...
      setAnnouncement(remainingBranches > 0
        ? `Connected "${fromStep?.title}" → "${toStep?.title}". Choose ${remainingBranches} more branch destination${remainingBranches === 1 ? '' : 's'}.`
        : `Connected "${fromStep?.title}" → "${toStep?.title}".`);
  ...
  const handleNodeKeyDown = (e, nodeId) => {
    if (e.key === 'Enter' || e.key === ' ') {
      ...
      if (keyboardSelectedId === nodeId) {
        ...
        const remainingBranches = Math.max(0, requiredOut - outgoingTargets.size);
        setAnnouncement(remainingBranches > 0
          ? `Connected steps. Select ${remainingBranches} more branch destination${remainingBranches === 1 ? '' : 's'}.`
          : 'Connected steps.');
  ...
  const handleCheck = () => {
    ...
    if (correctCount === totalRequired && incorrectCount === 0) {
      ...
      if (playSound) playSound('incorrect');
      const retryGuidance = incorrectCount > 0
        ? 'Incorrect connections will be removed.'
        : 'Add the missing connections and check again.';
...
const CrosswordGame = React.memo(({ data, onClose, playSound, onScoreUpdate, onGameComplete }) => {
  ...
  useEffect(() => {
    setGrid([]); setClues({ across: [], down: [] }); setUserState({});
    setIsWon(false); setWasRevealed(false); setShowErrors(false); setScore(0);
    setHintsUsed(0); setSelectedCell(null); setDirection('across');
    ...
    const occupied = [];
    tempGrid.forEach((row, r) => row.forEach((cell, c) => { if (cell) occupied.push({ r, c }); }));
    ...
    for (const direction of ['across', 'down']) newClues[direction] = newClues[direction].map(clue => ({ ...clue, row: clue.row - minRow, col: clue.col - minCol }));
    setGrid(compactGrid);
    setClues(newClues);
    ...
    <div ref={crosswordDialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="crossword-game-title" className="allo-glossary-print-dialog glossary-crossword-dialog fixed inset-0 z-[100] bg-white flex flex-col motion-safe:animate-in motion-safe:fade-in motion-safe:duration-300" data-help-key="crossword_game_container">
      ...
      <div className="flex-grow overflow-hidden flex flex-col md:flex-row">
         <div className="flex-grow p-4 overflow-auto bg-slate-100 flex flex-col items-center gap-3 relative print:bg-white print:overflow-visible print:p-0">
            ...
            <div className="max-w-full overflow-x-auto shrink-0 p-1 print:overflow-visible">
            <div
...
// Keep punctuation attached to words, including scripts without spaces.
const syntaxTokens = (sentence) => {
  if (/\s/.test(sentence.trim())) return sentence.trim().split(/\s+/);
  try {
    const tokens = [];
    for (const part of new Intl.Segmenter(undefined, { granularity: 'word' }).segment(sentence)) {
...
const SyntaxScramble = React.memo(({ text, onClose, playSound, onScoreUpdate, onGameComplete }) => {
  ...
  useEffect(() => {
    ...
    setCurrentSentenceIndex(0); setScore(0); setUserOrder([]); setShuffledWords([]);
    setGameStatus('playing'); setAnnouncement('');
  }, [text]);
  useEffect(() => {
    if (sentences.length > 0 && currentSentenceIndex < sentences.length) {
      ...
      syntaxRoundRef.current = false;
      setShuffledWords(shuffled); setUserOrder([]); setGameStatus('playing'); setAnnouncement('');
  ...
  useEffect(() => {
    ...
    if (onGameComplete) onGameComplete('syntaxScramble', {
      score, sentencesCompleted: sentences.length, totalSentences: sentences.length
    });
  ...
  const checkAnswer = () => {
    ...
    if (answer === target) {
      ...
      setGameStatus('correct'); setScore(s => s + 40);
  ...
  return (
    <div role="presentation" className={`fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-sm flex items-center justify-center p-4${useReducedMotion() ? '' : ' animate-in zoom-in-95'}` }>
      <div ref={syntaxDialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="syntax-game-title" className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="bg-indigo-600 p-4 text-white flex flex-wrap gap-3 justify-between items-center">
        ...
        <div className="p-4 sm:p-8 flex-grow flex flex-col items-center bg-slate-50 gap-5 overflow-y-auto">
           {sentences.length === 0 ? (
               <p role="status" className="w-full rounded-xl border border-amber-300 bg-amber-50 p-5 text-amber-900">{gameMessage(t, 'games.syntax.empty', 'Add a sentence with at least four words and fewer than 150 characters to play Syntax Scramble.')}</p>
           ) : gameStatus === 'complete' ? (
               ...
               <>
                <div className="w-full flex flex-wrap gap-2 justify-between text-xs font-bold text-slate-600 uppercase tracking-wider">
                    <span>{t('games.syntax.progress', { current: currentSentenceIndex + 1, total: sentences.length })}</span>
                    <span>{t('games.syntax.subtitle')}</span>
                </div>
                <div dir="auto" className={`w-full min-h-[80px] p-4 rounded-xl border-2 border-dashed flex flex-wrap gap-2 items-center justify-center transition-colors ${gameStatus === 'correct' ? 'bg-green-50 border-green-400' : 'bg-white border-slate-300'}`}>
                ...
                <p role="status" aria-live="polite" aria-atomic="true" className="min-h-6 text-center text-sm font-bold text-slate-700">{announcement}</p>
                ...
                <div dir="auto" className="flex flex-wrap gap-3 justify-center p-4 bg-slate-200/50 rounded-xl w-full border border-slate-400 min-h-[100px]">
...
const BingoGame = React.memo(({ data, onClose, settings, setSettings, onGenerate, bingoState, setBingoState, onGenerateAudio, selectedVoice, alloBotRef }) => {
  ...
  const stopCallerPlayback = () => {
      ...
      if (callerAudioRef.current) { callerAudioRef.current.pause(); try { callerAudioRef.current.currentTime = 0; } catch (_) {} callerAudioRef.current = null; }
      if (autoPlayTimerRef.current) { clearTimeout(autoPlayTimerRef.current); autoPlayTimerRef.current = null; }
      setIsAudioPlaying(false); setIsAutoPlaying(false);
  };
  const handleExitCaller = () => { stopCallerPlayback(); setIsCallerMode(false); };
  const handleCloseBingo = () => { stopCallerPlayback(); onClose(); };
  ...
  const toggleAutoPlay = () => {
      ...
      if (!newState && autoPlayTimerRef.current) { clearTimeout(autoPlayTimerRef.current); autoPlayTimerRef.current = null; }
  ...
  useEffect(() => {
      if (!isAudioPlaying && isAutoPlaying && currentCallIndex >= callerQueue.length - 1) { setIsAutoPlaying(false); return; }
        ...
        <div className="bg-white w-full max-w-6xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh] p-6 relative bingo-modal-container">
            ...
            {isCallerMode ? (
                <div className="flex-grow flex flex-col md:flex-row gap-4 overflow-y-auto">
                    <div className="flex-grow shrink-0 min-w-0 bg-slate-100 rounded-2xl border-4 border-teal-500 flex flex-col items-center justify-center p-4 sm:p-8 text-center relative shadow-inner">
                        {currentCallIndex >= 0 ? (
                            <div role="status" aria-live="polite" className="motion-safe:animate-in motion-safe:zoom-in motion-safe:duration-300 max-w-3xl">
                                ...
                                <h3 className="text-xl sm:text-3xl md:text-4xl break-words font-medium text-slate-800 leading-relaxed mb-6">
                                    "{callerQueue[currentCallIndex].def}"
                                </h3>
                                {callerQueue[currentCallIndex].translations && (
                                    <div className="pt-6 border-t border-slate-200">
                                        {Object.entries(callerQueue[currentCallIndex].translations).filter(([, trans]) => typeof trans === 'string').map(([lang, trans], idx) => (
                                            <p key={idx} className="text-xl md:text-2xl text-slate-600 italic mt-2">
                                                "{trans.includes(':') ? trans.slice(trans.indexOf(':') + 1).trim() : trans}"
                                            </p>
                                        ))}
                                    </div>
                                )}
                    ...
                    <div className="w-full md:w-64 max-h-64 md:max-h-none bg-slate-50 rounded-2xl border border-slate-400 flex flex-col overflow-hidden shrink-0">
    ...
    <div ref={studentBingoDialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="student-bingo-title" className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-center p-2 sm:p-4 motion-safe:animate-in motion-safe:fade-in motion-safe:zoom-in-95 motion-safe:duration-300">
        ...
        <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden border-4 border-indigo-500 flex flex-col max-h-[90vh]">
            ...
            <div className="p-2 sm:p-6 overflow-auto custom-scrollbar bg-indigo-50 flex-grow flex items-center justify-center">
                {grid.length === 0 && <p role="status" className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-amber-900">{gameMessage(t, 'games.bingo.empty', 'Add glossary terms to create a Bingo card.')}</p>}
                            ...
                            const cellContent = (
                                <>
                                    {showImages && cell.imageUrl && (
                                        <img src={cell.imageUrl} alt="" aria-hidden="true" className={`w-8 h-8 sm:w-10 sm:h-10 object-contain rounded mb-0.5 ${isMarked && cell.type !== 'free' ? 'opacity-100' : ''}`} />
                                    )}
                                    <span style={{ overflowWrap: 'anywhere' }} className={`w-full text-[11px] sm:text-xs font-bold leading-tight break-words ${isMarked && cell.type !== 'free' ? 'opacity-100' : ''}`}>
                                        {cell.text}
                                    </span>
                                    {isMarked && (
                                        <span className="absolute top-1 right-1 flex items-center justify-center z-10 pointer-events-none" aria-hidden="true">
                                            {cell.type === 'free' ? (
                                                ...
                                                <span className="w-4 h-4 rounded-full bg-indigo-700 text-white flex items-center justify-center text-xs font-black">✓</span>
                                            )}
                                        </span>
                                    )}
                                </>
...
const WordScrambleGame = React.memo(({ data, onClose, playSound, onScoreUpdate, onGameComplete }) => {
  ...
  const startScrambleRound = (items, practice = false) => {
      ...
      if (items.length > 0) { setScrambled(scrambleWord(items[0].letters)); requestAnimationFrame(() => scrambleInputRef.current?.focus()); }
      else setScrambled('');
  };
  ...
  return (
    <div ref={scrambleDialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="word-scramble-title" className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-sm flex items-center justify-center p-4 motion-safe:animate-in motion-safe:zoom-in-95">
        <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col relative max-h-[95vh] overflow-y-auto p-4 sm:p-8 text-center border-4 border-indigo-500">
            <div className="sr-only" role="status" aria-live="polite">{announcement}</div>
            <button
                type="button"
                onClick={onClose}
            ...
            <div className="mb-6">
                ...
                {isPracticeRound && <p className="mt-2 text-sm font-bold text-amber-800" data-game-practice-round>{gameMessage(t, 'games.review.practice_round', 'Practice round')} · {gameItems.length}/{originalItemsRef.current.length} {gameMessage(t, 'games.review.words', 'words')}</p>}
            </div>
            <div className="flex-grow flex flex-col items-center justify-center min-h-[300px] bg-slate-50 rounded-xl border-2 border-dashed border-slate-200 p-3 sm:p-6 gap-4 sm:gap-6">
                {isGameOver ? (
                    <div role="status" className="text-center motion-safe:animate-in motion-safe:zoom-in">
                        ...
                        <GameReviewScreen
                          ...
                          items={results.map(r => ({ label: r.term, detail: r.def, status: r.correct ? 'correct' : 'unanswered', note: !r.correct ? gameMessage(t, 'games.review.skipped', 'Skipped') : r.usedHint ? gameMessage(t, 'games.review.hint_used', 'Solved with a hint') : r.needsPractice ? gameMessage(t, 'games.review.another_try', 'Solved after another try') : undefined }))}
                          onPlayAgain={resetScrambleGame}
                          playAgainLabel={isPracticeRound ? gameMessage(t, 'games.scramble.restart_all', 'Restart all words') : undefined}
                    ...
                    <>
                        ...
                        <div className="flex flex-col gap-3 w-full max-w-xs motion-safe:animate-in motion-safe:slide-in-from-bottom-4 motion-safe:fade-in">
                            <input
                                ...
                                onKeyDown={(e) => { if (e.key === 'Enter' && !e.nativeEvent.isComposing) handleCheck(); }}
                            ...
                            />
                            <p role="status" className="min-h-6 text-sm font-bold text-slate-700">{feedback === 'correct' ? gameMessage(t, 'games.scramble.correct', 'Correct!') : feedback === 'incorrect' ? gameMessage(t, 'games.scramble.incorrect', 'Try again. Use the clue or ask for a hint.') : ''}</p>
                            <div className="flex gap-2 w-full">
                                <button type="button" disabled={feedback === 'correct' || hintLevel >= Math.max(1, hintCells.length - 1)} onClick={useHint} className="flex-1 py-3 rounded-xl font-bold text-amber-800 bg-amber-50 hover:bg-amber-100 border border-amber-200 transition-colors flex items-center justify-center gap-1 focus:ring-2 focus:ring-amber-600 focus:ring-offset-2" aria-label={t('games.scramble.get_hint_aria') || 'Get a hint'}>
...
// Items in `data.zones[zoneId]` are pre-classified as belonging to that zone;
// game shuffles them all into a bank, student drags each to the correct zone.
// Visual layout adapts to layoutMode: 'grid-2x2' (Frayer), 'columns-3' (STW),
// 'arc-5' (Story Map; rendered as 5-column row since true SVG arc + drag is
// out of scope for v1).
...
// ── SeeThinkWonderSortGame — wrapper ──
...
// ── StoryMapSortGame — wrapper. v1: classification (which stage does this belong to?). ──
// v2 (deferred): sequence ordering with arc visualization + drag-to-position.
