...
describe('Volume mathematics and diagram geometry',()=>{
const svgFor=k=>fs.readFileSync(folder+manifest.assets.find(a=>a.sourceKey===k).vectorFile,'utf8');
it('uses complete unit-cube models, equal layers and non-overlapping joined blocks',()=>{
const stack=manifest.models.stack.cubes;expect(stack).toHaveLength(60);
expect(new Set(stack.map(c=>[c.x,c.y,c.z].join(','))).size).toBe(60);
for(let z=0;z<5;z++)expect(stack.filter(c=>c.z===z)).toHaveLength(12);
const svg=svgFor('stack');
expect([...svg.matchAll(/data-face="top"/g)]).toHaveLength(12);
expect([...svg.matchAll(/data-face="right"/g)]).toHaveLength(15);
expect([...svg.matchAll(/data-face="left"/g)]).toHaveLength(20);
expect([...svgFor('exploded').matchAll(/data-face="top"/g)]).toHaveLength(60);
const joined=manifest.models.joined.cubes;expect(joined).toHaveLength(28);
expect(new Set(joined.map(c=>[c.x,c.y,c.z].join(','))).size).toBe(28);
expect(joined.filter(c=>c.color==='teal')).toHaveLength(16);
expect(joined.filter(c=>c.color==='gold')).toHaveLength(12);
expect([...svgFor('joined').matchAll(/data-face="top"/g)]).toHaveLength(14);
expect(manifest.models.compare.sets.map(s=>s.length)).toEqual([36,36]);
expect(svgFor('height')).toContain('M750 374H820V734H750');
expect(manifest.models.net.netCells).toHaveLength(6);
expect(new Set(manifest.models.net.netCells.map(c=>c.join(','))).size).toBe(6);
});
it('bounds prism and packaging claims and records each content refinement',()=>{
expect(get('simplified').data).toContain('Not every solid');
expect(get('faq').data[1].answer).toContain('same base area and perpendicular height');
expect(get('faq').data[5].answer).toContain('Mixed units still have meaning');
expect(get('concept-sort').data.items[4].content).toBe('The area of one wall to be painted');
expect(get('applied-challenge').data.brief.constraints[0]).toContain('rotations');
