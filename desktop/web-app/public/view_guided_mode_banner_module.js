...
function GuidedModeBanner({
  ...
  const guidedErrorGuidance = /\b(401|403)\b|unauthenticated|unauthorized|permission.denied|api[ _-]?key|missing.key|invalid.key/.test(guidedErrorSignal) ? t("guided.error_access") || "Check your AI connection and access settings, then retry. A missing or invalid key cannot be fixed by retrying alone." : /quota|insufficient[ _-]?credits|billing|usage.limit/.test(guidedErrorSignal) ? t("guided.error_quota") || "Check your AI provider usage allowance, or choose another configured provider before retrying." : /\b429\b|rate[ _-]?limit|too many requests|resource.exhausted/.test(guidedErrorSignal) ? t("guided.error_rate_limit") || "The service is busy. Wait a moment, then retry." : /network|fetch|connection|offline|timed?\s*out|timeout/.test(guidedErrorSignal) ? t("guided.error_network") || "Check your connection, then try again." : step.id === "image" ? t("guided.error_image") || "Check the image settings or try a simpler visual request." : t("guided.error_default") || "Review the source and settings, then retry. Your completed work is safe.";
...
${contextLines.join("\n")}` : goal;
  ...
  const resultPreviewText = (() => {
    ...
    const candidates = [typeof item?.data === "string" ? item.data : null, data.simplifiedText, data.text, data.content, item?.text, item?.content, item?.summary, data.summary, data.description];
