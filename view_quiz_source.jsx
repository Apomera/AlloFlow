...
var _lazyIcon = function (name) {
  ...
  function LiveResultsDashboard(p) {
    ...
    if (variant === 'gradebook') {
      ...
      body = <div><div className="flex items-center justify-end mb-2"><button type="button" onClick={exportCsv} className="inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded border border-slate-300 text-slate-700 bg-white hover:bg-slate-100 transition-colors" aria-label={t("a11y.export_gradebook_csv")} data-help-key="quiz_csv_export_btn" title={t("tooltips.download_gradebook_csv")}><span aria-hidden="true">📥 </span>{t("ui_common.export_csv")}</button></div><div className="overflow-x-auto"><table className="w-full text-sm border-collapse"><thead><tr className="bg-slate-100"><th scope="col" className="w-7 px-1 py-1.5" aria-label={t("a11y.expand_row")} /><th scope="col" className="text-left px-2 py-1.5 font-bold text-slate-700">Student</th><th scope="col" className="text-center px-2 py-1.5 font-bold text-slate-700">Answered</th><th scope="col" className="text-center px-2 py-1.5 font-bold text-slate-700">Correct</th><th scope="col" className="text-center px-2 py-1.5 font-bold text-slate-700">IDK</th></tr></thead><tbody>{data.studentRows.map(function (row) {
                var pct = row.totalAnswered > 0 ? Math.round(row.totalCorrect / row.totalAnswered * 100) : 0;
      ...
      body = <div className="space-y-2">{data.conceptCards.map(function (card) {
          ...
          return <div key={card.questionIdx} className={'p-3 rounded-lg border bg-' + color + '-50 border-' + color + '-200'}><div className="flex items-start justify-between gap-3 mb-1"><span className={'text-xs font-bold uppercase tracking-wider text-' + color + '-800'}>{urgency}</span>{card.totalAnswered > 0 && <span className={'text-xs font-bold px-2 py-0.5 rounded bg-' + color + '-200 text-' + color + '-900'}>{card.percentCorrect + '% correct'}</span>}</div><p className="text-sm text-slate-800 mb-2">{card.conceptText}</p><div className={'flex items-center gap-3 text-xs text-' + color + '-900'}><span>{card.correctCount + ' ✓'}</span><span>{card.incorrectCount + ' ✗'}</span>{card.idkCount > 0 && <span className="text-sky-700">{card.idkCount + ' 🤔'}</span>}<span className="text-slate-600">{'· ' + card.totalAnswered + ' / ' + data.totalStudents + ' students'}</span>{showExplainBtn && <button type="button" onClick={function () {
              ...
              }} className="ml-auto inline-flex items-center gap-1 text-xs font-bold px-2 py-1 rounded bg-indigo-600 text-white hover:bg-indigo-700 transition-colors" aria-label={t("a11y.explain_to_class")} data-help-key="quiz_explain_to_class_btn" title={t("tooltips.generate_concept_explainer")}><span aria-hidden="true">🎓 </span>{t("ui_common.explain_to_class")}</button>}</div></div>;
        })}</div>;
    } else if (variant === 'retentionCurve') {
  ...
  function FreeformItemCard(p) {
    ...
    return <div className="bg-white p-5 rounded-xl border border-slate-300 shadow-sm"><div className="flex items-start gap-3 mb-3"><span className="flex-shrink-0 bg-slate-100 text-slate-600 w-6 h-6 rounded-full flex items-center justify-center text-xs mt-0.5">{p.itemNumber}</span><div className="flex-1 min-w-0"><span className="inline-block text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-slate-100 text-slate-700 mb-1">{typeLabel}</span><p className="text-sm text-slate-800 leading-relaxed">{q.question || ''}</p></div></div>{q.type === 'fill-blank' ? <input aria-label={t("a11y.fill_in_blank")} type="text" value={response} onChange={function (ev) {
      ...
      }} placeholder={t("placeholders.missing_word_or_phrase")} disabled={grade.loading || grade.status === 'correct' || grade.status === 'idk'} className="w-full px-3 py-2 rounded-lg border border-slate-300 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 disabled:bg-slate-50" /> : <textarea aria-label={typeLabel + " response"} value={response} onChange={function (ev) {
        setResponse(ev.target.value);
        ...
        }} className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold transition-colors">{t("ui_common.try_again")}</button>}</div>{grade.status && <div className={'mt-3 p-3 rounded-lg border bg-' + statusColor + '-50 border-' + statusColor + '-300'} role="status" aria-live="polite"><div className="flex items-center gap-2 mb-1 flex-wrap"><span className={'text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-' + statusColor + '-200 text-' + statusColor + '-900'}>{statusLabel}</span>{aiExplainerEnabled && grade.status !== 'correct' && grade.status !== 'idk' && !explainer.open && <button type="button" onClick={requestExplainer} className="ml-auto text-xs font-bold px-2 py-0.5 rounded bg-indigo-600 hover:bg-indigo-700 text-white transition-colors" title={t("tooltips.quick_ai_explanation")}>🤖 Explain this</button>}</div>{grade.feedback && <p className={'text-sm text-' + statusColor + '-900 mb-2'}>{grade.feedback}</p>}{explainer.open && <div className="mt-2 p-3 bg-white border border-indigo-200 rounded-lg"><div className="text-[10px] uppercase font-bold tracking-wider text-indigo-700 mb-1">🤖 Quick explanation</div>{explainer.loading && <p className="text-sm text-indigo-700 italic">Generating explanation…</p>}{explainer.text && <p className="text-sm text-slate-800 leading-relaxed">{explainer.text}</p>}{explainer.error && <p className="text-sm text-rose-700">{explainer.error}</p>}{explainer.text && typeof p.callTTS === 'function' && <button type="button" onClick={function () {
            if (window.AlloSpeechPlayer) window.AlloSpeechPlayer.speak(explainer.text);
            else p.callTTS(explainer.text);
  ...
  function QuizView(props) {
    ...
    var explainerPanel = _aiExplainerEnabled ? <div key="ai-explainer" className="rounded-xl border border-indigo-200 bg-indigo-50 p-4 mb-6" role="region" aria-label="AI concept explainer"><div className="flex items-center gap-2 mb-2"><span className="text-lg" aria-hidden="true">🤖</span><h4 className="font-bold text-sm text-indigo-900">Don't know a concept? Ask for a quick explainer.</h4></div><p className="text-xs text-indigo-800 mb-2">Type any concept from the quiz (or any prior knowledge you're unsure about). The AI will give you a 60-90 word explanation tuned to your grade level.</p><div className="flex items-stretch gap-2"><input aria-label="Concept to explain" type="text" value={explainerInput} onChange={function (ev) {
        ...
        }} placeholder={_quizMode === 'pre-check' ? 'e.g., "what plants need to grow"' : 'e.g., "photosynthesis"'} className="flex-1 min-w-0 px-3 py-2 rounded-lg border border-indigo-300 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" /><button type="button" onClick={function () {
          explainConcept(explainerInput);
