...
function _renderDiagramSvg(tool, state, titleText) {
  ...
  if (tool === 'fractions') {
    ...
    return '<svg viewBox="0 0 ' + fW + ' 78" role="img" aria-label="' + frTitle + ': ' + frDesc + '" width="100%" style="max-width:360px"><title>' + frTitle + '</title><desc>' + frDesc + '</desc>' + fCells + '<text x="' + fW / 2 + '" y="' + (fBarY + fBarH + 22) + '" font-size="14" font-weight="bold" fill="#4f46e5" text-anchor="middle">' + esc(fNum + '/' + fDen) + '</text>' + '</svg>';
  }
  if (tool === 'base10') {
    ...
    for (var bhi = 0; bhi < Math.min(bH, 9); bhi++) {
      ...
      for (var bk = 1; bk < 10; bk++) {
        bParts += '<line x1="' + (bx + bk * bu) + '" y1="' + by0 + '" x2="' + (bx + bk * bu) + '" y2="' + (by0 + bu * 10) + '" stroke="#4f46e5" stroke-width="0.4"/>' + '<line x1="' + bx + '" y1="' + (by0 + bk * bu) + '" x2="' + (bx + bu * 10) + '" y2="' + (by0 + bk * bu) + '" stroke="#4f46e5" stroke-width="0.4"/>';
      }
      bx += bu * 10 + 10;
    }
  ...
  if (tool === 'protractor') {
    ...
    var pRad = pAng * Math.PI / 180,
      ...
      pvy = 112,
      pLen = 84;
    var pex = (pvx + pLen * Math.cos(pRad)).toFixed(1),
      pey = (pvy - pLen * Math.sin(pRad)).toFixed(1);
    ...
    return '<svg viewBox="0 0 220 140" role="img" aria-label="' + prTitle + ': ' + prDesc + '" width="100%" style="max-width:240px"><title>' + prTitle + '</title><desc>' + prDesc + '</desc>' + '<line x1="' + pvx + '" y1="' + pvy + '" x2="' + (pvx + pLen) + '" y2="' + pvy + '" stroke="#475569" stroke-width="2"/>' + '<line x1="' + pvx + '" y1="' + pvy + '" x2="' + pex + '" y2="' + pey + '" stroke="#4f46e5" stroke-width="2"/>' + '<circle cx="' + pvx + '" cy="' + pvy + '" r="3" fill="#475569"/>' + '<text x="' + (pvx + 30) + '" y="' + (pvy - 14) + '" font-size="14" font-weight="bold" fill="#4f46e5">' + esc(pAng + '°') + '</text>' + '</svg>';
  }
  return null;
}
