...
(function () {
  ...
  function alloFocusToolCard(toolId) {
    ...
    var SEL_TOOL_GUIDANCE = {
      ...
      somaticReset: { mode: 'Body reset', note: 'Choose a body area, then try a short stillness, breathing, or gentle movement practice. A compact keyboard-accessible picker keeps every visual manageable on small screens. Visuals include a predictable direction-marked linear path, a two-part Breath Orbit with solid and dotted phase arcs that become bolder while active plus a phase-shaped clockwise marker, and screen-reader phase cues; students can sample one breath of motion before the timer starts, then enlarge, still, or turn off the guide. In Quiet View, the enlarged visual becomes a keyboard- and touch-operable start/pause control. The countdown can be hidden; guidance words can be full, phase-only, or hidden; and breath counts and number ratings are optional.', boundary: 'This is not treatment or diagnosis. Keep movement small and pain-free; stop for pain, dizziness, or numbness and tell a trusted adult or health professional.' },
    ...
    var SEL_TEACHER_TOOL_META = {
      ...
      somaticReset: { time: '3-8 min', format: 'Solo or whole class', cue: 'The compact visual picker keeps every guide available without a dense button grid. The Breath Orbit uses a solid inhale arc, dotted exhale arc, handoff marker, bolder active phase, and a traveling marker that is round on inhale, diamond-shaped on exhale, and carries a clockwise chevron while moving, so the cycle does not depend on color alone. Let students sample one breath of motion before the timer, or choose Still. Offer full, phase-only, or hidden guidance words. Quiet View makes the enlarged visual a direct start/pause control. Offer the direction-marked linear guide and screen-reader phase cues, plus hidden-countdown, hidden-guidance, still-motion, no-visual, natural-breathing, and no-number choices; never require ratings or explanations of body sensations.' },
