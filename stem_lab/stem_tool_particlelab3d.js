...
(function () {
  ...
  window.StemLab.registerTool('particleLab3d', {
    ...
    render: function (ctx) {
      ...
      var experimentStep = !prediction.trim() ? 0 : ((!running && !history.length && !observation.trim()) ? 1 : (!observation.trim() ? 1 : 2));
      ...
      var experimentStepHint = experimentStep === 0
        ...
        : (experimentStep === 1
          ? ((!running && !history.length) ? 'Press Run, then watch motion and live measurements.' : 'Watch motion and record one measurement that supports your observation.')
      ...
      var stageActivityLabel = running
        ...
        : (history.length ? 'Pause to inspect measurements' : 'Press Run or Space to begin');
          ...
          h('section', { id: 'particle-stage', ref: stageRef, onKeyDown: onStageKeyDown, className: 'overflow-hidden border border-cyan-500/30 bg-slate-950 shadow-2xl shadow-cyan-950/40 ring-1 ring-white/5 ' + (fsActive ? 'h-screen rounded-none flex flex-col' : 'rounded-[24px]'), style: cssFullscreen ? { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100vh', zIndex: 99990, borderRadius: 0 } : undefined, 'aria-label': 'Three-dimensional particle simulation' },
            ...
            h('div', { id: 'particle-experiment-runway', role: 'region', 'aria-label': 'Experiment loop', className: 'border-b border-slate-800 bg-slate-900/80 px-4 py-2.5' },
              h('div', { className: 'flex flex-wrap items-center justify-between gap-2' },
                h('div', { className: 'flex items-center gap-2' }, h('span', { className: 'text-[10px] font-black uppercase tracking-[0.18em] text-cyan-300' }, 'Experiment loop'), h('span', { className: 'text-[11px] font-bold text-slate-300' }, currentProtocol ? currentProtocol.title : 'Free laboratory')),
                h('div', { id: 'particle-stage-status', role: 'status', 'aria-live': 'polite', className: 'text-[10px] font-bold text-slate-300' }, 'Now: ', h('span', { className: 'text-cyan-200' }, experimentStepLabel), ' • ', experimentStepHint)
              ),
              ...
              h('div', { className: 'mt-2 flex flex-wrap items-baseline gap-x-2 gap-y-1 border-t border-white/10 pt-2 text-[10px]' }, h('span', { className: 'font-black uppercase tracking-[0.14em] text-amber-200' }, 'What to notice'), h('span', { className: 'text-slate-300' }, evidenceCue))
            ),
            h('div', { className: 'relative min-h-[380px] overflow-hidden bg-slate-950 ' + (fsActive ? 'flex-1' : 'h-[520px]') },
              ...
              h('div', { className: 'pointer-events-none absolute right-3 top-16 w-[min(11rem,calc(100%-1.5rem))] rounded-xl border border-cyan-300/20 bg-slate-950/65 p-3 text-right shadow-lg backdrop-blur-md sm:top-3 sm:w-auto sm:min-w-[150px]' },
              ...
              ),              h('div', { id: 'particle-stage-activity', className: 'pointer-events-none absolute ' + (systemProbe || trace || transportMode ? 'bottom-[6rem]' : 'bottom-3') + ' right-3 w-[min(11rem,calc(100%-1.5rem))] sm:w-auto sm:max-w-[min(20rem,calc(100%-1.5rem))] rounded-xl border ' + (running ? 'border-emerald-300/30 bg-emerald-950/75 text-emerald-100' : 'border-white/15 bg-slate-950/75 text-slate-200') + ' px-3 py-2 shadow-lg backdrop-blur', role: 'status', 'aria-live': 'polite', 'aria-label': (running ? 'Live simulation. ' : 'Simulation paused. ') + stageActivityLabel + '. ' + stageActivityDetail }, h('div', { className: 'flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.14em]' }, h('span', { className: 'h-2 w-2 rounded-full ' + (running ? 'animate-pulse bg-emerald-300 shadow-[0_0_10px_2px_rgba(110,231,183,0.7)]' : 'bg-slate-400') }), running ? 'Live simulation' : 'Simulation paused'), h('div', { className: 'mt-1 text-[10px] font-bold' }, stageActivityLabel), h('div', { className: 'mt-0.5 text-[10px] opacity-75' }, stageActivityDetail)),
