...
> **Superseded 2026-09-05.** `npm run verify:school-rewards -- --pool=forks` now completes in one clean invocation: **16 files, 510 tests, exit code 0, 80 seconds.** The worker-start failures were the threads pool on this host; the forks pool does not hit them. Note the script already sets `--testTimeout=60000`, so passing your own duplicates the flag and vitest refuses to start.
...
Browser check:
```powershell
node dev-tools/school_rewards_demo_walkthrough.cjs
```
...
- Existing-file Node `r+` writes followed by `fs.ftruncateSync` worked. A fallback preserving the Python generator is saved as `dev-tools/rebuild_school_rewards_practice.cjs`. It executes the builder through page generation, then writes both outputs through Node handles.
...
4. Optional polish after feedback:
   - ~~Practice controls and the new practice introduction/guide are still English.~~ Done 2026-09-05: the practice wrapper (bar, welcome, demo guide, customize panel labels/placeholders, scenario labels, built-in tour steps, tour box) now follows the portal's language menu through a 97-entry dictionary in `dev-tools/build_school_rewards_practice.py` (`WRAP_ES`, `srPracticeI18n`), driven by a MutationObserver on the wrapper roots, `document.body` (tour box), and the `<html lang>` attribute. School-typed customize content is not translated. Status messages from the customize panel remain English (presenter-only).
...
- Re-ran `node dev-tools/school_rewards_demo_walkthrough.cjs` against the rebuilt page: exit 0, balances 9 → 29 → 14, stock 20 → 19, 17 of 19 admin cards collapsed, checklist complete, 390px mobile with no overflow, no page errors.
...
- Practice builder: welcome paragraph mentions Help; the five-minute demo guide gained an Orient step. Both practice pages rebuilt via `dev-tools/rebuild_school_rewards_practice.cjs`.
- Manual + quick cards updated and mirrored (see readiness report for the list).
- 2026-09-05 (later): Print Lab default FLIPPED to opt-in with the owner's go-ahead (`=== 'true'`); repository test + pathways seed updated; copy updated in Portal/manual/demo guide; catalogue 924/924.
- 2026-09-06: `reduceMotion: _reduceMotion` added to the `_ctx` literal in stem_lab_module.js (both copies). It was already computed at L~3538 from `.reduce-motion` + the media query, just never exposed; Tree Lab was the one consumer and got undefined. Additive only. If you add a ctx field, remember the SEL hub builds its own ctx separately.
- 2026-09-06: check_stem_ctx.cjs is now guard-aware (guardedNamesIn + a `guarded` set threaded through walk; findings keep only unguarded refs). 17 findings -> 3. The SEL 'bug' I reported earlier was a FALSE POSITIVE: sel_tool_strengths' Back button falls back to ctx.setSelHubTool, which the hub provides. Guard shapes are deliberately DIRECT-only, so `ctx.a.b || c` still reports. Recalibrate with a throwaway sel_hub/sel_tool_zz*.js fixture if you touch this again.
- 2026-09-06: gates. `check_stem_ctx` / `check_free_vars` / `check_stem_layout_defects` all clean for printlab, artstudio, archstudio. Worth knowing: artstudio declares `var ctx = canvas.getContext('2d')` in two nested scopes, so a hand-off added in the wrong place would silently take the no-navigation branch; check_free_vars is what proves the render-scope `ctx` is the one in play. Pre-existing, NOT mine: check_stem_ctx flags sel_tool_strengths.js for ctx.setStemLabTool (SEL hub does not provide it).
- 2026-09-06: i18n registration. `node dev-tools/register_ui_strings.cjs dev-tools/harvest_input_archstudio_print_2026-09-06.json` added the six stem.archstudio print keys; artstudio's two were already there. Root `ui_strings.js` only: the desktop copy is produced by `node build.js` (see its modulesToCopy list) and will pick them up on the next build. Do NOT hand-edit `desktop/web-app/public/ui_strings.js`, and do not run insert_pack_keys into `lang/` right now: spanish_castilian / spanish_latin_america / esperanto have .bak.handfix files dated today from another session.
- 2026-09-06: artstudio sculpt row. Five `mini + " flex-1"` buttons plus the Load model label now use `style: { flex: "1 1 96px" }`; all six were in that one row. stem_tool_shot.cjs gained `--wait=<ms>`: with `--pre=@prim3d_module.js --wait=8000` the sculpt tab renders (three.js comes from the CDN, so this shot needs network).
- 2026-09-05: archstudio sidebar 'Print & export' section (after Mirror & Symmetry) sharing `sendToPrintLab`/`exportSTL`; new i18n keys stem.archstudio.print_export / export_stl / export_stl_aria / print_note with English fallbacks. archstudio_visual gate: numeric px font sizes >= 10.
- 2026-09-05: visual pass. stem_tool_shot.cjs `--pre=<js|@file>` added; `--state` is the tool's own slice; Art Studio needs `studioStarted:true` to leave its landing (hand-offs now set it) and window.THREE (CDN) to render the sculpt tab, so that toolbar is unphotographed. Arch Studio's export group is at x~2250 in a horizontally scrolling pill row (pre-existing).
- 2026-09-05: Print Lab round trips. `returnToArtStudio()` sets `window.__alloArtStudioPendingSculpt` ({schema:'alloflow-artstudio-sculpt/1', recipe}) + `ctx.updateMulti('artStudio',{tab:'sculpt3d'})`; Art Studio consumes it in a mount effect via `window.StemLab.artStudioPure.readPendingSculpt` (exported for tests). `startInTool()` buttons on the empty Design tab. Art Studio's aria-label i18n gate (tests/artstudio_aria_labels_i18n.test.js) rewrites literal aria-labels; add none there.
- 2026-09-05: Architecture Studio -> Print Lab bridge. `readPendingArchStudioHandoff` (STL, sourceTool 'archStudio', no sourceModel, summary.meshDimensions from `inspectGeometryWorldBinaryStl`); archstudio's `exportSTL` refactored around `buildArchStl()` shared with `sendToPrintLab`. Geometry test still pins the literal `pending.sourceTool !== 'geometryWorld'`, kept intact. i18n keys `stem.archstudio.print_lab` / `print_lab_aria` use English fallbacks.
- 2026-09-05: Art Studio -> Print Lab bridge. `readPendingRecipeHandoff` in stem_tool_printlab.js accepts `{schema:'alloflow-print-source/1', sourceTool:'artStudio', format:'RECIPE', recipe}`; Art Studio sculpt toolbar sets `window.__alloPrintLabPendingHandoff` and calls `ctx.setStemLabTool('printLab')`. Live copies: stem_lab/ and desktop/web-app/public/stem_lab/ (see memory: top-level stem_tool_*.js are dead). New i18n key `stem.artstudio.sculpt_print_lab` uses the English fallback; the app-wide pack pipeline will list it as untranslated.
- 2026-09-05: Print Lab default. `printLabEnabled_()` returns true unless config is explicitly 'false', and repository.test pins a fresh book to true, so DEPLOYMENTS START WITH THE TAB SHOWN while the practice ledger hides it. Copy in the portal setting, manual section 8, and the demo guide now states this; new gate in manual.test ties all three to the Code.gs line. The default itself was left alone: it is deliberate and tested, so flipping it is the owner's call. New catalogue string translated; packs 923/923.
- 2026-09-05: presenter guide (`docs/school_rewards_admin_demo.md`) realigned with the practice page (Help step folded into the opening minute, Español note, quick-card leave-behinds, tour focus button) and gated in `school_rewards_manual.test.js` against the in-page demo guide. It had no test at all before.
- 2026-09-05: phone sweep (`dev-tools/school_rewards_mobile_sweep.cjs`). Layout claims hold (390px, no overflow, `nav.tabs` fixed bottom, all roles). Two fixes: `.help-more a,.help-links a` now `inline-flex;min-height:44px` (were 17-21px tall), and `label:has(input[type="checkbox"]){min-height:24px}` for the ten checkbox rows (were 16px; the box keeps its size, only the tap area grows). `:has()` is fine for the Workspace browsers this ships to. Both pinned in portal_features.
- 2026-09-05: two integrity gates added. `school_rewards_manual.test.js` now proves every internal anchor, the four `school-rewards-manual#...` deep links in Portal.html, and the manual->quick-cards anchor all resolve, plus section count vs TOC vs the docmeta claim. `school_rewards_practice.test.js` proves the WRAP_ES dictionary covers every wrapper string (58 markup + attrs + 3 scenarios + 16 tour + 6 runtime labels); both use lower-bound counts so an extraction regex that stops matching fails instead of passing empty. Calibration (broken copies) confirmed it flags an edited label, a reworded tour title, and a new untranslated line. Manual reviewed date -> September 5.
- 2026-09-05: print pass. `dev-tools/school_rewards_print_check.cjs` (real `page.pdf()`, Letter/0.5in). Quick cards already correct (3 pages, `.card{break-inside:avoid}` + `page-break-after:always`). Manual print CSS gained `figure.diagram,.glance,.tablewrap{break-inside:avoid;page-break-inside:avoid}`; docmeta page claim corrected to the reproducible **29** pages. Watch out: the count is only stable once every lazy figure has decoded; runs before that fix read 18 and 19 for the same file. The harness now awaits image load + decode + document.fonts.ready and returned 29 three times running. NOTE: the script's `straddling` list measures the UNPAGINATED flow, so it over-reports for blocks that compute `avoid` (Chromium relocates those); trust its `breakInside` field and the pdf page count, not its page numbers.
- 2026-09-05: wrapper a11y probe (`dev-tools/school_rewards_wrapper_a11y.cjs`): 37/37 text elements pass in light/dark/forced; keyboard walk clean. Tour box gained `[data-tour="focus"]` ("Go to the highlighted control", Spanish "Ir al control resaltado") and `aria-labelledby="practice-tour-title"`; onboarding test added.
- 2026-09-05: dark-theme fix for `.help-more a,.help-links a` (was #4d3598 on #161e2e); added to both the dark and prefers-contrast blocks in Portal.html; `scratch/school-rewards-help-theme-check.cjs` probes computed colours and screenshots dark + forced-colors. portal_features 18/18, manual suite 6/6.
- 2026-09-05: teacher guide chapter 20 (`docs/teacher-guide/chapters/20-for-school-leaders.md`) gained a "Recognition and the school store" section with the manual and quick-card links plus a trimester line in "A year in the hub"; guide rebuilt with `node dev-tools/build_teacher_guide.cjs` (its `--check` showed only my chapter as stale, so the rebuild touched nothing from the other session's chapter 19 edit). `manuals.html` card now links the quick cards and practice page; `README.md` gained a "Manuals and training" section (mirrored).
...
- 2026-09-05 round: `.help-more` deep links inside each role block of `#help-panel` (4 strings, Spanish added, packs 922/922, practice rebuilt); manual Store-note table after the scheduled-window figure; `#card-student` added to `school-rewards-quick-cards.html`. Suites rerun one file at a time with forks: onboarding 10/10, portal_i18n 6/6, portal_packs 25/25, plain_language 7/7.
- Manual visuals pass (2026-09-04): inline SVG points-flow diagram + "Shopping day at a glance" box + four fresh figures (09 replaced, 10–12 new). Capture: `node dev-tools/school_rewards_manual_capture.cjs` (crops portal shots below the sticky practice bar; the scheduled-window shot mutates the practice repo then clicks Refresh live availability rather than reloading, because the practice ledger does not persist that mutation across a reload). Render check: `node scratch/school-rewards-manual-render-check.cjs` (light + dark). No Portal code changed in this pass; no tests needed rerunning.
