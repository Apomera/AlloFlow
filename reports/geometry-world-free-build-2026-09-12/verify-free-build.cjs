...
const run=async function(){
 ...
 let page;
 const persist=()=>fs.writeFileSync(path.join(out,'final-free-build-browser.json'),JSON.stringify(r,null,2));
 const snap=async name=>{await page.screenshot({path:path.join(out,'final-'+name+'.png'),timeout:120000});console.log('SCREENSHOT '+name);persist();};
 ...
 const enter=async options=>{
  ...
  r.checks.nativeArrival=await page.evaluate(()=>({camera:__geoWorldEngine.camera.position.toArray(),entryAnimation:!!__geoWorldEngine._entryAnim,spawn:__geoWorldEngine._currentLesson.spawnPoint}));
 };
 try {
  ...
  await page.evaluate(()=>{window.__aimAt(0,5,0);__geoWorldEngine.camera.updateMatrixWorld(true);});await page.getByRole('button',{name:'Select and measure aimed build',exact:true}).click();
  await page.waitForFunction(()=>!!__geoWorldEngine._builderSelection,{}, {timeout:15000});
