...
async function main() {
 ...
 const [run,setRun]=React.useState({});window.reviewSetRun=setRun;const [resource,setResource]=React.useState(()=>empty?{...fixture.plan,data:{...fixture.plan.data,teachingScripts:[]}}:fixture.plan);const [editing,setEditing]=React.useState(false);window.reviewSetResource=setResource;const [availableMaterials,setMaterials]=React.useState([{id:'fraction-cards',title:'Fraction models',type:'source',data:{text:'Use models of equal wholes to compare halves and fourths.'}}]);window.reviewSetMaterials=setMaterials;
 ...
 t:key=>fixture.labels[key]||'',history:[],teachingScriptMaterials:availableMaterials,scriptRun:run,
 ...
 onGenerateTeachingScript:async()=>{if(window.reviewHold){setRun({planId:'review-plan',busy:true,stage:'Preparing a teaching script'});return new Promise(resolve=>window.reviewResolve=resolve);}setResource(previous=>({...previous,data:{...previous.data,teachingScripts:(previous.data.teachingScripts||[]).concat({...fixture.savedScript,id:'generated-review',title:'A new teaching sequence'})}}));return{ok:true}},
 ...
 onCancelTeachingScript:()=>{setRun({});if(window.reviewResolve)window.reviewResolve({ok:false,error:'cancelled'});},onOpenTeachingMaterial:()=>{},handleToggleIsEditingLessonPlan:()=>setEditing(previous=>!previous),handleCopyToClipboard:()=>{},handleExportPDF:()=>{},handleExport:()=>{},
 ...
 try {
  for(const viewport of [{name:'desktop',width:1280,height:900},{name:'phone',width:375,height:812},{name:'small-phone',width:320,height:760}]){
   ...
   if(viewport.name==='desktop'){
    ...
    await page.getByLabel('Lesson topic',{exact:true}).fill('Custom comparison focus');
    await page.evaluate(()=>window.reviewSetResource(previous=>({...previous,data:{...previous.data,objectives:['Explain equivalent fractions using the number line.']}})));
    await page.getByText('The saved lesson changed. Unedited script settings were refreshed; your custom settings were kept.',{exact:true}).waitFor();
    if(await page.getByLabel('Learning goal',{exact:true}).inputValue()!=='Explain equivalent fractions using the number line.')throw new Error('Lesson defaults did not refresh');
    if(await page.getByLabel('Lesson topic',{exact:true}).inputValue()!=='Custom comparison focus')throw new Error('Teacher setting was overwritten');
    await page.getByRole('button',{name:'Use current lesson defaults',exact:true}).click();
    ...
    if(!await settings.evaluate(node=>document.activeElement===node))throw new Error('Focus lost after cancellation');
    await settings.click();
    await page.evaluate(()=>{window.reviewHold=false;window.reviewSetMaterials([]);});
    await page.getByText('A selected resource was removed, changed lessons, or no longer has readable content. Review the available resources before generating.',{exact:true}).waitFor();
    if(!await page.getByRole('button',{name:'Generate script',exact:true}).isDisabled())throw new Error('Missing selected resource was silently ignored');
    await page.getByRole('button',{name:'Remove unavailable selections',exact:true}).click();
    await page.evaluate(()=>window.reviewSetMaterials([{id:'fraction-cards',title:'Fraction models',type:'source',data:{text:'Use models to compare fractions.'}}]));
    await page.getByLabel(/^Fraction models/).check();
    await page.getByRole('button',{name:'Generate script',exact:true}).click();
    ...
    results.push({editSave:'passed',generateNewVersion:'passed',focusAfterGeneration:'passed',defaultsRefresh:'passed',customSettingsPreserved:'passed',cancelWhileSettingsCollapsed:'passed',missingSelectionRecovery:'passed'});
   }
   await page.close();
  }
