...
const shareSessionSurfacesSource = readFileSync(resolve(process.cwd(), 'view_share_session_surfaces_source.jsx'), 'utf8');
...
describe('Canvas-managed QR auth sequencing', () => {
  ...
  it('authenticates homework reads and never shares local-preview fallback codes', () => {
    ...
    expect(rootSource).toContain("const HomeworkQrDialogView = _alloCreateFirstWaveCdnView('HomeworkQrDialogView'");
    expect(shareSessionSurfacesSource).toContain('Take-home assignment');
    expect(shareSessionSurfacesSource).toContain("qrShareModal.aiPolicy === 'student-byok' ? (t('share_collect.personal_ai_optional') || 'Personal AI optional') : (t('share_collect.student_ai_off') || 'Student AI off')");
    expect(shareSessionSurfacesSource).toContain('This QR does not join your class, show a session code, or connect to live pacing.');
...
describe('homework QR hardening', () => {
  it('keeps homework payloads separate from live-session joins and exposes teacher verification controls', () => {
    ...
    expect(shareSessionSurfacesSource).toContain('Revoke homework link');
    ...
    expect(shareSessionSurfacesSource).toContain('ref={homeworkQrDialogRef} tabIndex={-1}');
    expect(rootSource).toContain('const printQrSheet = useCallback');
    expect(shareSessionSurfacesSource).toContain("printQrSheet(qrShareSvg, 'AlloFlow homework assignment'");
    expect(shareSessionSurfacesSource).toContain("openStudentQrPreview(mbLive.joinUrl, 'live-session link as a student')");
    expect(shareSessionSurfacesSource).toContain("mbLive.aiPolicy === 'student-byok' ? 'Personal AI optional' : 'AI tools off'");
    ...
    expect(shareSessionSurfacesSource).toContain('Recent homework links');
  ...
  it('keeps shared asynchronous activities opt-in, mailbox-hosted, and visually subordinate to homework sharing', () => {
    ...
    expect(shareSessionSurfacesSource).toContain("{t('share_collect.manage_shared') || 'Manage shared'}");
    expect(shareSessionSurfacesSource).toContain("qrShareModal.sharedActivity.type === 'rating' ? 'class rating' : qrShareModal.sharedActivity.type === 'survey' ? 'survey' : 'class Word Cloud'");
    expect(shareSessionSurfacesSource).toContain('Only the anonymous distribution appears after the participation threshold.');
    expect(rootSource).toContain('!isTeacherMode && sharedHostedActivity &&');
    expect(shareSessionSurfacesSource).toContain('Students contribute on their own time.');
    expect(headerPublicModule).toBe(headerModule);
  });
});
