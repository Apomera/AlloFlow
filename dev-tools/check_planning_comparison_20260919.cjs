...
const root=path.resolve(__dirname,'..'),req=createRequire(path.join(root,'desktop/web-app/package.json')),out=path.join(root,'reports/planning-input-comparison-2026-09-19');
(async()=>{
 ...
 try{
  ...
  for(const width of [1280,390,320]){
   ...
   for(const name of ['teacher','local','missing','legacy']){
